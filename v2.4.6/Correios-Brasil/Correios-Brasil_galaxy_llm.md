...
| **Timestamp** | `2026-08-03T19:04:53.620716+00:00` |
| **Scan Duration** | `0.13s` |
| **Git Branch** | `master` |
...
| Concurrency Exposure | 0.0 | 98.9 | 11.0 | 0.0 | 0.0 |
...
| Documentation Exposure | 4.0 | 63.3 | 31.5 | 26.7 | 11.9 |
| Algorithmic DoS Exposure | 0.0 | 5.9 | 1.1 | 0.0 | 0.0 |
...
- `request` (@ `lib/utils/request.ts`) -> Impact: **5.7** | LOC: 11
...
- `typescript` (@ `rollup.config.js`) -> Impact: **1.8** | LOC: 5

## 8.5 ALGORITHMIC & DATABASE BOTTLENECKS
> Highlights the most computationally expensive and database-heavy functions across the repository.

### Highest Data Gravity (Database Complexity)
- `request` (@ `lib/utils/request.ts`) -> DB Complexity: **3**
- `convertXMLStringToJson` (@ `lib/utils/parser.ts`) -> DB Complexity: **2**

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `__monolith__` | 5 | 43.36 | 4.74% | 0.0% |
| `tests/static` | 1 | 10.72 | 0.0% | 0.0% |
| `lib/utils` | 5 | 5.43 | 9.15% | 20.0% |
| `lib` | 1 | 1.31 | 5.0% | 0.0% |
...
- `lib/utils/request.ts` -> **5.8643%** Exposure
- `rollup.config.js` -> **3.9064%** Exposure
...
### 1. `lib/utils/request.ts` (TYPESCRIPT) -> Cumulative Risk: **361.35**
- **Archetype:** `file_cluster_13` (Distance: 9.206 IQR)
- **Magnitude:** 0.92 | **LOC:** 27 | **CtrlFlow:** 37.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (98.9341%), Safety Score (80.0%), Documentation (41.743%)
- **Heaviest Functions:** `request` (Impact: 5.7)

### 2. `lib/utils/formatter.ts` (TYPESCRIPT) -> Cumulative Risk: **236.73**
- **Archetype:** `file_cluster_8` (Distance: 8.357 IQR)
- **Magnitude:** 1.84 | **LOC:** 68 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9918%), Documentation (23.3174%), Api Exposure (8.4635%)
- **Heaviest Functions:** `formatDateTime` (Impact: 2.6), `formatStatus` (Impact: 2.5), `formatLocal` (Impact: 2.5)

### 3. `lib/utils/parser.ts` (TYPESCRIPT) -> Cumulative Risk: **226.88**
- **Archetype:** `file_cluster_8` (Distance: 7.727 IQR)
- **Magnitude:** 0.56 | **LOC:** 16 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (80.0%), Safety Score (71.6667%), Documentation (63.3113%), Cognitive Load (5.0%)
- **Heaviest Functions:** `convertArrayBufferToString` (Impact: 2.3), `convertXMLStringToJson` (Impact: 2.1)

### 4. `rollup.config.js` (JAVASCRIPT) -> Cumulative Risk: **152.29**
- **Archetype:** `file_cluster_13` (Distance: 10.925 IQR)
- **Magnitude:** 5.1 | **LOC:** 43 | **CtrlFlow:** 25.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Dead Code (23.1475%), Documentation (11.9203%), Cognitive Load (7.2908%)
- **Heaviest Functions:** `typescript` (Impact: 1.8), `commonjs` (Impact: 1.6)

### 5. `lib/utils/URL.ts` (TYPESCRIPT) -> Cumulative Risk: **127.17**
- **Archetype:** `file_cluster_8` (Distance: 5.865 IQR)
- **Magnitude:** 1.57 | **LOC:** 11 | **CtrlFlow:** 33.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (60.0%), Documentation (55.2093%), Api Exposure (5.5789%), Cognitive Load (5.0%)

### 6. `lib/utils/validation.ts` (TYPESCRIPT) -> Cumulative Risk: **104.25**
- **Archetype:** `file_cluster_8` (Distance: 7.446 IQR)
- **Magnitude:** 0.54 | **LOC:** 9 | **CtrlFlow:** 33.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (46.6667%), Documentation (45.5563%), Api Exposure (5.7813%), Cognitive Load (5.0%)
- **Heaviest Functions:** `sanitization` (Impact: 4.3)

### 7. `lib/index.ts` (TYPESCRIPT) -> Cumulative Risk: **64.72**
- **Archetype:** `file_cluster_13` (Distance: 6.854 IQR)
- **Magnitude:** 1.31 | **LOC:** 6 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (26.6667%), Documentation (26.6577%), Api Exposure (5.7813%), Cognitive Load (5.0%)
...
**Top Internal Functions/Classes:**
  * `typescript` (Impact: 1.8 | O(N^2))
  * `commonjs` (Impact: 1.6 | O(N^2))
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 77.821
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` plugin-commonjs, package.json, plugin-node-resolve, plugin-typescript, rollup-plugin-terser, rollup-plugin-dts
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
**Top Internal Functions/Classes:**
  * `formatDateTime` (Impact: 2.6 | O(N^1))
  * `formatStatus` (Impact: 2.5 | O(N^1))
  * `formatLocal` (Impact: 2.5 | O(N^1))
  * `formatOrigin` (Impact: 2.5 | O(N^1))
    * *Intent:* /** * @param {string} str * Função responsável formatar o Local de entrega de uma encomenda
  * `formatDestiny` (Impact: 2.5 | O(N^1))
**Structural Signatures (Net Mitigated Signals):**
...
- **Global Archetype:** `file_cluster_13` (Drift: 9.206 IQR)
- **Local Micro-Species:** `Cluster 2: Type Definitions & Bypasses` (Drift: 6.65 IQR)
- **Top Global Matches:** file_cluster_13: 9.206, file_cluster_8: 9.269, file_cluster_4: 9.532
- **Magnitude:** 0.92 | **LOC:** 27 | **CtrlFlow:** 37.5% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N^2) | **DB Complexity:** 3
- **Risk Profile:** Cognitive Load (28.2388%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `request` (Impact: 5.7 | O(N^2) | DB: 3)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 77.821
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` https, axios, http
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Risk Profile:** Cognitive Load (5.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `convertArrayBufferToString` (Impact: 2.3 | O(N^1))
  * `convertXMLStringToJson` (Impact: 2.1 | O(N^1) | DB: 2)
**Structural Signatures (Net Mitigated Signals):**
...
- `rollup.config.js` (JAVASCRIPT) | Magnitude: 5.1 | Delta: **0.027 IQR** | Secondary Pull: `file_cluster_0`
  * Top Architectural Signatures: indent_spaces: 28, structural_boundaries: 6, import: 5, func_start: 4
- `lib/utils/request.ts` (TYPESCRIPT) | Magnitude: 0.92 | Delta: **0.063 IQR** | Secondary Pull: `file_cluster_8`
...
- `lib/utils/parser.ts` -> **Severity: 4926.949** (Blast Radius: 77.821 * Doc Risk: 63.3113%)
- `lib/utils/URL.ts` -> **Severity: 4296.443** (Blast Radius: 77.821 * Doc Risk: 55.2093%)
- `lib/utils/validation.ts` -> **Severity: 3545.237** (Blast Radius: 77.821 * Doc Risk: 45.5563%)
- `lib/utils/request.ts` -> **Severity: 3248.482** (Blast Radius: 77.821 * Doc Risk: 41.743%)
- `lib/index.ts` -> **Severity: 2074.529** (Blast Radius: 77.821 * Doc Risk: 26.6577%)
