...
| **Timestamp** | `2026-08-03T20:56:08.184869+00:00` |
| **Scan Duration** | `26.26s` |
| **Git Branch** | `master` |
...
|---|---|---|
| file_cluster_8 | 4945 | 53.2% |
| file_cluster_13 | 1063 | 11.4% |
| file_cluster_4 | 442 | 4.8% |
| Unknown | 441 | 4.7% |
| file_cluster_16 | 166 | 1.8% |
| file_cluster_11 | 140 | 1.5% |
| file_cluster_15 | 118 | 1.3% |
| file_cluster_12 | 98 | 1.1% |
| file_cluster_7 | 95 | 1.0% |
| file_cluster_6 | 72 | 0.8% |
| file_cluster_9 | 66 | 0.7% |
| file_cluster_0 | 47 | 0.5% |
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 0.0 | 100.0 | 27.2 | 29.6 | 5.0 |
| Error & Exception Exposure | 0.0 | 100.0 | 65.7 | 80.0 | 0.0 |
| Tech Debt Exposure | 0.0 | 100.0 | 41.6 | 23.7 | 0.0 |
| Testing Exposure | 0.0 | 80.0 | 16.6 | 2.3 | 0.0 |
...
| Documentation Exposure | 0.0 | 100.0 | 27.9 | 14.9 | 11.9 |
| Algorithmic DoS Exposure | 0.0 | 100.0 | 1.1 | 0.0 | 0.0 |
| Obfuscation & Evasion Surface | 0.0 | 100.0 | 0.1 | 0.0 | 0.0 |
| Exploit Generation Surface | 0.0 | 100.0 | 2.3 | 0.0 | 0.0 |
...
- `init` (@ `src/cmd/compile/internal/syntax/parser.go`) -> Impact: **1452.0** | LOC: 1410
- `packEfaceData` (@ `src/reflect/value.go`) -> Impact: **1420.8** | LOC: 1776
- `close` (@ `src/net/http/server.go`) -> Impact: **1353.0** | LOC: 1927
...
- `main` (@ `src/regexp/testdata/testregex.c`) -> Impact: **1337.0** | LOC: 1071
- `linkname` (@ `src/cmd/compile/internal/noder/reader.go`) -> Impact: **1256.4** | LOC: 1416
- `allocValToReg` (@ `src/cmd/compile/internal/ssa/regalloc.go`) -> Impact: **1132.2** | LOC: 507
- `loadByRegWidth` (@ `src/cmd/compile/internal/amd64/ssa.go`) -> Impact: **1044.9** | LOC: 1915
  * *Intent:* // loadByRegWidth returns the load instruction of the given register of a given width.
- `tracePrint` (@ `src/cmd/compile/internal/typecheck/typecheck.go`) -> Impact: **1017.6** | LOC: 1023
- `newdie` (@ `src/cmd/link/internal/ld/dwarf.go`) -> Impact: **935.2** | LOC: 1265
  * *Intent:* // Every DIE manufactured by the linker has at least an AT_name // attribute (but it will only be written out if it is listed in the abbrev). // The c...
- `initAt` (@ `src/runtime/traceback.go`) -> Impact: **919.6** | LOC: 951
  * *Intent:* // unwindTrap indicates that the initial PC and SP are from a trap, not a // return PC from a call. //

## 8.5 ALGORITHMIC & DATABASE BOTTLENECKS
> Highlights the most computationally expensive and database-heavy functions across the repository.

### Highest Time Complexity (Big-O)
- `TestHexdumpWords` (@ `src/runtime/hexdump_test.go`) -> **O(2^N) [Recursive]**
- `loop_select` (@ `src/crypto/internal/fips140/nistec/p256_asm_ppc64le.s`) -> **O(2^N) [Recursive]**
- `loop_select` (@ `src/crypto/internal/fips140/nistec/p256_asm_s390x.s`) -> **O(2^N) [Recursive]**
- `restore` (@ `src/runtime/asm_amd64.s`) -> **O(2^N) [Recursive]**
- `refreshChart` (@ `src/internal/trace/traceviewer/mmu.go`) -> **O(2^N) [Recursive]**
- `main` (@ `src/cmd/cgo/internal/testcshared/testdata/main3.c`) -> **O(2^N) [Recursive]**
  * *Intent:* // Copyright 2015 The Go Authors. All rights reserved. // Use of this source code is governed by a BSD-style // license that can be found in the LICEN...
- `HelloWorldF90` (@ `src/cmd/cgo/internal/testfortran/testdata/helloworld/helloworld.f90`) -> **O(2^N) [Recursive]**
- `allocValToReg` (@ `src/cmd/compile/internal/ssa/regalloc.go`) -> **O(2^N) [Recursive]**
- `mightContainHeapPointer` (@ `src/cmd/compile/internal/ssa/writebarrier.go`) -> **O(2^N) [Recursive]**
  * *Intent:* // mightContainHeapPointer reports whether the data currently at addresses // [ptr,ptr+size) might contain heap pointers. "currently" means at memory ...
- `main` (@ `src/cmd/compile/internal/test/testdata/gen/arithConstGen.go`) -> **O(2^N) [Recursive]**

### Highest Data Gravity (Database Complexity)
- `loadByRegWidth` (@ `src/cmd/compile/internal/amd64/ssa.go`) -> DB Complexity: **784**
  * *Intent:* // loadByRegWidth returns the load instruction of the given register of a given width.
- `ssaGenValue` (@ `src/cmd/compile/internal/arm64/ssa.go`) -> DB Complexity: **508**
- `testTransportRemovesDeadIdleConnections` (@ `src/net/http/transport_test.go`) -> DB Complexity: **412**
  * *Intent:* // This request is canceled when dial is queued, which preempts dialing.
- `Anonymous_Block_[Truncated]` (@ `src/syscall/mkall.sh`) -> DB Complexity: **367**
- `TestIdleConnTimeout` (@ `src/net/http/internal/http2/transport_test.go`) -> DB Complexity: **365**
- `linkname` (@ `src/cmd/compile/internal/noder/reader.go`) -> DB Complexity: **350**
- `keccakF1600Generic` (@ `src/crypto/internal/fips140/sha3/keccakf.go`) -> DB Complexity: **343**
  * *Intent:* // keccakF1600Generic applies the Keccak permutation.
- `lshNotNop` (@ `src/cmd/compile/internal/test/testdata/arith_test.go`) -> DB Complexity: **341**
  * *Intent:* //go:noinline
- `ssaGenValue` (@ `src/cmd/compile/internal/ppc64/ssa.go`) -> DB Complexity: **340**
- `ssaGenValue` (@ `src/cmd/compile/internal/loong64/ssa.go`) -> DB Complexity: **319**
...
| `src/runtime` | 770 | 93342.96 | 21.77% | 49.18% |
| `src/net` | 229 | 36470.56 | 31.91% | 66.3% |
| `src/cmd/compile/internal/ssa` | 116 | 30927.16 | 43.6% | 49.38% |
| `src/net/http` | 70 | 26675.98 | 37.73% | 52.48% |
| `src/syscall` | 274 | 22368.85 | 22.87% | 41.07% |
| `src/cmd/compile/internal/types2` | 97 | 21641.4 | 38.62% | 26.9% |
| `src/go/types` | 109 | 20551.28 | 35.36% | 27.88% |
| `src/os` | 163 | 19737.63 | 28.74% | 63.46% |
...
- `src/runtime/memmove_test.go` -> **97** Orphaned Functions | **0** Duplicates
...
- `src/hash/adler32/adler32_test.go` -> **100.0%** Exposure
- `src/hash/crc32/crc32_test.go` -> **100.0%** Exposure
- `src/hash/crc64/crc64_test.go` -> **100.0%** Exposure
- `src/math/rand/v2/chacha8_test.go` -> **100.0%** Exposure
- `src/math/bits/bits_tables.go` -> **98.3879%** Exposure
### Exploit Generation Surface
- `src/archive/tar/example_test.go` -> **100.0%** Exposure
- `src/archive/zip/reader.go` -> **100.0%** Exposure
- `src/archive/zip/writer_test.go` -> **100.0%** Exposure
- `src/cmd/addr2line/main.go` -> **100.0%** Exposure
- `src/cmd/api/main_test.go` -> **100.0%** Exposure
### Weaponizable Injection Vectors
- `src/archive/tar/example_test.go` -> **100.0%** Exposure
- `src/cmd/addr2line/main.go` -> **100.0%** Exposure
- `src/cmd/asm/internal/lex/lex.go` -> **100.0%** Exposure
- `src/cmd/asm/main.go` -> **100.0%** Exposure
- `src/cmd/buildid/buildid.go` -> **100.0%** Exposure
### Raw Memory Manipulation
- `src/runtime/cgo/libcgo.h` -> **0.4441%** Exposure
- `src/runtime/cgo/linux_syscall.c` -> **0.1544%** Exposure
- `src/runtime/testdata/testprogcgo/stackswitch.c` -> **0.0097%** Exposure
- `src/cmd/asm/internal/asm/testdata/arm64error.s` -> **0.0024%** Exposure
- `src/syscall/asm_plan9_arm.s` -> **0.0019%** Exposure
...
- `src/cmd/api/main_test.go` -> **100.0%** Exposure
- `src/cmd/cgo/gcc.go` -> **100.0%** Exposure
- `src/cmd/compile/internal/dwarfgen/linenum_test.go` -> **100.0%** Exposure
- `src/cmd/compile/internal/escape/alias.go` -> **100.0%** Exposure
- `src/cmd/compile/internal/inline/interleaved/interleaved.go` -> **100.0%** Exposure
...
### 1. `src/internal/types/testdata/check/typeparams.go` (GO) -> Cumulative Risk: **946.69**
- **Archetype:** `file_cluster_11` (Distance: 23.469 IQR)
- **Magnitude:** 0.14 | **LOC:** 511 | **CtrlFlow:** 13.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `reverse` (Impact: 17.2), `Unknown_Block` (Impact: 7.3), `Unknown_Block` (Impact: 3.9)

### 2. `src/internal/types/testdata/check/map1.go` (GO) -> Cumulative Risk: **884.84**
- **Archetype:** `file_cluster_4` (Distance: 12.984 IQR)
- **Magnitude:** 0.16 | **LOC:** 147 | **CtrlFlow:** 29.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Logic Bomb (100.0%)
- **Heaviest Functions:** `InOrder` (Impact: 25.2), `find` (Impact: 11.1), `Send` (Impact: 10.2)

### 3. `src/simd/archsimd/_gen/tmplgen/main.go` (GO) -> Cumulative Risk: **850.19**
- **Archetype:** `file_cluster_8` (Distance: 13.235 IQR)
- **Magnitude:** 927.5 | **LOC:** 1173 | **CtrlFlow:** 54.5% | **Authorship Centralization:** 83.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Logic Bomb (100.0%), Injection Surface (100.0%)
- **Heaviest Functions:** `classifyBooleanSIMD` (Impact: 100.1), `main` (Impact: 40.1), `oneTemplate` (Impact: 31.6)

### 4. `src/cmd/go/internal/work/action.go` (GO) -> Cumulative Risk: **811.24**
- **Archetype:** `file_cluster_0` (Distance: 14.328 IQR)
- **Magnitude:** 1208.76 | **LOC:** 1268 | **CtrlFlow:** 57.6% | **Authorship Centralization:** 54.5%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (98.7666%)
- **Heaviest Functions:** `vetAction` (Impact: 235.2), `CompileAction` (Impact: 110.4), `NewBuilder` (Impact: 54.8)

### 5. `src/cmd/link/internal/ld/main.go` (GO) -> Cumulative Risk: **800.79**
- **Archetype:** `file_cluster_8` (Distance: 13.301 IQR)
- **Magnitude:** 483.38 | **LOC:** 552 | **CtrlFlow:** 79.3% | **Authorship Centralization:** 20.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Logic Bomb (100.0%), Injection Surface (100.0%)
- **Heaviest Functions:** `Main` (Impact: 141.2), `startProfile` (Impact: 19.4), `Set` (Impact: 10.4)

### 6. `src/internal/types/testdata/check/map0.go` (GO) -> Cumulative Risk: **798.78**
- **Archetype:** `file_cluster_16` (Distance: 12.539 IQR)
- **Magnitude:** 0.1 | **LOC:** 114 | **CtrlFlow:** 32.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%)
- **Heaviest Functions:** `InOrder` (Impact: 25.2), `find` (Impact: 11.1), `Next` (Impact: 7.4)

### 7. `src/io/example_test.go` (GO) -> Cumulative Risk: **794.21**
- **Archetype:** `file_cluster_8` (Distance: 15.215 IQR)
- **Magnitude:** 284.82 | **LOC:** 285 | **CtrlFlow:** 88.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Logic Bomb (100.0%), Injection Surface (100.0%)
- **Heaviest Functions:** `ExampleReadAtLeast` (Impact: 8.2), `ExampleCopyBuffer` (Impact: 6.1), `ExampleReadFull` (Impact: 6.1)

### 8. `src/cmd/go/internal/work/build.go` (GO) -> Cumulative Risk: **790.35**
- **Archetype:** `file_cluster_4` (Distance: 13.177 IQR)
- **Magnitude:** 1034.52 | **LOC:** 965 | **CtrlFlow:** 52.1% | **Authorship Centralization:** 58.8%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `InstallPackages` (Impact: 131.1), `runBuild` (Impact: 65.0), `libname` (Impact: 64.4)

### 9. `src/net/example_test.go` (GO) -> Cumulative Risk: **790.21**
- **Archetype:** `file_cluster_8` (Distance: 13.552 IQR)
- **Magnitude:** 215.54 | **LOC:** 388 | **CtrlFlow:** 57.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Logic Bomb (100.0%), Injection Surface (100.0%), State Flux (99.9999%)
- **Heaviest Functions:** `ExampleListener` (Impact: 8.2), `ExampleUDPConn_WriteTo` (Impact: 7.9), `ExampleDialer_unix` (Impact: 6.2)

### 10. `src/cmd/link/internal/ld/lib.go` (GO) -> Cumulative Risk: **774.86**
- **Archetype:** `file_cluster_8` (Distance: 15.667 IQR)
- **Magnitude:** 1208.56 | **LOC:** 3136 | **CtrlFlow:** 84.6% | **Authorship Centralization:** 47.1%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Logic Bomb (100.0%), Injection Surface (100.0%)
- **Heaviest Functions:** `mangleTypeSym` (Impact: 362.7), `loadcgodirectives` (Impact: 41.0), `setArchSyms` (Impact: 19.4)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 3):` slices, testing, utf8, bytes, syntax, a, , reflect...
...
### `src/crypto/x509/testdata/nist-pkits/certs/DSAParametersInheritedCACert.crt` (PLAINTEXT | Tier 0 | AI Safe: 0.0%)
- **Global Archetype:** `Unknown Archetype` (Drift: N/A IQR)
- **Magnitude:** 5000.0 | **LOC:** 1 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
...
- `src/net/http/internal/chunked.go` (GO) | Magnitude: 254.86 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_tabs: 96, state_mutation: 82, branch: 41, encapsulation: 17
- `src/cmd/internal/cov/readcovdata.go` (GO) | Magnitude: 124.58 | Delta: **0.004 IQR** | Secondary Pull: `file_cluster_17`
...
- `src/crypto/x509/parser.go` (GO) | Magnitude: 5.87 | Delta: **0.007 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 552, state_mutation: 303, branch: 173, structural_boundaries: 106
- `src/cmd/go/internal/work/action.go` (GO) | Magnitude: 1208.76 | Delta: **0.014 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_tabs: 568, state_mutation: 495, api: 134, branch: 133
...
- `src/os/file_unix.go` (GO) | Magnitude: 287.08 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 178, state_mutation: 123, branch: 53, encapsulation: 47
- `src/cmd/compile/internal/ssa/sparsemap.go` (GO) | Magnitude: 72.18 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_15`
...
- `src/cmd/compile/internal/ssa/loopreschedchecks.go` (GO) | Magnitude: 579.04 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_6`
  * Top Architectural Signatures: state_mutation: 311, indent_tabs: 218, encapsulation: 85, branch: 78
...
- `src/encoding/binary/binary.go` (GO) | Magnitude: 1401.18 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_8`
...
- `src/runtime/tracemap.go` (GO) | Magnitude: 50.32 | Delta: **0.009 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 40, state_mutation: 26, encapsulation: 17, doc: 14

### Mixed-Responsibility Refactoring Targets for: file_cluster_13
- `src/index/suffixarray/suffixarray.go` (GO) | Magnitude: 364.36 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_15`
  * Top Architectural Signatures: state_mutation: 189, indent_tabs: 185, branch: 59, encapsulation: 49
- `src/cmd/compile/internal/syntax/error_test.go` (GO) | Magnitude: 75.36 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 48, state_mutation: 35, branch: 14, encapsulation: 13
- `src/cmd/link/internal/loader/symbolbuilder.go` (GO) | Magnitude: 75.12 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_15`
...
- `src/net/ip.go` (GO) | Magnitude: 684.7 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 306, state_mutation: 216, branch: 124, structural_boundaries: 88
- `src/go/types/alias.go` (GO) | Magnitude: 78.66 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_13`
...
- `src/cmd/compile/internal/types2/alias.go` (GO) | Magnitude: 78.66 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 34, pointers: 31, doc: 27, state_mutation: 24
- `src/sync/waitgroup.go` (GO) | Magnitude: 223.94 | Delta: **0.003 IQR** | Secondary Pull: `file_cluster_4`
  * Top Architectural Signatures: indent_tabs: 98, doc: 43, state_mutation: 36, branch: 33

### Mixed-Responsibility Refactoring Targets for: file_cluster_16
- `src/internal/types/testdata/check/map0.go` (GO) | Magnitude: 0.1 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_15`
  * Top Architectural Signatures: indent_tabs: 53, state_mutation: 33, structural_boundaries: 23, encapsulation: 19
- `src/internal/types/testdata/fixedbugs/issue51229.go` (GO) | Magnitude: 0.05 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 53, encapsulation: 33, state_mutation: 20, structural_boundaries: 18
- `src/internal/types/testdata/fixedbugs/issue51386.go` (GO) | Magnitude: 0.01 | Delta: **0.006 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 4, indent_tabs: 4, safety_bypasses: 3, structural_boundaries: 2
- `src/time/sleep.go` (GO) | Magnitude: 9.58 | Delta: **0.006 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: doc: 9, structural_boundaries: 5, args: 4, func_start: 4
- `src/sync/poolqueue.go` (GO) | Magnitude: 153.64 | Delta: **0.007 IQR** | Secondary Pull: `file_cluster_8`
...
- `src/cmd/compile/internal/logopt/logopt_test.go` (GO) | Magnitude: 156.28 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_8`
...
- `src/os/exec/example_test.go` (GO) | Magnitude: 209.48 | Delta: **0.007 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 120, indent_tabs: 112, encapsulation: 57, branch: 19

### Mixed-Responsibility Refactoring Targets for: file_cluster_6
- `src/encoding/json/v2_options.go` (GO) | Magnitude: 80.6 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
...
- `src/crypto/rsa/rsa.go` (GO) | Magnitude: 2.8 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_tabs: 143, state_mutation: 139, encapsulation: 39, branch: 37
- `src/runtime/rand.go` (GO) | Magnitude: 52.14 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_13`
...
- `src/internal/runtime/gc/scan/scan_test.go` (GO) | Magnitude: 381.7 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_15`
...
- `src/cmd/compile/internal/types2/compilersupport.go` (GO) | Magnitude: 12.88 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_0`
...
- `src/cmd/compile/internal/ssa/prove.go` -> Churn: **100.0%** | Cog Load: 52.9301% | Debt: 23.3267%
- `src/cmd/go/internal/modload/init.go` -> Churn: **85.87%** | Cog Load: 53.2687% | Debt: 11.6987%
- `src/net/url/url.go` -> Churn: **78.53%** | Cog Load: 45.9887% | Debt: 97.7666%
- `src/cmd/link/link_test.go` -> Churn: **77.19%** | Cog Load: 34.4987% | Debt: 53.6699%
- `src/cmd/compile/internal/amd64/ssa.go` -> Churn: **73.83%** | Cog Load: 50.2826% | Debt: 23.7324%
...
- `src/encoding/json/v2/arshal_default.go` -> **Joe Tsai** (100.0% isolated ownership) | Magnitude: 2696.26
- `src/runtime/chan_test.go` -> **Tobias Klauser** (100.0% isolated ownership) | Magnitude: 2513.32
- `src/cmd/compile/internal/syntax/parser.go` -> **Robert Griesemer** (100.0% isolated ownership) | Magnitude: 2402.18
...
- `src/math/bits.go` -> **Severity: 2202.472** (Blast Radius: 27.969 * Doc Risk: 78.7469%)
- `src/internal/goarch/goarch.go` -> **Severity: 868.6** (Blast Radius: 8.686 * Doc Risk: 100.0%)
- `src/cmd/vet/testdata/atomic/atomic.go` -> **Severity: 723.665** (Blast Radius: 19.627 * Doc Risk: 36.8709%)
- `src/bytes/bytes.go` -> **Severity: 723.269** (Blast Radius: 52.015 * Doc Risk: 13.905%)
- `src/cmd/compile/internal/objw/objw.go` -> **Severity: 470.329** (Blast Radius: 4.704 * Doc Risk: 99.985%)
