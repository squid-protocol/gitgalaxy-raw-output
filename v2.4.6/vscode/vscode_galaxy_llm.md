...
| **Timestamp** | `2026-08-03T20:08:08.422363+00:00` |
| **Scan Duration** | `37.98s` |
| **Git Branch** | `main` |
...
> **Architectural Drift Z-Score:** `4.561`
...
|---|---|---|
| file_cluster_8 | 3113 | 38.1% |
| file_cluster_13 | 2921 | 35.7% |
| file_cluster_4 | 1475 | 18.0% |
| file_cluster_16 | 101 | 1.2% |
| file_cluster_17 | 91 | 1.1% |
| file_cluster_0 | 87 | 1.1% |
| Unknown | 43 | 0.5% |
| file_cluster_11 | 31 | 0.4% |
| file_cluster_2 | 24 | 0.3% |
| file_cluster_9 | 10 | 0.1% |
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 0.0 | 100.0 | 43.2 | 30.7 | 100.0 |
| Error & Exception Exposure | 0.0 | 100.0 | 47.2 | 52.7 | 0.0 |
| Tech Debt Exposure | 0.0 | 100.0 | 24.3 | 0.0 | 0.0 |
| Testing Exposure | 0.0 | 80.0 | 16.1 | 2.3 | 0.0 |
| API Exposure | 0.0 | 19.8 | 3.3 | 2.1 | 0.0 |
...
| Documentation Exposure | 0.0 | 100.0 | 34.4 | 20.0 | 0.0 |
...
- `resolve` (@ `src/vs/base/common/path.ts`) -> Impact: **1877.5** | LOC: 1073
  * *Intent:* /*--------------------------------------------------------------------------------------------- * Copyright (c) Microsoft Corporation. All rights rese...
- `constructor` (@ `src/vs/base/browser/ui/tree/abstractTree.ts`) -> Impact: **1767.5** | LOC: 1390
- `_showOutput` (@ `src/vs/workbench/contrib/tasks/browser/abstractTaskService.ts`) -> Impact: **1418.0** | LOC: 1361
- `webviewPreloads` (@ `src/vs/workbench/contrib/notebook/browser/view/renderers/webviewPreloads.ts`) -> Impact: **1167.2** | LOC: 1564
- `_open` (@ `src/vs/workbench/contrib/terminal/browser/terminalInstance.ts`) -> Impact: **1152.2** | LOC: 1082
- `apiObject` (@ `src/vs/workbench/api/common/extHostChatAgents2.ts`) -> Impact: **1136.0** | LOC: 1001
- `suite` (@ `src/vs/workbench/contrib/terminalContrib/chatAgentTools/test/electron-browser/treeSitterCommandParser.test.ts`) -> Impact: **873.6** | LOC: 291
- `suite` (@ `src/vs/workbench/contrib/terminalContrib/chatAgentTools/test/electron-browser/runInTerminalTool.test.ts`) -> Impact: **799.6** | LOC: 1443
- `resolveCompletionItem` (@ `extensions/typescript-language-features/src/languageFeatures/completions.ts`) -> Impact: **780.2** | LOC: 724
- `intersectWithViewport` (@ `src/vs/editor/browser/viewParts/minimap/minimap.ts`) -> Impact: **780.2** | LOC: 1205
  * *Intent:* /**
...
- `fmt` (@ `cli/src/util/errors.rs`) -> **O(2^N) [Recursive]**
- `circleOfLife` (@ `src/vs/platform/files/test/node/fixtures/resolver/examples/conway.js`) -> **O(2^N) [Recursive]**
- `circleOfLife` (@ `src/vs/platform/files/test/node/fixtures/resolver/other/deep/conway.js`) -> **O(2^N) [Recursive]**
- `circleOfLife` (@ `src/vs/platform/files/test/node/fixtures/service/deep/conway.js`) -> **O(2^N) [Recursive]**
- `circleOfLife` (@ `src/vs/workbench/services/search/test/node/fixtures/examples/NullPoinderException.js`) -> **O(2^N) [Recursive]**
- `fmt` (@ `cli/src/util/errors.rs`) -> **O(2^N) [Recursive]**
  * *Intent:* #[derive(Debug)]
- `fmt` (@ `cli/src/util/errors.rs`) -> **O(2^N) [Recursive]**
  * *Intent:* #[derive(Debug)]
- `fmt` (@ `cli/src/util/errors.rs`) -> **O(2^N) [Recursive]**
  * *Intent:* #[derive(Debug)]
- `suite` (@ `src/vs/workbench/contrib/chat/test/common/promptSyntax/promptFileReference.test.ts`) -> **O(2^N) [Recursive]**
- `startTunnel` (@ `src/vs/workbench/contrib/remoteTunnel/electron-browser/remoteTunnel.contribution.ts`) -> **O(2^N) [Recursive]**

### Highest Data Gravity (Database Complexity)
- `suite` (@ `src/vs/base/test/common/uri.test.ts`) -> DB Complexity: **708**
- `suite` (@ `src/vs/base/test/common/oauth.test.ts`) -> DB Complexity: **663**
- `resolve` (@ `src/vs/base/common/path.ts`) -> DB Complexity: **511**
  * *Intent:* /*--------------------------------------------------------------------------------------------- * Copyright (c) Microsoft Corporation. All rights rese...
- `constructor` (@ `src/vs/base/browser/ui/tree/abstractTree.ts`) -> DB Complexity: **490**
- `layout` (@ `src/vs/base/browser/ui/list/listView.ts`) -> DB Complexity: **484**
- `_open` (@ `src/vs/workbench/contrib/terminal/browser/terminalInstance.ts`) -> DB Complexity: **444**
- `intersectWithViewport` (@ `src/vs/editor/browser/viewParts/minimap/minimap.ts`) -> DB Complexity: **417**
  * *Intent:* /**
- `suite` (@ `src/vs/workbench/contrib/chat/test/common/promptSyntax/utils/promptFilesLocator.test.ts`) -> DB Complexity: **401**
- `suite` (@ `src/vs/base/test/common/oauth.test.ts`) -> DB Complexity: **372**
- `suite` (@ `src/vs/platform/url/test/common/urlGlob.test.ts`) -> DB Complexity: **369**

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `__monolith__` | 12 | 5162.08 | 1.88% | 0.71% |
...
- `extensions/microsoft-authentication/packageMocks/dpapi/dpapi.js` -> **100.0%** Exposure
...
- `src/vs/workbench/api/common/extHostTypes.ts` -> **0** Orphaned Functions | **186** Duplicates
- `src/vs/workbench/contrib/extensions/browser/extensionsActions.ts` -> **0** Orphaned Functions | **134** Duplicates
- `src/vs/editor/common/config/editorOptions.ts` -> **0** Orphaned Functions | **119** Duplicates
- `src/vs/platform/quickinput/browser/quickInput.ts` -> **0** Orphaned Functions | **106** Duplicates
- `src/vs/base/common/async.ts` -> **0** Orphaned Functions | **97** Duplicates
...
22. **`extensions/terminal-suggest/scripts/pullFishBuiltins.ts`** -> AI Confidence: **99.32%**
23. **`extensions/typescript-language-features/src/languageFeatures/util/snippetForFunctionCall.ts`** -> AI Confidence: **99.32%**
24. **`src/vs/base/test/browser/comparers.test.ts`** -> AI Confidence: **99.32%**
25. **`src/vs/editor/common/model/intervalTree.ts`** -> AI Confidence: **99.32%**
26. **`src/vs/platform/product/common/product.ts`** -> AI Confidence: **99.32%**
27. **`cli/src/bin/code/legacy_args.rs`** -> AI Confidence: **99.31%**
...
- `src/vs/base/test/common/extpath.test.ts` -> **100.0%** Exposure
- `src/vs/base/test/common/labels.test.ts` -> **100.0%** Exposure
- `src/vs/base/test/common/normalization.test.ts` -> **100.0%** Exposure
- `src/vs/base/test/common/path.test.ts` -> **100.0%** Exposure
- `src/vs/base/test/common/resources.test.ts` -> **100.0%** Exposure
### Exploit Generation Surface
- `scripts/code-perf.js` -> **100.0%** Exposure
- `scripts/code-sessions-web.js` -> **100.0%** Exposure
- `src/vs/platform/files/test/node/fixtures/resolver/examples/conway.js` -> **100.0%** Exposure
- `src/vs/platform/files/test/node/fixtures/resolver/other/deep/conway.js` -> **100.0%** Exposure
- `src/vs/platform/files/test/node/fixtures/service/deep/conway.js` -> **100.0%** Exposure
### Weaponizable Injection Vectors
- `scripts/code-perf.js` -> **100.0%** Exposure
- `scripts/code-sessions-web.js` -> **100.0%** Exposure
- `src/vs/sessions/test/e2e/generate.cjs` -> **100.0%** Exposure
- `src/vs/sessions/test/e2e/test.cjs` -> **100.0%** Exposure
- `extensions/emmet/src/test/abbreviationAction.test.ts` -> **100.0%** Exposure
### Hardcoded Payload Artifacts
- `src/vs/platform/terminal/test/node/terminalEnvironment.test.ts` -> **99.9462%** Exposure
### Algorithmic DoS Exposure
- `extensions/extension-editing/src/extensionLinter.ts` -> **100.0%** Exposure
- `extensions/git/src/historyProvider.ts` -> **100.0%** Exposure
- `extensions/grunt/src/main.ts` -> **100.0%** Exposure
- `extensions/microsoft-authentication/src/node/cachedPublicClientApplication.ts` -> **100.0%** Exposure
- `extensions/npm/src/features/packageJSONContribution.ts` -> **100.0%** Exposure
...
### 1. `src/vs/workbench/contrib/mcp/browser/mcpServerActions.ts` (TYPESCRIPT) -> Cumulative Risk: **939.81**
- **Archetype:** `file_cluster_4` (Distance: 14.76 IQR)
- **Magnitude:** 225.3 | **LOC:** 1398 | **CtrlFlow:** 40.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Tech Debt (100.0%), Concurrency (100.0%), State Flux (100.0%)
- **Heaviest Functions:** `updateStatus` (Impact: 106.2), `update` (Impact: 42.0), `getContextMenuActions` (Impact: 23.1)

### 2. `resources/linux/snap/electron-launch` (SHELL) -> Cumulative Risk: **937.42**
- **Archetype:** `file_cluster_4` (Distance: 13.972 IQR)
- **Magnitude:** 293.2 | **LOC:** 280 | **CtrlFlow:** 63.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Algorithmic Dos (100.0%), Injection Surface (100.0%)
- **Heaviest Functions:** `is_subpath_[Truncated]` (Impact: 159.6), `prepend_dir` (Impact: 8.9), `append_dir` (Impact: 8.9)

### 3. `src/vs/editor/contrib/codeAction/browser/codeActionModel.ts` (TYPESCRIPT) -> Cumulative Risk: **896.3**
- **Archetype:** `file_cluster_13` (Distance: 13.864 IQR)
- **Magnitude:** 70.25 | **LOC:** 414 | **CtrlFlow:** 49.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `_update` (Impact: 240.4), `_getRangeOfSelectionUnlessWhitespaceEncl` (Impact: 44.1), `ignoreLightbulbOff` (Impact: 16.6)

### 4. `extensions/typescript-language-features/src/typescriptServiceClient.ts` (TYPESCRIPT) -> Cumulative Risk: **893.23**
- **Archetype:** `file_cluster_4` (Distance: 14.258 IQR)
- **Magnitude:** 164.35 | **LOC:** 1310 | **CtrlFlow:** 53.2% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `serviceExited` (Impact: 150.6), `dispatchEvent` (Impact: 90.0), `startService` (Impact: 54.6)

### 5. `extensions/grunt/src/main.ts` (TYPESCRIPT) -> Cumulative Risk: **870.61**
- **Archetype:** `file_cluster_4` (Distance: 12.854 IQR)
- **Magnitude:** 65.38 | **LOC:** 365 | **CtrlFlow:** 56.4% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `computeTasks` (Impact: 94.2), `getTask` (Impact: 45.6), `findGruntCommand` (Impact: 16.6)

### 6. `src/vs/workbench/contrib/extensions/browser/extensionsActions.ts` (TYPESCRIPT) -> Cumulative Risk: **869.58**
- **Archetype:** `file_cluster_4` (Distance: 15.507 IQR)
- **Magnitude:** 647.04 | **LOC:** 3252 | **CtrlFlow:** 46.3% | **Authorship Centralization:** 27.3%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `computeAndUpdateStatus` (Impact: 331.6), `run` (Impact: 121.8), `run` (Impact: 106.8)

### 7. `src/vs/platform/userDataProfile/common/userDataProfile.ts` (TYPESCRIPT) -> Cumulative Risk: **864.33**
- **Archetype:** `file_cluster_4` (Distance: 14.52 IQR)
- **Magnitude:** 123.55 | **LOC:** 734 | **CtrlFlow:** 63.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `toUserDataProfile` (Impact: 104.2), `updateProfiles` (Impact: 66.3), `profilesObject` (Impact: 56.8)

### 8. `src/vs/editor/browser/widget/diffEditor/diffEditorViewModel.ts` (TYPESCRIPT) -> Cumulative Risk: **863.52**
- **Archetype:** `file_cluster_17` (Distance: 13.866 IQR)
- **Magnitude:** 82.96 | **LOC:** 772 | **CtrlFlow:** 48.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `constructor` (Impact: 149.6), `fromDiffs` (Impact: 36.3), `normalizeRangeMapping` (Impact: 18.8)

### 9. `src/vs/workbench/contrib/chat/browser/chatManagement/chatModelsWidget.ts` (TYPESCRIPT) -> Cumulative Risk: **848.2**
- **Archetype:** `file_cluster_13` (Distance: 14.071 IQR)
- **Magnitude:** 120.66 | **LOC:** 1383 | **CtrlFlow:** 44.9% | **Authorship Centralization:** 75.8%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%), State Flux (99.9994%)
- **Heaviest Functions:** `createTable` (Impact: 144.0), `getModelHoverContent` (Impact: 48.5), `create` (Impact: 31.9)

### 10. `src/vs/workbench/contrib/notebook/common/notebookEditorModel.ts` (TYPESCRIPT) -> Cumulative Risk: **846.84**
- **Archetype:** `file_cluster_4` (Distance: 14.784 IQR)
- **Magnitude:** 80.46 | **LOC:** 397 | **CtrlFlow:** 46.0% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `load` (Impact: 35.9), `getSaveErrorMessage` (Impact: 33.5), `constructor` (Impact: 31.3)
...
- `scripts/xterm-update.js` (JAVASCRIPT) | Magnitude: 64.18 | Delta: **0.007 IQR** | Secondary Pull: `file_cluster_4`
...
- `src/vs/workbench/contrib/welcomeWalkthrough/browser/editor/vs_code_editor_walkthrough.ts` (TYPESCRIPT) | Magnitude: 21.79 | Delta: **0.013 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: branch: 49, structural_boundaries: 24, indent_tabs: 19, encapsulation: 15
- `src/vs/editor/browser/gpu/rectangleRenderer.wgsl.ts` (TYPESCRIPT) | Magnitude: 4.05 | Delta: **0.019 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 33, decorators: 14, structural_boundaries: 9, state_mutation: 5
...
- `resources/server/bin-dev/remote-cli/code.sh` (SHELL) | Magnitude: 43.18 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_9`
  * Top Architectural Signatures: state_mutation: 21, branch: 11, io: 4, structural_boundaries: 3
- `src/vs/base/browser/ui/tree/indexTreeModel.ts` (TYPESCRIPT) | Magnitude: 91.75 | Delta: **0.008 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 531, state_mutation: 312, branch: 151, structural_boundaries: 88
- `src/vs/base/common/fuzzyScorer.ts` (TYPESCRIPT) | Magnitude: 59.94 | Delta: **0.009 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 393, branch: 164, structural_boundaries: 95, immutability_locks: 94
- `scripts/test.sh` (SHELL) | Magnitude: 6.92 | Delta: **0.014 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 30, branch: 24, indent_tabs: 17, safety_bypasses: 11
- `extensions/terminal-suggest/src/fig/autocomplete-parser/parseArguments.ts` (TYPESCRIPT) | Magnitude: 43.43 | Delta: **0.019 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_tabs: 429, branch: 111, structural_boundaries: 97, state_mutation: 58

### Mixed-Responsibility Refactoring Targets for: file_cluster_12
- `scripts/test-documentation.sh` (SHELL) | Magnitude: 2.84 | Delta: **0.041 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: state_mutation: 12, branch: 9, safety_bypasses: 5, indent_tabs: 5
- `scripts/node-electron.sh` (SHELL) | Magnitude: 6.24 | Delta: **0.045 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 24, branch: 23, indent_tabs: 15, safety_bypasses: 6
- `resources/server/bin/helpers/check-requirements-linux.sh` (SHELL) | Magnitude: 241.3 | Delta: **0.056 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: state_mutation: 109, indent_spaces: 78, branch: 62, io: 48
...
- `src/vs/base/node/ps.sh` (SHELL) | Magnitude: 32.76 | Delta: **0.28 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 18, safety_bypasses: 14, state_mutation: 12, branch: 11

### Mixed-Responsibility Refactoring Targets for: file_cluster_13
- `src/vs/workbench/common/editor/editorGroupModel.ts` (TYPESCRIPT) | Magnitude: 156.36 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_tabs: 821, state_mutation: 805, branch: 251, structural_boundaries: 133
- `src/vs/workbench/contrib/terminalContrib/links/test/browser/linkTestUtils.ts` (TYPESCRIPT) | Magnitude: 2.22 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_tabs: 32, structural_boundaries: 19, args: 7, state_mutation: 6
- `src/vs/platform/remote/common/managedSocket.ts` (TYPESCRIPT) | Magnitude: 14.21 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_4`
  * Top Architectural Signatures: indent_tabs: 93, state_mutation: 51, structural_boundaries: 30, args: 19
- `src/vs/workbench/contrib/files/electron-browser/fileCommands.ts` (TYPESCRIPT) | Magnitude: 3.45 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
...
- `src/vs/workbench/contrib/terminalContrib/chatAgentTools/test/electron-browser/treeSitterCommandParser.test.ts` (TYPESCRIPT) | Magnitude: 95.01 | Delta: **0.008 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_tabs: 261, branch: 247, structural_boundaries: 239, args: 220

### Mixed-Responsibility Refactoring Targets for: file_cluster_16
- `src/vs/base/browser/ui/tree/objectTree.ts` (TYPESCRIPT) | Magnitude: 31.03 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 197, generics: 90, state_mutation: 81, structural_boundaries: 67
- `src/vs/editor/browser/gpu/objectCollectionBuffer.ts` (TYPESCRIPT) | Magnitude: 18.65 | Delta: **0.004 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: state_mutation: 131, indent_tabs: 120, structural_boundaries: 38, immutability_locks: 28
- `src/vs/base/common/observableInternal/utils/promise.ts` (TYPESCRIPT) | Magnitude: 10.37 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_4`
  * Top Architectural Signatures: indent_tabs: 59, state_mutation: 41, structural_boundaries: 31, concurrency: 21
- `src/vs/platform/mcp/common/mcpGateway.ts` (TYPESCRIPT) | Magnitude: 2.52 | Delta: **0.007 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 27, immutability_locks: 26, structural_boundaries: 22, doc: 18
- `src/vs/workbench/common/panecomposite.ts` (TYPESCRIPT) | Magnitude: 3.93 | Delta: **0.007 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: structural_boundaries: 6, func_start: 3, indent_tabs: 3, args: 2

### Mixed-Responsibility Refactoring Targets for: file_cluster_17
- `extensions/git/src/branchProtection.ts` (TYPESCRIPT) | Magnitude: 3.13 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 28, state_mutation: 17, structural_boundaries: 13, args: 9
- `src/vs/platform/userDataSync/common/keybindingsMerge.ts` (TYPESCRIPT) | Magnitude: 23.14 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 245, branch: 102, state_mutation: 89, structural_boundaries: 74
- `src/vs/workbench/api/common/extHostTypes/snippetString.ts` (TYPESCRIPT) | Magnitude: 10.78 | Delta: **0.003 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 71, state_mutation: 58, structural_boundaries: 15, branch: 12
- `src/vs/workbench/services/authentication/browser/authenticationExtensionsService.ts` (TYPESCRIPT) | Magnitude: 30.43 | Delta: **0.003 IQR** | Secondary Pull: `file_cluster_4`
  * Top Architectural Signatures: indent_tabs: 210, state_mutation: 157, structural_boundaries: 55, immutability_locks: 41
- `src/vs/workbench/api/test/browser/extHostTreeViews.test.ts` (TYPESCRIPT) | Magnitude: 18.22 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_4`
  * Top Architectural Signatures: indent_tabs: 286, structural_boundaries: 113, sec_high_risk_execution: 81, args: 63

### Mixed-Responsibility Refactoring Targets for: file_cluster_2
- `extensions/terminal-suggest/src/fig/fig-autocomplete-shared/specMetadata.ts` (TYPESCRIPT) | Magnitude: 8.61 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_16`
...
- `src/vscode-dts/vscode.proposed.findTextInFiles2.d.ts` (TYPESCRIPT) | Magnitude: 25.85 | Delta: **0.024 IQR** | Secondary Pull: `file_cluster_7`
  * Top Architectural Signatures: indent_tabs: 25, doc: 22, branch: 16, structural_boundaries: 9

### Mixed-Responsibility Refactoring Targets for: file_cluster_4
- `src/vs/workbench/contrib/extensions/test/electron-browser/extensionsViews.test.ts` (TYPESCRIPT) | Magnitude: 38.37 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_tabs: 452, structural_boundaries: 157, concurrency: 143, decorators: 103
- `src/vs/workbench/contrib/notebook/test/browser/contrib/outputCopyTests.test.ts` (TYPESCRIPT) | Magnitude: 5.22 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 93, structural_boundaries: 33, immutability_locks: 27, concurrency: 23
- `src/vs/editor/browser/widget/diffEditor/components/diffEditorViewZones/inlineDiffDeletedCodeMargin.ts` (TYPESCRIPT) | Magnitude: 25.4 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 137, state_mutation: 109, structural_boundaries: 39, concurrency: 30
- `src/vs/workbench/contrib/chat/browser/widgetHosts/chatQuick.ts` (TYPESCRIPT) | Magnitude: 67.81 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: state_mutation: 344, indent_tabs: 312, branch: 90, structural_boundaries: 61
- `src/vs/workbench/contrib/chat/test/browser/actions/chatTitleActions.test.ts` (TYPESCRIPT) | Magnitude: 14.49 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 180, structural_boundaries: 71, concurrency: 71, state_mutation: 37
...
- `src/vs/workbench/contrib/testing/common/testId.ts` (TYPESCRIPT) | Magnitude: 25.28 | Delta: **0.036 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 139, state_mutation: 96, structural_boundaries: 38, api: 35
...
- `src/bootstrap-fork.ts` (TYPESCRIPT) | Magnitude: 6.68 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 75, branch: 28, args: 17, structural_boundaries: 16
- `src/vs/platform/accessibility/browser/accessibleView.ts` (TYPESCRIPT) | Magnitude: 9.17 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
...
- `resources/server/bin/code-server-linux.sh` (SHELL) | Magnitude: 14.96 | Delta: **0.006 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: branch: 6, indent_tabs: 6, safety_bypasses: 4, args: 3
...
- `resources/server/bin/remote-cli/code-linux.sh` (SHELL) | Magnitude: 8.16 | Delta: **0.028 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 6, args: 1, safety: 1, reflection_metaprogramming: 1
- `src/vs/workbench/contrib/notebook/browser/notebookLogger.ts` (TYPESCRIPT) | Magnitude: 1.1 | Delta: **0.04 IQR** | Secondary Pull: `file_cluster_0`
  * Top Architectural Signatures: indent_tabs: 11, args: 4, func_start: 4, encapsulation: 3
...
- `src/vs/workbench/contrib/chat/browser/chat.contribution.ts` -> Churn: **100.0%** | Cog Load: 15.7544% | Debt: 76.927%
- `src/vs/workbench/contrib/chat/browser/agentSessions/agentSessionsViewer.ts` -> Churn: **91.52%** | Cog Load: 87.6846% | Debt: 83.7875%
- `src/vs/workbench/contrib/chat/browser/widget/input/chatInputPart.ts` -> Churn: **90.48%** | Cog Load: 87.3269% | Debt: 70.082%
...
- `src/vs/workbench/contrib/chat/browser/widget/chatWidget.ts` -> Churn: **84.35%** | Cog Load: 100.0% | Debt: 17.0037%

### 👤 Key Person Dependencies (High Impact + Siloed Knowledge)
These are massive, load-bearing files written almost entirely by a single developer. They represent severe 'Bus Factor' risk.

- `cli/src/tunnels/code_server.rs` -> **Josh Spicer** (100.0% isolated ownership) | Magnitude: 490.96
- `cli/src/commands/serve_web.rs` -> **Dan Plischke** (100.0% isolated ownership) | Magnitude: 304.82
- `resources/linux/snap/electron-launch` -> **Robo** (100.0% isolated ownership) | Magnitude: 293.2
- `src/vs/workbench/contrib/userDataProfile/browser/userDataProfilesEditorModel.ts` -> **Sandeep Somavarapu** (100.0% isolated ownership) | Magnitude: 277.02
...
- `src/vs/editor/common/languages/highlights/css.scm` -> **Severity: 867.181** (Blast Radius: 28.26 * Doc Risk: 30.6858%)
- `extensions/typescript-language-features/src/utils/fs.ts` -> **Severity: 769.483** (Blast Radius: 9.619 * Doc Risk: 79.9962%)
- `extensions/typescript-language-features/src/typescriptService.ts` -> **Severity: 248.683** (Blast Radius: 3.254 * Doc Risk: 76.4238%)
- `extensions/html-language-features/server/src/modes/languageModes.ts` -> **Severity: 213.515** (Blast Radius: 2.625 * Doc Risk: 81.339%)
