...
| **Timestamp** | `2026-08-03T20:03:05.281840+00:00` |
| **Scan Duration** | `0.21s` |
| **Git Branch** | `main` |
...
| Algorithmic DoS Exposure | 0.0 | 3.8 | 1.9 | 1.9 | 3.8 |
...
- `terser` (@ `rollup.config.js`) -> Impact: **2.5** | LOC: 11

## 8.5 ALGORITHMIC & DATABASE BOTTLENECKS
> Highlights the most computationally expensive and database-heavy functions across the repository.

### Highest Time Complexity (Big-O)
- `terser` (@ `rollup.config.js`) -> **O(N^3)**

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `src` | 1 | 45.6 | 6.1% | 0.0% |
| `__monolith__` | 5 | 45.4 | 1.22% | 9.8% |
...
- `rollup.config.js` -> **3.8406%** Exposure
...
### 2. `rollup.config.js` (JAVASCRIPT) -> Cumulative Risk: **176.47**
- **Archetype:** `file_cluster_8` (Distance: 7.247 IQR)
- **Magnitude:** 8.42 | **LOC:** 66 | **CtrlFlow:** 18.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (48.9756%), Documentation (11.9203%), Cognitive Load (6.0785%)
- **Heaviest Functions:** `onwarn` (Impact: 3.7), `terser` (Impact: 2.5)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` d3-time-format, d3-dispatch, d3-format, d3-time, d3-contour, d3-brush, d3-interpolate, d3-drag...
...
- **Global Archetype:** `file_cluster_8` (Drift: 7.247 IQR)
- **Local Micro-Species:** `Cluster 3: Pure View Layer Components (UI)` (Drift: 4.556 IQR)
- **Top Global Matches:** file_cluster_8: 7.247, file_cluster_13: 7.858, file_cluster_0: 8.153
- **Magnitude:** 8.42 | **LOC:** 66 | **CtrlFlow:** 18.2% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N^3) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (6.0785%), Tech Debt (48.9756%)
**Top Internal Functions/Classes:**
  * `onwarn` (Impact: 3.7 | O(N^1))
  * `terser` (Impact: 2.5 | O(N^3))
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 166.667
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` plugin-terser, fs, package.json, plugin-json, plugin-node-resolve
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- `rollup.config.js` (JAVASCRIPT) | Magnitude: 8.42 | Delta: **0.611 IQR** | Secondary Pull: `file_cluster_13`
