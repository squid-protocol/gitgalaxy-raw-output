...
| **Timestamp** | `2026-08-03T20:14:45.870940+00:00` |
| **Scan Duration** | `8.45s` |
| **Git Branch** | `master` |
...
|---|---|---|
| Modularity | 0.7257 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
> **Architectural Drift Z-Score:** `4.498`
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 0.0 | 100.0 | 64.0 | 78.0 | 5.0 |
| Error & Exception Exposure | 0.0 | 100.0 | 76.1 | 87.6 | 0.0 |
| Tech Debt Exposure | 0.0 | 100.0 | 36.0 | 13.9 | 0.0 |
| Testing Exposure | 0.0 | 80.0 | 7.5 | 2.3 | 2.3 |
...
| Documentation Exposure | 0.0 | 100.0 | 34.2 | 18.5 | 11.9 |
...
- `read` (@ `realtime/src/acquisition/openbci/java/src/OpenBCI_ADS1299.java`) -> Impact: **2254.8** | LOC: 583
- `main` (@ `realtime/src/acquisition/openbci/openbci2ft.c`) -> Impact: **708.9** | LOC: 738
- `mexFunction` (@ `src/nanstd.c`) -> Impact: **613.4** | LOC: 327
...
- `mexFunction` (@ `src/nanvar.c`) -> Impact: **613.4** | LOC: 327
  * *Intent:* #if defined (COMPILER_MSVC) #include <math.h> #define isnan _isnan #define INFINITY (HUGE_VAL+HUGE_VAL) #define NAN (INFINITY - INFINITY) #elif define...
- `ft_realtime_headlocalizer` (@ `realtime/online_meg/ft_realtime_headlocalizer.m`) -> Impact: **513.3** | LOC: 1068
...
- `main` (@ `realtime/src/acquisition/tmsi/tmsidriver.cpp`) -> Impact: **480.5** | LOC: 395
- `main` (@ `realtime/src/utilities/audio/ft2audio.c`) -> Impact: **362.8** | LOC: 398
- `mexFunction` (@ `src/nansum.c`) -> Impact: **357.8** | LOC: 179
  * *Intent:* #if defined (COMPILER_MSVC) #include <math.h> #define isnan _isnan #define INFINITY (HUGE_VAL+HUGE_VAL) #define NAN (INFINITY - INFINITY) #elif define...
- `tcpserver` (@ `realtime/src/buffer/src/tcpserver.c`) -> Impact: **322.2** | LOC: 209
- `append_file_as_chunk` (@ `realtime/src/acquisition/neuromag/process_tag.c`) -> Impact: **312.3** | LOC: 249

## 8.5 ALGORITHMIC & DATABASE BOTTLENECKS
> Highlights the most computationally expensive and database-heavy functions across the repository.

### Highest Time Complexity (Big-O)
- `serialize` (@ `realtime/src/buffer/python/FieldTrip.py`) -> **O(2^N) [Recursive]**
- `main` (@ `realtime/src/acquisition/tmsi/tmsidriver.cpp`) -> **O(2^N) [Recursive]**
- `unfreezeColors` (@ `contrib/nutmegtrip/private/unfreezeColors.m`) -> **O(2^N) [Recursive]**
  * *Intent:* % unfreezeColors Restore colors of a plot to original indexed color. (v2.3) % % Useful if you want to apply a new colormap to plots whose % colors wer...
- `sap_reverse_in_place` (@ `realtime/src/acquisition/siemens/src/siemensap.c`) -> **O(2^N) [Recursive]**
- `RTDevice` (@ `realtime/src/acquisition/tmsi/RTDevice.cpp`) -> **O(2^N) [Recursive]**
  * *Intent:* //If we have a handle then
- `read` (@ `realtime/src/acquisition/openbci/java/src/OpenBCI_ADS1299.java`) -> **O(2^N) [Recursive]**
- `read` (@ `realtime/src/acquisition/openbci/java/src/Serial.java`) -> **O(2^N) [Recursive]**
- `readBytesUntil` (@ `realtime/src/acquisition/openbci/java/src/Serial.java`) -> **O(2^N) [Recursive]**
- `read_spmeeg_event` (@ `fileio/private/read_spmeeg_event.m`) -> **O(2^N) [Recursive]**
  * *Intent:* % read_spmeeg_event() - import evtns from SPM5 and SPM8 meeg datasets % % Usage: % >> header = read_spmeeg_event(filename); % % Inputs: % filename - [...
- `append_file_as_chunk` (@ `realtime/src/acquisition/neuromag/process_tag.c`) -> **O(2^N) [Recursive]**

### Highest Data Gravity (Database Complexity)
- `main` (@ `realtime/src/acquisition/openbci/openbci2ft.c`) -> DB Complexity: **289**
- `main` (@ `realtime/src/acquisition/jaga/jaga2ft.c`) -> DB Complexity: **273**
  * *Intent:* /* see http://stackoverflow.com/questions/230062/whats-the-best-way-to-check-if-a-file-exists-in-c-cross-platform */
- `ft_realtime_headlocalizer` (@ `realtime/online_meg/ft_realtime_headlocalizer.m`) -> DB Complexity: **230**
  * *Intent:* % FT_REALTIME_HEADLOCALIZER is a real-time application for online visualization of % the head position for the CTF275 and the Neuromag/Elekta/Megin sy...
- `load_curry_data_file` (@ `fileio/private/load_curry_data_file.m`) -> DB Complexity: **173**
- `main` (@ `realtime/src/acquisition/tmsi/tmsidriver.cpp`) -> DB Complexity: **147**
- `PixelDataGrabber::fillXFormFromSAP` (@ `realtime/src/acquisition/siemens/src/PixelDataGrabber.cc`) -> DB Complexity: **135**
- `main` (@ `realtime/src/acquisition/ctf/ctf2ft_v1.c`) -> DB Complexity: **131**
- `main` (@ `realtime/src/utilities/audio/ft2audio.c`) -> DB Complexity: **130**
- `mexFunction` (@ `src/nanstd.c`) -> DB Complexity: **127**
  * *Intent:* #if defined (COMPILER_MSVC) #include <math.h> #define isnan _isnan #define INFINITY (HUGE_VAL+HUGE_VAL) #define NAN (INFINITY - INFINITY) #elif define...
- `mexFunction` (@ `src/nanvar.c`) -> DB Complexity: **127**
  * *Intent:* #if defined (COMPILER_MSVC) #include <math.h> #define isnan _isnan #define INFINITY (HUGE_VAL+HUGE_VAL) #define NAN (INFINITY - INFINITY) #elif define...

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `fileio/private` | 358 | 85294.52 | 71.38% | 39.89% |
| `private` | 317 | 68102.33 | 64.82% | 31.18% |
| `__monolith__` | 159 | 58526.63 | 73.63% | 45.91% |
| `fileio` | 34 | 37555.04 | 67.85% | 25.85% |
| `utilities` | 93 | 18585.88 | 67.82% | 49.28% |
| `forward/private` | 96 | 16506.19 | 66.03% | 27.28% |
| `plotting/private` | 90 | 15227.51 | 69.85% | 33.88% |
| `utilities/private` | 118 | 14637.0 | 67.14% | 32.73% |
| `plotting` | 30 | 11299.46 | 73.53% | 24.29% |
| `src` | 72 | 10432.58 | 63.34% | 40.88% |
...
- `realtime/src/buffer/java/nl/fcdonders/fieldtrip/BufferClient.java` -> **16** Orphaned Functions | **12** Duplicates
- `realtime/src/acquisition/openbci/java/src/Serial.java` -> **13** Orphaned Functions | **14** Duplicates
- `realtime/src/acquisition/openbci/java/src/OpenBCI_ADS1299.java` -> **22** Orphaned Functions | **3** Duplicates
...
177. **`compat/matlablt2010b/@uint64/max.c`** -> AI Confidence: **99.29%**
178. **`compat/matlablt2010b/@uint64/min.c`** -> AI Confidence: **99.29%**
179. **`compat/matlablt2010b/@uint64/minus.c`** -> AI Confidence: **99.29%**
180. **`compat/matlablt2010b/@uint64/plus.c`** -> AI Confidence: **99.29%**
181. **`compat/matlablt2010b/@uint64/rdivide.c`** -> AI Confidence: **99.29%**
182. **`compat/matlablt2010b/@uint64/times.c`** -> AI Confidence: **99.29%**
183. **`realtime/src/acquisition/siemens/include/unixtime.h`** -> AI Confidence: **99.29%**
184. **`realtime/src/acquisition/siemens/src/sap2matlab.c`** -> AI Confidence: **99.29%**
185. **`realtime/src/buffer/matlab/buffer_getevt.c`** -> AI Confidence: **99.29%**
186. **`realtime/src/buffer/matlab/buffer_putevt.c`** -> AI Confidence: **99.29%**
187. **`realtime/src/buffer/src/rdaserver.c`** -> AI Confidence: **99.29%**
188. **`realtime/src/buffer/test/demo_sinewave.c`** -> AI Confidence: **99.29%**
189. **`realtime/src/buffer/test/test_flushdat.c`** -> AI Confidence: **99.29%**
190. **`realtime/src/buffer/test/test_flushevt.c`** -> AI Confidence: **99.29%**
191. **`realtime/src/buffer/test/test_flushhdr.c`** -> AI Confidence: **99.29%**
192. **`realtime/src/buffer/test/test_getevt.c`** -> AI Confidence: **99.29%**
193. **`realtime/src/utilities/serial2event/serial2event.c`** -> AI Confidence: **99.29%**
194. **`src/ft_getopt.c`** -> AI Confidence: **99.29%**
195. **`src/ft_spike_sub_crossx.c`** -> AI Confidence: **99.29%**
196. **`src/mtimes3x3.c`** -> AI Confidence: **99.29%**
197. **`src/nanaccum.c`** -> AI Confidence: **99.29%**
198. **`src/sandwich2x2.c`** -> AI Confidence: **99.29%**
199. **`src/sandwich3x3.c`** -> AI Confidence: **99.29%**
200. **`src/splint_gh.c`** -> AI Confidence: **99.29%**
201. **`realtime/src/acquisition/siemens/include/compiler.h`** -> AI Confidence: **99.29%**
202. **`realtime/src/acquisition/siemens/include/platform.h`** -> AI Confidence: **99.29%**
203. **`realtime/src/buffer/src/compiler.h`** -> AI Confidence: **99.29%**
204. **`realtime/src/buffer/src/platform.h`** -> AI Confidence: **99.29%**
205. **`realtime/src/acquisition/gtec/CheckServerReply.cpp`** -> AI Confidence: **99.29%**
206. **`realtime/src/acquisition/gtec/EscapeXML.cpp`** -> AI Confidence: **99.29%**
207. **`realtime/src/acquisition/gtec/ReadMeasurementData.cpp`** -> AI Confidence: **99.29%**
208. **`realtime/src/acquisition/modeeg/modeeg2ft.cc`** -> AI Confidence: **99.29%**
209. **`realtime/src/acquisition/neurosky/thinkgear2ft.cc`** -> AI Confidence: **99.29%**
210. **`realtime/src/acquisition/siemens/src/PixelDataGrabber.cc`** -> AI Confidence: **99.29%**
211. **`realtime/src/acquisition/siemens/src/testFolderWatcher.cc`** -> AI Confidence: **99.29%**
212. **`realtime/src/buffer/cpp/mex/myfilter.cc`** -> AI Confidence: **99.29%**
213. **`realtime/src/acquisition/openbci/java/src/OpenBCI_ADS1299.java`** -> AI Confidence: **99.29%**
214. **`realtime/src/acquisition/openbci/java/src/openBCI2ft.java`** -> AI Confidence: **99.29%**
215. **`realtime/src/buffer/matlab/buffer.m`** -> AI Confidence: **99.29%**
216. **`utilities/printstruct.m`** -> AI Confidence: **99.29%**
217. **`realtime/src/buffer/cpp/ConsoleInput.h`** -> AI Confidence: **99.28%**
218. **`realtime/src/acquisition/neuromag/include/fiff.h`** -> AI Confidence: **99.26%**
219. **`realtime/src/buffer/cpp/OnlineDataManager.h`** -> AI Confidence: **99.25%**
220. **`realtime/src/acquisition/ctf/ctf2ft_v3.c`** -> AI Confidence: **99.24%**
221. **`realtime/src/buffer/test/test_socketserver.c`** -> AI Confidence: **99.24%**
222. **`src/read_24bit.c`** -> AI Confidence: **99.23%**
...
- `contrib/MOxUnit_fieldtrip/moxunit_fieldtrip_util_send_json.m` -> **100.0%** Exposure
- `contrib/MOxUnit_fieldtrip/tests/test_moxunit_fieldtrip_test_case.m` -> **100.0%** Exposure
- `contrib/MOxUnit_fieldtrip/tests/test_moxunit_fieldtrip_test_suite_filtering.m` -> **100.0%** Exposure
- `contrib/MOxUnit_fieldtrip/tests/test_moxunit_fieldtrip_util_send_json.m` -> **100.0%** Exposure
### Weaponizable Injection Vectors
- `contrib/MOxUnit_fieldtrip/moxunit_fieldtrip_util_send_json.m` -> **100.0%** Exposure
- `contrib/MOxUnit_fieldtrip/tests/test_moxunit_fieldtrip_util_send_json.m` -> **100.0%** Exposure
- `fileio/ft_write_event.m` -> **100.0%** Exposure
- `fileio/private/read_gmsh_binary.m` -> **100.0%** Exposure
- `forward/ft_headmodel_dipoli.m` -> **100.0%** Exposure
### Raw Memory Manipulation
- `realtime/src/acquisition/openbci/openbci2ft.c` -> **10.0%** Exposure
- `realtime/src/buffer/test/demo_sinewave.c` -> **10.0%** Exposure
- `realtime/src/buffer/test/event.c` -> **10.0%** Exposure
- `realtime/src/buffer/test/sinewave.c` -> **10.0%** Exposure
- `realtime/src/buffer/test/test_benchmark.c` -> **10.0%** Exposure
...
- `contrib/MOxUnit_fieldtrip/tests/test_moxunit_fieldtrip_test_case.m` -> **100.0%** Exposure
- `contrib/MOxUnit_fieldtrip/tests/test_moxunit_fieldtrip_test_suite_filtering.m` -> **100.0%** Exposure
- `contrib/MOxUnit_fieldtrip/tests/test_moxunit_fieldtrip_util_struct2json.m` -> **100.0%** Exposure
- `fileio/private/load_curry_data_file.m` -> **100.0%** Exposure
...
### 1. `realtime/src/buffer/java/bufferserver/data/SavingRingDataStore.java` (JAVA) -> Cumulative Risk: **800.92**
- **Archetype:** `file_cluster_13` (Distance: 11.659 IQR)
- **Magnitude:** 572.86 | **LOC:** 340 | **CtrlFlow:** 51.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%), Concurrency (99.9918%)
- **Heaviest Functions:** `putEvents` (Impact: 141.1), `putData` (Impact: 122.3), `putHeader` (Impact: 105.4)

### 2. `realtime/src/acquisition/openbci/java/src/Serial.java` (JAVA) -> Cumulative Risk: **788.37**
- **Archetype:** `file_cluster_4` (Distance: 13.887 IQR)
- **Magnitude:** 685.0 | **LOC:** 516 | **CtrlFlow:** 58.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Logic Bomb (100.0%), Tech Debt (99.9993%), Documentation (96.5359%)
- **Heaviest Functions:** `read` (Impact: 248.2), `readBytesUntil` (Impact: 71.0), `Serial` (Impact: 38.6)

### 3. `realtime/src/buffer/src/tcpsocket.c` (C) -> Cumulative Risk: **779.6**
- **Archetype:** `file_cluster_4` (Distance: 13.108 IQR)
- **Magnitude:** 394.02 | **LOC:** 238 | **CtrlFlow:** 83.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `tcpsocket` (Impact: 230.7), `cleanup_tcpsocket` (Impact: 9.6)

### 4. `fileio/private/read_ahdf5_hdr.m` (MATLAB) -> Cumulative Risk: **777.67**
- **Archetype:** `file_cluster_8` (Distance: 13.001 IQR)
- **Magnitude:** 137.92 | **LOC:** 67 | **CtrlFlow:** 68.4% | **Authorship Centralization:** 0.0%
...
### 5. `src/ft_getopt.c` (C) -> Cumulative Risk: **771.33**
- **Archetype:** `file_cluster_13` (Distance: 14.655 IQR)
- **Magnitude:** 220.04 | **LOC:** 127 | **CtrlFlow:** 94.3% | **Authorship Centralization:** 0.0%
...
### 6. `qsub/qsubfeval.m` (MATLAB) -> Cumulative Risk: **762.55**
- **Archetype:** `file_cluster_4` (Distance: 15.54 IQR)
- **Magnitude:** 362.24 | **LOC:** 535 | **CtrlFlow:** 64.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Logic Bomb (100.0%), Injection Surface (100.0%)
- **Heaviest Functions:** `qsubfeval` (Impact: 4.2)

### 7. `realtime/src/buffer/src/tcpserver.c` (C) -> Cumulative Risk: **757.98**
- **Archetype:** `file_cluster_4` (Distance: 13.16 IQR)
- **Magnitude:** 496.54 | **LOC:** 271 | **CtrlFlow:** 82.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `tcpserver` (Impact: 322.2), `cleanup_tcpserver` (Impact: 5.8)

### 8. `realtime/src/buffer/matlab/buffer.c` (C) -> Cumulative Risk: **751.86**
- **Archetype:** `file_cluster_13` (Distance: 11.954 IQR)
- **Magnitude:** 155.48 | **LOC:** 451 | **CtrlFlow:** 61.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Algorithmic Dos (100.0%), Concurrency (99.9985%), State Flux (99.9738%)
- **Heaviest Functions:** `exitFun` (Impact: 47.5), `add_hps_item` (Impact: 22.2), `lookup_hps_item` (Impact: 7.5)

### 9. `realtime/src/buffer/src/tcprequest.c` (C) -> Cumulative Risk: **751.09**
- **Archetype:** `file_cluster_13` (Distance: 13.252 IQR)
- **Magnitude:** 111.8 | **LOC:** 111 | **CtrlFlow:** 88.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%), Tech Debt (99.6604%)
- **Heaviest Functions:** `tcprequest` (Impact: 54.7)

### 10. `realtime/src/acquisition/tmsi/RTDevice.cpp` (CPP) -> Cumulative Risk: **743.17**
- **Archetype:** `file_cluster_13` (Distance: 13.879 IQR)
- **Magnitude:** 340.62 | **LOC:** 396 | **CtrlFlow:** 42.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%), Documentation (98.2778%)
- **Heaviest Functions:** `RTDevice::OpenHandle` (Impact: 37.6), `RTDevice` (Impact: 36.1), `RTDevice::Init` (Impact: 30.6)
...
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (72.6601%), Tech Debt (12.0991%)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `fixchantype` (Impact: 4.8 | O(N^1) | DB: 3)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTION to fill...
  * `fixchanunit` (Impact: 4.8 | O(N^1) | DB: 3)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTION to fill...
  * `recursive_read_header` (Impact: 2.4 | O(N^1) | DB: 2)
...
- **Risk Profile:** Cognitive Load (89.9084%), Tech Debt (19.5984%)
**Top Internal Functions/Classes:**
  * `ft_filetype` (Impact: 7.5 | O(N^1) | DB: 2)
...
### `realtime/src/acquisition/openbci/java/src/OpenBCI_ADS1299.java` (JAVA | Tier 2 | 🚨 AI THREAT: 99.29%)
- **Global Archetype:** `file_cluster_0` (Drift: 14.05 IQR)
- **Top Global Matches:** file_cluster_0: 14.05, file_cluster_9: 14.07, file_cluster_8: 14.089
- **Magnitude:** 2886.52 | **LOC:** 1064 | **CtrlFlow:** 85.1% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 2
- **Risk Profile:** Cognitive Load (63.0276%), Tech Debt (85.8184%)
**Top Internal Functions/Classes:**
  * `read` (Impact: 2254.8 | O(2^N))
  * `openSerialPort` (Impact: 286.5 | O(2^N) | DB: 1)
  * `OpenBCI_ADS1299` (Impact: 90.7 | O(2^N))
  * `updateSyncState` (Impact: 24.5 | O(N^2) | DB: 1)
  * `printRegisters` (Impact: 24.3 | O(2^N))
    * *Intent:* // serial_openBCI.write("j"); // send OpenBCI's 'j' commaned to make sure any already open SD file i...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 337`, `structural_boundaries: 59`, `args: 60`, `func_start: 54`, `class_start: 1`
* *Risk/State:* `safety_bypasses: 13`, `state_mutation: 25`, `dead_code: 26`, `duplicate_logic: 3`, `orphaned_logic: 22`
* *Architecture:* `io: 2`, `api: 55`, `import: 1`
* *Defense:* `safety: 3`, `immutability_locks: 23`
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 0.375
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` java.io.OutputStream
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (78.776%), Tech Debt (31.9038%)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `chan2source` (Impact: 17.1 | O(N^1) | DB: 14)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % convert between dat...
  * `print_segmentationinfo` (Impact: 12.0 | O(N^1) | DB: 13)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTION %%%%%%%...
  * `ft_checkdata` (Impact: 6.0 | O(N^1) | DB: 2)
    * *Intent:* % FT_CHECKDATA checks the input data of the main FieldTrip functions, e.g. whether the % type of dat...
  * `source2raw` (Impact: 2.4 | O(N^1) | DB: 2)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % convert between dat...
**Structural Signatures (Net Mitigated Signals):**
...
- **Global Archetype:** `file_cluster_13` (Drift: 14.571 IQR)
- **Local Micro-Species:** `Cluster 2: Inert Headers & Declarative Structures` (Drift: 5.035 IQR)
- **Top Global Matches:** file_cluster_13: 14.571, file_cluster_8: 14.652, file_cluster_11: 14.677
- **Magnitude:** 2043.54 | **LOC:** 1097 | **CtrlFlow:** 94.5% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(N^3) | **DB Complexity:** 289
- **Risk Profile:** Cognitive Load (82.2273%), Tech Debt (11.7682%)
**Top Internal Functions/Classes:**
  * `main` (Impact: 708.9 | O(N^3) | DB: 289)
  * `iniHandler` (Impact: 311.6 | O(N^1) | DB: 68)
  * `interpret16bitAsInt32` (Impact: 8.4 | O(N^2) | DB: 3)
  * `interpret24bitAsInt32` (Impact: 5.8 | O(N^1) | DB: 3)
  * `serialWriteSlow` (Impact: 2.4 | O(N^1) | DB: 4)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 343`, `structural_boundaries: 20`, `args: 6`, `func_start: 6`, `class_start: 2`
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 3):` socketserver.h, ini.h, stdio.h, buffer.h, string.h, math.h, signal.h, timestamp.h...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Risk Profile:** Cognitive Load (72.7561%), Tech Debt (64.3949%)
**Top Internal Functions/Classes:**
  * `ft_databrowser` (Impact: 8.7 | O(N^1) | DB: 6)
    * *Intent:* % FT_DATABROWSER can be used for visual inspection of data. Artifacts that were % detected by artifa...
  * `cb_datacursortext` (Impact: 8.1 | O(N^1) | DB: 10)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%...
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `cb_keyboard` (Impact: 13.7 | O(N^1) | DB: 7)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%...
  * `ft_sourceplot` (Impact: 10.5 | O(N^1) | DB: 4)
...
- **Algorithmic:** O(N) | **DB Complexity:** 0
- **Risk Profile:** Cognitive Load (66.8695%), Tech Debt (34.1815%)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `ft_read_data` (Impact: 16.9 | O(N^1) | DB: 10)
...
- `realtime/src/acquisition/openbci/java/src/OpenBCI_ADS1299.java` (JAVA) | Magnitude: 2886.52 | Delta: **0.02 IQR** | Secondary Pull: `file_cluster_9`
...
- `connectivity/ft_connectivity_pdc.m` (MATLAB) | Magnitude: 96.42 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_17`
...
- `realtime/src/rebuild.sh` (SHELL) | Magnitude: 84.4 | Delta: **0.009 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_spaces: 38, state_mutation: 35, branch: 32, safety_bypasses: 16
...
- `src/mxDeserialize_cpp.cpp` (CPP) | Magnitude: 31.88 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 21, macros: 10, structural_boundaries: 8, branch: 7
- `src/mxSerialize_cpp.cpp` (CPP) | Magnitude: 31.86 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 21, macros: 9, structural_boundaries: 8, branch: 7
- `realtime/src/buffer/cpp/GDF_BackgroundWriter.h` (C) | Magnitude: 216.34 | Delta: **0.003 IQR** | Secondary Pull: `file_cluster_4`
...
- `realtime/src/acquisition/siemens/python/fakeScanner.py` (PYTHON) | Magnitude: 19.14 | Delta: **0.006 IQR** | Secondary Pull: `file_cluster_8`
...
- `utilities/ft_trackusage.m` (MATLAB) | Magnitude: 128.98 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: state_mutation: 65, indent_spaces: 65, structural_boundaries: 30, branch: 18
- `fileio/private/read_eeglabheader.m` (MATLAB) | Magnitude: 85.46 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_9`
  * Top Architectural Signatures: state_mutation: 66, indent_spaces: 32, branch: 21, structural_boundaries: 10
- `realtime/online_mri/ft_omri_info_from_header.m` (MATLAB) | Magnitude: 145.48 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: state_mutation: 141, indent_tabs: 53, branch: 31, structural_boundaries: 12
...
- `realtime/src/buffer/src/interface.c` (C) | Magnitude: 509.64 | Delta: **0.004 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 269, state_mutation: 263, pointers: 199, branch: 57
- `realtime/src/buffer/java/bufferserver/data/SimpleDataStore.java` (JAVA) | Magnitude: 263.64 | Delta: **0.008 IQR** | Secondary Pull: `file_cluster_8`
...
- `realtime/src/buffer/src/socketserver.c` (C) | Magnitude: 705.6 | Delta: **0.139 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 339, state_mutation: 287, branch: 120, pointers: 112
...
- `compat/obsolete/ft_prepare_singleshell.m` (MATLAB) | Magnitude: 35.18 | Delta: **0.064 IQR** | Secondary Pull: `file_cluster_11`
...
- `src/splint_gh.c` (C) | Magnitude: 254.92 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_13`
...
- `nutmeg2fieldtrip.m` (MATLAB) | Magnitude: 263.72 | Delta: **0.003 IQR** | Secondary Pull: `file_cluster_17`
...
- `ft_electrodeplacement.m` -> Churn: **77.46%** | Cog Load: 69.9694% | Debt: 10.8343%
- `ft_statistics_montecarlo.m` -> Churn: **66.67%** | Cog Load: 83.1331% | Debt: 26.8038%
- `plotting/ft_plot_mesh.m` -> Churn: **66.67%** | Cog Load: 86.4113% | Debt: 0.0%
...
- `realtime/src/buffer/src/buffer.h` -> **Severity: 2016.517** (Blast Radius: 20.342 * Doc Risk: 99.1307%)
- `realtime/src/buffer/src/win32/stdint.h` -> **Severity: 1892.22** (Blast Radius: 19.101 * Doc Risk: 99.0639%)
- `realtime/src/buffer/src/platform_includes.h` -> **Severity: 710.467** (Blast Radius: 11.835 * Doc Risk: 60.031%)
- `realtime/src/buffer/src/message.h` -> **Severity: 636.797** (Blast Radius: 6.368 * Doc Risk: 99.9996%)
- `realtime/src/buffer/src/swapbytes.h` -> **Severity: 317.7** (Blast Radius: 3.177 * Doc Risk: 100.0%)
