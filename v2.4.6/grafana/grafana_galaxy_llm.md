...
| **Timestamp** | `2026-08-03T20:04:40.801429+00:00` |
| **Scan Duration** | `54.92s` |
| **Git Branch** | `main` |
...
> **Architectural Drift Z-Score:** `5.392`
...
|---|---|---|
| file_cluster_8 | 8352 | 60.3% |
| file_cluster_13 | 3448 | 24.9% |
| file_cluster_0 | 322 | 2.3% |
| file_cluster_4 | 246 | 1.8% |
| file_cluster_17 | 246 | 1.8% |
| file_cluster_2 | 230 | 1.7% |
| file_cluster_16 | 132 | 1.0% |
| file_cluster_11 | 64 | 0.5% |
| file_cluster_9 | 54 | 0.4% |
| file_cluster_15 | 16 | 0.1% |
| Unknown | 14 | 0.1% |
| file_cluster_12 | 12 | 0.1% |
...
| Documentation Exposure | 0.0 | 100.0 | 50.0 | 46.7 | 6.7 |
...
- `updateSchema` (@ `public/app/features/dashboard/state/DashboardMigrator.ts`) -> Impact: **1337.0** | LOC: 933
...
- `tweakAxis` (@ `packages/grafana-ui/src/graveyard/TimeSeries/utils.ts`) -> Impact: **1325.4** | LOC: 527
- `formatValue` (@ `public/app/core/components/TimeSeries/utils.ts`) -> Impact: **984.3** | LOC: 357
- `getStatementPosition` (@ `public/app/plugins/datasource/cloudwatch/language/cloudwatch-logs-sql/completion/statementPosition.ts`) -> Impact: **899.0** | LOC: 344
- `Generate` (@ `pkg/services/ngalert/models/testing.go`) -> Impact: **860.7** | LOC: 1248
- `combineFolderResponses` (@ `public/app/api/clients/folder/v1beta1/hooks.ts`) -> Impact: **848.2** | LOC: 418
  * *Intent:* /** * FolderDTO with optional ownerReferences * * (owner references will only be populated with app platform API + team folders functionality) */
- `prepConfig` (@ `public/app/plugins/panel/heatmap/utils.ts`) -> Impact: **838.9** | LOC: 478
- `convertToRowsLayout` (@ `public/app/features/dashboard/api/ResponseTransformers.ts`) -> Impact: **835.8** | LOC: 312
- `cleanupK8sDashboardResources` (@ `pkg/services/dashboards/service/dashboard_service.go`) -> Impact: **824.7** | LOC: 1054
...
- `Policy` (@ `public/app/features/alerting/unified/components/notification-policies/Policy.tsx`) -> Impact: **814.3** | LOC: 326

## 8.5 ALGORITHMIC & DATABASE BOTTLENECKS
> Highlights the most computationally expensive and database-heavy functions across the repository.

### Highest Time Complexity (Big-O)
- `describe` (@ `packages/grafana-data/src/field/fieldDisplay.test.ts`) -> **O(2^N) [Recursive]**
- `updateDatasourcePluginResetOption` (@ `packages/grafana-sql/src/components/configuration/TLSSecretsConfig.tsx`) -> **O(2^N) [Recursive]**
- `getStyles` (@ `packages/grafana-ui/src/components/InteractiveTable/InteractiveTable.tsx`) -> **O(2^N) [Recursive]**
- `describe` (@ `packages/grafana-ui/src/components/Table/TableNG/TableNG.test.tsx`) -> **O(2^N) [Recursive]**
- `duplicate` (@ `public/app/features/alerting/unified/components/notification-policies/EditDefaultPolicyForm.tsx`) -> **O(2^N) [Recursive]**
- `Policy` (@ `public/app/features/alerting/unified/components/notification-policies/Policy.tsx`) -> **O(2^N) [Recursive]**
- `onSetCondition` (@ `public/app/features/alerting/unified/components/rule-editor/QueryRows.tsx`) -> **O(2^N) [Recursive]**
- `updateNextViewRangeTime` (@ `public/app/features/explore/TraceView/components/TracePageHeader/TracePageHeader.tsx`) -> **O(2^N) [Recursive]**
- `removeHoverIndentGuideId` (@ `public/app/features/explore/TraceView/components/TraceTimelineViewer/SpanBarRow.tsx`) -> **O(2^N) [Recursive]**
- `describe` (@ `packages/grafana-data/src/field/fieldOverrides.test.ts`) -> **O(2^N) [Recursive]**

### Highest Data Gravity (Database Complexity)
- `describe` (@ `public/app/features/provisioning/utils/treeUtils.test.ts`) -> DB Complexity: **358**
- `NewTestSqlKvBackend` (@ `pkg/storage/unified/testing/storage_backend_sql_compatibility.go`) -> DB Complexity: **339**
- `getAppRoutes` (@ `public/app/routes/routes.tsx`) -> DB Complexity: **240**
- `Generate` (@ `pkg/services/ngalert/models/testing.go`) -> DB Complexity: **232**
- `cleanupK8sDashboardResources` (@ `pkg/services/dashboards/service/dashboard_service.go`) -> DB Complexity: **225**
  * *Intent:* // cleanupK8sDashboardResources cleans up resources marked for deletion in the k8s API. // It processes all organizations, finds dashboards with the t...
- `jsonDataToMetaJSONData` (@ `apps/plugins/pkg/app/meta/converter.go`) -> DB Complexity: **202**
  * *Intent:* // jsonDataToMetaJSONData converts a plugins.JSONData to a pluginsv0alpha1.MetaJSONData. // nolint:gocyclo
- `createGrafDir` (@ `pkg/tests/testinfra/testinfra.go`) -> DB Complexity: **200**
  * *Intent:* //nolint:gocyclo
- `ContactPointToContactPointExport` (@ `pkg/services/ngalert/api/compat_contact_points.go`) -> DB Complexity: **142**
  * *Intent:* // ContactPointToContactPointExport converts definitions.ContactPoint to notify.APIReceiver. // It uses special extension for json-iterator API that p...
- `parseIntegration` (@ `pkg/services/ngalert/api/compat_contact_points.go`) -> DB Complexity: **140**
  * *Intent:* //nolint:gocyclo
- `GetPostStartHooks` (@ `pkg/registry/apis/provisioning/register.go`) -> DB Complexity: **124**

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `pkg/util/xorm` | 41 | 13711.3 | 51.28% | 36.35% |
| `pkg/api` | 50 | 13075.24 | 39.03% | 25.89% |
| `pkg/storage/unified/resource` | 42 | 11844.26 | 32.76% | 33.72% |
...
| `pkg/services/ngalert/api` | 39 | 7417.96 | 29.33% | 51.17% |
| `apps/dashboard/pkg/migration/conversion` | 22 | 5679.4 | 44.5% | 43.87% |
| `devenv/docker/blocks/slow_proxy` | 4 | 5066.16 | 11.78% | 0.0% |
...
- `packages/grafana-api-clients/src/clients/rtkq/legacy/endpoints.gen.ts` -> **0** Orphaned Functions | **119** Duplicates
- `public/app/plugins/datasource/graphite/gfunc.ts` -> **0** Orphaned Functions | **105** Duplicates
- `pkg/plugins/manager/pluginfakes/fakes.go` -> **62** Orphaned Functions | **25** Duplicates
- `public/app/features/alerting/unified/utils/cloud-alertmanager-notifier-types.ts` -> **0** Orphaned Functions | **84** Duplicates
- `pkg/expr/mathexp/types.go` -> **0** Orphaned Functions | **36** Duplicates
...
941. **`devenv/create_docker_compose.sh`** -> AI Confidence: **99.29%**
942. **`devenv/docker/blocks/smtp/bootstrap.sh`** -> AI Confidence: **99.29%**
943. **`devenv/docker/buildcontainer/build_circle.sh`** -> AI Confidence: **99.29%**
944. **`devenv/frontend-service/local-init.sh`** -> AI Confidence: **99.29%**
945. **`e2e/start-and-run-suite`** -> AI Confidence: **99.29%**
946. **`packaging/docker/run.sh`** -> AI Confidence: **99.29%**
947. **`scripts/check-breaking-changes.sh`** -> AI Confidence: **99.29%**
948. **`scripts/ci/backend-tests/pkgs-with-tests-named.sh`** -> AI Confidence: **99.29%**
949. **`scripts/ci/backend-tests/shard.sh`** -> AI Confidence: **99.29%**
950. **`scripts/cleanup-husky.sh`** -> AI Confidence: **99.29%**
951. **`scripts/publish-npm-packages.sh`** -> AI Confidence: **99.29%**
952. **`scripts/stripnulls.sh`** -> AI Confidence: **99.29%**
953. **`scripts/validate-npm-packages.sh`** -> AI Confidence: **99.29%**
954. **`scripts/view-playwright-ci.sh`** -> AI Confidence: **99.29%**
955. **`.github/workflows/scripts/fr-notify.mts`** -> AI Confidence: **99.29%**
956. **`.github/workflows/scripts/pr-notify.mts`** -> AI Confidence: **99.29%**
957. **`packages/grafana-data/src/transformations/transformers/nulls/nullToUndefThreshold.ts`** -> AI Confidence: **99.29%**
958. **`packages/grafana-data/src/types/select.ts`** -> AI Confidence: **99.29%**
959. **`packages/grafana-runtime/src/utils/toDataQueryError.ts`** -> AI Confidence: **99.29%**
960. **`packages/grafana-ui/src/components/JSONFormatter/json_explorer/json_explorer.ts`** -> AI Confidence: **99.29%**
961. **`packages/grafana-ui/src/graveyard/GraphNG/nullToUndefThreshold.ts`** -> AI Confidence: **99.29%**
962. **`packages/grafana-ui/src/utils/logOptions.ts`** -> AI Confidence: **99.29%**
963. **`public/app/core/time_series2.ts`** -> AI Confidence: **99.29%**
964. **`public/app/features/alerting/unified/components/receivers/TemplateDataExamples.ts`** -> AI Confidence: **99.29%**
965. **`public/app/features/alerting/unified/utils/template-constants.ts`** -> AI Confidence: **99.29%**
966. **`public/app/plugins/datasource/azuremonitor/azureMetadata/resourceTypes.ts`** -> AI Confidence: **99.29%**
967. **`public/app/plugins/datasource/cloudwatch/language/cloudwatch-sql/completion/statementPosition.ts`** -> AI Confidence: **99.29%**
968. **`public/app/plugins/panel/heatmap/renderHistogram.tsx`** -> AI Confidence: **99.29%**
969. **`scripts/cli/themeTemplates/_variables.scss.tmpl.ts`** -> AI Confidence: **99.29%**
970. **`jest.config.js`** -> AI Confidence: **99.29%**
971. **`devenv/docker/blocks/mysql_opendata/Dockerfile`** -> AI Confidence: **99.29%**
972. **`packaging/docker/custom/Dockerfile`** -> AI Confidence: **99.29%**
973. **`scripts/verify-repo-update/Dockerfile.deb`** -> AI Confidence: **99.29%**
974. **`apps/advisor/Makefile`** -> AI Confidence: **99.29%**
975. **`apps/correlations/Makefile`** -> AI Confidence: **99.29%**
976. **`pkg/semconv/Makefile`** -> AI Confidence: **99.29%**
977. **`pkg/services/featuremgmt/strcase/snake.go`** -> AI Confidence: **99.29%**
978. **`pkg/services/pluginsintegration/pluginconfig/azure_settings.go`** -> AI Confidence: **99.29%**
979. **`pkg/services/queryhistory/writers.go`** -> AI Confidence: **99.29%**
980. **`pkg/services/sqlstore/migrations/usermig/service_account_multiple_org_login_migrator.go`** -> AI Confidence: **99.29%**
981. **`pkg/setting/setting_azure.go`** -> AI Confidence: **99.29%**
982. **`pkg/tsdb/azuremonitor/metrics/migrations.go`** -> AI Confidence: **99.29%**
983. **`pkg/util/xorm/session_delete.go`** -> AI Confidence: **99.29%**
984. **`devenv/docker/blocks/mysql_opendata/import_csv.sql`** -> AI Confidence: **99.29%**
985. **`pkg/registry/apis/iam/legacy/testdata/mysql--update_org_user-update_org_user_basic.sql`** -> AI Confidence: **99.29%**
986. **`pkg/registry/apis/iam/legacy/testdata/mysql--update_team-update_team_basic.sql`** -> AI Confidence: **99.29%**
987. **`pkg/registry/apis/iam/legacy/testdata/mysql--update_team_member_query-update_team_member_basic.sql`** -> AI Confidence: **99.29%**
988. **`pkg/registry/apis/iam/legacy/testdata/mysql--update_user-update_user_basic.sql`** -> AI Confidence: **99.29%**
989. **`pkg/registry/apis/iam/legacy/testdata/postgres--update_org_user-update_org_user_basic.sql`** -> AI Confidence: **99.29%**
990. **`pkg/registry/apis/iam/legacy/testdata/postgres--update_team-update_team_basic.sql`** -> AI Confidence: **99.29%**
991. **`pkg/registry/apis/iam/legacy/testdata/postgres--update_team_member_query-update_team_member_basic.sql`** -> AI Confidence: **99.29%**
992. **`pkg/registry/apis/iam/legacy/testdata/postgres--update_user-update_user_basic.sql`** -> AI Confidence: **99.29%**
993. **`pkg/registry/apis/iam/legacy/testdata/sqlite--update_org_user-update_org_user_basic.sql`** -> AI Confidence: **99.29%**
994. **`pkg/registry/apis/iam/legacy/testdata/sqlite--update_team-update_team_basic.sql`** -> AI Confidence: **99.29%**
995. **`pkg/registry/apis/iam/legacy/testdata/sqlite--update_team_member_query-update_team_member_basic.sql`** -> AI Confidence: **99.29%**
996. **`pkg/registry/apis/iam/legacy/testdata/sqlite--update_user-update_user_basic.sql`** -> AI Confidence: **99.29%**
997. **`pkg/registry/apis/iam/legacy/update_org_user.sql`** -> AI Confidence: **99.29%**
998. **`pkg/registry/apis/iam/legacy/update_team.sql`** -> AI Confidence: **99.29%**
999. **`pkg/registry/apis/iam/legacy/update_team_member_query.sql`** -> AI Confidence: **99.29%**
1000. **`pkg/registry/apis/iam/legacy/update_user.sql`** -> AI Confidence: **99.29%**
1001. **`pkg/storage/secret/encryption/data/data_key_disable.sql`** -> AI Confidence: **99.29%**
1002. **`pkg/storage/secret/encryption/data/data_key_disable_all.sql`** -> AI Confidence: **99.29%**
1003. **`pkg/storage/secret/encryption/data/encrypted_value_update.sql`** -> AI Confidence: **99.29%**
1004. **`pkg/storage/secret/encryption/data/encrypted_value_update_bulk.sql`** -> AI Confidence: **99.29%**
1005. **`pkg/storage/secret/encryption/testdata/mysql--data_key_disable-disable.sql`** -> AI Confidence: **99.29%**
1006. **`pkg/storage/secret/encryption/testdata/mysql--data_key_disable_all-disable.sql`** -> AI Confidence: **99.29%**
1007. **`pkg/storage/secret/encryption/testdata/mysql--encrypted_value_update-update.sql`** -> AI Confidence: **99.29%**
1008. **`pkg/storage/secret/encryption/testdata/mysql--encrypted_value_update_bulk-update_bulk_two_rows_for_same_namespace.sql`** -> AI Confidence: **99.29%**
1009. **`pkg/storage/secret/encryption/testdata/postgres--data_key_disable-disable.sql`** -> AI Confidence: **99.29%**
1010. **`pkg/storage/secret/encryption/testdata/postgres--data_key_disable_all-disable.sql`** -> AI Confidence: **99.29%**
1011. **`pkg/storage/secret/encryption/testdata/postgres--encrypted_value_update-update.sql`** -> AI Confidence: **99.29%**
1012. **`pkg/storage/secret/encryption/testdata/postgres--encrypted_value_update_bulk-update_bulk_two_rows_for_same_namespace.sql`** -> AI Confidence: **99.29%**
1013. **`pkg/storage/secret/encryption/testdata/sqlite--data_key_disable-disable.sql`** -> AI Confidence: **99.29%**
1014. **`pkg/storage/secret/encryption/testdata/sqlite--data_key_disable_all-disable.sql`** -> AI Confidence: **99.29%**
1015. **`pkg/storage/secret/encryption/testdata/sqlite--encrypted_value_update-update.sql`** -> AI Confidence: **99.29%**
1016. **`pkg/storage/secret/encryption/testdata/sqlite--encrypted_value_update_bulk-update_bulk_two_rows_for_same_namespace.sql`** -> AI Confidence: **99.29%**
1017. **`pkg/storage/secret/metadata/data/keeper_set_as_active.sql`** -> AI Confidence: **99.29%**
1018. **`pkg/storage/secret/metadata/data/keeper_update.sql`** -> AI Confidence: **99.29%**
1019. **`pkg/storage/secret/metadata/data/secure_value_create.sql`** -> AI Confidence: **99.29%**
1020. **`pkg/storage/secret/metadata/data/secure_value_set_inactive_all_from_group.sql`** -> AI Confidence: **99.29%**
1021. **`pkg/storage/secret/metadata/data/secure_value_set_version_to_active.sql`** -> AI Confidence: **99.29%**
1022. **`pkg/storage/secret/metadata/data/secure_value_set_version_to_inactive.sql`** -> AI Confidence: **99.29%**
1023. **`pkg/storage/secret/metadata/data/secure_value_updateExternalId.sql`** -> AI Confidence: **99.29%**
1024. **`pkg/storage/secret/metadata/testdata/mysql--keeper_set_as_active-keeper set as active.sql`** -> AI Confidence: **99.29%**
1025. **`pkg/storage/secret/metadata/testdata/mysql--keeper_update-update.sql`** -> AI Confidence: **99.29%**
1026. **`pkg/storage/secret/metadata/testdata/mysql--secure_value_set_inactive_all_from_group-set inactive all from group.sql`** -> AI Confidence: **99.29%**
1027. **`pkg/storage/secret/metadata/testdata/mysql--secure_value_set_version_to_active-set secure value version to active.sql`** -> AI Confidence: **99.29%**
1028. **`pkg/storage/secret/metadata/testdata/mysql--secure_value_updateExternalId-updateExternalId.sql`** -> AI Confidence: **99.29%**
1029. **`pkg/storage/secret/metadata/testdata/postgres--keeper_set_as_active-keeper set as active.sql`** -> AI Confidence: **99.29%**
1030. **`pkg/storage/secret/metadata/testdata/postgres--keeper_update-update.sql`** -> AI Confidence: **99.29%**
1031. **`pkg/storage/secret/metadata/testdata/postgres--secure_value_set_inactive_all_from_group-set inactive all from group.sql`** -> AI Confidence: **99.29%**
1032. **`pkg/storage/secret/metadata/testdata/postgres--secure_value_set_version_to_active-set secure value version to active.sql`** -> AI Confidence: **99.29%**
1033. **`pkg/storage/secret/metadata/testdata/postgres--secure_value_updateExternalId-updateExternalId.sql`** -> AI Confidence: **99.29%**
1034. **`pkg/storage/secret/metadata/testdata/sqlite--keeper_set_as_active-keeper set as active.sql`** -> AI Confidence: **99.29%**
1035. **`pkg/storage/secret/metadata/testdata/sqlite--keeper_update-update.sql`** -> AI Confidence: **99.29%**
1036. **`pkg/storage/secret/metadata/testdata/sqlite--secure_value_set_inactive_all_from_group-set inactive all from group.sql`** -> AI Confidence: **99.29%**
1037. **`pkg/storage/secret/metadata/testdata/sqlite--secure_value_set_version_to_active-set secure value version to active.sql`** -> AI Confidence: **99.29%**
1038. **`pkg/storage/secret/metadata/testdata/sqlite--secure_value_updateExternalId-updateExternalId.sql`** -> AI Confidence: **99.29%**
1039. **`pkg/storage/unified/resource/data/sqlkv_update_datastore.sql`** -> AI Confidence: **99.29%**
1040. **`pkg/storage/unified/resource/data/sqlkv_update_legacy_resource_history.sql`** -> AI Confidence: **99.29%**
1041. **`pkg/storage/unified/sql/data/resource_history_delete.sql`** -> AI Confidence: **99.29%**
1042. **`pkg/storage/unified/sql/data/resource_update.sql`** -> AI Confidence: **99.29%**
1043. **`pkg/storage/unified/sql/rvmanager/data/resource_history_update_rv.sql`** -> AI Confidence: **99.29%**
1044. **`pkg/storage/unified/sql/rvmanager/data/resource_update_rv.sql`** -> AI Confidence: **99.29%**
1045. **`pkg/storage/unified/sql/rvmanager/data/resource_version_update.sql`** -> AI Confidence: **99.29%**
1046. **`pkg/storage/unified/sql/testdata/mysql--resource_history_update_rv-single path.sql`** -> AI Confidence: **99.29%**
1047. **`pkg/storage/unified/sql/testdata/mysql--resource_update-single path.sql`** -> AI Confidence: **99.29%**
1048. **`pkg/storage/unified/sql/testdata/mysql--resource_version_update-increment resource version.sql`** -> AI Confidence: **99.29%**
1049. **`pkg/storage/unified/sql/testdata/postgres--resource_history_update_rv-single path.sql`** -> AI Confidence: **99.29%**
1050. **`pkg/storage/unified/sql/testdata/postgres--resource_update-single path.sql`** -> AI Confidence: **99.29%**
1051. **`pkg/storage/unified/sql/testdata/postgres--resource_version_update-increment resource version.sql`** -> AI Confidence: **99.29%**
1052. **`pkg/storage/unified/sql/testdata/sqlite--resource_history_update_rv-single path.sql`** -> AI Confidence: **99.29%**
1053. **`pkg/storage/unified/sql/testdata/sqlite--resource_update-single path.sql`** -> AI Confidence: **99.29%**
1054. **`pkg/storage/unified/sql/testdata/sqlite--resource_version_update-increment resource version.sql`** -> AI Confidence: **99.29%**
1055. **`packaging/autocomplete/powershell_autocomplete.ps1`** -> AI Confidence: **99.29%**
1056. **`public/app/features/canvas/elements/icon.tsx`** -> AI Confidence: **99.25%**

## 10.6 WEAPONIZABLE SURFACE EXPOSURES (RULE-BASED SAST)
> Secondary Evidence: The following files tripped specific static threat signatures. Use these to explain *why* the XGBoost model flagged the files above.

### Obfuscation & Evasion Surface
- `public/app/features/provisioning/utils/validators.test.ts` -> **0.0002%** Exposure
### Exploit Generation Surface
- `.github/workflows/scripts/crowdin/create-tasks.ts` -> **100.0%** Exposure
- `.github/workflows/scripts/pr-notify.mts` -> **100.0%** Exposure
- `e2e-playwright/plugin-e2e/mysql/mocks/mysql.mocks.ts` -> **100.0%** Exposure
- `packages/grafana-api-clients/src/clients/rtkq/advisor/v0alpha1/endpoints.gen.ts` -> **100.0%** Exposure
- `packages/grafana-api-clients/src/clients/rtkq/dashboard/v0alpha1/endpoints.gen.ts` -> **100.0%** Exposure
### Weaponizable Injection Vectors
- `devenv/bulk-folders/bulk-folders.sh` -> **100.0%** Exposure
- `packaging/deb/control/postinst` -> **100.0%** Exposure
- `packaging/rpm/control/postinst` -> **100.0%** Exposure
- `scripts/build-targz.sh` -> **100.0%** Exposure
- `scripts/download-catalog-plugins.sh` -> **100.0%** Exposure
...
- `packaging/docker/run.sh` -> **100.0%** Exposure
- `scripts/ci/backend-tests/pkgs-with-tests-named.sh` -> **100.0%** Exposure
...
### 1. `public/app/features/dashboard-scene/pages/DashboardScenePageStateManager.ts` (TYPESCRIPT) -> Cumulative Risk: **934.29**
- **Archetype:** `file_cluster_4` (Distance: 13.174 IQR)
- **Magnitude:** 87.38 | **LOC:** 1321 | **CtrlFlow:** 46.1% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `transformResponseToScene` (Impact: 133.7), `reloadDashboard` (Impact: 70.8), `loadDashboard` (Impact: 34.6)

### 2. `public/app/features/dashboard/dashgrid/DashboardGrid.tsx` (TYPESCRIPT) -> Cumulative Risk: **920.33**
- **Archetype:** `file_cluster_13` (Distance: 12.353 IQR)
- **Magnitude:** 37.4 | **LOC:** 422 | **CtrlFlow:** 48.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%)
- **Heaviest Functions:** `componentDidMount` (Impact: 40.4), `renderPanels` (Impact: 30.0), `buildLayout` (Impact: 23.7)

### 3. `e2e-playwright/utils/axe-a11y/reporter.ts` (TYPESCRIPT) -> Cumulative Risk: **899.06**
- **Archetype:** `file_cluster_4` (Distance: 12.804 IQR)
- **Magnitude:** 11.14 | **LOC:** 87 | **CtrlFlow:** 45.9% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `onTestEnd` (Impact: 35.6), `onEnd` (Impact: 17.0), `testsWithViolations` (Impact: 4.3)

### 4. `public/app/features/dashboard-scene/panel-edit/PanelEditor.tsx` (TYPESCRIPT) -> Cumulative Risk: **883.31**
- **Archetype:** `file_cluster_13` (Distance: 12.991 IQR)
- **Magnitude:** 48.86 | **LOC:** 437 | **CtrlFlow:** 38.0% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `_updateDataPane` (Impact: 47.1), `waitForPlugin` (Impact: 34.3), `gotPanelPlugin` (Impact: 17.9)

### 5. `public/app/features/dashboard/components/DashboardSettings/VersionsSettings.tsx` (TYPESCRIPT) -> Cumulative Risk: **875.57**
- **Archetype:** `file_cluster_17` (Distance: 13.183 IQR)
- **Magnitude:** 22.26 | **LOC:** 202 | **CtrlFlow:** 42.1% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `transformToRevisionModels` (Impact: 34.5), `getVersions` (Impact: 31.2), `render` (Impact: 25.1)

### 6. `packages/grafana-ui/src/components/uPlot/plugins/EventBusPlugin.tsx` (TYPESCRIPT) -> Cumulative Risk: **872.49**
- **Archetype:** `file_cluster_17` (Distance: 14.296 IQR)
- **Magnitude:** 10.56 | **LOC:** 161 | **CtrlFlow:** 43.2% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `useLayoutEffect` (Impact: 72.6), `EventBusPlugin` (Impact: 1.1)

### 7. `public/app/plugins/datasource/cloudwatch/query-runner/CloudWatchMetricsQueryRunner.ts` (TYPESCRIPT) -> Cumulative Risk: **872.47**
- **Archetype:** `file_cluster_17` (Distance: 12.025 IQR)
- **Magnitude:** 24.65 | **LOC:** 204 | **CtrlFlow:** 35.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Algorithmic Dos (100.0%), State Flux (99.9955%), Concurrency (99.9681%)
- **Heaviest Functions:** `queryFn` (Impact: 73.7), `alertOnThrottlingErrors` (Impact: 32.2), `enrichThrottlingErrorMessages` (Impact: 24.1)

### 8. `public/app/core/services/context_srv.ts` (TYPESCRIPT) -> Cumulative Risk: **870.2**
- **Archetype:** `file_cluster_4` (Distance: 12.673 IQR)
- **Magnitude:** 26.55 | **LOC:** 279 | **CtrlFlow:** 33.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (99.999%)
- **Heaviest Functions:** `scheduleTokenRotationJob` (Impact: 16.8), `rotateToken` (Impact: 11.3), `fetchUserPermissions` (Impact: 8.2)

### 9. `public/app/features/canvas/runtime/frame.tsx` (TYPESCRIPT) -> Cumulative Risk: **869.69**
- **Archetype:** `file_cluster_13` (Distance: 12.772 IQR)
- **Magnitude:** 52.42 | **LOC:** 270 | **CtrlFlow:** 60.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%)
- **Heaviest Functions:** `doAction` (Impact: 296.9), `constructor` (Impact: 25.2), `visit` (Impact: 8.3)

### 10. `public/app/AppWrapper.tsx` (TYPESCRIPT) -> Cumulative Risk: **868.49**
- **Archetype:** `file_cluster_13` (Distance: 11.445 IQR)
- **Magnitude:** 11.91 | **LOC:** 158 | **CtrlFlow:** 22.4% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), Algorithmic Dos (100.0%)
- **Heaviest Functions:** `componentDidMount` (Impact: 18.8), `render` (Impact: 17.7), `renderRoute` (Impact: 6.5)
...
- **Global Archetype:** `file_cluster_11` (Drift: 14.09 IQR)
- **Top Global Matches:** file_cluster_11: 14.09, file_cluster_15: 14.211, file_cluster_8: 14.286
- **Magnitude:** 1981.08 | **LOC:** 1144 | **CtrlFlow:** 76.7% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 69
- **Risk Profile:** Cognitive Load (61.4153%), Tech Debt (8.7912%)
**Top Internal Functions/Classes:**
  * `buildUpdates` (Impact: 608.6 | O(N^2) | DB: 69)
  * `genSelectSQL` (Impact: 64.3 | O(N^1) | DB: 13)
  * `genGetSQL` (Impact: 46.8 | O(N^1) | DB: 13)
  * `Join` (Impact: 42.3 | O(2^N) | DB: 21)
  * `And` (Impact: 39.4 | O(2^N) | DB: 13)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` fmt, core, driver, reflect, builder, strings, time
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `pkg/storage/unified/testing/storage_backend_sql_compatibility.go` (GO | Tier 2 | 🚨 AI THREAT: 99.35%)
- **Global Archetype:** `file_cluster_8` (Drift: 14.214 IQR)
- **Top Global Matches:** file_cluster_8: 14.214, file_cluster_7: 14.281, file_cluster_15: 14.392
- **Magnitude:** 1909.72 | **LOC:** 2675 | **CtrlFlow:** 69.3% | **Authorship Centralization:** 100.0%
- **Algorithmic:** O(N^2) | **DB Complexity:** 339
- **Risk Profile:** Cognitive Load (23.4483%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `NewTestSqlKvBackend` (Impact: 672.5 | O(N^2) | DB: 339)
  * `runSearchCompatibilityScenario` (Impact: 13.7 | O(N^1) | DB: 39)
  * `runTestSearchOperationsCompatibility` (Impact: 9.7 | O(N^1))
    * *Intent:* // Read from write backend
  * `numberToWord` (Impact: 6.3 | O(N^1) | DB: 1)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 212`, `structural_boundaries: 94`, `args: 29`, `func_start: 29`, `class_start: 7`
* *Risk/State:* `state_mutation: 879`, `orphaned_logic: 2`
* *Architecture:* `io: 1`, `api: 221`, `concurrency: 78`, `import: 1`
* *Defense:* `safety: 16`, `doc: 144`, `test: 246`, `immutability_locks: 4`, `cleanup: 3`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 3):` fmt, context, title, kv, uid, generation, json, apiVersion...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `pkg/storage/unified/resource/server.go` (GO | Tier 2 | 🚨 AI THREAT: 99.31%)
- **Global Archetype:** `file_cluster_8` (Drift: 14.414 IQR)
- **Top Global Matches:** file_cluster_8: 14.414, file_cluster_4: 14.471, file_cluster_7: 14.531
- **Magnitude:** 1892.78 | **LOC:** 1942 | **CtrlFlow:** 52.7% | **Authorship Centralization:** 100.0%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 27
- **Risk Profile:** Cognitive Load (39.0778%), Tech Debt (8.6232%)
**Top Internal Functions/Classes:**
  * `newEvent` (Impact: 89.8 | O(N^1) | DB: 22)
    * *Intent:* // Old value indicates an update -- otherwise a create // //nolint:gocyclo
  * `Watch` (Impact: 78.2 | O(N^1) | DB: 27)
    * *Intent:* //nolint:gocyclo
  * `List` (Impact: 68.8 | O(2^N) | DB: 7)
  * `Stop` (Impact: 56.6 | O(2^N) | DB: 13)
  * `delete` (Impact: 47.8 | O(2^N) | DB: 20)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 5):` fmt, context, validation, otel, semver, v1, scheduler, json...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `pkg/storage/unified/resource/search.go` (GO | Tier 2 | 🚨 AI THREAT: 99.31%)
- **Global Archetype:** `file_cluster_8` (Drift: 14.434 IQR)
- **Top Global Matches:** file_cluster_8: 14.434, file_cluster_4: 14.438, file_cluster_13: 14.572
- **Magnitude:** 1827.92 | **LOC:** 1552 | **CtrlFlow:** 54.9% | **Authorship Centralization:** 0.0%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 37
- **Risk Profile:** Cognitive Load (40.0245%), Tech Debt (7.892%)
**Top Internal Functions/Classes:**
  * `build` (Impact: 157.2 | O(2^N) | DB: 37)
  * `getOrCreateIndex` (Impact: 103.3 | O(2^N) | DB: 21)
  * `RebuildIndexes` (Impact: 72.6 | O(2^N) | DB: 17)
  * `GetStats` (Impact: 60.1 | O(2^N) | DB: 23)
  * `rebuildIndex` (Impact: 58.7 | O(2^N) | DB: 12)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 2):` fmt, v1, context, title, indexBuildTime, indexUpdateTime, go-cache, semver...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Algorithmic:** O(N) | **DB Complexity:** 39
- **Risk Profile:** Cognitive Load (51.8844%), Tech Debt (15.7786%)
**Top Internal Functions/Classes:**
  * `convertDashboardSpec_V1_to_V2alpha1` (Impact: 39.4 | O(N^1) | DB: 39)
  * `ConvertDashboard_V1_to_V2alpha1` (Impact: 23.8 | O(N^1) | DB: 11)
  * `getRepeatOptionsFromPanel` (Impact: 15.4 | O(N^1) | DB: 10)
  * `getRepeatOptionsFromLibraryPanel` (Impact: 12.9 | O(N^1) | DB: 3)
  * `transformCursorSyncToEnum` (Impact: 12.6 | O(N^1))
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 3):` fmt, v1, context, timeCompare, title, adhoc, gridPos, alphabeticalAsc...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Top Global Matches:** file_cluster_8: 14.94, file_cluster_4: 15.042, file_cluster_0: 15.084
- **Magnitude:** 1682.56 | **LOC:** 2337 | **CtrlFlow:** 56.1% | **Authorship Centralization:** 66.7%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 225
- **Risk Profile:** Cognitive Load (70.7358%), Tech Debt (10.7125%)
**Top Internal Functions/Classes:**
  * `cleanupK8sDashboardResources` (Impact: 824.7 | O(2^N) | DB: 225)
    * *Intent:* // cleanupK8sDashboardResources cleans up resources marked for deletion in the k8s API. // It proces...
  * `HasDashboardAccess` (Impact: 26.0 | O(N^1) | DB: 6)
    * *Intent:* // CanViewDashboard uses the access control service to check if the requested user can see a dashboa...
  * `executeCleanupWithLock` (Impact: 5.6 | O(N^1) | DB: 1)
  * `startK8sDeletedDashboardsCleanupJob` (Impact: 2.5 | O(N^1))
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 207`, `structural_boundaries: 162`, `args: 46`, `func_start: 46`, `class_start: 2`
* *Risk/State:* `safety_bypasses: 3`, `state_mutation: 663`, `dead_code: 1`, `planned_debt: 1`, `orphaned_logic: 2`
* *Architecture:* `api: 115`, `concurrency: 26`, `import: 1`
* *Defense:* `safety: 107`, `doc: 24`, `immutability_locks: 1`, `cleanup: 1`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 8):` fmt, context, dashboard, otel, 0, UID, builders, dashboardUid...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `pkg/services/ngalert/models/testing.go` (GO | Tier 2 | 🚨 AI THREAT: 99.16%)
- **Global Archetype:** `file_cluster_15` (Drift: 13.551 IQR)
- **Top Global Matches:** file_cluster_15: 13.551, file_cluster_8: 13.555, file_cluster_7: 13.761
- **Magnitude:** 1664.7 | **LOC:** 1651 | **CtrlFlow:** 36.9% | **Authorship Centralization:** 100.0%
- **Algorithmic:** O(N^6) | **DB Complexity:** 232
- **Risk Profile:** Cognitive Load (41.5103%), Tech Debt (8.5844%)
**Top Internal Functions/Classes:**
  * `Generate` (Impact: 860.7 | O(N^6) | DB: 232)
  * `With` (Impact: 2.7 | O(N^1) | DB: 1)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 101`, `structural_boundaries: 173`, `args: 106`, `func_start: 106`, `class_start: 7`
* *Risk/State:* `safety_bypasses: 4`, `state_mutation: 534`, `planned_debt: 3`
* *Architecture:* `api: 245`, `import: 1`
* *Defense:* `safety: 1`, `doc: 33`, `test: 2`, `sync_locks: 1`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 4):` fmt, uid, operator, instant, json, rand, strfmt, expression...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `pkg/tests/apis/provisioning/common/testing.go` (GO | Tier 2 | 🚨 AI THREAT: 99.24%)
- **Global Archetype:** `file_cluster_8` (Drift: 14.025 IQR)
- **Top Global Matches:** file_cluster_8: 14.025, file_cluster_7: 14.192, file_cluster_15: 14.222
- **Magnitude:** 1630.84 | **LOC:** 2798 | **CtrlFlow:** 54.7% | **Authorship Centralization:** 100.0%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 21
- **Risk Profile:** Cognitive Load (46.4886%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `deleteAndWait` (Impact: 90.7 | O(2^N) | DB: 13)
  * `DebugState` (Impact: 39.1 | O(N^1) | DB: 12)
  * `CreateRepo` (Impact: 39.1 | O(N^1) | DB: 21)
  * `SnapshotDashboardsBySourcePath` (Impact: 28.3 | O(N^1) | DB: 10)
  * `RequireRepoDashboardParent` (Impact: 27.6 | O(N^1) | DB: 3)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 10):` gittest, secure, title, rest, jobs, bytes, jobs, metadata...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` fmt, commercial, plugins, json, private, community, app, page...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `pkg/services/ngalert/store/alert_rule.go` (GO | Tier 2 | 🚨 AI THREAT: 99.31%)
- **Global Archetype:** `file_cluster_8` (Drift: 14.675 IQR)
- **Top Global Matches:** file_cluster_8: 14.675, file_cluster_11: 14.786, file_cluster_13: 14.792
- **Magnitude:** 1411.62 | **LOC:** 1874 | **CtrlFlow:** 59.9% | **Authorship Centralization:** 100.0%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 33
- **Risk Profile:** Cognitive Load (41.9201%), Tech Debt (8.869%)
**Top Internal Functions/Classes:**
  * `RenameTimeIntervalInNotificationSettings` (Impact: 57.6 | O(N^1) | DB: 19)
  * `GetAlertRuleVersions` (Impact: 54.2 | O(2^N) | DB: 11)
  * `DeleteAlertRulesByUID` (Impact: 52.4 | O(N^1) | DB: 23)
    * *Intent:* // DeleteAlertRulesByUID is a handler for deleting an alert rule.
  * `InsertAlertRules` (Impact: 46.5 | O(N^1) | DB: 33)
    * *Intent:* // InsertAlertRules is a handler for creating/updating alert rules. // Returns the UID and ID of rul...
  * `RenameReceiverInNotificationSettings` (Impact: 44.8 | O(N^1) | DB: 16)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 218`, `structural_boundaries: 146`, `args: 29`, `func_start: 29`, `class_start: 1`
* *Risk/State:* `safety_bypasses: 3`, `state_mutation: 798`, `dead_code: 1`, `fragile_debt: 1`
* *Architecture:* `api: 72`, `import: 1`
* *Defense:* `safety: 73`, `doc: 53`, `immutability_locks: 1`, `cleanup: 1`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` fmt, context, strings, entity, json, sqlstore, featuremgmt, maps...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `pkg/api/frontendsettings.go` (GO | Tier 2 | 🚨 AI THREAT: 99.35%)
- **Global Archetype:** `file_cluster_8` (Drift: 13.81 IQR)
- **Top Global Matches:** file_cluster_8: 13.81, file_cluster_13: 13.979, file_cluster_0: 14.005
- **Magnitude:** 1405.32 | **LOC:** 874 | **CtrlFlow:** 69.2% | **Authorship Centralization:** 100.0%
- **Algorithmic:** O(2^N) [Recursive] | **DB Complexity:** 49
- **Risk Profile:** Cognitive Load (52.9132%), Tech Debt (8.6096%)
**Top Internal Functions/Classes:**
  * `getFrontendSettings` (Impact: 197.5 | O(2^N) | DB: 49)
    * *Intent:* // getFrontendSettings returns a json object with all the settings needed for front end initialisati...
  * `getFSDataSources` (Impact: 136.5 | O(2^N) | DB: 40)
    * *Intent:* //nolint:gocyclo
  * `pluginSettings` (Impact: 55.2 | O(2^N) | DB: 16)
  * `GetFrontendAssets` (Impact: 51.3 | O(2^N) | DB: 25)
    * *Intent:* // Returns a file that is easy to check for changes // Any changes to the file means we should refre...
  * `availablePlugins` (Impact: 40.5 | O(2^N) | DB: 19)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 146`, `structural_boundaries: 65`, `args: 15`, `func_start: 15`, `class_start: 1`
* *Risk/State:* `safety_bypasses: 8`, `state_mutation: 554`, `dead_code: 1`, `orphaned_logic: 1`
* *Architecture:* `io: 1`, `api: 238`, `import: 1`
* *Defense:* `safety: 19`, `doc: 17`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.043
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 5):` fmt, barchart, context, status-history, strings, grafanads, sha256, stat...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- `public/app/features/dashboard-scene/scene/types/DashboardDropTarget.ts` (TYPESCRIPT) | Magnitude: 1.25 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: structural_boundaries: 10, branch: 6, indent_spaces: 6, safety: 5
- `pkg/registry/apis/provisioning/resources/authorizer.go` (GO) | Magnitude: 136.38 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_tabs: 116, state_mutation: 54, doc: 48, branch: 26
- `pkg/services/pluginsintegration/clientmiddleware/cookies_middleware.go` (GO) | Magnitude: 124.42 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_11`
...
- `pkg/middleware/validate_action_url.go` (GO) | Magnitude: 96.06 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_tabs: 74, state_mutation: 37, encapsulation: 23, structural_boundaries: 19
- `pkg/services/accesscontrol/database/database.go` (GO) | Magnitude: 843.48 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 508, indent_tabs: 314, encapsulation: 91, branch: 66
- `pkg/services/ngalert/tests/fakes/provisioning.go` (GO) | Magnitude: 199.84 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: state_mutation: 81, indent_tabs: 70, structural_boundaries: 27, branch: 18
- `public/app/plugins/panel/heatmap/utils.ts` (TYPESCRIPT) | Magnitude: 149.2 | Delta: **0.008 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_spaces: 691, state_mutation: 277, branch: 249, structural_boundaries: 115
- `packages/grafana-data/src/vector/CircularVector.ts` (TYPESCRIPT) | Magnitude: 30.4 | Delta: **0.011 IQR** | Secondary Pull: `file_cluster_16`
  * Top Architectural Signatures: state_mutation: 181, indent_spaces: 112, branch: 23, structural_boundaries: 23

### Mixed-Responsibility Refactoring Targets for: file_cluster_12
- `pkg/web/binding.go` (GO) | Magnitude: 156.9 | Delta: **0.022 IQR** | Secondary Pull: `file_cluster_13`
...
- `pkg/util/xorm/convert.go` (GO) | Magnitude: 484.16 | Delta: **0.077 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 296, state_mutation: 144, branch: 110, structural_boundaries: 109
- `scripts/check-breaking-changes.sh` (SHELL) | Magnitude: 7.31 | Delta: **0.083 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 37, state_mutation: 35, branch: 30, io: 25
- `scripts/download-catalog-plugins.sh` (SHELL) | Magnitude: 43.96 | Delta: **0.147 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_spaces: 238, branch: 142, state_mutation: 104, reflection_metaprogramming: 89

### Mixed-Responsibility Refactoring Targets for: file_cluster_13
- `public/app/plugins/datasource/influxdb/components/editor/config-v2/UrlAndAuthenticationSection.tsx` (TYPESCRIPT) | Magnitude: 29.07 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_spaces: 234, structural_boundaries: 46, branch: 39, ui_framework: 29
- `public/app/features/alerting/unified/mocks/server/handlers/grafanaRuler.ts` (TYPESCRIPT) | Magnitude: 12.86 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 208, structural_boundaries: 93, branch: 42, args: 39
...
- `public/app/features/dashboard-scene/settings/VersionsEditView.tsx` (TYPESCRIPT) | Magnitude: 25.94 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_spaces: 235, state_mutation: 81, structural_boundaries: 72, branch: 59
- `public/app/features/expressions/ExpressionDatasource.ts` (TYPESCRIPT) | Magnitude: 9.78 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_4`
  * Top Architectural Signatures: indent_spaces: 114, structural_boundaries: 50, concurrency: 32, generics: 12

### Mixed-Responsibility Refactoring Targets for: file_cluster_15
- `pkg/services/ngalert/models/testing.go` (GO) | Magnitude: 1664.7 | Delta: **0.004 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 815, state_mutation: 534, api: 245, structural_boundaries: 173
- `pkg/services/folder/tree.go` (GO) | Magnitude: 202.58 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 108, state_mutation: 102, api: 30, doc: 28
- `pkg/plugins/backendplugin/grpcplugin/log_wrapper.go` (GO) | Magnitude: 144.52 | Delta: **0.008 IQR** | Secondary Pull: `file_cluster_16`
  * Top Architectural Signatures: indent_tabs: 65, state_mutation: 30, encapsulation: 26, api: 24
- `pkg/storage/unified/migrations/registry.go` (GO) | Magnitude: 223.08 | Delta: **0.017 IQR** | Secondary Pull: `file_cluster_13`
...
- `public/app/core/history/RichHistoryStorage.ts` (TYPESCRIPT) | Magnitude: 1.81 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_13`
...
- `pkg/util/xorm/logger.go` (GO) | Magnitude: 128.58 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_15`
  * Top Architectural Signatures: indent_tabs: 54, api: 39, doc: 29, args: 25
- `packages/grafana-ui/src/components/DataSourceSettings/types.ts` (TYPESCRIPT) | Magnitude: 2.12 | Delta: **0.007 IQR** | Secondary Pull: `file_cluster_13`
...
- `public/app/plugins/datasource/cloudwatch/components/shared/LogGroups/LogGroupsField.tsx` (TYPESCRIPT) | Magnitude: 4.96 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 164, branch: 75, structural_boundaries: 63, args: 37
- `packages/grafana-prometheus/src/components/metrics-browser/useMetricsLabelsValues.ts` (TYPESCRIPT) | Magnitude: 20.23 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_2`
  * Top Architectural Signatures: indent_spaces: 174, immutability_locks: 43, structural_boundaries: 38, ui_framework: 32
- `pkg/storage/unified/sql/indexer_seeders/playlists.sql` (SQLITE) | Magnitude: 62.44 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_0`
  * Top Architectural Signatures: indent_spaces: 58, duplicate_logic: 29, state_mutation: 16, args: 14
- `public/app/features/alerting/unified/hooks/useAsync.tsx` (TYPESCRIPT) | Magnitude: 10.98 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_2`
  * Top Architectural Signatures: indent_spaces: 106, structural_boundaries: 55, generics: 46, ui_framework: 43
- `public/app/features/alerting/unified/components/rules/state-history/LogRecordViewer.tsx` (TYPESCRIPT) | Magnitude: 4.47 | Delta: **0.003 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 165, structural_boundaries: 48, ui_framework: 31, args: 27

### Mixed-Responsibility Refactoring Targets for: file_cluster_2
- `public/app/features/alerting/unified/components/saved-searches/SavedSearches.tsx` (TYPESCRIPT) | Magnitude: 16.53 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 367, structural_boundaries: 128, args: 76, branch: 53
- `public/app/plugins/datasource/loki/components/VariableQueryEditor.tsx` (TYPESCRIPT) | Magnitude: 3.42 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 102, structural_boundaries: 44, ui_framework: 21, args: 18
- `packages/grafana-ui/src/components/DateTimePickers/TimeZonePicker/TimeZoneOption.tsx` (TYPESCRIPT) | Magnitude: 7.0 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 116, structural_boundaries: 28, ui_framework: 26, immutability_locks: 14
- `public/app/features/alerting/unified/rule-list/components/ListItem.tsx` (TYPESCRIPT) | Magnitude: 3.11 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 72, structural_boundaries: 19, ui_framework: 16, generics: 12
- `public/app/plugins/datasource/cloudwatch/components/shared/LogGroups/LogGroupsSelector.tsx` (TYPESCRIPT) | Magnitude: 7.13 | Delta: **0.001 IQR** | Secondary Pull: `file_cluster_17`
  * Top Architectural Signatures: indent_spaces: 239, ui_framework: 64, structural_boundaries: 50, args: 46

### Mixed-Responsibility Refactoring Targets for: file_cluster_4
- `packages/grafana-prometheus/src/datasource.ts` (TYPESCRIPT) | Magnitude: 27.71 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 167, state_mutation: 115, structural_boundaries: 72, concurrency: 45
...
- `public/app/features/variables/query/VariableQueryRunner.ts` (TYPESCRIPT) | Magnitude: 19.08 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 151, structural_boundaries: 54, state_mutation: 45, concurrency: 29
...
- `pkg/services/query/expr_sql_schema.go` (GO) | Magnitude: 73.58 | Delta: **0.058 IQR** | Secondary Pull: `file_cluster_11`
...
- `e2e/old-arch/utils/support/scenarioContext.ts` (TYPESCRIPT) | Magnitude: 2.9 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 42, structural_boundaries: 19, args: 9, func_start: 9
- `e2e/utils/support/scenarioContext.ts` (TYPESCRIPT) | Magnitude: 2.9 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 42, structural_boundaries: 19, args: 9, func_start: 9
- `public/app/features/alerting/unified/components/rule-editor/FolderSelectorV2.tsx` (TYPESCRIPT) | Magnitude: 5.64 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 77, structural_boundaries: 15, ui_framework: 14, args: 12
...
- `public/app/features/dashboard/components/PanelEditor/getLibraryPanelOptions.tsx` (TYPESCRIPT) | Magnitude: 1.52 | Delta: **0.0 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 41, structural_boundaries: 13, import: 7, func_start: 6
...
- `public/app/features/dashboard-scene/utils/interactions.ts` -> Churn: **51.33%** | Cog Load: 10.995% | Debt: 74.2446%

### 👤 Key Person Dependencies (High Impact + Siloed Knowledge)
These are massive, load-bearing files written almost entirely by a single developer. They represent severe 'Bus Factor' risk.

- `pkg/storage/unified/testing/storage_backend_sql_compatibility.go` -> **Will Assis** (100.0% isolated ownership) | Magnitude: 1909.72
- `pkg/storage/unified/resource/server.go` -> **Rafael Bortolon Paulovic** (100.0% isolated ownership) | Magnitude: 1892.78
- `apps/dashboard/pkg/migration/conversion/v1_to_v2alpha1.go` -> **Kristina Demeshchik** (100.0% isolated ownership) | Magnitude: 1813.16
- `pkg/services/ngalert/models/testing.go` -> **Matheus Macabu** (100.0% isolated ownership) | Magnitude: 1664.7
- `pkg/tests/apis/provisioning/common/testing.go` -> **Gonzalo Trigueros Manzanas** (100.0% isolated ownership) | Magnitude: 1630.84
...
- `public/app/features/dashboard-scene/scene/DashboardScene.tsx` -> **Severity: 0.325** (Bridge: 0.0033 * Flux: 99.9979%)
...
- `pkg/apimachinery/errutil/log.go` -> **Severity: 1600.347** (Blast Radius: 16.008 * Doc Risk: 99.9717%)
- `public/app/types/unified-alerting-dto.ts` -> **Severity: 469.327** (Blast Radius: 4.713 * Doc Risk: 99.5813%)
- `public/app/types/store.ts` -> **Severity: 461.754** (Blast Radius: 4.621 * Doc Risk: 99.9252%)
- `pkg/registry/apis/iam/noopstorage/rest.go` -> **Severity: 414.732** (Blast Radius: 4.148 * Doc Risk: 99.9836%)
