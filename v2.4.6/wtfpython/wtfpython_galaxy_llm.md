...
| **Timestamp** | `2026-08-03T19:43:23.484344+00:00` |
| **Scan Duration** | `0.2s` |
| **Git Branch** | `master` |
...
| Error & Exception Exposure | 0.0 | 80.0 | 27.4 | 0.0 | 0.0 |
...
| Documentation Exposure | 4.8 | 58.1 | 21.4 | 9.3 | 6.7 |
...
- `parse_example_parts` (@ `irrelevant/notebook_generator.py`) -> Impact: **179.9** | LOC: 99
...
- `convert_to_cells` (@ `irrelevant/notebook_generator.py`) -> Impact: **45.7** | LOC: 66
  * *Intent:* # can be either output or normal code
- `convert_to_notebook` (@ `irrelevant/notebook_generator.py`) -> Impact: **21.6** | LOC: 31
- `inspect_and_sanitize_code_lines` (@ `irrelevant/notebook_generator.py`) -> Impact: **14.5** | LOC: 12
- `is_interactive_statement` (@ `irrelevant/notebook_generator.py`) -> Impact: **10.6** | LOC: 5
- `remove_from_beginning` (@ `irrelevant/notebook_generator.py`) -> Impact: **10.6** | LOC: 5
- `square` (@ `mixed_tabs_and_spaces.py`) -> Impact: **5.4** | LOC: 5
- `generate_code_block` (@ `irrelevant/notebook_generator.py`) -> Impact: **3.2** | LOC: 12
- `generate_markdown_block` (@ `irrelevant/notebook_generator.py`) -> Impact: **3.1** | LOC: 11
...
- `parse_example_parts` (@ `irrelevant/notebook_generator.py`) -> **O(N^6)**
  * *Intent:* """ Generates a code block that executes the given statements. :param statements: The list of statements to execute. :type statements: list(str) """
- `convert_to_cells` (@ `irrelevant/notebook_generator.py`) -> **O(N^6)**
  * *Intent:* # can be either output or normal code
- `convert_to_notebook` (@ `irrelevant/notebook_generator.py`) -> **O(N^3)**
- `inspect_and_sanitize_code_lines` (@ `irrelevant/notebook_generator.py`) -> **O(N^3)**
- `is_interactive_statement` (@ `irrelevant/notebook_generator.py`) -> **O(N^3)**
- `remove_from_beginning` (@ `irrelevant/notebook_generator.py`) -> **O(N^3)**

### Highest Data Gravity (Database Complexity)
- `parse_example_parts` (@ `irrelevant/notebook_generator.py`) -> DB Complexity: **17**
  * *Intent:* """ Generates a code block that executes the given statements. :param statements: The list of statements to execute. :type statements: list(str) """
- `convert_to_cells` (@ `irrelevant/notebook_generator.py`) -> DB Complexity: **4**
  * *Intent:* # can be either output or normal code
- `convert_to_notebook` (@ `irrelevant/notebook_generator.py`) -> DB Complexity: **3**
- `inspect_and_sanitize_code_lines` (@ `irrelevant/notebook_generator.py`) -> DB Complexity: **1**
- `generate_code_block` (@ `irrelevant/notebook_generator.py`) -> DB Complexity: **1**
- `generate_markdown_block` (@ `irrelevant/notebook_generator.py`) -> DB Complexity: **1**

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `irrelevant` | 3 | 393.3 | 38.34% | 8.5% |
| `irrelevant/obsolete` | 3 | 98.74 | 16.63% | 16.21% |
| `__monolith__` | 7 | 98.2 | 1.43% | 14.29% |
...
- `irrelevant/notebook_generator.py` -> **100.0%** Exposure
- `mixed_tabs_and_spaces.py` -> **4.9194%** Exposure
...
### 1. `irrelevant/notebook_generator.py` (PYTHON) -> Cumulative Risk: **666.63**
- **Archetype:** `file_cluster_8` (Distance: 11.547 IQR)
- **Magnitude:** 383.1 | **LOC:** 398 | **CtrlFlow:** 65.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Algorithmic Dos (100.0%), Logic Bomb (100.0%), State Flux (99.9982%)
- **Heaviest Functions:** `parse_example_parts` (Impact: 179.9), `convert_to_cells` (Impact: 45.7), `convert_to_notebook` (Impact: 21.6)

### 2. `irrelevant/insert_ids.py` (PYTHON) -> Cumulative Risk: **418.64**
- **Archetype:** `file_cluster_13` (Distance: 10.513 IQR)
- **Magnitude:** 9.2 | **LOC:** 25 | **CtrlFlow:** 44.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9999%), Cognitive Load (83.2018%), Safety Score (80.0%)
- **Heaviest Functions:** `generate_random_id_comment` (Impact: 1.9)

### 3. `irrelevant/obsolete/generate_contributions.py` (PYTHON) -> Cumulative Risk: **355.84**
- **Archetype:** `file_cluster_13` (Distance: 9.882 IQR)
- **Magnitude:** 21.66 | **LOC:** 54 | **CtrlFlow:** 81.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.3259%), Safety Score (60.303%), Tech Debt (48.6182%)

### 4. `irrelevant/obsolete/parse_readme.py` (PYTHON) -> Cumulative Risk: **297.29**
- **Archetype:** `file_cluster_8` (Distance: 10.513 IQR)
- **Magnitude:** 29.78 | **LOC:** 152 | **CtrlFlow:** 85.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (97.242%), Safety Score (69.3258%), Cognitive Load (16.5035%)

### 5. `snippets/2_tricky_strings.py` (PYTHON) -> Cumulative Risk: **239.08**
- **Archetype:** `file_cluster_8` (Distance: 10.983 IQR)
- **Magnitude:** 15.24 | **LOC:** 20 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (80.0%), Documentation (52.2369%), Cognitive Load (5.0%)
...
### 7. `mixed_tabs_and_spaces.py` (PYTHON) -> Cumulative Risk: **61.65**
- **Archetype:** `file_cluster_8` (Distance: 7.147 IQR)
- **Magnitude:** 6.52 | **LOC:** 8 | **CtrlFlow:** 33.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (40.0%), Api Exposure (5.7813%), Cognitive Load (5.0%), Algorithmic Dos (4.9194%)
- **Heaviest Functions:** `square` (Impact: 5.4)

### 8. `snippets/__init__.py` (PYTHON) -> Cumulative Risk: **18.49**
...
**Top Internal Functions/Classes:**
  * `parse_example_parts` (Impact: 179.9 | O(N^6) | DB: 17)
    * *Intent:* """ Generates a code block that executes the given statements. :param statements: The list of statem...
  * `convert_to_cells` (Impact: 45.7 | O(N^6) | DB: 4)
    * *Intent:* # can be either output or normal code
  * `convert_to_notebook` (Impact: 21.6 | O(N^3) | DB: 3)
  * `inspect_and_sanitize_code_lines` (Impact: 14.5 | O(N^3) | DB: 1)
  * `is_interactive_statement` (Impact: 10.6 | O(N^3))
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` pprint, json, os, sys
...
**Top Internal Functions/Classes:**
  * `generate_random_id_comment` (Impact: 1.9 | O(N^1))
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 45.455
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` typing, nox.sessions, nox
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- `irrelevant/notebook_generator.py` (PYTHON) | Magnitude: 383.1 | Delta: **0.187 IQR** | Secondary Pull: `file_cluster_13`
...
- `irrelevant/notebook_generator.py` -> **Severity: 2640.367** (Blast Radius: 45.455 * Doc Risk: 58.0875%)
- `snippets/2_tricky_strings.py` -> **Severity: 2374.428** (Blast Radius: 45.455 * Doc Risk: 52.2369%)
- `irrelevant/insert_ids.py` -> **Severity: 2218.254** (Blast Radius: 45.455 * Doc Risk: 48.8011%)
