...
> **Composition Archetype:** `Flat Modular Platform` (z +1.92; from the repo's file-archetype mix)
> **File Composition:** Annotated Framework Methods Files 35%, Large Core Modules 17%, Data / Markup / Trivial 14%, Declarative / Non-Code 9%, Tests & Verification Files 6%
...
- **Composition Archetype:** `State Mutators Files` (z +0.37)
...
- **Heaviest Functions:** `evaluate_expression` (Many-Argument Workhorses, Impact: 22.1), `unwind` (Compute Cores, Impact: 14.0), `from_context` (Compute Cores, Impact: 10.2)
...
- **Heaviest Functions:** `deref_unique_unroll` (Callbacks & Closures, Impact: 5.1), `deref_two` (Callbacks & Closures, Impact: 5.0), `drain_write_drain` (Callbacks & Closures, Impact: 5.0)
...
- **Heaviest Functions:** `get_or_init` (Generic / Templated Code, Impact: 7.4), `get_or_init` (Generic / Templated Code, Impact: 7.4), `get_or_init` (Generic / Templated Code, Impact: 7.4)
...
- **Heaviest Functions:** `try_insert` (Many-Argument Workhorses, Impact: 6.9), `try_push` (Compute Cores, Impact: 5.7), `pop` (Generic / Templated Code, Impact: 4.7)
...
- **Heaviest Functions:** `truncate` (Compute Cores, Impact: 9.4), `slice` (Compute Cores, Impact: 9.0), `shallow_clone_vec` (Many-Argument Workhorses, Impact: 8.4)
...
- **Heaviest Functions:** `yyjson_mut_doc_ptr_setx` (Many-Argument Workhorses, Impact: 42.0), `yyjson_mut_doc_ptr_addx` (Many-Argument Workhorses, Impact: 41.9), `yyjson_ptr_ctx_append` (Many-Argument Workhorses, Impact: 36.6)
...
- **Heaviest Functions:** `atomic_compare_exchange` (Many-Argument Workhorses, Impact: 38.3), `atomic_load` (Compute Cores, Impact: 16.2), `atomic_compare_exchange_zacas` (Many-Argument Workhorses, Impact: 3.4)

### 8. `orjson-3.11.8/include/cargo/bytecount-0.6.9/src/simd/generic.rs` (RUST) -> Cumulative Risk: **579.16**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.35)
...
- **Heaviest Functions:** `chunk_count` (Compute Cores, Impact: 14.7), `chunk_num_chars` (Compute Cores, Impact: 12.2), `u8x64_from_offset` (State Mutators, Impact: 1.9)

### 9. `orjson-3.11.8/include/cargo/once_cell-1.21.4/src/imp_std.rs` (RUST) -> Cumulative Risk: **570.02**
...
- **Heaviest Functions:** `wait` (Compute Cores, Impact: 13.6), `initialize_or_wait` (Compute Cores, Impact: 12.0), `stampede_once` (Tests & Verification, Impact: 8.8)

### 10. `orjson-3.11.8/include/cargo/unwinding-0.2.8/src/unwinder/mod.rs` (RUST) -> Cumulative Risk: **568.67**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.40)
- **Magnitude:** 172.98 | **LOC:** 431 | **CtrlFlow:** 9.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (84.8339%), Verification (80.0%)
- **Heaviest Functions:** `force_unwind_phase2` (Many-Argument Workhorses, Impact: 23.5), `raise_exception_phase2` (Many-Argument Workhorses, Impact: 18.3), `_Unwind_Backtrace` (Compute Cores, Impact: 15.9)
...
**Top Internal Functions/Classes:**
  ...
  * `value` **(Compute Cores)** (Impact: 42.2)
...
**Top Internal Functions/Classes:**
  * `quickcheck_fetch_max` **(Tests & Verification)** (Impact: 20.6)
  * `quickcheck_fetch_min` **(Tests & Verification)** (Impact: 20.6)
  * `stress_compare_exchange` **(Tests & Verification)** (Impact: 17.7)
  * `fetch_max` **(Tests & Verification)** (Impact: 17.4)
  * `fetch_min` **(Tests & Verification)** (Impact: 17.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parse` **(Compute Cores)** (Impact: 57.5)
...
**Top Internal Functions/Classes:**
  * `test_aarch64_registers` **(Tests & Verification)** (Impact: 3.6)
  * `test_power64_registers` **(Tests & Verification)** (Impact: 3.6)
...
**Top Internal Functions/Classes:**
  ...
  * `write` **(Compute Cores)** (Impact: 62.6)
...
**Top Internal Functions/Classes:**
  ...
  * `write` **(Many-Argument Workhorses)** (Impact: 13.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `has_msbuild_version` **(State Mutators)** (Impact: 18.9)
...
**Top Internal Functions/Classes:**
  ...
  * `alloc_impl` **(Annotated Framework Methods)** (Impact: 9.2)
  * `downgrade` **(Tests & Verification)** (Impact: 8.9)
    * *Intent:* /// Creates a new [`Weak`] pointer to this allocation. /// /// # Examples /// /// ``` /// use portab...
  * `upgrade` **(Annotated Framework Methods)** (Impact: 8.5)
...
**Top Internal Functions/Classes:**
  ...
  * `test_original_capacity_to_repr` **(Tests & Verification)** (Impact: 11.7)
  * `advance_unchecked` **(Compute Cores)** (Impact: 10.5)
...
**Top Internal Functions/Classes:**
  ...
  * `into_str` **(Compute Cores)** (Impact: 8.6)
...
**Top Internal Functions/Classes:**
  ...
  * `convert_raw` **(Compute Cores)** (Impact: 19.3)
...
**Top Internal Functions/Classes:**
  ...
  * `parse` **(Compute Cores)** (Impact: 10.8)
...
**Top Internal Functions/Classes:**
  * `PyObject_TypeCheck` **(Annotated Framework Methods)** (Impact: 3.7)
  * `PyType_FromMetaclass` **(Annotated Framework Methods)** (Impact: 2.6)
  * `PyType_FromModuleAndSpec` **(Annotated Framework Methods)** (Impact: 2.4)
  * `PyObject_GetOptionalAttr` **(Annotated Framework Methods)** (Impact: 2.4)
  * `PyObject_GetOptionalAttrString` **(Annotated Framework Methods)** (Impact: 2.4)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `stress` **(Tests & Verification)** (Impact: 8.9)
  * `advance_bytes_mut_remaining_capacity` **(Tests & Verification)** (Impact: 7.7)
  ...
  * `split_off_to_loop` **(Tests & Verification)** (Impact: 4.2)
  * `split_off_to_at_gt_len` **(Tests & Verification)** (Impact: 4.2)
**Contextual Mitigations & Amplifications:**
