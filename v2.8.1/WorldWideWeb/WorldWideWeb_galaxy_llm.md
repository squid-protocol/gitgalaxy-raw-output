...
> **Composition Archetype:** `Small Flat Repo` (z -0.54; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 32%, Large Core Modules 22%, Interface Declarations Files 11%, State Mutators Files 11%, Declarative / Non-Code 8%
...
- `readSGML` **(Compute Cores)** (@ `ParseHTML.h`) -> Impact: **260.8** | LOC: 556
  * *Intent:* #else
...
- `read_article` **(Compute Cores)** (@ `NewsAccess.m`) -> Impact: **43.0** | LOC: 119
...
- **Heaviest Functions:** `loadAnchor` (Compute Cores, Impact: 32.9), `searchDiagnostic` (Compute Cores, Impact: 13.8), `closeOthers` (Compute Cores, Impact: 11.1)
...
- **Heaviest Functions:** `Unknown_Block` (Many-Argument Workhorses, Impact: 60.2), `Unknown_Block` (Many-Argument Workhorses, Impact: 45.0), `Unknown_Block` (Many-Argument Workhorses, Impact: 40.1)
...
- **Heaviest Functions:** `applyStyle` (Many-Argument Workhorses, Impact: 64.8), `adjustWindow` (I/O & Config Routines, Impact: 19.5), `willChange` (Compute Cores, Impact: 18.2)
...
- **Heaviest Functions:** `loadAnchor` (Many-Argument Workhorses, Impact: 48.8), `ask_name` (Compute Cores, Impact: 29.5), `saveNode` (Compute Cores, Impact: 22.2)
...
- **Heaviest Functions:** `saveAs` (Compute Cores, Impact: 16.2), `loadDefaultStyleSheet` (I/O & Config Routines, Impact: 13.8), `open` (Compute Cores, Impact: 12.9)
...
- **Heaviest Functions:** `selectDiagnostic` (Compute Cores, Impact: 15.2), `newParent` (Type Conversions, Impact: 9.8), `moveBy` (Type Conversions, Impact: 9.3)

### 7. `HTStyle.m` (OBJECTIVE-C) -> Cumulative Risk: **598.86**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.38)
...
- **Heaviest Functions:** `HTStyleForRun` (Compute Cores, Impact: 25.7), `HTStyleRead` (Compute Cores, Impact: 18.0), `HTStyleSheetRead` (Type Conversions, Impact: 9.7)
...
- **Heaviest Functions:** `read_group` (Many-Argument Workhorses, Impact: 128.6), `accessName` (Many-Argument Workhorses, Impact: 65.2), `read_article` (Compute Cores, Impact: 43.0)
...
- **Heaviest Functions:** `readSGML` (Compute Cores, Impact: 260.8), `change_run` (Many-Argument Workhorses, Impact: 77.2), `parse_example` (Compute Cores, Impact: 21.0)
...
- **Heaviest Functions:** `writePrintInfo` (I/O & Config Routines, Impact: 4.1), `pickedUnits` (I/O & Config Routines, Impact: 2.2), `readPrintInfo` (I/O & Config Routines, Impact: 2.0)
...
**Top Internal Functions/Classes:**
  * `readSGML` **(Compute Cores)** (Impact: 260.8)
    * *Intent:* #else
  ...
  * `findSGML` **(Compute Cores)** (Impact: 10.8)
...
**Top Internal Functions/Classes:**
  ...
  * `accessName` **(Many-Argument Workhorses)** (Impact: 65.2)
    * *Intent:* // Open by name -accessName:anchor:diagnostic: // ------------
  * `read_article` **(Compute Cores)** (Impact: 43.0)
...
**Top Internal Functions/Classes:**
  ...
  * `HTStyleRead` **(Compute Cores)** (Impact: 18.0)
...
**Top Internal Functions/Classes:**
  ...
  * `open` **(Compute Cores)** (Impact: 12.9)
...
**Top Internal Functions/Classes:**
  * `loadAnchor` **(Compute Cores)** (Impact: 32.9)
...
**Top Internal Functions/Classes:**
  * `selectDiagnostic` **(Compute Cores)** (Impact: 15.2)
  ...
  * `free` **(I/O & Config Routines)** (Impact: 5.5)
...
**Top Internal Functions/Classes:**
  ...
  * `setTopBotForm` **(Interface Declarations)** (Impact: 1.8)
    * *Intent:* /* NIB outlet setting methods */
  * `setSideForm` **(Interface Declarations)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `registerAccess` **(Type Conversions)** (Impact: 3.1)
    * *Intent:* // Access Management functions // ===========================
  * `Do_getStartLength` **(Interface Declarations)** (Impact: 2.0)
    * *Intent:* /* Action Methods ** ============== */ #ifdef JUNK
  * `Do_getSel` **(Interface Declarations)** (Impact: 1.8)
    * *Intent:* /* Method: Get the extent of the selection */
  * `setSearchWindow` **(Interface Declarations)** (Impact: 1.7)
...
**Top Internal Functions/Classes:**
  ...
  * `name` **(I/O & Config Routines)** (Impact: 1.2)
...
**Top Internal Functions/Classes:**
  * `main` **(State Mutators)** (Impact: 7.3)
