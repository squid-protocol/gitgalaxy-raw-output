...
> **Composition Archetype:** `Typed Library` (z +1.28; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 48%, Interface Declarations Files 13%, Data / Markup / Trivial 10%, Defensive Guards Files 10%, Declarative / Non-Code 5%
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.94)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 82.0), `on_event` (Defensive Guards, Impact: 54.7), `_process_messages` (Many-Argument Workhorses, Impact: 52.6)
...
- **Heaviest Functions:** `_get_box_model` (Many-Argument Workhorses, Impact: 114.7), `scroll_to_region` (Many-Argument Workhorses, Impact: 108.5), `_scroll_to` (Many-Argument Workhorses, Impact: 74.5)
...
- **Heaviest Functions:** `_forward_event` (Many-Argument Workhorses, Impact: 73.6), `_refresh_layout` (Many-Argument Workhorses, Impact: 46.0), `_move_focus` (Many-Argument Workhorses, Impact: 39.4)
...
- **Heaviest Functions:** `run` (Defensive Guards, Impact: 4.7), `check_sleeps` (Interface Declarations, Impact: 4.0), `profile_sleep` (Generic / Templated Code, Impact: 3.2)
...
- **Heaviest Functions:** `take_svg_screenshot` (Many-Argument Workhorses, Impact: 45.8), `format_svg` (Many-Argument Workhorses, Impact: 13.4), `auto_pilot` (Compute Cores, Impact: 8.0)
...
- **Heaviest Functions:** `action_move` (Defensive Guards, Impact: 11.3), `__init__` (Many-Argument Workhorses, Impact: 8.0), `on_tile_clicked` (Defensive Guards, Impact: 7.3)

### 7. `src/textual/widgets/_list_view.py` (PYTHON) -> Cumulative Risk: **643.93**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.20)
...
- **Heaviest Functions:** `remove_items` (Compute Cores, Impact: 20.8), `watch_index` (Defensive Guards, Impact: 19.4), `pop` (Compute Cores, Impact: 19.0)
...
- **Heaviest Functions:** `render_bar` (Many-Argument Workhorses, Impact: 67.9), `__rich_console__` (Many-Argument Workhorses, Impact: 23.4), `_on_mouse_move` (Compute Cores, Impact: 9.7)

### 9. `src/textual/dom.py` (PYTHON) -> Cumulative Risk: **637.24**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.96)
...
- **Heaviest Functions:** `query_one` (Many-Argument Workhorses, Impact: 45.6), `query_exactly_one` (Many-Argument Workhorses, Impact: 39.3), `query_ancestor` (Defensive Guards, Impact: 21.9)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 24.4), `__init__` (Generic / Templated Code, Impact: 10.4), `__init__` (Many-Argument Workhorses, Impact: 8.1)
...
**Top Internal Functions/Classes:**
  ...
  * `on_event` **(Defensive Guards)** (Impact: 54.7)
  ...
  * `_display` **(Many-Argument Workhorses)** (Impact: 35.5)
...
**Top Internal Functions/Classes:**
  * `test_click_selection_disabled_when_allow_select_is_false` **(C Struct Operations)** (Impact: 12.7)
  ...
  * `test_panel_border_title_colors` **(Interface Declarations)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  * `test_sort_by_function_sum` **(Interface Declarations)** (Impact: 6.8)
  ...
  * `test_move_cursor_respects_animate_parameter` **(Interface Declarations)** (Impact: 3.8)
  ...
  * `on_mount` **(Interface Declarations)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_set_document` **(Many-Argument Workhorses)** (Impact: 39.2)
  ...
  * `_redo_batch` **(Compute Cores)** (Impact: 21.5)
...
**Top Internal Functions/Classes:**
  ...
  * `to_strip` **(Compute Cores)** (Impact: 34.6)
  * `join` **(Compute Cores)** (Impact: 32.4)
...
**Top Internal Functions/Classes:**
  * `_forward_event` **(Many-Argument Workhorses)** (Impact: 73.6)
  ...
  * `_handle_mouse_move` **(Defensive Guards)** (Impact: 28.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_parse_border` **(Defensive Guards)** (Impact: 21.9)
...
**Top Internal Functions/Classes:**
  ...
  * `watch_cursor_line` **(Many-Argument Workhorses)** (Impact: 19.4)
  * `_build` **(Compute Cores)** (Impact: 19.3)
...
**Top Internal Functions/Classes:**
  * `test_compose_order` **(Interface Declarations)** (Impact: 7.1)
  * `test_sort_children_no_key` **(Interface Declarations)** (Impact: 5.8)
    * *Intent:* """Test sorting with no key."""
  * `test_sort_children` **(Interface Declarations)** (Impact: 5.7)
    * *Intent:* """Test the sort_children method."""
  * `test_of_type` **(Defensive Guards)** (Impact: 4.2)
  * `compose` **(Generic / Templated Code)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `render_line` **(Compute Cores)** (Impact: 27.4)
  ...
  * `_watch_value` **(Compute Cores)** (Impact: 15.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_on_message` **(Compute Cores)** (Impact: 18.8)
    * *Intent:* """Called to process a message. Args: message: A Message object. """
  * `_process_messages_loop` **(Defensive Guards)** (Impact: 18.6)
    * *Intent:* """Process messages until the queue is closed."""
  * `_close_messages` **(Defensive Guards)** (Impact: 16.6)
...
**Top Internal Functions/Classes:**
  ...
  * `line_post` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_worker_cancellation` **(Interface Declarations)** (Impact: 3.9)
