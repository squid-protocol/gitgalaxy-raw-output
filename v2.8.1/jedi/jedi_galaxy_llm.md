...
> **Composition Archetype:** `Hub-Coupled App` (z -0.49; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 37%, Large Core Modules 19%, Interface Declarations Files 14%, Defensive Guards Files 13%, Declarative / Non-Code 6%
...
- `process_params` **(Compute Cores)** (@ `jedi/inference/star_args.py`) -> Impact: **83.5** | LOC: 112
...
- `_check_for_exception_catch` **(Defensive Guards)** (@ `jedi/inference/analysis.py`) -> Impact: **69.2** | LOC: 88
...
- **Heaviest Functions:** `py__getattribute__` (Many-Argument Workhorses, Impact: 54.0), `create_context` (Compute Cores, Impact: 29.6), `_check_for_additional_knowledge` (Many-Argument Workhorses, Impact: 21.1)
...
- **Heaviest Functions:** `get_yield_lazy_values` (Compute Cores, Impact: 33.4), `_find_overload_functions` (Compute Cores, Impact: 28.0), `get_return_values` (Compute Cores, Impact: 24.5)
...
- **Heaviest Functions:** `goto` (Compute Cores, Impact: 45.9), `get_kind` (Compute Cores, Impact: 18.3), `assignment_indexes` (Compute Cores, Impact: 14.7)
...
- **Heaviest Functions:** `infer_call_of_leaf` (Many-Argument Workhorses, Impact: 31.2), `parse_dotted_names` (Compute Cores, Impact: 27.4), `get_names_of_node` (Defensive Guards, Impact: 7.6)
...
- **Heaviest Functions:** `process_params` (Compute Cores, Impact: 83.5), `_remove_given_params` (Compute Cores, Impact: 16.4), `_iter_nodes_for_param` (Compute Cores, Impact: 12.9)
...
- **Heaviest Functions:** `infer_type_vars` (Compute Cores, Impact: 21.0), `is_same_class` (Compute Cores, Impact: 13.6), `define_generics` (Compute Cores, Impact: 13.4)
...
- **Heaviest Functions:** `_infer_comparison_part` (Many-Argument Workhorses, Impact: 112.3), `tree_name_to_values` (Many-Argument Workhorses, Impact: 65.0), `infer_atom` (Defensive Guards, Impact: 52.6)
...
- **Heaviest Functions:** `wrapper` (Compute Cores, Impact: 33.2), `tree_name_to_values` (Compute Cores, Impact: 23.9), `_infer_field` (Compute Cores, Impact: 19.3)
...
- **Heaviest Functions:** `_get` (Many-Argument Workhorses, Impact: 28.2), `_parse_function_doc` (Compute Cores, Impact: 22.7), `execute_annotation` (Compute Cores, Impact: 10.9)
...
- **Heaviest Functions:** `walk` (Compute Cores, Impact: 10.8), `__init__` (Encapsulated Accessors, Impact: 2.4), `__init__` (Parameter Forwarders, Impact: 1.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_complete_in_string` **(Compute Cores)** (Impact: 31.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_get_code_for_stack` **(Compute Cores)** (Impact: 27.9)
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 28.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_assert_interpreter_complete` **(Defensive Guards)** (Impact: 10.2)
  * `test_custom__getitem__` **(C Struct Operations)** (Impact: 9.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `definition` **(Many-Argument Workhorses)** (Impact: 17.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_nested` **(Many-Argument Workhorses)** (Impact: 11.4)
...
**Top Internal Functions/Classes:**
  * `_get` **(Many-Argument Workhorses)** (Impact: 28.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_remove_unwanted_expression_nodes` **(Many-Argument Workhorses)** (Impact: 27.7)
  ...
  * `_find_inputs_and_outputs` **(Compute Cores)** (Impact: 16.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_get_dataclass_transform_signatures` **(Compute Cores)** (Impact: 22.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_check_for_additional_knowledge` **(Many-Argument Workhorses)** (Impact: 21.1)
  * `_get_global_filters_for_name` **(Compute Cores)** (Impact: 16.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_get_signatures` **(Defensive Guards)** (Impact: 14.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 52 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_find_overload_functions` **(Compute Cores)** (Impact: 28.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_getitem` **(Defensive Guards)** (Impact: 11.2)
  ...
  * `_get_value_filters` **(Defensive Guards)** (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_find_type_from_comment_hint` **(Many-Argument Workhorses)** (Impact: 28.0)
  * `_infer_param` **(Compute Cores)** (Impact: 26.6)
...
**Top Internal Functions/Classes:**
  ...
  * `get_default_project` **(Compute Cores)** (Impact: 22.3)
  ...
  * `_try_to_skip_duplicates` **(Compute Cores)** (Impact: 10.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 56 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_find_python_files_in_sys_path` **(Compute Cores)** (Impact: 18.3)
  * `search_in_file_ios` **(Many-Argument Workhorses)** (Impact: 18.1)
  * `gitignored_paths` **(Compute Cores)** (Impact: 14.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `get_executable_nodes` **(Compute Cores)** (Impact: 27.7)
...
**Top Internal Functions/Classes:**
  ...
  * `_load_from_typeshed` **(Many-Argument Workhorses)** (Impact: 21.5)
  * `import_module_decorator` **(Compute Cores)** (Impact: 18.7)
  * `_get_typeshed_directories` **(Compute Cores)** (Impact: 17.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 46 instances
