...
> **Composition Archetype:** `Typed Library` (z +0.67; from the repo's file-archetype mix)
> **File Composition:** Defensive Guards Files 30%, Generic / Templated Code Files 30%, Data / Markup / Trivial 22%, Large Core Modules 13%, Declarative / Non-Code 2%
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.23)
...
- **Heaviest Functions:** `do_filesizeformat` (Compute Cores, Impact: 43.1), `do_urlize` (Many-Argument Workhorses, Impact: 35.0), `do_indent` (Many-Argument Workhorses, Impact: 26.9)

### 2. `jinja2-3.1.6/src/jinja2/environment.py` (PYTHON) -> Cumulative Risk: **707.64**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.81)
...
- **Heaviest Functions:** `_filter_test_common` (Many-Argument Workhorses, Impact: 50.6), `overlay` (Many-Argument Workhorses, Impact: 42.7), `compile_templates` (Many-Argument Workhorses, Impact: 32.3)
...
- **Heaviest Functions:** `__call__` (Many-Argument Workhorses, Impact: 37.8), `new_context` (Many-Argument Workhorses, Impact: 26.8), `call` (Many-Argument Workhorses, Impact: 22.4)
...
- **Heaviest Functions:** `async_variant` (Type Conversions, Impact: 10.6), `decorator` (Type Conversions, Impact: 10.4), `wrapper` (Compute Cores, Impact: 5.7)

### 5. `jinja2-3.1.6/src/jinja2/compiler.py` (PYTHON) -> Cumulative Risk: **678.18**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.61)
...
- **Heaviest Functions:** `visit_For` (Many-Argument Workhorses, Impact: 90.9), `visit_Template` (Many-Argument Workhorses, Impact: 56.0), `visit_Output` (Many-Argument Workhorses, Impact: 53.9)

### 6. `jinja2-3.1.6/src/jinja2/parser.py` (PYTHON) -> Cumulative Risk: **645.46**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.66)
...
- **Heaviest Functions:** `_fail_ut_eof` (Many-Argument Workhorses, Impact: 33.3), `parse_tuple` (Many-Argument Workhorses, Impact: 32.5), `subparse` (Defensive Guards, Impact: 26.6)
...
- **Heaviest Functions:** `visit_For` (Many-Argument Workhorses, Impact: 23.3), `visit_Name` (Generic / Templated Code, Impact: 11.7), `branch_update` (Defensive Guards, Impact: 11.5)
...
- **Heaviest Functions:** `parse` (Many-Argument Workhorses, Impact: 58.0), `_make_node` (Many-Argument Workhorses, Impact: 42.0), `_parse_block` (Many-Argument Workhorses, Impact: 28.3)
...
- **Heaviest Functions:** `native_concat` (Defensive Guards, Impact: 11.5), `_output_child_to_const` (Defensive Guards, Impact: 7.3), `render_async` (Defensive Guards, Impact: 6.7)
...
- **Heaviest Functions:** `urlize` (Many-Argument Workhorses, Impact: 108.9), `generate_lorem_ipsum` (Many-Argument Workhorses, Impact: 29.5), `select_autoescape` (Many-Argument Workhorses, Impact: 16.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_load_template` **(Many-Argument Workhorses)** (Impact: 19.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 14.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_install_callables` **(Many-Argument Workhorses)** (Impact: 14.4)
  * `_install_null` **(Defensive Guards)** (Impact: 13.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `compile_rules` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  ...
  * `split_template_path` **(Compute Cores)** (Impact: 12.1)
...
**Top Internal Functions/Classes:**
  ...
  * `test_custom_code_generator` **(C Struct Operations)** (Impact: 5.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `find_spec` **(Many-Argument Workhorses)** (Impact: 5.0)
  * `test_pep_451_import_hook` **(Interface Declarations)** (Impact: 3.5)
  * `test_error_includes_paths` **(Defensive Guards)** (Impact: 2.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_get_default_cache_dir` **(Defensive Guards)** (Impact: 17.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_duplicate_required_or_scoped` **(Compute Cores)** (Impact: 9.7)
  * `test_required_with_scope` **(Defensive Guards)** (Impact: 6.3)
...
**Top Internal Functions/Classes:**
  ...
  * `test_filters_deterministic` **(Callbacks & Closures)** (Impact: 6.2)
...
**Top Internal Functions/Classes:**
  * `test_restricted` **(Tests & Verification)** (Impact: 5.8)
