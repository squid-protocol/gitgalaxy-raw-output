...
> **File Composition:** Large Core Modules 30%, Interface Declarations Files 17%, Data / Markup / Trivial 15%, Parameter Forwarders Files 12%, Declarative / Non-Code 9%
...
- `_pre_render` **(Compute Cores)** (@ `kivy/core/text/markup.py`) -> Impact: **148.3** | LOC: 307
...
- `log_cgl_funcs` **(Compute Cores)** (@ `kivy/graphics/cgl.pyx`) -> Impact: **142.9** | LOC: 259
...
- **Heaviest Functions:** `mainloop` (Compute Cores, Impact: 101.9), `create_window` (Many-Argument Workhorses, Impact: 42.3), `_update_modifiers` (Compute Cores, Impact: 35.8)

### 2. `kivy/input/providers/linuxwacom.py` (PYTHON) -> Cumulative Risk: **721.81**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.29)
...
- **Heaviest Functions:** `_thread_run` (Many-Argument Workhorses, Impact: 85.0), `__init__` (Many-Argument Workhorses, Impact: 20.6), `process` (Defensive Guards, Impact: 14.1)
...
- **Heaviest Functions:** `_next_frame_run` (Many-Argument Workhorses, Impact: 44.9), `_redraw` (Compute Cores, Impact: 25.2), `play` (Compute Cores, Impact: 9.9)
...
- **Heaviest Functions:** `async_idle` (Compute Cores, Impact: 19.2), `idle` (Compute Cores, Impact: 16.0), `_check_ready` (Many-Argument Workhorses, Impact: 12.9)
...
- **Heaviest Functions:** `_thread_run` (Many-Argument Workhorses, Impact: 69.9), `__init__` (Many-Argument Workhorses, Impact: 22.6), `depack` (Compute Cores, Impact: 14.8)
...
- **Heaviest Functions:** `callback_docstring` (Many-Argument Workhorses, Impact: 63.2), `is_cython_extension` (Compute Cores, Impact: 22.3), `callback_signature` (Defensive Guards, Impact: 6.4)
...
- **Heaviest Functions:** `_player_callback` (Compute Cores, Impact: 8.5), `load` (Compute Cores, Impact: 6.5), `_do_eos` (Encapsulated Accessors, Impact: 5.4)
...
- **Heaviest Functions:** `show_property` (Many-Argument Workhorses, Impact: 94.5), `keyboard_shortcut` (Many-Argument Workhorses, Impact: 46.9), `on_widget` (Many-Argument Workhorses, Impact: 17.4)
...
- **Heaviest Functions:** `_render_layout` (Many-Argument Workhorses, Impact: 78.0), `_set_cc_options` (Many-Argument Workhorses, Impact: 46.8), `_ft2_scan_fontfile_to_fontfamily_cache` (Compute Cores, Impact: 30.0)
...
- **Heaviest Functions:** `load` (Compute Cores, Impact: 8.2), `_do_eos` (Compute Cores, Impact: 6.2), `play` (Compute Cores, Impact: 6.0)
...
**Top Internal Functions/Classes:**
  ...
  * `keyboard_on_key_down` **(Many-Argument Workhorses)** (Impact: 64.9)
...
**Top Internal Functions/Classes:**
  * `_scroll_initialize` **(Compute Cores)** (Impact: 56.3)
  ...
  * `_change_touch_mode` **(Compute Cores)** (Impact: 48.1)
...
**Top Internal Functions/Classes:**
  * `build_extended` **(Compute Cores)** (Impact: 92.1)
  * `build_smooth` **(Compute Cores)** (Impact: 56.9)
    * *Intent:* # FIXME: Some artifacts can be observed, depending on the line width, # overdraw_width and radius. T...
  * `build_legacy` **(Compute Cores)** (Impact: 34.4)
  * `prebuild_rounded_rectangle` **(Compute Cores)** (Impact: 32.5)
  * `__init__` **(Compute Cores)** (Impact: 29.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `build` **(Compute Cores)** (Impact: 30.0)
  * `build` **(Compute Cores)** (Impact: 27.8)
  * `build` **(Compute Cores)** (Impact: 24.4)
  * `build` **(Compute Cores)** (Impact: 22.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 46.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_pre_render` **(Compute Cores)** (Impact: 148.3)
  ...
  * `p_restricted` **(Many-Argument Workhorses)** (Impact: 21.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_recognize_tick` **(Compute Cores)** (Impact: 23.5)
  ...
  * `_add_result` **(Many-Argument Workhorses)** (Impact: 20.8)
...
**Top Internal Functions/Classes:**
  * `determine_gl_flags` **(Compute Cores)** (Impact: 30.3)
...
**Top Internal Functions/Classes:**
  ...
  * `render` **(Compute Cores)** (Impact: 44.5)
...
**Top Internal Functions/Classes:**
  ...
  * `__cinit__` **(Many-Argument Workhorses)** (Impact: 33.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_update_widget_tree_node` **(Defensive Guards)** (Impact: 14.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 36.9)
  ...
  * `_set_filename` **(Compute Cores)** (Impact: 24.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_build_rule` **(Compute Cores)** (Impact: 21.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `reload` **(Compute Cores)** (Impact: 25.2)
...
**Top Internal Functions/Classes:**
  * `async_idle` **(Compute Cores)** (Impact: 19.2)
  * `idle` **(Compute Cores)** (Impact: 16.0)
  * `_check_ready` **(Many-Argument Workhorses)** (Impact: 12.9)
...
**Top Internal Functions/Classes:**
  ...
  * `process` **(Compute Cores)** (Impact: 21.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_apply_filters` **(Defensive Guards)** (Impact: 21.4)
  ...
  * `_create_files_entries` **(Many-Argument Workhorses)** (Impact: 18.0)
...
**Top Internal Functions/Classes:**
  * `mainloop` **(Compute Cores)** (Impact: 101.9)
    * *Intent:* # for android/iOS, we don't want to have any event nor executing our # main loop while the pause is ...
  * `create_window` **(Many-Argument Workhorses)** (Impact: 42.3)
  * `_update_modifiers` **(Compute Cores)** (Impact: 35.8)
  * `_event_filter` **(Many-Argument Workhorses)** (Impact: 26.2)
