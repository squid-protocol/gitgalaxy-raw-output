...
> **Composition Archetype:** `Flat Modular Platform` (z -0.08; from the repo's file-archetype mix)
> **File Composition:** Defensive Guards Files 23%, Declarative / Non-Code 18%, Large Core Modules 17%, Callbacks & Closures Files 10%, Interface Declarations Files 7%
...
| connectivity | 16955 | 3226 | 8 | `src/plugin-sdk/index.ts` |
...
- `renderApp` **(Compute Cores)** (@ `ui/src/ui/app-render.ts`) -> Impact: **433.5** | LOC: 1004
- `monitorMattermostProvider` **(Defensive Guards)** (@ `extensions/mattermost/src/mattermost/monitor.ts`) -> Impact: **425.4** | LOC: 843
- `runEmbeddedPiAgent` **(Compute Cores)** (@ `src/agents/pi-embedded-runner/run.ts`) -> Impact: **420.6** | LOC: 973
- `attachGatewayWsMessageHandler` **(Compute Cores)** (@ `src/gateway/server/ws-connection/message-handler.ts`) -> Impact: **405.8** | LOC: 961
- `renderSessionsCard` **(Many-Argument Workhorses)** (@ `ui/src/ui/views/usage-render-overview.ts`) -> Impact: **392.1** | LOC: 242
- `runEmbeddedAttempt` **(Compute Cores)** (@ `src/agents/pi-embedded-runner/run/attempt.ts`) -> Impact: **374.9** | LOC: 1105
...
- **Heaviest Functions:** `dismiss` (Many-Argument Workhorses, Impact: 32.9), `presentFinal` (Many-Argument Workhorses, Impact: 31.1), `guardToken` (Defensive Guards, Impact: 21.7)
...
- **Heaviest Functions:** `init` (Compute Cores, Impact: 26.3), `requestCanvasRehydrate` (Compute Cores, Impact: 21.6), `handleCanvasA2UIActionFromWebView` (Defensive Guards, Impact: 16.3)
...
- **Heaviest Functions:** `setValue` (Defensive Guards, Impact: 45.9), `valueAtPath` (Defensive Guards, Impact: 19.8), `applyUIConfig` (Defensive Guards, Impact: 13.0)
...
- **Heaviest Functions:** `menuWillOpen` (Compute Cores, Impact: 22.9), `refreshCache` (Defensive Guards, Impact: 18.2), `captureMenuWidthIfAvailable` (Defensive Guards, Impact: 17.9)
...
- **Heaviest Functions:** `renderUsageTab` (Defensive Guards, Impact: 110.5), `onSelectSession` (Defensive Guards, Impact: 50.8), `onSelectDay` (Defensive Guards, Impact: 21.9)
...
- **Heaviest Functions:** `buildSandboxCreateArgs` (Compute Cores, Impact: 86.1), `execDockerRaw` (Defensive Guards, Impact: 50.7), `ensureSandboxContainer` (Compute Cores, Impact: 36.3)
...
- **Heaviest Functions:** `connectReq` (Defensive Guards, Impact: 177.1), `startServerWithClient` (Defensive Guards, Impact: 34.0), `rpcReq` (Many-Argument Workhorses, Impact: 25.9)
...
- **Heaviest Functions:** `editChannelDiscord` (Compute Cores, Impact: 28.3), `createChannelDiscord` (Compute Cores, Impact: 11.7), `setChannelPermissionDiscord` (Compute Cores, Impact: 6.0)
...
- **Heaviest Functions:** `createThreadDiscord` (Many-Argument Workhorses, Impact: 36.5), `searchMessagesDiscord` (Defensive Guards, Impact: 16.7), `readMessagesDiscord` (Many-Argument Workhorses, Impact: 15.2)
...
- **Heaviest Functions:** `waitForEmbeddedPiRunEnd` (Compute Cores, Impact: 17.2), `clearActiveEmbeddedRun` (State Mutators, Impact: 10.8), `setActiveEmbeddedRun` (State Mutators, Impact: 8.8)
...
**Top Internal Functions/Classes:**
  ...
  * `setScenePhase` **(Compute Cores)** (Impact: 27.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `preparePlaybackInput` **(Compute Cores)** (Impact: 50.6)
  * `resolveVoiceId` **(Defensive Guards)** (Impact: 27.5)
...
**Top Internal Functions/Classes:**
  ...
  * `maybeRepairOpenPolicyAllowFrom` **(Compute Cores)** (Impact: 68.4)
...
**Top Internal Functions/Classes:**
  ...
  * `readStore` **(Compute Cores)** (Impact: 105.0)
  * `resolveDiscordModelPickerRoute` **(Defensive Guards)** (Impact: 51.1)
  * `deliverDiscordInteractionReply` **(Compute Cores)** (Impact: 44.0)
**Contextual Mitigations & Amplifications:**
...
- `ui/src/ui/navigation.ts` -> **Severity: 0.212** (Embedded: 0.0029 * Error Risk: 72.3029%)
- `src/wizard/prompts.ts` -> **Severity: 0.196** (Embedded: 0.0035 * Error Risk: 56.3934%)
- `ui/src/ui/app.ts` -> **Severity: 0.181** (Embedded: 0.002 * Error Risk: 89.1056%)
- `ui/src/ui/uuid.ts` -> **Severity: 0.178** (Embedded: 0.0019 * Error Risk: 93.445%)
- `ui/src/i18n/lib/translate.ts` -> **Severity: 0.167** (Embedded: 0.0021 * Error Risk: 81.5832%)
