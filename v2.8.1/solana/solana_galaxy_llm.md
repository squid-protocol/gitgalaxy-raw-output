...
| Avg Path Length | 1.0913 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **File Composition:** Large Core Modules 27%, Declarative / Non-Code 14%, State Mutators Files 13%, Data / Markup / Trivial 12%, Many-Argument Workhorses Files 6%
...
| connectivity | 15899 | 1139 | 25 | `runtime/src/bank.rs` |
...
| dead_code | 6871 | 872 | 10 | `rpc-client/src/rpc_client.rs` |
...
- `main` **(Compute Cores)** (@ `validator/src/main.rs`) -> Impact: **172.2** | LOC: 1183
...
### 1. `multinode-demo/validator.sh` (SHELL) -> Cumulative Risk: **678.55**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.82)
- **Magnitude:** 392.0 | **LOC:** 368 | **CtrlFlow:** 28.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.9754%)
- **Heaviest Functions:** `usage_[Truncated]` (Compute Cores, Impact: 114.9), `setup_validator_accounts` (Compute Cores, Impact: 24.5), `kill_node` (I/O & Config Routines, Impact: 4.0)

### 2. `multinode-demo/bootstrap-validator.sh` (SHELL) -> Cumulative Risk: **676.17**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +2.30)
...
- **Heaviest Functions:** `Anonymous_Block` (Compute Cores, Impact: 65.6), `Anonymous_Block` (I/O & Config Routines, Impact: 7.0), `kill_node` (I/O & Config Routines, Impact: 4.5)
...
- **Heaviest Functions:** `__colo_node_status_result_normalize` (Compute Cores, Impact: 9.4), `colo_whoami` (I/O & Config Routines, Impact: 8.8), `colo_instance_run_foreach` (Interface Declarations, Impact: 6.6)
...
- **Heaviest Functions:** `Anonymous_Block` (Compute Cores, Impact: 142.1), `__global_context__` (Many-Argument Workhorses, Impact: 38.5), `Anonymous_Block` (Many-Argument Workhorses, Impact: 5.0)
...
- **Heaviest Functions:** `cloud_CreateInstances` (Many-Argument Workhorses, Impact: 54.4), `cloud_Initialize` (Many-Argument Workhorses, Impact: 9.6), `__cloud_FindInstances` (Compute Cores, Impact: 6.2)
...
- **Heaviest Functions:** `setup_connection` (Many-Argument Workhorses, Impact: 58.4), `handle_connection` (Many-Argument Workhorses, Impact: 56.4), `handle_chunk` (Many-Argument Workhorses, Impact: 52.9)
...
- **Heaviest Functions:** `cloud_CreateInstances` (Many-Argument Workhorses, Impact: 66.6), `cloud_WaitForInstanceReady` (Defensive Guards, Impact: 7.3), `cloud_FetchFile` (Many-Argument Workhorses, Impact: 5.7)
...
- **Heaviest Functions:** `handle_connection` (Many-Argument Workhorses, Impact: 37.0), `listen` (Many-Argument Workhorses, Impact: 12.9), `recv_timeout` (Defensive Guards, Impact: 10.0)
...
- **Heaviest Functions:** `__global_context__` (I/O & Config Routines, Impact: 8.2), `Anonymous_Block` (I/O & Config Routines, Impact: 4.7), `bootstrapInstall` (Interface Declarations, Impact: 3.2)
...
- **Heaviest Functions:** `test_ancestors_smaller` (I/O & Config Routines, Impact: 13.7), `test_ancestors_permutations` (I/O & Config Routines, Impact: 10.4), `compare_ancestors` (Tests & Verification, Impact: 5.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_get_confirmed_signatures_for_address2` **(Tests & Verification)** (Impact: 66.5)
...
**Top Internal Functions/Classes:**
  ...
  * `test_fuzz_instructions` **(Compute Cores)** (Impact: 36.9)
  * `test_bank_update_sysvar_account` **(Tests & Verification)** (Impact: 22.8)
  * `test_rent_complex` **(Tests & Verification)** (Impact: 20.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 5 instances
...
**Top Internal Functions/Classes:**
  ...
  * `latest_slot` **(Many-Argument Workhorses)** (Impact: 31.0)
...
**Top Internal Functions/Classes:**
  ...
  * `adjust_lockouts_with_slot_history` **(Compute Cores)** (Impact: 52.0)
...
**Top Internal Functions/Classes:**
  ...
  * `test_mark_valid_then_descendant_invalid` **(Tests & Verification)** (Impact: 22.3)
...
**Top Internal Functions/Classes:**
  * `test_redelegate` **(Compute Cores)** (Impact: 54.6)
  ...
  * `test_deactivate_delinquent` **(Compute Cores)** (Impact: 20.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 8 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.005803
...
- `ci/rust-version.sh` -> **Severity: 0.966** (Embedded: 0.01 * Error Risk: 96.806%)
