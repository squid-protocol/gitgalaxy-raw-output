...
> **Composition Archetype:** `Typed Library` (z -0.60; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 60%, Generic / Templated Code Files 18%, Declarative / Non-Code 5%, Defensive Guards Files 3%, Large Core Modules 3%
...
- **Composition Archetype:** `Compute Cores Files` (z +0.85)
...
- **Heaviest Functions:** `show_code` (Many-Argument Workhorses, Impact: 28.8), `lnotab_interpreted` (Compute Cores, Impact: 9.4), `show_hex` (Compute Cores, Impact: 8.4)
...
- **Heaviest Functions:** `hack_line_numbers` (Compute Cores, Impact: 15.5), `read` (Defensive Guards, Impact: 3.8), `write` (Defensive Guards, Impact: 3.8)
...
- **Heaviest Functions:** `should_skip` (Compute Cores, Impact: 34.7), `run_tests_with_coverage` (Many-Argument Workhorses, Impact: 13.3), `main` (Compute Cores, Impact: 11.2)

### 4. `coverage-7.13.5/lab/warn_executed.py` (PYTHON) -> Cumulative Risk: **535.44**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.65)
...
- **Heaviest Functions:** `find_not_partial_lines` (Many-Argument Workhorses, Impact: 28.5), `analyze_warnings` (Many-Argument Workhorses, Impact: 14.4), `find_executed_excluded_lines` (Many-Argument Workhorses, Impact: 7.8)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 27.9), `select_files` (Compute Cores, Impact: 5.4), `total_for_files` (Compute Cores, Impact: 4.2)

### 6. `coverage-7.13.5/lab/select_contexts.py` (PYTHON) -> Cumulative Risk: **513.92**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.19)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 25.0)

### 7. `coverage-7.13.5/lab/run_sysmon.py` (PYTHON) -> Cumulative Risk: **501.18**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.10)
...
- **Heaviest Functions:** `show_off_off` (Generic / Templated Code, Impact: 4.8), `bytes_to_lines` (Interface Declarations, Impact: 4.6), `show_off` (Parameter Forwarders, Impact: 4.2)
...
- **Heaviest Functions:** `one_file` (Many-Argument Workhorses, Impact: 51.4), `arc_ascii_art` (Compute Cores, Impact: 29.6), `main` (Compute Cores, Impact: 16.9)

### 9. `coverage-7.13.5/lab/run_trace.py` (PYTHON) -> Cumulative Risk: **478.56**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.43)
...
- **Heaviest Functions:** `trace` (Many-Argument Workhorses, Impact: 11.2)
...
- **Heaviest Functions:** `show_configs` (Compute Cores, Impact: 12.2), `_read_config` (Defensive Guards, Impact: 8.1), `show_help` (Interface Declarations, Impact: 2.1)
...
**Top Internal Functions/Classes:**
  ...
  * `test_update_conflicting_file_tracers` **(Tests & Verification)** (Impact: 7.9)
  * `test_update_file_tracer_vs_no_file_tracer` **(Tests & Verification)** (Impact: 7.8)
...
**Top Internal Functions/Classes:**
  ...
  * `test_switch_context_unstarted` **(Tests & Verification)** (Impact: 4.8)
    * *Intent:* # Coverage must be started to switch context msg = "Cannot switch context, coverage is not started" ...
  * `test_combine_no_usable_files` **(Tests & Verification)** (Impact: 4.4)
...
**Top Internal Functions/Classes:**
  ...
  * `measurable_line` **(Compute Cores)** (Impact: 7.7)
...
**Top Internal Functions/Classes:**
  ...
  * `message` **(Compute Cores)** (Impact: 10.8)
...
**Top Internal Functions/Classes:**
  * `make_rigged_file` **(Many-Argument Workhorses)** (Impact: 5.1)
  ...
  * `test_dotpy_not_python_ignored` **(Tests & Verification)** (Impact: 4.2)
  ...
  * `test_dotpy_not_python` **(Tests & Verification)** (Impact: 3.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_tweak_error_checking` **(Tests & Verification)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `test_line_endings` **(Many-Argument Workhorses)** (Impact: 4.9)
  * `test_fuzzed_double_parse` **(Tests & Verification)** (Impact: 4.8)
    * *Intent:* # https://bugs.chromium.org/p/oss-fuzz/issues/detail?id=50381 # The second parse used to raise `Type...
  * `test_os_error` **(Tests & Verification)** (Impact: 4.5)
...
**Top Internal Functions/Classes:**
  ...
  * `test_globs_to_regex` **(Defensive Guards)** (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `first_all_blanks` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  ...
  * `re_lines` **(Defensive Guards)** (Impact: 10.6)
...
**Top Internal Functions/Classes:**
  * `_same_python_executable` **(Compute Cores)** (Impact: 9.9)
  ...
  * `test_file_count` **(Tests & Verification)** (Impact: 8.6)
  * `test_re_lines_inverted` **(Defensive Guards)** (Impact: 6.9)
  * `test_file_exists` **(Tests & Verification)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 20 instances
...
**Top Internal Functions/Classes:**
  ...
  * `test_no_data_to_report_on_annotate` **(Tests & Verification)** (Impact: 3.1)
    * *Intent:* # Reporting with no data produces a nice message and no output # directory. with pytest.raises(NoDat...
  * `test_no_data_to_report_on_html` **(Tests & Verification)** (Impact: 3.1)
    * *Intent:* # Reporting with no data produces a nice message and no output # directory. with pytest.raises(NoDat...
  * `test_no_data_to_report_on_xml` **(Tests & Verification)** (Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `get_qualname` **(Interface Declarations)** (Impact: 3.5)
