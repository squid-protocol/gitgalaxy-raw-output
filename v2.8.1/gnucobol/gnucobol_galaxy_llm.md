...
> **Composition Archetype:** `Mainframe / COBOL & Config` (z +1.45; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 33%, Data / Markup / Trivial 22%, I/O & Config Routines Files 22%, Large Core Modules 15%, Compute Cores Files 2%
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 5.9 | 0.0 | 0.0 |
...
| connectivity | 2646 | 78 | 24 | `libcob/common.h` |
...
- `output_long_integer` **(Compute Cores)** (@ `cobc/codegen.c`) -> Impact: **1091.6** | LOC: 2429
...
- `cb_build_binary_op` **(Many-Argument Workhorses)** (@ `cobc/tree.c`) -> Impact: **442.4** | LOC: 487
  * *Intent:* /* Expression */
- `cob_gen_optim` **(Compute Cores)** (@ `cobc/codeoptim.c`) -> Impact: **421.7** | LOC: 1532
...
### 1. `cobc/tree.c` (C) -> Cumulative Risk: **637.62**
...
- **Heaviest Functions:** `cb_build_intrinsic` (Many-Argument Workhorses, Impact: 511.7), `cb_build_binary_op` (Many-Argument Workhorses, Impact: 442.4), `compare_field_literal` (Many-Argument Workhorses, Impact: 324.0)
...
- **Heaviest Functions:** `cob_check_numval` (Many-Argument Workhorses, Impact: 230.2), `cob_check_numval_f` (Compute Cores, Impact: 93.1), `cob_intr_locale_date` (Many-Argument Workhorses, Impact: 73.1)
...
- **Heaviest Functions:** `cob_gettmpdir` (Compute Cores, Impact: 756.0), `cob_correct_numeric` (Compute Cores, Impact: 156.6), `get_config_val` (Many-Argument Workhorses, Impact: 151.7)
...
- **Heaviest Functions:** `cb_emit_call` (Many-Argument Workhorses, Impact: 315.4), `validate_move_from_num_lit` (Many-Argument Workhorses, Impact: 250.9), `cb_build_identifier` (Compute Cores, Impact: 209.7)
...
- **Heaviest Functions:** `cob_decimal_set_display` (Many-Argument Workhorses, Impact: 544.6), `cob_decimal_get_binary` (Many-Argument Workhorses, Impact: 83.1), `cob_decimal_do_round` (Many-Argument Workhorses, Impact: 73.3)
...
- **Heaviest Functions:** `cb_add_error_to_listing` (Many-Argument Workhorses, Impact: 50.2), `print_error` (Many-Argument Workhorses, Impact: 31.7), `cb_verify_x` (Many-Argument Workhorses, Impact: 30.8)

### 7. `libcob/fextfh.c` (C) -> Cumulative Risk: **601.12**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.30)
...
- **Heaviest Functions:** `EXTFH` (Compute Cores, Impact: 230.7), `copy_fcd_to_file` (Compute Cores, Impact: 105.2), `copy_file_to_fcd` (Many-Argument Workhorses, Impact: 97.1)
...
- **Heaviest Functions:** `field_accept` (Many-Argument Workhorses, Impact: 631.2), `cob_screen_get_all` (Many-Argument Workhorses, Impact: 225.6), `cob_convert_key` (Compute Cores, Impact: 82.5)
...
- **Heaviest Functions:** `cob_move_display_to_edited` (Compute Cores, Impact: 281.4), `cob_move` (Compute Cores, Impact: 269.6), `cob_get_s64_pic9` (Compute Cores, Impact: 65.0)
...
- **Heaviest Functions:** `inspect_common` (Many-Argument Workhorses, Impact: 62.0), `cob_unstring_into` (Many-Argument Workhorses, Impact: 46.5), `cob_inspect_converting` (Defensive Guards, Impact: 19.0)
...
**Top Internal Functions/Classes:**
  ...
  * `cb_build_identifier` **(Compute Cores)** (Impact: 209.7)
...
**Top Internal Functions/Classes:**
  * `output_long_integer` **(Compute Cores)** (Impact: 1091.6)
...
**Top Internal Functions/Classes:**
  * `usage` **(Compute Cores)** (Impact: 91.3)
    * *Intent:* ;
  * `screen_option` **(Compute Cores)** (Impact: 88.8)
    * *Intent:* ;
  * `statement` **(Compute Cores)** (Impact: 71.4)
    * *Intent:* ;
  * `accp_attr` **(Compute Cores)** (Impact: 67.0)
    * *Intent:* ;
...
**Top Internal Functions/Classes:**
  ...
  * `check_current_date` **(Compute Cores)** (Impact: 150.0)
  * `cb_config_entry` **(Compute Cores)** (Impact: 145.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Mitigated Memory Allocs:* 1 instances
...
- **Risk Profile:** Cognitive Load (76.509%), Tech Debt (9.3659%)
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 7422.9 | **LOC:** 6804 | **CtrlFlow:** 31.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (95.3914%), Tech Debt (11.1343%)
**Top Internal Functions/Classes:**
  ...
  * `cb_build_picture` **(Compute Cores)** (Impact: 202.4)
  * `finalize_report` **(Compute Cores)** (Impact: 156.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `cob_screen_get_all` **(Many-Argument Workhorses)** (Impact: 225.6)
    * *Intent:* #endif
  * `cob_convert_key` **(Compute Cores)** (Impact: 82.5)
...
**Top Internal Functions/Classes:**
  * `cob_move_display_to_edited` **(Compute Cores)** (Impact: 281.4)
    * *Intent:* /* Edited */
  * `cob_move` **(Compute Cores)** (Impact: 269.6)
  ...
  * `cob_move_edited_to_display` **(Compute Cores)** (Impact: 61.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `cb_parse_xfd` **(Compute Cores)** (Impact: 114.2)
...
**Top Internal Functions/Classes:**
  ...
  * `report_line` **(Compute Cores)** (Impact: 112.5)
    * *Intent:* /* * GENERATE one report-line */
  * `dumpFlags` **(Compute Cores)** (Impact: 78.7)
    * *Intent:* #ifdef COB_DEBUG_LOG
  * `cob_report_terminate` **(Compute Cores)** (Impact: 76.1)
...
**Top Internal Functions/Classes:**
  * `start` **(Compute Cores)** (Impact: 263.4)
  * `err_handling` **(Compute Cores)** (Impact: 63.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `error` **(Compute Cores)** (Impact: 89.2)
