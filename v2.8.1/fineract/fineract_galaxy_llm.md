...
> **File Composition:** Annotated Framework Methods Files 23%, Data / Markup / Trivial 16%, State Mutators Files 12%, Declarative / Non-Code 10%, Interface Declarations Files 10%
...
- `validateForCreate` **(Compute Cores)** (@ `fineract-provider/src/main/java/org/apache/fineract/portfolio/loanproduct/serialization/LoanProductDataValidator.java`) -> Impact: **188.2** | LOC: 682
...
- **Heaviest Functions:** `commonDatatableSettings` (Many-Argument Workhorses, Impact: 6.5), `payLoanCharge` (Compute Cores, Impact: 5.8), `saveOrUpdateAttendance` (Parameter Forwarders, Impact: 4.4)
...
- **Heaviest Functions:** `fetchAccountMappingDetailsForLoanProduct` (Compute Cores, Impact: 98.3), `setAccrualPeriodicSavingsProductToGLAccountMaps` (Compute Cores, Impact: 34.5), `setCashSavingsProductToGLAccountMaps` (Compute Cores, Impact: 26.1)
...
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.80)
...
- **Heaviest Functions:** `postInterest` (Many-Argument Workhorses, Impact: 111.8), `modifyApplication` (Many-Argument Workhorses, Impact: 97.7), `recalculateDailyBalances` (Many-Argument Workhorses, Impact: 67.1)
...
- **Heaviest Functions:** `periodicInterestRate` (Many-Argument Workhorses, Impact: 96.8), `LoanApplicationTerms` (Many-Argument Workhorses, Impact: 69.9), `calculateTotalInterestForPeriod` (Many-Argument Workhorses, Impact: 69.2)
...
- **Heaviest Functions:** `getActualDepositPeriod` (Compute Cores, Impact: 25.8), `DepositAccountTermAndPreClosure` (Many-Argument Workhorses, Impact: 18.9), `update` (I/O & Config Routines, Impact: 15.0)
...
- **Heaviest Functions:** `fromInt` (Compute Cores, Impact: 55.3), `ChargeTimeType` (State Mutators, Impact: 1.9), `validSavingsValues` (Interface Declarations, Impact: 1.3)
...
- **Heaviest Functions:** `importWorkbook` (Many-Argument Workhorses, Impact: 105.2), `publishEvent` (Many-Argument Workhorses, Impact: 6.8), `createDocument` (Generic / Templated Code, Impact: 2.9)
...
- **Heaviest Functions:** `Charge` (Many-Argument Workhorses, Impact: 105.1), `update` (I/O & Config Routines, Impact: 64.4), `fromJson` (Many-Argument Workhorses, Impact: 11.1)
...
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z -0.13)
...
- **Heaviest Functions:** `updateSummaryWithPivotConfig` (Many-Argument Workhorses, Impact: 85.1), `updateRunningBalanceAndPivotDate` (I/O & Config Routines, Impact: 13.2), `updateFromInterestPeriodSummaries` (Generic / Templated Code, Impact: 9.2)
...
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.75)
...
- **Heaviest Functions:** `updatePaidAmountBy` (Many-Argument Workhorses, Impact: 19.8), `undoPaidOrPartiallyAmountBy` (Many-Argument Workhorses, Impact: 15.5), `updateWaivedAmount` (Compute Cores, Impact: 14.2)
...
**Top Internal Functions/Classes:**
  * `commonDatatableSettings` **(Many-Argument Workhorses)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  ...
  * `verifyLoanRepaymentSchedule` **(Compute Cores)** (Impact: 36.6)
  * `verifyLoanRepaymentSchedule` **(Compute Cores)** (Impact: 26.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `fetchValuesOfTransaction` **(Compute Cores)** (Impact: 67.3)
  * `fetchValuesOfFilteredTransaction` **(Compute Cores)** (Impact: 56.5)
  * `fetchValuesOfLoanTermVariations` **(Compute Cores)** (Impact: 46.3)
  * `getActualValuesList` **(Compute Cores)** (Impact: 43.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `getDisburseLoanAsJSON` **(Many-Argument Workhorses)** (Impact: 12.2)
  ...
  * `applyLoanCommand` **(Compute Cores)** (Impact: 11.1)
    * *Intent:* // TODO: Rewrite to use fineract-client instead! // Example: org.apache.fineract.integrationtests.co...
  * `getApproveLoanAsJSON` **(Many-Argument Workhorses)** (Impact: 9.9)
    * *Intent:* // TODO: Rewrite to use fineract-client instead! // Example: org.apache.fineract.integrationtests.co...
...
**Top Internal Functions/Classes:**
  ...
  * `validateForCreate` **(Compute Cores)** (Impact: 188.2)
...
**Top Internal Functions/Classes:**
  ...
  * `calculatePeriodsBetweenDates` **(Compute Cores)** (Impact: 34.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_balance_correction_on0215_disbursedAmt100_dayInYears360_daysInMonth30_repayEvery1Month` **(Tests & Verification)** (Impact: 14.5)
    * *Intent:* /** * This test case tests a period early and late repayment with balance correction */
  * `test_emi_calculator_performance` **(Tests & Verification)** (Impact: 13.6)
  * `test_two_reschedules_disbursedAmt100_dayInYears360_daysInMonth30_repayEvery1Month` **(Tests & Verification)** (Impact: 13.2)
  * `test_reschedule_disbursedAmt100_dayInYears360_daysInMonth30_repayEvery1Month` **(Tests & Verification)** (Impact: 13.1)
  * `test_reschedule_partial_period_disbursedAmt100_dayInYears360_daysInMonth30_repayEvery1Month` **(Tests & Verification)** (Impact: 13.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getDisbursedAmount` **(Interface Declarations)** (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `validateForCreate` **(Compute Cores)** (Impact: 149.6)
...
**Top Internal Functions/Classes:**
  * `LoanProductsApiResourceSwagger` **(State Mutators)** (Impact: 1.1)
  * `LoanProductChargeData` **(State Mutators)** (Impact: 1.1)
  * `LoanProductChargeToGLAccountMapper` **(State Mutators)** (Impact: 1.1)
  * `PostLoanProductsRequest` **(State Mutators)** (Impact: 1.1)
  * `AllowAttributeOverrides` **(State Mutators)** (Impact: 1.1)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `reduceAdvanceAndLateTotalsForRepaymentPeriod` **(Many-Argument Workhorses)** (Impact: 17.1)
  * `resetDerivedComponents` **(Compute Cores)** (Impact: 14.7)
  * `isOutstandingBalanceNotZero` **(Callbacks & Closures)** (Impact: 10.8)
  * `trackAdvanceAndLateTotalsForRepaymentPeriod` **(Compute Cores)** (Impact: 8.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 68 instances
