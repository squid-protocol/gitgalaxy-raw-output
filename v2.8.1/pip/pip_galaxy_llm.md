...
> **Composition Archetype:** `Typed Library` (z +0.24; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 28%, Data / Markup / Trivial 25%, Large Core Modules 16%, Defensive Guards Files 13%, Declarative / Non-Code 5%
...
- `__init__` **(Many-Argument Workhorses)** (@ `src/pip/_vendor/rich/style.py`) -> Impact: **247.8** | LOC: 75
...
- **Heaviest Functions:** `build_digest_header` (Many-Argument Workhorses, Impact: 57.5), `_basic_auth_str` (Defensive Guards, Impact: 14.2), `handle_401` (Many-Argument Workhorses, Impact: 12.2)
...
- **Heaviest Functions:** `__init__` (Compute Cores, Impact: 14.6), `format_for_error` (Compute Cores, Impact: 7.7), `__eq__` (Defensive Guards, Impact: 5.5)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 34.1), `_check_metadata_consistency` (Defensive Guards, Impact: 9.8), `__init__` (Many-Argument Workhorses, Impact: 9.8)

### 4. `src/pip/_internal/resolution/resolvelib/base.py` (PYTHON) -> Cumulative Risk: **685.12**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.75)
...
- **Heaviest Functions:** `is_satisfied_by` (Generic / Templated Code, Impact: 7.3), `__and__` (Defensive Guards, Impact: 5.6), `from_ireq` (Generic / Templated Code, Impact: 5.3)
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.06)
...
- **Heaviest Functions:** `is_secure_origin` (Many-Argument Workhorses, Impact: 31.0), `user_agent` (I/O & Config Routines, Impact: 27.9), `__init__` (Many-Argument Workhorses, Impact: 16.8)
...
- **Heaviest Functions:** `which` (Many-Argument Workhorses, Impact: 39.0), `match_hostname` (Compute Cores, Impact: 35.0), `detect_encoding` (Defensive Guards, Impact: 30.0)
...
- **Heaviest Functions:** `distutils_scheme` (Many-Argument Workhorses, Impact: 57.6), `get_bin_prefix` (Interface Declarations, Impact: 5.8), `get_scheme` (Many-Argument Workhorses, Impact: 4.3)
...
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.47)
...
- **Heaviest Functions:** `setup_logging` (Many-Argument Workhorses, Impact: 31.9), `emit` (Defensive Guards, Impact: 11.7), `handleError` (Compute Cores, Impact: 9.5)
...
**Top Internal Functions/Classes:**
  ...
  * `test_install_builds_wheels` **(Defensive Guards)** (Impact: 14.1)
  ...
  * `test_install_editable_with_wrong_egg_name` **(Defensive Guards)** (Impact: 10.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 109.2)
  * `_unpack` **(Compute Cores)** (Impact: 66.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_preprocess_lexer_input` **(Compute Cores)** (Impact: 31.8)
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 46.9)
...
**Top Internal Functions/Classes:**
  ...
  * `get_netrc_auth` **(Compute Cores)** (Impact: 21.2)
    * *Intent:* """Returns the Requests tuple auth for a given url from netrc."""
  * `_validate_header_part` **(Defensive Guards)** (Impact: 20.9)
...
**Top Internal Functions/Classes:**
  * `evaluate_link` **(Compute Cores)** (Impact: 75.6)
  ...
  * `get_applicable_candidates` **(Compute Cores)** (Impact: 19.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__str__` **(Compute Cores)** (Impact: 18.1)
