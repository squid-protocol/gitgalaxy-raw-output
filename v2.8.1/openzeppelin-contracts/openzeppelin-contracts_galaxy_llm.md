...
> **Composition Archetype:** `Hub-Coupled App` (z +1.29; from the repo's file-archetype mix)
> **File Composition:** Interface Declarations Files 29%, Data / Markup / Trivial 14%, Large Core Modules 13%, Parameter Forwarders Files 8%, Tests & Verification Files 7%
...
- `shouldBehaveLikeAccessControlDefaultAdminRules` **(Tests & Verification)** (@ `test/access/AccessControl.behavior.js`) -> Impact: **101.2** | LOC: 604
- `shouldBehaveLikeAccountERC7579` **(Tests & Verification)** (@ `test/account/extensions/AccountERC7579.behavior.js`) -> Impact: **88.3** | LOC: 692
- `shouldBehaveLikeERC721` **(Tests & Verification)** (@ `test/token/ERC721/ERC721.behavior.js`) -> Impact: **80.1** | LOC: 741
...
- `fixture` **(Tests & Verification)** (@ `test/governance/extensions/GovernorTimelockAccess.test.js`) -> Impact: **52.9** | LOC: 738
- `shouldTransferTokensByUsers` **(Callbacks & Closures)** (@ `test/token/ERC721/ERC721.behavior.js`) -> Impact: **50.9** | LOC: 117
- `shouldBehaveLikeERC1155` **(Tests & Verification)** (@ `test/token/ERC1155/ERC1155.behavior.js`) -> Impact: **49.7** | LOC: 854
...
- **Heaviest Functions:** `vote` (Defensive Guards, Impact: 22.4), `proposalStatesToBitMap` (Compute Cores, Impact: 13.2), `forgeMessage` (Defensive Guards, Impact: 10.4)
...
- **Heaviest Functions:** `constructor` (Defensive Guards, Impact: 41.9), `newAccount` (Defensive Guards, Impact: 19.8), `packValidationData` (Defensive Guards, Impact: 16.7)
...
- **Heaviest Functions:** `serializeReceipt` (Defensive Guards, Impact: 4.8), `constructor` (Callbacks & Closures, Impact: 2.5), `getTransactionProof` (Interface Declarations, Impact: 1.6)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `fixture` (Interface Declarations, Impact: 1.1)

### 5. `test/utils/cryptography/ERC1271.behavior.js` (JAVASCRIPT) -> Cumulative Risk: **570.36**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z -0.04)
- **Magnitude:** 93.22 | **LOC:** 112 | **CtrlFlow:** 14.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `shouldBehaveLikeERC1271` (Tests & Verification, Impact: 23.5)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `fixture` (Interface Declarations, Impact: 1.2)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `makeTree` (Callbacks & Closures, Impact: 2.6), `fixture` (Interface Declarations, Impact: 1.2)

### 8. `scripts/solc-versions.js` (JAVASCRIPT) -> Cumulative Risk: **545.34**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.27)
...
- **Heaviest Functions:** `compile` (Parameter Forwarders, Impact: 3.8)

### 9. `test/helpers/account.js` (JAVASCRIPT) -> Cumulative Risk: **541.81**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -0.24)
...
- **Heaviest Functions:** `impersonate` (Defensive Guards, Impact: 9.2)
...
- **Heaviest Functions:** `tryRead` (Compute Cores, Impact: 24.2), `run` (Callbacks & Closures, Impact: 1.6), `read` (Interface Declarations, Impact: 1.5)
...
**Top Internal Functions/Classes:**
  ...
  * `self` **(Tests & Verification)** (Impact: 4.1)
  * `before` **(Tests & Verification)** (Impact: 2.5)
  * `self` **(Tests & Verification)** (Impact: 2.2)
  * `self` **(Tests & Verification)** (Impact: 2.2)
...
**Top Internal Functions/Classes:**
  * `extract_2_1` **(Defensive Guards)** (Impact: 3.8)
  * `extract_4_1` **(Defensive Guards)** (Impact: 3.8)
  * `extract_4_2` **(Defensive Guards)** (Impact: 3.8)
  * `extract_6_1` **(Defensive Guards)** (Impact: 3.8)
  * `extract_6_2` **(Defensive Guards)** (Impact: 3.8)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeERC721` **(Tests & Verification)** (Impact: 80.1)
  * `shouldTransferTokensByUsers` **(Callbacks & Closures)** (Impact: 50.9)
  * `tx` **(Tests & Verification)** (Impact: 22.1)
  * `shouldTransferSafely` **(Callbacks & Closures)** (Impact: 16.2)
  * `shouldBehaveLikeERC721Enumerable` **(Tests & Verification)** (Impact: 13.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `fixture` **(Tests & Verification)** (Impact: 52.9)
  * `setAccessManagerIgnored` **(Tests & Verification)** (Impact: 8.1)
  * `prepareOperation` **(Interface Declarations)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `fixture` **(Tests & Verification)** (Impact: 40.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeAccessControlDefaultAdminRules` **(Tests & Verification)** (Impact: 101.2)
  * `shouldBehaveLikeAccessControl` **(Tests & Verification)** (Impact: 23.7)
  * `shouldBehaveLikeAccessControlEnumerable` **(Tests & Verification)** (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `signature` **(Tests & Verification)** (Impact: 31.5)
  * `fixture` **(Tests & Verification)** (Impact: 26.1)
  * `shouldPropose` **(Tests & Verification)** (Impact: 3.9)
...
**Top Internal Functions/Classes:**
  * `parseShare` **(Tests & Verification)** (Impact: 34.5)
  * `parseToken` **(Interface Declarations)** (Impact: 1.5)
...
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeERC1155` **(Tests & Verification)** (Impact: 49.7)
  * `batchTransferWasSuccessful` **(Tests & Verification)** (Impact: 4.5)
  * `transferWasSuccessful` **(Tests & Verification)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `testAsDelayedOperation` **(Tests & Verification)** (Impact: 8.7)
...
**Top Internal Functions/Classes:**
  * `_insert` **(Many-Argument Workhorses)** (Impact: 13.4)
  ...
  * `_insert` **(Many-Argument Workhorses)** (Impact: 13.4)
    * *Intent:* /** * @dev Pushes a (`key`, `value`) pair into an ordered list of checkpoints, either by inserting a...
  * `_upperBinaryLookup` **(Many-Argument Workhorses)** (Impact: 9.7)
...
**Top Internal Functions/Classes:**
  ...
  * `genOperationBatch` **(Many-Argument Workhorses)** (Impact: 2.9)
...
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeAccountERC7579` **(Tests & Verification)** (Impact: 88.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `forgeRequests` **(Tests & Verification)** (Impact: 35.5)
  * `estimateRequest` **(Tests & Verification)** (Impact: 16.6)
...
**Top Internal Functions/Classes:**
  ...
  * `uint256` **(Interface Declarations)** (Impact: 1.7)
  * `fixture` **(Interface Declarations)** (Impact: 1.6)
  * `splitHighLow` **(Interface Declarations)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `shouldBehaveLikeTransparentUpgradeableProxy` **(Tests & Verification)** (Impact: 20.0)
...
**Top Internal Functions/Classes:**
  * `shouldOnlyRevertOnErrors` **(Tests & Verification)** (Impact: 5.7)
...
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeVotes` **(Tests & Verification)** (Impact: 36.4)
  * `getWeight` **(Tests & Verification)** (Impact: 32.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `packValidationData` **(Defensive Guards)** (Impact: 16.7)
...
**Top Internal Functions/Classes:**
  * `fixture` **(Tests & Verification)** (Impact: 29.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_tryParseHexUintUncheckedBounds` **(Type Conversions)** (Impact: 7.2)
