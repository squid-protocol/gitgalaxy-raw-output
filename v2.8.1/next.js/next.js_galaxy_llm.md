...
|---|---|---|
| Modularity | 0.8271 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
> **Composition Archetype:** `Flat Modular Platform` (z +1.15; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 45%, Interface Declarations Files 19%, Declarative / Non-Code 11%, Callbacks & Closures Files 7%, I/O & Config Routines Files 3%
...
| connectivity | 46135 | 17670 | 3 | `packages/next/src/compiled/@vercel/og/index.edge.js` |
...
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -1.78)
...
- **Heaviest Functions:** `createFlightRouterStateFromLoaderTreeImpl` (Many-Argument Workhorses, Impact: 93.9), `createFlightRouterStateFromLoaderTree` (Many-Argument Workhorses, Impact: 3.9), `createRouteTreePrefetch` (Many-Argument Workhorses, Impact: 3.8)

### 2. `packages/next/src/compiled/@babel/runtime/helpers/wrapAsyncGenerator.js` (JAVASCRIPT) -> Cumulative Risk: **759.61**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.41)
...
- **Heaviest Functions:** `AsyncGenerator` (Defensive Guards, Impact: 22.4), `resume` (Defensive Guards, Impact: 13.1), `settle` (Compute Cores, Impact: 11.3)

### 3. `packages/next/src/server/dev/hot-reloader-turbopack.ts` (TYPESCRIPT) -> Cumulative Risk: **757.84**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.52)
...
- **Heaviest Functions:** `createHotReloaderTurbopack` (Many-Argument Workhorses, Impact: 700.0), `onHMR` (Many-Argument Workhorses, Impact: 112.4), `clearRequireCache` (Many-Argument Workhorses, Impact: 61.3)
...
- **Heaviest Functions:** `pushStartInstance` (Many-Argument Workhorses, Impact: 1286.7), `renderElement` (Many-Argument Workhorses, Impact: 505.7), `pushAttribute` (Many-Argument Workhorses, Impact: 240.9)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%)
- **Heaviest Functions:** `pushStartInstance` (Many-Argument Workhorses, Impact: 1286.7), `renderElement` (Many-Argument Workhorses, Impact: 482.5), `pushAttribute` (Many-Argument Workhorses, Impact: 232.4)
...
- **Heaviest Functions:** `render404` (Compute Cores, Impact: 57.5), `createServer` (Compute Cores, Impact: 50.2), `setupWebSocketHandler` (Defensive Guards, Impact: 14.6)

### 8. `packages/next/src/server/lib/router-server.ts` (TYPESCRIPT) -> Cumulative Risk: **744.75**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
...
- **Heaviest Functions:** `initialize` (Compute Cores, Impact: 298.6), `handleRequest` (Compute Cores, Impact: 165.4), `invokeRender` (Many-Argument Workhorses, Impact: 41.6)

### 9. `packages/next/src/bundles/webpack/packages/HotModuleReplacement.runtime.js` (JAVASCRIPT) -> Cumulative Risk: **744.22**
...
- **Heaviest Functions:** `exports` (Callbacks & Closures, Impact: 85.5), `createModuleHotObject` (Defensive Guards, Impact: 57.3), `accept` (Defensive Guards, Impact: 22.6)

### 10. `packages/next/src/compiled/webpack/HotModuleReplacement.runtime.js` (JAVASCRIPT) -> Cumulative Risk: **744.22**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.61)
- **Magnitude:** 512.16 | **LOC:** 408 | **CtrlFlow:** 18.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9999%)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.006672
...
- `packages/next/src/server/route-modules/app-page/module.ts` -> **Severity: 0.064** (Bridge: 0.0007 * Flux: 96.4325%)
...
- `packages/next/src/server/after/after-context.ts` -> **Severity: 0.038** (Bridge: 0.0004 * Flux: 90.2745%)
...
- `packages/next/link.js` -> **Severity: 3.013** (Embedded: 0.0467 * Error Risk: 64.5656%)
- `test/lib/next-test-utils.ts` -> **Severity: 2.452** (Embedded: 0.0374 * Error Risk: 65.5184%)
- `packages/next/server.js` -> **Severity: 2.227** (Embedded: 0.0284 * Error Risk: 78.3421%)
- `test/lib/next-webdriver.ts` -> **Severity: 1.885** (Embedded: 0.0227 * Error Risk: 83.1381%)
- `turbopack/crates/turbo-tasks-fs/src/embed/fs.rs` -> **Severity: 1.318** (Embedded: 0.0272 * Error Risk: 48.4699%)
