...
> **Composition Archetype:** `Small Flat Repo` (z -1.36; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 35%, Data / Markup / Trivial 15%, Large Core Modules 15%, Defensive Guards Files 10%, Compute Cores Files 6%
...
### 1. `src/blst_t.hpp` (CPP) -> Cumulative Risk: **707.5**
...
- **Heaviest Functions:** `operator^` (Compute Cores, Impact: 18.6), `operator^` (Compute Cores, Impact: 18.6), `from` (Compute Cores, Impact: 17.0)

### 2. `bindings/go/blst.go` (GO) -> Cumulative Risk: **690.79**
...
- **Heaviest Functions:** `P1AffinesMult` (Many-Argument Workhorses, Impact: 185.3), `P2AffinesMult` (Many-Argument Workhorses, Impact: 185.3), `coreAggregateVerifyPkInG1` (Many-Argument Workhorses, Impact: 78.9)

### 3. `src/exports.c` (C) -> Cumulative Risk: **681.31**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.32)
...
- **Heaviest Functions:** `blst_uint64_from_fp` (Compute Cores, Impact: 9.6), `blst_uint64_from_fr` (Compute Cores, Impact: 9.6), `blst_sk_inverse` (Defensive Guards, Impact: 8.1)
...
- **Heaviest Functions:** `keygen` (Many-Argument Workhorses, Impact: 33.9), `HKDF_Expand` (Many-Argument Workhorses, Impact: 24.7), `HMAC_init` (Many-Argument Workhorses, Impact: 16.0)
...
- **Heaviest Functions:** `blst_p1_mult` (Many-Argument Workhorses, Impact: 23.8), `POINTonE1_Uncompress_Z` (Compute Cores, Impact: 13.7), `POINTonE1_Deserialize_BE` (Compute Cores, Impact: 10.2)

### 6. `src/asm/x86_64-xlate.pl` (PERL) -> Cumulative Risk: **599.3**
- **Archetype:** `file_cluster_8` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +2.06)
...
- **Heaviest Functions:** `re` (Many-Argument Workhorses, Impact: 265.1), `re` (Compute Cores, Impact: 114.3), `out` (Compute Cores, Impact: 76.3)
...
- **Heaviest Functions:** `blst_p2_mult` (Many-Argument Workhorses, Impact: 23.8), `POINTonE2_Deserialize_BE` (Compute Cores, Impact: 12.4), `POINTonE2_Uncompress_Z` (Compute Cores, Impact: 11.8)
...
- **Heaviest Functions:** `PAIRING_Aggregate_PK_in_G1` (Many-Argument Workhorses, Impact: 91.6), `PAIRING_Aggregate_PK_in_G2` (Many-Argument Workhorses, Impact: 91.3), `blst_pairing_merge` (Compute Cores, Impact: 37.1)

### 9. `src/no_asm.h` (C) -> Cumulative Risk: **565.02**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `mul_mont_n` (Many-Argument Workhorses, Impact: 26.5), `mul_mont_nonred_n` (Many-Argument Workhorses, Impact: 20.5), `redc_mont_n` (Many-Argument Workhorses, Impact: 19.1)

### 10. `src/bytes.h` (C) -> Cumulative Risk: **563.88**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `bytes_from_hexascii` (Defensive Guards, Impact: 16.9), `limbs_from_hexascii` (Defensive Guards, Impact: 16.9), `le_bytes_from_limbs` (Many-Argument Workhorses, Impact: 15.2)
...
- **Magnitude:** 3296.72 | **LOC:** 3631 | **CtrlFlow:** 22.1% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (63.5937%), Tech Debt (30.6942%)
...
**Top Internal Functions/Classes:**
  ...
  * `PAIRING_FinalVerify` **(Compute Cores)** (Impact: 19.4)
...
**Top Internal Functions/Classes:**
  * `pippenger_window_size` **(Defensive Guards)** (Impact: 12.1)
...
**Top Internal Functions/Classes:**
  * `TestSignMultipleVerifyAggregateMinPk` **(Compute Cores)** (Impact: 19.5)
  ...
  * `BenchmarkBatchUncompressMinPk` **(State Mutators)** (Impact: 11.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `POINTonE1_Uncompress_Z` **(Compute Cores)** (Impact: 13.7)
  * `POINTonE1_Deserialize_BE` **(Compute Cores)** (Impact: 10.2)
  * `POINTonE1_Deserialize_Z` **(Compute Cores)** (Impact: 9.7)
...
**Top Internal Functions/Classes:**
  ...
  * `POINTonE2_Deserialize_BE` **(Compute Cores)** (Impact: 12.4)
  * `POINTonE2_Uncompress_Z` **(Compute Cores)** (Impact: 11.8)
  * `POINTonE2_Deserialize_Z` **(Compute Cores)** (Impact: 9.7)
  * `POINTonE2_Uncompress_BE` **(Compute Cores)** (Impact: 8.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `vec_select` **(Many-Argument Workhorses)** (Impact: 40.7)
  * `vec_zero` **(Defensive Guards)** (Impact: 7.5)
  * `vec_is_equal` **(Defensive Guards)** (Impact: 7.0)
  * `vec_is_zero` **(Defensive Guards)** (Impact: 6.1)
  * `vec_cswap` **(Many-Argument Workhorses)** (Impact: 5.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `Xupdate` **(Compute Cores)** (Impact: 6.8)
...
**Top Internal Functions/Classes:**
  ...
  * `HMAC_init` **(Many-Argument Workhorses)** (Impact: 16.0)
  * `parent_SK_to_lamport_PK` **(Many-Argument Workhorses)** (Impact: 13.7)
  * `HKDF_Extract` **(Many-Argument Workhorses)** (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `bytes_from_hexascii` **(Defensive Guards)** (Impact: 16.9)
  * `limbs_from_hexascii` **(Defensive Guards)** (Impact: 16.9)
  * `le_bytes_from_limbs` **(Many-Argument Workhorses)** (Impact: 15.2)
  * `limbs_from_be_bytes` **(Many-Argument Workhorses)** (Impact: 4.8)
  * `limbs_from_le_bytes` **(Many-Argument Workhorses)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `line_add` **(Many-Argument Workhorses)** (Impact: 12.1)
    * *Intent:* /* * Copyright Supranational LLC * Licensed under the Apache License, Version 2.0, see LICENSE for d...
  * `add_n_dbl_n` **(Many-Argument Workhorses)** (Impact: 11.6)
...
**Top Internal Functions/Classes:**
  * `mul_fp2x2` **(Many-Argument Workhorses)** (Impact: 7.0)
  * `sqr_fp12` **(Compute Cores)** (Impact: 6.8)
    * *Intent:* #endif
  * `sqr_fp2x2` **(Compute Cores)** (Impact: 6.0)
  * `blst_bendian_from_fp12` **(Defensive Guards)** (Impact: 5.9)
  * `mul_by_xy00z0_fp12` **(Many-Argument Workhorses)** (Impact: 5.4)
**Contextual Mitigations & Amplifications:**
