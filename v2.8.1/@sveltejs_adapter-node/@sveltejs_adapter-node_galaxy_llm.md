...
> **File Composition:** Data / Markup / Trivial 22%, Defensive Guards Files 22%, Callbacks & Closures Files 11%, Compute Cores Files 11%, Declarative / Non-Code 11%
...
- **Heaviest Functions:** `find` (Defensive Guards, Impact: 34.2), `parse$1` (Defensive Guards, Impact: 28.3), `use` (Defensive Guards, Impact: 25.8)
...
- **Heaviest Functions:** `sirv` (Defensive Guards, Impact: 76.6), `parseSetCookie` (Defensive Guards, Impact: 38.3), `parseString` (Defensive Guards, Impact: 35.7)

### 3. `package/index.js` (JAVASCRIPT) -> Cumulative Risk: **481.65**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.10)
...
- **Heaviest Functions:** `adapt` (Compute Cores, Impact: 17.4), `instrumentation` (Callbacks & Closures, Impact: 1.2), `read` (Callbacks & Closures, Impact: 1.1)
...
- **Heaviest Functions:** `plugin` (State Mutators, Impact: 2.9)
...
- **Heaviest Functions:** `timeout_env` (Compute Cores, Impact: 9.9), `env` (Compute Cores, Impact: 5.4), `parsing_error` (State Mutators, Impact: 4.2)
...
- **Heaviest Functions:** `installPolyfills` (Interface Declarations, Impact: 3.6)
...
**Top Internal Functions/Classes:**
  * `timeout_env` **(Compute Cores)** (Impact: 9.9)
...
**Top Internal Functions/Classes:**
  * `installPolyfills` **(Interface Declarations)** (Impact: 3.6)
...
**Top Internal Functions/Classes:**
  * `plugin` **(State Mutators)** (Impact: 2.9)
**Structural Signatures (Net Mitigated Signals):**
