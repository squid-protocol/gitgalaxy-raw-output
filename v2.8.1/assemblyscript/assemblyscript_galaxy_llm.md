...
> **Composition Archetype:** `Flat Modular Platform` (z +0.44; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 47%, Interface Declarations Files 16%, Declarative / Non-Code 12%, Large Core Modules 6%, State Mutators Files 6%
...
- `unsafeNext` **(Compute Cores)** (@ `src/tokenizer.ts`) -> Impact: **275.1** | LOC: 445
...
- `canOverflow` **(Compute Cores)** (@ `src/flow.ts`) -> Impact: **225.8** | LOC: 256
...
- `build` **(Compute Cores)** (@ `src/bindings/js.ts`) -> Impact: **179.3** | LOC: 507
...
### 1. `cli/index.js` (JAVASCRIPT) -> Cumulative Risk: **718.53**
...
- **Heaviest Functions:** `main` (Many-Argument Workhorses, Impact: 411.8), `prepareResult` (Compute Cores, Impact: 57.1), `getFile` (Compute Cores, Impact: 52.1)

### 2. `std/assembly/util/bytes.ts` (TYPESCRIPT) -> Cumulative Risk: **667.86**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.32)
...
- **Heaviest Functions:** `FILL` (Many-Argument Workhorses, Impact: 46.7), `REVERSE` (Many-Argument Workhorses, Impact: 16.6)

### 3. `bin/asc.js` (JAVASCRIPT) -> Cumulative Risk: **665.1**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.28)
- **Magnitude:** 43.72 | **LOC:** 36 | **CtrlFlow:** 32.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `on` (Callbacks & Closures, Impact: 1.1)

### 4. `std/assembly/util/uri.ts` (TYPESCRIPT) -> Cumulative Risk: **643.06**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.92)
...
- **Heaviest Functions:** `encode` (Many-Argument Workhorses, Impact: 60.8), `decode` (Many-Argument Workhorses, Impact: 58.3), `loadHex` (Compute Cores, Impact: 5.5)

### 5. `std/portable/index.js` (JAVASCRIPT) -> Cumulative Risk: **641.62**
...
- **Heaviest Functions:** `defaultComparator` (Type Conversions, Impact: 14.5), `isArrayLike` (Defensive Guards, Impact: 7.4), `AssertionError` (State Mutators, Impact: 6.1)
...
- **Heaviest Functions:** `build` (Compute Cores, Impact: 179.3), `makeLowerToValue` (Many-Argument Workhorses, Impact: 115.5), `makeLiftFromValue` (Many-Argument Workhorses, Impact: 59.6)

### 7. `std/assembly/date.ts` (TYPESCRIPT) -> Cumulative Risk: **633.23**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.12)
...
- **Heaviest Functions:** `fromString` (Compute Cores, Impact: 29.4), `toISOString` (I/O & Config Routines, Impact: 4.8), `daysSinceEpoch` (Parameter Forwarders, Impact: 4.4)
...
- **Heaviest Functions:** `slice` (Generic / Templated Code, Impact: 26.5), `concat` (Generic / Templated Code, Impact: 24.3), `includes` (Generic / Templated Code, Impact: 16.4)
...
- **Heaviest Functions:** `builtin_assert` (Compute Cores, Impact: 122.6), `builtin_v128_shuffle` (Compute Cores, Impact: 46.5), `ensureVisitMembersOf` (Compute Cores, Impact: 43.2)
...
- **Heaviest Functions:** `utf8Read` (Compute Cores, Impact: 31.6), `utf8Write` (Many-Argument Workhorses, Impact: 21.2), `utf8Length` (Defensive Guards, Impact: 15.0)
...
**Top Internal Functions/Classes:**
  ...
  * `ensureVisitMembersOf` **(Compute Cores)** (Impact: 43.2)
...
**Top Internal Functions/Classes:**
  * `pow` **(Compute Cores)** (Impact: 137.3)
...
**Top Internal Functions/Classes:**
  ...
  * `parseExpressionStart` **(Compute Cores)** (Impact: 158.1)
    * *Intent:* // expressions
  * `parseExpression` **(Compute Cores)** (Impact: 144.3)
...
**Top Internal Functions/Classes:**
  * `initialize` **(Compute Cores)** (Impact: 137.8)
  ...
  * `tryMerge` **(Compute Cores)** (Impact: 60.3)
...
**Top Internal Functions/Classes:**
  * `build` **(Compute Cores)** (Impact: 179.3)
...
**Top Internal Functions/Classes:**
  * `unsafeNext` **(Compute Cores)** (Impact: 275.1)
...
**Top Internal Functions/Classes:**
  * `main` **(Many-Argument Workhorses)** (Impact: 411.8)
    * *Intent:* /** Runs the command line utility using the specified arguments array. */
  * `prepareResult` **(Compute Cores)** (Impact: 57.1)
...
**Top Internal Functions/Classes:**
  * `visitNode` **(Compute Cores)** (Impact: 104.4)
  * `visitClassDeclaration` **(Compute Cores)** (Impact: 43.3)
...
**Top Internal Functions/Classes:**
  ...
  * `walkFunction` **(Interface Declarations)** (Impact: 3.2)
    * *Intent:* /** Walks a specific function. */
  * `walkGlobal` **(Interface Declarations)** (Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `itoa_buffered` **(Compute Cores)** (Impact: 36.8)
...
**Top Internal Functions/Classes:**
  ...
  * `decodeUnsafe` **(Many-Argument Workhorses)** (Impact: 30.1)
    * *Intent:* // @ts-ignore: decorator
  * `toLowerCase` **(Compute Cores)** (Impact: 21.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `build` **(Interface Declarations)** (Impact: 21.1)
