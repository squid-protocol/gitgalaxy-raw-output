...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.42; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 47%, Declarative / Non-Code 15%, Large Core Modules 14%, Interface Declarations Files 7%, State Mutators Files 6%
...
- `createVisitor` **(Compute Cores)** (@ `examples/jsm/loaders/VRMLLoader.js`) -> Impact: **763.7** | LOC: 2659
- `constructor` **(Compute Cores)** (@ `src/renderers/WebGLRenderer.js`) -> Impact: **637.8** | LOC: 2545
  * *Intent:* /** * Constructs a new WebGL renderer. * * @param {WebGLRenderer~Options} [parameters] - The configuration parameter. */
- `parse` **(Compute Cores)** (@ `examples/jsm/loaders/SVGLoader.js`) -> Impact: **511.2** | LOC: 1852
...
- `parse` **(Compute Cores)** (@ `examples/jsm/loaders/EXRLoader.js`) -> Impact: **441.6** | LOC: 2213
...
- `sourceDecorator` **(Compute Cores)** (@ `manual/resources/prettify.js`) -> Impact: **396.1** | LOC: 908
...
- **Heaviest Functions:** `constructor` (Compute Cores, Impact: 20.1), `constructor` (Compute Cores, Impact: 20.0), `select` (Compute Cores, Impact: 18.2)
...
- **Heaviest Functions:** `createMP4` (Many-Argument Workhorses, Impact: 44.0), `constructor` (Many-Argument Workhorses, Impact: 41.4), `constructor` (Many-Argument Workhorses, Impact: 25.4)
...
- **Heaviest Functions:** `Widget` (Compute Cores, Impact: 83.4), `buildKeyMap` (Callbacks & Closures, Impact: 24.6), `resolveAutoHints` (Callbacks & Closures, Impact: 20.3)

### 4. `editor/js/libs/codemirror/addon/tern.js` (JAVASCRIPT) -> Cumulative Risk: **711.52**
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `parseFnType` (Compute Cores, Impact: 194.3), `buildRequest` (Many-Argument Workhorses, Impact: 44.5), `makeTooltip` (Many-Argument Workhorses, Impact: 33.8)

### 5. `editor/js/Loader.js` (JAVASCRIPT) -> Cumulative Risk: **694.38**
...
- **Heaviest Functions:** `Loader` (Compute Cores, Impact: 198.6), `loadFile` (Many-Argument Workhorses, Impact: 127.9), `loadFiles` (Defensive Guards, Impact: 56.3)

### 6. `manual/resources/threejs-lesson-utils.js` (JAVASCRIPT) -> Cumulative Risk: **690.91**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
...
- **Heaviest Functions:** `addDiagram` (Many-Argument Workhorses, Impact: 35.7), `init` (Compute Cores, Impact: 25.9), `render` (Many-Argument Workhorses, Impact: 16.4)

### 7. `editor/js/Viewport.XR.js` (JAVASCRIPT) -> Cumulative Risk: **688.37**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 33.0), `onControllerEvent` (Compute Cores, Impact: 9.8), `onSelect` (Defensive Guards, Impact: 8.9)

### 8. `src/renderers/common/Renderer.js` (JAVASCRIPT) -> Cumulative Risk: **688.15**
...
- **Heaviest Functions:** `_renderScene` (Many-Argument Workhorses, Impact: 115.3), `_projectObject` (Many-Argument Workhorses, Impact: 99.3), `renderObject` (Many-Argument Workhorses, Impact: 87.0)

### 9. `src/nodes/tsl/TSLCore.js` (JAVASCRIPT) -> Cumulative Risk: **685.67**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `ShaderNodeProxy` (Many-Argument Workhorses, Impact: 47.5), `call` (Compute Cores, Impact: 29.4), `ConvertType` (Defensive Guards, Impact: 24.9)

### 10. `editor/js/libs/app.js` (JAVASCRIPT) -> Cumulative Risk: **676.83**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.76)
- **Magnitude:** 207.66 | **LOC:** 291 | **CtrlFlow:** 12.3% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (97.1582%)
- **Heaviest Functions:** `Player` (I/O & Config Routines, Impact: 35.2), `load` (Defensive Guards, Impact: 26.4), `setSize` (State Mutators, Impact: 6.1)
...
**Top Internal Functions/Classes:**
  ...
  * `workerStart` **(Compute Cores)** (Impact: 202.7)
  * `setupWorker` **(Compute Cores)** (Impact: 156.6)
  * `initiateExpression` **(Many-Argument Workhorses)** (Impact: 149.4)
  * `dataFunctionManager` **(Compute Cores)** (Impact: 127.3)
...
**Top Internal Functions/Classes:**
  ...
  * `createVisitor` **(Compute Cores)** (Impact: 763.7)
...
**Top Internal Functions/Classes:**
  * `parse` **(Compute Cores)** (Impact: 441.6)
...
**Top Internal Functions/Classes:**
  * `parse` **(Compute Cores)** (Impact: 511.2)
  ...
  * `parsePathNode` **(Compute Cores)** (Impact: 146.9)
...
**Top Internal Functions/Classes:**
  * `constructor` **(Compute Cores)** (Impact: 637.8)
...
**Top Internal Functions/Classes:**
  * `processMeshAsync` **(Compute Cores)** (Impact: 117.3)
...
**Top Internal Functions/Classes:**
  ...
  * `setJointValue` **(Compute Cores)** (Impact: 40.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getLocalMaterial` **(Compute Cores)** (Impact: 103.3)
...
**Top Internal Functions/Classes:**
  * `Rhino3dmWorker` **(Compute Cores)** (Impact: 118.1)
    * *Intent:* /* WEB WORKER */
