...
> **File Composition:** Large Core Modules 34%, Data / Markup / Trivial 18%, Declarative / Non-Code 14%, Many-Argument Workhorses Files 12%, Compute Cores Files 5%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 25.1 | 11.8 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 37.0 | 2.5 | 80.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 7.6 | 2.2 | 0.0 |
...
| connectivity | 7750 | 404 | 29 | `src/sqliteInt.h` |
...
| dead_code | 2177 | 338 | 10 | `ext/jni/src/org/sqlite/jni/wrapper1/Sqlite.java` |
...
| `autosetup` | 12 | 25670.24 | 43.98% | 36.62% |
...
- **Heaviest Functions:** `do_then_apply_sql` (Defensive Guards, Impact: 13.6), `number_name` (Compute Cores, Impact: 11.0), `do_conflict_test` (Compute Cores, Impact: 10.6)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 120.4), `testCollation` (Compute Cores, Impact: 28.0), `call` (Defensive Guards, Impact: 25.4)
...
- **Heaviest Functions:** `f` (Defensive Guards, Impact: 47.5), `f` (Defensive Guards, Impact: 12.8), `f` (I/O & Config Routines, Impact: 10.2)

### 4. `ext/jni/src/org/sqlite/jni/capi/Tester1.java` (JAVA) -> Cumulative Risk: **667.66**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 150.4), `call` (Many-Argument Workhorses, Impact: 30.3), `testCollation` (Compute Cores, Impact: 28.7)

### 5. `ext/wasm/GNUmakefile` (MAKEFILE) -> Cumulative Risk: **665.64**
- **Archetype:** `file_cluster_3` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +2.27)
...
- **Heaviest Functions:** `t-stripccomments` (Many-Argument Workhorses, Impact: 70.5), `default` (Compute Cores, Impact: 40.0), `all` (Compute Cores, Impact: 10.2)
...
- **Heaviest Functions:** `sqlite3ParseUri` (Many-Argument Workhorses, Impact: 204.7), `openDatabase` (Many-Argument Workhorses, Impact: 183.3), `sqlite3_test_control` (Many-Argument Workhorses, Impact: 142.8)
...
- **Heaviest Functions:** `sqlitecon::DoCommand` (Many-Argument Workhorses, Impact: 68.0), `sqlitecon::SelectTo` (Many-Argument Workhorses, Impact: 15.3), `sqlitecon::_edit` (Compute Cores, Impact: 10.5)
...
- **Heaviest Functions:** `sqlite3Fts5TermsetAdd` (Many-Argument Workhorses, Impact: 24.4), `sqlite3Fts5PoslistNext64` (Many-Argument Workhorses, Impact: 19.8), `sqlite3Fts5BufferSize` (Many-Argument Workhorses, Impact: 12.9)
...
- **Heaviest Functions:** `installOpfsSAHPoolVfs` (Defensive Guards, Impact: 24.9), `importDb` (Defensive Guards, Impact: 23.9), `importDbChunked` (Defensive Guards, Impact: 22.5)
...
- **Heaviest Functions:** `execSql` (Many-Argument Workhorses, Impact: 87.8), `getCommandByName` (Compute Cores, Impact: 31.0), `process` (Many-Argument Workhorses, Impact: 29.9)
...
**Top Internal Functions/Classes:**
  ...
  * `fts5FlushOneHash` **(Compute Cores)** (Impact: 92.3)
...
**Top Internal Functions/Classes:**
  ...
  * `fillInUnixFile` **(Many-Argument Workhorses)** (Impact: 114.2)
    * *Intent:* /**************************************************************************** **********************...
  * `proxyTakeConch` **(Compute Cores)** (Impact: 95.8)
...
**Top Internal Functions/Classes:**
  ...
  * `sqlite3VdbeMemPrettyPrint` **(Compute Cores)** (Impact: 51.3)
    * *Intent:* #ifdef SQLITE_DEBUG /* ** Write a nice string representation of the contents of cell pMem ** into bu...
  * `applyAffinity` **(Many-Argument Workhorses)** (Impact: 29.6)
...
**Top Internal Functions/Classes:**
  ...
  * `sqlite3VdbeHalt` **(Compute Cores)** (Impact: 93.6)
...
**Top Internal Functions/Classes:**
  ...
  * `rbuObjIterGetIndexWhere` **(Compute Cores)** (Impact: 86.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.001835
...
**Top Internal Functions/Classes:**
  ...
  * `walIndexRecover` **(Compute Cores)** (Impact: 65.0)
...
**Top Internal Functions/Classes:**
  ...
  * `trimFunc` **(Many-Argument Workhorses)** (Impact: 54.0)
