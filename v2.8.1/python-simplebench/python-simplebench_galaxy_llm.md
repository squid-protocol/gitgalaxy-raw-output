...
> **Composition Archetype:** `Typed Library` (z +1.61; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 64%, Declarative / Non-Code 11%, Generic / Templated Code Files 8%, Many-Argument Workhorses Files 4%, Interface Declarations Files 4%
...
- `media` **(C Struct Operations)** (@ `documentation/html/_static/pygments.css`) -> Impact: **96.7** | LOC: 174
...
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.20)
...
- **Heaviest Functions:** `formatCopyText` (Many-Argument Workhorses, Impact: 56.5), `runWhenDOMLoaded` (Callbacks & Closures, Impact: 9.1), `clearSelection` (Callbacks & Closures, Impact: 4.8)
...
- **Heaviest Functions:** `_performSearch` (Many-Argument Workhorses, Impact: 46.3), `performTermsSearch` (Defensive Guards, Impact: 43.2), `objectSearchCallback` (Defensive Guards, Impact: 27.2)
...
- **Heaviest Functions:** `formatCopyText` (Many-Argument Workhorses, Impact: 56.5), `filterText` (Parameter Forwarders, Impact: 3.9), `escapeRegExp` (Interface Declarations, Impact: 3.0)
...
- **Heaviest Functions:** `main` (Many-Argument Workhorses, Impact: 59.7), `_configure_session_from_args` (Many-Argument Workhorses, Impact: 36.4), `_create_parser` (I/O & Config Routines, Impact: 5.0)

### 5. `src/simplebench/runners.py` (PYTHON) -> Cumulative Risk: **593.4**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.29)
...
- **Heaviest Functions:** `default_runner` (Many-Argument Workhorses, Impact: 74.0), `calibrate_rounds` (Many-Argument Workhorses, Impact: 71.0), `_run_timed_iteration` (Many-Argument Workhorses, Impact: 27.9)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 91.5), `run` (Defensive Guards, Impact: 30.4), `validate_action_signature` (Many-Argument Workhorses, Impact: 28.8)

### 7. `src/simplebench/exceptions/__init__.py` (PYTHON) -> Cumulative Risk: **578.7**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.22)
- **Magnitude:** 87.04 | **LOC:** 440 | **CtrlFlow:** 4.9% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), State Flux (99.9589%), Api Exposure (85.0923%)
- **Heaviest Functions:** `__init__` (Defensive Guards, Impact: 7.4), `generate_message` (Compute Cores, Impact: 5.9), `__init__` (Generic / Templated Code, Impact: 3.0)

### 8. `src/simplebench/decorators.py` (PYTHON) -> Cumulative Risk: **576.17**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `benchmark` (Many-Argument Workhorses, Impact: 159.0), `decorator` (Compute Cores, Impact: 24.3), `validate_timer` (Many-Argument Workhorses, Impact: 14.9)

### 9. `src/simplebench/stats/stats.py` (PYTHON) -> Cumulative Risk: **559.79**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__eq__` (Compute Cores, Impact: 26.9), `__eq__` (Compute Cores, Impact: 26.7), `from_dict` (Many-Argument Workhorses, Impact: 12.6)

### 10. `src/simplebench/session.py` (PYTHON) -> Cumulative Risk: **555.55**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.63)
...
- **Heaviest Functions:** `report` (Compute Cores, Impact: 47.4), `__init__` (Many-Argument Workhorses, Impact: 31.5), `run` (Compute Cores, Impact: 19.4)
...
**Top Internal Functions/Classes:**
  ...
  * `validate_dirpath` **(Compute Cores)** (Impact: 43.7)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 16.0)
...
**Top Internal Functions/Classes:**
  * `update` **(Defensive Guards)** (Impact: 38.1)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 23.0)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 20.6)
  ...
  * `update` **(Many-Argument Workhorses)** (Impact: 10.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `Stemmer` **(Compute Cores)** (Impact: 57.9)
...
**Top Internal Functions/Classes:**
  ...
  * `get_registered_cases` **(Interface Declarations)** (Impact: 1.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_decorator_use_field_for_n_valid` **(Tests & Verification)** (Impact: 6.8)
    * *Intent:* """Test that the ``@benchmark`` decorator works correctly with valid use_field_for_n."""
  * `test_benchmark_decorator_registers_case` **(Tests & Verification)** (Impact: 5.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_validate_variation_cols` **(Defensive Guards)** (Impact: 14.1)
    * *Intent:* """Validate the variation_cols dictionary. Args: value (dict[str, str]): The variation_cols dictiona...
  * `_validate_variation_marks` **(Defensive Guards)** (Impact: 12.5)
  ...
  * `_validate_iterations` **(Defensive Guards)** (Impact: 8.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_create_parser` **(I/O & Config Routines)** (Impact: 5.0)
