...
> **Composition Archetype:** `Flat Modular Platform` (z +0.48; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 22%, Data / Markup / Trivial 21%, Annotated Framework Methods Files 14%, State Mutators Files 11%, Tests & Verification Files 11%
...
- `read_stream` **(Compute Cores)** (@ `actix-multipart/src/field.rs`) -> Impact: **57.3** | LOC: 73
  * *Intent:* /// Reads content chunk of body part with unknown length. /// /// The `Content-Length` header for body part is not necessary.
- `connect` **(Compute Cores)** (@ `awc/src/ws.rs`) -> Impact: **56.6** | LOC: 171
...
- **Heaviest Functions:** `send_request` (Many-Argument Workhorses, Impact: 39.5), `send_body` (Defensive Guards, Impact: 19.7), `handshake` (Generic / Templated Code, Impact: 2.3)

### 2. `actix-files/src/files.rs` (RUST) -> Cumulative Risk: **597.16**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.05)
- **Magnitude:** 137.18 | **LOC:** 485 | **CtrlFlow:** 4.8% | **Authorship Centralization:** 75.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (94.4839%), Verification (80.0%), Tech Debt (78.6881%)
- **Heaviest Functions:** `register` (Compute Cores, Impact: 11.6), `new_service` (Compute Cores, Impact: 8.6), `new` (Compute Cores, Impact: 5.0)

### 3. `actix-http/src/test.rs` (RUST) -> Cumulative Risk: **588.14**
...
- **Heaviest Functions:** `read` (Defensive Guards, Impact: 13.0), `read` (Compute Cores, Impact: 9.4), `poll_read` (Generic / Templated Code, Impact: 6.8)

### 4. `actix-http/src/body/utils.rs` (RUST) -> Cumulative Risk: **586.91**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +1.16)
...
- **Heaviest Functions:** `to_bytes_limited` (Many-Argument Workhorses, Impact: 16.3), `to_bytes_limited_streams` (Tests & Verification, Impact: 2.0), `to_bytes_limited_complete` (Tests & Verification, Impact: 1.9)

### 5. `actix-files/src/lib.rs` (RUST) -> Cumulative Risk: **584.8**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +0.23)
...
- **Heaviest Functions:** `test_named_file_empty_range_headers` (Tests & Verification, Impact: 3.9), `test_named_file_content_length_headers` (Tests & Verification, Impact: 3.9), `test_redirect_to_slash_directory` (Tests & Verification, Impact: 3.8)

### 6. `actix-files/src/service.rs` (RUST) -> Cumulative Risk: **573.17**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.58)
- **Magnitude:** 160.94 | **LOC:** 322 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (97.5247%), Documentation (90.0%), Verification (80.0%)
- **Heaviest Functions:** `call` (Compute Cores, Impact: 53.6), `find_compressed` (Compute Cores, Impact: 20.8), `serve_named_file_with_encoding` (Many-Argument Workhorses, Impact: 10.8)
...
- **Heaviest Functions:** `poll` (Many-Argument Workhorses, Impact: 74.8), `skip_until_boundary` (Compute Cores, Impact: 33.2), `read_boundary` (Compute Cores, Impact: 19.5)
...
- **Heaviest Functions:** `poll` (Compute Cores, Impact: 31.0), `build_next_uri` (Compute Cores, Impact: 15.9), `test_redirect_cross_origin_headers` (Tests & Verification, Impact: 14.2)

### 9. `actix-http/src/h2/dispatcher.rs` (RUST) -> Cumulative Risk: **567.24**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.29)
...
- **Heaviest Functions:** `handle_response` (Many-Argument Workhorses, Impact: 30.2), `poll` (Many-Argument Workhorses, Impact: 28.7), `prepare_response` (Many-Argument Workhorses, Impact: 28.0)
...
- **Heaviest Functions:** `poll_next` (Defensive Guards, Impact: 16.8), `need_read` (Defensive Guards, Impact: 9.5), `prepare_waking_test` (Compute Cores, Impact: 8.8)
...
**Top Internal Functions/Classes:**
  ...
  * `can_be_returned_from_fn` **(Tests & Verification)** (Impact: 5.7)
...
**Top Internal Functions/Classes:**
  ...
  * `decode` **(Compute Cores)** (Impact: 27.1)
  * `decode` **(Compute Cores)** (Impact: 26.4)
  * `record` **(Type Conversions)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `check_full_url` **(Tests & Verification)** (Impact: 8.1)
  * `cookie` **(Defensive Guards)** (Impact: 7.5)
    * *Intent:* /// Return request cookie.
  * `test_data` **(Tests & Verification)** (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  * `test_named_file_empty_range_headers` **(Tests & Verification)** (Impact: 3.9)
  * `test_named_file_content_length_headers` **(Tests & Verification)** (Impact: 3.9)
  * `test_redirect_to_slash_directory` **(Tests & Verification)** (Impact: 3.8)
  * `test_named_file_content_encoding_gzip` **(Tests & Verification)** (Impact: 3.4)
  * `test_named_file_content_range_headers` **(Tests & Verification)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 5 instances
...
**Top Internal Functions/Classes:**
  ...
  * `test_services_vec` **(Tests & Verification)** (Impact: 6.8)
  * `test_services_macro` **(Tests & Verification)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `resolver` **(Callbacks & Closures)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `h2_flow_control_window_sizes` **(Tests & Verification)** (Impact: 11.2)
  * `content_length_truncated` **(Tests & Verification)** (Impact: 9.7)
  * `expect_continue_h1` **(Tests & Verification)** (Impact: 8.6)
  * `expect_continue` **(Tests & Verification)** (Impact: 8.5)
...
**Top Internal Functions/Classes:**
  ...
  * `custom_request_replace` **(Many-Argument Workhorses)** (Impact: 11.3)
...
**Top Internal Functions/Classes:**
  * `test_server_cookies` **(Tests & Verification)** (Impact: 9.8)
  * `test_data_drop` **(Tests & Verification)** (Impact: 6.4)
    * *Intent:* // allow deprecated App::data
  * `poll_next` **(Generic / Templated Code)** (Impact: 5.8)
  * `body_gzip_large` **(Tests & Verification)** (Impact: 4.5)
    * *Intent:* // .request(actix_web::http::Method::GET, srv.url("/raw")) // .no_decompress() // .append_header((AC...
  * `test_body_gzip_large_random` **(Tests & Verification)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  * `poll` **(Compute Cores)** (Impact: 31.0)
  * `build_next_uri` **(Compute Cores)** (Impact: 15.9)
  * `test_redirect_cross_origin_headers` **(Tests & Verification)** (Impact: 14.2)
  * `test` **(State Mutators)** (Impact: 9.1)
...
**Top Internal Functions/Classes:**
  ...
  * `can_be_returned_from_fn` **(Tests & Verification)** (Impact: 6.6)
...
**Top Internal Functions/Classes:**
  * `client_cookie_handling` **(Tests & Verification)** (Impact: 10.1)
  * `response_timeout` **(Tests & Verification)** (Impact: 9.6)
  * `connection_wait_queue` **(Tests & Verification)** (Impact: 5.3)
  * `connection_wait_queue_force_close` **(Tests & Verification)** (Impact: 5.3)
  * `client_basic_auth` **(Tests & Verification)** (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `h2_content_length` **(Tests & Verification)** (Impact: 8.2)
  * `load_body` **(Generic / Templated Code)** (Impact: 6.7)
  * `h2_headers` **(Tests & Verification)** (Impact: 5.1)
  * `h2_body1` **(Tests & Verification)** (Impact: 4.0)
  * `h2_response_http_error_handling` **(Tests & Verification)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
* *Amplified Cascading Flux:* 3 instances
