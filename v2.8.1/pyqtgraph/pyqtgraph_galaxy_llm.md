...
> **Composition Archetype:** `Hub-Coupled App` (z -0.09; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 26%, Data / Markup / Trivial 25%, Interface Declarations Files 17%, Declarative / Non-Code 10%, Parameter Forwarders Files 8%
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 73.0), `__init__` (Many-Argument Workhorses, Impact: 38.7), `startQtEventLoop` (Many-Argument Workhorses, Impact: 15.6)
...
- **Heaviest Functions:** `runParallel` (Compute Cores, Impact: 41.2), `__init__` (Many-Argument Workhorses, Impact: 19.0), `__exit__` (Defensive Guards, Impact: 13.0)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 15.6), `get_state` (Defensive Guards, Impact: 12.8), `renderView` (Compute Cores, Impact: 11.8)
...
- **Heaviest Functions:** `handleRequest` (Compute Cores, Impact: 78.7), `send` (Many-Argument Workhorses, Impact: 51.3), `callObj` (Many-Argument Workhorses, Impact: 23.6)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 19.4), `resize` (Compute Cores, Impact: 18.9), `get_qpainterpath_element_array` (Compute Cores, Impact: 15.2)
...
- **Heaviest Functions:** `process` (Many-Argument Workhorses, Impact: 29.6), `updateKeys` (Defensive Guards, Impact: 23.8), `process` (Compute Cores, Impact: 21.1)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 32.8), `buildCetSubMenu` (Compute Cores, Impact: 22.6), `buildUserSubMenu` (Defensive Guards, Impact: 12.7)
...
- **Heaviest Functions:** `map` (Many-Argument Workhorses, Impact: 24.2), `addNew` (Defensive Guards, Impact: 16.9), `__init__` (Type Conversions, Impact: 9.2)
...
- **Heaviest Functions:** `paintGL` (Many-Argument Workhorses, Impact: 97.3), `paint` (Many-Argument Workhorses, Impact: 68.7), `dataBounds` (Many-Argument Workhorses, Impact: 64.0)
...
- **Heaviest Functions:** `paintGL` (Many-Argument Workhorses, Impact: 43.5), `_prepareData` (Compute Cores, Impact: 31.0), `setup` (Compute Cores, Impact: 31.0)
...
**Top Internal Functions/Classes:**
  ...
  * `mouseDragEvent` **(Compute Cores)** (Impact: 42.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_updateMaxTextSize` **(Compute Cores)** (Impact: 30.7)
...
**Top Internal Functions/Classes:**
  ...
  * `updateData` **(Many-Argument Workhorses)** (Impact: 64.0)
...
**Top Internal Functions/Classes:**
  ...
  * `diff` **(Compute Cores)** (Impact: 27.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_getDisplayDataset` **(Compute Cores)** (Impact: 92.8)
  ...
  * `viewRangeChanged` **(Many-Argument Workhorses)** (Impact: 21.2)
...
**Top Internal Functions/Classes:**
  ...
  * `setImage` **(Many-Argument Workhorses)** (Impact: 59.7)
  * `render` **(Compute Cores)** (Impact: 51.2)
...
**Top Internal Functions/Classes:**
  ...
  * `processOrder` **(Compute Cores)** (Impact: 19.2)
...
**Top Internal Functions/Classes:**
  * `handleRequest` **(Compute Cores)** (Impact: 78.7)
  ...
  * `result` **(Many-Argument Workhorses)** (Impact: 19.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_getFromFile` **(Compute Cores)** (Impact: 30.7)
...
**Top Internal Functions/Classes:**
  ...
  * `setOrientation` **(Compute Cores)** (Impact: 12.2)
...
**Top Internal Functions/Classes:**
  ...
  * `setValue` **(Defensive Guards)** (Impact: 16.9)
...
**Top Internal Functions/Classes:**
  ...
  * `resolveAndHookupParameterChild` **(Many-Argument Workhorses)** (Impact: 11.8)
  * `_nameToTitle` **(Many-Argument Workhorses)** (Impact: 11.2)
...
**Top Internal Functions/Classes:**
  ...
  * `transmissionCurve` **(Compute Cores)** (Impact: 11.1)
