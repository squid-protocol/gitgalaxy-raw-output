...
> **File Composition:** Declarative / Non-Code 33%, Data / Markup / Trivial 16%, Many-Argument Workhorses Files 15%, Interface Declarations Files 11%, Large Core Modules 9%
...
- **Heaviest Functions:** `_sodium_runtime_arm_cpu_features` (Compute Cores, Impact: 38.5), `_sodium_runtime_intel_cpu_features` (Compute Cores, Impact: 20.1), `_cpuid` (Type Conversions, Impact: 7.3)
...
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.39)
...
- **Heaviest Functions:** `crypto_secretbox_xchacha20poly1305_open_detached` (Many-Argument Workhorses, Impact: 31.7), `crypto_secretbox_xchacha20poly1305_detached` (Many-Argument Workhorses, Impact: 26.6), `crypto_secretbox_xchacha20poly1305_open_easy` (Many-Argument Workhorses, Impact: 5.6)
...
- **Heaviest Functions:** `crypto_secretstream_xchacha20poly1305_pull` (Many-Argument Workhorses, Impact: 34.1), `crypto_secretstream_xchacha20poly1305_push` (Many-Argument Workhorses, Impact: 21.4), `crypto_secretstream_xchacha20poly1305_rekey` (Compute Cores, Impact: 8.4)
...
- **Heaviest Functions:** `crypto_core_hchacha20` (Many-Argument Workhorses, Impact: 11.6), `crypto_core_hchacha20_outputbytes` (Interface Declarations, Impact: 1.2), `crypto_core_hchacha20_inputbytes` (Interface Declarations, Impact: 1.2)
...
- **Heaviest Functions:** `aes_gcm_encrypt_generic` (Many-Argument Workhorses, Impact: 94.1), `aes_gcm_decrypt_generic` (Many-Argument Workhorses, Impact: 73.5), `gh_ad_blocks` (Many-Argument Workhorses, Impact: 24.7)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `aes_gcm_encrypt_generic` (Many-Argument Workhorses, Impact: 87.3), `aes_gcm_decrypt_generic` (Many-Argument Workhorses, Impact: 73.5), `gh_ad_blocks` (Many-Argument Workhorses, Impact: 24.7)

### 7. `pynacl-1.6.2/src/libsodium/src/libsodium/sodium/codecs.c` (C) -> Cumulative Risk: **641.77**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.33)
...
- **Heaviest Functions:** `sodium_base642bin` (Many-Argument Workhorses, Impact: 72.5), `sodium_hex2bin` (Many-Argument Workhorses, Impact: 45.5), `sodium_bin2base64` (Many-Argument Workhorses, Impact: 42.3)

### 8. `pynacl-1.6.2/src/libsodium/src/libsodium/crypto_core/salsa/ref/core_salsa_ref.c` (C) -> Cumulative Risk: **640.79**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.44)
...
- **Heaviest Functions:** `crypto_core_salsa` (Many-Argument Workhorses, Impact: 11.6), `crypto_core_salsa20` (Parameter Forwarders, Impact: 2.6), `crypto_core_salsa2012` (Parameter Forwarders, Impact: 2.6)
...
- **Heaviest Functions:** `_crypto_scalarmult_ed25519` (Many-Argument Workhorses, Impact: 21.5), `_crypto_scalarmult_ed25519_base` (Many-Argument Workhorses, Impact: 11.2), `_crypto_scalarmult_ed25519_is_inf` (Compute Cores, Impact: 3.5)
...
- **Heaviest Functions:** `escrypt_r` (Many-Argument Workhorses, Impact: 34.9), `escrypt_gensalt_r` (Many-Argument Workhorses, Impact: 30.3), `crypto_pwhash_scryptsalsa208sha256_ll` (Many-Argument Workhorses, Impact: 20.3)
...
**Top Internal Functions/Classes:**
  * `ge25519_double_scalarmult_vartime` **(Many-Argument Workhorses)** (Impact: 31.1)
    * *Intent:* */
  * `slide_vartime` **(Compute Cores)** (Impact: 22.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_LT_COMPILER_PIC` **(Compute Cores)** (Impact: 20.1)
...
**Top Internal Functions/Classes:**
  ...
  * `poly1305_block_copy31` **(Many-Argument Workhorses)** (Impact: 13.4)
...
**Top Internal Functions/Classes:**
  ...
  * `_sodium_base642bin_skip_padding` **(Many-Argument Workhorses)** (Impact: 18.3)
...
**Top Internal Functions/Classes:**
  ...
  * `allocate_memory` **(Compute Cores)** (Impact: 27.0)
  * `argon2_initial_hash` **(Many-Argument Workhorses)** (Impact: 22.1)
  * `argon2_pick_best_implementation` **(Compute Cores)** (Impact: 15.5)
...
**Top Internal Functions/Classes:**
  ...
  * `fe25519_cmov` **(Many-Argument Workhorses)** (Impact: 3.5)
    * *Intent:* */
  * `fe25519_mul32` **(Many-Argument Workhorses)** (Impact: 3.4)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `structural_boundaries: 2`, `args: 14`, `func_start: 14`
...
**Top Internal Functions/Classes:**
  ...
  * `encode64` **(Many-Argument Workhorses)** (Impact: 14.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `safe_read` **(Defensive Guards)** (Impact: 17.1)
    * *Intent:* /* LCOV_EXCL_STOP */
  * `randombytes_internal_random_stir` **(Compute Cores)** (Impact: 14.4)
    * *Intent:* #endif /* _WIN32 */ /* * (Re)seed the generator using the entropy source */
  * `randombytes_internal_random_random_dev_open` **(Compute Cores)** (Impact: 13.8)
    * *Intent:* # endif /* LCOV_EXCL_START */
  * `randombytes_internal_random_buf` **(Defensive Guards)** (Impact: 9.9)
    * *Intent:* /* * Put `size` random bytes into `buf` and overwrite the key */
  * `randombytes_getentropy` **(Defensive Guards)** (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `stream_ref_xor_ic` **(Many-Argument Workhorses)** (Impact: 6.5)
  * `chacha_ivsetup` **(Encapsulated Accessors)** (Impact: 6.4)
...
**Top Internal Functions/Classes:**
  ...
  * `blockmix_salsa8` **(Many-Argument Workhorses)** (Impact: 5.9)
    * *Intent:* /* * blockmix_salsa8(Bin, Bout, r): * Compute Bout = BlockMix_{salsa20/8, r}(Bin). * The input Bin m...
  * `integerify` **(Defensive Guards)** (Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `aegis128l_mac` **(Many-Argument Workhorses)** (Impact: 16.1)
  * `aegis128l_init` **(Many-Argument Workhorses)** (Impact: 5.5)
    * *Intent:* #define RATE 32
  * `aegis128l_declast` **(Many-Argument Workhorses)** (Impact: 3.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `fill_block_with_xor` **(Many-Argument Workhorses)** (Impact: 11.6)
  * `fill_block` **(Many-Argument Workhorses)** (Impact: 11.4)
