...
> **Composition Archetype:** `Typed Library` (z +1.15; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 33%, Interface Declarations Files 19%, Generic / Templated Code Files 15%, Large Core Modules 13%, Declarative / Non-Code 11%
...
- `write_headers` **(Many-Argument Workhorses)** (@ `tornado/http1connection.py`) -> Impact: **64.7** | LOC: 86
- `_read_message` **(Many-Argument Workhorses)** (@ `tornado/http1connection.py`) -> Impact: **63.2** | LOC: 121
...
- **Heaviest Functions:** `_receive_frame` (Compute Cores, Impact: 36.6), `_write_frame` (Many-Argument Workhorses, Impact: 31.0), `__init__` (Many-Argument Workhorses, Impact: 26.3)
...
- **Heaviest Functions:** `run` (Compute Cores, Impact: 92.9), `_get_ssl_options` (Compute Cores, Impact: 18.7), `_handle_exception` (Many-Argument Workhorses, Impact: 17.4)
...
- **Heaviest Functions:** `connect` (Many-Argument Workhorses, Impact: 35.4), `_create_stream` (Many-Argument Workhorses, Impact: 28.3), `on_connect_done` (Many-Argument Workhorses, Impact: 13.8)
...
- **Heaviest Functions:** `_parse` (Many-Argument Workhorses, Impact: 157.5), `__init__` (Many-Argument Workhorses, Impact: 54.6), `__getitem__` (Defensive Guards, Impact: 12.9)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 26.1), `_apply_xheaders` (Compute Cores, Impact: 13.2), `initialize` (Many-Argument Workhorses, Impact: 9.9)
...
- **Heaviest Functions:** `handle_request` (Defensive Guards, Impact: 22.6), `environ` (Compute Cores, Impact: 15.8), `_log` (Defensive Guards, Impact: 8.9)

### 7. `tornado/auth.py` (PYTHON) -> Cumulative Risk: **745.75**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.33)
...
- **Heaviest Functions:** `_on_authentication_verified` (Compute Cores, Impact: 36.0), `twitter_request` (Many-Argument Workhorses, Impact: 30.5), `authorize_redirect` (Many-Argument Workhorses, Impact: 19.0)
...
- **Heaviest Functions:** `chain_future` (Defensive Guards, Impact: 17.3), `run_on_executor` (Compute Cores, Impact: 13.5), `copy` (Defensive Guards, Impact: 10.4)

### 9. `tornado/web.py` (PYTHON) -> Cumulative Risk: **706.23**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.16)
...
- **Heaviest Functions:** `set_cookie` (Many-Argument Workhorses, Impact: 73.6), `get` (Many-Argument Workhorses, Impact: 60.1), `render` (Many-Argument Workhorses, Impact: 41.7)
...
- **Heaviest Functions:** `update_handler` (Compute Cores, Impact: 18.9), `_run_select` (Defensive Guards, Impact: 16.0), `initialize` (Many-Argument Workhorses, Impact: 14.4)
...
**Top Internal Functions/Classes:**
  * `_on_authentication_verified` **(Compute Cores)** (Impact: 36.0)
  * `twitter_request` **(Many-Argument Workhorses)** (Impact: 30.5)
  * `authorize_redirect` **(Many-Argument Workhorses)** (Impact: 19.0)
  * `_oauth_request_token_url` **(Many-Argument Workhorses)** (Impact: 16.9)
...
**Top Internal Functions/Classes:**
  * `_handle_events` **(Many-Argument Workhorses)** (Impact: 32.6)
  * `start_tls` **(Many-Argument Workhorses)** (Impact: 26.1)
  * `close` **(Defensive Guards)** (Impact: 23.1)
  * `_read_to_buffer_loop` **(Compute Cores)** (Impact: 17.6)
    * *Intent:* # This method is called from _handle_read and _try_inline_read. if self._read_bytes is not None: tar...
  * `_signal_closed` **(Defensive Guards)** (Impact: 17.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `write_headers` **(Many-Argument Workhorses)** (Impact: 64.7)
  * `_read_message` **(Many-Argument Workhorses)** (Impact: 63.2)
  * `_read_body` **(Many-Argument Workhorses)** (Impact: 31.7)
  * `_read_chunked_body` **(Defensive Guards)** (Impact: 17.2)
...
**Top Internal Functions/Classes:**
  ...
  * `add` **(Many-Argument Workhorses)** (Impact: 18.8)
...
**Top Internal Functions/Classes:**
  ...
  * `get_app` **(C Struct Operations)** (Impact: 5.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_get_ssl_options` **(Compute Cores)** (Impact: 18.7)
  * `_handle_exception` **(Many-Argument Workhorses)** (Impact: 17.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_streaming_callback_coroutine` **(Generic / Templated Code)** (Impact: 5.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
...
**Top Internal Functions/Classes:**
  ...
  * `test_body_sanity_checks` **(Compute Cores)** (Impact: 7.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_init_close_race` **(Interface Declarations)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `load_translations` **(Compute Cores)** (Impact: 32.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_run_select` **(Defensive Guards)** (Impact: 16.0)
...
**Top Internal Functions/Classes:**
  ...
  * `set` **(Defensive Guards)** (Impact: 18.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Rce:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `__new__` **(Many-Argument Workhorses)** (Impact: 12.8)
  * `close` **(Compute Cores)** (Impact: 8.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_handle_socket` **(Many-Argument Workhorses)** (Impact: 13.6)
    * *Intent:* """Called by libcurl when it wants to change the file descriptors it cares about. """
  * `_handle_events` **(Defensive Guards)** (Impact: 10.8)
    * *Intent:* """Called by IOLoop when there is activity on one of our file descriptors. """
  * `_curl_debug` **(Compute Cores)** (Impact: 10.6)
  * `_process_queue` **(Compute Cores)** (Impact: 10.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Compute Cores)** (Impact: 18.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_garbage_collection` **(Compute Cores)** (Impact: 5.4)
    * *Intent:* # Test that timed-out waiters are occasionally cleaned from the queue. sem = locks.Semaphore(value=0...
  * `test_context_manager_contended` **(Tests & Verification)** (Impact: 5.1)
  * `test_acquire_fifo` **(Tests & Verification)** (Impact: 5.0)
...
**Top Internal Functions/Classes:**
  ...
  * `wait` **(Many-Argument Workhorses)** (Impact: 20.7)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 9.7)
  * `configure` **(Defensive Guards)** (Impact: 8.4)
  * `_update_next` **(Compute Cores)** (Impact: 8.4)
**Contextual Mitigations & Amplifications:**
