...
> **Composition Archetype:** `Flat Modular Platform` (z +0.04; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 30%, Data / Markup / Trivial 19%, Declarative / Non-Code 13%, Interface Declarations Files 12%, Callbacks & Closures Files 6%
...
| connectivity | 54914 | 5358 | 16 | `src/vscode-dts/vscode.d.ts` |
...
- `webviewPreloads` **(Compute Cores)** (@ `src/vs/workbench/contrib/notebook/browser/view/renderers/webviewPreloads.ts`) -> Impact: **604.7** | LOC: 1741
...
- **Heaviest Functions:** `getLanguageModes` (Many-Argument Workhorses, Impact: 28.5), `isCompletionItemData` (Defensive Guards, Impact: 5.8), `getAllModesInDocument` (Compute Cores, Impact: 4.7)
...
- **Heaviest Functions:** `findGroup` (Many-Argument Workhorses, Impact: 32.0), `createModalEditorPart` (Defensive Guards, Impact: 30.5), `applyWorkingSet` (Defensive Guards, Impact: 24.2)
...
- **Heaviest Functions:** `triggerGenerators` (Defensive Guards, Impact: 20.3), `getGeneratorContext` (Defensive Guards, Impact: 11.5), `triggerGenerator` (Compute Cores, Impact: 10.7)
...
- **Heaviest Functions:** `set` (Many-Argument Workhorses, Impact: 13.5), `acceptExternal` (Compute Cores, Impact: 11.7), `delete` (Compute Cores, Impact: 8.1)
...
- **Heaviest Functions:** `release` (Compute Cores, Impact: 11.7), `get` (Compute Cores, Impact: 9.7), `_scheduleTrim` (Defensive Guards, Impact: 8.8)
...
- **Heaviest Functions:** `createExtHostQuickOpen` (Many-Argument Workhorses, Impact: 223.2), `showQuickPick` (Many-Argument Workhorses, Impact: 53.3), `_fireDidTriggerItemButton` (Compute Cores, Impact: 16.8)
...
- **Heaviest Functions:** `observerEditsInResponse` (Many-Argument Workhorses, Impact: 116.5), `constructor` (Many-Argument Workhorses, Impact: 39.8), `handleResponseParts` (Compute Cores, Impact: 37.8)

### 8. `src/vs/sessions/services/workspace/browser/workspaceContextService.ts` (TYPESCRIPT) -> Cumulative Risk: **768.27**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z -0.12)
...
- **Heaviest Functions:** `_doUpdateFolders` (Many-Argument Workhorses, Impact: 26.9), `updateFolders` (Compute Cores, Impact: 10.8), `doUpdateFolders` (Callbacks & Closures, Impact: 4.2)
...
- **Heaviest Functions:** `_layout` (Compute Cores, Impact: 43.3), `constructor` (Many-Argument Workhorses, Impact: 28.6), `constructor` (Many-Argument Workhorses, Impact: 27.5)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `getWorkerBootstrapUrl` (Defensive Guards, Impact: 34.0), `createBlobWorker` (Compute Cores, Impact: 7.2), `constructor` (Callbacks & Closures, Impact: 6.6)
...
**Top Internal Functions/Classes:**
  ...
  * `message` **(Compute Cores)** (Impact: 42.0)
...
- `test/automation/src/electron.ts` -> **Severity: 0.625** (Embedded: 0.0067 * Error Risk: 93.6178%)
- `extensions/typescript-language-features/src/utils/dispose.ts` -> **Severity: 0.45** (Embedded: 0.0058 * Error Risk: 77.3566%)
- `extensions/esbuild-extension-common.mts` -> **Severity: 0.445** (Embedded: 0.0061 * Error Risk: 73.189%)
