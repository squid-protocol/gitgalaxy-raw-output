...
> **Composition Archetype:** `Small Flat Repo` (z -0.45; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 40%, Declarative / Non-Code 38%, Interface Declarations Files 6%, Large Core Modules 6%, I/O & Config Routines Files 5%
...
- **Heaviest Functions:** `windows_slashify` (Many-Argument Workhorses, Impact: 367.3), `run_cmd` (Compute Cores, Impact: 58.6), `run_piped` (Compute Cores, Impact: 22.9)
...
- **Heaviest Functions:** `time_ack` (Many-Argument Workhorses, Impact: 23.1), `grab_versions` (Compute Cores, Impact: 17.4), `color` (I/O & Config Routines, Impact: 9.1)
...
- **Heaviest Functions:** `foo` (Interface Declarations, Impact: 1.1), `bar` (Interface Declarations, Impact: 1.1), `foo` (Interface Declarations, Impact: 1.1)
...
- **Heaviest Functions:** `print_line_with_options` (Many-Argument Workhorses, Impact: 96.7), `pmif_context` (Many-Argument Workhorses, Impact: 68.5), `pmif_normal` (Many-Argument Workhorses, Impact: 65.8)

### 5. `xt/man.t` (PERL) -> Cumulative Risk: **480.24**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +2.83)
...
- **Heaviest Functions:** `_populate_man_options` (Compute Cores, Impact: 29.6), `strip_special_chars` (Compute Cores, Impact: 7.5), `check_for_option_in_man_output` (Interface Declarations, Impact: 5.1)
...
- **Heaviest Functions:** `_check_it` (Parameter Forwarders, Impact: 4.2)

### 7. `t/ack-passthru.t` (PERL) -> Cumulative Risk: **468.24**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.25)
...
- **Heaviest Functions:** `color_match` (Compute Cores, Impact: 5.8)
...
- **Heaviest Functions:** `populate_filetypes` (I/O & Config Routines, Impact: 6.0), `is_filetype` (Parameter Forwarders, Impact: 5.6), `filetypes` (Interface Declarations, Impact: 3.1)

### 9. `t/boolean.t` (PERL) -> Cumulative Risk: **442.67**
- **Archetype:** `file_cluster_3` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +1.45)
...
- **Heaviest Functions:** `_movies_are` (Compute Cores, Impact: 7.6), `_argjoin` (Parameter Forwarders, Impact: 2.2)
...
- **Heaviest Functions:** `_populate_help_options` (I/O & Config Routines, Impact: 7.2), `option_in_usage` (Compute Cores, Impact: 6.5), `get_help_options` (Interface Declarations, Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `run_piped` **(Compute Cores)** (Impact: 22.9)
...
**Top Internal Functions/Classes:**
  ...
  * `counts_valid` **(I/O & Config Routines)** (Impact: 5.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
**Top Internal Functions/Classes:**
  * `_populate_man_options` **(Compute Cores)** (Impact: 29.6)
  * `strip_special_chars` **(Compute Cores)** (Impact: 7.5)
  * `check_for_option_in_man_output` **(Interface Declarations)** (Impact: 5.1)
