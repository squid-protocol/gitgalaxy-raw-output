...
> **File Composition:** Data / Markup / Trivial 67%, Declarative / Non-Code 31%, Large Core Modules 1%, Interface Declarations Files 1%, Compute Cores Files 0%
...
- `GetImageInfo` **(Compute Cores)** (@ `exiftool`) -> Impact: **892.5** | LOC: 1015
  * *Intent:* #------------------------------------------------------------------------------ # Get image information from EXIF data in file (or write file if writi...
- `GetImageInfo` **(Compute Cores)** (@ `windows_exiftool`) -> Impact: **892.5** | LOC: 1015
  * *Intent:* #------------------------------------------------------------------------------ # Get image information from EXIF data in file (or write file if writi...
...
### 1. `exiftool` (PERL) -> Cumulative Risk: **737.07**
...
- **Heaviest Functions:** `GetImageInfo` (Compute Cores, Impact: 892.5), `SetImageInfo` (Many-Argument Workhorses, Impact: 445.3), `ScanDir` (Many-Argument Workhorses, Impact: 130.4)

### 2. `windows_exiftool` (PERL) -> Cumulative Risk: **704.36**
- **Archetype:** `file_cluster_17` (Distance: N/A IQR)
...
### 3. `t/TestLib.pm` (PERL) -> Cumulative Risk: **630.05**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (96.4316%)
- **Heaviest Functions:** `nearEnough` (Compute Cores, Impact: 187.2), `testCompare` (Many-Argument Workhorses, Impact: 77.4), `check` (Many-Argument Workhorses, Impact: 39.6)

### 4. `build_geolocation` (PERL) -> Cumulative Risk: **506.74**
- **Archetype:** `file_cluster_12` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.61)
...
- **Heaviest Functions:** `GetFileSize` (Interface Declarations, Impact: 1.8)
...
- **Heaviest Functions:** `Warn` (Interface Declarations, Impact: 1.2)
...
- **Heaviest Functions:** `CryptTest` (Compute Cores, Impact: 25.2)
...
- **Composition Archetype:** `Declarative / Non-Code` (z +1.90)
...
- **Archetype:** `file_cluster_12` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +1.76)
...
### 10. `t/Geotag.t` (PERL) -> Cumulative Risk: **269.2**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +0.94)
- **Magnitude:** 65.82 | **LOC:** 225 | **CtrlFlow:** 51.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (99.9415%), Safety Score (81.0035%), Cognitive Load (76.459%), Churn (7.09%)
...
- **Magnitude:** 5680.7 | **LOC:** 8139 | **CtrlFlow:** 46.1% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (73.479%), Tech Debt (8.8674%)
**Top Internal Functions/Classes:**
  * `GetImageInfo` **(Compute Cores)** (Impact: 892.5)
...
- **Global Archetype:** `file_cluster_17` (Drift: 0.0 IQR)
- **Magnitude:** 5574.52 | **LOC:** 5069 | **CtrlFlow:** 50.3% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (98.1448%), Tech Debt (10.0344%)
**Top Internal Functions/Classes:**
  * `GetImageInfo` **(Compute Cores)** (Impact: 892.5)
    * *Intent:* #------------------------------------------------------------------------------ # Get image informat...
  * `SetImageInfo` **(Many-Argument Workhorses)** (Impact: 445.3)
...
**Top Internal Functions/Classes:**
  * `GetFileSize` **(Interface Declarations)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
