...
> **Composition Archetype:** `Typed Library` (z +0.27; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 32%, Large Core Modules 32%, Data / Markup / Trivial 13%, Declarative / Non-Code 8%, Defensive Guards Files 5%
...
- `_help_format` **(Many-Argument Workhorses)** (@ `pydantic_settings-2.13.1/pydantic_settings/sources/providers/cli.py`) -> Impact: **65.0** | LOC: 26
...
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.99)
...
- **Heaviest Functions:** `_settings_init_sources` (Many-Argument Workhorses, Impact: 359.6), `run` (Many-Argument Workhorses, Impact: 45.8), `_settings_restore_init_kwarg_names` (Many-Argument Workhorses, Impact: 30.1)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 43.5), `get_field_value` (Many-Argument Workhorses, Impact: 28.2), `__getitem__` (Compute Cores, Impact: 11.1)
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.19)
...
- **Heaviest Functions:** `_extract_field_info` (Many-Argument Workhorses, Impact: 48.6), `_replace_field_names_case_insensitively` (Many-Argument Workhorses, Impact: 41.9), `__init__` (Many-Argument Workhorses, Impact: 40.1)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 57.5), `validate_secrets_path` (Compute Cores, Impact: 19.9), `load_secrets` (Generic / Templated Code, Impact: 4.5)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 230.8), `_add_parser_args` (Many-Argument Workhorses, Impact: 158.9), `_add_parser_submodels` (Many-Argument Workhorses, Impact: 116.2)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 17.9), `_load_remote` (Compute Cores, Impact: 16.5), `__getitem__` (Compute Cores, Impact: 14.8)
...
- **Heaviest Functions:** `__call__` (Compute Cores, Impact: 27.2), `__init__` (Many-Argument Workhorses, Impact: 19.5), `_read_env_files` (Generic / Templated Code, Impact: 7.8)

### 8. `pydantic_settings-2.13.1/pydantic_settings/sources/providers/env.py` (PYTHON) -> Cumulative Risk: **562.5**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +1.39)
...
- **Heaviest Functions:** `explode_env_vars` (Many-Argument Workhorses, Impact: 61.5), `next_field` (Many-Argument Workhorses, Impact: 40.8), `prepare_field_value` (Many-Argument Workhorses, Impact: 36.5)
...
- **Heaviest Functions:** `_get_alias_names` (Many-Argument Workhorses, Impact: 51.3), `_annotation_contains_types` (Many-Argument Workhorses, Impact: 49.8), `_annotation_is_complex` (Defensive Guards, Impact: 22.3)
...
- **Heaviest Functions:** `__call__` (Defensive Guards, Impact: 16.9), `get_field_value` (Many-Argument Workhorses, Impact: 13.5), `find_case_path` (Many-Argument Workhorses, Impact: 12.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_help_format` **(Many-Argument Workhorses)** (Impact: 65.0)
...
**Top Internal Functions/Classes:**
  * `test_env_parse_enums` **(Defensive Guards)** (Impact: 13.9)
...
**Top Internal Functions/Classes:**
  * `test_cli_annotation_exceptions` **(C Struct Operations)** (Impact: 26.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_settings_restore_init_kwarg_names` **(Many-Argument Workhorses)** (Impact: 30.1)
  * `_settings_build_values` **(Defensive Guards)** (Impact: 23.5)
...
**Top Internal Functions/Classes:**
  * `mock_secret_client_factory` **(Compute Cores)** (Impact: 14.2)
  * `_create_client` **(Compute Cores)** (Impact: 14.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_coerce_env_val_strict` **(Defensive Guards)** (Impact: 13.1)
...
**Top Internal Functions/Classes:**
  * `test_multiple_file_yaml_deep_merge` **(Defensive Guards)** (Impact: 7.2)
  ...
  * `test_invalid_yaml_config_section_nested_path` **(Tests & Verification)** (Impact: 4.3)
  ...
  * `test_yaml_config_section_non_dict_intermediate` **(Tests & Verification)** (Impact: 4.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_secret_name_map` **(Generic / Templated Code)** (Impact: 9.5)
  * `_select_case_insensitive_secret` **(Generic / Templated Code)** (Impact: 8.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_load_remote` **(Compute Cores)** (Impact: 16.5)
  * `__getitem__` **(Compute Cores)** (Impact: 14.8)
  * `_extract_field_info` **(Type Conversions)** (Impact: 10.4)
  * `__init__` **(Generic / Templated Code)** (Impact: 8.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_env_ignore_empty` **(C Struct Operations)** (Impact: 5.8)
  * `test_invalid_options` **(Tests & Verification)** (Impact: 3.4)
  * `settings_customise_sources` **(Many-Argument Workhorses)** (Impact: 3.3)
...
**Top Internal Functions/Classes:**
  ...
  * `first_not_none` **(Callbacks & Closures)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_pyproject_toml_file_parent` **(Defensive Guards)** (Impact: 3.7)
  * `test_pyproject_toml_no_file_too_shallow` **(Defensive Guards)** (Impact: 3.5)
  * `test_pyproject_toml_file_explicit` **(Defensive Guards)** (Impact: 3.4)
  * `test_pyproject_toml_file` **(Defensive Guards)** (Impact: 2.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_snake_case_conversion_missing_alias` **(Tests & Verification)** (Impact: 5.4)
  * `test_azure_key_vault_settings_source` **(Defensive Guards)** (Impact: 4.1)
    * *Intent:* """Test AzureKeyVaultSettingsSource."""
  * `_raise_resource_not_found_when_getting_parent_secret_name` **(Encapsulated Accessors)** (Impact: 3.9)
  * `test_dash_to_underscore_translation` **(Tests & Verification)** (Impact: 3.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_multiple_file_json_merge` **(Defensive Guards)** (Impact: 6.7)
...
**Top Internal Functions/Classes:**
  ...
  * `_read_env_files` **(Generic / Templated Code)** (Impact: 7.8)
  * `read_env_file` **(Many-Argument Workhorses)** (Impact: 5.8)
  * `_static_read_env_file` **(Generic / Templated Code)** (Impact: 5.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_read_file` **(Generic / Templated Code)** (Impact: 3.7)
...
**Top Internal Functions/Classes:**
  * `test_multiple_file_toml_merge` **(Defensive Guards)** (Impact: 7.2)
  * `test_toml_file` **(Defensive Guards)** (Impact: 3.1)
  * `settings_customise_sources` **(Generic / Templated Code)** (Impact: 3.1)
  * `settings_customise_sources` **(Generic / Templated Code)** (Impact: 3.1)
  * `settings_customise_sources` **(Generic / Templated Code)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `settings_customise_sources` **(Generic / Templated Code)** (Impact: 3.1)
  * `test_precedence_env_over_dotenv` **(C Struct Operations)** (Impact: 2.3)
  * `test_precedence_dotenv_over_secrets` **(C Struct Operations)** (Impact: 2.3)
    * *Intent:* # create dotenv env_file = tmp_path / '.env' env_file.write_text('FOO=from-dotenv\n') # create secre...
  * `test_init_kwargs_override_env_with_alias_and_extra_forbid` **(Defensive Guards)** (Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_read_file` **(Generic / Templated Code)** (Impact: 3.8)
  * `__repr__` **(Generic / Templated Code)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_pick_pyproject_toml_file` **(Compute Cores)** (Impact: 11.5)
