...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.20; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 22%, Declarative / Non-Code 21%, Many-Argument Workhorses Files 19%, Data / Markup / Trivial 15%, Parameter Forwarders Files 5%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 46.0 | 43.0 | 0.0 |
...
- **Heaviest Functions:** `get_from_config` (I/O & Config Routines, Impact: 10.2), `get_estimator_path` (Many-Argument Workhorses, Impact: 7.4), `track_train_score` (Defensive Guards, Impact: 5.5)
...
- **Heaviest Functions:** `pytest_collection_modifyitems` (Many-Argument Workhorses, Impact: 58.1), `pytest_generate_tests` (Compute Cores, Impact: 16.3), `pytest_configure` (Compute Cores, Impact: 11.0)
...
- **Heaviest Functions:** `node_split_best` (Many-Argument Workhorses, Impact: 80.7), `node_split_random` (Many-Argument Workhorses, Impact: 67.8), `init` (Many-Argument Workhorses, Impact: 18.2)
...
- **Heaviest Functions:** `_convert_container` (Many-Argument Workhorses, Impact: 95.3), `check_docstring_parameters` (Many-Argument Workhorses, Impact: 63.2), `assert_docstring_consistency` (Many-Argument Workhorses, Impact: 54.4)
...
- **Heaviest Functions:** `clip` (Many-Argument Workhorses, Impact: 37.5), `isdtype` (Many-Argument Workhorses, Impact: 33.6), `argsort` (Many-Argument Workhorses, Impact: 17.4)
...
- **Heaviest Functions:** `_safely_cast_index_arrays` (Many-Argument Workhorses, Impact: 52.9), `_smallest_admissible_index_dtype` (Many-Argument Workhorses, Impact: 33.4), `_sparse_min_or_max` (Compute Cores, Impact: 24.9)
...
- **Heaviest Functions:** `make_data` (Compute Cores, Impact: 12.9), `make_estimator` (Compute Cores, Impact: 5.9), `make_data` (Compute Cores, Impact: 5.7)
...
- **Heaviest Functions:** `_route_params` (Many-Argument Workhorses, Impact: 47.9), `_get_class_level_metadata_request_values` (Many-Argument Workhorses, Impact: 27.4), `process_routing` (Many-Argument Workhorses, Impact: 27.0)
...
- **Heaviest Functions:** `_cmpkey` (Many-Argument Workhorses, Impact: 42.7), `__init__` (Compute Cores, Impact: 34.3), `_parse_letter_version` (Compute Cores, Impact: 17.3)

### 10. `build_tools/circle/build_doc.sh` (SHELL) -> Cumulative Risk: **628.92**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.75)
...
- **Heaviest Functions:** `get_build_type` (I/O & Config Routines, Impact: 20.5), `__global_context__` (I/O & Config Routines, Impact: 7.1), `Anonymous_Block` (I/O & Config Routines, Impact: 6.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_check_solver` **(Compute Cores)** (Impact: 23.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_fit` **(Many-Argument Workhorses)** (Impact: 26.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_check_w_h` **(Many-Argument Workhorses)** (Impact: 49.5)
    * *Intent:* """Check W and H, or initialize them."""
  * `_minibatch_convergence` **(Many-Argument Workhorses)** (Impact: 48.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 200 instances
...
**Top Internal Functions/Classes:**
  * `_allocate_parameter_mem` **(Many-Argument Workhorses)** (Impact: 80.2)
  * `_fit` **(Many-Argument Workhorses)** (Impact: 48.0)
...
**Top Internal Functions/Classes:**
  ...
  * `test_metadata_routing_for_pipeline` **(Compute Cores)** (Impact: 10.2)
    * *Intent:* # split and partial_fit not relevant for pipelines """Test that metadata is routed correctly for pip...
  * `test_step_name_validation` **(Tests & Verification)** (Impact: 9.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_check_interaction_cst` **(Defensive Guards)** (Impact: 24.3)
    * *Intent:* """Check and validation for interaction constraints."""
  * `_print_iteration_stats` **(Compute Cores)** (Impact: 21.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_kfold_valueerrors` **(Tests & Verification)** (Impact: 10.6)
