...
> **Composition Archetype:** `Small Flat Repo` (z +0.88; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 83%, Declarative / Non-Code 14%, Interface Declarations Files 1%, Callbacks & Closures Files 0%, Large Core Modules 0%
...
- `translateMarkdown` **(Many-Argument Workhorses)** (@ `scripts/translate.ts`) -> Impact: **15.4** | LOC: 29
...
- **Heaviest Functions:** `resolveInfo` (Defensive Guards, Impact: 17.9), `loadInfo` (Compute Cores, Impact: 9.6), `loadQuiz` (Generic / Templated Code, Impact: 4.8)
...
- **Heaviest Functions:** `insertInfoReadme` (Many-Argument Workhorses, Impact: 26.6), `updateIndexREADME` (Compute Cores, Impact: 15.4), `quizToBadge` (Defensive Guards, Impact: 11.6)

### 3. `scripts/translate.ts` (TYPESCRIPT) -> Cumulative Risk: **608.4**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.78)
...
- **Heaviest Functions:** `translateMarkdown` (Many-Argument Workhorses, Impact: 15.4), `translateAllQuizzes` (Compute Cores, Impact: 7.6), `translateQuiz` (Many-Argument Workhorses, Impact: 4.7)
...
- **Heaviest Functions:** `action` (Many-Argument Workhorses, Impact: 52.1), `createMessageBody` (Compute Cores, Impact: 10.7), `getCodeBlock` (Compute Cores, Impact: 10.4)
...
- **Heaviest Functions:** `build` (Callbacks & Closures, Impact: 8.2)
...
- **Heaviest Functions:** `generatePlayground` (I/O & Config Routines, Impact: 24.3), `takeSnapshot` (Compute Cores, Impact: 6.8), `isQuizWritable` (Compute Cores, Impact: 6.3)
...
- **Heaviest Functions:** `action` (Many-Argument Workhorses, Impact: 29.2)
...
- **Heaviest Functions:** `action` (Many-Argument Workhorses, Impact: 27.1)

### 9. `questions/01290-hard-pinia/test-cases.ts` (TYPESCRIPT) -> Cumulative Risk: **337.55**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.26)
...
- **Heaviest Functions:** `increment` (State Mutators, Impact: 1.6), `setNum` (State Mutators, Impact: 1.6), `reset` (Interface Declarations, Impact: 1.4)

### 10. `questions/02828-hard-classpublickeys/test-cases.ts` (TYPESCRIPT) -> Cumulative Risk: **336.81**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.46)
...
- **Heaviest Functions:** `constructor` (State Mutators, Impact: 1.2), `getNum` (Interface Declarations, Impact: 1.1)
...
**Top Internal Functions/Classes:**
  ...
  * `loadInfo` **(Compute Cores)** (Impact: 9.6)
  * `loadQuiz` **(Generic / Templated Code)** (Impact: 4.8)
  * `loadQuizByNo` **(Interface Declarations)** (Impact: 3.4)
  * `loadFile` **(Interface Declarations)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `translateMarkdown` **(Many-Argument Workhorses)** (Impact: 15.4)
  * `translateAllQuizzes` **(Compute Cores)** (Impact: 7.6)
  * `translateQuiz` **(Many-Argument Workhorses)** (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  * `build` **(Callbacks & Closures)** (Impact: 8.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 7 instances
