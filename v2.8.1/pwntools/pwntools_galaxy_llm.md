...
> **File Composition:** Declarative / Non-Code 32%, I/O & Config Routines Files 18%, Data / Markup / Trivial 16%, Interface Declarations Files 8%, Large Core Modules 8%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 65.4 | 3.7 | 0.0 | 0.0 |
...
| connectivity | 2313 | 187 | 6 | `pwnlib/data/includes/generator/linux/diet/unistd.h` |
...
- **Heaviest Functions:** `make_function` (Many-Argument Workhorses, Impact: 34.6), `res` (Compute Cores, Impact: 25.3), `get_context_from_dirpath` (Compute Cores, Impact: 8.3)
...
- **Heaviest Functions:** `update_locked` (Compute Cores, Impact: 35.5), `output` (Many-Argument Workhorses, Impact: 28.0), `put` (Compute Cores, Impact: 23.2)
...
- **Heaviest Functions:** `send` (Compute Cores, Impact: 34.1), `register` (Compute Cores, Impact: 25.1), `unregister` (Defensive Guards, Impact: 21.2)

### 4. `pwnlib/encoders/arm/alphanumeric/ARM_Instructions.py` (PYTHON) -> Cumulative Risk: **675.8**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.70)
...
- **Heaviest Functions:** `dpimm` (Many-Argument Workhorses, Impact: 32.9), `dpshiftreg` (Many-Argument Workhorses, Impact: 29.3), `dpshiftimm` (Many-Argument Workhorses, Impact: 22.0)
...
- **Heaviest Functions:** `__setitem__` (Compute Cores, Impact: 16.4), `__init__` (Compute Cores, Impact: 10.5), `__setattr__` (Compute Cores, Impact: 6.2)

### 6. `pwnlib/encoders/arm/alphanumeric/alphanum_byte.py` (PYTHON) -> Cumulative Risk: **666.21**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.18)
...
- **Heaviest Functions:** `off_gen` (Compute Cores, Impact: 7.5), `alphanumeric_get_byte_ltmax` (Interface Declarations, Impact: 4.5), `alphanumeric_get_complement` (Interface Declarations, Impact: 4.5)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 19.7), `_build_args` (Defensive Guards, Impact: 14.5), `_build_structures` (Compute Cores, Impact: 13.1)
...
- **Heaviest Functions:** `readline` (Many-Argument Workhorses, Impact: 34.1), `redisplay` (Compute Cores, Impact: 17.6), `update_search_results` (Compute Cores, Impact: 11.0)
...
- **Heaviest Functions:** `log_file` (Many-Argument Workhorses, Impact: 18.7), `setter` (Defensive Guards, Impact: 16.4), `device` (Defensive Guards, Impact: 16.3)
...
- **Heaviest Functions:** `_get_timeout_seconds` (Compute Cores, Impact: 11.2), `countdown` (Compute Cores, Impact: 9.9), `local` (Compute Cores, Impact: 5.6)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 43.9)
...
**Top Internal Functions/Classes:**
  ...
  * `corefile` **(Compute Cores)** (Impact: 19.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_gdbserver_port` **(Many-Argument Workhorses)** (Impact: 16.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_flat` **(Defensive Guards)** (Impact: 36.0)
...
**Top Internal Functions/Classes:**
  * `compile` **(Compute Cores)** (Impact: 18.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_parse_nt_file` **(Compute Cores)** (Impact: 31.2)
