...
|---|---|---|
| Modularity | 0.7719 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 4.1858 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.71; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 24%, Declarative / Non-Code 18%, Large Core Modules 18%, Interface Declarations Files 9%, Defensive Guards Files 8%
...
| connectivity | 32473 | 4484 | 9 | `engine/src/flutter/shell/platform/android/test/io/flutter/util/KeyCodes.java` |
...
- **Heaviest Functions:** `copyNativeCodeAssetsMacOS` (Many-Argument Workhorses, Impact: 26.6), `_targetLocationMacOS` (Many-Argument Workhorses, Impact: 21.6), `copyNativeCodeAssetsMacOSFlutterTester` (Many-Argument Workhorses, Impact: 19.2)

### 2. `engine/src/flutter/skwasm/library_skwasm_support.js` (JAVASCRIPT) -> Cumulative Risk: **764.11**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.26)
...
- **Heaviest Functions:** `$skwasm_support_setup` (Callbacks & Closures, Impact: 49.7), `_skwasm_connectThread` (Compute Cores, Impact: 31.1), `eventListener` (Compute Cores, Impact: 31.0)
...
- **Heaviest Functions:** `Chrome` (Many-Argument Workhorses, Impact: 33.2), `generateUserDirectory` (Defensive Guards, Impact: 23.6), `_spawnChromiumProcess` (Many-Argument Workhorses, Impact: 17.7)
...
- **Heaviest Functions:** `_downloadFontAsset` (Defensive Guards, Impact: 12.1), `loadFontFromList` (Defensive Guards, Impact: 11.5), `loadAssetFonts` (Generic / Templated Code, Impact: 10.1)
...
- **Heaviest Functions:** `handleCallback` (Defensive Guards, Impact: 8.8), `callbackHandler` (Defensive Guards, Impact: 6.5), `setSize` (Annotated Framework Methods, Impact: 4.8)
...
- **Heaviest Functions:** `loadAssetFonts` (Compute Cores, Impact: 19.5), `getMissingCodePoints` (Defensive Guards, Impact: 15.2), `loadFontFromList` (Defensive Guards, Impact: 13.3)
...
- **Heaviest Functions:** `main` (I/O & Config Routines, Impact: 19.8), `_validateMacOSPodfile` (State Mutators, Impact: 8.7), `_validateIosPodfile` (State Mutators, Impact: 8.5)
...
- **Heaviest Functions:** `findCostsForFile` (Compute Cores, Impact: 24.4), `findGlobalsForFile` (Compute Cores, Impact: 6.3), `dependenciesAt` (Generic / Templated Code, Impact: 6.2)
...
- **Heaviest Functions:** `findPreviewFunctions` (Defensive Guards, Impact: 21.5), `_onFileSystemEvent` (Defensive Guards, Impact: 21.4), `_fileRemoved` (Defensive Guards, Impact: 15.9)
...
- **Heaviest Functions:** `copyNativeCodeAssetsIOS` (Many-Argument Workhorses, Impact: 22.5), `_targetLocationIOS` (Compute Cores, Impact: 13.0), `assetTargetLocationsIOS` (Generic / Templated Code, Impact: 6.3)
...
**Top Internal Functions/Classes:**
  ...
  * `getSemanticsData` **(Defensive Guards)** (Impact: 89.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_finalizeEditing` **(Defensive Guards)** (Impact: 47.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.001135
...
**Top Internal Functions/Classes:**
  * `describeSemanticsConfiguration` **(Compute Cores)** (Impact: 138.1)
...
**Top Internal Functions/Classes:**
  * `RelayoutWhenSystemFontsChangeMixin` **(Compute Cores)** (Impact: 238.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000279 | `Ripple Effect (Closeness):` 0.028914
...
**Top Internal Functions/Classes:**
  ...
  * `_updatePages` **(Compute Cores)** (Impact: 66.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.00048
...
**Top Internal Functions/Classes:**
  ...
  * `updateChild` **(Defensive Guards)** (Impact: 56.9)
  ...
  * `size` **(Defensive Guards)** (Impact: 33.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00046 | `Ripple Effect (Closeness):` 0.03816
...
**Top Internal Functions/Classes:**
  ...
  * `_getTextDirection` **(Compute Cores)** (Impact: 21.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 9.5e-05 | `Ripple Effect (Closeness):` 0.024379
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.021996
...
**Top Internal Functions/Classes:**
  ...
  * `calculateBoundedFloatingCursorOffset` **(Compute Cores)** (Impact: 46.5)
    * *Intent:* /// Returns the position within the text field closest to the raw cursor offset. /// /// See also: /...
  * `describeSemanticsConfiguration` **(Compute Cores)** (Impact: 35.1)
...
**Top Internal Functions/Classes:**
  * `handleMethodCall` **(Compute Cores)** (Impact: 56.6)
...
**Top Internal Functions/Classes:**
  * `createAccessibilityNodeInfo` **(Compute Cores)** (Impact: 203.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 9e-06 | `Ripple Effect (Closeness):` 0.006909
...
**Top Internal Functions/Classes:**
  ...
  * `_saveLayer` **(Defensive Guards)** (Impact: 12.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.3e-05 | `Ripple Effect (Closeness):` 0.016372
...
- `packages/flutter_tools/lib/src/base/process.dart` -> **Severity: 0.05** (Bridge: 0.0006 * Flux: 81.1052%)
- `packages/flutter/lib/src/widgets/framework.dart` -> **Severity: 0.046** (Bridge: 0.0005 * Flux: 99.4501%)
- `packages/flutter/lib/src/widgets/widget_inspector.dart` -> **Severity: 0.028** (Bridge: 0.0003 * Flux: 98.9053%)
...
- `engine/src/flutter/skwasm/string.cc` -> **Severity: 6.86** (Embedded: 0.1032 * Error Risk: 66.5013%)
- `engine/src/flutter/lib/web_ui/lib/src/engine/skwasm/skwasm_impl/memory.dart` -> **Severity: 6.782** (Embedded: 0.1312 * Error Risk: 51.7136%)
- `engine/src/flutter/impeller/geometry/scalar.h` -> **Severity: 3.901** (Embedded: 0.0532 * Error Risk: 73.3123%)
- `engine/src/flutter/fml/memory/ref_ptr.h` -> **Severity: 3.251** (Embedded: 0.0473 * Error Risk: 68.691%)
- `engine/src/flutter/fml/closure.h` -> **Severity: 2.986** (Embedded: 0.0425 * Error Risk: 70.2063%)
