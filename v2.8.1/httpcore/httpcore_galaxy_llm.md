...
> **Composition Archetype:** `Typed Library` (z +0.47; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 50%, Data / Markup / Trivial 14%, Defensive Guards Files 11%, Interface Declarations Files 9%, Large Core Modules 9%
...
- `__init__` **(Many-Argument Workhorses)** (@ `httpcore-1.0.9/httpcore/_async/connection_pool.py`) -> Impact: **30.1** | LOC: 79
- `__init__` **(Many-Argument Workhorses)** (@ `httpcore-1.0.9/httpcore/_sync/connection_pool.py`) -> Impact: **30.1** | LOC: 79
...
- `_assign_requests_to_connections` **(Compute Cores)** (@ `httpcore-1.0.9/httpcore/_sync/connection_pool.py`) -> Impact: **29.0** | LOC: 70
  * *Intent:* """ Manage the state of the connection pool, assigning incoming requests to connections as available. Called whenever a new request is added or remove...
- `handle_async_request` **(Many-Argument Workhorses)** (@ `httpcore-1.0.9/httpcore/_async/socks_proxy.py`) -> Impact: **28.4** | LOC: 84
- `handle_request` **(Many-Argument Workhorses)** (@ `httpcore-1.0.9/httpcore/_sync/socks_proxy.py`) -> Impact: **28.4** | LOC: 84
- `handle_async_request` **(Compute Cores)** (@ `httpcore-1.0.9/httpcore/_async/http_proxy.py`) -> Impact: **26.5** | LOC: 79
- `handle_request` **(Compute Cores)** (@ `httpcore-1.0.9/httpcore/_sync/http_proxy.py`) -> Impact: **26.5** | LOC: 79
- `_receive_events` **(Many-Argument Workhorses)** (@ `httpcore-1.0.9/httpcore/_async/http2.py`) -> Impact: **26.4** | LOC: 47
- `_receive_events` **(Many-Argument Workhorses)** (@ `httpcore-1.0.9/httpcore/_sync/http2.py`) -> Impact: **26.4** | LOC: 47
...
- **Heaviest Functions:** `handle_async_request` (Compute Cores, Impact: 26.5), `__init__` (Many-Argument Workhorses, Impact: 24.9), `merge_headers` (Type Conversions, Impact: 14.7)
...
- **Heaviest Functions:** `wait` (Compute Cores, Impact: 11.1), `current_async_library` (Defensive Guards, Impact: 8.2), `__aexit__` (Generic / Templated Code, Impact: 7.2)
...
- **Heaviest Functions:** `trace` (Compute Cores, Impact: 18.9), `atrace` (Compute Cores, Impact: 18.9), `__init__` (Many-Argument Workhorses, Impact: 13.1)
...
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `handle_async_request` (Many-Argument Workhorses, Impact: 15.7), `_receive_event` (Compute Cores, Impact: 11.8), `_receive_response_headers` (Defensive Guards, Impact: 9.9)
...
- **Heaviest Functions:** `get_extra_info` (Defensive Guards, Impact: 14.8), `connect_tcp` (Many-Argument Workhorses, Impact: 14.5), `connect_unix_socket` (Many-Argument Workhorses, Impact: 12.2)
...
- **Heaviest Functions:** `_connect` (Compute Cores, Impact: 22.1), `handle_async_request` (Compute Cores, Impact: 15.6), `__init__` (Many-Argument Workhorses, Impact: 11.8)
...
- **Heaviest Functions:** `handle_async_request` (Many-Argument Workhorses, Impact: 28.4), `_init_socks5_connection` (Many-Argument Workhorses, Impact: 18.7), `__init__` (Many-Argument Workhorses, Impact: 13.6)
...
- **Heaviest Functions:** `handle_request` (Compute Cores, Impact: 26.5), `__init__` (Many-Argument Workhorses, Impact: 24.9), `merge_headers` (Type Conversions, Impact: 14.7)

### 9. `httpcore-1.0.9/httpcore/_async/connection_pool.py` (PYTHON) -> Cumulative Risk: **715.86**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.24)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 30.1), `_assign_requests_to_connections` (Compute Cores, Impact: 29.0), `handle_async_request` (Defensive Guards, Impact: 13.9)
...
- **Heaviest Functions:** `_receive_events` (Many-Argument Workhorses, Impact: 26.4), `handle_async_request` (Many-Argument Workhorses, Impact: 19.0), `_receive_response` (Defensive Guards, Impact: 13.1)
...
**Top Internal Functions/Classes:**
  ...
  * `test_connection_pool_concurrency` **(Interface Declarations)** (Impact: 6.1)
    * *Intent:* """ HTTP/1.1 requests made in concurrency must not ever exceed the maximum number of allowable conne...
  * `test_connection_pool_with_http2` **(I/O & Config Routines)** (Impact: 6.0)
    * *Intent:* """ Test a connection pool with HTTP/2 requests. """
  * `test_connection_pool_concurrency_same_domain_closing` **(Interface Declarations)** (Impact: 6.0)
    * *Intent:* """ HTTP/1.1 requests made in concurrency must not ever exceed the maximum number of allowable conne...
...
**Top Internal Functions/Classes:**
  * `_receive_events` **(Many-Argument Workhorses)** (Impact: 26.4)
  * `handle_async_request` **(Many-Argument Workhorses)** (Impact: 19.0)
  * `_receive_response` **(Defensive Guards)** (Impact: 13.1)
    * *Intent:* # Receiving the response...
  * `_response_closed` **(Compute Cores)** (Impact: 12.8)
  * `_send_request_headers` **(Many-Argument Workhorses)** (Impact: 11.5)
...
**Top Internal Functions/Classes:**
  * `wait` **(Compute Cores)** (Impact: 11.1)
...
**Top Internal Functions/Classes:**
  * `handle_async_request` **(Many-Argument Workhorses)** (Impact: 15.7)
  * `_receive_event` **(Compute Cores)** (Impact: 11.8)
  * `_receive_response_headers` **(Defensive Guards)** (Impact: 9.9)
    * *Intent:* # Receiving the response...
  * `_response_closed` **(Compute Cores)** (Impact: 7.7)
  * `_receive_response_body` **(Defensive Guards)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `iter_stream` **(Defensive Guards)** (Impact: 10.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_receive_events` **(Many-Argument Workhorses)** (Impact: 26.4)
  * `handle_request` **(Many-Argument Workhorses)** (Impact: 19.0)
  * `_receive_response` **(Defensive Guards)** (Impact: 13.1)
    * *Intent:* # Receiving the response...
  * `_response_closed` **(Compute Cores)** (Impact: 12.8)
  * `_send_request_headers` **(Many-Argument Workhorses)** (Impact: 11.5)
...
**Top Internal Functions/Classes:**
  * `handle_async_request` **(Many-Argument Workhorses)** (Impact: 28.4)
  ...
  * `is_available` **(Compute Cores)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_write_error_with_response_sent` **(Interface Declarations)** (Impact: 4.6)
...
**Top Internal Functions/Classes:**
  * `handle_async_request` **(Compute Cores)** (Impact: 26.5)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 24.9)
  * `merge_headers` **(Type Conversions)** (Impact: 14.7)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 5.0)
  * `create_connection` **(Compute Cores)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_http11_early_hints` **(Interface Declarations)** (Impact: 2.7)
    * *Intent:* """ HTTP "103 Early Hints" is an interim response. We simply ignore it and return the final response...
  * `test_http11_upgrade_connection` **(Interface Declarations)** (Impact: 2.6)
  ...
  * `test_http11_expect_continue` **(Interface Declarations)** (Impact: 2.5)
...
**Top Internal Functions/Classes:**
  ...
  * `run_async_requests` **(Defensive Guards)** (Impact: 14.4)
...
**Top Internal Functions/Classes:**
  * `handle_request` **(Many-Argument Workhorses)** (Impact: 15.7)
  * `_receive_event` **(Compute Cores)** (Impact: 11.8)
  * `_receive_response_headers` **(Defensive Guards)** (Impact: 9.9)
    * *Intent:* # Receiving the response...
  * `_response_closed` **(Compute Cores)** (Impact: 7.7)
...
**Top Internal Functions/Classes:**
  * `_connect` **(Compute Cores)** (Impact: 22.1)
  * `handle_async_request` **(Compute Cores)** (Impact: 15.6)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 11.8)
  * `is_available` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `handle_request` **(Compute Cores)** (Impact: 26.5)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 24.9)
  * `merge_headers` **(Type Conversions)** (Impact: 14.7)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 5.0)
  * `create_connection` **(Compute Cores)** (Impact: 4.5)
...
**Top Internal Functions/Classes:**
  * `handle_request` **(Many-Argument Workhorses)** (Impact: 28.4)
  * `_init_socks5_connection` **(Many-Argument Workhorses)** (Impact: 18.7)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 13.6)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 10.7)
  * `is_available` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `test_http2_remote_max_streams_update` **(I/O & Config Routines)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  * `write` **(Generic / Templated Code)** (Impact: 6.3)
...
**Top Internal Functions/Classes:**
  * `_connect` **(Compute Cores)** (Impact: 22.1)
  * `handle_request` **(Compute Cores)** (Impact: 15.6)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 11.8)
  * `is_available` **(Compute Cores)** (Impact: 7.6)
  * `info` **(Generic / Templated Code)** (Impact: 5.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_perform_io` **(Defensive Guards)** (Impact: 11.6)
...
**Top Internal Functions/Classes:**
  ...
  * `connect_tcp` **(Generic / Templated Code)** (Impact: 3.1)
  * `start_tls` **(Generic / Templated Code)** (Impact: 2.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 13 instances
...
**Top Internal Functions/Classes:**
  ...
  * `write` **(Generic / Templated Code)** (Impact: 8.7)
