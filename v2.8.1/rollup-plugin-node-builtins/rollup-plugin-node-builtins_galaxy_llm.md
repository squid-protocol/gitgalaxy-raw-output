...
> **File Composition:** Large Core Modules 39%, Callbacks & Closures Files 16%, Data / Markup / Trivial 16%, Defensive Guards Files 13%, State Mutators Files 10%
...
- **Heaviest Functions:** `afterTransform` (Defensive Guards, Impact: 13.1), `Transform` (Defensive Guards, Impact: 11.4), `_write` (Compute Cores, Impact: 10.5)
...
- **Heaviest Functions:** `write` (Compute Cores, Impact: 17.5), `detectIncompleteChar` (Compute Cores, Impact: 14.3), `StringDecoder` (Compute Cores, Impact: 12.9)
...
- **Heaviest Functions:** `removeListener` (Defensive Guards, Impact: 44.3), `_addListener` (Many-Argument Workhorses, Impact: 41.2), `emit` (Compute Cores, Impact: 31.7)
...
**Top Internal Functions/Classes:**
  * `intercept` **(Callbacks & Closures)** (Impact: 5.0)
  * `bind` **(Callbacks & Closures)** (Impact: 2.0)
  * `run` **(Callbacks & Closures)** (Impact: 1.9)
  * `createEmitError` **(Interface Declarations)** (Impact: 1.7)
