...
> **File Composition:** Declarative / Non-Code 25%, Interface Declarations Files 18%, Large Core Modules 16%, Compute Cores Files 10%, Data / Markup / Trivial 9%
...
- **Heaviest Functions:** `loaduvm` (Many-Argument Workhorses, Impact: 18.2), `deallocuvm` (Many-Argument Workhorses, Impact: 15.2), `copyuvm` (Type Conversions, Impact: 13.7)
...
- **Heaviest Functions:** `filewrite` (Many-Argument Workhorses, Impact: 22.0), `fileread` (Compute Cores, Impact: 10.9), `fileclose` (Compute Cores, Impact: 9.7)
...
- **Heaviest Functions:** `create` (Many-Argument Workhorses, Impact: 24.5), `sys_open` (I/O & Config Routines, Impact: 15.4), `argfd` (C Struct Operations, Impact: 14.8)
...
- **Heaviest Functions:** `procdump` (Compute Cores, Impact: 14.6), `sleep` (C Struct Operations, Impact: 10.5), `growproc` (Compute Cores, Impact: 9.4)
...
- **Heaviest Functions:** `memmove` (Many-Argument Workhorses, Impact: 12.9), `strncmp` (Compute Cores, Impact: 10.4), `safestrcpy` (Compute Cores, Impact: 8.7)

### 6. `fs.c` (C) -> Cumulative Risk: **607.59**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `C Struct Operations Files` (z +1.20)
...
- **Heaviest Functions:** `writei` (Many-Argument Workhorses, Impact: 26.1), `readi` (Many-Argument Workhorses, Impact: 21.4), `namex` (C Struct Operations, Impact: 19.7)
...
- **Heaviest Functions:** `printf` (Many-Argument Workhorses, Impact: 42.4), `printint` (Many-Argument Workhorses, Impact: 19.2), `putc` (Encapsulated Accessors, Impact: 2.0)

### 8. `grep.c` (C) -> Cumulative Risk: **595.38**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.60)
...
- **Heaviest Functions:** `matchhere` (Compute Cores, Impact: 14.5), `matchstar` (Compute Cores, Impact: 12.4), `grep` (Compute Cores, Impact: 11.7)
...
- **Heaviest Functions:** `__global_context__` (Many-Argument Workhorses, Impact: 114.4), `Anonymous_Block` (Compute Cores, Impact: 5.8), `Anonymous_Block` (I/O & Config Routines, Impact: 3.5)

### 10. `ulib.c` (C) -> Cumulative Risk: **575.77**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.27)
...
- **Heaviest Functions:** `gets` (Compute Cores, Impact: 9.5), `strchr` (Type Conversions, Impact: 5.6), `strcmp` (Compute Cores, Impact: 5.5)
...
**Top Internal Functions/Classes:**
  ...
  * `dirlookup` **(C Struct Operations)** (Impact: 15.2)
...
**Top Internal Functions/Classes:**
  ...
  * `runcmd` **(C Struct Operations)** (Impact: 26.4)
    * *Intent:* // Execute cmd. Never returns.
  * `parseredirs` **(C Struct Operations)** (Impact: 17.2)
  * `nulterminate` **(C Struct Operations)** (Impact: 15.0)
    * *Intent:* // NUL-terminate all the counted strings.
  * `parseexec` **(C Struct Operations)** (Impact: 12.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `mappages` **(Many-Argument Workhorses)** (Impact: 13.3)
...
**Top Internal Functions/Classes:**
  ...
  * `consoleintr` **(Compute Cores)** (Impact: 27.6)
    * *Intent:* #define C(x) ((x)-'@') // Control-x
  * `printint` **(Many-Argument Workhorses)** (Impact: 17.2)
  * `consoleread` **(Many-Argument Workhorses)** (Impact: 15.8)
  * `cgaputc` **(Compute Cores)** (Impact: 14.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `memmove` **(Many-Argument Workhorses)** (Impact: 12.9)
...
**Top Internal Functions/Classes:**
  * `mpinit` **(I/O & Config Routines)** (Impact: 15.4)
...
**Top Internal Functions/Classes:**
  * `printf` **(Many-Argument Workhorses)** (Impact: 42.4)
    * *Intent:* // Print to the given fd. Only understands %d, %x, %p, %s.
  * `printint` **(Many-Argument Workhorses)** (Impact: 19.2)
...
**Top Internal Functions/Classes:**
  ...
  * `begin_op` **(Interface Declarations)** (Impact: 6.8)
  ...
  * `initlog` **(C Struct Operations)** (Impact: 3.5)
  * `install_trans` **(Interface Declarations)** (Impact: 2.7)
...
**Top Internal Functions/Classes:**
  ...
  * `morecore` **(Compute Cores)** (Impact: 5.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `main` **(State Mutators)** (Impact: 10.0)
...
**Top Internal Functions/Classes:**
  ...
  * `piperead` **(Many-Argument Workhorses)** (Impact: 13.1)
  * `pipeclose` **(Compute Cores)** (Impact: 11.2)
  * `pipewrite` **(Many-Argument Workhorses)** (Impact: 11.1)
    * *Intent:* //PAGEBREAK: 40
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `bread` **(C Struct Operations)** (Impact: 4.0)
    * *Intent:* // Return a locked buf with the contents of the indicated block.
  * `bwrite` **(Interface Declarations)** (Impact: 3.2)
    * *Intent:* // Write b's contents to disk. Must be locked.
  * `binit` **(Interface Declarations)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `argptr` **(C Struct Operations)** (Impact: 10.7)
  ...
  * `syscall` **(Interface Declarations)** (Impact: 5.8)
  * `fetchint` **(C Struct Operations)** (Impact: 5.7)
...
**Top Internal Functions/Classes:**
  ...
  * `popcli` **(Interface Declarations)** (Impact: 5.5)
  ...
  * `holding` **(Interface Declarations)** (Impact: 3.3)
