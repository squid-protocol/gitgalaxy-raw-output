...
> **File Composition:** Data / Markup / Trivial 43%, Interface Declarations Files 43%, Declarative / Non-Code 14%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 60.0 | 19.0 | 13.4 | 0.0 |
...
- `use_region` **(Many-Argument Workhorses)** (@ `smmap-5.0.3/smmap/mman.py`) -> Impact: **19.8** | LOC: 39
...
- `_collect_lru_region` **(Compute Cores)** (@ `smmap-5.0.3/smmap/mman.py`) -> Impact: **17.6** | LOC: 40
...
- `begin_access` **(Many-Argument Workhorses)** (@ `smmap-5.0.3/smmap/buf.py`) -> Impact: **16.0** | LOC: 27
...
- `__init__` **(Many-Argument Workhorses)** (@ `smmap-5.0.3/smmap/mman.py`) -> Impact: **12.8** | LOC: 32
...
- `__init__` **(Many-Argument Workhorses)** (@ `smmap-5.0.3/smmap/util.py`) -> Impact: **11.7** | LOC: 38
...
### 1. `smmap-5.0.3/smmap/buf.py` (PYTHON) -> Cumulative Risk: **577.36**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.59)
- **Magnitude:** 137.74 | **LOC:** 144 | **CtrlFlow:** 29.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (96.7018%), Verification (80.0%)
- **Heaviest Functions:** `__getslice__` (Many-Argument Workhorses, Impact: 19.6), `begin_access` (Many-Argument Workhorses, Impact: 16.0), `__getitem__` (Defensive Guards, Impact: 10.9)

### 2. `smmap-5.0.3/smmap/util.py` (PYTHON) -> Cumulative Risk: **539.03**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (85.0859%), Verification (80.0%)
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 11.7), `increment_client_count` (Defensive Guards, Impact: 7.5), `file_size` (Defensive Guards, Impact: 6.2)

### 3. `smmap-5.0.3/smmap/mman.py` (PYTHON) -> Cumulative Risk: **534.61**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (97.7682%), Verification (80.0%)
- **Heaviest Functions:** `_obtain_region` (Many-Argument Workhorses, Impact: 55.2), `use_region` (Many-Argument Workhorses, Impact: 19.8), `_collect_lru_region` (Compute Cores, Impact: 17.6)

### 4. `smmap-5.0.3/smmap/__init__.py` (PYTHON) -> Cumulative Risk: **256.83**
...
- **Primary Risk Drivers:** State Flux (99.8341%), Safety Score (85.6049%), Stability (50.0%), Cognitive Load (19.0934%)

### 5. `smmap-5.0.3/setup.py` (PYTHON) -> Cumulative Risk: **215.83**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +0.33)
- **Magnitude:** 21.96 | **LOC:** 53 | **CtrlFlow:** 4.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (91.6827%), Safety Score (62.5811%), Stability (50.0%), Cognitive Load (9.2672%)
...
- **Magnitude:** 426.44 | **LOC:** 589 | **CtrlFlow:** 20.6% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (46.2091%), Tech Debt (12.5845%)
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 12.8)
...
- **Magnitude:** 141.86 | **LOC:** 223 | **CtrlFlow:** 9.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (47.2081%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 11.7)
  ...
  * `file_size` **(Defensive Guards)** (Impact: 6.2)
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 137.74 | **LOC:** 144 | **CtrlFlow:** 29.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (40.4571%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `__getslice__` **(Many-Argument Workhorses)** (Impact: 19.6)
  * `begin_access` **(Many-Argument Workhorses)** (Impact: 16.0)
...
- **Magnitude:** 21.96 | **LOC:** 53 | **CtrlFlow:** 4.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (9.2672%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
