...
> **File Composition:** Declarative / Non-Code 21%, I/O & Config Routines Files 15%, Data / Markup / Trivial 12%, Interface Declarations Files 11%, Large Core Modules 10%
...
- **Heaviest Functions:** `craftInvokerFunction` (Defensive Guards, Impact: 65.0), `__embind_register_std_string` (Defensive Guards, Impact: 51.4), `genericPointerToWireType` (Defensive Guards, Impact: 41.5)
...
- **Heaviest Functions:** `loadSVGImage` (Callbacks & Closures, Impact: 5.1), `onload` (Callbacks & Closures, Impact: 1.4)

### 3. `scripts/jest/jest-global-setup.ts` (TYPESCRIPT) -> Cumulative Risk: **622.53**
- **Archetype:** `file_cluster_3` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.60)
...
- **Heaviest Functions:** `exports` (I/O & Config Routines, Impact: 5.9)
...
- **Heaviest Functions:** `load` (Many-Argument Workhorses, Impact: 53.4), `crossOrigin` (Compute Cores, Impact: 14.6), `determineCrossOrigin` (Compute Cores, Impact: 11.4)
...
- **Heaviest Functions:** `renderUpdate` (I/O & Config Routines, Impact: 9.6), `onclick` (Defensive Guards, Impact: 3.9), `addBunnies` (I/O & Config Routines, Impact: 3.6)

### 6. `src/rendering/renderers/gpu/GpuEncoderSystem.ts` (TYPESCRIPT) -> Cumulative Risk: **600.94**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.48)
...
- **Heaviest Functions:** `draw` (Compute Cores, Impact: 25.5), `_setShaderBindGroups` (Type Conversions, Impact: 7.7), `setGeometry` (State Mutators, Impact: 6.2)
...
- **Heaviest Functions:** `getEnabledRenderers` (Generic / Templated Code, Impact: 8.4), `runScene` (Many-Argument Workhorses, Impact: 6.8)

### 8. `src/rendering/renderers/canvas/utils/canvasUtils.ts` (TYPESCRIPT) -> Cumulative Risk: **599.46**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -1.18)
...
- **Heaviest Functions:** `getCanvasSource` (Compute Cores, Impact: 59.4), `tintWithPerPixel` (Many-Argument Workhorses, Impact: 23.4), `tintWithMultiply` (Many-Argument Workhorses, Impact: 21.4)
...
- **Heaviest Functions:** `getBitmapTextLayout` (Many-Argument Workhorses, Impact: 152.8), `checkIsOverflow` (Compute Cores, Impact: 61.6), `alignJustify` (State Mutators, Impact: 7.3)
...
- **Heaviest Functions:** `rotate` (Defensive Guards, Impact: 7.5), `reflect` (Type Conversions, Impact: 6.0), `project` (Type Conversions, Impact: 5.9)
...
**Top Internal Functions/Classes:**
  ...
  * `normalizeStringPosix` **(Compute Cores)** (Impact: 62.3)
...
**Top Internal Functions/Classes:**
  ...
  * `fill` **(Compute Cores)** (Impact: 21.3)
...
**Top Internal Functions/Classes:**
  ...
  * `startNewLine` **(Compute Cores)** (Impact: 57.4)
...
**Top Internal Functions/Classes:**
  ...
  * `_onPointerDown` **(Compute Cores)** (Impact: 11.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_setFillAndStrokeStyles` **(Many-Argument Workhorses)** (Impact: 12.5)
    * *Intent:* /** * Sets fill and stroke styles on the canvas context for text rendering. * @param context - The c...
  * `_getAlignmentOffset` **(Compute Cores)** (Impact: 8.7)
...
**Top Internal Functions/Classes:**
  ...
  * `fitBounds` **(Many-Argument Workhorses)** (Impact: 11.7)
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Compute Cores)** (Impact: 12.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_calculateFilterArea` **(Compute Cores)** (Impact: 16.0)
  ...
  * `_setupBindGroupsAndRender` **(Many-Argument Workhorses)** (Impact: 9.8)
...
**Top Internal Functions/Classes:**
  * `convertV7Tov8Style` **(Compute Cores)** (Impact: 53.0)
  ...
  * `_isFillStyle` **(Defensive Guards)** (Impact: 8.7)
  * `constructor` **(State Mutators)** (Impact: 8.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `destroyGpuRenderTarget` **(Compute Cores)** (Impact: 89.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_updateAccessibleObjects` **(Compute Cores)** (Impact: 11.3)
...
**Top Internal Functions/Classes:**
  ...
  * `alignJustify` **(State Mutators)** (Impact: 7.3)
...
- `src/extensions/Extensions.ts` -> **Severity: 14.857** (Embedded: 0.1857 * Error Risk: 80.0226%)
...
- `src/rendering/renderers/shared/texture/Texture.ts` -> **Severity: 13.56** (Embedded: 0.1507 * Error Risk: 90.01%)
- `src/rendering/renderers/shared/GCSystem.ts` -> **Severity: 11.368** (Embedded: 0.1271 * Error Risk: 89.421%)
