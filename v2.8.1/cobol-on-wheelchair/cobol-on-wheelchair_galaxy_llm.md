...
> **File Composition:** Data / Markup / Trivial 40%, Declarative / Non-Code 30%, I/O & Config Routines Files 30%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 66.4 | 10.2 | 0.0 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 87.6 | 32.9 | 0.0 | 0.0 |
...
- `file-control` **(I/O & Config Routines)** (@ `cow.cbl`) -> Impact: **31.6** | LOC: 252
- `start-readfile` **(I/O & Config Routines)** (@ `cowtemplate.cbl`) -> Impact: **6.7** | LOC: 53
- `file-control` **(I/O & Config Routines)** (@ `cowtemplate.cbl`) -> Impact: **4.1** | LOC: 42
- `web-header` **(I/O & Config Routines)** (@ `cow.cbl`) -> Impact: **2.3** | LOC: 26
- `RUN` **(I/O & Config Routines)** (@ `Dockerfile`) -> Impact: **2.1** | LOC: 2
- `RUN` **(I/O & Config Routines)** (@ `Dockerfile`) -> Impact: **2.1** | LOC: 3
- `start-checkfilestatus` **(I/O & Config Routines)** (@ `cowtemplate.cbl`) -> Impact: **2.1** | LOC: 3
- `RUN` **(I/O & Config Routines)** (@ `Dockerfile`) -> Impact: **1.1** | LOC: 3
- `CMD` **(I/O & Config Routines)** (@ `Dockerfile`) -> Impact: **1.1** | LOC: 1
- `ENTRYPOINT` **(I/O & Config Routines)** (@ `Dockerfile`) -> Impact: **1.1** | LOC: 1
...
|---|---|---|---|---|
| `__monolith__` | 6 | 228.76 | 20.06% | 13.66% |
| `controllers` | 3 | 57.18 | 9.25% | 0.0% |
| `tutorial` | 1 | 4.8 | 0.0% | 0.0% |

## 10. TARGETED STRUCTURAL SURFACE VECTORS (formerly Risk Vectors, Top 5 by Surface)
### Highest Debt Markers (formerly Tech Debt; Fragile/Planned)
- `cowtemplate.cbl` -> **66.4144%** Exposure
- `cow.cbl` -> **15.531%** Exposure
...
- `cowtemplate.cbl` -> **3** Orphaned Functions | **0** Duplicates
- `cow.cbl` -> **1** Orphaned Functions | **0** Duplicates
...
### 1. `cowtemplate.cbl` (COBOL) -> Cumulative Risk: **538.98**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.58)
- **Magnitude:** 118.1 | **LOC:** 152 | **CtrlFlow:** 32.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (99.4597%), Documentation (87.617%)
- **Heaviest Functions:** `start-readfile` (I/O & Config Routines, Impact: 6.7), `file-control` (I/O & Config Routines, Impact: 4.1), `start-checkfilestatus` (I/O & Config Routines, Impact: 2.1)

### 2. `cow.cbl` (COBOL) -> Cumulative Risk: **433.05**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.87)
- **Magnitude:** 83.72 | **LOC:** 365 | **CtrlFlow:** 12.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.8685%), Documentation (87.617%), Safety Score (79.2458%)
- **Heaviest Functions:** `file-control` (I/O & Config Routines, Impact: 31.6), `web-header` (I/O & Config Routines, Impact: 2.3), `bad-query-error` (Interface Declarations, Impact: 1.1)

### 3. `controllers/showsum.cbl` (COBOL) -> Cumulative Risk: **289.04**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +0.35)
- **Magnitude:** 23.56 | **LOC:** 51 | **CtrlFlow:** 7.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (96.0834%), Safety Score (71.9676%), Cognitive Load (15.1871%)

### 4. `controllers/showname.cbl` (COBOL) -> Cumulative Risk: **206.85**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +0.44)
- **Magnitude:** 18.38 | **LOC:** 38 | **CtrlFlow:** 10.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (62.5811%), State Flux (31.0026%), Cognitive Load (7.4468%)

### 5. `Dockerfile` (DOCKERFILE) -> Cumulative Risk: **199.07**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.53)
- **Magnitude:** 8.72 | **LOC:** 13 | **CtrlFlow:** 18.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (87.617%), Cognitive Load (5.5201%), Api Exposure (3.5258%)
- **Heaviest Functions:** `RUN` (I/O & Config Routines, Impact: 2.1), `RUN` (I/O & Config Routines, Impact: 2.1), `RUN` (I/O & Config Routines, Impact: 1.1)

### 6. `config.cbl` (COBOL) -> Cumulative Risk: **137.8**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Data / Markup / Trivial` (z +0.00)
- **Magnitude:** 16.08 | **LOC:** 11 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (68.9974%), Safety Score (66.5013%), Verification (2.2977%)

### 7. `controllers/index.cbl` (COBOL) -> Cumulative Risk: **107.42**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +0.50)
- **Magnitude:** 15.24 | **LOC:** 26 | **CtrlFlow:** 8.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Cognitive Load (5.1174%), Verification (2.2977%)

### 8. `downhill.sh` (SHELL) -> Cumulative Risk: **2.31**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Data / Markup / Trivial` (z +0.00)
- **Magnitude:** 1.14 | **LOC:** 4 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Verification (2.3138%)
...
- **Magnitude:** 118.1 | **LOC:** 152 | **CtrlFlow:** 32.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (78.7361%), Tech Debt (66.4144%)
**Top Internal Functions/Classes:**
  * `start-readfile` **(I/O & Config Routines)** (Impact: 6.7)
  * `file-control` **(I/O & Config Routines)** (Impact: 4.1)
  * `start-checkfilestatus` **(I/O & Config Routines)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 33 instances
* *State Mutation (weighted view):* 101
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 36`, `structural_boundaries: 24`, `args: 6`, `func_start: 3`, `class_start: 2`
* *Risk/State:* `high_risk_execution: 1`, `state_mutation: 35`, `unreferenced_by_name: 3`
...
- **Magnitude:** 83.72 | **LOC:** 365 | **CtrlFlow:** 12.9% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (36.1262%), Tech Debt (15.531%)
**Top Internal Functions/Classes:**
  * `file-control` **(I/O & Config Routines)** (Impact: 31.6)
  * `web-header` **(I/O & Config Routines)** (Impact: 2.3)
  * `bad-query-error` **(Interface Declarations)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 13 instances
* *State Mutation (weighted view):* 42
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 24`, `structural_boundaries: 47`, `args: 10`, `func_start: 3`, `class_start: 4`
* *Risk/State:* `state_mutation: 16`, `dead_code: 2`, `unreferenced_by_name: 1`
...
**Top Internal Functions/Classes:**
  ...
  * `CMD` **(I/O & Config Routines)** (Impact: 1.1)
  * `ENTRYPOINT` **(I/O & Config Routines)** (Impact: 1.1)
**Structural Signatures (Net Mitigated Signals):**
