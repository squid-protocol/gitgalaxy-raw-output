...
> **Composition Archetype:** `Typed Library` (z -0.54; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 27%, Data / Markup / Trivial 22%, Large Core Modules 15%, Declarative / Non-Code 11%, Defensive Guards Files 10%
...
- `parse_cert_ext` **(Compute Cores)** (@ `cryptography-46.0.6/src/rust/src/x509/certificate.rs`) -> Impact: **151.0** | LOC: 179
...
### 1. `cryptography-46.0.6/src/cryptography/x509/extensions.py` (PYTHON) -> Cumulative Risk: **762.08**
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 73.2), `__init__` (Many-Argument Workhorses, Impact: 66.5), `__init__` (Many-Argument Workhorses, Impact: 34.0)

### 2. `cryptography-46.0.6/src/cryptography/hazmat/decrepit/ciphers/algorithms.py` (PYTHON) -> Cumulative Risk: **733.53**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.97)
...
- **Heaviest Functions:** `__init__` (Compute Cores, Impact: 5.5), `__init__` (Parameter Forwarders, Impact: 1.8), `__init__` (Parameter Forwarders, Impact: 1.8)
...
- **Heaviest Functions:** `__init__` (Defensive Guards, Impact: 9.7), `__init__` (Defensive Guards, Impact: 6.4), `__init__` (Defensive Guards, Impact: 6.0)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 19.7), `__init__` (Many-Argument Workhorses, Impact: 8.0), `_common_args_checks` (Generic / Templated Code, Impact: 6.5)
...
- **Heaviest Functions:** `aes_key_unwrap_with_padding` (Many-Argument Workhorses, Impact: 26.2), `aes_key_unwrap` (Many-Argument Workhorses, Impact: 13.1), `aes_key_wrap_with_padding` (Many-Argument Workhorses, Impact: 11.2)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 21.4), `validate_for_algorithm` (Defensive Guards, Impact: 12.7), `_check_aes_key_length` (Generic / Templated Code, Impact: 8.9)

### 7. `cryptography-46.0.6/src/cryptography/hazmat/primitives/ciphers/algorithms.py` (PYTHON) -> Cumulative Risk: **709.68**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.90)
...
- **Heaviest Functions:** `__init__` (Encapsulated Accessors, Impact: 4.4), `__init__` (Parameter Forwarders, Impact: 1.8), `__init__` (Parameter Forwarders, Impact: 1.8)

### 8. `cryptography-46.0.6/src/cryptography/hazmat/bindings/openssl/binding.py` (PYTHON) -> Cumulative Risk: **675.94**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.38)
...
- **Heaviest Functions:** `build_conditional_library` (Generic / Templated Code, Impact: 9.5), `_verify_package_version` (Defensive Guards, Impact: 6.9), `_openssl_assert` (Defensive Guards, Impact: 3.5)
...
- **Heaviest Functions:** `__init__` (Defensive Guards, Impact: 12.9), `calculate_max_pss_salt_length` (Defensive Guards, Impact: 5.7), `__init__` (Defensive Guards, Impact: 5.1)

### 10. `cryptography-46.0.6/src/cryptography/x509/name.py` (PYTHON) -> Cumulative Risk: **665.11**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 41.8), `__init__` (Defensive Guards, Impact: 14.8), `_read_re` (Compute Cores, Impact: 10.7)
...
**Top Internal Functions/Classes:**
  ...
  * `test_ne` **(Defensive Guards)** (Impact: 6.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_generate_ca_and_leaf` **(Many-Argument Workhorses)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 73.2)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 66.5)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 34.0)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 33.9)
  * `__init__` **(Defensive Guards)** (Impact: 27.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `load_rsa_nist_vectors` **(Compute Cores)** (Impact: 32.5)
  * `load_kasvs_dh_vectors` **(Compute Cores)** (Impact: 32.3)
    * *Intent:* """ Loads data out of the KASVS key exchange vector data """
...
**Top Internal Functions/Classes:**
  ...
  * `test_rsa_encrypt_oaep_sha2` **(Many-Argument Workhorses)** (Impact: 8.4)
...
**Top Internal Functions/Classes:**
  * `parse_cert_ext` **(Compute Cores)** (Impact: 151.0)
...
**Top Internal Functions/Classes:**
  ...
  * `encode_certificate_policies` **(Compute Cores)** (Impact: 65.2)
...
**Top Internal Functions/Classes:**
  ...
  * `calculate_digest_and_algorithm` **(Generic / Templated Code)** (Impact: 25.4)
  * `py_int_to_bn` **(Generic / Templated Code)** (Impact: 11.1)
  * `bn_to_py_int` **(Generic / Templated Code)** (Impact: 4.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_skip_fips_format` **(Compute Cores)** (Impact: 14.8)
...
**Top Internal Functions/Classes:**
  ...
  * `symmetric_decrypt` **(Many-Argument Workhorses)** (Impact: 19.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 15 instances
* *State Mutation (weighted view):* 48
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `response_status` **(Compute Cores)** (Impact: 21.9)
  * `single_extensions` **(Compute Cores)** (Impact: 15.4)
  * `certificates` **(Many-Argument Workhorses)** (Impact: 12.2)
  * `load_der_ocsp_response` **(Many-Argument Workhorses)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
**Top Internal Functions/Classes:**
  ...
  * `not_valid_after` **(Compute Cores)** (Impact: 13.6)
...
**Top Internal Functions/Classes:**
  ...
  * `create_encryption_ctx` **(Many-Argument Workhorses)** (Impact: 25.4)
  * `finalize_with_tag` **(Many-Argument Workhorses)** (Impact: 24.2)
  * `update_into` **(Many-Argument Workhorses)** (Impact: 24.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parse_general_name` **(Compute Cores)** (Impact: 45.7)
  * `py_to_datetime` **(Compute Cores)** (Impact: 32.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_kbkdf_cmac_counter_mode_test` **(Many-Argument Workhorses)** (Impact: 9.0)
