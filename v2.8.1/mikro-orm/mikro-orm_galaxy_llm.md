...
> **File Composition:** Declarative / Non-Code 40%, State Mutators Files 14%, Data / Markup / Trivial 10%, Interface Declarations Files 8%, Large Core Modules 7%
...
- `isSimpleRegExp` **(Compute Cores)** (@ `packages/sql/src/query/QueryBuilderHelper.ts`) -> Impact: **395.6** | LOC: 983
...
- `rule` **(Compute Cores)** (@ `packages/sql/src/schema/SchemaComparator.ts`) -> Impact: **336.3** | LOC: 387
...
- **Heaviest Functions:** `getAllColumns` (Many-Argument Workhorses, Impact: 102.5), `getAllIndexes` (Many-Argument Workhorses, Impact: 78.5), `str` (Compute Cores, Impact: 61.5)
...
- **Composition Archetype:** `Defensive Guards Files` (z +1.45)
...
- **Heaviest Functions:** `getEntityClassOrSchema` (Many-Argument Workhorses, Impact: 33.8), `discoverEntities` (Defensive Guards, Impact: 20.1)
...
- **Heaviest Functions:** `getAllIndexes` (Many-Argument Workhorses, Impact: 122.3), `createTableColumn` (Compute Cores, Impact: 58.1), `getAllColumns` (Many-Argument Workhorses, Impact: 50.2)

### 4. `packages/sql/src/dialects/mysql/MySqlSchemaHelper.ts` (TYPESCRIPT) -> Cumulative Risk: **731.44**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.34)
...
- **Heaviest Functions:** `getAllIndexes` (Many-Argument Workhorses, Impact: 37.0), `loadViews` (Defensive Guards, Impact: 33.0), `getAllColumns` (Many-Argument Workhorses, Impact: 31.2)
...
- **Heaviest Functions:** `renameFields` (Many-Argument Workhorses, Impact: 126.6), `buildFields` (Many-Argument Workhorses, Impact: 42.6), `nativeUpdateMany` (Many-Argument Workhorses, Impact: 36.6)

### 6. `packages/core/src/utils/clone.ts` (TYPESCRIPT) -> Cumulative Risk: **717.29**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.91)
...
- **Heaviest Functions:** `clone` (Compute Cores, Impact: 73.7), `_clone` (Compute Cores, Impact: 61.0), `getPropertyDescriptor` (Generic / Templated Code, Impact: 5.9)
...
- **Heaviest Functions:** `nativeInsertMany` (Many-Argument Workhorses, Impact: 213.4), `nativeUpdateMany` (Many-Argument Workhorses, Impact: 189.3), `mapJoinedProps` (Many-Argument Workhorses, Impact: 160.2)
...
- **Heaviest Functions:** `runQuery` (Many-Argument Workhorses, Impact: 94.1), `createUpdatePayload` (Compute Cores, Impact: 36.5), `log` (Callbacks & Closures, Impact: 32.3)
...
- **Heaviest Functions:** `diffToSQL` (Compute Cores, Impact: 79.3), `execute` (Defensive Guards, Impact: 26.3), `clear` (Defensive Guards, Impact: 24.5)
...
- **Heaviest Functions:** `execute` (Generic / Templated Code, Impact: 12.5), `constructor` (Defensive Guards, Impact: 7.1), `executeQuery` (Generic / Templated Code, Impact: 6.4)
...
**Top Internal Functions/Classes:**
  ...
  * `addParams` **(Compute Cores)** (Impact: 127.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `initColumnType` **(Defensive Guards)** (Impact: 102.5)
...
**Top Internal Functions/Classes:**
  ...
  * `createBooksWithTags` **(Interface Declarations)** (Impact: 2.0)
  * `afterFlush` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `failHandler` **(Interface Declarations)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `isSimpleRegExp` **(Compute Cores)** (Impact: 395.6)
    * *Intent:* /** * Checks whether the RE can be rewritten to simple LIKE query */
  * `mapper` **(Many-Argument Workhorses)** (Impact: 151.1)
  * `isPrefixed` **(Compute Cores)** (Impact: 144.9)
...
**Top Internal Functions/Classes:**
  * `formula` **(Tests & Verification)** (Impact: 9.1)
  * `onFlush` **(Compute Cores)** (Impact: 6.6)
  * `onFlush` **(Interface Declarations)** (Impact: 5.0)
...
**Top Internal Functions/Classes:**
  ...
  * `getPropertyComparator` **(Compute Cores)** (Impact: 40.8)
**Contextual Mitigations & Amplifications:**
