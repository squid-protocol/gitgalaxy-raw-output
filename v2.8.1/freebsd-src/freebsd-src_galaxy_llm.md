...
| Analyzed Artifacts (Scanned) | 84672 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 25001 |
| Total LOC | 17556506 |
...
|---|---|---|
| Modularity | 0.8089 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Articulation Pts | 3806 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| C | 43330 | 12581197 | 51.2% |
| PLAINTEXT | 10102 | 8231 | 11.9% |
| CPP | 8588 | 2968379 | 10.1% |
| SHELL | 6444 | 398736 | 7.6% |
| MAKEFILE | 5961 | 229410 | 7.0% |
| YAML | 4594 | 388633 | 5.4% |
...
> **File Composition:** Declarative / Non-Code 30%, Data / Markup / Trivial 27%, Large Core Modules 10%, I/O & Config Routines Files 7%, C Struct Operations Files 6%
...
*Total Excluded Artifacts: 25001*
...
|---|---|---|---|---|
| memory | 4628325 | 44023 | 117 | `sys/netinet/tcp_stacks/rack.c` |
| cleanup | 51085 | 10787 | 1 | `contrib/unbound/util/configparser.c` |
| guards | 1024998 | 36926 | 30 | `contrib/llvm-project/clang/lib/Headers/altivec.h` |
| danger | 270771 | 20118 | 7 | `contrib/llvm-project/compiler-rt/lib/sanitizer_common/sanitizer_common_interceptors.inc` |
| concurrency | 36390 | 5382 | 0 | `sys/cam/ctl/ctl.c` |
| connectivity | 398847 | 38875 | 11 | `contrib/llvm-project/clang/lib/Headers/avx512vlintrin.h` |
...
| events | 23945 | 4918 | 0 | `crypto/openssh/configure.ac` |
...
| debt | 145079 | 19246 | 3 | `contrib/netbsd-tests/kernel/t_ptrace_wait.c` |
| mutation | 4167700 | 54690 | 118 | `contrib/one-true-awk/testdir/funstack.in` |
| dead_code | 185494 | 24928 | 4 | `sys/dev/sfxge/common/efx_regs_mcdi.h` |
| credential | 1460 | 122 | 0 | `contrib/expat/tests/nsalloc_tests.c` |
| threat | 203191 | 23884 | 4 | `sys/dev/cxgbe/firmware/t4fw_interface.h` |
...
| `crypto/openssl/apps` | 79 | 110414.21 | 63.81% | 8.57% |
...
- **Unknown Dependencies:** `350886` packages imported that bypass the Zero-Trust whitelist.
...
- **Heaviest Functions:** `wait_for_completion_interruptible_timeout` (Compute Cores, Impact: 17.4), `vchiq_thread_create` (Many-Argument Workhorses, Impact: 10.6), `wait_for_completion_interruptible` (Compute Cores, Impact: 8.1)
...
- **Heaviest Functions:** `midi_open` (Many-Argument Workhorses, Impact: 33.9), `midi_write` (Many-Argument Workhorses, Impact: 25.4), `midi_read` (Many-Argument Workhorses, Impact: 23.2)
...
- **Heaviest Functions:** `ichsmb_device_intr` (Compute Cores, Impact: 39.8), `ichsmb_bwrite` (Many-Argument Workhorses, Impact: 19.4), `ichsmb_bread` (Many-Argument Workhorses, Impact: 16.9)
...
- **Heaviest Functions:** `rman_reserve_resource` (Many-Argument Workhorses, Impact: 105.7), `rman_adjust_resource` (Many-Argument Workhorses, Impact: 78.0), `rman_manage_region` (Many-Argument Workhorses, Impact: 53.9)

### 5. `sys/dev/nvmf/controller/nvmft_qpair.c` (C) -> Cumulative Risk: **792.3**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `C Struct Operations Files` (z -0.30)
...
- **Heaviest Functions:** `nvmft_receive_capsule` (C Struct Operations, Impact: 12.1), `_nvmft_send_response` (C Struct Operations, Impact: 10.3), `nvmft_datamove_task` (C Struct Operations, Impact: 8.0)

### 6. `sys/dev/hid/hidraw.c` (C) -> Cumulative Risk: **783.04**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `C Struct Operations Files` (z -0.70)
...
- **Heaviest Functions:** `hidraw_ioctl` (Many-Argument Workhorses, Impact: 249.4), `hidraw_read` (Many-Argument Workhorses, Impact: 36.0), `hidraw_write` (Many-Argument Workhorses, Impact: 26.4)

### 7. `sys/dev/sound/pci/spicds.c` (C) -> Cumulative Risk: **781.53**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `spicds_set` (Many-Argument Workhorses, Impact: 101.0), `spicds_wrcd` (Many-Argument Workhorses, Impact: 35.1), `spicds_init` (Compute Cores, Impact: 13.6)

### 8. `crypto/openssh/regress/test-exec.sh` (SHELL) -> Cumulative Risk: **772.68**
...
- **Heaviest Functions:** `__global_context__` (Many-Argument Workhorses, Impact: 25.6), `Anonymous_Block` (Compute Cores, Impact: 22.8), `puttysetup` (Many-Argument Workhorses, Impact: 16.8)

### 9. `sys/dev/sound/pcm/mixer.c` (C) -> Cumulative Risk: **772.63**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `C Struct Operations Files` (z +0.63)
...
- **Heaviest Functions:** `mixer_ioctl_channel` (Many-Argument Workhorses, Impact: 90.0), `mixer_ioctl_cmd` (Many-Argument Workhorses, Impact: 89.8), `mixer_set` (Many-Argument Workhorses, Impact: 89.5)

### 10. `contrib/ofed/opensm/complib/cl_event.c` (C) -> Cumulative Risk: **770.67**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.10)
- **Magnitude:** 1.18 | **LOC:** 176 | **CtrlFlow:** 18.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9994%), Concurrency (99.8181%)
- **Heaviest Functions:** `cl_event_wait_on` (Many-Argument Workhorses, Impact: 35.2), `cl_event_signal` (Compute Cores, Impact: 5.1), `cl_event_destroy` (Compute Cores, Impact: 3.5)
...
**Top Internal Functions/Classes:**
  * `rack_output` **(Compute Cores)** (Impact: 1065.8)
...
**Top Internal Functions/Classes:**
  ...
  * `bbr_set_sockopt` **(Compute Cores)** (Impact: 319.1)
...
**Top Internal Functions/Classes:**
  ...
  * `bxe_alloc_hsi_mem` **(Compute Cores)** (Impact: 96.0)
  * `bxe_eioctl` **(Many-Argument Workhorses)** (Impact: 95.7)
  * `bxe_init_hw_common` **(Compute Cores)** (Impact: 95.0)
...
**Top Internal Functions/Classes:**
  ...
  * `sysctl_meminfo` **(Compute Cores)** (Impact: 137.5)
  * `t4_ioctl` **(Many-Argument Workhorses)** (Impact: 132.8)
  * `t4_attach` **(Compute Cores)** (Impact: 130.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `nfssvc_idname` **(Compute Cores)** (Impact: 139.9)
...
**Top Internal Functions/Classes:**
  * `ipf_checkl4sum` **(Compute Cores)** (Impact: 955.9)
...
**Top Internal Functions/Classes:**
  ...
  * `ocs_hw_init` **(Compute Cores)** (Impact: 159.4)
...
**Top Internal Functions/Classes:**
  ...
  * `sge_intr_handler` **(Many-Argument Workhorses)** (Impact: 63.0)
...
- `contrib/llvm-project/llvm/include/llvm/ADT/SmallVector.h` -> **Severity: 2.449** (Embedded: 0.0335 * Error Risk: 73.1012%)
- `contrib/llvm-project/llvm/include/llvm/Support/ErrorHandling.h` -> **Severity: 2.046** (Embedded: 0.0292 * Error Risk: 70.0%)
- `sys/sys/linker_set.h` -> **Severity: 2.031** (Embedded: 0.0282 * Error Risk: 71.9676%)
- `sys/sys/_iovec.h` -> **Severity: 1.867** (Embedded: 0.0289 * Error Risk: 64.5656%)
- `sys/sys/kernel.h` -> **Severity: 1.843** (Embedded: 0.0349 * Error Risk: 52.8008%)
...
- `sys/sys/malloc.h` -> **Severity: 215.9** (Blast Radius: 2.159 * Doc Risk: 100.0%)
