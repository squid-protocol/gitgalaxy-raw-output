...
> **File Composition:** Annotated Framework Methods Files 22%, Data / Markup / Trivial 22%, Tests & Verification Files 19%, Declarative / Non-Code 12%, Compute Cores Files 5%
...
- `parseParameter` **(Many-Argument Workhorses)** (@ `retrofit/src/main/java/retrofit2/RequestFactory.java`) -> Impact: **28.7** | LOC: 37
...
- `enqueue` **(Annotated Framework Methods)** (@ `retrofit-mock/src/main/java/retrofit2/mock/BehaviorCall.java`) -> Impact: **22.8** | LOC: 60
- `parseHeaders` **(Defensive Guards)** (@ `retrofit/src/main/java/retrofit2/RequestFactory.java`) -> Impact: **22.0** | LOC: 24
...
- **Heaviest Functions:** `enqueue` (Annotated Framework Methods, Impact: 22.8), `run` (Annotated Framework Methods, Impact: 12.7), `execute` (Generic / Templated Code, Impact: 5.5)
...
- **Heaviest Functions:** `canonicalizeForPath` (Many-Argument Workhorses, Impact: 21.1), `RequestBuilder` (Many-Argument Workhorses, Impact: 19.6), `get` (Compute Cores, Impact: 16.9)
...
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.03)
- **Magnitude:** 113.82 | **LOC:** 96 | **CtrlFlow:** 19.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `scrollNavigationToSelectedElement` (Callbacks & Closures, Impact: 8.8), `revealNavigationForCurrentPage` (Defensive Guards, Impact: 7.9), `revealParents` (Callbacks & Closures, Impact: 4.6)
...
- **Heaviest Functions:** `enqueue` (Defensive Guards, Impact: 10.5), `getRawCall` (Defensive Guards, Impact: 8.2), `parseResponse` (Defensive Guards, Impact: 6.1)

### 5. `retrofit/src/main/java/retrofit2/CompletableFutureCallAdapterFactory.java` (JAVA) -> Cumulative Risk: **581.42**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.52)
...
- **Heaviest Functions:** `onResponse` (Generic / Templated Code, Impact: 5.6), `cancel` (Annotated Framework Methods, Impact: 3.2), `CallCancelCompletableFuture` (Generic / Templated Code, Impact: 3.0)

### 6. `retrofit-adapters/java8/src/main/java/retrofit2/adapter/java8/Java8CallAdapterFactory.java` (JAVA) -> Cumulative Risk: **538.61**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.79)
...
- **Heaviest Functions:** `adapt` (Generic / Templated Code, Impact: 7.3), `onResponse` (Generic / Templated Code, Impact: 5.6), `adapt` (Generic / Templated Code, Impact: 4.2)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `build` (Interface Declarations, Impact: 3.2), `Builder` (Annotated Framework Methods, Impact: 3.1), `networkBehavior` (Annotated Framework Methods, Impact: 3.1)

### 8. `retrofit/src/main/java/retrofit2/Utils.java` (JAVA) -> Cumulative Risk: **527.02**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `resolve` (Many-Argument Workhorses, Impact: 69.5), `equals` (Defensive Guards, Impact: 38.6), `getGenericSupertype` (Compute Cores, Impact: 33.5)

### 9. `retrofit/src/main/java/retrofit2/KotlinExtensions.kt` (KOTLIN) -> Cumulative Risk: **518.36**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.73)
- **Magnitude:** 35.38 | **LOC:** 126 | **CtrlFlow:** 6.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.6481%), Concurrency (94.845%), Documentation (76.9231%)
- **Heaviest Functions:** `onResponse` (Generic / Templated Code, Impact: 9.7), `onResponse` (Generic / Templated Code, Impact: 5.5), `onFailure` (Generic / Templated Code, Impact: 1.9)
...
- **Heaviest Functions:** `enqueue` (Annotated Framework Methods, Impact: 10.7), `execute` (Annotated Framework Methods, Impact: 4.7), `FakeCall` (Generic / Templated Code, Impact: 3.8)
...
**Top Internal Functions/Classes:**
  * `multipartPartMapWithEncoding` **(Tests & Verification)** (Impact: 7.7)
  * `multipartWithEncoding` **(Tests & Verification)** (Impact: 7.6)
  * `malformedAnnotationRelativeUrlThrows` **(Tests & Verification)** (Impact: 6.8)
  * `malformedParameterRelativeUrlThrows` **(Tests & Verification)** (Impact: 6.8)
  * `headersDoNotOverwriteEachOther` **(Tests & Verification)** (Impact: 6.1)
...
**Top Internal Functions/Classes:**
  ...
  * `parseParameter` **(Many-Argument Workhorses)** (Impact: 28.7)
  * `parseHeaders` **(Defensive Guards)** (Impact: 22.0)
  * `boxIfPrimitive` **(Compute Cores)** (Impact: 16.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `unresolvableResponseTypeThrows` **(Tests & Verification)** (Impact: 16.1)
  * `unresolvableParameterTypeThrows` **(Tests & Verification)** (Impact: 16.1)
  * `callAdapterFactoryQueriedCanDelegateTwiceWithoutRecursion` **(Tests & Verification)** (Impact: 14.9)
  * `callAdapterFactoryQueriedCanDelegate` **(Tests & Verification)** (Impact: 12.2)
  * `callAdapterFactoryQueried` **(Tests & Verification)** (Impact: 11.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `conversionProblemIncomingMaskedByConverterIsUnwrapped` **(Tests & Verification)** (Impact: 3.8)
  * `fatalErrorsAreNotCaughtEnqueue` **(Tests & Verification)** (Impact: 3.5)
  * `requestThrowingBeforeEnqueueFailsEnqueue` **(Tests & Verification)** (Impact: 3.4)
  * `requestThrowingNonFatalErrorBeforeEnqueueFailsEnqueue` **(Tests & Verification)** (Impact: 3.4)
  * `requestAfterEnqueueFailingThrows` **(Tests & Verification)** (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `validateServiceInterface` **(Compute Cores)** (Impact: 18.5)
...
**Top Internal Functions/Classes:**
  ...
  * `apply` **(Compute Cores)** (Impact: 17.3)
  * `apply` **(Compute Cores)** (Impact: 13.5)
...
**Top Internal Functions/Classes:**
  * `canonicalizeForPath` **(Many-Argument Workhorses)** (Impact: 21.1)
  * `RequestBuilder` **(Many-Argument Workhorses)** (Impact: 19.6)
  * `get` **(Compute Cores)** (Impact: 16.9)
...
**Top Internal Functions/Classes:**
  * `bodyThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.6)
  * `responseThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.6)
  * `resultThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.8)
  * `bodyThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.5)
  * `responseThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `responseThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.6)
  * `resultThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.8)
  * `bodyThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.4)
  * `responseThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
* *Amplified Cascading Flux:* 14 instances
* *Concurrency (weighted view):* 77
...
- **Risk Profile:** Cognitive Load (100.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `resultThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.8)
  * `bodyThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.6)
  * `responseThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.6)
  * `bodyThrowingInOnSuccessDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.4)
  * `responseThrowingInOnSuccessDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
* *Amplified Cascading Flux:* 14 instances
* *Concurrency (weighted view):* 77
...
**Top Internal Functions/Classes:**
  ...
  * `bodyThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.6)
  * `responseThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.6)
  * `bodyThrowingInOnSuccessDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.3)
  * `responseThrowingInOnSuccessDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
* *Amplified Cascading Flux:* 14 instances
* *Concurrency (weighted view):* 77
...
- **Risk Profile:** Cognitive Load (100.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `setUp` **(Tests & Verification)** (Impact: 3.1)
  * `asyncCanBeCanceled` **(Tests & Verification)** (Impact: 2.8)
  * `asyncCanBeCanceledBeforeStart` **(Tests & Verification)** (Impact: 2.6)
  * `asyncFailureTriggersFailureAfterDelay` **(Tests & Verification)** (Impact: 2.5)
  * `asyncSuccessCalledAfterDelay` **(Tests & Verification)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
...
**Top Internal Functions/Classes:**
  * `bodyThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 5.8)
  * `responseThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 5.8)
  * `resultThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 5.0)
  * `bodyThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.7)
  * `responseThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `bodyThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.8)
  * `responseThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.8)
  * `resultThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.9)
  * `bodyThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.5)
  * `responseThrowingInOnCompleteDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
...
**Top Internal Functions/Classes:**
  ...
  * `bodyThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.8)
  * `responseThrowingInOnErrorDeliveredToPlugin` **(Tests & Verification)** (Impact: 4.8)
  * `bodyThrowingInOnSuccessDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.5)
  * `responseThrowingInOnSuccessDeliveredToPlugin` **(Tests & Verification)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
* *Amplified Cascading Flux:* 8 instances
* *Concurrency (weighted view):* 37
