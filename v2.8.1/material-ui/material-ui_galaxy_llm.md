...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +1.50; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 57%, Declarative / Non-Code 38%, Interface Declarations Files 1%, Callbacks & Closures Files 1%, Large Core Modules 1%
...
- `useAutocomplete` **(Compute Cores)** (@ `packages/mui-material/src/useAutocomplete/useAutocomplete.js`) -> Impact: **514.9** | LOC: 1248
...
- `getThemedComponents` **(Compute Cores)** (@ `packages-internal/core-docs/src/branding/brandingTheme.ts`) -> Impact: **302.5** | LOC: 1170
...
- **Heaviest Functions:** `runJscodeshiftTransform` (Many-Argument Workhorses, Impact: 30.5), `runPostcssTransform` (Defensive Guards, Impact: 16.4), `builder` (Compute Cores, Impact: 14.9)
...
- **Heaviest Functions:** `buildSingleProject` (Defensive Guards, Impact: 32.9), `buildApi` (Many-Argument Workhorses, Impact: 12.2), `removeOutdatedApiDocsTranslations` (Callbacks & Closures, Impact: 7.6)
...
- **Heaviest Functions:** `wait` (Defensive Guards, Impact: 8.1), `constructor` (State Mutators, Impact: 3.7), `process` (Callbacks & Closures, Impact: 2.9)
...
- **Heaviest Functions:** `InitCodeCopy` (Callbacks & Closures, Impact: 24.7), `CodeCopyProvider` (Compute Cores, Impact: 16.2), `handleClick` (Defensive Guards, Impact: 12.3)
...
- **Heaviest Functions:** `annotateComponentDefinition` (Many-Argument Workhorses, Impact: 114.0), `generateComponentApi` (Many-Argument Workhorses, Impact: 90.6), `attachPropsTable` (Defensive Guards, Impact: 78.6)
...
- **Heaviest Functions:** `annotateHookDefinition` (Many-Argument Workhorses, Impact: 119.8), `attachTranslations` (Defensive Guards, Impact: 43.5), `attachTable` (Defensive Guards, Impact: 43.0)
...
- **Heaviest Functions:** `retry` (Defensive Guards, Impact: 8.4), `bail` (Defensive Guards, Impact: 8.3)

### 8. `packages-internal/api-docs-builder/utils/extractInfoFromEnum.ts` (TYPESCRIPT) -> Cumulative Risk: **609.72**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -1.14)
- **Magnitude:** 68.44 | **LOC:** 71 | **CtrlFlow:** 19.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `extractInfoFromEnum` (Defensive Guards, Impact: 22.6), `parseProperty` (Defensive Guards, Impact: 6.7)
...
- **Heaviest Functions:** `parseTest` (Compute Cores, Impact: 21.7), `getRefInstance` (Compute Cores, Impact: 17.2), `findConformanceDescriptor` (Compute Cores, Impact: 10.1)
...
- **Heaviest Functions:** `Ad` (Compute Cores, Impact: 34.1), `render` (Interface Declarations, Impact: 2.5), `PleaseDisableAdblock` (I/O & Config Routines, Impact: 2.0)
...
**Top Internal Functions/Classes:**
  * `useAutocomplete` **(Compute Cores)** (Impact: 514.9)
...
**Top Internal Functions/Classes:**
  ...
  * `changeValue` **(Compute Cores)** (Impact: 33.9)
...
**Top Internal Functions/Classes:**
  ...
  * `startMaybeSwiping` **(Compute Cores)** (Impact: 131.1)
  ...
  * `getTranslate` **(Many-Argument Workhorses)** (Impact: 4.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `SelectInput` **(Many-Argument Workhorses)** (Impact: 217.4)
    * *Intent:* /** * @ignore - internal component. */
  * `onKeyUp` **(Compute Cores)** (Impact: 82.3)
...
**Top Internal Functions/Classes:**
  ...
  * `getTabsMeta` **(Defensive Guards)** (Impact: 35.5)
  * `getConditionalElements` **(Callbacks & Closures)** (Impact: 21.8)
  * `handleScrollButtonEnd` **(Compute Cores)** (Impact: 21.7)
...
**Top Internal Functions/Classes:**
  ...
  * `handleMouseOver` **(Callbacks & Closures)** (Impact: 13.9)
...
**Top Internal Functions/Classes:**
  ...
  * `attachColorScheme` **(Many-Argument Workhorses)** (Impact: 26.0)
...
**Top Internal Functions/Classes:**
  * `demoLoader` **(Compute Cores)** (Impact: 75.0)
...
**Top Internal Functions/Classes:**
  ...
  * `useRovingTabIndexItem` **(Callbacks & Closures)** (Impact: 19.6)
...
**Top Internal Functions/Classes:**
  * `wrapper` **(Tests & Verification)** (Impact: 39.8)
  * `root` **(Tests & Verification)** (Impact: 6.9)
  * `startScrollButtonIcon` **(Tests & Verification)** (Impact: 6.8)
  * `handleChange` **(Callbacks & Closures)** (Impact: 5.6)
  * `getBoundingClientRect` **(Tests & Verification)** (Impact: 5.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `onMouseDown` **(Compute Cores)** (Impact: 47.2)
...
**Top Internal Functions/Classes:**
  ...
  * `handleAutoFill` **(Compute Cores)** (Impact: 27.4)
...
- `packages/mui-material/src/styles/createTheme.ts` -> **Severity: 6.852** (Embedded: 0.0807 * Error Risk: 84.8883%)
- `packages/mui-utils/src/deepmerge/deepmerge.ts` -> **Severity: 4.866** (Embedded: 0.0555 * Error Risk: 87.6243%)
