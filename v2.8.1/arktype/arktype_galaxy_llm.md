...
> **Composition Archetype:** `Hub-Coupled App` (z +0.58; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 38%, Large Core Modules 13%, Data / Markup / Trivial 12%, Callbacks & Closures Files 11%, Generic / Templated Code Files 9%
...
- `constructor` **(Compute Cores)** (@ `ark/schema/node.ts`) -> Impact: **73.7** | LOC: 124
...
- `appendSpreadBranch` **(Many-Argument Workhorses)** (@ `ark/type/parser/tupleLiteral.ts`) -> Impact: **63.2** | LOC: 190
...
- **Heaviest Functions:** `_traverse` (Many-Argument Workhorses, Impact: 72.0), `structure` (Many-Argument Workhorses, Impact: 63.5), `get` (Defensive Guards, Impact: 62.1)
...
- **Heaviest Functions:** `_intersectSequences` (Compute Cores, Impact: 86.9), `reduce` (Defensive Guards, Impact: 54.9), `sequence` (Defensive Guards, Impact: 41.4)

### 3. `ark/type/parser/reduce/dynamic.ts` (TYPESCRIPT) -> Cumulative Risk: **627.81**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.72)
...
- **Heaviest Functions:** `pushRootToBranch` (Defensive Guards, Impact: 18.1), `previousOperator` (Defensive Guards, Impact: 9.7), `reduceLeftBound` (Compute Cores, Impact: 6.2)
...
- **Heaviest Functions:** `applyMorphsAtPath` (Compute Cores, Impact: 19.9), `finalize` (Compute Cores, Impact: 12.1), `applyQueuedMorphs` (I/O & Config Routines, Impact: 4.8)
...
- **Heaviest Functions:** `benchFn` (Many-Argument Workhorses, Impact: 15.7), `createAssertion` (Generic / Templated Code, Impact: 7.0), `createStatMethod` (Generic / Templated Code, Impact: 6.5)
...
- **Heaviest Functions:** `shiftUntilEscapable` (Compute Cores, Impact: 14.8), `shiftUntil` (Compute Cores, Impact: 6.1), `writeUnmatchedGroupCloseMessage` (Generic / Templated Code, Impact: 5.9)
...
- **Heaviest Functions:** `walkPaths` (Defensive Guards, Impact: 28.8), `cpR` (Compute Cores, Impact: 12.8), `findPackageRoot` (Defensive Guards, Impact: 9.5)
...
- **Heaviest Functions:** `constructor` (Compute Cores, Impact: 73.7), `_transform` (Compute Cores, Impact: 58.4), `writeSelectAssertionMessage` (Generic / Templated Code, Impact: 37.3)
...
- **Heaviest Functions:** `intersectConstraints` (Compute Cores, Impact: 28.1), `constraintKeyParser` (Generic / Templated Code, Impact: 11.8), `flattenConstraints` (Callbacks & Closures, Impact: 11.0)
...
- **Heaviest Functions:** `invoke` (Defensive Guards, Impact: 14.4), `referenceToId` (Defensive Guards, Impact: 14.1), `indexPropAccess` (C Struct Operations, Impact: 13.2)
...
**Top Internal Functions/Classes:**
  * `analyzeTypeInstantiations` **(Compute Cores)** (Impact: 25.3)
...
**Top Internal Functions/Classes:**
  ...
  * `getDuplicatesOf` **(Compute Cores)** (Impact: 21.6)
...
**Top Internal Functions/Classes:**
  * `snap` **(I/O & Config Routines)** (Impact: 18.7)
  * `toFile` **(Defensive Guards)** (Impact: 15.1)
  * `inline` **(Compute Cores)** (Impact: 14.2)
...
**Top Internal Functions/Classes:**
  * `appendSpreadBranch` **(Many-Argument Workhorses)** (Impact: 63.2)
  * `parseTupleLiteral` **(Compute Cores)** (Impact: 27.7)
...
**Top Internal Functions/Classes:**
  ...
  * `createAssertion` **(Generic / Templated Code)** (Impact: 7.0)
  * `createStatMethod` **(Generic / Templated Code)** (Impact: 6.5)
  * `loopAsyncCalls` **(Callbacks & Closures)** (Impact: 6.2)
  * `markAssertion` **(Callbacks & Closures)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `default` **(Callbacks & Closures)** (Impact: 6.6)
  * `value` **(Callbacks & Closures)** (Impact: 5.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
* *State Mutation (weighted view):* 6
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `preparseKey` **(Compute Cores)** (Impact: 39.7)
...
**Top Internal Functions/Classes:**
  ...
  * `parseBlock` **(Callbacks & Closures)** (Impact: 9.3)
...
**Top Internal Functions/Classes:**
  ...
  * `getSourceFileOrThrow` **(Annotated Framework Methods)** (Impact: 8.6)
...
**Top Internal Functions/Classes:**
  ...
  * `preparseOwnAliasEntry` **(Defensive Guards)** (Impact: 17.6)
  * `define` **(Generic / Templated Code)** (Impact: 14.9)
  * `parseOwnDefinitionFormat` **(Compute Cores)** (Impact: 14.8)
  * `preparseOwnDefinitionFormat` **(Defensive Guards)** (Impact: 5.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `never` **(Tests & Verification)** (Impact: 6.4)
...
**Top Internal Functions/Classes:**
  ...
  * `parseBranchTuple` **(Callbacks & Closures)** (Impact: 9.5)
  * `instanceof` **(Callbacks & Closures)** (Impact: 8.2)
  * `writeMalformedFunctionalExpressionMessage` **(Generic / Templated Code)** (Impact: 8.0)
  * `defineIndexOneParsers` **(Callbacks & Closures)** (Impact: 4.3)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `splitByKeys` **(I/O & Config Routines)** (Impact: 8.0)
...
**Top Internal Functions/Classes:**
  ...
  * `getSpreadVariadicElementsTuple` **(Callbacks & Closures)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
