...
> **File Composition:** Declarative / Non-Code 47%, Data / Markup / Trivial 22%, Interface Declarations Files 6%, Large Core Modules 6%, State Mutators Files 4%
...
| dead_code | 11892 | 4221 | 1 | `lib/include/lasxintrin.h` |
...
- `GetInstructionSize` **(Compute Cores)** (@ `lib/libtsan/interception/interception_win.cpp`) -> Impact: **515.4** | LOC: 435
...
- **Composition Archetype:** `Compute Cores Files` (z +0.63)
...
- **Heaviest Functions:** `__pthread_rwlock_timedwrlock` (Compute Cores, Impact: 28.2), `pthread_rwlock_wrlock` (Compute Cores, Impact: 18.9), `pthread_rwlock_destroy` (Compute Cores, Impact: 16.2)
...
- **Heaviest Functions:** `__aio_get_queue` (Defensive Guards, Impact: 40.5), `io_thread_func` (Compute Cores, Impact: 30.4), `__aio_atfork` (Compute Cores, Impact: 19.8)

### 3. `lib/libcxx/src/memory_resource.cpp` (CPP) -> Cumulative Risk: **714.46**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.64)
...
- **Heaviest Functions:** `unsynchronized_pool_resource::unsynchronized_pool_resource` (Compute Cores, Impact: 25.6), `__default_memory_resource` (Compute Cores, Impact: 22.5), `unsynchronized_pool_resource::do_allocate` (Compute Cores, Impact: 21.7)
...
- **Heaviest Functions:** `hashWithSeed` (Type Conversions, Impact: 12.5), `hashWithSeed` (Type Conversions, Impact: 11.9), `hashWithSeed` (Type Conversions, Impact: 10.1)
...
- **Heaviest Functions:** `sem_init` (Many-Argument Workhorses, Impact: 15.5), `__sem_timedwait` (C Struct Operations, Impact: 14.0), `sem_post_multiple` (Compute Cores, Impact: 13.7)
...
- **Heaviest Functions:** `pthread_create` (Many-Argument Workhorses, Impact: 65.8), `__dyn_tls_pthread` (Many-Argument Workhorses, Impact: 58.1), `pthread_cancel` (Compute Cores, Impact: 40.5)
...
- **Heaviest Functions:** `updateBuildStatus` (Compute Cores, Impact: 21.6), `updateStepStatus` (Compute Cores, Impact: 13.5), `fuzzUpdateCoverage` (Defensive Guards, Impact: 11.2)
...
- **Heaviest Functions:** `__timedwait_cp` (Many-Argument Workhorses, Impact: 30.9), `__futex4_cp` (Compute Cores, Impact: 23.2), `__timedwait` (Parameter Forwarders, Impact: 2.9)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `__lgammal_r` (Compute Cores, Impact: 73.4), `sin_pi` (Compute Cores, Impact: 10.9), `__lgammal_r` (Parameter Forwarders, Impact: 1.9)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `ns_parserr` (Many-Argument Workhorses, Impact: 45.4), `ns_initparse` (Many-Argument Workhorses, Impact: 17.4), `ns_skiprr` (Many-Argument Workhorses, Impact: 16.8)
...
**Top Internal Functions/Classes:**
  ...
  * `finish` **(Compute Cores)** (Impact: 68.8)
  * `format` **(Defensive Guards)** (Impact: 68.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `fmov` **(Defensive Guards)** (Impact: 51.8)
...
**Top Internal Functions/Classes:**
  ...
  * `simd_transpose` **(I/O & Config Routines)** (Impact: 7.0)
  * `simd_transpose` **(I/O & Config Routines)** (Impact: 7.0)
  * `simd_transpose` **(Compute Cores)** (Impact: 6.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parse_atom` **(Compute Cores)** (Impact: 116.8)
    * *Intent:* */
...
**Top Internal Functions/Classes:**
  * `_mm_mask_fmadd_pd` **(Many-Argument Workhorses)** (Impact: 2.7)
  * `_mm_mask3_fmadd_pd` **(Many-Argument Workhorses)** (Impact: 2.7)
  * `_mm_maskz_fmadd_pd` **(Many-Argument Workhorses)** (Impact: 2.7)
  * `_mm_mask_fmsub_pd` **(Many-Argument Workhorses)** (Impact: 2.7)
  * `_mm_maskz_fmsub_pd` **(Many-Argument Workhorses)** (Impact: 2.7)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.00035
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000112
