...
> **Composition Archetype:** `Typed Library` (z +0.82; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 59%, Data / Markup / Trivial 11%, Many-Argument Workhorses Files 9%, Declarative / Non-Code 5%, Large Core Modules 5%
...
- `__normalize_segments` **(Compute Cores)** (@ `pathspec-1.0.4/pathspec/patterns/gitignore/basic.py`) -> Impact: **44.4** | LOC: 91
...
- `__on_match` **(Many-Argument Workhorses)** (@ `pathspec-1.0.4/benchmarks/hyperscan_gitignore_r1.py`) -> Impact: **33.5** | LOC: 36
- `__on_match` **(Many-Argument Workhorses)** (@ `pathspec-1.0.4/benchmarks/hyperscan_gitignore_r1.py`) -> Impact: **33.5** | LOC: 36
...
- **Heaviest Functions:** `__on_match` (Many-Argument Workhorses, Impact: 33.5), `__on_match` (Many-Argument Workhorses, Impact: 33.5), `on_match` (Many-Argument Workhorses, Impact: 30.9)
...
- **Heaviest Functions:** `_init_db` (Many-Argument Workhorses, Impact: 13.2), `match_file` (Generic / Templated Code, Impact: 6.3), `match_file` (Generic / Templated Code, Impact: 6.2)
...
- **Heaviest Functions:** `_init_db` (Many-Argument Workhorses, Impact: 32.5), `__on_match` (Many-Argument Workhorses, Impact: 25.3), `on_match` (Many-Argument Workhorses, Impact: 23.3)
...
- **Heaviest Functions:** `pattern_to_regex` (Many-Argument Workhorses, Impact: 90.8), `_translate_segment_glob` (Compute Cores, Impact: 33.3), `escape` (Defensive Guards, Impact: 15.1)
...
- **Heaviest Functions:** `run_match` (Generic / Templated Code, Impact: 3.6), `bench_sm_gitignore_v1` (Generic / Templated Code, Impact: 2.5), `bench_sm_gitignore_v2` (Generic / Templated Code, Impact: 2.5)

### 6. `pathspec-1.0.4/pathspec/util.py` (PYTHON) -> Cumulative Risk: **578.14**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.61)
...
- **Heaviest Functions:** `_iter_tree_entries_next` (Many-Argument Workhorses, Impact: 30.8), `detailed_match_files` (Many-Argument Workhorses, Impact: 30.2), `_iter_tree_files_next` (Many-Argument Workhorses, Impact: 22.6)
...
- **Heaviest Functions:** `__normalize_segments` (Compute Cores, Impact: 44.4), `pattern_to_regex` (Many-Argument Workhorses, Impact: 38.8), `__translate_segments` (Many-Argument Workhorses, Impact: 27.6)
...
- **Heaviest Functions:** `from_lines` (Many-Argument Workhorses, Impact: 16.6), `__init__` (Many-Argument Workhorses, Impact: 13.3), `match_files` (Many-Argument Workhorses, Impact: 13.1)

### 9. `pathspec-1.0.4/pathspec/_backends/hyperscan/gitignore.py` (PYTHON) -> Cumulative Risk: **549.14**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.18)
...
- **Heaviest Functions:** `_init_db` (Many-Argument Workhorses, Impact: 40.9), `__on_match` (Many-Argument Workhorses, Impact: 25.6), `match_file` (Generic / Templated Code, Impact: 6.5)
...
- **Heaviest Functions:** `_init_set` (Many-Argument Workhorses, Impact: 35.7), `match_file` (Compute Cores, Impact: 21.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_05_match_tree_files` **(I/O & Config Routines)** (Impact: 4.6)
    * *Intent:* """ Test matching a file tree. """
...
**Top Internal Functions/Classes:**
  ...
  * `check_match_file` **(Many-Argument Workhorses)** (Impact: 20.1)
...
**Top Internal Functions/Classes:**
  * `__on_match` **(Many-Argument Workhorses)** (Impact: 33.5)
  * `__on_match` **(Many-Argument Workhorses)** (Impact: 33.5)
  * `on_match` **(Many-Argument Workhorses)** (Impact: 30.9)
  * `on_match` **(Many-Argument Workhorses)** (Impact: 30.9)
  * `match_file` **(Compute Cores)** (Impact: 24.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__on_match` **(Many-Argument Workhorses)** (Impact: 25.3)
...
**Top Internal Functions/Classes:**
  * `__normalize_segments` **(Compute Cores)** (Impact: 44.4)
...
**Top Internal Functions/Classes:**
  * `test_02_many` **(Compute Cores)** (Impact: 5.7)
...
**Top Internal Functions/Classes:**
  * `_init_db` **(Many-Argument Workhorses)** (Impact: 13.2)
  * `match_file` **(Generic / Templated Code)** (Impact: 6.3)
  * `match_file` **(Generic / Templated Code)** (Impact: 6.2)
  * `__on_match` **(Many-Argument Workhorses)** (Impact: 6.0)
  * `__on_match` **(Many-Argument Workhorses)** (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__on_match` **(Many-Argument Workhorses)** (Impact: 25.6)
  * `match_file` **(Generic / Templated Code)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  ...
  * `match_file` **(Compute Cores)** (Impact: 21.4)
    * *Intent:* """ Check the file against the patterns. *file* (:class:`str`) is the normalized file path to check....
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `match_file` **(Generic / Templated Code)** (Impact: 6.5)
    * *Intent:* """ Check the file against the patterns. *file* (:class:`str`) is the normalized file path to check....
  * `__on_match` **(Many-Argument Workhorses)** (Impact: 6.4)
  * `_make_db` **(Interface Declarations)** (Impact: 1.4)
...
**Top Internal Functions/Classes:**
  ...
  * `match_file` **(Generic / Templated Code)** (Impact: 6.3)
    * *Intent:* """ Check the file against the patterns. *file* (:class:`str`) is the normalized file path to check....
  * `_make_set` **(Interface Declarations)** (Impact: 1.4)
