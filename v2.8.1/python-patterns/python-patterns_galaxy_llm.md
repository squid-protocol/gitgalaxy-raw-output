...
> **Composition Archetype:** `Typed Library` (z +1.89; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 42%, Interface Declarations Files 17%, Data / Markup / Trivial 14%, Defensive Guards Files 12%, Declarative / Non-Code 5%
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 88.0 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 71.4 | 81.8 | 100.0 |
...
- `__init__` **(Compute Cores)** (@ `patterns/behavioral/catalog.py`) -> Impact: **7.6** | LOC: 14
...
### 1. `patterns/other/hsm/hsm.py` (PYTHON) -> Cumulative Risk: **610.58**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.38)
- **Magnitude:** 107.66 | **LOC:** 178 | **CtrlFlow:** 1.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.9976%), State Flux (93.6121%)
- **Heaviest Functions:** `on_message` (Compute Cores, Impact: 5.4), `__init__` (State Mutators, Impact: 2.4), `_next_state` (Defensive Guards, Impact: 2.0)

### 2. `patterns/structural/mvc.py` (PYTHON) -> Cumulative Risk: **599.21**
...
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (97.3403%), Safety Score (86.9243%), Verification (80.0%)
- **Heaviest Functions:** `resolve` (Generic / Templated Code, Impact: 7.2), `show_item_information` (Type Conversions, Impact: 5.0), `show_item_list` (Generic / Templated Code, Impact: 4.2)

### 3. `patterns/creational/builder.py` (PYTHON) -> Cumulative Risk: **577.63**
...
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), State Flux (99.1837%), Documentation (91.6667%)
- **Heaviest Functions:** `build_size` (Generic / Templated Code, Impact: 2.9), `construct_building` (Generic / Templated Code, Impact: 1.7), `__init__` (Generic / Templated Code, Impact: 1.6)

### 4. `patterns/behavioral/catalog.py` (PYTHON) -> Cumulative Risk: **554.35**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9995%), Tech Debt (99.9979%), Safety Score (86.7472%)
- **Heaviest Functions:** `__init__` (Compute Cores, Impact: 7.6), `__init__` (Generic / Templated Code, Impact: 7.3), `__init__` (Generic / Templated Code, Impact: 7.2)

### 5. `patterns/other/graph_search.py` (PYTHON) -> Cumulative Risk: **553.02**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.50)
- **Magnitude:** 148.68 | **LOC:** 160 | **CtrlFlow:** 33.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (99.8392%), Verification (80.0%)
- **Heaviest Functions:** `find_shortest_path_dfs` (Generic / Templated Code, Impact: 18.7), `find_shortest_path_bfs` (Many-Argument Workhorses, Impact: 15.8), `find_path_dfs` (Generic / Templated Code, Impact: 14.1)

### 6. `patterns/behavioral/memento.py` (PYTHON) -> Cumulative Risk: **547.92**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9932%), Safety Score (94.8548%), Documentation (80.6452%)
- **Heaviest Functions:** `memento` (Generic / Templated Code, Impact: 5.6), `rollback` (Generic / Templated Code, Impact: 3.0), `commit` (Generic / Templated Code, Impact: 2.9)

### 7. `patterns/other/blackboard.py` (PYTHON) -> Cumulative Risk: **511.68**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9844%), Documentation (85.7143%), Api Exposure (66.221%)
- **Heaviest Functions:** `run_loop` (Compute Cores, Impact: 6.2), `is_eager_to_contribute` (Generic / Templated Code, Impact: 4.3), `__init__` (Generic / Templated Code, Impact: 1.8)

### 8. `patterns/structural/flyweight_with_metaclass.py` (PYTHON) -> Cumulative Risk: **510.05**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (98.9013%), Safety Score (89.9735%)
- **Heaviest Functions:** `__call__` (Many-Argument Workhorses, Impact: 4.5), `__new__` (Many-Argument Workhorses, Impact: 2.8), `_serialize_params` (Type Conversions, Impact: 2.5)

### 9. `patterns/behavioral/visitor.py` (PYTHON) -> Cumulative Risk: **490.75**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.33)
- **Magnitude:** 33.76 | **LOC:** 76 | **CtrlFlow:** 14.3% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9254%), Safety Score (91.4175%), Documentation (77.7778%)
- **Heaviest Functions:** `visit` (Generic / Templated Code, Impact: 9.5), `generic_visit` (Generic / Templated Code, Impact: 2.3), `visit_B` (Generic / Templated Code, Impact: 2.3)

### 10. `patterns/behavioral/strategy.py` (PYTHON) -> Cumulative Risk: **488.72**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.25)
- **Magnitude:** 53.42 | **LOC:** 93 | **CtrlFlow:** 17.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.6316%), Documentation (91.6667%), Safety Score (69.4842%)
- **Heaviest Functions:** `__set__` (Generic / Templated Code, Impact: 8.2), `validate` (Defensive Guards, Impact: 5.7), `apply_discount` (Generic / Templated Code, Impact: 4.6)
...
- **Magnitude:** 148.68 | **LOC:** 160 | **CtrlFlow:** 33.3% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (44.7308%), Tech Debt (0.0%)
...
- **Global Archetype:** `file_cluster_0` (Drift: 0.0 IQR)
- **Magnitude:** 107.66 | **LOC:** 178 | **CtrlFlow:** 1.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (16.5765%), Tech Debt (99.9976%)
**Top Internal Functions/Classes:**
  * `on_message` **(Compute Cores)** (Impact: 5.4)
  * `__init__` **(State Mutators)** (Impact: 2.4)
  * `_next_state` **(Defensive Guards)** (Impact: 2.0)
...
- **Magnitude:** 93.4 | **LOC:** 217 | **CtrlFlow:** 14.2% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (27.8624%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Magnitude:** 87.28 | **LOC:** 176 | **CtrlFlow:** 16.5% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (49.1598%), Tech Debt (99.9979%)
**Top Internal Functions/Classes:**
  * `__init__` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `test_car_adapter_shall_make_very_loud_noise` **(Interface Declarations)** (Impact: 1.8)
  * `setUp` **(Interface Declarations)** (Impact: 1.7)
  * `test_dog_adapter_shall_make_noise` **(Interface Declarations)** (Impact: 1.7)
  * `test_cat_adapter_shall_make_noise` **(Interface Declarations)** (Impact: 1.7)
  * `test_human_adapter_shall_make_noise` **(Interface Declarations)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
...
- **Global Archetype:** `file_cluster_15` (Drift: 0.0 IQR)
- **Magnitude:** 60.54 | **LOC:** 146 | **CtrlFlow:** 10.6% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (48.0551%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  ...
  * `__get__` **(Defensive Guards)** (Impact: 2.7)
...
- **Risk Profile:** Cognitive Load (7.7835%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_0` (Drift: 0.0 IQR)
- **Magnitude:** 57.72 | **LOC:** 143 | **CtrlFlow:** 8.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (22.1602%), Tech Debt (99.9844%)
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 53.42 | **LOC:** 93 | **CtrlFlow:** 17.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (56.5102%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (29.7156%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (28.7507%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_given_standby_on_message_switchover_shall_call_hsm_methods` **(Tests & Verification)** (Impact: 2.1)
  * `test_calling_next_state_shall_change_current_state` **(Defensive Guards)** (Impact: 1.7)
  * `test_method_perform_switchover_shall_return_specifically` **(Interface Declarations)** (Impact: 1.7)
...
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 46.54 | **LOC:** 113 | **CtrlFlow:** 5.4% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (20.4236%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `build_size` **(Generic / Templated Code)** (Impact: 2.9)
  * `construct_building` **(Generic / Templated Code)** (Impact: 1.7)
  * `__init__` **(Generic / Templated Code)** (Impact: 1.6)
  * `build_floor` **(Interface Declarations)** (Impact: 1.5)
  * `build_size` **(Interface Declarations)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Risk Profile:** Cognitive Load (41.3495%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_0` (Drift: 0.0 IQR)
- **Magnitude:** 45.62 | **LOC:** 64 | **CtrlFlow:** 11.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (59.7342%), Tech Debt (98.9013%)
**Top Internal Functions/Classes:**
  * `__call__` **(Many-Argument Workhorses)** (Impact: 4.5)
  * `__new__` **(Many-Argument Workhorses)** (Impact: 2.8)
    * *Intent:* """ Set up object pool :param name: class name :param parents: class parents :param dct: dict: inclu...
  * `_serialize_params` **(Type Conversions)** (Impact: 2.5)
...
- **Risk Profile:** Cognitive Load (17.7172%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (37.7524%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (22.5083%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `__init__` **(Generic / Templated Code)** (Impact: 2.2)
...
- **Risk Profile:** Cognitive Load (39.3924%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  ...
  * `test_router_register_resolve_and_unknown` **(Tests & Verification)** (Impact: 1.4)
  * `test_productmodel_get_raises_keyerror` **(Tests & Verification)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `setUp` **(Interface Declarations)** (Impact: 1.9)
  * `test_extended_property_values_cloning` **(Callbacks & Closures)** (Impact: 1.7)
  * `test_cloning_propperty_innate_values` **(Tests & Verification)** (Impact: 1.6)
  * `test_cloning_propperty_assigned_values` **(Tests & Verification)** (Impact: 1.6)
  * `test_particular_properties_retrieving` **(Tests & Verification)** (Impact: 1.6)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (10.5409%), Tech Debt (37.7541%)
**Top Internal Functions/Classes:**
  ...
  * `get_current_time_as_html_fragment` **(Interface Declarations)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
