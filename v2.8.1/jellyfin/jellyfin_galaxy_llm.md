...
> **File Composition:** Declarative / Non-Code 25%, State Mutators Files 21%, Data / Markup / Trivial 13%, Large Core Modules 10%, Defensive Guards Files 7%
...
- **Heaviest Functions:** `GetEmbyRecordingsAsync` (Many-Argument Workhorses, Impact: 39.3), `GetTimers` (Callbacks & Closures, Impact: 26.6), `AddRecordingInfo` (Defensive Guards, Impact: 23.3)
...
- **Heaviest Functions:** `Normalize` (Many-Argument Workhorses, Impact: 45.8), `GetMediaSourcesInternal` (Many-Argument Workhorses, Impact: 23.1), `GetChannelStream` (Many-Argument Workhorses, Impact: 11.4)
...
- **Heaviest Functions:** `UpdateTimersForSeriesTimer` (Many-Argument Workhorses, Impact: 38.6), `CopyProgramInfoToTimerInfo` (Many-Argument Workhorses, Impact: 23.4), `CreateTimer` (Defensive Guards, Impact: 21.4)
...
- **Heaviest Functions:** `ParseLogLine` (Compute Cores, Impact: 44.8), `StartStreamingLog` (Defensive Guards, Impact: 15.9), `JobLogger` (State Mutators, Impact: 1.6)
...
- **Heaviest Functions:** `RefreshAllMetadata` (Many-Argument Workhorses, Impact: 25.2), `GetSeasonEpisodes` (Many-Argument Workhorses, Impact: 17.9), `FilterEpisodesBySeason` (Defensive Guards, Impact: 13.3)
...
- **Heaviest Functions:** `GetSearchResultsInternal` (Many-Argument Workhorses, Impact: 46.5), `ResultToMetadataResult` (Many-Argument Workhorses, Impact: 9.6), `GetResult` (Generic / Templated Code, Impact: 6.5)

### 7. `MediaBrowser.Providers/Plugins/Omdb/OmdbEpisodeProvider.cs` (CSHARP) -> Cumulative Risk: **641.08**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.09)
...
- **Heaviest Functions:** `GetMetadata` (Many-Argument Workhorses, Impact: 8.5), `OmdbEpisodeProvider` (State Mutators, Impact: 2.7), `GetSearchResults` (Generic / Templated Code, Impact: 1.9)
...
- **Heaviest Functions:** `GetChannelNumber` (Many-Argument Workhorses, Impact: 40.3), `GetChannelInfo` (Many-Argument Workhorses, Impact: 20.6), `GetChannelsAsync` (Many-Argument Workhorses, Impact: 19.8)

### 9. `Jellyfin.Server.Implementations/Activity/ActivityManager.cs` (CSHARP) -> Cumulative Risk: **629.06**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.27)
...
- **Heaviest Functions:** `GetPagedResultAsync` (Callbacks & Closures, Impact: 29.6), `ApplyOrdering` (Defensive Guards, Impact: 13.6), `MapOrderBy` (Callbacks & Closures, Impact: 3.5)
...
- **Heaviest Functions:** `ProcessResult` (Many-Argument Workhorses, Impact: 41.1), `GetMetadata` (Defensive Guards, Impact: 10.0), `DownloadInfo` (Compute Cores, Impact: 4.7)
...
**Top Internal Functions/Classes:**
  ...
  * `GetOptimalAudioStream` **(Defensive Guards)** (Impact: 69.8)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 1349.4 | **LOC:** 2689 | **CtrlFlow:** 14.0% | **Authorship Centralization:** 50.0%
...
**Top Internal Functions/Classes:**
  ...
  * `AddInheritedImages` **(Defensive Guards)** (Impact: 76.2)
...
**Top Internal Functions/Classes:**
  ...
  * `ReadFromDictNode` **(Compute Cores)** (Impact: 34.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `GetUserData` **(Many-Argument Workhorses)** (Impact: 17.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `RefreshMetadata` **(Many-Argument Workhorses)** (Impact: 68.3)
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 985.3 | **LOC:** 1882 | **CtrlFlow:** 17.1% | **Authorship Centralization:** 53.8%
...
**Top Internal Functions/Classes:**
  ...
  * `AddRecordingInfo` **(Defensive Guards)** (Impact: 23.3)
...
**Top Internal Functions/Classes:**
  ...
  * `FetchFromVideoNode` **(Compute Cores)** (Impact: 51.7)
  * `FetchThumbNode` **(Many-Argument Workhorses)** (Impact: 35.3)
...
**Top Internal Functions/Classes:**
  ...
  * `RenameUser` **(Defensive Guards)** (Impact: 10.6)
    * *Intent:* /// <inheritdoc/>
  * `GetAuthenticationProviders` **(Defensive Guards)** (Impact: 9.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `FetchFromSharesNode` **(Compute Cores)** (Impact: 25.0)
  * `FetchDataFromPersonsNode` **(Compute Cores)** (Impact: 22.4)
    * *Intent:* /// <summary> /// Fetches the data from persons node. /// </summary> /// <param name="reader">The re...
  * `FetchFromTagsNode` **(Compute Cores)** (Impact: 19.1)
  * `FetchFromTaglinesNode` **(Compute Cores)** (Impact: 18.9)
...
**Top Internal Functions/Classes:**
  ...
  * `SaveImage` **(Many-Argument Workhorses)** (Impact: 23.0)
    * *Intent:* /// <inheritdoc/>
  * `CanRefreshMetadata` **(Many-Argument Workhorses)** (Impact: 21.1)
...
**Top Internal Functions/Classes:**
  ...
  * `GetMediaFolders` **(Defensive Guards)** (Impact: 11.4)
  * `GetTvView` **(Many-Argument Workhorses)** (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `GetBindAddress` **(Defensive Guards)** (Impact: 31.9)
    * *Intent:* /// <inheritdoc/>
...
**Top Internal Functions/Classes:**
  ...
  * `ExtractAllExtractableSubtitlesInternal` **(Many-Argument Workhorses)** (Impact: 26.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 7 instances
...
- `MediaBrowser.Controller/Entities/BaseItem.cs` -> Churn: **68.74%** | Cog Load: 24.6003% | Debt: 75.7351%
