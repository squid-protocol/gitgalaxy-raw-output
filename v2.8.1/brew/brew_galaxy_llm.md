...
> **Composition Archetype:** `Small Flat Repo` (z -1.29; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 29%, Large Core Modules 22%, Data / Markup / Trivial 21%, I/O & Config Routines Files 5%, State Mutators Files 4%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 74.5 | 14.8 | 6.7 | 0.0 |
...
- **Composition Archetype:** `Compute Cores Files` (z +1.17)
...
- **Heaviest Functions:** `wrap_then_do` (Compute Cores, Impact: 31.9), `format` (Compute Cores, Impact: 31.8), `no_forbidden_pattern` (Many-Argument Workhorses, Impact: 17.1)
...
- **Heaviest Functions:** `setup-ruby-path` (I/O & Config Routines, Impact: 24.5), `need_vendored_ruby` (I/O & Config Routines, Impact: 5.5), `can_use_ruby_from_path` (I/O & Config Routines, Impact: 4.4)
...
- **Heaviest Functions:** `opoo` (Compute Cores, Impact: 10.6), `onoe` (Compute Cores, Impact: 10.6), `ohai` (Compute Cores, Impact: 7.5)
...
- **Heaviest Functions:** `install_bundler_gems!` (Many-Argument Workhorses, Impact: 54.4), `install_gem!` (Many-Argument Workhorses, Impact: 15.3), `setup_gem_environment!` (Compute Cores, Impact: 10.2)

### 5. `Library/Homebrew/utils/github.rb` (RUBY) -> Cumulative Risk: **598.74**
...
- **Heaviest Functions:** `create_bump_pr` (Many-Argument Workhorses, Impact: 59.3), `check_for_duplicate_pull_requests` (Many-Argument Workhorses, Impact: 34.9), `fetch_pull_requests` (Many-Argument Workhorses, Impact: 23.0)
...
- **Heaviest Functions:** `curl_check_http_content` (Many-Argument Workhorses, Impact: 147.6), `curl_http_content_headers_and_checksum` (Many-Argument Workhorses, Impact: 43.9), `curl_with_workarounds` (Many-Argument Workhorses, Impact: 35.6)

### 7. `bin/brew` (SHELL) -> Cumulative Risk: **535.59**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.85)
...
- **Heaviest Functions:** `export_homebrew_env_file` (Compute Cores, Impact: 9.6), `symlink_target_directory` (Compute Cores, Impact: 9.4), `Anonymous_Block` (Defensive Guards, Impact: 9.1)

### 8. `Library/Homebrew/rust/brew-rs/src/utils/formatter.rs` (RUST) -> Cumulative Risk: **527.37**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.53)
- **Magnitude:** 36.54 | **LOC:** 99 | **CtrlFlow:** 3.4% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (96.257%)
- **Heaviest Functions:** `ohai` (Interface Declarations, Impact: 4.6), `prefix` (Many-Argument Workhorses, Impact: 4.5), `headline` (State Mutators, Impact: 2.3)

### 9. `Library/Homebrew/rust/brew-rs/src/homebrew.rs` (RUST) -> Cumulative Risk: **521.14**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
...
- **Heaviest Functions:** `print_sections` (Compute Cores, Impact: 15.0), `list_directories` (Generic / Templated Code, Impact: 9.6), `env_bool` (Defensive Guards, Impact: 5.1)

### 10. `Library/Homebrew/utils/github/api.rb` (RUBY) -> Cumulative Risk: **518.02**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `open_rest` (Many-Argument Workhorses, Impact: 31.9), `raise_error` (Many-Argument Workhorses, Impact: 19.2), `credentials_error_message` (Compute Cores, Impact: 15.1)
...
**Top Internal Functions/Classes:**
  ...
  * `load_formula_json` **(Many-Argument Workhorses)** (Impact: 35.3)
...
**Top Internal Functions/Classes:**
  ...
  * `pypi_info` **(Compute Cores)** (Impact: 17.7)
  * `basic_metadata` **(Defensive Guards)** (Impact: 17.7)
...
- **Global Archetype:** `file_cluster_10` (Drift: 0.0 IQR)
- **Magnitude:** 381.42 | **LOC:** 516 | **CtrlFlow:** 18.9% | **Authorship Centralization:** 33.3%
...
**Top Internal Functions/Classes:**
  * `resolve_formula_names` **(Compute Cores)** (Impact: 15.1)
...
**Top Internal Functions/Classes:**
  ...
  * `pretty_duration` **(Interface Declarations)** (Impact: 6.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `odie_if_defined` **(I/O & Config Routines)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `Anonymous_Block` **(I/O & Config Routines)** (Impact: 5.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parse_license_expression` **(Defensive Guards)** (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `extract_version_from_url` **(Interface Declarations)** (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  ...
  * `number_readable` **(Callbacks & Closures)** (Impact: 4.5)
  * `truncate` **(Many-Argument Workhorses)** (Impact: 4.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `need_vendored_ruby` **(I/O & Config Routines)** (Impact: 5.5)
    * *Intent:* # HOMEBREW_FORCE_VENDOR_RUBY is from the user environment # shellcheck disable=SC2154
  * `can_use_ruby_from_path` **(I/O & Config Routines)** (Impact: 4.4)
...
**Top Internal Functions/Classes:**
  ...
  * `prepend_path_in_profile` **(I/O & Config Routines)** (Impact: 6.4)
...
- `Library/Homebrew/rust/brew-rs/src/utils/tty.rs` -> Churn: **57.97%** | Cog Load: 10.2474% | Debt: 85.0995%
