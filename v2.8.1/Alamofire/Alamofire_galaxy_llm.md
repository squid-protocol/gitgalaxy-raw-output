...
> **Composition Archetype:** `Small Flat Repo` (z -0.52; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 41%, Tests & Verification Files 15%, Large Core Modules 13%, Interface Declarations Files 8%, Declarative / Non-Code 8%
...
- `encode` **(Many-Argument Workhorses)** (@ `Source/Core/ParameterEncoding.swift`) -> Impact: **96.6** | LOC: 200
...
- `set` **(Defensive Guards)** (@ `Source/Features/URLEncodedFormEncoder.swift`) -> Impact: **54.5** | LOC: 51
  * *Intent:* /// Recursive backing method to `set(to:at:)`.
- `init` **(Many-Argument Workhorses)** (@ `Source/Core/Session.swift`) -> Impact: **41.3** | LOC: 32
...
- `urlSession` **(Many-Argument Workhorses)** (@ `Source/Core/SessionDelegate.swift`) -> Impact: **34.9** | LOC: 27
- `init` **(Many-Argument Workhorses)** (@ `Source/Core/Session.swift`) -> Impact: **30.5** | LOC: 33
  * *Intent:* /// careful testing and profiling. `nil` by default. /// - serializationQueue: `DispatchQueue` on which to perform all response serialization. By defa...
- `serialize` **(Defensive Guards)** (@ `Source/Features/ResponseSerialization.swift`) -> Impact: **30.4** | LOC: 27
- `append` **(Many-Argument Workhorses)** (@ `Source/Features/MultipartFormData.swift`) -> Impact: **30.3** | LOC: 70
...
- `urlSession` **(Defensive Guards)** (@ `Source/Core/SessionDelegate.swift`) -> Impact: **30.0** | LOC: 13
- `prepare` **(Defensive Guards)** (@ `Example/Source/MasterViewController.swift`) -> Impact: **29.4** | LOC: 33
  * *Intent:* // MARK: - UIStoryboardSegue
- `executeMultipartFormDataUploadRequestWithProgress` **(Defensive Guards)** (@ `Tests/UploadTests.swift`) -> Impact: **28.8** | LOC: 67
...
- **Heaviest Functions:** `init` (Many-Argument Workhorses, Impact: 13.6), `streamSerializer` (Generic / Templated Code, Impact: 13.4), `close` (Defensive Guards, Impact: 11.6)
...
- **Heaviest Functions:** `set` (Defensive Guards, Impact: 54.5), `convert` (Many-Argument Workhorses, Impact: 14.7), `encode` (Compute Cores, Impact: 13.5)
...
- **Heaviest Functions:** `_response` (Many-Argument Workhorses, Impact: 23.4), `init` (Many-Argument Workhorses, Impact: 12.9), `updateDownloadProgress` (Defensive Guards, Impact: 10.4)
...
- **Heaviest Functions:** `request` (Defensive Guards, Impact: 6.3), `publishString` (Generic / Templated Code, Impact: 5.5), `publishString` (Generic / Templated Code, Impact: 5.5)

### 5. `Source/Features/MultipartUpload.swift` (SWIFT) -> Cumulative Risk: **508.95**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.32)
...
- **Heaviest Functions:** `build` (Interface Declarations, Impact: 5.3), `init` (Generic / Templated Code, Impact: 2.4), `asURLRequest` (Callbacks & Closures, Impact: 1.4)
...
- **Heaviest Functions:** `init` (Many-Argument Workhorses, Impact: 13.7), `task` (Defensive Guards, Impact: 12.7), `inputStream` (Defensive Guards, Impact: 5.7)
...
- **Heaviest Functions:** `cURLDescription` (Defensive Guards, Impact: 27.4), `appendResponseSerializer` (Defensive Guards, Impact: 12.4), `init` (Many-Argument Workhorses, Impact: 12.1)
...
- **Heaviest Functions:** `init` (Many-Argument Workhorses, Impact: 13.6), `didReceiveResponse` (Defensive Guards, Impact: 9.8), `validate` (Annotated Framework Methods, Impact: 8.1)
...
- **Heaviest Functions:** `adapt` (Defensive Guards, Impact: 24.0), `retry` (Many-Argument Workhorses, Impact: 19.6), `refresh` (Many-Argument Workhorses, Impact: 15.2)
...
- **Heaviest Functions:** `adapt` (Defensive Guards, Impact: 23.8), `deflate` (Type Conversions, Impact: 1.8), `adler32Checksum` (Callbacks & Closures, Impact: 1.7)
...
**Top Internal Functions/Classes:**
  * `testThatDataStreamWorksCorrectlyWithMultipleSerialQueues` **(Tests & Verification)** (Impact: 27.9)
  * `testThatDataStreamWorksCorrectlyWithMultipleConcurrentQueues` **(Tests & Verification)** (Impact: 26.6)
  * `testThatDataCanBeStreamedAndDecodedAtTheSameTime` **(Tests & Verification)** (Impact: 24.7)
  * `testThatDataCanBeStreamedManyTimes` **(Tests & Verification)** (Impact: 21.6)
  * `testThatDataStreamCanBeCancelledInClosure` **(Defensive Guards)** (Impact: 17.5)
...
**Top Internal Functions/Classes:**
  * `retry` **(Many-Argument Workhorses)** (Impact: 19.4)
  * `executeAuthorizationHeaderTest` **(Tests & Verification)** (Impact: 19.1)
  * `testThatSessionCallsRequestRetrierForAllResponseSerializersThatThrowError` **(Tests & Verification)** (Impact: 16.7)
  * `testThatDownloadRequestWithInvalidURLStringThrowsResponseHandlerError` **(Tests & Verification)** (Impact: 14.3)
  * `testThatRetriedRequestsCanBeMassCancelled` **(Tests & Verification)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  * `init` **(Many-Argument Workhorses)** (Impact: 41.3)
    * *Intent:* /// careful testing and profiling. `nil` by default. /// - serializationQueue: `DispatchQueue` on wh...
  * `init` **(Many-Argument Workhorses)** (Impact: 30.5)
    * *Intent:* /// careful testing and profiling. `nil` by default. /// - serializationQueue: `DispatchQueue` on wh...
  * `download` **(Many-Argument Workhorses)** (Impact: 23.0)
  ...
  * `streamRequest` **(Many-Argument Workhorses)** (Impact: 20.0)
...
**Top Internal Functions/Classes:**
  * `testRequestResponseWithProgress` **(Tests & Verification)** (Impact: 14.1)
  * `testPOSTRequestWithJSONParametersCURLDescription` **(Tests & Verification)** (Impact: 12.7)
  * `testThatDataRequestOnHTTPResponseCanCancel` **(Tests & Verification)** (Impact: 12.6)
  * `testPOSTRequestWithBase64EncodedImages` **(Tests & Verification)** (Impact: 12.4)
  * `testThatDataRequestOnHTTPResponseCanAllow` **(Tests & Verification)** (Impact: 11.5)
**Contextual Mitigations & Amplifications:**
