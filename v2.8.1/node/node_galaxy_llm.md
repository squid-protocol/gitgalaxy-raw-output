...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.18; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 24%, Data / Markup / Trivial 21%, Interface Declarations Files 15%, Large Core Modules 11%, State Mutators Files 8%
...
| connectivity | 92247 | 10289 | 6 | `deps/v8/test/mjsunit/harmony/modules-skip-large2.mjs` |
...
| threat | 87315 | 9947 | 3 | `deps/openssl/openssl/include/openssl/cryptoerr_legacy.h` |
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 415.5), `defaultEval` (Many-Argument Workhorses, Impact: 105.6), `_handleError` (Defensive Guards, Impact: 60.0)

### 2. `deps/npm/lib/commands/team.js` (JAVASCRIPT) -> Cumulative Risk: **770.16**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.12)
...
- **Heaviest Functions:** `listUsers` (Defensive Guards, Impact: 16.3), `listTeams` (Defensive Guards, Impact: 16.3), `exec` (Compute Cores, Impact: 15.4)
...
- **Heaviest Functions:** `benchWebStream` (Many-Argument Workhorses, Impact: 10.2), `benchIter` (Many-Argument Workhorses, Impact: 10.1), `benchClassic` (Many-Argument Workhorses, Impact: 10.0)
...
- **Heaviest Functions:** `exec` (Compute Cores, Impact: 15.0), `set` (Compute Cores, Impact: 11.6), `get` (Defensive Guards, Impact: 9.7)
...
- **Heaviest Functions:** `uv__io_poll` (Many-Argument Workhorses, Impact: 87.2), `uv_cpu_info` (Many-Argument Workhorses, Impact: 72.0), `uv__iou_init` (Many-Argument Workhorses, Impact: 42.1)
...
- **Heaviest Functions:** `benchIter` (Many-Argument Workhorses, Impact: 10.1), `benchClassic` (Many-Argument Workhorses, Impact: 10.0), `benchWebStream` (Many-Argument Workhorses, Impact: 10.0)
...
- **Heaviest Functions:** `writer` (Compute Cores, Impact: 155.1), `readFileHandle` (Defensive Guards, Impact: 56.5), `pull` (Defensive Guards, Impact: 48.0)
...
- **Heaviest Functions:** `benchClassic` (Many-Argument Workhorses, Impact: 23.9), `benchWebStream` (Many-Argument Workhorses, Impact: 19.0), `benchIter` (Many-Argument Workhorses, Impact: 16.4)

### 9. `deps/npm/lib/commands/shrinkwrap.js` (JAVASCRIPT) -> Cumulative Risk: **758.19**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z -0.64)
...
- **Heaviest Functions:** `exec` (I/O & Config Routines, Impact: 18.9)
...
- **Heaviest Functions:** `benchClassic` (Many-Argument Workhorses, Impact: 10.4), `benchWebStream` (Many-Argument Workhorses, Impact: 10.3), `benchIter` (Defensive Guards, Impact: 10.0)
...
**Top Internal Functions/Classes:**
  * `_malloc` **(Compute Cores)** (Impact: 609.8)
...
**Top Internal Functions/Classes:**
  * `Simulator::DecodeTypeImmediate` **(Compute Cores)** (Impact: 318.6)
  ...
  * `Simulator::DecodeTypeRegisterSPECIAL` **(Compute Cores)** (Impact: 262.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.8e-05 | `Ripple Effect (Closeness):` 0.00597
...
**Top Internal Functions/Classes:**
  ...
  * `psa_key_derivation_set_maximum_capacity` **(Compute Cores)** (Impact: 61.4)
...
**Top Internal Functions/Classes:**
  ...
  * `DisassemblingDecoder::SubstituteImmediateField` **(Compute Cores)** (Impact: 135.8)
  * `DisassemblingDecoder::SubstituteRegisterField` **(Compute Cores)** (Impact: 121.6)
**Contextual Mitigations & Amplifications:**
...
- `deps/LIEF/third-party/frozen/include/frozen/algorithm.h` -> **Severity: 5.012** (Embedded: 0.0532 * Error Risk: 94.1152%)
- `deps/v8/src/base/hashing.h` -> **Severity: 4.625** (Embedded: 0.0516 * Error Risk: 89.7035%)
- `deps/v8/src/base/bits.h` -> **Severity: 3.775** (Embedded: 0.0465 * Error Risk: 81.2251%)
- `deps/v8/src/base/vector.h` -> **Severity: 3.436** (Embedded: 0.0648 * Error Risk: 53.0471%)
- `deps/v8/src/base/macros.h` -> **Severity: 3.267** (Embedded: 0.0643 * Error Risk: 50.8042%)
