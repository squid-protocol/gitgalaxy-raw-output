...
> **File Composition:** Data / Markup / Trivial 46%, Callbacks & Closures Files 15%, Large Core Modules 11%, Compute Cores Files 6%, Interface Declarations Files 5%
...
| connectivity | 97987 | 10218 | 13 | `staging/src/k8s.io/api/core/v1/types.go` |
...
- `TestKubelet_HandlePodCleanups` **(Compute Cores)** (@ `pkg/kubelet/kubelet_pods_test.go`) -> Impact: **285.1** | LOC: 1092
...
- **Composition Archetype:** `I/O & Config Routines Files` (z +2.08)
...
- **Heaviest Functions:** `kube::golang::build_binaries_for_platform` (Compute Cores, Impact: 31.1), `kube::golang::build_binaries` (I/O & Config Routines, Impact: 26.4), `kube::golang::is_statically_linked` (Compute Cores, Impact: 17.7)
...
- **Heaviest Functions:** `newTerminationOrdering` (Compute Cores, Impact: 16.5), `waitForTurn` (Compute Cores, Impact: 11.4), `containerTerminated` (Callbacks & Closures, Impact: 3.2)

### 3. `hack/update-codegen.sh` (SHELL) -> Cumulative Risk: **734.99**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.71)
...
- **Heaviest Functions:** `codegen::openapi` (Compute Cores, Impact: 19.7), `codegen::validation` (Compute Cores, Impact: 17.2), `codegen::conversions` (Compute Cores, Impact: 16.8)

### 4. `hack/lib/etcd.sh` (SHELL) -> Cumulative Risk: **726.46**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +1.02)
- **Magnitude:** 139.56 | **LOC:** 193 | **CtrlFlow:** 24.5% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** Tech Debt (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `kube::etcd::validate` (Defensive Guards, Impact: 13.2), `kube::etcd::stop` (Defensive Guards, Impact: 12.8), `kube::etcd::install` (I/O & Config Routines, Impact: 8.3)

### 5. `hack/make-rules/verify.sh` (SHELL) -> Cumulative Risk: **718.58**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.66)
...
- **Heaviest Functions:** `run-checks` (Defensive Guards, Impact: 24.6), `__global_context__` (I/O & Config Routines, Impact: 10.4), `run-cmd` (Defensive Guards, Impact: 5.9)

### 6. `staging/src/k8s.io/kubelet/pkg/cri/streaming/remotecommand/httpstream.go` (GO) -> Cumulative Risk: **689.32**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `createHTTPStreamStreams` (Many-Argument Workhorses, Impact: 43.1), `waitForStreams` (Compute Cores, Impact: 23.8), `waitForStreams` (Compute Cores, Impact: 23.4)

### 7. `hack/verify-golangci-lint.sh` (SHELL) -> Cumulative Risk: **685.83**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +2.19)
...
- **Heaviest Functions:** `usage_[Truncated]` (Compute Cores, Impact: 43.2), `run` (I/O & Config Routines, Impact: 19.9), `__global_context__` (I/O & Config Routines, Impact: 1.5)

### 8. `staging/src/k8s.io/apiserver/pkg/storage/cacher/ready.go` (GO) -> Cumulative Risk: **682.09**
...
- **Heaviest Functions:** `set` (Compute Cores, Impact: 24.0), `waitAndReadGeneration` (Compute Cores, Impact: 9.7), `readGenerationLocked` (Callbacks & Closures, Impact: 8.8)

### 9. `staging/src/k8s.io/apiserver/pkg/storage/cacher/cache_watcher.go` (GO) -> Cumulative Risk: **678.79**
...
- **Heaviest Functions:** `convertToWatchEvent` (Compute Cores, Impact: 23.2), `processInterval` (Many-Argument Workhorses, Impact: 22.1), `process` (Compute Cores, Impact: 18.6)

### 10. `hack/local-up-cluster.sh` (SHELL) -> Cumulative Risk: **675.7**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.90)
- **Magnitude:** 772.82 | **LOC:** 1550 | **CtrlFlow:** 16.5% | **Authorship Centralization:** 20.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9944%), Safety Score (88.5068%), Documentation (80.176%)
- **Heaviest Functions:** `start_kubelet` (I/O & Config Routines, Impact: 41.8), `start_apiserver` (I/O & Config Routines, Impact: 23.8), `cleanup` (I/O & Config Routines, Impact: 21.6)
...
**Top Internal Functions/Classes:**
  ...
  * `validateService` **(Compute Cores)** (Impact: 119.6)
    * *Intent:* // ValidateService tests if required fields/annotations of a Service are valid.
  * `validateWindows` **(Compute Cores)** (Impact: 96.5)
