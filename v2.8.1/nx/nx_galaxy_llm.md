...
|---|---|---|
| Modularity | 0.7331 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.68; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 35%, Declarative / Non-Code 22%, Large Core Modules 8%, Defensive Guards Files 7%, Many-Argument Workhorses Files 6%
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 19.0 | 3.3 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 77.3 | 5.6 | 0.0 | 0.0 |
...
- `extractPropertiesFromObjectLiteral` **(Compute Cores)** (@ `packages/eslint/src/generators/utils/flat-config/ast-utils.ts`) -> Impact: **525.3** | LOC: 1737
...
- `getOutput` **(Tests & Verification)** (@ `packages/eslint/src/generators/utils/flat-config/ast-utils.spec.ts`) -> Impact: **260.2** | LOC: 1611
...
### 1. `packages/nx/src/daemon/client/client.ts` (TYPESCRIPT) -> Cumulative Risk: **742.37**
...
- **Heaviest Functions:** `registerFileWatcher` (Many-Argument Workhorses, Impact: 32.9), `enabled` (Compute Cores, Impact: 23.3), `reconnectFileWatcher` (I/O & Config Routines, Impact: 20.4)

### 2. `packages/nx/src/daemon/server/project-graph-incremental-recomputation.ts` (TYPESCRIPT) -> Cumulative Risk: **739.15**
...
- **Heaviest Functions:** `processFilesAndCreateAndSerializeProjectGraph` (Compute Cores, Impact: 33.8), `addUpdatedAndDeletedFiles` (Many-Argument Workhorses, Impact: 26.6), `isStale` (I/O & Config Routines, Impact: 25.6)

### 3. `packages/js/src/plugins/jest/start-local-registry.ts` (TYPESCRIPT) -> Cumulative Risk: **725.81**
...
- **Heaviest Functions:** `startLocalRegistry` (Defensive Guards, Impact: 41.5), `listener` (Defensive Guards, Impact: 27.4)

### 4. `packages/nx/src/executors/run-commands/running-tasks.ts` (TYPESCRIPT) -> Cumulative Risk: **718.78**
...
- **Heaviest Functions:** `addListeners` (Compute Cores, Impact: 30.1), `createProcess` (Many-Argument Workhorses, Impact: 28.0), `processEnv` (Many-Argument Workhorses, Impact: 21.7)

### 5. `packages/nx/src/utils/child-process.ts` (TYPESCRIPT) -> Cumulative Risk: **715.81**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.55)
...
- **Heaviest Functions:** `runNxAsync` (Defensive Guards, Impact: 32.9), `runNxSync` (Defensive Guards, Impact: 18.2), `getRunNxBaseCommand` (Compute Cores, Impact: 13.1)

### 6. `astro-docs/public/global-scripts.js` (JAVASCRIPT) -> Cumulative Risk: **714.32**
...
- **Heaviest Functions:** `setupSearchTracking` (Callbacks & Closures, Impact: 13.7), `resultClickHandler` (Compute Cores, Impact: 10.1), `openSearchFromStorage` (Interface Declarations, Impact: 6.2)

### 7. `packages/nx/src/command-line/graph/graph.ts` (TYPESCRIPT) -> Cumulative Risk: **710.82**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `generateGraph` (Many-Argument Workhorses, Impact: 151.8), `startServer` (Many-Argument Workhorses, Impact: 79.5), `filter` (Many-Argument Workhorses, Impact: 70.5)
...
- **Heaviest Functions:** `webpack` (Defensive Guards, Impact: 50.7), `withLess` (Defensive Guards, Impact: 44.1), `addLessToRuleTest` (Defensive Guards, Impact: 33.1)

### 9. `packages/webpack/src/utils/webpack/plugins/postcss-cli-resources.ts` (TYPESCRIPT) -> Cumulative Risk: **707.6**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `process` (Many-Argument Workhorses, Impact: 34.7), `Once` (Compute Cores, Impact: 28.2), `PostcssCliResources` (Compute Cores, Impact: 20.7)

### 10. `packages/js/src/utils/swc/compile-swc.ts` (TYPESCRIPT) -> Cumulative Risk: **706.93**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `handleCallback` (Compute Cores, Impact: 30.8), `compileSwc` (Defensive Guards, Impact: 30.6), `compileSwcWatch` (Callbacks & Closures, Impact: 26.2)
...
- `packages/create-nx-workspace/bin/create-nx-workspace.ts` -> Churn: **77.35%** | Cog Load: 89.4823% | Debt: 0.0%
- `packages/nx/src/tasks-runner/task-orchestrator.ts` -> Churn: **66.2%** | Cog Load: 81.7286% | Debt: 8.0096%
- `packages/create-nx-workspace/src/create-workspace.ts` -> Churn: **65.26%** | Cog Load: 97.3508% | Debt: 0.0%
- `packages/nx/src/ai/set-up-ai-agents/set-up-ai-agents.ts` -> Churn: **61.03%** | Cog Load: 51.6429% | Debt: 0.0%
- `packages/gradle/project-graph/build.gradle.kts` -> Churn: **59.82%** | Cog Load: 62.5467% | Debt: 0.0%
...
- `packages/devkit/src/utils/semver.ts` -> **Severity: 0.156** (Bridge: 0.0016 * Flux: 96.7474%)
...
- `packages/nx/src/config/workspace-json-project-json.ts` -> **Severity: 4.328** (Embedded: 0.0716 * Error Risk: 60.4327%)
- `packages/nx/src/utils/workspace-root.ts` -> **Severity: 4.292** (Embedded: 0.0767 * Error Risk: 55.9714%)
- `packages/nx/src/generators/tree.ts` -> **Severity: 3.953** (Embedded: 0.0585 * Error Risk: 67.6235%)
- `packages/nx/src/utils/delayed-spinner.ts` -> **Severity: 3.737** (Embedded: 0.042 * Error Risk: 89.0664%)
- `packages/nx/src/hasher/task-hasher.ts` -> **Severity: 3.527** (Embedded: 0.0483 * Error Risk: 72.9544%)
