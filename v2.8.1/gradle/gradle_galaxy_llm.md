...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +1.71; from the repo's file-archetype mix)
> **File Composition:** Annotated Framework Methods Files 25%, Data / Markup / Trivial 17%, Tests & Verification Files 15%, Interface Declarations Files 10%, State Mutators Files 9%
...
- `doResolveAugmentation` **(Many-Argument Workhorses)** (@ `platforms/core-configuration/declarative-dsl-core/src/main/kotlin/org/gradle/internal/declarativedsl/analysis/FunctionCallResolver.kt`) -> Impact: **222.1** | LOC: 670
...
- `stateFileForSharedObjects` **(Compute Cores)** (@ `platforms/core-configuration/configuration-cache/src/main/kotlin/org/gradle/internal/cc/impl/ConfigurationCacheState.kt`) -> Impact: **95.8** | LOC: 836
...
### 1. `subprojects/core/src/testFixtures/groovy/org/gradle/util/internal/MockExecutor.java` (JAVA) -> Cumulative Risk: **768.2**
...
- **Heaviest Functions:** `scheduleAtFixedRate` (Generic / Templated Code, Impact: 4.7), `scheduleWithFixedDelay` (Generic / Templated Code, Impact: 4.7), `schedule` (Generic / Templated Code, Impact: 4.2)
...
- **Heaviest Functions:** `showIndividualChange` (Compute Cores, Impact: 26.8), `reportChanges` (Compute Cores, Impact: 9.1), `onChangeToInputs` (Generic / Templated Code, Impact: 7.6)

### 3. `platforms/core-execution/persistent-cache/src/main/java/org/gradle/cache/internal/btree/FreeListBlockStore.java` (JAVA) -> Cumulative Risk: **715.0**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -2.26)
...
- **Heaviest Functions:** `alloc` (Compute Cores, Impact: 13.3), `add` (Many-Argument Workhorses, Impact: 10.5), `open` (Annotated Framework Methods, Impact: 6.4)

### 4. `platforms/core-runtime/wrapper-main/src/integTest/groovy/org/gradle/integtests/WrapperGenerationIntegrationTest.groovy` (GROOVY) -> Cumulative Risk: **699.51**
...
- **Heaviest Functions:** `"wrapper task succeeds if distribution url from command-line results in relative uri (no scheme)"` (Tests & Verification, Impact: 3.6), `"no generated wrapper scripts for invalid distribution type from command-line"` (Tests & Verification, Impact: 3.4), `"wrapper JAR does not contain version in manifest"` (Tests & Verification, Impact: 3.0)

### 5. `subprojects/core/src/integTest/groovy/org/gradle/api/AbstractCommandLineOrderTaskIntegrationTest.groovy` (GROOVY) -> Cumulative Risk: **697.78**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z -0.43)
...
- **Heaviest Functions:** `produces` (Compute Cores, Impact: 9.2), `dependencyFor` (Interface Declarations, Impact: 4.6), `taskPath` (Interface Declarations, Impact: 3.0)

### 6. `platforms/ide/tooling-api/src/crossVersionTest/groovy/org/gradle/integtests/tooling/r950/HelpConsumerCrossVersionSpec.groovy` (GROOVY) -> Cumulative Risk: **697.5**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z -1.31)
...
- **Heaviest Functions:** `"requesting help via #entryPoint is ignored"` (Tests & Verification, Impact: 3.4), `"prints help and ignores tasks when --help is present"` (Tests & Verification, Impact: 2.8), `"help takes precedence over version flags"` (Tests & Verification, Impact: 1.7)

### 7. `platforms/core-runtime/logging/src/main/java/org/gradle/internal/logging/LoggingConfigurationBuildOptions.java` (JAVA) -> Cumulative Risk: **684.81**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.70)
- **Magnitude:** 176.66 | **LOC:** 288 | **CtrlFlow:** 14.2% | **Authorship Centralization:** 28.6%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.3378%), Verification (80.0%)
- **Heaviest Functions:** `applyFromProperty` (Compute Cores, Impact: 14.7), `applyFromCommandLine` (Compute Cores, Impact: 14.5), `applyFromCommandLine` (Annotated Framework Methods, Impact: 7.3)

### 8. `platforms/core-runtime/collections/src/jmh/java/org/gradle/internal/collect/bench/PersistentMapBenchmark.java` (JAVA) -> Cumulative Risk: **680.76**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -1.58)
...
- **Heaviest Functions:** `modify` (Generic / Templated Code, Impact: 10.2), `modify` (Generic / Templated Code, Impact: 8.2), `constructionOneByOne` (Annotated Framework Methods, Impact: 4.6)

### 9. `subprojects/core/src/main/java/org/gradle/api/internal/AbstractTask.java` (JAVA) -> Cumulative Risk: **680.33**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -0.30)
- **Magnitude:** 773.9 | **LOC:** 1099 | **CtrlFlow:** 7.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.9987%), Tech Debt (99.9956%), Documentation (99.262%)
- **Heaviest Functions:** `wrap` (Defensive Guards, Impact: 10.9), `setActions` (Generic / Templated Code, Impact: 7.7), `acceptServiceReferences` (Generic / Templated Code, Impact: 7.6)

### 10. `subprojects/core/src/integTest/groovy/org/gradle/api/tasks/options/TaskOptionIntegrationTest.groovy` (GROOVY) -> Cumulative Risk: **679.7**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z -0.67)
- **Magnitude:** 336.56 | **LOC:** 582 | **CtrlFlow:** 10.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Cognitive Load (98.7184%)
- **Heaviest Functions:** `"set value of property of type #propertyType of type #optionType when #description for Java task"` (I/O & Config Routines, Impact: 10.5), `"set value of property of type #propertyType of type #optionType when #description for Groovy task"` (I/O & Config Routines, Impact: 10.5), `"can evaluate option value of type #optionType when #description for Java task on command line"` (Tests & Verification, Impact: 8.2)
...
**Top Internal Functions/Classes:**
  ...
  * `printStatement` **(Compute Cores)** (Impact: 21.7)
...
**Top Internal Functions/Classes:**
  * `resolveFromModule` **(Compute Cores)** (Impact: 52.2)
  * `checkCyclicInheritance` **(Compute Cores)** (Impact: 38.0)
...
**Top Internal Functions/Classes:**
  ...
  * `AbstractClassGenerator` **(Many-Argument Workhorses)** (Impact: 44.0)
  * `validateMethod` **(Many-Argument Workhorses)** (Impact: 36.8)
  * `assembleProperties` **(Compute Cores)** (Impact: 23.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `collectProvidersForClassHierarchyOf` **(Generic / Templated Code)** (Impact: 29.1)
  * `getService` **(Compute Cores)** (Impact: 24.4)
  * `ConstructorService` **(Many-Argument Workhorses)** (Impact: 23.0)
  * `instanceRealized` **(Defensive Guards)** (Impact: 21.1)
  * `find` **(Defensive Guards)** (Impact: 19.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `namedConsumer` **(Generic / Templated Code)** (Impact: 12.8)
  * `"cannot set value after value finalized"` **(Tests & Verification)** (Impact: 6.8)
  * `"cannot set value after value finalized implicitly and before queried"` **(Tests & Verification)** (Impact: 6.8)
  * `"cannot set value after value finalized implicitly"` **(Tests & Verification)** (Impact: 6.8)
  * `"cannot set value after value finalized after value finalized implicitly"` **(Tests & Verification)** (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `visitArrayElements` **(Compute Cores)** (Impact: 37.1)
  * `visitAnnotationValues` **(Compute Cores)** (Impact: 25.4)
