...
> **File Composition:** Data / Markup / Trivial 50%, Generic / Templated Code Files 12%, Declarative / Non-Code 8%, Parameter Forwarders Files 8%, State Mutators Files 8%
...
- `rustc_minor_version` **(Interface Declarations)** (@ `build.rs`) -> Impact: **6.5** | LOC: 10
...
- `main` **(Interface Declarations)** (@ `build.rs`) -> Impact: **5.8** | LOC: 17
- `test_advanced` **(Tests & Verification)** (@ `tests/test.rs`) -> Impact: **5.7** | LOC: 54
...
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.33)
...
- **Heaviest Functions:** `to_tokens` (Compute Cores, Impact: 5.4), `to_tokens` (Defensive Guards, Impact: 3.7), `to_tokens` (Parameter Forwarders, Impact: 1.9)

### 2. `src/runtime.rs` (RUST) -> Cumulative Risk: **540.78**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +1.39)
...
- **Heaviest Functions:** `respan_token_tree` (Compute Cores, Impact: 5.9), `ident_maybe_raw` (Defensive Guards, Impact: 5.5), `parse_spanned` (Annotated Framework Methods, Impact: 4.3)

### 3. `benches/lib.rs` (RUST) -> Cumulative Risk: **409.91**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.28)
...
- **Heaviest Functions:** `deserialize` (Generic / Templated Code, Impact: 30.0), `visit_map` (Defensive Guards, Impact: 14.8), `visit_seq` (Generic / Templated Code, Impact: 6.8)
...
- **Heaviest Functions:** `append_separated` (Generic / Templated Code, Impact: 7.2), `do_append_separated` (Generic / Templated Code, Impact: 6.8), `append_terminated` (Generic / Templated Code, Impact: 5.0)
...
- **Heaviest Functions:** `fmt` (Defensive Guards, Impact: 5.6), `fmt` (Parameter Forwarders, Impact: 1.9), `fmt` (Parameter Forwarders, Impact: 1.9)
...
- **Heaviest Functions:** `rustc_minor_version` (Interface Declarations, Impact: 6.5), `main` (Interface Declarations, Impact: 5.8)

### 7. `tests/test.rs` (RUST) -> Cumulative Risk: **270.35**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +0.12)
...
- **Heaviest Functions:** `test_advanced` (Tests & Verification, Impact: 5.7), `test_type_inference_for_span` (Tests & Verification, Impact: 4.3), `test_quote_impl` (Tests & Verification, Impact: 4.0)
...
- **Heaviest Functions:** `quote` (Interface Declarations, Impact: 1.2), `main` (State Mutators, Impact: 1.1)
...
- **Heaviest Functions:** `join_spans` (Defensive Guards, Impact: 3.4), `__span` (State Mutators, Impact: 1.6), `__span` (State Mutators, Impact: 1.6)
...
- **Heaviest Functions:** `ui` (Tests & Verification, Impact: 1.4)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_spanned` **(Annotated Framework Methods)** (Impact: 4.3)
  * `push_group_spanned` **(Annotated Framework Methods)** (Impact: 2.8)
  * `push_lifetime_spanned` **(Annotated Framework Methods)** (Impact: 2.5)
...
**Top Internal Functions/Classes:**
  * `test_advanced` **(Tests & Verification)** (Impact: 5.7)
  * `test_type_inference_for_span` **(Tests & Verification)** (Impact: 4.3)
  * `test_quote_impl` **(Tests & Verification)** (Impact: 4.0)
  * `test_quote_spanned_impl` **(Tests & Verification)** (Impact: 4.0)
  * `test_array` **(Interface Declarations)** (Impact: 2.2)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `rustc_minor_version` **(Interface Declarations)** (Impact: 6.5)
  * `main` **(Interface Declarations)** (Impact: 5.8)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `ui` **(Tests & Verification)** (Impact: 1.4)
**Structural Signatures (Net Mitigated Signals):**
