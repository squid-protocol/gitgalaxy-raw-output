...
> **File Composition:** Data / Markup / Trivial 44%, Declarative / Non-Code 26%, Interface Declarations Files 7%, Callbacks & Closures Files 6%, Large Core Modules 4%
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 20.9 | 6.0 | 0.0 | 0.0 |
...
- `getPrismaClient` **(Compute Cores)** (@ `packages/client/src/runtime/getPrismaClient.ts`) -> Impact: **198.2** | LOC: 767
- `testIf` **(Tests & Verification)** (@ `packages/client/src/__tests__/buffer-small.test.ts`) -> Impact: **188.9** | LOC: 2194
...
### 1. `packages/cli/src/Init.ts` (TYPESCRIPT) -> Cumulative Risk: **729.57**
...
- **Heaviest Functions:** `parse` (Many-Argument Workhorses, Impact: 156.0), `defaultEnv` (Many-Argument Workhorses, Impact: 60.6), `defaultSchema` (Compute Cores, Impact: 30.4)

### 2. `packages/migrate/src/commands/DbPull.ts` (TYPESCRIPT) -> Cumulative Risk: **714.77**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.26)
...
- **Heaviest Functions:** `parse` (Many-Argument Workhorses, Impact: 87.9), `help` (Interface Declarations, Impact: 4.5), `getWarningMessage` (Interface Declarations, Impact: 3.2)
...
- **Heaviest Functions:** `parse` (Defensive Guards, Impact: 30.0), `createExecutor` (Defensive Guards, Impact: 24.2), `normalizeMySQLConnectionString` (Compute Cores, Impact: 9.5)
...
- **Heaviest Functions:** `requestBatch` (Defensive Guards, Impact: 51.9), `constructor` (Defensive Guards, Impact: 39.4), `transaction` (Defensive Guards, Impact: 19.6)
...
- **Heaviest Functions:** `buildClient` (Compute Cores, Impact: 37.2), `validateDmmfAgainstDenylists` (Compute Cores, Impact: 29.5), `generateClient` (Compute Cores, Impact: 15.1)
...
- **Heaviest Functions:** `parse` (Defensive Guards, Impact: 30.8), `resolveDatabase` (Defensive Guards, Impact: 19.1), `link` (Defensive Guards, Impact: 18.9)
...
- **Heaviest Functions:** `getPrismaClient` (Compute Cores, Impact: 198.2), `constructor` (Defensive Guards, Impact: 70.3), `_transactionWithCallback` (Defensive Guards, Impact: 38.2)
...
- **Heaviest Functions:** `parse` (Many-Argument Workhorses, Impact: 33.6), `help` (Interface Declarations, Impact: 4.5)
...
- **Heaviest Functions:** `run` (Many-Argument Workhorses, Impact: 252.4), `manualInstallAndReturn` (Compute Cores, Impact: 65.5), `parse` (Many-Argument Workhorses, Impact: 28.6)
...
- **Heaviest Functions:** `sendMessage` (Compute Cores, Impact: 11.7), `initSingleton` (Callbacks & Closures, Impact: 11.1), `handleResponse` (Compute Cores, Impact: 10.7)
...
**Top Internal Functions/Classes:**
  * `testIf` **(Tests & Verification)** (Impact: 188.9)
...
**Top Internal Functions/Classes:**
  ...
  * `publish` **(Compute Cores)** (Impact: 55.8)
  ...
  * `getAllVersionsPublishedFor` **(Callbacks & Closures)** (Impact: 14.0)
...
- `packages/client/tests/functional/_utils/setupTestSuiteMatrix.ts` -> **Severity: 2.614** (Embedded: 0.033 * Error Risk: 79.3216%)
- `packages/client/src/runtime/getPrismaClient.ts` -> **Severity: 2.472** (Embedded: 0.0379 * Error Risk: 65.238%)
- `packages/client/tests/functional/_utils/getTestSuiteInfo.ts` -> **Severity: 2.4** (Embedded: 0.036 * Error Risk: 66.6384%)
