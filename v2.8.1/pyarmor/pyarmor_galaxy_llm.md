...
> **Composition Archetype:** `Hub-Coupled App` (z +0.11; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 28%, Large Core Modules 23%, Interface Declarations Files 19%, Declarative / Non-Code 14%, Compute Cores Files 4%
...
- **Heaviest Functions:** `process_extra` (Compute Cores, Impact: 17.8), `process` (Many-Argument Workhorses, Impact: 17.6), `async_obfuscate_scripts` (Compute Cores, Impact: 17.6)
...
- **Heaviest Functions:** `format_platform` (Compute Cores, Impact: 32.2), `__init__` (Many-Argument Workhorses, Impact: 31.3), `version_info` (Compute Cores, Impact: 29.3)
...
- **Heaviest Functions:** `rebuild` (Compute Cores, Impact: 36.4), `generate_output` (Many-Argument Workhorses, Impact: 23.7), `_get_encoding` (Compute Cores, Impact: 18.8)
...
- **Heaviest Functions:** `__getattr__` (Type Conversions, Impact: 11.1), `process` (Encapsulated Accessors, Impact: 2.4), `trace` (Parameter Forwarders, Impact: 2.4)
...
- **Heaviest Functions:** `do_push` (Defensive Guards, Impact: 12.9), `do_cd` (Compute Cores, Impact: 12.8), `get_sections` (Compute Cores, Impact: 12.4)
...
- **Heaviest Functions:** `prepare` (Many-Argument Workhorses, Impact: 70.8), `_register_offline_license` (Many-Argument Workhorses, Impact: 36.1), `request_device_regfile` (Many-Argument Workhorses, Impact: 27.1)

### 7. `src/cli/__main__.py` (PYTHON) -> Cumulative Risk: **628.95**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.34)
...
- **Heaviest Functions:** `cmd_reg` (Compute Cores, Impact: 52.8), `check_gen_context` (Compute Cores, Impact: 47.5), `format_gen_args` (Compute Cores, Impact: 44.7)
...
- **Heaviest Functions:** `_parse_known_args` (Many-Argument Workhorses, Impact: 105.7), `_format_usage` (Many-Argument Workhorses, Impact: 63.3), `_format_actions_usage` (Many-Argument Workhorses, Impact: 60.8)
...
- **Heaviest Functions:** `get_platform` (I/O & Config Routines, Impact: 16.9)

### 10. `src/cli/command.py` (PYTHON) -> Cumulative Risk: **611.21**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.90)
...
- **Heaviest Functions:** `cmd_init` (Many-Argument Workhorses, Impact: 60.8), `cmd_build` (Many-Argument Workhorses, Impact: 47.9), `cmd_env` (Many-Argument Workhorses, Impact: 31.6)
...
**Top Internal Functions/Classes:**
  ...
  * `calibrate` **(Interface Declarations)** (Impact: 3.2)
  * `calibrate` **(Interface Declarations)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `load` **(Compute Cores)** (Impact: 45.5)
  ...
  * `_preview_autofix_3` **(Many-Argument Workhorses)** (Impact: 21.9)
  * `_preview_autofix_2` **(Many-Argument Workhorses)** (Impact: 13.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_fixup_darwin_rtbinary` **(Many-Argument Workhorses)** (Impact: 15.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_named_config` **(Compute Cores)** (Impact: 10.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `encrypt_files` **(Many-Argument Workhorses)** (Impact: 31.0)
    * *Intent:* '''Encrypt all the files, all the encrypted scripts will be plused with a suffix 'e', for example, h...
  * `_parse_file_args` **(Compute Cores)** (Impact: 25.7)
  * `do_capsule` **(Compute Cores)** (Impact: 16.7)
...
**Top Internal Functions/Classes:**
  ...
  * `test_with_statement` **(Interface Declarations)** (Impact: 12.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 46 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_list_value` **(Many-Argument Workhorses)** (Impact: 30.7)
  * `list_options` **(Many-Argument Workhorses)** (Impact: 25.9)
  * `_remove` **(Many-Argument Workhorses)** (Impact: 25.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `gen_parser` **(Compute Cores)** (Impact: 19.1)
...
**Top Internal Functions/Classes:**
  ...
  * `do_cd` **(Compute Cores)** (Impact: 12.8)
  ...
  * `do_use` **(Type Conversions)** (Impact: 11.2)
