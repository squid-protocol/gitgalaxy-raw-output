...
> **Composition Archetype:** `Hub-Coupled App` (z -0.70; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 32%, Interface Declarations Files 16%, Defensive Guards Files 11%, Data / Markup / Trivial 11%, Declarative / Non-Code 11%
...
| connectivity | 5328 | 162 | 70 | `pyarrow-23.0.1/pyarrow/tests/test_pandas.py` |
...
- **Heaviest Functions:** `convert_status` (Compute Cores, Impact: 29.6), `__exit__` (Many-Argument Workhorses, Impact: 19.2), `__cinit__` (Compute Cores, Impact: 8.2)
...
- **Heaviest Functions:** `_cmpkey` (Many-Argument Workhorses, Impact: 42.5), `__init__` (Compute Cores, Impact: 34.5), `_parse_letter_version` (Compute Cores, Impact: 17.2)

### 3. `pyarrow-23.0.1/pyarrow/public-api.pxi` (PYTHON) -> Cumulative Risk: **718.56**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.57)
...
- **Heaviest Functions:** `pyarrow_wrap_data_type` (Compute Cores, Impact: 47.5), `pyarrow_wrap_scalar` (Compute Cores, Impact: 8.1), `pyarrow_wrap_chunked_array` (Interface Declarations, Impact: 4.9)

### 4. `pyarrow-23.0.1/pyarrow/src/arrow/python/datetime.cc` (CPP) -> Cumulative Risk: **700.49**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.42)
- **Magnitude:** 397.28 | **LOC:** 666 | **CtrlFlow:** 15.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9994%), Tech Debt (93.789%)
- **Heaviest Functions:** `TzinfoToString` (Compute Cores, Impact: 31.2), `PyTime_convert_int` (Many-Argument Workhorses, Impact: 30.5), `StringToTzinfo` (Compute Cores, Impact: 21.7)
...
- **Heaviest Functions:** `Visit` (Compute Cores, Impact: 86.6), `GetType` (Compute Cores, Impact: 78.4), `DatetimeUnitName` (Compute Cores, Impact: 25.7)
...
- **Heaviest Functions:** `MapPyError` (Compute Cores, Impact: 23.8), `PyBuffer::Init` (Type Conversions, Impact: 4.8), `IsPyError` (Interface Declarations, Impact: 4.6)

### 7. `pyarrow-23.0.1/pyarrow/ipc.pxi` (PYTHON) -> Cumulative Risk: **676.43**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.30)
...
- **Heaviest Functions:** `compression` (Defensive Guards, Impact: 21.6), `from_stream` (Defensive Guards, Impact: 14.2), `read_record_batch` (Many-Argument Workhorses, Impact: 12.1)
...
- **Heaviest Functions:** `_import_pandas` (Many-Argument Workhorses, Impact: 16.7), `_check_import` (Defensive Guards, Impact: 9.3), `uses_string_dtype` (Defensive Guards, Impact: 6.1)
...
- **Heaviest Functions:** `UnboxIntegerAsInt64` (Compute Cores, Impact: 50.3), `CIntFromPythonImpl` (Many-Argument Workhorses, Impact: 21.7), `CIntFromPythonImpl` (Many-Argument Workhorses, Impact: 17.6)
...
- **Heaviest Functions:** `PyObject_Vectorcall` (Many-Argument Workhorses, Impact: 43.7), `PyDict_Pop` (Many-Argument Workhorses, Impact: 28.1), `PyTime_PerfCounter` (Compute Cores, Impact: 26.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_check_array_roundtrip` **(Many-Argument Workhorses)** (Impact: 16.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_check_cast_case` **(Defensive Guards)** (Impact: 17.1)
...
**Top Internal Functions/Classes:**
  ...
  * `start_call` **(Many-Argument Workhorses)** (Impact: 15.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_add_any_metadata` **(Compute Cores)** (Impact: 34.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `test_read_options` **(Compute Cores)** (Impact: 21.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_set_options` **(Many-Argument Workhorses)** (Impact: 22.0)
...
**Top Internal Functions/Classes:**
  ...
  * `test_s3fs_limited_permissions_create_bucket` **(Tests & Verification)** (Impact: 10.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Rce:* 1 instances
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 63.5)
  ...
  * `_parse` **(Compute Cores)** (Impact: 24.5)
  * `_str_see_also` **(Defensive Guards)** (Impact: 18.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 116 instances
