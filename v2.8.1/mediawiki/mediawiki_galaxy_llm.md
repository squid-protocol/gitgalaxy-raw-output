...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +1.33; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 50%, Interface Declarations Files 18%, Declarative / Non-Code 10%, Large Core Modules 9%, Defensive Guards Files 3%
...
| connectivity | 34224 | 4942 | 8 | `includes/MainConfigSchema.php` |
...
| threat | 3893 | 451 | 0 | `resources/lib/ooui/oojs-ui-core.js` |
...
- `handleExternalLinks` **(Compute Cores)** (@ `includes/Parser/Parser.php`) -> Impact: **712.1** | LOC: 1910
...
- `handleTables` **(Compute Cores)** (@ `includes/Parser/Parser.php`) -> Impact: **691.8** | LOC: 1872
  * *Intent:* /** * Parse the wiki syntax used to render tables. * * @param string $text * @return string */
- `handleInternalLinks2` **(Compute Cores)** (@ `includes/Parser/Parser.php`) -> Impact: **683.6** | LOC: 1877
...
- `makeFormattedData` **(Compute Cores)** (@ `includes/Media/FormatMetadata.php`) -> Impact: **558.7** | LOC: 709
...
- `requireTokenizer` **(Compute Cores)** (@ `resources/lib/vue-test-utils/vue-test-utils.browser.js`) -> Impact: **501.4** | LOC: 1427
...
- **Heaviest Functions:** `finalSetup` (Compute Cores, Impact: 74.8), `__construct` (I/O & Config Routines, Impact: 5.0)

### 2. `includes/OutputTransform/Stages/ParsoidLanguageConverter.php` (PHP) -> Cumulative Risk: **665.1**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -1.69)
...
- **Heaviest Functions:** `convertNode` (Many-Argument Workhorses, Impact: 122.0), `transformDOM` (Many-Argument Workhorses, Impact: 28.2), `shouldRun` (Compute Cores, Impact: 8.3)
...
- **Heaviest Functions:** `normalizeName` (Callbacks & Closures, Impact: 63.5), `Request` (Defensive Guards, Impact: 58.3), `fetch` (Defensive Guards, Impact: 46.4)
...
- **Heaviest Functions:** `upload` (Callbacks & Closures, Impact: 9.4), `setFilenameFromFile` (Defensive Guards, Impact: 8.8), `uploadToStash` (Callbacks & Closures, Impact: 8.4)
...
- **Heaviest Functions:** `getActionProcess` (Callbacks & Closures, Impact: 12.0), `postMessage` (Defensive Guards, Impact: 11.2), `getSetupProcess` (Callbacks & Closures, Impact: 9.7)
...
- **Heaviest Functions:** `createSetupStore` (Many-Argument Workhorses, Impact: 217.6), `registerPiniaDevtools` (Defensive Guards, Impact: 62.6), `set` (Compute Cores, Impact: 42.4)
...
- **Heaviest Functions:** `setParserOutput` (Compute Cores, Impact: 9.2), `finishUpdate` (I/O & Config Routines, Impact: 7.8), `__construct` (Many-Argument Workhorses, Impact: 5.3)
...
- **Heaviest Functions:** `fetchAPIresults` (Compute Cores, Impact: 16.6), `getResults` (Callbacks & Closures, Impact: 8.7), `MwWidgetsMediaResourceProvider` (Defensive Guards, Impact: 7.9)
...
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.96)
...
- **Heaviest Functions:** `openBrowserWindowCoveringElement` (Many-Argument Workhorses, Impact: 32.1), `showDialog` (Callbacks & Closures, Impact: 23.6), `getWindowDimensions` (Defensive Guards, Impact: 15.7)
...
- **Heaviest Functions:** `replaceWithVue` (Compute Cores, Impact: 15.5), `onLanguageChange` (Callbacks & Closures, Impact: 6.7), `menuItemSlot` (Interface Declarations, Impact: 1.5)
...
**Top Internal Functions/Classes:**
  ...
  * `handleExternalLinks` **(Compute Cores)** (Impact: 712.1)
    * *Intent:* /** * Replace external links (REL) * * Note: this is all very hackish and the order of execution mat...
  * `handleTables` **(Compute Cores)** (Impact: 691.8)
    * *Intent:* /** * Parse the wiki syntax used to render tables. * * @param string $text * @return string */
  * `handleInternalLinks2` **(Compute Cores)** (Impact: 683.6)
    * *Intent:* /** * Process [[ ]] wikilinks (RIL) * @param string &$s * @return LinkHolderArray */
...
**Top Internal Functions/Classes:**
  ...
  * `formatNumInternal` **(Compute Cores)** (Impact: 76.8)
...
**Top Internal Functions/Classes:**
  ...
  * `sendCacheControl` **(Compute Cores)** (Impact: 326.1)
    * *Intent:* /** * Send cache control HTTP headers */
  * `getHeadLinksSyndicationArray` **(Compute Cores)** (Impact: 85.0)
...
**Top Internal Functions/Classes:**
  * `getPreviewText` **(Compute Cores)** (Impact: 125.1)
  ...
  * `handleStatus` **(Compute Cores)** (Impact: 85.5)
...
**Top Internal Functions/Classes:**
  * `upgradeRow` **(Compute Cores)** (Impact: 325.6)
...
**Top Internal Functions/Classes:**
  ...
  * `createSidebarItem` **(Compute Cores)** (Impact: 27.1)
...
**Top Internal Functions/Classes:**
  * `showDiffPage` **(Compute Cores)** (Impact: 438.1)
...
**Top Internal Functions/Classes:**
  ...
  * `getLinkList` **(Compute Cores)** (Impact: 95.9)
  ...
  * `mergeMapStrategy` **(Compute Cores)** (Impact: 35.2)
...
**Top Internal Functions/Classes:**
  * `selectSQLText` **(Many-Argument Workhorses)** (Impact: 136.8)
    * *Intent:* /** @inheritDoc */
  * `makeList` **(Compute Cores)** (Impact: 104.5)
    * *Intent:* /** @inheritDoc */
  ...
  * `buildSuperlative` **(Compute Cores)** (Impact: 25.3)
...
**Top Internal Functions/Classes:**
  ...
  * `continueAuthentication` **(Compute Cores)** (Impact: 118.6)
    * *Intent:* * new AuthenticationRequests should be made (if any), then * AuthManager::continueAuthentication() s...
  * `continueAccountCreation` **(Compute Cores)** (Impact: 115.3)
...
- **Risk Profile:** Cognitive Load (33.3225%), Tech Debt (10.9075%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `getFileListPageInternal` **(Many-Argument Workhorses)** (Impact: 34.1)
    * *Intent:* /** * Do not call this function outside of SwiftFileBackendFileList * * @param string $fullCont Reso...
  * `requestMultiWithAuth` **(Defensive Guards)** (Impact: 31.4)
