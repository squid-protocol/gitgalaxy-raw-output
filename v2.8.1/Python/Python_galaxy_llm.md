...
> **Composition Archetype:** `Flat Modular Platform` (z +1.00; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 24%, Compute Cores Files 23%, Generic / Templated Code Files 16%, Many-Argument Workhorses Files 16%, Large Core Modules 8%
...
- **Heaviest Functions:** `dfs` (Many-Argument Workhorses, Impact: 27.9), `dfs` (Many-Argument Workhorses, Impact: 27.9), `cycle_nodes` (Compute Cores, Impact: 25.2)
...
- **Heaviest Functions:** `multi_a_star` (Many-Argument Workhorses, Impact: 41.6), `expand_state` (Many-Argument Workhorses, Impact: 38.0), `do_something` (Many-Argument Workhorses, Impact: 26.0)
...
- **Heaviest Functions:** `sift_down` (Many-Argument Workhorses, Impact: 17.1), `build_heap` (Compute Cores, Impact: 5.7), `sift_up` (Compute Cores, Impact: 5.7)
...
- **Heaviest Functions:** `_normalize_graph` (Many-Argument Workhorses, Impact: 25.7), `_algorithm` (Compute Cores, Impact: 11.4), `relabel` (Compute Cores, Impact: 11.0)
...
- **Heaviest Functions:** `fib_recursive` (Compute Cores, Impact: 9.2), `matrix_pow_np` (Many-Argument Workhorses, Impact: 9.0), `fib_recursive_cached` (Generic / Templated Code, Impact: 8.6)
...
- **Heaviest Functions:** `delete_value` (Compute Cores, Impact: 7.4), `insert_before_node` (Generic / Templated Code, Impact: 6.5), `insert_after_node` (Generic / Templated Code, Impact: 6.5)

### 7. `matrix/matrix_class.py` (PYTHON) -> Cumulative Risk: **594.58**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.68)
...
- **Heaviest Functions:** `add_column` (Defensive Guards, Impact: 23.0), `__init__` (Defensive Guards, Impact: 20.0), `add_row` (Defensive Guards, Impact: 18.8)
...
- **Heaviest Functions:** `encrypt_and_write_to_file` (Many-Argument Workhorses, Impact: 14.5), `read_from_file_and_decrypt` (Type Conversions, Impact: 11.4), `main` (I/O & Config Routines, Impact: 9.6)

### 9. `graphs/minimum_spanning_tree_prims.py` (PYTHON) -> Cumulative Risk: **588.74**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.36)
...
- **Heaviest Functions:** `top_to_bottom` (Many-Argument Workhorses, Impact: 18.4), `bottom_to_top` (Many-Argument Workhorses, Impact: 18.1), `prisms_algorithm` (Compute Cores, Impact: 13.8)
...
- **Heaviest Functions:** `derivative` (Many-Argument Workhorses, Impact: 28.1), `gradient` (Many-Argument Workhorses, Impact: 11.7), `__init__` (Generic / Templated Code, Impact: 6.3)
...
**Top Internal Functions/Classes:**
  ...
  * `remove` **(Compute Cores)** (Impact: 44.3)
    * *Intent:* """Remove label from this tree."""
  * `_insert_repair` **(Compute Cores)** (Impact: 30.1)
...
**Top Internal Functions/Classes:**
  * `merge_heaps` **(Compute Cores)** (Impact: 40.1)
...
**Top Internal Functions/Classes:**
  ...
  * `goldbach` **(Compute Cores)** (Impact: 21.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_calculate_gradient_from_pool` **(Many-Argument Workhorses)** (Impact: 11.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_locate_node` **(Compute Cores)** (Impact: 13.8)
...
**Top Internal Functions/Classes:**
  ...
  * `make_common_ground` **(Compute Cores)** (Impact: 11.1)
  * `put` **(Many-Argument Workhorses)** (Impact: 10.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Rce:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `get_surrounding_prey` **(Compute Cores)** (Impact: 12.4)
...
**Top Internal Functions/Classes:**
  ...
  * `_reassign_nodes` **(Compute Cores)** (Impact: 12.6)
  * `_put` **(Many-Argument Workhorses)** (Impact: 11.8)
  * `_search` **(Generic / Templated Code)** (Impact: 8.5)
...
**Top Internal Functions/Classes:**
  ...
  * `find_unit_clauses` **(Compute Cores)** (Impact: 21.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_construct_points` **(Defensive Guards)** (Impact: 9.3)
  * `convex_hull_recursive` **(Compute Cores)** (Impact: 9.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_is_same_kind` **(Compute Cores)** (Impact: 17.5)
