...
| Analyzed Artifacts (Scanned) | 8896 |
...
|---|---|---|
| Modularity | 0.8147 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Articulation Pts | 1187 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| TYPESCRIPT | 8183 | 564813 | 92.0% |
| JSON | 189 | 7400 | 2.1% |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.94; from the repo's file-archetype mix)
> **File Composition:** Callbacks & Closures Files 21%, Data / Markup / Trivial 20%, Declarative / Non-Code 18%, Generic / Templated Code Files 12%, Defensive Guards Files 7%
...
- `.ts`: 152x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 3x Unsupported Format (.undeterminable), 2x Excluded (Saturation: Line 6 exceeds 500 chars)
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.9 | 19.7 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 3.6 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 5.1 | 2.3 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 65.1 | 100.0 | 100.0 |
...
| guards | 20270 | 3589 | 5 | `ee/packages/federation-matrix/tests/end-to-end/messaging.spec.ts` |
| danger | 8621 | 1959 | 2 | `ee/packages/abac/src/service.spec.ts` |
| concurrency | 56691 | 3276 | 12 | `apps/meteor/tests/end-to-end/api/livechat/00-rooms.ts` |
| connectivity | 22621 | 6996 | 5 | `packages/models/src/models/Users.ts` |
| io | 5343 | 748 | 0 | `apps/meteor/app/emoji-emojione/client/people-sprites.css` |
| crypto | 9 | 4 | 0 | `packages/node-poplib/src/index.js` |
| ipc | 138 | 57 | 0 | `apps/meteor/tests/end-to-end/api/chat.ts` |
| time | 3014 | 683 | 0 | `apps/meteor/tests/end-to-end/api/livechat/17-dashboards-ee.ts` |
| serialization | 796 | 276 | 0 | `apps/meteor/tests/end-to-end/api/methods.ts` |
| regex | 1335 | 506 | 0 | `apps/meteor/app/markdown/lib/parser/original/markdown.js` |
| events | 4223 | 893 | 1 | `packages/node-poplib/src/index.js` |
| tests | 41657 | 981 | 2 | `apps/meteor/tests/end-to-end/api/users.ts` |
| docs | 2390 | 907 | 1 | `packages/eslint-config/index.js` |
| debt | 2293 | 765 | 0 | `apps/meteor/tests/unit/app/importer/server/userConverter.spec.ts` |
| mutation | 88060 | 6813 | 23 | `apps/meteor/tests/end-to-end/api/livechat/00-rooms.ts` |
| dead_code | 1176 | 569 | 0 | `apps/meteor/app/ui/client/lib/codeMirror/codeMirror.ts` |
...
| ml_ai | 598 | 237 | 0 | `apps/meteor/tests/end-to-end/api/rooms.ts` |
...
- `loadAPI` **(Compute Cores)** (@ `apps/meteor/ee/server/apps/communication/rest.ts`) -> Impact: **225.7** | LOC: 1233
- `addManagementRoutes` **(Compute Cores)** (@ `apps/meteor/ee/server/apps/communication/rest.ts`) -> Impact: **224.6** | LOC: 1212
...
- `saveDepartment` **(Many-Argument Workhorses)** (@ `apps/meteor/app/livechat/server/lib/departmentsLib.ts`) -> Impact: **114.2** | LOC: 128
  * *Intent:* /** * @param {string|null} _id - The department id * @param {Partial<import('@rocket.chat/core-typings').ILivechatDepartment>} departmentData * @param...
...
- `scripts/todo-issue/src/github.ts` -> **100.0%** Exposure
...
- **Unknown Dependencies:** `18285` packages imported that bypass the Zero-Trust whitelist.

## 11. CUMULATIVE RISK HITLIST (Top 10 Highest Risk Files)
> Cumulative Risk is the sum of all individual risk exposures. These files represent the highest multi-dimensional technical debt and architectural fragility.

### 1. `apps/meteor/app/ui/client/lib/ChatMessages.ts` (TYPESCRIPT) -> Cumulative Risk: **750.18**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.20)
- **Magnitude:** 225.3 | **LOC:** 198 | **CtrlFlow:** 21.3% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `editMessage` (Defensive Guards, Impact: 20.2), `toPreviousMessage` (I/O & Config Routines, Impact: 11.9), `toNextMessage` (Interface Declarations, Impact: 8.2)

### 2. `apps/meteor/app/integrations/server/lib/triggerHandler.ts` (TYPESCRIPT) -> Cumulative Risk: **748.34**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.10)
- **Magnitude:** 834.34 | **LOC:** 841 | **CtrlFlow:** 30.5% | **Authorship Centralization:** 37.5%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.9996%)
- **Heaviest Functions:** `executeTriggerUrl` (Many-Argument Workhorses, Impact: 151.3), `getTriggersToExecute` (Compute Cores, Impact: 57.2), `mapEventArgsToData` (Compute Cores, Impact: 52.6)

### 3. `apps/meteor/server/services/authorization/canAccessRoom.ts` (TYPESCRIPT) -> Cumulative Risk: **742.84**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -1.01)
- **Magnitude:** 160.1 | **LOC:** 140 | **CtrlFlow:** 52.0% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `_validateIfAlreadyJoined` (Defensive Guards, Impact: 35.1), `_validateAccessToPublicRoomsInTeams` (Defensive Guards, Impact: 22.3), `_validateAccessToPublicRooms` (Defensive Guards, Impact: 16.2)

### 4. `apps/meteor/app/ui-utils/client/lib/RoomHistoryManager.ts` (TYPESCRIPT) -> Cumulative Risk: **739.17**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.55)
- **Magnitude:** 356.42 | **LOC:** 350 | **CtrlFlow:** 21.4% | **Authorship Centralization:** 25.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `getMore` (Compute Cores, Impact: 28.2), `loadSurroundingMessages` (Compute Cores, Impact: 17.6), `processMessage` (Defensive Guards, Impact: 16.3)

### 5. `apps/meteor/packages/autoupdate/autoupdate_client.js` (JAVASCRIPT) -> Cumulative Risk: **738.64**
- **Archetype:** `file_cluster_8` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.17)
- **Magnitude:** 110.52 | **LOC:** 198 | **CtrlFlow:** 18.1% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9004%), Tech Debt (98.6464%)
- **Heaviest Functions:** `checkNewVersionDocument` (Compute Cores, Impact: 27.3), `onReady` (Callbacks & Closures, Impact: 22.4), `waitUntilCssLoads` (Callbacks & Closures, Impact: 9.8)

### 6. `apps/meteor/client/lib/e2ee/rocketchat.e2e.room.ts` (TYPESCRIPT) -> Cumulative Risk: **735.53**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.10)
- **Magnitude:** 629.32 | **LOC:** 814 | **CtrlFlow:** 16.4% | **Authorship Centralization:** 20.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `decryptMessageContent` (Defensive Guards, Impact: 23.1), `filterMutation` (Defensive Guards, Impact: 16.0), `decrypt` (Defensive Guards, Impact: 14.7)

### 7. `packages/agenda/src/Job.ts` (TYPESCRIPT) -> Cumulative Risk: **734.32**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.04)
- **Magnitude:** 284.84 | **LOC:** 294 | **CtrlFlow:** 19.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.9995%)
- **Heaviest Functions:** `_computeFromInterval` (Defensive Guards, Impact: 21.0), `jobCallback` (Defensive Guards, Impact: 17.3), `run` (I/O & Config Routines, Impact: 13.7)

### 8. `apps/meteor/app/lib/server/functions/sendMessage.ts` (TYPESCRIPT) -> Cumulative Risk: **734.02**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.30)
- **Magnitude:** 216.68 | **LOC:** 299 | **CtrlFlow:** 23.1% | **Authorship Centralization:** 12.5%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), State Flux (97.7109%), Cognitive Load (90.6864%)
- **Heaviest Functions:** `sendMessage` (Many-Argument Workhorses, Impact: 52.9), `validateMessage` (Many-Argument Workhorses, Impact: 23.9), `prepareMessageObject` (Many-Argument Workhorses, Impact: 15.4)

### 9. `apps/meteor/app/api/server/helpers/getUserFromParams.ts` (TYPESCRIPT) -> Cumulative Risk: **731.66**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.04)
- **Magnitude:** 4.74 | **LOC:** 60 | **CtrlFlow:** 61.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `getUserFromParams` (Defensive Guards, Impact: 30.8), `getUserListFromParams` (Compute Cores, Impact: 21.1)

### 10. `apps/meteor/app/ui-message/client/messageBox/createComposerAPI.ts` (TYPESCRIPT) -> Cumulative Risk: **731.58**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.14)
- **Magnitude:** 241.72 | **LOC:** 359 | **CtrlFlow:** 13.2% | **Authorship Centralization:** 25.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9953%)
- **Heaviest Functions:** `setText` (Defensive Guards, Impact: 34.9), `replaceText` (Defensive Guards, Impact: 15.2), `wrapSelection` (Compute Cores, Impact: 12.2)
...
- **Risk Profile:** Cognitive Load (91.7895%), Tech Debt (0.0%)
...
- **Risk Profile:** Cognitive Load (20.7707%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (81.9045%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (14.315%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (90.6283%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `getSubscriptionForRoom` **(Tests & Verification)** (Impact: 12.0)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (53.7615%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (100.0%), Tech Debt (20.3009%)
**Contextual Mitigations & Amplifications:**
...
- `packages/apps-engine/src/server/managers/UIActionButtonManager.ts` -> **Severity: 0.25** (Bridge: 0.0027 * Flux: 93.9509%)
...
- `apps/meteor/client/lib/streamer/emitter.ts` -> **Severity: 4.7** (Embedded: 0.0675 * Error Risk: 69.6048%)
- `apps/meteor/tests/mocks/client/meteor.ts` -> **Severity: 3.593** (Embedded: 0.0556 * Error Risk: 64.5656%)
- `packages/core-services/src/lib/mongo.ts` -> **Severity: 2.669** (Embedded: 0.0368 * Error Risk: 72.5402%)
- `packages/livechat/src/lib/locale.js` -> **Severity: 2.171** (Embedded: 0.0286 * Error Risk: 75.925%)
- `packages/livechat/src/i18next.ts` -> **Severity: 2.162** (Embedded: 0.0338 * Error Risk: 63.9091%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `apps/meteor/tests/mocks/server/mongodb.ts` -> **Severity: 2420.7** (Blast Radius: 24.207 * Doc Risk: 100.0%)
- `apps/meteor/server/database/utils.ts` -> **Severity: 823.364** (Blast Radius: 9.998 * Doc Risk: 82.3529%)
- `apps/meteor/client/lib/streamer/emitter.ts` -> **Severity: 766.7** (Blast Radius: 7.667 * Doc Risk: 100.0%)
- `packages/livechat/src/i18next.ts` -> **Severity: 566.4** (Blast Radius: 5.664 * Doc Risk: 100.0%)
- `apps/meteor/client/lib/queryKeys.ts` -> **Severity: 479.5** (Blast Radius: 4.795 * Doc Risk: 100.0%)
