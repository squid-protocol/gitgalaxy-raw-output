...
> **File Composition:** Data / Markup / Trivial 38%, Declarative / Non-Code 22%, Large Core Modules 16%, Many-Argument Workhorses Files 11%, Compute Cores Files 6%
...
- **Heaviest Functions:** `keydb_add_resource` (Many-Argument Workhorses, Impact: 130.4), `internal_keydb_search` (Many-Argument Workhorses, Impact: 125.8), `maybe_create_keyring_or_box` (Many-Argument Workhorses, Impact: 90.5)
...
- **Heaviest Functions:** `_tlv_parser_next` (Many-Argument Workhorses, Impact: 82.8), `cram_octet_string` (Compute Cores, Impact: 54.3), `tlv_expect_object` (Many-Argument Workhorses, Impact: 36.6)

### 3. `g10/misc.c` (C) -> Cumulative Risk: **680.42**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.68)
...
- **Heaviest Functions:** `pct_expando` (Many-Argument Workhorses, Impact: 146.1), `parse_options` (Many-Argument Workhorses, Impact: 85.6), `map_cipher_openpgp_to_gcry` (Compute Cores, Impact: 53.0)
...
- **Heaviest Functions:** `keydb_search` (Many-Argument Workhorses, Impact: 149.9), `keydb_add_resource` (Many-Argument Workhorses, Impact: 84.9), `maybe_create_keybox` (Many-Argument Workhorses, Impact: 63.5)
...
- **Heaviest Functions:** `be_kbx_search` (Many-Argument Workhorses, Impact: 26.9), `be_kbx_update` (Many-Argument Workhorses, Impact: 26.2), `be_kbx_insert` (Many-Argument Workhorses, Impact: 25.9)
...
- **Heaviest Functions:** `percent_data_escape` (Many-Argument Workhorses, Impact: 73.0), `percent_plus_escape` (Compute Cores, Impact: 24.5), `do_unescape` (Many-Argument Workhorses, Impact: 23.7)

### 7. `common/convert.c` (C) -> Cumulative Risk: **671.08**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.67)
...
- **Heaviest Functions:** `hex2str` (Many-Argument Workhorses, Impact: 37.8), `hexcolon2bin` (Compute Cores, Impact: 35.6), `hex2fixedbuf` (Many-Argument Workhorses, Impact: 23.1)
...
- **Heaviest Functions:** `learn_status_cb` (Compute Cores, Impact: 285.5), `agent_pkdecrypt` (Many-Argument Workhorses, Impact: 116.5), `agent_get_passphrase` (Many-Argument Workhorses, Impact: 88.8)
...
- **Heaviest Functions:** `do_make_filename` (Many-Argument Workhorses, Impact: 100.2), `substitute_vars` (Many-Argument Workhorses, Impact: 53.2), `do_percent_escape` (Many-Argument Workhorses, Impact: 44.9)
...
- **Heaviest Functions:** `do_export_one_keyblock` (Many-Argument Workhorses, Impact: 391.1), `transfer_format_to_openpgp` (Many-Argument Workhorses, Impact: 219.7), `do_export_stream` (Many-Argument Workhorses, Impact: 158.8)
