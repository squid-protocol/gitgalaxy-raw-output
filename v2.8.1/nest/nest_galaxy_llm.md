...
> **Composition Archetype:** `Flat Modular Platform` (z -0.65; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 34%, Declarative / Non-Code 24%, State Mutators Files 9%, Interface Declarations Files 8%, Callbacks & Closures Files 6%
...
- `extractor` **(Tests & Verification)** (@ `integration/versioning/e2e/custom-versioning-fastify.spec.ts`) -> Impact: **95.9** | LOC: 956
...
- `extractor` **(Tests & Verification)** (@ `integration/versioning/e2e/custom-versioning.spec.ts`) -> Impact: **45.1** | LOC: 705
...
- **Heaviest Functions:** `create` (Compute Cores, Impact: 30.8), `ensureHttpServerExists` (Defensive Guards, Impact: 10.4), `constructor` (Defensive Guards, Impact: 9.2)

### 2. `packages/microservices/external/kafka.interface.ts` (TYPESCRIPT) -> Cumulative Risk: **649.88**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.34)
...
- **Heaviest Functions:** `produce` (Compute Cores, Impact: 12.1), `fetch` (Compute Cores, Impact: 10.7), `createTopics` (State Mutators, Impact: 6.0)
...
- **Heaviest Functions:** `constructor` (State Mutators, Impact: 7.4), `close` (Interface Declarations, Impact: 4.4), `create` (State Mutators, Impact: 3.5)
...
- **Heaviest Functions:** `wildcardMessageHandler` (Interface Declarations, Impact: 4.5), `sharedWildcardMessageHandler` (Interface Declarations, Impact: 4.5), `concurrent` (Callbacks & Closures, Impact: 3.5)
...
- **Heaviest Functions:** `on` (Generic / Templated Code, Impact: 7.5), `registerEndListener` (Callbacks & Closures, Impact: 7.0), `createRetryStrategy` (Compute Cores, Impact: 6.5)
...
- **Heaviest Functions:** `middie` (Many-Argument Workhorses, Impact: 70.6), `Holder` (Defensive Guards, Impact: 39.5), `done` (Defensive Guards, Impact: 38.5)
...
- **Heaviest Functions:** `setupChannel` (Compute Cores, Impact: 19.4), `handleMessage` (Defensive Guards, Impact: 15.4), `listener` (Defensive Guards, Impact: 11.7)
...
- **Heaviest Functions:** `handleMessage` (Compute Cores, Impact: 11.3), `constructor` (Compute Cores, Impact: 10.1), `combineStreamsAndThrowIfRetriable` (Callbacks & Closures, Impact: 10.0)
...
- **Heaviest Functions:** `mergePacketOptions` (Defensive Guards, Impact: 15.4), `publishPacket` (Defensive Guards, Impact: 11.9), `dispatchEvent` (Defensive Guards, Impact: 10.8)
...
- **Heaviest Functions:** `constructor` (Compute Cores, Impact: 10.3), `connect` (Callbacks & Closures, Impact: 7.5), `createResponseCallback` (Interface Declarations, Impact: 6.5)
...
**Top Internal Functions/Classes:**
  ...
  * `createMiddlewareFactory` **(Compute Cores)** (Impact: 21.2)
...
**Top Internal Functions/Classes:**
  * `writeObservableToGrpc` **(Compute Cores)** (Impact: 27.4)
    * *Intent:* /** * Writes an observable to a GRPC call. * * This function will ensure that backpressure is manage...
  * `collectDeepServices` **(Many-Argument Workhorses)** (Impact: 23.9)
...
**Top Internal Functions/Classes:**
  ...
  * `addProvider` **(Compute Cores)** (Impact: 15.7)
...
**Top Internal Functions/Classes:**
  ...
  * `use` **(Many-Argument Workhorses)** (Impact: 15.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `isStatic` **(Compute Cores)** (Impact: 31.2)
...
**Top Internal Functions/Classes:**
  ...
  * `printAsJson` **(Compute Cores)** (Impact: 20.4)
  * `getJsonLogObject` **(Compute Cores)** (Impact: 13.8)
  * `getInspectOptions` **(I/O & Config Routines)** (Impact: 13.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `validateExecutionCount` **(Defensive Guards)** (Impact: 5.1)
...
**Top Internal Functions/Classes:**
  * `scanForModules` **(Compute Cores)** (Impact: 27.5)
  * `reflectKeyMetadata` **(Many-Argument Workhorses)** (Impact: 15.2)
  * `insertProvider` **(Compute Cores)** (Impact: 12.8)
...
**Top Internal Functions/Classes:**
  ...
  * `subscribe` **(Interface Declarations)** (Impact: 1.5)
...
**Top Internal Functions/Classes:**
  ...
  * `dispatchEvent` **(Compute Cores)** (Impact: 10.5)
  * `mergeHeaders` **(Defensive Guards)** (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `formatAddress` **(Compute Cores)** (Impact: 13.8)
  * `setGlobalPrefix` **(Defensive Guards)** (Impact: 9.3)
  * `init` **(Interface Declarations)** (Impact: 7.1)
**Contextual Mitigations & Amplifications:**
...
- `packages/common/utils/shared.utils.ts` -> **Severity: 9.151** (Embedded: 0.1062 * Error Risk: 86.1586%)
- `packages/core/injector/instance-wrapper.ts` -> **Severity: 3.614** (Embedded: 0.0437 * Error Risk: 82.6031%)
- `packages/core/application-config.ts` -> **Severity: 3.454** (Embedded: 0.0421 * Error Risk: 82.0794%)
- `packages/core/injector/module.ts` -> **Severity: 3.375** (Embedded: 0.0425 * Error Risk: 79.4246%)
- `packages/core/injector/container.ts` -> **Severity: 2.537** (Embedded: 0.0305 * Error Risk: 83.2686%)
