...
| Analyzed Artifacts (Scanned) | 26459 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 3848 |
| Total LOC | 2098822 |
...
|---|---|---|
| Modularity | 0.5781 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
> **Composition Archetype:** `Flat Modular Platform` (z +1.96; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 41%, Interface Declarations Files 24%, Declarative / Non-Code 12%, Annotated Framework Methods Files 7%, Large Core Modules 6%
...
| connectivity | 74881 | 8234 | 6 | `stdlib/private/StdlibCollectionUnittest/MinimalCollections.swift` |
...
| docs | 169479 | 3487 | 3 | `include/swift/AST/Decl.h` |
...
| dead_code | 67029 | 10980 | 4 | `test/Runtime/null_vtable_entry_ptrauth.swift` |
| credential | 534 | 184 | 0 | `test/SILGen/objc_async_from_swift.swift` |
| threat | 14553 | 3660 | 1 | `test/attr/attr_objc.swift` |
...
- **Heaviest Functions:** `_unlock` (Compute Cores, Impact: 83.6), `yield` (Compute Cores, Impact: 58.4), `yield` (Compute Cores, Impact: 53.8)
...
- **Heaviest Functions:** `threadedExecute` (Many-Argument Workhorses, Impact: 27.6), `threadedExecute` (Many-Argument Workhorses, Impact: 14.2), `threadedExecute` (Many-Argument Workhorses, Impact: 8.0)
...
- **Heaviest Functions:** `completion` (Compute Cores, Impact: 14.9), `init` (Defensive Guards, Impact: 9.4), `withInputFiles` (Callbacks & Closures, Impact: 3.5)

### 4. `stdlib/public/core/Slice.swift` (SWIFT) -> Cumulative Risk: **711.63**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -1.17)
...
- **Heaviest Functions:** `replaceSubrange` (Generic / Templated Code, Impact: 8.2), `insert` (Generic / Templated Code, Impact: 8.0), `insert` (Compute Cores, Impact: 7.9)

### 5. `utils/swift_build_sdk_interfaces.py` (PYTHON) -> Cumulative Risk: **704.9**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -2.29)
...
- **Heaviest Functions:** `process_module` (Compute Cores, Impact: 22.4), `main` (I/O & Config Routines, Impact: 21.4), `process_module_files` (Compute Cores, Impact: 20.3)
...
- **Heaviest Functions:** `CompileJobCacheResult::forEachLoadedOutput` (Defensive Guards, Impact: 19.1), `CompileJobCacheResult::Builder::addOutput` (Compute Cores, Impact: 5.8), `CompileJobCacheResult::forEachOutput` (Interface Declarations, Impact: 4.8)
...
- **Heaviest Functions:** `should_install` (Compute Cores, Impact: 10.8), `_for_each_host_target` (Compute Cores, Impact: 8.4), `should_build` (Compute Cores, Impact: 7.3)
...
- **Heaviest Functions:** `SolverTrail::Change::dump` (Many-Argument Workhorses, Impact: 152.0), `SolverTrail::Change::undo` (Compute Cores, Impact: 146.3), `SolverTrail::dumpActiveScopeChanges` (Many-Argument Workhorses, Impact: 79.2)
...
- **Heaviest Functions:** `constraints::simplifyLocator` (Many-Argument Workhorses, Impact: 399.1), `transform` (Many-Argument Workhorses, Impact: 101.2), `ConstraintSystem::inferKeyPathLiteralCapability` (Compute Cores, Impact: 96.9)

### 10. `utils/swift_build_support/swift_build_support/products/swift_testing.py` (PYTHON) -> Cumulative Risk: **696.86**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.40)
...
- **Heaviest Functions:** `build` (Compute Cores, Impact: 10.2), `_for_each_host_target` (Compute Cores, Impact: 8.4), `_cmake_product` (Encapsulated Accessors, Impact: 2.2)
...
**Top Internal Functions/Classes:**
  * `ContextualFailure::diagnoseAsError` **(Compute Cores)** (Impact: 93.8)
  ...
  * `GenericArgumentsMismatchFailure::diagnoseAsError` **(Compute Cores)** (Impact: 73.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `DeclDeserializer::deserializeDeclCommon` **(Compute Cores)** (Impact: 196.9)
...
**Top Internal Functions/Classes:**
  ...
  * `RValueEmitter::visitKeyPathExpr` **(Compute Cores)** (Impact: 92.5)
