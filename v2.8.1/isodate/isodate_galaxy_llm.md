...
> **File Composition:** Defensive Guards Files 32%, Compute Cores Files 16%, Data / Markup / Trivial 16%, Declarative / Non-Code 16%, Large Core Modules 11%
...
- `__rsub__` **(Defensive Guards)** (@ `isodate-0.7.2/src/isodate/duration.py`) -> Impact: **15.9** | LOC: 41
...
- **Heaviest Functions:** `totimedelta` (Compute Cores, Impact: 16.7), `__add__` (Defensive Guards, Impact: 16.2), `__rsub__` (Defensive Guards, Impact: 15.9)
...
- **Heaviest Functions:** `parse_date` (Many-Argument Workhorses, Impact: 62.5), `build_date_regexps` (Many-Argument Workhorses, Impact: 13.4), `date_isoformat` (Parameter Forwarders, Impact: 2.4)
...
- **Heaviest Functions:** `parse_duration` (Many-Argument Workhorses, Impact: 44.5), `duration_isoformat` (Defensive Guards, Impact: 15.0)

### 4. `isodate-0.7.2/src/isodate/tzinfo.py` (PYTHON) -> Cumulative Risk: **524.93**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.44)
...
- **Heaviest Functions:** `utcoffset` (Compute Cores, Impact: 5.6), `dst` (Compute Cores, Impact: 5.6), `__init__` (Encapsulated Accessors, Impact: 2.6)

### 5. `isodate-0.7.2/src/isodate/isostrf.py` (PYTHON) -> Cumulative Risk: **486.59**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.56)
...
- **Heaviest Functions:** `_strfduration` (Many-Argument Workhorses, Impact: 54.4), `repl` (Compute Cores, Impact: 38.7), `_strfdt` (Compute Cores, Impact: 12.8)
...
- **Heaviest Functions:** `parse_time` (Type Conversions, Impact: 23.4), `build_time_regexps` (I/O & Config Routines, Impact: 4.5), `time_isoformat` (Parameter Forwarders, Impact: 2.1)
...
- **Heaviest Functions:** `tz_isoformat` (Compute Cores, Impact: 20.7), `build_tzinfo` (Many-Argument Workhorses, Impact: 14.2), `parse_tzinfo` (Compute Cores, Impact: 12.4)
...
- **Heaviest Functions:** `parse_datetime` (Defensive Guards, Impact: 2.3), `datetime_isoformat` (Parameter Forwarders, Impact: 2.2)
...
- **Heaviest Functions:** `test_format` (Defensive Guards, Impact: 7.2), `tz_patch` (Compute Cores, Impact: 5.9), `localtime_mock` (Compute Cores, Impact: 5.4)
...
**Top Internal Functions/Classes:**
  ...
  * `__add__` **(Defensive Guards)** (Impact: 16.2)
    * *Intent:* """ Durations can be added with Duration, timedelta, date and datetime objects. """
  * `__rsub__` **(Defensive Guards)** (Impact: 15.9)
...
**Top Internal Functions/Classes:**
  ...
  * `repl` **(Compute Cores)** (Impact: 38.7)
    * *Intent:* """ lookup format command and return corresponding replacement. """
  * `_strfdt` **(Compute Cores)** (Impact: 12.8)
...
**Top Internal Functions/Classes:**
  ...
  * `add_re` **(Interface Declarations)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `build_tzinfo` **(Many-Argument Workhorses)** (Impact: 14.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_isdst` **(Encapsulated Accessors)** (Impact: 2.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_pickle_datetime` **(Interface Declarations)** (Impact: 2.3)
    * *Intent:* """Parse an ISO datetime string and compare it to the expected value."""
  * `test_pickle_utc` **(Tests & Verification)** (Impact: 1.1)
