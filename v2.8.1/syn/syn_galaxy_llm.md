...
> **Composition Archetype:** `Small Flat Repo` (z -0.13; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 33%, Tests & Verification Files 17%, State Mutators Files 13%, Data / Markup / Trivial 12%, Declarative / Non-Code 6%
...
- `parse_impl` **(Compute Cores)** (@ `src/item.rs`) -> Impact: **84.4** | LOC: 95
...
- **Heaviest Functions:** `parse_lit_float` (Compute Cores, Impact: 44.1), `parse_lit_int` (Compute Cores, Impact: 41.2), `parse_lit_c_str_cooked` (Compute Cores, Impact: 29.3)
...
- **Heaviest Functions:** `punct_helper` (Many-Argument Workhorses, Impact: 23.2), `peek_punct` (Compute Cores, Impact: 20.0), `parse` (Defensive Guards, Impact: 7.8)
...
- **Heaviest Functions:** `to_string` (Compute Cores, Impact: 6.5), `add_assign` (Parameter Forwarders, Impact: 4.0), `mul_assign` (Parameter Forwarders, Impact: 4.0)

### 4. `src/whitespace.rs` (RUST) -> Cumulative Risk: **546.43**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.99)
...
- **Heaviest Functions:** `skip` (Compute Cores, Impact: 46.8), `is_whitespace` (State Mutators, Impact: 4.4)
...
- **Heaviest Functions:** `atom_expr` (Compute Cores, Impact: 149.3), `parse_expr` (Many-Argument Workhorses, Impact: 84.5), `parse_with_earlier_boundary_rule` (Compute Cores, Impact: 76.5)
...
- **Heaviest Functions:** `parse_terminated_with` (Generic / Templated Code, Impact: 15.2), `parse_separated_nonempty_with` (Generic / Templated Code, Impact: 11.5), `fold` (Defensive Guards, Impact: 9.4)
...
- **Heaviest Functions:** `node` (Many-Argument Workhorses, Impact: 54.6), `visit` (Many-Argument Workhorses, Impact: 34.8), `generate` (Annotated Framework Methods, Impact: 5.0)
...
- **Heaviest Functions:** `parse_rest_of_item` (Many-Argument Workhorses, Impact: 236.8), `parse_impl` (Compute Cores, Impact: 84.4), `parse` (Compute Cores, Impact: 75.5)
...
- **Heaviest Functions:** `skip` (Compute Cores, Impact: 8.1), `group` (Defensive Guards, Impact: 7.9), `create` (Defensive Guards, Impact: 7.8)
...
- **Heaviest Functions:** `parse_stmt` (Compute Cores, Impact: 103.8), `stmt_local` (Compute Cores, Impact: 33.7), `stmt_expr` (Many-Argument Workhorses, Impact: 31.9)
...
**Top Internal Functions/Classes:**
  ...
  * `trailer_helper` **(Compute Cores)** (Impact: 70.4)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_impl` **(Compute Cores)** (Impact: 84.4)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_multiple` **(Many-Argument Workhorses)** (Impact: 37.1)
...
**Top Internal Functions/Classes:**
  ...
  * `parse` **(Compute Cores)** (Impact: 16.3)
...
**Top Internal Functions/Classes:**
  ...
  * `fold` **(Defensive Guards)** (Impact: 9.4)
...
**Top Internal Functions/Classes:**
  ...
  * `rightmost_subexpression_precedence` **(Compute Cores)** (Impact: 13.3)
...
**Top Internal Functions/Classes:**
  ...
  * `peek_punct` **(Compute Cores)** (Impact: 20.0)
  * `parse` **(Defensive Guards)** (Impact: 7.8)
  * `keyword` **(Defensive Guards)** (Impact: 5.7)
  * `peek_keyword` **(Defensive Guards)** (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `introspect_type` **(Compute Cores)** (Impact: 17.0)
...
**Top Internal Functions/Classes:**
  ...
  * `test_fixup` **(Compute Cores)** (Impact: 45.5)
...
**Top Internal Functions/Classes:**
  ...
  * `base_dir_filter` **(Compute Cores)** (Impact: 12.6)
  * `edition` **(Annotated Framework Methods)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `format_field` **(Compute Cores)** (Impact: 23.8)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_meta_after_path` **(Compute Cores)** (Impact: 16.0)
  * `fmt` **(Compute Cores)** (Impact: 10.8)
  * `parse_inner` **(Generic / Templated Code)** (Impact: 7.2)
  * `single_parse_inner` **(Generic / Templated Code)** (Impact: 6.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parse` **(State Mutators)** (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `generate` **(Annotated Framework Methods)** (Impact: 4.6)
...
**Top Internal Functions/Classes:**
  ...
  * `generate` **(Annotated Framework Methods)** (Impact: 5.0)
  * `requires_full` **(State Mutators)** (Impact: 3.0)
...
**Top Internal Functions/Classes:**
  ...
  * `generate` **(Annotated Framework Methods)** (Impact: 4.4)
  * `requires_full` **(State Mutators)** (Impact: 3.0)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_unnamed` **(Annotated Framework Methods)** (Impact: 6.2)
    * *Intent:* /// Parses an unnamed (tuple struct) field.
  * `next` **(Annotated Framework Methods)** (Impact: 5.1)
...
**Top Internal Functions/Classes:**
  ...
  * `data_struct` **(Compute Cores)** (Impact: 27.2)
  * `to_tokens` **(Compute Cores)** (Impact: 10.7)
  * `data_enum` **(Generic / Templated Code)** (Impact: 5.0)
  * `data_union` **(Generic / Templated Code)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
