...
| Avg Path Length | 2.8882 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Flat Modular Platform` (z -0.46; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 78%, Large Core Modules 5%, Declarative / Non-Code 5%, State Mutators Files 3%, Callbacks & Closures Files 2%
...
| connectivity | 23162 | 2288 | 2 | `cli/tsc/dts/typescript.d.ts` |
...
| dead_code | 11974 | 1506 | 1 | `cli/tsc/dts/lib.webworker.d.ts` |
| credential | 548 | 246 | 0 | `tools/deno.lock.json` |
| threat | 3417 | 446 | 0 | `libs/core/core.d.ts` |
...
- `iterateSubpatterns` **(Compute Cores)** (@ `ext/node/polyfills/_fs/_fs_glob.ts`) -> Impact: **217.5** | LOC: 262
- `importKeyRSA` **(Many-Argument Workhorses)** (@ `ext/crypto/00_crypto.js`) -> Impact: **214.5** | LOC: 420
- `emitKeys` **(Compute Cores)** (@ `ext/node/polyfills/internal/readline/utils.mjs`) -> Impact: **208.1** | LOC: 456
  * *Intent:* */
...
- **Heaviest Functions:** `lookup` (Defensive Guards, Impact: 39.6), `createLookupPromise` (Many-Argument Workhorses, Impact: 21.8), `_resolve` (Many-Argument Workhorses, Impact: 13.2)
...
- **Heaviest Functions:** `WriteStream` (Many-Argument Workhorses, Impact: 33.1), `ReadStream` (Defensive Guards, Impact: 32.7), `_read` (Defensive Guards, Impact: 22.1)
...
- **Heaviest Functions:** `verifyOneShot` (Many-Argument Workhorses, Impact: 87.3), `signOneShot` (Many-Argument Workhorses, Impact: 57.2), `verify` (Many-Argument Workhorses, Impact: 22.3)

### 4. `ext/node/polyfills/internal/child_process.ts` (TYPESCRIPT) -> Cumulative Risk: **741.27**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.28)
...
- **Heaviest Functions:** `spawnInternal` (Many-Argument Workhorses, Impact: 131.9), `escapeShellArg` (Compute Cores, Impact: 117.8), `normalizeSpawnArguments` (Many-Argument Workhorses, Impact: 108.8)

### 5. `ext/fs/interface.rs` (RUST) -> Cumulative Risk: **738.03**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.30)
...
- **Heaviest Functions:** `from` (Compute Cores, Impact: 22.2), `write_file_sync` (Defensive Guards, Impact: 11.8), `write_file_async` (Defensive Guards, Impact: 11.8)

### 6. `ext/node/polyfills/http.ts` (TYPESCRIPT) -> Cumulative Risk: **729.84**
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 181.3), `matchKnownFields` (Compute Cores, Impact: 177.4), `_writeHeader` (Compute Cores, Impact: 91.5)
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 152.2), `__initWorkerThreads` (Many-Argument Workhorses, Impact: 60.6), `webMessagePortToNodeMessagePort` (Callbacks & Closures, Impact: 32.2)

### 8. `ext/node/polyfills/internal/stream_base_commons.ts` (TYPESCRIPT) -> Cumulative Risk: **722.25**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `onStreamRead` (Many-Argument Workhorses, Impact: 45.2), `handleWriteReq` (Many-Argument Workhorses, Impact: 31.9), `setStreamTimeout` (Many-Argument Workhorses, Impact: 18.1)

### 9. `ext/node/polyfills/internal/fs/handle.ts` (TYPESCRIPT) -> Cumulative Risk: **714.56**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.26)
- **Magnitude:** 356.92 | **LOC:** 579 | **CtrlFlow:** 16.4% | **Authorship Centralization:** 36.8%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9179%)
- **Heaviest Functions:** `readPromise` (Many-Argument Workhorses, Impact: 46.8), `readableWebStream` (Defensive Guards, Impact: 24.5), `writePromise` (Many-Argument Workhorses, Impact: 21.8)

### 10. `ext/node/ops/http.rs` (RUST) -> Cumulative Risk: **711.1**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `op_node_http_request_with_conn` (Many-Argument Workhorses, Impact: 110.7), `read` (Compute Cores, Impact: 23.4), `op_node_http_response_reclaim_conn` (Many-Argument Workhorses, Impact: 11.9)
...
- `tests/unit/test_util.ts` -> **Severity: 0.959** (Embedded: 0.0121 * Error Risk: 79.3013%)
- `ext/web/01_console.js` -> **Severity: 0.798** (Embedded: 0.0104 * Error Risk: 77.0386%)
- `ext/node/polyfills/internal/errors.ts` -> **Severity: 0.698** (Embedded: 0.0108 * Error Risk: 64.7449%)
- `ext/node/polyfills/internal/util/colors.ts` -> **Severity: 0.67** (Embedded: 0.0068 * Error Risk: 99.0075%)
- `ext/webidl/00_webidl.js` -> **Severity: 0.567** (Embedded: 0.0094 * Error Risk: 59.9908%)
