...
> **File Composition:** Large Core Modules 33%, Data / Markup / Trivial 22%, Declarative / Non-Code 17%, Parameter Forwarders Files 11%, Defensive Guards Files 6%
...
- `__init__` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/fallback.py`) -> Impact: **109.2** | LOC: 91
- `_unpack` **(Compute Cores)** (@ `msgpack-1.1.2/msgpack/fallback.py`) -> Impact: **66.0** | LOC: 73
...
- **Heaviest Functions:** `_pack_inner` (Many-Argument Workhorses, Impact: 123.8), `__init__` (Many-Argument Workhorses, Impact: 16.8), `_pack` (Compute Cores, Impact: 10.6)
...
- **Heaviest Functions:** `unpack_callback_ext` (Many-Argument Workhorses, Impact: 55.9), `unpack_callback_map_item` (Many-Argument Workhorses, Impact: 25.7), `unpack_callback_raw` (Many-Argument Workhorses, Impact: 13.2)
...
- **Heaviest Functions:** `unpack_execute` (Many-Argument Workhorses, Impact: 270.2), `unpack_construct` (Encapsulated Accessors, Impact: 2.4), `unpack_skip` (Encapsulated Accessors, Impact: 2.4)

### 4. `msgpack-1.1.2/msgpack/_unpacker.pyx` (PYTHON) -> Cumulative Risk: **628.23**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.62)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 64.0), `init_ctx` (Many-Argument Workhorses, Impact: 62.8), `unpackb` (Many-Argument Workhorses, Impact: 51.4)
...
- **Heaviest Functions:** `_pack` (Many-Argument Workhorses, Impact: 128.6), `__init__` (Many-Argument Workhorses, Impact: 109.2), `_unpack` (Compute Cores, Impact: 66.0)
...
- **Heaviest Functions:** `msgpack_pack_ext` (Many-Argument Workhorses, Impact: 32.6), `msgpack_pack_short` (Compute Cores, Impact: 20.6), `msgpack_pack_int` (Compute Cores, Impact: 20.6)
...
- **Heaviest Functions:** `unpack_container_header` (Many-Argument Workhorses, Impact: 56.2)
...
- **Heaviest Functions:** `from_bytes` (Compute Cores, Impact: 9.7), `__init__` (Defensive Guards, Impact: 9.1), `__new__` (Defensive Guards, Impact: 8.4)

### 9. `msgpack-1.1.2/msgpack/exceptions.py` (PYTHON) -> Cumulative Risk: **494.07**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.83)
...
- **Heaviest Functions:** `__init__` (Parameter Forwarders, Impact: 2.1), `__str__` (Interface Declarations, Impact: 1.5)

### 10. `msgpack-1.1.2/msgpack/pack.h` (C) -> Cumulative Risk: **477.56**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -0.57)
...
- **Heaviest Functions:** `msgpack_pack_write` (Defensive Guards, Impact: 7.1)
...
**Top Internal Functions/Classes:**
  ...
  * `unpack_init` **(I/O & Config Routines)** (Impact: 2.0)
  * `unpack_data` **(Interface Declarations)** (Impact: 1.6)
    * *Intent:* */
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `msgpack_pack_raw` **(Compute Cores)** (Impact: 14.8)
    * *Intent:* /* * Raw */
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `unpack_callback_ext` **(Many-Argument Workhorses)** (Impact: 55.9)
    * *Intent:* #include "datetime.h"
  * `unpack_callback_map_item` **(Many-Argument Workhorses)** (Impact: 25.7)
  * `unpack_callback_raw` **(Many-Argument Workhorses)** (Impact: 13.2)
  * `unpack_timestamp` **(Many-Argument Workhorses)** (Impact: 13.2)
    * *Intent:* /* * Unpack ext buffer to a timestamp. Pulled from msgpack-c timestamp.h. */
  * `unpack_callback_map` **(Many-Argument Workhorses)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `msgpack_pack_write` **(Defensive Guards)** (Impact: 7.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
**Top Internal Functions/Classes:**
  ...
  * `__str__` **(Interface Declarations)** (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
