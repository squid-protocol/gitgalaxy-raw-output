...
> **Composition Archetype:** `Hub-Coupled App` (z -0.38; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 42%, Large Core Modules 19%, Callbacks & Closures Files 11%, Compute Cores Files 10%, State Mutators Files 4%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 76.2 | 100.0 | 100.0 |
...
- `Next` **(Compute Cores)** (@ `internal/js_lexer/js_lexer.go`) -> Impact: **241.6** | LOC: 632
...
### 1. `lib/shared/common.ts` (TYPESCRIPT) -> Cumulative Risk: **785.83**
...
- **Heaviest Functions:** `buildOrContextImpl` (Many-Argument Workhorses, Impact: 196.2), `flagsForBuildOptions` (Many-Argument Workhorses, Impact: 160.2), `buildOrContextContinue` (Many-Argument Workhorses, Impact: 100.5)

### 2. `lib/npm/browser.ts` (TYPESCRIPT) -> Cumulative Risk: **777.78**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.55)
...
- **Heaviest Functions:** `initialize` (Callbacks & Closures, Impact: 9.3), `startRunningService` (Callbacks & Closures, Impact: 7.0), `onmessage` (Compute Cores, Impact: 6.1)

### 3. `lib/deno/wasm.ts` (TYPESCRIPT) -> Cumulative Risk: **774.06**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `startRunningService` (Callbacks & Closures, Impact: 8.8), `initialize` (Callbacks & Closures, Impact: 6.4), `onmessage` (Compute Cores, Impact: 6.1)

### 4. `lib/npm/node.ts` (TYPESCRIPT) -> Cumulative Risk: **758.26**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `extractProperties` (Compute Cores, Impact: 22.3), `esbuildCommandAndArgs` (Defensive Guards, Impact: 11.6), `transformSync` (Compute Cores, Impact: 11.1)
...
- **Heaviest Functions:** `getCachePath` (Compute Cores, Impact: 21.5), `extractFileFromTarGzip` (Defensive Guards, Impact: 11.4), `spawnNew` (Callbacks & Closures, Impact: 11.1)
...
- **Heaviest Functions:** `buildWasmLib` (Compute Cores, Impact: 17.7), `generateWorkerCode` (Callbacks & Closures, Impact: 13.3), `updateVersionGo` (Compute Cores, Impact: 13.2)
...
- **Heaviest Functions:** `ServeHTTP` (Many-Argument Workhorses, Impact: 152.8), `Serve` (Compute Cores, Impact: 94.8), `broadcastBuildResult` (Compute Cores, Impact: 46.1)

### 8. `lib/shared/worker.ts` (TYPESCRIPT) -> Cumulative Risk: **699.99**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.18)
- **Magnitude:** 111.84 | **LOC:** 103 | **CtrlFlow:** 23.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `read` (Many-Argument Workhorses, Impact: 19.0), `tryToInstantiateModule` (Compute Cores, Impact: 11.3), `writeSync` (Callbacks & Closures, Impact: 11.2)
...
- **Heaviest Functions:** `__decorateElement` (Defensive Guards, Impact: 130.9), `__runInitializers` (Callbacks & Closures, Impact: 9.2), `prettyPrint` (Defensive Guards, Impact: 7.5)

### 10. `scripts/decorator-tests.ts` (TYPESCRIPT) -> Cumulative Risk: **669.94**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.08)
...
- **Heaviest Functions:** `prettyPrint` (Defensive Guards, Impact: 7.5), `capture` (Callbacks & Closures, Impact: 7.4), `capture` (Callbacks & Closures, Impact: 6.9)
...
**Top Internal Functions/Classes:**
  * `scanImportsAndExports` **(Compute Cores)** (Impact: 189.8)
...
**Top Internal Functions/Classes:**
  ...
  * `registerClassPrivateTests` **(Defensive Guards)** (Impact: 21.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__runInitializers` **(Callbacks & Closures)** (Impact: 9.2)
  * `prettyPrint` **(Defensive Guards)** (Impact: 7.5)
  * `__decoratorContext` **(Compute Cores)** (Impact: 7.4)
  * `__privateMethod` **(Callbacks & Closures)** (Impact: 6.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `parseFile` **(Compute Cores)** (Impact: 225.0)
  * `processScannedFiles` **(Compute Cores)** (Impact: 154.8)
...
**Top Internal Functions/Classes:**
  ...
  * `capture` **(Callbacks & Closures)** (Impact: 6.9)
...
**Top Internal Functions/Classes:**
  ...
  * `createChannel` **(Compute Cores)** (Impact: 98.8)
...
**Top Internal Functions/Classes:**
  * `Next` **(Compute Cores)** (Impact: 241.6)
  ...
  * `parseNumericLiteralOrDot` **(Compute Cores)** (Impact: 120.3)
  * `NextInsideJSXElement` **(Compute Cores)** (Impact: 56.0)
  * `scanCommentText` **(Compute Cores)** (Impact: 46.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `FoldBinaryOperator` **(Compute Cores)** (Impact: 111.1)
    * *Intent:* // This function intentionally avoids mutating the input AST so it can be // called after the AST ha...
...
**Top Internal Functions/Classes:**
  ...
  * `TestRequireResolve` **(Tests & Verification)** (Impact: 16.8)
  * `TestKeepNamesAllForms` **(Compute Cores)** (Impact: 16.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `mangleRules` **(Compute Cores)** (Impact: 63.5)
  * `parseSelectorRule` **(Compute Cores)** (Impact: 48.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `serveImpl` **(Compute Cores)** (Impact: 21.4)
  * `parseOptionsForRun` **(Compute Cores)** (Impact: 18.0)
...
**Top Internal Functions/Classes:**
  * `TestTSNamespaceExports` **(Compute Cores)** (Impact: 73.3)
  * `TestTSClass` **(Compute Cores)** (Impact: 64.7)
...
**Top Internal Functions/Classes:**
  ...
  * `markSyntaxFeature` **(Compute Cores)** (Impact: 64.5)
...
**Top Internal Functions/Classes:**
  ...
  * `printTokens` **(Compute Cores)** (Impact: 99.9)
  ...
  * `printMediaQuery` **(Compute Cores)** (Impact: 73.2)
**Contextual Mitigations & Amplifications:**
