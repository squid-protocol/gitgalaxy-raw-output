...
> **Composition Archetype:** `Small Flat Repo` (z -0.44; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 26%, Defensive Guards Files 18%, Data / Markup / Trivial 15%, Generic / Templated Code Files 11%, Interface Declarations Files 9%
...
- `_process_chunk` **(Compute Cores)** (@ `google_api_core-2.30.2/google/api_core/_rest_streaming_base.py`) -> Impact: **43.7** | LOC: 42
...
- `_expand_variable_match` **(Defensive Guards)** (@ `google_api_core-2.30.2/google/api_core/path_template.py`) -> Impact: **29.9** | LOC: 37
...
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.28)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 30.3), `_cancel_operation` (Many-Argument Workhorses, Impact: 11.3), `_get_operation` (Many-Argument Workhorses, Impact: 11.2)
...
- **Heaviest Functions:** `retry_target_stream` (Many-Argument Workhorses, Impact: 51.3), `__call__` (Generic / Templated Code, Impact: 6.1), `retry_wrapped_func` (Generic / Templated Code, Impact: 2.5)
...
- **Heaviest Functions:** `create_channel` (Many-Argument Workhorses, Impact: 22.2), `wrap_errors` (Defensive Guards, Impact: 8.3), `_wrapped_aiter` (Defensive Guards, Impact: 3.3)
...
- **Heaviest Functions:** `_thread_main` (Many-Argument Workhorses, Impact: 21.6), `__iter__` (Compute Cores, Impact: 18.7), `_recoverable` (Many-Argument Workhorses, Impact: 10.7)
...
- **Heaviest Functions:** `_handle_refreshed_operation` (Defensive Guards, Impact: 14.0), `_refresh_and_update` (Compute Cores, Impact: 7.2), `make` (Many-Argument Workhorses, Impact: 4.6)
...
- **Heaviest Functions:** `__aiter__` (Generic / Templated Code, Impact: 4.6), `async_generator` (Interface Declarations, Impact: 3.2), `pages` (Generic / Templated Code, Impact: 3.1)

### 7. `google_api_core-2.30.2/google/api_core/operations_v1/operations_rest_client_async.py` (PYTHON) -> Cumulative Risk: **664.43**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.23)
...
- **Heaviest Functions:** `list_operations` (Many-Argument Workhorses, Impact: 16.2), `cancel_operation` (Many-Argument Workhorses, Impact: 7.7), `__init__` (Many-Argument Workhorses, Impact: 7.5)
...
- **Heaviest Functions:** `list_operations` (Many-Argument Workhorses, Impact: 10.0), `cancel_operation` (Many-Argument Workhorses, Impact: 8.5), `get_operation` (Many-Argument Workhorses, Impact: 8.2)
...
- **Heaviest Functions:** `_set_result_from_operation` (Compute Cores, Impact: 10.1), `_refresh_and_update` (Compute Cores, Impact: 7.5), `_refresh_http` (Many-Argument Workhorses, Impact: 5.0)
...
- **Heaviest Functions:** `_create_composite_credentials` (Many-Argument Workhorses, Impact: 39.1), `__call__` (Many-Argument Workhorses, Impact: 25.3), `create_channel` (Many-Argument Workhorses, Impact: 18.6)
...
**Top Internal Functions/Classes:**
  * `_generator_mock` **(Many-Argument Workhorses)** (Impact: 14.8)
  * `test___call___generator_retry_hitting_timeout` **(Many-Argument Workhorses)** (Impact: 13.9)
  * `test___call___with_iterable_throw` **(C Struct Operations)** (Impact: 8.3)
  ...
  * `test___call___with_iterable_close` **(C Struct Operations)** (Impact: 8.0)
...
**Top Internal Functions/Classes:**
  ...
  * `test_list_operations_rest_pager_async` **(Defensive Guards)** (Impact: 14.3)
  ...
  * `_get_operations_client` **(Compute Cores)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_fatal_exceptions_can_inform_consumer` **(Many-Argument Workhorses)** (Impact: 4.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_wrap_stream_errors_aiter` **(Tests & Verification)** (Impact: 3.0)
  * `test_wrap_stream_errors_aiter_non_rpc_error` **(Tests & Verification)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `done` **(Interface Declarations)** (Impact: 3.2)
  * `test_double_callback_concurrency` **(Interface Declarations)** (Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_parse_grpc_error_details` **(Defensive Guards)** (Impact: 11.9)
  * `from_grpc_error` **(Compute Cores)** (Impact: 8.4)
...
**Top Internal Functions/Classes:**
  * `_thread_main` **(Many-Argument Workhorses)** (Impact: 21.6)
  ...
  * `_recoverable` **(Many-Argument Workhorses)** (Impact: 10.7)
  ...
  * `_reopen` **(Compute Cores)** (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `api_request` **(Defensive Guards)** (Impact: 11.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_utcnow_monotonic` **(Interface Declarations)** (Impact: 2.3)
  * `test_wrap_method_with_custom_client_info` **(Tests & Verification)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_generator_mock` **(Many-Argument Workhorses)** (Impact: 12.5)
  ...
  * `test___call___with_generator_send` **(Defensive Guards)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  ...
  * `test___getitem___with_conditions` **(Tests & Verification)** (Impact: 3.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_expand_variable_match` **(Defensive Guards)** (Impact: 29.9)
    * *Intent:* """Expand a matched variable with its value. Args: positional_vars (list): A list of positional vari...
  * `_replace_variable_with_pattern` **(Compute Cores)** (Impact: 19.8)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Defensive Guards)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_get_default_mtls_endpoint` **(Compute Cores)** (Impact: 9.9)
    * *Intent:* """Converts api endpoint to mTLS endpoint. Convert "*.sandbox.googleapis.com" and "*.googleapis.com"...
  * `get_transport_class` **(Compute Cores)** (Impact: 8.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_field_mask_repeated_diffs` **(I/O & Config Routines)** (Impact: 2.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
**Top Internal Functions/Classes:**
  * `_field_mask_helper` **(Many-Argument Workhorses)** (Impact: 23.3)
  * `_set_field_on_message` **(Defensive Guards)** (Impact: 21.3)
...
**Top Internal Functions/Classes:**
  * `test_error_details_from_grpc_response_unknown_error` **(Tests & Verification)** (Impact: 5.0)
  ...
  * `test_from_grpc_error_bare_call` **(Interface Declarations)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 17 instances
