...
> **File Composition:** Data / Markup / Trivial 32%, Declarative / Non-Code 17%, Large Core Modules 12%, I/O & Config Routines Files 11%, Defensive Guards Files 9%
...
- `updateCulling` **(Compute Cores)** (@ `src/renderer/scene/culling.js`) -> Impact: **70.5** | LOC: 150
...
- **Heaviest Functions:** `spectatorLoop` (Compute Cores, Impact: 16.4), `updatePlayerSprite` (Defensive Guards, Impact: 15.5), `transitionCeilings` (Defensive Guards, Impact: 13.2)

### 2. `src/shared/maps.js` (JAVASCRIPT) -> Cumulative Risk: **647.16**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.73)
...
- **Heaviest Functions:** `loadMap` (Compute Cores, Impact: 11.7), `getNextMap` (Interface Declarations, Impact: 3.2), `applyPlayerStart` (State Mutators, Impact: 2.4)
...
- **Heaviest Functions:** `getSectorDamageAt` (Compute Cores, Impact: 20.5), `checkSectorDamage` (Defensive Guards, Impact: 13.6), `damagePlayer` (Compute Cores, Impact: 13.0)
...
- **Heaviest Functions:** `fireWeapon` (Compute Cores, Impact: 19.4), `rocketExplosion` (Compute Cores, Impact: 17.1), `checkWeaponHit` (I/O & Config Routines, Impact: 16.6)

### 5. `src/renderer/scene/entities/things.js` (JAVASCRIPT) -> Cumulative Risk: **580.83**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.24)
...
- **Heaviest Functions:** `buildThings` (I/O & Config Routines, Impact: 29.3)
...
- **Heaviest Functions:** `setupUnlock` (Callbacks & Closures, Impact: 7.5), `unlock` (Callbacks & Closures, Impact: 7.5), `playSound` (Callbacks & Closures, Impact: 6.2)
...
- **Heaviest Functions:** `nearby` (Compute Cores, Impact: 23.2), `initDebugMenu` (I/O & Config Routines, Impact: 18.4), `updateDebugStats` (I/O & Config Routines, Impact: 8.9)
...
- **Heaviest Functions:** `updateSingleEnemy` (Many-Argument Workhorses, Impact: 105.2), `pickMoveDirection` (Defensive Guards, Impact: 79.8), `moveEnemyToward` (Many-Argument Workhorses, Impact: 40.4)
...
- **Heaviest Functions:** `hasLineOfSight` (Many-Argument Workhorses, Impact: 66.8)

### 10. `index.js` (JAVASCRIPT) -> Cumulative Risk: **545.21**
...
- **Heaviest Functions:** `gameLoop` (Compute Cores, Impact: 8.1), `init` (Interface Declarations, Impact: 3.1), `debug` (Callbacks & Closures, Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `updateCulling` **(Compute Cores)** (Impact: 70.5)
...
**Top Internal Functions/Classes:**
  * `spectatorLoop` **(Compute Cores)** (Impact: 16.4)
...
**Top Internal Functions/Classes:**
  * `fireWeapon` **(Compute Cores)** (Impact: 19.4)
  ...
  * `findHitscanTarget` **(Many-Argument Workhorses)** (Impact: 15.3)
...
**Top Internal Functions/Classes:**
  ...
  * `damagePlayer` **(Compute Cores)** (Impact: 13.0)
  ...
  * `resetGameState` **(State Mutators)** (Impact: 1.5)
...
**Top Internal Functions/Classes:**
  ...
  * `releaseJoystick` **(Callbacks & Closures)** (Impact: 11.6)
...
**Top Internal Functions/Classes:**
  ...
  * `tryUseLift` **(I/O & Config Routines)** (Impact: 10.7)
  ...
  * `activateLift` **(Callbacks & Closures)** (Impact: 5.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `buildSectorAdjacency` **(I/O & Config Routines)** (Impact: 9.6)
    * *Intent:* /** * Builds the sector adjacency graph from the current map's linedefs/sidedefs. * Called once per ...
  * `isSectorAlerted` **(Interface Declarations)** (Impact: 1.6)
...
**Top Internal Functions/Classes:**
  ...
  * `applyPlayerStart` **(State Mutators)** (Impact: 2.4)
