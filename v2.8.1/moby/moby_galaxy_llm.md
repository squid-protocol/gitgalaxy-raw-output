...
> **File Composition:** Data / Markup / Trivial 31%, Callbacks & Closures Files 13%, Defensive Guards Files 12%, Large Core Modules 12%, Tests & Verification Files 10%
...
- `TestRunDisallowBindMountingRootToRoot` **(Compute Cores)** (@ `integration-cli/docker_cli_run_test.go`) -> Impact: **408.0** | LOC: 1344
...
- `TestBuildExposeMorePorts` **(Compute Cores)** (@ `integration-cli/docker_cli_build_test.go`) -> Impact: **256.5** | LOC: 1792
...
- **Heaviest Functions:** `Build` (Many-Argument Workhorses, Impact: 101.5), `toBuildkitPruneInfo` (Compute Cores, Impact: 32.9), `toBuildkitExtraHosts` (Compute Cores, Impact: 20.3)

### 2. `daemon/cluster/executor/container/adapter.go` (GO) -> Cumulative Risk: **709.21**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.42)
...
- **Heaviest Functions:** `pullImage` (Compute Cores, Impact: 33.1), `logs` (Compute Cores, Impact: 26.7), `waitNodeAttachments` (Compute Cores, Impact: 16.3)
...
- **Heaviest Functions:** `getDiskUsage` (Many-Argument Workhorses, Impact: 77.9), `getEvents` (Many-Argument Workhorses, Impact: 73.3), `getInfo` (Many-Argument Workhorses, Impact: 25.6)
...
- **Heaviest Functions:** `initService` (Defensive Guards, Impact: 25.7), `Fire` (Compute Cores, Impact: 24.8), `Execute` (Compute Cores, Impact: 21.0)
...
- **Heaviest Functions:** `BenchmarkConcurrentContainerActions` (Defensive Guards, Impact: 22.4), `BenchmarkLogsCLIRotateFollow` (Callbacks & Closures, Impact: 8.2), `TearDownTest` (Callbacks & Closures, Impact: 1.9)
...
- **Heaviest Functions:** `Client` (Compute Cores, Impact: 39.4), `checkTable` (Many-Argument Workhorses, Impact: 36.1), `doNetworkStatsQueue` (Compute Cores, Impact: 17.2)
...
- **Heaviest Functions:** `volumesToAPI` (Many-Argument Workhorses, Impact: 34.6), `filtersToBy` (Defensive Guards, Impact: 19.0), `withPrune` (Defensive Guards, Impact: 9.4)
...
- **Heaviest Functions:** `getTailReader` (Many-Argument Workhorses, Impact: 35.0), `Decode` (I/O & Config Routines, Impact: 12.6), `readRecord` (Defensive Guards, Impact: 8.1)

### 9. `daemon/logs.go` (GO) -> Cumulative Risk: **672.65**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -0.04)
...
- **Heaviest Functions:** `ContainerLogs` (Many-Argument Workhorses, Impact: 75.0), `mergeAndVerifyLogConfig` (Compute Cores, Impact: 9.5), `getLogger` (Defensive Guards, Impact: 6.7)
...
- **Heaviest Functions:** `withFetchProgress` (Many-Argument Workhorses, Impact: 42.8), `fetch` (Many-Argument Workhorses, Impact: 23.8), `applyLayer` (Defensive Guards, Impact: 17.5)
...
**Top Internal Functions/Classes:**
  * `TestBuildExposeMorePorts` **(Compute Cores)** (Impact: 256.5)
...
**Top Internal Functions/Classes:**
  ...
  * `UnmarshalJSON` **(Compute Cores)** (Impact: 58.5)
  ...
  * `validateConfiguration` **(Compute Cores)** (Impact: 29.6)
**Contextual Mitigations & Amplifications:**
