...
> **Composition Archetype:** `Flat Modular Platform` (z +1.67; from the repo's file-archetype mix)
> **File Composition:** I/O & Config Routines Files 28%, Many-Argument Workhorses Files 27%, Compute Cores Files 15%, Data / Markup / Trivial 11%, Defensive Guards Files 7%
...
- `__global_context__` **(Compute Cores)** (@ `fileio/ft_filetype.m`) -> Impact: **681.4** | LOC: 1268
...
- **Heaviest Functions:** `Serial` (Many-Argument Workhorses, Impact: 33.7), `read` (Defensive Guards, Impact: 23.4), `readBytesUntil` (Compute Cores, Impact: 13.9)
...
- **Heaviest Functions:** `makeuniform` (Compute Cores, Impact: 11.3), `Anonymous_Block` (Defensive Guards, Impact: 10.1), `Anonymous_Block` (I/O & Config Routines, Impact: 9.8)

### 3. `utilities/private/fixsource.m` (MATLAB) -> Cumulative Risk: **762.7**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.31)
...
- **Heaviest Functions:** `writeChannelSettings` (Compute Cores, Impact: 169.8), `syncWithHardware` (Compute Cores, Impact: 80.1), `interpretBinaryStream` (Compute Cores, Impact: 55.6)

### 5. `realtime/src/buffer/src/tcpsocket.c` (C) -> Cumulative Risk: **723.49**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.70)
...
- **Heaviest Functions:** `tcpsocket` (Compute Cores, Impact: 70.2), `cleanup_tcpsocket` (Compute Cores, Impact: 8.0)

### 6. `plotting/private/coordsys2label.m` (MATLAB) -> Cumulative Risk: **704.76**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.14)
...
- **Heaviest Functions:** `coordsys2label` (Many-Argument Workhorses, Impact: 87.1), `trim` (I/O & Config Routines, Impact: 3.3)

### 7. `private/equidistant_locate.m` (MATLAB) -> Cumulative Risk: **701.82**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.10)
...
- **Heaviest Functions:** `equidistant_locate` (Many-Argument Workhorses, Impact: 163.4), `uniformquadrant` (State Mutators, Impact: 2.3)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 106.4), `syncWithHardware` (Compute Cores, Impact: 16.8)
...
- **Heaviest Functions:** `ft_selectdata` (Many-Argument Workhorses, Impact: 199.9), `makeselection` (Many-Argument Workhorses, Impact: 142.8), `cellmatselect` (Many-Argument Workhorses, Impact: 77.2)
...
- **Heaviest Functions:** `nearest` (Many-Argument Workhorses, Impact: 44.8), `Anonymous_Block` (I/O & Config Routines, Impact: 10.0), `mbreal` (Compute Cores, Impact: 7.7)
...
**Top Internal Functions/Classes:**
  * `__global_context__` **(Compute Cores)** (Impact: 681.4)
    * *Intent:* % FT_FILETYPE determines the filetype of many EEG/MEG/MRI data files by % looking at the name, exten...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 121.5)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % start determining t...
  * `filetype_true` **(Compute Cores)** (Impact: 45.8)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTION that al...
  * `filetype_check_ced_spike6mat` **(Compute Cores)** (Impact: 45.5)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTION that ch...
  * `filetype_check_openvibe_mat` **(Compute Cores)** (Impact: 36.9)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTION that ch...
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
- **Magnitude:** 4143.4 | **LOC:** 3097 | **CtrlFlow:** 25.3% | **Authorship Centralization:** 60.0%
...
**Top Internal Functions/Classes:**
  ...
  * `__global_context__` **(Compute Cores)** (Impact: 162.7)
    * *Intent:* % FT_READ_HEADER reads header information from a variety of EEG, MEG and other time % series data fi...
...
**Top Internal Functions/Classes:**
  * `ft_geometryplot` **(Compute Cores)** (Impact: 228.4)
...
**Top Internal Functions/Classes:**
  * `__global_context__` **(Compute Cores)** (Impact: 150.5)
  ...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 32.1)
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` **(Compute Cores)** (Impact: 110.3)
  * `freq2raw` **(Compute Cores)** (Impact: 105.6)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % convert between dat...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 99.2)
...
**Top Internal Functions/Classes:**
  ...
  * `opto2lay` **(Many-Argument Workhorses)** (Impact: 200.0)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTION % conve...
  * `readlay` **(Compute Cores)** (Impact: 182.5)
...
**Top Internal Functions/Classes:**
  * `cb_redraw` **(Many-Argument Workhorses)** (Impact: 173.2)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%...
  * `cb_help` **(Compute Cores)** (Impact: 160.4)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%...
...
**Top Internal Functions/Classes:**
  ...
  * `__global_context__` **(I/O & Config Routines)** (Impact: 64.5)
    * *Intent:* % FT_READ_HEADSHAPE reads the fiducials and/or the measured headshape and/or meshes % that describe ...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 13.2)
...
**Top Internal Functions/Classes:**
  * `sync` **(Interface Declarations)** (Impact: 6.2)
...
**Top Internal Functions/Classes:**
  ...
  * `makeselection` **(Many-Argument Workhorses)** (Impact: 60.9)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % SUBFUNCTIONS %%%%%%%%%%%%...
  * `cellmatselect` **(Compute Cores)** (Impact: 48.0)
...
**Top Internal Functions/Classes:**
  ...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 127.4)
    * *Intent:* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ...
  * `__global_context__` **(I/O & Config Routines)** (Impact: 36.5)
    * *Intent:* % FT_CONNECTIVITYANALYSIS computes various measures of connectivity between % MEG/EEG channels or be...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 16.8)
