...
| Analyzed Artifacts (Scanned) | 5623 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 2065 |
| Total LOC | 1419492 |
...
|---|---|---|
| Modularity | 0.7828 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 5.0018 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
| Articulation Pts | 411 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| CPP | 3387 | 993785 | 60.2% |
| C | 838 | 289574 | 14.9% |
...
> **File Composition:** Large Core Modules 24%, Declarative / Non-Code 21%, Many-Argument Workhorses Files 13%, Data / Markup / Trivial 11%, Interface Declarations Files 8%
...
|---|---|---|
| Unclassified | 5369 | 95.5% |
| Unknown | 6 | 0.1% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 248 | 4.4% |

## 5. EXCLUDED ARTIFACTS (Unparsable or Shielded Files)
*Total Excluded Artifacts: 2065*
...
- `.markdown`: 285x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir)
- `.cmake`: 180x Excluded (Unsupported Extension: '.cmake'), 1x Unsupported Format (.cmake)
- `.html`: 70x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Machine-Generated Documentation Signature)
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 31.1 | 19.3 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 56.1 | 68.3 | 0.0 |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 9.5 | 0.6 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 66.0 | 100.0 | 100.0 |
...
|---|---|---|---|---|
| memory | 238439 | 3809 | 107 | `3rdparty/openjpeg/openjp2/j2k.c` |
| cleanup | 1363 | 386 | 0 | `modules/java/test/android_test/src/org/opencv/test/OpenCVTestCase.java` |
| guards | 127590 | 3919 | 54 | `3rdparty/include/vulkan/vulkan_core.h` |
| danger | 12949 | 1651 | 5 | `3rdparty/include/vulkan/vulkan_core.h` |
| concurrency | 2142 | 339 | 0 | `modules/gapi/test/streaming/gapi_streaming_utils_test.cpp` |
| connectivity | 20882 | 2747 | 10 | `3rdparty/libtiff/tiffio.h` |
| io | 2162 | 447 | 0 | `platforms/android/build_sdk.py` |
| crypto | 3 | 3 | 0 | `modules/python/test/tests_common.py` |
| ipc | 128 | 46 | 0 | `modules/gapi/test/streaming/gapi_streaming_tests.cpp` |
| time | 88 | 40 | 0 | `hal/openvx/hal/openvx_hal.cpp` |
...
| events | 1385 | 202 | 0 | `modules/gapi/src/api/kernels_core.cpp` |
| tests | 15856 | 562 | 0 | `modules/core/test/test_mat.cpp` |
| docs | 16314 | 2023 | 4 | `3rdparty/openjpeg/openjp2/openjpeg.h` |
| debt | 9607 | 1629 | 4 | `hal/riscv-rvv/src/imgproc/color.cpp` |
| mutation | 475677 | 4175 | 221 | `3rdparty/libjpeg/jidctint.c` |
| dead_code | 20207 | 2785 | 9 | `modules/ts/src/ts_gtest.cpp` |
| credential | 31 | 15 | 0 | `samples/dnn/models.yml` |
| threat | 15214 | 2178 | 4 | `3rdparty/include/vulkan/vulkan_core.h` |
| ml_ai | 3648 | 490 | 0 | `3rdparty/libjpeg-turbo/simd/mips64/loongson-mmintrin.h` |
| ui | 557 | 102 | 0 | `modules/highgui/src/window_QT.h` |
...
- **Unknown Dependencies:** `20391` packages imported that bypass the Zero-Trust whitelist.
...
- **Heaviest Functions:** `VideoCapture_obsensor::getProperty` (Compute Cores, Impact: 53.0), `VideoCapture_obsensor::VideoCapture_obsensor` (Compute Cores, Impact: 37.0), `VideoCapture_obsensor::retrieveFrame` (Compute Cores, Impact: 22.7)
...
- **Heaviest Functions:** `deliverTouchEvent` (Compute Cores, Impact: 21.1), `puzzleFrame` (Compute Cores, Impact: 13.3), `prepareGameSize` (Type Conversions, Impact: 7.8)

### 3. `samples/python/digits.py` (PYTHON) -> Cumulative Risk: **727.15**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -1.04)
...
- **Heaviest Functions:** `evaluate_model` (Many-Argument Workhorses, Impact: 9.9), `split2d` (Compute Cores, Impact: 6.4), `preprocess_hog` (Compute Cores, Impact: 5.3)
...
- **Heaviest Functions:** `initializeCamera` (Compute Cores, Impact: 42.0), `synchronized` (I/O & Config Routines, Impact: 25.6), `run` (Interface Declarations, Impact: 4.5)

### 5. `platforms/ios/build_visionos_framework.py` (PYTHON) -> Cumulative Risk: **717.8**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.20)
...
- **Heaviest Functions:** `getBuildCommand` (Type Conversions, Impact: 6.6), `checkCMakeVersion` (Defensive Guards, Impact: 2.9), `getCMakeArgs` (Parameter Forwarders, Impact: 2.3)
...
- **Heaviest Functions:** `RobustMatcher::symmetryTest` (Many-Argument Workhorses, Impact: 25.6), `RobustMatcher::fastRobustMatch` (Many-Argument Workhorses, Impact: 13.9), `RobustMatcher::robustMatch` (Many-Argument Workhorses, Impact: 9.0)
...
- **Heaviest Functions:** `ClfMilBoost::update` (Many-Argument Workhorses, Impact: 30.4), `ClfOnlineStump::update` (Many-Argument Workhorses, Impact: 23.2), `ClfMilBoost::classify` (Compute Cores, Impact: 13.7)
...
- **Heaviest Functions:** `captureOutput` (Many-Argument Workhorses, Impact: 63.5), `adjustLayoutToInterfaceOrientation` (Compute Cores, Impact: 27.2), `layoutPreviewLayer` (I/O & Config Routines, Impact: 22.1)

### 9. `modules/js/src/loader.js` (JAVASCRIPT) -> Cumulative Risk: **712.42**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.06)
...
- **Heaviest Functions:** `loadOpenCV` (Defensive Guards, Impact: 83.0)
...
- **Heaviest Functions:** `V4L2Context::queryUvcDeviceInfoList` (I/O & Config Routines, Impact: 25.9), `xioctl` (Compute Cores, Impact: 14.8), `V4L2StreamChannel::start` (C Struct Operations, Impact: 13.4)
...
**Top Internal Functions/Classes:**
  ...
  * `write_chunks_before_idat` **(Compute Cores)** (Impact: 157.1)
...
**Top Internal Functions/Classes:**
  ...
  * `CvDTree::split_node_data` **(Compute Cores)** (Impact: 82.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 0.621
...
**Top Internal Functions/Classes:**
  ...
  * `png_init_read_transformations` **(Compute Cores)** (Impact: 176.2)
...
**Top Internal Functions/Classes:**
  ...
  * `DescriptorBuilder::OptionInterpreter::SetOptionValue` **(Compute Cores)** (Impact: 158.2)
...
**Top Internal Functions/Classes:**
  * `QRDetect::getQuadrilateral` **(Compute Cores)** (Impact: 98.5)
...
**Top Internal Functions/Classes:**
  * `gemmImpl` **(Many-Argument Workhorses)** (Impact: 417.6)
  * `GEMMSingleMul` **(Many-Argument Workhorses)** (Impact: 221.2)
  * `MulTransposedR` **(Many-Argument Workhorses)** (Impact: 95.3)
    * *Intent:* #if !defined(CV_MULTRANSPOSED_BASELINE_ONLY) || defined(CV_CPU_BASELINE_MODE) /*********************...
  * `getMulTransposedFunc` **(Compute Cores)** (Impact: 76.5)
  * `perspectiveTransform_` **(Many-Argument Workhorses)** (Impact: 73.0)
    * *Intent:* /****************************************************************************************\ * Perspec...
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000177
...
- `modules/core/include/opencv2/core/utility.hpp` -> **Severity: 0.086** (Bridge: 0.0009 * Flux: 99.5502%)
- `modules/core/include/opencv2/core/base.hpp` -> **Severity: 0.049** (Bridge: 0.0005 * Flux: 99.9738%)
- `modules/core/include/opencv2/core/cvdef.h` -> **Severity: 0.039** (Bridge: 0.0006 * Flux: 61.7204%)
- `modules/core/include/opencv2/core/types.hpp` -> **Severity: 0.028** (Bridge: 0.0005 * Flux: 62.3308%)
- `modules/dnn/src/cuda4dnn/csl/stream.hpp` -> **Severity: 0.023** (Bridge: 0.0003 * Flux: 78.1336%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `3rdparty/flatbuffers/include/flatbuffers/vector.h` -> **Severity: 10.048** (Embedded: 0.1719 * Error Risk: 58.4467%)
- `modules/core/include/opencv2/core/base.hpp` -> **Severity: 9.552** (Embedded: 0.1082 * Error Risk: 88.304%)
- `modules/core/include/opencv2/core/utility.hpp` -> **Severity: 8.747** (Embedded: 0.0983 * Error Risk: 88.9841%)
- `3rdparty/flatbuffers/include/flatbuffers/buffer.h` -> **Severity: 8.702** (Embedded: 0.1287 * Error Risk: 67.6276%)
- `3rdparty/flatbuffers/include/flatbuffers/base.h` -> **Severity: 8.672** (Embedded: 0.1416 * Error Risk: 61.2515%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `3rdparty/flatbuffers/include/flatbuffers/vector.h` -> **Severity: 2667.4** (Blast Radius: 26.674 * Doc Risk: 100.0%)
- `3rdparty/flatbuffers/include/flatbuffers/string.h` -> **Severity: 2077.1** (Blast Radius: 20.771 * Doc Risk: 100.0%)
- `modules/core/include/opencv2/core/cvdef.h` -> **Severity: 1958.56** (Blast Radius: 24.482 * Doc Risk: 80.0%)
- `3rdparty/flatbuffers/include/flatbuffers/base.h` -> **Severity: 1928.921** (Blast Radius: 20.773 * Doc Risk: 92.8571%)
- `modules/core/src/algorithm.cpp` -> **Severity: 1478.1** (Blast Radius: 14.781 * Doc Risk: 100.0%)
