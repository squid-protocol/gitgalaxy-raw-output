...
> **Composition Archetype:** `Small Flat Repo` (z -0.33; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 36%, Declarative / Non-Code 23%, Large Core Modules 17%, I/O & Config Routines Files 15%, Many-Argument Workhorses Files 4%
...
- `main` **(Compute Cores)** (@ `recipes/demos/sdl2-gears/gears.c`) -> Impact: **68.9** | LOC: 200
...
- **Heaviest Functions:** `run_tui_cook` (Many-Argument Workhorses, Impact: 225.5), `parse_args` (Compute Cores, Impact: 167.9), `repo_inner` (Many-Argument Workhorses, Impact: 41.9)
...
- **Heaviest Functions:** `new_recursive` (Many-Argument Workhorses, Impact: 59.0), `guess_version` (Defensive Guards, Impact: 19.7), `new` (Many-Argument Workhorses, Impact: 17.8)
...
- **Heaviest Functions:** `load_cached_repo` (Compute Cores, Impact: 10.6), `download_increment` (Type Conversions, Impact: 6.5), `fetch_end` (Interface Declarations, Impact: 4.6)
...
- **Heaviest Functions:** `build` (Many-Argument Workhorses, Impact: 225.3), `auto_deps_from_dynamic_linking` (Many-Argument Workhorses, Impact: 108.5), `build_remote` (Many-Argument Workhorses, Impact: 50.0)

### 5. `mk/qemu.mk` (MAKEFILE) -> Cumulative Risk: **580.42**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.74)
...
- **Heaviest Functions:** `qemu-deps` (I/O & Config Routines, Impact: 4.2), `qemu-deps` (I/O & Config Routines, Impact: 2.1), `qemu-deps` (I/O & Config Routines, Impact: 2.1)
...
- **Heaviest Functions:** `walk_tree_entry` (Many-Argument Workhorses, Impact: 70.7), `walk_file_tree` (Many-Argument Workhorses, Impact: 30.0), `display_pkg_fn` (Many-Argument Workhorses, Impact: 9.8)

### 7. `src/config.rs` (RUST) -> Cumulative Risk: **570.29**
...
- **Heaviest Functions:** `init_config` (I/O & Config Routines, Impact: 18.1), `translate_mirror` (Defensive Guards, Impact: 11.5), `extract_env` (Defensive Guards, Impact: 5.5)

### 8. `src/cook/package.rs` (RUST) -> Cumulative Risk: **566.63**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `package` (Many-Argument Workhorses, Impact: 65.6), `package_toml` (Many-Argument Workhorses, Impact: 46.8), `package_handle_push` (Many-Argument Workhorses, Impact: 17.9)

### 9. `src/cook/fs.rs` (RUST) -> Cumulative Risk: **561.1**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `get_git_remote_tracking` (Compute Cores, Impact: 44.0), `move_dir_all_inner_fn` (Generic / Templated Code, Impact: 29.4), `check_files_present` (Compute Cores, Impact: 20.2)
...
- **Heaviest Functions:** `new_recursive_nonstop` (Compute Cores, Impact: 29.0), `new_recursive` (Compute Cores, Impact: 16.9), `from_path` (Defensive Guards, Impact: 7.8)
...
**Top Internal Functions/Classes:**
  ...
  * `main_inner` **(Compute Cores)** (Impact: 30.8)
...
**Top Internal Functions/Classes:**
  ...
  * `fetch_offline` **(Compute Cores)** (Impact: 40.2)
  * `fetch_apply_patches` **(Many-Argument Workhorses)** (Impact: 19.0)
  * `fetch_extract_tar` **(Many-Argument Workhorses)** (Impact: 15.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `fedora` **(Compute Cores)** (Impact: 27.3)
    * *Intent:* ############################################################################### # This function take...
...
**Top Internal Functions/Classes:**
  * `main` **(Compute Cores)** (Impact: 68.9)
...
**Top Internal Functions/Classes:**
  ...
  * `package_target` **(State Mutators)** (Impact: 4.6)
...
**Top Internal Functions/Classes:**
  ...
  * `sync` **(Interface Declarations)** (Impact: 2.8)
  * `init` **(Interface Declarations)** (Impact: 2.5)
  * `reshape` **(Encapsulated Accessors)** (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_args` **(Interface Declarations)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_5` (Drift: 0.0 IQR)
- **Magnitude:** 125.3 | **LOC:** 255 | **CtrlFlow:** 11.0% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (19.7377%), Tech Debt (69.7059%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `is_unwanted_arch` **(Many-Argument Workhorses)** (Impact: 18.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `cloexec` **(Interface Declarations)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `get_hostname` **(State Mutators)** (Impact: 2.0)
  * `get_short_commit` **(State Mutators)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `fetch_end` **(Interface Declarations)** (Impact: 4.6)
...
**Top Internal Functions/Classes:**
  * `new_recursive_nonstop` **(Compute Cores)** (Impact: 29.0)
...
**Top Internal Functions/Classes:**
  ...
  * `prefix` **(I/O & Config Routines)** (Impact: 1.1)
