...
|---|---|---|
| Modularity | 0.4515 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.1278 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 1.6879 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Flat Modular Platform` (z +0.12; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 34%, Declarative / Non-Code 19%, Interface Declarations Files 14%, Compute Cores Files 9%, Parameter Forwarders Files 6%
...
| dead_code | 3954 | 2496 | 1 | `tool/net/definitions.lua` |
...
- `main` **(Compute Cores)** (@ `examples/romanize.c`) -> Impact: **734.2** | LOC: 931
...
- **Heaviest Functions:** `__maps_unmark` (C Struct Operations, Impact: 17.7), `__maps_mark` (C Struct Operations, Impact: 17.5), `__maps_balloc` (Defensive Guards, Impact: 12.1)

### 2. `tool/cosmocc/bin/cosmocross` (SHELL) -> Cumulative Risk: **710.76**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +2.60)
...
- **Heaviest Functions:** `log_command` (Compute Cores, Impact: 126.9), `fatal_error_[Truncated]` (Interface Declarations, Impact: 1.8), `__global_context__` (I/O & Config Routines, Impact: 1.4)

### 3. `libc/intrin/cursor.c` (C) -> Cumulative Risk: **709.64**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z -0.49)
...
- **Heaviest Functions:** `__cursor_unref` (Compute Cores, Impact: 6.5), `__cursor_new` (Interface Declarations, Impact: 6.0), `__cursor_ref` (Interface Declarations, Impact: 3.1)

### 4. `libc/intrin/clock_gettime-freebsd.c` (C) -> Cumulative Risk: **701.53**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Encapsulated Accessors Files` (z +1.07)
...
- **Heaviest Functions:** `sys_clock_gettime_freebsd_vdso` (C Struct Operations, Impact: 29.2), `binuptime` (C Struct Operations, Impact: 19.6), `gettc` (Compute Cores, Impact: 18.9)
...
- **Heaviest Functions:** `sys_kill_nt` (Compute Cores, Impact: 45.9)

### 6. `libc/intrin/fds.c` (C) -> Cumulative Risk: **699.73**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.84)
...
- **Heaviest Functions:** `__init_fds` (Compute Cores, Impact: 38.8), `TokAtoi` (Compute Cores, Impact: 11.3), `SetupWinStd` (Compute Cores, Impact: 8.5)

### 7. `net/http/ssh.c` (C) -> Cumulative Risk: **699.61**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.09)
...
- **Heaviest Functions:** `ParseOpensshKnownHost` (Many-Argument Workhorses, Impact: 38.6), `ParseOpensshKnownHosts` (Many-Argument Workhorses, Impact: 28.9), `ParseOpensshPublicKey` (Many-Argument Workhorses, Impact: 27.9)
...
- **Heaviest Functions:** `_pthread_cancel_sig` (Many-Argument Workhorses, Impact: 19.9), `_pthread_cancel_single` (Compute Cores, Impact: 14.5), `pthread_testcancel_np` (Interface Declarations, Impact: 8.8)
...
- **Heaviest Functions:** `sys_fork_nt_parent` (Compute Cores, Impact: 83.4), `sys_fork_nt_child` (Compute Cores, Impact: 20.4), `ViewOrDie` (Defensive Guards, Impact: 10.4)
...
- **Heaviest Functions:** `cosmo_trace_save` (Compute Cores, Impact: 11.4), `cosmo_trace_event` (Many-Argument Workhorses, Impact: 7.2), `cosmo_trace_begin` (Compute Cores, Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `RenderingTick` **(Compute Cores)** (Impact: 75.6)
  * `ReadKeyboard` **(Compute Cores)** (Impact: 70.0)
...
**Top Internal Functions/Classes:**
  ...
  * `kloghandle` **(Compute Cores)** (Impact: 31.9)
...
**Top Internal Functions/Classes:**
  ...
  * `ConsumeWhitespace` **(Many-Argument Workhorses)** (Impact: 11.9)
  * `CharsetAdd` **(Defensive Guards)** (Impact: 11.3)
  * `Buffer` **(Defensive Guards)** (Impact: 11.2)
  * `IsSpace` **(Compute Cores)** (Impact: 8.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `detectX86FamilyModel` **(Many-Argument Workhorses)** (Impact: 8.6)
**Contextual Mitigations & Amplifications:**
