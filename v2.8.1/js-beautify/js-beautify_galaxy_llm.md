...
> **Composition Archetype:** `Small Flat Repo` (z +1.82; from the repo's file-archetype mix)
> **File Composition:** Callbacks & Closures Files 38%, Defensive Guards Files 21%, Data / Markup / Trivial 18%, Compute Cores Files 9%, Parameter Forwarders Files 9%
...
- `beautify` **(Compute Cores)** (@ `package/js/src/css/beautifier.js`) -> Impact: **167.3** | LOC: 386
...
- **Heaviest Functions:** `run_tests` (Callbacks & Closures, Impact: 46.9), `unpack_chunk` (Defensive Guards, Impact: 6.7), `get_chunks` (Callbacks & Closures, Impact: 5.9)
...
- **Heaviest Functions:** `get_regexp` (Defensive Guards, Impact: 12.9), `read` (Compute Cores, Impact: 12.7), `__match` (Defensive Guards, Impact: 9.3)
...
- **Heaviest Functions:** `ensure_empty_line_above` (Defensive Guards, Impact: 11.1), `trim` (Defensive Guards, Impact: 9.2), `get_code` (Defensive Guards, Impact: 8.3)
...
- **Heaviest Functions:** `_read_template` (Compute Cores, Impact: 33.6), `TemplatablePattern` (Compute Cores, Impact: 11.7), `__set_templated_pattern` (I/O & Config Routines, Impact: 10.6)

### 5. `package/js/src/html/beautifier.js` (JAVASCRIPT) -> Cumulative Risk: **671.77**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.53)
...
- **Heaviest Functions:** `TagOpenParserToken` (Many-Argument Workhorses, Impact: 303.2), `_set_tag_position` (Many-Argument Workhorses, Impact: 275.2), `_do_optional_end_element` (Defensive Guards, Impact: 83.1)
...
- **Heaviest Functions:** `handle_word` (Defensive Guards, Impact: 249.0), `handle_operator` (Defensive Guards, Impact: 121.4), `handle_start_expr` (Defensive Guards, Impact: 103.9)
...
- **Heaviest Functions:** `detect` (Callbacks & Closures, Impact: 15.5), `run_tests` (Callbacks & Closures, Impact: 3.1), `starts_with` (Callbacks & Closures, Impact: 1.9)
...
- **Heaviest Functions:** `Pattern` (State Mutators, Impact: 4.2), `read` (Callbacks & Closures, Impact: 3.4), `until_after` (Callbacks & Closures, Impact: 1.8)

### 9. `package/js/src/javascript/tokenizer.js` (JAVASCRIPT) -> Cumulative Risk: **604.55**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.70)
...
- **Heaviest Functions:** `Tokenizer` (Compute Cores, Impact: 11.6), `in_array` (Defensive Guards, Impact: 1.9)

### 10. `package/js/src/core/tokenstream.js` (JAVASCRIPT) -> Cumulative Risk: **595.81**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.57)
...
- **Heaviest Functions:** `peek` (Callbacks & Closures, Impact: 6.1), `add` (Callbacks & Closures, Impact: 3.2), `next` (Callbacks & Closures, Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `beautify` **(Compute Cores)** (Impact: 167.3)
    * *Intent:* /*_____________________--------------------_____________________*/
...
**Top Internal Functions/Classes:**
  ...
  * `test` **(Callbacks & Closures)** (Impact: 9.2)
...
**Top Internal Functions/Classes:**
  * `_read_template` **(Compute Cores)** (Impact: 33.6)
  ...
  * `read_options` **(Callbacks & Closures)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_fix_quotes` **(Tests & Verification)** (Impact: 16.2)
...
**Top Internal Functions/Classes:**
  * `Options` **(Compute Cores)** (Impact: 19.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 14 instances
...
**Top Internal Functions/Classes:**
  * `Options` **(Compute Cores)** (Impact: 8.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `get_beautify` **(Callbacks & Closures)** (Impact: 2.9)
    * *Intent:* **/
