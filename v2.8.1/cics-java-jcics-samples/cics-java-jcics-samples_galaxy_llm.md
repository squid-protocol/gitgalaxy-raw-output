...
> **Composition Archetype:** `Small Flat Repo` (z +0.31; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 47%, I/O & Config Routines Files 21%, Defensive Guards Files 11%, Interface Declarations Files 7%, Large Core Modules 7%
...
- `main` **(Compute Cores)** (@ `projects/com.ibm.cicsdev.vsam/src/com/ibm/cicsdev/vsam/ksds/KsdsExample5.java`) -> Impact: **11.7** | LOC: 64
...
- `MAIN-PROCESSING` **(I/O & Config Routines)** (@ `src/Cobol/EDUCHAN.cbl`) -> Impact: **10.4** | LOC: 87
  * *Intent:* * -----------------------------------------------------------
- `main` **(Compute Cores)** (@ `projects/com.ibm.cicsdev.vsam/src/com/ibm/cicsdev/vsam/esds/EsdsExample5.java`) -> Impact: **10.0** | LOC: 58
  * *Intent:* /** * Main entry point to a CICS OSGi program. * * The FQ name of this class should be added to the CICS-MainClass entry in * the parent OSGi bundle's...
...
### 1. `src/Cobol/EC01.cbl` (COBOL) -> Cumulative Risk: **467.84**
...
- **Heaviest Functions:** `A-CONTROL` (I/O & Config Routines, Impact: 4.7), `ZZX-CICS-ERROR-ROUTINE` (I/O & Config Routines, Impact: 3.8), `ZZX-EXIT` (Interface Declarations, Impact: 1.1)

### 2. `src/Cobol/EDUCHAN.cbl` (COBOL) -> Cumulative Risk: **467.68**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.68)
- **Magnitude:** 31.94 | **LOC:** 173 | **CtrlFlow:** 5.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.0149%), Safety Score (66.0756%)
- **Heaviest Functions:** `MAIN-PROCESSING` (I/O & Config Routines, Impact: 10.4), `ABEND-ROUTINE` (I/O & Config Routines, Impact: 1.4), `RESP-ERROR` (Interface Declarations, Impact: 1.2)
...
- **Heaviest Functions:** `generate` (I/O & Config Routines, Impact: 3.0), `generateDescription` (Interface Declarations, Impact: 2.1), `getKey` (Interface Declarations, Impact: 2.0)
...
- **Heaviest Functions:** `readFromQueue` (I/O & Config Routines, Impact: 4.8), `writeToQueue` (I/O & Config Routines, Impact: 3.7), `main` (I/O & Config Routines, Impact: 2.7)
...
- **Heaviest Functions:** `readFromQueue` (I/O & Config Routines, Impact: 4.8), `writeToQueue` (I/O & Config Routines, Impact: 3.7), `main` (Interface Declarations, Impact: 2.5)

### 6. `projects/com.ibm.cicsdev.vsam/src/com/ibm/cicsdev/vsam/ksds/KsdsExample5.java` (JAVA) -> Cumulative Risk: **301.73**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.53)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 11.7)
...
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (88.0797%), Safety Score (61.3692%), Tech Debt (37.7541%)
- **Heaviest Functions:** `main` (Compute Cores, Impact: 10.0)
...
- **Heaviest Functions:** `main` (Defensive Guards, Impact: 11.3), `buildChannel` (I/O & Config Routines, Impact: 2.2), `linkProg` (Defensive Guards, Impact: 1.9)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 7.0), `getTerminalString` (I/O & Config Routines, Impact: 4.8), `parseTerminalString` (Interface Declarations, Impact: 3.6)
...
- **Heaviest Functions:** `main` (I/O & Config Routines, Impact: 3.3), `linkProg` (Defensive Guards, Impact: 2.0), `buildCommarea` (Interface Declarations, Impact: 1.7)
...
**Top Internal Functions/Classes:**
  ...
  * `ReturnCode` **(State Mutators)** (Impact: 1.6)
...
**Top Internal Functions/Classes:**
  ...
  * `writeToQueue` **(I/O & Config Routines)** (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  * `readFromQueue` **(I/O & Config Routines)** (Impact: 3.8)
...
**Top Internal Functions/Classes:**
  * `readFromQueue` **(I/O & Config Routines)** (Impact: 3.3)
    * *Intent:* /** * Simple read of string data from a TSQ. */
  * `writeToQueue` **(I/O & Config Routines)** (Impact: 3.0)
    * *Intent:* /** * Write of Java string data to a TSQ. */
  * `main` **(I/O & Config Routines)** (Impact: 2.7)
    * *Intent:* /** * Main entry point to a CICS OSGi program. * * The FQ name of this class should be added to the ...
...
**Top Internal Functions/Classes:**
  ...
  * `parseTerminalString` **(Interface Declarations)** (Impact: 3.6)
...
**Top Internal Functions/Classes:**
  * `updateQueue` **(I/O & Config Routines)** (Impact: 5.2)
...
**Top Internal Functions/Classes:**
  * `main` **(Compute Cores)** (Impact: 11.7)
    * *Intent:* /** * Main entry point to a CICS OSGi program. * * The FQ name of this class should be added to the ...
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
* *State Mutation (weighted view):* 6
