...
> **Composition Archetype:** `Typed Library` (z -0.70; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 46%, Data / Markup / Trivial 19%, Generic / Templated Code Files 13%, Large Core Modules 11%, Defensive Guards Files 5%
...
- `from_string` **(Compute Cores)** (@ `docutils-0.22.4/docutils/utils/_roman_numerals.py`) -> Impact: **69.1** | LOC: 101
...
- **Heaviest Functions:** `visit_image` (Compute Cores, Impact: 58.6), `should_be_compact_paragraph` (Defensive Guards, Impact: 29.2), `visit_field_list` (Defensive Guards, Impact: 20.4)
...
- **Heaviest Functions:** `visit_inline` (Defensive Guards, Impact: 21.9), `visit_literal` (Compute Cores, Impact: 18.8), `visit_topic` (Defensive Guards, Impact: 11.2)
...
- **Heaviest Functions:** `list_start` (Compute Cores, Impact: 30.8), `astext` (Compute Cores, Impact: 22.9), `depart_document` (Compute Cores, Impact: 15.2)
...
- **Heaviest Functions:** `starttag` (Many-Argument Workhorses, Impact: 61.5), `visit_math` (Many-Argument Workhorses, Impact: 45.1), `visit_image` (Many-Argument Workhorses, Impact: 39.6)
...
- **Heaviest Functions:** `make_field_element` (Many-Argument Workhorses, Impact: 50.7), `generate_image` (Many-Argument Workhorses, Impact: 46.5), `visit_image` (Compute Cores, Impact: 32.6)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 67.8), `encode` (Compute Cores, Impact: 49.3), `visit_literal_block` (Many-Argument Workhorses, Impact: 45.5)
...
- **Heaviest Functions:** `build_contents` (Many-Argument Workhorses, Impact: 25.6), `apply` (Defensive Guards, Impact: 15.4), `update_section_numbers` (Many-Argument Workhorses, Impact: 14.4)

### 8. `docutils-0.22.4/docutils/parsers/rst/directives/images.py` (PYTHON) -> Cumulative Risk: **665.96**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.77)
...
- **Heaviest Functions:** `run` (Defensive Guards, Impact: 28.4), `run` (Compute Cores, Impact: 23.6), `figwidth_value` (Interface Declarations, Impact: 4.5)
...
- **Heaviest Functions:** `run` (Defensive Guards, Impact: 20.8), `run` (Defensive Guards, Impact: 12.9), `run` (Defensive Guards, Impact: 6.6)
...
- **Heaviest Functions:** `keys` (Compute Cores, Impact: 59.7), `go` (Compute Cores, Impact: 28.9), `getIncrementals` (Compute Cores, Impact: 21.5)
...
**Top Internal Functions/Classes:**
  ...
  * `visit_image` **(Compute Cores)** (Impact: 32.6)
...
**Top Internal Functions/Classes:**
  ...
  * `encode` **(Compute Cores)** (Impact: 49.3)
  ...
  * `visit_entry` **(Compute Cores)** (Impact: 34.0)
...
**Top Internal Functions/Classes:**
  * `list_start` **(Compute Cores)** (Impact: 30.8)
  ...
  * `_visit_reference_with_macro` **(Compute Cores)** (Impact: 13.3)
...
**Top Internal Functions/Classes:**
  * `visit_image` **(Compute Cores)** (Impact: 58.6)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_latex_math` **(Compute Cores)** (Impact: 50.2)
...
**Top Internal Functions/Classes:**
  * `apply` **(Compute Cores)** (Impact: 38.5)
  * `visit_reference` **(Compute Cores)** (Impact: 33.7)
...
**Top Internal Functions/Classes:**
  ...
  * `check_args` **(Compute Cores)** (Impact: 18.2)
...
**Top Internal Functions/Classes:**
  ...
  * `relative_path` **(Compute Cores)** (Impact: 13.0)
...
**Top Internal Functions/Classes:**
  ...
  * `get_csv_data` **(Compute Cores)** (Impact: 20.4)
...
**Top Internal Functions/Classes:**
  ...
  * `structure_from_cells` **(Compute Cores)** (Impact: 14.7)
...
**Top Internal Functions/Classes:**
  * `apply` **(Compute Cores)** (Impact: 51.7)
...
**Top Internal Functions/Classes:**
  ...
  * `transfer_attributes` **(Compute Cores)** (Impact: 9.3)
