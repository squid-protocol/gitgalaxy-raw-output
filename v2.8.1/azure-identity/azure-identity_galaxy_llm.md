...
> **Composition Archetype:** `Typed Library` (z +0.13; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 27%, Defensive Guards Files 20%, Large Core Modules 16%, Tests & Verification Files 10%, Interface Declarations Files 6%
...
- `__init__` **(Many-Argument Workhorses)** (@ `azure_identity-1.25.3/azure/identity/_credentials/managed_identity.py`) -> Impact: **46.4** | LOC: 79
- `__init__` **(Many-Argument Workhorses)** (@ `azure_identity-1.25.3/azure/identity/aio/_credentials/managed_identity.py`) -> Impact: **46.4** | LOC: 78
- `__init__` **(Many-Argument Workhorses)** (@ `azure_identity-1.25.3/azure/identity/_credentials/on_behalf_of.py`) -> Impact: **45.6** | LOC: 49
- `__init__` **(Many-Argument Workhorses)** (@ `azure_identity-1.25.3/azure/identity/aio/_credentials/on_behalf_of.py`) -> Impact: **43.3** | LOC: 44
- `resolve_tenant` **(Many-Argument Workhorses)** (@ `azure_identity-1.25.3/azure/identity/_internal/utils.py`) -> Impact: **42.8** | LOC: 51
- `__init__` **(Many-Argument Workhorses)** (@ `azure_identity-1.25.3/azure/identity/_internal/msal_credentials.py`) -> Impact: **39.8** | LOC: 37
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.15)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 38.0), `get_token` (Many-Argument Workhorses, Impact: 6.6), `get_token_info` (Type Conversions, Impact: 5.6)
...
- **Heaviest Functions:** `post` (Many-Argument Workhorses, Impact: 22.5), `raise_for_status` (Compute Cores, Impact: 16.5), `_store_auth_error` (Defensive Guards, Impact: 10.8)

### 3. `azure_identity-1.25.3/samples/credential_creation_code_snippets.py` (PYTHON) -> Cumulative Risk: **737.48**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.88)
...
- **Heaviest Functions:** `create_certificate_credential` (Interface Declarations, Impact: 2.9), `create_certificate_credential_async` (Interface Declarations, Impact: 2.9), `create_workload_identity_credential` (Interface Declarations, Impact: 1.8)
...
- **Heaviest Functions:** `get_client_credential` (Many-Argument Workhorses, Impact: 39.5), `load_pkcs12_certificate` (Defensive Guards, Impact: 11.6), `__init__` (Generic / Templated Code, Impact: 2.8)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 43.3), `_request_token` (Defensive Guards, Impact: 5.0), `_acquire_token_silently` (Generic / Templated Code, Impact: 2.1)
...
- **Heaviest Functions:** `_get_token_base` (Many-Argument Workhorses, Impact: 21.5), `get_token` (Many-Argument Workhorses, Impact: 14.1), `run_command_line` (Defensive Guards, Impact: 10.3)
...
- **Heaviest Functions:** `_get_token_base` (Many-Argument Workhorses, Impact: 31.8), `get_token` (Many-Argument Workhorses, Impact: 9.9), `close` (Generic / Templated Code, Impact: 3.1)
...
- **Heaviest Functions:** `_get_token_base` (Many-Argument Workhorses, Impact: 31.9), `get_token` (Many-Argument Workhorses, Impact: 8.8), `__init__` (Generic / Templated Code, Impact: 6.2)
...
- **Heaviest Functions:** `get_client` (Generic / Templated Code, Impact: 5.7), `send` (Generic / Templated Code, Impact: 4.0), `get_unavailable_message` (Generic / Templated Code, Impact: 1.8)

### 10. `azure_identity-1.25.3/azure/identity/aio/_credentials/azure_cli.py` (PYTHON) -> Cumulative Risk: **686.02**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.24)
...
- **Heaviest Functions:** `_get_token_base` (Many-Argument Workhorses, Impact: 33.9), `_run_command` (Defensive Guards, Impact: 28.3), `get_token` (Many-Argument Workhorses, Impact: 14.1)
...
**Top Internal Functions/Classes:**
  ...
  * `test_empty_cache` **(Tests & Verification)** (Impact: 7.8)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 22.3)
  * `get_cached_access_token` **(Many-Argument Workhorses)** (Impact: 19.6)
    * *Intent:* # Do not return a cached token if claims are provided. if kwargs.get("claims"): return None tenant =...
  * `_get_on_behalf_of_request` **(Many-Argument Workhorses)** (Impact: 18.8)
  * `_get_refresh_token_on_behalf_of_request` **(Many-Argument Workhorses)** (Impact: 18.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_imds_retry_policy` **(Compute Cores)** (Impact: 12.0)
...
**Top Internal Functions/Classes:**
  ...
  * `send` **(Defensive Guards)** (Impact: 5.7)
    * *Intent:* # ensure the `claims` and `tenant_id` keywords from credential's `get_token` method don't make it to...
  * `load_settings` **(I/O & Config Routines)** (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_get_account` **(Many-Argument Workhorses)** (Impact: 30.8)
  * `_get_cached_access_token` **(Type Conversions)** (Impact: 19.1)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 18.3)
  * `_initialize_cache` **(Defensive Guards)** (Impact: 16.8)
    * *Intent:* # If no cache options were provided, the default cache will be used. This credential accepts the # u...
  * `_filtered_accounts` **(Generic / Templated Code)** (Impact: 13.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_authority` **(Tests & Verification)** (Impact: 11.2)
...
**Top Internal Functions/Classes:**
  * `test_retries_token_requests` **(Tests & Verification)** (Impact: 7.5)
    * *Intent:* """The client should retry token requests"""
  * `test_error_reporting` **(Tests & Verification)** (Impact: 4.3)
  * `test_multitenant_cache` **(I/O & Config Routines)** (Impact: 4.2)
  * `test_request_url` **(Defensive Guards)** (Impact: 4.1)
  * `test_exceptions_do_not_expose_secrets` **(Tests & Verification)** (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_authority` **(Tests & Verification)** (Impact: 11.1)
    * *Intent:* """the credential should accept authority configuration by keyword argument or environment"""
...
**Top Internal Functions/Classes:**
  * `test_chain_attempts_all_credentials` **(Tests & Verification)** (Impact: 4.2)
  * `test_context_manager` **(Tests & Verification)** (Impact: 3.5)
  * `test_managed_identity_imds_probe` **(Tests & Verification)** (Impact: 3.5)
  * `test_close` **(Tests & Verification)** (Impact: 3.4)
  * `test_chain_raises_for_unexpected_error` **(Tests & Verification)** (Impact: 3.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_run_command` **(Defensive Guards)** (Impact: 31.9)
