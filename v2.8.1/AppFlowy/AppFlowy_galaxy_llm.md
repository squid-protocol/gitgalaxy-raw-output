...
> **Composition Archetype:** `Flat Modular Platform` (z -0.29; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 62%, State Mutators Files 10%, Large Core Modules 8%, Generic / Templated Code Files 6%, Declarative / Non-Code 5%
...
- `getPositionForChild` **(Compute Cores)** (@ `frontend/appflowy_flutter/packages/appflowy_popover/lib/src/layout.dart`) -> Impact: **79.9** | LOC: 143
...
- `getPositionForChild` **(Compute Cores)** (@ `frontend/appflowy_flutter/packages/flowy_infra_ui/lib/src/flowy_overlay/flowy_popover_layout.dart`) -> Impact: **69.5** | LOC: 108
- `getPositionForChild` **(Compute Cores)** (@ `frontend/appflowy_flutter/packages/flowy_infra_ui/lib/src/flowy_overlay/layout.dart`) -> Impact: **69.5** | LOC: 108
...
- **Heaviest Functions:** `add_or_remove_row_when_cell_changed` (Many-Argument Workhorses, Impact: 22.4), `delete_row` (Generic / Templated Code, Impact: 7.0), `can_group` (State Mutators, Impact: 6.5)
...
- **Heaviest Functions:** `process_next_task` (Compute Cores, Impact: 16.1), `run` (Compute Cores, Impact: 9.7), `add_task` (Compute Cores, Impact: 4.3)
...
- **Heaviest Functions:** `create_or_delete_group_when_cell_changed` (Many-Argument Workhorses, Impact: 19.9), `add_or_remove_row_when_cell_changed` (Many-Argument Workhorses, Impact: 13.3), `delete_row` (Generic / Templated Code, Impact: 11.5)
...
- **Heaviest Functions:** `apply_changeset` (Defensive Guards, Impact: 22.0), `cmp_row` (Many-Argument Workhorses, Impact: 19.3), `did_create_row` (Compute Cores, Impact: 10.0)

### 5. `frontend/rust-lib/lib-infra/src/ref_map.rs` (RUST) -> Cumulative Risk: **664.32**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.94)
...
- **Heaviest Functions:** `remove` (Defensive Guards, Impact: 9.5), `insert` (Defensive Guards, Impact: 6.4), `get` (Generic / Templated Code, Impact: 1.9)
...
- **Heaviest Functions:** `connect_server` (Compute Cores, Impact: 5.5), `remove_server` (Defensive Guards, Impact: 5.5), `tool_list` (Generic / Templated Code, Impact: 3.8)
...
- **Heaviest Functions:** `delete_group` (Defensive Guards, Impact: 11.2), `create_group` (Defensive Guards, Impact: 7.7), `update_type_option_when_update_group` (Many-Argument Workhorses, Impact: 7.2)
...
- **Heaviest Functions:** `apply_filter` (Many-Argument Workhorses, Impact: 41.4), `apply_changeset` (Defensive Guards, Impact: 29.7), `fill_cells` (Defensive Guards, Impact: 22.8)

### 10. `frontend/rust-lib/flowy-storage/src/uploader.rs` (RUST) -> Cumulative Risk: **639.59**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.98)
...
- **Heaviest Functions:** `process_next` (Compute Cores, Impact: 25.8), `run` (Compute Cores, Impact: 17.6), `remove_task` (Many-Argument Workhorses, Impact: 14.3)
...
**Top Internal Functions/Classes:**
  ...
  * `update_field_type_option_handler` **(Defensive Guards)** (Impact: 13.1)
  * `update_relation_cell_handler` **(Compute Cores)** (Impact: 12.6)
  * `get_primary_field_handler` **(Generic / Templated Code)** (Impact: 11.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `get_user_profile_handler` **(Compute Cores)** (Impact: 10.1)
...
**Top Internal Functions/Classes:**
  ...
  * `update_rag_ids` **(Many-Argument Workhorses)** (Impact: 15.6)
...
**Top Internal Functions/Classes:**
  * `create_view_handler` **(Generic / Templated Code)** (Impact: 9.3)
  * `create_orphan_view_handler` **(Generic / Templated Code)** (Impact: 9.3)
  * `update_recent_views_handler` **(Generic / Templated Code)** (Impact: 7.6)
  * `set_default_publish_view_handler` **(Generic / Templated Code)** (Impact: 7.6)
  * `remove_user_from_shared_page_handler` **(Generic / Templated Code)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `user_sign_in_with_url` **(Defensive Guards)** (Impact: 10.0)
  * `get_user_awareness_doc_state` **(Many-Argument Workhorses)** (Impact: 9.8)
...
**Top Internal Functions/Classes:**
  ...
  * `test_chunked_bytes_large_file` **(Tests & Verification)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `get_question_id_from_answer_id` **(Many-Argument Workhorses)** (Impact: 13.2)
...
**Top Internal Functions/Classes:**
  ...
  * `process_search_results` **(Compute Cores)** (Impact: 17.1)
...
**Top Internal Functions/Classes:**
  ...
  * `update_group` **(Many-Argument Workhorses)** (Impact: 3.4)
