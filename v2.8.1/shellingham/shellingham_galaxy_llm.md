...
> **File Composition:** Data / Markup / Trivial 30%, Defensive Guards Files 20%, Large Core Modules 20%, Compute Cores Files 10%, Declarative / Non-Code 10%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 38.3 | 16.7 | 0.0 |
...
- `_get_interpreter_shell` **(Compute Cores)** (@ `shellingham-1.5.4/src/shellingham/posix/__init__.py`) -> Impact: **13.0** | LOC: 18
...
- `_get_shell` **(Compute Cores)** (@ `shellingham-1.5.4/src/shellingham/posix/__init__.py`) -> Impact: **11.2** | LOC: 16
...
- `iter_process_parents` **(Compute Cores)** (@ `shellingham-1.5.4/src/shellingham/posix/proc.py`) -> Impact: **6.0** | LOC: 17
  * *Intent:* """Try to look up the process tree via the /proc interface."""
- `_iter_process_parents` **(Defensive Guards)** (@ `shellingham-1.5.4/src/shellingham/posix/__init__.py`) -> Impact: **5.8** | LOC: 13
...
- `_iter_process_parents` **(Compute Cores)** (@ `shellingham-1.5.4/src/shellingham/posix/proc.py`) -> Impact: **5.6** | LOC: 8
...
### 1. `shellingham-1.5.4/src/shellingham/posix/proc.py` (PYTHON) -> Cumulative Risk: **539.33**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.8499%), Safety Score (97.2501%)
- **Heaviest Functions:** `iter_process_parents` (Compute Cores, Impact: 6.0), `_iter_process_parents` (Compute Cores, Impact: 5.6), `_get_ppid` (Type Conversions, Impact: 3.9)

### 2. `shellingham-1.5.4/src/shellingham/nt.py` (PYTHON) -> Cumulative Risk: **530.71**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (94.6508%)
- **Heaviest Functions:** `get_shell` (Defensive Guards, Impact: 12.0), `check` (Compute Cores, Impact: 6.3), `_check_expected` (Interface Declarations, Impact: 4.7)

### 3. `shellingham-1.5.4/src/shellingham/posix/ps.py` (PYTHON) -> Cumulative Risk: **489.51**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -0.80)
- **Magnitude:** 41.62 | **LOC:** 52 | **CtrlFlow:** 19.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.8499%), Safety Score (80.8455%)
- **Heaviest Functions:** `iter_process_parents` (Defensive Guards, Impact: 15.9)

### 4. `shellingham-1.5.4/src/shellingham/__init__.py` (PYTHON) -> Cumulative Risk: **479.88**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -0.41)
- **Magnitude:** 23.08 | **LOC:** 24 | **CtrlFlow:** 15.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9849%), Safety Score (70.2063%)
- **Heaviest Functions:** `detect_shell` (Defensive Guards, Impact: 7.7)

### 5. `shellingham-1.5.4/src/shellingham/posix/__init__.py` (PYTHON) -> Cumulative Risk: **458.09**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.47)
- **Magnitude:** 86.04 | **LOC:** 113 | **CtrlFlow:** 31.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (96.0638%), Tech Debt (53.2847%)
- **Heaviest Functions:** `_get_interpreter_shell` (Compute Cores, Impact: 13.0), `_get_shell` (Compute Cores, Impact: 11.2), `get_shell` (Type Conversions, Impact: 7.3)

### 6. `shellingham-1.5.4/tests/test_posix.py` (PYTHON) -> Cumulative Risk: **327.33**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (77.7778%), Safety Score (72.8429%), Stability (50.0%)
- **Heaviest Functions:** `patch` (Compute Cores, Impact: 5.5), `unpatch` (Defensive Guards, Impact: 4.6), `test_get_shell` (Defensive Guards, Impact: 2.4)

### 7. `shellingham-1.5.4/src/shellingham/_core.py` (PYTHON) -> Cumulative Risk: **259.54**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (66.5013%), Stability (50.0%), Api Exposure (23.9466%)

### 8. `shellingham-1.5.4/src/shellingham/posix/_core.py` (PYTHON) -> Cumulative Risk: **129.65**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
...
### 9. `shellingham-1.5.4/setup.py` (PYTHON) -> Cumulative Risk: **52.3**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Data / Markup / Trivial` (z +0.00)
...
- **Magnitude:** 117.24 | **LOC:** 164 | **CtrlFlow:** 9.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (58.9199%), Tech Debt (21.9173%)
**Top Internal Functions/Classes:**
  * `get_shell` **(Defensive Guards)** (Impact: 12.0)
  * `check` **(Compute Cores)** (Impact: 6.3)
  * `_check_expected` **(Interface Declarations)** (Impact: 4.7)
  * `_get_full_path` **(Interface Declarations)** (Impact: 4.6)
...
- **Magnitude:** 61.58 | **LOC:** 84 | **CtrlFlow:** 13.6% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (37.0649%), Tech Debt (99.8499%)
**Top Internal Functions/Classes:**
  * `iter_process_parents` **(Compute Cores)** (Impact: 6.0)
    * *Intent:* """Try to look up the process tree via the /proc interface."""
  * `_iter_process_parents` **(Compute Cores)** (Impact: 5.6)
    * *Intent:* # Inner generator function so we correctly throw an error eagerly if proc # is not supported, rather...
  * `_get_ppid` **(Type Conversions)** (Impact: 3.9)
  * `detect_proc` **(Interface Declarations)** (Impact: 3.7)
    * *Intent:* """Detect /proc filesystem style. This checks the /proc/{pid} directory for possible formats. Return...
  * `_get_cmdline` **(Type Conversions)** (Impact: 3.3)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_0` (Drift: 0.0 IQR)
- **Magnitude:** 41.62 | **LOC:** 52 | **CtrlFlow:** 19.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (50.9162%), Tech Debt (99.8499%)
**Top Internal Functions/Classes:**
