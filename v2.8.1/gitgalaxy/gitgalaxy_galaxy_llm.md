...
| Avg Path Length | 1.569 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Hub-Coupled App` (z +0.56; from the repo's file-archetype mix)
> **File Composition:** Defensive Guards Files 25%, Declarative / Non-Code 18%, Data / Markup / Trivial 14%, Large Core Modules 14%, Tests & Verification Files 9%
...
- **Heaviest Functions:** `_slice_by_braces` (Many-Argument Workhorses, Impact: 1131.1), `_calculate_block_metrics` (Many-Argument Workhorses, Impact: 314.8), `splice` (Many-Argument Workhorses, Impact: 266.7)

### 2. `gitgalaxy/core/prism.py` (PYTHON) -> Cumulative Risk: **586.4**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.02)
...
- **Heaviest Functions:** `_strip_single_line_comments` (Many-Argument Workhorses, Impact: 124.1), `_mask_perl_line` (Compute Cores, Impact: 75.4), `_compile_regex_matrix` (Many-Argument Workhorses, Impact: 59.5)

### 3. `gitgalaxy/recorders/llm_recorder.py` (PYTHON) -> Cumulative Risk: **576.91**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.36)
...
- **Heaviest Functions:** `_build_markdown` (Many-Argument Workhorses, Impact: 773.7), `generate_artifacts` (Many-Argument Workhorses, Impact: 37.7), `_generate_sqlite_graph` (Many-Argument Workhorses, Impact: 13.4)
...
- **Heaviest Functions:** `calculate_risk_vector` (Many-Argument Workhorses, Impact: 250.1), `summarize_galaxy_metrics` (Many-Argument Workhorses, Impact: 193.3), `generate_forensic_report` (Many-Argument Workhorses, Impact: 69.1)
...
- **Heaviest Functions:** `execute_pipeline` (Many-Argument Workhorses, Impact: 275.4), `_process_file_worker` (Compute Cores, Impact: 135.5), `_resolve_dependency_graph` (Compute Cores, Impact: 105.8)
...
- **Heaviest Functions:** `generate_report` (Many-Argument Workhorses, Impact: 264.4), `descale` (Defensive Guards, Impact: 14.1), `format_label` (Compute Cores, Impact: 7.6)
...
- **Heaviest Functions:** `record_mission` (Many-Argument Workhorses, Impact: 119.0), `__init__` (Generic / Templated Code, Impact: 7.2), `_intern` (Generic / Templated Code, Impact: 4.2)
...
- **Heaviest Functions:** `inspect` (Many-Argument Workhorses, Impact: 353.4), `_tier_4_heuristic_discovery` (Many-Argument Workhorses, Impact: 122.9), `_evaluate_ecosystem_gravity` (Many-Argument Workhorses, Impact: 81.6)
...
- **Heaviest Functions:** `audit` (Many-Argument Workhorses, Impact: 151.5), `__init__` (Many-Argument Workhorses, Impact: 10.7), `_is_dead_code` (Defensive Guards, Impact: 8.0)
...
- **Heaviest Functions:** `main` (I/O & Config Routines, Impact: 31.5), `process_payload` (Many-Argument Workhorses, Impact: 24.7), `record_dead_code` (Many-Argument Workhorses, Impact: 14.4)
...
**Top Internal Functions/Classes:**
  ...
  * `_process_file_worker` **(Compute Cores)** (Impact: 135.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_outbound` **(Defensive Guards)** (Impact: 4.4)
**Contextual Mitigations & Amplifications:**
* *Sec High Risk Execution:* 1 instances
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_build_folded_resolution_map` **(Generic / Templated Code)** (Impact: 12.0)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Compute Cores)** (Impact: 5.9)
...
**Top Internal Functions/Classes:**
  * `_resolve_dependency_graph` **(Compute Cores)** (Impact: 50.2)
  ...
  * `get_nth_degree` **(Compute Cores)** (Impact: 10.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_iter_candidate_files` **(Compute Cores)** (Impact: 11.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_initialize_history_scan` **(Compute Cores)** (Impact: 9.1)
    * *Intent:* """Dispatches the survey engines to establish boundaries and churn cache."""
  * `_load_ignored_revs` **(Defensive Guards)** (Impact: 7.9)
