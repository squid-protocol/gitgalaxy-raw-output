...
> **Composition Archetype:** `Flat Modular Platform` (z -0.10; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 38%, Declarative / Non-Code 25%, Callbacks & Closures Files 14%, Large Core Modules 7%, State Mutators Files 4%
...
- `tokenBase` **(Compute Cores)** (@ `ThirdParty/codemirror-5.52.0/mode/stylus/stylus.js`) -> Impact: **565.8** | LOC: 682
  * *Intent:* /** * Tokenizers */
- `cesiumGoogleEarthDbRootParser` **(Compute Cores)** (@ `packages/engine/Source/ThirdParty/google-earth-dbroot-parser.js`) -> Impact: **556.3** | LOC: 915
...
- `expression` **(Compute Cores)** (@ `ThirdParty/codemirror-5.52.0/mode/soy/soy.js`) -> Impact: **304.3** | LOC: 371
- `toObject` **(Compute Cores)** (@ `packages/engine/Source/ThirdParty/google-earth-dbroot-parser.js`) -> Impact: **271.7** | LOC: 238
...
- **Heaviest Functions:** `countColumn` (Many-Argument Workhorses, Impact: 20.3), `findFirst` (Compute Cores, Impact: 18.6), `findColumn` (Compute Cores, Impact: 12.7)
...
- **Heaviest Functions:** `query` (Compute Cores, Impact: 60.0), `put` (Defensive Guards, Impact: 13.4), `_getTarget` (Defensive Guards, Impact: 6.4)
...
- **Composition Archetype:** `Compute Cores Files` (z +0.49)
...
- **Heaviest Functions:** `onScrollWheel` (Compute Cores, Impact: 62.6), `wheelEventDelta` (Compute Cores, Impact: 13.1), `wheelEventPixels` (Interface Declarations, Impact: 1.7)

### 4. `ThirdParty/dojo-release-1.10.4/dojo/_base/configRhino.js` (JAVASCRIPT) -> Cumulative Risk: **757.73**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.66)
...
- **Heaviest Functions:** `rhinoDojoConfig` (Many-Argument Workhorses, Impact: 34.0), `injectUrl` (Defensive Guards, Impact: 5.8), `log` (Callbacks & Closures, Impact: 5.5)
...
- **Heaviest Functions:** `locateNodeInLineView` (Compute Cores, Impact: 62.7), `domTextBetween` (Many-Argument Workhorses, Impact: 51.6), `pollContent` (Compute Cores, Impact: 39.5)
...
- **Heaviest Functions:** `Deferred` (Compute Cores, Impact: 63.5), `notify` (Defensive Guards, Impact: 21.9), `then` (Many-Argument Workhorses, Impact: 12.2)
...
- **Heaviest Functions:** `scanForBracket` (Many-Argument Workhorses, Impact: 60.1), `findMatchingBracket` (Compute Cores, Impact: 37.1), `matchBrackets` (Compute Cores, Impact: 31.4)

### 8. `ThirdParty/codemirror-5.52.0/src/display/focus.js` (JAVASCRIPT) -> Cumulative Risk: **737.73**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.68)
...
- **Heaviest Functions:** `onFocus` (Compute Cores, Impact: 13.1), `onBlur` (Callbacks & Closures, Impact: 7.5), `delayBlurEvent` (Callbacks & Closures, Impact: 3.2)

### 9. `ThirdParty/dojo-release-1.10.4/dojo/request/util.js` (JAVASCRIPT) -> Cumulative Risk: **734.82**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.28)
- **Magnitude:** 247.3 | **LOC:** 159 | **CtrlFlow:** 33.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `parseArgs` (Defensive Guards, Impact: 31.6), `deferred` (Many-Argument Workhorses, Impact: 26.8), `deepCopy` (Defensive Guards, Impact: 14.6)

### 10. `ThirdParty/codemirror-5.52.0/addon/hint/show-hint.js` (JAVASCRIPT) -> Cumulative Risk: **730.21**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.30)
...
- **Heaviest Functions:** `Widget` (Compute Cores, Impact: 70.2), `buildKeyMap` (Callbacks & Closures, Impact: 24.6), `resolveAutoHints` (Callbacks & Closures, Impact: 20.3)
...
**Top Internal Functions/Classes:**
  * `cesiumGoogleEarthDbRootParser` **(Compute Cores)** (Impact: 556.3)
  * `verify` **(Compute Cores)** (Impact: 334.3)
  * `toObject` **(Compute Cores)** (Impact: 271.7)
  * `fromObject` **(Defensive Guards)** (Impact: 215.2)
  * `decode` **(Compute Cores)** (Impact: 169.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `evalInput` **(Compute Cores)** (Impact: 117.0)
...
- **Magnitude:** 2766.82 | **LOC:** 5293 | **CtrlFlow:** 18.4% | **Authorship Centralization:** 38.1%
...
**Top Internal Functions/Classes:**
  ...
  * `splitLongitudePolyline` **(Compute Cores)** (Impact: 53.2)
...
**Top Internal Functions/Classes:**
  ...
  * `testColorBlendMode` **(Tests & Verification)** (Impact: 5.9)
  * `numberOfChildrenWithoutContent` **(Interface Declarations)** (Impact: 4.8)
  * `checkPointAndFeatureCounts` **(Tests & Verification)** (Impact: 4.1)
...
**Top Internal Functions/Classes:**
  * `Model` **(Compute Cores)** (Impact: 130.7)
  ...
  * `updateClamping` **(Compute Cores)** (Impact: 21.5)
...
**Top Internal Functions/Classes:**
  * `gltfCallback` **(Tests & Verification)** (Impact: 137.2)
