...
> **File Composition:** Large Core Modules 32%, Data / Markup / Trivial 20%, Defensive Guards Files 12%, State Mutators Files 11%, Declarative / Non-Code 9%
...
- **Heaviest Functions:** `run` (Defensive Guards, Impact: 7.6), `asyncCallback` (Many-Argument Workhorses, Impact: 7.5), `threadMain` (Defensive Guards, Impact: 2.2)
...
- **Heaviest Functions:** `mainAsyncCallback` (Defensive Guards, Impact: 7.5), `asyncCallback` (Defensive Guards, Impact: 7.4), `run` (Defensive Guards, Impact: 7.0)
...
- **Heaviest Functions:** `notifySlow` (Many-Argument Workhorses, Impact: 24.8), `pop` (Defensive Guards, Impact: 19.7), `steal` (Many-Argument Workhorses, Impact: 15.0)
...
- **Heaviest Functions:** `Intrusive` (Defensive Guards, Impact: 11.4), `pop` (Defensive Guards, Impact: 9.9), `push` (State Mutators, Impact: 2.0)
...
- **Heaviest Functions:** `start_completion` (Many-Argument Workhorses, Impact: 98.9), `tick` (Many-Argument Workhorses, Impact: 57.7), `perform` (Compute Cores, Impact: 51.5)
...
- **Heaviest Functions:** `perform` (Defensive Guards, Impact: 85.7), `syscall_result` (Many-Argument Workhorses, Impact: 80.0), `tick` (Many-Argument Workhorses, Impact: 70.0)
...
- **Heaviest Functions:** `xev_timer_reset` (Many-Argument Workhorses, Impact: 10.4), `xev_timer_run` (Many-Argument Workhorses, Impact: 9.8), `xev_timer_cancel` (Many-Argument Workhorses, Impact: 9.8)
...
- **Heaviest Functions:** `CreateFile` (Many-Argument Workhorses, Impact: 12.3), `SetInformationJobObject` (Many-Argument Workhorses, Impact: 9.9), `ReadFile` (Type Conversions, Impact: 8.8)
...
- **Heaviest Functions:** `Intrusive` (Many-Argument Workhorses, Impact: 47.1), `meld` (Many-Argument Workhorses, Impact: 12.1), `remove` (Defensive Guards, Impact: 11.8)

### 10. `src/watcher/stream.zig` (ZIG) -> Cumulative Risk: **440.42**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.66)
...
- **Heaviest Functions:** `Writeable` (Many-Argument Workhorses, Impact: 70.9), `Stream` (Defensive Guards, Impact: 47.1), `read` (Many-Argument Workhorses, Impact: 44.5)
...
**Top Internal Functions/Classes:**
  ...
  * `callback` **(Defensive Guards)** (Impact: 11.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `TCPStream` **(Compute Cores)** (Impact: 40.8)
...
**Top Internal Functions/Classes:**
  ...
  * `AsyncEventFd` **(Compute Cores)** (Impact: 27.6)
...
**Top Internal Functions/Classes:**
  ...
  * `UDPSendMsg` **(Compute Cores)** (Impact: 23.1)
...
**Top Internal Functions/Classes:**
  * `notifySlow` **(Many-Argument Workhorses)** (Impact: 24.8)
...
**Top Internal Functions/Classes:**
  ...
  * `FileStream` **(Compute Cores)** (Impact: 47.5)
...
**Top Internal Functions/Classes:**
  * `TimerLoop` **(Compute Cores)** (Impact: 38.9)
...
**Top Internal Functions/Classes:**
  ...
  * `ProcessIocp` **(Compute Cores)** (Impact: 29.2)
...
**Top Internal Functions/Classes:**
  ...
  * `xev_timer_run` **(Many-Argument Workhorses)** (Impact: 9.8)
  * `xev_timer_cancel` **(Many-Argument Workhorses)** (Impact: 9.8)
...
**Top Internal Functions/Classes:**
  * `main` **(I/O & Config Routines)** (Impact: 7.2)
    * *Intent:* #endif
  * `hrtime` **(I/O & Config Routines)** (Impact: 3.0)
