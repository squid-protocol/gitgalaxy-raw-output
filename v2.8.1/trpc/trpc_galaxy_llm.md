...
> **Composition Archetype:** `Flat Modular Platform` (z +0.14; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 40%, Declarative / Non-Code 17%, Callbacks & Closures Files 11%, Interface Declarations Files 9%, Defensive Guards Files 6%
...
- `pluginLlmsTxt` **(Compute Cores)** (@ `www/docusaurus.config.ts`) -> Impact: **129.8** | LOC: 305
...
- `postBuild` **(Compute Cores)** (@ `www/docusaurus.config.ts`) -> Impact: **117.8** | LOC: 320
- `getWSConnectionHandler` **(Compute Cores)** (@ `packages/server/src/adapters/ws.ts`) -> Impact: **89.6** | LOC: 462
- `extractTwoslashBlocks` **(Compute Cores)** (@ `www/scripts/check-twoslash.ts`) -> Impact: **79.1** | LOC: 224
...
### 1. `packages/client/src/links/internals/httpUtils.ts` (TYPESCRIPT) -> Cumulative Risk: **649.47**
...
- **Heaviest Functions:** `getUrl` (Compute Cores, Impact: 16.9), `fetchHTTPResponse` (Defensive Guards, Impact: 14.2), `throwIfAborted` (Defensive Guards, Impact: 7.9)

### 2. `www/src/components/sponsors/script.pull.ts` (TYPESCRIPT) -> Cumulative Risk: **629.21**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `main` (I/O & Config Routines, Impact: 16.1), `calculateWeight` (Compute Cores, Impact: 8.7), `flattenSponsor` (Defensive Guards, Impact: 6.4)

### 3. `packages/server/src/unstable-core-do-not-import/http/contentType.ts` (TYPESCRIPT) -> Cumulative Risk: **625.51**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.03)
...
- **Heaviest Functions:** `parse` (Defensive Guards, Impact: 48.9), `memo` (Callbacks & Closures, Impact: 13.2), `getRawInput` (Defensive Guards, Impact: 13.2)
...
- **Heaviest Functions:** `filterIgnored` (Defensive Guards, Impact: 10.4), `assertCleanGitTree` (Defensive Guards, Impact: 2.5)

### 5. `scripts/entrypoints.ts` (TYPESCRIPT) -> Cumulative Risk: **611.72**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.68)
...
- **Heaviest Functions:** `generateEntrypoints` (Compute Cores, Impact: 28.6), `writeFileSyncRecursive` (State Mutators, Impact: 3.8)

### 6. `packages/server/src/adapters/ws.ts` (TYPESCRIPT) -> Cumulative Risk: **606.95**
...
- **Heaviest Functions:** `getWSConnectionHandler` (Compute Cores, Impact: 89.6), `handleRequest` (Many-Argument Workhorses, Impact: 59.0), `onabort` (I/O & Config Routines, Impact: 33.2)
...
- **Heaviest Functions:** `fetchRequestHandler` (Defensive Guards, Impact: 25.5), `responseMeta` (Defensive Guards, Impact: 19.8), `trimSlashes` (Callbacks & Closures, Impact: 4.6)
...
- **Heaviest Functions:** `createContext` (Callbacks & Closures, Impact: 1.1)

### 9. `www/docusaurus.config.ts` (TYPESCRIPT) -> Cumulative Risk: **579.92**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
...
- **Heaviest Functions:** `pluginLlmsTxt` (Compute Cores, Impact: 129.8), `postBuild` (Compute Cores, Impact: 117.8), `findMdxFilesRecursively` (Defensive Guards, Impact: 19.7)

### 10. `packages/client/src/links/wsLink/wsClient/wsClient.ts` (TYPESCRIPT) -> Cumulative Risk: **577.94**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `setupWebSocketListeners` (Defensive Guards, Impact: 22.4), `handleCloseOrError` (Defensive Guards, Impact: 22.4), `constructor` (Defensive Guards, Impact: 17.1)
...
**Top Internal Functions/Classes:**
  ...
  * `onError` **(Callbacks & Closures)** (Impact: 6.8)
  * `onopen` **(Tests & Verification)** (Impact: 4.9)
  * `waitForClientState` **(Generic / Templated Code)** (Impact: 4.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `replaceHooksWithOptions` **(Compute Cores)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `getWSConnectionHandler` **(Compute Cores)** (Impact: 89.6)
...
- **Risk Profile:** Cognitive Load (100.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `[Symbol.asyncIterator]` **(Interface Declarations)** (Impact: 3.3)
  * `[Symbol.asyncIterator]` **(Interface Declarations)** (Impact: 3.2)
  * `[Symbol.asyncIterator]` **(I/O & Config Routines)** (Impact: 2.3)
  * `serverResourceForStream` **(Callbacks & Closures)** (Impact: 2.2)
