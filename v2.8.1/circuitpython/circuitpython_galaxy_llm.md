...
|---|---|---|
| Modularity | 0.575 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 2.7822 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.90; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 43%, Data / Markup / Trivial 27%, Interface Declarations Files 12%, Large Core Modules 7%, Parameter Forwarders Files 2%
...
| dead_code | 6937 | 1454 | 3 | `ports/zephyr-cp/common-hal/wifi/Radio.c` |
...
- **Heaviest Functions:** `build_circuitpython` (Compute Cores, Impact: 77.3), `determine_enabled_modules` (Many-Argument Workhorses, Impact: 30.9), `preprocess_and_split_defs` (Many-Argument Workhorses, Impact: 5.6)
...
- **Heaviest Functions:** `common_hal_busdisplay_busdisplay_construct` (Many-Argument Workhorses, Impact: 101.8), `_refresh_area` (Many-Argument Workhorses, Impact: 34.3), `common_hal_busdisplay_busdisplay_set_brightness` (Compute Cores, Impact: 29.9)
...
- **Heaviest Functions:** `run_command` (Many-Argument Workhorses, Impact: 130.4), `compile` (Many-Argument Workhorses, Impact: 22.0), `run_function` (Many-Argument Workhorses, Impact: 15.1)
...
- **Heaviest Functions:** `displayio_tilegrid_fill_area` (Many-Argument Workhorses, Impact: 112.0), `common_hal_displayio_tilegrid_construct` (Many-Argument Workhorses, Impact: 61.2), `displayio_tilegrid_get_refresh_areas` (Compute Cores, Impact: 49.8)

### 5. `ports/zephyr-cp/supervisor/port.c` (C) -> Cumulative Risk: **706.47**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.07)
...
- **Heaviest Functions:** `port_realloc` (Defensive Guards, Impact: 15.1), `port_heap_init` (I/O & Config Routines, Impact: 10.4), `port_malloc` (Defensive Guards, Impact: 9.6)
...
- **Heaviest Functions:** `refresh_list` (I/O & Config Routines, Impact: 28.8), `compareValues` (Defensive Guards, Impact: 14.3), `upload` (Compute Cores, Impact: 14.1)
...
- **Heaviest Functions:** `audiodelays_chorus_get_buffer` (Many-Argument Workhorses, Impact: 88.3), `common_hal_audiodelays_chorus_construct` (Many-Argument Workhorses, Impact: 27.5), `common_hal_audiodelays_chorus_deinited` (Interface Declarations, Impact: 3.1)
...
- **Heaviest Functions:** `common_hal_bleio_characteristic_construct` (Many-Argument Workhorses, Impact: 87.5), `bleio_characteristic_access_cb` (Many-Argument Workhorses, Impact: 38.0), `characteristic_on_ble_gap_evt` (Compute Cores, Impact: 22.5)
...
- **Heaviest Functions:** `filesystem_init` (Compute Cores, Impact: 53.9), `filesystem_for_path` (Compute Cores, Impact: 9.7), `filesystem_set_writable_by_usb` (Compute Cores, Impact: 5.5)

### 10. `ports/espressif/common-hal/mipidsi/Display.c` (C) -> Cumulative Risk: **693.47**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.09)
- **Magnitude:** 265.44 | **LOC:** 292 | **CtrlFlow:** 16.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9991%), Tech Debt (92.1064%)
- **Heaviest Functions:** `common_hal_mipidsi_display_construct` (Many-Argument Workhorses, Impact: 113.2), `common_hal_mipidsi_display_set_brightness` (Compute Cores, Impact: 20.5), `common_hal_mipidsi_display_deinit` (Compute Cores, Impact: 12.9)
...
**Top Internal Functions/Classes:**
  ...
  * `create_name` **(Compute Cores)** (Impact: 189.9)
    * *Intent:* #endif /* FF_USE_FIND && FF_FS_MINIMIZE <= 1 */ /*--------------------------------------------------...
  * `find_volume` **(Compute Cores)** (Impact: 168.6)
...
**Top Internal Functions/Classes:**
  ...
  * `compile_expr_stmt` **(Compute Cores)** (Impact: 76.7)
    * *Intent:* #endif
...
**Top Internal Functions/Classes:**
  ...
  * `mpz_set_from_float` **(Compute Cores)** (Impact: 32.2)
...
**Top Internal Functions/Classes:**
  ...
  * `GIF_playFrame` **(Many-Argument Workhorses)** (Impact: 17.4)
...
**Top Internal Functions/Classes:**
  * `_reply` **(Compute Cores)** (Impact: 211.0)
  * `_process_request` **(Compute Cores)** (Impact: 120.1)
  * `_write_file_and_reply` **(Many-Argument Workhorses)** (Impact: 56.6)
  * `_reply_with_file` **(Many-Argument Workhorses)** (Impact: 45.4)
  * `supervisor_start_web_workflow` **(Compute Cores)** (Impact: 41.5)
    * *Intent:* #endif
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `mp_init` **(Compute Cores)** (Impact: 34.6)
...
- **Risk Profile:** Cognitive Load (68.6748%), Tech Debt (31.8176%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `_label_to_end` **(Interface Declarations)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `frame_attr` **(Many-Argument Workhorses)** (Impact: 19.4)
...
- **Risk Profile:** Cognitive Load (96.9493%), Tech Debt (42.5465%)
**Top Internal Functions/Classes:**
