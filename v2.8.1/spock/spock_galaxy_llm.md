...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +2.10; from the repo's file-archetype mix)
> **File Composition:** Annotated Framework Methods Files 21%, Tests & Verification Files 19%, Data / Markup / Trivial 15%, Interface Declarations Files 13%, State Mutators Files 9%
...
- `register` **(Compute Cores)** (@ `build-logic/asciidoc-extensions/src/main/java/org/spockframework/plugins/asciidoctor/IncludedSourceLinker.java`) -> Impact: **166.0** | LOC: 632
...
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.52)
...
- **Heaviest Functions:** `ParameterizedFeatureChildExecutor` (Many-Argument Workhorses, Impact: 27.6), `executionFinished` (Compute Cores, Impact: 14.7), `executionFinished` (Callbacks & Closures, Impact: 5.6)

### 2. `spock-core/src/main/java/org/spockframework/compiler/ConditionRewriter.java` (JAVA) -> Cumulative Risk: **650.55**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.48)
...
- **Heaviest Functions:** `rewriteCondition` (Defensive Guards, Impact: 15.2), `rewriteToSpockRuntimeCall` (Many-Argument Workhorses, Impact: 13.4), `surroundWithTryCatch` (Many-Argument Workhorses, Impact: 12.6)

### 3. `spock-core/src/main/java/org/spockframework/mock/runtime/MockInteraction.java` (JAVA) -> Cumulative Risk: **648.68**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -0.16)
...
- **Heaviest Functions:** `MockInteraction` (Many-Argument Workhorses, Impact: 12.2), `matchesTargetAndMethod` (Defensive Guards, Impact: 7.6), `computeSimilarityScore` (Defensive Guards, Impact: 6.7)
...
- **Heaviest Functions:** `buildBlocks` (Compute Cores, Impact: 17.9), `buildFixtureMethod` (Compute Cores, Impact: 15.4), `addBlock` (Compute Cores, Impact: 13.1)
...
- **Composition Archetype:** `Annotated Framework Methods Files` (z -0.25)
...
- **Heaviest Functions:** `AsyncRunListener` (Defensive Guards, Impact: 6.1), `run` (Defensive Guards, Impact: 3.7), `addEvent` (Interface Declarations, Impact: 3.0)
...
- **Heaviest Functions:** `startOf` (Compute Cores, Impact: 9.4), `visitTernaryExpression` (Annotated Framework Methods, Impact: 6.3), `visitShortTernaryExpression` (Annotated Framework Methods, Impact: 6.2)
...
- **Heaviest Functions:** `next` (I/O & Config Routines, Impact: 14.9), `createIterators` (Generic / Templated Code, Impact: 14.7), `haveNext` (Defensive Guards, Impact: 13.4)
...
- **Heaviest Functions:** `visitMethodAgain` (Defensive Guards, Impact: 15.6), `visitCleanupBlock` (Compute Cores, Impact: 11.8), `moveVariableDeclarations` (Compute Cores, Impact: 9.8)
...
- **Heaviest Functions:** `replaceExpr` (Compute Cores, Impact: 4.6), `visitArrayExpression` (Annotated Framework Methods, Impact: 3.2), `replaceAllExprs` (Generic / Templated Code, Impact: 3.1)
...
- **Heaviest Functions:** `invoke` (Many-Argument Workhorses, Impact: 19.7), `runFeature` (Compute Cores, Impact: 7.9), `doRunCleanup` (Compute Cores, Impact: 7.4)
...
**Top Internal Functions/Classes:**
  ...
  * `visitAllImports` **(Compute Cores)** (Impact: 25.5)
  * `visitPropertyExpression` **(Defensive Guards)** (Impact: 22.0)
  * `visitMethod` **(Compute Cores)** (Impact: 19.4)
...
**Top Internal Functions/Classes:**
  * `register` **(Compute Cores)** (Impact: 166.0)
  ...
  * `determineTagsToFilterFor` **(Compute Cores)** (Impact: 18.8)
...
**Top Internal Functions/Classes:**
  * `"Accessing real fields through global spy works like in reality"` **(Tests & Verification)** (Impact: 8.5)
    * *Intent:* /** * This test should verify that the resolution logic around fields and properties * works identic...
  * `overwriteSuperField` **(Interface Declarations)** (Impact: 3.3)
  * `"calls real method if mocked call provides no result"` **(Tests & Verification)** (Impact: 2.6)
  * `"does not call real method if mocked call provides result"` **(Tests & Verification)** (Impact: 2.6)
  * `"calls real method if call isn't mocked"` **(Tests & Verification)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `createIterators` **(Generic / Templated Code)** (Impact: 14.7)
  * `haveNext` **(Defensive Guards)** (Impact: 13.4)
  * `getPreviousDataTableProviders` **(Generic / Templated Code)** (Impact: 12.8)
...
**Top Internal Functions/Classes:**
  * `rewriteCondition` **(Defensive Guards)** (Impact: 15.2)
  * `rewriteToSpockRuntimeCall` **(Many-Argument Workhorses)** (Impact: 13.4)
  * `surroundWithTryCatch` **(Many-Argument Workhorses)** (Impact: 12.6)
...
**Top Internal Functions/Classes:**
  ...
  * `rewriteTableLikeParameterization` **(Compute Cores)** (Impact: 17.7)
  * `rewriteMultiParameterization` **(Defensive Guards)** (Impact: 14.9)
  * `turnIntoSimpleParameterization` **(Compute Cores)** (Impact: 14.5)
  * `verifyDerivedDataVariableIsNotCombined` **(Defensive Guards)** (Impact: 11.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `visitCleanupBlock` **(Compute Cores)** (Impact: 11.8)
    * *Intent:* */
  * `moveVariableDeclarations` **(Compute Cores)** (Impact: 9.8)
    * *Intent:* /* * Moves variable declarations from one statement list to another. Initializer * expressions are k...
  * `transformRhsExpressionIfNecessary` **(Callbacks & Closures)** (Impact: 9.6)
  * `moveInteractions` **(Generic / Templated Code)** (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 32 instances
...
**Top Internal Functions/Classes:**
  * `createMockImpl` **(Generic / Templated Code)** (Impact: 25.7)
  ...
  * `createMock` **(Generic / Templated Code)** (Impact: 9.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.2e-05 | `Ripple Effect (Closeness):` 0.0018
...
**Top Internal Functions/Classes:**
  ...
  * `parse` **(Many-Argument Workhorses)** (Impact: 16.5)
...
**Top Internal Functions/Classes:**
  ...
  * `hasValidArguments` **(Compute Cores)** (Impact: 9.2)
...
**Top Internal Functions/Classes:**
  * `invoke` **(Many-Argument Workhorses)** (Impact: 19.7)
  * `runFeature` **(Compute Cores)** (Impact: 7.9)
  * `doRunCleanup` **(Compute Cores)** (Impact: 7.4)
  * `doRunSetupSpec` **(Compute Cores)** (Impact: 7.3)
  * `doRunSetup` **(Compute Cores)** (Impact: 7.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  * `isInitializerOrFixtureMethod` **(Annotated Framework Methods)** (Impact: 9.2)
  * `collectAll` **(Generic / Templated Code)** (Impact: 6.5)
  * `filterFeatures` **(Compute Cores)** (Impact: 6.0)
  * `toFeatureName` **(Annotated Framework Methods)** (Impact: 6.0)
  * `collectSpecHierarchy` **(Generic / Templated Code)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004009 | `Ripple Effect (Closeness):` 0.114779
...
**Top Internal Functions/Classes:**
  * `getAnnotation` **(Defensive Guards)** (Impact: 10.9)
  ...
  * `getInvocationTarget` **(Defensive Guards)** (Impact: 7.6)
  * `getAssertionMessage` **(Defensive Guards)** (Impact: 7.4)
  * `isDynamicTypedExpression` **(Type Conversions)** (Impact: 6.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 9 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001096 | `Ripple Effect (Closeness):` 0.091129
...
**Top Internal Functions/Classes:**
  ...
  * `visitArrayExpression` **(Annotated Framework Methods)** (Impact: 3.2)
  * `replaceAllExprs` **(Generic / Templated Code)** (Impact: 3.1)
  * `visitPropertyExpression` **(Annotated Framework Methods)** (Impact: 2.1)
  * `visitRangeExpression` **(Annotated Framework Methods)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `startOf` **(Compute Cores)** (Impact: 9.4)
    * *Intent:* // searches for token backwards from beginning of node (exclusive) // IDEA: move to class TextPositi...
  * `visitTernaryExpression` **(Annotated Framework Methods)** (Impact: 6.3)
  * `visitShortTernaryExpression` **(Annotated Framework Methods)** (Impact: 6.2)
  * `visitBinaryExpression` **(Annotated Framework Methods)** (Impact: 4.8)
  * `visitPropertyExpression` **(Annotated Framework Methods)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `determinePrimaryCandidate` **(Many-Argument Workhorses)** (Impact: 12.8)
  * `getBeanName` **(Many-Argument Workhorses)** (Impact: 12.3)
  * `registerSpy` **(Generic / Templated Code)** (Impact: 10.6)
  * `register` **(Defensive Guards)** (Impact: 8.4)
  * `getExistingBeans` **(Generic / Templated Code)** (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 11 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6e-06 | `Ripple Effect (Closeness):` 0.0008
...
**Top Internal Functions/Classes:**
  * `buildBlocks` **(Compute Cores)** (Impact: 17.9)
  * `buildFixtureMethod` **(Compute Cores)** (Impact: 15.4)
  * `addBlock` **(Compute Cores)** (Impact: 13.1)
  * `visitMethod` **(Compute Cores)** (Impact: 11.9)
  * `isFixtureMethod` **(Compute Cores)** (Impact: 9.4)
...
**Top Internal Functions/Classes:**
  * `handleImplicitCondition` **(Compute Cores)** (Impact: 13.2)
  * `doVisitBinaryExpression` **(Compute Cores)** (Impact: 11.3)
  * `handleThrownCall` **(Compute Cores)** (Impact: 9.6)
  * `handleInteraction` **(Defensive Guards)** (Impact: 8.0)
  * `handleImplicitCallOnMethodParam` **(Compute Cores)** (Impact: 7.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 19 instances
...
**Top Internal Functions/Classes:**
  ...
  * `setDeclarationOrder` **(Interface Declarations)** (Impact: 1.6)
  * `setExecutionOrder` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003671 | `Ripple Effect (Closeness):` 0.11592
...
**Top Internal Functions/Classes:**
  * `clearError` **(Defensive Guards)** (Impact: 4.7)
  * `printf` **(Annotated Framework Methods)** (Impact: 4.4)
  * `format` **(Annotated Framework Methods)** (Impact: 4.4)
  * `append` **(Annotated Framework Methods)** (Impact: 4.4)
  * `write` **(Annotated Framework Methods)** (Impact: 4.3)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `convertCount` **(Defensive Guards)** (Impact: 7.4)
...
**Top Internal Functions/Classes:**
  ...
  * `setCall` **(Defensive Guards)** (Impact: 12.1)
  * `addArgs` **(Defensive Guards)** (Impact: 10.8)
  * `addArg` **(Defensive Guards)** (Impact: 9.9)
  * `selectNameConstraint` **(Defensive Guards)** (Impact: 8.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 8 instances
...
**Top Internal Functions/Classes:**
  * `createFeature` **(Compute Cores)** (Impact: 23.8)
  * `findMethod` **(Callbacks & Closures)** (Impact: 5.8)
  * `build` **(Interface Declarations)** (Impact: 5.6)
  * `buildFixtureMethods` **(Interface Declarations)** (Impact: 5.5)
  * `buildFields` **(Interface Declarations)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.4e-05 | `Ripple Effect (Closeness):` 0.0032
...
- **Risk Profile:** Cognitive Load (10.1393%), Tech Debt (99.3614%)
**Top Internal Functions/Classes:**
  * `visitAnnotations` **(Type Conversions)** (Impact: 8.0)
  * `visitConstructorOrMethod` **(Annotated Framework Methods)** (Impact: 7.4)
  * `getExpressions` **(Generic / Templated Code)** (Impact: 6.0)
  * `indexAstNodes` **(Interface Declarations)** (Impact: 5.5)
  * `visitStatement` **(Defensive Guards)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.2e-05 | `Ripple Effect (Closeness):` 0.0056
...
- `spock-core/src/main/java/spock/lang/Specification.java` -> **Severity: 0.475** (Bridge: 0.0107 * Flux: 44.2886%)
- `spock-core/src/main/java/org/spockframework/runtime/model/SpecInfo.java` -> **Severity: 0.381** (Bridge: 0.004 * Flux: 95.108%)
- `spock-core/src/main/java/org/spockframework/runtime/model/FeatureInfo.java` -> **Severity: 0.287** (Bridge: 0.0037 * Flux: 78.1336%)
