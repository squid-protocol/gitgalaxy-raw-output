...
| Analyzed Artifacts (Scanned) | 22494 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 8673 |
| Total LOC | 4902897 |
...
|---|---|---|
| Modularity | 0.6694 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 4.2351 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
| Articulation Pts | 947 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| CPP | 14795 | 4093643 | 65.8% |
| C | 3228 | 429050 | 14.4% |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.35; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 22%, Data / Markup / Trivial 19%, Interface Declarations Files 17%, Declarative / Non-Code 15%, I/O & Config Routines Files 7%
...
|---|---|---|---|---|
| memory | 1390372 | 15444 | 123 | `interpreter/llvm-project/llvm/lib/Transforms/Vectorize/SLPVectorizer.cpp` |
| cleanup | 4642 | 1507 | 0 | `interpreter/llvm-project/clang/lib/Parse/ParsePragma.cpp` |
| guards | 530360 | 14409 | 54 | `builtins/glew/src/glew.c` |
| danger | 28768 | 4018 | 2 | `misc/minicern/src/zebra.f` |
| concurrency | 5289 | 658 | 0 | `gui/webdisplay/src/RWebWindow.cxx` |
| connectivity | 87991 | 11255 | 9 | `interpreter/llvm-project/clang/lib/Headers/avx512vlintrin.h` |
...
| debt | 43211 | 5951 | 4 | `interpreter/llvm-project/clang/include/clang/AST/OpenMPClause.h` |
| mutation | 1208051 | 15699 | 113 | `builtins/glew/src/glew.c` |
| dead_code | 120547 | 10604 | 12 | `interpreter/llvm-project/clang/include/clang/ASTMatchers/ASTMatchers.h` |
| credential | 160 | 55 | 0 | `roottest/cling/template/typedef/cmspb_orig.C` |
| threat | 32467 | 9344 | 1 | `graf2d/win32gdk/gdk/src/gdk/gdkkeysyms.h` |
...
- **Unknown Dependencies:** `119411` packages imported that bypass the Zero-Trust whitelist.
...
- **Heaviest Functions:** `ROOT::Internal::RClusterPool::GetCluster` (Many-Argument Workhorses, Impact: 68.3), `ROOT::Internal::RClusterPool::WaitFor` (Compute Cores, Impact: 25.2), `ROOT::Internal::RClusterPool::ExecReadClusters` (I/O & Config Routines, Impact: 10.8)
...
- **Heaviest Functions:** `onGeoPainterReady` (Compute Cores, Impact: 66.8), `processMouseMove` (Compute Cores, Impact: 35.4), `jsrootOrbitContext` (Compute Cores, Impact: 23.1)
...
- **Heaviest Functions:** `CPyCppyy::InitializerListConverter::SetArg` (Many-Argument Workhorses, Impact: 299.4), `CPyCppyy::InitializerListConverter::Clear` (Callbacks & Closures, Impact: 171.2), `CPyCppyy::CreateConverter` (Compute Cores, Impact: 121.4)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 143.3), `CreateTFDatasets` (Many-Argument Workhorses, Impact: 52.5), `CreateNumPyGenerators` (Many-Argument Workhorses, Impact: 37.4)
...
- **Heaviest Functions:** `showLeftArea` (Defensive Guards, Impact: 57.7), `closeMethodDialog` (Many-Argument Workhorses, Impact: 46.5), `onFileMenuAction` (Compute Cores, Impact: 38.8)

### 6. `roofit/xroofit/src/Asymptotics.cxx` (CPP) -> Cumulative Risk: **729.38**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.09)
...
- **Heaviest Functions:** `xRooFit::Asymptotics::PValue` (Many-Argument Workhorses, Impact: 61.1), `xRooFit::Asymptotics::k` (Many-Argument Workhorses, Impact: 47.0), `xRooFit::Asymptotics::Phi_m` (Many-Argument Workhorses, Impact: 25.9)

### 7. `tree/ntuple/src/RFieldMeta.cxx` (CPP) -> Cumulative Risk: **729.03**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.19)
...
- **Heaviest Functions:** `ROOT::RProxiedCollectionField::RProxiedCollectionField` (Compute Cores, Impact: 57.1), `ROOT::REnumField::REnumField` (Compute Cores, Impact: 48.1), `ROOT::RClassField::BeforeConnectPageSource` (Compute Cores, Impact: 41.6)
...
- **Heaviest Functions:** `typeof_scope` (Many-Argument Workhorses, Impact: 53.5), `cppclass_getattr_impl` (Many-Argument Workhorses, Impact: 21.5), `box_instance` (Defensive Guards, Impact: 15.6)
...
- **Heaviest Functions:** `processHighlight` (Many-Argument Workhorses, Impact: 17.4), `BrowseElement` (Compute Cores, Impact: 14.5), `createSummaryModel` (Many-Argument Workhorses, Impact: 13.1)

### 10. `interpreter/llvm-project/llvm/utils/spirv-sim/instructions.py` (PYTHON) -> Cumulative Risk: **714.08**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Encapsulated Accessors Files` (z -1.31)
...
- **Heaviest Functions:** `__init__` (Compute Cores, Impact: 14.4), `_advance_ip` (Many-Argument Workhorses, Impact: 6.6), `_impl` (Compute Cores, Impact: 6.5)
...
**Top Internal Functions/Classes:**
  ...
  * `glewIsSupported` **(Compute Cores)** (Impact: 442.0)
    * *Intent:* #endif
  * `eglewIsSupported` **(Compute Cores)** (Impact: 254.5)
  ...
  * `eglewInit` **(Compute Cores)** (Impact: 182.9)
...
**Top Internal Functions/Classes:**
  ...
  * `handle_request` **(Compute Cores)** (Impact: 723.1)
  ...
  * `uninitialize_openssl` **(Compute Cores)** (Impact: 454.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `CodeGenFunction::EmitX86BuiltinExpr` **(Compute Cores)** (Impact: 1294.1)
  ...
  * `CodeGenFunction::EmitAMDGPUBuiltinExpr` **(Compute Cores)** (Impact: 732.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `IntExprEvaluator::VisitBuiltinCallExpr` **(Compute Cores)** (Impact: 860.1)
  * `EvaluateStmt` **(Many-Argument Workhorses)** (Impact: 437.2)
    * *Intent:* // Evaluate a statement.
  * `CheckICE` **(Compute Cores)** (Impact: 425.3)
  * `EvaluateComparisonBinaryOperator` **(Many-Argument Workhorses)** (Impact: 332.9)
  * `PointerExprEvaluator::VisitBuiltinCallExpr` **(Compute Cores)** (Impact: 234.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `TSpectrum2Painter::Paint` **(Compute Cores)** (Impact: 446.0)
...
**Top Internal Functions/Classes:**
  ...
  * `TMinuit::mncros` **(Compute Cores)** (Impact: 179.0)
    * *Intent:* //////////////////////////////////////////////////////////////////////////////// /// Find point wher...
  * `TMinuit::mnhelp` **(Compute Cores)** (Impact: 141.8)
    * *Intent:* //////////////////////////////////////////////////////////////////////////////// /// HELP routine fo...
  * `TMinuit::mnset` **(Compute Cores)** (Impact: 138.6)
...
**Top Internal Functions/Classes:**
  ...
  * `xRooNode::Draw` **(Compute Cores)** (Impact: 615.2)
...
- `interpreter/llvm-project/clang/lib/Headers/llvm_libc_wrappers/string.h` -> **Severity: 26.778** (Embedded: 0.2751 * Error Risk: 97.3403%)
- `bindings/pyroot/pythonizations/test/memory.py` -> **Severity: 13.184** (Embedded: 0.2029 * Error Risk: 64.9934%)
- `core/base/inc/Rtypes.h` -> **Severity: 12.203** (Embedded: 0.1544 * Error Risk: 79.0109%)
- `core/base/inc/TObject.h` -> **Severity: 11.407** (Embedded: 0.1328 * Error Risk: 85.8697%)
- `core/base/inc/TStorage.h` -> **Severity: 10.737** (Embedded: 0.1077 * Error Risk: 99.7395%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `core/base/inc/Rtypes.h` -> **Severity: 1768.9** (Blast Radius: 17.689 * Doc Risk: 100.0%)
- `core/base/inc/TString.h` -> **Severity: 1043.048** (Blast Radius: 10.664 * Doc Risk: 97.8102%)
- `interpreter/llvm-project/llvm/include/llvm/ADT/iterator.h` -> **Severity: 863.7** (Blast Radius: 8.637 * Doc Risk: 100.0%)
- `roottest/root/io/simple/cstring.C` -> **Severity: 798.9** (Blast Radius: 7.989 * Doc Risk: 100.0%)
