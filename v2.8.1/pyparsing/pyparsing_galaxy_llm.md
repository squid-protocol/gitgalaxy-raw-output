...
> **Composition Archetype:** `Small Flat Repo` (z -1.08; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 29%, Declarative / Non-Code 18%, Large Core Modules 15%, Interface Declarations Files 11%, Defensive Guards Files 8%
...
- **Heaviest Functions:** `make_compressed_re` (Many-Argument Workhorses, Impact: 89.6), `_collapse_string_to_ranges` (Many-Argument Workhorses, Impact: 21.6), `replaced_by_pep8` (Defensive Guards, Impact: 14.3)
...
- **Heaviest Functions:** `run_tests` (Many-Argument Workhorses, Impact: 172.1), `__init__` (Many-Argument Workhorses, Impact: 170.4), `__init__` (Many-Argument Workhorses, Impact: 94.8)
...
- **Heaviest Functions:** `with_line_numbers` (Many-Argument Workhorses, Impact: 100.9), `assertRunTestResults` (Many-Argument Workhorses, Impact: 59.5), `restore` (Compute Cores, Impact: 13.1)
...
- **Heaviest Functions:** `one_of` (Many-Argument Workhorses, Impact: 87.7), `infix_notation` (Many-Argument Workhorses, Impact: 86.2), `nested_expr` (Many-Argument Workhorses, Impact: 64.3)

### 5. `pyparsing-3.3.2/dest/inv_regex.py` (PYTHON) -> Cumulative Risk: **607.81**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.08)
...
- **Heaviest Functions:** `handle_repetition` (Compute Cores, Impact: 12.3), `handle_macro` (Type Conversions, Impact: 12.3), `make_generator` (Interface Declarations, Impact: 9.2)
...
- **Heaviest Functions:** `_to_diagram_element` (Many-Argument Workhorses, Impact: 224.5), `to_railroad` (Many-Argument Workhorses, Impact: 35.0), `mark_for_extraction` (Many-Argument Workhorses, Impact: 21.0)
...
- **Heaviest Functions:** `dump` (Many-Argument Workhorses, Impact: 40.6), `__init__` (Many-Argument Workhorses, Impact: 39.1), `pop` (Many-Argument Workhorses, Impact: 21.1)
...
- **Heaviest Functions:** `with_attribute` (Many-Argument Workhorses, Impact: 16.7), `pa` (Generic / Templated Code, Impact: 10.5), `with_class` (Many-Argument Workhorses, Impact: 8.0)
...
- **Heaviest Functions:** `explain_exception` (Many-Argument Workhorses, Impact: 25.6), `found` (Compute Cores, Impact: 9.2), `mark_input_line` (Generic / Templated Code, Impact: 8.8)
...
- **Heaviest Functions:** `update_special_changes` (Many-Argument Workhorses, Impact: 6.8), `camel_to_snake` (Generic / Templated Code, Impact: 5.9), `show_diffs` (Parameter Forwarders, Impact: 3.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_do_command` **(Compute Cores)** (Impact: 16.5)
  * `_do_command` **(Compute Cores)** (Impact: 12.8)
  * `_do_command` **(Compute Cores)** (Impact: 12.8)
  * `_do_command` **(Compute Cores)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_enablePackrat_emits_DeprecationWarning_and_cleanup` **(Tests & Verification)** (Impact: 3.5)
    * *Intent:* # Record initial packrat state; restore it after test completes initially_enabled = getattr(ParserEl...
  * `test_runTests_emits_DeprecationWarning` **(Tests & Verification)** (Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_convert_abs_day_reference_to_date` **(Compute Cores)** (Impact: 17.1)
  * `make_weekday_time_references` **(I/O & Config Routines)** (Impact: 13.4)
  * `_compute_timestamp` **(Compute Cores)** (Impact: 11.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_apply_op` **(Defensive Guards)** (Impact: 30.9)
...
**Top Internal Functions/Classes:**
  ...
  * `make_term` **(Compute Cores)** (Impact: 10.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_split_words` **(Compute Cores)** (Impact: 11.4)
...
**Top Internal Functions/Classes:**
  ...
  * `execute` **(Compute Cores)** (Impact: 10.9)
...
**Top Internal Functions/Classes:**
  ...
  * `expand_state_definition` **(Many-Argument Workhorses)** (Impact: 15.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_load_functions_from_file` **(Defensive Guards)** (Impact: 18.6)
  * `_print_functions` **(Interface Declarations)** (Impact: 5.5)
  * `_build_nodes_from_stmt_seq` **(Generic / Templated Code)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  * `handle_repetition` **(Compute Cores)** (Impact: 12.3)
  * `handle_macro` **(Type Conversions)** (Impact: 12.3)
  * `make_generator` **(Interface Declarations)** (Impact: 9.2)
