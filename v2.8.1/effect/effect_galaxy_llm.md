...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.33; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 35%, Data / Markup / Trivial 23%, Callbacks & Closures Files 11%, Generic / Templated Code Files 11%, Large Core Modules 10%
...
- `escapeMermaidLabel` **(Compute Cores)** (@ `packages/effect/src/Graph.ts`) -> Impact: **503.8** | LOC: 2014
  * *Intent:* /** @internal */
...
- `default` **(Compute Cores)** (@ `packages/ai/anthropic/src/Generated.ts`) -> Impact: **197.0** | LOC: 3279
  * *Intent:* /** * Deleted object type. * * For file deletion, this is always `"file_deleted"`. */
- `valuesOf` **(Compute Cores)** (@ `packages/effect/test/TArray.test.ts`) -> Impact: **182.6** | LOC: 1022
...
- **Heaviest Functions:** `resolveQuantile` (Many-Argument Workhorses, Impact: 35.6), `update` (Compute Cores, Impact: 14.3), `snapshot` (Compute Cores, Impact: 8.8)
...
- **Heaviest Functions:** `executeStream` (Defensive Guards, Impact: 15.2), `onMessage` (Compute Cores, Impact: 12.3), `try` (Defensive Guards, Impact: 9.1)

### 3. `packages/cluster/src/Runners.ts` (TYPESCRIPT) -> Cumulative Risk: **610.93**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.44)
...
- **Heaviest Functions:** `notifyWith` (Compute Cores, Impact: 20.6), `notifyLocal` (Callbacks & Closures, Impact: 9.5), `notify` (Callbacks & Closures, Impact: 9.4)
...
- **Heaviest Functions:** `allSuccesses` (Defensive Guards, Impact: 117.7), `forkWithScopeOverride` (Generic / Templated Code, Impact: 92.4), `withEarlyRelease` (Generic / Templated Code, Impact: 85.2)
...
- **Heaviest Functions:** `array` (Compute Cores, Impact: 9.2), `done` (Generic / Templated Code, Impact: 6.1), `single` (Interface Declarations, Impact: 4.7)
...
- **Heaviest Functions:** `unsafeCompleteSubscribers` (Generic / Templated Code, Impact: 30.5), `unsafeStrategyCompletePollers` (Many-Argument Workhorses, Impact: 23.6), `unsafeOnPubSubEmptySpace` (Compute Cores, Impact: 16.7)
...
- **Heaviest Functions:** `compile` (Many-Argument Workhorses, Impact: 120.8), `placeholderNoIncrement` (Compute Cores, Impact: 79.2), `statement` (Many-Argument Workhorses, Impact: 25.5)
...
- **Heaviest Functions:** `writeFromRemote` (Compute Cores, Impact: 23.7), `withRemoteUncommited` (Callbacks & Closures, Impact: 10.6), `makeIndexedDb` (Defensive Guards, Impact: 8.8)
...
- **Heaviest Functions:** `unsafeRebuild` (Callbacks & Closures, Impact: 4.2), `unsafeGet` (Interface Declarations, Impact: 2.3), `constructor` (Generic / Templated Code, Impact: 1.9)
...
- **Heaviest Functions:** `catch` (Compute Cores, Impact: 135.4), `getSystemMessageMode` (Compute Cores, Impact: 101.7), `annotateStreamResponse` (Many-Argument Workhorses, Impact: 51.3)
...
**Top Internal Functions/Classes:**
  * `allSuccesses` **(Defensive Guards)** (Impact: 117.7)
    * *Intent:* /* @internal */
...
**Top Internal Functions/Classes:**
  * `escapeMermaidLabel` **(Compute Cores)** (Impact: 503.8)
    * *Intent:* /** @internal */
...
**Top Internal Functions/Classes:**
  ...
  * `default` **(Compute Cores)** (Impact: 197.0)
    * *Intent:* /** * Deleted object type. * * For file deletion, this is always `"file_deleted"`. */
...
**Top Internal Functions/Classes:**
  ...
  * `typeAST` **(Callbacks & Closures)** (Impact: 31.0)
    * *Intent:* /** * @since 3.10.0 */
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `computeResult` **(Compute Cores)** (Impact: 64.0)
    * *Intent:* // --------------------------------------------- // compute result // ------------------------------...
  * `getLiterals` **(Compute Cores)** (Impact: 52.7)
    * *Intent:* /** @internal */
...
**Top Internal Functions/Classes:**
  ...
  * `getOrder` **(Compute Cores)** (Impact: 66.1)
    * *Intent:* /** @internal */
  * `[Symbol.iterator]` **(Compute Cores)** (Impact: 54.9)
...
**Top Internal Functions/Classes:**
  ...
  * `makeBoundedPubSub` **(Compute Cores)** (Impact: 13.7)
    * *Intent:* /** @internal */
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
...
**Top Internal Functions/Classes:**
  ...
  * `getZshCompletions` **(Compute Cores)** (Impact: 28.8)
    * *Intent:* /** @internal */
  * `getHelpInternal` **(Compute Cores)** (Impact: 24.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `unexpectedStatus` **(Callbacks & Closures)** (Impact: 5.5)
  * `default` **(I/O & Config Routines)** (Impact: 4.7)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `intlTimeZone` **(Compute Cores)** (Impact: 40.8)
    * *Intent:* // ============================================================================= // formatting // ==...
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `catch` **(Compute Cores)** (Impact: 135.4)
...
**Top Internal Functions/Classes:**
  ...
  * `collectWhileEffect` **(Generic / Templated Code)** (Impact: 20.0)
    * *Intent:* /** @internal */
  * `intersectWithLoop` **(Compute Cores)** (Impact: 12.3)
    * *Intent:* /** @internal */
  * `fixed` **(Callbacks & Closures)** (Impact: 11.7)
    * *Intent:* /** @internal */
...
- `packages/effect/src/Function.ts` -> **Severity: 15.169** (Embedded: 0.2406 * Error Risk: 63.0579%)
- `packages/effect/src/Schema.ts` -> **Severity: 11.951** (Embedded: 0.2028 * Error Risk: 58.9413%)
- `packages/effect/src/Context.ts` -> **Severity: 10.106** (Embedded: 0.1066 * Error Risk: 94.8338%)
- `packages/effect/src/Layer.ts` -> **Severity: 7.939** (Embedded: 0.1097 * Error Risk: 72.3617%)
- `packages/effect/src/ParseResult.ts` -> **Severity: 4.878** (Embedded: 0.0618 * Error Risk: 78.8744%)
