...
> **File Composition:** Data / Markup / Trivial 60%, Callbacks & Closures Files 20%, Interface Declarations Files 20%
...
- `BODY_CHUNKTRAILERS` **(Callbacks & Closures)** (@ `package/http-parser.js`) -> Impact: **5.7** | LOC: 14
- `finish` **(Callbacks & Closures)** (@ `package/http-parser.js`) -> Impact: **5.5** | LOC: 11
...
- **Heaviest Functions:** `HEADER` (Defensive Guards, Impact: 37.7), `execute` (Defensive Guards, Impact: 21.8), `parseHeader` (Defensive Guards, Impact: 14.9)
...
- **Heaviest Functions:** `onMessageComplete` (I/O & Config Routines, Impact: 4.5), `HEADER` (Interface Declarations, Impact: 1.1), `BODY_CHUNKHEAD` (Interface Declarations, Impact: 1.1)
...
**Top Internal Functions/Classes:**
  ...
  * `HEADER` **(Interface Declarations)** (Impact: 1.1)
    * *Intent:* /** * For older versions of node (v6.x and older?), that return `skipBody=1` or `skipBody=true`, nee...
  * `BODY_CHUNKHEAD` **(Interface Declarations)** (Impact: 1.1)
  * `BODY_CHUNK` **(Interface Declarations)** (Impact: 1.1)
  * `BODY_CHUNKEMPTYLINE` **(Interface Declarations)** (Impact: 1.1)
**Structural Signatures (Net Mitigated Signals):**
