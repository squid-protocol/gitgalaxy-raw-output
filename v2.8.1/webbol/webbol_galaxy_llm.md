...
> **File Composition:** I/O & Config Routines Files 38%, Data / Markup / Trivial 23%, Declarative / Non-Code 23%, Compute Cores Files 8%, Interface Declarations Files 8%
...
- `DECODE-HEX-CHAR` **(Compute Cores)** (@ `url-decode.cbl`) -> Impact: **48.8** | LOC: 76
...
- `INIT-SOCKET` **(Interface Declarations)** (@ `webserver.cbl`) -> Impact: **7.8** | LOC: 55
...
- **Heaviest Functions:** `MAIN-LOGIC` (I/O & Config Routines, Impact: 22.9)

### 2. `path-utils.cbl` (COBOL) -> Cumulative Risk: **634.64**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.27)
...
- **Heaviest Functions:** `MAIN-LOGIC` (I/O & Config Routines, Impact: 14.6), `LS-RETURN-CODE` (I/O & Config Routines, Impact: 1.1)

### 3. `url-decode.cbl` (COBOL) -> Cumulative Risk: **591.6**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +2.78)
...
- **Heaviest Functions:** `DECODE-HEX-CHAR` (Compute Cores, Impact: 48.8), `MAIN-LOGIC` (I/O & Config Routines, Impact: 9.5)

### 4. `file-ops.cbl` (COBOL) -> Cumulative Risk: **573.52**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.18)
...
- **Heaviest Functions:** `MAIN-LOGIC` (I/O & Config Routines, Impact: 23.7), `FILE-CONTROL` (I/O & Config Routines, Impact: 2.5)
...
- **Heaviest Functions:** `INIT-SOCKET` (Interface Declarations, Impact: 7.8), `HANDLE-REQUEST` (I/O & Config Routines, Impact: 6.5), `MAIN-LOGIC` (I/O & Config Routines, Impact: 4.0)
...
- **Heaviest Functions:** `MAIN-LOGIC` (I/O & Config Routines, Impact: 13.9), `BUILD-200-RESPONSE` (I/O & Config Routines, Impact: 3.0), `BUILD-404-RESPONSE` (I/O & Config Routines, Impact: 1.5)
...
- **Heaviest Functions:** `url-decode.o` (I/O & Config Routines, Impact: 1.3), `run` (I/O & Config Routines, Impact: 1.2), `all` (I/O & Config Routines, Impact: 1.1)
...
**Top Internal Functions/Classes:**
  * `DECODE-HEX-CHAR` **(Compute Cores)** (Impact: 48.8)
...
**Top Internal Functions/Classes:**
  * `MAIN-LOGIC` **(I/O & Config Routines)** (Impact: 14.6)
    * *Intent:* *> Main path validation and sanitization logic
  * `LS-RETURN-CODE` **(I/O & Config Routines)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `INIT-SOCKET` **(Interface Declarations)** (Impact: 7.8)
    * *Intent:* *> Create and configure a TCP socket for the web server
