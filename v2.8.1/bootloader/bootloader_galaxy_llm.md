...
> **Composition Archetype:** `Small Flat Repo` (z +0.72; from the repo's file-archetype mix)
> **File Composition:** Annotated Framework Methods Files 26%, Interface Declarations Files 18%, Large Core Modules 17%, Data / Markup / Trivial 14%, State Mutators Files 11%
...
- **Heaviest Functions:** `read_exact_into` (Many-Argument Workhorses, Impact: 9.7), `read_exact_at` (Many-Argument Workhorses, Impact: 5.2), `seek` (Compute Cores, Impact: 3.9)
...
- **Heaviest Functions:** `build_bios_stage_2` (I/O & Config Routines, Impact: 10.3), `build_uefi_bootloader` (I/O & Config Routines, Impact: 9.9), `build_bios_boot_sector` (I/O & Config Routines, Impact: 9.2)
...
- **Heaviest Functions:** `classify_fat_entry` (Many-Argument Workhorses, Impact: 17.1), `find_file_in_root_dir` (Many-Argument Workhorses, Impact: 14.3), `parse` (Defensive Guards, Impact: 10.7)
...
- **Heaviest Functions:** `write_char` (Compute Cores, Impact: 7.7), `write_rendered_char` (Compute Cores, Impact: 5.6), `write_pixel` (Many-Argument Workhorses, Impact: 5.4)
...
- **Heaviest Functions:** `main_inner` (Many-Argument Workhorses, Impact: 26.3), `init_logger` (Many-Argument Workhorses, Impact: 18.1), `create_page_tables` (Many-Argument Workhorses, Impact: 13.9)
...
- **Heaviest Functions:** `create_uefi_tftp_folder` (Compute Cores, Impact: 15.5), `create_fat_filesystem_image` (Many-Argument Workhorses, Impact: 11.2), `create_bios_image` (Compute Cores, Impact: 10.1)

### 7. `bios/boot_sector/src/boot.s` (ASSEMBLY) -> Cumulative Risk: **455.69**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.54)
...
- **Heaviest Functions:** `enable_a20` (State Mutators, Impact: 3.2), `_start` (Interface Declarations, Impact: 2.2), `check_int13h_extensions` (Parameter Forwarders, Impact: 2.1)

### 8. `bios/stage-3/src/paging.rs` (RUST) -> Cumulative Risk: **453.6**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.14)
...
- **Heaviest Functions:** `create_mappings` (Type Conversions, Impact: 3.8), `enable_paging` (Interface Declarations, Impact: 1.5), `init` (State Mutators, Impact: 1.2)
...
- **Heaviest Functions:** `first_stage` (Compute Cores, Impact: 7.9), `second_stage_start` (Interface Declarations, Impact: 1.2), `partition_table_raw` (State Mutators, Impact: 1.1)
...
- **Heaviest Functions:** `try_load_file` (Many-Argument Workhorses, Impact: 19.7), `start` (Many-Argument Workhorses, Impact: 17.6), `split_array_ref` (Interface Declarations, Impact: 3.2)
...
**Top Internal Functions/Classes:**
  ...
  * `load_segments` **(Compute Cores)** (Impact: 23.5)
...
**Top Internal Functions/Classes:**
  ...
  * `parse` **(Defensive Guards)** (Impact: 10.7)
...
**Top Internal Functions/Classes:**
  ...
  * `deserialize` **(Compute Cores)** (Impact: 6.3)
  * `random` **(Tests & Verification)** (Impact: 5.8)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `test_multiple_regions` **(Tests & Verification)** (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `as_ref` **(State Mutators)** (Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `create_uefi_fat_partition` **(Annotated Framework Methods)** (Impact: 4.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
* *State Mutation (weighted view):* 13
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `build_uefi_bootloader` **(I/O & Config Routines)** (Impact: 9.9)
...
**Top Internal Functions/Classes:**
  ...
  * `query` **(Many-Argument Workhorses)** (Impact: 4.2)
  * `query` **(Generic / Templated Code)** (Impact: 1.9)
  * `enable` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `get_char_raster` **(State Mutators)** (Impact: 3.3)
...
**Top Internal Functions/Classes:**
  ...
  * `detect_rsdp` **(Annotated Framework Methods)** (Impact: 4.0)
  * `panic` **(Annotated Framework Methods)** (Impact: 3.5)
  * `create_page_tables` **(Interface Declarations)** (Impact: 3.1)
    * *Intent:* /// Creates page table abstraction types for both the bootloader and kernel page tables.
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
**Top Internal Functions/Classes:**
  * `read_exact_into` **(Many-Argument Workhorses)** (Impact: 9.7)
  * `read_exact_at` **(Many-Argument Workhorses)** (Impact: 5.2)
  * `seek` **(Compute Cores)** (Impact: 3.9)
...
**Top Internal Functions/Classes:**
  ...
  * `split_array_ref` **(Interface Declarations)** (Impact: 3.2)
  ...
  * `_start` **(Annotated Framework Methods)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 6
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `create_fat_filesystem` **(Compute Cores)** (Impact: 14.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
...
**Top Internal Functions/Classes:**
  * `run_qemu` **(Compute Cores)** (Impact: 8.7)
  * `run_test_kernel_internal` **(Defensive Guards)** (Impact: 7.7)
  * `run_test_kernel_on_uefi_pxe` **(Annotated Framework Methods)** (Impact: 2.2)
  * `run_test_kernel_with_config_file` **(State Mutators)** (Impact: 2.0)
  * `run_test_kernel_on_uefi` **(Annotated Framework Methods)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `copy_to_protected_mode` **(Annotated Framework Methods)** (Impact: 3.8)
  * `new` **(Interface Declarations)** (Impact: 2.1)
  * `enter_protected_mode_and_jump_to_stage_3` **(Parameter Forwarders)** (Impact: 1.9)
  * `clear_interrupts_and_load` **(Generic / Templated Code)** (Impact: 1.8)
  * `read_from_protected_mode` **(Annotated Framework Methods)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `tsc_entropy` **(Interface Declarations)** (Impact: 5.0)
  ...
  * `build_rng` **(Interface Declarations)** (Impact: 3.8)
