...
> **File Composition:** Generic / Templated Code Files 27%, Large Core Modules 23%, Tests & Verification Files 15%, Declarative / Non-Code 12%, Defensive Guards Files 12%
...
- `_mock_pyinfo` **(Many-Argument Workhorses)** (@ `python_discovery-1.2.1/tests/windows/conftest.py`) -> Impact: **35.6** | LOC: 26
...
- `_find_interpreter` **(Many-Argument Workhorses)** (@ `python_discovery-1.2.1/src/python_discovery/_discovery.py`) -> Impact: **25.7** | LOC: 25
...
- **Composition Archetype:** `Defensive Guards Files` (z -0.85)
...
- **Heaviest Functions:** `parse_version` (Defensive Guards, Impact: 19.0), `load_exe` (Defensive Guards, Impact: 18.8), `load_threaded` (Defensive Guards, Impact: 13.9)

### 2. `python_discovery-1.2.1/src/python_discovery/_windows/_propose.py` (PYTHON) -> Cumulative Risk: **533.02**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.06)
...
- **Heaviest Functions:** `propose_interpreters` (Many-Argument Workhorses, Impact: 23.1)
...
- **Heaviest Functions:** `_get_via_file_cache` (Many-Argument Workhorses, Impact: 41.8), `_run_subprocess` (Many-Argument Workhorses, Impact: 30.8), `from_exe` (Many-Argument Workhorses, Impact: 14.1)
...
- **Heaviest Functions:** `satisfies` (Compute Cores, Impact: 33.1), `discover_exe` (Many-Argument Workhorses, Impact: 31.3), `_check_exe` (Many-Argument Workhorses, Impact: 29.8)

### 5. `python_discovery-1.2.1/src/python_discovery/_specifier.py` (PYTHON) -> Cumulative Risk: **499.1**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.93)
...
- **Heaviest Functions:** `from_string` (Compute Cores, Impact: 32.3), `__lt__` (Defensive Guards, Impact: 18.0), `from_string` (Defensive Guards, Impact: 13.6)

### 6. `python_discovery-1.2.1/src/python_discovery/_discovery.py` (PYTHON) -> Cumulative Risk: **494.88**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.34)
...
- **Heaviest Functions:** `_find_interpreter` (Many-Argument Workhorses, Impact: 25.7), `_read_python_version_file` (Compute Cores, Impact: 19.8), `propose_interpreters` (Many-Argument Workhorses, Impact: 19.2)
...
- **Heaviest Functions:** `py_info_clear` (Generic / Templated Code, Impact: 6.1), `read` (Defensive Guards, Impact: 5.0), `write` (Generic / Templated Code, Impact: 2.1)

### 8. `python_discovery-1.2.1/src/python_discovery/_compat.py` (PYTHON) -> Cumulative Risk: **462.6**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.52)
...
- **Heaviest Functions:** `fs_path_id` (Generic / Templated Code, Impact: 4.3), `fs_is_case_sensitive` (Interface Declarations, Impact: 3.2)

### 9. `python_discovery-1.2.1/tests/py_info/test_py_info_exe_based_of.py` (PYTHON) -> Cumulative Risk: **377.7**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.46)
...
- **Heaviest Functions:** `test_discover_ok` (Many-Argument Workhorses, Impact: 19.9), `_discover_base_folders` (Type Conversions, Impact: 3.5), `test_discover_empty_folder` (Generic / Templated Code, Impact: 1.9)
...
- **Heaviest Functions:** `test_discovery_via_path` (Many-Argument Workhorses, Impact: 52.3), `test_shim_resolved_to_real_binary` (Many-Argument Workhorses, Impact: 21.2), `test_discovery_absolute_path_with_try_first` (Defensive Guards, Impact: 9.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_init_sysconfig` **(Compute Cores)** (Impact: 22.8)
  * `_satisfies_version_specifier` **(Compute Cores)** (Impact: 20.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_discovery_absolute_path_with_try_first` **(Defensive Guards)** (Impact: 9.8)
  * `test_uv_python` **(Many-Argument Workhorses)** (Impact: 8.9)
  * `_create_versioned_binary` **(Generic / Templated Code)** (Impact: 7.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_check_exe_mismatch_not_exact` **(Defensive Guards)** (Impact: 5.7)
  * `test_check_exe_mismatch_exact` **(Defensive Guards)** (Impact: 5.7)
  * `test_resolve_to_system_circle` **(Tests & Verification)** (Impact: 4.5)
...
**Top Internal Functions/Classes:**
  * `_find_interpreter` **(Many-Argument Workhorses)** (Impact: 25.7)
  * `_read_python_version_file` **(Compute Cores)** (Impact: 19.8)
  ...
  * `_propose_explicit` **(Generic / Templated Code)** (Impact: 15.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 50 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_generate_not_match_current_interpreter_version` **(Generic / Templated Code)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_get_from_cache` **(Many-Argument Workhorses)** (Impact: 13.1)
  * `_extract_between_cookies` **(Generic / Templated Code)** (Impact: 10.7)
...
**Top Internal Functions/Classes:**
  ...
  * `_check_standard` **(Type Conversions)** (Impact: 9.6)
...
**Top Internal Functions/Classes:**
  * `_mock_pyinfo` **(Many-Argument Workhorses)** (Impact: 35.6)
  * `satisfies` **(Compute Cores)** (Impact: 17.8)
  * `_make_open_key_ex` **(Defensive Guards)** (Impact: 16.4)
  * `_open_key_ex` **(Defensive Guards)** (Impact: 13.4)
  * `_mock_registry` **(Defensive Guards)** (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_propose_interpreters_try_first_with_duplicate` **(Defensive Guards)** (Impact: 6.1)
...
**Top Internal Functions/Classes:**
  ...
  * `test_resolve_py_info_script_pkgutil_returns_none` **(Tests & Verification)** (Impact: 3.1)
  * `test_run_subprocess_with_cookies` **(Tests & Verification)** (Impact: 2.3)
...
**Top Internal Functions/Classes:**
  * `test_version_parse` **(Defensive Guards)** (Impact: 2.7)
  * `test_version_invalid_raises` **(Tests & Verification)** (Impact: 2.1)
  * `test_version_lt` **(Generic / Templated Code)** (Impact: 2.1)
  * `test_specifier_invalid_raises` **(Tests & Verification)** (Impact: 2.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_discover_base_folders` **(Type Conversions)** (Impact: 3.5)
  * `test_discover_empty_folder` **(Generic / Templated Code)** (Impact: 1.9)
  * `_fs_supports_symlink` **(Generic / Templated Code)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_get_required_precision_attribute_error` **(Tests & Verification)** (Impact: 1.6)
  * `test_specifier_parse_failure_fallback` **(Tests & Verification)** (Impact: 1.2)
  * `test_version_specifier_satisfies_micro` **(Tests & Verification)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `fs_is_case_sensitive` **(Interface Declarations)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
**Top Internal Functions/Classes:**
  * `_skip_if_test_in_system` **(Generic / Templated Code)** (Impact: 4.5)
  * `_ensure_py_info_cache_empty` **(Generic / Templated Code)** (Impact: 1.6)
