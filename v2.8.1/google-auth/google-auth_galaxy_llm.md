...
> **Composition Archetype:** `Hub-Coupled App` (z +0.43; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 26%, Defensive Guards Files 25%, Data / Markup / Trivial 21%, Interface Declarations Files 9%, Many-Argument Workhorses Files 6%
...
- `__init__` **(Many-Argument Workhorses)** (@ `google_auth-2.49.1/google/auth/identity_pool.py`) -> Impact: **54.2** | LOC: 124
...
- `__init__` **(Many-Argument Workhorses)** (@ `google_auth-2.49.1/google/auth/compute_engine/credentials.py`) -> Impact: **45.0** | LOC: 78
- `request` **(Many-Argument Workhorses)** (@ `google_auth-2.49.1/google/auth/transport/_aiohttp_requests.py`) -> Impact: **44.4** | LOC: 129
- `__init__` **(Many-Argument Workhorses)** (@ `google_auth-2.49.1/google/auth/pluggable.py`) -> Impact: **39.1** | LOC: 103
...
- `__init__` **(Many-Argument Workhorses)** (@ `google_auth-2.49.1/google/auth/external_account.py`) -> Impact: **38.0** | LOC: 100
...
- **Heaviest Functions:** `request` (Many-Argument Workhorses, Impact: 44.4), `__call__` (Many-Argument Workhorses, Impact: 8.3), `__init__` (Compute Cores, Impact: 5.5)

### 2. `google_auth-2.49.1/google/auth/aio/transport/sessions.py` (PYTHON) -> Cumulative Risk: **706.94**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.95)
...
- **Heaviest Functions:** `request` (Many-Argument Workhorses, Impact: 35.5), `__init__` (Defensive Guards, Impact: 14.9), `configure_mtls_channel` (Many-Argument Workhorses, Impact: 14.0)

### 3. `google_auth-2.49.1/google/oauth2/_client_async.py` (PYTHON) -> Cumulative Risk: **687.87**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.50)
...
- **Heaviest Functions:** `_token_endpoint_request_no_throw` (Many-Argument Workhorses, Impact: 29.8), `refresh_grant` (Many-Argument Workhorses, Impact: 11.8), `_token_endpoint_request` (Many-Argument Workhorses, Impact: 7.2)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 54.2), `get_subject_token` (Many-Argument Workhorses, Impact: 15.6), `_validate_file_or_url_config` (Compute Cores, Impact: 13.4)
...
- **Heaviest Functions:** `_create_self_signed_jwt` (Compute Cores, Impact: 24.4), `__init__` (Many-Argument Workhorses, Impact: 22.1), `__init__` (Many-Argument Workhorses, Impact: 20.8)
...
- **Heaviest Functions:** `obtain_challenge_input` (Defensive Guards, Impact: 27.9), `_obtain_challenge_input_webauthn` (Many-Argument Workhorses, Impact: 22.6), `obtain_challenge_input` (Parameter Forwarders, Impact: 3.7)
...
- **Heaviest Functions:** `from_string` (Compute Cores, Impact: 6.5), `from_string` (Many-Argument Workhorses, Impact: 3.1), `verify` (Defensive Guards, Impact: 2.4)

### 8. `google_auth-2.49.1/google/oauth2/_reauth_async.py` (PYTHON) -> Cumulative Risk: **636.13**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.58)
...
- **Heaviest Functions:** `refresh_grant` (Many-Argument Workhorses, Impact: 31.4), `_obtain_rapt` (Many-Argument Workhorses, Impact: 16.4), `_run_next_challenge` (Many-Argument Workhorses, Impact: 14.3)
...
- **Heaviest Functions:** `attach_to_ssl_context` (Compute Cores, Impact: 13.1), `sign_callback` (Many-Argument Workhorses, Impact: 8.2), `load_signer_lib` (Compute Cores, Impact: 7.2)

### 10. `google_auth-2.49.1/google/oauth2/_credentials_async.py` (PYTHON) -> Cumulative Risk: **619.37**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.68)
...
- **Heaviest Functions:** `refresh` (Compute Cores, Impact: 17.9), `before_request` (Parameter Forwarders, Impact: 5.1)
...
**Top Internal Functions/Classes:**
  ...
  * `test_constructor_raises_no_auth_request_error` **(Tests & Verification)** (Impact: 4.6)
  * `test_constructor_raises_incorrect_credentials_error` **(Tests & Verification)** (Impact: 4.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_refresh_success_with_valid_trust_boundary` **(Many-Argument Workhorses)** (Impact: 4.1)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `test_revoke_failed` **(Compute Cores)** (Impact: 6.9)
  * `make_pluggable` **(Many-Argument Workhorses)** (Impact: 5.4)
  * `test_retrieve_subject_token_missing_error_code_message` **(Tests & Verification)** (Impact: 5.2)
  * `test_retrieve_subject_token_failed_interactive_mode` **(Many-Argument Workhorses)** (Impact: 5.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 38.0)
  * `_perform_refresh_token` **(Many-Argument Workhorses)** (Impact: 28.6)
  * `get_project_id` **(Compute Cores)** (Impact: 19.7)
    * *Intent:* """Retrieves the project ID corresponding to the workload identity or workforce pool. For workforce ...
  * `_build_trust_boundary_lookup_url` **(Compute Cores)** (Impact: 10.1)
...
**Top Internal Functions/Classes:**
  * `_create_self_signed_jwt` **(Compute Cores)** (Impact: 24.4)
    * *Intent:* """Create a self-signed JWT from the credentials if requirements are met. Args: audience (str): The ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 22.1)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 20.8)
  * `_perform_refresh_token` **(Compute Cores)** (Impact: 15.1)
  * `_with_use_iam_endpoint` **(Compute Cores)** (Impact: 8.3)
...
**Top Internal Functions/Classes:**
  ...
  * `test_use_client_cert_fallback` **(Defensive Guards)** (Impact: 5.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_verify_iat_and_exp` **(Compute Cores)** (Impact: 10.7)
...
**Top Internal Functions/Classes:**
  ...
  * `token_factory` **(Many-Argument Workhorses)** (Impact: 13.4)
...
**Top Internal Functions/Classes:**
  * `refresh` **(Many-Argument Workhorses)** (Impact: 33.8)
  ...
  * `refresh_handler` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_scopes_to_string` **(Interface Declarations)** (Impact: 5.7)
  * `test_padded_urlsafe_b64decode` **(Interface Declarations)** (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  ...
  * `test_get_universe_domain_retryable_error_success` **(Interface Declarations)** (Impact: 4.4)
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 54.2)
  ...
  * `_validate_file_or_url_config` **(Compute Cores)** (Impact: 13.4)
  * `_validate_certificate_config` **(Compute Cores)** (Impact: 9.4)
  * `_create_default_metrics_options` **(Defensive Guards)** (Impact: 9.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 41 instances
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Defensive Guards)** (Impact: 14.9)
  * `configure_mtls_channel` **(Many-Argument Workhorses)** (Impact: 14.0)
    * *Intent:* """Configure the client certificate and key for SSL connection. The function does nothing unless `GO...
  * `_do_configure` **(I/O & Config Routines)** (Impact: 7.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_build_trust_boundary_lookup_url` **(Defensive Guards)** (Impact: 8.6)
    * *Intent:* """Builds and returns the URL for the trust boundary lookup API for GCE."""
  * `_retrieve_info` **(Compute Cores)** (Impact: 8.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_prepare_request_for_mds` **(Defensive Guards)** (Impact: 9.9)
