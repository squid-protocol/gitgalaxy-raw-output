...
> **Composition Archetype:** `Mainframe / COBOL & Config` (z +0.82; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 29%, I/O & Config Routines Files 24%, Declarative / Non-Code 17%, Interface Declarations Files 10%, Large Core Modules 7%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 65.5 | 100.0 | 100.0 |
...
### 1. `multiboot/osdev/kernel.c` (C) -> Cumulative Risk: **535.32**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%), Tech Debt (93.1734%)
- **Heaviest Functions:** `terminal_putchar` (Compute Cores, Impact: 4.7), `terminal_initialize` (Defensive Guards, Impact: 3.6), `strlen` (Defensive Guards, Impact: 3.1)

### 2. `nasm/protected_mode_thiscouldbebetter.asm` (ASSEMBLY) -> Cumulative Risk: **478.57**
- **Archetype:** `file_cluster_1` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +1.81)
- **Magnitude:** 17.42 | **LOC:** 116 | **CtrlFlow:** 1.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (94.5687%), State Flux (86.8266%)
- **Heaviest Functions:** `for_each_char` (Interface Declarations, Impact: 3.3), `stage2` (Parameter Forwarders, Impact: 3.2), `gdt` (I/O & Config Routines, Impact: 1.5)

### 3. `bios_pixel.S` (ASSEMBLY) -> Cumulative Risk: **475.03**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.67)
- **Magnitude:** 12.92 | **LOC:** 30 | **CtrlFlow:** 6.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (91.6827%), Safety Score (86.8266%)
- **Heaviest Functions:** `start` (Many-Argument Workhorses, Impact: 5.5), `end` (I/O & Config Routines, Impact: 1.1)

### 4. `bios_pixel_line.S` (ASSEMBLY) -> Cumulative Risk: **475.02**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.77)
- **Magnitude:** 12.32 | **LOC:** 20 | **CtrlFlow:** 6.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (91.6827%), Safety Score (86.8266%)
- **Heaviest Functions:** `start` (Parameter Forwarders, Impact: 4.9), `end` (I/O & Config Routines, Impact: 1.1)

### 5. `multiboot/hello-world/main.c` (C) -> Cumulative Risk: **459.79**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%), Safety Score (89.4999%)
- **Heaviest Functions:** `puts` (Parameter Forwarders, Impact: 5.1), `clear` (Interface Declarations, Impact: 4.5), `putc` (Parameter Forwarders, Impact: 2.6)

### 6. `c_hello_world/main.c` (C) -> Cumulative Risk: **455.42**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.33)
- **Magnitude:** 9.44 | **LOC:** 13 | **CtrlFlow:** 16.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (91.6827%), Safety Score (82.388%)
- **Heaviest Functions:** `main` (Interface Declarations, Impact: 2.2)

### 7. `uefi/Makefile` (MAKEFILE) -> Cumulative Risk: **434.08**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.68)
- **Magnitude:** 13.16 | **LOC:** 38 | **CtrlFlow:** 3.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (95.2574%), Safety Score (70.2063%)
- **Heaviest Functions:** `%.efi` (Interface Declarations, Impact: 1.2), `all` (I/O & Config Routines, Impact: 1.1), `hello.so` (I/O & Config Routines, Impact: 1.1)

### 8. `bios_sleep.S` (ASSEMBLY) -> Cumulative Risk: **414.78**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.29)
- **Magnitude:** 11.72 | **LOC:** 23 | **CtrlFlow:** 12.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (81.7574%), Safety Score (71.9676%)
- **Heaviest Functions:** `one_sec` (Parameter Forwarders, Impact: 6.5), `infinite` (Parameter Forwarders, Impact: 1.9)

### 9. `smp.S` (ASSEMBLY) -> Cumulative Risk: **396.64**
- **Archetype:** `file_cluster_1` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +2.73)
- **Magnitude:** 9.04 | **LOC:** 106 | **CtrlFlow:** 4.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.9983%), Safety Score (88.6667%)
- **Heaviest Functions:** `not_started` (I/O & Config Routines, Impact: 2.8), `interrupt_handler` (I/O & Config Routines, Impact: 2.2), `init` (Interface Declarations, Impact: 2.0)

### 10. `no-linker-script/main.S` (ASSEMBLY) -> Cumulative Risk: **392.6**
- **Archetype:** `file_cluster_1` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +1.15)
- **Magnitude:** 10.7 | **LOC:** 21 | **CtrlFlow:** 15.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (97.0688%), Safety Score (83.5974%)
- **Heaviest Functions:** `loop` (Compute Cores, Impact: 6.0), `_start` (Parameter Forwarders, Impact: 2.0), `halt` (I/O & Config Routines, Impact: 1.3)
...
- **Magnitude:** 75.76 | **LOC:** 114 | **CtrlFlow:** 11.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (61.1719%), Tech Debt (93.1734%)
...
**Top Internal Functions/Classes:**
  ...
  * `clear` **(Interface Declarations)** (Impact: 4.5)
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (13.9745%)
**Top Internal Functions/Classes:**
  * `CLEAR_LABEL` **(Many-Argument Workhorses)** (Impact: 9.5)
  * `GS` **(Many-Argument Workhorses)** (Impact: 4.5)
...
- **Risk Profile:** Cognitive Load (4.48%), Tech Debt (18.5204%)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (7.0506%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Magnitude:** 17.42 | **LOC:** 116 | **CtrlFlow:** 1.8% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (15.4465%), Tech Debt (94.5687%)
**Top Internal Functions/Classes:**
  * `for_each_char` **(Interface Declarations)** (Impact: 3.3)
  * `stage2` **(Parameter Forwarders)** (Impact: 3.2)
  * `gdt` **(I/O & Config Routines)** (Impact: 1.5)
  * `loop_forever` **(Interface Declarations)** (Impact: 1.2)
  * `end_for_each_char` **(I/O & Config Routines)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (5.9493%), Tech Debt (99.2567%)
**Top Internal Functions/Classes:**
  * `.loop` **(Compute Cores)** (Impact: 7.4)
  * `b32` **(Interface Declarations)** (Impact: 2.2)
  * `print32` **(Interface Declarations)** (Impact: 1.6)
...
- **Risk Profile:** Cognitive Load (5.2957%), Tech Debt (99.7654%)
**Top Internal Functions/Classes:**
...
### `nasm/bios_one_char.asm` (ASSEMBLY | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_15` (Drift: 0.0 IQR)
