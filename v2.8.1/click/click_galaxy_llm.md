...
> **Composition Archetype:** `Typed Library` (z +0.30; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 32%, Large Core Modules 24%, Data / Markup / Trivial 16%, Interface Declarations Files 16%, Defensive Guards Files 6%
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.97)
...
- **Heaviest Functions:** `convert` (Many-Argument Workhorses, Impact: 39.4), `convert_type` (Defensive Guards, Impact: 34.1), `convert` (Many-Argument Workhorses, Impact: 28.3)
...
- **Heaviest Functions:** `_get_windows_console_stream` (Many-Argument Workhorses, Impact: 15.1), `readinto` (Compute Cores, Impact: 11.8), `write` (Compute Cores, Impact: 6.1)
...
- **Heaviest Functions:** `open_url` (Defensive Guards, Impact: 43.5), `__init__` (Many-Argument Workhorses, Impact: 41.7), `_pipepager` (Many-Argument Workhorses, Impact: 28.7)
...
- **Heaviest Functions:** `format_message` (Compute Cores, Impact: 27.4), `show` (Compute Cores, Impact: 11.4), `format_message` (Compute Cores, Impact: 7.6)
...
- **Heaviest Functions:** `_resolve_context` (Many-Argument Workhorses, Impact: 23.1), `_resolve_incomplete` (Many-Argument Workhorses, Impact: 22.2), `_is_incomplete_option` (Many-Argument Workhorses, Impact: 17.1)

### 6. `click-8.3.2/src/click/core.py` (PYTHON) -> Cumulative Risk: **627.95**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.83)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 244.9), `__init__` (Many-Argument Workhorses, Impact: 140.0), `get_help_extra` (Many-Argument Workhorses, Impact: 70.0)
...
- **Heaviest Functions:** `style` (Many-Argument Workhorses, Impact: 110.9), `prompt` (Many-Argument Workhorses, Impact: 75.2), `confirm` (Many-Argument Workhorses, Impact: 37.5)
...
- **Heaviest Functions:** `_get_value_from_state` (Many-Argument Workhorses, Impact: 26.5), `__init__` (Many-Argument Workhorses, Impact: 24.2), `_unpack_args` (Many-Argument Workhorses, Impact: 23.7)
...
- **Heaviest Functions:** `echo` (Many-Argument Workhorses, Impact: 54.0), `make_default_short_help` (Compute Cores, Impact: 23.2), `_detect_program_name` (Many-Argument Workhorses, Impact: 18.2)
...
- **Heaviest Functions:** `version_option` (Many-Argument Workhorses, Impact: 58.1), `command` (Many-Argument Workhorses, Impact: 32.4), `callback` (Defensive Guards, Impact: 17.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_build_prompt` **(Many-Argument Workhorses)** (Impact: 16.6)
  * `echo_via_pager` **(Defensive Guards)** (Impact: 13.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 24.2)
  * `_unpack_args` **(Many-Argument Workhorses)** (Impact: 23.7)
  * `_match_short_opt` **(Many-Argument Workhorses)** (Impact: 21.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_make_text_stream` **(Many-Argument Workhorses)** (Impact: 8.3)
  * `should_strip_ansi` **(Generic / Templated Code)** (Impact: 7.3)
  * `_find_binary_reader` **(Type Conversions)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  * `test_hiding_of_unset_sentinel_in_callbacks` **(I/O & Config Routines)** (Impact: 6.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_progressbar_item_show_func` **(Callbacks & Closures)** (Impact: 9.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_object_propagation` **(Annotated Framework Methods)** (Impact: 8.0)
...
**Top Internal Functions/Classes:**
  * `test_invoke_with_threads_writing_to_streams` **(Tests & Verification)** (Impact: 5.1)
    * *Intent:* """Threads writing to ``click.echo()`` during invocation."""
  * `cli` **(Interface Declarations)** (Impact: 4.7)
  * `test_invoke_with_thread_pool` **(Tests & Verification)** (Impact: 3.7)
...
**Top Internal Functions/Classes:**
  * `test_exit_code_and_output_from_sys_exit` **(Annotated Framework Methods)** (Impact: 4.4)
    * *Intent:* # See issue #362 @click.command() def cli_string(): click.echo("hello world") sys.exit("error") @cli...
  * `test_runner_with_stream` **(Tests & Verification)** (Impact: 4.0)
  * `test_runner` **(Tests & Verification)** (Impact: 3.8)
  * `test_echo_stdin_stream` **(Tests & Verification)** (Impact: 3.8)
...
**Top Internal Functions/Classes:**
  ...
  * `format_message` **(Compute Cores)** (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_files_closed` **(Annotated Framework Methods)** (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `_get_windows_console_stream` **(Many-Argument Workhorses)** (Impact: 15.1)
  * `readinto` **(Compute Cores)** (Impact: 11.8)
  * `write` **(Compute Cores)** (Impact: 6.1)
  * `get_buffer` **(Defensive Guards)** (Impact: 5.7)
  * `_get_error_message` **(Generic / Templated Code)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 15 instances
...
**Top Internal Functions/Classes:**
  ...
  * `test_formatting_usage_error_metavar_bad_arg` **(Annotated Framework Methods)** (Impact: 4.9)
  * `test_wrapping_long_options_strings` **(Annotated Framework Methods)** (Impact: 4.5)
  * `test_wrapping_long_command_name` **(Annotated Framework Methods)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
