...
> **File Composition:** Large Core Modules 27%, Data / Markup / Trivial 20%, Declarative / Non-Code 16%, Interface Declarations Files 13%, I/O & Config Routines Files 10%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 67.4 | 93.1 | 100.0 |
...
| connectivity | 9865 | 884 | 10 | `engine/src/funcs.h` |
...
- `MCProperty::parse` **(Compute Cores)** (@ `engine/src/property.cpp`) -> Impact: **759.0** | LOC: 665
...
- **Heaviest Functions:** `deployDo` (Many-Argument Workhorses, Impact: 160.3), `deployAutoconfigureSDKs` (Compute Cores, Impact: 25.4), `deployGetIphoneOSes` (Compute Cores, Impact: 21.8)
...
- **Heaviest Functions:** `requestSend` (Defensive Guards, Impact: 13.2), `requestCallbackLoad` (Compute Cores, Impact: 10.1), `requestCreate` (Many-Argument Workhorses, Impact: 7.9)
...
- **Heaviest Functions:** `runGetBenchmarkFileNames_Recursive` (Many-Argument Workhorses, Impact: 31.1), `runSingleCommand` (Many-Argument Workhorses, Impact: 18.1), `runGetBenchmarkCommandNames` (Compute Cores, Impact: 17.1)
...
- **Heaviest Functions:** `builderFetchEngine` (Compute Cores, Impact: 55.1), `dietAndStrip` (Compute Cores, Impact: 46.6), `builderBuild` (Many-Argument Workhorses, Impact: 43.2)

### 5. `revmobile/src/reviphoneproxy.mm` (OBJECTIVE-C) -> Cumulative Risk: **742.72**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +1.73)
...
- **Heaviest Functions:** `main` (Many-Argument Workhorses, Impact: 46.5), `getSimDeviceSet` (I/O & Config Routines, Impact: 12.8), `isValid` (I/O & Config Routines, Impact: 5.5)
...
- **Heaviest Functions:** `run` (I/O & Config Routines, Impact: 22.1), `setHeaders` (Compute Cores, Impact: 10.8), `URLLoader` (Compute Cores, Impact: 8.3)
...
- **Heaviest Functions:** `downloadNewVersion` (Many-Argument Workhorses, Impact: 32.5), `launchDownloadedInstaller` (Compute Cores, Impact: 29.6), `downloadLatestRevisionComplete` (Compute Cores, Impact: 11.1)
...
- **Heaviest Functions:** `OAuth2` (Many-Argument Workhorses, Impact: 68.8), `__HandleRequest` (Compute Cores, Impact: 19.8), `__RemoveDialog` (Defensive Guards, Impact: 9.2)
...
- **Heaviest Functions:** `handleRequestPayment` (Many-Argument Workhorses, Impact: 22.6), `mapResponseCode` (Compute Cores, Impact: 18.3), `OnSucceedGetInboxList` (Compute Cores, Impact: 13.4)
...
- **Heaviest Functions:** `fetchConflictingMac` (Compute Cores, Impact: 14.0), `fetchConflictingApps` (I/O & Config Routines, Impact: 5.8), `fetchConflictingLinux` (Interface Declarations, Impact: 4.6)
...
**Top Internal Functions/Classes:**
  * `yyparse` **(Compute Cores)** (Impact: 370.6)
  * `yysymprint` **(Many-Argument Workhorses)** (Impact: 13.5)
    * *Intent:* `--------------------------------*/
  * `yystpcpy` **(Compute Cores)** (Impact: 9.5)
    * *Intent:* # endif # endif # ifndef yystpcpy # if defined (__GLIBC__) && defined (_STRING_H) && defined (_GNU_S...
  * `yydestruct` **(Compute Cores)** (Impact: 7.9)
    * *Intent:* `-----------------------------------------------*/
  * `yystrlen` **(Compute Cores)** (Impact: 7.8)
...
**Top Internal Functions/Classes:**
  * `ulGetFormat` **(Compute Cores)** (Impact: 119.9)
...
**Top Internal Functions/Classes:**
  ...
  * `MCObject::load` **(Compute Cores)** (Impact: 163.2)
...
**Top Internal Functions/Classes:**
  * `MCButton::mup` **(Compute Cores)** (Impact: 215.6)
  * `MCButton::mfocus` **(Compute Cores)** (Impact: 185.3)
  * `MCButton::kdown` **(Compute Cores)** (Impact: 152.8)
  * `MCButton::save` **(Many-Argument Workhorses)** (Impact: 136.5)
  * `MCButton::load` **(Compute Cores)** (Impact: 132.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `MCU_fix_path` **(Compute Cores)** (Impact: 64.5)
...
**Top Internal Functions/Classes:**
  ...
  * `MCCard::relayer` **(Compute Cores)** (Impact: 94.7)
  * `MCCard::mdown` **(Compute Cores)** (Impact: 93.1)
  * `MCCard::getchildbyordinal` **(Compute Cores)** (Impact: 70.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `MCGroup::load` **(Compute Cores)** (Impact: 138.3)
...
**Top Internal Functions/Classes:**
  * `MCSocket::readsome` **(Compute Cores)** (Impact: 327.8)
    * *Intent:* #endif
...
- `libfoundation/include/foundation-auto.h` -> **Severity: 0.019** (Bridge: 0.0003 * Flux: 64.2653%)
