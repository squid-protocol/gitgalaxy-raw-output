...
| Avg Path Length | 3.2674 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.59; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 35%, Declarative / Non-Code 20%, Large Core Modules 16%, Interface Declarations Files 8%, Defensive Guards Files 7%
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 12.0 | 0.7 | 0.0 |
...
| connectivity | 13659 | 821 | 19 | `src/server.h` |
...
- `parseOptions` **(Compute Cores)** (@ `src/redis-cli.c`) -> Impact: **583.8** | LOC: 418
  * *Intent:* /*------------------------------------------------------------------------------ * User interface *---------------------------------------------------...
- `parser_tokenize` **(Compute Cores)** (@ `deps/jemalloc/test/unit/stats_print.c`) -> Impact: **562.4** | LOC: 557
- `debugCommand` **(Compute Cores)** (@ `src/debug.c`) -> Impact: **492.6** | LOC: 772
  * *Intent:* #endif
...
- **Heaviest Functions:** `RM_Call` (Many-Argument Workhorses, Impact: 208.4), `moduleCreateArgvFromUserFormat` (Many-Argument Workhorses, Impact: 134.2), `RM_SetCommandInfo` (Many-Argument Workhorses, Impact: 72.7)
...
- **Heaviest Functions:** `scanGenericCommand` (Many-Argument Workhorses, Impact: 185.1), `getKeysUsingKeySpecs` (Many-Argument Workhorses, Impact: 103.7), `dbSetValue` (Many-Argument Workhorses, Impact: 100.2)
...
- **Heaviest Functions:** `parseExtendedStringArgumentsOrReply` (Many-Argument Workhorses, Impact: 281.5), `setGenericCommand` (Many-Argument Workhorses, Impact: 184.4), `lcsCommand` (Compute Cores, Impact: 105.4)
...
- **Heaviest Functions:** `bioProcessBackgroundJobs` (Compute Cores, Impact: 48.1), `bioCreateCompRq` (Many-Argument Workhorses, Impact: 14.5), `bioInit` (I/O & Config Routines, Impact: 12.7)
...
- **Heaviest Functions:** `genRedisInfoString` (Many-Argument Workhorses, Impact: 324.9), `processCommand` (Compute Cores, Impact: 212.8), `main` (Many-Argument Workhorses, Impact: 187.1)
...
- **Heaviest Functions:** `memoryCommand` (Compute Cores, Impact: 58.6), `getMemoryDoctorReport` (Compute Cores, Impact: 42.5), `objectCommand` (Compute Cores, Impact: 33.7)
...
- **Heaviest Functions:** `getNodeByQuery` (Many-Argument Workhorses, Impact: 268.8), `migrateCommand` (Compute Cores, Impact: 123.0), `clusterCommand` (Compute Cores, Impact: 86.7)
...
- **Heaviest Functions:** `listInsertNode` (Many-Argument Workhorses, Impact: 19.3), `listDup` (Compute Cores, Impact: 13.1), `listIndex` (Compute Cores, Impact: 12.8)
...
- **Heaviest Functions:** `streamReplyWithRange` (Many-Argument Workhorses, Impact: 214.9), `streamParseAddOrTrimArgsOrReply` (Many-Argument Workhorses, Impact: 171.4), `xreadCommand` (Compute Cores, Impact: 158.4)
...
- **Heaviest Functions:** `unblockClient` (Compute Cores, Impact: 36.2), `blockForKeys` (Many-Argument Workhorses, Impact: 29.1), `signalKeyAsReadyLogic` (Many-Argument Workhorses, Impact: 24.8)
...
**Top Internal Functions/Classes:**
  * `parseOptions` **(Compute Cores)** (Impact: 583.8)
    * *Intent:* /*------------------------------------------------------------------------------ * User interface *-...
  ...
  * `clusterManagerFixOpenSlot` **(Compute Cores)** (Impact: 141.0)
...
**Top Internal Functions/Classes:**
  ...
  * `clusterLoadConfig` **(Compute Cores)** (Impact: 162.2)
...
**Top Internal Functions/Classes:**
  ...
  * `xreadCommand` **(Compute Cores)** (Impact: 158.4)
...
**Top Internal Functions/Classes:**
  ...
  * `sentinelRefreshInstanceInfo` **(Compute Cores)** (Impact: 158.3)
...
**Top Internal Functions/Classes:**
  * `readSyncBulkPayload` **(Compute Cores)** (Impact: 132.6)
...
**Top Internal Functions/Classes:**
  * `parseOptions` **(Compute Cores)** (Impact: 257.4)
  ...
  * `fetchClusterConfiguration` **(Compute Cores)** (Impact: 58.2)
...
**Top Internal Functions/Classes:**
  ...
  * `rewriteObject` **(Many-Argument Workhorses)** (Impact: 75.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.6e-05 | `Ripple Effect (Closeness):` 0.016962
...
- `deps/jemalloc/include/jemalloc/internal/mutex.h` -> **Severity: 0.052** (Bridge: 0.0005 * Flux: 99.3568%)
...
- `deps/jemalloc/test/include/test/math.h` -> **Severity: 8.977** (Embedded: 0.0901 * Error Risk: 99.6599%)
- `deps/jemalloc/include/msvc_compat/C99/stdint.h` -> **Severity: 5.708** (Embedded: 0.1021 * Error Risk: 55.9026%)
- `deps/jemalloc/include/jemalloc/internal/jemalloc_internal_types.h` -> **Severity: 4.667** (Embedded: 0.0703 * Error Risk: 66.3831%)
- `deps/jemalloc/include/jemalloc/internal/util.h` -> **Severity: 4.548** (Embedded: 0.0884 * Error Risk: 51.4702%)
- `deps/jemalloc/test/include/test/jemalloc_test.h.in` -> **Severity: 4.41** (Embedded: 0.0802 * Error Risk: 54.9834%)
