...
> **Composition Archetype:** `Flat Modular Platform` (z -0.34; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 30%, Large Core Modules 14%, Interface Declarations Files 13%, Declarative / Non-Code 11%, Tests & Verification Files 11%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 97.3 | 3.1 | 0.0 | 0.0 |
...
| connectivity | 11548 | 1725 | 5 | `actionpack/test/dispatch/routing_test.rb` |
...
| threat | 1158 | 507 | 1 | `activestorage/app/assets/javascripts/activestorage.js` |
...
- `indexes` **(Compute Cores)** (@ `activerecord/lib/active_record/connection_adapters/postgresql/schema_statements.rb`) -> Impact: **191.1** | LOC: 1248
...
- `test_remove_column_with_if_not_exists_not_set` **(Compute Cores)** (@ `activerecord/test/cases/migration_test.rb`) -> Impact: **148.6** | LOC: 1813
...
- `load_defaults` **(Compute Cores)** (@ `railties/lib/rails/application/configuration.rb`) -> Impact: **100.4** | LOC: 310
...
- **Heaviest Functions:** `resolve_payload` (Compute Cores, Impact: 25.4), `notify` (Many-Argument Workhorses, Impact: 19.5), `resolve_tags` (Callbacks & Closures, Impact: 11.2)
...
- **Heaviest Functions:** `send_stream` (Many-Argument Workhorses, Impact: 12.0), `process` (Callbacks & Closures, Impact: 9.4), `make_response!` (C Struct Operations, Impact: 6.1)
...
- **Heaviest Functions:** `load_defaults` (Compute Cores, Impact: 100.4), `autoload_lib_once` (Defensive Guards, Impact: 7.6), `content_security_policy` (Compute Cores, Impact: 7.4)

### 4. `activemodel/lib/active_model/secure_password.rb` (RUBY) -> Cumulative Risk: **735.89**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.53)
- **Magnitude:** 81.18 | **LOC:** 324 | **CtrlFlow:** 20.2% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9209%), Tech Debt (97.57%), Safety Score (94.4165%)
- **Heaviest Functions:** `has_secure_password` (Many-Argument Workhorses, Impact: 36.9), `initialize` (Many-Argument Workhorses, Impact: 8.5), `register_algorithm` (Compute Cores, Impact: 3.6)
...
- **Heaviest Functions:** `fresh_when` (Many-Argument Workhorses, Impact: 34.0), `http_cache_forever` (Parameter Forwarders, Impact: 3.8), `expires_in` (Callbacks & Closures, Impact: 2.5)
...
- **Heaviest Functions:** `clean_frame` (Callbacks & Closures, Impact: 12.9), `clean` (Compute Cores, Impact: 9.3), `first_clean_frame` (Callbacks & Closures, Impact: 4.9)
...
- **Heaviest Functions:** `define_attribute_method_pattern` (Many-Argument Workhorses, Impact: 13.7), `define_attribute_accessor_method` (C Struct Operations, Impact: 10.7), `define_call` (Many-Argument Workhorses, Impact: 9.8)

### 8. `actionpack/lib/action_dispatch/middleware/debug_locks.rb` (RUBY) -> Cumulative Risk: **720.99**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.53)
...
- **Heaviest Functions:** `render_details` (Compute Cores, Impact: 22.7), `blocked_by?` (Compute Cores, Impact: 21.0), `call` (I/O & Config Routines, Impact: 4.8)
...
- **Heaviest Functions:** `watching?` (Callbacks & Closures, Impact: 10.9), `initialize` (Callbacks & Closures, Impact: 10.0), `directories_to_watch` (Compute Cores, Impact: 5.2)
...
- **Heaviest Functions:** `connection_gid` (Callbacks & Closures, Impact: 4.7), `tests` (Callbacks & Closures, Impact: 4.7), `determine_default_channel` (Interface Declarations, Impact: 4.6)
...
**Top Internal Functions/Classes:**
  ...
  * `assert_distance_of_time_in_words` **(Tests & Verification)** (Impact: 6.0)
...
**Top Internal Functions/Classes:**
  * `test_shallow_nested_resources` **(Tests & Verification)** (Impact: 6.5)
  * `test_colon_containing_custom_param` **(I/O & Config Routines)** (Impact: 4.4)
  * `test_custom_resource_routes_are_scoped` **(Tests & Verification)** (Impact: 3.6)
  * `test_namespace_containing_numbers` **(I/O & Config Routines)** (Impact: 3.5)
  * `test_namespaced_shallow_routes_with_shallow_prefix_option` **(Tests & Verification)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_label_with_symbols` **(Compute Cores)** (Impact: 4.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_build_and_create_from_association_should_respect_passed_attributes_over_default_scope` **(Tests & Verification)** (Impact: 2.7)
  * `test_create_with_bang_on_has_many_when_parent_is_new_raises` **(I/O & Config Routines)** (Impact: 2.5)
  * `test_create` **(Tests & Verification)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `indexes` **(Compute Cores)** (Impact: 191.1)
    * *Intent:* # Returns an array of indexes for the given table.
...
**Top Internal Functions/Classes:**
  * `test_blank_like_arguments_to_query_methods_dont_raise_errors` **(Tests & Verification)** (Impact: 6.8)
  * `test_count_with_distinct` **(Tests & Verification)** (Impact: 4.5)
  * `test_unscope_with_arel_sql` **(Tests & Verification)** (Impact: 4.5)
  * `test_where_with_take_memoization` **(Tests & Verification)** (Impact: 3.6)
...
**Top Internal Functions/Classes:**
  * `test_autosave_has_one_association_callbacks_get_called_once` **(Tests & Verification)** (Impact: 5.2)
  ...
  * `test_indexed_errors_should_be_properly_translated` **(Tests & Verification)** (Impact: 4.3)
  * `test_autosave_collection_association_callbacks_get_called_once` **(I/O & Config Routines)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `whole_form` **(Compute Cores)** (Impact: 5.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_eager_with_has_and_belongs_to_many_and_limit` **(Tests & Verification)** (Impact: 3.5)
  * `test_association_loading_notification` **(Tests & Verification)** (Impact: 3.5)
  * `test_eager_with_default_scope_as_class_method_using_find_method` **(Tests & Verification)** (Impact: 3.4)
  * `test_eager_count_performed_on_a_has_many_association_with_multi_table_conditional` **(Tests & Verification)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_remove_column_with_if_not_exists_not_set` **(Compute Cores)** (Impact: 148.6)
  ...
  * `test_internal_metadata_create_table_wont_be_affected_by_schema_cache` **(Tests & Verification)** (Impact: 5.3)
...
**Top Internal Functions/Classes:**
  * `test_belongs_to_with_primary_key_counter` **(Tests & Verification)** (Impact: 4.7)
  * `test_optional_relation_can_be_set_per_model` **(I/O & Config Routines)** (Impact: 4.3)
  * `test_eager_loading_with_primary_key` **(Defensive Guards)** (Impact: 3.3)
  * `__global_context__` **(I/O & Config Routines)** (Impact: 3.2)
  * `test_optional_relation` **(Tests & Verification)** (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Rce:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `test_bind_variables` **(Tests & Verification)** (Impact: 8.6)
  * `test_find_by_records` **(Tests & Verification)** (Impact: 5.2)
  * `test_count_by_sql` **(Tests & Verification)** (Impact: 4.3)
...
**Top Internal Functions/Classes:**
  ...
  * `run_callbacks` **(Compute Cores)** (Impact: 28.4)
...
**Top Internal Functions/Classes:**
  * `test_setting_cookie_with_secure_when_always_write_cookie_is_true` **(Tests & Verification)** (Impact: 2.4)
  ...
  * `test_setting_cookie_with_secure` **(Tests & Verification)** (Impact: 2.3)
  * `test_legacy_hmac_aes_cbc_json_mode_falls_back_to_authenticated_encrypted_cookie` **(Tests & Verification)** (Impact: 2.3)
...
**Top Internal Functions/Classes:**
  * `derive_fk_query_constraints` **(Compute Cores)** (Impact: 27.4)
  * `initialize` **(Many-Argument Workhorses)** (Impact: 17.0)
  * `reflection_class_for` **(Compute Cores)** (Impact: 10.6)
...
- `activerecord/lib/active_record/connection_adapters/abstract/connection_pool.rb` -> Churn: **68.38%** | Cog Load: 51.5412% | Debt: 11.562%
