...
> **File Composition:** Large Core Modules 30%, Data / Markup / Trivial 22%, Annotated Framework Methods Files 13%, State Mutators Files 10%, Declarative / Non-Code 7%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 36.4 | 25.6 | 0.0 |
...
| connectivity | 10485 | 1494 | 13 | `ruff-0.15.9/crates/ruff_python_ast/src/nodes.rs` |
...
| dead_code | 5472 | 1171 | 5 | `ruff-0.15.9/crates/ruff_formatter/src/builders.rs` |
...
- **Heaviest Functions:** `extract_noqa_line_for` (Many-Argument Workhorses, Impact: 45.1), `extract_isort_directives` (Compute Cores, Impact: 32.0), `from_comment` (Defensive Guards, Impact: 14.1)
...
- **Heaviest Functions:** `fmt` (Compute Cores, Impact: 14.5), `next` (Type Conversions, Impact: 6.6), `set` (Annotated Framework Methods, Impact: 6.4)
...
- **Composition Archetype:** `Interface Declarations Files` (z -0.02)
...
- **Heaviest Functions:** `e[T]` (Interface Declarations, Impact: 1.5), `g[T]` (Interface Declarations, Impact: 1.5), `h[T]` (Interface Declarations, Impact: 1.5)
...
- **Composition Archetype:** `Tests & Verification Files` (z +0.66)
...
- **Heaviest Functions:** `if_` (Compute Cores, Impact: 12.3), `too_many_return_statements` (Many-Argument Workhorses, Impact: 5.3), `nested_def_ignored` (Tests & Verification, Impact: 4.8)

### 5. `ruff-0.15.9/crates/ruff_db/src/system/path.rs` (RUST) -> Cumulative Risk: **595.47**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +1.01)
...
- **Heaviest Functions:** `absolute` (Generic / Templated Code, Impact: 10.5), `absolute` (Compute Cores, Impact: 10.3), `next` (Compute Cores, Impact: 7.7)

### 6. `ruff-0.15.9/crates/ruff_linter/src/rules/pylint/rules/too_many_statements.rs` (RUST) -> Cumulative Risk: **593.43**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z -0.33)
...
- **Heaviest Functions:** `num_statements` (Compute Cores, Impact: 16.3), `many_statements` (Tests & Verification, Impact: 8.4), `if_else_if_corner` (Tests & Verification, Impact: 5.7)
...
- **Heaviest Functions:** `write_char` (Many-Argument Workhorses, Impact: 15.7), `output_layout_with_checker` (Many-Argument Workhorses, Impact: 13.7), `output_layout_with_checker` (Many-Argument Workhorses, Impact: 12.4)
...
- **Heaviest Functions:** `check_line` (Many-Argument Workhorses, Impact: 164.6), `next` (Compute Cores, Impact: 37.2), `check_lines` (Many-Argument Workhorses, Impact: 32.7)
...
- **Heaviest Functions:** `block_comments` (Compute Cores, Impact: 38.9), `has_comments` (Generic / Templated Code, Impact: 10.8), `intersects` (State Mutators, Impact: 9.3)
...
- **Heaviest Functions:** `emit` (Many-Argument Workhorses, Impact: 18.0), `fmt` (Defensive Guards, Impact: 15.6), `fmt` (Many-Argument Workhorses, Impact: 12.7)
...
**Top Internal Functions/Classes:**
  ...
  * `lex_string` **(Compute Cores)** (Impact: 47.6)
    * *Intent:* /// Lex a string literal.
  * `test_fstring_escape` **(Tests & Verification)** (Impact: 44.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `undefined_in_gen_exp_nested` **(Tests & Verification)** (Impact: 9.7)
  * `f401_preview_refined_submodule_handling` **(Many-Argument Workhorses)** (Impact: 8.2)
  * `defined_in_gen_exp` **(Tests & Verification)** (Impact: 7.4)
...
**Top Internal Functions/Classes:**
  ...
  * `resolve_qualified_name` **(Compute Cores)** (Impact: 47.3)
...
**Top Internal Functions/Classes:**
  ...
  * `quote` **(Tests & Verification)** (Impact: 25.0)
...
**Top Internal Functions/Classes:**
  ...
  * `write_node` **(Compute Cores)** (Impact: 27.9)
    * *Intent:* # ------------------------------------------------------------------------------ # Node structs
  * `fields_in_source_order` **(Generic / Templated Code)** (Impact: 10.7)
...
**Top Internal Functions/Classes:**
  * `replace_unprintable` **(Compute Cores)** (Impact: 25.8)
...
**Top Internal Functions/Classes:**
  ...
  * `triple_quoted` **(Compute Cores)** (Impact: 25.3)
