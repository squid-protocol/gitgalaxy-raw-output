...
> **Composition Archetype:** `Small Flat Repo` (z +0.01; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 73%, Data / Markup / Trivial 14%, Compute Cores Files 3%, I/O & Config Routines Files 2%, Interface Declarations Files 2%
...
- **Heaviest Functions:** `resolve_deps` (Compute Cores, Impact: 54.3), `download_recipe` (Compute Cores, Impact: 28.4), `install_perl_module` (Compute Cores, Impact: 20.7)

### 2. `share/rex-tab-completion.bash` (SHELL) -> Cumulative Risk: **508.42**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +2.64)
...
- **Heaviest Functions:** `_rex` (Compute Cores, Impact: 20.7), `_rex_fix_colon_reply` (I/O & Config Routines, Impact: 2.4), `__global_context__` (I/O & Config Routines, Impact: 1.5)

### 3. `t/summary.t` (PERL) -> Cumulative Risk: **483.13**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +2.41)
...
- **Heaviest Functions:** `test_summary` (Compute Cores, Impact: 8.7), `create_tasks` (Callbacks & Closures, Impact: 6.2)
...
- **Heaviest Functions:** `_setup_test` (Compute Cores, Impact: 17.6)
...
- **Heaviest Functions:** `__global_context__` (I/O & Config Routines, Impact: 10.7), `Anonymous_Block` (I/O & Config Routines, Impact: 4.5), `_rex_hosts` (I/O & Config Routines, Impact: 3.2)
...
- **Heaviest Functions:** `expected_params` (Compute Cores, Impact: 8.2), `before_hook` (Parameter Forwarders, Impact: 2.6), `after_change_hook` (Parameter Forwarders, Impact: 2.5)
...
- **Heaviest Functions:** `Rex::Helper::Run::i_run` (Defensive Guards, Impact: 5.0), `Rex::Commands::File::file` (Parameter Forwarders, Impact: 3.9), `Rex::Commands::Fs::unlink` (Interface Declarations, Impact: 1.6)
...
- **Heaviest Functions:** `test_rsync` (Many-Argument Workhorses, Impact: 17.2)

### 9. `t/write_utf8_files.t` (PERL) -> Cumulative Risk: **383.43**
- **Archetype:** `file_cluster_3` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +2.40)
...
- **Heaviest Functions:** `get_encoding` (Defensive Guards, Impact: 6.5), `write_file` (Interface Declarations, Impact: 2.0)

### 10. `t/hooks_in_rexfile_tasks_in_pkg.t` (PERL) -> Cumulative Risk: **381.23**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +2.08)
...
**Top Internal Functions/Classes:**
  ...
  * `_rex_hosts` **(I/O & Config Routines)** (Impact: 3.2)
    * *Intent:* # return hosts managed by rex or any other host availabe via zsh's _hosts function
  * `_hostgroups` **(I/O & Config Routines)** (Impact: 2.2)
  * `_envs` **(I/O & Config Routines)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `git_repo_ok` **(Tests & Verification)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
