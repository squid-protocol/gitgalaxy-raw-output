...
> **Composition Archetype:** `Flat Modular Platform` (z +1.26; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 33%, Interface Declarations Files 28%, Declarative / Non-Code 10%, Large Core Modules 8%, Parameter Forwarders Files 8%
...
- `analyse_c_function_call` **(Compute Cores)** (@ `Cython/Compiler/ExprNodes.py`) -> Impact: **159.3** | LOC: 207
...
- **Heaviest Functions:** `analyse_c_function_call` (Compute Cores, Impact: 159.3), `generate_assignment_code` (Many-Argument Workhorses, Impact: 132.9), `coerce_to` (Many-Argument Workhorses, Impact: 118.9)
...
- **Heaviest Functions:** `generate_function_definitions` (Many-Argument Workhorses, Impact: 332.7), `analyse` (Many-Argument Workhorses, Impact: 231.7), `generate_tuple_and_keyword_parsing_code` (Many-Argument Workhorses, Impact: 172.6)
...
- **Heaviest Functions:** `best_match` (Many-Argument Workhorses, Impact: 137.0), `declaration_code` (Many-Argument Workhorses, Impact: 76.5), `write_noexcept_performance_hint` (Many-Argument Workhorses, Impact: 52.0)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 13.9), `generate_assignment_code` (Many-Argument Workhorses, Impact: 11.7), `setup_temp_expr` (Compute Cores, Impact: 11.3)
...
- **Heaviest Functions:** `declare_c_class` (Many-Argument Workhorses, Impact: 257.4), `declare_cfunction` (Many-Argument Workhorses, Impact: 225.2), `declare_var` (Many-Argument Workhorses, Impact: 130.1)
...
- **Heaviest Functions:** `runtests` (Many-Argument Workhorses, Impact: 229.1), `compile` (Many-Argument Workhorses, Impact: 131.2), `build_tests` (Many-Argument Workhorses, Impact: 99.0)

### 7. `Cython/Compiler/ModuleNode.py` (PYTHON) -> Cumulative Risk: **730.89**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.38)
- **Magnitude:** 4499.26 | **LOC:** 4346 | **CtrlFlow:** 29.9% | **Authorship Centralization:** 43.5%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (95.3333%), Safety Score (93.7608%)
- **Heaviest Functions:** `generate_dealloc_function` (Many-Argument Workhorses, Impact: 136.9), `generate_richcmp_function` (Many-Argument Workhorses, Impact: 129.2), `generate_new_function` (Many-Argument Workhorses, Impact: 126.6)
...
- **Heaviest Functions:** `p_statement` (Many-Argument Workhorses, Impact: 133.9), `p_c_simple_declarator` (Many-Argument Workhorses, Impact: 105.8), `p_c_class_definition` (Compute Cores, Impact: 79.7)

### 9. `Demos/benchmarks/bm_cython.pyx` (PYTHON) -> Cumulative Risk: **716.6**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.38)
...
- **Heaviest Functions:** `_slice_memoryview` (Many-Argument Workhorses, Impact: 9.8), `_slice_memoryview_py` (Many-Argument Workhorses, Impact: 9.4), `run_benchmark` (Compute Cores, Impact: 7.6)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 19.6), `report_error` (Defensive Guards, Impact: 15.2), `warning` (Compute Cores, Impact: 12.7)
...
**Top Internal Functions/Classes:**
  * `analyse_c_function_call` **(Compute Cores)** (Impact: 159.3)
...
**Top Internal Functions/Classes:**
  ...
  * `p_c_class_definition` **(Compute Cores)** (Impact: 79.7)
...
**Top Internal Functions/Classes:**
  ...
  * `flatten_parallel_assignments` **(Compute Cores)** (Impact: 50.4)
...
**Top Internal Functions/Classes:**
  ...
  * `handle_directory` **(Compute Cores)** (Impact: 94.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_regen_test_file` **(Compute Cores)** (Impact: 10.9)
  * `build_class_name` **(Compute Cores)** (Impact: 9.9)
  * `_print_setitem_Seq` **(Generic / Templated Code)** (Impact: 2.1)
  * `_print_setitem_Map` **(Generic / Templated Code)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `proxyval` **(Compute Cores)** (Impact: 48.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_comp_2` **(Interface Declarations)** (Impact: 12.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `types_coroutine` **(Annotated Framework Methods)** (Impact: 5.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `parse_expr` **(Compute Cores)** (Impact: 62.6)
...
**Top Internal Functions/Classes:**
  * `_break_funcname` **(Compute Cores)** (Impact: 42.2)
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 1451.24 | **LOC:** 2474 | **CtrlFlow:** 53.3% | **Authorship Centralization:** 55.6%
- **Risk Profile:** Cognitive Load (70.8407%), Tech Debt (8.1366%)
**Top Internal Functions/Classes:**
  ...
  * `__Pyx_dict_iterator` **(Many-Argument Workhorses)** (Impact: 48.8)
...
**Top Internal Functions/Classes:**
  * `__Pyx_PyGen__FetchStopIterationValue` **(Compute Cores)** (Impact: 62.4)
...
**Top Internal Functions/Classes:**
  * `__cinit__` **(Many-Argument Workhorses)** (Impact: 42.2)
...
**Top Internal Functions/Classes:**
  ...
  * `__pyx_FusedFunction_getitem` **(Compute Cores)** (Impact: 38.1)
...
- `Cython/Compiler/ModuleNode.py` -> Churn: **85.03%** | Cog Load: 76.195% | Debt: 10.8386%
