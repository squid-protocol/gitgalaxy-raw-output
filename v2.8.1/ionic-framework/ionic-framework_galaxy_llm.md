...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.86; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 35%, Data / Markup / Trivial 34%, Callbacks & Closures Files 9%, State Mutators Files 6%, Large Core Modules 4%
...
- `setup` **(Compute Cores)** (@ `packages/vue/src/components/IonRouterOutlet.ts`) -> Impact: **89.5** | LOC: 510
...
- **Composition Archetype:** `Interface Declarations Files` (z +1.46)
...
- **Heaviest Functions:** `useOverlay` (Many-Argument Workhorses, Impact: 43.1), `handleDismiss` (State Mutators, Impact: 3.3), `[willPresentEventName]` (Interface Declarations, Impact: 3.1)
...
- **Heaviest Functions:** `present` (Compute Cores, Impact: 12.7), `createControllerComponent` (Generic / Templated Code, Impact: 11.2), `componentDidUpdate` (Compute Cores, Impact: 8.9)
...
- **Heaviest Functions:** `componentDidUpdate` (Compute Cores, Impact: 10.9), `createOverlayComponent` (Generic / Templated Code, Impact: 9.2), `present` (Interface Declarations, Impact: 8.2)
...
- **Heaviest Functions:** `setActivatedElement` (Compute Cores, Impact: 17.1), `removeActivated` (Compute Cores, Impact: 10.5), `getActivatableTarget` (Compute Cores, Impact: 9.6)

### 5. `core/src/utils/input-shims/hacks/input-blurring.ts` (TYPESCRIPT) -> Cumulative Risk: **675.85**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -1.05)
...
- **Heaviest Functions:** `onTouchend` (Callbacks & Closures, Impact: 13.5), `enableInputBlurring` (Callbacks & Closures, Impact: 1.3), `onScroll` (Callbacks & Closures, Impact: 1.2)

### 6. `core/src/components/header/header.tsx` (TYPESCRIPT) -> Cumulative Risk: **673.43**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.34)
- **Magnitude:** 103.02 | **LOC:** 238 | **CtrlFlow:** 16.1% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Safety Score (82.9845%)
- **Heaviest Functions:** `setupCondenseHeader` (Compute Cores, Impact: 17.0), `checkCollapsibleHeader` (I/O & Config Routines, Impact: 9.9), `destroyCollapsibleHeader` (I/O & Config Routines, Impact: 5.8)

### 7. `core/src/utils/framework-delegate.ts` (TYPESCRIPT) -> Cumulative Risk: **664.27**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.58)
...
- **Heaviest Functions:** `attachComponent` (Many-Argument Workhorses, Impact: 36.0), `attachViewToDom` (Many-Argument Workhorses, Impact: 34.4), `detachComponent` (Callbacks & Closures, Impact: 5.8)

### 8. `packages/vue/src/utils/overlays.ts` (TYPESCRIPT) -> Cumulative Risk: **660.99**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `present` (Compute Cores, Impact: 19.3), `createInlineComponent` (Callbacks & Closures, Impact: 12.5), `renderChildren` (I/O & Config Routines, Impact: 9.3)

### 9. `packages/react/src/components/createOverlayComponent.tsx` (TYPESCRIPT) -> Cumulative Risk: **655.61**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
...
- **Heaviest Functions:** `createOverlayComponent` (Generic / Templated Code, Impact: 11.2), `componentDidUpdate` (Compute Cores, Impact: 10.9), `present` (Interface Declarations, Impact: 8.3)

### 10. `packages/react-router/src/ReactRouter/StackManager.tsx` (TYPESCRIPT) -> Cumulative Risk: **649.24**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `transitionPage` (Many-Argument Workhorses, Impact: 67.5), `handlePageTransition` (Compute Cores, Impact: 53.6), `runCommit` (Compute Cores, Impact: 45.1)
...
**Top Internal Functions/Classes:**
  ...
  * `renderDatetime` **(Compute Cores)** (Impact: 20.6)
...
**Top Internal Functions/Classes:**
  ...
  * `initParentRemovalObserver` **(Compute Cores)** (Impact: 20.1)
...
**Top Internal Functions/Classes:**
  ...
  * `verifyInputValues` **(Tests & Verification)** (Impact: 4.0)
...
- **Risk Profile:** Cognitive Load (46.7225%), Tech Debt (27.6914%)
**Top Internal Functions/Classes:**
  * `setupiOSNativeRefresher` **(Compute Cores)** (Impact: 48.9)
...
**Top Internal Functions/Classes:**
  * `knobStyle` **(Compute Cores)** (Impact: 52.8)
  ...
  * `getKnobPosition` **(Many-Argument Workhorses)** (Impact: 16.5)
...
**Top Internal Functions/Classes:**
  ...
  * `createAnimation` **(Compute Cores)** (Impact: 11.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.3e-05 | `Ripple Effect (Closeness):` 0.015558
...
- `core/scripts/testing/scripts.js` -> **Severity: 9.67** (Embedded: 0.137 * Error Risk: 70.5637%)
- `core/src/components/router/utils/interface.ts` -> **Severity: 3.82** (Embedded: 0.0588 * Error Risk: 64.946%)
- `core/src/global/ionic-global.ts` -> **Severity: 3.632** (Embedded: 0.038 * Error Risk: 95.6516%)
- `core/src/utils/input-shims/hacks/common.ts` -> **Severity: 3.566** (Embedded: 0.0369 * Error Risk: 96.7705%)
- `core/src/utils/platform.ts` -> **Severity: 1.606** (Embedded: 0.0196 * Error Risk: 82.0129%)
