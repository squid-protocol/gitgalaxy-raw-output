...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.15; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 28%, Data / Markup / Trivial 22%, Defensive Guards Files 18%, Interface Declarations Files 9%, Generic / Templated Code Files 5%
...
- **Heaviest Functions:** `_cmpkey` (Many-Argument Workhorses, Impact: 42.7), `__init__` (Compute Cores, Impact: 34.3), `_parse_letter_version` (Compute Cores, Impact: 17.2)
...
- **Heaviest Functions:** `run_parallel` (Defensive Guards, Impact: 14.4), `setup` (Compute Cores, Impact: 11.6), `inner` (Callbacks & Closures, Impact: 11.1)

### 3. `pandas/core/frame.py` (PYTHON) -> Cumulative Risk: **693.21**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.32)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 153.0), `from_records` (Many-Argument Workhorses, Impact: 146.7), `set_index` (Many-Argument Workhorses, Impact: 99.4)
...
- **Heaviest Functions:** `get_indexer_non_unique` (Many-Argument Workhorses, Impact: 79.3), `get_indexer_non_unique` (Many-Argument Workhorses, Impact: 42.0), `_get_bool_indexer` (Many-Argument Workhorses, Impact: 25.9)

### 5. `asv_bench/benchmarks/indexing.py` (PYTHON) -> Cumulative Risk: **689.88**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.47)
...
- **Heaviest Functions:** `setup` (Defensive Guards, Impact: 16.9), `setup` (Compute Cores, Impact: 6.0), `setup` (Compute Cores, Impact: 5.8)
...
- **Heaviest Functions:** `to_offset` (Many-Argument Workhorses, Impact: 66.8), `_apply` (Many-Argument Workhorses, Impact: 46.4), `wrapper` (Compute Cores, Impact: 37.3)
...
- **Heaviest Functions:** `__eq__` (Defensive Guards, Impact: 9.1), `__eq__` (Defensive Guards, Impact: 7.2), `patch_pyarrow` (Interface Declarations, Impact: 4.7)

### 8. `asv_bench/benchmarks/series_methods.py` (PYTHON) -> Cumulative Risk: **686.74**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.01)
...
- **Heaviest Functions:** `setup` (Compute Cores, Impact: 13.2), `setup` (Many-Argument Workhorses, Impact: 12.0), `setup` (Compute Cores, Impact: 11.1)
...
- **Heaviest Functions:** `compute_dict_like` (Many-Argument Workhorses, Impact: 46.3), `reconstruct_func` (Many-Argument Workhorses, Impact: 39.6), `wrap_results_dict_like` (Many-Argument Workhorses, Impact: 38.7)

### 10. `asv_bench/benchmarks/algorithms.py` (PYTHON) -> Cumulative Risk: **685.53**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.72)
...
- **Heaviest Functions:** `setup` (Many-Argument Workhorses, Impact: 28.1), `setup` (Many-Argument Workhorses, Impact: 23.4), `setup` (Many-Argument Workhorses, Impact: 9.4)
...
**Top Internal Functions/Classes:**
  ...
  * `__getitem__` **(Compute Cores)** (Impact: 60.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_datetime_to_stata_elapsed_vec` **(Compute Cores)** (Impact: 47.3)
    * *Intent:* """ Convert from datetime to SIF. https://www.stata.com/help.cgi?datetime Parameters ---------- date...
  * `_read_old_header` **(Many-Argument Workhorses)** (Impact: 46.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_supports_reduction` **(Compute Cores)** (Impact: 40.1)
...
**Top Internal Functions/Classes:**
  ...
  * `array_equivalent_object` **(Compute Cores)** (Impact: 37.8)
...
**Top Internal Functions/Classes:**
  ...
  * `test_constructor_data_aware_dtype_naive` **(Many-Argument Workhorses)** (Impact: 19.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_loc_setitem_str_to_small_float_conversion_type` **(Compute Cores)** (Impact: 15.0)
...
**Top Internal Functions/Classes:**
  * `_maybe_coerce_merge_keys` **(Compute Cores)** (Impact: 118.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_validate_color_args` **(Compute Cores)** (Impact: 50.0)
  ...
  * `_adorn_subplots` **(Compute Cores)** (Impact: 37.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_indices_concatenation_order` **(Interface Declarations)** (Impact: 13.7)
