...
> **File Composition:** Data / Markup / Trivial 44%, Callbacks & Closures Files 23%, Declarative / Non-Code 10%, Generic / Templated Code Files 6%, I/O & Config Routines Files 5%
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 4.9 | 1.0 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 64.8 | 4.9 | 0.0 | 0.0 |
...
- `ProviderChildren` **(Compute Cores)** (@ `components/config-provider/index.tsx`) -> Impact: **93.5** | LOC: 400
...
### 1. `tests/shared/imageTest.tsx` (TYPESCRIPT) -> Cumulative Risk: **677.38**
...
- **Heaviest Functions:** `imageTest` (Many-Argument Workhorses, Impact: 59.5), `requestListener` (Compute Cores, Impact: 40.3), `imageDemoTest` (Callbacks & Closures, Impact: 25.2)

### 2. `scripts/print-changelog.ts` (TYPESCRIPT) -> Cumulative Risk: **629.41**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.60)
...
- **Heaviest Functions:** `printLog` (Compute Cores, Impact: 66.8), `validate` (Compute Cores, Impact: 25.3), `validate` (Compute Cores, Impact: 13.6)

### 3. `components/notification/__tests__/util.ts` (TYPESCRIPT) -> Cumulative Risk: **617.07**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +2.03)
- **Magnitude:** 49.8 | **LOC:** 31 | **CtrlFlow:** 25.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `triggerMotionEnd` (Defensive Guards, Impact: 8.2), `awaitPromise` (Interface Declarations, Impact: 2.2)

### 4. `components/message/__tests__/util.ts` (TYPESCRIPT) -> Cumulative Risk: **616.35**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +2.20)
- **Magnitude:** 45.16 | **LOC:** 25 | **CtrlFlow:** 11.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `triggerMotionEnd` (Callbacks & Closures, Impact: 3.6), `awaitPromise` (Interface Declarations, Impact: 2.2)

### 5. `scripts/pre-publish.ts` (TYPESCRIPT) -> Cumulative Risk: **611.62**
...
- **Heaviest Functions:** `showMessage` (Many-Argument Workhorses, Impact: 28.2), `runPrePublish` (I/O & Config Routines, Impact: 20.1), `downloadArtifact` (Many-Argument Workhorses, Impact: 7.9)

### 6. `.dumi/theme/utils/index.ts` (TYPESCRIPT) -> Cumulative Risk: **609.37**
...
- **Heaviest Functions:** `getMenuItems` (Many-Argument Workhorses, Impact: 60.5), `sortFn` (Defensive Guards, Impact: 44.9), `getLocalizedPathname` (Many-Argument Workhorses, Impact: 39.5)

### 7. `scripts/generate-cssinjs.ts` (TYPESCRIPT) -> Cumulative Risk: **606.85**
...
- **Heaviest Functions:** `useStyle` (Callbacks & Closures, Impact: 6.0), `useStyle` (Compute Cores, Impact: 5.5), `Demo` (Defensive Guards, Impact: 3.4)

### 8. `components/typography/hooks/useCopyClick.ts` (TYPESCRIPT) -> Cumulative Risk: **593.85**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.22)
...
- **Heaviest Functions:** `cleanCopyId` (Defensive Guards, Impact: 13.3), `useCopyClick` (Callbacks & Closures, Impact: 3.5)

### 9. `scripts/post-publish.ts` (TYPESCRIPT) -> Cumulative Risk: **592.74**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
...
- **Heaviest Functions:** `process` (Compute Cores, Impact: 31.8), `matchDeprecated` (Defensive Guards, Impact: 4.7)
...
- **Heaviest Functions:** `beforeUpload` (Callbacks & Closures, Impact: 2.5), `App` (Callbacks & Closures, Impact: 2.4), `onload` (Interface Declarations, Impact: 1.9)
...
**Top Internal Functions/Classes:**
  * `App` **(Tests & Verification)** (Impact: 17.8)
    * *Intent:* // base size
  * `App` **(Tests & Verification)** (Impact: 15.4)
    * *Intent:* // base size
  * `validator` **(Defensive Guards)** (Impact: 10.1)
  * `App` **(Tests & Verification)** (Impact: 8.8)
...
**Top Internal Functions/Classes:**
  * `filter` **(Tests & Verification)** (Impact: 32.5)
  ...
  * `onFilter` **(Tests & Verification)** (Impact: 20.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `onClick` **(Callbacks & Closures)** (Impact: 23.6)
...
**Top Internal Functions/Classes:**
  ...
  * `open` **(Interface Declarations)** (Impact: 4.7)
  * `configWarp` **(Callbacks & Closures)** (Impact: 4.1)
  * `holderRender` **(Tests & Verification)** (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `title` **(Compute Cores)** (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `onPaginationChange` **(Many-Argument Workhorses)** (Impact: 20.7)
    * *Intent:* // ========================== Pagination ==========================
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `allElement` **(Tests & Verification)** (Impact: 12.3)
  * `getIndeterminateSelection` **(Tests & Verification)** (Impact: 11.4)
  * `renderedNames` **(Defensive Guards)** (Impact: 10.4)
  * `getCheckboxProps` **(Defensive Guards)** (Impact: 10.4)
  * `onRow` **(Tests & Verification)** (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `imageDemoTest` **(Callbacks & Closures)** (Impact: 25.2)
    * *Intent:* // eslint-disable-next-line jest/no-export
...
**Top Internal Functions/Classes:**
  * `printLog` **(Compute Cores)** (Impact: 66.8)
  * `validate` **(Compute Cores)** (Impact: 25.3)
...
**Top Internal Functions/Classes:**
  ...
  * `DynamicPropsTestCase` **(Tests & Verification)** (Impact: 3.7)
  * `fn` **(Interface Declarations)** (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `App` **(Tests & Verification)** (Impact: 4.2)
  * `get` **(Tests & Verification)** (Impact: 4.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 13 instances
...
**Top Internal Functions/Classes:**
  ...
  * `onFileDrop` **(Compute Cores)** (Impact: 14.9)
...
**Top Internal Functions/Classes:**
  * `getLocale` **(Compute Cores)** (Impact: 27.3)
...
**Top Internal Functions/Classes:**
  * `onChange` **(Tests & Verification)** (Impact: 15.2)
  * `onChange` **(Tests & Verification)** (Impact: 12.6)
  * `onChange` **(Tests & Verification)** (Impact: 7.5)
  * `onChange` **(Tests & Verification)** (Impact: 4.7)
  * `onChange` **(Callbacks & Closures)** (Impact: 4.3)
**Contextual Mitigations & Amplifications:**
...
- `eslint.config.mjs` -> Churn: **64.77%** | Cog Load: 3.2425% | Debt: 71.1436%
...
- `components/_util/warning.ts` -> **Severity: 5.694** (Embedded: 0.0736 * Error Risk: 77.3837%)
- `tests/utils.tsx` -> **Severity: 3.595** (Embedded: 0.0638 * Error Risk: 56.3293%)
- `components/_util/type.ts` -> **Severity: 3.577** (Embedded: 0.0418 * Error Risk: 85.5202%)
- `tests/shared/demoTest.tsx` -> **Severity: 3.485** (Embedded: 0.0499 * Error Risk: 69.8464%)
