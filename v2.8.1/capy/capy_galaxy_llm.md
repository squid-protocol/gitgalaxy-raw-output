...
> **Composition Archetype:** `Hub-Coupled App` (z +0.22; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 38%, Defensive Guards Files 19%, State Mutators Files 16%, Data / Markup / Trivial 7%, Interface Declarations Files 7%
...
- `Atom` **(Compute Cores)** (@ `src/data.zig`) -> Impact: **118.7** | LOC: 592
...
- `GridLayout` **(Defensive Guards)** (@ `src/containers.zig`) -> Impact: **81.2** | LOC: 238
...
- `RowLayout` **(Compute Cores)** (@ `src/containers.zig`) -> Impact: **54.3** | LOC: 81
  * *Intent:* /// Arranges items horizontally.
- `ColumnLayout` **(Compute Cores)** (@ `src/containers.zig`) -> Impact: **53.9** | LOC: 74
...
### 1. `src/backends/wasm/capy-worker.js` (JAVASCRIPT) -> Cumulative Risk: **744.19**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.65)
...
- **Heaviest Functions:** `clock_time_get` (Many-Argument Workhorses, Impact: 15.2), `fd_write` (Many-Argument Workhorses, Impact: 14.5), `waitForAnswer` (Compute Cores, Impact: 9.9)

### 2. `src/backends/wasm/capy.js` (JAVASCRIPT) -> Cumulative Risk: **736.32**
...
- **Heaviest Functions:** `pushAnswer` (Defensive Guards, Impact: 31.6), `addChunk` (Compute Cores, Impact: 17.0), `fillImage` (Many-Argument Workhorses, Impact: 14.7)
...
- **Heaviest Functions:** `roundedRectangleEx` (Many-Argument Workhorses, Impact: 16.1), `text` (Type Conversions, Impact: 8.0), `gtkCanvasDraw` (Defensive Guards, Impact: 7.8)
...
- **Heaviest Functions:** `setChild` (Defensive Guards, Impact: 5.6), `show` (Interface Declarations, Impact: 3.2), `resize` (State Mutators, Impact: 2.1)
...
- **Heaviest Functions:** `Events` (Compute Cores, Impact: 9.5), `runStep` (Compute Cores, Impact: 8.9), `setChild` (Defensive Guards, Impact: 5.5)
...
- **Heaviest Functions:** `Events` (Defensive Guards, Impact: 13.6), `runOnUiThread` (Many-Argument Workhorses, Impact: 8.2), `onClick` (Defensive Guards, Impact: 7.3)
...
- **Heaviest Functions:** `showNativeMessageDialog` (Type Conversions, Impact: 9.3), `init` (Interface Declarations, Impact: 3.4), `runStep` (Interface Declarations, Impact: 1.7)

### 8. `src/backends/macos/CapyAppDelegate.zig` (ZIG) -> Cumulative Risk: **524.49**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.16)
...
- **Heaviest Functions:** `get` (Defensive Guards, Impact: 8.2), `a` (State Mutators, Impact: 2.4), `a` (Parameter Forwarders, Impact: 2.4)
...
- **Heaviest Functions:** `draw` (Type Conversions, Impact: 13.6), `getPreferredSize` (Defensive Guards, Impact: 5.5), `show` (Defensive Guards, Impact: 3.1)
...
- **Heaviest Functions:** `process` (Many-Argument Workhorses, Impact: 98.3), `Events` (Type Conversions, Impact: 82.1), `setFullscreen` (Many-Argument Workhorses, Impact: 14.2)
...
**Top Internal Functions/Classes:**
  * `Atom` **(Compute Cores)** (Impact: 118.7)
  ...
  * `animate` **(Many-Argument Workhorses)** (Impact: 16.2)
...
**Top Internal Functions/Classes:**
  * `GridLayout` **(Defensive Guards)** (Impact: 81.2)
    * *Intent:* /// Grid layout based on the CSS Grid algorithm
  * `RowLayout` **(Compute Cores)** (Impact: 54.3)
    * *Intent:* /// Arranges items horizontally.
  * `ColumnLayout` **(Compute Cores)** (Impact: 53.9)
    * *Intent:* /// Arranges items vertically.
...
**Top Internal Functions/Classes:**
  * `clock_time_get` **(Many-Argument Workhorses)** (Impact: 15.2)
...
**Top Internal Functions/Classes:**
  * `mainLoop` **(Compute Cores)** (Impact: 41.3)
  ...
  * `renderf32` **(State Mutators)** (Impact: 9.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `gtkKeyPress` **(Many-Argument Workhorses)** (Impact: 26.4)
...
**Top Internal Functions/Classes:**
  ...
  * `invoke` **(Defensive Guards)** (Impact: 12.8)
...
**Top Internal Functions/Classes:**
  * `simulationThread` **(Compute Cores)** (Impact: 33.1)
  * `onMouseButton` **(Many-Argument Workhorses)** (Impact: 15.7)
  * `onDraw` **(Defensive Guards)** (Impact: 13.4)
...
**Top Internal Functions/Classes:**
  ...
  * `setCallback` **(State Mutators)** (Impact: 4.9)
...
**Top Internal Functions/Classes:**
  ...
  * `deinit` **(State Mutators)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
