...
| Avg Path Length | 7.3156 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **File Composition:** Declarative / Non-Code 21%, Large Core Modules 17%, Data / Markup / Trivial 16%, Callbacks & Closures Files 13%, Interface Declarations Files 8%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 79.6 | 2.9 | 0.0 | 0.0 |
...
| connectivity | 4885 | 895 | 7 | `app/styles/_variables.scss` |
...
- `parse` **(Compute Cores)** (@ `app/src/lib/actions-log-parser/action-log-parser.ts`) -> Impact: **260.3** | LOC: 567
...
- `safeDirectoryName` **(Compute Cores)** (@ `app/src/ui/add-repository/create-repository.tsx`) -> Impact: **149.0** | LOC: 660
...
- `buildDefaultMenu` **(Compute Cores)** (@ `app/src/main-process/menu/build-default-menu.ts`) -> Impact: **142.5** | LOC: 558
...
- `getDescriptionForError` **(Defensive Guards)** (@ `app/src/lib/git/core.ts`) -> Impact: **133.4** | LOC: 140
- `onDismiss` **(I/O & Config Routines)** (@ `app/src/ui/app.tsx`) -> Impact: **114.5** | LOC: 829
- `getNextPagePathFromLink` **(Compute Cores)** (@ `app/src/lib/api.ts`) -> Impact: **100.5** | LOC: 369
...
- **Heaviest Functions:** `abort` (Compute Cores, Impact: 31.2), `createHooksProxy` (Callbacks & Closures, Impact: 8.8), `tryExit` (Defensive Guards, Impact: 4.0)
...
- **Heaviest Functions:** `launch` (Compute Cores, Impact: 38.5), `getShellPath` (Compute Cores, Impact: 31.8), `getAvailableShells` (I/O & Config Routines, Impact: 24.9)
...
- **Heaviest Functions:** `getShellEnv` (Many-Argument Workhorses, Impact: 33.8)

### 4. `app/src/lib/git/submodule.ts` (TYPESCRIPT) -> Cumulative Risk: **638.09**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.00)
...
- **Heaviest Functions:** `updateSubmodulesAfterOperation` (Many-Argument Workhorses, Impact: 33.1), `listSubmodules` (Compute Cores, Impact: 11.3), `parse` (Compute Cores, Impact: 6.6)
...
- **Heaviest Functions:** `_recordCommitStats` (Many-Argument Workhorses, Impact: 49.0), `_undoMultiCommitOperation` (Many-Argument Workhorses, Impact: 44.1), `_mergeBranch` (Many-Argument Workhorses, Impact: 42.0)
...
- **Heaviest Functions:** `onSubmit` (I/O & Config Routines, Impact: 21.0), `componentWillMount` (I/O & Config Routines, Impact: 13.6), `renderActiveTab` (I/O & Config Routines, Impact: 13.1)

### 7. `app/src/lib/shells/win32.ts` (TYPESCRIPT) -> Cumulative Risk: **629.62**
- **Archetype:** `file_cluster_8` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +2.68)
...
- **Heaviest Functions:** `findHyper` (Compute Cores, Impact: 89.0), `getAvailableShells` (I/O & Config Routines, Impact: 19.9), `findPowerShell` (I/O & Config Routines, Impact: 8.9)
...
- **Heaviest Functions:** `withTrampolineEnv` (Many-Argument Workhorses, Impact: 39.0), `getCredentialUrl` (Defensive Guards, Impact: 22.2), `getHasRejectedCredentialsForEndpoint` (Defensive Guards, Impact: 7.6)
...
- **Heaviest Functions:** `constructor` (Compute Cores, Impact: 20.5), `addEventListener` (Callbacks & Closures, Impact: 3.3), `addEventListener` (Generic / Templated Code, Impact: 3.2)

### 10. `app/src/lib/git/commit.ts` (TYPESCRIPT) -> Cumulative Risk: **621.18**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.79)
...
- **Heaviest Functions:** `createCommit` (Many-Argument Workhorses, Impact: 45.4), `createMergeCommit` (Many-Argument Workhorses, Impact: 12.8)
...
**Top Internal Functions/Classes:**
  ...
  * `onGitStoreUpdated` **(Compute Cores)** (Impact: 40.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.022046 | `Ripple Effect (Closeness):` 0.058454
...
**Top Internal Functions/Classes:**
  ...
  * `getMultiCommitOperationSuccessBanner` **(Many-Argument Workhorses)** (Impact: 24.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.041074 | `Ripple Effect (Closeness):` 0.066308
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000498 | `Ripple Effect (Closeness):` 0.002357
...
**Top Internal Functions/Classes:**
  ...
  * `emitUpdatesForChangedTags` **(Compute Cores)** (Impact: 19.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002632 | `Ripple Effect (Closeness):` 0.047445
...
**Top Internal Functions/Classes:**
  * `closestRow` **(Many-Argument Workhorses)** (Impact: 34.3)
  ...
  * `getDiscardLabel` **(Compute Cores)** (Impact: 23.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000148 | `Ripple Effect (Closeness):` 0.001571
...
**Top Internal Functions/Classes:**
  * `parse` **(Compute Cores)** (Impact: 260.3)
  ...
  * `parseCommandEnd` **(Compute Cores)** (Impact: 59.5)
  * `updateLineMetaData` **(I/O & Config Routines)** (Impact: 8.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `getNextPagePathFromLink` **(Compute Cores)** (Impact: 100.5)
  ...
  * `fetchAll` **(Generic / Templated Code)** (Impact: 20.4)
  ...
  * `ghRequest` **(Many-Argument Workhorses)** (Impact: 15.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.029064 | `Ripple Effect (Closeness):` 0.151956
...
**Top Internal Functions/Classes:**
  ...
  * `onRowMouseDown` **(Compute Cores)** (Impact: 62.3)
  * `getRowRenderer` **(Compute Cores)** (Impact: 28.6)
  * `onRowMouseUp` **(Compute Cores)** (Impact: 28.0)
  * `componentDidUpdate` **(Compute Cores)** (Impact: 27.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004523 | `Ripple Effect (Closeness):` 0.058626
...
**Top Internal Functions/Classes:**
  * `onRowMouseDown` **(Compute Cores)** (Impact: 60.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000386 | `Ripple Effect (Closeness):` 0.008557
  * `Imports (Out-Degree: 10):` equality, non-fatal-exception, range, drag-drop, focus-container, id-pool, list-item-insertion-overlay, list-row...
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000128 | `Ripple Effect (Closeness):` 0.002175
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000135 | `Ripple Effect (Closeness):` 0.005616
...
**Top Internal Functions/Classes:**
  ...
  * `setup` **(Compute Cores)** (Impact: 18.7)
...
**Top Internal Functions/Classes:**
  * `_upsertGitHubRepository` **(Many-Argument Workhorses)** (Impact: 48.2)
  * `putOwner` **(Defensive Guards)** (Impact: 25.8)
  * `getPermissionsString` **(Compute Cores)** (Impact: 15.0)
  * `toGitHubRepository` **(Many-Argument Workhorses)** (Impact: 14.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001216 | `Ripple Effect (Closeness):` 0.053507
...
**Top Internal Functions/Classes:**
  ...
  * `renderChangedFile` **(Compute Cores)** (Impact: 32.6)
  * `onContextMenu` **(Compute Cores)** (Impact: 16.0)
  * `renderCommitMessageForm` **(I/O & Config Routines)** (Impact: 13.5)
  * `getPlaceholderMessage` **(Compute Cores)** (Impact: 13.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000379 | `Ripple Effect (Closeness):` 0.003273
...
**Top Internal Functions/Classes:**
  * `getDescriptionForError` **(Defensive Guards)** (Impact: 133.4)
  * `git` **(Many-Argument Workhorses)** (Impact: 81.9)
  * `push` **(Compute Cores)** (Impact: 40.4)
  * `constructor` **(Many-Argument Workhorses)** (Impact: 19.4)
  * `isGitError` **(Compute Cores)** (Impact: 8.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 15 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.014417 | `Ripple Effect (Closeness):` 0.078081
...
**Top Internal Functions/Classes:**
  * `getContextMenuForSingleCommit` **(Compute Cores)** (Impact: 67.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.009072 | `Ripple Effect (Closeness):` 0.062628
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.015142 | `Ripple Effect (Closeness):` 0.062891
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000146 | `Ripple Effect (Closeness):` 0.000786
...
**Top Internal Functions/Classes:**
  ...
  * `onTooltipRef` **(Defensive Guards)** (Impact: 17.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000504 | `Ripple Effect (Closeness):` 0.100021
...
- `app/src/lib/menu-update.ts` -> **Severity: 3.068** (Bridge: 0.0323 * Flux: 94.9147%)
- `app/src/ui/repository-settings/repository-settings.tsx` -> **Severity: 2.681** (Bridge: 0.029 * Flux: 92.4809%)
- `app/src/ui/dialog/dialog.tsx` -> **Severity: 1.35** (Bridge: 0.0169 * Flux: 79.7283%)
- `app/src/lib/stores/app-store.ts` -> **Severity: 1.192** (Bridge: 0.022 * Flux: 54.0705%)
- `app/src/lib/trampoline/trampoline-environment.ts` -> **Severity: 1.092** (Bridge: 0.0114 * Flux: 95.7191%)
