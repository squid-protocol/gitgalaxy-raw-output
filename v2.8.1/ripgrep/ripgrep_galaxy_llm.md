...
> **Composition Archetype:** `Small Flat Repo` (z -0.95; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 31%, Large Core Modules 24%, Declarative / Non-Code 12%, State Mutators Files 8%, Tests & Verification Files 6%
...
- `doc_long` **(Tests & Verification)** (@ `crates/core/flags/defs.rs`) -> Impact: **263.4** | LOC: 1648
- `_rg` **(Compute Cores)** (@ `crates/core/flags/complete/rg.zsh`) -> Impact: **96.8** | LOC: 396
...
- **Composition Archetype:** `Large Core Modules` (z +1.30)
...
- **Heaviest Functions:** `_rg` (Compute Cores, Impact: 96.8), `__global_context__` (I/O & Config Routines, Impact: 9.0), `Anonymous_Block` (Compute Cores, Impact: 5.9)
...
- **Heaviest Functions:** `main` (I/O & Config Routines, Impact: 39.2), `collect_benchmarks` (Many-Argument Workhorses, Impact: 18.3), `download` (Compute Cores, Impact: 13.4)
...
- **Heaviest Functions:** `new` (Compute Cores, Impact: 24.4), `matches_into` (Defensive Guards, Impact: 10.8), `is_match` (Defensive Guards, Impact: 9.5)

### 4. `crates/ignore/examples/walk.rs` (RUST) -> Cumulative Risk: **518.58**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +2.47)
...
- **Heaviest Functions:** `main` (Callbacks & Closures, Impact: 15.3), `path` (Interface Declarations, Impact: 3.1)
...
- **Heaviest Functions:** `interpolate` (Many-Argument Workhorses, Impact: 24.6), `find_cap_ref` (Defensive Guards, Impact: 14.7), `interpolate_string` (Generic / Templated Code, Impact: 5.2)
...
- **Heaviest Functions:** `generate_work` (Many-Argument Workhorses, Impact: 50.1), `run_one` (Compute Cores, Impact: 35.1), `visit` (Compute Cores, Impact: 29.8)
...
- **Heaviest Functions:** `build_many` (Compute Cores, Impact: 13.8), `try_find_iter` (Generic / Templated Code, Impact: 8.9), `find_candidate_line` (Defensive Guards, Impact: 6.0)
...
- **Heaviest Functions:** `doc_long` (Tests & Verification, Impact: 263.4), `doc_long` (Compute Cores, Impact: 19.8), `doc_long` (Compute Cores, Impact: 17.3)
...
- **Heaviest Functions:** `match_by_line_fast` (Compute Cores, Impact: 43.7), `match_by_line_slow` (Compute Cores, Impact: 40.8), `after_context_by_line` (Many-Argument Workhorses, Impact: 27.7)
...
- **Heaviest Functions:** `parse_star` (Defensive Guards, Impact: 44.0), `tokens_to_regex` (Many-Argument Workhorses, Impact: 37.1), `parse_class` (Compute Cores, Impact: 35.7)
...
**Top Internal Functions/Classes:**
  * `doc_long` **(Tests & Verification)** (Impact: 263.4)
  * `doc_long` **(Compute Cores)** (Impact: 19.8)
  * `doc_long` **(Compute Cores)** (Impact: 17.3)
  * `doc_long` **(Compute Cores)** (Impact: 15.7)
...
**Top Internal Functions/Classes:**
  ...
  * `run_one` **(Compute Cores)** (Impact: 35.1)
...
**Top Internal Functions/Classes:**
  * `matched_ignore` **(Many-Argument Workhorses)** (Impact: 66.3)
    * *Intent:* /// Performs matching only on the ignore files for this directory and /// all parent directories.
  * `add_child_path` **(Compute Cores)** (Impact: 32.4)
    * *Intent:* /// Like add_child, but takes a full path and returns an IgnoreInner.
  * `add_parents` **(Compute Cores)** (Impact: 30.8)
...
**Top Internal Functions/Classes:**
  ...
  * `after_context_by_line` **(Many-Argument Workhorses)** (Impact: 27.7)
  * `match_by_line_fast_invert` **(Compute Cores)** (Impact: 23.0)
  * `before_context_by_line` **(Many-Argument Workhorses)** (Impact: 17.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `matches_candidate_into` **(Many-Argument Workhorses)** (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `replace_with_captures_at` **(Many-Argument Workhorses)** (Impact: 6.4)
    * *Intent:* /// Replaces every match in the given haystack with the result of calling /// `append` with the matc...
  * `interpolate` **(Many-Argument Workhorses)** (Impact: 5.9)
...
**Top Internal Functions/Classes:**
  ...
  * `impossible` **(Tests & Verification)** (Impact: 12.2)
  * `repetition` **(Tests & Verification)** (Impact: 11.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `matched_stripped` **(Many-Argument Workhorses)** (Impact: 15.2)
...
**Top Internal Functions/Classes:**
  ...
  * `from_path` **(Compute Cores)** (Impact: 19.2)
    * *Intent:* /// Returns a hyperlink path from an OS path.
  * `begin` **(Generic / Templated Code)** (Impact: 13.0)
  ...
  * `validate_scheme` **(Compute Cores)** (Impact: 9.7)
...
**Top Internal Functions/Classes:**
  ...
  * `roll` **(Interface Declarations)** (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  ...
  * `test` **(Compute Cores)** (Impact: 20.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_rg_types` **(Interface Declarations)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `candidate_lines` **(Tests & Verification)** (Impact: 4.6)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_low` **(I/O & Config Routines)** (Impact: 9.5)
