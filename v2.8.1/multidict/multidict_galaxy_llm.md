...
> **Composition Archetype:** `Typed Library` (z -0.16; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 25%, Data / Markup / Trivial 19%, Large Core Modules 17%, Defensive Guards Files 12%, Interface Declarations Files 12%
...
- `multidict_itemsview_or2` **(Compute Cores)** (@ `multidict-6.7.1/multidict/_multilib/views.h`) -> Impact: **38.8** | LOC: 84
...
- **Heaviest Functions:** `_parse_args` (Many-Argument Workhorses, Impact: 38.3), `__eq__` (Defensive Guards, Impact: 20.1), `new` (Compute Cores, Impact: 16.9)
...
- **Heaviest Functions:** `PyConfig_Get` (Compute Cores, Impact: 51.9), `PyObject_Vectorcall` (Many-Argument Workhorses, Impact: 43.6), `PyDict_Pop` (Many-Argument Workhorses, Impact: 28.1)

### 3. `multidict-6.7.1/multidict/_multilib/parser.h` (C) -> Cumulative Risk: **589.84**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Encapsulated Accessors Files` (z +0.90)
...
- **Heaviest Functions:** `parse2` (Many-Argument Workhorses, Impact: 103.3), `raise_unexpected_kwarg` (Encapsulated Accessors, Impact: 2.2), `raise_missing_posarg` (Encapsulated Accessors, Impact: 2.2)
...
- **Heaviest Functions:** `md_update_from_seq` (Many-Argument Workhorses, Impact: 64.2), `md_repr` (Many-Argument Workhorses, Impact: 55.9), `md_update_from_ht` (Many-Argument Workhorses, Impact: 39.5)
...
- **Heaviest Functions:** `multidict_view_richcompare` (Many-Argument Workhorses, Impact: 58.5), `multidict_itemsview_or2` (Compute Cores, Impact: 38.8), `multidict_itemsview_sub1` (Compute Cores, Impact: 38.8)
...
- **Heaviest Functions:** `htkeys_set_index` (Defensive Guards, Impact: 17.2), `htkeys_build_indices` (Defensive Guards, Impact: 16.9), `htkeys_get_index` (Compute Cores, Impact: 15.0)
...
- **Heaviest Functions:** `istr_new` (Many-Argument Workhorses, Impact: 15.9), `IStr_New` (Many-Argument Workhorses, Impact: 7.0), `istr_init` (Compute Cores, Impact: 5.9)

### 8. `multidict-6.7.1/multidict/_multilib/state.h` (C) -> Cumulative Risk: **485.22**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.17)
...
- **Heaviest Functions:** `PyType_GetModuleByDef` (Defensive Guards, Impact: 19.8), `get_mod_state_by_def_checked` (Compute Cores, Impact: 6.0), `get_mod_state` (Defensive Guards, Impact: 1.8)

### 9. `multidict-6.7.1/multidict/_multidict.c` (C) -> Cumulative Risk: **483.61**
...
- **Heaviest Functions:** `_multidict_extend` (Many-Argument Workhorses, Impact: 64.7), `_multidict_extend_parse_args` (Many-Argument Workhorses, Impact: 54.4), `multidict_tp_richcompare` (Many-Argument Workhorses, Impact: 34.5)

### 10. `multidict-6.7.1/multidict/_abc.py` (PYTHON) -> Cumulative Risk: **482.62**
...
- **Heaviest Functions:** `getall` (Generic / Templated Code, Impact: 2.0), `getall` (Generic / Templated Code, Impact: 2.0), `getone` (Generic / Templated Code, Impact: 2.0)
...
**Top Internal Functions/Classes:**
  ...
  * `new` **(Compute Cores)** (Impact: 16.9)
  * `popall` **(Many-Argument Workhorses)** (Impact: 15.4)
  * `__init__` **(Defensive Guards)** (Impact: 15.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `PyTime_PerfCounter` **(Compute Cores)** (Impact: 26.1)
...
**Top Internal Functions/Classes:**
  ...
  * `multidict_itemsview_or2` **(Compute Cores)** (Impact: 38.8)
  * `multidict_itemsview_sub1` **(Compute Cores)** (Impact: 38.8)
...
- **Magnitude:** 796.18 | **LOC:** 1592 | **CtrlFlow:** 15.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (64.3237%), Tech Debt (8.3725%)
**Top Internal Functions/Classes:**
  ...
  * `_multidict_clone_fast` **(Many-Argument Workhorses)** (Impact: 23.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_cimultidict_extend_istr_with_kwargs` **(Generic / Templated Code)** (Impact: 10.9)
  * `test_multidict_extend_str_with_kwargs` **(Type Conversions)** (Impact: 9.3)
  * `test_cimultidict_update_istr_with_kwargs` **(Generic / Templated Code)** (Impact: 8.9)
  * `test_cimultidict_extend_istr` **(Generic / Templated Code)** (Impact: 8.9)
  * `test_cimultidict_pop_istr` **(Generic / Templated Code)** (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_multidict_shrink_regression` **(I/O & Config Routines)** (Impact: 8.1)
...
**Top Internal Functions/Classes:**
  * `htkeys_set_index` **(Defensive Guards)** (Impact: 17.2)
    * *Intent:* /* write to indices. */
  * `htkeys_build_indices` **(Defensive Guards)** (Impact: 16.9)
    * *Intent:* */
  * `htkeys_get_index` **(Compute Cores)** (Impact: 15.0)
    * *Intent:* /* lookup indices. returns DKIX_EMPTY, DKIX_DUMMY, or ix >=0 */
  * `htkeys_new` **(Defensive Guards)** (Impact: 14.8)
  * `_ht_bit_length` **(Compute Cores)** (Impact: 14.4)
...
**Top Internal Functions/Classes:**
  * `test_keys_view_equals` **(Type Conversions)** (Impact: 5.6)
  * `test_keys_view_not_equals` **(Type Conversions)** (Impact: 5.6)
  * `test_keys_view_more` **(Type Conversions)** (Impact: 5.6)
  * `test_keys_view_more_or_equal` **(Type Conversions)** (Impact: 5.6)
  * `test_keys_view_less` **(Type Conversions)** (Impact: 5.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_popone_key_error` **(Defensive Guards)** (Impact: 2.8)
  * `test_pop_key_error` **(Defensive Guards)** (Impact: 2.8)
  * `test_popall_key_error` **(Defensive Guards)** (Impact: 2.8)
...
**Top Internal Functions/Classes:**
  ...
  * `pytest_generate_tests` **(Generic / Templated Code)** (Impact: 3.1)
...
**Top Internal Functions/Classes:**
  * `multidict_iter_init` **(Compute Cores)** (Impact: 8.1)
  * `multidict_items_iter_iternext` **(Compute Cores)** (Impact: 7.0)
  * `multidict_values_iter_iternext` **(Interface Declarations)** (Impact: 5.0)
  * `multidict_keys_iter_iternext` **(Interface Declarations)** (Impact: 5.0)
  * `multidict_items_iter_new` **(Interface Declarations)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `istr_new` **(Many-Argument Workhorses)** (Impact: 15.9)
  * `IStr_New` **(Many-Argument Workhorses)** (Impact: 7.0)
  * `istr_init` **(Compute Cores)** (Impact: 5.9)
  * `istr_reduce` **(Compute Cores)** (Impact: 5.3)
  * `istr_dealloc` **(Interface Declarations)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 8 instances
* *State Mutation (weighted view):* 24
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `get_mod_state_by_def_checked` **(Compute Cores)** (Impact: 6.0)
  * `get_mod_state` **(Defensive Guards)** (Impact: 1.8)
  * `get_mod_state_by_cls` **(Defensive Guards)** (Impact: 1.8)
  * `get_mod_state_by_def` **(Defensive Guards)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 7 instances
* *State Mutation (weighted view):* 22
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `_test_pop` **(Interface Declarations)** (Impact: 4.3)
  * `_test_popall` **(Interface Declarations)** (Impact: 4.3)
  * `_test_popone` **(Interface Declarations)** (Impact: 4.3)
  * `_test_del` **(Interface Declarations)** (Impact: 4.3)
  * `_test_pop_with_default` **(Interface Declarations)** (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `test_create_ci_multidict_proxy_from_multidict` **(Tests & Verification)** (Impact: 4.8)
  * `test_create_cimultidict_proxy_from_nonmultidict` **(Tests & Verification)** (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  ...
  * `test_pickle` **(Defensive Guards)** (Impact: 2.1)
  * `test_pickle_proxy` **(Generic / Templated Code)** (Impact: 2.1)
  * `test_pickle_istr` **(Defensive Guards)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_discover_path_importables` **(Compute Cores)** (Impact: 8.0)
  * `test_c_extension_preferred_by_default` **(Tests & Verification)** (Impact: 3.7)
    * *Intent:* """Verify that the C-extension is exposed by default."""
  * `_find_all_importables` **(Generic / Templated Code)** (Impact: 3.4)
    * *Intent:* """Find all importables in the project. Return them in order. """
  * `import_path` **(Tests & Verification)** (Impact: 3.2)
    * *Intent:* """Return an importable from the multidict package."""
  * `test_no_warnings` **(I/O & Config Routines)** (Impact: 2.3)
