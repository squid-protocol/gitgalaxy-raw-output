...
> **Composition Archetype:** `Flat Modular Platform` (z +1.16; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 83%, Annotated Framework Methods Files 6%, Tests & Verification Files 4%, Large Core Modules 2%, Declarative / Non-Code 1%
...
- `maintain` **(Compute Cores)** (@ `core/src/main/java/hudson/model/Queue.java`) -> Impact: **78.8** | LOC: 237
...
- **Heaviest Functions:** `updateListBox` (Many-Argument Workhorses, Impact: 47.5), `onSuccess` (Defensive Guards, Impact: 13.1), `hasChanged` (Compute Cores, Impact: 12.9)
...
- **Heaviest Functions:** `init` (I/O & Config Routines, Impact: 11.7), `show` (State Mutators, Impact: 9.7), `init` (Callbacks & Closures, Impact: 3.6)

### 3. `src/main/js/components/dropdowns/autocomplete.js` (JAVASCRIPT) -> Cumulative Risk: **663.6**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -1.24)
- **Magnitude:** 84.06 | **LOC:** 117 | **CtrlFlow:** 13.6% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `init` (Callbacks & Closures, Impact: 19.6), `updateSuggestions` (Compute Cores, Impact: 14.6), `convertSuggestionToItem` (Callbacks & Closures, Impact: 6.2)
...
- **Composition Archetype:** `Annotated Framework Methods Files` (z -0.06)
...
- **Heaviest Functions:** `handle` (Annotated Framework Methods, Impact: 7.6), `createContent` (Annotated Framework Methods, Impact: 4.7), `getId` (Annotated Framework Methods, Impact: 1.2)
...
- **Heaviest Functions:** `check` (Compute Cores, Impact: 32.9), `updateStartStopWindow` (I/O & Config Routines, Impact: 9.7), `doCheck` (Defensive Guards, Impact: 3.3)
...
- **Heaviest Functions:** `updateComputerList` (Compute Cores, Impact: 24.6), `createNewComputerForNode` (Defensive Guards, Impact: 19.6), `updateComputer` (Defensive Guards, Impact: 9.7)
...
- **Heaviest Functions:** `populateEnableDisableInfo` (Many-Argument Workhorses, Impact: 23.1), `initPluginRowHandling` (Callbacks & Closures, Impact: 17.5), `applyFilter` (I/O & Config Routines, Impact: 15.5)

### 8. `core/src/main/java/hudson/util/io/ReopenableFileOutputStream.java` (JAVA) -> Cumulative Risk: **614.83**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -1.82)
...
- **Heaviest Functions:** `current` (Defensive Guards, Impact: 4.5), `close` (Annotated Framework Methods, Impact: 2.4), `write` (Annotated Framework Methods, Impact: 2.2)

### 9. `websocket/jetty12-ee9/src/main/java/jenkins/websocket/Jetty12EE9Provider.java` (JAVA) -> Cumulative Risk: **610.68**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -0.65)
...
- **Heaviest Functions:** `handle` (Generic / Templated Code, Impact: 10.8), `createWebSocket` (Annotated Framework Methods, Impact: 5.1), `init` (Interface Declarations, Impact: 3.1)
...
- **Heaviest Functions:** `init` (Defensive Guards, Impact: 27.2), `appendButtons` (Defensive Guards, Impact: 11.9), `show` (Callbacks & Closures, Impact: 9.9)
...
**Top Internal Functions/Classes:**
  ...
  * `loadTasks` **(Compute Cores)** (Impact: 29.6)
...
**Top Internal Functions/Classes:**
  ...
  * `registerMinMaxValidator` **(Compute Cores)** (Impact: 50.2)
...
**Top Internal Functions/Classes:**
  ...
  * `invoke` **(Compute Cores)** (Impact: 48.5)
...
**Top Internal Functions/Classes:**
  ...
  * `getPasswordValue` **(Compute Cores)** (Impact: 43.4)
...
**Top Internal Functions/Classes:**
  ...
  * `doConnectionStatusImpl` **(Compute Cores)** (Impact: 27.9)
...
**Top Internal Functions/Classes:**
  ...
  * `doPluginsSearch` **(Compute Cores)** (Impact: 41.8)
  * `initTasks` **(Defensive Guards)** (Impact: 40.8)
...
**Top Internal Functions/Classes:**
  * `maintain` **(Compute Cores)** (Impact: 78.8)
  ...
  * `makeFlyWeightTaskBuildable` **(Compute Cores)** (Impact: 20.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getLog` **(Compute Cores)** (Impact: 14.9)
...
**Top Internal Functions/Classes:**
  ...
  * `showInstallProgress` **(Compute Cores)** (Impact: 46.8)
    * *Intent:* // Define actions
  * `updateStatus` **(Compute Cores)** (Impact: 36.5)
...
**Top Internal Functions/Classes:**
  * `submitImpl` **(Compute Cores)** (Impact: 36.1)
  * `_poll` **(Compute Cores)** (Impact: 31.4)
  ...
  * `workspaceOffline` **(Compute Cores)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `AIXProcess` **(Compute Cores)** (Impact: 16.7)
  * `Darwin` **(Compute Cores)** (Impact: 16.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getBuildStabilityHealthReport` **(Compute Cores)** (Impact: 26.1)
...
**Top Internal Functions/Classes:**
  ...
  * `Data` **(Compute Cores)** (Impact: 31.8)
  * `getNeededDependencies` **(Compute Cores)** (Impact: 21.4)
...
**Top Internal Functions/Classes:**
  * `fromString` **(Compute Cores)** (Impact: 40.7)
    * *Intent:* /** * Parses a {@link RangeSet} from a string like "1-3,5,7-9" */
  * `locationChanged` **(Defensive Guards)** (Impact: 18.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getStaticHelpUrl` **(Many-Argument Workhorses)** (Impact: 17.2)
    * *Intent:* /** * @since 2.475 */
  * `doHelpImpl` **(Compute Cores)** (Impact: 15.6)
  * `buildFillDependencies` **(Compute Cores)** (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 7.7)
...
**Top Internal Functions/Classes:**
  * `foldableCauseAction` **(Tests & Verification)** (Impact: 16.6)
  * `pendingsConsistenceAfterErrorDuringMaintain` **(Tests & Verification)** (Impact: 15.9)
  * `queueApiOutputShouldBeFilteredByUserPermission` **(Tests & Verification)** (Impact: 12.2)
  * `waitUntilWaitingListIsEmpty` **(Defensive Guards)** (Impact: 7.7)
  * `flyweightTasks` **(Tests & Verification)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `printCommandLine` **(Compute Cores)** (Impact: 18.5)
  ...
  * `call` **(Annotated Framework Methods)** (Impact: 12.9)
  * `maskedPrintCommandLine` **(Defensive Guards)** (Impact: 10.8)
...
**Top Internal Functions/Classes:**
  ...
  * `filterQueueItemTest` **(Compute Cores)** (Impact: 20.6)
  * `create` **(Many-Argument Workhorses)** (Impact: 20.6)
    * *Intent:* /** * @since 2.475 */
  * `getComputers` **(Defensive Guards)** (Impact: 13.1)
  * `isRelevant` **(Generic / Templated Code)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  * `dummyRangeRangeSingleShouldSuccess` **(Tests & Verification)** (Impact: 19.1)
  * `dummyRangeRangeSingleShouldFailIfBuildRangeContainsZeroAndNegative` **(Tests & Verification)** (Impact: 17.3)
  * `act` **(Compute Cores)** (Impact: 14.9)
  * `dummyRangeRangeSingleShouldFailIfBuildRangeContainsANegativeNumber` **(Tests & Verification)** (Impact: 14.6)
  * `dummyRangeNumberSingleShouldSuccess` **(Tests & Verification)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getVersionOf` **(Compute Cores)** (Impact: 9.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `testValidator290` **(Tests & Verification)** (Impact: 35.9)
  * `main` **(Compute Cores)** (Impact: 28.3)
    * *Intent:* /** * Validator for checking URL parsing * @param args - URLs to validate */
  * `testIsValid` **(Compute Cores)** (Impact: 26.3)
