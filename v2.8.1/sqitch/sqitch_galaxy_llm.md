...
> **Composition Archetype:** `Small Flat Repo` (z +0.92; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 42%, Data / Markup / Trivial 32%, Large Core Modules 19%, Interface Declarations Files 4%, Annotated Framework Methods Files 1%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 95.3 | 35.2 | 49.4 | 0.0 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 22.6 | 0.0 | 0.0 |
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 2.7 | 0.4 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 95.3 | 3.1 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 9.4 | 0.0 | 0.0 |
...
| cleanup | 57 | 27 | 1 | `t/rework.t` |
| guards | 605 | 98 | 15 | `lib/App/Sqitch/Engine/pg.sql` |
| danger | 298 | 60 | 7 | `t/engine.t` |
| concurrency | 28 | 8 | 0 | `t/rebase.t` |
| connectivity | 59 | 13 | 0 | `t/engine.t` |
| io | 82 | 22 | 2 | `t/plan.t` |
| crypto | 0 | 0 | 0 | - |
| ipc | 8 | 4 | 0 | `lib/App/Sqitch/Engine/Upgrade/sqlite-1.0.sql` |
| time | 122 | 26 | 5 | `lib/App/Sqitch/Engine/sqlite.sql` |
| serialization | 0 | 0 | 0 | - |
| regex | 306 | 36 | 9 | `t/config.t` |
| events | 241 | 47 | 3 | `t/config.t` |
| tests | 1536 | 52 | 33 | `t/engine.t` |
| docs | 0 | 0 | 0 | - |
| debt | 145 | 46 | 3 | `t/engine.t` |
| mutation | 6464 | 92 | 135 | `t/engine.t` |
...
- `SELECT_Statement` **(Unclassified)** (@ `lib/App/Sqitch/Engine/Upgrade/oracle-1.1.sql`) -> Impact: **12.5** | LOC: 10
  * *Intent:* -- Rename the dependencies check constraint.
...
- `configure` **(Compute Cores)** (@ `inc/Menlo/Sqitch.pm`) -> Impact: **6.3** | LOC: 13
...
| `lib/App/Sqitch/Engine/Upgrade` | 20 | 281.98 | 2.64% | 0.0% |
...
### 1. `t/cx_cmd_role.t` (PERL) -> Cumulative Risk: **514.99**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.28)
- **Magnitude:** 38.04 | **LOC:** 110 | **CtrlFlow:** 5.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9998%), Safety Score (84.8079%)
- **Heaviest Functions:** `options` (Interface Declarations, Impact: 1.3)

### 2. `xt/dependency_report` (PERL) -> Cumulative Risk: **461.81**
- **Archetype:** `file_cluster_16` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.29)
- **Magnitude:** 134.2 | **LOC:** 184 | **CtrlFlow:** 42.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.1085%), Verification (80.0%), Safety Score (70.0905%)
- **Heaviest Functions:** `report_on` (Compute Cores, Impact: 43.9), `run` (Compute Cores, Impact: 39.3), `is_build` (Compute Cores, Impact: 8.9)

### 3. `t/target.t` (PERL) -> Cumulative Risk: **456.32**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.75)
- **Magnitude:** 182.0 | **LOC:** 735 | **CtrlFlow:** 37.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9677%), Safety Score (78.3786%)
- **Heaviest Functions:** `_load` (Interface Declarations, Impact: 1.2)

### 4. `t/base.t` (PERL) -> Cumulative Risk: **452.32**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.991%), Safety Score (86.4623%)
- **Heaviest Functions:** `puke` (Interface Declarations, Impact: 1.1)

### 5. `t/conn_cmd_role.t` (PERL) -> Cumulative Risk: **435.48**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.23)
- **Magnitude:** 27.14 | **LOC:** 113 | **CtrlFlow:** 3.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.95%), Safety Score (74.1457%)
- **Heaviest Functions:** `options` (Interface Declarations, Impact: 1.3)

### 6. `t/command.t` (PERL) -> Cumulative Risk: **434.25**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.79)
- **Magnitude:** 146.94 | **LOC:** 784 | **CtrlFlow:** 18.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.7579%), Safety Score (73.4988%)
- **Heaviest Functions:** `options` (I/O & Config Routines, Impact: 1.4)

### 7. `t/plan.t` (PERL) -> Cumulative Risk: **432.51**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (98.7058%), Safety Score (71.6591%)
- **Heaviest Functions:** `change` (Compute Cores, Impact: 18.5), `prag` (Compute Cores, Impact: 13.7), `tag` (Compute Cores, Impact: 6.4)

### 8. `t/mysql.t` (PERL) -> Cumulative Risk: **427.78**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.34)
- **Magnitude:** 123.72 | **LOC:** 742 | **CtrlFlow:** 24.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.1176%), Safety Score (67.6451%)
- **Heaviest Functions:** `do` (Parameter Forwarders, Impact: 3.7)

### 9. `t/engine.t` (PERL) -> Cumulative Risk: **426.84**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.50)
- **Magnitude:** 444.26 | **LOC:** 3749 | **CtrlFlow:** 5.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (78.6646%), Safety Score (53.5401%)
- **Heaviest Functions:** `current_state` (Annotated Framework Methods, Impact: 3.0), `begin_work` (Interface Declarations, Impact: 2.0), `finish_work` (Interface Declarations, Impact: 2.0)

### 10. `inc/Menlo/Sqitch.pm` (PERL) -> Cumulative Risk: **424.02**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.41)
- **Magnitude:** 58.94 | **LOC:** 314 | **CtrlFlow:** 4.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (77.2822%), Safety Score (59.9611%)
- **Heaviest Functions:** `new` (Compute Cores, Impact: 12.1), `configure` (Compute Cores, Impact: 6.3), `save_meta` (Parameter Forwarders, Impact: 4.4)
...
- **Magnitude:** 444.26 | **LOC:** 3749 | **CtrlFlow:** 5.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (17.4741%), Tech Debt (12.3361%)
**Top Internal Functions/Classes:**
  * `current_state` **(Annotated Framework Methods)** (Impact: 3.0)
  * `begin_work` **(Interface Declarations)** (Impact: 2.0)
  * `finish_work` **(Interface Declarations)** (Impact: 2.0)
  * `are_deployed_changes` **(Interface Declarations)** (Impact: 1.5)
  * `change_id_for` **(Interface Declarations)** (Impact: 1.5)
...
- **Global Archetype:** `file_cluster_14` (Drift: 0.0 IQR)
- **Magnitude:** 401.12 | **LOC:** 2062 | **CtrlFlow:** 14.8% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (26.3661%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `change` **(Compute Cores)** (Impact: 18.5)
  * `prag` **(Compute Cores)** (Impact: 13.7)
  * `tag` **(Compute Cores)** (Impact: 6.4)
  * `ts` **(Annotated Framework Methods)** (Impact: 4.9)
  * `dep` **(Interface Declarations)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
### `t/lib/upgradable_registries/firebird.sql` (SQLITE | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 340.02 | **LOC:** 328 | **CtrlFlow:** 22.9% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (71.4074%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (15.6567%), Tech Debt (14.3973%)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 180.5 | **LOC:** 746 | **CtrlFlow:** 18.6% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (53.9392%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (27.6891%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (33.224%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_5` (Drift: 0.0 IQR)
- **Magnitude:** 146.94 | **LOC:** 784 | **CtrlFlow:** 18.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (37.9229%), Tech Debt (9.789%)
**Top Internal Functions/Classes:**
  * `options` **(I/O & Config Routines)** (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (18.6206%), Tech Debt (11.2905%)
**Top Internal Functions/Classes:**
  * `contents_of` **(Annotated Framework Methods)** (Impact: 4.5)
...
- **Risk Profile:** Cognitive Load (24.6304%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 134.2 | **LOC:** 184 | **CtrlFlow:** 42.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (56.4454%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (29.3737%), Tech Debt (18.7746%)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 27 instances
...
- **Global Archetype:** `file_cluster_5` (Drift: 0.0 IQR)
- **Magnitude:** 123.72 | **LOC:** 742 | **CtrlFlow:** 24.3% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (45.7259%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (19.7108%), Tech Debt (9.547%)
**Top Internal Functions/Classes:**
  * `dep` **(Interface Declarations)** (Impact: 1.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 23 instances
...
- **Global Archetype:** `file_cluster_5` (Drift: 0.0 IQR)
- **Magnitude:** 119.84 | **LOC:** 727 | **CtrlFlow:** 33.1% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (48.7807%), Tech Debt (14.9592%)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 116.94 | **LOC:** 566 | **CtrlFlow:** 21.7% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (42.8349%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (22.6399%), Tech Debt (19.8283%)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_3` (Drift: 0.0 IQR)
- **Magnitude:** 105.14 | **LOC:** 650 | **CtrlFlow:** 23.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (32.741%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (23.8606%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (23.3967%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (16.9143%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (21.0222%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
### `lib/App/Sqitch/Engine/pg.sql` (SQLITE | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
...
- **Global Archetype:** `file_cluster_3` (Drift: 0.0 IQR)
- **Magnitude:** 80.36 | **LOC:** 402 | **CtrlFlow:** 29.9% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (45.1412%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Amplified Cascading Flux:* 18 instances
...
- `t/mysql.t` -> **David E. Wheeler** (100.0% isolated ownership) | Magnitude: 123.72
- `t/oracle.t` -> **David E. Wheeler** (100.0% isolated ownership) | Magnitude: 119.84
- `t/pg.t` -> **David E. Wheeler** (100.0% isolated ownership) | Magnitude: 116.94
- `t/snowflake.t` -> **David E. Wheeler** (100.0% isolated ownership) | Magnitude: 105.14
...
- `t/config.t` -> **Severity: 6.708** (Embedded: 0.1032 * Error Risk: 65.0256%)
- `t/options.t` -> **Severity: 4.168** (Embedded: 0.0549 * Error Risk: 75.8996%)
- `t/command.t` -> **Severity: 2.645** (Embedded: 0.036 * Error Risk: 73.4988%)
- `t/change.t` -> **Severity: 2.493** (Embedded: 0.0431 * Error Risk: 57.7912%)
- `t/target.t` -> **Severity: 2.21** (Embedded: 0.0282 * Error Risk: 78.3786%)
