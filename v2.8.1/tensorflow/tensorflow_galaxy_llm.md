...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.47; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 44%, Data / Markup / Trivial 19%, Large Core Modules 14%, Interface Declarations Files 7%, Many-Argument Workhorses Files 5%
...
| connectivity | 31437 | 5074 | 3 | `tensorflow/tools/ci_build/osx/arm64/tensorflow_metal_plugin_test.py` |
...
| dead_code | 28237 | 5558 | 3 | `tensorflow/tools/ci_build/osx/arm64/tensorflow_metal_plugin_test.py` |
...
- **Composition Archetype:** `Interface Declarations Files` (z +0.89)
...
- **Heaviest Functions:** `testPSPreemptionErrorType` (Type Conversions, Impact: 14.9), `testFetchFromPSAfterWorkerFailure` (Compute Cores, Impact: 12.2), `testWorkerPreemptionErrorType` (Type Conversions, Impact: 11.6)

### 2. `tensorflow/python/keras/optimizer_v2/rmsprop.py` (PYTHON) -> Cumulative Risk: **723.45**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.98)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 26.8), `_resource_apply_sparse` (Many-Argument Workhorses, Impact: 24.9), `_resource_apply_dense` (Many-Argument Workhorses, Impact: 20.1)
...
- **Heaviest Functions:** `_save` (Many-Argument Workhorses, Impact: 23.7), `__init__` (Many-Argument Workhorses, Impact: 23.6), `end` (Compute Cores, Impact: 12.9)
...
- **Heaviest Functions:** `ProcessRunner::Run` (Many-Argument Workhorses, Impact: 66.3), `ProcessRunner::Init` (I/O & Config Routines, Impact: 23.6), `ProcessRunner::RunInprocess` (Many-Argument Workhorses, Impact: 20.7)
...
- **Heaviest Functions:** `call` (Many-Argument Workhorses, Impact: 103.3), `call` (Many-Argument Workhorses, Impact: 65.6), `__call__` (Many-Argument Workhorses, Impact: 49.8)
...
- **Heaviest Functions:** `dtensor_reduce` (Many-Argument Workhorses, Impact: 32.0), `convert_inputs_to_dtensor` (Defensive Guards, Impact: 18.7), `convert_per_replica_to_dtensor` (Many-Argument Workhorses, Impact: 12.2)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 20.5), `__init__` (Many-Argument Workhorses, Impact: 11.4), `build` (Compute Cores, Impact: 11.3)
...
- **Heaviest Functions:** `Timeline::GenerateGraphTimeline` (Compute Cores, Impact: 53.3), `ChromeTraceFormatter::EmitCounter` (Many-Argument Workhorses, Impact: 21.6), `Timeline::AllocateLanes` (I/O & Config Routines, Impact: 14.8)
...
- **Heaviest Functions:** `GrapplerItem::NodesToPreserve` (Compute Cores, Impact: 22.1), `GrapplerItem::AddDevice` (Compute Cores, Impact: 12.1), `GrapplerItem::AddDevices` (Compute Cores, Impact: 6.3)

### 10. `tensorflow/python/ops/sparse_grad.py` (PYTHON) -> Cumulative Risk: **671.34**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Encapsulated Accessors Files` (z +0.03)
...
- **Heaviest Functions:** `_SparseTensorDenseMatMulGrad` (Compute Cores, Impact: 35.5), `_SparseDenseCwiseMulOrDivGrad` (Many-Argument Workhorses, Impact: 7.6), `_SparseSparseMaximumGrad` (Encapsulated Accessors, Impact: 3.7)
...
**Top Internal Functions/Classes:**
  ...
  * `Delegate::PrepareOpsToDelegate` **(Compute Cores)** (Impact: 169.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 37.7)
...
**Top Internal Functions/Classes:**
  * `NewOperationParser` **(Many-Argument Workhorses)** (Impact: 196.4)
...
- **Risk Profile:** Cognitive Load (67.2456%), Tech Debt (15.9348%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 79.2)
...
**Top Internal Functions/Classes:**
  ...
  * `ArithmeticOptimizer::SimplifyArithmeticOps` **(Compute Cores)** (Impact: 77.2)
  ...
  * `TrySimplify` **(Compute Cores)** (Impact: 48.1)
**Contextual Mitigations & Amplifications:**
