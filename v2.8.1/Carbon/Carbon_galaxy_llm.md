...
> **Composition Archetype:** `Flat Modular Platform` (z +1.93; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 62%, Data / Markup / Trivial 23%, Interface Declarations Files 6%, Tests & Verification Files 4%, Large Core Modules 1%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 30.1 | 0.4 | 0.0 | 0.0 |
...
- `__call` **(Compute Cores)** (@ `src/Carbon/CarbonPeriod.php`) -> Impact: **101.0** | LOC: 149
...
- `getOpenCollectiveSponsors` **(Compute Cores)** (@ `sponsors.php`) -> Impact: **77.2** | LOC: 223
...
- `set` **(Compute Cores)** (@ `src/Carbon/CarbonInterval.php`) -> Impact: **63.8** | LOC: 98
...
- **Heaviest Functions:** `diffForHumans` (Defensive Guards, Impact: 33.2), `diffInMonths` (Many-Argument Workhorses, Impact: 24.3), `calendar` (Defensive Guards, Impact: 16.8)
...
- **Heaviest Functions:** `trans` (Compute Cores, Impact: 9.1), `getFromCatalogue` (Defensive Guards, Impact: 9.0), `getPrivateProperty` (Callbacks & Closures, Impact: 2.0)

### 3. `src/Carbon/CarbonPeriod.php` (PHP) -> Cumulative Risk: **523.2**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__call` (Compute Cores, Impact: 101.0), `__construct` (Defensive Guards, Impact: 84.3), `initializeSerialization` (Defensive Guards, Impact: 36.0)

### 4. `src/Carbon/CarbonInterval.php` (PHP) -> Cumulative Risk: **521.18**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__construct` (Many-Argument Workhorses, Impact: 261.8), `forHumans` (Many-Argument Workhorses, Impact: 141.9), `fromString` (Compute Cores, Impact: 111.5)
...
- **Heaviest Functions:** `nextOrPreviousDay` (Compute Cores, Impact: 9.3), `change` (Callbacks & Closures, Impact: 6.5), `nthOfQuarter` (Type Conversions, Impact: 5.6)

### 6. `src/Carbon/Traits/Units.php` (PHP) -> Cumulative Risk: **510.45**
...
- **Heaviest Functions:** `addUTCUnit` (Many-Argument Workhorses, Impact: 64.2), `addUnit` (Many-Argument Workhorses, Impact: 56.2), `add` (Many-Argument Workhorses, Impact: 21.7)
...
- **Heaviest Functions:** `fromName` (Compute Cores, Impact: 22.4), `singular` (Interface Declarations, Impact: 4.8), `plural` (Interface Declarations, Impact: 4.8)
...
- **Heaviest Functions:** `toPeriod` (Many-Argument Workhorses, Impact: 19.3), `toISOString` (Compute Cores, Impact: 10.5), `format` (Defensive Guards, Impact: 9.3)

### 9. `src/Carbon/Traits/Creator.php` (PHP) -> Cumulative Risk: **494.88**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `create` (Many-Argument Workhorses, Impact: 62.3), `rawCreateFromFormat` (Many-Argument Workhorses, Impact: 53.2), `createFromIsoFormat` (Many-Argument Workhorses, Impact: 48.2)

### 10. `sponsors.php` (PHP) -> Cumulative Risk: **494.07**
...
- **Heaviest Functions:** `getOpenCollectiveSponsors` (Compute Cores, Impact: 77.2), `getMaxHistoryMonthsByAmount` (Interface Declarations, Impact: 4.8), `getHtmlAttribute` (Type Conversions, Impact: 1.8)
...
**Top Internal Functions/Classes:**
  * `__call` **(Compute Cores)** (Impact: 101.0)
  ...
  * `parseIso8601` **(Compute Cores)** (Impact: 21.1)
...
**Top Internal Functions/Classes:**
  ...
  * `testDiffInMonthsWithTimezone` **(Tests & Verification)** (Impact: 3.0)
  * `testDiffInHoursFilteredWorkHoursPerWeek` **(Callbacks & Closures)** (Impact: 2.5)
  * `testDiffForHumansWithMagicMethods` **(Tests & Verification)** (Impact: 2.5)
  * `testDiffWithTimezones` **(Tests & Verification)** (Impact: 2.5)
...
**Top Internal Functions/Classes:**
  ...
  * `testTimeZoneOfUnserialized` **(Tests & Verification)** (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `testDiffInHoursFilteredWorkHoursPerWeek` **(Callbacks & Closures)** (Impact: 2.5)
  * `testDiffForHumansWithMagicMethods` **(Tests & Verification)** (Impact: 2.5)
  * `testDiffInHoursWithTimezones` **(Tests & Verification)** (Impact: 2.0)
  * `testDiffForHumansWithOptions` **(Tests & Verification)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `cleanWordFromTranslationString` **(Compute Cores)** (Impact: 30.3)
...
**Top Internal Functions/Classes:**
  * `compileDoc` **(Compute Cores)** (Impact: 34.0)
...
**Top Internal Functions/Classes:**
  ...
  * `isStartOfDay` **(Compute Cores)** (Impact: 15.9)
...
**Top Internal Functions/Classes:**
  * `testIs` **(Tests & Verification)** (Impact: 4.8)
    * *Intent:* #[TestWith([null])] #[TestWith(['2025-03-31 12:00'])]
  * `testIsSameMonthOfSameYear` **(Tests & Verification)** (Impact: 4.2)
  * `testHasFormatWithModifiers` **(Tests & Verification)** (Impact: 3.9)
  * `testIsFebruary29` **(Tests & Verification)** (Impact: 3.4)
    * *Intent:* #[TestWith([null])] #[TestWith(['2024-02-29 12:00'])] #[TestWith(['2024-03-01 12:00'])] #[TestWith([...
  * `testHasFormatWithSingleLetter` **(Tests & Verification)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `acceptClosuresFrom` **(Defensive Guards)** (Impact: 16.5)
...
**Top Internal Functions/Classes:**
  ...
  * `setDefaultTimezone` **(Defensive Guards)** (Impact: 15.1)
  * `matchFormatPattern` **(Compute Cores)** (Impact: 14.8)
...
**Top Internal Functions/Classes:**
  ...
  * `testIsSameMonth` **(Tests & Verification)** (Impact: 2.5)
  * `testIsSameMonthTrueWithDateTime` **(Tests & Verification)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `add` **(Many-Argument Workhorses)** (Impact: 21.7)
...
**Top Internal Functions/Classes:**
  * `testEnums` **(Tests & Verification)** (Impact: 7.5)
  ...
  * `testInstance` **(Tests & Verification)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `testChangeStartDateDuringIteration` **(Tests & Verification)** (Impact: 5.5)
  * `testSkip` **(I/O & Config Routines)** (Impact: 5.5)
  * `testChangeEndDateDuringIteration` **(I/O & Config Routines)** (Impact: 5.4)
  * `testKeepIncreasingRecurrencesDuringIteration` **(Tests & Verification)** (Impact: 5.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `compareChunkLists` **(Defensive Guards)** (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `testShortMonthNameInFormat` **(Tests & Verification)** (Impact: 3.6)
  * `testFallbackLocales` **(I/O & Config Routines)** (Impact: 3.5)
  * `testSetLocaleWithMalformedLocale` **(Interface Declarations)** (Impact: 3.3)
...
**Top Internal Functions/Classes:**
  * `testFluentSetters` **(Tests & Verification)** (Impact: 9.7)
  * `testInvertDateInterval` **(Tests & Verification)** (Impact: 4.2)
  * `testSetDateClass` **(Tests & Verification)** (Impact: 3.4)
  * `testToggleOptionsOnAndOff` **(Tests & Verification)** (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `getOpenCollectiveSponsors` **(Compute Cores)** (Impact: 77.2)
  * `getMaxHistoryMonthsByAmount` **(Interface Declarations)** (Impact: 4.8)
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `testMillenniumGetter` **(Tests & Verification)** (Impact: 2.0)
  * `testQuarterFirstOfMonth` **(Annotated Framework Methods)** (Impact: 2.0)
  * `testQuarterMiddleOfMonth` **(Annotated Framework Methods)** (Impact: 2.0)
  * `testQuarterLastOfMonth` **(Annotated Framework Methods)** (Impact: 2.0)
  * `testGetUtcTrue` **(Tests & Verification)** (Impact: 2.0)
**Structural Signatures (Net Mitigated Signals):**
