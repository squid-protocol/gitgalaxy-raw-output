...
> **File Composition:** Data / Markup / Trivial 38%, Defensive Guards Files 12%, Large Core Modules 12%, Declarative / Non-Code 10%, Generic / Templated Code Files 9%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.9 | 42.6 | 53.4 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 41.4 | 14.3 | 0.0 |
...
| connectivity | 52418 | 5194 | 16 | `task-sdk/tests/task_sdk/execution_time/test_task_runner.py` |
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.52)
...
- **Heaviest Functions:** `expand_kwargs` (Defensive Guards, Impact: 25.8), `_create_task_group` (Many-Argument Workhorses, Impact: 10.1), `expand` (Generic / Templated Code, Impact: 6.0)
...
- **Heaviest Functions:** `list_pipelines` (Many-Argument Workhorses, Impact: 26.6), `list_jobs` (Many-Argument Workhorses, Impact: 23.3), `get_run_tasks` (Generic / Templated Code, Impact: 9.9)
...
- **Heaviest Functions:** `is_safe_url` (Compute Cores, Impact: 26.1), `inner` (Many-Argument Workhorses, Impact: 21.3), `inner` (Many-Argument Workhorses, Impact: 21.2)

### 4. `providers/microsoft/azure/src/airflow/providers/microsoft/azure/hooks/synapse.py` (PYTHON) -> Cumulative Risk: **748.5**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.35)
...
- **Heaviest Functions:** `wait_for_pipeline_run_status` (Many-Argument Workhorses, Impact: 16.9), `wait_for_job_run_status` (Many-Argument Workhorses, Impact: 14.1), `get_async_conn` (Compute Cores, Impact: 13.2)
...
- **Heaviest Functions:** `_read_frame` (Compute Cores, Impact: 17.2), `asend` (Defensive Guards, Impact: 8.6), `send` (Defensive Guards, Impact: 8.1)
...
- **Heaviest Functions:** `run` (Compute Cores, Impact: 23.6), `run` (Defensive Guards, Impact: 13.7), `run` (Defensive Guards, Impact: 10.6)
...
- **Heaviest Functions:** `request_information` (Many-Argument Workhorses, Impact: 41.0), `default_pagination` (Generic / Templated Code, Impact: 23.3), `paginated_run` (Many-Argument Workhorses, Impact: 21.6)
...
- **Heaviest Functions:** `list_keys` (Many-Argument Workhorses, Impact: 52.9), `is_keys_unchanged_async` (Many-Argument Workhorses, Impact: 38.7), `copy_object` (Many-Argument Workhorses, Impact: 35.9)

### 9. `providers/google/src/airflow/providers/google/cloud/hooks/gen_ai.py` (PYTHON) -> Cumulative Risk: **717.39**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.60)
...
- **Heaviest Functions:** `upload_file` (Defensive Guards, Impact: 7.2), `supervised_fine_tuning_train` (Many-Argument Workhorses, Impact: 7.0), `create_embeddings` (Many-Argument Workhorses, Impact: 5.9)
...
- **Heaviest Functions:** `_get_task_instances` (Many-Argument Workhorses, Impact: 24.1), `run` (Compute Cores, Impact: 21.8), `run` (Compute Cores, Impact: 19.6)
...
**Top Internal Functions/Classes:**
  ...
  * `run_task` **(Many-Argument Workhorses)** (Impact: 64.8)
...
**Top Internal Functions/Classes:**
  * `_build_detail_lines` **(Compute Cores)** (Impact: 152.6)
    * *Intent:* """Build the full list of detail lines for a PR entry (not truncated)."""
  * `run_interactive` **(Compute Cores)** (Impact: 148.4)
  * `_build_pr_table` **(Compute Cores)** (Impact: 126.1)
  ...
  * `_build_header` **(Compute Cores)** (Impact: 52.7)
...
**Top Internal Functions/Classes:**
  ...
  * `test_schedulable_task_exist_when_rerun_removed_upstream_mapped_task` **(Compute Cores)** (Impact: 19.8)
...
**Top Internal Functions/Classes:**
  * `validate_deserialized_task` **(Defensive Guards)** (Impact: 44.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_get_providers_test_types_to_run` **(Compute Cores)** (Impact: 36.9)
  * `provider_dependency_bump` **(Compute Cores)** (Impact: 34.9)
