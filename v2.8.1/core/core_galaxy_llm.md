...
|---|---|---|
| Modularity | 0.3506 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +2.43; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 35%, Generic / Templated Code Files 29%, Defensive Guards Files 17%, Large Core Modules 6%, Many-Argument Workhorses Files 5%
...
| connectivity | 106536 | 14392 | 11 | `tests/test_config_entries.py` |
...
- `_create_or_update_task` **(Compute Cores)** (@ `homeassistant/components/habitica/services.py`) -> Impact: **145.6** | LOC: 282
...
- **Heaviest Functions:** `async_initialize_triggers` (Many-Argument Workhorses, Impact: 55.1), `_register_trigger_platform` (Many-Argument Workhorses, Impact: 30.7), `async_get_all_descriptions` (Compute Cores, Impact: 25.1)

### 2. `homeassistant/components/open_router/config_flow.py` (PYTHON) -> Cumulative Risk: **761.69**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -1.02)
...
- **Heaviest Functions:** `async_step_init` (Many-Argument Workhorses, Impact: 23.4), `async_step_init` (Defensive Guards, Impact: 12.7), `async_step_user` (Defensive Guards, Impact: 6.7)

### 3. `homeassistant/components/frontend/storage.py` (PYTHON) -> Cumulative Risk: **758.1**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.94)
...
- **Heaviest Functions:** `websocket_subscribe_user_data` (Generic / Templated Code, Impact: 7.7), `websocket_get_user_data` (Generic / Templated Code, Impact: 7.3), `async_set_item` (Generic / Templated Code, Impact: 6.4)
...
- **Heaviest Functions:** `async_step_model` (Many-Argument Workhorses, Impact: 61.3), `async_step_init` (Many-Argument Workhorses, Impact: 35.3), `async_step_init` (Many-Argument Workhorses, Impact: 13.5)
...
- **Heaviest Functions:** `async_create_fix_flow` (Defensive Guards, Impact: 21.7), `async_manage_ble_scanner_firmware_unsupported_issue` (Many-Argument Workhorses, Impact: 12.2), `async_manage_open_wifi_ap_issue` (Many-Argument Workhorses, Impact: 12.2)

### 6. `homeassistant/components/wilight/switch.py` (PYTHON) -> Cumulative Risk: **724.11**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -1.21)
...
- **Heaviest Functions:** `extra_state_attributes` (Compute Cores, Impact: 15.7), `async_setup_entry` (Defensive Guards, Impact: 10.9), `async_set_trigger` (Generic / Templated Code, Impact: 10.5)
...
- **Heaviest Functions:** `_async_form_step` (Many-Argument Workhorses, Impact: 34.1), `_show_next_step` (Many-Argument Workhorses, Impact: 27.4), `_update_and_remove_omitted_optional_keys` (Many-Argument Workhorses, Impact: 25.9)

### 8. `homeassistant/helpers/condition.py` (PYTHON) -> Cumulative Risk: **721.77**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.95)
...
- **Heaviest Functions:** `async_numeric_state` (Many-Argument Workhorses, Impact: 79.7), `time` (Many-Argument Workhorses, Impact: 67.5), `state` (Many-Argument Workhorses, Impact: 51.2)
...
- **Heaviest Functions:** `async_call_action_from_config` (Many-Argument Workhorses, Impact: 19.2), `async_get_actions` (Compute Cores, Impact: 15.6), `async_get_action_capabilities` (Compute Cores, Impact: 14.7)

### 10. `homeassistant/components/homekit_controller/switch.py` (PYTHON) -> Cumulative Risk: **717.94**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.76)
...
- **Heaviest Functions:** `async_setup_entry` (Generic / Templated Code, Impact: 7.8), `extra_state_attributes` (Generic / Templated Code, Impact: 6.5), `async_add_characteristic` (Generic / Templated Code, Impact: 3.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_statistic_during_period_hole` **(Defensive Guards)** (Impact: 36.9)
...
**Top Internal Functions/Classes:**
  * `_assert_extract_from_target_command_result` **(Type Conversions)** (Impact: 25.1)
  ...
  * `_apply_entities_changes` **(Defensive Guards)** (Impact: 16.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000294 | `Ripple Effect (Closeness):` 0.247744
...
**Top Internal Functions/Classes:**
  * `test_async_get_hass_can_be_called` **(Defensive Guards)** (Impact: 11.6)
  ...
  * `test_chained_logging_hits_log_timeout` **(Defensive Guards)** (Impact: 6.5)
  * `test_chained_logging_misses_log_timeout` **(Defensive Guards)** (Impact: 6.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_assert_entry` **(Defensive Guards)** (Impact: 20.8)
...
**Top Internal Functions/Classes:**
  ...
  * `consume_progress_flow` **(Many-Argument Workhorses)** (Impact: 10.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_subscribe_conditions` **(Defensive Guards)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.001134
...
- `homeassistant/core.py` -> **Severity: 51.157** (Embedded: 0.532 * Error Risk: 96.1546%)
- `homeassistant/util/logging.py` -> **Severity: 34.344** (Embedded: 0.3447 * Error Risk: 99.6285%)
- `homeassistant/helpers/typing.py` -> **Severity: 28.901** (Embedded: 0.3119 * Error Risk: 92.6705%)
- `homeassistant/util/async_.py` -> **Severity: 26.82** (Embedded: 0.295 * Error Risk: 90.9162%)
- `homeassistant/helpers/entity.py` -> **Severity: 26.765** (Embedded: 0.293 * Error Risk: 91.334%)
