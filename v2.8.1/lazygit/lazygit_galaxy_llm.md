...
> **Composition Archetype:** `Flat Modular Platform` (z +1.85; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 48%, Callbacks & Closures Files 27%, Large Core Modules 6%, Data / Markup / Trivial 4%, I/O & Config Routines Files 3%
...
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.00)
...
- **Heaviest Functions:** `onNewRepo` (Many-Argument Workhorses, Impact: 40.3), `Run` (Compute Cores, Impact: 21.7), `initialContext` (Compute Cores, Impact: 18.4)

### 2. `pkg/snake/snake.go` (GO) -> Cumulative Risk: **693.83**
- **Archetype:** `file_cluster_9` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.61)
...
- **Heaviest Functions:** `tick` (Compute Cores, Impact: 20.1), `newDirection` (Compute Cores, Impact: 16.1), `gameLoop` (I/O & Config Routines, Impact: 8.2)

### 3. `pkg/gui/background.go` (GO) -> Cumulative Risk: **681.54**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -1.02)
...
- **Heaviest Functions:** `goEvery` (Callbacks & Closures, Impact: 15.7), `startBackgroundRoutines` (I/O & Config Routines, Impact: 12.4), `startBackgroundFetch` (Callbacks & Closures, Impact: 4.5)

### 4. `pkg/gui/presentation/graph/cell.go` (GO) -> Cumulative Risk: **668.34**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.92)
...
- **Heaviest Functions:** `getBoxDrawingChars` (Compute Cores, Impact: 180.7), `render` (Compute Cores, Impact: 14.4), `cachedSprint` (Compute Cores, Impact: 11.5)

### 5. `pkg/gui/context/base_context.go` (GO) -> Cumulative Risk: **655.48**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.20)
...
- **Heaviest Functions:** `AddOnDoubleClickFn` (State Mutators, Impact: 4.6), `AddOnClickFn` (State Mutators, Impact: 4.6), `AddOnClickFocusedMainViewFn` (State Mutators, Impact: 4.6)
...
- **Heaviest Functions:** `getConflictedCommitImpl` (Many-Argument Workhorses, Impact: 43.0), `getHydratedTodoCommits` (Many-Argument Workhorses, Impact: 24.9), `GetCommits` (Compute Cores, Impact: 22.4)
...
- **Heaviest Functions:** `blameAddedLines` (Compute Cores, Impact: 40.4), `parseDiff` (Compute Cores, Impact: 27.9), `HandleFindBaseCommitForFixupPress` (I/O & Config Routines, Impact: 20.2)
...
- **Heaviest Functions:** `start` (Callbacks & Closures, Impact: 9.9), `WithInlineStatus` (Callbacks & Closures, Impact: 9.8), `stop` (Compute Cores, Impact: 7.3)

### 9. `pkg/gui/patch_exploring/state.go` (GO) -> Cumulative Risk: **630.62**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.56)
...
- **Heaviest Functions:** `NewState` (Many-Argument Workhorses, Impact: 52.9), `SelectPreviousHunk` (I/O & Config Routines, Impact: 8.9), `selectionRangeForCurrentBlockOfChanges` (I/O & Config Routines, Impact: 8.2)
...
- **Heaviest Functions:** `GetDisplayStrings` (Compute Cores, Impact: 17.2), `OnMenuPress` (Compute Cores, Impact: 11.1), `GetNonModelItems` (I/O & Config Routines, Impact: 9.0)
...
**Top Internal Functions/Classes:**
  ...
  * `JSONSchemaExtend` **(Callbacks & Closures)** (Impact: 1.8)
...
**Top Internal Functions/Classes:**
  ...
  * `initialContext` **(Compute Cores)** (Impact: 18.4)
  * `checkForChangedConfigsThatDontAutoReload` **(Compute Cores)** (Impact: 12.9)
...
**Top Internal Functions/Classes:**
  * `pressWithLock` **(Compute Cores)** (Impact: 31.4)
...
**Top Internal Functions/Classes:**
  ...
  * `getBisectStatus` **(Many-Argument Workhorses)** (Impact: 28.3)
  * `getBisectStatusText` **(Compute Cores)** (Impact: 16.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `midRebaseMoveCommandEnabled` **(Compute Cores)** (Impact: 17.4)
    * *Intent:* // Ensures that if we are mid-rebase, we're only selecting commits that can be moved
  * `canDropCommits` **(Many-Argument Workhorses)** (Impact: 17.4)
  * `drop` **(Many-Argument Workhorses)** (Impact: 16.8)
  * `moveFixupCommitToOwnerStackedBranch` **(Compute Cores)** (Impact: 13.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `changeCustomCommandStreamAndOutputToOutputEnum` **(Compute Cores)** (Impact: 33.6)
  * `NewAppConfig` **(Many-Argument Workhorses)** (Impact: 22.5)
    * *Intent:* // NewAppConfig makes a new app config
  * `migrateUserConfig` **(Many-Argument Workhorses)** (Impact: 15.8)
...
**Top Internal Functions/Classes:**
  ...
  * `getRebasingCommits` **(Compute Cores)** (Impact: 21.3)
...
**Top Internal Functions/Classes:**
  ...
  * `selectionRangeForCurrentBlockOfChanges` **(I/O & Config Routines)** (Impact: 8.2)
...
**Top Internal Functions/Classes:**
  ...
  * `promptForBaseGithubRepo` **(Callbacks & Closures)** (Impact: 10.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `divergenceStr` **(Many-Argument Workhorses)** (Impact: 14.4)
  * `prColor` **(Compute Cores)** (Impact: 10.6)
  * `match` **(Callbacks & Closures)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 37 instances
...
**Top Internal Functions/Classes:**
  ...
  * `removeEmptyDirs` **(Defensive Guards)** (Impact: 20.7)
...
**Top Internal Functions/Classes:**
  ...
  * `fileNameAtDepth` **(Many-Argument Workhorses)** (Impact: 17.3)
  * `getColorForChangeStatus` **(Compute Cores)** (Impact: 12.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `viewUpstreamOptions` **(Compute Cores)** (Impact: 25.5)
  * `delete` **(Compute Cores)** (Impact: 16.0)
  * `createPullRequestMenu` **(Compute Cores)** (Impact: 13.5)
  * `withPrFgColor` **(Compute Cores)** (Impact: 12.8)
  * `withPrBgColor` **(Compute Cores)** (Impact: 12.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `render` **(Compute Cores)** (Impact: 14.4)
  * `cachedSprint` **(Compute Cores)** (Impact: 11.5)
  * `setRight` **(Callbacks & Closures)** (Impact: 5.5)
    * *Intent:* //nolint:unparam
  * `setLeft` **(Callbacks & Closures)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `assertLines` **(Compute Cores)** (Impact: 27.5)
...
**Top Internal Functions/Classes:**
  ...
  * `equalHashes` **(Encapsulated Accessors)** (Impact: 5.6)
...
**Top Internal Functions/Classes:**
  ...
  * `getMidSectionWeights` **(Compute Cores)** (Impact: 18.4)
  * `sidePanelChildren` **(Compute Cores)** (Impact: 18.0)
  * `splitMainPanelSideBySide` **(Compute Cores)** (Impact: 12.2)
...
**Top Internal Functions/Classes:**
  ...
  * `RewordCommit` **(Many-Argument Workhorses)** (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getCredentialPromptFn` **(Callbacks & Closures)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `TestGetWindowDimensions` **(Compute Cores)** (Impact: 12.1)
...
**Top Internal Functions/Classes:**
  * `blameAddedLines` **(Compute Cores)** (Impact: 40.4)
  ...
  * `removeFixupCommits` **(Compute Cores)** (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
