...
> **Composition Archetype:** `Flat Modular Platform` (z -0.68; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 25%, Data / Markup / Trivial 20%, Large Core Modules 12%, State Mutators Files 12%, Many-Argument Workhorses Files 11%
...
| connectivity | 13783 | 996 | 4 | `racket/src/bc/src/schpriv.h` |
...
| dead_code | 4555 | 1009 | 2 | `racket/src/ChezScheme/mats/foreign2.c` |
...
- **Composition Archetype:** `Interface Declarations Files` (z +0.90)
...
- **Heaviest Functions:** `rwlock_wrlock` (Compute Cores, Impact: 14.9), `rwlock_rdlock` (Compute Cores, Impact: 14.8), `mzrt_rwlock_wrlock_worker` (Compute Cores, Impact: 14.8)
...
- **Heaviest Functions:** `scheme_rational_divide` (Compute Cores, Impact: 36.8), `scheme_rational_eq` (Compute Cores, Impact: 30.7), `scheme_rational_round` (Compute Cores, Impact: 27.6)

### 3. `racket/src/ChezScheme/c/schlib.c` (C) -> Cumulative Risk: **680.5**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.00)
...
- **Heaviest Functions:** `S_call_help` (Many-Argument Workhorses, Impact: 39.6), `Ssymbol_to_string` (Compute Cores, Impact: 7.6), `S_put_arg` (Compute Cores, Impact: 6.3)
...
- **Heaviest Functions:** `gzfilebuf::open` (Compute Cores, Impact: 17.6), `gzfilebuf::attach` (Compute Cores, Impact: 17.6), `gzfilebuf::overflow` (Compute Cores, Impact: 15.7)
...
- **Heaviest Functions:** `open_write` (Many-Argument Workhorses, Impact: 107.1), `finish_unix_fd_creation` (Many-Argument Workhorses, Impact: 76.6), `open_read` (Many-Argument Workhorses, Impact: 58.2)
...
- **Heaviest Functions:** `SanitizeHTML` (Compute Cores, Impact: 406.3), `UpdateResults` (Compute Cores, Impact: 131.3), `Search` (Many-Argument Workhorses, Impact: 67.8)
...
- **Heaviest Functions:** `handle_signal` (Many-Argument Workhorses, Impact: 30.6), `do_error` (Many-Argument Workhorses, Impact: 26.4), `S_overflow` (Many-Argument Workhorses, Impact: 23.2)
...
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.94)
...
- **Heaviest Functions:** `check_types` (Many-Argument Workhorses, Impact: 67.2), `ppcfltest2` (Many-Argument Workhorses, Impact: 5.2), `ppcfltest` (Many-Argument Workhorses, Impact: 4.6)
...
- **Heaviest Functions:** `rktio_locale_recase` (Many-Argument Workhorses, Impact: 40.0), `init_iconv` (Compute Cores, Impact: 34.0), `rktio_system_language_country` (Compute Cores, Impact: 32.3)
...
**Top Internal Functions/Classes:**
  ...
  * `check_sleep` **(Compute Cores)** (Impact: 88.4)
  * `current_stats` **(Compute Cores)** (Impact: 84.8)
...
**Top Internal Functions/Classes:**
  ...
  * `zuo_main` **(Compute Cores)** (Impact: 110.8)
    * *Intent:* /*======================================================================*/ /* main */ /*============...
  * `zuo_fd_open_output` **(Compute Cores)** (Impact: 69.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `do_chaperone_struct` **(Many-Argument Workhorses)** (Impact: 253.6)
    * *Intent:* /**********************************************************************/
  * `make_struct_type` **(Compute Cores)** (Impact: 158.5)
  * `scheme_init_struct` **(Compute Cores)** (Impact: 112.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3e-06 | `Ripple Effect (Closeness):` 0.000237
...
**Top Internal Functions/Classes:**
  * `scheme_expt` **(Compute Cores)** (Impact: 136.8)
  * `scheme_init_number` **(Compute Cores)** (Impact: 98.1)
  ...
  * `atan_prim` **(Compute Cores)** (Impact: 89.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `do_GC_gcollect` **(Compute Cores)** (Impact: 129.3)
    * *Intent:* #endif
...
**Top Internal Functions/Classes:**
  ...
  * `make_output_port` **(Compute Cores)** (Impact: 126.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.00182
...
- `racket/src/ChezScheme/c/types.h` -> **Severity: 0.994** (Embedded: 0.0142 * Error Risk: 70.0311%)
- `racket/src/bc/include/scheme.h` -> **Severity: 0.874** (Embedded: 0.0144 * Error Risk: 60.9076%)
- `racket/src/bc/foreign/libffi/include/ffi_common.h` -> **Severity: 0.826** (Embedded: 0.0111 * Error Risk: 74.2466%)
- `racket/src/bc/src/schpriv.h` -> **Severity: 0.81** (Embedded: 0.0149 * Error Risk: 54.3294%)
- `racket/src/bc/src/schexn.h` -> **Severity: 0.764** (Embedded: 0.0096 * Error Risk: 79.6655%)
