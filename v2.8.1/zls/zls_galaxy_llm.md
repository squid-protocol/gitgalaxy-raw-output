...
> **Composition Archetype:** `Hub-Coupled App` (z +1.45; from the repo's file-archetype mix)
> **File Composition:** Defensive Guards Files 29%, State Mutators Files 19%, Data / Markup / Trivial 18%, Large Core Modules 15%, Declarative / Non-Code 14%
...
- `resolveTypeOfNodeUncached` **(Defensive Guards)** (@ `src/analysis.zig`) -> Impact: **442.6** | LOC: 1092
...
- `writeNodeTokens` **(Defensive Guards)** (@ `src/features/semantic_tokens.zig`) -> Impact: **193.1** | LOC: 675
...
- **Heaviest Functions:** `lastToken` (Many-Argument Workhorses, Impact: 126.2), `indexOfBreakTarget` (Defensive Guards, Impact: 45.8), `fullPtrTypeComponents` (Compute Cores, Impact: 37.1)
...
- **Heaviest Functions:** `SegmentedList` (Many-Argument Workhorses, Impact: 82.4), `BaseIterator` (Compute Cores, Impact: 24.1), `shrinkCapacity` (Many-Argument Workhorses, Impact: 11.6)
...
- **Heaviest Functions:** `isVarDeclAlias` (Compute Cores, Impact: 15.0), `mergeIntersection` (Many-Argument Workhorses, Impact: 13.5), `next` (Compute Cores, Impact: 11.1)
...
- **Heaviest Functions:** `resolveTypeOfNodeUncached` (Defensive Guards, Impact: 442.6), `resolveExpressionTypeFromAncestors` (Many-Argument Workhorses, Impact: 211.3), `getFieldAccessType` (Many-Argument Workhorses, Impact: 167.3)

### 5. `src/configuration.zig` (ZIG) -> Cumulative Risk: **500.82**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.75)
...
- **Heaviest Functions:** `eql` (Compute Cores, Impact: 21.0), `free` (Defensive Guards, Impact: 20.9), `deinit` (State Mutators, Impact: 6.2)
...
- **Heaviest Functions:** `renderLineDiff` (Defensive Guards, Impact: 9.8), `printDocumentScope` (State Mutators, Impact: 8.7), `printLine` (Compute Cores, Impact: 6.0)

### 7. `src/features/completions.zig` (ZIG) -> Cumulative Risk: **494.11**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.95)
...
- **Heaviest Functions:** `getSwitchOrStructInitContext` (Many-Argument Workhorses, Impact: 122.7), `completeFileSystemStringLiteral` (Defensive Guards, Impact: 119.8), `declToCompletion` (Defensive Guards, Impact: 103.1)
...
- **Heaviest Functions:** `getDiscardLoc` (Defensive Guards, Impact: 29.7), `parse` (Defensive Guards, Impact: 22.4), `getParamRemovalRange` (Compute Cores, Impact: 17.4)
...
- **Heaviest Functions:** `TracyAllocator` (Defensive Guards, Impact: 20.9), `resizeFn` (Defensive Guards, Impact: 10.4), `remapFn` (Defensive Guards, Impact: 10.4)
...
- **Heaviest Functions:** `normalizePercentEncoded` (Many-Argument Workhorses, Impact: 15.8), `isFileScheme` (Compute Cores, Impact: 6.0), `isSchemeChar` (Interface Declarations, Impact: 4.5)
...
**Top Internal Functions/Classes:**
  * `resolveTypeOfNodeUncached` **(Defensive Guards)** (Impact: 442.6)
...
**Top Internal Functions/Classes:**
  ...
  * `invalidateBuildFileWorker` **(Defensive Guards)** (Impact: 32.4)
...
**Top Internal Functions/Classes:**
  * `writeNodeTokens` **(Defensive Guards)** (Impact: 193.1)
...
**Top Internal Functions/Classes:**
  ...
  * `shrinkCapacity` **(Many-Argument Workhorses)** (Impact: 11.6)
...
**Top Internal Functions/Classes:**
  * `sourceIndexToTokenIndex` **(Compute Cores)** (Impact: 28.8)
  ...
  * `tokenToLoc` **(Compute Cores)** (Impact: 15.8)
  * `multilineLocAtIndex` **(Many-Argument Workhorses)** (Impact: 15.3)
...
**Top Internal Functions/Classes:**
  * `testCompletionWithOptions` **(Defensive Guards)** (Impact: 103.5)
...
**Top Internal Functions/Classes:**
  * `referenceNode` **(Defensive Guards)** (Impact: 83.2)
...
**Top Internal Functions/Classes:**
  * `TracyAllocator` **(Defensive Guards)** (Impact: 20.9)
  * `resizeFn` **(Defensive Guards)** (Impact: 10.4)
...
**Top Internal Functions/Classes:**
  * `isVarDeclAlias` **(Compute Cores)** (Impact: 15.0)
  ...
  * `appendToBucket` **(Many-Argument Workhorses)** (Impact: 6.7)
...
**Top Internal Functions/Classes:**
  ...
  * `from` **(Interface Declarations)** (Impact: 4.8)
