...
> **Composition Archetype:** `Flat Modular Platform` (z -0.82; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 22%, Declarative / Non-Code 18%, State Mutators Files 16%, Large Core Modules 14%, Defensive Guards Files 8%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 54.9 | 96.6 | 100.0 |
...
| connectivity | 24922 | 3783 | 11 | `src/modules/MouseWithoutBorders/App/Class/NativeMethods.cs` |
...
- `RunPanoramaStitchSelfTest` **(Compute Cores)** (@ `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp`) -> Impact: **259.4** | LOC: 1328
...
- **Heaviest Functions:** `DragDropStep05Ex` (Compute Cores, Impact: 13.8), `DragDropStep04` (I/O & Config Routines, Impact: 12.3), `DragDropStep01` (Compute Cores, Impact: 9.8)
...
- **Heaviest Functions:** `LightSwitchSettings::LoadSettings` (Compute Cores, Impact: 36.1), `LightSwitchSettings::InitFileWatcher` (I/O & Config Routines, Impact: 9.8), `LightSwitchSettings::~LightSwitchSettings` (State Mutators, Impact: 5.3)
...
- **Heaviest Functions:** `ModeSelector_SelectionChanged` (Compute Cores, Impact: 18.4), `ViewModel_PropertyChanged` (Many-Argument Workhorses, Impact: 17.6), `CityAutoSuggestBox_TextChanged` (Defensive Guards, Impact: 11.2)

### 4. `src/modules/cmdpal/Microsoft.CmdPal.UI.ViewModels/TopLevelCommandManager.cs` (CSHARP) -> Cumulative Risk: **715.86**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `AppendCommandsWhenReadyAsync` (Defensive Guards, Impact: 40.3), `TryLoadCommandsAsync` (Defensive Guards, Impact: 32.6), `RegisterAndLoadCommandsAsync` (Many-Argument Workhorses, Impact: 29.7)

### 5. `src/modules/cmdpal/Microsoft.CmdPal.UI/Helpers/Icons/IconLoaderService.cs` (CSHARP) -> Cumulative Risk: **714.88**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
...
- **Heaviest Functions:** `EnqueueLoad` (Many-Argument Workhorses, Impact: 12.6), `LoadIconCoreAsync` (Many-Argument Workhorses, Impact: 12.1), `ProcessQueueAsync` (Interface Declarations, Impact: 11.7)

### 6. `src/modules/ZoomIt/ZoomIt/Zoomit.cpp` (CPP) -> Cumulative Risk: **713.97**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `MainWndProc` (Many-Argument Workhorses, Impact: 755.7), `OptionsProc` (Many-Argument Workhorses, Impact: 476.8), `LiveZoomWndProc` (Many-Argument Workhorses, Impact: 331.6)
...
- **Heaviest Functions:** `PrepareToSwitchToMachine` (Many-Argument Workhorses, Impact: 31.7), `MouseEvent` (Many-Argument Workhorses, Impact: 26.4), `KeybdEvent` (Defensive Guards, Impact: 8.5)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `OnInstallProgress` (Compute Cores, Impact: 29.7), `OnUninstallProgress` (Compute Cores, Impact: 22.1), `Invoke` (I/O & Config Routines, Impact: 9.4)
...
- **Heaviest Functions:** `Previewer_PropertyChanged` (Defensive Guards, Impact: 11.3), `UpdateTooltipAsync` (Defensive Guards, Impact: 11.2), `KeyboardAccelerator_Space_Invoked` (Compute Cores, Impact: 8.0)
...
- **Heaviest Functions:** `LanguagesComboBox_SelectionChanged` (Defensive Guards, Impact: 53.2), `KeyPressed` (Compute Cores, Impact: 42.6), `RegionClickCanvas_MouseUp` (Many-Argument Workhorses, Impact: 23.3)
...
- **Risk Profile:** Cognitive Load (92.6937%), Tech Debt (8.6757%)
**Top Internal Functions/Classes:**
  ...
  * `RunPanoramaStitchSelfTest` **(Compute Cores)** (Impact: 259.4)
...
**Top Internal Functions/Classes:**
  ...
  * `OnKeyDown` **(Compute Cores)** (Impact: 84.7)
    * *Intent:* /// <inheritdoc/>
  * `ReadFormattedData` **(Compute Cores)** (Impact: 56.9)
  * `CalculateDataColumnCharWidth` **(Compute Cores)** (Impact: 55.4)
...
**Top Internal Functions/Classes:**
  ...
  * `Form_DragOver` **(Compute Cores)** (Impact: 27.9)
...
**Top Internal Functions/Classes:**
  ...
  * `HandleControlKeyword` **(Compute Cores)** (Impact: 31.2)
...
**Top Internal Functions/Classes:**
  ...
  * `OnCustomActionsCollectionChanged` **(Defensive Guards)** (Impact: 24.7)
  * `AdvancedPasteViewModel` **(Defensive Guards)** (Impact: 23.0)
  * `MigrateLegacyAIEnablement` **(I/O & Config Routines)** (Impact: 21.1)
  * `OnPasteAIProvidersCollectionChanged` **(Defensive Guards)** (Impact: 20.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `UninstallPackageIdentityMSIXCA` **(Compute Cores)** (Impact: 21.7)
...
**Top Internal Functions/Classes:**
  ...
  * `D2DOverlayWindow::show` **(Compute Cores)** (Impact: 36.2)
...
**Top Internal Functions/Classes:**
  ...
  * `ShowToolTip` **(Many-Argument Workhorses)** (Impact: 20.9)
  * `SendOrReceiveARandomDataBlockPerInitialIV` **(Defensive Guards)** (Impact: 20.7)
...
- **Risk Profile:** Cognitive Load (91.8198%), Tech Debt (38.6861%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `WndProc` **(Compute Cores)** (Impact: 46.2)
    * *Intent:* // private bool checkClipboard = false;
  * `PaintMyNameOnDesktop` **(I/O & Config Routines)** (Impact: 26.6)
  * `ShowMessageOnLogonDesktop` **(Compute Cores)** (Impact: 18.3)
  * `ChangeIcon` **(Compute Cores)** (Impact: 16.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  * `ParseRegistryFile` **(Compute Cores)** (Impact: 159.6)
...
**Top Internal Functions/Classes:**
  ...
  * `Dispose` **(State Mutators)** (Impact: 3.2)
...
**Top Internal Functions/Classes:**
  ...
  * `DelayRenderer` **(Defensive Guards)** (Impact: 21.6)
  * `Items_PreviewKeyDown` **(Defensive Guards)** (Impact: 20.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  * `MoveRight` **(Compute Cores)** (Impact: 67.6)
    * *Intent:* #endif
  * `MoveLeft` **(Compute Cores)** (Impact: 60.8)
  ...
  * `MoveUp` **(Compute Cores)** (Impact: 36.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `QueryResults` **(Compute Cores)** (Impact: 41.2)
  * `SetHotkey` **(Many-Argument Workhorses)** (Impact: 21.2)
  * `OpenResultsEvent` **(Compute Cores)** (Impact: 20.3)
  * `RegisterHotkey` **(Defensive Guards)** (Impact: 16.8)
  * `UpdateResultsListViewAfterQuery` **(Many-Argument Workhorses)** (Impact: 13.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `GetShortcutRemapCountByType` **(Compute Cores)** (Impact: 67.2)
