...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +1.11; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 76%, Large Core Modules 8%, State Mutators Files 6%, Declarative / Non-Code 4%, Callbacks & Closures Files 1%
...
- `pandocApp` **(Compute Cores)** (@ `wasm/index.js`) -> Impact: **369.4** | LOC: 1329
  * *Intent:* // Petite Vue app definition
- `inlineToMarkdown` **(Compute Cores)** (@ `src/Text/Pandoc/Writers/Markdown/Inline.hs`) -> Impact: **243.1** | LOC: 393
  * *Intent:* -- | Convert Pandoc inline element to markdown.
- `blockToMarkdown'` **(Compute Cores)** (@ `src/Text/Pandoc/Writers/Markdown.hs`) -> Impact: **213.9** | LOC: 398
...
- `inlineToHtml` **(Compute Cores)** (@ `src/Text/Pandoc/Writers/HTML.hs`) -> Impact: **131.2** | LOC: 269
...
- `buildOptions` **(Compute Cores)** (@ `wasm/index.js`) -> Impact: **118.9** | LOC: 218
...
- **Heaviest Functions:** `pandocApp` (Compute Cores, Impact: 369.4), `buildOptions` (Compute Cores, Impact: 118.9), `loadExampleFromBuffer` (Many-Argument Workhorses, Impact: 69.0)

### 2. `wasm/pandoc.js` (JAVASCRIPT) -> Cumulative Risk: **673.26**
...
- **Heaviest Functions:** `convert` (Many-Argument Workhorses, Impact: 24.8), `query` (I/O & Config Routines, Impact: 3.8), `addFile` (Parameter Forwarders, Impact: 2.2)

### 3. `src/Text/Pandoc/Sources.hs` (HASKELL) -> Cumulative Risk: **497.97**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z -0.64)
...
- **Heaviest Functions:** `ensureFinalNewlines` (Defensive Guards, Impact: 9.4), `updateSourcePos` (Callbacks & Closures, Impact: 8.5), `satisfy` (Interface Declarations, Impact: 4.5)

### 4. `tools/extract-changes.lua` (LUA) -> Cumulative Risk: **496.23**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.30)
...
- **Heaviest Functions:** `Pandoc` (Compute Cores, Impact: 12.1)
...
- **Heaviest Functions:** `Div` (Callbacks & Closures, Impact: 9.4), `__global_context__` (I/O & Config Routines, Impact: 1.5)
...
- **Heaviest Functions:** `Anonymous_Block` (Compute Cores, Impact: 7.7), `Anonymous_Block` (Compute Cores, Impact: 6.5), `add_transitive_deps` (Compute Cores, Impact: 6.3)

### 7. `src/Text/Pandoc/Writers/Muse.hs` (HASKELL) -> Cumulative Risk: **473.26**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.64)
...
- **Heaviest Functions:** `inlineToMuse` (Callbacks & Closures, Impact: 65.7), `blockToMuse` (Callbacks & Closures, Impact: 24.6), `simpleTable` (Compute Cores, Impact: 19.2)

### 8. `src/Text/Pandoc/Writers/LaTeX.hs` (HASKELL) -> Cumulative Risk: **469.42**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.43)
- **Magnitude:** 723.92 | **LOC:** 1512 | **CtrlFlow:** 18.9% | **Authorship Centralization:** 55.6%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (86.7347%), Verification (80.0%), Safety Score (53.8648%)
- **Heaviest Functions:** `blockToLaTeX` (Compute Cores, Impact: 134.5), `inlineToLaTeX` (Compute Cores, Impact: 131.6), `pandocToLaTeX` (Compute Cores, Impact: 74.8)

### 9. `src/Text/Pandoc/Chunks.hs` (HASKELL) -> Cumulative Risk: **455.35**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.80)
...
- **Heaviest Functions:** `fixInternalReferences` (Callbacks & Closures, Impact: 24.4), `tocEntryToLink` (Compute Cores, Impact: 18.5), `makeChunks` (Many-Argument Workhorses, Impact: 14.6)
...
- **Heaviest Functions:** `bodyPartToBlocks` (Compute Cores, Impact: 46.5), `parPartToInlines'` (Compute Cores, Impact: 33.7), `runStyleToTransform` (Compute Cores, Impact: 24.6)
...
**Top Internal Functions/Classes:**
  * `pandocApp` **(Compute Cores)** (Impact: 369.4)
    * *Intent:* // Petite Vue app definition
  * `buildOptions` **(Compute Cores)** (Impact: 118.9)
  ...
  * `downloadAsExample` **(Compute Cores)** (Impact: 49.0)
...
**Top Internal Functions/Classes:**
  * `inlineToHtml` **(Compute Cores)** (Impact: 131.2)
    * *Intent:* -- | Convert Pandoc inline element to HTML.
  * `obfuscateLink` **(Many-Argument Workhorses)** (Impact: 124.3)
    * *Intent:* -- | Obfuscate a "mailto:" link.
  * `pandocToHtml` **(Compute Cores)** (Impact: 56.8)
...
**Top Internal Functions/Classes:**
  ...
  * `inlineToLaTeX` **(Compute Cores)** (Impact: 131.6)
    * *Intent:* -- | Convert inline element to LaTeX
  * `pandocToLaTeX` **(Compute Cores)** (Impact: 74.8)
...
**Top Internal Functions/Classes:**
  * `blockToMarkdown'` **(Compute Cores)** (Impact: 213.9)
...
**Top Internal Functions/Classes:**
  * `elemToParPart'` **(Compute Cores)** (Impact: 39.5)
  ...
  * `childElemToRun` **(Callbacks & Closures)** (Impact: 17.7)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `figure` **(Many-Argument Workhorses)** (Impact: 14.2)
...
**Top Internal Functions/Classes:**
  * `inlineToMarkdown` **(Compute Cores)** (Impact: 243.1)
    * *Intent:* -- | Convert Pandoc inline element to markdown.
...
**Top Internal Functions/Classes:**
  ...
  * `resolveReferences` **(Callbacks & Closures)** (Impact: 21.2)
  * `unicodeTransform` **(Callbacks & Closures)** (Impact: 16.5)
...
**Top Internal Functions/Classes:**
  ...
  * `formatBorder` **(Many-Argument Workhorses)** (Impact: 13.4)
  * `splitSentences` **(Callbacks & Closures)** (Impact: 11.2)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `blockToOpenXML'` **(Compute Cores)** (Impact: 86.2)
  ...
  * `generateImgElt` **(Compute Cores)** (Impact: 10.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
...
**Top Internal Functions/Classes:**
  ...
  * `go` **(Callbacks & Closures)** (Impact: 16.6)
...
- `src/Text/Pandoc/Shared.hs` -> **Severity: 5.339** (Embedded: 0.108 * Error Risk: 49.4364%)
- `src/Text/Pandoc/Writers/Shared.hs` -> **Severity: 4.751** (Embedded: 0.1 * Error Risk: 47.5088%)
- `test/Tests/Helpers.hs` -> **Severity: 3.986** (Embedded: 0.0781 * Error Risk: 51.0464%)
- `src/Text/Pandoc/Chunks.hs` -> **Severity: 3.815** (Embedded: 0.0764 * Error Risk: 49.9477%)
