...
| Avg Path Length | 4.4935 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.53; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 23%, Declarative / Non-Code 23%, Data / Markup / Trivial 18%, Many-Argument Workhorses Files 11%, Encapsulated Accessors Files 7%
...
| connectivity | 14920 | 1158 | 22 | `Modules/_decimal/libmpdec/mpdecimal.h` |
...
| dead_code | 4885 | 606 | 7 | `Modules/_testclinic.c` |
...
- **Composition Archetype:** `Interface Declarations Files` (z +0.96)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 20.0), `determine_num_threads_and_affinity` (Type Conversions, Impact: 11.6), `benchmark` (Compute Cores, Impact: 11.0)
...
- **Heaviest Functions:** `readAsync` (Defensive Guards, Impact: 12.9), `pollAsync` (Compute Cores, Impact: 9.0), `onTimeout` (Callbacks & Closures, Impact: 5.0)
...
- **Heaviest Functions:** `Java_org_python_testbed_PythonTestRunner_runPython` (Many-Argument Workhorses, Impact: 18.2), `redirect_stream` (Compute Cores, Impact: 9.7), `Java_org_python_testbed_PythonTestRunner_redirectStdioToLogcat` (Compute Cores, Impact: 5.8)
...
- **Heaviest Functions:** `gradle_task` (Compute Cores, Impact: 24.4), `logcat_task` (Compute Cores, Impact: 22.0), `parse_args` (I/O & Config Routines, Impact: 19.4)
...
- **Heaviest Functions:** `mi_option_init` (Compute Cores, Impact: 45.8), `_mi_fputs` (Type Conversions, Impact: 18.5), `_mi_strnicmp` (Defensive Guards, Impact: 14.3)
...
- **Heaviest Functions:** `mi_atomic_load_explicit` (Compute Cores, Impact: 11.0), `mi_atomic_store_explicit` (Compute Cores, Impact: 8.4), `mi_atomic_storei64_explicit` (Compute Cores, Impact: 8.4)
...
- **Heaviest Functions:** `get_coding_spec` (Many-Argument Workhorses, Impact: 49.4), `_PyTokenizer_check_coding_spec` (Many-Argument Workhorses, Impact: 38.4), `_PyTokenizer_translate_newlines` (Many-Argument Workhorses, Impact: 33.6)

### 8. `Platforms/emscripten/web_example/python.worker.mjs` (JAVASCRIPT) -> Cumulative Risk: **674.72**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.50)
...
- **Heaviest Functions:** `onmessage` (Defensive Guards, Impact: 6.4), `stderr` (Defensive Guards, Impact: 5.1), `stdout` (Callbacks & Closures, Impact: 4.8)

### 9. `Modules/binascii.c` (C) -> Cumulative Risk: **674.04**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.07)
- **Magnitude:** 2242.0 | **LOC:** 2597 | **CtrlFlow:** 23.4% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (96.7467%)
- **Heaviest Functions:** `binascii_b2a_qp_impl` (Many-Argument Workhorses, Impact: 243.9), `binascii_a2b_base64_impl` (Many-Argument Workhorses, Impact: 106.9), `binascii_a2b_ascii85_impl` (Many-Argument Workhorses, Impact: 97.3)

### 10. `Modules/_ssl.c` (C) -> Cumulative Risk: **670.08**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.31)
- **Magnitude:** 4430.3 | **LOC:** 7422 | **CtrlFlow:** 19.2% | **Authorship Centralization:** 23.5%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9975%), Safety Score (88.5221%)
- **Heaviest Functions:** `fill_and_set_sslerror` (Many-Argument Workhorses, Impact: 135.0), `_get_peer_alt_names` (Many-Argument Workhorses, Impact: 110.0), `newPySSLSocket` (Many-Argument Workhorses, Impact: 97.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.7e-05 | `Ripple Effect (Closeness):` 0.000795
...
- **Magnitude:** 7294.5 | **LOC:** 12874 | **CtrlFlow:** 18.7% | **Authorship Centralization:** 25.8%
...
**Top Internal Functions/Classes:**
  ...
  * `subtype_dealloc` **(Compute Cores)** (Impact: 52.2)
...
**Top Internal Functions/Classes:**
  ...
  * `socket_exec` **(Compute Cores)** (Impact: 124.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `os_timerfd_settime` **(Many-Argument Workhorses)** (Impact: 53.7)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.34
  * `Choke Point (Betweenness):` 1e-05 | `Ripple Effect (Closeness):` 0.000795
...
**Top Internal Functions/Classes:**
  * `dictiter_iternextitem` **(Compute Cores)** (Impact: 440.9)
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 4430.3 | **LOC:** 7422 | **CtrlFlow:** 19.2% | **Authorship Centralization:** 23.5%
- **Risk Profile:** Cognitive Load (76.0856%), Tech Debt (57.7912%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `_Py_EnterRecursiveCallUnchecked` **(Compute Cores)** (Impact: 419.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6e-06 | `Ripple Effect (Closeness):` 0.002936
...
**Top Internal Functions/Classes:**
  ...
  * `codegen_visit_expr` **(Compute Cores)** (Impact: 96.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.001073
...
**Top Internal Functions/Classes:**
  ...
  * `context_init` **(Many-Argument Workhorses)** (Impact: 47.0)
  * `_decimal_Decimal_quantize` **(Many-Argument Workhorses)** (Impact: 25.0)
  * `_decimal_Decimal_to_integral_value` **(Many-Argument Workhorses)** (Impact: 24.9)
  * `_decimal_Decimal_to_integral` **(Many-Argument Workhorses)** (Impact: 24.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2e-06 | `Ripple Effect (Closeness):` 0.000596
...
**Top Internal Functions/Classes:**
  ...
  * `quorem` **(Compute Cores)** (Impact: 31.0)
    * *Intent:* bits (28--31) are zero and bit 27 is set. */
  * `mult` **(Compute Cores)** (Impact: 30.6)
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 3108.14 | **LOC:** 3906 | **CtrlFlow:** 24.0% | **Authorship Centralization:** 41.7%
...
**Top Internal Functions/Classes:**
  ...
  * `config_read` **(Compute Cores)** (Impact: 61.2)
...
**Top Internal Functions/Classes:**
  ...
  * `optimize_load_fast` **(Compute Cores)** (Impact: 105.0)
  ...
  * `const_folding_safe_multiply` **(Compute Cores)** (Impact: 49.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-05 | `Ripple Effect (Closeness):` 0.002435
...
**Top Internal Functions/Classes:**
  ...
  * `posonly_keywords_opt_kwonly_opt` **(Many-Argument Workhorses)** (Impact: 30.4)
  * `keywords_opt_kwonly` **(Many-Argument Workhorses)** (Impact: 30.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3e-06 | `Ripple Effect (Closeness):` 0.000596
...
**Top Internal Functions/Classes:**
  ...
  * `code_new_impl` **(Many-Argument Workhorses)** (Impact: 66.9)
    * *Intent:* [clinic start generated code]*/
  * `intern_constants` **(Compute Cores)** (Impact: 58.8)
    * *Intent:* constants. */
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.6e-05 | `Ripple Effect (Closeness):` 0.002087
...
- `Python/optimizer_bytecodes.c` -> Churn: **95.52%** | Cog Load: 72.1594% | Debt: 8.189%
...
- `Objects/typeobject.c` -> Churn: **74.32%** | Cog Load: 51.4174% | Debt: 22.7732%
...
- `Python/thread.c` -> **Severity: 0.788** (Bridge: 0.0079 * Flux: 99.9732%)
- `Include/internal/mimalloc/mimalloc/atomic.h` -> **Severity: 0.646** (Bridge: 0.0081 * Flux: 80.2283%)
- `Include/internal/pycore_gc.h` -> **Severity: 0.376** (Bridge: 0.004 * Flux: 95.0056%)
- `Objects/mimalloc/os.c` -> **Severity: 0.279** (Bridge: 0.0028 * Flux: 99.9993%)
- `Include/internal/pycore_mimalloc.h` -> **Severity: 0.265** (Bridge: 0.0058 * Flux: 45.5672%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `Include/internal/pycore_llist.h` -> **Severity: 8.983** (Embedded: 0.0946 * Error Risk: 94.9241%)
- `Include/cpython/funcobject.h` -> **Severity: 7.742** (Embedded: 0.0918 * Error Risk: 84.2905%)
- `Include/cpython/context.h` -> **Severity: 7.678** (Embedded: 0.0918 * Error Risk: 83.5974%)
- `Include/cpython/genobject.h` -> **Severity: 7.678** (Embedded: 0.0918 * Error Risk: 83.5974%)
- `Include/moduleobject.h` -> **Severity: 7.46** (Embedded: 0.0918 * Error Risk: 81.223%)
