...
| Analyzed Artifacts (Scanned) | 20350 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 2169 |
| Total LOC | 2476150 |
...
|---|---|---|
| Modularity | 0.8879 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
> **Composition Archetype:** `Flat Modular Platform` (z -0.20; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 23%, Data / Markup / Trivial 22%, Callbacks & Closures Files 16%, Large Core Modules 9%, Interface Declarations Files 7%
...
| connectivity | 61470 | 8493 | 6 | `config/routes.rb` |
...
| docs | 6468 | 1245 | 0 | `packages/react-dnd-test-backend/index.js` |
...
| dead_code | 11478 | 4540 | 2 | `app/models/user.rb` |
| credential | 225 | 126 | 0 | `spec/apis/v1/submissions_api_spec.rb` |
| threat | 7711 | 1742 | 0 | `ui/shared/assignments/backbone/models/Assignment.js` |
...
- `Anonymous_Block` **(Compute Cores)** (@ `spec/controllers/assignments_controller_spec.rb`) -> Impact: **747.8** | LOC: 3896
...
- `Anonymous_Block` **(Tests & Verification)** (@ `spec/controllers/application_controller_spec.rb`) -> Impact: **631.6** | LOC: 3691
  * *Intent:* # # Canvas is free software: you can redistribute it and/or modify it under # the terms of the GNU Affero General Public License as published by the F...
...
- `app/models/authentication_provider/apple.rb` -> **100.0%** Exposure
...
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.78)
...
- **Heaviest Functions:** `processResult` (Defensive Guards, Impact: 33.3), `getDifferentiationTags` (Callbacks & Closures, Impact: 6.8), `getSections` (Callbacks & Closures, Impact: 5.1)
...
- **Heaviest Functions:** `update_account_associations` (Many-Argument Workhorses, Impact: 57.8), `name_parts` (Compute Cores, Impact: 45.9), `upcoming_events` (Compute Cores, Impact: 43.7)
...
- **Heaviest Functions:** `tool_dimensions` (Many-Argument Workhorses, Impact: 356.0), `content_tag_redirect` (Many-Argument Workhorses, Impact: 207.1), `js_env` (Defensive Guards, Impact: 140.2)
...
- **Heaviest Functions:** `startExport` (Many-Argument Workhorses, Impact: 19.8), `constructor` (Many-Argument Workhorses, Impact: 18.5), `monitorExport` (Compute Cores, Impact: 18.0)
...
- **Heaviest Functions:** `tabs_available` (Compute Cores, Impact: 139.7), `Anonymous_Block` (Compute Cores, Impact: 35.6), `get_special_account` (Many-Argument Workhorses, Impact: 27.6)
...
- **Heaviest Functions:** `_mouseDown` (Defensive Guards, Impact: 21.1), `_mouseMove` (Defensive Guards, Impact: 12.3), `_mouseUp` (Defensive Guards, Impact: 5.1)
...
- **Heaviest Functions:** `run_callbacks` (Compute Cores, Impact: 26.3), `filter_callbacks` (Callbacks & Closures, Impact: 7.6), `suspended_callback?` (Compute Cores, Impact: 6.2)

### 8. `bin/wip_open_source_lint.sh` (SHELL) -> Cumulative Risk: **717.7**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.59)
...
- **Heaviest Functions:** `run_parallel_tasks` (Compute Cores, Impact: 9.4), `log_and_run_command` (Compute Cores, Impact: 5.0), `test_migrations` (I/O & Config Routines, Impact: 4.9)
...
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `setToken` (Callbacks & Closures, Impact: 5.4), `constructor` (Callbacks & Closures, Impact: 4.7), `subscribe` (Callbacks & Closures, Impact: 4.6)
...
- **Heaviest Functions:** `check_right?` (Many-Argument Workhorses, Impact: 39.6), `grants_all_rights?` (Compute Cores, Impact: 10.8), `permission_cache_key_for` (Callbacks & Closures, Impact: 10.6)
...
**Top Internal Functions/Classes:**
  ...
  * `submission_spec_model` **(Tests & Verification)** (Impact: 5.1)
  * `peer_review_assignment` **(Tests & Verification)** (Impact: 3.6)
  * `submission_for_some_user` **(Tests & Verification)** (Impact: 3.3)
  * `setup_account_for_turnitin` **(Tests & Verification)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `concurrent_inserts` **(Tests & Verification)** (Impact: 336.5)
  ...
  * `student_unread_count_counts` **(Tests & Verification)** (Impact: 204.5)
  * `assignment` **(Compute Cores)** (Impact: 73.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `create_assignment_json` **(Many-Argument Workhorses)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 93.8)
...
- **Risk Profile:** Cognitive Load (92.791%), Tech Debt (12.348%)
**Top Internal Functions/Classes:**
  * `SlickGrid` **(Many-Argument Workhorses)** (Impact: 712.4)
    * *Intent:* // //////////////////////////////////////////////////////////////////////////////////////////// // S...
  * `setupColumnResize` **(Compute Cores)** (Impact: 80.8)
...
**Top Internal Functions/Classes:**
  ...
  * `__global_context__` **(Compute Cores)** (Impact: 52.8)
...
**Top Internal Functions/Classes:**
  ...
  * `cached_account_users` **(Tests & Verification)** (Impact: 174.0)
  * `setup_DA` **(Tests & Verification)** (Impact: 160.5)
  * `default_scheme` **(Compute Cores)** (Impact: 101.6)
  * `calculated_value` **(Tests & Verification)** (Impact: 99.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `render` **(Defensive Guards)** (Impact: 51.3)
  * `handleViewOptionsUpdated` **(Compute Cores)** (Impact: 46.6)
  * `gradeSubmission` **(Compute Cores)** (Impact: 43.5)
...
**Top Internal Functions/Classes:**
  ...
  * `buildUrl` **(Compute Cores)** (Impact: 22.8)
  * `handleFileUpload` **(Compute Cores)** (Impact: 22.1)
...
**Top Internal Functions/Classes:**
  ...
  * `infer_values` **(Compute Cores)** (Impact: 42.9)
...
**Top Internal Functions/Classes:**
  * `handleSubmissionSelectionChange` **(Compute Cores)** (Impact: 83.4)
  ...
  * `handleGradeSubmit` **(Compute Cores)** (Impact: 55.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `updateFormQuestion` **(Compute Cores)** (Impact: 75.6)
    * *Intent:* // Updates the question's form when the type changes
  * `formulaQuestion` **(Compute Cores)** (Impact: 70.5)
  * `processData` **(Compute Cores)** (Impact: 57.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
