...
| Analyzed Artifacts (Scanned) | 4699 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 1123 |
| Total LOC | 231904 |
...
> **Composition Archetype:** `Flat Modular Platform` (z -1.21; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 68%, Large Core Modules 5%, Declarative / Non-Code 5%, Interface Declarations Files 4%, Generic / Templated Code Files 4%
...
|---|---|---|
| Unclassified | 4481 | 95.4% |
| Unknown | 13 | 0.3% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 204 | 4.3% |
| Static: Minified & Vendor Opaque Mass | 1 | 0.0% |

## 5. EXCLUDED ARTIFACTS (Unparsable or Shielded Files)
*Total Excluded Artifacts: 1123*
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 33.9 | 0.0 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 24.6 | 0.0 | 0.0 |
...
| connectivity | 11442 | 1511 | 5 | `test/support/windows-integration/plugins/module_utils/Ansible.Service.cs` |
...
- **Composition Archetype:** `Compute Cores Files` (z +0.69)
...
- **Heaviest Functions:** `collect` (Many-Argument Workhorses, Impact: 72.5), `is_systemd_managed` (Compute Cores, Impact: 6.2), `is_systemd_managed_offline` (Compute Cores, Impact: 6.1)
...
- **Heaviest Functions:** `load_callbacks` (Compute Cores, Impact: 32.4), `send_callback` (Many-Argument Workhorses, Impact: 31.4), `__init__` (Many-Argument Workhorses, Impact: 25.1)
...
- **Heaviest Functions:** `parse_distribution_file_Debian` (Many-Argument Workhorses, Impact: 95.0), `parse_distribution_file_SUSE` (Many-Argument Workhorses, Impact: 71.9), `parse_distribution_file_Amazon` (Many-Argument Workhorses, Impact: 28.1)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 37.1), `_get_loader` (Many-Argument Workhorses, Impact: 30.0), `_iter_modules_impl` (Compute Cores, Impact: 22.0)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 30.5), `__init__` (Many-Argument Workhorses, Impact: 28.1), `from_value` (Defensive Guards, Impact: 7.2)
...
- **Heaviest Functions:** `open` (Many-Argument Workhorses, Impact: 106.1), `fetch_url` (Many-Argument Workhorses, Impact: 62.7), `redirect_request` (Many-Argument Workhorses, Impact: 62.4)

### 7. `lib/ansible/galaxy/dependency_resolution/providers.py` (PYTHON) -> Cumulative Risk: **689.89**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -1.06)
...
- **Heaviest Functions:** `_find_matches` (Many-Argument Workhorses, Impact: 90.3), `find_matches` (Many-Argument Workhorses, Impact: 19.1), `get_dependencies` (Compute Cores, Impact: 12.2)
...
- **Heaviest Functions:** `get_interfaces_info` (Compute Cores, Impact: 35.3), `parse_interface_line` (Many-Argument Workhorses, Impact: 11.9), `parse_ether_line` (Parameter Forwarders, Impact: 4.8)

### 9. `lib/ansible/_internal/_wrapt.py` (PYTHON) -> Cumulative Risk: **687.85**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.09)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 34.4), `__call__` (Compute Cores, Impact: 26.0), `__get__` (Many-Argument Workhorses, Impact: 17.1)
...
- **Heaviest Functions:** `_cmp` (Compute Cores, Impact: 32.9), `parse` (Compute Cores, Impact: 18.2), `_cmp` (Defensive Guards, Impact: 11.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_validate_docs` **(Compute Cores)** (Impact: 97.8)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 46.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_execute_install_role` **(Compute Cores)** (Impact: 64.7)
  * `execute_install` **(Compute Cores)** (Impact: 54.6)
...
**Top Internal Functions/Classes:**
  ...
  * `visit_ImportFrom` **(Compute Cores)** (Impact: 43.6)
...
**Top Internal Functions/Classes:**
  ...
  * `main` **(Compute Cores)** (Impact: 81.8)
...
**Top Internal Functions/Classes:**
  ...
  * `main` **(Compute Cores)** (Impact: 78.3)
    * *Intent:* # ===========================================
