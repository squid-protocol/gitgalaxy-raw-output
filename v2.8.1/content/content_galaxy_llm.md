...
> **Composition Archetype:** `Guard/Validation-Heavy` (z +1.49; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 100%, Declarative / Non-Code 0%, Defensive Guards Files 0%, Large Core Modules 0%, Compute Cores Files 0%
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 5.6 | 0.0 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 3.6 | 0.0 | 0.0 |
...
- `postAboutDangerousContent` **(Many-Argument Workhorses)** (@ `scripts/analyze-pr-build.js`) -> Impact: **77.0** | LOC: 100
  * *Intent:* /** * Constructs a comment reporting any dangerous external URLs. * @param {Doc[]} docs - Array of built document objects. * @param {Array} patch - Ar...
- `postAboutFlaws` **(Compute Cores)** (@ `scripts/analyze-pr-build.js`) -> Impact: **51.2** | LOC: 88
  * *Intent:* /** * Constructs a comment reporting document flaws. * @param {Doc[]} docs - Array of built document objects. * @param {object} config - Configuration...
- `checkFile` **(Many-Argument Workhorses)** (@ `scripts/filecheck/checker.js`) -> Impact: **48.1** | LOC: 131
  * *Intent:* /** * Check a single file for naming, type, reference, and compression rules. * @param {string} filePath * @param {CheckerOptions} [options={}] * @ret...
- `analyzePR` **(Many-Argument Workhorses)** (@ `scripts/analyze-pr-build.js`) -> Impact: **41.3** | LOC: 99
  * *Intent:* /** * @import { Doc } from "@mdn/rari" */ /** * Main function to analyze a PR build directory and post (or print) a comment. * @param {string} buildDi...
...
- `checkCompression` **(Defensive Guards)** (@ `scripts/filecheck/checker.js`) -> Impact: **35.4** | LOC: 85
  * *Intent:* /** * Compresses an image (if supported) and optionally saves the result, * enforcing size and compression delta constraints. * @param {string} filePa...
...
### 1. `scripts/front-matter_utils.js` (JAVASCRIPT) -> Cumulative Risk: **624.16**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9998%), Cognitive Load (98.0343%)
- **Heaviest Functions:** `checkFrontMatter` (Many-Argument Workhorses, Impact: 40.1), `getAjvValidator` (Interface Declarations, Impact: 3.1), `areAttributesInOrder` (Defensive Guards, Impact: 2.0)

### 2. `scripts/front-matter_linter.js` (JAVASCRIPT) -> Cumulative Risk: **577.11**
...
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.3356%)
- **Heaviest Functions:** `resolveDirectory` (Defensive Guards, Impact: 16.7), `lintFrontMatter` (Defensive Guards, Impact: 14.3)

### 3. `scripts/sort_and_unique_file_lines.js` (JAVASCRIPT) -> Cumulative Risk: **527.47**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -0.95)
- **Magnitude:** 54.12 | **LOC:** 64 | **CtrlFlow:** 32.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (83.9955%)
- **Heaviest Functions:** `equalsIgnoreCase` (Defensive Guards, Impact: 26.0)

### 4. `scripts/update-moved-file-links.js` (JAVASCRIPT) -> Cumulative Risk: **511.75**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.00)
- **Magnitude:** 121.16 | **LOC:** 146 | **CtrlFlow:** 21.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), State Flux (99.999%), Cognitive Load (91.2529%)
- **Heaviest Functions:** `movedFiles` (Many-Argument Workhorses, Impact: 18.2), `getImageSlug` (Defensive Guards, Impact: 7.6)

### 5. `scripts/analyze-pr-build.js` (JAVASCRIPT) -> Cumulative Risk: **491.44**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.9957%), State Flux (99.9933%), Safety Score (79.2051%)
- **Heaviest Functions:** `postAboutDangerousContent` (Many-Argument Workhorses, Impact: 77.0), `postAboutFlaws` (Compute Cores, Impact: 51.2), `analyzePR` (Many-Argument Workhorses, Impact: 41.3)

### 6. `scripts/utils.js` (JAVASCRIPT) -> Cumulative Risk: **482.85**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.83)
- **Magnitude:** 58.22 | **LOC:** 92 | **CtrlFlow:** 22.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (85.9425%), Safety Score (65.1355%)
- **Heaviest Functions:** `execGit` (Many-Argument Workhorses, Impact: 15.3), `getLocations` (Defensive Guards, Impact: 9.9), `walkSync` (Compute Cores, Impact: 6.2)

### 7. `scripts/log-url-issues.js` (JAVASCRIPT) -> Cumulative Risk: **454.79**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +2.40)
- **Magnitude:** 98.08 | **LOC:** 219 | **CtrlFlow:** 20.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9929%), Safety Score (81.7574%)
- **Heaviest Functions:** `getDeletedSlugs` (Compute Cores, Impact: 17.7), `getFragmentDetails` (Compute Cores, Impact: 12.6), `getFileAnchors` (Interface Declarations, Impact: 4.8)

### 8. `scripts/content/release-firefox.js` (JAVASCRIPT) -> Cumulative Risk: **451.84**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.84)
- **Magnitude:** 146.86 | **LOC:** 282 | **CtrlFlow:** 13.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), State Flux (99.9123%), Safety Score (74.0484%)
- **Heaviest Functions:** `updateReleaseNotes` (Many-Argument Workhorses, Impact: 25.5), `main` (I/O & Config Routines, Impact: 16.3), `deactivatePreviousStable` (Defensive Guards, Impact: 5.7)

### 9. `scripts/filecheck/utils.js` (JAVASCRIPT) -> Cumulative Risk: **425.74**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.80)
- **Magnitude:** 26.74 | **LOC:** 43 | **CtrlFlow:** 16.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (91.6827%), Safety Score (62.5811%)
- **Heaviest Functions:** `correctContentPathFromEnv` (Defensive Guards, Impact: 9.3), `correctPathFromEnv` (Interface Declarations, Impact: 3.2), `parseEnvValue` (Defensive Guards, Impact: 1.9)

### 10. `scripts/filecheck/checker.js` (JAVASCRIPT) -> Cumulative Risk: **421.28**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.17)
- **Magnitude:** 198.74 | **LOC:** 399 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.9948%), State Flux (82.8154%), Safety Score (52.3061%)
- **Heaviest Functions:** `checkFile` (Many-Argument Workhorses, Impact: 48.1), `checkCompression` (Defensive Guards, Impact: 35.4), `resolveDirectory` (Compute Cores, Impact: 9.3)
...
- **Magnitude:** 513.66 | **LOC:** 620 | **CtrlFlow:** 24.0% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (71.1033%), Tech Debt (11.2905%)
**Top Internal Functions/Classes:**
  * `postAboutDangerousContent` **(Many-Argument Workhorses)** (Impact: 77.0)
    * *Intent:* /** * Constructs a comment reporting any dangerous external URLs. * @param {Doc[]} docs - Array of b...
  * `postAboutFlaws` **(Compute Cores)** (Impact: 51.2)
    * *Intent:* /** * Constructs a comment reporting document flaws. * @param {Doc[]} docs - Array of built document...
  * `analyzePR` **(Many-Argument Workhorses)** (Impact: 41.3)
    * *Intent:* /** * @import { Doc } from "@mdn/rari" */ /** * Main function to analyze a PR build directory and po...
  * `coerce` **(Compute Cores)** (Impact: 19.4)
  * `getPatchLines` **(Defensive Guards)** (Impact: 12.3)
    * *Intent:* /** * Extracts added (new) lines from patch objects. * @param {Array} patches - Array of patch objec...
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 198.74 | **LOC:** 399 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (30.2299%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `checkFile` **(Many-Argument Workhorses)** (Impact: 48.1)
    * *Intent:* /** * Check a single file for naming, type, reference, and compression rules. * @param {string} file...
  ...
  * `resolveDirectory` **(Compute Cores)** (Impact: 9.3)
    * *Intent:* /** * Resolve a path to a list of files that should be checked. * @param {string} file * @returns {P...
  * `runChecker` **(Defensive Guards)** (Impact: 7.1)
    * *Intent:* /** * Run the checker across a set of files or directories. * @param {string[]} filesAndDirectories ...
  * `formatSize` **(Interface Declarations)** (Impact: 4.7)
    * *Intent:* /** * @param {number} bytes * @returns {string} */
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 146.86 | **LOC:** 282 | **CtrlFlow:** 13.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (56.1896%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  ...
  * `formatDate` **(Interface Declarations)** (Impact: 1.9)
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 121.16 | **LOC:** 146 | **CtrlFlow:** 21.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (91.2529%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `getPath` **(Interface Declarations)** (Impact: 1.6)
  * `getContent` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 98.08 | **LOC:** 219 | **CtrlFlow:** 20.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (56.485%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `getDeletedSlugs` **(Compute Cores)** (Impact: 17.7)
  * `getFragmentDetails` **(Compute Cores)** (Impact: 12.6)
  * `getFileAnchors` **(Interface Declarations)** (Impact: 4.8)
  * `getFileContent` **(Interface Declarations)** (Impact: 3.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
- **Magnitude:** 93.54 | **LOC:** 101 | **CtrlFlow:** 24.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (98.0343%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `checkFrontMatter` **(Many-Argument Workhorses)** (Impact: 40.1)
  * `getAjvValidator` **(Interface Declarations)** (Impact: 3.1)
  * `areAttributesInOrder` **(Defensive Guards)** (Impact: 2.0)
  * `getRelativePath` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 86.16 | **LOC:** 125 | **CtrlFlow:** 19.4% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (94.9113%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 58.22 | **LOC:** 92 | **CtrlFlow:** 22.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (27.9414%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  ...
  * `stringToFragment` **(Interface Declarations)** (Impact: 4.6)
    * *Intent:* /* * Convert Markdown header into URL slug. */
  * `isImagePath` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 54.12 | **LOC:** 64 | **CtrlFlow:** 32.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (78.3421%), Tech Debt (65.1355%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (99.5504%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
