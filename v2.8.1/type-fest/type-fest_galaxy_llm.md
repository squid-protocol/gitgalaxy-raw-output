...
> **Composition Archetype:** `Typed Library` (z +2.23; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 68%, Data / Markup / Trivial 20%, Interface Declarations Files 4%, Generic / Templated Code Files 3%, State Mutators Files 1%
...
- `incorrectTwoslashFormatErrorAt` **(Compute Cores)** (@ `lint-rules/validate-jsdoc-codeblocks.test.js`) -> Impact: **122.8** | LOC: 1778
- `create` **(Defensive Guards)** (@ `lint-rules/validate-jsdoc-codeblocks.js`) -> Impact: **38.7** | LOC: 95
- `create` **(Compute Cores)** (@ `lint-rules/require-exported-types.js`) -> Impact: **38.1** | LOC: 111
- `TSTypeAliasDeclaration` **(Compute Cores)** (@ `lint-rules/validate-jsdoc-codeblocks.js`) -> Impact: **36.4** | LOC: 78
...
- **Heaviest Functions:** `create` (Defensive Guards, Impact: 38.7), `TSTypeAliasDeclaration` (Compute Cores, Impact: 36.4), `validateTwoslashTypes` (Many-Argument Workhorses, Impact: 31.0)

### 2. `test-d/multidimensional-readonly-array.ts` (TYPESCRIPT) -> Cumulative Risk: **408.35**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +6.54)
...
- **Heaviest Functions:** `createArray` (Type Conversions, Impact: 6.4)

### 3. `test-d/multidimensional-array.ts` (TYPESCRIPT) -> Cumulative Risk: **396.7**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +6.39)
...
- **Heaviest Functions:** `createArray` (Type Conversions, Impact: 3.4)
...
- **Heaviest Functions:** `narrowingTest3` (Generic / Templated Code, Impact: 3.7), `narrowingTest2` (Generic / Templated Code, Impact: 3.6), `narrowingTest` (Generic / Templated Code, Impact: 3.5)
...
- **Heaviest Functions:** `narrowingTest2` (Compute Cores, Impact: 5.4), `narrowingTest3` (Generic / Templated Code, Impact: 3.7), `narrowingTest` (Generic / Templated Code, Impact: 3.5)

### 6. `lint-processors/jsdoc-codeblocks.js` (JAVASCRIPT) -> Cumulative Risk: **387.58**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -0.29)
...
- **Heaviest Functions:** `postprocess` (Defensive Guards, Impact: 27.3), `preprocess` (Defensive Guards, Impact: 21.8), `indentsUptoIndex` (Many-Argument Workhorses, Impact: 9.1)
...
- **Heaviest Functions:** `create` (Compute Cores, Impact: 11.4), `ExportNamedDeclaration` (Defensive Guards, Impact: 6.0), `fix` (Interface Declarations, Impact: 1.6)

### 8. `lint-rules/require-exported-types.js` (JAVASCRIPT) -> Cumulative Risk: **382.21**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.57)
...
- **Heaviest Functions:** `create` (Compute Cores, Impact: 38.1), `checkExportedType` (Compute Cores, Impact: 10.8)
...
- **Heaviest Functions:** `narrowingTest2` (Compute Cores, Impact: 7.2), `narrowingTest3` (Compute Cores, Impact: 5.5), `narrowingTest` (Compute Cores, Impact: 5.2)

### 10. `test-d/abstract-class.ts` (TYPESCRIPT) -> Cumulative Risk: **372.62**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.18)
...
- **Heaviest Functions:** `constructor` (Parameter Forwarders, Impact: 2.0), `withBar` (Generic / Templated Code, Impact: 1.8), `constructor` (Interface Declarations, Impact: 1.6)
...
**Top Internal Functions/Classes:**
  ...
  * `TSTypeAliasDeclaration` **(Compute Cores)** (Impact: 36.4)
...
**Top Internal Functions/Classes:**
  * `incorrectTwoslashFormatErrorAt` **(Compute Cores)** (Impact: 122.8)
  * `invalidCodeblockErrorAt` **(Interface Declarations)** (Impact: 1.5)
  * `incorrectTwoslashTypeErrorAt` **(Interface Declarations)** (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `errorAt` **(Compute Cores)** (Impact: 8.8)
    * *Intent:* */
...
**Top Internal Functions/Classes:**
  * `function_` **(Compute Cores)** (Impact: 8.1)
...
**Top Internal Functions/Classes:**
  * `create` **(Compute Cores)** (Impact: 38.1)
...
**Top Internal Functions/Classes:**
  ...
  * `fix` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Interface Declarations)** (Impact: 1.6)
...
**Top Internal Functions/Classes:**
  ...
  * `narrowingTest` **(Compute Cores)** (Impact: 5.2)
**Structural Signatures (Net Mitigated Signals):**
