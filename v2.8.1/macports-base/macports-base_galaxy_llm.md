...
> **Composition Archetype:** `Mainframe / COBOL & Config` (z +0.54; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 94%, I/O & Config Routines Files 3%, Declarative / Non-Code 2%, Compute Cores Files 0%
...
- `Anonymous_Block` **(Compute Cores)** (@ `portmgr/packaging/buildall.sh`) -> Impact: **9.3** | LOC: 17
- `Anonymous_Block` **(Compute Cores)** (@ `portmgr/packaging/buildall.sh`) -> Impact: **9.2** | LOC: 15
- `MP_LIB_MD5` **(I/O & Config Routines)** (@ `aclocal.m4`) -> Impact: **7.5** | LOC: 408
  * *Intent:* # MP_LIB_MD5 #--------------------------------------- # Check for an md5 implementation
- `prepchroot` **(Compute Cores)** (@ `portmgr/packaging/buildall.sh`) -> Impact: **7.5** | LOC: 9
  * *Intent:* # Set up the base chroot image
- `export_path` **(I/O & Config Routines)** (@ `setupenv.bash.in`) -> Impact: **7.2** | LOC: 25
...
- `export_manpath` **(I/O & Config Routines)** (@ `setupenv.bash.in`) -> Impact: **6.0** | LOC: 21
...
- **Heaviest Functions:** `mkchrootbase` (Compute Cores, Impact: 28.6), `Anonymous_Block` (I/O & Config Routines, Impact: 13.6), `teardownchroot` (Compute Cores, Impact: 10.5)

### 2. `setupenv.bash.in` (SHELL) -> Cumulative Risk: **465.38**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +2.46)
...
- **Heaviest Functions:** `export_path` (I/O & Config Routines, Impact: 7.2), `export_manpath` (I/O & Config Routines, Impact: 6.0), `export_display` (Interface Declarations, Impact: 2.2)

### 3. `aclocal.m4` (M4) -> Cumulative Risk: **408.63**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.96)
...
- **Heaviest Functions:** `MP_LIB_MD5` (I/O & Config Routines, Impact: 7.5), `MP_CONFIG_SUBDIR` (Many-Argument Workhorses, Impact: 3.2), `_MP_LIST_CONTAINS` (State Mutators, Impact: 3.0)

### 4. `doc/Makefile.in` (MAKEFILE) -> Cumulative Risk: **313.74**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.98)
...
- **Heaviest Functions:** `install` (I/O & Config Routines, Impact: 9.6), `.dep` (I/O & Config Routines, Impact: 2.4), `%.html` (Defensive Guards, Impact: 2.2)
...
- **Heaviest Functions:** `pkgIndex.tcl` (I/O & Config Routines, Impact: 3.1), `.c.o` (Interface Declarations, Impact: 1.2), `all` (I/O & Config Routines, Impact: 1.2)
...
- **Heaviest Functions:** `MACPORTS_VERSION_MACRO` (I/O & Config Routines, Impact: 1.1)
...
- **Heaviest Functions:** `SC_ENABLE_LANGINFO` (Many-Argument Workhorses, Impact: 4.5), `SC_CONFIG_CFLAGS` (Many-Argument Workhorses, Impact: 4.5), `SC_PATH_X` (Many-Argument Workhorses, Impact: 4.0)

### 8. `Mk/macports.subdir.mk` (MAKEFILE) -> Cumulative Risk: **224.66**
- **Archetype:** `file_cluster_3` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.52)
...
- **Heaviest Functions:** `all` (I/O & Config Routines, Impact: 4.2), `clean` (I/O & Config Routines, Impact: 4.2), `test` (I/O & Config Routines, Impact: 4.2)
...
- **Heaviest Functions:** `distclean` (I/O & Config Routines, Impact: 1.2), `all` (I/O & Config Routines, Impact: 1.1), `install` (I/O & Config Routines, Impact: 1.1)

### 10. `portmgr/dmg/InstallationCheck` (SHELL) -> Cumulative Risk: **146.0**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +1.05)
...
- **Heaviest Functions:** `Anonymous_Block` (Unclassified, Impact: 3.2), `Anonymous_Block` (Unclassified, Impact: 2.3), `Anonymous_Block` (Unclassified, Impact: 2.2)
