...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.03; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 36%, Large Core Modules 17%, Callbacks & Closures Files 16%, Declarative / Non-Code 9%, Tests & Verification Files 5%
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 63.3 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 39.1 | 12.5 | 0.0 |
...
| connectivity | 9068 | 815 | 18 | `config/config.go` |
...
- `refresh` **(Compute Cores)** (@ `discovery/aws/rds.go`) -> Impact: **268.2** | LOC: 528
...
- `main` **(Compute Cores)** (@ `cmd/prometheus/main.go`) -> Impact: **228.8** | LOC: 1215
...
- **Heaviest Functions:** `ApplyConfig` (Compute Cores, Impact: 33.6), `NewManager` (Many-Argument Workhorses, Impact: 26.4), `Run` (Compute Cores, Impact: 13.7)

### 2. `promql/parser/generated_parser.y` (YACC) -> Cumulative Risk: **674.61**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.34)
...
- **Heaviest Functions:** `maybe_label` (Compute Cores, Impact: 36.0), `metric_identifier` (Compute Cores, Impact: 31.1), `binary_expr` (Compute Cores, Impact: 26.4)

### 3. `cmd/prometheus/main.go` (GO) -> Cumulative Risk: **650.29**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.44)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 228.8), `setFeatureListOptions` (Compute Cores, Impact: 68.2), `reloadConfig` (Many-Argument Workhorses, Impact: 26.1)
...
- **Heaviest Functions:** `NewMergeQuerier` (Many-Argument Workhorses, Impact: 21.4), `NewMergeChunkQuerier` (Many-Argument Workhorses, Impact: 21.4), `Next` (I/O & Config Routines, Impact: 20.4)
...
- **Heaviest Functions:** `loadWAL` (Many-Argument Workhorses, Impact: 242.2), `loadWBL` (Many-Argument Workhorses, Impact: 133.8), `processWALSamples` (Many-Argument Workhorses, Impact: 82.2)
...
- **Heaviest Functions:** `Run` (Many-Argument Workhorses, Impact: 70.5), `New` (Many-Argument Workhorses, Impact: 31.6), `UnmarshalYAML` (Type Conversions, Impact: 31.5)
...
- **Heaviest Functions:** `testConvertClassicHistogramsToNHCB` (Many-Argument Workhorses, Impact: 102.0), `testScrapeLoopAppend` (Many-Argument Workhorses, Impact: 34.3), `TestScrapeLoopAppend_WithStorage` (Compute Cores, Impact: 30.6)

### 8. `notifier/sendloop.go` (GO) -> Cumulative Risk: **627.4**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.87)
...
- **Heaviest Functions:** `sendAll` (Compute Cores, Impact: 13.4), `add` (Compute Cores, Impact: 11.8), `loop` (I/O & Config Routines, Impact: 10.1)
...
- **Heaviest Functions:** `readVarbitInt` (Compute Cores, Impact: 33.0), `readVarbitUint` (Compute Cores, Impact: 31.4), `putVarbitInt` (Type Conversions, Impact: 20.6)
...
- **Heaviest Functions:** `TestNoDeadlock` (Compute Cores, Impact: 10.9), `labelsWithHashCollision` (I/O & Config Routines, Impact: 3.9), `stripeSeriesWithCollidingSeries` (Interface Declarations, Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `TestOOOHistogramCompactionWithCounterResets` **(Compute Cores)** (Impact: 64.4)
  * `testChunkQuerierOOOQuery` **(Many-Argument Workhorses)** (Impact: 48.9)
  * `TestStaleSeriesCompaction` **(Tests & Verification)** (Impact: 30.8)
...
**Top Internal Functions/Classes:**
  ...
  * `TestScrapeLoopAppend_WithStorage` **(Compute Cores)** (Impact: 30.6)
    * *Intent:* // TestScrapeLoopAppend_WithStorage tests appends and storage integration for the // large input fil...
  * `testScrapeLoopRun` **(Compute Cores)** (Impact: 29.7)
  * `testScrapeLoopRunCreatesStaleMarkersOnSampleLimit` **(Compute Cores)** (Impact: 24.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `BenchmarkLoadWLs` **(Compute Cores)** (Impact: 58.8)
  ...
  * `TestChunkSnapshot` **(Compute Cores)** (Impact: 41.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `TestOOOHistogramCompactionWithCounterResets_AppendV2` **(Compute Cores)** (Impact: 64.4)
...
**Top Internal Functions/Classes:**
  ...
  * `TestChunkSnapshot_AppenderV2` **(Compute Cores)** (Impact: 44.5)
  * `TestHeadAppenderV2_Delete_e2e` **(Compute Cores)** (Impact: 30.5)
  * `TestHeadAppenderV2_Append_DifferentEncodingSameSeries` **(Compute Cores)** (Impact: 26.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `loadChunkSnapshot` **(Compute Cores)** (Impact: 57.7)
