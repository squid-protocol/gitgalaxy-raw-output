...
> **Composition Archetype:** `Hub-Coupled App` (z +0.31; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 26%, Data / Markup / Trivial 17%, Large Core Modules 16%, Interface Declarations Files 12%, State Mutators Files 10%
...
- `_parse` **(Compute Cores)** (@ `src/parser/pdb-parser.ts`) -> Impact: **178.8** | LOC: 617
...
- `calculateHydrogensCharge` **(Compute Cores)** (@ `src/chemistry/valence-model.ts`) -> Impact: **138.9** | LOC: 179
...
- **Heaviest Functions:** `deepEqual` (Compute Cores, Impact: 42.0), `throttle` (Many-Argument Workhorses, Impact: 31.9), `download` (Compute Cores, Impact: 30.4)
...
- **Heaviest Functions:** `quicksortIP` (Many-Argument Workhorses, Impact: 71.0), `quicksortCmp` (Many-Argument Workhorses, Impact: 60.2), `quickselectCmp` (Many-Argument Workhorses, Impact: 38.8)

### 3. `src/math/vector-utils.ts` (TYPESCRIPT) -> Cumulative Risk: **655.24**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z -0.43)
...
- **Heaviest Functions:** `lineLineIntersect` (Many-Argument Workhorses, Impact: 20.2), `computeBoundingBox` (Compute Cores, Impact: 12.5), `projectPointOnVector` (Compute Cores, Impact: 8.4)
...
- **Heaviest Functions:** `onHandleContent` (Callbacks & Closures, Impact: 7.1)
...
- **Heaviest Functions:** `tick` (Compute Cores, Impact: 8.0), `_tick` (Compute Cores, Impact: 8.0), `_init` (State Mutators, Impact: 6.5)
...
- **Heaviest Functions:** `JacobiSVDImpl` (Many-Argument Workhorses, Impact: 130.4), `svd` (Many-Argument Workhorses, Impact: 36.0), `multiply` (Many-Argument Workhorses, Impact: 9.4)

### 7. `scripts/js/slimer/gallery.js` (JAVASCRIPT) -> Cumulative Risk: **636.09**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.11)
...
- **Heaviest Functions:** `renderExample` (Callbacks & Closures, Impact: 10.8), `getExampleNames` (Callbacks & Closures, Impact: 7.6), `buildExamplePage` (Many-Argument Workhorses, Impact: 5.9)
...
- **Heaviest Functions:** `setData` (Many-Argument Workhorses, Impact: 27.8), `getSurfaceWorker` (Many-Argument Workhorses, Impact: 22.4), `getDataSize` (Compute Cores, Impact: 21.2)
...
- **Heaviest Functions:** `setFilter` (Many-Argument Workhorses, Impact: 49.5), `constructor` (Compute Cores, Impact: 9.1), `_getFilterHash` (Parameter Forwarders, Impact: 2.1)
...
- **Heaviest Functions:** `refresh` (I/O & Config Routines, Impact: 20.1), `getSelection` (Compute Cores, Impact: 13.9), `eachAtom` (Compute Cores, Impact: 9.5)
...
**Top Internal Functions/Classes:**
  ...
  * `string2buf` **(Compute Cores)** (Impact: 28.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_parse` **(Compute Cores)** (Impact: 114.7)
...
**Top Internal Functions/Classes:**
  ...
  * `_parse` **(Compute Cores)** (Impact: 178.8)
...
**Top Internal Functions/Classes:**
  * `assignSecondaryStructure` **(Compute Cores)** (Impact: 70.6)
...
**Top Internal Functions/Classes:**
  ...
  * `setCollapsed` **(State Mutators)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `createOption` **(Many-Argument Workhorses)** (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `projectTorus` **(Compute Cores)** (Impact: 22.2)
...
**Top Internal Functions/Classes:**
  ...
  * `interpolateTangent` **(Many-Argument Workhorses)** (Impact: 11.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `postProcess` **(Compute Cores)** (Impact: 18.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `defaultFileRepresentation` **(Compute Cores)** (Impact: 44.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_parse` **(Compute Cores)** (Impact: 77.0)
  * `type` **(Interface Declarations)** (Impact: 1.1)
