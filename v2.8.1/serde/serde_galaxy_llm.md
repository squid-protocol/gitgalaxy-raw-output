...
> **Composition Archetype:** `Small Flat Repo` (z +1.45; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 36%, State Mutators Files 28%, Generic / Templated Code Files 10%, Declarative / Non-Code 6%, Large Core Modules 6%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 4.5 | 0.0 | 0.0 |
...
- `from_ast` **(Compute Cores)** (@ `serde_derive/src/internals/attr.rs`) -> Impact: **105.4** | LOC: 168
...
### 1. `serde_derive/src/internals/attr.rs` (RUST) -> Cumulative Risk: **515.54**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.56)
...
- **Heaviest Functions:** `from_ast` (Many-Argument Workhorses, Impact: 216.1), `from_ast` (Many-Argument Workhorses, Impact: 180.0), `from_ast` (Compute Cores, Impact: 105.4)

### 2. `serde_derive/src/bound.rs` (RUST) -> Cumulative Risk: **497.56**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.13)
...
- **Heaviest Functions:** `with_bound` (Many-Argument Workhorses, Impact: 83.0), `visit_type` (Many-Argument Workhorses, Impact: 14.3), `with_where_predicates_from_variants` (Many-Argument Workhorses, Impact: 13.2)

### 3. `serde_core/src/de/value.rs` (RUST) -> Cumulative Risk: **476.92**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.13)
...
- **Heaviest Functions:** `next_element_seed` (Defensive Guards, Impact: 9.3), `size_hint` (State Mutators, Impact: 7.5), `deserialize_tuple` (Generic / Templated Code, Impact: 6.6)

### 4. `serde/src/private/de.rs` (RUST) -> Cumulative Risk: **468.35**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
...
- **Heaviest Functions:** `deserialize_enum` (Many-Argument Workhorses, Impact: 11.2), `deserialize_enum` (Many-Argument Workhorses, Impact: 11.0), `borrow_cow_str` (Generic / Templated Code, Impact: 10.7)
...
- **Heaviest Functions:** `deserialize_seq` (Many-Argument Workhorses, Impact: 39.5), `deserialize_seq_in_place` (Many-Argument Workhorses, Impact: 26.8), `deserialize_body` (Defensive Guards, Impact: 20.3)

### 6. `serde_derive/src/de/struct_.rs` (RUST) -> Cumulative Risk: **438.77**
...
- **Heaviest Functions:** `deserialize_map` (Many-Argument Workhorses, Impact: 113.9), `deserialize_map_in_place` (Many-Argument Workhorses, Impact: 59.9), `deserialize` (Many-Argument Workhorses, Impact: 51.5)

### 7. `serde/src/private/ser.rs` (RUST) -> Cumulative Risk: **437.35**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
...
- **Heaviest Functions:** `serialize` (Many-Argument Workhorses, Impact: 17.8), `fmt` (Annotated Framework Methods, Impact: 4.3), `serialize_tagged_newtype` (Generic / Templated Code, Impact: 3.6)

### 8. `serde_derive/src/de/identifier.rs` (RUST) -> Cumulative Risk: **436.46**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `deserialize_identifier` (Many-Argument Workhorses, Impact: 88.2), `deserialize_custom` (Many-Argument Workhorses, Impact: 31.5), `deserialize_generated` (Many-Argument Workhorses, Impact: 20.1)

### 9. `serde_derive/src/internals/ctxt.rs` (RUST) -> Cumulative Risk: **433.68**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z -0.28)
- **Magnitude:** 31.14 | **LOC:** 68 | **CtrlFlow:** 14.3% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (96.0834%), Safety Score (86.2271%), Api Exposure (62.4271%)
- **Heaviest Functions:** `drop` (Compute Cores, Impact: 5.9), `check` (Interface Declarations, Impact: 4.9), `error_spanned_by` (State Mutators, Impact: 2.4)

### 10. `serde_core/src/ser/impossible.rs` (RUST) -> Cumulative Risk: **432.88**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.14)
- **Magnitude:** 58.38 | **LOC:** 217 | **CtrlFlow:** 15.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), Verification (80.0%)
- **Heaviest Functions:** `serialize_field` (Generic / Templated Code, Impact: 4.4), `serialize_field` (Generic / Templated Code, Impact: 4.4), `serialize_element` (Generic / Templated Code, Impact: 3.8)
...
**Top Internal Functions/Classes:**
  ...
  * `from_ast` **(Many-Argument Workhorses)** (Impact: 180.0)
    * *Intent:* /// Extract out the `#[serde(...)]` attributes from an item.
  * `from_ast` **(Compute Cores)** (Impact: 105.4)
...
**Top Internal Functions/Classes:**
  ...
  * `visit_map` **(Defensive Guards)** (Impact: 10.1)
...
**Top Internal Functions/Classes:**
  ...
  * `deserialize` **(Generic / Templated Code)** (Impact: 17.7)
  * `visit_map` **(Defensive Guards)** (Impact: 14.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `next_element_seed` **(Defensive Guards)** (Impact: 9.3)
  * `size_hint` **(State Mutators)** (Impact: 7.5)
...
**Top Internal Functions/Classes:**
  ...
  * `fmt` **(Annotated Framework Methods)** (Impact: 4.3)
...
**Top Internal Functions/Classes:**
  ...
  * `flatten_any_after_flatten_struct` **(Tests & Verification)** (Impact: 8.0)
  * `test_partially_untagged_enum_desugared` **(I/O & Config Routines)** (Impact: 7.2)
...
**Top Internal Functions/Classes:**
  ...
  * `with_where_predicates_from_variants` **(Many-Argument Workhorses)** (Impact: 13.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_atomics` **(Tests & Verification)** (Impact: 4.7)
  * `deserialize` **(Generic / Templated Code)** (Impact: 3.2)
  * `assert_de_tokens_ignore` **(Annotated Framework Methods)** (Impact: 2.6)
...
**Top Internal Functions/Classes:**
  ...
  * `format_u8` **(Compute Cores)** (Impact: 9.7)
  * `serialize` **(Generic / Templated Code)** (Impact: 9.7)
...
**Top Internal Functions/Classes:**
  ...
  * `check_variant_skip_attrs` **(Compute Cores)** (Impact: 24.3)
...
**Top Internal Functions/Classes:**
  * `test_gen` **(Annotated Framework Methods)** (Impact: 40.4)
    * *Intent:* //////////////////////////////////////////////////////////////////////////
...
**Top Internal Functions/Classes:**
  ...
  * `visit_seq` **(Compute Cores)** (Impact: 6.7)
...
**Top Internal Functions/Classes:**
  * `test_u64` **(Tests & Verification)** (Impact: 2.5)
  * `test_u128` **(Tests & Verification)** (Impact: 2.5)
  * `test_i8` **(Tests & Verification)** (Impact: 2.0)
  * `test_i16` **(Tests & Verification)** (Impact: 2.0)
  * `test_i32` **(Tests & Verification)** (Impact: 2.0)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_enum_skipped` **(Tests & Verification)** (Impact: 2.2)
  * `test_system_time` **(Tests & Verification)** (Impact: 1.9)
  * `test_cannot_serialize_paths` **(Tests & Verification)** (Impact: 1.6)
  * `test_result` **(Tests & Verification)** (Impact: 1.5)
  * `test_slice` **(Tests & Verification)** (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
