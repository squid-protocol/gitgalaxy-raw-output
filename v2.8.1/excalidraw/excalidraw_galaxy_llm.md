...
> **Composition Archetype:** `Flat Modular Platform` (z +0.05; from the repo's file-archetype mix)
> **File Composition:** Callbacks & Closures Files 22%, Declarative / Non-Code 21%, Data / Markup / Trivial 18%, Large Core Modules 10%, Many-Argument Workhorses Files 7%
...
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.30)
...
- **Heaviest Functions:** `loadSceneOrLibraryFromBlob` (Many-Argument Workhorses, Impact: 34.3), `getMimeType` (Compute Cores, Impact: 22.5), `normalizeFile` (Defensive Guards, Impact: 22.5)

### 2. `packages/excalidraw/hooks/useLibraryItemSvg.ts` (TYPESCRIPT) -> Cumulative Risk: **664.99**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.89)
...
- **Heaviest Functions:** `useLibraryItemSvg` (Many-Argument Workhorses, Impact: 25.3), `exportLibraryItemToSvg` (Callbacks & Closures, Impact: 2.2), `deleteItemsFromLibraryCache` (Callbacks & Closures, Impact: 1.9)
...
- **Heaviest Functions:** `broadcastScene` (Many-Argument Workhorses, Impact: 18.1), `_broadcastSocketData` (Defensive Guards, Impact: 14.9), `broadcastMouseLocation` (Defensive Guards, Impact: 6.8)
...
- **Heaviest Functions:** `loadFilesFromFirebase` (Defensive Guards, Impact: 20.3), `saveToFirebase` (Many-Argument Workhorses, Impact: 11.0), `isSavedToFirebase` (Callbacks & Closures, Impact: 7.7)

### 5. `packages/excalidraw/components/ToolButton.tsx` (TYPESCRIPT) -> Cumulative Risk: **648.25**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.66)
...
- **Heaviest Functions:** `onClick` (Compute Cores, Impact: 44.7), `onClick` (Interface Declarations, Impact: 2.0), `onClick` (Interface Declarations, Impact: 2.0)
...
- **Heaviest Functions:** `deepEqual` (Compute Cores, Impact: 19.2), `debugLogger` (I/O & Config Routines, Impact: 11.8), `logTimeAverage` (Compute Cores, Impact: 11.1)

### 7. `packages/utils/src/export.ts` (TYPESCRIPT) -> Cumulative Risk: **643.38**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.88)
- **Magnitude:** 154.54 | **LOC:** 217 | **CtrlFlow:** 26.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `exportToBlob` (Compute Cores, Impact: 22.9), `exportToCanvas` (Defensive Guards, Impact: 21.5), `exportToClipboard` (Compute Cores, Impact: 13.6)
...
- **Heaviest Functions:** `saveFiles` (Defensive Guards, Impact: 25.7), `isFileTracked` (Callbacks & Closures, Impact: 7.5), `updateStaleImageStatuses` (Callbacks & Closures, Impact: 6.9)
...
- **Heaviest Functions:** `craftInvokerFunction` (Many-Argument Workhorses, Impact: 68.6), `__embind_register_std_string` (Compute Cores, Impact: 53.5), `genericPointerToWireType` (Compute Cores, Impact: 43.8)
...
- **Heaviest Functions:** `normalize` (Defensive Guards, Impact: 32.5), `getValue` (Callbacks & Closures, Impact: 12.5), `flush` (Compute Cores, Impact: 9.5)
...
**Top Internal Functions/Classes:**
  ...
  * `genericPointerToWireType` **(Compute Cores)** (Impact: 43.8)
...
**Top Internal Functions/Classes:**
  ...
  * `effector` **(Compute Cores)** (Impact: 80.9)
  * `maybeDragNewGenericElement` **(Many-Argument Workhorses)** (Impact: 45.9)
  * `handleAppOnDrop` **(Compute Cores)** (Impact: 43.0)
...
**Top Internal Functions/Classes:**
  ...
  * `normalizeSelectedPoints` **(Compute Cores)** (Impact: 139.8)
...
**Top Internal Functions/Classes:**
  ...
  * `getMinMaxXYFromCurvePathOps` **(Compute Cores)** (Impact: 25.2)
...
**Top Internal Functions/Classes:**
  ...
  * `extract` **(Compute Cores)** (Impact: 23.6)
  * `getNextCommandsByCategory` **(Compute Cores)** (Impact: 20.7)
  * `perform` **(Defensive Guards)** (Impact: 14.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `createLibraryUpdate` **(Compute Cores)** (Impact: 14.1)
