...
> **Composition Archetype:** `Flat Modular Platform` (z -0.88; from the repo's file-archetype mix)
...
| connectivity | 8823 | 731 | 10 | `wp-content/themes/twentytwentyone/style-rtl.css` |
...
- `parseEBML` **(Compute Cores)** (@ `wp-includes/ID3/module.audio-video.matroska.php`) -> Impact: **563.0** | LOC: 738
...
### 2. `wp-includes/js/backbone.js` (JAVASCRIPT) -> Cumulative Risk: **699.17**
...
### 7. `wp-includes/js/underscore.js` (JAVASCRIPT) -> Cumulative Risk: **648.5**
...
- **Heaviest Functions:** `isEqual` (Defensive Guards, Impact: 107.4), `collectNonEnumProps` (Defensive Guards, Impact: 16.4), `restArguments` (Compute Cores, Impact: 15.0)
...
**Top Internal Functions/Classes:**
  ...
  * `wp_prepare_attachment_for_js` **(Compute Cores)** (Impact: 86.2)
...
**Top Internal Functions/Classes:**
  ...
  * `Analyze` **(Compute Cores)** (Impact: 108.7)
    * *Intent:* /** * @return bool */
...
**Top Internal Functions/Classes:**
  ...
  * `parse_tax_query` **(Compute Cores)** (Impact: 78.8)
...
- **Risk Profile:** Cognitive Load (28.657%), Tech Debt (31.2239%)
**Top Internal Functions/Classes:**
