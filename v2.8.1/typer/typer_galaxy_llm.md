...
> **File Composition:** Interface Declarations Files 36%, Data / Markup / Trivial 28%, Defensive Guards Files 19%, Generic / Templated Code Files 5%, Parameter Forwarders Files 4%
...
- `get_help_record` **(Compute Cores)** (@ `typer-0.24.1/typer/core.py`) -> Impact: **61.9** | LOC: 94
...
- `get_click_type` **(Compute Cores)** (@ `typer-0.24.1/typer/main.py`) -> Impact: **60.5** | LOC: 101
...
- **Heaviest Functions:** `complete` (Compute Cores, Impact: 10.7), `get_completion_args` (Generic / Templated Code, Impact: 6.2), `get_completion_args` (Generic / Templated Code, Impact: 6.2)
...
- **Heaviest Functions:** `get_help_record` (Compute Cores, Impact: 61.9), `_main` (Many-Argument Workhorses, Impact: 61.6), `_get_default_string` (Many-Argument Workhorses, Impact: 42.6)

### 3. `typer-0.24.1/typer/utils.py` (PYTHON) -> Cumulative Risk: **716.73**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.31)
...
- **Heaviest Functions:** `get_params_from_function` (Compute Cores, Impact: 25.2), `__str__` (Compute Cores, Impact: 7.6), `parse_boolean_env_var` (Generic / Templated Code, Impact: 7.4)

### 4. `typer-0.24.1/typer/cli.py` (PYTHON) -> Cumulative Risk: **656.9**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.95)
...
- **Heaviest Functions:** `get_docs_for_click` (Many-Argument Workhorses, Impact: 80.8), `get_typer_from_module` (Defensive Guards, Impact: 22.1), `docs` (Many-Argument Workhorses, Impact: 21.9)

### 5. `typer-0.24.1/typer/models.py` (PYTHON) -> Cumulative Risk: **630.93**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.42)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 40.2), `__init__` (Many-Argument Workhorses, Impact: 29.9), `__init__` (Many-Argument Workhorses, Impact: 11.2)
...
- **Heaviest Functions:** `get_click_type` (Compute Cores, Impact: 60.5), `get_click_param` (Compute Cores, Impact: 46.2), `solve_typer_info_help` (Defensive Guards, Impact: 32.9)
...
- **Heaviest Functions:** `shell_complete` (Many-Argument Workhorses, Impact: 14.3), `show_callback` (Defensive Guards, Impact: 12.9), `install_callback` (Defensive Guards, Impact: 10.5)
...
- **Heaviest Functions:** `install` (Many-Argument Workhorses, Impact: 21.8), `install_powershell` (Defensive Guards, Impact: 14.1), `install_zsh` (Many-Argument Workhorses, Impact: 9.3)
...
- **Heaviest Functions:** `_print_options_panel` (Many-Argument Workhorses, Impact: 64.2), `rich_format_help` (Many-Argument Workhorses, Impact: 55.0), `_get_parameter_help` (Many-Argument Workhorses, Impact: 50.4)
...
- **Heaviest Functions:** `complete_name` (Interface Declarations, Impact: 4.5), `main` (Interface Declarations, Impact: 1.7)
...
**Top Internal Functions/Classes:**
  * `get_click_type` **(Compute Cores)** (Impact: 60.5)
  ...
  * `get_param_callback` **(Compute Cores)** (Impact: 31.9)
...
**Top Internal Functions/Classes:**
  * `get_help_record` **(Compute Cores)** (Impact: 61.9)
  ...
  * `_typer_param_setup_autocompletion_compat` **(Defensive Guards)** (Impact: 17.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_get_help_text` **(Many-Argument Workhorses)** (Impact: 12.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_callback_4_list_none` **(Tests & Verification)** (Impact: 6.1)
  * `main` **(Generic / Templated Code)** (Impact: 4.6)
  * `test_too_many_parsers` **(Tests & Verification)** (Impact: 4.1)
  * `test_forward_references` **(Tests & Verification)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__str__` **(Compute Cores)** (Impact: 7.6)
  * `parse_boolean_env_var` **(Generic / Templated Code)** (Impact: 7.4)
  * `_split_annotation_from_typer_annotations` **(Generic / Templated Code)** (Impact: 6.2)
  * `_param_type_to_user_string` **(Generic / Templated Code)** (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  * `opt` **(Interface Declarations)** (Impact: 4.5)
  * `opt` **(Interface Declarations)** (Impact: 4.5)
  * `opt` **(Generic / Templated Code)** (Impact: 4.5)
  * `test_optional` **(Tests & Verification)** (Impact: 3.9)
  * `test_union_type_optional` **(Tests & Verification)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_install_completion_placeholder_function` **(Generic / Templated Code)** (Impact: 7.8)
    * *Intent:* # Create a fake command function to extract the completion parameters
  * `_install_completion_no_auto_placeholder_function` **(Generic / Templated Code)** (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_scripts` **(Defensive Guards)** (Impact: 3.8)
...
**Top Internal Functions/Classes:**
  * `test_forbid_default_factory_and_default_value_in_annotated` **(Tests & Verification)** (Impact: 2.6)
  * `test_forbid_default_and_default_factory_with_default_param` **(Tests & Verification)** (Impact: 2.6)
  * `test_forbid_annotated_param_and_default_param` **(Tests & Verification)** (Impact: 2.4)
  * `test_allow_default_factory_with_default_param` **(Tests & Verification)** (Impact: 2.1)
  * `test_forbid_default_value_in_annotated_argument` **(Tests & Verification)** (Impact: 1.8)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `test_completion_install_bash` **(I/O & Config Routines)** (Impact: 3.9)
  * `test_completion_install_powershell` **(Tests & Verification)** (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  * `cmd` **(Interface Declarations)** (Impact: 4.5)
  * `test_annotated_option_with_argname_doesnt_mutate_multiple_calls` **(Tests & Verification)** (Impact: 3.9)
  * `test_annotated_custom_path` **(Tests & Verification)** (Impact: 2.9)
  * `test_annotated_argument_with_default_factory` **(Tests & Verification)** (Impact: 1.9)
  * `test_annotated_argument_with_default` **(Tests & Verification)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_completion_colon_bash_all` **(Interface Declarations)** (Impact: 1.8)
  * `test_completion_colon_bash_partial` **(Interface Declarations)** (Impact: 1.8)
  * `test_completion_colon_bash_single` **(Interface Declarations)** (Impact: 1.8)
  * `test_completion_colon_powershell_all` **(Interface Declarations)** (Impact: 1.8)
  * `test_completion_colon_powershell_partial` **(Interface Declarations)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_scripts` **(Defensive Guards)** (Impact: 3.5)
  * `test_help` **(Defensive Guards)** (Impact: 1.8)
  * `test_help_items` **(Defensive Guards)** (Impact: 1.8)
...
**Top Internal Functions/Classes:**
  * `test_typo_suggestion_multiple_matches` **(Tests & Verification)** (Impact: 2.9)
    * *Intent:* """Test that multiple suggestions are shown when there are multiple close matches"""
  * `test_typo_suggestion_exact_match_works` **(Tests & Verification)** (Impact: 2.0)
    * *Intent:* """Test that exact matches still work normally"""
  * `test_typo_suggestion_enabled` **(Tests & Verification)** (Impact: 1.8)
    * *Intent:* """Test that typo suggestions work when enabled"""
  * `test_typo_suggestion_no_matches` **(Tests & Verification)** (Impact: 1.8)
    * *Intent:* """Test that no suggestions are shown when there are no close matches"""
  * `test_typo_suggestion_disabled` **(Tests & Verification)** (Impact: 1.8)
...
**Top Internal Functions/Classes:**
  ...
  * `test_completion_source_bash` **(Interface Declarations)** (Impact: 1.7)
  * `test_completion_source_powershell` **(Interface Declarations)** (Impact: 1.7)
  * `test_completion_source_pwsh` **(Interface Declarations)** (Impact: 1.7)
  * `test_show_completion` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 10 instances
* *Amplified Rce:* 10 instances
* *Amplified Cascading Flux:* 3 instances
