...
> **Composition Archetype:** `Flat Modular Platform` (z -1.47; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 51%, Declarative / Non-Code 17%, Large Core Modules 8%, Callbacks & Closures Files 5%, Interface Declarations Files 4%
...
- `exportTo` **(Compute Cores)** (@ `packages/yarnpkg-core/sources/Manifest.ts`) -> Impact: **154.6** | LOC: 217
...
- **Heaviest Functions:** `reportInfoOnce` (Defensive Guards, Impact: 18.5), `reportWarningOnce` (Defensive Guards, Impact: 18.5), `reportErrorOnce` (Defensive Guards, Impact: 18.5)
...
- **Heaviest Functions:** `read` (Defensive Guards, Impact: 85.5), `write` (Defensive Guards, Impact: 13.6), `appendFile` (Defensive Guards, Impact: 12.7)
...
- **Heaviest Functions:** `parseRequest` (Compute Cores, Impact: 61.4), `[RequestType.PackageInfo]` (Many-Argument Workhorses, Impact: 29.4), `listener` (Callbacks & Closures, Impact: 26.1)

### 4. `packages/plugin-pack/sources/packUtils.ts` (TYPESCRIPT) -> Cumulative Risk: **727.17**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.86)
...
- **Heaviest Functions:** `maybeRejectPath` (Defensive Guards, Impact: 43.3), `genPackStream` (Compute Cores, Impact: 36.6), `walk` (Compute Cores, Impact: 33.6)
...
- **Heaviest Functions:** `pipevp` (Many-Argument Workhorses, Impact: 50.0), `closeStreams` (Callbacks & Closures, Impact: 18.6), `execvp` (Many-Argument Workhorses, Impact: 16.9)
...
- **Heaviest Functions:** `copySync` (Many-Argument Workhorses, Impact: 33.6), `mkdirpPromise` (Defensive Guards, Impact: 24.5), `mkdirpSync` (Defensive Guards, Impact: 24.5)
...
- **Heaviest Functions:** `evaluateVariable` (Many-Argument Workhorses, Impact: 95.4), `interpolateArguments` (Many-Argument Workhorses, Impact: 55.7), `redirect` (Many-Argument Workhorses, Impact: 52.6)
...
- **Heaviest Functions:** `copyFileViaIndex` (Many-Argument Workhorses, Impact: 88.5), `copyImpl` (Many-Argument Workhorses, Impact: 58.7), `copyFolder` (Many-Argument Workhorses, Impact: 46.6)
...
- **Heaviest Functions:** `genCommitMessage` (Compute Cores, Impact: 19.3), `isYarnFile` (Compute Cores, Impact: 11.1), `checkConsensus` (Compute Cores, Impact: 9.5)
...
- **Heaviest Functions:** `loadPatchFiles` (Callbacks & Closures, Impact: 24.9), `extractPackageToDisk` (Defensive Guards, Impact: 16.8), `parseSpec` (Compute Cores, Impact: 15.0)
...
**Top Internal Functions/Classes:**
  ...
  * `applyVirtualResolutionMutations` **(Compute Cores)** (Impact: 145.4)
  ...
  * `install` **(Compute Cores)** (Impact: 66.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getFileSource` **(Compute Cores)** (Impact: 24.2)
...
**Top Internal Functions/Classes:**
  ...
  * `exportTo` **(Compute Cores)** (Impact: 154.6)
...
**Top Internal Functions/Classes:**
  * `limitOpenFiles` **(Defensive Guards)** (Impact: 28.5)
  ...
  * `findMount` **(Compute Cores)** (Impact: 14.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
...
- **Risk Profile:** Cognitive Load (100.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `onentry` **(Tests & Verification)** (Impact: 17.8)
