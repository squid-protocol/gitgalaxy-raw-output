...
> **Composition Archetype:** `Flat Modular Platform` (z +0.42; from the repo's file-archetype mix)
> **File Composition:** Many-Argument Workhorses Files 22%, Defensive Guards Files 20%, Large Core Modules 14%, Declarative / Non-Code 11%, Data / Markup / Trivial 10%
...
- **Composition Archetype:** `Interface Declarations Files` (z +1.74)
...
- **Heaviest Functions:** `_bidirectional_dijkstra` (Many-Argument Workhorses, Impact: 113.4), `_bidirectional_pred_succ` (Many-Argument Workhorses, Impact: 88.4), `shortest_simple_paths` (Many-Argument Workhorses, Impact: 38.7)

### 2. `benchmarks/benchmarks/benchmark_algorithms.py` (PYTHON) -> Cumulative Risk: **677.55**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +0.17)
...
- **Heaviest Functions:** `_make_weighted_benchmark_graphs` (Compute Cores, Impact: 13.6), `dijkstra_relaxation_worst_case` (Compute Cores, Impact: 5.2), `_make_tournament_benchmark_graphs` (Interface Declarations, Impact: 3.2)

### 3. `networkx/algorithms/centrality/closeness.py` (PYTHON) -> Cumulative Risk: **649.45**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.35)
...
- **Heaviest Functions:** `incremental_closeness_centrality` (Many-Argument Workhorses, Impact: 48.7), `closeness_centrality` (Many-Argument Workhorses, Impact: 46.5)
...
- **Heaviest Functions:** `_pagerank_python` (Many-Argument Workhorses, Impact: 59.9), `_pagerank_scipy` (Many-Argument Workhorses, Impact: 49.2), `google_matrix` (Many-Argument Workhorses, Impact: 31.1)
...
- **Heaviest Functions:** `add_attributes` (Many-Argument Workhorses, Impact: 64.5), `make_graph` (Compute Cores, Impact: 38.5), `add_edge` (Many-Argument Workhorses, Impact: 33.9)
...
- **Heaviest Functions:** `pytest_collection_modifyitems` (Compute Cores, Impact: 13.2), `pytest_configure` (Compute Cores, Impact: 11.5), `pytest_addoption` (Interface Declarations, Impact: 3.7)

### 7. `networkx/algorithms/similarity.py` (PYTHON) -> Cumulative Risk: **607.5**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.11)
...
- **Heaviest Functions:** `optimize_edit_paths` (Many-Argument Workhorses, Impact: 722.2), `match_edges` (Many-Argument Workhorses, Impact: 118.5), `get_edit_ops` (Many-Argument Workhorses, Impact: 103.9)

### 8. `networkx/algorithms/centrality/betweenness.py` (PYTHON) -> Cumulative Risk: **601.33**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.72)
...
- **Heaviest Functions:** `_rescale` (Many-Argument Workhorses, Impact: 74.3), `betweenness_centrality` (Many-Argument Workhorses, Impact: 38.6), `edge_betweenness_centrality` (Many-Argument Workhorses, Impact: 31.9)
...
- **Heaviest Functions:** `display` (Many-Argument Workhorses, Impact: 291.6), `draw_networkx_edges` (Many-Argument Workhorses, Impact: 263.1), `draw_networkx_edge_labels` (Many-Argument Workhorses, Impact: 138.7)

### 10. `networkx/algorithms/centrality/reaching.py` (PYTHON) -> Cumulative Risk: **595.3**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.12)
...
- **Heaviest Functions:** `local_reaching_centrality` (Many-Argument Workhorses, Impact: 38.7), `global_reaching_centrality` (Many-Argument Workhorses, Impact: 18.2), `_average_weight` (Many-Argument Workhorses, Impact: 9.2)
...
**Top Internal Functions/Classes:**
  ...
  * `make_graph` **(Compute Cores)** (Impact: 38.5)
  ...
  * `add_edges` **(Defensive Guards)** (Impact: 27.4)
...
**Top Internal Functions/Classes:**
  ...
  * `is_maximal_matching` **(Compute Cores)** (Impact: 28.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_feasible_look_ahead_same_labels` **(Defensive Guards)** (Impact: 28.4)
  ...
  * `test_same_in_out_degrees_no_candidate` **(Many-Argument Workhorses)** (Impact: 17.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 172 instances
...
**Top Internal Functions/Classes:**
  * `add_graph_element` **(Compute Cores)** (Impact: 45.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_feasible_node_pair` **(Compute Cores)** (Impact: 80.7)
...
**Top Internal Functions/Classes:**
  ...
  * `_write_partition` **(Compute Cores)** (Impact: 22.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_parse_network_text` **(Compute Cores)** (Impact: 65.4)
  ...
  * `_find_sources` **(Callbacks & Closures)** (Impact: 13.1)
...
**Top Internal Functions/Classes:**
  ...
  * `unconstrained_bridge_augmentation` **(Compute Cores)** (Impact: 38.7)
