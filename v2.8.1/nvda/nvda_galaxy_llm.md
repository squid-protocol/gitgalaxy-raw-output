...
> **File Composition:** Large Core Modules 33%, Data / Markup / Trivial 18%, Interface Declarations Files 12%, Declarative / Non-Code 12%, Generic / Templated Code Files 10%
...
- **Heaviest Functions:** `chooseNVDAObjectOverlayClasses` (Defensive Guards, Impact: 18.8), `_getFormatFieldAtRange` (Defensive Guards, Impact: 9.8), `__init__` (Type Conversions, Impact: 8.7)

### 2. `source/globalCommands.py` (PYTHON) -> Cumulative Risk: **729.59**
...
- **Heaviest Functions:** `script_navigatorObject_current` (Defensive Guards, Impact: 42.3), `script_toggleScreenCurtain` (Many-Argument Workhorses, Impact: 35.5), `_reportFormattingHelper` (Many-Argument Workhorses, Impact: 33.9)
...
- **Heaviest Functions:** `makeSettings` (Many-Argument Workhorses, Impact: 94.4), `makeSettings` (Many-Argument Workhorses, Impact: 49.7), `makeSettings` (Many-Argument Workhorses, Impact: 40.2)

### 4. `source/_bridge/base.py` (PYTHON) -> Cumulative Risk: **695.47**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `_bgEventLoop` (Defensive Guards, Impact: 18.8), `_createPipe` (Many-Argument Workhorses, Impact: 16.1), `terminate` (Defensive Guards, Impact: 13.8)

### 5. `source/_synthDrivers32/sapi4.py` (PYTHON) -> Cumulative Risk: **692.95**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `_set_voice` (Many-Argument Workhorses, Impact: 54.5), `speak` (Defensive Guards, Impact: 39.9), `IAudio_UnClaim` (Compute Cores, Impact: 15.4)

### 6. `source/remotePythonConsole.py` (PYTHON) -> Cumulative Risk: **680.09**
...
- **Heaviest Functions:** `handle` (Defensive Guards, Impact: 6.1), `setPrompt` (Parameter Forwarders, Impact: 3.7), `execute` (Type Conversions, Impact: 1.9)

### 7. `source/audioDucking.py` (PYTHON) -> Cumulative Risk: **671.47**
...
- **Heaviest Functions:** `setAudioDuckingMode` (Compute Cores, Impact: 23.7), `enable` (Compute Cores, Impact: 21.9), `_unensureDucked` (Defensive Guards, Impact: 9.5)

### 8. `source/hwIo/ioThread.py` (PYTHON) -> Cumulative Risk: **669.45**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.50)
- **Magnitude:** 171.68 | **LOC:** 272 | **CtrlFlow:** 12.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9999%), Concurrency (99.6453%), Documentation (95.0%)
- **Heaviest Functions:** `queueAsCompletionRoutine` (Many-Argument Workhorses, Impact: 13.5), `_internalApc` (Defensive Guards, Impact: 9.8), `_internalCompletionRoutine` (Many-Argument Workhorses, Impact: 9.4)

### 9. `source/speech/speech.py` (PYTHON) -> Cumulative Risk: **668.53**
...
- **Heaviest Functions:** `getFormatFieldSpeech` (Many-Argument Workhorses, Impact: 848.0), `getTextInfoSpeech` (Many-Argument Workhorses, Impact: 397.3), `getControlFieldSpeech` (Many-Argument Workhorses, Impact: 262.4)

### 10. `source/appModules/outlook.py` (PYTHON) -> Cumulative Risk: **668.11**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.74)
- **Magnitude:** 813.7 | **LOC:** 792 | **CtrlFlow:** 21.9% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (97.854%), Tech Debt (96.5336%)
- **Heaviest Functions:** `chooseNVDAObjectOverlayClasses` (Many-Argument Workhorses, Impact: 68.9), `_get_name` (Defensive Guards, Impact: 53.2), `event_NVDAObject_init` (Compute Cores, Impact: 25.7)
...
**Top Internal Functions/Classes:**
  ...
  * `script_toggleScreenCurtain` **(Many-Argument Workhorses)** (Impact: 35.5)
...
**Top Internal Functions/Classes:**
  * `_addTextWithFields` **(Many-Argument Workhorses)** (Impact: 88.5)
...
**Top Internal Functions/Classes:**
  * `findOverlayClasses` **(Compute Cores)** (Impact: 141.5)
...
**Top Internal Functions/Classes:**
  * `_normalizeControlField` **(Compute Cores)** (Impact: 55.4)
  ...
  * `_iterHeadings` **(Many-Argument Workhorses)** (Impact: 40.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_set_selection` **(Many-Argument Workhorses)** (Impact: 34.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_get_selection` **(Compute Cores)** (Impact: 40.1)
  ...
  * `_get__overlapInfo` **(Compute Cores)** (Impact: 30.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (53.5983%), Tech Debt (20.0211%)
**Top Internal Functions/Classes:**
  ...
  * `detectAndGenerateColumnFormatXML` **(Compute Cores)** (Impact: 49.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `IUIAutomationFocusChangedEventHandler_HandleFocusChangedEvent` **(Compute Cores)** (Impact: 54.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_findSimpleNext` **(Many-Argument Workhorses)** (Impact: 53.0)
  * `_get_presentationType` **(Compute Cores)** (Impact: 36.1)
  * `event_mouseMove` **(Many-Argument Workhorses)** (Impact: 34.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_setSystemConfig` **(Many-Argument Workhorses)** (Impact: 25.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_findCaretOffsetFromLocation` **(Many-Argument Workhorses)** (Impact: 32.9)
  * `_getFieldsInRange` **(Many-Argument Workhorses)** (Impact: 29.6)
  * `processWindowChunksInLine` **(Many-Argument Workhorses)** (Impact: 27.6)
  * `_getSelectionOffsets` **(Defensive Guards)** (Impact: 21.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `move` **(Many-Argument Workhorses)** (Impact: 101.7)
  * `__init__` **(Defensive Guards)** (Impact: 29.9)
  ...
  * `_getUnitOffsets` **(Many-Argument Workhorses)** (Impact: 23.5)
...
**Top Internal Functions/Classes:**
  ...
  * `pumpAll` **(Compute Cores)** (Impact: 32.2)
...
- `source/gui/installerGui.py` -> Churn: **56.67%** | Cog Load: 63.8788% | Debt: 18.7231%
- `source/speech/speech.py` -> Churn: **52.18%** | Cog Load: 84.2424% | Debt: 0.0%
