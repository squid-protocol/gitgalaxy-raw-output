...
> **File Composition:** Data / Markup / Trivial 19%, Generic / Templated Code Files 15%, Large Core Modules 15%, Many-Argument Workhorses Files 15%, Defensive Guards Files 12%
...
- `open` **(Many-Argument Workhorses)** (@ `wheel-0.46.3/src/wheel/wheelfile.py`) -> Impact: **30.7** | LOC: 33
...
- **Composition Archetype:** `Compute Cores Files` (z +2.13)
...
- **Heaviest Functions:** `open` (Many-Argument Workhorses, Impact: 30.7), `__init__` (Many-Argument Workhorses, Impact: 20.7), `write_files` (Compute Cores, Impact: 16.6)
...
- **Heaviest Functions:** `calculate_macosx_platform_tag` (Many-Argument Workhorses, Impact: 50.8), `extract_macosx_min_system_version` (Defensive Guards, Impact: 22.6), `read_mach_header` (C Struct Operations, Impact: 18.0)

### 4. `wheel-0.46.3/src/wheel/_commands/tags.py` (PYTHON) -> Cumulative Risk: **557.24**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.13)
...
- **Heaviest Functions:** `tags` (Many-Argument Workhorses, Impact: 56.0), `_compute_tags` (Type Conversions, Impact: 7.5)
...
- **Heaviest Functions:** `get_tag` (Compute Cores, Impact: 28.3), `egg2dist` (Many-Argument Workhorses, Impact: 25.5), `license_paths` (Compute Cores, Impact: 24.8)
...
- **Heaviest Functions:** `parser` (I/O & Config Routines, Impact: 7.2), `parse_build_tag` (Generic / Templated Code, Impact: 6.0), `tags_f` (Compute Cores, Impact: 5.1)
...
- **Heaviest Functions:** `generate_requirements` (Compute Cores, Impact: 14.2), `pkginfo_to_metadata` (Compute Cores, Impact: 14.0), `split_sections` (Compute Cores, Impact: 12.6)
...
- **Heaviest Functions:** `main` (Interface Declarations, Impact: 2.5)

### 9. `wheel-0.46.3/src/wheel/_commands/pack.py` (PYTHON) -> Cumulative Risk: **423.82**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.23)
...
- **Heaviest Functions:** `pack` (Many-Argument Workhorses, Impact: 30.9), `compute_tagline` (Generic / Templated Code, Impact: 6.2)
...
- **Heaviest Functions:** `test_pack` (Many-Argument Workhorses, Impact: 32.6)
...
**Top Internal Functions/Classes:**
  ...
  * `read_mach_header` **(C Struct Operations)** (Impact: 18.0)
...
**Top Internal Functions/Classes:**
  * `open` **(Many-Argument Workhorses)** (Impact: 30.7)
...
**Top Internal Functions/Classes:**
  ...
  * `test_convert_egg_directory` **(Many-Argument Workhorses)** (Impact: 9.6)
...
**Top Internal Functions/Classes:**
  ...
  * `split_sections` **(Compute Cores)** (Impact: 12.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_compute_tags` **(Type Conversions)** (Impact: 7.5)
...
**Top Internal Functions/Classes:**
  ...
  * `test_testzip_bad_hash` **(Tests & Verification)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `tags_f` **(Compute Cores)** (Impact: 5.1)
...
**Top Internal Functions/Classes:**
  * `test_compare_sdists` **(Many-Argument Workhorses)** (Impact: 6.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_pkginfo_to_metadata` **(Tests & Verification)** (Impact: 4.3)
  * `test_metadata_deprecated` **(Tests & Verification)** (Impact: 2.2)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `_not_warning` **(Generic / Templated Code)** (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `test_import_bdist_wheel` **(Tests & Verification)** (Impact: 2.1)
**Structural Signatures (Net Mitigated Signals):**
