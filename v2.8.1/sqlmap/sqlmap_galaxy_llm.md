...
> **Composition Archetype:** `Hub-Coupled App` (z +2.09; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 28%, Interface Declarations Files 26%, Compute Cores Files 20%, Parameter Forwarders Files 9%, Large Core Modules 6%
...
- `getPrivileges` **(Compute Cores)** (@ `plugins/generic/users.py`) -> Impact: **245.0** | LOC: 292
- `getTables` **(Compute Cores)** (@ `plugins/generic/databases.py`) -> Impact: **234.7** | LOC: 259
- `main` **(Compute Cores)** (@ `sqlmap.py`) -> Impact: **180.9** | LOC: 478
  * *Intent:* """ Main function of sqlmap when running from command line. """
- `dictionaryAttack` **(Compute Cores)** (@ `lib/utils/hash.py`) -> Impact: **160.7** | LOC: 329
...
- **Heaviest Functions:** `do_REQUEST` (Compute Cores, Impact: 71.7), `do_POST` (Compute Cores, Impact: 12.6), `finish_request` (Defensive Guards, Impact: 4.3)
...
- **Heaviest Functions:** `timeout` (Many-Argument Workhorses, Impact: 13.4), `run` (Defensive Guards, Impact: 4.6), `__init__` (Interface Declarations, Impact: 1.6)
...
- **Heaviest Functions:** `osPwn` (Compute Cores, Impact: 93.8), `osShell` (Compute Cores, Impact: 24.4), `osSmb` (Compute Cores, Impact: 20.8)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 12.4), `toDict` (Compute Cores, Impact: 11.2), `parse` (Defensive Guards, Impact: 10.2)
...
- **Heaviest Functions:** `goahead` (Compute Cores, Impact: 93.8), `parse_starttag` (Compute Cores, Impact: 60.2), `_convert_ref` (Compute Cores, Impact: 24.7)
...
- **Heaviest Functions:** `getColumns` (Many-Argument Workhorses, Impact: 133.5), `getTables` (Compute Cores, Impact: 43.5), `getDbs` (Compute Cores, Impact: 6.6)
...
- **Heaviest Functions:** `dictionaryAttack` (Compute Cores, Impact: 160.7), `_bruteProcessVariantB` (Many-Argument Workhorses, Impact: 86.8), `_bruteProcessVariantA` (Many-Argument Workhorses, Impact: 86.1)
...
- **Heaviest Functions:** `getColumns` (Many-Argument Workhorses, Impact: 148.6), `getTables` (Compute Cores, Impact: 49.1), `getPrivileges` (Many-Argument Workhorses, Impact: 17.5)
...
- **Heaviest Functions:** `runGui` (Compute Cores, Impact: 73.2), `populate_tab` (Compute Cores, Impact: 24.0), `run` (I/O & Config Routines, Impact: 19.2)
...
- **Heaviest Functions:** `dumpTable` (Many-Argument Workhorses, Impact: 313.6), `dumpFoundColumn` (Many-Argument Workhorses, Impact: 59.2), `dumpFoundTables` (Compute Cores, Impact: 35.8)
...
**Top Internal Functions/Classes:**
  ...
  * `getTables` **(Compute Cores)** (Impact: 234.7)
...
**Top Internal Functions/Classes:**
  * `dictionaryAttack` **(Compute Cores)** (Impact: 160.7)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Interface Declarations)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parse_starttag` **(Compute Cores)** (Impact: 60.2)
    * *Intent:* # Internal -- handle starttag, return length or -1 if not terminated
  * `_convert_ref` **(Compute Cores)** (Impact: 24.7)
...
**Top Internal Functions/Classes:**
  ...
  * `run` **(Compute Cores)** (Impact: 32.4)
    * *Intent:* """Main UI loop"""
  * `_import_config` **(Compute Cores)** (Impact: 30.4)
...
**Top Internal Functions/Classes:**
  ...
  * `osShell` **(Compute Cores)** (Impact: 24.4)
...
**Top Internal Functions/Classes:**
  * `tableExists` **(Compute Cores)** (Impact: 82.4)
...
**Top Internal Functions/Classes:**
  * `main` **(Compute Cores)** (Impact: 180.9)
    * *Intent:* """ Main function of sqlmap when running from command line. """
  * `checkEnvironment` **(I/O & Config Routines)** (Impact: 7.2)
...
**Top Internal Functions/Classes:**
  * `readFile` **(Compute Cores)** (Impact: 41.7)
...
**Top Internal Functions/Classes:**
  * `main` **(Compute Cores)** (Impact: 78.7)
  ...
  * `usage` **(Interface Declarations)** (Impact: 5.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_dataToScr` **(Many-Argument Workhorses)** (Impact: 11.5)
  * `_stackedWriteFileCertutilExe` **(Many-Argument Workhorses)** (Impact: 9.3)
...
**Top Internal Functions/Classes:**
  ...
  * `init` **(Interface Declarations)** (Impact: 3.9)
