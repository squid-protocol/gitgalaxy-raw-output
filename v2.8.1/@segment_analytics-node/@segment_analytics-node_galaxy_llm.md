...
> **File Composition:** Data / Markup / Trivial 39%, Callbacks & Closures Files 17%, Declarative / Non-Code 17%, Large Core Modules 11%, Defensive Guards Files 6%
...
- `flush` **(Callbacks & Closures)** (@ `package/src/app/analytics-node.ts`) -> Impact: **12.8** | LOC: 30
...
- `_dispatch` **(Defensive Guards)** (@ `package/src/app/analytics-node.ts`) -> Impact: **7.8** | LOC: 18
...
- **Heaviest Functions:** `send` (Compute Cores, Impact: 41.3), `constructor` (Defensive Guards, Impact: 22.6), `enqueue` (Defensive Guards, Impact: 17.1)
...
- **Heaviest Functions:** `constructor` (Defensive Guards, Impact: 18.8), `flush` (Callbacks & Closures, Impact: 12.8), `page` (Defensive Guards, Impact: 11.5)
...
- **Heaviest Functions:** `tryAdd` (Compute Cores, Impact: 7.1), `constructor` (State Mutators, Impact: 1.6), `calculateSize` (Interface Declarations, Impact: 1.6)

### 4. `package/src/app/dispatch-emit.ts` (TYPESCRIPT) -> Cumulative Risk: **376.38**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.93)
...
- **Heaviest Functions:** `dispatchAndEmit` (Many-Argument Workhorses, Impact: 12.6), `normalizeDispatchCb` (Callbacks & Closures, Impact: 3.2)

### 5. `package/src/app/event-queue.ts` (TYPESCRIPT) -> Cumulative Risk: **364.05**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +0.46)
...
- **Heaviest Functions:** `getAttempts` (Defensive Guards, Impact: 4.4), `updateAttempts` (Interface Declarations, Impact: 1.6), `constructor` (State Mutators, Impact: 1.1)
...
- **Heaviest Functions:** `normalizeEvent` (State Mutators, Impact: 3.3), `createNodePlugin` (Callbacks & Closures, Impact: 2.4), `createConfiguredNodePlugin` (Callbacks & Closures, Impact: 2.2)

### 7. `package/src/app/event-factory.ts` (TYPESCRIPT) -> Cumulative Risk: **339.57**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.72)
...
- **Heaviest Functions:** `screen` (Generic / Templated Code, Impact: 2.1), `onFinishedEvent` (Callbacks & Closures, Impact: 1.7), `page` (Generic / Templated Code, Impact: 1.5)
...
- **Heaviest Functions:** `system` (Interface Declarations, Impact: 1.1)
...
- **Heaviest Functions:** `validateSettings` (Defensive Guards, Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `createBatch` **(Callbacks & Closures)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `flush` **(Callbacks & Closures)** (Impact: 12.8)
  ...
  * `_dispatch` **(Defensive Guards)** (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `calculateSize` **(Interface Declarations)** (Impact: 1.6)
...
**Top Internal Functions/Classes:**
  ...
  * `updateAttempts` **(Interface Declarations)** (Impact: 1.6)
