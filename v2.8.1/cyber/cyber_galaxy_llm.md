...
> **Composition Archetype:** `Hub-Coupled App` (z -0.60; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 27%, Data / Markup / Trivial 18%, Interface Declarations Files 15%, Declarative / Non-Code 13%, Defensive Guards Files 12%
...
- **Heaviest Functions:** `execBytecode` (Compute Cores, Impact: 63.9), `ipow` (Compute Cores, Impact: 16.7), `retain` (Many-Argument Workhorses, Impact: 12.5)
...
- **Heaviest Functions:** `copyImm64Simple` (Many-Argument Workhorses, Impact: 29.9), `copyImm64` (Many-Argument Workhorses, Impact: 19.8), `patch_imm64` (Defensive Guards, Impact: 7.5)
...
- **Heaviest Functions:** `__attribute__` (Annotated Framework Methods, Impact: 5.2), `__attribute__` (Annotated Framework Methods, Impact: 4.3), `__attribute__` (Annotated Framework Methods, Impact: 4.3)
...
- **Heaviest Functions:** `callUnion` (Many-Argument Workhorses, Impact: 23.7), `unwindStack` (Many-Argument Workhorses, Impact: 22.4), `markValue` (Many-Argument Workhorses, Impact: 19.3)
...
- **Heaviest Functions:** `cl_vm_evalx` (Many-Argument Workhorses, Impact: 8.5), `cl_vm_eval_path` (Many-Argument Workhorses, Impact: 7.9), `cl_vm_compile` (Defensive Guards, Impact: 7.5)
...
- **Heaviest Functions:** `gen_func` (Defensive Guards, Impact: 60.6), `genStmt` (Defensive Guards, Impact: 26.6), `genExpr` (Defensive Guards, Impact: 19.6)
...
- **Heaviest Functions:** `patchForBlockJumps` (Many-Argument Workhorses, Impact: 28.5), `patchBreaks` (Many-Argument Workhorses, Impact: 19.1), `nextTempDestValue` (Defensive Guards, Impact: 14.8)
...
- **Heaviest Functions:** `cb_byte_fmt` (Defensive Guards, Impact: 25.7), `cb_float_fmt` (Defensive Guards, Impact: 11.1), `cbi_unwrap_addr` (Compute Cores, Impact: 6.3)
...
- **Heaviest Functions:** `encodeMemory` (Many-Argument Workhorses, Impact: 65.6), `encodeRexPrefix` (Many-Argument Workhorses, Impact: 26.4), `encode` (Many-Argument Workhorses, Impact: 21.9)
...
- **Heaviest Functions:** `fromSlice` (Type Conversions, Impact: 3.8), `from_bytes` (Interface Declarations, Impact: 3.1), `vm_evalx` (Parameter Forwarders, Impact: 2.6)
...
**Top Internal Functions/Classes:**
  * `parseTightTermLeft` **(Defensive Guards)** (Impact: 84.9)
  ...
  * `parseForStatement` **(Defensive Guards)** (Impact: 53.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `genCompare` **(Defensive Guards)** (Impact: 38.7)
...
**Top Internal Functions/Classes:**
  * `intFmtExt` **(Many-Argument Workhorses)** (Impact: 26.0)
...
**Top Internal Functions/Classes:**
  ...
  * `allocVector` **(Many-Argument Workhorses)** (Impact: 15.2)
...
**Top Internal Functions/Classes:**
  * `execBytecode` **(Compute Cores)** (Impact: 63.9)
  * `ipow` **(Compute Cores)** (Impact: 16.7)
    * *Intent:* // Exponentiation by squaring.
  * `retain` **(Many-Argument Workhorses)** (Impact: 12.5)
  * `releaseOnly` **(Many-Argument Workhorses)** (Impact: 11.2)
  * `allocObjectInit` **(Compute Cores)** (Impact: 8.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `freeHeapPages` **(Defensive Guards)** (Impact: 14.0)
...
**Top Internal Functions/Classes:**
  * `writeSymName` **(Defensive Guards)** (Impact: 80.6)
...
**Top Internal Functions/Classes:**
  ...
  * `isConstEligible` **(Compute Cores)** (Impact: 13.5)
...
**Top Internal Functions/Classes:**
  ...
  * `tokenize` **(Compute Cores)** (Impact: 24.4)
...
**Top Internal Functions/Classes:**
  ...
  * `patchBreaks` **(Many-Argument Workhorses)** (Impact: 19.1)
  ...
  * `deinit` **(State Mutators)** (Impact: 11.9)
  * `addReportFmt` **(Many-Argument Workhorses)** (Impact: 11.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 3 instances
...
**Top Internal Functions/Classes:**
  ...
  * `loader` **(Many-Argument Workhorses)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
