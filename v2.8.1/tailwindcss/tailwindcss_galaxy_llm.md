...
> **Composition Archetype:** `Flat Modular Platform` (z -0.61; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 28%, Declarative / Non-Code 24%, Large Core Modules 12%, Compute Cores Files 10%, Tests & Verification Files 5%
...
- `createUtilities` **(Compute Cores)** (@ `packages/tailwindcss/src/utilities.ts`) -> Impact: **320.9** | LOC: 1779
...
- `constantFoldDeclarationAst` **(Compute Cores)** (@ `packages/tailwindcss/src/constant-fold-declaration.ts`) -> Impact: **194.1** | LOC: 244
...
- **Heaviest Functions:** `test` (Many-Argument Workhorses, Impact: 213.4), `spawn` (Compute Cores, Impact: 49.9), `exec` (Many-Argument Workhorses, Impact: 36.2)
...
- **Heaviest Functions:** `migratePostCSSConfig` (Defensive Guards, Impact: 43.8), `migratePostCSSJSConfig` (Compute Cores, Impact: 43.2), `migratePostCSSJsonConfig` (Compute Cores, Impact: 36.4)
...
- **Composition Archetype:** `Compute Cores Files` (z -0.47)
...
- **Heaviest Functions:** `split` (Compute Cores, Impact: 70.0)
...
- **Heaviest Functions:** `tailwindcss` (Compute Cores, Impact: 117.7), `generate` (Many-Argument Workhorses, Impact: 83.8), `onDependency` (Compute Cores, Impact: 47.9)
...
- **Heaviest Functions:** `parseCss` (Many-Argument Workhorses, Impact: 244.9), `build` (Compute Cores, Impact: 41.8), `build` (Compute Cores, Impact: 35.8)

### 6. `packages/@tailwindcss-upgrade/src/stylesheet.ts` (TYPESCRIPT) -> Cumulative Risk: **638.64**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.79)
...
- **Heaviest Functions:** `constructor` (Defensive Guards, Impact: 9.1), `analyzeImportPaths` (I/O & Config Routines, Impact: 9.1), `containsRule` (Callbacks & Closures, Impact: 8.2)

### 7. `packages/@tailwindcss-upgrade/src/codemods/css/analyze.ts` (TYPESCRIPT) -> Cumulative Risk: **638.36**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.15)
...
- **Heaviest Functions:** `analyze` (Compute Cores, Impact: 107.9), `import` (Compute Cores, Impact: 104.6)
...
- **Heaviest Functions:** `migrateTheme` (Many-Argument Workhorses, Impact: 117.7), `migrateContent` (Defensive Guards, Impact: 42.5), `canMigrateConfig` (Compute Cores, Impact: 40.1)
...
- **Heaviest Functions:** `runCommandLine` (Defensive Guards, Impact: 20.1), `streamStdin` (Compute Cores, Impact: 18.2), `formatCandidateResults` (Compute Cores, Impact: 9.2)
...
- **Heaviest Functions:** `ensureSourceDetectionRootExists` (Compute Cores, Impact: 61.6), `loadModule` (Many-Argument Workhorses, Impact: 31.0), `resolveCssId` (Compute Cores, Impact: 13.1)
...
**Top Internal Functions/Classes:**
  * `createUtilities` **(Compute Cores)** (Impact: 320.9)
...
**Top Internal Functions/Classes:**
  ...
  * `loadStylesheet` **(Annotated Framework Methods)** (Impact: 6.0)
  * `loadStylesheet` **(Tests & Verification)** (Impact: 5.1)
  * `module` **(Tests & Verification)** (Impact: 4.2)
  * `module` **(Annotated Framework Methods)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `module` **(Compute Cores)** (Impact: 6.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `withBOM` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `loadModule` **(Tests & Verification)** (Impact: 5.5)
  * `module` **(Tests & Verification)** (Impact: 4.2)
  * `loadModule` **(Tests & Verification)** (Impact: 3.0)
  * `loadModule` **(Interface Declarations)** (Impact: 2.1)
...
**Top Internal Functions/Classes:**
  * `loadModule` **(Compute Cores)** (Impact: 13.0)
  * `typography` **(Tests & Verification)** (Impact: 6.8)
  * `loadModule` **(Tests & Verification)** (Impact: 5.2)
  * `loadModule` **(Tests & Verification)** (Impact: 5.2)
...
**Top Internal Functions/Classes:**
  ...
  * `compileFn` **(Compute Cores)** (Impact: 81.9)
