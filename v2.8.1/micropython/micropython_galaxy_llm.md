...
| Avg Path Length | 2.5351 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +0.21; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 30%, Declarative / Non-Code 29%, Interface Declarations Files 13%, Large Core Modules 11%, Parameter Forwarders Files 4%
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 9.1 | 0.1 | 0.0 |
...
| connectivity | 16877 | 2444 | 9 | `py/obj.h` |
...
- `adc_wait_for_eoc_or_timeout` **(Many-Argument Workhorses)** (@ `ports/stm32/adc.c`) -> Impact: **406.0** | LOC: 845
...
- **Heaviest Functions:** `_serve` (Defensive Guards, Impact: 12.7), `open_connection` (Defensive Guards, Impact: 12.4), `stream_awrite` (Compute Cores, Impact: 9.3)
...
- **Heaviest Functions:** `run_until_complete` (Compute Cores, Impact: 37.1), `wait_io_event` (Compute Cores, Impact: 18.2), `remove` (Compute Cores, Impact: 12.7)
...
- **Heaviest Functions:** `_run_iter` (I/O & Config Routines, Impact: 18.8), `__next__` (Interface Declarations, Impact: 4.6), `_promote_to_task` (Defensive Guards, Impact: 4.3)
...
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `gather` (Defensive Guards, Impact: 42.2), `wait_for` (Defensive Guards, Impact: 11.4), `done` (Defensive Guards, Impact: 11.3)
...
- **Heaviest Functions:** `mp_call_prepare_args_n_kw_var` (Many-Argument Workhorses, Impact: 86.0), `mp_unary_op` (Compute Cores, Impact: 68.1), `mp_convert_member_lookup` (Many-Argument Workhorses, Impact: 45.2)

### 6. `shared/libc/string0.c` (C) -> Cumulative Risk: **675.07**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z -1.17)
...
- **Heaviest Functions:** `strncmp` (Defensive Guards, Impact: 26.6), `strcmp` (Compute Cores, Impact: 17.9), `memcpy` (Many-Argument Workhorses, Impact: 17.7)

### 7. `py/makeqstrdefs.py` (PYTHON) -> Cumulative Risk: **674.44**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.23)
- **Magnitude:** 240.38 | **LOC:** 238 | **CtrlFlow:** 28.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9244%)
- **Heaviest Functions:** `process_file` (Compute Cores, Impact: 28.8), `preprocess` (Defensive Guards, Impact: 14.1), `cat_together` (I/O & Config Routines, Impact: 9.8)
...
- **Heaviest Functions:** `ra_rtc_set_adjustment` (Compute Cores, Impact: 30.4), `ra_rtc_init` (Compute Cores, Impact: 16.3), `ra_rtc_set_subclock` (Compute Cores, Impact: 13.1)
...
- **Heaviest Functions:** `mp_obj_int_binary_op` (Many-Argument Workhorses, Impact: 129.1), `mp_obj_int_to_bytes_impl` (Many-Argument Workhorses, Impact: 24.1), `mp_obj_int_unary_op` (Compute Cores, Impact: 20.7)
...
- **Heaviest Functions:** `add_af` (Many-Argument Workhorses, Impact: 23.6), `validate_cpu_pin_name` (Compute Cores, Impact: 13.4), `load_inputs` (Compute Cores, Impact: 9.7)
...
**Top Internal Functions/Classes:**
  ...
  * `create_name` **(Compute Cores)** (Impact: 182.7)
...
**Top Internal Functions/Classes:**
  ...
  * `USBH_AUDIO_InterfaceInit` **(Compute Cores)** (Impact: 29.6)
...
**Top Internal Functions/Classes:**
  ...
  * `detect_test_platform` **(Compute Cores)** (Impact: 28.3)
...
**Top Internal Functions/Classes:**
  ...
  * `lwip_getaddrinfo` **(Compute Cores)** (Impact: 49.1)
...
**Top Internal Functions/Classes:**
  * `adc_wait_for_eoc_or_timeout` **(Many-Argument Workhorses)** (Impact: 406.0)
  * `adc_config_channel` **(Compute Cores)** (Impact: 58.0)
  ...
  * `adcx_init_periph` **(Compute Cores)** (Impact: 49.6)
...
**Top Internal Functions/Classes:**
  ...
  * `central_gap_event_cb` **(Compute Cores)** (Impact: 33.3)
...
**Top Internal Functions/Classes:**
  ...
  * `mp_init` **(Compute Cores)** (Impact: 36.5)
...
**Top Internal Functions/Classes:**
  ...
  * `is_string_or_bytes` **(Compute Cores)** (Impact: 29.1)
  * `next_char` **(Compute Cores)** (Impact: 27.2)
  * `skip_whitespace` **(Compute Cores)** (Impact: 25.5)
...
**Top Internal Functions/Classes:**
  * `eth_mac_init` **(Compute Cores)** (Impact: 185.2)
  ...
  * `eth_netif_output` **(Compute Cores)** (Impact: 25.8)
**Contextual Mitigations & Amplifications:**
...
- `tools/ci.sh` -> Churn: **94.14%** | Cog Load: 29.9748% | Debt: 100.0%
...
- `py/mpprint.h` -> **Severity: 3.584** (Embedded: 0.0636 * Error Risk: 56.3934%)
- `py/runtime.h` -> **Severity: 3.455** (Embedded: 0.0851 * Error Risk: 40.6188%)
- `py/obj.h` -> **Severity: 3.439** (Embedded: 0.0867 * Error Risk: 39.6814%)
- `py/cstack.h` -> **Severity: 2.803** (Embedded: 0.0537 * Error Risk: 52.1415%)
