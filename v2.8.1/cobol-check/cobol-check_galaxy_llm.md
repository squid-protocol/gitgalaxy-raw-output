...
> **Composition Archetype:** `Small Flat Repo` (z -0.23; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 35%, Declarative / Non-Code 16%, Interface Declarations Files 13%, Tests & Verification Files 9%, Large Core Modules 8%
...
- `parseText` **(Compute Cores)** (@ `src/main/java/org/openmainframeproject/cobolcheck/features/launcher/Formatter/Formats/Formatter.java`) -> Impact: **41.6** | LOC: 69
...
- `it_parses_file_section_variables_from_split_lines` **(Tests & Verification)** (@ `src/test/java/org/openmainframeproject/cobolcheck/InterpreterControllerTest.java`) -> Impact: **32.2** | LOC: 84
...
- **Heaviest Functions:** `run` (Compute Cores, Impact: 17.2), `updateFromDisk` (Defensive Guards, Impact: 9.7), `updateFromContents` (Many-Argument Workhorses, Impact: 6.3)
...
- **Heaviest Functions:** `activate` (Compute Cores, Impact: 75.6), `discoverTests` (Defensive Guards, Impact: 30.4), `createDirectoryItems` (Compute Cores, Impact: 29.5)
...
- **Heaviest Functions:** `findFile` (Many-Argument Workhorses, Impact: 21.6), `runCobolCheck` (Defensive Guards, Impact: 17.5), `getCobolProgramPathForGivenContext` (Compute Cores, Impact: 16.8)
...
- **Heaviest Functions:** `getCommentText` (Compute Cores, Impact: 8.8), `getMockDescription` (Interface Declarations, Impact: 5.2), `getArgumentText` (Interface Declarations, Impact: 3.4)
...
- **Heaviest Functions:** `setCounts` (I/O & Config Routines, Impact: 10.1), `setCurrentTestCaseFailure` (Parameter Forwarders, Impact: 1.9), `setCurrentTestCaseErrorMessage` (Parameter Forwarders, Impact: 1.9)
...
- **Heaviest Functions:** `setTestCounts` (I/O & Config Routines, Impact: 7.0), `setCurrentTestCaseFailure` (Annotated Framework Methods, Impact: 2.1), `setCurrentTestCaseErrorMessage` (Annotated Framework Methods, Impact: 2.1)

### 7. `src/main/java/org/openmainframeproject/cobolcheck/features/interpreter/LineRepository.java` (JAVA) -> Cumulative Risk: **604.61**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.24)
...
- **Heaviest Functions:** `addAccumulatedTokensFromCopyStatementToCopyTokens` (Compute Cores, Impact: 9.1), `addExpandedCopyStatementsToFileSectionStatements` (I/O & Config Routines, Impact: 8.8), `addFileControlStatement` (Interface Declarations, Impact: 3.1)
...
- **Heaviest Functions:** `handleCobolCheckOut` (Many-Argument Workhorses, Impact: 25.2), `handleCobolCheckOutput` (Compute Cores, Impact: 15.6), `showWebWiev` (Callbacks & Closures, Impact: 6.1)
...
- **Heaviest Functions:** `setConfiguration` (Many-Argument Workhorses, Impact: 26.1), `getConfigurationValueFor` (Compute Cores, Impact: 15.4), `getConfigurationMap` (Compute Cores, Impact: 13.5)
...
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.80)
...
- **Heaviest Functions:** `getAllBranchingParagraphs` (Generic / Templated Code, Impact: 13.7), `getBeforeEachParagraphLines` (Generic / Templated Code, Impact: 3.6), `getAfterEachParagraphLines` (Generic / Templated Code, Impact: 3.6)
...
**Top Internal Functions/Classes:**
  ...
  * `getNextTokenFromTestSuite` **(Compute Cores)** (Impact: 14.3)
...
**Top Internal Functions/Classes:**
  * `multiple_mock_perform_evaluates_are_generated_correctly` **(Tests & Verification)** (Impact: 14.4)
  * `single_mock_call_with_args_gets_generated_correctly_with_comment` **(Tests & Verification)** (Impact: 13.8)
  * `multiple_mock_sections_gets_generated_correctly` **(I/O & Config Routines)** (Impact: 13.2)
  * `single_mock_section_gets_generated_correctly_with_comment` **(Tests & Verification)** (Impact: 12.7)
  * `single_mock_paragraph_gets_generated_correctly_with_comment` **(Tests & Verification)** (Impact: 12.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `it_catches_unexpected_keyword_inside_before_each_block` **(Tests & Verification)** (Impact: 18.4)
  * `it_catches_unexpected_keyword_in_a_verify_context` **(Tests & Verification)** (Impact: 16.2)
  * `it_catches_unexpected_keyword_in_a_mock_context` **(Tests & Verification)** (Impact: 15.3)
  * `it_catches_unexpected_keyword_at_the_end_of_mock_context_with_arguments_and_commas_1` **(Tests & Verification)** (Impact: 15.3)
  * `it_catches_unexpected_keyword_at_the_end_of_mock_context_with_arguments_and_commas_2` **(Tests & Verification)** (Impact: 15.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `it_parses_file_section_variables_from_split_lines` **(Tests & Verification)** (Impact: 32.2)
    * *Intent:* // verify the variables in FILE-SECTION may be split on multiple lines
  * `it_parses_WS_section_variables_from_split_lines` **(Tests & Verification)** (Impact: 10.4)
  * `it_stores_file_section_lines_in_lineRepository` **(Tests & Verification)** (Impact: 9.3)
    * *Intent:* // Test the lines in the DATA DIVISION FILE SECTION are stored in the lineRepository
  * `it_updates_numeric_fields` **(Tests & Verification)** (Impact: 7.8)
  * `it_sets_CBL_rules_on_first_line_when_empty` **(Tests & Verification)** (Impact: 6.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `it_throws_if_the_mock_that_verify_references_does_not_exist` **(Tests & Verification)** (Impact: 8.9)
  * `multiple_testcases_with_same_name_exists_for_a_testsuite` **(Tests & Verification)** (Impact: 7.5)
  * `it_parses_testsuite_with_sequnece_area` **(Tests & Verification)** (Impact: 6.5)
  * `verify_can_attach_to_call_mock_with_arguments` **(Tests & Verification)** (Impact: 6.4)
  * `it_generates_after_each_branching_paragraphs_correctly_with_comments` **(Tests & Verification)** (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `setCurrentTestCaseFailure` **(Annotated Framework Methods)** (Impact: 2.1)
  * `setCurrentTestCaseErrorMessage` **(Annotated Framework Methods)** (Impact: 2.1)
  * `setNumberOfAllTests` **(Annotated Framework Methods)** (Impact: 1.6)
  * `setNumberOffAllFailures` **(Annotated Framework Methods)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getCorrectRunContext` **(Compute Cores)** (Impact: 9.1)
  * `resolveFilenameSuffixes` **(Generic / Templated Code)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `FILE-CONTROL` **(I/O & Config Routines)** (Impact: 11.0)
...
**Top Internal Functions/Classes:**
  ...
  * `getStringContinuationSign` **(Compute Cores)** (Impact: 25.8)
...
**Top Internal Functions/Classes:**
  ...
  * `ExcludeBetweenTags` **(Many-Argument Workhorses)** (Impact: 15.0)
...
**Top Internal Functions/Classes:**
  ...
  * `readLine` **(Interface Declarations)** (Impact: 4.0)
    * *Intent:* /** * Reads the next line of the cobol file. * * @return (CobolLine) The line that was read * * @thr...
  * `readTillEndOfStatement` **(Interface Declarations)** (Impact: 3.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getContentFromFilesystem` **(Defensive Guards)** (Impact: 6.2)
...
**Top Internal Functions/Classes:**
  ...
  * `handleEndOfWord` **(Many-Argument Workhorses)** (Impact: 21.9)
  * `isDecimalPoint` **(Many-Argument Workhorses)** (Impact: 7.2)
  ...
  * `addTokenAndClearBuffer` **(Generic / Templated Code)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `Anonymous_Block` **(I/O & Config Routines)** (Impact: 6.7)
...
**Top Internal Functions/Classes:**
  * `parseText` **(Compute Cores)** (Impact: 41.6)
    * *Intent:* /** * Parses text from text results to a data transfer object style. The style is given * in the con...
  * `instantiateBasedOnStyle` **(Compute Cores)** (Impact: 14.7)
  * `setTestCaseValues` **(Compute Cores)** (Impact: 12.5)
  * `getFailureType` **(Compute Cores)** (Impact: 9.2)
...
**Top Internal Functions/Classes:**
  * `swapStringsOutWithMappedTokens` **(Compute Cores)** (Impact: 32.1)
  ...
  * `swapMappedTokensOutWithSavedStrings` **(Generic / Templated Code)** (Impact: 5.5)
...
**Top Internal Functions/Classes:**
  * `processCommandLineArgumentArray` **(Compute Cores)** (Impact: 22.1)
  * `storeOptionSettings` **(Compute Cores)** (Impact: 15.7)
  ...
  * `lookupOption` **(Compute Cores)** (Impact: 6.1)
...
**Top Internal Functions/Classes:**
  * `getBoilerplateCodeFromCopybooks` **(Compute Cores)** (Impact: 9.6)
  ...
  * `concatenateTestSuites` **(Interface Declarations)** (Impact: 3.2)
...
**Top Internal Functions/Classes:**
  ...
  * `setCurrentTestSuiteName` **(Interface Declarations)** (Impact: 1.8)
  * `setNumberOfAllTests` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
...
**Top Internal Functions/Classes:**
  ...
  * `extractCopybookNameFrom` **(Compute Cores)** (Impact: 6.6)
