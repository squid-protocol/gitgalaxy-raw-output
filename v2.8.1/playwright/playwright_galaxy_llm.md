...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.73; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 33%, Declarative / Non-Code 26%, Large Core Modules 14%, Callbacks & Closures Files 8%, Interface Declarations Files 5%
...
| threat | 1509 | 195 | 0 | `tests/assets/reading-list/vue_2.6.14.js` |
...
- `ChildReconciler` **(Defensive Guards)** (@ `tests/assets/reading-list/react-dom_16.14.0.js`) -> Impact: **314.9** | LOC: 924
  * *Intent:* // to be able to optimize each path individually by branching early. This needs // a compiler or we can do it manually. Helpers that don't need this b...
- `frameSnapshotStreamer` **(Many-Argument Workhorses)** (@ `packages/playwright-core/src/server/trace/recorder/snapshotterInjected.ts`) -> Impact: **297.9** | LOC: 624
...
- **Heaviest Functions:** `constructor` (Defensive Guards, Impact: 33.5), `_withElement` (Defensive Guards, Impact: 12.9), `_expect` (Compute Cores, Impact: 9.0)
...
- **Heaviest Functions:** `screencastStart` (Defensive Guards, Impact: 13.3), `constructor` (Callbacks & Closures, Impact: 12.9), `setNetworkInterceptionPatterns` (Compute Cores, Impact: 9.4)
...
- **Heaviest Functions:** `keydown` (Many-Argument Workhorses, Impact: 11.0), `wheel` (Defensive Guards, Impact: 9.2), `toModifiersMask` (Compute Cores, Impact: 7.7)
...
- **Heaviest Functions:** `_describe` (Many-Argument Workhorses, Impact: 63.8), `_modifier` (Compute Cores, Impact: 39.6), `_createTest` (Many-Argument Workhorses, Impact: 33.3)
...
- **Heaviest Functions:** `toModifiersMask` (Compute Cores, Impact: 7.7), `toButtonsMask` (Compute Cores, Impact: 6.2), `toButtonNumber` (Interface Declarations, Impact: 6.1)
...
- **Heaviest Functions:** `addInitAgentsCommand` (Compute Cores, Impact: 14.0), `addTestMCPServerCommand` (Callbacks & Closures, Impact: 8.2), `addMergeReportsCommand` (Defensive Guards, Impact: 6.7)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `_onAttachedToTarget` (Compute Cores, Impact: 25.0), `startTracing` (Compute Cores, Impact: 23.7), `connect` (Many-Argument Workhorses, Impact: 13.2)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `_onPageProxyCreated` (Compute Cores, Impact: 10.9), `doCreateNewContext` (Defensive Guards, Impact: 10.6), `_onDownloadCreated` (Compute Cores, Impact: 10.1)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `_initialize` (Compute Cores, Impact: 50.4), `calculateUserAgentMetadata` (Compute Cores, Impact: 34.7), `_updateViewport` (Compute Cores, Impact: 26.7)
...
- **Heaviest Functions:** `constructor` (Compute Cores, Impact: 34.2), `setCookies` (Defensive Guards, Impact: 23.3), `onTabOpenListener` (Many-Argument Workhorses, Impact: 17.2)
...
**Top Internal Functions/Classes:**
  ...
  * `ghaPullRequestEnv` **(Interface Declarations)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
- `tests/config/utils.ts` -> **Severity: 8.285** (Embedded: 0.0938 * Error Risk: 88.288%)
- `packages/playwright-core/src/utils/isomorphic/locatorGenerators.ts` -> **Severity: 6.631** (Embedded: 0.0778 * Error Risk: 85.2055%)
- `packages/playwright-core/src/utils/isomorphic/trace/traceModel.ts` -> **Severity: 6.57** (Embedded: 0.0677 * Error Risk: 96.982%)
- `packages/playwright-core/src/utils/isomorphic/selectorParser.ts` -> **Severity: 6.347** (Embedded: 0.0668 * Error Risk: 95.0818%)
- `packages/playwright-core/src/utils/isomorphic/stringUtils.ts` -> **Severity: 6.046** (Embedded: 0.0707 * Error Risk: 85.5519%)
