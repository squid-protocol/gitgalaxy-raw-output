...
> **Composition Archetype:** `Small Flat Repo` (z +0.99; from the repo's file-archetype mix)
> **File Composition:** Parameter Forwarders Files 35%, Data / Markup / Trivial 21%, I/O & Config Routines Files 17%, Declarative / Non-Code 11%, Interface Declarations Files 5%
...
- `loop` **(Many-Argument Workhorses)** (@ `Chapter 14/upper2.s`) -> Impact: **10.5** | LOC: 15
  * *Intent:* // The loop is until byte pointed to by R1 is non-zero
...
- `loop` **(Parameter Forwarders)** (@ `Chapter 14/upper3.s`) -> Impact: **9.3** | LOC: 7
  * *Intent:* // The loop is until byte pointed to by R1 is non-zero
...
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.07)
...
- **Heaviest Functions:** `main` (I/O & Config Routines, Impact: 6.4)
...
- **Heaviest Functions:** `mytoupper` (Compute Cores, Impact: 9.6), `main` (I/O & Config Routines, Impact: 1.5), `dummy` (Interface Declarations, Impact: 1.2)

### 3. `Chapter 15/upper.c` (C) -> Cumulative Risk: **481.83**
- **Archetype:** `file_cluster_3` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +3.29)
- **Magnitude:** 25.42 | **LOC:** 33 | **CtrlFlow:** 15.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9254%), Safety Score (79.7611%)
- **Heaviest Functions:** `mytoupper` (Compute Cores, Impact: 9.5), `main` (I/O & Config Routines, Impact: 1.4)

### 4. `Chapter 14/upper4.s` (ASSEMBLY) -> Cumulative Risk: **409.93**
- **Archetype:** `file_cluster_12` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +2.69)
...
- **Heaviest Functions:** `loop` (Many-Argument Workhorses, Impact: 11.9), `toupper` (Parameter Forwarders, Impact: 4.6)

### 5. `Chapter 02/movexamps.s` (ASSEMBLY) -> Cumulative Risk: **387.6**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z -0.14)
...
- **Heaviest Functions:** `_start` (Parameter Forwarders, Impact: 4.0)
...
- **Heaviest Functions:** `_start` (Parameter Forwarders, Impact: 3.9)
...
- **Heaviest Functions:** `DownloadCreditCardNumbers` (Parameter Forwarders, Impact: 2.5), `_start` (Parameter Forwarders, Impact: 2.5), `calltoupper` (Parameter Forwarders, Impact: 2.4)

### 8. `Chapter 12/maincomp.s` (ASSEMBLY) -> Cumulative Risk: **361.2**
- **Archetype:** `file_cluster_1` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.74)
...
- **Heaviest Functions:** `loop` (Compute Cores, Impact: 7.6), `next` (Parameter Forwarders, Impact: 2.5), `main` (Interface Declarations, Impact: 2.1)
...
- **Archetype:** `file_cluster_1` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.25)
...
- **Heaviest Functions:** `cont` (Many-Argument Workhorses, Impact: 8.5), `_start` (Parameter Forwarders, Impact: 4.2), `loop` (Parameter Forwarders, Impact: 3.9)
...
**Top Internal Functions/Classes:**
  ...
  * `loop2` **(Interface Declarations)** (Impact: 3.1)
  * `loop3` **(Interface Declarations)** (Impact: 3.0)
...
**Top Internal Functions/Classes:**
  ...
  * `printloop` **(Many-Argument Workhorses)** (Impact: 5.8)
...
**Top Internal Functions/Classes:**
  ...
  * `l9` **(Interface Declarations)** (Impact: 2.1)
...
**Top Internal Functions/Classes:**
  * `mytoupper` **(Compute Cores)** (Impact: 9.5)
    * *Intent:* #include <stdio.h>
  * `main` **(I/O & Config Routines)** (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
* *State Mutation (weighted view):* 12
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `letter` **(Interface Declarations)** (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `main` **(Interface Declarations)** (Impact: 2.1)
...
**Top Internal Functions/Classes:**
  ...
  * `exit` **(I/O & Config Routines)** (Impact: 1.1)
    * *Intent:* // Setup the parameters to exit the program // and then call the kernel to do it.
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `printloop` **(Many-Argument Workhorses)** (Impact: 6.0)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `l8` **(I/O & Config Routines)** (Impact: 2.4)
  * `l3` **(State Mutators)** (Impact: 2.0)
  * `l7` **(State Mutators)** (Impact: 1.8)
  * `l2` **(Interface Declarations)** (Impact: 1.7)
