...
> **Composition Archetype:** `Hub-Coupled App` (z -1.62; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 33%, Large Core Modules 20%, Interface Declarations Files 10%, Many-Argument Workhorses Files 8%, Data / Markup / Trivial 8%
...
- **Heaviest Functions:** `upb_Arena_Fuse` (Compute Cores, Impact: 15.3), `_upb_Arena_WasLastAlloc` (Defensive Guards, Impact: 14.8), `upb_Arena_Init` (Many-Argument Workhorses, Impact: 14.2)
...
- **Heaviest Functions:** `assign_msg_wellknowntype` (Compute Cores, Impact: 50.2), `_upb_MessageDef_InsertField` (Many-Argument Workhorses, Impact: 34.7), `_upb_MessageDef_LinkMiniTable` (Compute Cores, Impact: 26.8)
...
- **Heaviest Functions:** `_upb_FileDef_Create` (Many-Argument Workhorses, Impact: 69.0), `_upb_FileDef_FindEdition` (Compute Cores, Impact: 28.9), `upb_FileDef_EditionName` (Compute Cores, Impact: 9.1)
...
- **Heaviest Functions:** `parse_default` (Many-Argument Workhorses, Impact: 113.0), `_upb_FieldDef_Create` (Many-Argument Workhorses, Impact: 110.0), `resolve_subdef` (Many-Argument Workhorses, Impact: 34.6)
...
- **Heaviest Functions:** `create_enumdef` (Many-Argument Workhorses, Impact: 33.3), `upb_EnumDef_MiniDescriptorEncode` (Many-Argument Workhorses, Impact: 21.9), `_upb_EnumDefs_New` (Many-Argument Workhorses, Impact: 8.2)
...
- **Heaviest Functions:** `_upb_OneofDefs_Finalize` (Many-Argument Workhorses, Impact: 17.3), `_upb_OneofDef_Insert` (Many-Argument Workhorses, Impact: 16.2), `create_oneofdef` (Many-Argument Workhorses, Impact: 13.8)
...
- **Heaviest Functions:** `_upb_MtDataEncoder_MaybePutModifiers` (Many-Argument Workhorses, Impact: 19.8), `upb_MtDataEncoder_PutEnumValue` (Many-Argument Workhorses, Impact: 11.2), `upb_MtDataEncoder_PutBase92Varint` (Many-Argument Workhorses, Impact: 10.4)
...
- **Heaviest Functions:** `upb_Message_Freeze` (Many-Argument Workhorses, Impact: 39.9), `upb_Message_DeleteUnknown` (Many-Argument Workhorses, Impact: 36.5), `_upb_Message_DiscardUnknown_shallow` (Compute Cores, Impact: 6.3)
...
- **Heaviest Functions:** `upb_MiniTable_Link` (Many-Argument Workhorses, Impact: 30.8), `upb_MiniTable_SetSubMessage` (Many-Argument Workhorses, Impact: 20.0), `upb_MiniTable_SetSubEnum` (Many-Argument Workhorses, Impact: 11.4)
...
- **Heaviest Functions:** `rm` (Many-Argument Workhorses, Impact: 31.2), `Wyhash` (Many-Argument Workhorses, Impact: 28.5), `insert` (Many-Argument Workhorses, Impact: 19.2)
...
**Top Internal Functions/Classes:**
  * `_AddInitMethod` **(Defensive Guards)** (Impact: 68.4)
    * *Intent:* """Adds an __init__ method to cls."""
  * `init` **(Defensive Guards)** (Impact: 65.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_PrintUnknownFields` **(Compute Cores)** (Impact: 35.6)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 34.6)
  * `_LazyLoadOptions` **(Compute Cores)** (Impact: 22.0)
...
**Top Internal Functions/Classes:**
  ...
  * `PyUpb_Message_InitRepeatedMessageAttribute` **(Many-Argument Workhorses)** (Impact: 24.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_CheckConflictRegister` **(Many-Argument Workhorses)** (Impact: 26.4)
...
**Top Internal Functions/Classes:**
  ...
  * `set_default_default` **(Compute Cores)** (Impact: 33.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_FieldToJsonObject` **(Compute Cores)** (Impact: 42.1)
  ...
  * `_ConvertFloat` **(Defensive Guards)** (Impact: 29.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_upb_MessageDef_LinkMiniTable` **(Compute Cores)** (Impact: 26.8)
  * `create_msgdef` **(Many-Argument Workhorses)** (Impact: 23.3)
  * `_upb_MessageDef_EncodeMessage` **(Many-Argument Workhorses)** (Impact: 21.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `PyUpb_RepeatedContainer_Sort` **(Many-Argument Workhorses)** (Impact: 23.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_CheckDurationValid` **(Compute Cores)** (Impact: 16.2)
  * `FromJsonString` **(Defensive Guards)** (Impact: 15.4)
    * *Intent:* """Converts a string to Duration. Args: value: A string to be converted. The string must end with 's...
  * `_SetStructValue` **(Defensive Guards)** (Impact: 14.8)
  * `_internal_compare` **(Defensive Guards)** (Impact: 12.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `PyUpb_DescriptorBase_CopyToProto` **(Many-Argument Workhorses)** (Impact: 10.9)
  * `PyUpb_DescriptorBase_GetSerializedProto` **(Many-Argument Workhorses)** (Impact: 9.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_upb_Message_GetNonExtensionField` **(Many-Argument Workhorses)** (Impact: 9.6)
  * `upb_Message_ClearExtension` **(C Struct Operations)** (Impact: 9.5)
  * `upb_Message_ClearBaseField` **(Compute Cores)** (Impact: 9.4)
  * `_upb_Message_GetExtensionField` **(Many-Argument Workhorses)** (Impact: 7.4)
  * `upb_Message_ResizeArrayUninitialized` **(C Struct Operations)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `PyUpb_GenericSequence_IsEqual` **(Compute Cores)** (Impact: 17.3)
    * *Intent:* // A sequence container can only be equal to another sequence container, or (for // backward compati...
  * `PyUpb_ByNameMap_Get` **(Compute Cores)** (Impact: 13.1)
  * `PyUpb_ByNumberMap_Get` **(Compute Cores)** (Impact: 11.2)
  * `PyUpb_ByNameMap_Subscript` **(Compute Cores)** (Impact: 11.1)
  * `PyUpb_ByNameMap_Items` **(Compute Cores)** (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `TryGetHexDigit` **(Compute Cores)** (Impact: 11.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `remove_filedef` **(Compute Cores)** (Impact: 18.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `PyUpb_DescriptorPool_FindFieldByName` **(Compute Cores)** (Impact: 15.3)
    * *Intent:* /* * PyUpb_DescriptorPool_FindFieldByName() * * Implements: * DescriptorPool.FindFieldByName(self, n...
  * `PyUpb_DescriptorPool_FindMethodByName` **(Compute Cores)** (Impact: 15.1)
  * `PyUpb_DescriptorPool_SetFeatureSetDefaults` **(Compute Cores)** (Impact: 14.2)
  * `PyUpb_DescriptorPool_FindOneofByName` **(Compute Cores)** (Impact: 13.4)
