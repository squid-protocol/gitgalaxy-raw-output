...
> **File Composition:** Data / Markup / Trivial 43%, Declarative / Non-Code 20%, Large Core Modules 8%, Interface Declarations Files 5%, Defensive Guards Files 5%
...
| threat | 460 | 154 | 0 | `packages/wrangler/src/__tests__/core/handle-errors.test.ts` |
...
- `deploy` **(Compute Cores)** (@ `packages/wrangler/src/deploy/deploy.ts`) -> Impact: **297.8** | LOC: 894
- `unescapeIdentity` **(Compute Cores)** (@ `packages/local-explorer-ui/src/drivers/sqlite/parsers.ts`) -> Impact: **262.8** | LOC: 900
...
- **Known Shadow Routes:** `GET /api/r2/buckets/{var}/objects`, `GET /`, `GET /api/d1/database`, `GET /*`, `POST /api/todos`
...
- **Composition Archetype:** `State Mutators Files` (z +0.29)
...
- **Heaviest Functions:** `cloudflareTest` (Defensive Guards, Impact: 40.7), `config` (Defensive Guards, Impact: 32.3), `load` (Compute Cores, Impact: 6.8)

### 2. `packages/vite-plugin-cloudflare/playground/durable-objects/src/index.ts` (TYPESCRIPT) -> Cumulative Risk: **744.33**
...
- **Heaviest Functions:** `fetch` (Compute Cores, Impact: 19.2), `increment` (Interface Declarations, Impact: 3.2), `decrement` (Interface Declarations, Impact: 3.2)

### 3. `fixtures/unbound-durable-object/src/index.ts` (TYPESCRIPT) -> Cumulative Risk: **740.58**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `fetch` (Many-Argument Workhorses, Impact: 19.8), `increment` (Interface Declarations, Impact: 3.1), `decrement` (Interface Declarations, Impact: 3.1)
...
- **Heaviest Functions:** `warn` (Defensive Guards, Impact: 65.4), `createInMemoryLogger` (Many-Argument Workhorses, Impact: 20.3), `loadConfig` (Compute Cores, Impact: 15.8)

### 5. `packages/wrangler/src/workflows/commands/instances/describe.ts` (TYPESCRIPT) -> Cumulative Risk: **714.71**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.60)
...
- **Heaviest Functions:** `logStep` (Many-Argument Workhorses, Impact: 65.1), `renderInstanceDetails` (Many-Argument Workhorses, Impact: 31.3), `getLastSuccessfulStep` (Compute Cores, Impact: 12.4)

### 6. `packages/miniflare/src/workers/images/images.worker.ts` (TYPESCRIPT) -> Cumulative Risk: **713.04**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.89)
...
- **Heaviest Functions:** `list` (Defensive Guards, Impact: 36.5), `upload` (Defensive Guards, Impact: 34.6), `update` (Defensive Guards, Impact: 16.8)
...
- **Heaviest Functions:** `fetch` (Compute Cores, Impact: 75.6), `isPlainObject` (Defensive Guards, Impact: 12.2), `Native` (Defensive Guards, Impact: 9.5)

### 8. `packages/wrangler/src/r2/helpers/domain.ts` (TYPESCRIPT) -> Cumulative Risk: **702.43**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -1.10)
- **Magnitude:** 201.82 | **LOC:** 227 | **CtrlFlow:** 11.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `tableFromCustomDomainListResponse` (Compute Cores, Impact: 12.6), `configureCustomDomainSettings` (Many-Argument Workhorses, Impact: 9.2), `attachCustomDomainToBucket` (Many-Argument Workhorses, Impact: 8.7)

### 9. `packages/vitest-pool-workers/src/worker/index.ts` (TYPESCRIPT) -> Cumulative Risk: **701.63**
...
- **Heaviest Functions:** `monkeypatchedSetTimeout` (Defensive Guards, Impact: 29.2), `handleVitestRunRequest` (Callbacks & Closures, Impact: 19.6), `onModuleRunner` (Callbacks & Closures, Impact: 9.3)
...
- **Heaviest Functions:** `StudioTableHeaderResizer` (Defensive Guards, Impact: 32.9), `onMouseMove` (Compute Cores, Impact: 18.2), `onMouseUp` (Callbacks & Closures, Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `collectCoreBindingsPerEnvironment` **(Compute Cores)** (Impact: 180.1)
...
**Top Internal Functions/Classes:**
  ...
  * `handleRuntimeStdio` **(Interface Declarations)** (Impact: 6.6)
  * `unsafeModuleFallbackService` **(Tests & Verification)** (Impact: 4.8)
  * `CUSTOM` **(Defensive Guards)** (Impact: 4.7)
  * `testEncoding` **(Tests & Verification)** (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
...
- `packages/wrangler/src/logger.ts` -> **Severity: 3.976** (Embedded: 0.0777 * Error Risk: 51.1375%)
- `packages/wrangler/src/utils/log-file.ts` -> **Severity: 3.405** (Embedded: 0.0508 * Error Risk: 67.054%)
- `packages/wrangler/src/utils/format-message.ts` -> **Severity: 2.627** (Embedded: 0.0508 * Error Risk: 51.7132%)
- `packages/cli/interactive.ts` -> **Severity: 2.187** (Embedded: 0.0329 * Error Risk: 66.4866%)
- `packages/cli/select-list.ts` -> **Severity: 2.081** (Embedded: 0.0245 * Error Risk: 84.9455%)
