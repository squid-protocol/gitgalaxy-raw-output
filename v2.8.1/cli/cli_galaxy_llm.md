...
> **Composition Archetype:** `Flat Modular Platform` (z -0.33; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 81%, Callbacks & Closures Files 4%, Declarative / Non-Code 3%, Large Core Modules 3%, Defensive Guards Files 2%
...
- `logHandler` **(Compute Cores)** (@ `workspaces/config/test/index.js`) -> Impact: **153.4** | LOC: 947
...
- **Heaviest Functions:** `tnock` (Defensive Guards, Impact: 35.6), `noMatch` (Defensive Guards, Impact: 23.3), `putPackagePayload` (Defensive Guards, Impact: 15.9)
...
- **Heaviest Functions:** `assignCommonProperties` (Defensive Guards, Impact: 63.5), `processDeps` (Many-Argument Workhorses, Impact: 21.2), `createBundledTree` (Compute Cores, Impact: 20.4)
...
- **Heaviest Functions:** `constructor` (Compute Cores, Impact: 33.2), `workspaceDependencySet` (Many-Argument Workhorses, Impact: 25.6), `audit` (Compute Cores, Impact: 11.9)
...
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.33)
...
- **Heaviest Functions:** `filter` (Many-Argument Workhorses, Impact: 114.6), `reify` (Compute Cores, Impact: 67.0), `updateNodes` (Compute Cores, Impact: 61.1)
...
- **Heaviest Functions:** `init` (I/O & Config Routines, Impact: 26.2), `shouldAudit` (Defensive Guards, Impact: 17.7), `toJSON` (I/O & Config Routines, Impact: 9.1)
...
- **Heaviest Functions:** `nodeMatches` (Compute Cores, Impact: 54.6), `hasParent` (Compute Cores, Impact: 45.9), `semverPseudo` (Compute Cores, Impact: 37.5)
...
- **Heaviest Functions:** `buildDepStep` (Compute Cores, Impact: 70.8), `nodeFromSpec` (Many-Argument Workhorses, Impact: 57.4), `applyUserRequestsToNode` (Compute Cores, Impact: 52.0)
...
- **Heaviest Functions:** `openFundingUrl` (Defensive Guards, Impact: 25.5), `retrievePackageMetadata` (Defensive Guards, Impact: 18.7), `exec` (Compute Cores, Impact: 13.8)
...
**Top Internal Functions/Classes:**
  ...
  * `extract` **(Compute Cores)** (Impact: 23.7)
...
**Top Internal Functions/Classes:**
  ...
  * `buildLinkedActualForDiff` **(Compute Cores)** (Impact: 28.6)
...
**Top Internal Functions/Classes:**
  ...
  * `warn` **(Compute Cores)** (Impact: 85.5)
  * `rootAndWs` **(Compute Cores)** (Impact: 78.6)
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Compute Cores)** (Impact: 69.3)
...
**Top Internal Functions/Classes:**
  ...
  * `buildOmitList` **(Compute Cores)** (Impact: 30.5)
...
**Top Internal Functions/Classes:**
  * `logHandler` **(Compute Cores)** (Impact: 153.4)
  * `logHandler` **(Compute Cores)** (Impact: 33.3)
  * `readFile` **(Callbacks & Closures)** (Impact: 21.9)
...
**Top Internal Functions/Classes:**
  * `nodeMatches` **(Compute Cores)** (Impact: 54.6)
  * `hasParent` **(Compute Cores)** (Impact: 45.9)
    * *Intent:* // checks if a given node has a direct parent in any of the nodes provided in // the compare nodes a...
  * `semverPseudo` **(Compute Cores)** (Impact: 37.5)
  * `vulnPseudo` **(Compute Cores)** (Impact: 26.4)
...
**Top Internal Functions/Classes:**
  ...
  * `publish` **(State Mutators)** (Impact: 13.0)
...
**Top Internal Functions/Classes:**
  ...
  * `cleanSnapshot` **(Interface Declarations)** (Impact: 1.6)
...
**Top Internal Functions/Classes:**
  ...
  * `getJsonOutputItem` **(Compute Cores)** (Impact: 37.8)
...
**Top Internal Functions/Classes:**
  ...
  * `createBundledTree` **(Compute Cores)** (Impact: 20.4)
  * `processEdges` **(Many-Argument Workhorses)** (Impact: 19.9)
  * `makeIdealGraph` **(Compute Cores)** (Impact: 19.4)
...
**Top Internal Functions/Classes:**
  ...
  * `apply` **(Callbacks & Closures)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `cleanSnapshot` **(Compute Cores)** (Impact: 20.7)
...
**Top Internal Functions/Classes:**
  ...
  * `read` **(Callbacks & Closures)** (Impact: 5.5)
