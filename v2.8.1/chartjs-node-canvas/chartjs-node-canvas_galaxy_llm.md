...
> **File Composition:** Data / Markup / Trivial 54%, Callbacks & Closures Files 23%, Defensive Guards Files 8%, Large Core Modules 8%, State Mutators Files 8%
...
- `renderChart` **(Many-Argument Workhorses)** (@ `package/src/animatedChartJSNodeCanvas.ts`) -> Impact: **22.1** | LOC: 41
...
- `renderToBuffer` **(Callbacks & Closures)** (@ `package/src/chartJSNodeCanvas.ts`) -> Impact: **6.0** | LOC: 17
...
- **Heaviest Functions:** `renderChart` (Many-Argument Workhorses, Impact: 22.1), `onComplete` (Compute Cores, Impact: 8.1), `onProgress` (Callbacks & Closures, Impact: 4.8)
...
- **Heaviest Functions:** `initialize` (Defensive Guards, Impact: 29.5), `constructor` (Defensive Guards, Impact: 12.4), `toBuffer` (Compute Cores, Impact: 6.0)
...
- **Heaviest Functions:** `main` (I/O & Config Routines, Impact: 4.4), `chartCallback` (Callbacks & Closures, Impact: 2.7), `beforeDraw` (Callbacks & Closures, Impact: 1.9)
...
- **Heaviest Functions:** `renderToStream` (Defensive Guards, Impact: 13.1), `renderToDataURL` (Callbacks & Closures, Impact: 6.0), `renderToBuffer` (Callbacks & Closures, Impact: 6.0)
...
- **Heaviest Functions:** `freshRequire` (Callbacks & Closures, Impact: 1.9)

### 6. `package/src/backgroundColourPlugin.ts` (TYPESCRIPT) -> Cumulative Risk: **339.39**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.44)
...
- **Heaviest Functions:** `constructor` (State Mutators, Impact: 2.2), `beforeDraw` (Interface Declarations, Impact: 1.9)
...
**Top Internal Functions/Classes:**
  * `renderChart` **(Many-Argument Workhorses)** (Impact: 22.1)
  * `onComplete` **(Compute Cores)** (Impact: 8.1)
  * `onProgress` **(Callbacks & Closures)** (Impact: 4.8)
  * `renderToBuffer` **(Callbacks & Closures)** (Impact: 4.5)
...
**Top Internal Functions/Classes:**
  ...
  * `renderToBuffer` **(Callbacks & Closures)** (Impact: 6.0)
    * *Intent:* /** * Render to a buffer. * @see https://github.com/Automattic/node-canvas#canvastobuffer * * @param...
  * `renderChart` **(Compute Cores)** (Impact: 4.9)
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `constructor` **(State Mutators)** (Impact: 2.2)
  * `beforeDraw` **(Interface Declarations)** (Impact: 1.9)
**Structural Signatures (Net Mitigated Signals):**
