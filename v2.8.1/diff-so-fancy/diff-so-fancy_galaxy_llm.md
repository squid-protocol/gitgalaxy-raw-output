...
> **File Composition:** Data / Markup / Trivial 50%, I/O & Config Routines Files 29%, Compute Cores Files 7%, Large Core Modules 7%, State Mutators Files 7%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 54.8 | 88.2 | 0.0 |
...
- `do_dsf_stuff` **(Compute Cores)** (@ `diff-so-fancy`) -> Impact: **118.6** | LOC: 312
  * *Intent:* ################################################################################# ####################################################################...
...
- `color` **(Compute Cores)** (@ `diff-so-fancy`) -> Impact: **22.2** | LOC: 29
  * *Intent:* # Borrowed from: https://www.perturb.org/display/1167_Perl_ANSI_colors.html # String format: '115', '165_bold', '10_on_140', 'reset', 'on_173', 'red',...
...
### 1. `lib/DiffHighlight.pm` (PERL) -> Cumulative Risk: **638.53**
- **Archetype:** `file_cluster_12` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.94)
- **Magnitude:** 287.1 | **LOC:** 327 | **CtrlFlow:** 34.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (94.0476%)
- **Heaviest Functions:** `highlight_pair` (Compute Cores, Impact: 37.5), `handle_line` (Compute Cores, Impact: 31.0), `is_pair_interesting` (Many-Argument Workhorses, Impact: 17.1)

### 2. `diff-so-fancy` (PERL) -> Cumulative Risk: **630.11**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Churn (100.0%), Safety Score (98.6257%)
- **Heaviest Functions:** `do_dsf_stuff` (Compute Cores, Impact: 118.6), `color` (Compute Cores, Impact: 22.2), `show_debug_info` (Compute Cores, Impact: 19.6)

### 3. `test/diff-so-fancy.bats` (SHELL) -> Cumulative Risk: **337.34**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (93.9913%), Documentation (88.1765%), Churn (42.34%)
- **Heaviest Functions:** `Anonymous_Block` (Tests & Verification, Impact: 2.5), `__global_context__` (I/O & Config Routines, Impact: 1.5), `setup_file` (I/O & Config Routines, Impact: 1.4)

### 4. `test/git_ansi_color.pl` (PERL) -> Cumulative Risk: **335.22**
- **Archetype:** `file_cluster_1` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.90)
- **Magnitude:** 158.22 | **LOC:** 254 | **CtrlFlow:** 28.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (93.7119%), Documentation (88.1765%), Cognitive Load (38.7762%)
- **Heaviest Functions:** `git_ansi_color` (I/O & Config Routines, Impact: 25.1), `color` (I/O & Config Routines, Impact: 12.4), `git_ansi_color2` (I/O & Config Routines, Impact: 12.2)

### 5. `test/bugs.bats` (SHELL) -> Cumulative Risk: **332.74**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (91.9267%), Documentation (88.1765%), Churn (52.13%)
- **Heaviest Functions:** `Anonymous_Block` (Tests & Verification, Impact: 1.6), `__global_context__` (I/O & Config Routines, Impact: 1.5), `Anonymous_Block` (Tests & Verification, Impact: 1.4)

### 6. `test/git-config.bats` (SHELL) -> Cumulative Risk: **330.91**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.33)
- **Magnitude:** 83.28 | **LOC:** 172 | **CtrlFlow:** 2.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (98.2735%), Documentation (88.1765%), Cognitive Load (40.1927%)
- **Heaviest Functions:** `setup_dsf_git_config` (Interface Declarations, Impact: 4.7), `ansi_color` (Parameter Forwarders, Impact: 4.2), `rgb_color` (Parameter Forwarders, Impact: 2.2)

### 7. `test/test_helper/util.bash` (SHELL) -> Cumulative Risk: **314.96**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +2.35)
- **Magnitude:** 27.36 | **LOC:** 48 | **CtrlFlow:** 10.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Safety Score (81.1095%), Cognitive Load (14.2479%)
- **Heaviest Functions:** `setup_default_dsf_git_config` (I/O & Config Routines, Impact: 4.0), `teardown_default_dsf_git_config` (I/O & Config Routines, Impact: 3.3), `load_fixture` (Interface Declarations, Impact: 1.6)

### 8. `diff-so-fancy.plugin.zsh` (SHELL) -> Cumulative Risk: **165.47**
...
- **Primary Risk Drivers:** Safety Score (82.388%), State Flux (50.0%), Churn (22.33%), Cognitive Load (8.0173%)
- **Heaviest Functions:** `Anonymous_Block` (Unclassified, Impact: 2.1), `__global_context__` (Unclassified, Impact: 1.4)

### 9. `update-deps.sh` (SHELL) -> Cumulative Risk: **7.45**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Data / Markup / Trivial` (z +0.00)
- **Magnitude:** 2.24 | **LOC:** 5 | **CtrlFlow:** 50.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (5.1174%), Verification (2.3301%)
- **Heaviest Functions:** `__global_context__` (Unclassified, Impact: 2.2)
...
- **Magnitude:** 1116.94 | **LOC:** 1442 | **CtrlFlow:** 39.4% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (73.1234%), Tech Debt (9.5066%)
**Top Internal Functions/Classes:**
  * `do_dsf_stuff` **(Compute Cores)** (Impact: 118.6)
    * *Intent:* ################################################################################# ##################...
  * `color` **(Compute Cores)** (Impact: 22.2)
    * *Intent:* # Borrowed from: https://www.perturb.org/display/1167_Perl_ANSI_colors.html # String format: '115', ...
...
- **Global Archetype:** `file_cluster_1` (Drift: 0.0 IQR)
- **Magnitude:** 158.22 | **LOC:** 254 | **CtrlFlow:** 28.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (38.7762%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  ...
  * `compare_color` **(Compute Cores)** (Impact: 6.5)
    * *Intent:* ############################################################################### ####################...
  * `trim` **(I/O & Config Routines)** (Impact: 5.4)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 92.72 | **LOC:** 257 | **CtrlFlow:** 0.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (9.8262%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `Anonymous_Block` **(Tests & Verification)** (Impact: 2.5)
    * *Intent:* # see https://git.io/vrOF4
  * `__global_context__` **(I/O & Config Routines)** (Impact: 1.5)
  * `setup_file` **(I/O & Config Routines)** (Impact: 1.4)
  * `Anonymous_Block` **(Tests & Verification)** (Impact: 1.4)
  * `Anonymous_Block` **(Tests & Verification)** (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_15` (Drift: 0.0 IQR)
- **Magnitude:** 83.28 | **LOC:** 172 | **CtrlFlow:** 2.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (40.1927%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `setup_dsf_git_config` **(Interface Declarations)** (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` **(Tests & Verification)** (Impact: 1.6)
  * `__global_context__` **(I/O & Config Routines)** (Impact: 1.5)
  * `Anonymous_Block` **(Tests & Verification)** (Impact: 1.4)
...
**Top Internal Functions/Classes:**
  ...
  * `teardown_default_dsf_git_config` **(I/O & Config Routines)** (Impact: 3.3)
  * `load_fixture` **(Interface Declarations)** (Impact: 1.6)
  * `__global_context__` **(I/O & Config Routines)** (Impact: 1.5)
