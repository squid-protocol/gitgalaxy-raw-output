...
> **File Composition:** Data / Markup / Trivial 30%, Defensive Guards Files 18%, I/O & Config Routines Files 16%, Large Core Modules 14%, Interface Declarations Files 8%
...
- **Heaviest Functions:** `_find_no_duplicates` (Many-Argument Workhorses, Impact: 21.5), `remove_cookie_by_name` (Many-Argument Workhorses, Impact: 18.7), `_find` (Many-Argument Workhorses, Impact: 16.6)
...
- **Heaviest Functions:** `resolve_redirects` (Many-Argument Workhorses, Impact: 59.2), `send` (Many-Argument Workhorses, Impact: 25.8), `merge_environment_settings` (Many-Argument Workhorses, Impact: 25.3)
...
- **Heaviest Functions:** `send` (Many-Argument Workhorses, Impact: 47.8), `cert_verify` (Many-Argument Workhorses, Impact: 44.4), `_urllib3_request_context` (Many-Argument Workhorses, Impact: 21.9)

### 5. `requests-2.33.1/src/requests/structures.py` (PYTHON) -> Cumulative Risk: **537.51**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Parameter Forwarders Files` (z +1.27)
...
- **Heaviest Functions:** `__eq__` (Type Conversions, Impact: 5.5), `__init__` (Encapsulated Accessors, Impact: 4.2), `lower_items` (Interface Declarations, Impact: 3.0)

### 6. `requests-2.33.1/tests/testserver/server.py` (PYTHON) -> Cumulative Risk: **534.04**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__exit__` (Many-Argument Workhorses, Impact: 9.6), `__init__` (Many-Argument Workhorses, Impact: 8.2), `consume_socket_content` (Compute Cores, Impact: 7.7)

### 7. `requests-2.33.1/src/requests/help.py` (PYTHON) -> Cumulative Risk: **499.08**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.87)
...
- **Heaviest Functions:** `info` (I/O & Config Routines, Impact: 8.8), `_implementation` (I/O & Config Routines, Impact: 8.5), `main` (Interface Declarations, Impact: 1.1)

### 8. `requests-2.33.1/src/requests/hooks.py` (PYTHON) -> Cumulative Risk: **491.07**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +1.98)
...
- **Heaviest Functions:** `dispatch_hook` (Defensive Guards, Impact: 14.0), `default_hooks` (Interface Declarations, Impact: 2.1)
...
- **Heaviest Functions:** `should_bypass_proxies` (Compute Cores, Impact: 34.1), `super_len` (Defensive Guards, Impact: 27.5), `get_netrc_auth` (Compute Cores, Impact: 22.9)
...
- **Heaviest Functions:** `_encode_files` (Defensive Guards, Impact: 48.4), `prepare_body` (Many-Argument Workhorses, Impact: 46.3), `__init__` (Many-Argument Workhorses, Impact: 43.2)
...
**Top Internal Functions/Classes:**
  * `test_https_warnings` **(Compute Cores)** (Impact: 13.3)
...
**Top Internal Functions/Classes:**
  ...
  * `get_netrc_auth` **(Compute Cores)** (Impact: 22.9)
...
**Top Internal Functions/Classes:**
  ...
  * `should_strip_auth` **(Many-Argument Workhorses)** (Impact: 21.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_urllib3_request_context` **(Many-Argument Workhorses)** (Impact: 21.9)
...
**Top Internal Functions/Classes:**
  * `build_digest_header` **(Many-Argument Workhorses)** (Impact: 57.5)
    * *Intent:* """ :rtype: str """
  * `_basic_auth_str` **(Defensive Guards)** (Impact: 14.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_digestauth_401_only_sent_once` **(Interface Declarations)** (Impact: 3.2)
    * *Intent:* """Ensure we correctly respond to a 401 challenge once, and then stop responding if challenged again...
  * `test_fragment_update_on_redirect` **(Interface Declarations)** (Impact: 3.1)
  ...
  * `test_digestauth_only_on_4xx` **(Interface Declarations)** (Impact: 2.8)
...
**Top Internal Functions/Classes:**
  * `__exit__` **(Many-Argument Workhorses)** (Impact: 9.6)
  ...
  * `_handle_requests` **(Compute Cores)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
...
**Top Internal Functions/Classes:**
  ...
  * `test_basic_waiting_server` **(Interface Declarations)** (Impact: 2.1)
...
**Top Internal Functions/Classes:**
  ...
  * `lower_items` **(Interface Declarations)** (Impact: 3.0)
    * *Intent:* """Like iteritems(), but with all lowercase keys."""
  * `__iter__` **(Interface Declarations)** (Impact: 2.9)
...
**Top Internal Functions/Classes:**
  ...
  * `__reduce__` **(Interface Declarations)** (Impact: 1.9)
...
**Top Internal Functions/Classes:**
  * `check_compatibility` **(Defensive Guards)** (Impact: 15.7)
  * `_check_cryptography` **(Defensive Guards)** (Impact: 3.4)
...
**Top Internal Functions/Classes:**
  ...
  * `prepare_url` **(Interface Declarations)** (Impact: 1.8)
    * *Intent:* # Issue #1483: Make sure the URL always has a trailing slash httpbin_url = value.url.rstrip("/") + "...
  * `inner` **(Interface Declarations)** (Impact: 1.5)
  * `httpbin` **(Interface Declarations)** (Impact: 1.5)
  * `httpbin_secure` **(Interface Declarations)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_resolve_char_detection` **(Defensive Guards)** (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  * `_init` **(Interface Declarations)** (Impact: 8.8)
  * `doc` **(Interface Declarations)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
