...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +4.01; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 64%, Interface Declarations Files 17%, Data / Markup / Trivial 8%, Large Core Modules 3%, Generic / Templated Code Files 2%
...
- `GetNameFor` **(Compute Cores)** (@ `package/include/node/cppgc/internal/name-trait.h`) -> Impact: **11.9** | LOC: 31
- `WriteBarrier` **(Many-Argument Workhorses)** (@ `package/include/node/cppgc/internal/pointer-policies.h`) -> Impact: **11.9** | LOC: 15
- `ReadExternalPointerField` **(Many-Argument Workhorses)** (@ `package/include/node/v8-internal.h`) -> Impact: **11.4** | LOC: 28
- `jlh` **(Many-Argument Workhorses)** (@ `package/share/doc/node/lldb_commands.py`) -> Impact: **11.2** | LOC: 24
...
- **Composition Archetype:** `Compute Cores Files` (z +0.61)
...
- **Heaviest Functions:** `OPENSSL_load_u64_le` (Compute Cores, Impact: 4.7), `OPENSSL_load_u64_be` (Compute Cores, Impact: 4.7), `OPENSSL_store_u64_le` (Compute Cores, Impact: 4.5)
...
- **Heaviest Functions:** `bta` (Compute Cores, Impact: 29.3), `ptr_arg_cmd` (Many-Argument Workhorses, Impact: 19.6), `jlh` (Many-Argument Workhorses, Impact: 11.2)
...
- **Heaviest Functions:** `ReadCppHeapPointerField` (Many-Argument Workhorses, Impact: 11.1), `CheckTagOf` (I/O & Config Routines, Impact: 3.6), `GetCppHeapPointerTableBase` (I/O & Config Routines, Impact: 1.7)
...
- **Heaviest Functions:** `WriteBarrier` (Many-Argument Workhorses, Impact: 11.9), `AssigningBarrier` (Defensive Guards, Impact: 5.9), `AssigningBarrier` (Defensive Guards, Impact: 5.9)
...
- **Heaviest Functions:** `Unref` (Defensive Guards, Impact: 2.4), `Unwrap` (Defensive Guards, Impact: 1.9), `Wrap` (Defensive Guards, Impact: 1.8)
...
- **Heaviest Functions:** `TypeCheckLocal` (Generic / Templated Code, Impact: 23.4), `LocalBase` (I/O & Config Routines, Impact: 2.3), `LocalVector` (I/O & Config Routines, Impact: 2.3)
...
- **Heaviest Functions:** `ReadExternalPointerField` (Many-Argument Workhorses, Impact: 11.4), `GetRoot` (Compute Cores, Impact: 9.5), `ExternalPointerCanBeEmpty` (Compute Cores, Impact: 6.0)

### 8. `package/include/node/cppgc/member.h` (OBJECTIVE-C) -> Cumulative Risk: **440.09**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +2.06)
...
- **Heaviest Functions:** `BasicMember` (I/O & Config Routines, Impact: 2.3), `BasicMember` (I/O & Config Routines, Impact: 2.3), `BasicMember` (I/O & Config Routines, Impact: 2.2)
...
- **Heaviest Functions:** `ERR_GET_LIB` (Interface Declarations, Impact: 3.1), `ERR_GET_RFLAGS` (Interface Declarations, Impact: 3.1), `ERR_GET_REASON` (Interface Declarations, Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `jlh` **(Many-Argument Workhorses)** (Impact: 11.2)
    * *Intent:* """Print v8::(Maybe)?Local value"""
  * `no_arg_cmd` **(Compute Cores)** (Impact: 10.8)
  * `jca` **(Compute Cores)** (Impact: 10.5)
...
**Top Internal Functions/Classes:**
  * `NODE_SET_METHOD` **(Many-Argument Workhorses)** (Impact: 2.7)
    * *Intent:* // Used to be a macro, hence the uppercase name.
  * `NODE_SET_METHOD` **(Many-Argument Workhorses)** (Impact: 2.6)
    * *Intent:* // Used to be a macro, hence the uppercase name.
  * `NODE_SET_PROTOTYPE_METHOD` **(Many-Argument Workhorses)** (Impact: 2.6)
    * *Intent:* #define NODE_SET_METHOD node::NODE_SET_METHOD // Used to be a macro, hence the uppercase name. // No...
  * `NODE_V8_UNIXTIME` **(Interface Declarations)** (Impact: 1.6)
  * `RemoveEnvironmentCleanupHook` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 7 instances
...
**Top Internal Functions/Classes:**
  ...
  * `LocalVector` **(I/O & Config Routines)** (Impact: 2.3)
  * `LocalBase` **(I/O & Config Routines)** (Impact: 2.2)
