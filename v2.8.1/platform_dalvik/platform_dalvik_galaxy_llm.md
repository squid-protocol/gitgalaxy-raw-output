...
> **File Composition:** Data / Markup / Trivial 41%, Annotated Framework Methods Files 16%, Large Core Modules 15%, Interface Declarations Files 14%, State Mutators Files 5%
...
- `parseInstruction` **(Compute Cores)** (@ `dx/src/com/android/dx/cf/code/BytecodeArray.java`) -> Impact: **748.9** | LOC: 602
...
- `ropFor` **(Compute Cores)** (@ `dexgen/src/com/android/dexgen/rop/code/Rops.java`) -> Impact: **254.0** | LOC: 116
  * *Intent:* * shared instance. * * <p><b>Note:</b> This method does not do complete error checking on * its arguments, and so it may return an instance which seem...
...
- **Heaviest Functions:** `parseFlags` (Compute Cores, Impact: 116.5), `dumpMethod` (Many-Argument Workhorses, Impact: 52.4), `processAllFiles` (I/O & Config Routines, Impact: 32.5)
...
- **Heaviest Functions:** `readHeader` (Compute Cores, Impact: 15.0), `writeMap` (Compute Cores, Impact: 10.8), `readMap` (Compute Cores, Impact: 8.4)
...
- **Heaviest Functions:** `transform` (Compute Cores, Impact: 64.7), `adjustTypeList` (Type Conversions, Impact: 4.7), `adjust` (Interface Declarations, Impact: 4.7)
...
- **Heaviest Functions:** `test2_set1` (I/O & Config Routines, Impact: 9.3), `test2_bitCount` (I/O & Config Routines, Impact: 8.2), `test_makeBitSet` (Tests & Verification, Impact: 6.0)

### 5. `dx/junit-tests/com/android/dx/util/ListIntSetTest.java` (JAVA) -> Cumulative Risk: **554.81**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +2.14)
...
- **Heaviest Functions:** `test_mergeA` (I/O & Config Routines, Impact: 6.3), `test_mergeB` (I/O & Config Routines, Impact: 6.3), `test_mergeWithBitIntSet` (I/O & Config Routines, Impact: 6.3)

### 6. `dx/etc/mainDexClasses` (SHELL) -> Cumulative Risk: **551.9**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.88)
...
- **Heaviest Functions:** `Anonymous_Block` (Compute Cores, Impact: 11.1), `__global_context__` (I/O & Config Routines, Impact: 6.5), `Anonymous_Block` (I/O & Config Routines, Impact: 4.5)

### 7. `dx/junit-tests/com/android/dx/util/IntListTest.java` (JAVA) -> Cumulative Risk: **550.24**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +0.55)
...
- **Heaviest Functions:** `test_contains` (Tests & Verification, Impact: 6.9), `label` (Encapsulated Accessors, Impact: 1.9), `test_addSorted` (Tests & Verification, Impact: 1.4)
...
- **Heaviest Functions:** `write` (Compute Cores, Impact: 19.3), `IndentingWriter` (Compute Cores, Impact: 17.1), `synchronized` (I/O & Config Routines, Impact: 14.2)
...
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +1.44)
...
- **Heaviest Functions:** `test_merge` (I/O & Config Routines, Impact: 6.2), `test_mergeWithListIntSet` (I/O & Config Routines, Impact: 6.2), `test_mergeAndExpand` (I/O & Config Routines, Impact: 6.2)
...
**Top Internal Functions/Classes:**
  ...
  * `processFileBytes` **(Many-Argument Workhorses)** (Impact: 22.2)
...
**Top Internal Functions/Classes:**
  ...
  * `parseTableswitch` **(Compute Cores)** (Impact: 8.7)
...
**Top Internal Functions/Classes:**
  * `ropFor` **(Compute Cores)** (Impact: 254.0)
    * *Intent:* * shared instance. * * <p><b>Note:</b> This method does not do complete error checking on * its argu...
  * `pickBinaryOp` **(Many-Argument Workhorses)** (Impact: 80.7)
    * *Intent:* * Returns the appropriate binary arithmetic rop for the given type * and arguments. The result is a ...
...
**Top Internal Functions/Classes:**
  ...
  * `forEachNonSubBlockDepthFirst0` **(Many-Argument Workhorses)** (Impact: 11.5)
...
**Top Internal Functions/Classes:**
  ...
  * `readIntoMap` **(Generic / Templated Code)** (Impact: 13.9)
  * `readSortableTypes` **(Compute Cores)** (Impact: 12.7)
    * *Intent:* /** * Reads just enough data on each class so that we can sort it and then find * it later. */
  * `transformCode` **(Many-Argument Workhorses)** (Impact: 12.1)
  * `transformMethods` **(Many-Argument Workhorses)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `simulateMath` **(Compute Cores)** (Impact: 84.6)
  ...
  * `simulatePhi` **(Compute Cores)** (Impact: 16.0)
...
**Top Internal Functions/Classes:**
  ...
  * `insertPlainInsnBefore` **(Many-Argument Workhorses)** (Impact: 13.6)
    * *Intent:* /** * Inserts a new PlainInsn before the given instruction. * TODO: move this somewhere more appropr...
  * `processRegister` **(Generic / Templated Code)** (Impact: 11.6)
...
**Top Internal Functions/Classes:**
  ...
  * `onSourceChanged` **(Many-Argument Workhorses)** (Impact: 8.8)
...
**Top Internal Functions/Classes:**
  ...
  * `readCatchHandler` **(Compute Cores)** (Impact: 6.3)
  * `findCatchHandlerIndex` **(Compute Cores)** (Impact: 5.6)
...
**Top Internal Functions/Classes:**
  ...
  * `checkForEmptyRange` **(Compute Cores)** (Impact: 20.6)
...
**Top Internal Functions/Classes:**
  ...
  * `annotate` **(Compute Cores)** (Impact: 11.3)
    * *Intent:* /** {@inheritDoc} */
  * `write` **(Many-Argument Workhorses)** (Impact: 11.1)
    * *Intent:* /** {@inheritDoc} */
