...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.70; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 21%, Data / Markup / Trivial 17%, Declarative / Non-Code 15%, Interface Declarations Files 13%, Many-Argument Workhorses Files 8%
...
- `.svg`: 41x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Machine-Generated Source Code Signature: 203 LOC), 1x Excluded (Machine-Generated Source Code Signature: 220 LOC)
...
- `parse_cbor_internal` **(Compute Cores)** (@ `rapidfuzz-3.14.3/extern/taskflow/3rd-party/nlohmann/json.hpp`) -> Impact: **447.8** | LOC: 434
  * *Intent:* */
...
- **Composition Archetype:** `Interface Declarations Files` (z +1.84)
...
- **Heaviest Functions:** `tbb_thread_v3::internal_start` (Compute Cores, Impact: 23.0), `thread_sleep_v3` (Compute Cores, Impact: 15.4), `tbb_thread_v3::join` (Compute Cores, Impact: 12.3)

### 2. `rapidfuzz-3.14.3/extern/taskflow/3rd-party/tbb/python/tbb/__init__.py` (PYTHON) -> Cumulative Risk: **804.03**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.45)
- **Magnitude:** 250.42 | **LOC:** 326 | **CtrlFlow:** 21.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Concurrency (99.948%), Tech Debt (99.1845%)
- **Heaviest Functions:** `__exit__` (Defensive Guards, Impact: 14.1), `__enter__` (Defensive Guards, Impact: 9.3), `__init__` (Type Conversions, Impact: 8.7)

### 3. `rapidfuzz-3.14.3/extern/taskflow/3rd-party/tbb/src/tbbproxy/tbbproxy.cpp` (CPP) -> Cumulative Risk: **739.08**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.60)
...
- **Heaviest Functions:** `_load` (Many-Argument Workhorses, Impact: 50.4), `load` (Many-Argument Workhorses, Impact: 45.4), `error` (Many-Argument Workhorses, Impact: 30.5)
...
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.76)
...
- **Heaviest Functions:** `lmpar2` (Many-Argument Workhorses, Impact: 57.1)

### 5. `rapidfuzz-3.14.3/extern/taskflow/3rd-party/eigen-3.3.7/unsupported/Eigen/CXX11/src/ThreadPool/NonBlockingThreadPool.h` (CPP) -> Cumulative Risk: **733.49**
...
- **Heaviest Functions:** `WorkerLoop` (Compute Cores, Impact: 17.4), `NonBlockingThreadPoolTempl` (State Mutators, Impact: 12.3), `WaitForWork` (Compute Cores, Impact: 10.7)

### 6. `rapidfuzz-3.14.3/extern/taskflow/3rd-party/tbb/src/perf/time_hash_map.cpp` (CPP) -> Cumulative Risk: **732.31**
...
- **Heaviest Functions:** `test` (Compute Cores, Impact: 31.1), `test` (Compute Cores, Impact: 29.5), `main` (Compute Cores, Impact: 9.6)

### 7. `rapidfuzz-3.14.3/extern/taskflow/tfprof/js/index.js` (JAVASCRIPT) -> Cumulative Risk: **731.71**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +1.05)
- **Magnitude:** 413.76 | **LOC:** 996 | **CtrlFlow:** 6.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `main` (Callbacks & Closures, Impact: 37.5), `_adjust_tb` (Callbacks & Closures, Impact: 9.6), `_render_rankGraph` (Interface Declarations, Impact: 9.4)
...
- **Heaviest Functions:** `CacheBin::putList` (Many-Argument Workhorses, Impact: 41.8), `OperationPreprocessor::operator()` (Compute Cores, Impact: 39.4), `CacheBin::cleanToThreshold` (Many-Argument Workhorses, Impact: 25.9)
...
- **Heaviest Functions:** `initialize_hardware_concurrency_info` (Compute Cores, Impact: 34.8), `initialize_hardware_concurrency_info` (Compute Cores, Impact: 16.6), `FindProcessorGroupIndex` (Compute Cores, Impact: 14.2)

### 10. `rapidfuzz-3.14.3/extern/taskflow/3rd-party/tbb/src/tbb/concurrent_vector.cpp` (CPP) -> Cumulative Risk: **719.2**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -1.62)
...
- **Heaviest Functions:** `concurrent_vector_base_v3::internal_compact` (Many-Argument Workhorses, Impact: 50.5), `concurrent_vector_base_v3::internal_assign` (Many-Argument Workhorses, Impact: 38.8), `concurrent_vector_base_v3::helper::enable_segment` (Many-Argument Workhorses, Impact: 27.5)
...
**Top Internal Functions/Classes:**
  * `parse_cbor_internal` **(Compute Cores)** (Impact: 447.8)
    * *Intent:* */
  * `parse_msgpack_internal` **(Compute Cores)** (Impact: 293.4)
    * *Intent:* ///////////// // MsgPack // ///////////// /*! */
  * `scan_string` **(Compute Cores)** (Impact: 269.2)
    * *Intent:* */
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 9e-05 | `Ripple Effect (Closeness):` 0.002707
...
**Top Internal Functions/Classes:**
  ...
  * `prettify` **(Compute Cores)** (Impact: 45.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000256 | `Ripple Effect (Closeness):` 0.019439
...
**Top Internal Functions/Classes:**
  ...
  * `_getopt_initialize_a` **(Many-Argument Workhorses)** (Impact: 15.1)
  * `_getopt_initialize_w` **(Many-Argument Workhorses)** (Impact: 15.1)
...
**Top Internal Functions/Classes:**
  ...
  * `pipeline_2P_SP_264VideoFormat` **(Compute Cores)** (Impact: 62.7)
    * *Intent:* // ---------------------------------------------------------------------------- // two pipes (SP), L...
  * `pipeline_1P_S_264VideoFormat` **(Compute Cores)** (Impact: 62.0)
    * *Intent:* // ---------------------------------------------------------------------------- // one pipe (S), L l...
  * `pl` **(Compute Cores)** (Impact: 56.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `internal_remove_edge` **(Many-Argument Workhorses)** (Impact: 61.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000328 | `Ripple Effect (Closeness):` 0.008377
...
- `rapidfuzz-3.14.3/extern/taskflow/3rd-party/tbb/examples/common/utility/utility.h` -> **Severity: 4.499** (Embedded: 0.0633 * Error Risk: 71.0817%)
- `rapidfuzz-3.14.3/extern/taskflow/3rd-party/tbb/examples/parallel_for/tachyon/src/types.h` -> **Severity: 4.233** (Embedded: 0.0495 * Error Risk: 85.5921%)
- `rapidfuzz-3.14.3/extern/taskflow/taskflow/core/observer.hpp` -> **Severity: 3.836** (Embedded: 0.0454 * Error Risk: 84.5777%)
- `rapidfuzz-3.14.3/extern/taskflow/3rd-party/ff/parallel_for_internals.hpp` -> **Severity: 3.601** (Embedded: 0.039 * Error Risk: 92.4293%)
- `rapidfuzz-3.14.3/extern/taskflow/taskflow/core/executor.hpp` -> **Severity: 3.576** (Embedded: 0.0537 * Error Risk: 66.6502%)
