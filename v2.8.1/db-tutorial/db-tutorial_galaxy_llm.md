...
> **Composition Archetype:** `Small Flat Repo` (z -0.67; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 35%, Declarative / Non-Code 22%, Large Core Modules 9%, Tests & Verification Files 9%, Annotated Framework Methods Files 7%
...
- `updateCpms` **(Compute Cores)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter07.java`) -> Impact: **73.1** | LOC: 76
...
- `createIndex` **(Defensive Guards)** (@ `codes/javadb/elasticsearch/elasticsearch6/src/main/java/io/github/dunwu/javadb/elasticsearch/mapper/BaseEsMapper.java`) -> Impact: **40.2** | LOC: 21
...
- **Heaviest Functions:** `cache_rows` (Compute Cores, Impact: 8.4), `can_cache` (Compute Cores, Impact: 7.4), `add_to_cart` (Many-Argument Workhorses, Impact: 7.1)
...
- **Heaviest Functions:** `testShopppingCartCookies` (Defensive Guards, Impact: 21.5), `testCacheRows` (Defensive Guards, Impact: 20.6), `testLoginCookies` (Defensive Guards, Impact: 19.9)
...
- **Heaviest Functions:** `sharded_bpop_helper` (Many-Argument Workhorses, Impact: 21.3), `purchase_item_with_lock` (Many-Argument Workhorses, Impact: 10.4), `call` (Many-Argument Workhorses, Impact: 10.2)
...
- **Heaviest Functions:** `copy_logs_to_redis` (Many-Argument Workhorses, Impact: 28.7), `readblocks_gz` (Compute Cores, Impact: 22.9), `process_logs_from_redis` (Many-Argument Workhorses, Impact: 19.6)
...
- **Heaviest Functions:** `get_shard_results_thread` (Many-Argument Workhorses, Impact: 21.2), `search_and_zsort` (Many-Argument Workhorses, Impact: 20.2), `syndicate_status` (Many-Argument Workhorses, Impact: 18.0)
...
- **Heaviest Functions:** `clean_counters` (Compute Cores, Impact: 17.7), `import_ips_to_redis` (Compute Cores, Impact: 16.5), `import_ips_to_redis` (Compute Cores, Impact: 14.8)
...
- **Heaviest Functions:** `filter_content` (Many-Argument Workhorses, Impact: 22.8), `refill_timeline` (Many-Argument Workhorses, Impact: 17.1), `process_filters` (Compute Cores, Impact: 15.5)
...
- **Heaviest Functions:** `article_vote` (Defensive Guards, Impact: 9.1), `add_to_cart` (Many-Argument Workhorses, Impact: 7.1), `article_vote` (Many-Argument Workhorses, Impact: 6.9)
...
- **Heaviest Functions:** `update_cpms` (Compute Cores, Impact: 29.1), `search_and_zsort` (Many-Argument Workhorses, Impact: 20.8), `parse` (Compute Cores, Impact: 19.1)
...
- **Heaviest Functions:** `update_aggregates` (Many-Argument Workhorses, Impact: 17.6), `test_user_location` (Compute Cores, Impact: 13.9), `get_code` (Compute Cores, Impact: 13.4)
...
**Top Internal Functions/Classes:**
  * `updateCpms` **(Compute Cores)** (Impact: 73.1)
...
**Top Internal Functions/Classes:**
  ...
  * `parse` **(Compute Cores)** (Impact: 19.1)
...
**Top Internal Functions/Classes:**
  ...
  * `readlines` **(Many-Argument Workhorses)** (Impact: 10.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getScrollData` **(Many-Argument Workhorses)** (Impact: 17.3)
  * `toEntity` **(Defensive Guards)** (Impact: 17.0)
  * `getRowFromResult` **(Compute Cores)** (Impact: 11.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `acquire_lock_with_timeout` **(Many-Argument Workhorses)** (Impact: 9.7)
...
**Top Internal Functions/Classes:**
  * `saveRankWithRegions` **(Compute Cores)** (Impact: 21.8)
  ...
  * `getRankByMemberWithRegions` **(Compute Cores)** (Impact: 19.0)
    * *Intent:* /** * 根据 member，查询成员在排行榜中的排名，从 0 开始计数 * <p> * 如果成员不在排行榜，则统一返回 {@link #TOTAL_RANK_LENGTH} * * @param ...
  * `deleteWithAutoAdjust` **(Many-Argument Workhorses)** (Impact: 18.1)
...
**Top Internal Functions/Classes:**
  ...
  * `add_to_cart` **(Many-Argument Workhorses)** (Impact: 7.1)
    * *Intent:* # <end id="_1311_14471_8270"/> # 代码清单 2-4 # <start id="_1311_14471_8279"/>
  * `cache_request` **(Many-Argument Workhorses)** (Impact: 6.9)
...
**Top Internal Functions/Classes:**
  * `createIndex` **(Defensive Guards)** (Impact: 40.2)
...
**Top Internal Functions/Classes:**
  * `pojoPageByLastId` **(Tests & Verification)** (Impact: 9.9)
  * `pojoPageByLastIdInDay` **(Tests & Verification)** (Impact: 9.9)
  * `pojoScroll` **(Tests & Verification)** (Impact: 9.0)
  * `pojoScrollInDay` **(Tests & Verification)** (Impact: 9.0)
  * `batchSave` **(Tests & Verification)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `updateTokenPipeline` **(Many-Argument Workhorses)** (Impact: 16.3)
...
**Top Internal Functions/Classes:**
  * `clear` **(Tests & Verification)** (Impact: 6.4)
  * `test04` **(Tests & Verification)** (Impact: 6.3)
  * `test05` **(Tests & Verification)** (Impact: 6.3)
  * `test06` **(Tests & Verification)** (Impact: 6.2)
  * `test02` **(Tests & Verification)** (Impact: 5.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `onResponse` **(Compute Cores)** (Impact: 10.3)
  * `pojoScroll` **(Callbacks & Closures)** (Impact: 9.9)
  * `pojoPageByLastId` **(Callbacks & Closures)** (Impact: 9.8)
  * `deleteIndex` **(Defensive Guards)** (Impact: 6.8)
  * `saveBatch` **(Tests & Verification)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
