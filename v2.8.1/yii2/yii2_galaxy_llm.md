...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +2.21; from the repo's file-archetype mix)
> **File Composition:** Interface Declarations Files 37%, Data / Markup / Trivial 16%, Large Core Modules 14%, Declarative / Non-Code 10%, Defensive Guards Files 6%
...
- **Heaviest Functions:** `saveMessagesToDb` (Many-Argument Workhorses, Impact: 106.0), `saveMessagesToPO` (Many-Argument Workhorses, Impact: 77.7), `saveMessagesCategoryToPHP` (Many-Argument Workhorses, Impact: 71.1)
...
- **Heaviest Functions:** `extractColumnType` (Many-Argument Workhorses, Impact: 30.6), `loadTableConstraints` (Compute Cores, Impact: 25.5), `createColumn` (Compute Cores, Impact: 20.3)
...
- **Heaviest Functions:** `toArray` (Many-Argument Workhorses, Impact: 38.2), `filter` (Compute Cores, Impact: 37.4), `getValue` (Defensive Guards, Impact: 26.2)
...
- **Heaviest Functions:** `actionUp` (Compute Cores, Impact: 26.5), `actionRedo` (Compute Cores, Impact: 23.7), `actionMark` (Compute Cores, Impact: 23.5)
...
- **Heaviest Functions:** `loadColumnSchema` (Compute Cores, Impact: 32.6), `loadTableConstraints` (Compute Cores, Impact: 29.3), `findColumns` (Defensive Guards, Impact: 16.1)
...
- **Heaviest Functions:** `bindActionParams` (Defensive Guards, Impact: 48.7), `runAction` (Defensive Guards, Impact: 43.3), `getActionArgsHelp` (Compute Cores, Impact: 39.6)

### 7. `framework/db/pgsql/Schema.php` (PHP) -> Cumulative Risk: **534.24**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.51)
...
- **Heaviest Functions:** `findColumns` (Compute Cores, Impact: 39.9), `loadTableConstraints` (Compute Cores, Impact: 29.3), `loadColumnSchema` (Defensive Guards, Impact: 25.9)
...
- **Heaviest Functions:** `truncateHtml` (Many-Argument Workhorses, Impact: 40.6), `matchWildcard` (Many-Argument Workhorses, Impact: 27.9), `convertIniSizeToBytes` (Type Conversions, Impact: 13.5)
...
- **Heaviest Functions:** `runAction` (Compute Cores, Impact: 21.6), `findLayoutFile` (Compute Cores, Impact: 21.6), `bindInjectedParams` (Defensive Guards, Impact: 18.9)
...
- **Heaviest Functions:** `filterUnionTypeActionParam` (Compute Cores, Impact: 46.2), `bindActionParams` (Defensive Guards, Impact: 32.8), `filterSingleTypeActionParam` (Compute Cores, Impact: 26.1)
...
**Top Internal Functions/Classes:**
  ...
  * `lastExcludeMatchingFromList` **(Defensive Guards)** (Impact: 27.4)
...
**Top Internal Functions/Classes:**
  ...
  * `testAmbiguousColumnIndexBy` **(Compute Cores)** (Impact: 13.2)
    * *Intent:* /** * Ensure no ambiguous column error occurs on indexBy with JOIN. * * @see https://github.com/yiis...
  * `testJoinWith` **(Tests & Verification)** (Impact: 10.0)
  * `testLoadRelations` **(Tests & Verification)** (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `getScreenSize` **(Compute Cores)** (Impact: 28.1)
...
**Top Internal Functions/Classes:**
  ...
  * `setActivePlaceholder` **(Defensive Guards)** (Impact: 6.3)
...
**Top Internal Functions/Classes:**
  ...
  * `filterByModels` **(Compute Cores)** (Impact: 31.2)
    * *Intent:* /** * @param array $models */
  * `prefixKeyColumns` **(Defensive Guards)** (Impact: 16.8)
...
**Top Internal Functions/Classes:**
  ...
  * `getActionArgsHelp` **(Compute Cores)** (Impact: 39.6)
  ...
  * `parseDocCommentTags` **(Compute Cores)** (Impact: 10.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getNewMigrations` **(Compute Cores)** (Impact: 21.4)
...
**Top Internal Functions/Classes:**
  ...
  * `sendStreamAsFile` **(Many-Argument Workhorses)** (Impact: 21.8)
  ...
  * `prepare` **(Compute Cores)** (Impact: 16.1)
...
**Top Internal Functions/Classes:**
  ...
  * `initConnection` **(I/O & Config Routines)** (Impact: 11.1)
...
**Top Internal Functions/Classes:**
  ...
  * `normalizeDefinition` **(Defensive Guards)** (Impact: 23.9)
...
**Top Internal Functions/Classes:**
  * `loadColumnSchema` **(Compute Cores)** (Impact: 32.6)
  * `loadTableConstraints` **(Compute Cores)** (Impact: 29.3)
  * `findColumns` **(Defensive Guards)** (Impact: 16.1)
  ...
  * `resolveTableNames` **(Compute Cores)** (Impact: 11.7)
...
**Top Internal Functions/Classes:**
  ...
  * `oldBuildOrderByAndLimit` **(Many-Argument Workhorses)** (Impact: 19.1)
  ...
  * `normalizeTableRowData` **(Defensive Guards)** (Impact: 14.8)
