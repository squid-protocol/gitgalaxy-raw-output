...
> **Composition Archetype:** `Hub-Coupled App` (z +0.26; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 46%, Interface Declarations Files 21%, Parameter Forwarders Files 8%, Data / Markup / Trivial 6%, Many-Argument Workhorses Files 6%
...
- `dump` **(Compute Cores)** (@ `impacket/examples/secretsdump.py`) -> Impact: **153.0** | LOC: 316
...
- **Heaviest Functions:** `__new__` (Many-Argument Workhorses, Impact: 12.5), `setter` (Compute Cores, Impact: 6.4), `__init__` (Parameter Forwarders, Impact: 3.7)
...
- **Heaviest Functions:** `parseRow` (Many-Argument Workhorses, Impact: 192.0), `kerberosLogin` (Many-Argument Workhorses, Impact: 168.8), `_parseValue` (Many-Argument Workhorses, Impact: 142.9)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 38.4), `__init__` (Many-Argument Workhorses, Impact: 30.0), `fragment_by_list` (Compute Cores, Impact: 20.2)
...
- **Heaviest Functions:** `getNTLMSSPType3` (Many-Argument Workhorses, Impact: 78.4), `computeResponseNTLMv2` (Many-Argument Workhorses, Impact: 31.2), `KXKEY` (Many-Argument Workhorses, Impact: 31.1)
...
- **Heaviest Functions:** `smbComNtCreateAndX` (Many-Argument Workhorses, Impact: 134.2), `findFirst2` (Many-Argument Workhorses, Impact: 117.6), `smb2Create` (Many-Argument Workhorses, Impact: 112.1)
...
- **Heaviest Functions:** `fromString` (Compute Cores, Impact: 27.3), `fromString` (Compute Cores, Impact: 19.8), `getData` (Compute Cores, Impact: 17.4)
...
- **Heaviest Functions:** `bind` (Many-Argument Workhorses, Impact: 107.5), `recv` (Compute Cores, Impact: 44.0), `_transport_send` (Many-Argument Workhorses, Impact: 37.6)
...
- **Heaviest Functions:** `getCredential` (Compute Cores, Impact: 31.5), `parseFile` (Many-Argument Workhorses, Impact: 29.2), `__init__` (Compute Cores, Impact: 21.3)
...
- **Heaviest Functions:** `kerberosLogin` (Many-Argument Workhorses, Impact: 116.5), `login` (Many-Argument Workhorses, Impact: 97.5), `create` (Many-Argument Workhorses, Impact: 77.1)
...
- **Heaviest Functions:** `marshalMe` (Compute Cores, Impact: 85.9), `createMethods` (Many-Argument Workhorses, Impact: 64.0), `printClass` (Many-Argument Workhorses, Impact: 59.3)
...
**Top Internal Functions/Classes:**
  ...
  * `dump` **(Compute Cores)** (Impact: 153.0)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 52.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_set_element` **(Many-Argument Workhorses)** (Impact: 23.7)
  * `__set_field_values` **(Many-Argument Workhorses)** (Impact: 17.2)
  * `delete_element` **(Compute Cores)** (Impact: 15.0)
  * `_find_element` **(Many-Argument Workhorses)** (Impact: 10.9)
  * `__get_field_position` **(Compute Cores)** (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `fragment_by_list` **(Compute Cores)** (Impact: 20.2)
...
**Top Internal Functions/Classes:**
  * `bind` **(Many-Argument Workhorses)** (Impact: 107.5)
...
**Top Internal Functions/Classes:**
  ...
  * `known_sid` **(Compute Cores)** (Impact: 11.4)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 33.0)
...
**Top Internal Functions/Classes:**
  * `test_SamrQueryInformationUser2_SamrSetInformationUser2` **(Compute Cores)** (Impact: 9.2)
  * `test_hSamrQueryInformationUser2_hSamrSetInformationUser2` **(Compute Cores)** (Impact: 9.2)
  * `test_SamrUnicodeChangePasswordUser2` **(Compute Cores)** (Impact: 7.9)
  * `test_SamrAddMemberToGroup_SamrRemoveMemberFromGroup` **(Defensive Guards)** (Impact: 7.5)
  * `test_SamrQueryInformationAlias_SamrSetInformationAlias` **(Compute Cores)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__str__` **(Interface Declarations)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__parse_lp_data` **(Defensive Guards)** (Impact: 23.2)
...
**Top Internal Functions/Classes:**
  ...
  * `getKerberosS4U2SelfU2U` **(Compute Cores)** (Impact: 15.1)
...
**Top Internal Functions/Classes:**
  ...
  * `__connectDrds` **(Many-Argument Workhorses)** (Impact: 33.5)
  * `__decryptHash` **(Many-Argument Workhorses)** (Impact: 31.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_compileSimpleFilter` **(Compute Cores)** (Impact: 60.2)
