...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z -0.60; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 35%, Data / Markup / Trivial 21%, Large Core Modules 19%, Interface Declarations Files 6%, Many-Argument Workhorses Files 5%
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 19.2 | 4.7 | 0.0 |
...
| connectivity | 43534 | 2511 | 28 | `bsd/nfs/nfs.h` |
...
| dead_code | 14135 | 1438 | 9 | `bsd/net/if.c` |
...
- **Heaviest Functions:** `commpage_init_cpu_capabilities` (I/O & Config Routines, Impact: 23.4), `commpage_populate` (I/O & Config Routines, Impact: 17.6), `commpage_is_in_pfz64` (Compute Cores, Impact: 9.2)
...
- **Heaviest Functions:** `_audit_ppsratecheck` (Many-Argument Workhorses, Impact: 21.8), `audit_sysctl_malloc_debug` (Many-Argument Workhorses, Impact: 21.2), `_audit_malloc` (Many-Argument Workhorses, Impact: 19.8)
...
- **Heaviest Functions:** `mt_fixed_task_counts` (Many-Argument Workhorses, Impact: 20.4), `mt_fixed_thread_counts` (Many-Argument Workhorses, Impact: 15.1), `mt_mtc_update_fixed_counts` (Many-Argument Workhorses, Impact: 11.3)
...
- **Heaviest Functions:** `commpage_init_cpu_capabilities` (I/O & Config Routines, Impact: 27.6), `commpage_allocate` (Many-Argument Workhorses, Impact: 25.1), `commpage_populate_one` (Many-Argument Workhorses, Impact: 18.1)

### 5. `bsd/security/audit/audit_syscalls.c` (C) -> Cumulative Risk: **682.57**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.47)
...
- **Heaviest Functions:** `auditon` (Many-Argument Workhorses, Impact: 414.9), `audit` (Many-Argument Workhorses, Impact: 68.9), `setaudit_addr` (Many-Argument Workhorses, Impact: 27.2)
...
- **Heaviest Functions:** `dtrace_getpcstack` (Many-Argument Workhorses, Impact: 87.3), `dtrace_getufpstack` (Many-Argument Workhorses, Impact: 39.6), `dtrace_getarg` (Many-Argument Workhorses, Impact: 28.6)
...
- **Heaviest Functions:** `setreuid` (Many-Argument Workhorses, Impact: 56.5), `setregid` (Many-Argument Workhorses, Impact: 49.4), `setpgid` (Many-Argument Workhorses, Impact: 43.8)
...
- **Heaviest Functions:** `vn_rename` (Many-Argument Workhorses, Impact: 205.7), `vnode_getattr` (Many-Argument Workhorses, Impact: 92.0), `vnode_setattr` (Many-Argument Workhorses, Impact: 81.4)
...
- **Heaviest Functions:** `fork1` (Many-Argument Workhorses, Impact: 84.7), `forkproc` (Compute Cores, Impact: 65.7), `uthread_alloc` (Many-Argument Workhorses, Impact: 46.0)
...
- **Heaviest Functions:** `machine_trace_thread64` (Many-Argument Workhorses, Impact: 61.4), `kdp_i386_trap` (Many-Argument Workhorses, Impact: 50.1), `machine_trace_thread` (Many-Argument Workhorses, Impact: 43.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.136502
...
**Top Internal Functions/Classes:**
  * `OSKext::load` **(Many-Argument Workhorses)** (Impact: 796.9)
    * *Intent:* /********************************************************************* *****************************...
  * `OSKext::jettisonLinkeditSegment` **(Compute Cores)** (Impact: 468.4)
...
- **Risk Profile:** Cognitive Load (91.1936%), Tech Debt (21.4951%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `IOPMrootDomain::powerChangeDone` **(Compute Cores)** (Impact: 168.5)
...
**Top Internal Functions/Classes:**
  ...
  * `slab_free` **(Compute Cores)** (Impact: 130.8)
...
**Top Internal Functions/Classes:**
  ...
  * `key_cmpspidx_withmask` **(Compute Cores)** (Impact: 131.5)
  ...
  * `key_timehandler` **(Compute Cores)** (Impact: 125.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `nfs_vnop_write` **(Compute Cores)** (Impact: 220.6)
  ...
  * `nfs_vnop_pageout` **(Compute Cores)** (Impact: 158.0)
...
**Top Internal Functions/Classes:**
  ...
  * `IOService::probeCandidates` **(Compute Cores)** (Impact: 178.3)
...
**Top Internal Functions/Classes:**
  ...
  * `IOGeneralMemoryDescriptor::wireVirtual` **(Compute Cores)** (Impact: 143.8)
**Contextual Mitigations & Amplifications:**
...
- `osfmk/i386/cpu_data.h` -> **Severity: 0.187** (Bridge: 0.0019 * Flux: 99.8948%)
- `osfmk/kern/thread.h` -> **Severity: 0.088** (Bridge: 0.0101 * Flux: 8.6515%)
...
- `bsd/sys/cdefs.h` -> **Severity: 15.389** (Embedded: 0.2844 * Error Risk: 54.1064%)
- `EXTERNAL_HEADERS/AvailabilityInternal.h` -> **Severity: 13.303** (Embedded: 0.1365 * Error Risk: 97.4586%)
- `EXTERNAL_HEADERS/Availability.h` -> **Severity: 10.737** (Embedded: 0.1835 * Error Risk: 58.5068%)
- `osfmk/kern/queue.h` -> **Severity: 10.28** (Embedded: 0.1095 * Error Risk: 93.841%)
- `libkern/os/overflow.h` -> **Severity: 9.407** (Embedded: 0.1257 * Error Risk: 74.8553%)
