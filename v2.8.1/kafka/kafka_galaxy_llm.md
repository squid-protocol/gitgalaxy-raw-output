...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +1.21; from the repo's file-archetype mix)
> **File Composition:** Tests & Verification Files 21%, Annotated Framework Methods Files 20%, Large Core Modules 18%, Data / Markup / Trivial 15%, Interface Declarations Files 9%
...
| connectivity | 69128 | 5883 | 23 | `group-coordinator/src/test/java/org/apache/kafka/coordinator/group/GroupMetadataManagerTest.java` |
...
| threat | 546 | 172 | 0 | `streams/streams-scala/src/main/scala/org/apache/kafka/streams/scala/ImplicitConversions.scala` |
...
- **Heaviest Functions:** `call` (I/O & Config Routines, Impact: 11.6), `stop` (Compute Cores, Impact: 8.0), `start` (Generic / Templated Code, Impact: 6.8)
...
- **Heaviest Functions:** `add` (Compute Cores, Impact: 5.5), `foreach` (Generic / Templated Code, Impact: 4.6), `remove` (Interface Declarations, Impact: 3.4)
...
- **Heaviest Functions:** `renew` (Compute Cores, Impact: 12.5), `takeAcknowledgedRecords` (Callbacks & Closures, Impact: 8.6), `acknowledgeAll` (Compute Cores, Impact: 7.6)

### 4. `trogdor/src/main/java/org/apache/kafka/trogdor/workload/SustainedConnectionWorker.java` (JAVA) -> Cumulative Risk: **691.57**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
...
- **Heaviest Functions:** `start` (Many-Argument Workhorses, Impact: 16.2), `stop` (Compute Cores, Impact: 8.4), `refresh` (I/O & Config Routines, Impact: 6.0)

### 5. `server-common/src/main/java/org/apache/kafka/server/util/timer/TimerTask.java` (JAVA) -> Cumulative Risk: **691.37**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.68)
- **Magnitude:** 52.76 | **LOC:** 55 | **CtrlFlow:** 7.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `setTimerTaskEntry` (Interface Declarations, Impact: 3.4), `synchronized` (I/O & Config Routines, Impact: 2.5), `cancel` (Interface Declarations, Impact: 2.3)
...
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +1.08)
...
- **Heaviest Functions:** `schedule` (Many-Argument Workhorses, Impact: 24.9), `CoordinatorExecutorImpl` (Generic / Templated Code, Impact: 2.5), `executeTask` (Generic / Templated Code, Impact: 1.8)

### 7. `trogdor/src/main/java/org/apache/kafka/trogdor/workload/ProduceBenchWorker.java` (JAVA) -> Cumulative Risk: **685.26**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.50)
...
- **Heaviest Functions:** `call` (Defensive Guards, Impact: 11.3), `takeTransactionAction` (Compute Cores, Impact: 11.2), `run` (Defensive Guards, Impact: 9.6)
...
- **Heaviest Functions:** `call` (I/O & Config Routines, Impact: 11.6), `consumeTasks` (Callbacks & Closures, Impact: 8.6), `start` (Generic / Templated Code, Impact: 6.8)

### 9. `clients/src/main/java/org/apache/kafka/clients/consumer/MockConsumer.java` (JAVA) -> Cumulative Risk: **683.74**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -0.23)
...
- **Heaviest Functions:** `poll` (Compute Cores, Impact: 23.4), `resetOffsetPosition` (Compute Cores, Impact: 15.1), `subscribe` (Compute Cores, Impact: 14.9)

### 10. `streams/src/main/java/org/apache/kafka/streams/state/internals/NamedCache.java` (JAVA) -> Cumulative Risk: **678.53**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.85)
...
- **Heaviest Functions:** `flush` (Compute Cores, Impact: 20.6), `computeSubSet` (Compute Cores, Impact: 18.6), `put` (Compute Cores, Impact: 15.4)
...
**Top Internal Functions/Classes:**
  ...
  * `testReconciliationProcess` **(Tests & Verification)** (Impact: 23.1)
...
**Top Internal Functions/Classes:**
  ...
  * `testAddRaftVoterRequest` **(Tests & Verification)** (Impact: 23.3)
  * `testRemoveRaftVoterRequest` **(Defensive Guards)** (Impact: 23.0)
  * `defaultQuorumInfo` **(Compute Cores)** (Impact: 19.1)
  * `testDescribeTopicsWithDescribeTopicPartitionsApiEdgeCase` **(Tests & Verification)** (Impact: 13.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `alterPartitionReassignments` **(Compute Cores)** (Impact: 74.9)
  * `getCreateTopicsCall` **(Many-Argument Workhorses)** (Impact: 52.6)
  * `updateFeatures` **(Compute Cores)** (Impact: 51.4)
  * `describeReplicaLogDirs` **(Compute Cores)** (Impact: 49.8)
...
**Top Internal Functions/Classes:**
  * `assertionFailedMessage` **(Compute Cores)** (Impact: 28.8)
  * `testMaybeInitializeWithErrorPartitionResponse` **(Tests & Verification)** (Impact: 25.6)
  * `testWriteShareGroupStateFailure` **(Tests & Verification)** (Impact: 21.4)
  * `testContainsAbortMarker` **(Tests & Verification)** (Impact: 17.7)
  * `testWriteShareGroupStateWithInvalidTopicsData` **(Tests & Verification)** (Impact: 15.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `verifyDescribeConfigsResponse` **(Many-Argument Workhorses)** (Impact: 31.6)
  * `createOffsetFetchRequest` **(Compute Cores)** (Impact: 16.5)
  * `createFetchResponse` **(Compute Cores)** (Impact: 13.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (20.7674%), Tech Debt (8.7641%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  * `shouldCloseAndReviveUncorruptedTasksWhenTimeoutExceptionThrownFromCommitDuringHandleCorruptedWithEOS` **(Tests & Verification)** (Impact: 28.0)
  * `shouldCloseAndReviveUncorruptedTasksWhenTimeoutExceptionThrownFromCommitDuringRevocationWithEOS` **(Tests & Verification)** (Impact: 26.6)
  * `shouldCloseAndReviveUncorruptedTasksWhenTimeoutExceptionThrownFromCommitWithAlos` **(Tests & Verification)** (Impact: 23.6)
  * `shouldCloseAndReviveUncorruptedTasksWhenTimeoutExceptionThrownFromCommitDuringRevocationWithAlos` **(Tests & Verification)** (Impact: 21.2)
  * `shouldCommitAllActiveTasksThatNeedCommittingOnHandleRevocationWithEosV2` **(Tests & Verification)** (Impact: 17.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (51.9891%), Tech Debt (13.7236%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `handleVoteResponse` **(Compute Cores)** (Impact: 38.7)
...
**Top Internal Functions/Classes:**
  ...
  * `testPollSendsRequestToJoin` **(Tests & Verification)** (Impact: 13.9)
  * `prepareOffsetCommitResponse` **(Generic / Templated Code)** (Impact: 13.1)
  * `newConsumerConfig` **(Many-Argument Workhorses)** (Impact: 10.4)
  * `listOffsetsResponse` **(Generic / Templated Code)** (Impact: 10.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 14 instances
...
**Top Internal Functions/Classes:**
  ...
  * `testRenewAcknowledgementNoResultInPoll` **(Tests & Verification)** (Impact: 14.2)
  * `testFetchWithThrottledDeliveryValidateDeliveryCount` **(Tests & Verification)** (Impact: 12.7)
  * `ComplexShareConsumer` **(Many-Argument Workhorses)** (Impact: 12.4)
  * `testShareConsumerAfterCoordinatorMovement` **(Tests & Verification)** (Impact: 12.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `buildSinkNode` **(Many-Argument Workhorses)** (Impact: 55.2)
  * `buildProcessorNode` **(Many-Argument Workhorses)** (Impact: 49.0)
  * `maybeAddToResetList` **(Many-Argument Workhorses)** (Impact: 35.6)
  * `connectProcessorAndStateStore` **(Compute Cores)** (Impact: 34.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `alterOffsetsSinkConnector` **(Generic / Templated Code)** (Impact: 26.1)
  * `testConnectorGeneratesTooManyTasks` **(Tests & Verification)** (Impact: 23.7)
  * `mockVersionedTaskIsolation` **(Generic / Templated Code)** (Impact: 22.4)
  * `mockAdminListConsumerGroupOffsets` **(Generic / Templated Code)** (Impact: 15.5)
  * `testAlterOffsetsSinkConnectorDeleteOffsetsError` **(Tests & Verification)** (Impact: 15.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 5 instances
...
**Top Internal Functions/Classes:**
  * `tableNamedMaterializedCountWithTopologyConfigShouldPreserveTopologyStructure` **(Tests & Verification)** (Impact: 26.3)
  * `tableNamedMaterializedCountShouldPreserveTopologyStructure` **(Tests & Verification)** (Impact: 24.8)
  * `tableAnonymousMaterializedCountShouldPreserveTopologyStructure` **(Tests & Verification)** (Impact: 24.8)
  * `tableAnonymousStoreTypedMaterializedCountShouldPreserveTopologyStructure` **(Tests & Verification)** (Impact: 24.7)
  * `addProcessorWithStore` **(Many-Argument Workhorses)** (Impact: 23.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `testAcksAndIdempotenceForIdempotentProducers` **(Tests & Verification)** (Impact: 11.1)
  * `testInitTransactionsWithKeepPreparedTxnAndTwoPhaseCommit` **(Defensive Guards)** (Impact: 9.5)
  * `testSendNotAllowedInPreparedTransactionState` **(Tests & Verification)** (Impact: 8.3)
  * `testSendOffsetsNotAllowedInPreparedTransactionState` **(Tests & Verification)** (Impact: 8.1)
**Contextual Mitigations & Amplifications:**
...
- `core/src/main/java/kafka/server/share/SharePartition.java` -> Churn: **88.58%** | Cog Load: 51.9891% | Debt: 13.7236%
