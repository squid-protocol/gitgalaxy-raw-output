...
> **Composition Archetype:** `Small Flat Repo` (z +1.67; from the repo's file-archetype mix)
> **File Composition:** Interface Declarations Files 44%, Large Core Modules 29%, Declarative / Non-Code 10%, Data / Markup / Trivial 8%, Compute Cores Files 2%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 87.1 | 11.6 | 0.0 | 0.0 |
...
- `_test_context` **(Compute Cores)** (@ `greenlet-3.3.2/src/greenlet/tests/test_contextvars.py`) -> Impact: **42.8** | LOC: 60
...
- `_check_deltas` **(Compute Cores)** (@ `greenlet-3.3.2/src/greenlet/tests/leakcheck.py`) -> Impact: **31.8** | LOC: 48
...
- **Heaviest Functions:** `PythonState::operator<<` (Compute Cores, Impact: 21.7), `PythonState::operator>>` (Compute Cores, Impact: 21.5), `PythonState::did_finish` (Compute Cores, Impact: 15.3)
...
- **Heaviest Functions:** `StackState::copy_from_stack` (Many-Argument Workhorses, Impact: 13.7), `StackState::copy_stack_to_heap` (Defensive Guards, Impact: 13.6), `StackState::copy_heap_to_stack` (Defensive Guards, Impact: 8.0)
...
- **Heaviest Functions:** `Greenlet::on_switchstack_or_initialstub_failure` (Many-Argument Workhorses, Impact: 19.3), `operator<<=` (Defensive Guards, Impact: 15.3), `Greenlet::check_switch_allowed` (I/O & Config Routines, Impact: 13.2)
...
- **Heaviest Functions:** `_bm_recur_frame` (Defensive Guards, Impact: 10.5), `bm_switch_deep` (Defensive Guards, Impact: 10.3), `bm_switch_shallow` (Defensive Guards, Impact: 5.6)
...
- **Heaviest Functions:** `UserGreenlet::inner_bootstrap` (Many-Argument Workhorses, Impact: 32.7), `UserGreenlet::g_switch` (I/O & Config Routines, Impact: 15.8), `UserGreenlet::g_initialstub` (Compute Cores, Impact: 13.0)
...
- **Heaviest Functions:** `ExceptionState::tp_traverse` (Defensive Guards, Impact: 3.9), `ExceptionState::operator>>` (Defensive Guards, Impact: 3.2), `ExceptionState::clear` (I/O & Config Routines, Impact: 2.5)
...
- **Heaviest Functions:** `normalize` (I/O & Config Routines, Impact: 12.9), `GreenletChecker` (Compute Cores, Impact: 6.8), `operator=` (Compute Cores, Impact: 6.6)
...
- **Heaviest Functions:** `GreenletVectorHandler` (Compute Cores, Impact: 17.0), `x86_slp_get_third_oldest_handler` (I/O & Config Routines, Impact: 7.1), `x86_slp_show_seh_chain` (Interface Declarations, Impact: 6.0)
...
- **Composition Archetype:** `Compute Cores Files` (z -0.41)
...
- **Heaviest Functions:** `PyThreadState_LeaveTracing` (Compute Cores, Impact: 6.2), `PyThreadState_EnterTracing` (Compute Cores, Impact: 4.7)
...
- **Heaviest Functions:** `green_repr` (Compute Cores, Impact: 20.2), `_green_dealloc_kill_started_non_main_greenlet` (Compute Cores, Impact: 18.0), `green_init` (Many-Argument Workhorses, Impact: 13.3)
...
**Top Internal Functions/Classes:**
  ...
  * `test_threaded_adv_leak` **(Interface Declarations)** (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `ContextExactChecker` **(Interface Declarations)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `PythonState::operator>>` **(Compute Cores)** (Impact: 21.5)
    * *Intent:* #endif
  * `PythonState::did_finish` **(Compute Cores)** (Impact: 15.3)
...
**Top Internal Functions/Classes:**
  ...
  * `wrap_refcount` **(Tests & Verification)** (Impact: 9.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_green_dealloc_kill_started_non_main_greenlet` **(Compute Cores)** (Impact: 18.0)
...
**Top Internal Functions/Classes:**
  ...
  * `UserGreenlet::g_initialstub` **(Compute Cores)** (Impact: 13.0)
...
**Top Internal Functions/Classes:**
  ...
  * `a` **(Interface Declarations)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_bm_recur_frame` **(Defensive Guards)** (Impact: 10.5)
...
**Top Internal Functions/Classes:**
  * `StackState::copy_from_stack` **(Many-Argument Workhorses)** (Impact: 13.7)
...
**Top Internal Functions/Classes:**
  ...
  * `set_clocks_used_doing_gc` **(Interface Declarations)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_trace_events_multiple_greenlets_switching_siblings` **(Interface Declarations)** (Impact: 3.8)
  ...
  * `test_trace_events_multiple_greenlets_switching` **(Interface Declarations)** (Impact: 3.2)
  * `_check_trace_events_from_greenlet_sets_profiler` **(Encapsulated Accessors)** (Impact: 2.7)
  * `test_a_greenlet_tracing` **(Interface Declarations)** (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `run` **(I/O & Config Routines)** (Impact: 3.4)
...
**Top Internal Functions/Classes:**
  * `test_switch_kwargs` **(Many-Argument Workhorses)** (Impact: 15.2)
  * `test_switch` **(Compute Cores)** (Impact: 13.2)
    * *Intent:* // // In C23, there is a standard syntax for attributes, and // GCC defines an attribute to use with...
  * `test_setparent` **(Compute Cores)** (Impact: 11.6)
  * `test_getcurrent` **(Compute Cores)** (Impact: 6.3)
  * `test_new_greenlet` **(Compute Cores)** (Impact: 6.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 6 instances
* *State Mutation (weighted view):* 18
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `test_exception_switch_and_do_in_g2` **(Compute Cores)** (Impact: 14.1)
  ...
  * `PyInit__test_extension_cpp` **(Interface Declarations)** (Impact: 4.2)
...
**Top Internal Functions/Classes:**
  ...
  * `x86_slp_show_seh_chain` **(Interface Declarations)** (Impact: 6.0)
  ...
  * `slp_switch` **(Interface Declarations)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_find_platform_headers` **(Interface Declarations)** (Impact: 1.1)
  * `_find_impl_headers` **(Interface Declarations)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__next__` **(Interface Declarations)** (Impact: 3.2)
  * `g` **(Interface Declarations)** (Impact: 3.0)
  * `__init__` **(Parameter Forwarders)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 9 instances
* *State Mutation (weighted view):* 31
...
**Top Internal Functions/Classes:**
  * `PyGreenlet_Switch` **(Many-Argument Workhorses)** (Impact: 10.9)
  * `PyGreenlet_New` **(Defensive Guards)** (Impact: 8.4)
  * `PyGreenlet_Throw` **(Defensive Guards)** (Impact: 5.2)
  * `Extern_PyGreenlet_MAIN` **(Interface Declarations)** (Impact: 3.3)
  * `Extern_PyGreenlet_ACTIVE` **(Interface Declarations)** (Impact: 3.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
