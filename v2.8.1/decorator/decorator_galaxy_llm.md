...
> **File Composition:** Data / Markup / Trivial 50%, Large Core Modules 33%, Declarative / Non-Code 17%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 79.5 | 40.6 | 42.3 | 0.0 |
...
- `dispatch_on` **(Compute Cores)** (@ `decorator-5.2.1/src/decorator.py`) -> Impact: **33.4** | LOC: 103
...
- `gen_func_dec` **(Compute Cores)** (@ `decorator-5.2.1/src/decorator.py`) -> Impact: **31.2** | LOC: 86
...
- `_dispatch` **(Defensive Guards)** (@ `decorator-5.2.1/src/decorator.py`) -> Impact: **10.8** | LOC: 17
- `dec` **(Callbacks & Closures)** (@ `decorator-5.2.1/src/decorator.py`) -> Impact: **10.4** | LOC: 9
...
### 1. `decorator-5.2.1/src/decorator.py` (PYTHON) -> Cumulative Risk: **680.22**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (99.1339%), Concurrency (80.777%)
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 56.5), `dispatch_on` (Compute Cores, Impact: 33.4), `gen_func_dec` (Compute Cores, Impact: 31.2)

### 2. `decorator-5.2.1/tests/documentation.py` (PYTHON) -> Cumulative Risk: **431.16**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (83.9481%), Documentation (79.5082%), Concurrency (78.7865%)
- **Heaviest Functions:** `__call__` (Defensive Guards, Impact: 12.8), `blocking` (Defensive Guards, Impact: 9.5), `_memoize` (Compute Cores, Impact: 8.4)

### 3. `decorator-5.2.1/performance.sh` (SHELL) -> Cumulative Risk: **52.31**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Declarative / Non-Code` (z +0.46)
- **Magnitude:** 1.86 | **LOC:** 14 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Stability (50.0%), Verification (2.3094%)
- **Heaviest Functions:** `__global_context__` (Unclassified, Impact: 1.6)
...
- **Magnitude:** 335.68 | **LOC:** 1891 | **CtrlFlow:** 8.3% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (23.2065%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `__call__` **(Defensive Guards)** (Impact: 12.8)
  * `blocking` **(Defensive Guards)** (Impact: 9.5)
  * `_memoize` **(Compute Cores)** (Impact: 8.4)
