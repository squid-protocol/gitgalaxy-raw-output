...
> **Composition Archetype:** `Typed Library` (z +1.62; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 52%, Defensive Guards Files 17%, Generic / Templated Code Files 12%, Declarative / Non-Code 6%, Interface Declarations Files 4%
...
- `feed_info` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/md.py`) -> Impact: **57.7** | LOC: 74
  * *Intent:* """Optimized feed using pre-computed character info."""
- `characters_popularity_compare` **(Compute Cores)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/cd.py`) -> Impact: **50.0** | LOC: 99
...
- `feed_info` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/md.py`) -> Impact: **36.2** | LOC: 44
  * *Intent:* """Optimized feed using pre-computed character info."""
...
- **Heaviest Functions:** `cut_sequence_chunks` (Many-Argument Workhorses, Impact: 65.9), `_character_flags` (Defensive Guards, Impact: 17.2), `any_specified_encoding` (Compute Cores, Impact: 13.7)
...
- **Heaviest Functions:** `is_suspiciously_successive_range` (Compute Cores, Impact: 67.6), `feed_info` (Many-Argument Workhorses, Impact: 57.7), `update` (Many-Argument Workhorses, Impact: 38.3)
...
- **Heaviest Functions:** `append` (Defensive Guards, Impact: 18.3), `__lt__` (Defensive Guards, Impact: 13.2), `language` (Compute Cores, Impact: 11.2)

### 4. `charset_normalizer-3.4.7/_mypyc_hook/backend.py` (PYTHON) -> Cumulative Risk: **566.44**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.04)
...
- **Heaviest Functions:** `get_requires_for_build_wheel` (Generic / Templated Code, Impact: 9.0)

### 5. `charset_normalizer-3.4.7/src/charset_normalizer/cd.py` (PYTHON) -> Cumulative Risk: **541.82**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.96)
...
- **Heaviest Functions:** `characters_popularity_compare` (Compute Cores, Impact: 50.0), `alpha_unicode_split` (Compute Cores, Impact: 29.1), `coherence_ratio` (Many-Argument Workhorses, Impact: 26.4)
...
- **Heaviest Functions:** `detect` (Many-Argument Workhorses, Impact: 47.1)
...
- **Heaviest Functions:** `cli_detect` (Compute Cores, Impact: 86.9), `__call__` (Defensive Guards, Impact: 18.2), `query_yes_no` (Compute Cores, Impact: 14.5)

### 8. `charset_normalizer-3.4.7/src/charset_normalizer/api.py` (PYTHON) -> Cumulative Risk: **513.06**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.40)
...
- **Heaviest Functions:** `from_bytes` (Many-Argument Workhorses, Impact: 650.5), `is_binary` (Many-Argument Workhorses, Impact: 16.5), `from_path` (Many-Argument Workhorses, Impact: 4.8)
...
- **Heaviest Functions:** `test_impl` (Compute Cores, Impact: 12.5), `git_clone` (Compute Cores, Impact: 7.7), `downstream_niquests` (Compute Cores, Impact: 7.2)

### 10. `charset_normalizer-3.4.7/tests/test_thread_safety.py` (PYTHON) -> Cumulative Risk: **369.98**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.64)
...
- **Heaviest Functions:** `_detect` (Generic / Templated Code, Impact: 13.3), `test_concurrent_detection` (Defensive Guards, Impact: 9.3), `test_concurrent_detection_repeated` (Generic / Templated Code, Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `feed_info` **(Many-Argument Workhorses)** (Impact: 57.7)
    * *Intent:* """Optimized feed using pre-computed character info."""
  * `update` **(Many-Argument Workhorses)** (Impact: 38.3)
    * *Intent:* """Update all properties for *character* (called once per character)."""
  * `mess_ratio` **(Many-Argument Workhorses)** (Impact: 38.3)
  * `feed_info` **(Many-Argument Workhorses)** (Impact: 36.2)
    * *Intent:* """Optimized feed using pre-computed character info."""
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `characters_popularity_compare` **(Compute Cores)** (Impact: 50.0)
  ...
  * `alphabet_languages` **(Compute Cores)** (Impact: 17.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_character_flags` **(Defensive Guards)** (Impact: 17.2)
...
**Top Internal Functions/Classes:**
  * `append` **(Defensive Guards)** (Impact: 18.3)
  ...
  * `output` **(Compute Cores)** (Impact: 10.1)
...
**Top Internal Functions/Classes:**
  ...
  * `downstream_niquests` **(Compute Cores)** (Impact: 7.2)
  * `downstream_requests` **(Compute Cores)** (Impact: 7.1)
  * `coverage` **(I/O & Config Routines)** (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_with_minimal_and_alt` **(Interface Declarations)** (Impact: 2.1)
  * `test_single_file_normalize` **(Defensive Guards)** (Impact: 2.0)
  * `test_multiple_file` **(Interface Declarations)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_mb_cutting_chk` **(Interface Declarations)** (Impact: 2.7)
...
**Top Internal Functions/Classes:**
  * `_detect` **(Generic / Templated Code)** (Impact: 13.3)
...
**Top Internal Functions/Classes:**
  ...
  * `setup_method` **(Interface Declarations)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `test_small_payload_confidence_altered` **(Interface Declarations)** (Impact: 2.2)
  * `test_detect_dict_keys` **(Interface Declarations)** (Impact: 2.0)
  * `test_detect_dict_value_type` **(Interface Declarations)** (Impact: 2.0)
  * `test_detect_dict_value` **(Interface Declarations)** (Impact: 1.7)
  * `test_utf8_sig_not_striped` **(Interface Declarations)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `test_issue_gh520` **(Interface Declarations)** (Impact: 1.5)
    * *Intent:* """Verify that minorities does not strip basic latin characters!"""
  * `test_issue_gh509` **(Interface Declarations)** (Impact: 1.5)
    * *Intent:* """Two common ASCII punctuations should render as-is."""
  * `test_issue_gh498` **(Interface Declarations)** (Impact: 1.5)
    * *Intent:* """This case was mistaken for utf-16-le, this should never happen again."""
  * `test_unicode_edge_case` **(Interface Declarations)** (Impact: 1.4)
**Structural Signatures (Net Mitigated Signals):**
