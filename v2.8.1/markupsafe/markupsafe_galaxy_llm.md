...
> **File Composition:** Data / Markup / Trivial 33%, Generic / Templated Code Files 33%, Defensive Guards Files 13%, Compute Cores Files 7%, I/O & Config Routines Files 7%
...
- `escape_unicode` **(Defensive Guards)** (@ `markupsafe-3.0.3/src/markupsafe/_speedups.c`) -> Impact: **13.2** | LOC: 21
...
- `escape_unicode_kind1` **(Compute Cores)** (@ `markupsafe-3.0.3/src/markupsafe/_speedups.c`) -> Impact: **6.9** | LOC: 25
- `__getattr__` **(Compute Cores)** (@ `markupsafe-3.0.3/src/markupsafe/__init__.py`) -> Impact: **6.4** | LOC: 15
...
- **Heaviest Functions:** `__mod__` (Defensive Guards, Impact: 11.0), `format_field` (Defensive Guards, Impact: 10.8), `striptags` (Compute Cores, Impact: 8.6)

### 2. `markupsafe-3.0.3/src/markupsafe/_speedups.c` (C) -> Cumulative Risk: **538.26**
...
- **Heaviest Functions:** `escape_unicode` (Defensive Guards, Impact: 13.2), `escape_unicode_kind1` (Compute Cores, Impact: 6.9), `escape_unicode_kind2` (Compute Cores, Impact: 5.4)

### 3. `markupsafe-3.0.3/setup.py` (PYTHON) -> Cumulative Risk: **416.38**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +2.08)
...
- **Heaviest Functions:** `run_setup` (Interface Declarations, Impact: 4.5), `build_extension` (Defensive Guards, Impact: 4.0), `show_message` (Interface Declarations, Impact: 3.1)
...
- **Heaviest Functions:** `test_markup_leaks` (I/O & Config Routines, Impact: 3.9)
...
- **Heaviest Functions:** `__html_format__` (Generic / Templated Code, Impact: 5.6), `test_complex_custom_formatting` (Interface Declarations, Impact: 4.2), `test_string_interpolation` (Generic / Templated Code, Impact: 2.1)
...
- **Heaviest Functions:** `_escape_inner` (Generic / Templated Code, Impact: 1.5)
...
- **Heaviest Functions:** `_escape_inner` (Generic / Templated Code, Impact: 1.8)
...
- **Heaviest Functions:** `test_escape` (Generic / Templated Code, Impact: 1.8), `__init__` (Generic / Templated Code, Impact: 1.8), `__class__` (Generic / Templated Code, Impact: 1.6)

### 9. `markupsafe-3.0.3/tests/conftest.py` (PYTHON) -> Cumulative Risk: **243.23**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.39)
...
- **Heaviest Functions:** `pytest_report_header` (Generic / Templated Code, Impact: 2.3), `_mod` (Type Conversions, Impact: 1.6)

### 10. `markupsafe-3.0.3/tests/test_exception_custom_html.py` (PYTHON) -> Cumulative Risk: **242.77**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.57)
- **Magnitude:** 6.3 | **LOC:** 24 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Safety Score (53.8495%), Stability (50.0%), Documentation (33.3333%)
- **Heaviest Functions:** `test_exception_custom_html` (Tests & Verification, Impact: 1.6), `__html__` (Generic / Templated Code, Impact: 1.5)
...
**Top Internal Functions/Classes:**
  ...
  * `__getattr__` **(Compute Cores)** (Impact: 6.4)
...
- **Magnitude:** 177.64 | **LOC:** 201 | **CtrlFlow:** 15.8% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (70.2401%), Tech Debt (16.0766%)
**Top Internal Functions/Classes:**
  * `escape_unicode` **(Defensive Guards)** (Impact: 13.2)
  * `escape_unicode_kind1` **(Compute Cores)** (Impact: 6.9)
  * `escape_unicode_kind2` **(Compute Cores)** (Impact: 5.4)
  * `escape_unicode_kind4` **(Compute Cores)** (Impact: 5.4)
...
**Top Internal Functions/Classes:**
  ...
  * `test_complex_custom_formatting` **(Interface Declarations)** (Impact: 4.2)
...
**Top Internal Functions/Classes:**
  * `run_setup` **(Interface Declarations)** (Impact: 4.5)
  * `build_extension` **(Defensive Guards)** (Impact: 4.0)
  * `show_message` **(Interface Declarations)** (Impact: 3.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_mod` **(Type Conversions)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_exception_custom_html` **(Tests & Verification)** (Impact: 1.6)
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_escape_inner` **(Generic / Templated Code)** (Impact: 1.8)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_escape_inner` **(Generic / Templated Code)** (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
