...
> **Composition Archetype:** `Small Flat Repo` (z +1.10; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 49%, Declarative / Non-Code 18%, Interface Declarations Files 9%, Callbacks & Closures Files 8%, I/O & Config Routines Files 5%
...
- **Heaviest Functions:** `handleUrl` (Callbacks & Closures, Impact: 8.6), `onEnd` (Callbacks & Closures, Impact: 5.2)

### 2. `src/utils/ResourceLoader/models/ImageLoader.ts` (TYPESCRIPT) -> Cumulative Risk: **591.9**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.39)
...
- **Heaviest Functions:** `handleUrl` (Callbacks & Closures, Impact: 6.1), `onEnd` (Callbacks & Closures, Impact: 3.8)

### 3. `src/ui/screens/Game/HUD/HealthBar/Face/Face.tsx` (TYPESCRIPT) -> Cumulative Risk: **567.27**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.55)
- **Magnitude:** 31.28 | **LOC:** 55 | **CtrlFlow:** 12.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `getFaceState` (Interface Declarations, Impact: 3.6), `startIteration` (Callbacks & Closures, Impact: 3.5), `render` (Interface Declarations, Impact: 1.2)
...
- **Heaviest Functions:** `applyBrightness` (Parameter Forwarders, Impact: 2.5), `createImageElement` (Callbacks & Closures, Impact: 2.1), `createImageCanvasContext` (Interface Declarations, Impact: 1.9)
...
- **Heaviest Functions:** `load` (Compute Cores, Impact: 8.6), `handler` (Callbacks & Closures, Impact: 6.7)

### 6. `src/ui/screens/Game/HUD/HealthColorFilter/HealthColorFilter.tsx` (TYPESCRIPT) -> Cumulative Risk: **452.33**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +0.61)
...
- **Heaviest Functions:** `render` (I/O & Config Routines, Impact: 6.9)

### 7. `src/State/Loader.ts` (TYPESCRIPT) -> Cumulative Risk: **425.76**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.57)
...
- **Heaviest Functions:** `loadResources` (Callbacks & Closures, Impact: 5.8), `onUpdate` (Callbacks & Closures, Impact: 4.8), `addLoadedItem` (State Mutators, Impact: 1.6)
...
- **Heaviest Functions:** `render` (Generic / Templated Code, Impact: 4.0), `startMenu` (Callbacks & Closures, Impact: 2.5), `onClick` (Callbacks & Closures, Impact: 1.4)

### 9. `src/levels/level_2/level.ts` (TYPESCRIPT) -> Cumulative Risk: **412.84**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.67)
...
- **Heaviest Functions:** `setLevel` (I/O & Config Routines, Impact: 7.4), `start` (State Mutators, Impact: 1.1)

### 10. `src/models/House/index.ts` (TYPESCRIPT) -> Cumulative Risk: **410.51**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z -0.41)
- **Magnitude:** 10.84 | **LOC:** 41 | **CtrlFlow:** 9.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (88.0797%), Safety Score (59.0077%)
- **Heaviest Functions:** `constructor` (State Mutators, Impact: 4.8), `start` (State Mutators, Impact: 1.4)
...
**Top Internal Functions/Classes:**
  ...
  * `createImageElement` **(Callbacks & Closures)** (Impact: 2.1)
  * `createImageCanvasContext` **(Interface Declarations)** (Impact: 1.9)
  * `getImageData` **(Generic / Templated Code)** (Impact: 1.8)
  * `getUrlFromImageData` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
**Top Internal Functions/Classes:**
  * `getFaceState` **(Interface Declarations)** (Impact: 3.6)
  * `startIteration` **(Callbacks & Closures)** (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  ...
  * `normalize` **(Interface Declarations)** (Impact: 1.7)
  * `normalize180` **(Interface Declarations)** (Impact: 1.7)
  * `toRad` **(Interface Declarations)** (Impact: 1.6)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `getExactPath` **(Interface Declarations)** (Impact: 1.6)
**Structural Signatures (Net Mitigated Signals):**
