...
> **Composition Archetype:** `Hub-Coupled App` (z +0.82; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 26%, Interface Declarations Files 20%, Generic / Templated Code Files 19%, Data / Markup / Trivial 18%, Many-Argument Workhorses Files 4%
...
- **Heaviest Functions:** `foreign_key_constraint` (Many-Argument Workhorses, Impact: 49.4), `table` (Many-Argument Workhorses, Impact: 33.2), `_ensure_table_for_fk` (Defensive Guards, Impact: 10.7)
...
- **Heaviest Functions:** `_exclude_constraint` (Many-Argument Workhorses, Impact: 33.0), `_cleanup_index_expr` (Many-Argument Workhorses, Impact: 31.6), `compare_indexes` (Many-Argument Workhorses, Impact: 30.9)
...
- **Heaviest Functions:** `add_constraint` (Compute Cores, Impact: 18.8), `requires_recreate_in_batch` (Defensive Guards, Impact: 16.9), `_guess_if_default_is_unparenthesized_sql_expr` (Compute Cores, Impact: 16.6)
...
- **Heaviest Functions:** `create_foreign_key` (Many-Argument Workhorses, Impact: 20.2), `from_constraint` (Compute Cores, Impact: 19.3), `batch_create_foreign_key` (Many-Argument Workhorses, Impact: 17.6)

### 5. `alembic-1.18.4/alembic/ddl/mssql.py` (PYTHON) -> Cumulative Risk: **649.37**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.91)
...
- **Heaviest Functions:** `alter_column` (Many-Argument Workhorses, Impact: 77.2), `_compare_identity_default` (Compute Cores, Impact: 12.8), `visit_column_comment` (Many-Argument Workhorses, Impact: 11.2)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 33.9), `__init__` (Generic / Templated Code, Impact: 5.0), `column_names_optional` (Generic / Templated Code, Impact: 3.0)

### 7. `alembic-1.18.4/alembic/autogenerate/compare/util.py` (PYTHON) -> Cumulative Risk: **644.67**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.56)
...
- **Heaviest Functions:** `_return_from_cache` (Many-Argument Workhorses, Impact: 38.9), `_pre_cache` (Many-Argument Workhorses, Impact: 31.2), `_apply_reflectinfo_conv` (Defensive Guards, Impact: 12.6)
...
- **Heaviest Functions:** `add_column` (Many-Argument Workhorses, Impact: 39.6), `__init__` (Many-Argument Workhorses, Impact: 10.0), `drop_column` (Generic / Templated Code, Impact: 7.1)

### 9. `alembic-1.18.4/alembic/ddl/impl.py` (PYTHON) -> Cumulative Risk: **639.27**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +0.71)
...
- **Heaviest Functions:** `alter_column` (Many-Argument Workhorses, Impact: 59.4), `_exec` (Many-Argument Workhorses, Impact: 41.4), `bulk_insert` (Many-Argument Workhorses, Impact: 33.2)
...
- **Heaviest Functions:** `configure` (Many-Argument Workhorses, Impact: 29.7), `run_migrations` (Many-Argument Workhorses, Impact: 27.8), `__init__` (Many-Argument Workhorses, Impact: 26.5)
...
**Top Internal Functions/Classes:**
  * `_repr_type` **(Many-Argument Workhorses)** (Impact: 40.1)
  * `_alter_column` **(Compute Cores)** (Impact: 32.5)
  * `_uq_constraint` **(Many-Argument Workhorses)** (Impact: 32.1)
  * `_add_table` **(Compute Cores)** (Impact: 30.5)
  * `_render_column` **(Compute Cores)** (Impact: 30.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_multi_fk_fixture` **(Many-Argument Workhorses)** (Impact: 7.9)
  * `_literal_ck_fixture` **(Many-Argument Workhorses)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `from_constraint` **(Compute Cores)** (Impact: 19.3)
...
**Top Internal Functions/Classes:**
  * `test_remove_connection_uq` **(Compute Cores)** (Impact: 18.9)
  * `test_remove_connection_index` **(Compute Cores)** (Impact: 18.5)
  * `include_object` **(Defensive Guards)** (Impact: 15.2)
  * `test_drop_table_w_uq_constraint` **(Compute Cores)** (Impact: 13.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_add_foreign_key_dialect_kw` **(Compute Cores)** (Impact: 7.7)
  * `test_alter_column_computed_not_supported` **(Many-Argument Workhorses)** (Impact: 6.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_make_foreign_key` **(Compute Cores)** (Impact: 22.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 26.5)
  * `begin_transaction` **(Many-Argument Workhorses)** (Impact: 25.7)
  * `autocommit_block` **(Compute Cores)** (Impact: 20.2)
...
**Top Internal Functions/Classes:**
  * `_assert_sql` **(Many-Argument Workhorses)** (Impact: 21.3)
  * `test_help_text` **(Defensive Guards)** (Impact: 16.0)
  * `_eq_cmd_output` **(Many-Argument Workhorses)** (Impact: 11.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_grab_table_elements` **(Defensive Guards)** (Impact: 24.9)
  * `_setup_referent` **(Many-Argument Workhorses)** (Impact: 21.8)
...
**Top Internal Functions/Classes:**
  ...
  * `_column_args_match` **(Many-Argument Workhorses)** (Impact: 19.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_partial_id_resolve_too_short` **(Interface Declarations)** (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  ...
  * `_cleanup_index_expr` **(Many-Argument Workhorses)** (Impact: 31.6)
...
**Top Internal Functions/Classes:**
  * `test_reordering_example_wo_copy` **(Compute Cores)** (Impact: 11.6)
  ...
  * `_test_tz` **(Many-Argument Workhorses)** (Impact: 8.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_assert_downgrade` **(Many-Argument Workhorses)** (Impact: 5.3)
  * `_assert_upgrade` **(Many-Argument Workhorses)** (Impact: 5.3)
...
**Top Internal Functions/Classes:**
  * `_inis_from_config` **(Compute Cores)** (Impact: 27.9)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 20.3)
  * `_inspect_function` **(Compute Cores)** (Impact: 18.8)
...
**Top Internal Functions/Classes:**
  ...
  * `history` **(Many-Argument Workhorses)** (Impact: 41.4)
...
**Top Internal Functions/Classes:**
  * `_setup_revision_files` **(Many-Argument Workhorses)** (Impact: 19.0)
  ...
  * `_setup_for_fixture` **(Compute Cores)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_mysql_colspec` **(Many-Argument Workhorses)** (Impact: 19.6)
**Contextual Mitigations & Amplifications:**
