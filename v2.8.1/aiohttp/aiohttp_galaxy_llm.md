...
> **Composition Archetype:** `Typed Library` (z -0.11; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 34%, Defensive Guards Files 14%, Data / Markup / Trivial 13%, Large Core Modules 12%, Tests & Verification Files 8%
...
- `__init__` **(Many-Argument Workhorses)** (@ `aiohttp-3.13.5/aiohttp/client.py`) -> Impact: **217.9** | LOC: 159
...
- `feed_data` **(Many-Argument Workhorses)** (@ `aiohttp-3.13.5/aiohttp/http_parser.py`) -> Impact: **94.6** | LOC: 148
...
- **Composition Archetype:** `Generic / Templated Code Files` (z -0.32)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 21.4), `__init__` (Defensive Guards, Impact: 6.8), `__init__` (Generic / Templated Code, Impact: 6.5)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 17.0), `__init__` (Many-Argument Workhorses, Impact: 13.8), `__init__` (Many-Argument Workhorses, Impact: 8.2)

### 3. `aiohttp-3.13.5/aiohttp/web_response.py` (PYTHON) -> Cumulative Risk: **770.03**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.22)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 92.7), `set_cookie` (Many-Argument Workhorses, Impact: 45.9), `_prepare_headers` (Compute Cores, Impact: 38.6)
...
- **Heaviest Functions:** `readuntil` (Many-Argument Workhorses, Impact: 27.9), `read` (Compute Cores, Impact: 22.8), `_read_nowait_chunk` (Compute Cores, Impact: 20.8)
...
- **Heaviest Functions:** `__init__` (Many-Argument Workhorses, Impact: 108.9), `send` (Many-Argument Workhorses, Impact: 51.7), `_merge_ssl_params` (Many-Argument Workhorses, Impact: 40.7)
...
- **Heaviest Functions:** `_handle` (Compute Cores, Impact: 24.7), `__init__` (Many-Argument Workhorses, Impact: 17.8), `_set_loop` (Generic / Templated Code, Impact: 11.2)
...
- **Heaviest Functions:** `data_received` (Compute Cores, Impact: 38.1), `connection_lost` (Many-Argument Workhorses, Impact: 26.2), `should_close` (Compute Cores, Impact: 11.8)
...
- **Heaviest Functions:** `_handle_frame` (Many-Argument Workhorses, Impact: 90.3), `_feed_data` (Many-Argument Workhorses, Impact: 77.0), `_read_from_buffer` (Compute Cores, Impact: 7.6)
...
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.10)
- **Magnitude:** 177.04 | **LOC:** 122 | **CtrlFlow:** 25.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `normalize_path_middleware` (Many-Argument Workhorses, Impact: 41.5), `impl` (Compute Cores, Impact: 27.4), `_check_request_resolves` (Generic / Templated Code, Impact: 4.0)
...
**Top Internal Functions/Classes:**
  ...
  * `create_connection` **(Compute Cores)** (Impact: 24.6)
...
**Top Internal Functions/Classes:**
  ...
  * `test_request_tracing` **(Defensive Guards)** (Impact: 7.7)
...
**Top Internal Functions/Classes:**
  ...
  * `test_read_eof_unread_data_no_warning` **(Tests & Verification)** (Impact: 5.2)
...
**Top Internal Functions/Classes:**
  ...
  * `test_write_large_payload_deflate_compression_chunked_data_in_eof` **(Many-Argument Workhorses)** (Impact: 11.2)
  * `test_write_large_payload_deflate_compression_chunked_data_in_eof_all_zlib` **(Many-Argument Workhorses)** (Impact: 11.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_read_chunk_by_length_doesnt_breaks_reader` **(Compute Cores)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 192 instances
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 217.9)
  * `_ws_connect` **(Many-Argument Workhorses)** (Impact: 157.8)
  * `_prepare_headers` **(Defensive Guards)** (Impact: 11.1)
...
**Top Internal Functions/Classes:**
  ...
  * `_close` **(Compute Cores)** (Impact: 41.2)
...
**Top Internal Functions/Classes:**
  ...
  * `_strip_server` **(Defensive Guards)** (Impact: 3.7)
  * `test_last_modified_invalid_type` **(Tests & Verification)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `update_body_from_data` **(Defensive Guards)** (Impact: 34.0)
    * *Intent:* """Update request body from data."""
  * `__init__` **(Many-Argument Workhorses)** (Impact: 31.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_textio_payload_reads_in_chunks` **(Tests & Verification)** (Impact: 8.1)
    * *Intent:* """Test TextIOPayload reads data in chunks of READ_SIZE, not all at once."""
  * `test_iobase_payload_reads_in_chunks` **(Tests & Verification)** (Impact: 8.0)
    * *Intent:* """Test IOBasePayload reads data in chunks of READ_SIZE, not all at once."""
  * `mock_read` **(Compute Cores)** (Impact: 7.7)
  * `mock_read` **(Compute Cores)** (Impact: 7.7)
  * `test_iobase_payload_large_content_length` **(Interface Declarations)** (Impact: 5.5)
...
**Top Internal Functions/Classes:**
  ...
  * `read_chunk` **(Compute Cores)** (Impact: 28.1)
...
**Top Internal Functions/Classes:**
  ...
  * `test_register_uncommon_http_methods` **(Tests & Verification)** (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 39.4)
  * `__init__` **(Defensive Guards)** (Impact: 29.6)
