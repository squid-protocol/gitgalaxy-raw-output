...
| Avg Path Length | 1.3032 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Flat Modular Platform` (z +0.11; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 48%, Large Core Modules 24%, Interface Declarations Files 6%, I/O & Config Routines Files 6%, Data / Markup / Trivial 5%
...
- **Heaviest Functions:** `NO_CARRY5` (Many-Argument Workhorses, Impact: 14.5), `MATCH_SEARCH` (Many-Argument Workhorses, Impact: 8.8), `FOUND_DO` (Compute Cores, Impact: 7.1)
...
- **Heaviest Functions:** `DST_SLTLP` (Many-Argument Workhorses, Impact: 13.0), `DST_BUFNXT` (Many-Argument Workhorses, Impact: 10.2), `DST_BUFLP` (Many-Argument Workhorses, Impact: 9.6)
...
- **Heaviest Functions:** `chk_al5` (Parameter Forwarders, Impact: 5.8), `not_stack` (Parameter Forwarders, Impact: 5.1), `chk_al6` (Parameter Forwarders, Impact: 4.9)
...
- **Heaviest Functions:** `compare` (Many-Argument Workhorses, Impact: 28.9), `strcmpis` (Compute Cores, Impact: 14.7), `strcmpi` (Compute Cores, Impact: 9.4)
...
- **Heaviest Functions:** `ALT_Ext_Chk` (Compute Cores, Impact: 6.8), `NOCHR` (Compute Cores, Impact: 6.3), `RD_Ext_Chk` (Compute Cores, Impact: 6.1)

### 6. `v4.0/src/MEMM/MEMM/MAPDMA.C` (C) -> Cumulative Risk: **582.69**
- **Archetype:** `file_cluster_16` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.37)
...
- **Heaviest Functions:** `SwapDMAPages` (Many-Argument Workhorses, Impact: 74.8), `GetPteFromIndex` (Compute Cores, Impact: 8.0)
...
- **Heaviest Functions:** `FRAGCHK` (Many-Argument Workhorses, Impact: 9.7), `GotPath` (Type Conversions, Impact: 7.5), `RDOK` (Many-Argument Workhorses, Impact: 7.2)
...
- **Heaviest Functions:** `loopx` (Parameter Forwarders, Impact: 4.4), `Copy_Vector` (Parameter Forwarders, Impact: 1.9), `Exit` (Interface Declarations, Impact: 1.2)
...
- **Heaviest Functions:** `rootpath` (Many-Argument Workhorses, Impact: 35.1), `fPathErr` (Compute Cores, Impact: 23.4), `strbscan` (Compute Cores, Impact: 3.9)
...
- **Heaviest Functions:** `SRCH` (Many-Argument Workhorses, Impact: 10.7), `len_ok` (Many-Argument Workhorses, Impact: 9.9), `EWriteOK` (Parameter Forwarders, Impact: 9.6)
...
**Top Internal Functions/Classes:**
  ...
  * `GRP3` **(Many-Argument Workhorses)** (Impact: 21.5)
...
**Top Internal Functions/Classes:**
  ...
  * `SMALREC` **(Many-Argument Workhorses)** (Impact: 12.8)
...
**Top Internal Functions/Classes:**
  ...
  * `GET_AVAIL` **(Many-Argument Workhorses)** (Impact: 13.5)
...
**Top Internal Functions/Classes:**
  ...
  * `dodir` **(Many-Argument Workhorses)** (Impact: 38.3)
    * *Intent:* /**/
  * `doadd` **(C Struct Operations)** (Impact: 23.8)
    * *Intent:* /**/
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `GET_AVAIL` **(Many-Argument Workhorses)** (Impact: 16.8)
    * *Intent:* ; ; Allocate drive memory ;
  * `CHECK_SYS` **(Compute Cores)** (Impact: 15.1)
...
**Top Internal Functions/Classes:**
  * `LF160` **(Many-Argument Workhorses)** (Impact: 12.2)
    * *Intent:* ; buffer ; ; Completed updating LVB, adjust pointers
  * `LS160` **(Many-Argument Workhorses)** (Impact: 12.2)
...
**Top Internal Functions/Classes:**
  ...
  * `abort_exit` **(Many-Argument Workhorses)** (Impact: 11.1)
