...
> **Composition Archetype:** `Small Flat Repo` (z -0.69; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 69%, Declarative / Non-Code 10%, Large Core Modules 10%, Compute Cores Files 5%, Defensive Guards Files 2%
...
- `_repr_pen_commands` **(Compute Cores)** (@ `fonttools-4.62.1/Tests/pens/utils.py`) -> Impact: **28.0** | LOC: 50
...
- **Heaviest Functions:** `visitLeaf` (Defensive Guards, Impact: 14.8), `visit` (Defensive Guards, Impact: 7.7), `visitList` (Many-Argument Workhorses, Impact: 7.3)
...
- **Heaviest Functions:** `write_checksum` (Many-Argument Workhorses, Impact: 51.0), `check_checksum` (Compute Cores, Impact: 17.8), `_read_binary` (Interface Declarations, Impact: 1.6)

### 3. `fonttools-4.62.1/Snippets/rename-fonts.py` (PYTHON) -> Cumulative Risk: **592.13**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.36)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 23.7), `add_family_suffix` (Compute Cores, Impact: 15.2), `get_current_family_name` (Compute Cores, Impact: 9.5)
...
- **Heaviest Functions:** `bumpversion` (Many-Argument Workhorses, Impact: 32.9), `git_tag` (Many-Argument Workhorses, Impact: 16.6), `format_changelog` (Compute Cores, Impact: 11.6)
...
- **Heaviest Functions:** `ProcessTable` (Compute Cores, Impact: 15.5), `ProcessFont` (Compute Cores, Impact: 7.8), `ProcessFiles` (Compute Cores, Impact: 6.2)
...
- **Heaviest Functions:** `find_fonts` (Many-Argument Workhorses, Impact: 29.8), `main` (Compute Cores, Impact: 27.8), `build_otf` (Many-Argument Workhorses, Impact: 16.9)

### 7. `fonttools-4.62.1/Snippets/otf2ttf.py` (PYTHON) -> Cumulative Risk: **511.23**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.23)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 13.4), `otf_to_ttf` (Defensive Guards, Impact: 10.1), `update_hmtx` (Defensive Guards, Impact: 5.4)
...
- **Heaviest Functions:** `main` (Compute Cores, Impact: 11.6)
...
- **Heaviest Functions:** `roundTrip` (Many-Argument Workhorses, Impact: 20.1), `main` (Defensive Guards, Impact: 7.1), `usage` (Interface Declarations, Impact: 1.1)
...
- **Heaviest Functions:** `AddGlyphVariations` (Many-Argument Workhorses, Impact: 19.4), `GetCoordinates` (Compute Cores, Impact: 11.8), `AddFontVariations` (Defensive Guards, Impact: 3.9)
...
**Top Internal Functions/Classes:**
  ...
  * `_writePointPenCommand` **(Compute Cores)** (Impact: 14.0)
  * `_dictToString` **(Defensive Guards)** (Impact: 13.7)
  * `_listToString` **(Defensive Guards)** (Impact: 12.2)
  * `_tupleToString` **(Defensive Guards)** (Impact: 12.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_repr_pen_commands` **(Compute Cores)** (Impact: 28.0)
...
**Top Internal Functions/Classes:**
  ...
  * `_read_binary` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `main` **(Compute Cores)** (Impact: 14.7)
