...
> **Composition Archetype:** `Hub-Coupled App` (z +2.11; from the repo's file-archetype mix)
> **File Composition:** Interface Declarations Files 26%, Annotated Framework Methods Files 19%, Tests & Verification Files 18%, Large Core Modules 17%, Data / Markup / Trivial 6%
...
- `refine` **(Compute Cores)** (@ `com.sap.adt.abapcleaner/src/com/sap/adt/abapcleaner/parser/TokenTypeRefinerRnd.java`) -> Impact: **180.2** | LOC: 125
...
- `create` **(Compute Cores)** (@ `com.sap.adt.abapcleaner/src/com/sap/adt/abapcleaner/programbase/CommandLineArgs.java`) -> Impact: **178.4** | LOC: 208
...
- **Heaviest Functions:** `executeOn` (Many-Argument Workhorses, Impact: 31.2), `alignSection` (Many-Argument Workhorses, Impact: 10.3), `getExample` (Callbacks & Closures, Impact: 3.4)
...
- **Composition Archetype:** `Interface Declarations Files` (z +0.97)
...
- **Heaviest Functions:** `startAnnotation` (Many-Argument Workhorses, Impact: 25.4), `addToken` (Many-Argument Workhorses, Impact: 9.8), `addComma` (Compute Cores, Impact: 7.4)
...
- **Heaviest Functions:** `processKeyPressedEvent` (Compute Cores, Impact: 179.5), `getLineBackground` (Compute Cores, Impact: 106.9), `paintCode` (Many-Argument Workhorses, Impact: 102.0)
...
- **Heaviest Functions:** `CleanException` (Compute Cores, Impact: 22.4), `CleanException` (Compute Cores, Impact: 11.5), `getMessage` (Annotated Framework Methods, Impact: 6.5)
...
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z +1.05)
...
- **Heaviest Functions:** `executeOn` (Many-Argument Workhorses, Impact: 94.2), `getExample` (I/O & Config Routines, Impact: 6.9), `getDescription` (Annotated Framework Methods, Impact: 3.2)
...
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.42)
...
- **Heaviest Functions:** `executeOn` (Many-Argument Workhorses, Impact: 81.5), `executeOn` (Many-Argument Workhorses, Impact: 44.5), `getExample` (I/O & Config Routines, Impact: 19.0)
...
- **Heaviest Functions:** `executeOn` (Compute Cores, Impact: 66.1), `createSeparator` (Compute Cores, Impact: 21.5), `getExample` (I/O & Config Routines, Impact: 3.5)
...
- **Heaviest Functions:** `setWhitespace` (Many-Argument Workhorses, Impact: 36.2), `createSpecial` (Parameter Forwarders, Impact: 4.3), `getMonoLineWidth` (Annotated Framework Methods, Impact: 3.2)
...
- **Composition Archetype:** `Annotated Framework Methods Files` (z +0.88)
...
- **Heaviest Functions:** `executeOn` (Many-Argument Workhorses, Impact: 44.8), `condenseParameterLists` (Compute Cores, Impact: 13.5), `setForceLineBreakAndIndent` (Compute Cores, Impact: 12.8)
...
- **Composition Archetype:** `Interface Declarations Files` (z +0.94)
...
- **Heaviest Functions:** `search` (Many-Argument Workhorses, Impact: 99.4), `reprocessSelection` (Many-Argument Workhorses, Impact: 65.9), `isLineBitHighlighted` (Compute Cores, Impact: 17.7)
...
**Top Internal Functions/Classes:**
  * `canAddToDdl` **(Compute Cores)** (Impact: 122.5)
  * `changesSySubrc` **(Compute Cores)** (Impact: 109.8)
...
**Top Internal Functions/Classes:**
  * `createContents` **(Compute Cores)** (Impact: 87.0)
    * *Intent:* /** * Create contents of the window. */
  * `importProfiles` **(Compute Cores)** (Impact: 45.2)
...
**Top Internal Functions/Classes:**
  ...
  * `getWordFrequencies` **(Compute Cores)** (Impact: 113.7)
...
**Top Internal Functions/Classes:**
  ...
  * `getLineBackground` **(Compute Cores)** (Impact: 106.9)
...
**Top Internal Functions/Classes:**
  ...
  * `determineKeepOnSingleLine` **(Many-Argument Workhorses)** (Impact: 37.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `negate` **(Compute Cores)** (Impact: 94.1)
...
**Top Internal Functions/Classes:**
  ...
  * `assertMemoryAccessType` **(Compute Cores)** (Impact: 23.0)
  * `testAccessTypeTableExpression` **(Tests & Verification)** (Impact: 17.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getTypeName` **(Many-Argument Workhorses)** (Impact: 20.6)
  * `clearMaps` **(Compute Cores)** (Impact: 15.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getResult` **(Compute Cores)** (Impact: 53.2)
...
**Top Internal Functions/Classes:**
  ...
  * `changeLineBreaksAfterColumn` **(Many-Argument Workhorses)** (Impact: 30.6)
...
**Top Internal Functions/Classes:**
  * `testBoundStructuresWith2KeywordsUnused` **(Compute Cores)** (Impact: 22.1)
...
**Top Internal Functions/Classes:**
  ...
  * `variableMatchesFilter` **(Compute Cores)** (Impact: 30.6)
...
**Top Internal Functions/Classes:**
  ...
  * `determineLayout` **(Many-Argument Workhorses)** (Impact: 23.5)
  * `executeOn` **(Defensive Guards)** (Impact: 16.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 69 instances
...
**Top Internal Functions/Classes:**
  ...
  * `getTotalMultiLineWidth` **(Interface Declarations)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
