...
> **Composition Archetype:** `Flat Modular Platform` (z +2.34; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 52%, Declarative / Non-Code 21%, State Mutators Files 11%, Interface Declarations Files 9%, Callbacks & Closures Files 3%
...
- `createSyntacticTypeNodeBuilder` **(Compute Cores)** (@ `src/compiler/expressionToTypeNode.ts`) -> Impact: **654.3** | LOC: 927
  * *Intent:* /** @internal */
...
- **Heaviest Functions:** `start` (Compute Cores, Impact: 205.2), `startDelayed` (Many-Argument Workhorses, Impact: 155.0), `killChild` (Compute Cores, Impact: 74.4)
...
- **Heaviest Functions:** `dedent` (Compute Cores, Impact: 19.2), `getByteOrderMarkLength` (Compute Cores, Impact: 18.9), `guessIndentation` (Compute Cores, Impact: 12.0)

### 3. `src/harness/fakesHosts.ts` (TYPESCRIPT) -> Cumulative Risk: **692.37**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.67)
...
- **Heaviest Functions:** `getSourceFile` (Compute Cores, Impact: 32.9), `readDirectory` (Compute Cores, Impact: 12.4), `readDirectory` (Compute Cores, Impact: 12.4)
...
- **Heaviest Functions:** `tryGetConfig` (Compute Cores, Impact: 96.5), `runTests` (Compute Cores, Impact: 16.8), `startTestEnvironment` (Interface Declarations, Impact: 6.7)
...
- **Heaviest Functions:** `createProgram` (Many-Argument Workhorses, Impact: 672.3), `verifyCompilerOptions` (Compute Cores, Impact: 186.8), `getJSSyntacticDiagnosticsForFile` (Compute Cores, Impact: 163.9)
...
- **Heaviest Functions:** `bindWorker` (Compute Cores, Impact: 183.6), `declareSymbol` (Many-Argument Workhorses, Impact: 139.8), `bindChildren` (Compute Cores, Impact: 78.0)
...
- **Heaviest Functions:** `startNodeSession` (Many-Argument Workhorses, Impact: 139.9), `initializeNodeSystem` (Compute Cores, Impact: 73.5), `watchDirectory` (Many-Argument Workhorses, Impact: 40.4)
...
- **Heaviest Functions:** `preProcessFile` (Many-Argument Workhorses, Impact: 259.8), `tryConsumeImport` (Compute Cores, Impact: 43.2), `tryConsumeExport` (Compute Cores, Impact: 27.6)
...
- **Heaviest Functions:** `sanitizeTestFilePath` (Compute Cores, Impact: 206.7), `doTypeAndSymbolBaseline` (Many-Argument Workhorses, Impact: 174.4), `doJsEmitBaseline` (Many-Argument Workhorses, Impact: 79.5)
...
- **Heaviest Functions:** `sanitizeLog` (Compute Cores, Impact: 60.8), `handleLoggerGroup` (Callbacks & Closures, Impact: 11.7), `replaceAll` (Many-Argument Workhorses, Impact: 11.1)
...
**Top Internal Functions/Classes:**
  ...
  * `parseTagComments` **(Compute Cores)** (Impact: 71.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000119
...
**Top Internal Functions/Classes:**
  ...
  * `scan` **(Compute Cores)** (Impact: 207.9)
...
**Top Internal Functions/Classes:**
  ...
  * `verifyCompilerOptions` **(Compute Cores)** (Impact: 186.8)
...
**Top Internal Functions/Classes:**
  ...
  * `verifyCompletionsWorker` **(Compute Cores)** (Impact: 98.3)
...
**Top Internal Functions/Classes:**
  ...
  * `bindChildren` **(Compute Cores)** (Impact: 78.0)
...
**Top Internal Functions/Classes:**
  ...
  * `computeNamedDeclarations` **(Compute Cores)** (Impact: 62.4)
  * `synchronizeHostDataWorker` **(Compute Cores)** (Impact: 56.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `makeUnitsFromTest` **(Compute Cores)** (Impact: 39.4)
  ...
  * `makeUnitsFromTest` **(Compute Cores)** (Impact: 39.1)
    * *Intent:* /** Given a test file containing // @Filename directives, return an array of named units of code to ...
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `foo` **(Many-Argument Workhorses)** (Impact: 10.6)
...
- `src/compiler/transformers/module/module.ts` -> **Severity: 0.113** (Embedded: 0.0016 * Error Risk: 68.3464%)
- `src/compiler/path.ts` -> **Severity: 0.077** (Embedded: 0.0009 * Error Risk: 83.111%)
- `src/compiler/core.ts` -> **Severity: 0.053** (Embedded: 0.0006 * Error Risk: 88.7926%)
- `scripts/eslint/rules/utils.cjs` -> **Severity: 0.052** (Embedded: 0.0008 * Error Risk: 64.1725%)
- `src/compiler/transformers/jsx.ts` -> **Severity: 0.028** (Embedded: 0.0004 * Error Risk: 68.1511%)
