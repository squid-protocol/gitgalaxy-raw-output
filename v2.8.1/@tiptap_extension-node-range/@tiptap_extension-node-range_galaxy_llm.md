...
> **File Composition:** Data / Markup / Trivial 44%, Interface Declarations Files 22%, Callbacks & Closures Files 11%, Large Core Modules 11%, Many-Argument Workhorses Files 11%
...
- `extendBackwards` **(Interface Declarations)** (@ `package/src/helpers/NodeRangeSelection.ts`) -> Impact: **3.8** | LOC: 16
...
- **Heaviest Functions:** `mousedown` (Compute Cores, Impact: 30.2), `addProseMirrorPlugins` (I/O & Config Routines, Impact: 27.6), `decorations` (Compute Cores, Impact: 10.5)
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 25.7), `create` (Parameter Forwarders, Impact: 5.0), `eq` (Compute Cores, Impact: 4.4)
...
- **Heaviest Functions:** `getSelectionRanges` (Many-Argument Workhorses, Impact: 21.6)
...
- **Heaviest Functions:** `getNodeRangeDecorations` (Callbacks & Closures, Impact: 5.5)
...
- **Heaviest Functions:** `constructor` (State Mutators, Impact: 1.9), `resolve` (Interface Declarations, Impact: 1.7), `map` (Interface Declarations, Impact: 1.6)
...
- **Heaviest Functions:** `isNodeRangeSelection` (Defensive Guards, Impact: 1.6)
...
**Top Internal Functions/Classes:**
  ...
  * `extendBackwards` **(Interface Declarations)** (Impact: 3.8)
  * `extendForwards` **(Interface Declarations)** (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `resolve` **(Interface Declarations)** (Impact: 1.7)
  * `map` **(Interface Declarations)** (Impact: 1.6)
**Structural Signatures (Net Mitigated Signals):**
