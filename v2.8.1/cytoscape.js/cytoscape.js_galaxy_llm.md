...
> **Composition Archetype:** `Flat Modular Platform` (z +1.99; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 44%, Callbacks & Closures Files 22%, Declarative / Non-Code 18%, Large Core Modules 8%, Defensive Guards Files 3%
...
- `load` **(Compute Cores)** (@ `src/extensions/renderer/base/load-listeners.mjs`) -> Impact: **525.7** | LOC: 1954
...
- **Heaviest Functions:** `applyParsedProperty` (Many-Argument Workhorses, Impact: 125.4), `updateStyleHints` (Compute Cores, Impact: 46.9), `updateTransitions` (Compute Cores, Impact: 44.4)
...
- **Heaviest Functions:** `toggleClass` (Compute Cores, Impact: 28.2), `classes` (Compute Cores, Impact: 19.8), `flashClass` (Callbacks & Closures, Impact: 7.7)
...
- **Heaviest Functions:** `load` (Compute Cores, Impact: 525.7), `makeEvent` (Compute Cores, Impact: 169.9), `mousemoveHandler` (Compute Cores, Impact: 120.8)

### 4. `src/extensions/renderer/canvas/export-image.mjs` (JAVASCRIPT) -> Cumulative Risk: **680.54**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.97)
...
- **Heaviest Functions:** `bufferCanvasImage` (Compute Cores, Impact: 26.1), `output` (Defensive Guards, Impact: 19.5), `getB64Uri` (Callbacks & Closures, Impact: 10.6)

### 5. `src/animation.mjs` (JAVASCRIPT) -> Cumulative Risk: **679.08**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -0.52)
...
- **Heaviest Functions:** `Animation` (Many-Argument Workhorses, Impact: 18.4), `promise` (Callbacks & Closures, Impact: 9.5), `progress` (Defensive Guards, Impact: 8.1)
...
- **Heaviest Functions:** `roundRectangleIntersectLine` (Many-Argument Workhorses, Impact: 73.2), `finiteLinesIntersect` (Many-Argument Workhorses, Impact: 53.9), `polygonIntersectLine` (Many-Argument Workhorses, Impact: 36.1)
...
- **Heaviest Functions:** `applyBypass` (Many-Argument Workhorses, Impact: 58.2), `overrideBypass` (Many-Argument Workhorses, Impact: 25.7), `removeBypasses` (Many-Argument Workhorses, Impact: 13.7)
...
- **Primary Risk Drivers:** Api Exposure (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `uuid` (Compute Cores, Impact: 9.8), `copy` (Callbacks & Closures, Impact: 9.1), `removeFromArray` (Defensive Guards, Impact: 8.6)
...
- **Heaviest Functions:** `draw` (Many-Argument Workhorses, Impact: 34.2), `getOrCreateAtlas` (Many-Argument Workhorses, Impact: 17.3), `gc` (I/O & Config Routines, Impact: 11.6)
...
- **Heaviest Functions:** `render` (Compute Cores, Impact: 171.7), `createGradientStyleFor` (Many-Argument Workhorses, Impact: 64.9), `drawSelectionRectangle` (Compute Cores, Impact: 32.3)
...
**Top Internal Functions/Classes:**
  * `load` **(Compute Cores)** (Impact: 525.7)
...
**Top Internal Functions/Classes:**
  ...
  * `updateCompoundBounds` **(Compute Cores)** (Impact: 67.9)
  * `update` **(Compute Cores)** (Impact: 55.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `tryToCorrectInvalidPoints` **(Compute Cores)** (Impact: 34.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `drawOutline` **(Compute Cores)** (Impact: 68.3)
...
**Top Internal Functions/Classes:**
  ...
  * `updateStyleHints` **(Compute Cores)** (Impact: 46.9)
  * `updateTransitions` **(Compute Cores)** (Impact: 44.4)
    * *Intent:* // diffProps : { name => { prev, next } }
  * `cleanNum` **(Compute Cores)** (Impact: 40.3)
...
**Top Internal Functions/Classes:**
  ...
  * `drawSelectionRectangle` **(Compute Cores)** (Impact: 32.3)
...
**Top Internal Functions/Classes:**
  ...
  * `remove` **(Compute Cores)** (Impact: 56.5)
...
**Top Internal Functions/Classes:**
  ...
  * `addEle` **(Compute Cores)** (Impact: 27.6)
...
**Top Internal Functions/Classes:**
  ...
  * `dequeue` **(Compute Cores)** (Impact: 18.8)
...
**Top Internal Functions/Classes:**
  * `kMeans` **(Compute Cores)** (Impact: 25.4)
  * `kMedoids` **(Compute Cores)** (Impact: 23.4)
...
**Top Internal Functions/Classes:**
  * `getLabelText` **(Compute Cores)** (Impact: 58.1)
...
**Top Internal Functions/Classes:**
  * `createDefaultTodos` **(Interface Declarations)** (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  ...
  * `eleTextBiggerThanMin` **(Callbacks & Closures)** (Impact: 6.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `draw` **(Many-Argument Workhorses)** (Impact: 10.9)
...
**Top Internal Functions/Classes:**
  ...
  * `assignClusters` **(Many-Argument Workhorses)** (Impact: 13.3)
  * `setOptions` **(Callbacks & Closures)** (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
