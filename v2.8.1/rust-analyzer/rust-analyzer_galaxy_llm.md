...
> **Composition Archetype:** `Flat Modular Platform` (z -0.40; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 29%, Large Core Modules 22%, Tests & Verification Files 17%, Generic / Templated Code Files 9%, State Mutators Files 6%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 14.8 | 4.6 | 0.0 |
...
| dead_code | 11005 | 927 | 21 | `crates/parser/test_data/generated/runner.rs` |
| credential | 0 | 0 | 0 | - |
...
### 1. `editors/code/src/commands.ts` (TYPESCRIPT) -> Cumulative Risk: **740.4**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +1.38)
...
### 2. `editors/code/src/client.ts` (TYPESCRIPT) -> Cumulative Risk: **726.4**
...
- **Heaviest Functions:** `createClient` (Many-Argument Workhorses, Impact: 146.7), `handleDiagnostics` (Many-Argument Workhorses, Impact: 65.0), `provideCodeActions` (Many-Argument Workhorses, Impact: 29.0)
...
- **Heaviest Functions:** `removeByFile` (Callbacks & Closures, Impact: 23.8), `addTest` (Defensive Guards, Impact: 20.6), `addTestGroup` (Compute Cores, Impact: 9.3)

### 4. `editors/code/src/config.ts` (TYPESCRIPT) -> Cumulative Risk: **695.36**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
...
- **Heaviest Functions:** `substituteVariablesInEnv` (Compute Cores, Impact: 47.7), `toggleCheckOnSave` (Compute Cores, Impact: 21.7), `runnablesExtraEnv` (Defensive Guards, Impact: 20.2)

### 5. `editors/code/src/ctx.ts` (TYPESCRIPT) -> Cumulative Risk: **694.42**
...
- **Heaviest Functions:** `prepareSyntaxTreeView` (Defensive Guards, Impact: 29.6), `updateStatusBarItem` (I/O & Config Routines, Impact: 17.3), `updateCommands` (Defensive Guards, Impact: 16.9)

### 6. `crates/parser/src/grammar/items.rs` (RUST) -> Cumulative Risk: **660.4**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
...
- **Heaviest Functions:** `opt_item` (Many-Argument Workhorses, Impact: 66.7), `opt_item_without_modifiers` (Compute Cores, Impact: 31.3), `item_or_macro` (Many-Argument Workhorses, Impact: 18.9)
...
### 8. `crates/hir-ty/src/next_solver/interner.rs` (RUST) -> Cumulative Risk: **656.78**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.55)
- **Magnitude:** 905.58 | **LOC:** 2709 | **CtrlFlow:** 7.0% | **Authorship Centralization:** 70.8%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (96.7262%), Api Exposure (93.1832%), Churn (91.3%)
- **Heaviest Functions:** `new` (Compute Cores, Impact: 25.1), `for_each_relevant_impl` (Many-Argument Workhorses, Impact: 22.9), `unsizing_params_for_adt` (Compute Cores, Impact: 19.5)

### 9. `editors/code/src/debug.ts` (TYPESCRIPT) -> Cumulative Risk: **651.14**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
...
- **Heaviest Functions:** `getDebugConfiguration` (Many-Argument Workhorses, Impact: 58.5), `getDebugConfig` (Many-Argument Workhorses, Impact: 15.7), `makeDebugConfig` (Compute Cores, Impact: 15.2)

### 10. `crates/ide-assists/src/handlers/toggle_async_sugar.rs` (RUST) -> Cumulative Risk: **647.51**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
...
- **Heaviest Functions:** `sugar_impl_future_into_async` (Compute Cores, Impact: 36.5), `desugar_async_into_impl_future` (Defensive Guards, Impact: 23.2), `unwrap_future_output` (Compute Cores, Impact: 7.5)
...
- **Magnitude:** 3351.76 | **LOC:** 7283 | **CtrlFlow:** 10.9% | **Authorship Centralization:** 48.6%
...
**Top Internal Functions/Classes:**
  ...
  * `fn_ptr_type` **(Many-Argument Workhorses)** (Impact: 22.7)
...
- **Magnitude:** 2014.86 | **LOC:** 3193 | **CtrlFlow:** 19.2% | **Authorship Centralization:** 50.0%
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 1669.5 | **LOC:** 6541 | **CtrlFlow:** 9.0% | **Authorship Centralization:** 41.2%
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 1639.02 | **LOC:** 2489 | **CtrlFlow:** 26.7% | **Authorship Centralization:** 60.0%
...
**Top Internal Functions/Classes:**
  ...
  * `handle_runnables` **(Compute Cores)** (Impact: 55.6)
  * `handle_will_rename_files` **(Compute Cores)** (Impact: 50.0)
  * `handle_code_action_resolve` **(Compute Cores)** (Impact: 40.3)
  * `handle_completion_resolve` **(Compute Cores)** (Impact: 39.5)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 1401.1 | **LOC:** 11407 | **CtrlFlow:** 1.6% | **Authorship Centralization:** 57.1%
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 1232.06 | **LOC:** 3697 | **CtrlFlow:** 19.3% | **Authorship Centralization:** 86.7%
- **Risk Profile:** Cognitive Load (8.3173%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `else_completion_after_if` **(Compute Cores)** (Impact: 370.9)
  * `inside_faulty_format_args_completions_work` **(Compute Cores)** (Impact: 184.9)
...
**Top Internal Functions/Classes:**
  ...
  * `resolve_record_field` **(Many-Argument Workhorses)** (Impact: 36.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `find_branch_root` **(Compute Cores)** (Impact: 22.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2e-06 | `Ripple Effect (Closeness):` 0.001614
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2e-06 | `Ripple Effect (Closeness):` 0.00142
...
**Top Internal Functions/Classes:**
  ...
  * `highlight_related` **(Compute Cores)** (Impact: 50.4)
  ...
  * `highlight_break_points` **(Compute Cores)** (Impact: 37.6)
  * `highlight_branch_exit_points` **(Defensive Guards)** (Impact: 30.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3e-06 | `Ripple Effect (Closeness):` 0.001065
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 1019.04 | **LOC:** 4368 | **CtrlFlow:** 6.9% | **Authorship Centralization:** 28.1%
- **Risk Profile:** Cognitive Load (3.4579%), Tech Debt (89.2807%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `resolve_macros` **(Compute Cores)** (Impact: 72.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4.7e-05 | `Ripple Effect (Closeness):` 0.028708
...
- `crates/hir-ty/src/lower.rs` -> Churn: **93.14%** | Cog Load: 9.5967% | Debt: 78.3962%
- `crates/hir-ty/src/next_solver/interner.rs` -> Churn: **91.3%** | Cog Load: 10.0202% | Debt: 85.2534%
- `crates/syntax/src/ast/syntax_factory/constructors.rs` -> Churn: **88.24%** | Cog Load: 7.6281% | Debt: 95.3488%
- `crates/hir-ty/src/infer.rs` -> Churn: **87.68%** | Cog Load: 12.6465% | Debt: 99.9088%

### 👤 Key Person Dependencies (High Impact + Siloed Knowledge)
These are massive, load-bearing files written almost entirely by a single developer. They represent severe 'Bus Factor' risk.

- `crates/ide-completion/src/tests/expression.rs` -> **A4-Tacks** (86.7% isolated ownership) | Magnitude: 1232.06
