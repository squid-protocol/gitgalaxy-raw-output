...
> **File Composition:** Large Core Modules 30%, State Mutators Files 27%, Declarative / Non-Code 15%, Generic / Templated Code Files 11%, Data / Markup / Trivial 7%
...
- `__promisify__` **(Many-Argument Workhorses)** (@ `node/fs.d.ts`) -> Impact: **12.7** | LOC: 10
...
- **Composition Archetype:** `Interface Declarations Files` (z +1.52)
...
- **Heaviest Functions:** `read` (Generic / Templated Code, Impact: 3.7), `pipeThrough` (Generic / Templated Code, Impact: 3.5), `pipeTo` (Generic / Templated Code, Impact: 3.5)

### 2. `node/web-globals/timers.d.ts` (TYPESCRIPT) -> Cumulative Risk: **542.2**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.38)
...
- **Heaviest Functions:** `setInterval` (Generic / Templated Code, Impact: 4.2), `setTimeout` (Generic / Templated Code, Impact: 4.2), `setImmediate` (Generic / Templated Code, Impact: 1.9)
...
- **Heaviest Functions:** `postMessageToThread` (Parameter Forwarders, Impact: 4.8), `addEventListener` (Generic / Templated Code, Impact: 4.2), `addEventListener` (Parameter Forwarders, Impact: 4.2)
...
- **Heaviest Functions:** `inspect` (Compute Cores, Impact: 9.0), `deprecate` (Generic / Templated Code, Impact: 6.8), `callbackify` (Generic / Templated Code, Impact: 6.0)
...
- **Heaviest Functions:** `fill` (Compute Cores, Impact: 6.0), `slice` (Compute Cores, Impact: 5.2), `subarray` (Compute Cores, Impact: 5.2)
...
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (80.0%)
- **Heaviest Functions:** `fill` (Compute Cores, Impact: 6.0), `slice` (Generic / Templated Code, Impact: 5.2), `subarray` (Generic / Templated Code, Impact: 5.2)
...
- **Heaviest Functions:** `dir` (Compute Cores, Impact: 5.2), `table` (Compute Cores, Impact: 5.2), `assert` (Tests & Verification, Impact: 3.5)
...
- **Heaviest Functions:** `finished` (Generic / Templated Code, Impact: 23.1), `write` (Compute Cores, Impact: 8.1), `reduce` (Generic / Templated Code, Impact: 6.2)
...
- **Heaviest Functions:** `__promisify__` (Many-Argument Workhorses, Impact: 12.7), `writeSync` (Many-Argument Workhorses, Impact: 10.1), `__promisify__` (State Mutators, Impact: 7.5)
...
- **Heaviest Functions:** `setLocalAddress` (Compute Cores, Impact: 5.2), `__promisify__` (Generic / Templated Code, Impact: 3.5), `__promisify__` (State Mutators, Impact: 3.5)
...
**Top Internal Functions/Classes:**
  * `__promisify__` **(Many-Argument Workhorses)** (Impact: 12.7)
...
**Top Internal Functions/Classes:**
  ...
  * `end` **(Callbacks & Closures)** (Impact: 6.0)
...
**Top Internal Functions/Classes:**
  ...
  * `crc32` **(State Mutators)** (Impact: 3.5)
  ...
  * `brotliCompressSync` **(State Mutators)** (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  ...
  * `__promisify__` **(Generic / Templated Code)** (Impact: 3.5)
  * `__promisify__` **(State Mutators)** (Impact: 3.5)
  * `__promisify__` **(State Mutators)** (Impact: 3.5)
  * `__promisify__` **(State Mutators)** (Impact: 3.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `connect` **(Callbacks & Closures)** (Impact: 9.2)
  * `connect` **(Callbacks & Closures)** (Impact: 6.0)
...
**Top Internal Functions/Classes:**
  ...
  * `send` **(Interface Declarations)** (Impact: 4.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `off` **(Parameter Forwarders)** (Impact: 4.0)
...
**Top Internal Functions/Classes:**
  ...
  * `send` **(Many-Argument Workhorses)** (Impact: 7.7)
  * `send` **(Parameter Forwarders)** (Impact: 6.2)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `copyWithin` **(State Mutators)** (Impact: 4.0)
...
**Top Internal Functions/Classes:**
  * `cursorTo` **(Callbacks & Closures)** (Impact: 6.0)
  ...
  * `listenerCount` **(Generic / Templated Code)** (Impact: 3.7)
  * `constructor` **(State Mutators)** (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  ...
  * `getCppHeapStatistics` **(State Mutators)** (Impact: 2.9)
...
**Top Internal Functions/Classes:**
  ...
  * `cancel` **(Interface Declarations)** (Impact: 2.9)
  * `error` **(Interface Declarations)** (Impact: 2.9)
**Structural Signatures (Net Mitigated Signals):**
