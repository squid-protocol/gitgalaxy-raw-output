...
> **Composition Archetype:** `Small Flat Repo` (z -0.50; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 49%, Data / Markup / Trivial 20%, Large Core Modules 12%, Compute Cores Files 6%, I/O & Config Routines Files 3%
...
- `client_mode` **(Compute Cores)** (@ `masses/mass-check`) -> Impact: **148.3** | LOC: 387
...
### 1. `t/SATest.pm` (PERL) -> Cumulative Risk: **716.74**
...
- **Heaviest Functions:** `sa_t_init` (Compute Cores, Impact: 106.8), `start_spamd` (Compute Cores, Impact: 66.1), `patterns_run_cb` (Compute Cores, Impact: 45.5)
...
- **Heaviest Functions:** `mpm_specific_config` (Compute Cores, Impact: 26.0), `get_libexecdir_apxs` (I/O & Config Routines, Impact: 6.5), `apache_module_path` (Compute Cores, Impact: 6.0)
...
- **Heaviest Functions:** `makeSortable` (Compute Cores, Impact: 41.5), `getInnerText` (Defensive Guards, Impact: 33.4), `guessType` (Compute Cores, Impact: 22.4)
...
- **Heaviest Functions:** `generate_messages` (Many-Argument Workhorses, Impact: 243.6), `wanted` (Many-Argument Workhorses, Impact: 181.0), `client_mode` (Compute Cores, Impact: 148.3)
...
- **Heaviest Functions:** `mux_input` (Many-Argument Workhorses, Impact: 28.2), `headers` (Compute Cores, Impact: 25.7), `body` (Compute Cores, Impact: 16.6)

### 6. `Makefile.PL` (PERL) -> Cumulative Risk: **626.49**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
...
- **Heaviest Functions:** `MY::postamble` (Compute Cores, Impact: 71.2), `MY::constants` (Compute Cores, Impact: 37.6), `MY::dist` (Compute Cores, Impact: 12.5)
...
- **Heaviest Functions:** `_opensocket` (Many-Argument Workhorses, Impact: 926.1), `message_filter` (Many-Argument Workhorses, Impact: 182.0), `message_tell` (Many-Argument Workhorses, Impact: 136.6)

### 8. `t/dnsbl_subtests.t` (PERL) -> Cumulative Risk: **604.75**
- **Archetype:** `file_cluster_16` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.98)
...
- **Heaviest Functions:** `reply_handler` (Many-Argument Workhorses, Impact: 46.8), `find_free_port` (Compute Cores, Impact: 30.7), `process_sample_urls` (Compute Cores, Impact: 19.9)

### 9. `spamc/utils.c` (C) -> Cumulative Risk: **603.88**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.81)
...
- **Heaviest Functions:** `fd_timeout_read` (Many-Argument Workhorses, Impact: 26.7), `ssl_timeout_read` (Many-Argument Workhorses, Impact: 20.0), `full_write` (Many-Argument Workhorses, Impact: 19.3)

### 10. `spamc/getopt.c` (C) -> Cumulative Risk: **595.04**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z -0.25)
- **Magnitude:** 422.34 | **LOC:** 352 | **CtrlFlow:** 33.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (96.5979%)
- **Heaviest Functions:** `spamc_getopt_long` (Many-Argument Workhorses, Impact: 125.5), `spamc_getopt` (Many-Argument Workhorses, Impact: 46.0), `main` (Compute Cores, Impact: 30.4)
...
**Top Internal Functions/Classes:**
  ...
  * `client_mode` **(Compute Cores)** (Impact: 148.3)
...
**Top Internal Functions/Classes:**
  ...
  * `generate_scoremap_chart` **(Compute Cores)** (Impact: 61.0)
...
**Top Internal Functions/Classes:**
  ...
  * `print_usage` **(Compute Cores)** (Impact: 30.9)
...
**Top Internal Functions/Classes:**
  ...
  * `dist_across_dests` **(I/O & Config Routines)** (Impact: 17.2)
...
**Top Internal Functions/Classes:**
  ...
  * `gen_report_freqs_all` **(Many-Argument Workhorses)** (Impact: 45.6)
    * *Intent:* # ---------------------------------------------------------------------------
  * `locate_input` **(Compute Cores)** (Impact: 40.0)
    * *Intent:* # ---------------------------------------------------------------------------
...
**Top Internal Functions/Classes:**
  ...
  * `main` **(Compute Cores)** (Impact: 28.2)
...
**Top Internal Functions/Classes:**
  ...
  * `optiserr` **(Many-Argument Workhorses)** (Impact: 25.2)
  * `longoptiserr` **(Many-Argument Workhorses)** (Impact: 21.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 2 instances
...
**Top Internal Functions/Classes:**
  * `read_ranges` **(Compute Cores)** (Impact: 65.3)
  ...
  * `readscores` **(I/O & Config Routines)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `usage` **(Compute Cores)** (Impact: 11.8)
