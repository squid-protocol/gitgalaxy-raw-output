...
> **Composition Archetype:** `Small Flat Repo` (z +2.41; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 51%, Interface Declarations Files 23%, Declarative / Non-Code 11%, I/O & Config Routines Files 9%, Large Core Modules 3%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.4 | 25.7 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 9.0 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 8.4 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 73.7 | 7.2 | 0.0 | 0.0 |
...
| danger | 779 | 102 | 2 | `src/webui/src/main/java/com/ibm/cics/cip/bankliberty/web/vsam/Customer.java` |
...
| ui | 666 | 56 | 2 | `src/bank-application-frontend/src/content/CustomerCreationPage/CustomerCreationPage.js` |
...
- `ED010` **(Compute Cores)** (@ `src/base/cobol_src/BNK1CCS.cbl`) -> Impact: **98.8** | LOC: 355
...
|---|---|---|---|---|
| `src/base/cobol_src` | 29 | 9701.0 | 82.08% | 34.97% |
...
### 1. `src/base/cobol_src/CRECUST.cbl` (COBOL) -> Cumulative Risk: **756.38**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%), Cognitive Load (98.2526%)
- **Heaviest Functions:** `CC010` (I/O & Config Routines, Impact: 66.0), `P010` (I/O & Config Routines, Impact: 17.7), `GLCV010` (I/O & Config Routines, Impact: 14.7)

### 2. `src/base/cobol_src/BNK1CCA.cbl` (COBOL) -> Cumulative Risk: **665.25**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
...
- **Heaviest Functions:** `GCD010` (I/O & Config Routines, Impact: 26.4), `A010` (I/O & Config Routines, Impact: 13.7), `SM010` (I/O & Config Routines, Impact: 13.5)

### 3. `src/base/cobol_src/BNK1DCS.cbl` (COBOL) -> Cumulative Risk: **664.81**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.43)
- **Magnitude:** 892.12 | **LOC:** 2057 | **CtrlFlow:** 5.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (95.692%)
- **Heaviest Functions:** `A010` (I/O & Config Routines, Impact: 19.9), `ED2010` (I/O & Config Routines, Impact: 19.1), `SM010` (I/O & Config Routines, Impact: 15.0)

### 4. `src/base/cobol_src/BNK1DAC.cbl` (COBOL) -> Cumulative Risk: **660.16**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
...
- **Heaviest Functions:** `A010` (I/O & Config Routines, Impact: 19.4), `SM010` (I/O & Config Routines, Impact: 15.0), `DAD010` (I/O & Config Routines, Impact: 11.5)

### 5. `src/base/cobol_src/BNK1UAC.cbl` (COBOL) -> Cumulative Risk: **655.29**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (96.2111%)
- **Heaviest Functions:** `VD010` (I/O & Config Routines, Impact: 28.5), `A010` (I/O & Config Routines, Impact: 16.9), `SM010` (I/O & Config Routines, Impact: 13.5)

### 6. `src/base/cobol_src/DBCRFUN.cbl` (COBOL) -> Cumulative Risk: **654.31**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9872%), Cognitive Load (88.6439%)
- **Heaviest Functions:** `WTPD010` (I/O & Config Routines, Impact: 15.0), `AH010` (I/O & Config Routines, Impact: 14.9), `UAD010` (I/O & Config Routines, Impact: 14.0)

### 7. `src/base/cobol_src/BNK1CRA.cbl` (COBOL) -> Cumulative Risk: **653.92**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Heaviest Functions:** `VA010` (I/O & Config Routines, Impact: 19.5), `UCD010` (I/O & Config Routines, Impact: 17.9), `A010` (I/O & Config Routines, Impact: 15.4)

### 8. `src/base/cobol_src/BNK1CCS.cbl` (COBOL) -> Cumulative Risk: **652.52**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (97.4%)
- **Heaviest Functions:** `ED010` (Compute Cores, Impact: 98.8), `A010` (I/O & Config Routines, Impact: 15.9), `SM010` (I/O & Config Routines, Impact: 15.0)

### 9. `src/base/cobol_src/BNK1TFN.cbl` (COBOL) -> Cumulative Risk: **651.66**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (95.6098%)
- **Heaviest Functions:** `GCD010` (I/O & Config Routines, Impact: 19.9), `WS-NUM-COUNT-MINUS` (I/O & Config Routines, Impact: 14.0), `A010` (I/O & Config Routines, Impact: 13.7)

### 10. `src/base/cobol_src/CREACC.cbl` (COBOL) -> Cumulative Risk: **649.76**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.18)
- **Magnitude:** 454.4 | **LOC:** 1248 | **CtrlFlow:** 4.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9985%), Cognitive Load (95.8604%)
- **Heaviest Functions:** `FNA010` (I/O & Config Routines, Impact: 23.5), `CD010` (I/O & Config Routines, Impact: 23.2), `REQUIRED-SORT-CODE2` (I/O & Config Routines, Impact: 10.4)
...
**Top Internal Functions/Classes:**
  * `ED010` **(Compute Cores)** (Impact: 98.8)
...
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 659.38 | **LOC:** 1498 | **CtrlFlow:** 8.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (98.2526%), Tech Debt (9.1307%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `FILE-CONTROL` **(I/O & Config Routines)** (Impact: 24.2)
...
**Top Internal Functions/Classes:**
  ...
  * `Anonymous_Block` **(I/O & Config Routines)** (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `updateThis` **(I/O & Config Routines)** (Impact: 6.5)
  * `addToDB` **(I/O & Config Routines)** (Impact: 5.5)
  * `deleteFromDB` **(Type Conversions)** (Impact: 5.0)
  * `inDB` **(I/O & Config Routines)** (Impact: 5.0)
...
**Top Internal Functions/Classes:**
  * `setCustomerControlEyecatcher` **(Interface Declarations)** (Impact: 3.4)
  * `setCustomerControlSortcode` **(Interface Declarations)** (Impact: 3.4)
  * `setCustomerControlNumber` **(Interface Declarations)** (Impact: 3.4)
  * `setNumberOfCustomers` **(Interface Declarations)** (Impact: 3.4)
  * `setLastCustomerNumber` **(Interface Declarations)** (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
