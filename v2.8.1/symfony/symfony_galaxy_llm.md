...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +2.67; from the repo's file-archetype mix)
> **File Composition:** Interface Declarations Files 29%, Data / Markup / Trivial 28%, Declarative / Non-Code 20%, Defensive Guards Files 7%, Large Core Modules 4%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 40.7 | 6.1 | 0.0 |
...
- **Heaviest Functions:** `requestHeaderStart` (Compute Cores, Impact: 33.9), `requestStart` (Defensive Guards, Impact: 6.3), `connectionAcquired` (Many-Argument Workhorses, Impact: 4.5)
...
- **Heaviest Functions:** `request` (Many-Argument Workhorses, Impact: 41.7), `getClient` (Compute Cores, Impact: 36.3), `handlePush` (Defensive Guards, Impact: 11.1)
...
- **Heaviest Functions:** `buildRules` (Compute Cores, Impact: 22.9), `saveRules` (Defensive Guards, Impact: 15.4), `getEmojisCodePoints` (I/O & Config Routines, Impact: 11.3)
...
- **Heaviest Functions:** `castFunctionAbstract` (Many-Argument Workhorses, Impact: 59.2), `getSignature` (Compute Cores, Impact: 36.5), `castParameter` (Many-Argument Workhorses, Impact: 33.3)
...
- **Heaviest Functions:** `testSign` (Many-Argument Workhorses, Impact: 7.9), `testCanonicalizeHeader` (Many-Argument Workhorses, Impact: 3.4), `getCanonicalizeHeaderData` (I/O & Config Routines, Impact: 3.2)
...
- **Heaviest Functions:** `stream` (Many-Argument Workhorses, Impact: 101.2), `passthruStream` (Many-Argument Workhorses, Impact: 83.3), `__construct` (Many-Argument Workhorses, Impact: 46.4)
...
- **Heaviest Functions:** `getPushedResponse` (Many-Argument Workhorses, Impact: 71.4), `followRedirects` (Many-Argument Workhorses, Impact: 71.0), `generateResponse` (Many-Argument Workhorses, Impact: 44.8)

### 8. `src/Symfony/Bundle/FrameworkBundle/Command/AbstractConfigCommand.php` (PHP) -> Cumulative Risk: **639.57**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.39)
...
- **Heaviest Functions:** `findExtension` (Compute Cores, Impact: 31.5), `listNonBundleExtensions` (Compute Cores, Impact: 14.6), `listBundles` (Compute Cores, Impact: 9.6)
...
- **Heaviest Functions:** `translateElement` (Compute Cores, Impact: 12.7), `translateAttribute` (Compute Cores, Impact: 11.6), `setFlag` (Compute Cores, Impact: 9.3)
...
- **Heaviest Functions:** `__construct` (Compute Cores, Impact: 28.6), `readAndWrite` (Compute Cores, Impact: 13.7), `getDescriptors` (I/O & Config Routines, Impact: 3.0)
...
**Top Internal Functions/Classes:**
  ...
  * `addService` **(Compute Cores)** (Impact: 110.3)
...
- **Magnitude:** 2596.78 | **LOC:** 3589 | **CtrlFlow:** 23.2% | **Authorship Centralization:** 32.4%
...
**Top Internal Functions/Classes:**
  ...
  * `getTrustedValues` **(Compute Cores)** (Impact: 46.1)
  ...
  * `prepareBaseUrl` **(Compute Cores)** (Impact: 26.1)
...
**Top Internal Functions/Classes:**
  ...
  * `getNextEmbedBlock` **(Compute Cores)** (Impact: 77.5)
...
**Top Internal Functions/Classes:**
  ...
  * `compareEquality` **(Compute Cores)** (Impact: 49.1)
  * `evaluateFilterExpression` **(Compute Cores)** (Impact: 46.9)
  * `evaluateScalar` **(Compute Cores)** (Impact: 45.3)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 1385.18 | **LOC:** 3160 | **CtrlFlow:** 4.9% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (63.6331%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `assertCachePoolServiceDefinitionIsCreated` **(Many-Argument Workhorses)** (Impact: 19.4)
  * `createContainerFromFile` **(Many-Argument Workhorses)** (Impact: 18.4)
  * `testWorkflows` **(Tests & Verification)** (Impact: 12.9)
  * `testMessengerTransports` **(Tests & Verification)** (Impact: 12.9)
...
**Top Internal Functions/Classes:**
  ...
  * `denormalizeParameter` **(Many-Argument Workhorses)** (Impact: 60.2)
    * *Intent:* /** * @internal */
  * `updateData` **(Many-Argument Workhorses)** (Impact: 46.2)
...
**Top Internal Functions/Classes:**
  * `testRun` **(Compute Cores)** (Impact: 35.0)
...
**Top Internal Functions/Classes:**
  * `testDenormalizeNullCoalescingValues` **(Tests & Verification)** (Impact: 11.8)
  ...
  * `setAttributeValue` **(Many-Argument Workhorses)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `normalizeBody` **(Defensive Guards)** (Impact: 133.3)
  ...
  * `resolveUrl` **(Defensive Guards)** (Impact: 73.2)
...
**Top Internal Functions/Classes:**
  * `connect` **(Annotated Framework Methods)** (Impact: 3.2)
  * `migrate` **(Annotated Framework Methods)** (Impact: 3.2)
  * `pconnect` **(Annotated Framework Methods)** (Impact: 3.2)
...
**Top Internal Functions/Classes:**
  ...
  * `parseCallable` **(Many-Argument Workhorses)** (Impact: 42.2)
...
**Top Internal Functions/Classes:**
  ...
  * `doFetch` **(Defensive Guards)** (Impact: 18.6)
...
- `src/Symfony/Bundle/FrameworkBundle/DependencyInjection/FrameworkExtension.php` -> Churn: **89.7%** | Cog Load: 68.0405% | Debt: 0.0%
- `src/Symfony/Bundle/FrameworkBundle/Tests/DependencyInjection/FrameworkExtensionTestCase.php` -> Churn: **75.17%** | Cog Load: 63.6331% | Debt: 0.0%
