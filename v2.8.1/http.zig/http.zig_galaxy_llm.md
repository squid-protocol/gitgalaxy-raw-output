...
> **Composition Archetype:** `Hub-Coupled App` (z +1.38; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 39%, State Mutators Files 24%, Defensive Guards Files 18%, Data / Markup / Trivial 6%, Compute Cores Files 3%
...
- **Heaviest Functions:** `Worker` (Compute Cores, Impact: 43.2), `spawn` (Compute Cores, Impact: 22.5), `ThreadPool` (Defensive Guards, Impact: 18.3)

### 2. `src/worker.zig` (ZIG) -> Cumulative Risk: **547.91**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.01)
- **Magnitude:** 1006.44 | **LOC:** 1971 | **CtrlFlow:** 10.4% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (97.1098%), Churn (93.58%)
- **Heaviest Functions:** `NonBlocking` (Many-Argument Workhorses, Impact: 112.2), `Blocking` (Defensive Guards, Impact: 79.7), `handleRequest` (Many-Argument Workhorses, Impact: 48.8)

### 3. `src/t.zig` (ZIG) -> Cumulative Risk: **545.8**
...
- **Heaviest Functions:** `allocInit` (Defensive Guards, Impact: 23.0), `expect` (Defensive Guards, Impact: 14.1), `read` (Defensive Guards, Impact: 13.3)

### 4. `src/httpz.zig` (ZIG) -> Cumulative Risk: **543.08**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.01)
...
- **Heaviest Functions:** `Server` (Defensive Guards, Impact: 106.8), `upgradeWebsocket` (Defensive Guards, Impact: 31.6), `init` (Defensive Guards, Impact: 30.8)

### 5. `src/key_value.zig` (ZIG) -> Cumulative Risk: **536.25**
- **Archetype:** `file_cluster_14` (Distance: N/A IQR)
...
- **Heaviest Functions:** `KeyValue` (Type Conversions, Impact: 24.0), `init` (Type Conversions, Impact: 9.3), `get` (Compute Cores, Impact: 5.6)

### 6. `src/testing.zig` (ZIG) -> Cumulative Risk: **525.64**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Defensive Guards Files` (z +0.16)
- **Magnitude:** 318.64 | **LOC:** 668 | **CtrlFlow:** 11.7% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (97.6061%), Documentation (97.0588%), Churn (66.67%)
- **Heaviest Functions:** `compareValue` (Defensive Guards, Impact: 35.0), `parseWithAllocator` (Defensive Guards, Impact: 17.7), `escapeString` (Defensive Guards, Impact: 13.3)

### 7. `src/response.zig` (ZIG) -> Cumulative Risk: **502.63**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.11)
...
- **Heaviest Functions:** `prepareHeader` (Compute Cores, Impact: 27.4), `serializeCookie` (Many-Argument Workhorses, Impact: 27.4), `headerOpts` (Defensive Guards, Impact: 11.4)
...
- **Heaviest Functions:** `unescape` (Many-Argument Workhorses, Impact: 30.6), `isValid` (Defensive Guards, Impact: 11.0), `parse` (Defensive Guards, Impact: 3.6)

### 9. `src/request.zig` (ZIG) -> Cumulative Risk: **469.45**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
...
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.7207%), Documentation (93.1034%), Churn (52.83%)
- **Heaviest Functions:** `parse` (Defensive Guards, Impact: 43.1), `parseHeaders` (Many-Argument Workhorses, Impact: 34.4), `prepareForBody` (Many-Argument Workhorses, Impact: 29.4)
...
- **Heaviest Functions:** `Router` (Defensive Guards, Impact: 58.1), `getRoute` (Many-Argument Workhorses, Impact: 56.7), `addRoute` (Many-Argument Workhorses, Impact: 53.1)
...
**Top Internal Functions/Classes:**
  ...
  * `getContentDispotionAttributes` **(Compute Cores)** (Impact: 29.2)
    * *Intent:* // I'm sorry
  * `parseMultiFormData` **(Compute Cores)** (Impact: 28.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `write` **(Compute Cores)** (Impact: 8.2)
...
**Top Internal Functions/Classes:**
  ...
  * `status` **(State Mutators)** (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  ...
  * `free` **(State Mutators)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `localhost` **(Interface Declarations)** (Impact: 1.6)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `index` **(State Mutators)** (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Memory Alloc (weighted view):* 0
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `addNames` **(State Mutators)** (Impact: 1.9)
...
**Top Internal Functions/Classes:**
  ...
  * `main` **(Interface Declarations)** (Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `index` **(State Mutators)** (Impact: 2.5)
  * `main` **(Interface Declarations)** (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `build` **(Compute Cores)** (Impact: 15.8)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `main` **(Interface Declarations)** (Impact: 2.0)
