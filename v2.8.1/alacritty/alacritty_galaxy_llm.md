...
> **Composition Archetype:** `Small Flat Repo` (z -0.57; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 61%, Large Core Modules 17%, State Mutators Files 5%, Declarative / Non-Code 4%, Tests & Verification Files 3%
...
- `deserialize` **(Compute Cores)** (@ `alacritty/src/config/bindings.rs`) -> Impact: **67.2** | LOC: 212
...
- **Heaviest Functions:** `push_escaped_arg` (Compute Cores, Impact: 16.9), `cmdline` (Defensive Guards, Impact: 6.5), `test_escape` (Tests & Verification, Impact: 5.6)
...
- **Heaviest Functions:** `user_event` (Many-Argument Workhorses, Impact: 81.0), `handle_event` (Many-Argument Workhorses, Impact: 58.2), `paste` (Many-Argument Workhorses, Impact: 24.1)
...
- **Heaviest Functions:** `fill_cube` (Compute Cores, Impact: 22.0), `from_str` (Compute Cores, Impact: 12.4), `fill_gray_ramp` (Defensive Guards, Impact: 8.1)

### 4. `extra/completions/alacritty.bash` (SHELL) -> Cumulative Risk: **544.0**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.10)
...
- **Heaviest Functions:** `_alacritty` (Many-Argument Workhorses, Impact: 194.2), `Anonymous_Block` (Compute Cores, Impact: 5.2)

### 5. `alacritty_terminal/src/term/search.rs` (RUST) -> Cumulative Risk: **534.38**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +1.77)
...
- **Heaviest Functions:** `regex_search_internal` (Many-Argument Workhorses, Impact: 97.4), `bracket_search` (Compute Cores, Impact: 31.5), `skip_fullwidth` (Many-Argument Workhorses, Impact: 21.8)

### 6. `alacritty_terminal/src/grid/row.rs` (RUST) -> Cumulative Risk: **527.6**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Annotated Framework Methods Files` (z -0.32)
...
- **Heaviest Functions:** `shrink` (Generic / Templated Code, Impact: 7.8), `reset` (Generic / Templated Code, Impact: 6.2), `grow` (Annotated Framework Methods, Impact: 3.9)

### 7. `alacritty_terminal/src/selection.rs` (RUST) -> Cumulative Risk: **504.76**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +1.91)
...
- **Heaviest Functions:** `rotate` (Many-Argument Workhorses, Impact: 38.5), `contains_cell` (Many-Argument Workhorses, Impact: 28.3), `range_simple` (Many-Argument Workhorses, Impact: 21.8)
...
- **Heaviest Functions:** `new` (Many-Argument Workhorses, Impact: 22.3), `convert_custom_env` (Compute Cores, Impact: 11.8), `load_conpty` (Interface Declarations, Impact: 5.9)
...
- **Heaviest Functions:** `line_to_string` (Many-Argument Workhorses, Impact: 43.3), `input` (Compute Cores, Impact: 31.6), `clear_screen` (Compute Cores, Impact: 19.1)

### 10. `alacritty_config_derive/src/config_deserialize/de_struct.rs` (RUST) -> Cumulative Risk: **491.32**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.67)
...
- **Heaviest Functions:** `field_deserializer` (Many-Argument Workhorses, Impact: 26.9), `derive_deserialize` (Generic / Templated Code, Impact: 17.5), `visit_map` (Generic / Templated Code, Impact: 10.0)
...
**Top Internal Functions/Classes:**
  ...
  * `input` **(Compute Cores)** (Impact: 31.6)
    * *Intent:* /// A character to be displayed.
  * `clear_screen` **(Compute Cores)** (Impact: 19.1)
...
**Top Internal Functions/Classes:**
  ...
  * `normal_mouse_report` **(Many-Argument Workhorses)** (Impact: 23.6)
...
**Top Internal Functions/Classes:**
  ...
  * `inline_search_right` **(Many-Argument Workhorses)** (Impact: 19.3)
...
**Top Internal Functions/Classes:**
  ...
  * `deserialize` **(Compute Cores)** (Impact: 67.2)
  * `triggers_match` **(Compute Cores)** (Impact: 18.4)
  * `deserialize` **(Compute Cores)** (Impact: 18.1)
...
**Top Internal Functions/Classes:**
  ...
  * `draw_line` **(Many-Argument Workhorses)** (Impact: 35.2)
    * *Intent:* /// Xiaolin Wu's line drawing from (`from_x`, `from_y`) to (`to_x`, `to_y`).
  * `powerline_drawing` **(Many-Argument Workhorses)** (Impact: 29.6)
...
**Top Internal Functions/Classes:**
  ...
  * `update_matches` **(Compute Cores)** (Impact: 21.9)
...
**Top Internal Functions/Classes:**
  ...
  * `grow_columns` **(Many-Argument Workhorses)** (Impact: 73.1)
    * *Intent:* /// Grow number of columns in each row, reflowing if necessary.
  * `resize` **(Many-Argument Workhorses)** (Impact: 7.9)
...
**Top Internal Functions/Classes:**
  ...
  * `reset_region` **(Generic / Templated Code)** (Impact: 8.1)
...
**Top Internal Functions/Classes:**
  * `new` **(Compute Cores)** (Impact: 39.2)
  * `new` **(Many-Argument Workhorses)** (Impact: 27.0)
  * `compute_fg_rgb` **(Many-Argument Workhorses)** (Impact: 16.2)
...
**Top Internal Functions/Classes:**
  ...
  * `recv` **(Compute Cores)** (Impact: 7.6)
  * `new` **(Many-Argument Workhorses)** (Impact: 5.9)
...
**Top Internal Functions/Classes:**
  ...
  * `update_config` **(Compute Cores)** (Impact: 21.0)
...
**Top Internal Functions/Classes:**
  ...
  * `get_platform_window` **(Annotated Framework Methods)** (Impact: 5.7)
...
**Top Internal Functions/Classes:**
  ...
  * `remove_target` **(Tests & Verification)** (Impact: 6.1)
  * `remove_duplicates` **(Tests & Verification)** (Impact: 4.1)
  * `pop` **(Annotated Framework Methods)** (Impact: 3.3)
    * *Intent:* /// Remove the currently visible message.
  * `truncates_long_messages` **(Tests & Verification)** (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 21 instances
...
**Top Internal Functions/Classes:**
  ...
  * `from_str` **(Compute Cores)** (Impact: 12.4)
...
**Top Internal Functions/Classes:**
  ...
  * `compute_index` **(Tests & Verification)** (Impact: 6.0)
  ...
  * `shrink_lines` **(Annotated Framework Methods)** (Impact: 3.9)
