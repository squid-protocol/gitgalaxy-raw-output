...
> **Composition Archetype:** `Small Flat Repo` (z +4.28; from the repo's file-archetype mix)
> **File Composition:** Tests & Verification Files 74%, Compute Cores Files 6%, Many-Argument Workhorses Files 5%, Declarative / Non-Code 4%, State Mutators Files 4%
...
- `parse_stmt_line` **(Compute Cores)** (@ `src/parser.rs`) -> Impact: **100.1** | LOC: 236
  * *Intent:* /// Parse a single statement from a comments-removed trimmed line. `line_no` used for error messages.
- `for_statements_invalid_construction_errors` **(Compute Cores)** (@ `src/parser/blackbox_tests/for_stmt_tests.rs`) -> Impact: **67.4** | LOC: 88
- `parse_function` **(Compute Cores)** (@ `src/parser.rs`) -> Impact: **65.0** | LOC: 123
  * *Intent:* /// Parse function starting at index `start_i`. /// Returns (Function, index after function end).
- `if_statements_invalid_construction_errors` **(Compute Cores)** (@ `src/parser/blackbox_tests/if_stmt_tests.rs`) -> Impact: **63.1** | LOC: 83
- `invalid_construction_errors` **(Compute Cores)** (@ `src/parser/blackbox_tests/while_stmt_tests.rs`) -> Impact: **63.1** | LOC: 83
...
- **Heaviest Functions:** `find_top_level_op_any` (Compute Cores, Impact: 57.6), `split_char_top_level` (Compute Cores, Impact: 45.8), `count_braces_outside_strings` (Compute Cores, Impact: 21.6)

### 2. `src/semantic/branch_analysis.rs` (RUST) -> Cumulative Risk: **621.62**
- **Archetype:** `file_cluster_7` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +1.82)
...
- **Heaviest Functions:** `return_branch_analysis` (Many-Argument Workhorses, Impact: 123.1), `dead_code_analysis` (Compute Cores, Impact: 56.7)

### 3. `src/parser.rs` (RUST) -> Cumulative Risk: **597.12**
- **Archetype:** `file_cluster_4` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z -0.09)
...
- **Heaviest Functions:** `parse_stmt_line` (Compute Cores, Impact: 100.1), `parse_function` (Compute Cores, Impact: 65.0), `parse_block` (Compute Cores, Impact: 43.0)
...
- **Heaviest Functions:** `transpile_stmt` (Compute Cores, Impact: 59.8), `holy_expr_to_rust_expr` (Compute Cores, Impact: 38.4), `transpile_function` (Compute Cores, Impact: 15.3)

### 5. `src/compile.rs` (RUST) -> Cumulative Risk: **572.22**
- **Archetype:** `file_cluster_15` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +0.20)
...
- **Heaviest Functions:** `compile` (Compute Cores, Impact: 6.1)
...
- **Heaviest Functions:** `coerce_integer_literal_to_type_helper` (Type Conversions, Impact: 48.0), `get_bigger_type_of_two_integers` (Compute Cores, Impact: 12.8), `stmt_span` (State Mutators, Impact: 3.8)

### 7. `src/semantic.rs` (RUST) -> Cumulative Risk: **560.61**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +0.70)
...
- **Heaviest Functions:** `check_stmts` (Many-Argument Workhorses, Impact: 393.5), `check_call` (Many-Argument Workhorses, Impact: 45.9), `check_function` (Many-Argument Workhorses, Impact: 19.5)

### 8. `src/semantic/blackbox_tests.rs` (RUST) -> Cumulative Risk: **546.56**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `State Mutators Files` (z +1.60)
...
- **Heaviest Functions:** `get_non_boolean_conditions` (I/O & Config Routines, Impact: 8.3), `get_many_boolean_conditions` (I/O & Config Routines, Impact: 7.7), `get_many_boolean_conditions_no_dyn_arr` (I/O & Config Routines, Impact: 7.7)

### 9. `src/semantic/constants.rs` (RUST) -> Cumulative Risk: **534.37**
- **Archetype:** `file_cluster_11` (Distance: N/A IQR)
- **Composition Archetype:** `Many-Argument Workhorses Files` (z +2.15)
...
- **Heaviest Functions:** `eval_const_expr_and_fold_it_hazmat` (Many-Argument Workhorses, Impact: 143.2), `eval_const_expr_and_fold_it` (Many-Argument Workhorses, Impact: 13.4), `truncate_to_uint_type_hazmat` (Type Conversions, Impact: 4.6)

### 10. `src/ast/types_tests.rs` (RUST) -> Cumulative Risk: **522.19**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +0.06)
...
- **Heaviest Functions:** `default_value_all_valid_variants` (Tests & Verification, Impact: 9.3), `no_type_is_both_integer_and_float` (Tests & Verification, Impact: 7.2), `no_type_is_both_integer_and_array` (Tests & Verification, Impact: 7.2)
...
**Top Internal Functions/Classes:**
  * `invalid_construction_errors` **(Compute Cores)** (Impact: 63.1)
  * `invalid_construction_errors` **(Compute Cores)** (Impact: 62.0)
  * `trailing_kw_errors` **(Compute Cores)** (Impact: 46.0)
...
**Top Internal Functions/Classes:**
  * `parse_stmt_line` **(Compute Cores)** (Impact: 100.1)
    * *Intent:* /// Parse a single statement from a comments-removed trimmed line. `line_no` used for error messages...
  * `parse_function` **(Compute Cores)** (Impact: 65.0)
    * *Intent:* /// Parse function starting at index `start_i`. /// Returns (Function, index after function end).
...
**Top Internal Functions/Classes:**
  * `if_statements_invalid_construction_errors` **(Compute Cores)** (Impact: 63.1)
  * `if_statements_trailing_kw_errors` **(Compute Cores)** (Impact: 51.2)
  * `if_statements_with_elif_vars_and_literals` **(Tests & Verification)** (Impact: 39.5)
  * `if_statements_vars_and_literals` **(Tests & Verification)** (Impact: 25.9)
  * `if_statements_trailing_types_errors` **(Compute Cores)** (Impact: 24.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `holy_expr_to_rust_expr` **(Compute Cores)** (Impact: 38.4)
...
**Top Internal Functions/Classes:**
  * `const_evaluated_numeric_comparison` **(Tests & Verification)** (Impact: 12.7)
    * *Intent:* // This includes greater than/ less than
  * `const_evaluated_logical_and_with_equal_comparison` **(Tests & Verification)** (Impact: 12.6)
  * `const_all_arth_binop_on_literals` **(Tests & Verification)** (Impact: 12.1)
  * `const_evaluated_equal_comparison` **(Tests & Verification)** (Impact: 10.7)
  * `const_unary_negate_on_signed_consts` **(Tests & Verification)** (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 11 instances
...
**Top Internal Functions/Classes:**
  * `vardecl_in_if_else_branch_moves_upstream_var` **(Tests & Verification)** (Impact: 16.1)
  * `vardecl_in_if_elif_branch_moves_upstream_var` **(Tests & Verification)** (Impact: 16.1)
  * `vardecl_in_if_main_branch_does_not_move_upstream_const` **(Tests & Verification)** (Impact: 16.0)
  * `vardecl_moving_local_var_in_if_stmt_elif_branch` **(Tests & Verification)** (Impact: 13.8)
  * `vardecl_moving_local_var_in_if_stmt_else_branch` **(Tests & Verification)** (Impact: 13.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `find_top_level_op_any` **(Compute Cores)** (Impact: 57.6)
  * `split_char_top_level` **(Compute Cores)** (Impact: 45.8)
...
**Top Internal Functions/Classes:**
  * `vars_and_unsigned_integer_literals_mixed` **(Tests & Verification)** (Impact: 24.9)
    * *Intent:* // Unsigned integer literals
  * `vars_and_signed_integer_literals_mixed_in_var_decl` **(Tests & Verification)** (Impact: 22.1)
    * *Intent:* // Signed integer literals
  * `vars_and_signed_integer_literals_mixed` **(Tests & Verification)** (Impact: 21.5)
    * *Intent:* // Signed integer literals
  * `unsigned_literals_only_in_var_decl` **(Tests & Verification)** (Impact: 19.6)
  * `unsigned_literals_only` **(Tests & Verification)** (Impact: 19.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `for_statements_invalid_construction_errors` **(Compute Cores)** (Impact: 67.4)
  * `for_statements_trailing_kw_errors` **(Compute Cores)** (Impact: 46.0)
  * `for_statements_trailing_types_errors` **(Compute Cores)** (Impact: 24.6)
  * `for_statements_trailing_exprs_errors` **(Compute Cores)** (Impact: 23.6)
  * `for_statements_literal` **(Tests & Verification)** (Impact: 19.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `invalid_errors` **(Compute Cores)** (Impact: 48.4)
  * `invalid_errors` **(Compute Cores)** (Impact: 48.4)
  * `in_if_with_else_with_elif_stmt` **(Tests & Verification)** (Impact: 8.1)
  * `in_if_else_with_elif_stmt` **(Tests & Verification)** (Impact: 8.0)
  * `in_if_with_else_stmt` **(Tests & Verification)** (Impact: 7.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `eval_const_expr_and_fold_it` **(Many-Argument Workhorses)** (Impact: 13.4)
...
**Top Internal Functions/Classes:**
  * `unescaped_inner_double_quote_is_error` **(Tests & Verification)** (Impact: 36.5)
  * `empty_string_literal_as_arg` **(Tests & Verification)** (Impact: 13.8)
  * `whitespace_only_placeholder_is_captured` **(Tests & Verification)** (Impact: 4.5)
  * `braces_inside_double_quotes_not_counted` **(Tests & Verification)** (Impact: 4.0)
  * `backslash_escaped_quote_inside_string` **(Tests & Verification)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 3
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `dead_code_analysis` **(Compute Cores)** (Impact: 56.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `variable_redeclaration_without_value_allowed` **(Tests & Verification)** (Impact: 9.4)
    * *Intent:* // Same as above.
  * `variable_redeclaration_with_value_allowed` **(Tests & Verification)** (Impact: 9.3)
    * *Intent:* // Not allowed in semantics phase, but, this is **syntactically** correct
  * `var_decl_array` **(Tests & Verification)** (Impact: 8.2)
  * `var_decl_nested_array` **(Tests & Verification)** (Impact: 8.2)
  * `var_decl_deeply_nested_array` **(Tests & Verification)** (Impact: 8.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `after_var_decl_with_value` **(Tests & Verification)** (Impact: 9.3)
  * `below_var_decl_with_value` **(Tests & Verification)** (Impact: 9.2)
  * `with_var_decl_with_value` **(Tests & Verification)** (Impact: 9.2)
  * `below_var_decl_without_value` **(Tests & Verification)** (Impact: 8.1)
  * `after_var_decl_without_value` **(Tests & Verification)** (Impact: 8.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `default_value_all_valid_variants` **(Tests & Verification)** (Impact: 9.3)
    * *Intent:* // Type::get_default_value
  * `no_type_is_both_integer_and_float` **(Tests & Verification)** (Impact: 7.2)
    * *Intent:* // A type cannot be both integer AND float, ever.
  * `no_type_is_both_integer_and_array` **(Tests & Verification)** (Impact: 7.2)
    * *Intent:* // No type can be both an integer and an array. Ever. //
  * `no_type_is_both_float_and_array` **(Tests & Verification)** (Impact: 7.2)
    * *Intent:* // No type can be both a float and an array. Ever. //
  * `fixed_array_to_dynamic_array_type_full_pass` **(Tests & Verification)** (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 17 instances
...
**Top Internal Functions/Classes:**
  * `const_decl_in_if_else_branch` **(Tests & Verification)** (Impact: 11.2)
  * `const_decl_in_if_main_branch` **(Tests & Verification)** (Impact: 10.1)
  * `const_decl_in_if_elif_branch` **(Tests & Verification)** (Impact: 10.1)
  * `const_decl_in_while_loop` **(Tests & Verification)** (Impact: 10.0)
  * `const_decl_in_for_branch` **(Tests & Verification)** (Impact: 10.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `function_unterminated_errors` **(Compute Cores)** (Impact: 14.2)
  * `function_single_return_invalid_type_errors` **(Tests & Verification)** (Impact: 13.6)
  * `function_multiple_return_one_type_invalid_type_errors` **(Tests & Verification)** (Impact: 9.6)
  * `function_multiple_return_invalid_type_errors` **(Tests & Verification)** (Impact: 9.5)
  * `function_nested_array_return_type` **(Tests & Verification)** (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 6 instances
...
**Top Internal Functions/Classes:**
  * `if_statement_elif_branch_inside_infinite_stmt_break_errors` **(Tests & Verification)** (Impact: 6.5)
  * `infinite_statement_inside_if_stmt_break_errors` **(Tests & Verification)** (Impact: 6.5)
  * `if_statement_main_branch_inside_infinite_stmt_break_errors` **(Tests & Verification)** (Impact: 6.4)
    * *Intent:* // Same as above, but this is an if statement, inside infinite statement..
  * `if_statement_else_branch_inside_infinite_stmt_break_errors` **(Tests & Verification)** (Impact: 6.4)
  * `infinite_statement_while_statement_nested_branch_break` **(Tests & Verification)** (Impact: 5.8)
...
**Top Internal Functions/Classes:**
  * `array_slice_both_bounds_in_const` **(Tests & Verification)** (Impact: 9.2)
  * `array_slice_open_start_in_const` **(Tests & Verification)** (Impact: 9.2)
  * `array_slice_open_end_in_const` **(Tests & Verification)** (Impact: 9.2)
  * `array_slice_both_bounds_in_var_decl` **(Tests & Verification)** (Impact: 9.0)
  * `array_slice_open_start_in_var_decl` **(Tests & Verification)** (Impact: 9.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `multi_assign_locked_vars_errors` **(Tests & Verification)** (Impact: 8.8)
  * `unlock_func_arg_in_while_loop_errors` **(Tests & Verification)** (Impact: 4.2)
  * `lock_func_arg_in_while_loop_errors` **(Tests & Verification)** (Impact: 4.2)
  * `lock_literal_errors` **(Tests & Verification)** (Impact: 3.9)
  * `unlock_literal_errors` **(Tests & Verification)** (Impact: 3.9)
**Structural Signatures (Net Mitigated Signals):**
