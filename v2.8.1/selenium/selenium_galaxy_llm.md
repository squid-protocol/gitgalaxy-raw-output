...
> **Composition Archetype:** `Flat Modular Platform` (z -0.36; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 23%, Declarative / Non-Code 12%, Tests & Verification Files 12%, Interface Declarations Files 10%, Large Core Modules 10%
...
| threat | 2249 | 591 | 1 | `common/src/web/javascriptPage.html` |
...
- `InputManager::GetKeyInfo` **(Compute Cores)** (@ `cpp/iedriver/InputManager.cpp`) -> Impact: **231.7** | LOC: 304
...
- `parse` **(Compute Cores)** (@ `common/devtools/pdl.py`) -> Impact: **184.2** | LOC: 123
...
- **Heaviest Functions:** `ProcessReceivedMessage` (Compute Cores, Impact: 79.5), `ExecuteCommandAsync` (Many-Argument Workhorses, Impact: 19.7), `ReceiveMessagesLoopAsync` (Defensive Guards, Impact: 19.6)

### 2. `rb/lib/selenium/webdriver/common/websocket_connection.rb` (RUBY) -> Cumulative Risk: **712.67**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `I/O & Config Routines Files` (z +1.85)
...
- **Heaviest Functions:** `attach_socket_listener` (I/O & Config Routines, Impact: 8.2), `remove_callback` (Callbacks & Closures, Impact: 5.7), `callback_thread` (Callbacks & Closures, Impact: 5.0)
...
- **Heaviest Functions:** `SetFilesAsync` (Generic / Templated Code, Impact: 5.2), `PerformActionsAsync` (Generic / Templated Code, Impact: 4.8), `ReleaseActionsAsync` (Generic / Templated Code, Impact: 4.3)
...
- **Heaviest Functions:** `SubscribeAsync` (Generic / Templated Code, Impact: 7.9), `ProcessEventsAwaiterAsync` (Interface Declarations, Impact: 7.2), `InvokeHandlerAsync` (Defensive Guards, Impact: 4.2)

### 5. `java/src/org/openqa/selenium/bidi/network/FetchTimingInfo.java` (JAVA) -> Cumulative Risk: **678.6**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z -0.46)
...
- **Heaviest Functions:** `fromJson` (Compute Cores, Impact: 48.5), `FetchTimingInfo` (Many-Argument Workhorses, Impact: 5.1), `getTimeOrigin` (Interface Declarations, Impact: 1.1)
...
- **Heaviest Functions:** `ContinueRequestAsync` (Defensive Guards, Impact: 14.3), `ContinueResponseAsync` (Defensive Guards, Impact: 14.3), `ProvideResponseAsync` (Defensive Guards, Impact: 14.3)
...
- **Heaviest Functions:** `__init__` (Defensive Guards, Impact: 12.1), `execute` (Compute Cores, Impact: 11.6), `remove_callback` (Compute Cores, Impact: 8.3)
...
- **Heaviest Functions:** `from_json` (Defensive Guards, Impact: 44.3), `from_json` (Defensive Guards, Impact: 29.5), `set_viewport` (Many-Argument Workhorses, Impact: 28.8)
...
- **Heaviest Functions:** `fromJson` (Compute Cores, Impact: 44.7), `ResponseData` (Many-Argument Workhorses, Impact: 4.9), `getHeadersSize` (Annotated Framework Methods, Impact: 1.2)
...
- **Primary Risk Drivers:** Cognitive Load (100.0%), Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `PrintAsync` (Defensive Guards, Impact: 18.3), `LocateNodesAsync` (Defensive Guards, Impact: 11.5), `SetViewportAsync` (Defensive Guards, Impact: 10.7)
...
**Top Internal Functions/Classes:**
  * `InputManager::GetKeyInfo` **(Compute Cores)** (Impact: 231.7)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 27.7)
  ...
  * `start_devtools` **(Compute Cores)** (Impact: 20.3)
...
**Top Internal Functions/Classes:**
  ...
  * `and` **(Generic / Templated Code)** (Impact: 11.4)
    * *Intent:* /** * An expectation with the logical and condition of the given list of conditions. * * <p>Each con...
  * `resultAsString` **(Defensive Guards)** (Impact: 10.2)
  * `elementSelectionStateToBe` **(Defensive Guards)** (Impact: 10.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `whenAllThrow` **(Tests & Verification)** (Impact: 13.3)
  * `fails_ifAtLeastOneElementIsVisible` **(Tests & Verification)** (Impact: 11.8)
  * `urlToBe_negative` **(Tests & Verification)** (Impact: 11.6)
  * `urlMatches_negative` **(Tests & Verification)** (Impact: 11.6)
  * `whenAllFailed` **(Tests & Verification)** (Impact: 11.1)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `IECommandExecutor::DispatchCommand` **(Compute Cores)** (Impact: 38.9)
...
**Top Internal Functions/Classes:**
  * `BrowserFactory::AttachToBrowserUsingActiveAccessibility` **(Compute Cores)** (Impact: 56.7)
  * `BrowserFactory::AttachToBrowserUsingShellWindows` **(Compute Cores)** (Impact: 42.4)
  * `BrowserFactory::LaunchBrowserProcess` **(Compute Cores)** (Impact: 34.0)
  * `BrowserFactory::GetZoomLevel` **(Compute Cores)** (Impact: 33.6)
...
**Top Internal Functions/Classes:**
  ...
  * `SendKeysCommandHandler::LegacySendKeysToFileUploadAlert` **(Compute Cores)** (Impact: 30.5)
**Contextual Mitigations & Amplifications:**
