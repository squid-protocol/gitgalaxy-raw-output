...
> **File Composition:** Data / Markup / Trivial 33%, Large Core Modules 16%, State Mutators Files 11%, Callbacks & Closures Files 11%, Declarative / Non-Code 7%
...
| connectivity | 79919 | 4600 | 16 | `src/syscall/types_windows.go` |
...
| threat | 11855 | 1493 | 2 | `src/reflect/value.go` |
...
- `TestDirectoryJunction` **(Compute Cores)** (@ `src/os/os_windows_test.go`) -> Impact: **571.7** | LOC: 2044
...
- `regalloc` **(Compute Cores)** (@ `src/cmd/compile/internal/ssa/regalloc.go`) -> Impact: **513.2** | LOC: 1242
...
- `parseSpans` **(Compute Cores)** (@ `src/go/doc/comment/parse.go`) -> Impact: **383.0** | LOC: 901
...
- **Composition Archetype:** `Generic / Templated Code Files` (z +1.45)
...
- **Heaviest Functions:** `Unknown_Block` (Compute Cores, Impact: 49.0), `Unknown_Block` (Type Conversions, Impact: 4.8), `Unknown_Block` (Type Conversions, Impact: 4.8)
...
- **Heaviest Functions:** `Unknown_Block` (Compute Cores, Impact: 10.9), `Unknown_Block` (Compute Cores, Impact: 10.9), `Unknown_Block` (Compute Cores, Impact: 10.9)
...
- **Heaviest Functions:** `runList` (Many-Argument Workhorses, Impact: 279.8), `collectDepsErrors` (Compute Cores, Impact: 14.6), `collectDeps` (Compute Cores, Impact: 6.5)
...
- **Heaviest Functions:** `switches0` (Compute Cores, Impact: 74.8), `rangeloops1` (Compute Cores, Impact: 36.7), `switches1` (Compute Cores, Impact: 33.0)
...
- **Heaviest Functions:** `lookupIP` (Many-Argument Workhorses, Impact: 46.6), `lookupPort` (Many-Argument Workhorses, Impact: 41.1), `lookupSRV` (Many-Argument Workhorses, Impact: 19.4)
...
- **Heaviest Functions:** `InstallPackages` (Many-Argument Workhorses, Impact: 81.4), `runBuild` (Many-Argument Workhorses, Impact: 59.0), `runInstall` (Many-Argument Workhorses, Impact: 30.1)
...
- **Heaviest Functions:** `propagateCancel` (Compute Cores, Impact: 25.3), `value` (Compute Cores, Impact: 24.1), `cancel` (Many-Argument Workhorses, Impact: 17.6)

### 8. `src/internal/syscall/windows/zsyscall_windows.go` (GO) -> Cumulative Risk: **716.13**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +3.77)
...
- **Heaviest Functions:** `adjustTokenPrivileges` (Type Conversions, Impact: 8.5), `WSAGetOverlappedResult` (Type Conversions, Impact: 7.9), `OpenThreadToken` (Type Conversions, Impact: 7.3)

### 9. `src/cmd/covdata/subtractintersect.go` (GO) -> Cumulative Risk: **697.71**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z -1.11)
...
- **Heaviest Functions:** `VisitFuncCounterData` (Compute Cores, Impact: 17.3), `BeginCounterDataFile` (Many-Argument Workhorses, Impact: 15.3), `Setup` (Callbacks & Closures, Impact: 8.6)
...
- **Heaviest Functions:** `testRaceRead` (Defensive Guards, Impact: 17.7), `TestNoRaceCase` (State Mutators, Impact: 9.2), `TestRaceRange` (Compute Cores, Impact: 8.3)
...
**Top Internal Functions/Classes:**
  ...
  * `stmt` **(Compute Cores)** (Impact: 252.5)
