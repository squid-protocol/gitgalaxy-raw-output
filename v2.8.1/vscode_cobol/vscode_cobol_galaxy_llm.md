...
> **File Composition:** Data / Markup / Trivial 49%, Large Core Modules 19%, Declarative / Non-Code 8%, Many-Argument Workhorses Files 6%, Interface Declarations Files 6%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 8.7 | 0.0 | 0.0 |
...
- `get` **(Compute Cores)** (@ `src/sourceformat.ts`) -> Impact: **104.0** | LOC: 174
- `activate` **(Compute Cores)** (@ `src/extension.ts`) -> Impact: **101.8** | LOC: 424
- `provideDocumentSymbols` **(Compute Cores)** (@ `src/vssymbolprovider.ts`) -> Impact: **94.3** | LOC: 154
  * *Intent:* // eslint-disable-next-line @typescript-eslint/no-unused-vars
...
- **Heaviest Functions:** `activateCommonCommands` (Compute Cores, Impact: 126.2), `checkExtensions` (Compute Cores, Impact: 68.7), `getExtensionInformation` (Compute Cores, Impact: 37.4)
...
- **Heaviest Functions:** `processToken` (Many-Argument Workhorses, Impact: 612.4), `constructor` (Many-Argument Workhorses, Impact: 280.2), `newCOBOLToken` (Many-Argument Workhorses, Impact: 87.9)
...
- **Heaviest Functions:** `getCopybookdirs_defaults` (Many-Argument Workhorses, Impact: 53.5), `initSettings` (Many-Argument Workhorses, Impact: 19.5), `getConfigValue` (Compute Cores, Impact: 17.7)

### 4. `src/vscodesourcehandler.ts` (TYPESCRIPT) -> Cumulative Risk: **709.38**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.73)
...
- **Heaviest Functions:** `getLine` (Compute Cores, Impact: 41.3), `sendCommentCallback` (Many-Argument Workhorses, Impact: 35.2), `getCommentAtLine` (Compute Cores, Impact: 13.2)
...
- **Heaviest Functions:** `foldTokenLine` (Many-Argument Workhorses, Impact: 87.2), `setupFilePaths` (Compute Cores, Impact: 61.6), `enforceFileExtensions` (Many-Argument Workhorses, Impact: 52.2)
...
- **Heaviest Functions:** `addExtension` (Compute Cores, Impact: 74.8), `constructor` (Compute Cores, Impact: 25.3), `getItemsFromMap` (Compute Cores, Impact: 23.7)
...
- **Heaviest Functions:** `activate` (Compute Cores, Impact: 101.8), `handleScopedChange` (Compute Cores, Impact: 70.9), `validateInput` (Compute Cores, Impact: 35.5)
...
- **Heaviest Functions:** `processScannedDocumentForUnusedSymbols` (Many-Argument Workhorses, Impact: 59.7), `updateLinter` (Compute Cores, Impact: 50.6), `findCopyBookDirectory` (Many-Argument Workhorses, Impact: 40.9)
...
- **Heaviest Functions:** `executeTab` (Many-Argument Workhorses, Impact: 26.3), `singleSelectionUnTab` (Many-Argument Workhorses, Impact: 25.2), `cobolUnTabSize` (Compute Cores, Impact: 11.6)
...
- **Heaviest Functions:** `generate_partial_graph` (Many-Argument Workhorses, Impact: 28.6), `render` (Many-Argument Workhorses, Impact: 8.6), `_getWebviewContent` (Many-Argument Workhorses, Impact: 5.5)
...
**Top Internal Functions/Classes:**
  ...
  * `checkExtensions` **(Compute Cores)** (Impact: 68.7)
...
**Top Internal Functions/Classes:**
  ...
  * `handleScopedChange` **(Compute Cores)** (Impact: 70.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getConfigValue` **(Compute Cores)** (Impact: 17.7)
  * `get_resource_settings_via_uri` **(Many-Argument Workhorses)** (Impact: 9.4)
  * `isOutlineEnabled` **(Interface Declarations)** (Impact: 8.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getConstantsOrVariables` **(Many-Argument Workhorses)** (Impact: 21.6)
  * `getAllConstantsOrVariables` **(Many-Argument Workhorses)** (Impact: 21.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `getExactFunctionSnipetOrPartialSnippet` **(Many-Argument Workhorses)** (Impact: 27.8)
  * `getExactDollorOrPartial` **(Many-Argument Workhorses)** (Impact: 27.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Compute Cores)** (Impact: 25.3)
  * `getItemsFromMap` **(Compute Cores)** (Impact: 23.7)
  * `setupSourceViewTree` **(Defensive Guards)** (Impact: 17.3)
  * `addExtensionIfInList` **(Many-Argument Workhorses)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `parseField` **(Compute Cores)** (Impact: 59.4)
  * `parseBms` **(Compute Cores)** (Impact: 39.6)
  * `constructor` **(Callbacks & Closures)** (Impact: 14.1)
...
**Top Internal Functions/Classes:**
  * `getLine` **(Compute Cores)** (Impact: 39.8)
  ...
  * `findShortWorkspaceFilename` **(Compute Cores)** (Impact: 17.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `provideDocumentSymbols` **(Compute Cores)** (Impact: 94.3)
    * *Intent:* // eslint-disable-next-line @typescript-eslint/no-unused-vars
  * `provideDocumentSymbols` **(Compute Cores)** (Impact: 28.1)
    * *Intent:* // eslint-disable-next-line @typescript-eslint/no-unused-vars
  * `provideDocumentSymbols` **(Compute Cores)** (Impact: 24.0)
    * *Intent:* // eslint-disable-next-line @typescript-eslint/no-unused-vars
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `removeAllProgramSymbols` **(Compute Cores)** (Impact: 14.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getScanData` **(Many-Argument Workhorses)** (Impact: 12.6)
  * `getCobScannerDirectory` **(Interface Declarations)** (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `extractCopyBookFilename` **(Compute Cores)** (Impact: 23.5)
  * `getURIForCopybookInDirectory` **(Defensive Guards)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `getShortWorkspaceFilename` **(Many-Argument Workhorses)** (Impact: 23.5)
  * `_findCopyBookInDirectory` **(Many-Argument Workhorses)** (Impact: 17.3)
  * `_findCopyBookInDirectoryViaURL` **(Many-Argument Workhorses)** (Impact: 17.2)
...
**Top Internal Functions/Classes:**
  * `get` **(Compute Cores)** (Impact: 104.0)
  * `isValidFixedLine` **(Compute Cores)** (Impact: 12.0)
  * `getFileFormat` **(Defensive Guards)** (Impact: 8.0)
  * `isNumber` **(Interface Declarations)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `updateJCLDecorations` **(State Mutators)** (Impact: 6.8)
  * `isEnabledViaWorkspace4jcl` **(Interface Declarations)** (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  ...
  * `processFileShowFooter` **(Many-Argument Workhorses)** (Impact: 15.1)
  * `cacheUpdateRequired` **(Defensive Guards)** (Impact: 9.7)
  * `processFileShowHeader` **(Compute Cores)** (Impact: 7.5)
  * `sendMessage` **(Interface Declarations)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 16 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_getWebviewContent` **(Many-Argument Workhorses)** (Impact: 5.5)
  * `get_programWindowState` **(Compute Cores)** (Impact: 4.0)
  * `constructor` **(Callbacks & Closures)** (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
