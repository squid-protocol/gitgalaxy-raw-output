...
> **File Composition:** Interface Declarations Files 34%, Data / Markup / Trivial 19%, Large Core Modules 19%, Defensive Guards Files 7%, Declarative / Non-Code 6%
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 36.4 | 27.5 | 0.0 |
...
| credential | 0 | 0 | 0 | - |
| threat | 21 | 17 | 1 | `src/Handler/CurlMultiHandler.php` |
...
- `GuzzleServer` **(Compute Cores)** (@ `tests/server.js`) -> Impact: **102.1** | LOC: 206
  * *Intent:* /** * Guzzle node.js server * @class */
...
### 1. `src/Cookie/CookieJar.php` (PHP) -> Cumulative Risk: **580.11**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Compute Cores Files` (z +1.40)
- **Magnitude:** 220.1 | **LOC:** 308 | **CtrlFlow:** 27.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Churn (100.0%), State Flux (99.9998%), Safety Score (91.8795%)
- **Heaviest Functions:** `setCookie` (Compute Cores, Impact: 27.0), `clear` (Compute Cores, Impact: 23.6), `withCookieHeader` (Compute Cores, Impact: 13.9)

### 2. `src/RedirectMiddleware.php` (PHP) -> Cumulative Risk: **502.97**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Churn (100.0%), Safety Score (91.352%)
- **Heaviest Functions:** `modifyRequest` (Many-Argument Workhorses, Impact: 26.4), `checkRedirect` (Many-Argument Workhorses, Impact: 16.0), `__invoke` (Defensive Guards, Impact: 11.7)

### 3. `src/Middleware.php` (PHP) -> Cumulative Risk: **487.17**
...
- **Primary Risk Drivers:** Spec Match (100.0%), Churn (100.0%), State Flux (99.7628%), Tech Debt (98.6851%)
- **Heaviest Functions:** `tap` (Callbacks & Closures, Impact: 9.5), `log` (Defensive Guards, Impact: 9.3), `httpErrors` (Callbacks & Closures, Impact: 6.7)

### 4. `src/Handler/CurlMultiHandler.php` (PHP) -> Cumulative Risk: **478.28**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.19)
- **Magnitude:** 143.4 | **LOC:** 288 | **CtrlFlow:** 19.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (95.0478%), Safety Score (88.9157%)
- **Heaviest Functions:** `__construct` (Defensive Guards, Impact: 9.4), `tick` (I/O & Config Routines, Impact: 8.8), `processMessages` (I/O & Config Routines, Impact: 7.2)

### 5. `src/PrepareBodyMiddleware.php` (PHP) -> Cumulative Risk: **472.82**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.68)
- **Magnitude:** 88.82 | **LOC:** 106 | **CtrlFlow:** 29.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (96.3529%), Tech Debt (83.1789%)
- **Heaviest Functions:** `addExpectHeader` (Many-Argument Workhorses, Impact: 21.8), `__invoke` (Compute Cores, Impact: 19.2), `__construct` (Interface Declarations, Impact: 1.6)

### 6. `src/Utils.php` (PHP) -> Cumulative Risk: **467.52**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (98.8145%), Safety Score (93.8734%)
- **Heaviest Functions:** `isHostInNoProxy` (Compute Cores, Impact: 15.6), `idnUriConvert` (Compute Cores, Impact: 15.5), `chooseHandler` (Interface Declarations, Impact: 12.2)

### 7. `src/Handler/CurlFactory.php` (PHP) -> Cumulative Risk: **464.84**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z -0.31)
- **Magnitude:** 670.24 | **LOC:** 742 | **CtrlFlow:** 25.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (94.8924%), Documentation (70.3704%)
- **Heaviest Functions:** `applyHandlerOptions` (Many-Argument Workhorses, Impact: 122.4), `applyBody` (Many-Argument Workhorses, Impact: 21.9), `createRejection` (Many-Argument Workhorses, Impact: 20.4)

### 8. `src/Pool.php` (PHP) -> Cumulative Risk: **454.01**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.27)
- **Magnitude:** 69.08 | **LOC:** 126 | **CtrlFlow:** 14.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (89.3649%), Tech Debt (80.3174%)
- **Heaviest Functions:** `__construct` (Defensive Guards, Impact: 17.4), `cmpCallback` (Callbacks & Closures, Impact: 6.7), `batch` (Parameter Forwarders, Impact: 2.5)

### 9. `src/Handler/MockHandler.php` (PHP) -> Cumulative Risk: **448.75**
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (96.4904%), Safety Score (81.8474%)
- **Heaviest Functions:** `__invoke` (Defensive Guards, Impact: 31.0), `append` (Defensive Guards, Impact: 10.6), `__construct` (Compute Cores, Impact: 10.5)

### 10. `src/HandlerStack.php` (PHP) -> Cumulative Risk: **440.5**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.31)
- **Magnitude:** 186.46 | **LOC:** 276 | **CtrlFlow:** 16.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (98.4378%), Safety Score (98.0898%)
- **Heaviest Functions:** `splice` (Many-Argument Workhorses, Impact: 14.4), `remove` (Callbacks & Closures, Impact: 6.4), `debugCallable` (Compute Cores, Impact: 6.4)
...
- **Magnitude:** 670.24 | **LOC:** 742 | **CtrlFlow:** 25.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (60.9797%), Tech Debt (29.2308%)
**Top Internal Functions/Classes:**
  * `applyHandlerOptions` **(Many-Argument Workhorses)** (Impact: 122.4)
  * `applyBody` **(Many-Argument Workhorses)** (Impact: 21.9)
  * `createRejection` **(Many-Argument Workhorses)** (Impact: 20.4)
  * `retryFailedRewind` **(Defensive Guards)** (Impact: 17.8)
...
- **Risk Profile:** Cognitive Load (45.2393%), Tech Debt (34.802%)
**Top Internal Functions/Classes:**
  ...
  * `add_proxy` **(Many-Argument Workhorses)** (Impact: 21.6)
    * *Intent:* /** * @param mixed $value as passed via Request transfer options. */
  * `resolveHost` **(Defensive Guards)** (Impact: 20.3)
  * `createResponse` **(Defensive Guards)** (Impact: 18.1)
...
- **Risk Profile:** Cognitive Load (27.4793%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `testCreatesCurlHandle` **(I/O & Config Routines)** (Impact: 8.0)
  * `checkNoProxyForHost` **(Many-Argument Workhorses)** (Impact: 6.8)
  * `addDecodeResponse` **(Interface Declarations)** (Impact: 3.5)
  * `testBodyEofOnWindows` **(Interface Declarations)** (Impact: 3.2)
    * *Intent:* /** * https://github.com/guzzle/guzzle/issues/2735 */
  * `testCreatesConnectException` **(Interface Declarations)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 3 instances
...
**Top Internal Functions/Classes:**
  ...
  * `testIdnWithRedirect` **(Tests & Verification)** (Impact: 2.4)
    * *Intent:* /** * @requires extension idn */
  * `testCanMergeOnBaseUriWithRequest` **(Interface Declarations)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 3 instances
...
- **Risk Profile:** Cognitive Load (23.4795%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `testStreamAttributeKeepsStreamOpen` **(Tests & Verification)** (Impact: 4.3)
  * `testAutomaticallyDecompressGzip` **(Tests & Verification)** (Impact: 2.9)
  * `testAutomaticallyDecompressGzipHead` **(Interface Declarations)** (Impact: 2.9)
  * `testHandlesGarbageHttpServerGracefully` **(Interface Declarations)** (Impact: 2.9)
...
- **Risk Profile:** Cognitive Load (36.9087%), Tech Debt (62.5029%)
**Top Internal Functions/Classes:**
  * `applyOptions` **(Many-Argument Workhorses)** (Impact: 61.9)
    * *Intent:* /** * Applies the array of request options to a request. */
  * `configureDefaults` **(Compute Cores)** (Impact: 18.0)
    * *Intent:* /** * Configures the default options for a client. */
  * `buildUri` **(Defensive Guards)** (Impact: 12.8)
  * `prepareDefaults` **(Compute Cores)** (Impact: 11.6)
...
- **Risk Profile:** Cognitive Load (22.6567%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (22.4493%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `GuzzleServer` **(Compute Cores)** (Impact: 102.1)
    * *Intent:* /** * Guzzle node.js server * @class */
...
- **Risk Profile:** Cognitive Load (34.2511%), Tech Debt (85.509%)
**Top Internal Functions/Classes:**
  * `fromString` **(Compute Cores)** (Impact: 26.3)
  ...
  * `validate` **(I/O & Config Routines)** (Impact: 8.7)
...
- **Risk Profile:** Cognitive Load (43.3629%), Tech Debt (35.9641%)
**Top Internal Functions/Classes:**
  ...
  * `headers` **(Interface Declarations)** (Impact: 3.3)
...
- **Magnitude:** 226.5 | **LOC:** 385 | **CtrlFlow:** 25.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (36.6033%), Tech Debt (98.8145%)
**Top Internal Functions/Classes:**
  ...
  * `chooseHandler` **(Interface Declarations)** (Impact: 12.2)
    * *Intent:* /** * Chooses and creates a default handler to use based on the environment. * * The returned handle...
  * `getenv` **(Defensive Guards)** (Impact: 9.1)
  * `idnToAsci` **(Compute Cores)** (Impact: 8.4)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 220.1 | **LOC:** 308 | **CtrlFlow:** 27.6% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (39.346%), Tech Debt (88.3274%)
**Top Internal Functions/Classes:**
  ...
  * `getCookiePathFromRequest` **(Compute Cores)** (Impact: 9.4)
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 186.46 | **LOC:** 276 | **CtrlFlow:** 16.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (34.3186%), Tech Debt (98.4378%)
**Top Internal Functions/Classes:**
  * `splice` **(Many-Argument Workhorses)** (Impact: 14.4)
  ...
  * `debugCallable` **(Compute Cores)** (Impact: 6.4)
  ...
  * `resolve` **(Interface Declarations)** (Impact: 4.8)
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 158.18 | **LOC:** 213 | **CtrlFlow:** 24.8% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (37.9507%), Tech Debt (96.4904%)
**Top Internal Functions/Classes:**
  ...
  * `invokeStats` **(Defensive Guards)** (Impact: 9.5)
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 156.88 | **LOC:** 229 | **CtrlFlow:** 16.8% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (34.5561%), Tech Debt (30.5308%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (5.3397%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `testReturnsCookiesMatchingRequests` **(Compute Cores)** (Impact: 6.0)
    * *Intent:* /** * @dataProvider getMatchingCookiesDataProvider */
  * `testIgnoresCookiesForMismatchingDomains` **(Many-Argument Workhorses)** (Impact: 4.7)
    * *Intent:* /** * @dataProvider getDomainMatchesProvider */
  * `testDeletesCookiesByName` **(Interface Declarations)** (Impact: 3.1)
  * `testRemovesSelectively` **(I/O & Config Routines)** (Impact: 3.0)
  * `testOverwritesCookiesThatAreOlderOrDiscardable` **(Tests & Verification)** (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 143.4 | **LOC:** 288 | **CtrlFlow:** 19.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (37.8784%), Tech Debt (95.0478%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `testInvokesOnStatsFunctionForError` **(Tests & Verification)** (Impact: 2.0)
  * `testRejectsPromiseWhenOnHeadersFails` **(Interface Declarations)** (Impact: 1.8)
  * `testInvokesOnStatsFunctionForResponse` **(Tests & Verification)** (Impact: 1.8)
...
- **Risk Profile:** Cognitive Load (32.6965%), Tech Debt (99.7583%)
**Top Internal Functions/Classes:**
...
**Top Internal Functions/Classes:**
  ...
  * `testTracksHistory` **(Tests & Verification)** (Impact: 2.3)
    * *Intent:* /** * @dataProvider getHistoryUseCases */
  * `testAddsCookiesToRequests` **(Interface Declarations)** (Impact: 2.0)
...
- **Risk Profile:** Cognitive Load (35.3306%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `getClient` **(Interface Declarations)** (Impact: 3.3)
  ...
  * `testBatchesResults` **(Tests & Verification)** (Impact: 2.1)
...
- **Risk Profile:** Cognitive Load (4.2181%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  ...
  * `testHoldsValues` **(Tests & Verification)** (Impact: 2.5)
  * `testMatchesDomain` **(Tests & Verification)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 91.68 | **LOC:** 269 | **CtrlFlow:** 9.7% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (14.4926%), Tech Debt (98.6851%)
**Top Internal Functions/Classes:**
  * `tap` **(Callbacks & Closures)** (Impact: 9.5)
  ...
  * `cookies` **(Interface Declarations)** (Impact: 4.2)
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 88.82 | **LOC:** 106 | **CtrlFlow:** 29.5% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (40.9397%), Tech Debt (83.1789%)
**Top Internal Functions/Classes:**
  * `addExpectHeader` **(Many-Argument Workhorses)** (Impact: 21.8)
    * *Intent:* /** * Add expect header */
  * `__invoke` **(Compute Cores)** (Impact: 19.2)
  * `__construct` **(Interface Declarations)** (Impact: 1.6)
