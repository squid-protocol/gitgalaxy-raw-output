...
> **Composition Archetype:** `Small Flat Repo` (z -0.56; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 46%, Data / Markup / Trivial 19%, Large Core Modules 10%, Interface Declarations Files 8%, Callbacks & Closures Files 7%
...
- `onSlid2` **(Tests & Verification)** (@ `js/tests/unit/carousel.spec.js`) -> Impact: **63.0** | LOC: 779
- `expectDone` **(Tests & Verification)** (@ `js/tests/unit/tab.spec.js`) -> Impact: **61.2** | LOC: 904
...
- `getActiveId` **(Tests & Verification)** (@ `js/tests/unit/carousel.spec.js`) -> Impact: **30.8** | LOC: 416
- `off` **(Defensive Guards)** (@ `js/src/dom/event-handler.js`) -> Impact: **28.6** | LOC: 35
- `el` **(Tests & Verification)** (@ `js/tests/unit/util/index.spec.js`) -> Impact: **28.3** | LOC: 199
- `expectedDone` **(Tests & Verification)** (@ `js/tests/unit/tooltip.spec.js`) -> Impact: **26.4** | LOC: 347
- `clearMenus` **(Defensive Guards)** (@ `js/src/dropdown.js`) -> Impact: **25.9** | LOC: 37
- `restorePointerEvents` **(Tests & Verification)** (@ `js/tests/unit/carousel.spec.js`) -> Impact: **24.6** | LOC: 272
...
- **Heaviest Functions:** `focusInListener` (Tests & Verification, Impact: 4.5), `focusInListener` (Callbacks & Closures, Impact: 3.5), `focusInListener` (Callbacks & Closures, Impact: 3.2)
...
- **Heaviest Functions:** `getSelector` (Defensive Guards, Impact: 16.9), `prev` (Compute Cores, Impact: 5.8), `next` (Compute Cores, Impact: 5.8)

### 3. `js/tests/helpers/fixture.js` (JAVASCRIPT) -> Cumulative Risk: **508.45**
- **Archetype:** `file_cluster_5` (Distance: N/A IQR)
- **Composition Archetype:** `Callbacks & Closures Files` (z +0.93)
...
- **Heaviest Functions:** `each` (State Mutators, Impact: 3.1), `getFixture` (Interface Declarations, Impact: 2.9), `clearBodyAndDocument` (Callbacks & Closures, Impact: 2.4)

### 4. `js/src/tooltip.js` (JAVASCRIPT) -> Cumulative Risk: **497.45**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Large Core Modules` (z +0.23)
...
- **Heaviest Functions:** `show` (I/O & Config Routines, Impact: 13.8), `_setListeners` (Defensive Guards, Impact: 13.1), `hide` (I/O & Config Routines, Impact: 9.1)

### 5. `js/src/carousel.js` (JAVASCRIPT) -> Cumulative Risk: **492.24**
...
- **Heaviest Functions:** `_slide` (Many-Argument Workhorses, Impact: 20.9), `triggerEvent` (Compute Cores, Impact: 10.5), `to` (Defensive Guards, Impact: 9.5)
...
- **Heaviest Functions:** `_observerCallback` (Compute Cores, Impact: 14.6), `activate` (Compute Cores, Impact: 14.5), `jQueryInterface` (Defensive Guards, Impact: 7.8)

### 7. `js/src/base-component.js` (JAVASCRIPT) -> Cumulative Risk: **473.99**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +0.53)
...
- **Heaviest Functions:** `getOrCreateInstance` (Defensive Guards, Impact: 5.3), `constructor` (Compute Cores, Impact: 4.1), `dispose` (State Mutators, Impact: 2.4)

### 8. `js/tests/unit/util/index.spec.js` (JAVASCRIPT) -> Cumulative Risk: **473.45**
- **Archetype:** `file_cluster_10` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z +1.03)
...
- **Heaviest Functions:** `el` (Tests & Verification, Impact: 28.3), `functionBar` (Tests & Verification, Impact: 20.6), `el` (Tests & Verification, Impact: 3.9)
...
- **Heaviest Functions:** `addHandler` (Many-Argument Workhorses, Impact: 34.0), `off` (Defensive Guards, Impact: 28.6), `trigger` (Many-Argument Workhorses, Impact: 19.9)
...
- **Heaviest Functions:** `_onInteraction` (Compute Cores, Impact: 18.9), `jQueryInterface` (Defensive Guards, Impact: 4.9), `_maybeScheduleHide` (Callbacks & Closures, Impact: 4.7)
...
**Top Internal Functions/Classes:**
  * `test` **(Callbacks & Closures)** (Impact: 18.2)
  * `expectDropdownToBeOpened` **(Tests & Verification)** (Impact: 13.3)
  * `handleArrowUp` **(Tests & Verification)** (Impact: 6.3)
...
**Top Internal Functions/Classes:**
  * `expectedDone` **(Tests & Verification)** (Impact: 26.4)
  * `assertDone` **(Tests & Verification)** (Impact: 14.3)
  * `tip` **(Tests & Verification)** (Impact: 13.6)
  * `firstCallback` **(Tests & Verification)** (Impact: 9.1)
  * `title` **(Tests & Verification)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `onSlid2` **(Tests & Verification)** (Impact: 63.0)
  * `getActiveId` **(Tests & Verification)** (Impact: 30.8)
  * `restorePointerEvents` **(Tests & Verification)** (Impact: 24.6)
...
**Top Internal Functions/Classes:**
  ...
  * `showListener` **(Tests & Verification)** (Impact: 12.3)
  * `hideCallback` **(Tests & Verification)** (Impact: 10.9)
...
**Top Internal Functions/Classes:**
  * `expectDone` **(Tests & Verification)** (Impact: 61.2)
...
**Top Internal Functions/Classes:**
  * `expectEnd` **(Tests & Verification)** (Impact: 22.6)
  * `expectEnd` **(Tests & Verification)** (Impact: 10.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `expectEnd` **(Tests & Verification)** (Impact: 17.6)
  * `expectEnd` **(Callbacks & Closures)** (Impact: 9.1)
  * `target2Shown` **(Tests & Verification)** (Impact: 9.1)
  * `el` **(Callbacks & Closures)** (Impact: 7.3)
  * `handlerNestedCollapseOne` **(Tests & Verification)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `testElementIsActiveAfterScroll` **(Tests & Verification)** (Impact: 12.9)
  * `active` **(Callbacks & Closures)** (Impact: 4.3)
  * `activeId` **(Tests & Verification)** (Impact: 3.6)
  * `getElementScrollSpy` **(Interface Declarations)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `expected` **(Tests & Verification)** (Impact: 8.8)
...
**Top Internal Functions/Classes:**
  ...
  * `handler` **(Tests & Verification)** (Impact: 4.1)
...
**Top Internal Functions/Classes:**
  * `el` **(Tests & Verification)** (Impact: 28.3)
  * `functionBar` **(Tests & Verification)** (Impact: 20.6)
  * `el` **(Tests & Verification)** (Impact: 3.9)
  * `functionFoo` **(Tests & Verification)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 21 instances
...
**Top Internal Functions/Classes:**
  * `_addEventListeners` **(Callbacks & Closures)** (Impact: 10.9)
...
**Top Internal Functions/Classes:**
  ...
  * `title` **(Interface Declarations)** (Impact: 1.6)
  * `content` **(Interface Declarations)** (Impact: 1.6)
...
**Top Internal Functions/Classes:**
  * `isScrollBarHidden` **(Tests & Verification)** (Impact: 23.4)
    * *Intent:* // iOS, Android devices and macOS browsers hide scrollbar by default and show it only while scrollin...
  * `hasMarginAttr` **(Tests & Verification)** (Impact: 7.8)
  * `windowCalculations` **(Interface Declarations)** (Impact: 1.8)
  * `parseIntDecimal` **(Interface Declarations)** (Impact: 1.5)
  * `getPaddingX` **(Interface Declarations)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 16 instances
...
**Top Internal Functions/Classes:**
  * `_keydown` **(Compute Cores)** (Impact: 9.6)
  * `jQueryInterface` **(Defensive Guards)** (Impact: 7.8)
    * *Intent:* // Static
