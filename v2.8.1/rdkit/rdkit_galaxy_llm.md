...
> **File Composition:** Large Core Modules 23%, Data / Markup / Trivial 19%, Declarative / Non-Code 13%, Interface Declarations Files 12%, Many-Argument Workhorses Files 8%
...
| connectivity | 7569 | 964 | 9 | `Code/GraphMol/Wrap/rough_test.py` |
...
| dead_code | 7607 | 1189 | 8 | `Code/GraphMol/Wrap/rough_test.py` |
...
- **Heaviest Functions:** `buildProximityGraph` (Many-Argument Workhorses, Impact: 25.8), `formInitialClusters` (Many-Argument Workhorses, Impact: 17.8), `makeSubClusters` (Compute Cores, Impact: 16.0)
...
- **Heaviest Functions:** `ResonanceMolSupplier::buildCEMap` (Many-Argument Workhorses, Impact: 95.4), `ConjElectrons::checkChargesAndBondOrders` (Compute Cores, Impact: 50.5), `ConjElectrons::checkMetrics` (Compute Cores, Impact: 31.1)
...
- **Heaviest Functions:** `findBondPairsThatFragment` (Many-Argument Workhorses, Impact: 28.8), `checkConnectorsInDifferentFrags` (Many-Argument Workhorses, Impact: 27.7), `doInitialFragmentation` (Many-Argument Workhorses, Impact: 26.2)
...
- **Heaviest Functions:** `__exit__` (Many-Argument Workhorses, Impact: 14.4), `RunOneThreadPerLevel` (Compute Cores, Impact: 4.7), `RunManyThreadsPerLevel` (Compute Cores, Impact: 4.5)
...
- **Heaviest Functions:** `SDMolSupplier::buildIndexTo` (Compute Cores, Impact: 46.7), `SDMolSupplier::peekCheckForEnd` (Many-Argument Workhorses, Impact: 11.7), `SDMolSupplier::moveTo` (Compute Cores, Impact: 10.3)

### 6. `Code/JavaWrappers/gmwrapper/src-test/org/RDKit/ForceFieldsTests.java` (JAVA) -> Cumulative Risk: **688.06**
- **Archetype:** `file_cluster_0` (Distance: N/A IQR)
- **Composition Archetype:** `Tests & Verification Files` (z -0.82)
...
- **Heaviest Functions:** `testUFFTorsionalTerms` (I/O & Config Routines, Impact: 8.5), `testForUFFAngleBendTerms` (Tests & Verification, Impact: 7.5), `testSimpleUFFMoleculeOptimization` (I/O & Config Routines, Impact: 7.4)
...
- **Heaviest Functions:** `EmbeddedFrag::matchToTemplate` (Many-Argument Workhorses, Impact: 615.2), `checkStereoChemistry` (Many-Argument Workhorses, Impact: 83.2), `EmbeddedFrag::randomSampleFlipsAndPermutations` (Many-Argument Workhorses, Impact: 60.8)
...
- **Heaviest Functions:** `MorganBondInvGenerator::getBondInvariants` (Compute Cores, Impact: 20.5), `getMorganGenerator` (Many-Argument Workhorses, Impact: 8.6), `MorganFeatureAtomInvGenerator::fromJSON` (Compute Cores, Impact: 6.5)

### 9. `Code/GraphMol/Fingerprints/Wrap/testGenerators.py` (PYTHON) -> Cumulative Risk: **683.93**
- **Archetype:** `file_cluster_2` (Distance: N/A IQR)
- **Composition Archetype:** `Interface Declarations Files` (z +1.27)
...
- **Heaviest Functions:** `testMorganGeneratorMultiMol` (Compute Cores, Impact: 23.4), `testMorganGenerator` (Compute Cores, Impact: 5.9), `testNumpyFingerprints` (Compute Cores, Impact: 3.7)
...
- **Heaviest Functions:** `test4RingMatches` (Many-Argument Workhorses, Impact: 49.9), `__call__` (Many-Argument Workhorses, Impact: 32.5), `__call__` (Many-Argument Workhorses, Impact: 24.5)
...
**Top Internal Functions/Classes:**
  * `test25SDMolSupplier` **(Compute Cores)** (Impact: 32.2)
  * `test88QueryAtoms` **(Compute Cores)** (Impact: 27.7)
...
**Top Internal Functions/Classes:**
  ...
  * `MMFFMolProperties::computeMMFFCharges` **(Compute Cores)** (Impact: 168.0)
...
**Top Internal Functions/Classes:**
  * `DrawMol::getAtomSymbol` **(Compute Cores)** (Impact: 112.5)
    * *Intent:* // ****************************************************************************
...
**Top Internal Functions/Classes:**
  ...
  * `MarvinSuperatomSgroup::convertFromOneSuperAtom` **(Compute Cores)** (Impact: 53.7)
  * `MarvinMultipleSgroup::contractOneMultipleSgroup` **(Compute Cores)** (Impact: 49.0)
  * `MarvinSuperatomSgroupExpanded::convertToOneSuperAtom` **(Compute Cores)** (Impact: 48.9)
...
**Top Internal Functions/Classes:**
  ...
  * `AASequenceToMol` **(Compute Cores)** (Impact: 211.8)
...
**Top Internal Functions/Classes:**
  * `TEST_CASE` **(Compute Cores)** (Impact: 54.9)
  * `TEST_CASE` **(Many-Argument Workhorses)** (Impact: 40.5)
  * `TEST_CASE` **(Compute Cores)** (Impact: 29.0)
...
**Top Internal Functions/Classes:**
  ...
  * `TEST_CASE` **(Tests & Verification)** (Impact: 45.8)
  * `TEST_CASE` **(Tests & Verification)** (Impact: 39.3)
  * `TEST_CASE` **(Compute Cores)** (Impact: 32.8)
  * `TEST_CASE` **(Tests & Verification)** (Impact: 30.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `testGithub4114` **(Compute Cores)** (Impact: 17.3)
...
**Top Internal Functions/Classes:**
  ...
  * `check_roundtripped_properties` **(Compute Cores)** (Impact: 30.2)
...
**Top Internal Functions/Classes:**
  ...
  * `TEST_CASE` **(Tests & Verification)** (Impact: 27.3)
  * `TEST_CASE` **(Compute Cores)** (Impact: 25.1)
  * `TEST_CASE` **(Compute Cores)** (Impact: 23.1)
  * `TEST_CASE` **(Tests & Verification)** (Impact: 19.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `GetMarvinAtomInfo` **(Compute Cores)** (Impact: 138.1)
