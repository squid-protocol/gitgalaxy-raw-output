...
> **Composition Archetype:** `Hub-Coupled App` (z +0.61; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 48%, Interface Declarations Files 17%, Data / Markup / Trivial 8%, Compute Cores Files 7%, Declarative / Non-Code 7%
...
- `_channelzSessionHandler` **(Compute Cores)** (@ `package/src/server.ts`) -> Impact: **49.8** | LOC: 204
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 22.9), `sendMessage` (Defensive Guards, Impact: 11.2), `getCompressionHandler` (Compute Cores, Impact: 11.0)
...
- **Heaviest Functions:** `start` (Compute Cores, Impact: 22.4), `constructor` (Many-Argument Workhorses, Impact: 20.8), `onReceiveStatus` (Compute Cores, Impact: 10.9)

### 3. `package/src/filter-stack.ts` (TYPESCRIPT) -> Cumulative Risk: **702.48**
- **Archetype:** `file_cluster_6` (Distance: N/A IQR)
- **Composition Archetype:** `Generic / Templated Code Files` (z +2.29)
- **Magnitude:** 123.32 | **LOC:** 101 | **CtrlFlow:** 8.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `sendMetadata` (Generic / Templated Code, Impact: 3.3), `receiveMetadata` (Interface Declarations, Impact: 3.3), `sendMessage` (Generic / Templated Code, Impact: 3.3)
...
- **Heaviest Functions:** `_channelzSessionHandler` (Compute Cores, Impact: 49.8), `_sessionHandler` (Defensive Guards, Impact: 46.3), `addService` (Compute Cores, Impact: 34.3)
...
- **Heaviest Functions:** `createSession` (Defensive Guards, Impact: 58.9), `constructor` (Many-Argument Workhorses, Impact: 51.0), `createCall` (Many-Argument Workhorses, Impact: 36.8)
...
- **Heaviest Functions:** `createSsl` (Defensive Guards, Impact: 43.8), `getConnectionOptions` (Many-Argument Workhorses, Impact: 31.2), `connect` (Callbacks & Closures, Impact: 13.2)
...
- **Heaviest Functions:** `updateCertificates` (Compute Cores, Impact: 15.0), `constructor` (State Mutators, Impact: 6.2), `maybeStartWatchingFiles` (Callbacks & Closures, Impact: 6.0)
...
- **Heaviest Functions:** `getConfig` (Callbacks & Closures, Impact: 23.6), `runDeadlineTimer` (I/O & Config Routines, Impact: 16.1), `handleDeadline` (I/O & Config Routines, Impact: 14.7)
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 90.8), `mapHttpStatusCode` (Compute Cores, Impact: 17.2), `handleTrailers` (Defensive Guards, Impact: 12.4)
...
- **Heaviest Functions:** `constructor` (Many-Argument Workhorses, Impact: 37.2), `startResolution` (I/O & Config Routines, Impact: 24.8), `updateResolution` (I/O & Config Routines, Impact: 8.2)
...
**Top Internal Functions/Classes:**
  * `_channelzSessionHandler` **(Compute Cores)** (Impact: 49.8)
  * `_sessionHandler` **(Defensive Guards)** (Impact: 46.3)
  * `addService` **(Compute Cores)** (Impact: 34.3)
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Many-Argument Workhorses)** (Impact: 33.6)
  * `handleChildStatus` **(Compute Cores)** (Impact: 25.4)
  * `maybeRetryCall` **(Defensive Guards)** (Impact: 24.4)
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Defensive Guards)** (Impact: 24.8)
  * `sendStatus` **(Compute Cores)** (Impact: 24.1)
  * `start` **(Defensive Guards)** (Impact: 19.5)
  * `decompressMessage` **(Compute Cores)** (Impact: 19.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
**Top Internal Functions/Classes:**
  ...
  * `createFromJson` **(Compute Cores)** (Impact: 25.8)
...
**Top Internal Functions/Classes:**
  ...
  * `checkOptionalUnaryResponseArguments` **(Many-Argument Workhorses)** (Impact: 25.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `handleTrailers` **(Defensive Guards)** (Impact: 12.4)
...
**Top Internal Functions/Classes:**
  * `createSsl` **(Defensive Guards)** (Impact: 43.8)
  ...
  * `_createSecureConnector` **(Callbacks & Closures)** (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Defensive Guards)** (Impact: 21.5)
...
**Top Internal Functions/Classes:**
  ...
  * `GetServerSockets` **(Compute Cores)** (Impact: 12.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `onSubchannelStateUpdate` **(Many-Argument Workhorses)** (Impact: 26.2)
  ...
  * `connectToAddressList` **(Compute Cores)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 21 instances
* *Concurrency (weighted view):* 12
...
**Top Internal Functions/Classes:**
  * `constructor` **(Many-Argument Workhorses)** (Impact: 22.9)
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Many-Argument Workhorses)** (Impact: 14.5)
  ...
  * `onCallEnd` **(Callbacks & Closures)** (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Many-Argument Workhorses)** (Impact: 20.8)
...
**Top Internal Functions/Classes:**
  * `getConfig` **(Callbacks & Closures)** (Impact: 23.6)
  * `runDeadlineTimer` **(I/O & Config Routines)** (Impact: 16.1)
  * `handleDeadline` **(I/O & Config Routines)** (Impact: 14.7)
  * `constructor` **(Many-Argument Workhorses)** (Impact: 11.2)
  * `onReceiveStatus` **(Callbacks & Closures)** (Impact: 11.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
* *Amplified Cascading Flux:* 19 instances
...
**Top Internal Functions/Classes:**
  * `constructor` **(Many-Argument Workhorses)** (Impact: 37.2)
  ...
  * `lookup` **(Callbacks & Closures)** (Impact: 7.3)
  * `handleHealthStatus` **(State Mutators)** (Impact: 4.6)
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Many-Argument Workhorses)** (Impact: 20.6)
  ...
  * `getDefaultConfigSelector` **(Compute Cores)** (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 14 instances
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` **(Defensive Guards)** (Impact: 7.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `outputStatus` **(Defensive Guards)** (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `fromHttp2Headers` **(Compute Cores)** (Impact: 20.6)
