...
| **Timestamp** | `2026-08-07T05:08:30.994357+00:00` |
| **Scan Duration** | `1.66s` |
| **Git Branch** | `master` |
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 0.0 | 99.2 | 22.1 | 5.0 | 5.0 |
| Error & Exception Exposure | 0.0 | 100.0 | 32.9 | 0.0 | 0.0 |
| Tech Debt Exposure | 0.0 | 100.0 | 36.7 | 0.0 | 0.0 |
...
| Documentation Exposure | 0.0 | 100.0 | 32.7 | 15.5 | 0.0 |
...
- `mkchrootbase_[Truncated]` (@ `portmgr/packaging/buildall.sh`) -> Impact: **165.1** | LOC: 190
  * *Intent:* # Everything we need to create the base chroot disk image (populated from host)
- `export_path` (@ `setupenv.bash.in`) -> Impact: **19.2** | LOC: 25
...
- `clean` (@ `doc/Makefile.in`) -> Impact: **16.1** | LOC: 83
- `export_manpath` (@ `setupenv.bash.in`) -> Impact: **16.1** | LOC: 21
...
- `Anonymous_Block` (@ `portmgr/dmg/InstallationCheck`) -> Impact: **5.2** | LOC: 4
- `export_display` (@ `setupenv.bash.in`) -> Impact: **5.2** | LOC: 5
- `install` (@ `doc/Makefile.in`) -> Impact: **4.6** | LOC: 12
...
| `__monolith__` | 8 | 5264.96 | 13.6% | 27.45% |
...
2. **`portmgr/dmg/InstallationCheck`** -> AI Confidence: **99.17%**
3. **`portmgr/packaging/buildall.sh`** -> AI Confidence: **99.17%**
4. **`setupenv.bash.in`** -> AI Confidence: **99.17%**
5. **`Mk/macports.tea.mk`** -> AI Confidence: **99.06%**
6. **`doc/Makefile.in`** -> AI Confidence: **98.96%**
...
### 1. `portmgr/packaging/buildall.sh` (SHELL) -> Cumulative Risk: **575.67**
- **Archetype:** `file_cluster_12` (Distance: 12.47 IQR)
- **Magnitude:** 256.9 | **LOC:** 216 | **CtrlFlow:** 76.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (99.9699%), Cognitive Load (99.1656%)
- **Heaviest Functions:** `mkchrootbase_[Truncated]` (Impact: 165.1), `bomb` (Impact: 1.7), `__global_context__` (Impact: 1.5)

### 2. `portmgr/dmg/InstallationCheck` (SHELL) -> Cumulative Risk: **550.32**
- **Archetype:** `file_cluster_8` (Distance: 10.836 IQR)
- **Magnitude:** 24.8 | **LOC:** 32 | **CtrlFlow:** 71.4% | **Authorship Centralization:** 0.0%
...
### 3. `setupenv.bash.in` (SHELL) -> Cumulative Risk: **536.52**
- **Archetype:** `file_cluster_8` (Distance: 10.779 IQR)
- **Magnitude:** 63.96 | **LOC:** 93 | **CtrlFlow:** 76.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9998%), Cognitive Load (98.9013%), Safety Score (90.4651%)
- **Heaviest Functions:** `export_path` (Impact: 19.2), `export_manpath` (Impact: 16.1), `export_display` (Impact: 5.2)

### 4. `Mk/macports.tea.mk` (MAKEFILE) -> Cumulative Risk: **419.55**
- **Archetype:** `file_cluster_8` (Distance: 7.708 IQR)
- **Magnitude:** 7.84 | **LOC:** 26 | **CtrlFlow:** 50.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9978%), Documentation (98.1034%), Safety Score (89.9274%)
- **Heaviest Functions:** `install` (Impact: 1.2), `.c.o` (Impact: 1.1), `all` (Impact: 1.1)

### 5. `portmgr/packaging/SETUP.chroot.sh` (SHELL) -> Cumulative Risk: **419.02**
- **Archetype:** `file_cluster_8` (Distance: 6.02 IQR)
- **Magnitude:** 8.26 | **LOC:** 39 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9955%), Documentation (99.5384%), Safety Score (86.9295%)
- **Heaviest Functions:** `__global_context__` (Impact: 2.9)

### 6. `standard_configure.sh` (SHELL) -> Cumulative Risk: **327.41**
- **Archetype:** `file_cluster_8` (Distance: 13.883 IQR)
- **Magnitude:** 2.96 | **LOC:** 12 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), State Flux (99.9987%), Safety Score (86.2846%), Spec Match (20.0%)
- **Heaviest Functions:** `__global_context__` (Impact: 1.9)

### 7. `configure.ac` (M4) -> Cumulative Risk: **308.95**
- **Archetype:** `file_cluster_8` (Distance: 5.849 IQR)
- **Magnitude:** 73.54 | **LOC:** 511 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Safety Score (52.8307%), Churn (40.97%)

### 8. `Mk/macports.subdir.mk` (MAKEFILE) -> Cumulative Risk: **257.46**
- **Archetype:** `file_cluster_8` (Distance: 8.01 IQR)
- **Magnitude:** 15.9 | **LOC:** 31 | **CtrlFlow:** 100.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (99.8413%), Cognitive Load (43.0454%), Api Exposure (12.0%)
- **Heaviest Functions:** `all` (Impact: 2.1), `clean` (Impact: 2.1), `test` (Impact: 2.1)

### 9. `doc/Makefile.in` (MAKEFILE) -> Cumulative Risk: **252.78**
- **Archetype:** `file_cluster_8` (Distance: 7.182 IQR)
- **Magnitude:** 33.5 | **LOC:** 221 | **CtrlFlow:** 46.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (76.4313%), Safety Score (59.5903%), Cognitive Load (7.7037%)
- **Heaviest Functions:** `clean` (Impact: 16.1), `install` (Impact: 4.6), `man` (Impact: 1.1)

### 10. `aclocal.m4` (M4) -> Cumulative Risk: **216.52**
- **Archetype:** `file_cluster_8` (Distance: 7.516 IQR)
- **Magnitude:** 54.5 | **LOC:** 1207 | **CtrlFlow:** 11.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (48.0874%), Tech Debt (29.6781%), Churn (20.61%)
...
- `portmgr/packaging/buildall.sh` (SHELL) | Magnitude: 256.9 | Delta: **0.024 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_tabs: 133, branch: 109, safety_bypasses: 85, state_mutation: 85

### Mixed-Responsibility Refactoring Targets for: file_cluster_8
- `portmgr/dmg/InstallationCheck` (SHELL) | Magnitude: 24.8 | Delta: **0.193 IQR** | Secondary Pull: `file_cluster_12`
  * Top Architectural Signatures: branch: 15, structural_boundaries: 6, io: 5, panics_and_aborts: 5
...
- `setupenv.bash.in` (SHELL) | Magnitude: 63.96 | Delta: **0.299 IQR** | Secondary Pull: `file_cluster_12`
  * Top Architectural Signatures: indent_spaces: 36, branch: 35, state_mutation: 18, structural_boundaries: 11
...
- `Mk/macports.subdir.mk` -> **Severity: 394.673** (Blast Radius: 3.953 * Doc Risk: 99.8413%)
- `portmgr/packaging/SETUP.chroot.sh` -> **Severity: 393.475** (Blast Radius: 3.953 * Doc Risk: 99.5384%)
- `Mk/macports.tea.mk` -> **Severity: 387.803** (Blast Radius: 3.953 * Doc Risk: 98.1034%)
- `portmgr/dmg/InstallationCheck` -> **Severity: 220.754** (Blast Radius: 3.953 * Doc Risk: 55.8446%)
