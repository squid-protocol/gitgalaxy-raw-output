...
| **Timestamp** | `2026-08-07T05:24:16.443971+00:00` |
| **Scan Duration** | `0.6s` |
| **Git Branch** | `N/A` |
| **Git Commit** | `N/A` |
| **Git Remote** | `N/A` |
...
|---|---|---|
| Modularity | 0.3757 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Error & Exception Exposure | 0.0 | 94.9 | 55.0 | 68.0 | 0.0 |
| Tech Debt Exposure | 0.0 | 100.0 | 51.8 | 82.4 | 0.0 |
| Testing Exposure | 0.2 | 80.0 | 18.6 | 2.4 | 80.0 |
...
| Documentation Exposure | 1.4 | 100.0 | 46.6 | 45.5 | 3.0 |
...
- `_parse_operator` (@ `openpyxl-3.1.5/openpyxl/formula/tokenizer.py`) -> Impact: **94.1** | LOC: 220
- `to_tree` (@ `openpyxl-3.1.5/openpyxl/descriptors/serialisable.py`) -> Impact: **49.6** | LOC: 52
- `translate_range` (@ `openpyxl-3.1.5/openpyxl/formula/translate.py`) -> Impact: **48.3** | LOC: 26
- `check_error` (@ `openpyxl-3.1.5/openpyxl/cell/cell.py`) -> Impact: **47.8** | LOC: 124
- `lxml_write_cell` (@ `openpyxl-3.1.5/openpyxl/cell/_writer.py`) -> Impact: **46.8** | LOC: 42
- `__new__` (@ `openpyxl-3.1.5/openpyxl/descriptors/__init__.py`) -> Impact: **39.8** | LOC: 36
- `from_tree` (@ `openpyxl-3.1.5/openpyxl/descriptors/serialisable.py`) -> Impact: **39.2** | LOC: 56
- `_cells_by_row` (@ `openpyxl-3.1.5/openpyxl/worksheet/_read_only.py`) -> Impact: **39.1** | LOC: 41
- `read_worksheets` (@ `openpyxl-3.1.5/openpyxl/reader/excel.py`) -> Impact: **36.6** | LOC: 73
- `etree_write_cell` (@ `openpyxl-3.1.5/openpyxl/cell/_writer.py`) -> Impact: **31.2** | LOC: 42

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `openpyxl-3.1.5/openpyxl/worksheet` | 32 | 3065.52 | 21.47% | 71.52% |
| `openpyxl-3.1.5/openpyxl/drawing` | 16 | 1564.16 | 25.61% | 74.69% |
| `openpyxl-3.1.5/openpyxl/chart` | 33 | 1212.28 | 17.54% | 55.39% |
| `openpyxl-3.1.5/openpyxl/styles` | 16 | 1030.16 | 27.07% | 49.4% |
| `openpyxl-3.1.5/openpyxl/pivot` | 5 | 977.16 | 21.32% | 79.52% |
| `openpyxl-3.1.5/openpyxl/workbook` | 12 | 904.04 | 16.84% | 70.39% |
| `openpyxl-3.1.5/openpyxl/descriptors` | 9 | 779.15 | 29.45% | 44.44% |
| `openpyxl-3.1.5/openpyxl/cell` | 6 | 595.54 | 41.68% | 65.76% |
| `openpyxl-3.1.5/openpyxl/packaging` | 8 | 532.82 | 19.05% | 49.72% |
| `openpyxl-3.1.5/openpyxl/utils` | 11 | 455.1 | 12.76% | 23.13% |
...
### 1. `openpyxl-3.1.5/openpyxl/workbook/child.py` (PYTHON) -> Cumulative Risk: **630.44**
- **Archetype:** `file_cluster_0` (Distance: 10.938 IQR)
- **Magnitude:** 106.66 | **LOC:** 167 | **CtrlFlow:** 33.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (99.0483%), State Flux (93.2008%)
- **Heaviest Functions:** `title` (Impact: 18.7), `avoid_duplicate_name` (Impact: 18.2), `__init__` (Impact: 4.2)

### 2. `openpyxl-3.1.5/openpyxl/workbook/workbook.py` (PYTHON) -> Cumulative Risk: **627.56**
- **Archetype:** `file_cluster_13` (Distance: 12.926 IQR)
- **Magnitude:** 282.76 | **LOC:** 439 | **CtrlFlow:** 26.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9945%), Documentation (89.7537%), Tech Debt (84.9907%)
- **Heaviest Functions:** `active` (Impact: 11.3), `_add_sheet` (Impact: 10.7), `mime_type` (Impact: 10.7)

### 3. `openpyxl-3.1.5/openpyxl/styles/colors.py` (PYTHON) -> Cumulative Risk: **615.14**
- **Archetype:** `file_cluster_13` (Distance: 11.329 IQR)
- **Magnitude:** 105.84 | **LOC:** 173 | **CtrlFlow:** 26.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), State Flux (99.9926%), Verification (80.0%)
- **Heaviest Functions:** `__init__` (Impact: 18.8), `__set__` (Impact: 10.4), `__iter__` (Impact: 5.5)

### 4. `openpyxl-3.1.5/openpyxl/packaging/custom.py` (PYTHON) -> Cumulative Risk: **609.99**
- **Archetype:** `file_cluster_13` (Distance: 11.338 IQR)
- **Magnitude:** 149.04 | **LOC:** 290 | **CtrlFlow:** 29.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9999%), State Flux (99.7156%), Verification (80.0%)
- **Heaviest Functions:** `__init__` (Impact: 11.5), `from_tree` (Impact: 9.9), `to_tree` (Impact: 9.4)

### 5. `openpyxl-3.1.5/openpyxl/worksheet/dimensions.py` (PYTHON) -> Cumulative Risk: **602.74**
- **Archetype:** `file_cluster_13` (Distance: 11.262 IQR)
- **Magnitude:** 188.28 | **LOC:** 307 | **CtrlFlow:** 28.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9068%), Tech Debt (99.8425%), Verification (80.0%)
- **Heaviest Functions:** `__init__` (Impact: 23.4), `group` (Impact: 18.1), `__init__` (Impact: 17.9)

### 6. `openpyxl-3.1.5/openpyxl/formatting/rule.py` (PYTHON) -> Cumulative Risk: **602.53**
- **Archetype:** `file_cluster_8` (Distance: 10.582 IQR)
- **Magnitude:** 147.08 | **LOC:** 292 | **CtrlFlow:** 19.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9491%), Tech Debt (96.8813%), Verification (80.0%)
- **Heaviest Functions:** `ColorScaleRule` (Impact: 23.5), `__set__` (Impact: 14.4), `__init__` (Impact: 6.0)

### 7. `openpyxl-3.1.5/openpyxl/drawing/drawing.py` (PYTHON) -> Cumulative Risk: **602.47**
- **Archetype:** `file_cluster_13` (Distance: 12.078 IQR)
- **Magnitude:** 87.74 | **LOC:** 93 | **CtrlFlow:** 40.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.9999%), Documentation (92.412%)
- **Heaviest Functions:** `set_dimension` (Impact: 12.6), `anchor` (Impact: 6.1), `width` (Impact: 5.4)

### 8. `openpyxl-3.1.5/openpyxl/cell/read_only.py` (PYTHON) -> Cumulative Risk: **600.14**
- **Archetype:** `file_cluster_0` (Distance: 10.029 IQR)
- **Magnitude:** 81.1 | **LOC:** 137 | **CtrlFlow:** 8.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9999%), Documentation (99.7466%), Cognitive Load (96.9677%)
- **Heaviest Functions:** `number_format` (Impact: 5.5), `__eq__` (Impact: 5.4), `value` (Impact: 3.7)

### 9. `openpyxl-3.1.5/openpyxl/cell/cell.py` (PYTHON) -> Cumulative Risk: **598.37**
- **Archetype:** `file_cluster_13` (Distance: 12.716 IQR)
- **Magnitude:** 184.7 | **LOC:** 333 | **CtrlFlow:** 34.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9918%), Tech Debt (94.7081%), Verification (80.0%)
- **Heaviest Functions:** `check_error` (Impact: 47.8), `get_type` (Impact: 11.0), `get_time_format` (Impact: 9.2)

### 10. `openpyxl-3.1.5/openpyxl/pivot/table.py` (PYTHON) -> Cumulative Risk: **595.45**
- **Archetype:** `file_cluster_8` (Distance: 10.571 IQR)
- **Magnitude:** 479.06 | **LOC:** 1262 | **CtrlFlow:** 8.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.978%), Tech Debt (99.3162%), Verification (80.0%)
- **Heaviest Functions:** `__init__` (Impact: 17.2), `by_priority` (Impact: 9.3), `_write_rels` (Impact: 6.8)
...
- **Magnitude:** 542.44 | **LOC:** 908 | **CtrlFlow:** 42.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (26.0306%), Tech Debt (83.5449%)
**Top Internal Functions/Classes:**
  * `__getitem__` (Impact: 22.3)
  * `append` (Impact: 22.3)
    * *Intent:* """ Add a data-validation object to the sheet. The data-validation object defines the type of data-v...
  * `_move_cells` (Impact: 20.8)
  * `move_range` (Impact: 20.7)
    * *Intent:* * append(['This is A1', 'This is B1', 'This is C1']) * **or** append({'A' : 'This is A1', 'C' : 'Thi...
  * `iter_rows` (Impact: 19.2)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 14):` .formula, inspect, openpyxl.compat, openpyxl.formatting.formatting, .table, .page, .properties, openpyxl.workbook.child...
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 9):` openpyxl.descriptors.serialisable, openpyxl.worksheet.filters, openpyxl.descriptors.sequence, openpyxl.xml.functions, openpyxl.descriptors.excel, .fields, collections, openpyxl.descriptors...
...
- **Magnitude:** 347.6 | **LOC:** 966 | **CtrlFlow:** 1.1% | **Authorship Centralization:** 0.0%
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 9):` openpyxl.descriptors.serialisable, openpyxl.descriptors.nested, openpyxl.descriptors.sequence, openpyxl.xml.functions, openpyxl.descriptors.excel, .fields, .table, openpyxl.descriptors...
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
**Top Internal Functions/Classes:**
  * `active` (Impact: 11.3)
  * `_add_sheet` (Impact: 10.7)
  * `mime_type` (Impact: 10.7)
  * `_duplicate_name` (Impact: 9.2)
  * `create_sheet` (Impact: 8.7)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 25):` openpyxl.styles.colors, .defined_name, openpyxl.worksheet.worksheet, .child, openpyxl.utils.datetime, openpyxl.compat, openpyxl.styles.borders, .properties...
...
- **Magnitude:** 261.22 | **LOC:** 718 | **CtrlFlow:** 1.4% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (50.6296%), Tech Debt (99.5596%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 10.8)
  * `__init__` (Impact: 8.5)
  * `__init__` (Impact: 8.0)
  * `__init__` (Impact: 6.4)
  * `__init__` (Impact: 4.9)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 8):` openpyxl.descriptors.serialisable, openpyxl.descriptors.nested, .geometry, .colors, openpyxl.descriptors.excel, openpyxl.descriptors, .fill, .effect...
...
- **Magnitude:** 252.56 | **LOC:** 513 | **CtrlFlow:** 33.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (22.5915%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 18.0)
  * `__ne__` (Impact: 9.4)
  * `_check_title` (Impact: 9.1)
  * `isdisjoint` (Impact: 7.5)
  * `add` (Impact: 7.4)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 3):` openpyxl.descriptors.serialisable, openpyxl.descriptors.sequence, openpyxl.utils, copy, operator, openpyxl.descriptors, itertools
...
- **Magnitude:** 249.78 | **LOC:** 391 | **CtrlFlow:** 40.6% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (17.4444%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `write_row` (Impact: 16.6)
  * `write_tables` (Impact: 15.0)
    * *Intent:* """ Comments & VBA controls use VML and require an additional element that is no longer in the speci...
  * `get_stream` (Impact: 14.6)
  * `write_formatting` (Impact: 9.1)
  * `write_hyperlinks` (Impact: 7.5)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 10):` openpyxl.xml.functions, io, os, .hyperlink, .merge, openpyxl.comments.comment_sheet, .related, atexit...
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
- **Magnitude:** 222.52 | **LOC:** 487 | **CtrlFlow:** 18.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (28.5258%), Tech Debt (99.9951%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 20.7)
  * `_guess_operator` (Impact: 7.6)
  * `_get_subtype` (Impact: 7.4)
  * `to_tree` (Impact: 7.0)
  * `convert` (Impact: 5.8)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 4):` openpyxl.descriptors.serialisable, openpyxl.descriptors.sequence, openpyxl.utils, openpyxl.descriptors.excel, openpyxl.descriptors, re
...
- **Magnitude:** 197.2 | **LOC:** 585 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 6):` openpyxl.descriptors.serialisable, openpyxl.styles.colors, openpyxl.xml.constants, openpyxl.descriptors.excel, openpyxl.descriptors, .line
  * `Imported By (In-Degree: 7):` (Excluded from Brief to save tokens)
...
- **Magnitude:** 193.84 | **LOC:** 383 | **CtrlFlow:** 32.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (29.0625%), Tech Debt (97.7506%)
**Top Internal Functions/Classes:**
  * `_write` (Impact: 22.3)
  * `_blip_rels` (Impact: 14.6)
  * `__init__` (Impact: 8.0)
  * `_check_anchor` (Impact: 7.7)
    * *Intent:* """ Check whether an object has an existing Anchor object If not create a OneCellAnchor using the pr...
  * `_chart_rels` (Impact: 7.6)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 14):` openpyxl.descriptors.serialisable, openpyxl.descriptors.nested, openpyxl.chart._chart, openpyxl.drawing.image, openpyxl.utils, .geometry, openpyxl.descriptors.excel, .picture...
...
- **Magnitude:** 192.5 | **LOC:** 447 | **CtrlFlow:** 51.9% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (34.2882%), Tech Debt (10.8337%)
**Top Internal Functions/Classes:**
  * `_parse_operator` (Impact: 94.1)
  * `make_separator` (Impact: 5.5)
    * *Intent:* # Literal operands: # # Literal operands are always of type 'OPERAND' and can be of subtype # 'TEXT'...
  * `_parse_whitespace` (Impact: 3.8)
**Structural Signatures (Net Mitigated Signals):**
...
- **Top Global Matches:** file_cluster_13: 11.262, file_cluster_8: 11.377, file_cluster_7: 11.551
- **Magnitude:** 188.28 | **LOC:** 307 | **CtrlFlow:** 28.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (37.0803%), Tech Debt (99.8425%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 23.4)
  * `group` (Impact: 18.1)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 6):` openpyxl.descriptors.serialisable, openpyxl.xml.functions, openpyxl.utils, copy, openpyxl.utils.units, openpyxl.compat, openpyxl.descriptors, openpyxl.styles.styleable...
...
- **Magnitude:** 184.96 | **LOC:** 241 | **CtrlFlow:** 57.5% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (69.5115%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `to_tree` (Impact: 49.6)
  * `from_tree` (Impact: 39.2)
  * `__add__` (Impact: 16.3)
  * `__iter__` (Impact: 14.4)
  * `__eq__` (Impact: 9.1)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 3):` openpyxl.xml.functions, copy, , .sequence, .namespace, keyword, openpyxl.compat
...
**Top Internal Functions/Classes:**
  * `check_error` (Impact: 47.8)
  * `get_type` (Impact: 11.0)
  * `get_time_format` (Impact: 9.2)
  * `check_string` (Impact: 7.7)
  * `__init__` (Impact: 6.0)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 6):` openpyxl.utils, openpyxl.worksheet.formula, copy, openpyxl.worksheet.hyperlink, openpyxl.compat, openpyxl.styles, datetime, openpyxl.cell.rich_text...
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
- **Magnitude:** 182.98 | **LOC:** 386 | **CtrlFlow:** 26.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (30.5732%), Tech Debt (94.115%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 13.2)
  * `get` (Impact: 8.3)
  * `_initialise_columns` (Impact: 7.4)
  * `__set__` (Impact: 6.2)
  * `__init__` (Impact: 6.1)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 9):` openpyxl.descriptors.serialisable, openpyxl.descriptors.sequence, openpyxl.xml.functions, openpyxl.utils, openpyxl.utils.escape, .filters, openpyxl.descriptors.excel, .related...
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
- **Magnitude:** 177.56 | **LOC:** 273 | **CtrlFlow:** 40.7% | **Authorship Centralization:** 0.0%
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 2):` openpyxl.utils.datetime, .namespace, openpyxl, datetime, re
  * `Imported By (In-Degree: 3):` (Excluded from Brief to save tokens)

### `openpyxl-3.1.5/openpyxl/descriptors/excel.py` (PYTHON | Tier 1.5 | 🚨 AI THREAT: 98.83%)
- **Global Archetype:** `file_cluster_8` (Drift: 9.24 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 4.783 IQR)
- **Top Global Matches:** file_cluster_8: 9.24, file_cluster_13: 9.342, file_cluster_7: 9.682
- **Magnitude:** 165.15 | **LOC:** 113 | **CtrlFlow:** 13.3% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (4.4657%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 4`, `structural_boundaries: 26`, `args: 5`, `func_start: 5`, `class_start: 10`
* *Risk/State:* `state_mutation: 2`
* *Architecture:* `api: 10`, `import: 5`
* *Defense:* `safety: 1`, `doc: 6`
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 33.677
  * `Choke Point (Betweenness):` 0.001861 | `Ripple Effect (Closeness):` 0.38195
  * `Imports (Out-Degree: 3):` openpyxl.xml.functions, , .serialisable, openpyxl.compat, openpyxl.xml.constants
...
- **Magnitude:** 163.32 | **LOC:** 185 | **CtrlFlow:** 44.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (38.8993%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `from_string` (Impact: 18.2)
  * `__init__` (Impact: 16.1)
  * `__init__` (Impact: 16.1)
  * `__eq__` (Impact: 9.2)
  * `__eq__` (Impact: 9.2)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 3):` openpyxl.utils.cell, openpyxl.utils, .cell_range, openpyxl.descriptors, re
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
- **Magnitude:** 158.32 | **LOC:** 350 | **CtrlFlow:** 39.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (9.5786%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `read_worksheets` (Impact: 36.6)
  * `_validate_archive` (Impact: 18.6)
  * `read_chartsheet` (Impact: 11.0)
  * `_find_workbook_part` (Impact: 9.4)
  * `read` (Impact: 8.3)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 16):` openpyxl.pivot.table, ..tests, openpyxl.worksheet._reader, .strings, io, openpyxl.packaging.custom, openpyxl.styles.stylesheet, .drawings...
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)

### `openpyxl-3.1.5/openpyxl/worksheet/header_footer.py` (PYTHON | Tier 1.5 | 🚨 AI THREAT: 99.03%)
- **Global Archetype:** `file_cluster_13` (Drift: 11.735 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 4.378 IQR)
- **Top Global Matches:** file_cluster_13: 11.735, file_cluster_8: 11.834, file_cluster_7: 12.056
- **Magnitude:** 157.18 | **LOC:** 271 | **CtrlFlow:** 43.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (34.4853%), Tech Debt (99.9969%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 26.9)
  * `__str__` (Impact: 11.6)
    * *Intent:* """ keys = ('font', 'color', 'size') kw = dict((k, v) for match in FORMAT_REGEX.findall(text) for k,...
  * `from_str` (Impact: 10.9)
  * `__init__` (Impact: 9.4)
  * `__str__` (Impact: 7.5)
    * *Intent:* * &A Inserts the worksheet name * &B Toggles bold * &D or &[Date] Inserts the current date * &E Togg...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 28`, `structural_boundaries: 37`, `args: 13`, `func_start: 13`, `class_start: 3`
* *Risk/State:* `safety_bypasses: 2`, `state_mutation: 53`, `duplicate_logic: 8`
* *Architecture:* `api: 8`, `import: 6`
* *Defense:* `safety: 3`, `doc: 20`
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 5.569
  * `Choke Point (Betweenness):` 0.001318 | `Ripple Effect (Closeness):` 0.046336
  * `Imports (Out-Degree: 4):` openpyxl.descriptors.serialisable, openpyxl.xml.functions, openpyxl.utils.escape, openpyxl.descriptors, warnings, re
  * `Imported By (In-Degree: 5):` (Excluded from Brief to save tokens)
...
- **Top Global Matches:** file_cluster_13: 11.522, file_cluster_8: 11.8, file_cluster_7: 11.939
- **Magnitude:** 156.7 | **LOC:** 283 | **CtrlFlow:** 31.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (29.1285%), Tech Debt (96.5555%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 21.9)
  * `__getitem__` (Impact: 9.2)
  * `remove_duplicates` (Impact: 7.9)
  * `__setattr__` (Impact: 6.3)
  * `_recalculate` (Impact: 5.8)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 8):` openpyxl.descriptors.serialisable, .numbers, .fills, openpyxl.descriptors.excel, .alignment, openpyxl.compat, openpyxl.descriptors, .protection...
  * `Imported By (In-Degree: 5):` (Excluded from Brief to save tokens)

### `openpyxl-3.1.5/openpyxl/writer/excel.py` (PYTHON | Tier 1.5 | 🚨 AI THREAT: 99.24%)
...
- **Top Global Matches:** file_cluster_13: 11.04, file_cluster_8: 11.137, file_cluster_7: 11.392
- **Magnitude:** 151.56 | **LOC:** 296 | **CtrlFlow:** 39.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (21.2719%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_write_worksheets` (Impact: 21.4)
  * `_merge_vba` (Impact: 9.3)
  * `write_data` (Impact: 9.1)
    * *Intent:* """Write the various xml files into the zip archive."""
  * `_write_comment` (Impact: 8.0)
  * `write_worksheet` (Impact: 7.7)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 31`, `structural_boundaries: 47`, `args: 13`, `func_start: 13`, `class_start: 2`
* *Risk/State:* `safety_bypasses: 6`, `state_mutation: 52`
* *Architecture:* `api: 8`, `import: 15`
* *Defense:* `doc: 18`, `cleanup: 3`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.909
  * `Choke Point (Betweenness):` 0.001183 | `Ripple Effect (Closeness):` 0.005291
  * `Imports (Out-Degree: 13):` openpyxl.packaging.manifest, openpyxl.xml.functions, openpyxl.workbook._writer, openpyxl.drawing.spreadsheet_drawing, openpyxl.styles.stylesheet, openpyxl.comments.comment_sheet, openpyxl.packaging.extended, datetime...
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
**Top Internal Functions/Classes:**
  ...
  * `from_tree` (Impact: 9.9)
  * `to_tree` (Impact: 9.4)
  * `type` (Impact: 9.1)
  * `__getitem__` (Impact: 7.3)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 6):` openpyxl.descriptors.serialisable, openpyxl.descriptors.nested, openpyxl.descriptors.sequence, openpyxl.descriptors, warnings, openpyxl.xml.constants, .core
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)

### `openpyxl-3.1.5/openpyxl/formatting/rule.py` (PYTHON | Tier 1.5 | 🚨 AI THREAT: 98.82%)
- **Global Archetype:** `file_cluster_8` (Drift: 10.582 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 3.369 IQR)
- **Top Global Matches:** file_cluster_8: 10.582, file_cluster_13: 10.774, file_cluster_7: 10.965
- **Magnitude:** 147.08 | **LOC:** 292 | **CtrlFlow:** 19.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (32.7631%), Tech Debt (96.8813%)
**Top Internal Functions/Classes:**
  * `ColorScaleRule` (Impact: 23.5)
  * `__set__` (Impact: 14.4)
    * *Intent:* """ def __set__(self, instance, value): ref = None if value is not None and isinstance(value, str): ...
  * `__init__` (Impact: 6.0)
  * `IconSetRule` (Impact: 5.8)
  * `DataBarRule` (Impact: 3.5)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 13`, `structural_boundaries: 55`, `args: 11`, `func_start: 11`, `class_start: 7`
* *Risk/State:* `safety_bypasses: 3`, `state_mutation: 54`, `duplicate_logic: 5`
* *Architecture:* `api: 17`, `import: 6`
* *Defense:* `safety: 2`, `doc: 10`
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 3.896
  * `Choke Point (Betweenness):` 0.001576 | `Ripple Effect (Closeness):` 0.039197
  * `Imports (Out-Degree: 6):` openpyxl.descriptors.serialisable, openpyxl.styles.colors, openpyxl.utils.cell, openpyxl.descriptors.excel, openpyxl.descriptors, openpyxl.styles.differential
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)

### `openpyxl-3.1.5/openpyxl/styles/stylesheet.py` (PYTHON | Tier 1.5 | 🚨 AI THREAT: 99.16%)
- **Global Archetype:** `file_cluster_13` (Drift: 10.829 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 4.707 IQR)
- **Top Global Matches:** file_cluster_13: 10.829, file_cluster_8: 11.084, file_cluster_7: 11.419
- **Magnitude:** 146.62 | **LOC:** 275 | **CtrlFlow:** 34.5% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (28.0663%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_normalise_numbers` (Impact: 15.1)
  * `apply_stylesheet` (Impact: 14.2)
  * `__init__` (Impact: 11.9)
  * `_expand_named_style` (Impact: 11.7)
  * `write_stylesheet` (Impact: 10.3)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 30`, `structural_boundaries: 57`, `args: 10`, `func_start: 10`, `class_start: 1`
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000833 | `Ripple Effect (Closeness):` 0.014109
  * `Imports (Out-Degree: 14):` openpyxl.descriptors.serialisable, openpyxl.descriptors.sequence, openpyxl.utils.indexed_list, openpyxl.xml.functions, .borders, .numbers, .fills, .colors...
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
- `openpyxl-3.1.5/openpyxl/descriptors/container.py` (PYTHON) | Magnitude: 25.02 | Delta: **0.148 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 19, structural_boundaries: 12, api: 6, args: 5
- `openpyxl-3.1.5/openpyxl/comments/comments.py` (PYTHON) | Magnitude: 50.56 | Delta: **0.153 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 32, structural_boundaries: 15, state_mutation: 13, api: 12
- `openpyxl-3.1.5/openpyxl/cell/read_only.py` (PYTHON) | Magnitude: 81.1 | Delta: **0.233 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 87, structural_boundaries: 52, encapsulation: 43, api: 22
- `openpyxl-3.1.5/openpyxl/workbook/child.py` (PYTHON) | Magnitude: 106.66 | Delta: **0.304 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 92, structural_boundaries: 38, api: 21, args: 20

### Mixed-Responsibility Refactoring Targets for: file_cluster_13
- `openpyxl-3.1.5/openpyxl/chartsheet/properties.py` (PYTHON) | Magnitude: 8.22 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 16, structural_boundaries: 8, state_mutation: 3, import: 3
- `openpyxl-3.1.5/openpyxl/chart/print_settings.py` (PYTHON) | Magnitude: 17.82 | Delta: **0.012 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 34, structural_boundaries: 12, state_mutation: 9, import: 4
- `openpyxl-3.1.5/openpyxl/chart/radar_chart.py` (PYTHON) | Magnitude: 10.88 | Delta: **0.014 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 33, structural_boundaries: 19, import: 8, state_mutation: 6
- `openpyxl-3.1.5/openpyxl/worksheet/formula.py` (PYTHON) | Magnitude: 29.82 | Delta: **0.021 IQR** | Secondary Pull: `file_cluster_8`
...
- `openpyxl-3.1.5/openpyxl/descriptors/slots.py` (PYTHON) | Magnitude: 21.38 | Delta: **0.087 IQR** | Secondary Pull: `file_cluster_8`
...
- `openpyxl-3.1.5/openpyxl/compat/__init__.py` (PYTHON) | Magnitude: 35.78 | Delta: **0.005 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 26, structural_boundaries: 20, branch: 9, import: 5
- `openpyxl-3.1.5/openpyxl/chart/descriptors.py` (PYTHON) | Magnitude: 7.6 | Delta: **0.017 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 14, structural_boundaries: 10, doc: 4, class_start: 3
- `openpyxl-3.1.5/openpyxl/utils/inference.py` (PYTHON) | Magnitude: 21.78 | Delta: **0.02 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 19, branch: 17, structural_boundaries: 11, doc: 10
- `openpyxl-3.1.5/openpyxl/chart/chartspace.py` (PYTHON) | Magnitude: 73.82 | Delta: **0.022 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 138, state_mutation: 46, structural_boundaries: 40, import: 15
- `openpyxl-3.1.5/openpyxl/comments/author.py` (PYTHON) | Magnitude: 5.16 | Delta: **0.026 IQR** | Secondary Pull: `file_cluster_13`
...
- `openpyxl-3.1.5/openpyxl/utils/indexed_list.py` -> **Severity: 24.813** (Embedded: 0.2613 * Error Risk: 94.9464%)
- `openpyxl-3.1.5/openpyxl/chart/data_source.py` -> **Severity: 23.052** (Embedded: 0.3227 * Error Risk: 71.4277%)
- `openpyxl-3.1.5/openpyxl/descriptors/excel.py` -> **Severity: 20.386** (Embedded: 0.382 * Error Risk: 53.3746%)
- `openpyxl-3.1.5/openpyxl/descriptors/sequence.py` -> **Severity: 18.386** (Embedded: 0.3717 * Error Risk: 49.4607%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `openpyxl-3.1.5/openpyxl/descriptors/nested.py` -> **Severity: 4956.353** (Blast Radius: 49.815 * Doc Risk: 99.4952%)
- `openpyxl-3.1.5/openpyxl/chart/descriptors.py` -> **Severity: 4211.705** (Blast Radius: 65.487 * Doc Risk: 64.3136%)
- `openpyxl-3.1.5/openpyxl/descriptors/serialisable.py` -> **Severity: 3748.92** (Blast Radius: 76.703 * Doc Risk: 48.8758%)
- `openpyxl-3.1.5/openpyxl/descriptors/excel.py` -> **Severity: 3103.228** (Blast Radius: 33.677 * Doc Risk: 92.1468%)
- `openpyxl-3.1.5/openpyxl/descriptors/sequence.py` -> **Severity: 2793.025** (Blast Radius: 30.645 * Doc Risk: 91.1413%)
