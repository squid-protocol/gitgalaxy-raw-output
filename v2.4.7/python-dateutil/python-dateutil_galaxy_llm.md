...
| **Timestamp** | `2026-08-07T05:25:38.807394+00:00` |
| **Scan Duration** | `0.24s` |
| **Git Branch** | `N/A` |
| **Git Commit** | `N/A` |
| **Git Remote** | `N/A` |
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 2.6 | 75.7 | 24.1 | 18.0 | 5.0 |
| Error & Exception Exposure | 0.0 | 81.8 | 43.3 | 54.3 | 0.0 |
| Tech Debt Exposure | 0.0 | 98.2 | 32.2 | 4.8 | 0.0 |
| Testing Exposure | 0.2 | 80.0 | 35.9 | 2.7 | 80.0 |
...
| Documentation Exposure | 3.0 | 94.7 | 21.2 | 12.3 | 11.9 |
...
- `__init__` (@ `python-dateutil-2.6.1/dateutil/relativedelta.py`) -> Impact: **736.8** | LOC: 445
- `__str__` (@ `python-dateutil-2.6.1/dateutil/rrule.py`) -> Impact: **500.8** | LOC: 871
- `__init__` (@ `python-dateutil-2.6.1/dateutil/rrule.py`) -> Impact: **470.4** | LOC: 254
- `__repr__` (@ `python-dateutil-2.6.1/dateutil/tz/tz.py`) -> Impact: **172.9** | LOC: 445
...
- `_parse_rfc` (@ `python-dateutil-2.6.1/dateutil/tz/tz.py`) -> Impact: **157.3** | LOC: 236
- `get_token` (@ `python-dateutil-2.6.1/dateutil/parser.py`) -> Impact: **77.6** | LOC: 97
- `_fromutc` (@ `python-dateutil-2.6.1/dateutil/tz/_common.py`) -> Impact: **54.9** | LOC: 162
- `__init__` (@ `python-dateutil-2.6.1/dateutil/tz/tz.py`) -> Impact: **44.7** | LOC: 46
- `between` (@ `python-dateutil-2.6.1/dateutil/rrule.py`) -> Impact: **38.2** | LOC: 30
- `__repr__` (@ `python-dateutil-2.6.1/dateutil/tz/tz.py`) -> Impact: **34.1** | LOC: 93

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `python-dateutil-2.6.1/dateutil` | 8 | 2874.66 | 26.92% | 6.58% |
| `python-dateutil-2.6.1/dateutil/tz` | 4 | 1183.92 | 25.43% | 70.29% |
| `python-dateutil-2.6.1/dateutil/zoneinfo` | 1 | 87.28 | 35.46% | 90.29% |
| `python-dateutil-2.6.1` | 5 | 63.74 | 6.65% | 18.3% |
...
### 1. `python-dateutil-2.6.1/dateutil/tz/win.py` (PYTHON) -> Cumulative Risk: **575.9**
- **Archetype:** `file_cluster_13` (Distance: 12.522 IQR)
- **Magnitude:** 186.72 | **LOC:** 333 | **CtrlFlow:** 35.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9996%), Tech Debt (85.828%), Verification (80.0%)
- **Heaviest Functions:** `__eq__` (Impact: 23.4), `valuestodict` (Impact: 15.1), `__repr__` (Impact: 11.7)

### 2. `python-dateutil-2.6.1/dateutil/zoneinfo/__init__.py` (PYTHON) -> Cumulative Risk: **574.41**
- **Archetype:** `file_cluster_13` (Distance: 14.756 IQR)
- **Magnitude:** 87.28 | **LOC:** 184 | **CtrlFlow:** 39.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9933%), Tech Debt (90.2888%), Verification (80.0%)
- **Heaviest Functions:** `__init__` (Impact: 20.6), `get_zonefile_instance` (Impact: 7.6), `gettz` (Impact: 7.5)

### 3. `python-dateutil-2.6.1/dateutil/tz/tz.py` (PYTHON) -> Cumulative Risk: **568.47**
- **Archetype:** `file_cluster_13` (Distance: 12.835 IQR)
- **Magnitude:** 862.88 | **LOC:** 1512 | **CtrlFlow:** 52.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9771%), Tech Debt (98.2208%), Verification (80.0%)
- **Heaviest Functions:** `__repr__` (Impact: 172.9), `_parse_rfc` (Impact: 157.3), `__init__` (Impact: 44.7)

### 4. `python-dateutil-2.6.1/dateutil/relativedelta.py` (PYTHON) -> Cumulative Risk: **513.49**
- **Archetype:** `file_cluster_8` (Distance: 12.596 IQR)
- **Magnitude:** 913.6 | **LOC:** 550 | **CtrlFlow:** 72.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9999%), Verification (80.0%), Safety Score (78.0008%)
- **Heaviest Functions:** `__init__` (Impact: 736.8), `_sign` (Impact: 1.8)

### 5. `python-dateutil-2.6.1/dateutil/rrule.py` (PYTHON) -> Cumulative Risk: **503.25**
- **Archetype:** `file_cluster_8` (Distance: 12.862 IQR)
- **Magnitude:** 1600.0 | **LOC:** 1611 | **CtrlFlow:** 74.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.999%), Safety Score (81.5739%), Verification (80.0%)
- **Heaviest Functions:** `__str__` (Impact: 500.8), `__init__` (Impact: 470.4), `between` (Impact: 38.2)

### 6. `python-dateutil-2.6.1/dateutil/parser.py` (PYTHON) -> Cumulative Risk: **465.11**
- **Archetype:** `file_cluster_8` (Distance: 11.966 IQR)
- **Magnitude:** 284.6 | **LOC:** 1375 | **CtrlFlow:** 70.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (97.7992%), Verification (80.0%), Safety Score (56.0871%)
- **Heaviest Functions:** `get_token` (Impact: 77.6), `__init__` (Impact: 9.4), `_repr` (Impact: 5.5)

### 7. `python-dateutil-2.6.1/dateutil/_common.py` (PYTHON) -> Cumulative Risk: **403.42**
- **Archetype:** `file_cluster_8` (Distance: 10.524 IQR)
- **Magnitude:** 27.78 | **LOC:** 35 | **CtrlFlow:** 36.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (94.6694%), State Flux (74.1414%), Safety Score (52.5886%)
- **Heaviest Functions:** `__eq__` (Impact: 7.3), `__repr__` (Impact: 5.5), `__call__` (Impact: 5.4)

### 8. `python-dateutil-2.6.1/dateutil/tz/_common.py` (PYTHON) -> Cumulative Risk: **372.02**
- **Archetype:** `file_cluster_8` (Distance: 10.415 IQR)
- **Magnitude:** 121.24 | **LOC:** 395 | **CtrlFlow:** 35.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (97.121%), Verification (80.0%), Stability (50.0%)
- **Heaviest Functions:** `_fromutc` (Impact: 54.9), `enfold` (Impact: 7.6), `_fold_status` (Impact: 6.5)

### 9. `python-dateutil-2.6.1/updatezinfo.py` (PYTHON) -> Cumulative Risk: **300.23**
- **Archetype:** `file_cluster_13` (Distance: 9.452 IQR)
- **Magnitude:** 22.22 | **LOC:** 56 | **CtrlFlow:** 32.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (65.6296%), Stability (50.0%), Safety Score (48.2794%)
- **Heaviest Functions:** `main` (Impact: 17.4)

### 10. `python-dateutil-2.6.1/setup.py` (PYTHON) -> Cumulative Risk: **258.29**
- **Archetype:** `file_cluster_8` (Distance: 6.134 IQR)
- **Magnitude:** 15.82 | **LOC:** 48 | **CtrlFlow:** 10.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (91.5138%), Stability (50.0%), Documentation (11.9203%)
...
- **Top Global Matches:** file_cluster_8: 12.862, file_cluster_13: 12.923, file_cluster_7: 13.013
- **Magnitude:** 1600.0 | **LOC:** 1611 | **CtrlFlow:** 74.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (50.2609%), Tech Debt (19.9973%)
**Top Internal Functions/Classes:**
  * `__str__` (Impact: 500.8)
  * `__init__` (Impact: 470.4)
  * `between` (Impact: 38.2)
  * `__getitem__` (Impact: 23.6)
  * `xafter` (Impact: 21.4)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` six, warnings, math, fractions, heapq, itertools, dateutil.rrule, dateutil...
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` six, warnings, math, operator, calendar, datetime, ._common
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 2):` six, dateutil.zoneinfo, os, struct, time, bisect, dateutil, tz...
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` six, dateutil.parser, string, collections, re, io, time, ...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
**Top Internal Functions/Classes:**
  * `__eq__` (Impact: 23.4)
  * `valuestodict` (Impact: 15.1)
  * `__repr__` (Impact: 11.7)
  * `list` (Impact: 7.3)
  * `__init__` (Impact: 7.0)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 1):` six, struct, dateutil.tzwin, datetime, ctypes, ._common, six.moves
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)

### `python-dateutil-2.6.1/dateutil/tz/_common.py` (PYTHON | Tier 1.5 | 🚨 AI THREAT: 98.89%)
- **Global Archetype:** `file_cluster_8` (Drift: 10.415 IQR)
- **Local Micro-Species:** `Cluster 1: Declarative Glue & Initialization` (Drift: 4.835 IQR)
- **Top Global Matches:** file_cluster_8: 10.415, file_cluster_7: 10.582, file_cluster_0: 10.604
- **Magnitude:** 121.24 | **LOC:** 395 | **CtrlFlow:** 35.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (15.2177%), Tech Debt (97.121%)
**Top Internal Functions/Classes:**
  * `_fromutc` (Impact: 54.9)
  * `enfold` (Impact: 7.6)
  * `_fold_status` (Impact: 6.5)
  * `_validate_fromutc_inputs` (Impact: 5.8)
    * *Intent:* """ __slots__ = () @property def fold(self): return 1 def enfold(dt, fold=1): """
  * `tzname_in_python2` (Impact: 5.7)
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 22.148
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` six, functools, datetime
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 22.148
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` warnings, io, tarfile, pkgutil, contextlib, json, dateutil.tz
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 22.148
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` dateutil.zoneinfo, six.moves.urllib, os, io, json, hashlib
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 22.148
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` os.path, dateutil._version, os, codecs, setuptools, re
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Magnitude:** 15.04 | **LOC:** 90 | **CtrlFlow:** 62.5% | **Authorship Centralization:** 0.0%
...
- `python-dateutil-2.6.1/dateutil/tz/tz.py` (PYTHON) | Magnitude: 862.88 | Delta: **0.13 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 781, branch: 263, encapsulation: 262, structural_boundaries: 242
- `python-dateutil-2.6.1/dateutil/zoneinfo/__init__.py` (PYTHON) | Magnitude: 87.28 | Delta: **0.148 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_spaces: 55, structural_boundaries: 32, state_mutation: 22, branch: 21
- `python-dateutil-2.6.1/updatezinfo.py` (PYTHON) | Magnitude: 22.22 | Delta: **0.279 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 30, structural_boundaries: 19, branch: 9, import: 7
- `python-dateutil-2.6.1/dateutil/tz/win.py` (PYTHON) | Magnitude: 186.72 | Delta: **0.286 IQR** | Secondary Pull: `file_cluster_0`
...
- `python-dateutil-2.6.1/dateutil/rrule.py` (PYTHON) | Magnitude: 1600.0 | Delta: **0.061 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 1100, branch: 441, state_mutation: 394, encapsulation: 345
- `python-dateutil-2.6.1/dateutil/relativedelta.py` (PYTHON) | Magnitude: 913.6 | Delta: **0.134 IQR** | Secondary Pull: `file_cluster_13`
...
- `python-dateutil-2.6.1/dateutil/tz/_common.py` (PYTHON) | Magnitude: 121.24 | Delta: **0.167 IQR** | Secondary Pull: `file_cluster_7`
  * Top Architectural Signatures: indent_spaces: 149, structural_boundaries: 70, encapsulation: 50, branch: 38
- `python-dateutil-2.6.1/dateutil/parser.py` (PYTHON) | Magnitude: 284.6 | Delta: **0.195 IQR** | Secondary Pull: `file_cluster_13`
...
- `python-dateutil-2.6.1/dateutil/tz/win.py` -> **Severity: 14.262** (Embedded: 0.1838 * Error Risk: 77.5851%)
- `python-dateutil-2.6.1/dateutil/tz/tz.py` -> **Severity: 13.478** (Embedded: 0.1765 * Error Risk: 76.3769%)
- `python-dateutil-2.6.1/dateutil/relativedelta.py` -> **Severity: 10.488** (Embedded: 0.1345 * Error Risk: 78.0008%)
- `python-dateutil-2.6.1/dateutil/tzwin.py` -> **Severity: 10.019** (Embedded: 0.1225 * Error Risk: 81.7574%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `python-dateutil-2.6.1/dateutil/tz/win.py` -> **Severity: 10434.013** (Blast Radius: 268.077 * Doc Risk: 38.9217%)
- `python-dateutil-2.6.1/dateutil/_common.py` -> **Severity: 2096.738** (Blast Radius: 22.148 * Doc Risk: 94.6694%)
- `python-dateutil-2.6.1/dateutil/relativedelta.py` -> **Severity: 1090.727** (Blast Radius: 55.564 * Doc Risk: 19.6301%)
- `python-dateutil-2.6.1/dateutil/zoneinfo/__init__.py` -> **Severity: 937.66** (Blast Radius: 22.148 * Doc Risk: 42.3361%)
- `python-dateutil-2.6.1/dateutil/tz/tz.py` -> **Severity: 937.246** (Blast Radius: 78.626 * Doc Risk: 11.9203%)
