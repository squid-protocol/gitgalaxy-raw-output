...
| **Timestamp** | `2026-08-07T05:16:57.620907+00:00` |
| **Scan Duration** | `0.08s` |
| **Git Branch** | `N/A` |
| **Git Commit** | `N/A` |
| **Git Remote** | `N/A` |
...
| Error & Exception Exposure | 0.0 | 80.9 | 43.1 | 62.2 | 0.0 |
| Tech Debt Exposure | 0.0 | 100.0 | 28.5 | 0.0 | 0.0 |
| Testing Exposure | 0.3 | 2.8 | 2.0 | 2.3 | 2.3 |
...
- `promisify` (@ `package/implementation.js`) -> Impact: **31.6** | LOC: 77
- `fn` (@ `package/implementation.js`) -> Impact: **13.1** | LOC: 20
- `origApply` (@ `package/implementation.js`) -> Impact: **12.8** | LOC: 14
- `reject` (@ `package/implementation.js`) -> Impact: **7.3** | LOC: 8
- `forEach` (@ `package/implementation.js`) -> Impact: **5.5** | LOC: 7
- `getPolyfill` (@ `package/polyfill.js`) -> Impact: **5.5** | LOC: 6
- `shimUtilPromisify` (@ `package/shim.js`) -> Impact: **4.1** | LOC: 12
- `resolve` (@ `package/implementation.js`) -> Impact: **3.5** | LOC: 9
- `resolve` (@ `package/implementation.js`) -> Impact: **2.2** | LOC: 4
- `sPO` (@ `package/implementation.js`) -> Impact: **2.2** | LOC: 4

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `package` | 10 | 284.48 | 30.28% | 19.97% |

## 10. TARGETED RISK VECTORS (Top 5 by Exposure)
### Highest Tech Debt (Fragile/Planned)
- `package/implementation.js` -> **99.9999%** Exposure
...
- `package/implementation.js` -> **0** Orphaned Functions | **6** Duplicates
...
### 1. `package/implementation.js` (JAVASCRIPT) -> Cumulative Risk: **651.27**
- **Archetype:** `file_cluster_4` (Distance: 14.628 IQR)
- **Magnitude:** 197.0 | **LOC:** 123 | **CtrlFlow:** 41.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.9999%)
- **Heaviest Functions:** `promisify` (Impact: 31.6), `fn` (Impact: 13.1), `origApply` (Impact: 12.8)

### 2. `package/index.js` (JAVASCRIPT) -> Cumulative Risk: **529.5**
...
### 3. `package/shim.js` (JAVASCRIPT) -> Cumulative Risk: **441.13**
- **Archetype:** `file_cluster_13` (Distance: 12.199 IQR)
- **Magnitude:** 15.4 | **LOC:** 18 | **CtrlFlow:** 20.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Cognitive Load (89.4101%), Safety Score (79.8991%)
- **Heaviest Functions:** `shimUtilPromisify` (Impact: 4.1)

### 4. `package/polyfill.js` (JAVASCRIPT) -> Cumulative Risk: **294.39**
- **Archetype:** `file_cluster_13` (Distance: 14.111 IQR)
- **Magnitude:** 13.68 | **LOC:** 12 | **CtrlFlow:** 33.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Safety Score (62.1649%), Spec Match (60.0%), Stability (50.0%)
...
- **Global Archetype:** `file_cluster_4` (Drift: 14.628 IQR)
- **Local Micro-Species:** `Cluster 2: Procedural Core & Safety Wrappers` (Drift: 5.38 IQR)
- **Top Global Matches:** file_cluster_4: 14.628, file_cluster_13: 14.88, file_cluster_11: 15.084
- **Magnitude:** 197.0 | **LOC:** 123 | **CtrlFlow:** 41.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (99.8911%), Tech Debt (99.9999%)
**Top Internal Functions/Classes:**
  * `promisify` (Impact: 31.6)
  * `fn` (Impact: 13.1)
  * `origApply` (Impact: 12.8)
  * `reject` (Impact: 7.3)
  * `forEach` (Impact: 5.5)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 25`, `structural_boundaries: 35`, `args: 8`, `func_start: 18`
* *Risk/State:* `state_mutation: 76`, `duplicate_logic: 6`
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` es-object-atoms, call-bind, for-each, object.getownpropertydescriptors, define-data-property, safe-array-concat, has-proto, call-bound...
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 3):` implementation, shim, define-properties, polyfill, util
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- `package/implementation.js` (JAVASCRIPT) | Magnitude: 197.0 | Delta: **0.252 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: state_mutation: 76, indent_tabs: 67, structural_boundaries: 35, concurrency: 30
...
- `package/implementation.js` -> **Severity: 20.553** (Embedded: 0.254 * Error Risk: 80.9261%)
- `package/shim.js` -> **Severity: 17.755** (Embedded: 0.2222 * Error Risk: 79.8991%)
- `package/polyfill.js` -> **Severity: 15.541** (Embedded: 0.25 * Error Risk: 62.1649%)
