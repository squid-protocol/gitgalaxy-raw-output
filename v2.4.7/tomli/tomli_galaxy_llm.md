...
| **Timestamp** | `2026-08-07T05:27:00.502729+00:00` |
| **Scan Duration** | `0.11s` |
| **Git Branch** | `N/A` |
| **Git Commit** | `N/A` |
| **Git Remote** | `N/A` |
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 5.0 | 19.3 | 9.7 | 7.3 | 5.0 |
| Error & Exception Exposure | 0.0 | 86.7 | 48.4 | 53.5 | 0.0 |
...
| Documentation Exposure | 8.3 | 42.9 | 27.2 | 28.8 | 15.1 |
...
- `parse_multiline_str` (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **45.4** | LOC: 59
- `match_to_datetime` (@ `tomli-2.4.1/src/tomli/_re.py`) -> Impact: **23.6** | LOC: 31
- `match_to_localtime` (@ `tomli-2.4.1/src/tomli/_re.py`) -> Impact: **14.2** | LOC: 5
- `match_to_number` (@ `tomli-2.4.1/src/tomli/_re.py`) -> Impact: **10.6** | LOC: 4
- `make_safe_parse_float` (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **8.7** | LOC: 14
- `parse_hex_char` (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **8.6** | LOC: 11
- `cached_tz` (@ `tomli-2.4.1/src/tomli/_re.py`) -> Impact: **6.4** | LOC: 8
- `safe_parse_float` (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **6.2** | LOC: 5
  * *Intent:* # Pure Python should have raised RecursionError already.
- `is_unicode_scalar_value` (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **4.1** | LOC: 2
- `parse_literal_str` (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **2.1** | LOC: 7

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `tomli-2.4.1/src/tomli` | 4 | 241.48 | 9.7% | 0.0% |
...
### 1. `tomli-2.4.1/src/tomli/_parser.py` (PYTHON) -> Cumulative Risk: **363.28**
- **Archetype:** `file_cluster_8` (Distance: 11.06 IQR)
- **Magnitude:** 153.7 | **LOC:** 794 | **CtrlFlow:** 51.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Verification (80.0%), Stability (50.0%), Safety Score (46.6043%)
- **Heaviest Functions:** `parse_multiline_str` (Impact: 45.4), `make_safe_parse_float` (Impact: 8.7), `parse_hex_char` (Impact: 8.6)

### 2. `tomli-2.4.1/src/tomli/_re.py` (PYTHON) -> Cumulative Risk: **348.11**
- **Archetype:** `file_cluster_8` (Distance: 8.803 IQR)
- **Magnitude:** 63.14 | **LOC:** 120 | **CtrlFlow:** 53.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Verification (80.0%), Safety Score (60.4308%), Stability (50.0%)
- **Heaviest Functions:** `match_to_datetime` (Impact: 23.6), `match_to_localtime` (Impact: 14.2), `match_to_number` (Impact: 10.6)

### 3. `tomli-2.4.1/src/tomli/_types.py` (PYTHON) -> Cumulative Risk: **177.25**
- **Archetype:** `file_cluster_16` (Distance: 6.99 IQR)
- **Magnitude:** 12.08 | **LOC:** 11 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Safety Score (86.7036%), Stability (50.0%), Spec Match (26.6667%), Documentation (8.2626%)

### 4. `tomli-2.4.1/src/tomli/__init__.py` (PYTHON) -> Cumulative Risk: **91.96**
- **Archetype:** `file_cluster_8` (Distance: 6.261 IQR)
- **Magnitude:** 12.56 | **LOC:** 9 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Stability (50.0%), Spec Match (20.0%), Documentation (15.0515%), Cognitive Load (5.0%)
...
- **Top Global Matches:** file_cluster_8: 11.06, file_cluster_16: 11.075, file_cluster_13: 11.31
- **Magnitude:** 153.7 | **LOC:** 794 | **CtrlFlow:** 51.5% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (19.3%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `parse_multiline_str` (Impact: 45.4)
  * `make_safe_parse_float` (Impact: 8.7)
  * `parse_hex_char` (Impact: 8.6)
  * `safe_parse_float` (Impact: 6.2)
    * *Intent:* # Pure Python should have raised RecursionError already.
  * `is_unicode_scalar_value` (Impact: 4.1)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 2):` types, warnings, __future__, ._types, sys, ._re, collections.abc, typing
...
**Top Internal Functions/Classes:**
  * `match_to_datetime` (Impact: 23.6)
  * `match_to_localtime` (Impact: 14.2)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 1):` datetime, re, __future__, ._types, functools, typing
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
- `tomli-2.4.1/src/tomli/_parser.py` (PYTHON) | Magnitude: 153.7 | Delta: **0.015 IQR** | Secondary Pull: `file_cluster_16`
  * Top Architectural Signatures: indent_spaces: 473, branch: 134, structural_boundaries: 126, generics: 56
- `tomli-2.4.1/src/tomli/_re.py` (PYTHON) | Magnitude: 63.14 | Delta: **0.08 IQR** | Secondary Pull: `file_cluster_13`
...
- `tomli-2.4.1/src/tomli/_types.py` -> **Severity: 48.771** (Embedded: 0.5625 * Error Risk: 86.7036%)
- `tomli-2.4.1/src/tomli/_re.py` -> **Severity: 20.144** (Embedded: 0.3333 * Error Risk: 60.4308%)
- `tomli-2.4.1/src/tomli/_parser.py` -> **Severity: 11.651** (Embedded: 0.25 * Error Risk: 46.6043%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `tomli-2.4.1/src/tomli/_parser.py` -> **Severity: 8878.062** (Blast Radius: 206.916 * Doc Risk: 42.9066%)
- `tomli-2.4.1/src/tomli/_re.py` -> **Severity: 8519.734** (Blast Radius: 199.786 * Doc Risk: 42.6443%)
- `tomli-2.4.1/src/tomli/_types.py` -> **Severity: 3053.89** (Blast Radius: 369.604 * Doc Risk: 8.2626%)
- `tomli-2.4.1/src/tomli/__init__.py` -> **Severity: 1683.465** (Blast Radius: 111.847 * Doc Risk: 15.0515%)
