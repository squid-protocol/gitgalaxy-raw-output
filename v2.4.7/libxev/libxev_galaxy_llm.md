...
| **Timestamp** | `2026-08-07T04:29:21.903781+00:00` |
| **Scan Duration** | `0.43s` |
| **Git Branch** | `main` |
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 0.0 | 94.1 | 19.7 | 9.2 | 5.0 |
| Error & Exception Exposure | 0.0 | 93.2 | 36.5 | 50.7 | 0.0 |
| Tech Debt Exposure | 0.0 | 100.0 | 50.8 | 56.6 | 0.0 |
| Testing Exposure | 0.0 | 80.0 | 29.8 | 2.4 | 80.0 |
...
| Documentation Exposure | 0.0 | 100.0 | 42.6 | 39.9 | 0.0 |
...
- `TCPTests` (@ `src/watcher/tcp.zig`) -> Impact: **215.6** | LOC: 432
- `FileTests` (@ `src/watcher/file.zig`) -> Impact: **207.4** | LOC: 348
- `start` (@ `src/backend/epoll.zig`) -> Impact: **198.3** | LOC: 295
- `start_completion` (@ `src/backend/iocp.zig`) -> Impact: **164.7** | LOC: 315
  * *Intent:* // Start the completion.
- `tick` (@ `src/backend/kqueue.zig`) -> Impact: **151.4** | LOC: 257
  * *Intent:* /// Tick through the event loop once, waiting for at least "wait" completions /// to be processed by the loop itself.
- `Xev` (@ `src/dynamic.zig`) -> Impact: **135.5** | LOC: 205
...
- `Writeable` (@ `src/watcher/stream.zig`) -> Impact: **112.9** | LOC: 418

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `src/watcher` | 8 | 5579.14 | 16.47% | 87.12% |
| `src/backend` | 5 | 5192.56 | 21.99% | 89.02% |
| `src` | 13 | 2109.66 | 17.15% | 44.59% |
| `src/bench` | 12 | 724.28 | 30.66% | 70.38% |
| `examples` | 5 | 187.24 | 45.86% | 0.0% |
| `include` | 1 | 88.4 | 38.26% | 41.17% |
| `src/linux` | 1 | 47.24 | 14.23% | 0.0% |
| `__monolith__` | 3 | 37.54 | 4.03% | 0.0% |
| `website` | 4 | 36.36 | 5.93% | 24.08% |
...
- `src/watcher/tcp.zig` -> **0** Orphaned Functions | **35** Duplicates
- `src/c_api.zig` -> **27** Orphaned Functions | **5** Duplicates
- `src/watcher/stream.zig` -> **0** Orphaned Functions | **32** Duplicates
- `src/watcher/udp.zig` -> **0** Orphaned Functions | **32** Duplicates
...
### 1. `src/bench/async1.zig` (ZIG) -> Cumulative Risk: **622.35**
- **Archetype:** `file_cluster_4` (Distance: 13.555 IQR)
- **Magnitude:** 107.82 | **LOC:** 108 | **CtrlFlow:** 68.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.8741%), State Flux (93.907%), Verification (80.0%)
- **Heaviest Functions:** `run` (Impact: 23.4), `mainAsyncCallback` (Impact: 15.4), `asyncCallback` (Impact: 15.3)

### 2. `src/bench/async_pummel_1.zig` (ZIG) -> Cumulative Risk: **615.95**
- **Archetype:** `file_cluster_4` (Distance: 13.844 IQR)
- **Magnitude:** 82.06 | **LOC:** 84 | **CtrlFlow:** 56.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9931%), State Flux (99.3028%)
- **Heaviest Functions:** `run` (Impact: 23.9), `asyncCallback` (Impact: 15.5), `threadMain` (Impact: 3.2)

### 3. `src/backend/kqueue.zig` (ZIG) -> Cumulative Risk: **599.43**
- **Archetype:** `file_cluster_8` (Distance: 13.358 IQR)
- **Magnitude:** 1384.1 | **LOC:** 2900 | **CtrlFlow:** 72.4% | **Authorship Centralization:** 40.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Churn (100.0%), Tech Debt (97.3387%), State Flux (85.1623%)
- **Heaviest Functions:** `tick` (Impact: 151.4), `perform` (Impact: 115.1), `start` (Impact: 96.2)

### 4. `src/queue_mpsc.zig` (ZIG) -> Cumulative Risk: **555.38**
- **Archetype:** `file_cluster_4` (Distance: 13.224 IQR)
- **Magnitude:** 90.42 | **LOC:** 117 | **CtrlFlow:** 69.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.9979%), Tech Debt (93.691%), Verification (80.0%)
- **Heaviest Functions:** `Intrusive` (Impact: 26.9), `pop` (Impact: 20.5), `init` (Impact: 2.0)

### 5. `src/watcher/file.zig` (ZIG) -> Cumulative Risk: **542.17**
- **Archetype:** `file_cluster_8` (Distance: 12.454 IQR)
- **Magnitude:** 846.1 | **LOC:** 976 | **CtrlFlow:** 58.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.403%), State Flux (83.2179%), Verification (80.0%)
- **Heaviest Functions:** `FileTests` (Impact: 207.4), `FileStream` (Impact: 126.6), `FileDynamic` (Impact: 43.0)

### 6. `src/bench/million-timers.zig` (ZIG) -> Cumulative Risk: **539.97**
- **Archetype:** `file_cluster_13` (Distance: 14.777 IQR)
- **Magnitude:** 45.74 | **LOC:** 64 | **CtrlFlow:** 50.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9456%), Tech Debt (86.1395%), Cognitive Load (79.9731%)
- **Heaviest Functions:** `main` (Impact: 14.9), `timerCallback` (Impact: 7.8)

### 7. `src/watcher/tcp.zig` (ZIG) -> Cumulative Risk: **522.24**
- **Archetype:** `file_cluster_8` (Distance: 13.088 IQR)
- **Magnitude:** 928.1 | **LOC:** 936 | **CtrlFlow:** 67.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9666%), State Flux (92.9981%), Verification (80.0%)
- **Heaviest Functions:** `TCPTests` (Impact: 215.6), `TCPStream` (Impact: 83.2), `TCPDynamic` (Impact: 78.8)

### 8. `src/backend/io_uring.zig` (ZIG) -> Cumulative Risk: **518.05**
- **Archetype:** `file_cluster_8` (Distance: 13.233 IQR)
- **Magnitude:** 848.28 | **LOC:** 1792 | **CtrlFlow:** 69.6% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (95.6815%), State Flux (88.8162%), Verification (80.0%)
- **Heaviest Functions:** `invoke` (Impact: 119.5), `tick_` (Impact: 71.0), `add_` (Impact: 68.3)

### 9. `src/watcher/process.zig` (ZIG) -> Cumulative Risk: **511.5**
- **Archetype:** `file_cluster_8` (Distance: 12.116 IQR)
- **Magnitude:** 661.68 | **LOC:** 573 | **CtrlFlow:** 64.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9304%), Verification (80.0%), State Flux (78.0183%)
- **Heaviest Functions:** `ProcessTests` (Impact: 71.9), `ProcessIocp` (Impact: 66.6), `wait` (Impact: 62.9)

### 10. `src/backend/iocp.zig` (ZIG) -> Cumulative Risk: **510.56**
- **Archetype:** `file_cluster_8` (Distance: 13.302 IQR)
- **Magnitude:** 1138.96 | **LOC:** 2377 | **CtrlFlow:** 66.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (94.929%), Tech Debt (92.2307%), Verification (80.0%)
- **Heaviest Functions:** `start_completion` (Impact: 164.7), `tick` (Impact: 99.3), `perform` (Impact: 97.4)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 0):` main.zig, std, queue.zig, builtin, loop.zig, queue_mpsc.zig, ThreadPool.zig, heap.zig...
...
- **Magnitude:** 1138.96 | **LOC:** 2377 | **CtrlFlow:** 66.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (28.8897%), Tech Debt (92.2307%)
**Top Internal Functions/Classes:**
  * `start_completion` (Impact: 164.7)
    * *Intent:* // Start the completion.
  * `tick` (Impact: 99.3)
    * *Intent:* /// Tick through the event loop once, waiting for at least "wait" completions to be processed by ///...
  * `perform` (Impact: 97.4)
    * *Intent:* /// Perform the operation associated with this completion. This will perform the full blocking /// o...
  * `stop_completion` (Impact: 30.6)
    * *Intent:* /// Stop the completion. Fill `cancel_result` if it is non-null.
  * `timer_reset` (Impact: 24.5)
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` std, queue.zig, builtin, loop.zig, windows.zig, heap.zig
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `src/watcher/stream.zig` (ZIG | Tier 2 | 🚨 AI THREAT: 99.03%)
- **Global Archetype:** `file_cluster_8` (Drift: 11.729 IQR)
- **Top Global Matches:** file_cluster_8: 11.729, file_cluster_16: 11.932, file_cluster_7: 11.942
- **Magnitude:** 1119.04 | **LOC:** 1444 | **CtrlFlow:** 70.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (9.1184%), Tech Debt (97.7345%)
**Top Internal Functions/Classes:**
  * `Writeable` (Impact: 112.9)
  * `GenericStreamTests` (Impact: 111.2)
  * `Pollable` (Impact: 65.0)
  * `Stream` (Impact: 57.1)
    * *Intent:* /// Creates a stream type that is meant to be embedded within other types. /// A stream is something...
  * `Readable` (Impact: 54.8)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 225`, `structural_boundaries: 96`, `args: 48`, `func_start: 48`, `class_start: 7`
* *Risk/State:* `safety_bypasses: 32`, `state_mutation: 73`, `dead_code: 1`, `duplicate_logic: 32`
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 1):` common.zig, builtin, std, queue.zig
...
- **Magnitude:** 1037.1 | **LOC:** 2062 | **CtrlFlow:** 71.7% | **Authorship Centralization:** 0.0%
...
**Top Internal Functions/Classes:**
  ...
  * `perform` (Impact: 71.7)
    * *Intent:* /// Perform the operation associated with this completion. This will /// perform the full blocking o...
  * `stop_completion` (Impact: 30.5)
  * `timer_reset` (Impact: 27.4)
    * *Intent:* /// See io_uring.timer_reset for docs.
**Structural Signatures (Net Mitigated Signals):**
...
- **Global Archetype:** `file_cluster_8` (Drift: 13.088 IQR)
- **Top Global Matches:** file_cluster_8: 13.088, file_cluster_13: 13.115, file_cluster_0: 13.183
- **Magnitude:** 928.1 | **LOC:** 936 | **CtrlFlow:** 67.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (26.9705%), Tech Debt (99.9666%)
**Top Internal Functions/Classes:**
  * `TCPTests` (Impact: 215.6)
  * `TCPStream` (Impact: 83.2)
  * `TCPDynamic` (Impact: 78.8)
  * `init` (Impact: 18.3)
    * *Intent:* /// Initialize a new TCP with the family from the given address. Only /// the family is used, the ac...
  * `accept` (Impact: 17.5)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 183`, `structural_boundaries: 90`, `args: 42`, `func_start: 42`, `class_start: 1`
* *Risk/State:* `safety_bypasses: 25`, `state_mutation: 128`, `dead_code: 4`, `duplicate_logic: 35`
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 2):` std, common.zig, builtin, stream.zig, ThreadPool.zig
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `src/backend/io_uring.zig` (ZIG | Tier 2 | 🚨 AI THREAT: 99.17%)
- **Global Archetype:** `file_cluster_8` (Drift: 13.233 IQR)
- **Top Global Matches:** file_cluster_8: 13.233, file_cluster_7: 13.39, file_cluster_13: 13.469
- **Magnitude:** 848.28 | **LOC:** 1792 | **CtrlFlow:** 69.6% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (13.4215%), Tech Debt (95.6815%)
**Top Internal Functions/Classes:**
  * `invoke` (Impact: 119.5)
  ...
  * `readResult` (Impact: 27.1)
  * `timer_reset` (Impact: 24.4)
    * *Intent:* /// If the timer completion "c" is dead, this is equivalent to calling /// "timer" and "c_cancel" is...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 335`, `structural_boundaries: 146`, `args: 40`, `func_start: 40`, `class_start: 20`
* *Risk/State:* `safety_bypasses: 81`, `state_mutation: 208`, `dead_code: 3`, `planned_debt: 4`, `fragile_debt: 1`, `duplicate_logic: 21`
* *Architecture:* `io: 6`, `api: 33`, `import: 5`
* *Defense:* `safety: 160`, `doc: 157`, `test: 16`, `immutability_locks: 96`, `cleanup: 15`
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` std, queue.zig, builtin, loop.zig, timerfd.zig
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `src/watcher/file.zig` (ZIG | Tier 2 | 🚨 AI THREAT: 99.13%)
- **Global Archetype:** `file_cluster_8` (Drift: 12.454 IQR)
- **Top Global Matches:** file_cluster_8: 12.454, file_cluster_13: 12.604, file_cluster_7: 12.709
- **Magnitude:** 846.1 | **LOC:** 976 | **CtrlFlow:** 58.6% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (35.0505%), Tech Debt (99.403%)
**Top Internal Functions/Classes:**
  * `FileTests` (Impact: 207.4)
  * `FileStream` (Impact: 126.6)
    * *Intent:* /// An implementation of File that uses the stream abstractions.
  * `FileDynamic` (Impact: 43.0)
  * `pread` (Impact: 29.2)
  * `queuePWrite` (Impact: 29.0)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 167`, `structural_boundaries: 118`, `args: 34`, `func_start: 34`
* *Risk/State:* `safety_bypasses: 29`, `state_mutation: 117`, `dead_code: 1`, `duplicate_logic: 26`
* *Architecture:* `io: 23`, `api: 23`, `import: 5`
* *Defense:* `safety: 77`, `doc: 25`, `test: 7`, `immutability_locks: 73`, `cleanup: 11`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 2):` main.zig, std, common.zig, builtin, stream.zig
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Magnitude:** 784.12 | **LOC:** 1649 | **CtrlFlow:** 72.2% | **Authorship Centralization:** 0.0%
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` main.zig, std, queue.zig, builtin, heap.zig
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `src/watcher/async.zig` (ZIG | Tier 2 | 🚨 AI THREAT: 99.06%)
- **Global Archetype:** `file_cluster_8` (Drift: 12.069 IQR)
- **Top Global Matches:** file_cluster_8: 12.069, file_cluster_7: 12.262, file_cluster_1: 12.307
- **Magnitude:** 701.92 | **LOC:** 829 | **CtrlFlow:** 69.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (9.1162%), Tech Debt (99.9996%)
**Top Internal Functions/Classes:**
  * `AsyncMachPort` (Impact: 79.0)
    * *Intent:* /// Async implementation using mach ports (Darwin). /// /// This allocates a mach port per async req...
  * `AsyncEventFd` (Impact: 69.2)
    * *Intent:* /// Async implementation using eventfd (Unix/Linux).
  * `AsyncTests` (Impact: 66.4)
  * `AsyncDynamic` (Impact: 35.0)
  * `AsyncLoopState` (Impact: 24.9)
...
* *Risk/State:* `safety_bypasses: 13`, `state_mutation: 60`, `planned_debt: 2`, `duplicate_logic: 28`
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` common.zig, builtin, std, darwin.zig
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `src/watcher/udp.zig` (ZIG | Tier 2 | 🚨 AI THREAT: 99.13%)
- **Global Archetype:** `file_cluster_8` (Drift: 11.515 IQR)
- **Top Global Matches:** file_cluster_8: 11.515, file_cluster_7: 11.751, file_cluster_1: 11.825
- **Magnitude:** 687.76 | **LOC:** 1004 | **CtrlFlow:** 68.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (8.5752%), Tech Debt (99.9981%)
**Top Internal Functions/Classes:**
  * `UDPSendMsg` (Impact: 62.3)
    * *Intent:* /// UDP implementation that uses sendmsg/recvmsg
  * `UDPDynamic` (Impact: 60.9)
  * `UDPTests` (Impact: 53.7)
  * `UDPSendto` (Impact: 34.9)
    * *Intent:* /// UDP implementation that uses sendto/recvfrom.
  * `UDPSendtoIOCP` (Impact: 32.8)
    * *Intent:* /// UDP implementation that uses sendto/recvfrom.
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 103`, `structural_boundaries: 47`, `args: 39`, `func_start: 39`, `class_start: 3`
* *Risk/State:* `safety_bypasses: 33`, `state_mutation: 61`, `dead_code: 1`, `planned_debt: 1`, `duplicate_logic: 32`
...
- **Top Global Matches:** file_cluster_8: 12.116, file_cluster_13: 12.294, file_cluster_16: 12.345
- **Magnitude:** 661.68 | **LOC:** 573 | **CtrlFlow:** 64.3% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (22.3014%), Tech Debt (99.9304%)
**Top Internal Functions/Classes:**
  * `ProcessTests` (Impact: 71.9)
  * `ProcessIocp` (Impact: 66.6)
  * `wait` (Impact: 62.9)
  * `callback` (Impact: 61.6)
  * `ProcessPidFd` (Impact: 53.8)
    * *Intent:* /// Process implementation using pidfd (Linux).
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 110`, `structural_boundaries: 61`, `args: 25`, `func_start: 25`
* *Risk/State:* `safety_bypasses: 11`, `state_mutation: 67`, `duplicate_logic: 19`
* *Architecture:* `io: 3`, `api: 18`, `import: 4`
* *Defense:* `safety: 38`, `doc: 14`, `test: 3`, `immutability_locks: 50`, `cleanup: 11`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` common.zig, builtin, std, windows.zig
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `src/watcher/timer.zig` (ZIG | Tier 2 | 🚨 AI THREAT: 99.06%)
- **Global Archetype:** `file_cluster_8` (Drift: 12.98 IQR)
- **Top Global Matches:** file_cluster_8: 12.98, file_cluster_13: 13.08, file_cluster_7: 13.14
- **Magnitude:** 625.2 | **LOC:** 642 | **CtrlFlow:** 62.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (15.6318%), Tech Debt (99.9555%)
**Top Internal Functions/Classes:**
  * `TimerTests` (Impact: 102.0)
  * `TimerLoop` (Impact: 84.7)
    * *Intent:* /// An implementation that uses the loop timer methods.
  * `TimerDynamic` (Impact: 46.6)
  * `cancel` (Impact: 33.1)
...
* *Risk/State:* `safety_bypasses: 17`, `state_mutation: 96`, `dead_code: 2`, `duplicate_logic: 23`
...
- **Global Archetype:** `file_cluster_13` (Drift: 12.178 IQR)
- **Top Global Matches:** file_cluster_13: 12.178, file_cluster_16: 12.282, file_cluster_8: 12.366
- **Magnitude:** 500.26 | **LOC:** 525 | **CtrlFlow:** 72.5% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (11.3183%), Tech Debt (99.5176%)
**Top Internal Functions/Classes:**
  * `Xev` (Impact: 135.5)
    * *Intent:* /// The goal of this API is to match the static Xev() (in main.zig) /// API as closely as possible. ...
  * `DynamicCompletion` (Impact: 28.6)
  * `Union` (Impact: 24.8)
    * *Intent:* /// Creates a union type that can hold the implementation of a given /// backend by common field nam...
  * `DynamicReadBuffer` (Impact: 20.1)
  * `DynamicPollEvent` (Impact: 17.6)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 111`, `structural_boundaries: 42`, `args: 31`, `func_start: 31`, `class_start: 1`
* *Risk/State:* `safety_bypasses: 4`, `state_mutation: 27`, `dead_code: 2`, `duplicate_logic: 14`
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 2):` udp.zig, file.zig, stream.zig, process.zig, main.zig, std, loop.zig, tcp.zig...
...
- **Magnitude:** 440.12 | **LOC:** 828 | **CtrlFlow:** 70.3% | **Authorship Centralization:** 0.0%
...
- **Top Global Matches:** file_cluster_8: 13.258, file_cluster_7: 13.429, file_cluster_16: 13.483
- **Magnitude:** 276.7 | **LOC:** 380 | **CtrlFlow:** 58.2% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (27.1867%), Tech Debt (92.4142%)
**Top Internal Functions/Classes:**
  * `Intrusive` (Impact: 73.1)
...
- **Global Archetype:** `file_cluster_8` (Drift: 8.449 IQR)
- **Top Global Matches:** file_cluster_8: 8.449, file_cluster_7: 8.975, file_cluster_1: 9.415
- **Magnitude:** 248.02 | **LOC:** 252 | **CtrlFlow:** 73.3% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (9.2262%), Tech Debt (56.5985%)
**Top Internal Functions/Classes:**
  * `DeleteFileW` (Impact: 78.8)
  * `CreateFile` (Impact: 18.9)
  * `ReadFile` (Impact: 14.3)
  * `WriteFile` (Impact: 14.3)
  * `SetInformationJobObject` (Impact: 13.2)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 33`, `structural_boundaries: 12`, `args: 12`, `func_start: 12`, `class_start: 11`
* *Risk/State:* `safety_bypasses: 4`, `state_mutation: 2`, `duplicate_logic: 2`
* *Architecture:* `io: 3`, `api: 71`, `import: 1`
...
- **Magnitude:** 236.66 | **LOC:** 362 | **CtrlFlow:** 57.1% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (21.8245%), Tech Debt (99.9932%)
**Top Internal Functions/Classes:**
  * `readCallback` (Impact: 27.1)
  * `readCallback` (Impact: 24.4)
  * `main` (Impact: 16.5)
  * `acceptCallback` (Impact: 13.1)
  * `writeCallback` (Impact: 12.4)
**Structural Signatures (Net Mitigated Signals):**
...
- **Top Global Matches:** file_cluster_8: 12.193, file_cluster_13: 12.418, file_cluster_0: 12.52
- **Magnitude:** 226.96 | **LOC:** 361 | **CtrlFlow:** 72.3% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (17.344%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `xev_timer_run` (Impact: 11.7)
  * `xev_timer_reset` (Impact: 11.7)
  * `xev_timer_cancel` (Impact: 11.7)
  * `xev_async_wait` (Impact: 11.6)
  * `callback` (Impact: 10.7)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 47`, `structural_boundaries: 18`, `args: 34`, `func_start: 33`, `class_start: 2`
* *Risk/State:* `safety_bypasses: 28`, `state_mutation: 16`, `dead_code: 1`, `planned_debt: 2`, `duplicate_logic: 5`, `orphaned_logic: 27`
* *Architecture:* `api: 27`, `import: 3`
* *Defense:* `safety: 28`, `doc: 5`, `test: 1`, `immutability_locks: 45`, `cleanup: 4`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` main.zig, builtin, std
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- **Magnitude:** 121.88 | **LOC:** 180 | **CtrlFlow:** 63.6% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (23.0092%), Tech Debt (26.1351%)
**Top Internal Functions/Classes:**
  * `run` (Impact: 27.8)
  * `readCallback` (Impact: 27.3)
  * `writeCallback` (Impact: 13.1)
  * `closeCallback` (Impact: 8.4)
  * `init` (Impact: 5.6)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` xev, builtin, std
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
**Top Internal Functions/Classes:**
  * `run` (Impact: 23.4)
  * `mainAsyncCallback` (Impact: 15.4)
  * `asyncCallback` (Impact: 15.3)
  * `threadMain` (Impact: 7.5)
  * `init` (Impact: 5.3)
**Structural Signatures (Net Mitigated Signals):**
...
- **Magnitude:** 104.56 | **LOC:** 150 | **CtrlFlow:** 61.0% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (41.0769%), Tech Debt (32.2526%)
**Top Internal Functions/Classes:**
  * `run` (Impact: 27.4)
  * `readCallback` (Impact: 23.4)
  * `writeCallback` (Impact: 16.1)
  * `main` (Impact: 2.1)
**Structural Signatures (Net Mitigated Signals):**
...
- **Global Archetype:** `file_cluster_4` (Drift: 13.224 IQR)
- **Top Global Matches:** file_cluster_4: 13.224, file_cluster_6: 13.289, file_cluster_16: 13.341
- **Magnitude:** 90.42 | **LOC:** 117 | **CtrlFlow:** 69.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (41.2778%), Tech Debt (93.691%)
**Top Internal Functions/Classes:**
  * `Intrusive` (Impact: 26.9)
...
**Top Internal Functions/Classes:**
  * `run` (Impact: 23.9)
  * `asyncCallback` (Impact: 15.5)
  * `threadMain` (Impact: 3.2)
  * `main` (Impact: 2.1)
**Structural Signatures (Net Mitigated Signals):**
...
### `examples/million-timers.c` (C | Tier 1.5 | 🚨 AI THREAT: 98.93%)
- **Global Archetype:** `file_cluster_13` (Drift: 12.033 IQR)
- **Local Micro-Species:** `Cluster 2: Inert Headers & Declarative Structures` (Drift: 4.788 IQR)
- **Top Global Matches:** file_cluster_13: 12.033, file_cluster_8: 12.198, file_cluster_7: 12.661
- **Magnitude:** 78.88 | **LOC:** 92 | **CtrlFlow:** 35.3% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (60.6454%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `main` (Impact: 9.3)
    * *Intent:* #endif
  * `hrtime` (Impact: 3.9)
  ...
  * `timer_cb` (Impact: 1.2)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 6`, `structural_boundaries: 11`, `args: 3`, `func_start: 4`, `class_start: 1`
* *Risk/State:* `state_mutation: 44`, `orphaned_logic: 1`
* *Architecture:* `api: 19`, `import: 6`
* *Defense:* `cleanup: 2`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 9.813
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 1):` xev.h, stdlib.h, stdint.h, stdio.h, time.h, windows.h
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- `examples/async.c` (C) | Magnitude: 30.14 | Delta: **0.03 IQR** | Secondary Pull: `file_cluster_8`
  * Top Architectural Signatures: indent_spaces: 36, pointers: 21, structural_boundaries: 16, api: 12
- `src/api.zig` (ZIG) | Magnitude: 45.9 | Delta: **0.033 IQR** | Secondary Pull: `file_cluster_16`
...
- `website/pages/_document.tsx` (TYPESCRIPT) | Magnitude: 0.35 | Delta: **0.17 IQR** | Secondary Pull: `file_cluster_16`
  * Top Architectural Signatures: indent_spaces: 9, ui_framework: 4, generics: 4, structural_boundaries: 3

### Mixed-Responsibility Refactoring Targets for: file_cluster_4
- `src/queue_mpsc.zig` (ZIG) | Magnitude: 90.42 | Delta: **0.065 IQR** | Secondary Pull: `file_cluster_6`
  * Top Architectural Signatures: indent_spaces: 78, bitwise_ops: 29, doc: 24, branch: 23
- `src/bench/async1.zig` (ZIG) | Magnitude: 107.82 | Delta: **0.178 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 71, branch: 28, safety: 19, state_mutation: 18
- `src/bench/async_pummel_1.zig` (ZIG) | Magnitude: 82.06 | Delta: **0.292 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 46, branch: 18, state_mutation: 18, globals: 18

### Mixed-Responsibility Refactoring Targets for: file_cluster_7
- `src/loop.zig` (ZIG) | Magnitude: 26.12 | Delta: **0.002 IQR** | Secondary Pull: `file_cluster_8`
...
- `src/queue.zig` (ZIG) | Magnitude: 57.32 | Delta: **0.01 IQR** | Secondary Pull: `file_cluster_16`
  * Top Architectural Signatures: indent_spaces: 70, branch: 24, safety: 17, doc: 14
- `src/watcher/tcp.zig` (ZIG) | Magnitude: 928.1 | Delta: **0.027 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 822, branch: 183, state_mutation: 128, bitwise_ops: 104
- `src/bench/ping-pongs.zig` (ZIG) | Magnitude: 236.66 | Delta: **0.091 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 288, branch: 60, encapsulation: 59, globals: 50
- `src/watcher/timer.zig` (ZIG) | Magnitude: 625.2 | Delta: **0.1 IQR** | Secondary Pull: `file_cluster_13`
  * Top Architectural Signatures: indent_spaces: 558, branch: 101, state_mutation: 96, pointers: 75
...
- `src/backend/kqueue.zig` -> Churn: **100.0%** | Cog Load: 25.5177% | Debt: 97.3387%

### 👤 Key Person Dependencies (High Impact + Siloed Knowledge)
These are massive, load-bearing files written almost entirely by a single developer. They represent severe 'Bus Factor' risk.

- `src/watcher/file.zig` -> **Alexandre Negrel** (100.0% isolated ownership) | Magnitude: 846.1
- `src/windows.zig` -> **Mitchell Hashimoto** (100.0% isolated ownership) | Magnitude: 248.02
- `src/bench/async_pummel_1.zig` -> **Mitchell Hashimoto** (100.0% isolated ownership) | Magnitude: 82.06
...
- `include/xev.h` -> **Severity: 12.063** (Embedded: 0.1933 * Error Risk: 62.4025%)
- `src/watcher/common.zig` -> **Severity: 8.6** (Embedded: 0.0923 * Error Risk: 93.1715%)
- `src/bench/async_pummel_1.zig` -> **Severity: 3.431** (Embedded: 0.0462 * Error Risk: 74.3385%)
- `src/bench/async1.zig` -> **Severity: 3.413** (Embedded: 0.0462 * Error Risk: 73.956%)
- `src/ThreadPool.zig` -> **Severity: 2.549** (Embedded: 0.0385 * Error Risk: 66.284%)
...
- `src/bench/async1.zig` -> **Severity: 2777.557** (Blast Radius: 34.838 * Doc Risk: 79.7278%)
- `src/watcher/common.zig` -> **Severity: 2666.852** (Blast Radius: 57.985 * Doc Risk: 45.9921%)
