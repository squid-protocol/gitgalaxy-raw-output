...
| **Target Path** | `/srv/storage_16tb/projects/gitgalaxy/data/npm_top_200/node-gyp-build-optional-packages` |
| **Timestamp** | `2026-08-07T05:15:28.853227+00:00` |
| **Scan Duration** | `0.09s` |
| **Git Branch** | `N/A` |
| **Git Commit** | `N/A` |
| **Git Remote** | `N/A` |
...
> **Architectural Drift Z-Score:** `3.911`
...
|---|---|---|---|---|---|
| Cognitive Load Exposure | 5.0 | 66.9 | 25.6 | 5.0 | 5.0 |
| Error & Exception Exposure | 0.0 | 92.1 | 54.9 | 72.6 | 72.6 |
| Tech Debt Exposure | 0.0 | 100.0 | 33.3 | 0.0 | 0.0 |
| Testing Exposure | 0.4 | 2.7 | 1.3 | 0.9 | 2.7 |
...
2. **bin.js** (`package/bin.js`) — 0 inbound connections
3. **index.js** (`package/index.js`) — 0 inbound connections
4. **optional.js** (`package/optional.js`) — 0 inbound connections
...
- `build` (@ `package/bin.js`) -> Impact: **15.0** | LOC: 20
...
- `exec` (@ `package/bin.js`) -> Impact: **3.7** | LOC: 4
- `preinstall` (@ `package/bin.js`) -> Impact: **2.3** | LOC: 6
- `exec` (@ `package/bin.js`) -> Impact: **1.9** | LOC: 3

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Cog Load | Avg Debt |
|---|---|---|---|---|
| `package` | 5 | 99.3 | 15.39% | 20.0% |

## 10. TARGETED RISK VECTORS (Top 5 by Exposure)
### Highest Tech Debt (Fragile/Planned)
- `package/bin.js` -> **100.0%** Exposure
### Highest State Flux (Mutation/Volatility)
- `package/bin.js` -> **99.9958%** Exposure
### Highest Design Slop (Dead & Duplicated Logic)
- `package/bin.js` -> **0** Orphaned Functions | **5** Duplicates
...
### 1. `package/bin.js` (JAVASCRIPT) -> Cumulative Risk: **504.2**
- **Archetype:** `file_cluster_13` (Distance: 12.887 IQR)
- **Magnitude:** 70.96 | **LOC:** 83 | **CtrlFlow:** 57.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), State Flux (99.9958%), Safety Score (72.6115%)
- **Heaviest Functions:** `build` (Impact: 15.0), `hasFlag` (Impact: 8.4), `exec` (Impact: 6.8)

### 2. `package/optional.js` (JAVASCRIPT) -> Cumulative Risk: **162.36**
- **Archetype:** `file_cluster_8` (Distance: 6.655 IQR)
- **Magnitude:** 11.04 | **LOC:** 8 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Safety Score (92.0893%), Stability (50.0%), Spec Match (13.3333%), Cognitive Load (5.0%)
...
- **Local Micro-Species:** `Cluster 2: Procedural Core & Safety Wrappers` (Drift: 5.912 IQR)
- **Top Global Matches:** file_cluster_13: 12.887, file_cluster_11: 12.934, file_cluster_8: 13.204
- **Magnitude:** 70.96 | **LOC:** 83 | **CtrlFlow:** 57.6% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (66.9491%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `build` (Impact: 15.0)
  * `hasFlag` (Impact: 8.4)
    * *Intent:* // TODO (next major): remove in favor of env.npm_config_* which works since npm // 0.1.8 while npm_c...
  * `exec` (Impact: 6.8)
  * `preinstall` (Impact: 6.3)
  * `buildFromSource` (Impact: 4.2)
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 19`, `structural_boundaries: 14`, `args: 9`, `func_start: 10`
* *Risk/State:* `safety_bypasses: 1`, `high_risk_execution: 2`, `state_mutation: 21`, `planned_debt: 1`, `duplicate_logic: 5`
* *Architecture:* `io: 4`, `import: 4`
* *Defense:* `safety: 11`
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 200.0
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` package.json, child_process, os, path
...
- `package/bin.js` (JAVASCRIPT) | Magnitude: 70.96 | Delta: **0.047 IQR** | Secondary Pull: `file_cluster_11`
  * Top Architectural Signatures: indent_spaces: 51, state_mutation: 21, branch: 19, structural_boundaries: 14
...
- `package/bin.js` -> **Severity: 2384.06** (Blast Radius: 200.0 * Doc Risk: 11.9203%)
- `package/index.js` -> **Severity: 953.62** (Blast Radius: 200.0 * Doc Risk: 4.7681%)
- `package/optional.js` -> **Severity: 317.88** (Blast Radius: 200.0 * Doc Risk: 1.5894%)
