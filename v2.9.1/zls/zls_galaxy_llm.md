...
## 1. EXECUTIVE SUMMARY
- **Scope:** 117 analyzed artifact(s), 49370 LOC.
- **Load-bearing artifact:** `src/zls.zig` -- 24 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/zls.zig` -- pulls in 33 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `tests/utility/position_context.zig` at magnitude 5711.57 (structural weight, not risk).
...
| Avg Path Length | 2.1007 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.423`
> **Composition Archetype:** `Small Flat Repo (2)` (z +1.42; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules (2) 35%, Data / Markup / Trivial 18%, State Mutators Files 17%, Declarative / Non-Code 14%, Large Core Modules (3) 8%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 61.2 | 84.1 | 100.0 |
...
- `resolveTypeOfNodeUncached` **(Many-Argument Workhorses)** (@ `src/analysis.zig`) -> Impact: **442.6** | LOC: 1092
- `main` **(Defensive Guards)** (@ `src/build_runner/build_runner.zig`) -> Impact: **269.3** | LOC: 437
- `resolvePeerTypes` **(Many-Argument Workhorses)** (@ `src/analyser/InternPool.zig`) -> Impact: **234.8** | LOC: 495
- `resolveExpressionTypeFromAncestors` **(Many-Argument Workhorses)** (@ `src/analysis.zig`) -> Impact: **211.3** | LOC: 424
- `writeNodeTokens` **(Many-Argument Workhorses)** (@ `src/features/semantic_tokens.zig`) -> Impact: **193.1** | LOC: 675
- `getFieldAccessType` **(Many-Argument Workhorses)** (@ `src/analysis.zig`) -> Impact: **167.3** | LOC: 215
- `initializeHandler` **(Many-Argument Workhorses)** (@ `src/Server.zig`) -> Impact: **160.2** | LOC: 244
- `getSignatureInfo` **(Many-Argument Workhorses)** (@ `src/features/signature_help.zig`) -> Impact: **137.5** | LOC: 252
- `lastToken` **(Many-Argument Workhorses)** (@ `src/ast.zig`) -> Impact: **126.2** | LOC: 410
  * *Intent:* /// Similar to `std.zig.Ast.lastToken` but also handles ASTs with syntax errors.
- `getSwitchOrStructInitContext` **(Many-Argument Workhorses)** (@ `src/features/completions.zig`) -> Impact: **122.7** | LOC: 213
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 4.591; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (53.5%), Dead Code Surface (formerly Dead Code) (5.9%), Complexity Load (formerly Cognitive Load) (4.3%), Guard Balance (formerly Safety Score) (2.6%)
...
- **Blast Radius:** changing it is visible to **13** in-repo importer(s); it depends on **12**; blast radius 21.472; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Connectivity (formerly Api Exposure) (88.5%), Mutation Surface (formerly State Flux) (65.4%), Complexity Load (formerly Cognitive Load) (24.4%)
- **Documentation Coverage:** 84.1317% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolveTypeOfNodeUncached` **(Many-Argument Workhorses)** (Impact: 442.6)
  * `resolveExpressionTypeFromAncestors` **(Many-Argument Workhorses)** (Impact: 211.3)
  * `getFieldAccessType` **(Many-Argument Workhorses)** (Impact: 167.3)
  * `eql` **(Defensive Guards)** (Impact: 75.5)
  * `resolveType` **(Defensive Guards)** (Impact: 74.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **5**; blast radius 31.644; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (77.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (70.3%), Mutation Surface (formerly State Flux) (26.8%), Debt Markers (formerly Tech Debt) (10.6%)
- **Documentation Coverage:** 76.5336% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolvePeerTypes` **(Many-Argument Workhorses)** (Impact: 234.8)
  * `printInternal` **(Many-Argument Workhorses)** (Impact: 107.7)
  * `coerce` **(Many-Argument Workhorses)** (Impact: 102.3)
    * *Intent:* // --------------------------------------------- // UTILITY // -------------------------------------...
  * `eqlCustom` **(Many-Argument Workhorses)** (Impact: 94.4)
  * `coerceInMemoryAllowed` **(Many-Argument Workhorses)** (Impact: 64.8)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **13**; blast radius 11.86; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (94.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (89.7%), Complexity Load (formerly Cognitive Load) (40.3%), Connectivity (formerly Api Exposure) (38.5%)
- **Documentation Coverage:** 91.3043% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getSwitchOrStructInitContext` **(Many-Argument Workhorses)** (Impact: 122.7)
    * *Intent:* /// Looks for an identifier that can be passed to `collectContainerNodes()` /// Returns the token in...
  * `completeFileSystemStringLiteral` **(Defensive Guards)** (Impact: 119.8)
    * *Intent:* /// Asserts that `pos_context` is one of the following: /// - `.import_string_literal` /// - `.cincl...
  * `declToCompletion` **(Defensive Guards)** (Impact: 103.1)
  * `collectContainerFields` **(Many-Argument Workhorses)** (Impact: 94.4)
    * *Intent:* /// Given a Type that is a container, adds it's `.container_field*`s to completions
  * `getEnumLiteralContext` **(Many-Argument Workhorses)** (Impact: 47.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (81.1%), Complexity Load (formerly Cognitive Load) (38.5%), Guard Balance (formerly Safety Score) (23.7%)
- **Documentation Coverage:** 84.6154% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(Defensive Guards)** (Impact: 269.3)
  * `extractBuildInformation` **(Many-Argument Workhorses)** (Impact: 115.4)
  * `runPkgConfig` **(Defensive Guards)** (Impact: 70.5)
    * *Intent:* /// Run pkg-config for the given library name and parse the output, returning the arguments /// that...
  * `makeStep` **(Many-Argument Workhorses)** (Impact: 44.6)
    * *Intent:* /// Runs the "make" function of the single step `s`, updates its state, and then spawns newly-ready ...
  * `processPkgConfig` **(Defensive Guards)** (Impact: 28.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **27**; blast radius 12.907; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (82.7%), Mutation Surface (formerly State Flux) (65.5%), Connectivity (formerly Api Exposure) (52.7%), Concurrency Surface (formerly Concurrency) (15.2%)
- **Documentation Coverage:** 84.8837% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `initializeHandler` **(Many-Argument Workhorses)** (Impact: 160.2)
  * `handleResponse` **(Defensive Guards)** (Impact: 35.1)
  * `validateMessage` **(Defensive Guards)** (Impact: 26.4)
  * `codeActionHandler` **(Defensive Guards)** (Impact: 22.1)
  * `processMessageReportError` **(Defensive Guards)** (Impact: 22.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.4%), Connectivity (formerly Api Exposure) (83.3%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (60.9%)
- **Documentation Coverage:** 84.9057% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `lastToken` **(Many-Argument Workhorses)** (Impact: 126.2)
    * *Intent:* /// Similar to `std.zig.Ast.lastToken` but also handles ASTs with syntax errors.
  * `indexOfBreakTarget` **(Defensive Guards)** (Impact: 45.8)
  * `fullPtrTypeComponents` **(Compute Cores)** (Impact: 37.1)
  * `next` **(Defensive Guards)** (Impact: 32.1)
  * `next` **(Compute Cores)** (Impact: 28.7)
...
- **Blast Radius:** changing it is visible to **15** in-repo importer(s); it depends on **13**; blast radius 26.285; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (91.5%), Connectivity (formerly Api Exposure) (78.0%), Mutation Surface (formerly State Flux) (50.5%), Debt Markers (formerly Tech Debt) (19.0%)
- **Documentation Coverage:** 56.1798% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createAndStoreDocument` **(Many-Argument Workhorses)** (Impact: 38.9)
    * *Intent:* /// **Thread safe** takes an exclusive lock
  * `invalidateBuildFileWorker` **(Many-Argument Workhorses)** (Impact: 32.4)
  * `loadBuildConfiguration` **(Defensive Guards)** (Impact: 16.4)
    * *Intent:* /// Runs the build.zig and extracts include directories and packages
  * `loadDirectoryRecursive` **(Defensive Guards)** (Impact: 15.1)
  * `publishCimportDiagnostics` **(Defensive Guards)** (Impact: 15.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (54.7%), Connectivity (formerly Api Exposure) (27.7%), Mutation Surface (formerly State Flux) (26.5%), Complexity Load (formerly Cognitive Load) (15.2%)
- **Documentation Coverage:** 83.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `writeNodeTokens` **(Many-Argument Workhorses)** (Impact: 193.1)
  * `writeIdentifier` **(Defensive Guards)** (Impact: 23.2)
  * `writeFieldAccess` **(Defensive Guards)** (Impact: 22.1)
  * `writeVarDecl` **(Defensive Guards)** (Impact: 21.2)
  * `writeContainerField` **(Defensive Guards)** (Impact: 17.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 13.363; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.8%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (61.5%)
- **Documentation Coverage:** 67.725% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `SegmentedList` **(Many-Argument Workhorses)** (Impact: 82.4)
    * *Intent:* /// This is a stack data structure where pointers to indexes have the same lifetime as the data stru...
  * `BaseIterator` **(Compute Cores)** (Impact: 24.1)
  * `shrinkCapacity` **(Type Conversions)** (Impact: 11.6)
    * *Intent:* /// Only shrinks capacity or retains current capacity. /// It may fail to reduce the capacity in whi...
  * `writeToSlice` **(Many-Argument Workhorses)** (Impact: 11.2)
  * `growCapacity` **(Defensive Guards)** (Impact: 11.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (60.4%), Mutation Surface (formerly State Flux) (48.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (43.4%)
- **Documentation Coverage:** 80.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getDiscardLoc` **(Defensive Guards)** (Impact: 29.7)
    * *Intent:* /// takes the location of an identifier which is part of a discard `_ = location_here;` /// and retu...
  * `parse` **(Defensive Guards)** (Impact: 22.4)
  * `getParamRemovalRange` **(Compute Cores)** (Impact: 17.4)
  * `lessThan` **(Compute Cores)** (Impact: 15.1)
  * `detectIndentation` **(Compute Cores)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **22** in-repo importer(s); it depends on **3**; blast radius 58.471; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (39.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (26.0%)
- **Documentation Coverage:** 71.9512% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `sourceIndexToTokenIndex` **(Many-Argument Workhorses)** (Impact: 28.8)
  * `identifierIndexToLoc` **(Compute Cores)** (Impact: 25.2)
    * *Intent:* /// The source index must be at the start of the valid identifier. /// /// Supported formats: /// - ...
  * `tokenToLoc` **(Defensive Guards)** (Impact: 15.8)
  * `multilineLocAtIndex` **(Compute Cores)** (Impact: 15.3)
    * *Intent:* /// return the source location /// that starts `n` lines before the line at which `index` is located...
  * `pickTokenTag` **(Defensive Guards)** (Impact: 12.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 4 instances
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 4.591; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (69.9%), Connectivity (formerly Api Exposure) (53.5%), Guard Balance (formerly Safety Score) (26.3%), Concurrency Surface (formerly Concurrency) (14.8%)
- **Documentation Coverage:** 88.8889% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testCompletionWithOptions` **(Many-Argument Workhorses)** (Impact: 103.5)
  * `searchCompletionItemWithLabel` **(Defensive Guards)** (Impact: 7.9)
  * `testCompletionTextEdit` **(Defensive Guards)** (Impact: 6.6)
    * *Intent:* /// TODO this function should allow asserting where the cursor is placed after the text edit
  * `sort` **(Parameter Forwarders)** (Impact: 2.1)
  * `printLabels` **(State Mutators)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (67.5%), Connectivity (formerly Api Exposure) (30.8%), Mutation Surface (formerly State Flux) (18.2%), Complexity Load (formerly Cognitive Load) (10.3%)
- **Documentation Coverage:** 87.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `referenceNode` **(Many-Argument Workhorses)** (Impact: 83.2)
  * `referencesHandler` **(Defensive Guards)** (Impact: 47.5)
  * `gatherWorkspaceReferenceCandidates` **(Defensive Guards)** (Impact: 32.3)
  * `symbolReferences` **(Many-Argument Workhorses)** (Impact: 30.4)
  * `referenceNode` **(Defensive Guards)** (Impact: 26.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 3 instances
* *Amplified Cascading Flux:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (72.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (67.5%), Guard Balance (formerly Safety Score) (13.0%), Complexity Load (formerly Cognitive Load) (11.3%)
- **Documentation Coverage:** 73.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseArgs` **(Many-Argument Workhorses)** (Impact: 59.5)
  * `main` **(Defensive Guards)** (Impact: 42.6)
  * `logFn` **(Defensive Guards)** (Impact: 41.6)
  * `@"zls env"` **(Defensive Guards)** (Impact: 30.9)
  * `deinit` **(State Mutators)** (Impact: 5.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 8 instances
* *Mitigated Memory Allocs:* 33 instances
...
- **Blast Radius:** changing it is visible to **17** in-repo importer(s); it depends on **3**; blast radius 40.514; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (91.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (53.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (27.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `TracyAllocator` **(Type Conversions)** (Impact: 20.9)
  * `resizeFn` **(Type Conversions)** (Impact: 10.4)
  * `remapFn` **(Defensive Guards)** (Impact: 10.4)
  * `allocFn` **(Defensive Guards)** (Impact: 9.6)
  * `allocNamed` **(Compute Cores)** (Impact: 8.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **4**; blast radius 25.887; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (72.1%), Guard Balance (formerly Safety Score) (28.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (22.5%)
- **Documentation Coverage:** 83.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `nameToken` **(Defensive Guards)** (Impact: 12.9)
    * *Intent:* /// Returns a `.identifier` or `.builtin` token.
  * `getScopeDeclarationsConst` **(Type Conversions)** (Impact: 9.7)
  * `getScopeChildScopesConst` **(Type Conversions)** (Impact: 9.7)
  * `get` **(Defensive Guards)** (Impact: 5.7)
  * `getScopeDeclaration` **(Defensive Guards)** (Impact: 5.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 2 instances
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **2**; blast radius 19.178; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (44.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (36.6%), Mutation Surface (formerly State Flux) (30.2%), Concurrency Surface (formerly Concurrency) (16.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderNode` **(Defensive Guards)** (Impact: 45.5)
  * `moveSourceCursor` **(Defensive Guards)** (Impact: 15.1)
  * `renderToFile` **(Defensive Guards)** (Impact: 14.2)
  * `renderOptField` **(Defensive Guards)** (Impact: 9.9)
  * `renderOptTokenField` **(Defensive Guards)** (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Amplified Cascading Flux:* 11 instances
* *Memory Alloc (weighted view):* 1
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (65.9%), Connectivity (formerly Api Exposure) (40.5%), Guard Balance (formerly Safety Score) (20.4%), Debt Markers (formerly Tech Debt) (19.5%)
- **Documentation Coverage:** 90.9091% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `loop` **(Many-Argument Workhorses)** (Impact: 58.9)
  * `getErrorBundleFromStderr` **(Many-Argument Workhorses)** (Impact: 43.9)
  * `generateDiagnostics` **(Defensive Guards)** (Impact: 17.5)
  * `getErrorBundleFromAstCheck` **(Defensive Guards)** (Impact: 15.7)
  * `sendManualWatchUpdate` **(Defensive Guards)** (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **3**; blast radius 11.584; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (87.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (61.2%), Guard Balance (formerly Safety Score) (42.6%)
- **Documentation Coverage:** 89.1892% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `isVarDeclAlias` **(Defensive Guards)** (Impact: 15.0)
    * *Intent:* /// Check if the init expression is a sequence of field accesses /// where the last field name match...
  * `mergeIntersection` **(Many-Argument Workhorses)** (Impact: 13.5)
    * *Intent:* /// Asserts `@min(a.len, b.len) <= out.len`.
  * `next` **(Compute Cores)** (Impact: 11.1)
  * `appendToBucket` **(Compute Cores)** (Impact: 6.7)
  * `containsInBucket` **(Compute Cores)** (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 4 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (95.6%), Guard Balance (formerly Safety Score) (32.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (27.7%), Complexity Load (formerly Cognitive Load) (24.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getSignatureInfo` **(Many-Argument Workhorses)** (Impact: 137.5)
  * `from` **(Compute Cores)** (Impact: 4.8)
  * `fnProtoToSignatureInfo` **(State Mutators)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 13 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 4.394; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (48.6%), Mutation Surface (formerly State Flux) (43.6%), Connectivity (formerly Api Exposure) (26.6%), Guard Balance (formerly Safety Score) (18.3%)
- **Documentation Coverage:** 60.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `writeNodeInlayHint` **(Defensive Guards)** (Impact: 68.7)
  * `writeCallHint` **(Defensive Guards)** (Impact: 30.5)
    * *Intent:* /// writes parameter hints into `builder.hints`
  * `writeRangeInlayHint` **(Many-Argument Workhorses)** (Impact: 10.2)
    * *Intent:* /// creates a list of `InlayHint`'s from the given document /// only parameter hints are created ///...
  * `writeCallNodeHint` **(Defensive Guards)** (Impact: 9.5)
    * *Intent:* /// takes a Ast.full.Call (a function call), analysis its function expression, finds its declaration...
  * `writeForCaptureHint` **(Defensive Guards)** (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 8 instances
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **8**; blast radius 9.517; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (58.3%), Connectivity (formerly Api Exposure) (22.6%), Mutation Surface (formerly State Flux) (15.9%), Guard Balance (formerly Safety Score) (10.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `gotoHandler` **(Defensive Guards)** (Impact: 39.5)
  * `gotoDefinitionString` **(Many-Argument Workhorses)** (Impact: 30.3)
  * `gotoDefinitionSymbol` **(Defensive Guards)** (Impact: 24.2)
  * `gotoDefinitionStructInit` **(Defensive Guards)** (Impact: 18.4)
  * `gotoDefinitionFieldAccess` **(Many-Argument Workhorses)** (Impact: 18.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **3**; blast radius 14.745; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (62.0%), Guard Balance (formerly Safety Score) (20.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.7%), Complexity Load (formerly Cognitive Load) (4.4%)
- **Documentation Coverage:** 75.7576% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `render` **(Defensive Guards)** (Impact: 49.6)
  * `lessThan` **(Compute Cores)** (Impact: 8.6)
  * `assertFmt` **(Defensive Guards)** (Impact: 8.3)
    * *Intent:* // // //
  * `next` **(Defensive Guards)** (Impact: 7.9)
  * `removeUnusedFiles` **(Compute Cores)** (Impact: 6.3)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **3**; blast radius 9.517; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (76.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (67.5%), Connectivity (formerly Api Exposure) (53.0%)
- **Documentation Coverage:** 57.6923% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `eql` **(Compute Cores)** (Impact: 21.0)
  * `free` **(Defensive Guards)** (Impact: 20.9)
  * `deinit` **(State Mutators)** (Impact: 6.2)
  * `deinit` **(Defensive Guards)** (Impact: 3.7)
  * `validateConfiguration` **(State Mutators)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
