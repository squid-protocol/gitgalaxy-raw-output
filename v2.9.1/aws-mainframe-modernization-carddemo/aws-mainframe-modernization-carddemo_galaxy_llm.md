...
## 1. EXECUTIVE SUMMARY
- **Scope:** 224 analyzed artifact(s), 39902 LOC.
- **Load-bearing artifact:** `app/cpy/COCOM01Y.cpy` -- 21 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `samples/proc/BLDCIDB2.prc` -- pulls in 20 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `app/cbl/COACTUPC.cbl` at magnitude 2199.06 (structural weight, not risk).
...
| % Scanned of codebase = | 68.1% |
| Dominant Lang | COBOL |

## 3.5 MACRO-NETWORK TOPOLOGY (Resilience & Coupling)
| Metric | Value | Interpretation |
|---|---|---|
| Modularity | 0.4191 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | 0.021 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
...
> **Architectural Drift Z-Score:** `1.863`
> **Composition Archetype:** `Mid Flat Project` (z +1.86; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules 29%, Declarative / Non-Code 26%, Interface Declarations Files 20%, Data / Markup / Trivial 16%, Parameter Forwarders Files 4%
...
|---|---|---|
| Unclassified | 207 | 92.4% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 17 | 7.6% |
...
- `.drawio`: 1x Excluded (Unsupported Extension: '.drawio')
- `.awk`: 1x Unsupported Format (.awk)

## 6. STRUCTURAL SURFACE PROFILE (formerly Risk Exposure) ANALYSIS (0-100%)
| Structural Surface Vector | Min | Max | Mean | Med | Mode |
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 94.9 | 16.3 | 0.0 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 28.4 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 81.8 | 3.2 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 13.9 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 82.2 | 1.9 | 0.0 | 0.0 |
...
| Dead Code Surface (formerly Commented Logic Exposure) | 0.0 | 90.0 | 1.6 | 0.0 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 63.1 | 100.0 | 100.0 |
| Credential Material (formerly Hardcoded Payload Artifacts) | 0.0 | 96.0 | 0.5 | 0.0 | 0.0 |
...
|---|---|---|---|---|
| memory | 767 | 54 | 11 | `app/app-authorization-ims-db2-mq/cpy-bms/COPAU00.cpy` |
| cleanup | 104 | 44 | 2 | `app/app-transaction-type-db2/cbl/COTRTLIC.cbl` |
| guards | 194 | 44 | 3 | `app/asm/COBDATFT.asm` |
| danger | 373 | 55 | 2 | `app/cbl/COACTUPC.cbl` |
| concurrency | 62 | 28 | 1 | `samples/jcl/IMSMQCMP.jcl` |
| connectivity | 246 | 70 | 2 | `app/cbl/CBEXPORT.cbl` |
| io | 1622 | 110 | 19 | `app/cbl/CBSTM03A.CBL` |
| crypto | 0 | 0 | 0 | - |
| ipc | 205 | 45 | 3 | `app/app-transaction-type-db2/cbl/COTRTLIC.cbl` |
| time | 100 | 39 | 2 | `app/cbl/CBIMPORT.cbl` |
| serialization | 267 | 26 | 1 | `app/cbl/COACTUPC.cbl` |
| regex | 83 | 18 | 0 | `app/cbl/COACTUPC.cbl` |
| events | 107 | 79 | 1 | `app/app-vsam-mq/cbl/COACCT01.cbl` |
| tests | 0 | 0 | 0 | - |
| docs | 18 | 9 | 0 | `app/app-transaction-type-db2/cbl/COTRTLIC.cbl` |
| debt | 639 | 41 | 11 | `app/cbl/CBTRN03C.cbl` |
| mutation | 12432 | 141 | 186 | `app/cbl/COACTUPC.cbl` |
| dead_code | 140 | 45 | 2 | `app/cbl/COCRDLIC.cbl` |
| credential | 2 | 2 | 0 | `app/app-authorization-ims-db2-mq/cbl/PAUDBLOD.CBL` |
| threat | 1126 | 69 | 18 | `app/cbl/COACTUPC.cbl` |
| ml_ai | 4 | 2 | 0 | `scripts/upld_module.sh` |
| ui | 1269 | 42 | 4 | `app/bms/COACTUP.bms` |
...
4. **COPAUA0C.cbl** (`app/app-authorization-ims-db2-mq/cbl/COPAUA0C.cbl`) — 14 outbound dependencies
5. **COPAUS0C.cbl** (`app/app-authorization-ims-db2-mq/cbl/COPAUS0C.cbl`) — 14 outbound dependencies

## 8. CORE FUNCTION HITLIST (Heaviest Functions)
> *Note: The 'Impact' metric below represents Structural Magnitude (complexity, arguments, and length), NOT operational risk. These are the load-bearing pillars of the logic.*

- `1100-RECEIVE-MAP` **(I/O & Config Routines)** (@ `app/cbl/COACTUPC.cbl`) -> Impact: **117.3** | LOC: 387
...
- `VALIDATE-INPUT-DATA-FIELDS` **(I/O & Config Routines)** (@ `app/cbl/COTRN02C.cbl`) -> Impact: **56.1** | LOC: 203
...
- `YYYY-STORE-PFKEY` **(I/O & Config Routines)** (@ `app/cpy/CSSTRPFY.cpy`) -> Impact: **33.1** | LOC: 63
...
|---|---|---|---|---|
| `app/cbl` | 31 | 11262.38 | 70.87% | 9.45% |
| `app/app-authorization-ims-db2-mq/cbl` | 8 | 2104.36 | 51.6% | 15.14% |
| `app/app-transaction-type-db2/cbl` | 3 | 1996.2 | 69.26% | 13.22% |
...
| `scripts` | 10 | 27.94 | 19.59% | 0.0% |

## 10. TARGETED STRUCTURAL SURFACE VECTORS (formerly Risk Vectors, Top 5 by Surface)
### Highest Debt Markers (formerly Tech Debt; Fragile/Planned)
- `app/asm/MVSWAIT.asm` -> **81.7574%** Exposure
- `app/asm/COBDATFT.asm` -> **68.5397%** Exposure
- `app/app-transaction-type-db2/cpy/CSDB2RPY.cpy` -> **47.985%** Exposure
- `app/cbl/CSUTLDTC.cbl` -> **32.1778%** Exposure
- `app/app-transaction-type-db2/cbl/COBTUPDT.cbl` -> **21.7467%** Exposure
...
- `app/app-transaction-type-db2/cbl/COTRTLIC.cbl` -> **3** Orphaned Functions | **0** Duplicates
...
- **Unknown Dependencies:** `477` packages imported that bypass the Zero-Trust whitelist.
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.4%), Complexity Load (formerly Cognitive Load) (92.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 98.8095% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `1100-RECEIVE-MAP` **(I/O & Config Routines)** (Impact: 117.3)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 3.861
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 3.861; role: Pure Consumer (Orchestrator)
...
- **Documentation Coverage:** 99.1453% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.2%), Complexity Load (formerly Cognitive Load) (91.4%), Test Surface (formerly Verification) (80.0%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (74.7%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.3%), Complexity Load (formerly Cognitive Load) (82.2%), Test Surface (formerly Verification) (80.0%)
...
**Top Internal Functions/Classes:**
  ...
  * `3200-SETUP-SCREEN-VARS` **(Compute Cores)** (Impact: 18.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.4%), Complexity Load (formerly Cognitive Load) (92.1%), Test Surface (formerly Verification) (80.0%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (76.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (77.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `PROCESS-ENTER-KEY` **(I/O & Config Routines)** (Impact: 27.2)
    * *Intent:* *----------------------------------------------------------------* * PROCESS-ENTER-KEY *------------...
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.7%), Complexity Load (formerly Cognitive Load) (94.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `VALIDATE-INPUT-DATA-FIELDS` **(I/O & Config Routines)** (Impact: 56.1)
    * *Intent:* *----------------------------------------------------------------* * VALIDATE-INPUT-DATA-FIELDS *---...
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (92.6%), Guard Balance (formerly Safety Score) (92.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.0%), Complexity Load (formerly Cognitive Load) (91.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 87.931% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Magnitude:** 392.78 | **LOC:** 732 | **CtrlFlow:** 16.2% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (67.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `2700-A-CREATE-TCATBAL-REC` **(I/O & Config Routines)** (Impact: 6.1)
    * *Intent:* *---------------------------------------------------------------*
  * `2500-WRITE-REJECT-REC` **(I/O & Config Routines)** (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 51 instances
* *State Mutation (weighted view):* 238
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 100`, `structural_boundaries: 160`, `args: 2`, `func_start: 26`, `class_start: 1`
* *Risk/State:* `state_mutation: 136`
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.8%), Complexity Load (formerly Cognitive Load) (87.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 94.6429% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_12` (Drift: 0.0 IQR)
- **Magnitude:** 378.48 | **LOC:** 925 | **CtrlFlow:** 7.3% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `1000-XREFFILE-GET-NEXT` **(I/O & Config Routines)** (Impact: 6.1)
    * *Intent:* *---------------------------------------------------------------*
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 40 instances
* *State Mutation (weighted view):* 263
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 57`, `structural_boundaries: 112`, `args: 13`, `func_start: 25`, `class_start: 1`
* *Risk/State:* `safety_bypasses: 14`, `high_risk_execution: 4`, `state_mutation: 183`
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.8%), Complexity Load (formerly Cognitive Load) (80.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.3%), Complexity Load (formerly Cognitive Load) (91.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (77.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 321.74 | **LOC:** 653 | **CtrlFlow:** 15.6% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (61.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `1100-GET-ACCT-DATA` **(I/O & Config Routines)** (Impact: 7.0)
    * *Intent:* *---------------------------------------------------------------*
  * `1110-GET-XREF-DATA` **(I/O & Config Routines)** (Impact: 7.0)
    * *Intent:* *---------------------------------------------------------------*
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 40 instances
* *State Mutation (weighted view):* 191
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 86`, `structural_boundaries: 149`, `args: 3`, `func_start: 22`, `class_start: 1`
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 319.0 | **LOC:** 650 | **CtrlFlow:** 15.2% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (60.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `0000-TRANFILE-OPEN` **(I/O & Config Routines)** (Impact: 5.8)
    * *Intent:* *---------------------------------------------------------------*
  * `0100-REPTFILE-OPEN` **(I/O & Config Routines)** (Impact: 5.8)
    * *Intent:* *---------------------------------------------------------------*
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 36 instances
* *State Mutation (weighted view):* 188
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 83`, `structural_boundaries: 166`, `args: 2`, `func_start: 26`, `class_start: 1`
...
- **Global Archetype:** `file_cluster_2` (Drift: 0.0 IQR)
- **Magnitude:** 310.3 | **LOC:** 495 | **CtrlFlow:** 16.4% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (66.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `1000-DALYTRAN-GET-NEXT` **(I/O & Config Routines)** (Impact: 10.2)
    * *Intent:* ***************************************************************** * READS FILE * *******************...
...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 68`, `structural_boundaries: 120`, `args: 2`, `func_start: 18`, `class_start: 1`
* *Risk/State:* `state_mutation: 69`, `unreferenced_by_name: 1`
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (57.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.2%), Complexity Load (formerly Cognitive Load) (92.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 281.12 | **LOC:** 583 | **CtrlFlow:** 5.3% | **Authorship Centralization:** 100.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `3200-CREATE-ACCOUNT-EXP-REC` **(I/O & Config Routines)** (Impact: 3.8)
    * *Intent:* *****************************************************************
  * `5700-CREATE-CARD-EXPORT-RECORD` **(I/O & Config Routines)** (Impact: 3.5)
    * *Intent:* *****************************************************************
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
* *State Mutation (weighted view):* 110
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 21`, `structural_boundaries: 97`, `func_start: 27`, `class_start: 1`
* *Risk/State:* `state_mutation: 100`, `unreferenced_by_name: 1`
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 3.861; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (59.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `3000-GET-REQUEST` **(I/O & Config Routines)** (Impact: 8.8)
  * `4100-PUT-REPLY` **(I/O & Config Routines)** (Impact: 7.0)
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 3.861
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
...
- **Global Archetype:** `file_cluster_11` (Drift: 0.0 IQR)
- **Magnitude:** 221.16 | **LOC:** 431 | **CtrlFlow:** 11.5% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.861; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (63.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `3000-ARRFILE-OPEN` **(I/O & Config Routines)** (Impact: 5.8)
    * *Intent:* *---------------------------------------------------------------*
  * `4000-VBRFILE-OPEN` **(I/O & Config Routines)** (Impact: 5.8)
    * *Intent:* *---------------------------------------------------------------*
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 40 instances
* *State Mutation (weighted view):* 149
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 41`, `structural_boundaries: 108`, `args: 3`, `func_start: 18`, `class_start: 1`
* *Risk/State:* `state_mutation: 69`, `dead_code: 1`
...
- `app/cbl/CBEXPORT.cbl` -> **Arunkumar Selvam** (100.0% isolated ownership) | Magnitude: 281.12
- `app/cbl/CBIMPORT.cbl` -> **Arunkumar Selvam** (100.0% isolated ownership) | Magnitude: 179.54
...
- `app/cpy/CSSTRPFY.cpy` -> **Severity: 3.111** (Embedded: 0.0312 * Error Risk: 99.5536%)
- `app/cpy/CSSETATY.cpy` -> **Severity: 0.602** (Embedded: 0.0089 * Error Risk: 67.4492%)
- `app/cpy/CSUTLDPY.cpy` -> **Severity: 0.438** (Embedded: 0.0045 * Error Risk: 98.07%)
- `app/cpy/COADM02Y.cpy` -> **Severity: 0.263** (Embedded: 0.0045 * Error Risk: 59.0077%)
- `app/cpy/COMEN02Y.cpy` -> **Severity: 0.257** (Embedded: 0.0045 * Error Risk: 57.5315%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `app/cpy/CSSTRPFY.cpy` -> **Severity: 615.8** (Blast Radius: 6.158 * Doc Risk: 100.0%)
- `app/cpy/CSUTLDPY.cpy` -> **Severity: 408.0** (Blast Radius: 4.08 * Doc Risk: 100.0%)
- `app/app-authorization-ims-db2-mq/bms/COPAU00.bms` -> **Severity: 386.1** (Blast Radius: 3.861 * Doc Risk: 100.0%)
- `app/app-authorization-ims-db2-mq/bms/COPAU01.bms` -> **Severity: 386.1** (Blast Radius: 3.861 * Doc Risk: 100.0%)
- `app/app-transaction-type-db2/bms/COTRTLI.bms` -> **Severity: 386.1** (Blast Radius: 3.861 * Doc Risk: 100.0%)
