...
## 1. EXECUTIVE SUMMARY
- **Scope:** 213 analyzed artifact(s), 13096 LOC.
- **Load-bearing artifact:** `t/utf8.txt` -- 11 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `t/arg_constraints.t` -- pulls in 18 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `t/aggregate/live_component_controller_action_chained.t` at magnitude 308.6 (structural weight, not risk).
...
> **Architectural Drift Z-Score:** `2.797`
> **Composition Archetype:** `Mid Flat Project` (z +2.80; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 54%, Compute Cores Files 9%, Large Core Modules 9%, Interface Declarations Files 7%, Parameter Forwarders Files 6%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 92.3 | 33.4 | 40.8 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 46.9 | 0.0 | 0.0 |
...
- `run_tests` **(Many-Argument Workhorses)** (@ `t/aggregate/live_component_controller_action_chained.t`) -> Impact: **279.4** | LOC: 1147
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_visit.t`) -> Impact: **62.0** | LOC: 259
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_go.t`) -> Impact: **57.1** | LOC: 242
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_auto.t`) -> Impact: **40.0** | LOC: 160
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_action.t`) -> Impact: **37.1** | LOC: 242
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_forward.t`) -> Impact: **36.7** | LOC: 234
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_local.t`) -> Impact: **28.3** | LOC: 126
- `run_test_for` **(Compute Cores)** (@ `t/aggregate/live_component_controller_args.t`) -> Impact: **26.2** | LOC: 43
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_default.t`) -> Impact: **22.6** | LOC: 71
- `run_tests` **(I/O & Config Routines)** (@ `t/aggregate/live_component_controller_action_path.t`) -> Impact: **21.9** | LOC: 138
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (23.4%), Debt Markers (formerly Tech Debt) (15.9%), Mutation Surface (formerly State Flux) (14.1%), Complexity Load (formerly Cognitive Load) (11.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(Many-Argument Workhorses)** (Impact: 279.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 4.497; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (41.9%), Mutation Surface (formerly State Flux) (40.0%), Debt Markers (formerly Tech Debt) (32.2%), Complexity Load (formerly Cognitive Load) (15.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `user` **(Type Conversions)** (Impact: 8.2)
  * `user_object` **(Compute Cores)** (Impact: 8.2)
    * *Intent:* # Tests using this are current skipped pending coercion rethink
  * `default` **(Type Conversions)** (Impact: 8.2)
  * `tuple` **(Generic / Templated Code)** (Impact: 4.7)
  * `slurpy_tuple` **(Generic / Templated Code)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 4.497; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (48.7%), Mutation Surface (formerly State Flux) (10.7%), Connectivity (formerly Api Exposure) (10.3%), Complexity Load (formerly Cognitive Load) (7.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `stream_body_fh2` **(Compute Cores)** (Impact: 9.1)
    * *Intent:* # If you pull the file contents into a var, NOW you need to specify the # IO encoding on the FH. Ult...
  * `stream_body_fh` **(Compute Cores)** (Impact: 7.3)
    * *Intent:* # Stream a file with utf8 chars directly, you don't need to decode
  * `argend` **(Type Conversions)** (Impact: 4.8)
  * `file_upload` **(Parameter Forwarders)** (Impact: 4.4)
  * `file_upload_utf8_param` **(Parameter Forwarders)** (Impact: 4.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 4.497; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (39.4%), Mutation Surface (formerly State Flux) (15.1%), Debt Markers (formerly Tech Debt) (15.1%), Connectivity (formerly Api Exposure) (10.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get_env` **(Compute Cores)** (Impact: 9.3)
  * `filehandle` **(Compute Cores)** (Impact: 8.3)
  * `mounted` **(Type Conversions)** (Impact: 6.3)
  * `streaming_body` **(Callbacks & Closures)** (Impact: 4.5)
  * `streaming_body_with_charset` **(Callbacks & Closures)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (75.8%), Complexity Load (formerly Cognitive Load) (36.2%), Connectivity (formerly Api Exposure) (10.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `a` **(Compute Cores)** (Impact: 6.3)
  * `b` **(Compute Cores)** (Impact: 6.3)
  * `root` **(Annotated & Test Methods)** (Impact: 5.6)
  * `d` **(Annotated & Test Methods)** (Impact: 5.6)
  * `root` **(Compute Cores)** (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (52.0%), Guard Balance (formerly Safety Score) (29.2%), Complexity Load (formerly Cognitive Load) (20.0%), Connectivity (formerly Api Exposure) (2.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(I/O & Config Routines)** (Impact: 62.0)
  * `_begin` **(Parameter Forwarders)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (56.4%), Guard Balance (formerly Safety Score) (28.0%), Complexity Load (formerly Cognitive Load) (20.7%), Connectivity (formerly Api Exposure) (2.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(I/O & Config Routines)** (Impact: 57.1)
  * `_begin` **(Parameter Forwarders)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
* *State Mutation (weighted view):* 16
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.3%), Complexity Load (formerly Cognitive Load) (33.7%), Test Surface (formerly Verification) (2.3%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (94.1%), Guard Balance (formerly Safety Score) (71.2%), Complexity Load (formerly Cognitive Load) (19.3%), Connectivity (formerly Api Exposure) (9.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `write_component_file` **(Compute Cores)** (Impact: 6.5)
  * `make_component_file` **(Many-Argument Workhorses)** (Impact: 6.4)
  * `COMPONENT` **(Callbacks & Closures)** (Impact: 3.3)
  * `COMPONENT` **(Callbacks & Closures)** (Impact: 3.1)
  * `COMPONENT` **(Compute Cores)** (Impact: 2.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (87.0%), Guard Balance (formerly Safety Score) (84.2%), Connectivity (formerly Api Exposure) (4.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_test_for` **(Compute Cores)** (Impact: 26.2)
  * `run_tests` **(Interface Declarations)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (74.2%), Guard Balance (formerly Safety Score) (37.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (58.3%), Guard Balance (formerly Safety Score) (52.6%), Mutation Surface (formerly State Flux) (23.8%), Complexity Load (formerly Cognitive Load) (16.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `is_http` **(Annotated & Test Methods)** (Impact: 3.7)
  * `is_https` **(Annotated & Test Methods)** (Impact: 3.7)
  * `is_http_chain` **(Annotated & Test Methods)** (Impact: 3.7)
  * `is_https_chain` **(Annotated & Test Methods)** (Impact: 3.7)
  * `uri_for1` **(Annotated & Test Methods)** (Impact: 3.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 3
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (35.4%), Connectivity (formerly Api Exposure) (8.2%), Complexity Load (formerly Cognitive Load) (5.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (95.0%), Guard Balance (formerly Safety Score) (33.8%), Mutation Surface (formerly State Flux) (28.9%), Complexity Load (formerly Cognitive Load) (10.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `two` **(Annotated & Test Methods)** (Impact: 4.4)
  * `auto` **(Annotated & Test Methods)** (Impact: 3.9)
  * `end` **(Annotated & Test Methods)** (Impact: 3.9)
  * `begin` **(Annotated & Test Methods)** (Impact: 3.7)
  * `base` **(Annotated & Test Methods)** (Impact: 3.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 3
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (88.8%), Guard Balance (formerly Safety Score) (50.0%), Mutation Surface (formerly State Flux) (11.8%), Connectivity (formerly Api Exposure) (10.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `from_psgi_app` **(Annotated & Test Methods)** (Impact: 3.8)
  * `from_catalyst` **(Annotated & Test Methods)** (Impact: 3.7)
  * `classic_error` **(Annotated & Test Methods)** (Impact: 3.7)
  * `just_die` **(Annotated & Test Methods)** (Impact: 3.7)
  * `end` **(Compute Cores)** (Impact: 3.0)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (24.4%), Mutation Surface (formerly State Flux) (20.9%), Complexity Load (formerly Cognitive Load) (13.7%), Connectivity (formerly Api Exposure) (2.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(I/O & Config Routines)** (Impact: 36.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
* *State Mutation (weighted view):* 6
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (29.2%), Debt Markers (formerly Tech Debt) (28.4%), Mutation Surface (formerly State Flux) (17.4%), Complexity Load (formerly Cognitive Load) (15.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(I/O & Config Routines)** (Impact: 40.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 3
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 4.497; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (22.0%), Complexity Load (formerly Cognitive Load) (16.4%), Mutation Surface (formerly State Flux) (12.9%), Connectivity (formerly Api Exposure) (2.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(I/O & Config Routines)** (Impact: 37.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 3
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (70.7%), Complexity Load (formerly Cognitive Load) (33.4%), Dead Code Surface (formerly Dead Code) (7.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (77.1%), Guard Balance (formerly Safety Score) (50.0%), Mutation Surface (formerly State Flux) (11.9%), Connectivity (formerly Api Exposure) (10.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `from_psgi_app` **(Annotated & Test Methods)** (Impact: 3.8)
  * `from_catalyst` **(Annotated & Test Methods)** (Impact: 3.7)
  * `classic_error` **(Annotated & Test Methods)** (Impact: 3.7)
  * `just_die` **(Annotated & Test Methods)** (Impact: 3.7)
  * `as_psgi` **(Callbacks & Closures)** (Impact: 2.5)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 4.497; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (52.9%), Mutation Surface (formerly State Flux) (22.9%), Complexity Load (formerly Cognitive Load) (22.2%), Connectivity (formerly Api Exposure) (8.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `default` **(Type Conversions)** (Impact: 8.2)
  * `user` **(Generic / Templated Code)** (Impact: 4.2)
  * `user_slurps` **(Generic / Templated Code)** (Impact: 4.2)
  * `string_types` **(Annotated & Test Methods)** (Impact: 2.0)
  * `as_ref` **(Generic / Templated Code)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (93.1%), Guard Balance (formerly Safety Score) (63.9%), Debt Markers (formerly Tech Debt) (55.8%), Complexity Load (formerly Cognitive Load) (18.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `default` **(Annotated & Test Methods)** (Impact: 5.4)
  * `author` **(Parameter Forwarders)** (Impact: 4.3)
  * `story` **(Annotated & Test Methods)** (Impact: 3.8)
  * `top` **(Annotated & Test Methods)** (Impact: 3.7)
  * `root` **(Annotated & Test Methods)** (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (77.1%), Concurrency Surface (formerly Concurrency) (44.5%), Guard Balance (formerly Safety Score) (39.0%), Complexity Load (formerly Cognitive Load) (23.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testOption` **(Defensive Guards)** (Impact: 6.4)
  * `testBackgroundOptionWithFork` **(Defensive Guards)** (Impact: 2.5)
  * `_build_testapp` **(Defensive Guards)** (Impact: 2.5)
  * `testRestart` **(Type Conversions)** (Impact: 2.2)
  * `opthash` **(Interface Declarations)** (Impact: 1.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 4.497; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.2%), Guard Balance (formerly Safety Score) (92.3%), Complexity Load (formerly Cognitive Load) (87.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `prove` **(Compute Cores)** (Impact: 9.3)
  * `wait_port_timeout` **(Parameter Forwarders)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
