...
## 1. EXECUTIVE SUMMARY
- **Scope:** 31528 analyzed artifact(s), 4240016 LOC.
- **Load-bearing artifact:** `test/framework/src/main/java/org/elasticsearch/test/ESTestCase.java` -- 4351 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `x-pack/plugin/security/src/main/java/org/elasticsearch/xpack/security/Security.java` -- pulls in 468 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `build-tools-internal/src/main/resources/run.ssl/private-ca.key` at magnitude 5000.0 (structural weight, not risk).
...
| Analyzed Artifacts (Scanned) | 31528 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 7448 |
| Total LOC | 4240016 |
...
|---|---|---|
| Modularity | 0.5309 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
> **Architectural Drift Z-Score:** `3.35`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +3.35; from the repo's file-archetype mix)
> **File Composition:** Encapsulated Accessors Files 20%, Large Core Modules (3) 18%, Data / Markup / Trivial 15%, Large Core Modules 8%, State Mutators Files 7%
...
- `.csv-spec`: 179x Unsupported Format (.undeterminable), 4x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Saturation: Line 36 exceeds 500 chars)
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 79.9 | 100.0 | 100.0 |
...
| connectivity | 291468 | 27162 | 20 | `x-pack/plugin/esql/src/test/java/org/elasticsearch/xpack/esql/optimizer/LogicalPlanOptimizerTests.java` |
...
- `guard_balance_ratio` (guards / (danger + 1)): **1.4951**
...
- `x-pack/plugin/esql/compute/src/main/java/org/elasticsearch/compute/data/ElementType.java` -> **Severity: 1278.893** (Blast Radius: 20.782 * Doc Risk: 61.5385%)
- `server/src/main/java/org/elasticsearch/common/io/stream/StreamInput.java` -> **Severity: 1115.677** (Blast Radius: 27.712 * Doc Risk: 40.2597%)
- `libs/x-content/src/main/java/org/elasticsearch/xcontent/XContentBuilder.java` -> **Severity: 1052.448** (Blast Radius: 13.27 * Doc Risk: 79.3103%)
