...
## 1. EXECUTIVE SUMMARY
- **Scope:** 338 analyzed artifact(s), 23034 LOC.
- **Load-bearing artifact:** `src/main/java/org/openmainframeproject/cobolcheck/services/Constants.java` -- 45 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/test/java/org/openmainframeproject/cobolcheck/TestSuiteParserParsingTest.java` -- pulls in 25 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/main/java/org/openmainframeproject/cobolcheck/features/testSuiteParser/TestSuiteParser.java` at magnitude 1148.82 (structural weight, not risk).
...
| Avg Path Length | 1.6647 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `3.93`
> **Composition Archetype:** `Small Flat Repo (2)` (z +3.93; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 35%, Declarative / Non-Code 21%, Large Core Modules (3) 13%, Large Core Modules 6%, Interface Declarations Files 5%
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 18.5 | 0.0 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 43.0 | 56.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 15.8 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 5.8 | 2.3 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 12.2 | 2.9 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 49.2 | 50.0 | 0.0 |
...
| guards | 1085 | 95 | 9 | `src/main/java/org/openmainframeproject/cobolcheck/features/testSuiteParser/TestSuiteParser.java` |
...
| mutation | 6806 | 192 | 56 | `src/test/java/org/openmainframeproject/cobolcheck/MockingTest.java` |
| dead_code | 716 | 96 | 6 | `src/test/java/org/openmainframeproject/cobolcheck/InterpreterControllerTest.java` |
| credential | 0 | 0 | 0 | - |
...
- `getParsedTestSuiteLines` **(Many-Argument Workhorses)** (@ `src/main/java/org/openmainframeproject/cobolcheck/features/testSuiteParser/TestSuiteParser.java`) -> Impact: **340.9** | LOC: 547
...
- `setFlagsForCurrentLine` **(Many-Argument Workhorses)** (@ `src/main/java/org/openmainframeproject/cobolcheck/services/cobolLogic/Interpreter.java`) -> Impact: **88.7** | LOC: 94
...
- `activate` **(Compute Cores)** (@ `vs-code-extension/client/src/extension.ts`) -> Impact: **75.6** | LOC: 211
- `constructor` **(Many-Argument Workhorses)** (@ `vs-code-extension/client/src/services/CobolCheckOutputParser.ts`) -> Impact: **67.2** | LOC: 64
- `writeMultiLine` **(Many-Argument Workhorses)** (@ `src/main/java/org/openmainframeproject/cobolcheck/features/writer/CobolWriter.java`) -> Impact: **45.0** | LOC: 61
...
- `parseText` **(Many-Argument Workhorses)** (@ `src/main/java/org/openmainframeproject/cobolcheck/features/launcher/Formatter/Formats/Formatter.java`) -> Impact: **41.6** | LOC: 69
...
- `getLinesUntilKeywordHit` **(Many-Argument Workhorses)** (@ `src/main/java/org/openmainframeproject/cobolcheck/features/testSuiteParser/TestSuiteParser.java`) -> Impact: **38.2** | LOC: 49
- `extractTokensFrom` **(Compute Cores)** (@ `src/main/java/org/openmainframeproject/cobolcheck/features/testSuiteParser/KeywordExtractor.java`) -> Impact: **36.0** | LOC: 69
- `getKeywordFor` **(Compute Cores)** (@ `src/main/java/org/openmainframeproject/cobolcheck/features/testSuiteParser/Keywords.java`) -> Impact: **35.2** | LOC: 46
- `it_parses_file_section_variables_from_split_lines` **(I/O & Config Routines)** (@ `src/test/java/org/openmainframeproject/cobolcheck/InterpreterControllerTest.java`) -> Impact: **32.2** | LOC: 84
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 2.065; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.8%), Complexity Load (formerly Cognitive Load) (88.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 66.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getParsedTestSuiteLines` **(Many-Argument Workhorses)** (Impact: 340.9)
    * *Intent:* /** * Process the test suite as a series of tokens. When we have processed all the * input, getNextT...
  * `getLinesUntilKeywordHit` **(Many-Argument Workhorses)** (Impact: 38.2)
  * `addTestCodeForAssertion` **(Compute Cores)** (Impact: 21.2)
  * `getNextTokenFromTestSuite` **(Stateful Encapsulated Methods)** (Impact: 14.3)
    * *Intent:* * we exhaust the list of tokens. * When the file read routine returns a null reference, it means we ...
  * `handleEndOfVerifyStatement` **(Compute Cores)** (Impact: 13.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 2.065; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.0%), Complexity Load (formerly Cognitive Load) (74.9%), Connectivity (formerly Api Exposure) (9.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `multiple_mock_perform_evaluates_are_generated_correctly` **(I/O & Config Routines)** (Impact: 14.4)
  * `single_mock_call_with_args_gets_generated_correctly_with_comment` **(I/O & Config Routines)** (Impact: 13.8)
  * `multiple_mock_sections_gets_generated_correctly` **(I/O & Config Routines)** (Impact: 13.2)
  * `single_mock_section_gets_generated_correctly_with_comment` **(I/O & Config Routines)** (Impact: 12.7)
  * `single_mock_paragraph_gets_generated_correctly_with_comment` **(I/O & Config Routines)** (Impact: 12.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 2.065; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.9%), Complexity Load (formerly Cognitive Load) (89.7%), Connectivity (formerly Api Exposure) (8.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `it_catches_unexpected_keyword_inside_before_each_block` **(Annotated & Test Methods)** (Impact: 18.4)
  * `it_catches_unexpected_keyword_in_a_verify_context` **(Annotated & Test Methods)** (Impact: 16.2)
  * `it_catches_unexpected_keyword_in_a_mock_context` **(Annotated & Test Methods)** (Impact: 15.3)
  * `it_catches_unexpected_keyword_at_the_end_of_mock_context_with_arguments_and_commas_1` **(Annotated & Test Methods)** (Impact: 15.3)
  * `it_catches_unexpected_keyword_at_the_end_of_mock_context_with_arguments_and_commas_2` **(Annotated & Test Methods)** (Impact: 15.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **7**; blast radius 4.681; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.1%), Complexity Load (formerly Cognitive Load) (64.1%), Connectivity (formerly Api Exposure) (44.9%)
- **Documentation Coverage:** 42.1053% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `setFlagsForCurrentLine` **(Many-Argument Workhorses)** (Impact: 88.7)
    * *Intent:* /** * Sets flags based on a line, to be able to know which kinds of source * statements to look for ...
  * `isEndOfStatement` **(Compute Cores)** (Impact: 28.1)
    * *Intent:* /** * Recognizes end of statement when * (a) - sourceLine ends with a period * (b) - previous line c...
  * `shouldLineBeStubbed` **(Compute Cores)** (Impact: 21.9)
    * *Intent:* /** * @param line * @param state * @return true if the source line should be commented out */
  * `getBeginningArea` **(Compute Cores)** (Impact: 17.0)
    * *Intent:* /** * Looks through the prefix-spaces in the source line in order to determine the * beginning area ...
  * `updateCurrentDataStructure` **(Compute Cores)** (Impact: 15.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 2.065; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (80.0%), Complexity Load (formerly Cognitive Load) (23.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (19.8%), Connectivity (formerly Api Exposure) (11.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `it_parses_file_section_variables_from_split_lines` **(I/O & Config Routines)** (Impact: 32.2)
    * *Intent:* // verify the variables in FILE-SECTION may be split on multiple lines
  * `it_parses_WS_section_variables_from_split_lines` **(Annotated & Test Methods)** (Impact: 10.4)
  * `it_stores_file_section_lines_in_lineRepository` **(Annotated & Test Methods)** (Impact: 9.3)
    * *Intent:* // Test the lines in the DATA DIVISION FILE SECTION are stored in the lineRepository
  * `it_updates_numeric_fields` **(I/O & Config Routines)** (Impact: 7.8)
  * `it_sets_CBL_rules_on_first_line_when_empty` **(Annotated & Test Methods)** (Impact: 6.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 2.065; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (99.8%), Guard Balance (formerly Safety Score) (89.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `activate` **(Compute Cores)** (Impact: 75.6)
  * `discoverTests` **(Defensive Guards)** (Impact: 30.4)
  * `createDirectoryItems` **(Compute Cores)** (Impact: 29.5)
  * `gatherTestItems` **(Compute Cores)** (Impact: 19.5)
  * `getDirectoryItems` **(Compute Cores)** (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **25**; blast radius 2.065; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (72.4%), Complexity Load (formerly Cognitive Load) (37.6%), Connectivity (formerly Api Exposure) (9.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `it_throws_if_the_mock_that_verify_references_does_not_exist` **(Annotated & Test Methods)** (Impact: 8.9)
  * `multiple_testcases_with_same_name_exists_for_a_testsuite` **(Annotated & Test Methods)** (Impact: 7.5)
  * `it_parses_testsuite_with_sequnece_area` **(Annotated & Test Methods)** (Impact: 6.5)
  * `verify_can_attach_to_call_mock_with_arguments` **(Annotated & Test Methods)** (Impact: 6.4)
  * `it_generates_after_each_branching_paragraphs_correctly_with_comments` **(I/O & Config Routines)** (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **14**; blast radius 2.984; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.8%), Guard Balance (formerly Safety Score) (83.1%), Complexity Load (formerly Cognitive Load) (51.5%), Connectivity (formerly Api Exposure) (40.7%)
- **Documentation Coverage:** 83.7209% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `updateLineRepository` **(Stateful Encapsulated Methods)** (Impact: 24.9)
    * *Intent:* /** * Updates the line repository with the given line, if it might have further use when testing. * ...
  * `updateLineRepoByFileStatusToken` **(Stateful Encapsulated Methods)** (Impact: 21.2)
    * *Intent:* /** * If the given line contains a file status token, it will be added to a map to a * corresponding...
  * `updateNumericFields` **(Stateful Encapsulated Methods)** (Impact: 17.1)
    * *Intent:* /** * If the current line is a numeric field, it will be added to the list of numeric fields. * We n...
  * `updateDependencies` **(Stateful Encapsulated Methods)** (Impact: 10.0)
    * *Intent:* /** * Updates dependencies of the interpreter. * The following values can be updated, based on the g...
  * `updatePossibleMock` **(Stateful Encapsulated Methods)** (Impact: 8.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 2.065; role: Isolated/Orphan
...
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `setTestCounts` **(I/O & Config Routines)** (Impact: 7.0)
  * `setCurrentTestCaseFailure` **(Parameter Forwarders)** (Impact: 2.1)
  * `setCurrentTestCaseErrorMessage` **(Parameter Forwarders)** (Impact: 2.1)
  * `setNumberOfAllTests` **(Annotated & Test Methods)** (Impact: 1.6)
  * `setNumberOffAllFailures` **(Annotated & Test Methods)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **43** in-repo importer(s); it depends on **14**; blast radius 28.085; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (88.5%), Guard Balance (formerly Safety Score) (80.8%), Complexity Load (formerly Cognitive Load) (33.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getCharsetForPlatform` **(Compute Cores)** (Impact: 22.0)
  * `getTestResultFormatStyle` **(Compute Cores)** (Impact: 12.8)
  * `getTestResultFormat` **(Compute Cores)** (Impact: 10.7)
  * `getCorrectRunContext` **(Stateful Encapsulated Methods)** (Impact: 9.1)
  * `resolveFilenameSuffixes` **(Stateful Encapsulated Methods)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 219.56 | **LOC:** 470 | **CtrlFlow:** 13.7% | **Authorship Centralization:** 0.0%
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (84.6%), Complexity Load (formerly Cognitive Load) (55.8%), Debt Markers (formerly Tech Debt) (23.2%)
...
**Top Internal Functions/Classes:**
  ...
  * `2000-SPEAK` **(I/O & Config Routines)** (Impact: 7.8)
...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 58`, `structural_boundaries: 68`, `func_start: 19`, `class_start: 1`
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **8**; blast radius 3.228; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.9%), Concurrency Surface (formerly Concurrency) (98.3%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (69.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `findFile` **(Many-Argument Workhorses)** (Impact: 21.6)
  * `runCobolCheck` **(Defensive Guards)** (Impact: 17.5)
  * `getCobolProgramPathForGivenContext` **(Compute Cores)** (Impact: 16.8)
  * `getSourceFolderContextPath` **(Defensive Guards)** (Impact: 11.3)
  * `getFileName` **(Compute Cores)** (Impact: 11.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **8**; blast radius 3.464; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (51.3%)
- **Documentation Coverage:** 25.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `writeMultiLine` **(Many-Argument Workhorses)** (Impact: 45.0)
    * *Intent:* /** * Lines of test code in a test suite are Cobol-like, but not strictly Cobol. The descriptions fo...
  * `getStringContinuationSign` **(Stateful Encapsulated Methods)** (Impact: 25.8)
  * `writeLine` **(Compute Cores)** (Impact: 7.9)
    * *Intent:* /** * Writes a line of cobol code to the test output file. If the given line is too * long for cobol...
  * `writeLines` **(Generic / Templated Code)** (Impact: 4.5)
    * *Intent:* /** * Writes all the given lines of cobol code to the test output file. If any of the lines * are to...
  * `writeCommentedLines` **(Generic / Templated Code)** (Impact: 4.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 2.065; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.5%), Debt Markers (formerly Tech Debt) (90.8%), Complexity Load (formerly Cognitive Load) (71.9%)
- **Documentation Coverage:** 91.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `checkSyntaxInsideBlock` **(Many-Argument Workhorses)** (Impact: 23.0)
  * `checkExpectedTokenSyntax` **(Many-Argument Workhorses)** (Impact: 20.9)
  * `logIdenticalMocks` **(Compute Cores)** (Impact: 12.0)
  * `logVerifyReferencesNonExistentMock` **(Compute Cores)** (Impact: 11.9)
  * `logUnusedMocks` **(Compute Cores)** (Impact: 10.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **24** in-repo importer(s); it depends on **4**; blast radius 17.109; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Connectivity (formerly Api Exposure) (92.2%), Guard Balance (formerly Safety Score) (89.3%), Complexity Load (formerly Cognitive Load) (25.9%)
- **Documentation Coverage:** 54.5455% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `stubLine` **(Compute Cores)** (Impact: 16.8)
  * `ExcludeBetweenTags` **(Compute Cores)** (Impact: 15.0)
  * `occursFirst` **(Compute Cores)** (Impact: 10.6)
    * *Intent:* /** * Checks if a character occurs before another * * @param expectedFirst - The char expected to be...
  * `swapChars` **(Compute Cores)** (Impact: 10.5)
    * *Intent:* /** * Swaps two characters in a given string. * Example: swapChars("1.000.000,00", '.', ',') ~ "1,00...
  * `equalsAny` **(Generic / Templated Code)** (Impact: 9.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 2.065; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (99.9%), Guard Balance (formerly Safety Score) (97.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 62.963% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `appendNextMeaningfulLineToCurrentLine` **(Compute Cores)** (Impact: 11.8)
  * `readTillHitToken` **(Generic / Templated Code)** (Impact: 8.1)
    * *Intent:* /** * Reads lines from the current line, until a specific token is hit. This will forward the reader...
  * `peekNextMeaningfulLine` **(I/O & Config Routines)** (Impact: 8.0)
    * *Intent:* /** * Peeks the next line of the cobol file that is meaningful - that is; a line that is * not empty...
  * `readLine` **(I/O & Config Routines)** (Impact: 4.0)
    * *Intent:* /** * Reads the next line of the cobol file. * * @return (CobolLine) The line that was read * * @thr...
  * `readTillEndOfStatement` **(Generic / Templated Code)** (Impact: 3.9)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **8**; blast radius 2.358; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (93.7%), Complexity Load (formerly Cognitive Load) (93.4%)
- **Documentation Coverage:** 93.5484% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run` **(Compute Cores)** (Impact: 17.2)
  * `updateFromDisk` **(Defensive Guards)** (Impact: 9.7)
  * `updateFromContents` **(Many-Argument Workhorses)** (Impact: 6.3)
    * *Intent:* /** * Parses the tests from the input text, and updates the tests contained * by this file to be tho...
  * `getContentFromFilesystem` **(Stateful Encapsulated Methods)** (Impact: 6.2)
  * `setDirectoryDetails` **(Compute Cores)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 3.82; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.5%), Complexity Load (formerly Cognitive Load) (82.1%), Connectivity (formerly Api Exposure) (6.3%)
- **Documentation Coverage:** 75.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `extractTokensFrom` **(Compute Cores)** (Impact: 36.0)
  * `handleEndOfWord` **(Stateful Encapsulated Methods)** (Impact: 21.9)
  * `isDecimalPoint` **(Stateful Encapsulated Methods)** (Impact: 7.2)
    * *Intent:* /** * Decimal point is recognized when a period or comma appears between two numeric digits. */
  * `tokenListEndsDuringMultiToken` **(Generic / Templated Code)** (Impact: 4.7)
  * `addTokenAndClearBuffer` **(Encapsulated Accessors)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (100.0%), Complexity Load (formerly Cognitive Load) (80.7%), Debt Markers (formerly Tech Debt) (32.9%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Anonymous_Block` **(Compute Cores)** (Impact: 14.9)
  ...
  * `Anonymous_Block` **(Compute Cores)** (Impact: 6.7)
  ...
  * `__global_context__` **(I/O & Config Routines)** (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **11**; blast radius 2.29; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.8%)
- **Documentation Coverage:** 33.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseText` **(Many-Argument Workhorses)** (Impact: 41.6)
    * *Intent:* /** * Parses text from text results to a data transfer object style. The style is given * in the con...
  * `instantiateBasedOnStyle` **(Stateful Encapsulated Methods)** (Impact: 14.7)
  * `setTestCaseValues` **(Stateful Encapsulated Methods)** (Impact: 12.5)
  * `getFailureType` **(Stateful Encapsulated Methods)** (Impact: 9.2)
  * `Formatter` **(State Mutators)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 22 instances
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **5**; blast radius 6.472; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.3%), Complexity Load (formerly Cognitive Load) (58.8%), Connectivity (formerly Api Exposure) (15.7%)
- **Documentation Coverage:** 16.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `swapStringsOutWithMappedTokens` **(Many-Argument Workhorses)** (Impact: 32.1)
    * *Intent:* /** * Swaps out strings in the given line, with tags, so each string will be seen as a single * toke...
  * `extractTokensFrom` **(Compute Cores)** (Impact: 16.1)
    * *Intent:* /** * Extracts tokens meaningful for processing Cobol source code from a Cobol source line. * * Such...
  * `swapMappedTokensOutWithSavedStrings` **(Stateful Encapsulated Methods)** (Impact: 5.5)
  * `StringTokenizerExtractor` **(State Mutators)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **11**; blast radius 2.416; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.6%), Complexity Load (formerly Cognitive Load) (65.8%), Connectivity (formerly Api Exposure) (24.5%)
- **Documentation Coverage:** 76.9231% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `processCommandLineArgumentArray` **(Stateful Encapsulated Methods)** (Impact: 22.1)
  * `storeOptionSettings` **(Stateful Encapsulated Methods)** (Impact: 15.7)
    * *Intent:* /** * Convert an option specification string into a map of option keys and values. * * @param option...
  * `loadArgProgramPaths` **(I/O & Config Routines)** (Impact: 9.1)
  * `lookupOption` **(Stateful Encapsulated Methods)** (Impact: 6.1)
  * `ArgumentHandler` **(Compute Cores)** (Impact: 5.6)
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **11**; blast radius 3.218; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.6%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (46.0%)
- **Documentation Coverage:** 37.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getBoilerplateCodeFromCopybooks` **(Generic / Templated Code)** (Impact: 9.6)
    * *Intent:* /** * Gets the lines from the cobol-check boilerplate copybooks, one for Working-Storage * and one f...
  * `generateCobolLinesForUnmockedCalls` **(I/O & Config Routines)** (Impact: 5.0)
  * `getWorkingStorageTestCode` **(Generic / Templated Code)** (Impact: 3.8)
    * *Intent:* /** * Gets the Working-Storage Section test code to be inserted into the program being generated. * ...
  * `getProcedureDivisionTestCode` **(I/O & Config Routines)** (Impact: 3.7)
    * *Intent:* /** * Gets the PROCEDURE DIVISION test code to be inserted into the program being generated. * * @re...
  * `concatenateTestSuites` **(Parameter Forwarders)** (Impact: 3.2)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 2.714; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.1%), Debt Markers (formerly Tech Debt) (95.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (72.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `setCounts` **(I/O & Config Routines)** (Impact: 10.1)
  * `setCurrentTestCaseFailure` **(Parameter Forwarders)** (Impact: 1.9)
  * `setCurrentTestCaseErrorMessage` **(Parameter Forwarders)** (Impact: 1.9)
  * `setCurrentTestSuiteName` **(Parameter Forwarders)** (Impact: 1.8)
  * `setNumberOfAllTests` **(Parameter Forwarders)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **17**; blast radius 2.358; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.9%), Complexity Load (formerly Cognitive Load) (51.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (19.8%)
- **Documentation Coverage:** 80.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expand` **(Many-Argument Workhorses)** (Impact: 27.9)
  * `findCopyBookNameEndPositionInCopyStatement` **(Compute Cores)** (Impact: 14.7)
  * `expandDB2` **(Defensive Guards)** (Impact: 13.2)
  * `extractCopybookNameFrom` **(Stateful Encapsulated Methods)** (Impact: 6.6)
  * `extractCopybookNameFromCopyStatement` **(Compute Cores)** (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 16 instances
