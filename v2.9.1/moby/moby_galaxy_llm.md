...
## 1. EXECUTIVE SUMMARY
- **Scope:** 2893 analyzed artifact(s), 279331 LOC.
- **Load-bearing artifact:** `internal/testutil/fakecontext/context.go` -- 875 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `internal/namesgenerator/names-generator.go` -- pulls in 346 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `client/testdata/ca.pem` at magnitude 5000.0 (structural weight, not risk).
...
| Avg Path Length | 2.4041 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `4.487`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +4.49; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 31%, Large Core Modules (2) 13%, Callbacks & Closures Files 12%, Encapsulated Accessors Files 8%, Large Core Modules 8%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 55.5 | 50.0 | 50.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 53.2 | 56.4 | 100.0 |
...
| connectivity | 14780 | 1817 | 12 | `integration-cli/docker_cli_build_test.go` |
...
| dead_code | 7475 | 1734 | 7 | `integration-cli/docker_cli_build_test.go` |
| credential | 9 | 6 | 0 | `hack/make.ps1` |
| threat | 261 | 84 | 0 | `Dockerfile` |
...
- `TestRunDisallowBindMountingRootToRoot` **(Many-Argument Workhorses)** (@ `integration-cli/docker_cli_run_test.go`) -> Impact: **408.0** | LOC: 1344
- `createWindows` **(Many-Argument Workhorses)** (@ `daemon/internal/libcontainerd/local/local_windows.go`) -> Impact: **361.2** | LOC: 1057
- `NewDaemon` **(Many-Argument Workhorses)** (@ `daemon/daemon.go`) -> Impact: **296.3** | LOC: 559
  * *Intent:* // NewDaemon sets up everything for the daemon to be able to service // requests from the webserver.
- `TestBuildExposeMorePorts` **(Many-Argument Workhorses)** (@ `integration-cli/docker_cli_build_test.go`) -> Impact: **256.5** | LOC: 1792
- `Validate-DCO` **(Many-Argument Workhorses)** (@ `hack/make.ps1`) -> Impact: **215.1** | LOC: 388
  * *Intent:* # Validates the DCO marker is present on each commit
- `restore` **(Many-Argument Workhorses)** (@ `daemon/daemon.go`) -> Impact: **203.8** | LOC: 437
- `verifyPlatformContainerResources` **(Many-Argument Workhorses)** (@ `daemon/daemon_unix.go`) -> Impact: **171.9** | LOC: 159
  * *Intent:* // verifyPlatformContainerResources performs platform-specific validation of the container's resource-configuration
- `postContainersCreate` **(Many-Argument Workhorses)** (@ `daemon/server/router/container/container_routes.go`) -> Impact: **157.5** | LOC: 243
- `withMounts` **(Many-Argument Workhorses)** (@ `daemon/oci_linux.go`) -> Impact: **151.5** | LOC: 213
  * *Intent:* // withMounts sets the container's mounts
- `NewNetwork` **(Many-Argument Workhorses)** (@ `daemon/libnetwork/controller.go`) -> Impact: **134.3** | LOC: 237
...
- **Global Archetype:** `file_cluster_2` (Drift: N/A IQR)
- **Magnitude:** 5000.0 | **LOC:** 1 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); blast radius 0.36; role: Pure Producer (Foundation)
- **Top Surface Vectors:** None above 0%
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
* *Risk/State:* None
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.00246
  * `Imports (Out-Degree: 0):` None
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **85**; blast radius 0.106; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.4%), Mutation Surface (formerly State Flux) (59.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (53.5%), Guard Balance (formerly Safety Score) (47.3%)
- **Documentation Coverage:** 85.1459% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `TestBuildExposeMorePorts` **(Many-Argument Workhorses)** (Impact: 256.5)
  * `TestBuildEnvironmentReplacementEnv` **(Compute Cores)** (Impact: 27.1)
  * `TestBuildWithInaccessibleFilesInContext` **(Defensive Guards)** (Impact: 22.6)
    * *Intent:* // Issue #5270 - ensure we throw a better error than "unexpected EOF" // when we can't access files ...
  * `TestBuildAddBadLinks` **(Defensive Guards)** (Impact: 19.1)
  * `TestBuildSymlinkBreakout` **(Defensive Guards)** (Impact: 16.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **50**; blast radius 0.106; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.7%), Debt Markers (formerly Tech Debt) (93.7%), Concurrency Surface (formerly Concurrency) (61.9%), Guard Balance (formerly Safety Score) (58.1%)
- **Documentation Coverage:** 81.6216% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `TestRunDisallowBindMountingRootToRoot` **(Many-Argument Workhorses)** (Impact: 408.0)
  * `TestRunMount` **(Compute Cores)** (Impact: 28.5)
  * `TestRunNoDupVolumes` **(Compute Cores)** (Impact: 19.5)
    * *Intent:* // Test for GH#10618
  * `TestRunCreateVolumesInSymlinkDir` **(I/O & Config Routines)** (Impact: 12.0)
    * *Intent:* // Volume path is a symlink which also exists on the host, and the host side is a file not a dir // ...
  * `TestRunVolumesFromSymlinkPath` **(I/O & Config Routines)** (Impact: 11.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **41**; blast radius 0.106; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (78.7%), Connectivity (formerly Api Exposure) (58.0%)
- **Documentation Coverage:** 58.9189% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ipamAllocateVersion` **(Many-Argument Workhorses)** (Impact: 82.0)
  * `UnmarshalJSON` **(Type Conversions)** (Impact: 58.5)
  * `delete` **(Many-Argument Workhorses)** (Impact: 47.6)
    * *Intent:* // This function gets called in 3 ways: // - Delete() -- (false, false) // remove if endpoint count ...
  * `createEndpoint` **(Many-Argument Workhorses)** (Impact: 40.0)
  * `validateConfiguration` **(Defensive Guards)** (Impact: 29.6)
**Contextual Mitigations & Amplifications:**
...
- `daemon/logger/internal/logdriver/io.go` -> **Severity: 0.033** (Bridge: 0.0004 * Flux: 76.9689%)
- `hack/make/binary` -> **Severity: 0.009** (Bridge: 0.0003 * Flux: 31.0026%)
- `internal/testutil/fakecontext/context.go` -> **Severity: 0.007** (Bridge: 0.0003 * Flux: 20.8573%)
- `pkg/plugins/transport/http.go` -> **Severity: 0.007** (Bridge: 0.0001 * Flux: 99.1837%)
- `daemon/graphdriver/graphtest/testutil.go` -> **Severity: 0.004** (Bridge: 0.0 * Flux: 99.9463%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `daemon/logger/internal/logdriver/io.go` -> **Severity: 11.732** (Embedded: 0.2312 * Error Risk: 50.7407%)
- `hack/make/binary` -> **Severity: 9.781** (Embedded: 0.1563 * Error Risk: 62.5811%)
- `internal/testutil/fakecontext/context.go` -> **Severity: 9.629** (Embedded: 0.2871 * Error Risk: 33.5322%)
- `hack/make/binary-proxy` -> **Severity: 8.7** (Embedded: 0.1155 * Error Risk: 75.2927%)
- `hack/make/binary-daemon` -> **Severity: 8.316** (Embedded: 0.1155 * Error Risk: 71.9676%)
