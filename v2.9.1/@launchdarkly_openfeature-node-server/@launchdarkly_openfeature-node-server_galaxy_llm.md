...
## 1. EXECUTIVE SUMMARY
- **Scope:** 9 analyzed artifact(s), 16 LOC.
- **Load-bearing artifact:** `package/CONTRIBUTING.md` -- 1 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `package/README.md` -- pulls in 1 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `package/release-please-config.json` at magnitude 14.16 (structural weight, not risk).
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 101.523; role: Pure Consumer (Orchestrator)
