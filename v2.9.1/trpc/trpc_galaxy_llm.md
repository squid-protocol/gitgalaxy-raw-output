...
## 1. EXECUTIVE SUMMARY
- **Scope:** 1177 analyzed artifact(s), 83010 LOC.
- **Load-bearing artifact:** `packages/server/src/observable/observable.ts` -- 61 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `www/src/pages/index.tsx` -- pulls in 24 dependencies, the widest assembly point in the scan.
...
| Avg Path Length | 2.8112 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `3.48`
> **Composition Archetype:** `Hub-Coupled App` (z +3.48; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 40%, Declarative / Non-Code 17%, Callbacks & Closures Files 11%, Large Core Modules (2) 8%, Generic / Templated Code Files 5%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 95.2 | 21.0 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 4.0 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 1.7 | 0.0 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 17.6 | 3.5 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 79.3 | 3.3 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 46.9 | 33.3 | 0.0 |
...
| dead_code | 348 | 213 | 1 | `examples/next-prisma-websockets-starter/src/pages/index.tsx` |
| credential | 50 | 4 | 0 | `www/src/components/TwitterWall/script.output.ts` |
| threat | 70 | 33 | 0 | `packages/server/src/unstable-core-do-not-import/http/formDataToObject.test.ts` |
...
- `getProcedureTypeName` **(Many-Argument Workhorses)** (@ `packages/openapi/src/generate.ts`) -> Impact: **264.9** | LOC: 725
  * *Intent:* /** * Inspect `_def.type` and return the procedure type string, or null if this is * not a procedure (e.g. a nested router). */
- `createRootHooks` **(Defensive Guards)** (@ `packages/react-query/src/shared/hooks/createHooksInternal.tsx`) -> Impact: **249.4** | LOC: 688
  * *Intent:* /** */
- `transform` **(Many-Argument Workhorses)** (@ `packages/upgrade/src/transforms/hooksToOptions.ts`) -> Impact: **181.2** | LOC: 383
- `resolveResponse` **(Defensive Guards)** (@ `packages/server/src/unstable-core-do-not-import/http/resolveResponse.ts`) -> Impact: **156.6** | LOC: 558
- `pluginLlmsTxt` **(Defensive Guards)** (@ `www/docusaurus.config.ts`) -> Impact: **129.8** | LOC: 305
...
- `postBuild` **(Many-Argument Workhorses)** (@ `www/docusaurus.config.ts`) -> Impact: **117.8** | LOC: 320
- `getWSConnectionHandler` **(Defensive Guards)** (@ `packages/server/src/adapters/ws.ts`) -> Impact: **89.6** | LOC: 462
- `extractTwoslashBlocks` **(Defensive Guards)** (@ `www/scripts/check-twoslash.ts`) -> Impact: **79.1** | LOC: 224
- `request` **(Compute Cores)** (@ `examples/openapi-codegen/src/client/generated/client/client.gen.ts`) -> Impact: **72.1** | LOC: 170
- `migrateUseUtils` **(Compute Cores)** (@ `packages/upgrade/src/transforms/hooksToOptions.ts`) -> Impact: **69.3** | LOC: 169
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (71.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (27.1%), Complexity Load (formerly Cognitive Load) (7.2%), Guard Balance (formerly Safety Score) (2.8%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (39.6%), Complexity Load (formerly Cognitive Load) (16.4%), Guard Balance (formerly Safety Score) (9.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **5**; blast radius 1.618; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (95.5%), Guard Balance (formerly Safety Score) (62.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (39.6%), Connectivity (formerly Api Exposure) (30.5%)
- **Documentation Coverage:** 55.5556% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getProcedureTypeName` **(Many-Argument Workhorses)** (Impact: 264.9)
    * *Intent:* /** * Inspect `_def.type` and return the procedure type string, or null if this is * not a procedure...
  * `convertPlainObject` **(Many-Argument Workhorses)** (Impact: 44.2)
  * `convertUnionType` **(Many-Argument Workhorses)** (Impact: 36.6)
    * *Intent:* // --------------------------------------------------------------------------- // Union type convers...
  * `typeToJsonSchema` **(Many-Argument Workhorses)** (Impact: 34.8)
    * *Intent:* // --------------------------------------------------------------------------- // Type → JSON Schema...
  * `buildOpenAPIDocument` **(Many-Argument Workhorses)** (Impact: 32.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.005098
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **14**; blast radius 1.739; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (44.2%), Guard Balance (formerly Safety Score) (34.3%), Mutation Surface (formerly State Flux) (22.4%), Concurrency Surface (formerly Concurrency) (14.6%)
- **Documentation Coverage:** 88.5714% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createRootHooks` **(Defensive Guards)** (Impact: 249.4)
    * *Intent:* /** */
  * `useQuery` **(Defensive Guards)** (Impact: 53.6)
  * `useInfiniteQuery` **(Defensive Guards)** (Impact: 51.6)
  * `useSubscription` **(Many-Argument Workhorses)** (Impact: 49.4)
  * `usePrefetchInfiniteQuery` **(Defensive Guards)** (Impact: 26.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000185 | `Ripple Effect (Closeness):` 0.014006
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (30.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (27.4%), Guard Balance (formerly Safety Score) (15.6%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (44.1%), Complexity Load (formerly Cognitive Load) (43.8%), Guard Balance (formerly Safety Score) (43.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `nextIterable` **(I/O & Config Routines)** (Impact: 9.8)
  * `nextIterable` **(I/O & Config Routines)** (Impact: 8.3)
  * `nextIterable` **(I/O & Config Routines)** (Impact: 8.2)
  * `nextIterable` **(I/O & Config Routines)** (Impact: 8.2)
  * `nextIterable` **(I/O & Config Routines)** (Impact: 8.2)
**Contextual Mitigations & Amplifications:**
...
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (31.5%), Complexity Load (formerly Cognitive Load) (29.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (8.6%)
- **Documentation Coverage:** 97.6744% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `factory` **(Defensive Guards)** (Impact: 36.5)
  * `nextIterable` **(Defensive Guards)** (Impact: 11.1)
  * `onError` **(Defensive Guards)** (Impact: 6.8)
  * `onopen` **(I/O & Config Routines)** (Impact: 4.9)
  * `waitForClientState` **(Type Conversions)** (Impact: 4.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (48.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **3**; blast radius 1.467; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (95.2%), Connectivity (formerly Api Exposure) (58.9%), Complexity Load (formerly Cognitive Load) (3.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.014608
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.7%), Guard Balance (formerly Safety Score) (26.2%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (9.1%), Complexity Load (formerly Cognitive Load) (8.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (35.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (31.1%), Complexity Load (formerly Cognitive Load) (27.6%)
- **Documentation Coverage:** 97.2973% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createContext` **(Defensive Guards)** (Impact: 15.1)
  * `sleep` **(Compute Cores)** (Impact: 14.7)
  * `getCtx` **(Defensive Guards)** (Impact: 9.1)
  * `getCtxResource` **(I/O & Config Routines)** (Impact: 8.1)
    * *Intent:* /** * Test resource */
  * `next` **(Defensive Guards)** (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **8**; blast radius 4.7; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (98.5%), Mutation Surface (formerly State Flux) (86.2%), Guard Balance (formerly Safety Score) (59.3%), Connectivity (formerly Api Exposure) (53.8%)
- **Documentation Coverage:** 68.9655% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `jsonlStreamConsumer` **(Defensive Guards)** (Impact: 49.4)
    * *Intent:* /** * JSON Lines stream consumer */
  * `createBatchStreamProducer` **(Defensive Guards)** (Impact: 41.0)
  * `decodeChunkDefinition` **(Defensive Guards)** (Impact: 27.7)
  * `createStreamsManager` **(Callbacks & Closures)** (Impact: 12.3)
    * *Intent:* /** * Creates a handler for managing stream controllers and their lifecycle */
  * `encodeAsyncIterable` **(Defensive Guards)** (Impact: 11.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000489 | `Ripple Effect (Closeness):` 0.099595
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 0.565; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (62.1%), Complexity Load (formerly Cognitive Load) (31.1%)
- **Documentation Coverage:** 93.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `transform` **(Many-Argument Workhorses)** (Impact: 181.2)
  * `migrateUseUtils` **(Compute Cores)** (Impact: 69.3)
    * *Intent:* // Migrate trpc.useUtils() to useQueryClient()
  * `removeSuspenseDestructuring` **(Defensive Guards)** (Impact: 41.5)
  * `ensureUseTRPCCall` **(Compute Cores)** (Impact: 8.2)
  * `replaceHooksWithOptions` **(I/O & Config Routines)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.00085
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **13**; blast radius 2.463; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (96.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (51.2%), Guard Balance (formerly Safety Score) (40.0%), Complexity Load (formerly Cognitive Load) (38.7%)
- **Documentation Coverage:** 89.4737% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolveResponse` **(Defensive Guards)** (Impact: 156.6)
  * `initResponse` **(Defensive Guards)** (Impact: 38.9)
  * `create` **(Defensive Guards)** (Impact: 32.5)
  * `formatError` **(Defensive Guards)** (Impact: 12.6)
  * `formatError` **(Defensive Guards)** (Impact: 12.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.4e-05 | `Ripple Effect (Closeness):` 0.097626
...
- **Blast Radius:** changing it is visible to **16** in-repo importer(s); it depends on **12**; blast radius 8.806; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (65.2%), Connectivity (formerly Api Exposure) (65.1%), Guard Balance (formerly Safety Score) (49.3%)
- **Documentation Coverage:** 80.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getWSConnectionHandler` **(Defensive Guards)** (Impact: 89.6)
  * `handleRequest` **(Many-Argument Workhorses)** (Impact: 59.0)
  * `onabort` **(I/O & Config Routines)** (Impact: 33.2)
  * `applyWSSHandler` **(Defensive Guards)** (Impact: 12.7)
  * `getRawInput` **(I/O & Config Routines)** (Impact: 11.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 13 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000581 | `Ripple Effect (Closeness):` 0.043222
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (94.1%), Mutation Surface (formerly State Flux) (81.2%), Test Surface (formerly Verification) (80.0%), Credential Material (formerly Secrets Risk) (61.3%)
- **Documentation Coverage:** 41.9147% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `pluginLlmsTxt` **(Defensive Guards)** (Impact: 129.8)
    * *Intent:* /** * Thank you to prisma docs for this plugin <3 * https://github.com/prisma/docs/blob/22208d52e416...
  * `postBuild` **(Many-Argument Workhorses)** (Impact: 117.8)
  * `findMdxFilesRecursively` **(Defensive Guards)** (Impact: 19.7)
    * *Intent:* // Recursive function to find all .md/.mdx files
  * `configurePostCss` **(Compute Cores)** (Impact: 3.3)
  * `myPlugin` **(Interface Declarations)** (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
* *Amplified Cascading Flux:* 16 instances
* *Concurrency (weighted view):* 33
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 0.441; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (41.9%), Mutation Surface (formerly State Flux) (41.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `[Symbol.asyncIterator]` **(Parameter Forwarders)** (Impact: 3.3)
  * `[Symbol.asyncIterator]` **(Parameter Forwarders)** (Impact: 3.2)
  * `[Symbol.asyncIterator]` **(I/O & Config Routines)** (Impact: 2.3)
  * `serverResourceForStream` **(Generic / Templated Code)** (Impact: 2.2)
  * `start` **(State Mutators)** (Impact: 1.6)
...
- `packages/server/src/observable/observable.ts` -> **Severity: 0.085** (Bridge: 0.0009 * Flux: 92.732%)
- `packages/server/src/adapters/next.ts` -> **Severity: 0.058** (Bridge: 0.0012 * Flux: 50.0%)
- `packages/server/src/unstable-core-do-not-import/stream/jsonl.ts` -> **Severity: 0.042** (Bridge: 0.0005 * Flux: 86.2025%)
- `packages/server/src/adapters/ws.ts` -> **Severity: 0.038** (Bridge: 0.0006 * Flux: 65.2291%)
- `packages/client/src/__tests__/testClientResource.ts` -> **Severity: 0.03** (Bridge: 0.0018 * Flux: 16.4406%)
...
- `packages/server/src/observable/observable.ts` -> **Severity: 8.139** (Embedded: 0.1062 * Error Risk: 76.6446%)
- `packages/server/src/unstable-core-do-not-import/rootConfig.ts` -> **Severity: 7.988** (Embedded: 0.0999 * Error Risk: 80.0%)
- `packages/server/src/unstable-core-do-not-import/transformer.ts` -> **Severity: 7.798** (Embedded: 0.0976 * Error Risk: 79.873%)
- `packages/server/src/unstable-core-do-not-import/procedureBuilder.ts` -> **Severity: 7.675** (Embedded: 0.1031 * Error Risk: 74.4389%)
