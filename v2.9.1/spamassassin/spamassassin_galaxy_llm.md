...
| **Git Remote** | `https://github.com/apache/spamassassin` |
...
## 1. EXECUTIVE SUMMARY
- **Scope:** 518 analyzed artifact(s), 43439 LOC.
- **Load-bearing artifact:** `debian/rules` -- 8 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `masses/mass-check` -- pulls in 45 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `debian/GPG.KEY` at magnitude 5000.0 (structural weight, not risk).
...
| Metric | Value |
|---|---|
| Total Artifacts | 1881 |
| Analyzed Artifacts (Scanned) | 518 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 1363 |
| Total LOC | 43439 |
...
> **Architectural Drift Z-Score:** `3.192`
> **Composition Archetype:** `Mid Flat Project` (z +3.19; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 50%, Data / Markup / Trivial 20%, Compute Cores Files 10%, Large Core Modules (3) 6%, Large Core Modules 4%
...
- `.cf`: 235x Excluded (Unsupported Extension: '.cf'), 9x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir)
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 67.0 | 72.0 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 35.0 | 1.1 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 34.4 | 0.0 | 0.0 |
...
| events | 39 | 18 | 0 | `masses/rule-qa/automc/sorttable.js` |
| tests | 1379 | 236 | 5 | `t/uri.t` |
| docs | 95 | 25 | 0 | `spamc/spamc.pod` |
| debt | 1792 | 271 | 10 | `masses/mass-check` |
| mutation | 9488 | 407 | 34 | `spamc/libspamc.c` |
...
- `_opensocket` **(Many-Argument Workhorses)** (@ `spamc/libspamc.c`) -> Impact: **926.1** | LOC: 2243
...
- `set_config` **(Many-Argument Workhorses)** (@ `rulesrc/sandbox/dos/SIQ.pm`) -> Impact: **342.0** | LOC: 214
- `generate_messages` **(Many-Argument Workhorses)** (@ `masses/mass-check`) -> Impact: **243.6** | LOC: 267
...
- `adapt` **(Many-Argument Workhorses)** (@ `masses/garescorer.c`) -> Impact: **237.4** | LOC: 197
  * *Intent:* #ifdef LAMARCK
- `gen_class` **(Many-Argument Workhorses)** (@ `masses/rule-qa/corpus-hourly`) -> Impact: **221.6** | LOC: 272
- `read_args` **(Many-Argument Workhorses)** (@ `spamc/spamc.c`) -> Impact: **220.1** | LOC: 381
...
- `message_filter` **(Many-Argument Workhorses)** (@ `spamc/libspamc.c`) -> Impact: **182.0** | LOC: 375
- `wanted` **(Many-Argument Workhorses)** (@ `masses/mass-check`) -> Impact: **181.0** | LOC: 240
- `myMutation` **(Many-Argument Workhorses)** (@ `masses/garescorer.c`) -> Impact: **179.2** | LOC: 230
...
- `client_mode` **(I/O & Config Routines)** (@ `masses/mass-check`) -> Impact: **148.3** | LOC: 387
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **24**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.0%), Complexity Load (formerly Cognitive Load) (95.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_opensocket` **(Many-Argument Workhorses)** (Impact: 926.1)
    * *Intent:* /* * opensocket() * * Given a socket family (PF_INET or PF_INET6 or PF_UNIX), try to * create this s...
  * `message_filter` **(Many-Argument Workhorses)** (Impact: 182.0)
  * `message_tell` **(Many-Argument Workhorses)** (Impact: 136.6)
  * `transport_setup` **(Many-Argument Workhorses)** (Impact: 113.0)
    * *Intent:* * transport_setup() * * Given a "transport" object that says how we're to connect to the * spam daem...
  * `_opensocket` **(Many-Argument Workhorses)** (Impact: 89.5)
    * *Intent:* #else
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **45**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.2%), Complexity Load (formerly Cognitive Load) (87.5%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `generate_messages` **(Many-Argument Workhorses)** (Impact: 243.6)
    * *Intent:* # Input: # - Number of messages to generate (scalar) # - Hash of Arrays of outstanding requests (ref...
  * `wanted` **(Many-Argument Workhorses)** (Impact: 181.0)
  * `client_mode` **(I/O & Config Routines)** (Impact: 148.3)
    * *Intent:* # this is the function that implements client mode. generally, in a loop: # make a request of the se...
  * `usage` **(Compute Cores)** (Impact: 75.4)
  * `http_make_request` **(Many-Argument Workhorses)** (Impact: 59.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **24**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (79.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `show_default_view` **(Compute Cores)** (Impact: 124.6)
  * `read_freqs_file` **(Many-Argument Workhorses)** (Impact: 116.7)
  * `get_freqs_for_rule` **(Many-Argument Workhorses)** (Impact: 72.6)
  * `generate_scoremap_chart` **(Many-Argument Workhorses)** (Impact: 61.0)
  * `assemble_url` **(Compute Cores)** (Impact: 49.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (70.8%)
- **Documentation Coverage:** 64.7059% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `adapt` **(Many-Argument Workhorses)** (Impact: 237.4)
    * *Intent:* #ifdef LAMARCK
  * `myMutation` **(Many-Argument Workhorses)** (Impact: 179.2)
    * *Intent:* * This mutation function tosses a weighted coin for each allele. * If the allele is to be mutated, t...
  * `Crossover` **(Many-Argument Workhorses)** (Impact: 110.9)
    * *Intent:* /***************************************************************************** * Crossover implement...
  * `main` **(Many-Argument Workhorses)** (Impact: 54.1)
  * `showSummary` **(Compute Cores)** (Impact: 40.6)
    * *Intent:* #endif
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **35**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.4%), Complexity Load (formerly Cognitive Load) (96.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `sa_t_init` **(Compute Cores)** (Impact: 106.8)
    * *Intent:* # Set up for testing. Exports (as global vars): # out: $home: $HOME env variable # an environment va...
  * `start_spamd` **(Compute Cores)** (Impact: 66.1)
    * *Intent:* # out: $spamd_stderr
  * `patterns_run_cb` **(Compute Cores)** (Impact: 45.5)
    * *Intent:* # ---------------------------------------------------------------------------
  * `spamcrun` **(Many-Argument Workhorses)** (Impact: 38.3)
  * `ok_all_patterns` **(Compute Cores)** (Impact: 34.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (81.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (61.5%)
- **Documentation Coverage:** 62.069% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `set_config` **(Many-Argument Workhorses)** (Impact: 342.0)
  * `_parse_eval_call` **(Many-Argument Workhorses)** (Impact: 115.5)
  * `_send_siq_query` **(Many-Argument Workhorses)** (Impact: 82.5)
  * `_harvest_siq_responses` **(Compute Cores)** (Impact: 55.9)
  * `parsed_metadata` **(Compute Cores)** (Impact: 46.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **28**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.6%)
- **Documentation Coverage:** 71.4286% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `read_args` **(Many-Argument Workhorses)** (Impact: 220.1)
    * *Intent:* /** * Does the command line parsing for argv[]. * * Returns EX_OK or EX_TEMPFAIL if successful. EX_T...
  * `main` **(Many-Argument Workhorses)** (Impact: 133.6)
  * `combine_args` **(Many-Argument Workhorses)** (Impact: 50.6)
    * *Intent:* /* combine_args() :: parses spamc.conf for options, and combines those * with options passed via com...
  * `print_usage` **(I/O & Config Routines)** (Impact: 30.9)
  * `get_output_fd` **(Compute Cores)** (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (79.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_mbox_extract_all` **(Compute Cores)** (Impact: 72.2)
  * `parse_rfc822_date` **(Compute Cores)** (Impact: 72.2)
  * `_mklink` **(Compute Cores)** (Impact: 53.3)
  * `dist_across_dests` **(Compute Cores)** (Impact: 17.2)
  * `find_srcs` **(I/O & Config Routines)** (Impact: 13.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (100.0%), Complexity Load (formerly Cognitive Load) (88.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `makeSortable` **(Compute Cores)** (Impact: 41.5)
  * `getInnerText` **(Defensive Guards)** (Impact: 33.4)
  * `guessType` **(Compute Cores)** (Impact: 22.4)
  * `innerSortFunction` **(Compute Cores)** (Impact: 19.1)
  * `forEach` **(Defensive Guards)** (Impact: 19.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.1%), Complexity Load (formerly Cognitive Load) (82.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `gen_class` **(Many-Argument Workhorses)** (Impact: 123.7)
  * `gen_report_freqs_all` **(Many-Argument Workhorses)** (Impact: 45.6)
    * *Intent:* # ---------------------------------------------------------------------------
  * `locate_input` **(I/O & Config Routines)** (Impact: 40.0)
    * *Intent:* # ---------------------------------------------------------------------------
  * `start_hit_frequencies_at_rev` **(Compute Cores)** (Impact: 30.4)
    * *Intent:* # ---------------------------------------------------------------------------
  * `time_filter_fileset` **(Compute Cores)** (Impact: 27.8)
    * *Intent:* # ---------------------------------------------------------------------------
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.1%), Complexity Load (formerly Cognitive Load) (82.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_print_overlap_ratios` **(Many-Argument Workhorses)** (Impact: 68.7)
  * `readlogs` **(I/O & Config Routines)** (Impact: 43.8)
  * `compute_overlaps_for_rule` **(Compute Cores)** (Impact: 29.7)
  * `_print_scoremap` **(Compute Cores)** (Impact: 21.0)
  * `_hmap_to_overlap_ratio` **(Many-Argument Workhorses)** (Impact: 12.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (79.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `assemble_regexps` **(I/O & Config Routines)** (Impact: 39.0)
  * `expand_with_dots` **(Compute Cores)** (Impact: 37.0)
  * `subsume_with_dotstars` **(Compute Cores)** (Impact: 33.7)
  * `collapse_pats` **(I/O & Config Routines)** (Impact: 32.5)
  * `usage` **(Compute Cores)** (Impact: 23.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.3%), Complexity Load (formerly Cognitive Load) (85.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `gen_class` **(Many-Argument Workhorses)** (Impact: 221.6)
  * `locate` **(I/O & Config Routines)** (Impact: 31.9)
  * `sort_all` **(Compute Cores)** (Impact: 18.8)
  * `update_rsync` **(I/O & Config Routines)** (Impact: 14.8)
  * `current` **(Compute Cores)** (Impact: 12.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.2%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (71.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `MY::postamble` **(Compute Cores)** (Impact: 71.2)
  * `MY::constants` **(Compute Cores)** (Impact: 37.6)
    * *Intent:* # Now override the constants routine to add our own macros.
  * `MY::dist` **(Compute Cores)** (Impact: 12.5)
    * *Intent:* # Override some vars in the dist section.
  * `_set_macro_SYSCONFDIR` **(I/O & Config Routines)** (Impact: 12.3)
    * *Intent:* # repository. # # The first parameter must be one value from @REPOSITORIES. # # *SYSCONFDIR can be o...
  * `_set_macro_LOCALSTATEDIR` **(I/O & Config Routines)** (Impact: 12.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (75.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `train` **(Many-Argument Workhorses)** (Impact: 40.8)
    * *Intent:* /* Trains the perceptron using stochastic gradient descent. */
  * `main` **(Many-Argument Workhorses)** (Impact: 28.2)
  * `write_weights` **(Compute Cores)** (Impact: 27.1)
    * *Intent:* /* Writes out the weights in SpamAssassin score space. */
  * `get_random_test` **(I/O & Config Routines)** (Impact: 12.4)
    * *Intent:* /* Get a random test using roulette wheel selection. This is not used anymore. */
  * `usage` **(I/O & Config Routines)** (Impact: 8.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 3 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.863; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (79.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `spamc_getopt_long` **(Many-Argument Workhorses)** (Impact: 125.5)
  * `spamc_getopt` **(Many-Argument Workhorses)** (Impact: 46.0)
  * `main` **(Compute Cores)** (Impact: 30.4)
    * *Intent:* #ifdef TESTGETOPT
  * `optiserr` **(Stateful Encapsulated Methods)** (Impact: 25.2)
  * `longoptiserr` **(Stateful Encapsulated Methods)** (Impact: 21.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (77.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `compute_thresholds` **(I/O & Config Routines)** (Impact: 27.1)
  * `brc_line_to_score` **(Compute Cores)** (Impact: 26.4)
  * `compute_prob_for_token` **(Many-Argument Workhorses)** (Impact: 19.3)
  * `draw_hist` **(I/O & Config Routines)** (Impact: 17.9)
  * `cached_compute_prob_for_token` **(Many-Argument Workhorses)** (Impact: 12.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.2%), Complexity Load (formerly Cognitive Load) (82.5%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `generate_scores` **(Compute Cores)** (Impact: 33.2)
  * `fixup_meta_predicates` **(Compute Cores)** (Impact: 31.2)
  * `build_new_scores` **(I/O & Config Routines)** (Impact: 26.2)
  * `read_gascores` **(I/O & Config Routines)** (Impact: 17.4)
  * `read_oldscores` **(I/O & Config Routines)** (Impact: 14.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 69 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.863; role: Isolated/Orphan
...
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `read_ranges` **(I/O & Config Routines)** (Impact: 65.3)
  * `writetests_c` **(I/O & Config Routines)** (Impact: 22.9)
  * `readlogs` **(I/O & Config Routines)** (Impact: 21.1)
  * `writescores_c` **(I/O & Config Routines)** (Impact: 15.7)
  * `readscores` **(Interface Declarations)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `readrules` **(Compute Cores)** (Impact: 63.0)
    * *Intent:* # of your average parse
  * `lintrules` **(Compute Cores)** (Impact: 61.1)
  * `usage` **(I/O & Config Routines)** (Impact: 15.2)
  * `concat_rule_lang` **(Compute Cores)** (Impact: 7.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 69 instances
* *State Mutation (weighted view):* 207
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 1.863; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (79.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `plot_gp` **(I/O & Config Routines)** (Impact: 35.5)
  * `read_logs` **(Compute Cores)** (Impact: 29.9)
  * `summarise` **(I/O & Config Routines)** (Impact: 20.2)
  * `usage` **(I/O & Config Routines)** (Impact: 11.8)
  * `collapse_periods` **(I/O & Config Routines)** (Impact: 10.7)
**Contextual Mitigations & Amplifications:**
...
- `t/utf8.t` -> **Severity: 0.289** (Embedded: 0.0039 * Error Risk: 74.8553%)
- `t/header.t` -> **Severity: 0.134** (Embedded: 0.0019 * Error Risk: 69.385%)
- `t/debug.t` -> **Severity: 0.121** (Embedded: 0.0019 * Error Risk: 62.5811%)
- `t/plugin.t` -> **Severity: 0.121** (Embedded: 0.0019 * Error Risk: 62.5811%)
