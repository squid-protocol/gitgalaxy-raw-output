...
## 1. EXECUTIVE SUMMARY
- **Scope:** 17 analyzed artifact(s), 2724 LOC.
- **Load-bearing artifact:** `platformdirs-4.9.4/src/platformdirs/api.py` -- 5 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `platformdirs-4.9.4/tests/test_api.py` -- pulls in 12 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `platformdirs-4.9.4/src/platformdirs/windows.py` at magnitude 254.98 (structural weight, not risk).
...
> **File Composition:** Generic / Templated Code Files 53%, Large Core Modules (3) 18%, Data / Markup / Trivial 6%, Declarative / Non-Code 6%, Large Core Modules 6%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 58.0 | 78.0 | 100.0 |
...
- `test_android_active` **(Many-Argument Workhorses)** (@ `platformdirs-4.9.4/tests/test_api.py`) -> Impact: **27.8** | LOC: 26
- `test_android` **(Many-Argument Workhorses)** (@ `platformdirs-4.9.4/tests/test_android.py`) -> Impact: **26.1** | LOC: 42
- `test_windows` **(Many-Argument Workhorses)** (@ `platformdirs-4.9.4/tests/test_windows.py`) -> Impact: **24.8** | LOC: 45
- `test_macos_homebrew` **(Many-Argument Workhorses)** (@ `platformdirs-4.9.4/tests/test_macos.py`) -> Impact: **22.2** | LOC: 41
- `_android_folder` **(I/O & Config Routines)** (@ `platformdirs-4.9.4/src/platformdirs/android.py`) -> Impact: **15.2** | LOC: 45
  * *Intent:* """:returns: base folder for the Android OS or None if it cannot be found"""
- `_append_parts` **(Stateful Encapsulated Methods)** (@ `platformdirs-4.9.4/src/platformdirs/windows.py`) -> Impact: **14.7** | LOC: 14
- `get_win_folder_via_ctypes` **(Compute Cores)** (@ `platformdirs-4.9.4/src/platformdirs/windows.py`) -> Impact: **14.1** | LOC: 55
...
- `get_win_folder_if_csidl_name_not_env_var` **(Compute Cores)** (@ `platformdirs-4.9.4/src/platformdirs/windows.py`) -> Impact: **13.1** | LOC: 35
  * *Intent:* """Get a folder for a CSIDL name that does not exist as an environment variable."""
- `test_macos_xdg_env_vars` **(Defensive Guards)** (@ `platformdirs-4.9.4/tests/test_macos.py`) -> Impact: **12.8** | LOC: 12
- `test_macos` **(Many-Argument Workhorses)** (@ `platformdirs-4.9.4/tests/test_macos.py`) -> Impact: **12.1** | LOC: 41
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **8**; blast radius 68.352; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.3%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (61.6%)
- **Documentation Coverage:** 51.7857% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_append_parts` **(Stateful Encapsulated Methods)** (Impact: 14.7)
  * `get_win_folder_via_ctypes` **(Compute Cores)** (Impact: 14.1)
    * *Intent:* """Get folder via :func:`SHGetKnownFolderPath`. See https://learn.microsoft.com/en-us/windows/win32/...
  * `get_win_folder_if_csidl_name_not_env_var` **(Compute Cores)** (Impact: 13.1)
    * *Intent:* """Get a folder for a CSIDL name that does not exist as an environment variable."""
  * `get_win_folder_from_registry` **(Compute Cores)** (Impact: 8.9)
    * *Intent:* """Get folder from the registry. This is a fallback technique at best. I'm not sure if using the reg...
  * `get_win_folder_from_env_vars` **(Compute Cores)** (Impact: 6.6)
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **6**; blast radius 262.734; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (88.9%), Mutation Surface (formerly State Flux) (87.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (58.8%)
- **Documentation Coverage:** 5.8824% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 6.4)
  * `_append_app_name_and_version` **(Stateful Encapsulated Methods)** (Impact: 5.6)
  * `_first_item_as_path_if_multipath` **(Encapsulated Accessors)** (Impact: 3.7)
  * `_optionally_create_directory` **(Encapsulated Accessors)** (Impact: 3.6)
  * `iter_config_paths` **(Generic / Templated Code)** (Impact: 3.0)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **11**; blast radius 68.352; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (78.6%), Connectivity (formerly Api Exposure) (64.8%)
- **Documentation Coverage:** 9.4595% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `user_runtime_dir` **(Compute Cores)** (Impact: 7.8)
    * *Intent:* """:returns: runtime directory tied to the user, e.g. ``$XDG_RUNTIME_DIR/$appname/$version``. If ``$...
  * `_get_user_dirs_folder` **(Stateful Encapsulated Methods)** (Impact: 6.7)
    * *Intent:* """Return directory from user-dirs.dirs config file. See https://freedesktop.org/wiki/Software/xdg-u...
  * `site_runtime_dir` **(Generic / Templated Code)** (Impact: 5.0)
    * *Intent:* """:returns: runtime directory shared by users, e.g. ``/run/$appname/$version`` or ``$XDG_RUNTIME_DI...
  * `site_applications_dir` **(Generic / Templated Code)** (Impact: 4.4)
    * *Intent:* """:returns: applications directory shared by users, e.g. ``/usr/share/applications``"""
  * `user_data_dir` **(Generic / Templated Code)** (Impact: 4.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 33.837; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (36.1%), Guard Balance (formerly Safety Score) (23.7%), Connectivity (formerly Api Exposure) (8.5%)
- **Documentation Coverage:** 96.4286% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_windows` **(Many-Argument Workhorses)** (Impact: 24.8)
  * `_setup_ctypes_mocks` **(Stateful Encapsulated Methods)** (Impact: 7.4)
    * *Intent:* """Mock ctypes internals so get_win_folder_via_ctypes can be tested on non-Windows."""
  * `test_get_win_folder_via_ctypes_unknown_csidl` **(Defensive Guards)** (Impact: 6.3)
  * `test_pick_get_win_folder_ctypes` **(Defensive Guards)** (Impact: 4.7)
  * `test_get_win_folder_via_ctypes_null_result` **(Callbacks & Closures)** (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 14 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 33.837; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (78.5%), Guard Balance (formerly Safety Score) (51.5%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 59.5506% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_set_platform_dir_class` **(Stateful Encapsulated Methods)** (Impact: 6.7)
  * `user_data_dir` **(Many-Argument Workhorses)** (Impact: 3.9)
  * `user_config_dir` **(Many-Argument Workhorses)** (Impact: 3.9)
  * `user_cache_dir` **(Many-Argument Workhorses)** (Impact: 3.9)
  * `user_state_dir` **(Many-Argument Workhorses)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 33.837; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (22.4%), Connectivity (formerly Api Exposure) (10.4%), Guard Balance (formerly Safety Score) (8.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_func_to_path` **(Stateful Encapsulated Methods)** (Impact: 5.3)
  * `test_xdg_variable_not_set` **(Defensive Guards)** (Impact: 4.4)
  * `test_xdg_variable_empty_value` **(Defensive Guards)** (Impact: 4.4)
  * `test_xdg_variable_custom_value` **(Defensive Guards)** (Impact: 4.4)
  * `test_platform_on_win32` **(Defensive Guards)** (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 33.837; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (37.2%), Complexity Load (formerly Cognitive Load) (31.0%), Connectivity (formerly Api Exposure) (6.8%)
- **Documentation Coverage:** 96.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_macos_homebrew` **(Many-Argument Workhorses)** (Impact: 22.2)
  * `test_macos_xdg_env_vars` **(Defensive Guards)** (Impact: 12.8)
  * `test_macos` **(Many-Argument Workhorses)** (Impact: 12.1)
  * `test_macos_xdg_site_dirs` **(Defensive Guards)** (Impact: 7.3)
  * `test_macos_xdg_empty_falls_back` **(Many-Argument Workhorses)** (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **9**; blast radius 97.114; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.1%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (60.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_android_folder` **(I/O & Config Routines)** (Impact: 15.2)
    * *Intent:* """:returns: base folder for the Android OS or None if it cannot be found"""
  * `user_log_dir` **(Generic / Templated Code)** (Impact: 3.2)
    * *Intent:* """:returns: log directory tied to the user, same as `user_cache_dir` if not opinionated else ``log`...
  * `user_runtime_dir` **(Generic / Templated Code)** (Impact: 3.2)
    * *Intent:* """:returns: runtime directory tied to the user, same as `user_cache_dir` if not opinionated else ``...
  * `user_data_dir` **(Type Conversions)** (Impact: 1.6)
    * *Intent:* """:returns: data directory tied to the user, e.g. ``/data/user/<userid>/<packagename>/files/<AppNam...
  * `site_data_dir` **(Generic / Templated Code)** (Impact: 1.6)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 62.886; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (57.7%)
- **Documentation Coverage:** 8.5714% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_site_data_dirs` **(Encapsulated Accessors)** (Impact: 5.9)
  * `_site_config_dirs` **(Encapsulated Accessors)** (Impact: 5.9)
  * `_site_applications_dirs` **(Encapsulated Accessors)** (Impact: 5.9)
  * `site_data_dir` **(Generic / Templated Code)** (Impact: 4.4)
    * *Intent:* """:returns: data directories shared by users, from ``$XDG_DATA_DIRS`` if set, else platform default...
  * `site_config_dir` **(Generic / Templated Code)** (Impact: 4.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 33.837; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (49.8%), Guard Balance (formerly Safety Score) (35.9%), Connectivity (formerly Api Exposure) (9.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_android_active` **(Many-Argument Workhorses)** (Impact: 27.8)
  * `wrap` **(Defensive Guards)** (Impact: 9.3)
  * `mock_import` **(Generic / Templated Code)** (Impact: 7.9)
  * `_fake_import` **(Stateful Encapsulated Methods)** (Impact: 5.4)
  * `test_function_interface_is_in_sync` **(Defensive Guards)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 33.837; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (58.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (47.0%), Connectivity (formerly Api Exposure) (7.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_android` **(Many-Argument Workhorses)** (Impact: 26.1)
  * `test_android_folder_from_jnius` **(Defensive Guards)** (Impact: 6.5)
  * `test_android_ensure_exists_creates_opinion_subdir` **(Type Conversions)** (Impact: 3.1)
  * `test_android_folder_from_p4a` **(Defensive Guards)** (Impact: 2.8)
  * `test_android_folder_from_sys_path` **(Type Conversions)** (Impact: 2.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 33.837; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (75.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (46.4%), Connectivity (formerly Api Exposure) (7.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_compatibility` **(Defensive Guards)** (Impact: 11.5)
    * *Intent:* # Only test functions that are part of appdirs if getattr(appdirs, func, None) is None: pytest.skip(...
  * `test_has_all_functions` **(Generic / Templated Code)** (Impact: 4.3)
    * *Intent:* # Get all public function names from appdirs appdirs_function_names = [f[0] for f in getmembers(appd...
  * `test_has_all_properties` **(Callbacks & Closures)** (Impact: 3.4)
    * *Intent:* # Get names of all the properties of appdirs.AppDirs appdirs_property_names = [p[0] for p in getmemb...
  * `test_has_backward_compatible_class` **(Generic / Templated Code)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 39.59; role: Pure Producer (Foundation)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 62.599; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.1%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (48.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(I/O & Config Routines)** (Impact: 6.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 33.837; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (70.2%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (7.1%), Complexity Load (formerly Cognitive Load) (4.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `func_path` **(Type Conversions)** (Impact: 1.6)
  * `func` **(Type Conversions)** (Impact: 1.5)
  * `props` **(Generic / Templated Code)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 33.837; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (47.9%), Connectivity (formerly Api Exposure) (5.6%), Complexity Load (formerly Cognitive Load) (4.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_run_as_module` **(Defensive Guards)** (Impact: 2.3)
  * `test_props_same_as_test` **(Generic / Templated Code)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
