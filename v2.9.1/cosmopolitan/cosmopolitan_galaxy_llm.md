...
## 1. EXECUTIVE SUMMARY
- **Scope:** 6974 analyzed artifact(s), 296808 LOC.
- **Load-bearing artifact:** `libc/calls/calls.h` -- 609 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `Makefile` -- pulls in 166 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `libc/stdckdint.h` at magnitude 10476.0 (structural weight, not risk).
...
| Analyzed Artifacts (Scanned) | 6974 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 10546 |
| Total LOC | 296808 |
...
|---|---|---|
| Modularity | 0.4519 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.1279 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 1.688 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
| Articulation Pts | 282 | Number of single files that, if removed, shatter the network. |
...
> **Architectural Drift Z-Score:** `6.189`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +6.19; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 34%, Declarative / Non-Code 19%, Parameter Forwarders Files 16%, Compute Cores Files 11%, Many-Argument Workhorses Files 6%
...
*Total Excluded Artifacts: 10546*
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 29.8 | 0.0 | 0.0 |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 6.4 | 0.4 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 33.5 | 0.0 | 0.0 |
...
|---|---|---|---|---|
| memory | 44868 | 2690 | 12 | `tool/net/redbean.c` |
| cleanup | 2017 | 428 | 0 | `test/libc/intrin/mmap_test.c` |
| guards | 17246 | 2269 | 5 | `tool/net/redbean.c` |
| danger | 7610 | 1282 | 2 | `test/posix/access_test.c` |
| concurrency | 1756 | 311 | 0 | `libc/intrin/aarch64/atomics.S` |
| connectivity | 12610 | 3500 | 3 | `tool/net/definitions.lua` |
| io | 3269 | 436 | 0 | `test/libc/stdio/sscanf_test.c` |
| crypto | 0 | 0 | 0 | - |
| ipc | 1118 | 242 | 0 | `test/libc/intrin/mmap_test.c` |
| time | 152 | 66 | 0 | `test/libc/intrin/clock_gettime_test.c` |
| serialization | 10 | 5 | 0 | `test/tool/net/mapshared_test.lua` |
| regex | 325 | 101 | 0 | `test/libc/str/regex_test.c` |
| events | 662 | 204 | 0 | `test/libc/calls/poll_test.c` |
| tests | 18787 | 438 | 0 | `test/libc/stdio/sscanf_test.c` |
| docs | 4088 | 1628 | 1 | `tool/net/definitions.lua` |
| debt | 2160 | 508 | 0 | `ape/apeinstall.sh` |
| mutation | 61123 | 2547 | 19 | `tool/net/redbean.c` |
| dead_code | 3953 | 2496 | 1 | `tool/net/definitions.lua` |
| credential | 41 | 9 | 0 | `tool/net/demo/unix-info.lua` |
| threat | 2933 | 577 | 0 | `libc/integral/c.inc` |
| ml_ai | 1704 | 588 | 0 | `libc/complex.h` |
...
- `__vcscanf` **(Many-Argument Workhorses)** (@ `libc/stdio/vcscanf.c`) -> Impact: **907.7** | LOC: 797
...
- `main` **(Many-Argument Workhorses)** (@ `examples/romanize.c`) -> Impact: **734.2** | LOC: 931
- `__fmt` **(Many-Argument Workhorses)** (@ `libc/stdio/fmt.c`) -> Impact: **713.2** | LOC: 694
...
- `kformat` **(Many-Argument Workhorses)** (@ `libc/intrin/kprintf.greg.c`) -> Impact: **600.4** | LOC: 604
- `getAvailableFeatures` **(Many-Argument Workhorses)** (@ `libc/intrin/x86.c`) -> Impact: **562.3** | LOC: 289
- `Parse` **(Many-Argument Workhorses)** (@ `tool/net/ljson.c`) -> Impact: **518.8** | LOC: 481
- `PrintLambda` **(Many-Argument Workhorses)** (@ `tool/lambda/lib/print.c`) -> Impact: **343.1** | LOC: 298
- `demangle_read_type_impl` **(Many-Argument Workhorses)** (@ `libc/intrin/demangle.c`) -> Impact: **294.6** | LOC: 522
- `cosmo_args` **(Many-Argument Workhorses)** (@ `tool/args/args2.c`) -> Impact: **278.5** | LOC: 444
...
- `PrintDebruijn` **(Many-Argument Workhorses)** (@ `tool/lambda/lib/print.c`) -> Impact: **275.0** | LOC: 267
...
| `libc/intrin` | 476 | 24218.82 | 20.55% | 42.12% |
| `libc/calls` | 414 | 22017.02 | 31.74% | 47.5% |
| `libc/tinymath` | 287 | 17146.48 | 33.68% | 37.77% |
| `libc/sysv/consts` | 1214 | 14407.88 | 0.08% | 0.1% |
| `tool/net` | 31 | 13514.12 | 35.93% | 18.5% |
| `libc` | 35 | 11573.38 | 17.62% | 0.9% |
| `libc/stdio` | 181 | 10416.26 | 17.84% | 46.78% |
| `examples` | 66 | 9775.3 | 39.31% | 0.0% |
| `tool/viz` | 36 | 7383.56 | 51.65% | 37.99% |
...
- `ctl/uninitialized_move_n.h` -> **100.0%** Exposure
- `dsp/audio/cosmoaudio/test.c` -> **100.0%** Exposure
...
- **Unknown Dependencies:** `19177` packages imported that bypass the Zero-Trust whitelist.
...
- **Blast Radius:** changing it is visible to **55** in-repo importer(s); it depends on **2**; blast radius 1.318; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (85.6%), Complexity Load (formerly Cognitive Load) (64.4%), Connectivity (formerly Api Exposure) (55.0%)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.008486
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **123**; blast radius 0.334; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.8%), Complexity Load (formerly Cognitive Load) (91.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.6%)
- **Documentation Coverage:** 99.7151% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `HandleConnection` **(Compute Cores)** (Impact: 97.4)
  * `StoreAsset` **(Many-Argument Workhorses)** (Impact: 83.5)
  * `ServeAsset` **(Many-Argument Workhorses)** (Impact: 66.8)
  * `ConfigureCertificate` **(Many-Argument Workhorses)** (Impact: 58.2)
  * `LuaSetCookie` **(Compute Cores)** (Impact: 57.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8e-06 | `Ripple Effect (Closeness):` 0.00043
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Complexity Load (formerly Cognitive Load) (93.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 91.9679% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `demangle_read_type_impl` **(Many-Argument Workhorses)** (Impact: 294.6)
  * `demangle_read_uqname` **(Compute Cores)** (Impact: 210.7)
    * *Intent:* /* * read unqualified-name, unqualified name are operator-name, ctor-dtor-name, * source-name */
  * `demangle_push_type_qualifier` **(Many-Argument Workhorses)** (Impact: 183.6)
  * `demangle_read_encoding_impl` **(Compute Cores)** (Impact: 130.6)
  * `demangle` **(Many-Argument Workhorses)** (Impact: 106.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **26**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.7%), Complexity Load (formerly Cognitive Load) (95.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 43.75% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__fmt` **(Many-Argument Workhorses)** (Impact: 713.2)
    * *Intent:* * - `%#s` datum (radix 256 null-terminated ibm cp437) * - `%#x` int (radix 16 hexadecimal w/ 0x pref...
  * `__fmt_stoa` **(Many-Argument Workhorses)** (Impact: 230.0)
    * *Intent:* /** * Converts string to array. * * This is used by __fmt() to implement the %s and %c directives. T...
  * `__fmt_ntoa_format` **(Many-Argument Workhorses)** (Impact: 164.0)
  * `__fmt_bround` **(Many-Argument Workhorses)** (Impact: 66.8)
    * *Intent:* // round to prec hex digits after the "." // prec1 = incoming precision (after ".")
  * `__fmt_ntoa2` **(Many-Argument Workhorses)** (Impact: 46.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **75**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.1%), Complexity Load (formerly Cognitive Load) (93.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `HttpWorker` **(Compute Cores)** (Impact: 151.9)
    * *Intent:* // make thousands of http client handler threads // load balance incoming connections for port 8080 ...
  * `Read` **(Many-Argument Workhorses)** (Impact: 69.6)
  * `main` **(Many-Argument Workhorses)** (Impact: 53.7)
  * `SslRead` **(Many-Argument Workhorses)** (Impact: 38.1)
  * `ClaimWorker` **(Compute Cores)** (Impact: 35.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **46**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.7%), Complexity Load (formerly Cognitive Load) (94.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (5.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `PpuAccess` **(Many-Argument Workhorses)** (Impact: 84.0)
    * *Intent:* // External I/O: read or write
  * `RenderingTick` **(I/O & Config Routines)** (Impact: 75.6)
  * `ReadKeyboard` **(I/O & Config Routines)** (Impact: 70.0)
  * `Write` **(Compute Cores)** (Impact: 60.4)
  * `Access` **(Many-Argument Workhorses)** (Impact: 56.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.5%), Complexity Load (formerly Cognitive Load) (89.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 94.0594% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_TtyWrite` **(Many-Argument Workhorses)** (Impact: 114.5)
  * `TtySelectGraphicsRendition` **(Compute Cores)** (Impact: 81.2)
  * `_StartTty` **(Many-Argument Workhorses)** (Impact: 51.5)
  * `TtyCsi` **(Compute Cores)** (Impact: 46.7)
  * `TtySetType` **(Many-Argument Workhorses)** (Impact: 32.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **50**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.4%), Complexity Load (formerly Cognitive Load) (94.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 85.1852% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `kformat` **(Many-Argument Workhorses)** (Impact: 600.4)
  * `kloghandle` **(I/O & Config Routines)** (Impact: 31.9)
    * *Intent:* // returns log handle or -1 if logging shouldn't happen
  * `klog` **(Many-Argument Workhorses)** (Impact: 30.6)
    * *Intent:* #endif /* __x86_64__ */
  * `kemitquote` **(Many-Argument Workhorses)** (Impact: 11.8)
  * `_klog_serial` **(Defensive Guards)** (Impact: 7.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (98.6%), Guard Balance (formerly Safety Score) (98.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 5.8824% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__vcscanf` **(Many-Argument Workhorses)** (Impact: 907.7)
    * *Intent:* * All standard features are supported, except for positional argument * references (e.g. `%n$`) spec...
  * `ConsumeWhitespace` **(Stateful Encapsulated Methods)** (Impact: 11.9)
  * `CharsetAdd` **(C Struct Operations)** (Impact: 11.3)
  * `Buffer` **(Stateful Encapsulated Methods)** (Impact: 11.2)
  * `IsSpace` **(Stateful Encapsulated Methods)** (Impact: 8.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (55.9%), Mutation Surface (formerly State Flux) (33.5%), Connectivity (formerly Api Exposure) (13.7%)
- **Documentation Coverage:** 1.8672% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__global_context__` **(I/O & Config Routines)** (Impact: 247.5)
  * `unix.utimensat` **(State Mutators)** (Impact: 2.9)
    * *Intent:* --- possible to edit the timestamps on the symbolic link itself, --- rather than the file it points ...
  * `Database:apply_changeset` **(State Mutators)** (Impact: 2.7)
  * `OnProcessCreate` **(Parameter Forwarders)** (Impact: 2.5)
    * *Intent:* --- --- If this function is defined it'll be called from the main process --- each time redbean fork...
  * `Barf` **(Parameter Forwarders)** (Impact: 2.5)
    * *Intent:* --- --- --- For example: --- --- assert(Barf('x.txt', 'abc123')) --- assert(Barf('x.txt', 'XX', 0, 0...
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.094; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.0%), Complexity Load (formerly Cognitive Load) (90.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 83.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getAvailableFeatures` **(Many-Argument Workhorses)** (Impact: 562.3)
  * `getIntelProcessorTypeAndSubtype` **(Many-Argument Workhorses)** (Impact: 227.7)
  * `getAMDProcessorTypeAndSubtype` **(Many-Argument Workhorses)** (Impact: 160.9)
  * `__cpu_indicator_init` **(I/O & Config Routines)** (Impact: 11.4)
    * *Intent:* // A constructor function that is sets __cpu_model and __cpu_features2 with // the right values. Thi...
  * `detectX86FamilyModel` **(Stateful Encapsulated Methods)** (Impact: 8.6)
**Contextual Mitigations & Amplifications:**
...
- `tool/net/redbean.c` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `libc/runtime/runtime.h` -> **Severity: 4.777** (Embedded: 0.085 * Error Risk: 56.1752%)
- `libc/calls/calls.h` -> **Severity: 4.581** (Embedded: 0.0894 * Error Risk: 51.2193%)
- `libc/thread/thread.h` -> **Severity: 4.547** (Embedded: 0.0539 * Error Risk: 84.3989%)
- `libc/stdio/stdio.h` -> **Severity: 4.155** (Embedded: 0.0466 * Error Risk: 89.0903%)
- `libc/calls/struct/sigset.h` -> **Severity: 2.939** (Embedded: 0.039 * Error Risk: 75.2927%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `libc/runtime/runtime.h` -> **Severity: 905.9** (Blast Radius: 9.059 * Doc Risk: 100.0%)
- `libc/testlib/testlib.h` -> **Severity: 764.4** (Blast Radius: 7.644 * Doc Risk: 100.0%)
...
- `libc/tinymath/internal.h` -> **Severity: 218.7** (Blast Radius: 2.187 * Doc Risk: 100.0%)
