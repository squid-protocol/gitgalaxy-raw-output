...
## 1. EXECUTIVE SUMMARY
- **Scope:** 5 analyzed artifact(s), 166 LOC.
...
- **Top orchestrator:** `src/game.rs` -- pulls in 9 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/game.rs` at magnitude 52.68 (structural weight, not risk).
...
> **File Composition:** Data / Markup / Trivial 40%, Compute Cores Files 20%, Large Core Modules (3) 20%, Type Conversions Files 20%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 62.0 | 73.9 | 54.1 |
...
- `main` **(Compute Cores)** (@ `build.rs`) -> Impact: **15.8** | LOC: 36
  * *Intent:* // Example custom build script.
- `DG_GetKey` **(Defensive Guards)** (@ `src/game.rs`) -> Impact: **13.0** | LOC: 17
- `from_char` **(Type Conversions)** (@ `src/input.rs`) -> Impact: **4.6** | LOC: 7
- `DG_DrawFrame` **(Defensive Guards)** (@ `src/game.rs`) -> Impact: **3.4** | LOC: 8
- `DG_SetWindowTitle` **(Generic / Templated Code)** (@ `src/game.rs`) -> Impact: **3.3** | LOC: 9
- `draw_frame` **(Parameter Forwarders)** (@ `src/game.rs`) -> Impact: **2.3** | LOC: 1
- `init` **(Generic / Templated Code)** (@ `src/game.rs`) -> Impact: **1.9** | LOC: 9
- `set_window_title` **(Parameter Forwarders)** (@ `src/game.rs`) -> Impact: **1.8** | LOC: 1
- `DG_SleepMs` **(Type Conversions)** (@ `src/game.rs`) -> Impact: **1.6** | LOC: 4
- `get_key` **(Generic / Templated Code)** (@ `src/game.rs`) -> Impact: **1.5** | LOC: 1
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 200.0; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Guard Balance (formerly Safety Score) (57.9%), Mutation Surface (formerly State Flux) (25.9%), Concurrency Surface (formerly Concurrency) (21.4%)
- **Documentation Coverage:** 93.75% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `DG_GetKey` **(Defensive Guards)** (Impact: 13.0)
  * `DG_DrawFrame` **(Defensive Guards)** (Impact: 3.4)
  * `DG_SetWindowTitle` **(Generic / Templated Code)** (Impact: 3.3)
  * `draw_frame` **(Parameter Forwarders)** (Impact: 2.3)
  * `init` **(Generic / Templated Code)** (Impact: 1.9)
...
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (37.8%), Connectivity (formerly Api Exposure) (16.4%), Complexity Load (formerly Cognitive Load) (5.8%), Test Surface (formerly Verification) (2.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `from_char` **(Type Conversions)** (Impact: 4.6)
**Structural Signatures (Net Mitigated Signals):**
...
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (91.7%), Guard Balance (formerly Safety Score) (64.6%), Debt Markers (formerly Tech Debt) (37.8%), Complexity Load (formerly Cognitive Load) (26.9%)
- **Documentation Coverage:** 54.076% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(Compute Cores)** (Impact: 15.8)
    * *Intent:* // Example custom build script.
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
