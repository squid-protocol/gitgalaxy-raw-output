...
## 1. EXECUTIVE SUMMARY
- **Scope:** 4794 analyzed artifact(s), 583910 LOC.
- **Load-bearing artifact:** `packages/dev/core/src/scene.ts` -- 713 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `packages/tools/nodeEditor/src/blockTools.ts` -- pulls in 117 dependencies, the widest assembly point in the scan.
...
| Analyzed Artifacts (Scanned) | 4794 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 2125 |
| Total LOC | 583910 |
...
> **Architectural Drift Z-Score:** `4.094`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +4.09; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 27%, Large Core Modules (3) 20%, Declarative / Non-Code 12%, State Mutators Files 9%, Interface Declarations Files 7%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 45.9 | 58.9 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 3.2 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 16.0 | 2.3 | 0.0 |
...
| Mutation Surface (formerly State Flux Exposure) | 0.0 | 100.0 | 45.0 | 19.6 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 30.2 | 0.0 | 0.0 |
...
| threat | 2630 | 312 | 0 | `packages/tools/babylonServer/public/gltf_validator.js` |
...
- `attachControl` **(Many-Argument Workhorses)** (@ `packages/dev/core/src/Inputs/scene.inputManager.ts`) -> Impact: **434.3** | LOC: 546
  * *Intent:* /** * Attach events to the canvas (To handle actionManagers triggers and raise onPointerMove, onPointerDown and onPointerUp */
...
- `isReadyForSubMesh` **(Many-Argument Workhorses)** (@ `packages/dev/core/src/Materials/standardMaterial.ts`) -> Impact: **299.6** | LOC: 593
  * *Intent:* /** * Get if the submesh is ready to be used and all its information available. * Child classes can use it to update shaders */
...
- **Blast Radius:** changing it is visible to **252** in-repo importer(s); it depends on **43**; blast radius 4.529; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 5.277% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Parse` **(Many-Argument Workhorses)** (Impact: 249.1)
    * *Intent:* /** * Returns a new Mesh object parsed from the source provided. */
  ...
  * `_updateInstancedBuffers` **(Many-Argument Workhorses)** (Impact: 148.9)
    * *Intent:* /** * This method will also draw the instances if fillMode and effect are passed */
...
- **Blast Radius:** changing it is visible to **55** in-repo importer(s); it depends on **40**; blast radius 1.669; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.9%), Connectivity (formerly Api Exposure) (78.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (58.0%)
- **Documentation Coverage:** 14.5658% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_getRGBABufferInternalSizedFormat` **(Many-Argument Workhorses)** (Impact: 205.1)
    * *Intent:* /** */
  * `_createInternalTexture` **(Many-Argument Workhorses)** (Impact: 144.5)
    * *Intent:* /** * Creates an internal texture without binding it to a framebuffer */
  * `constructor` **(Many-Argument Workhorses)** (Impact: 114.8)
    * *Intent:* /** * Creates a new engine */
  * `createEffect` **(Many-Argument Workhorses)** (Impact: 113.7)
    * *Intent:* * Create a new effect (used to store vertex/fragment shaders) */
  * `_initGLContext` **(I/O & Config Routines)** (Impact: 112.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **656** in-repo importer(s); it depends on **13**; blast radius 20.098; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (84.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 6.2128% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `equals` **(Compute Cores)** (Impact: 32.8)
    * *Intent:* /** * Check equality between this matrix and a second one */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **713** in-repo importer(s); it depends on **83**; blast radius 19.582; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (84.8%), Concurrency Surface (formerly Concurrency) (65.7%)
- **Documentation Coverage:** 5.6801% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** changing it is visible to **33** in-repo importer(s); it depends on **83**; blast radius 0.637; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.4%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (77.6%)
- **Documentation Coverage:** 14.0244% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **48**; blast radius 0.09; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (87.8%), Test Surface (formerly Verification) (80.0%)
...
**Top Internal Functions/Classes:**
  ...
  * `_loadAnimationChannelAsync` **(Many-Argument Workhorses)** (Impact: 57.2)
    * *Intent:* /** * Loads a glTF animation channel. */
...
- **Blast Radius:** changing it is visible to **69** in-repo importer(s); it depends on **13**; blast radius 1.277; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Connectivity (formerly Api Exposure) (86.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 10.989% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_mergeCoroutine` **(Many-Argument Workhorses)** (Impact: 440.4)
    * *Intent:* /** */
  * `ComputeNormals` **(Many-Argument Workhorses)** (Impact: 119.6)
    * *Intent:* * * facetNormals : optional array of facet normals (vector3) * * facetPositions : optional array of ...
...
- **Blast Radius:** changing it is visible to **189** in-repo importer(s); it depends on **52**; blast radius 7.803; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (98.5%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 6.3889% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **34**; blast radius 0.084; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (76.3%)
- **Documentation Coverage:** 57.4074% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **41**; blast radius 0.292; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.8%), Guard Balance (formerly Safety Score) (81.3%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (72.4%)
- **Documentation Coverage:** 56.3953% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `_setTexture` **(Many-Argument Workhorses)** (Impact: 45.4)
    * *Intent:* // TODO: Refactor to share more logic with base Engine implementation. /** */
...
- **Blast Radius:** changing it is visible to **54** in-repo importer(s); it depends on **26**; blast radius 0.843; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.0%), Connectivity (formerly Api Exposure) (85.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 20.8845% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_processObservables` **(Many-Argument Workhorses)** (Impact: 62.8)
    * *Intent:* /** */
  * `_computeAlignment` **(Many-Argument Workhorses)** (Impact: 42.1)
    * *Intent:* /** */
...
- **Blast Radius:** changing it is visible to **261** in-repo importer(s); it depends on **42**; blast radius 5.976; role: Pure Producer (Foundation)
...
- **Documentation Coverage:** 5.3521% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `dispose` **(Many-Argument Workhorses)** (Impact: 66.0)
    * *Intent:* /** * Releases resources associated with this abstract mesh. */
  ...
  * `getClosestFacetAtLocalCoordinates` **(Many-Argument Workhorses)** (Impact: 37.2)
    * *Intent:* /** * Returns the closest mesh facet index at (x,y,z) local coordinates, null if not found */
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
- **Blast Radius:** changing it is visible to **31** in-repo importer(s); it depends on **45**; blast radius 0.409; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 10.1695% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `bindForSubMesh` **(Many-Argument Workhorses)** (Impact: 231.6)
    * *Intent:* /** * Binds the submesh data. */
  * `_prepareDefines` **(Many-Argument Workhorses)** (Impact: 221.9)
  * `isReadyForSubMesh` **(Many-Argument Workhorses)** (Impact: 174.3)
    * *Intent:* /** * Specifies that the submesh is ready to be used. */
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **23**; blast radius 0.116; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.4%), Test Surface (formerly Verification) (80.0%), Concurrency Surface (formerly Concurrency) (62.2%)
- **Documentation Coverage:** 24.7863% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `_GetCompressedChunks` **(Many-Argument Workhorses)** (Impact: 49.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 6 instances
...
- **Blast Radius:** changing it is visible to **24** in-repo importer(s); it depends on **35**; blast radius 0.585; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.4%), Test Surface (formerly Verification) (80.0%), Concurrency Surface (formerly Concurrency) (71.2%)
- **Documentation Coverage:** 8.2418% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `_render` **(Defensive Guards)** (Impact: 61.7)
  ...
  * `fillDefines` **(Many-Argument Workhorses)** (Impact: 40.5)
    * *Intent:* /** * Fill the defines array according to the current settings of the particle system */
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 9 instances
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **39**; blast radius 0.129; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.7%), Concurrency Surface (formerly Concurrency) (96.7%), Complexity Load (formerly Cognitive Load) (94.1%)
...
**Top Internal Functions/Classes:**
  ...
  * `targetLookup` **(Many-Argument Workhorses)** (Impact: 100.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **23**; blast radius 0.169; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (71.6%), Connectivity (formerly Api Exposure) (52.9%)
- **Documentation Coverage:** 11.6379% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `initConstraint` **(Many-Argument Workhorses)** (Impact: 126.2)
    * *Intent:* // constraint /** * Initializes a physics constraint with the given parameters. * * * This function ...
  * `initShape` **(Many-Argument Workhorses)** (Impact: 77.3)
    * *Intent:* /** * Initializes a physics shape with the given type and parameters. * * This code is useful for in...
  ...
  * `setPhysicsBodyTransformation` **(Many-Argument Workhorses)** (Impact: 34.4)
    * *Intent:* /** * Sets the transformation of the given physics body to the given transform node. * Sets the tran...
  * `_notifyCollisions` **(Compute Cores)** (Impact: 31.5)
    * *Intent:* /** * Runs thru all detected collisions and filter by body */
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **21**; blast radius 0.11; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.8%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (52.1%)
- **Documentation Coverage:** 3.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `digest` **(Many-Argument Workhorses)** (Impact: 63.9)
    * *Intent:* * Thus the particles generated from `digest()` have their property `position` set yet. * {delta} (op...
...
- **Blast Radius:** changing it is visible to **23** in-repo importer(s); it depends on **44**; blast radius 0.302; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (70.6%)
- **Documentation Coverage:** 14.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `bindForSubMesh` **(Many-Argument Workhorses)** (Impact: 109.9)
    * *Intent:* /** * Binds the submesh data. */
  * `constructor` **(Many-Argument Workhorses)** (Impact: 61.7)
    * *Intent:* /** * Instantiates a new OpenPBRMaterial instance. * */
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** changing it is visible to **15** in-repo importer(s); it depends on **38**; blast radius 0.345; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.4%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (56.0%)
- **Documentation Coverage:** 9.5506% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `Parse` **(Many-Argument Workhorses)** (Impact: 82.9)
    * *Intent:* /** * Parses a serialized ShadowGenerator and returns a new ShadowGenerator. */
  ...
  * `_initializeShadowMap` **(I/O & Config Routines)** (Impact: 43.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** changing it is visible to **19** in-repo importer(s); it depends on **39**; blast radius 0.22; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.7%), Test Surface (formerly Verification) (80.0%), Concurrency Surface (formerly Concurrency) (58.4%)
- **Documentation Coverage:** 11.3636% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `render` **(Many-Argument Workhorses)** (Impact: 69.5)
    * *Intent:* /** * Renders the particle system in its current state */
  * `_initialize` **(I/O & Config Routines)** (Impact: 55.0)
...
- **Blast Radius:** changing it is visible to **40** in-repo importer(s); it depends on **24**; blast radius 0.529; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (82.9%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (77.9%)
- **Documentation Coverage:** 31.3559% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `ImportAnimations` **(Many-Argument Workhorses)** (Impact: 56.8)
    * *Intent:* * Import animations from a file into a scene */
...
- **Blast Radius:** changing it is visible to **57** in-repo importer(s); it depends on **25**; blast radius 0.571; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.4%), Connectivity (formerly Api Exposure) (84.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 1.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `PrepareDefinesForIBL` **(Many-Argument Workhorses)** (Impact: 88.4)
    * *Intent:* /** * Prepare defines relating to IBL logic. */
  * `PrepareDefinesAndAttributesForMorphTargets` **(Many-Argument Workhorses)** (Impact: 76.6)
    * *Intent:* /** * Prepares the list of attributes and defines required for morph targets. */
  * `BindIBLParameters` **(Many-Argument Workhorses)** (Impact: 72.2)
    * *Intent:* * Update parameters for IBL */
  * `PrepareDefinesForLight` **(Many-Argument Workhorses)** (Impact: 61.2)
    * *Intent:* * Prepares the defines related to the light information passed in parameter */
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **16**; blast radius 0.12; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.3%), Complexity Load (formerly Cognitive Load) (91.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 71.831% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `pointerUpAnimation` **(I/O & Config Routines)** (Impact: 57.0)
  ...
  * `updateSwatches` **(Many-Argument Workhorses)** (Impact: 35.9)
...
- `packages/dev/core/src/Misc/observable.ts` -> **Severity: 26.595** (Embedded: 0.2763 * Error Risk: 96.2629%)
- `packages/dev/core/src/scene.ts` -> **Severity: 21.832** (Embedded: 0.2575 * Error Risk: 84.7728%)
- `packages/dev/core/src/Maths/math.vector.ts` -> **Severity: 21.735** (Embedded: 0.2577 * Error Risk: 84.3312%)
- `packages/dev/core/src/Misc/logger.ts` -> **Severity: 19.777** (Embedded: 0.2267 * Error Risk: 87.2386%)
