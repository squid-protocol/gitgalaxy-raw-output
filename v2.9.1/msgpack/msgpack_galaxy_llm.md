...
## 1. EXECUTIVE SUMMARY
- **Scope:** 18 analyzed artifact(s), 2854 LOC.
- **Load-bearing artifact:** `msgpack-1.1.2/msgpack/ext.py` -- 4 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `msgpack-1.1.2/msgpack/fallback.py` -- pulls in 8 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `msgpack-1.1.2/msgpack/fallback.py` at magnitude 967.66 (structural weight, not risk).
...
| Avg Path Length | 1.1176 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **File Composition:** Data / Markup / Trivial 22%, Declarative / Non-Code 22%, Parameter Forwarders Files 22%, Large Core Modules (3) 17%, Encapsulated Accessors Files 11%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 55.0 | 69.2 | 100.0 |
...
- `unpack_execute` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/unpack_template.h`) -> Impact: **270.2** | LOC: 309
- `_pack` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/fallback.py`) -> Impact: **128.6** | LOC: 123
- `_pack_inner` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/_packer.pyx`) -> Impact: **123.8** | LOC: 105
  * *Intent:* # returns -2 when default should(o) be called
- `__init__` **(Stateful Encapsulated Methods)** (@ `msgpack-1.1.2/msgpack/fallback.py`) -> Impact: **109.2** | LOC: 91
- `_unpack` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/fallback.py`) -> Impact: **66.0** | LOC: 73
- `__init__` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/_unpacker.pyx`) -> Impact: **64.0** | LOC: 59
- `init_ctx` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/_unpacker.pyx`) -> Impact: **62.8** | LOC: 55
- `unpack_container_header` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/unpack_container_header.h`) -> Impact: **56.2** | LOC: 50
- `unpack_callback_ext` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/unpack.h`) -> Impact: **55.9** | LOC: 90
  * *Intent:* #include "datetime.h"
- `unpackb` **(Many-Argument Workhorses)** (@ `msgpack-1.1.2/msgpack/_unpacker.pyx`) -> Impact: **51.4** | LOC: 68
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **8**; blast radius 47.173; role: Transceiver (Middle-Tier)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 38.906; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.9%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (70.5%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 64.0)
  * `init_ctx` **(Many-Argument Workhorses)** (Impact: 62.8)
  * `unpackb` **(Many-Argument Workhorses)** (Impact: 51.4)
  * `_unpack` **(Many-Argument Workhorses)** (Impact: 25.6)
  * `append_buffer` **(Many-Argument Workhorses)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 55.441; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.3%), Complexity Load (formerly Cognitive Load) (84.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `unpack_execute` **(Many-Argument Workhorses)** (Impact: 270.2)
  * `unpack_construct` **(Encapsulated Accessors)** (Impact: 2.4)
    * *Intent:* #undef NEXT_CS #undef SWITCH_RANGE_BEGIN #undef SWITCH_RANGE #undef SWITCH_RANGE_DEFAULT #undef SWIT...
  * `unpack_skip` **(Encapsulated Accessors)** (Impact: 2.4)
  * `unpack_init` **(Stateful Encapsulated Methods)** (Impact: 2.0)
  * `unpack_data` **(Encapsulated Accessors)** (Impact: 1.6)
    * *Intent:* */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 38.906; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (99.4%), Guard Balance (formerly Safety Score) (93.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 69.2308% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_pack_inner` **(Many-Argument Workhorses)** (Impact: 123.8)
    * *Intent:* # returns -2 when default should(o) be called
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 16.8)
  * `_pack` **(Compute Cores)** (Impact: 10.6)
  * `pack_map_pairs` **(Compute Cores)** (Impact: 7.9)
    * *Intent:* """ Pack *pairs* as msgpack map type. *pairs* should be a sequence of pairs. (`len(pairs)` and `for ...
  * `pack` **(Defensive Guards)** (Impact: 5.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 55.441; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.1%), Test Surface (formerly Verification) (80.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `msgpack_pack_ext` **(Many-Argument Workhorses)** (Impact: 32.6)
    * *Intent:* /* * Ext */
  * `msgpack_pack_short` **(Compute Cores)** (Impact: 20.6)
  * `msgpack_pack_int` **(Compute Cores)** (Impact: 20.6)
  * `msgpack_pack_long` **(Compute Cores)** (Impact: 16.8)
  * `msgpack_pack_raw` **(Type Conversions)** (Impact: 14.8)
    * *Intent:* /* * Raw */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 38.906; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (97.9%), Guard Balance (formerly Safety Score) (86.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `unpack_callback_ext` **(Many-Argument Workhorses)** (Impact: 55.9)
    * *Intent:* #include "datetime.h"
  * `unpack_callback_map_item` **(Stateful Encapsulated Methods)** (Impact: 25.7)
  * `unpack_callback_raw` **(Stateful Encapsulated Methods)** (Impact: 13.2)
  * `unpack_timestamp` **(Stateful Encapsulated Methods)** (Impact: 13.2)
    * *Intent:* /* * Unpack ext buffer to a timestamp. Pulled from msgpack-c timestamp.h. */
  * `unpack_callback_map` **(Stateful Encapsulated Methods)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **3**; blast radius 116.827; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.0%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (71.2%)
- **Documentation Coverage:** 20.6897% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `from_bytes` **(Compute Cores)** (Impact: 9.7)
    * *Intent:* """Unpack bytes into a `Timestamp` object. Used for pure-Python msgpack unpacking. :param b: Payload...
  * `__init__` **(Defensive Guards)** (Impact: 9.1)
    * *Intent:* """Initialize a Timestamp object. :param int seconds: Number of seconds since the UNIX epoch (00:00:...
  * `__new__` **(Defensive Guards)** (Impact: 8.4)
  * `to_bytes` **(Compute Cores)** (Impact: 8.1)
    * *Intent:* """Pack this Timestamp object into bytes. Used for pure-Python msgpack packing. :returns data: Paylo...
  * `__eq__` **(Compute Cores)** (Impact: 5.4)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 86.03; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (81.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (71.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `unpack_container_header` **(Many-Argument Workhorses)** (Impact: 56.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 38.906; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (39.2%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 38.906; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (75.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (45.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `msgpack_pack_write` **(Stateful Encapsulated Methods)** (Impact: 7.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 38.906; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (77.5%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (14.2%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `pack` **(Parameter Forwarders)** (Impact: 2.4)
    * *Intent:* """ Pack object `o` and write it to `stream` See :class:`Packer` for options. """
  * `unpack` **(Parameter Forwarders)** (Impact: 2.2)
    * *Intent:* """ Unpack an object from `stream`. Raises `ExtraData` when `stream` contains extra bytes. See :clas...
  * `packb` **(Parameter Forwarders)** (Impact: 2.1)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **5**; blast radius 55.441; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (34.5%), Test Surface (formerly Verification) (2.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); blast radius 83.757; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (91.7%), Guard Balance (formerly Safety Score) (78.6%), Connectivity (formerly Api Exposure) (71.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Parameter Forwarders)** (Impact: 2.1)
  * `__str__` **(Parameter Forwarders)** (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **5**; blast radius 102.565; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (8.3%), Test Surface (formerly Verification) (2.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 47.173; role: Pure Producer (Foundation)
