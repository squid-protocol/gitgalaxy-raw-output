...
## 1. EXECUTIVE SUMMARY
- **Scope:** 331 analyzed artifact(s), 24622 LOC.
- **Load-bearing artifact:** `poetry_core-2.3.2/src/poetry/core/packages/utils/utils.py` -- 22 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `poetry_core-2.3.2/src/poetry/core/masonry/builders/wheel.py` -- pulls in 30 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `poetry_core-2.3.2/src/poetry/core/version/markers.py` at magnitude 1498.64 (structural weight, not risk).
...
| Avg Path Length | 2.7085 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `4.603`
> **Composition Archetype:** `Small Flat Repo (2)` (z +4.60; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 66%, Generic / Templated Code Files 18%, Large Core Modules (2) 7%, Declarative / Non-Code 2%, Large Core Modules (3) 2%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 34.7 | 0.0 | 0.0 |
...
- `_configure_package_metadata` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/factory.py`) -> Impact: **120.2** | LOC: 151
- `create_nested_marker` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/packages/utils/utils.py`) -> Impact: **85.1** | LOC: 109
- `_validate_legacy_vs_project` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/factory.py`) -> Impact: **84.4** | LOC: 128
- `create_from_pep_508` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/packages/dependency.py`) -> Impact: **76.3** | LOC: 140
- `_merge_single_markers` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/version/markers.py`) -> Impact: **75.0** | LOC: 99
- `__init__` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/version/markers.py`) -> Impact: **71.1** | LOC: 81
- `parse_single_constraint` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/constraints/version/parser.py`) -> Impact: **69.2** | LOC: 143
- `_validate_strict` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/factory.py`) -> Impact: **59.9** | LOC: 78
- `union` **(Defensive Guards)** (@ `poetry_core-2.3.2/src/poetry/core/constraints/generic/union_constraint.py`) -> Impact: **59.3** | LOC: 77
- `create_dependency` **(Many-Argument Workhorses)** (@ `poetry_core-2.3.2/src/poetry/core/factory.py`) -> Impact: **56.2** | LOC: 145
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **18**; blast radius 51.557; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (98.4%), Complexity Load (formerly Cognitive Load) (95.8%), Guard Balance (formerly Safety Score) (95.7%)
- **Documentation Coverage:** 93.9698% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_merge_single_markers` **(Many-Argument Workhorses)** (Impact: 75.0)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 71.1)
  * `intersect_simplify` **(Defensive Guards)** (Impact: 39.5)
    * *Intent:* """ Finds a couple of easy simplifications for intersection on MarkerUnions: - intersection with any...
  * `union_simplify` **(Defensive Guards)** (Impact: 39.4)
    * *Intent:* """ Finds a couple of easy simplifications for union on MultiMarkers: - union with any marker that a...
  * `_compact_markers` **(Many-Argument Workhorses)** (Impact: 38.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003873 | `Ripple Effect (Closeness):` 0.074242
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **25**; blast radius 4.837; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (78.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_configure_package_metadata` **(Many-Argument Workhorses)** (Impact: 120.2)
  * `_validate_legacy_vs_project` **(Many-Argument Workhorses)** (Impact: 84.4)
  * `_validate_strict` **(Many-Argument Workhorses)** (Impact: 59.9)
  * `create_dependency` **(Many-Argument Workhorses)** (Impact: 56.2)
  * `_configure_package_dependencies` **(Many-Argument Workhorses)** (Impact: 52.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001681 | `Ripple Effect (Closeness):` 0.024242
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.784; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (53.8%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (22.6%), Connectivity (formerly Api Exposure) (11.3%)
- **Documentation Coverage:** 93.1818% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_complex_intersection_with_itertools_product_duplicates` **(Compute Cores)** (Impact: 42.8)
    * *Intent:* """ Real-world example from https://github.com/python-poetry/poetry/issues/10250. (Only occurs if th...
  * `test_complex_union_is_deterministic` **(Compute Cores)** (Impact: 33.4)
    * *Intent:* """ This test might fail sporadically if marker operations are not deterministic! """
  * `test_intersection_avoids_combinatorial_explosion` **(Defensive Guards)** (Impact: 26.5)
    * *Intent:* """ combinatorial explosion without AtomicMultiMarker and AtomicMarkerUnion based gevent constraint ...
  * `test_merging_python_version_and_python_full_version` **(Many-Argument Workhorses)** (Impact: 19.4)
  * `test_complex_union` **(I/O & Config Routines)** (Impact: 18.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 1.784; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (42.3%), Complexity Load (formerly Cognitive Load) (22.6%), Connectivity (formerly Api Exposure) (10.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_create_poetry` **(Defensive Guards)** (Impact: 46.8)
  * `test_create_poetry_with_license_type` **(Many-Argument Workhorses)** (Impact: 41.1)
  * `complete_legacy_duplicate_warnings` **(I/O & Config Routines)** (Impact: 25.8)
  * `test_create_poetry_with_nested_dependency_groups` **(Many-Argument Workhorses)** (Impact: 16.9)
  * `test_validate_with_license_type` **(Many-Argument Workhorses)** (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **18** in-repo importer(s); it depends on **22**; blast radius 24.301; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.9%), Complexity Load (formerly Cognitive Load) (93.2%), Connectivity (formerly Api Exposure) (55.5%)
- **Documentation Coverage:** 91.3043% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_from_pep_508` **(Many-Argument Workhorses)** (Impact: 76.3)
  * `to_pep_508` **(Many-Argument Workhorses)** (Impact: 36.2)
  * `marker` **(Stateful Encapsulated Methods)** (Impact: 31.5)
  * `_make_file_or_dir_dep` **(Many-Argument Workhorses)** (Impact: 20.4)
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 13.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003962 | `Ripple Effect (Closeness):` 0.067366
...
- **Blast Radius:** changing it is visible to **22** in-repo importer(s); it depends on **16**; blast radius 48.9; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.1%), Complexity Load (formerly Cognitive Load) (68.6%), Connectivity (formerly Api Exposure) (65.1%)
- **Documentation Coverage:** 57.1429% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_nested_marker` **(Many-Argument Workhorses)** (Impact: 85.1)
  * `normalize_python_version_markers` **(Compute Cores)** (Impact: 40.4)
  * `convert_markers` **(Defensive Guards)** (Impact: 26.3)
  * `url_to_path` **(Compute Cores)** (Impact: 20.4)
    * *Intent:* """ Convert an RFC8089 file URI to path. The logic used here is borrowed from pip https://github.com...
  * `add_constraint` **(Generic / Templated Code)** (Impact: 8.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 85 instances
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **22**; blast radius 3.874; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (76.9%)
- **Documentation Coverage:** 88.764% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `all_classifiers` **(Compute Cores)** (Impact: 21.0)
  * `to_dependency` **(Defensive Guards)** (Impact: 16.2)
  * `with_dependency_groups` **(Many-Argument Workhorses)** (Impact: 13.1)
  * `__repr__` **(Compute Cores)** (Impact: 12.7)
  * `full_pretty_version` **(Compute Cores)** (Impact: 12.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **9**; blast radius 7.849; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (94.7%), Guard Balance (formerly Safety Score) (80.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `difference` **(Many-Argument Workhorses)** (Impact: 53.1)
  * `intersect` **(Defensive Guards)** (Impact: 36.2)
  * `allows` **(Compute Cores)** (Impact: 33.5)
  * `union` **(Compute Cores)** (Impact: 32.0)
  * `allows_any` **(Defensive Guards)** (Impact: 18.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **22**; blast radius 3.358; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (74.0%)
- **Documentation Coverage:** 90.4762% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `find_packages` **(Many-Argument Workhorses)** (Impact: 43.6)
  * `build_setup` **(Compute Cores)** (Impact: 43.3)
  * `convert_dependencies` **(Many-Argument Workhorses)** (Impact: 34.8)
  * `build` **(Many-Argument Workhorses)** (Impact: 14.5)
  * `find_files_to_add` **(Generic / Templated Code)** (Impact: 8.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **20**; blast radius 2.397; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (61.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (43.9%), Complexity Load (formerly Cognitive Load) (27.0%)
- **Documentation Coverage:** 94.8718% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_package_target_dir` **(Defensive Guards)** (Impact: 9.4)
  * `test_make_setup` **(Defensive Guards)** (Impact: 8.7)
  * `test_convert_dependencies` **(I/O & Config Routines)** (Impact: 8.2)
  * `test_sdist_members_mtime_default` **(Defensive Guards)** (Impact: 8.0)
  * `test_default_with_excluded_data` **(Defensive Guards)** (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6.3e-05 | `Ripple Effect (Closeness):` 0.00404
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **12**; blast radius 3.258; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (74.9%)
- **Documentation Coverage:** 92.1053% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `find_files_to_add` **(Many-Argument Workhorses)** (Impact: 40.4)
    * *Intent:* """ Finds all files to add to the tarball """
  * `find_excluded_files` **(Stateful Encapsulated Methods)** (Impact: 29.5)
  * `get_metadata_content` **(Compute Cores)** (Impact: 25.8)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 13.4)
  * `is_excluded` **(Compute Cores)** (Impact: 12.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000258 | `Ripple Effect (Closeness):` 0.013499
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **30**; blast radius 3.281; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (59.1%)
- **Documentation Coverage:** 87.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_build` **(Stateful Encapsulated Methods)** (Impact: 19.7)
  * `build` **(Many-Argument Workhorses)** (Impact: 18.1)
  * `_add_pth` **(Stateful Encapsulated Methods)** (Impact: 11.2)
  * `_add_file` **(Stateful Encapsulated Methods)** (Impact: 10.6)
  * `tag` **(Compute Cores)** (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000427 | `Ripple Effect (Closeness):` 0.009091
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **9**; blast radius 6.712; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (95.3%), Guard Balance (formerly Safety Score) (91.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 97.7778% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `difference` **(Defensive Guards)** (Impact: 40.5)
  * `of` **(Defensive Guards)** (Impact: 28.3)
  * `excludes_single_wildcard_range` **(Compute Cores)** (Impact: 15.1)
  * `intersect` **(Compute Cores)** (Impact: 11.4)
  * `allows_any` **(Compute Cores)** (Impact: 11.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 38 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 1.784; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (21.9%), Guard Balance (formerly Safety Score) (12.0%), Connectivity (formerly Api Exposure) (11.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_package_satisfies_on_repositories` **(Defensive Guards)** (Impact: 10.8)
  * `test_package_authors_valid` **(Defensive Guards)** (Impact: 5.5)
  * `test_all_classifiers_no_license_classifiers_if_spdx` **(Defensive Guards)** (Impact: 4.5)
  * `test_all_classifiers_with_license_classifiers_if_no_spdx` **(Defensive Guards)** (Impact: 3.4)
  * `test_package_url_groups_optional` **(Generic / Templated Code)** (Impact: 2.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **7**; blast radius 5.422; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (94.5%), Guard Balance (formerly Safety Score) (88.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `union` **(Defensive Guards)** (Impact: 59.3)
  * `intersect` **(Defensive Guards)** (Impact: 45.0)
  * `invert` **(Defensive Guards)** (Impact: 10.4)
  * `allows_any` **(Defensive Guards)** (Impact: 9.1)
  * `allows_all` **(Defensive Guards)** (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **11**; blast radius 1.973; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.9%), Complexity Load (formerly Cognitive Load) (80.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_single_constraint` **(Many-Argument Workhorses)** (Impact: 69.2)
  * `_parse_constraint` **(Many-Argument Workhorses)** (Impact: 24.6)
  * `_make_x_constraint_range` **(Stateful Encapsulated Methods)** (Impact: 19.5)
  * `parse_marker_version_constraint` **(Generic / Templated Code)** (Impact: 2.0)
  * `parse_constraint` **(Generic / Templated Code)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **8**; blast radius 4.444; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.1%), Complexity Load (formerly Cognitive Load) (86.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dependencies_for_locking` **(Compute Cores)** (Impact: 23.3)
  * `_enrich_dependency` **(Stateful Encapsulated Methods)** (Impact: 21.8)
  * `dependencies` **(Compute Cores)** (Impact: 18.1)
  * `remove_dependency` **(Stateful Encapsulated Methods)** (Impact: 9.6)
  * `_resolve_included_dependency_groups` **(Stateful Encapsulated Methods)** (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4.5e-05 | `Ripple Effect (Closeness):` 0.046707
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **15**; blast radius 2.163; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (63.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (20.8%), Guard Balance (formerly Safety Score) (11.5%)
- **Documentation Coverage:** 81.6901% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_wheel_includes_licenses_in_correct_paths` **(Defensive Guards)** (Impact: 10.9)
  * `test_wheel_package_target_dir` **(Defensive Guards)** (Impact: 9.5)
  * `test_tag` **(Defensive Guards)** (Impact: 9.5)
    * *Intent:* """Tests that tag returns a valid tag if a build script is used, no matter if poetry-core lives insi...
  * `test_dist_info_date_time_default_value` **(Defensive Guards)** (Impact: 6.4)
  * `clear_samples_build` **(Type Conversions)** (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **8**; blast radius 7.887; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (82.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `next_prerelease` **(Defensive Guards)** (Impact: 18.2)
  * `to_string` **(Defensive Guards)** (Impact: 9.5)
  * `next_postrelease` **(Defensive Guards)** (Impact: 7.7)
  * `next_devrelease` **(Defensive Guards)** (Impact: 6.7)
  * `__post_init__` **(Defensive Guards)** (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **7**; blast radius 8.324; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.0%), Complexity Load (formerly Cognitive Load) (81.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `normalize_url` **(Compute Cores)** (Impact: 24.0)
  * `url` **(Compute Cores)** (Impact: 11.6)
  * `get_ignored_files` **(Generic / Templated Code)** (Impact: 11.2)
  * `parse` **(Compute Cores)** (Impact: 9.5)
  * `executable` **(Defensive Guards)** (Impact: 8.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **10**; blast radius 4.889; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (74.8%), Connectivity (formerly Api Exposure) (68.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 17.9)
  * `metadata_hashes` **(Defensive Guards)** (Impact: 13.2)
  * `hashes` **(Generic / Templated Code)** (Impact: 10.2)
  * `egg_fragment` **(Generic / Templated Code)** (Impact: 7.3)
  * `subdirectory_fragment` **(Generic / Templated Code)** (Impact: 7.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000101 | `Ripple Effect (Closeness):` 0.044628
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **10**; blast radius 8.851; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (78.5%), Guard Balance (formerly Safety Score) (71.6%), Connectivity (formerly Api Exposure) (66.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `union` **(Defensive Guards)** (Impact: 24.2)
  * `allows_all` **(Defensive Guards)** (Impact: 22.1)
  * `allows_any` **(Defensive Guards)** (Impact: 22.1)
  * `intersect` **(Defensive Guards)** (Impact: 13.1)
  * `intersect` **(Defensive Guards)** (Impact: 9.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.784; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (44.0%), Connectivity (formerly Api Exposure) (8.1%), Complexity Load (formerly Cognitive Load) (7.6%)
- **Documentation Coverage:** 92.3077% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_dependencies_for_locking` **(Defensive Guards)** (Impact: 37.5)
  * `create_dependency` **(Many-Argument Workhorses)** (Impact: 20.4)
  * `test_include_dependency_groups` **(Defensive Guards)** (Impact: 16.2)
  * `test_add_dependency_adds_to_correct_list` **(Generic / Templated Code)** (Impact: 12.1)
  * `test_dependencies` **(Generic / Templated Code)** (Impact: 11.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.784; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (43.5%), Connectivity (formerly Api Exposure) (11.2%), Guard Balance (formerly Safety Score) (7.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_dependency_from_pep_508_complex` **(Defensive Guards)** (Impact: 9.9)
  * `test_dependency_from_pep_508_with_python_full_version` **(Defensive Guards)** (Impact: 6.8)
  * `test_dependency_from_pep_508_with_python_version_union_of_multi` **(Defensive Guards)** (Impact: 5.7)
  * `test_dependency_from_pep_508_with_not_in_op_marker` **(Defensive Guards)** (Impact: 3.7)
  * `test_dependency_from_pep_508_with_python_version` **(Defensive Guards)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **6**; blast radius 5.109; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.5%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 95.3488% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `is_same_source_as` **(Compute Cores)** (Impact: 40.8)
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 10.3)
  * `_normalize_source_url` **(Stateful Encapsulated Methods)** (Impact: 7.4)
  * `provides` **(Generic / Templated Code)** (Impact: 4.0)
    * *Intent:* """ Helper method to determine if this package provides the given specification. This determination ...
  * `with_features` **(Generic / Templated Code)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 16 instances
* *State Mutation (weighted view):* 53
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000127 | `Ripple Effect (Closeness):` 0.045141
...
- `poetry_core-2.3.2/src/poetry/core/packages/dependency.py` -> **Severity: 0.396** (Bridge: 0.004 * Flux: 100.0%)
...
- `poetry_core-2.3.2/src/poetry/core/factory.py` -> **Severity: 0.168** (Bridge: 0.0017 * Flux: 100.0%)
