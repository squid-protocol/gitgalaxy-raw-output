...
## 1. EXECUTIVE SUMMARY
- **Scope:** 135 analyzed artifact(s), 32032 LOC.
- **Load-bearing artifact:** `codegen/src/full.rs` -- 7 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/lib.rs` -- pulls in 222 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/expr.rs` at magnitude 2069.86 (structural weight, not risk).
...
> **Architectural Drift Z-Score:** `1.38`
> **Composition Archetype:** `Mid Flat Project` (z +1.38; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules (3) 24%, State Mutators Files 15%, Data / Markup / Trivial 12%, Declarative / Non-Code 10%, Generic / Templated Code Files 10%
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 16.6 | 4.8 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 79.1 | 100.0 | 100.0 |
...
| connectivity | 1620 | 88 | 27 | `src/item.rs` |
...
| dead_code | 911 | 87 | 18 | `src/parse.rs` |
| credential | 0 | 0 | 0 | - |
...
- `ambig_ty` **(Many-Argument Workhorses)** (@ `src/ty.rs`) -> Impact: **256.7** | LOC: 294
- `parse_rest_of_item` **(Many-Argument Workhorses)** (@ `src/item.rs`) -> Impact: **236.8** | LOC: 176
- `scan_right` **(Many-Argument Workhorses)** (@ `src/fixup.rs`) -> Impact: **212.5** | LOC: 282
- `atom_expr` **(Compute Cores)** (@ `src/expr.rs`) -> Impact: **149.3** | LOC: 76
  * *Intent:* // Parse all atomic expressions which don't have to worry about precedence // interactions, as they are fully contained.
- `expand_impl_body` **(Many-Argument Workhorses)** (@ `codegen/src/snapshot.rs`) -> Impact: **118.1** | LOC: 170
- `parse_stmt` **(Compute Cores)** (@ `src/stmt.rs`) -> Impact: **103.8** | LOC: 67
- `parse` **(Compute Cores)** (@ `src/op.rs`) -> Impact: **86.5** | LOC: 61
- `parse_expr` **(Many-Argument Workhorses)** (@ `src/expr.rs`) -> Impact: **84.5** | LOC: 80
- `parse_impl` **(Many-Argument Workhorses)** (@ `src/item.rs`) -> Impact: **84.4** | LOC: 95
- `parse_with_earlier_boundary_rule` **(Compute Cores)** (@ `src/expr.rs`) -> Impact: **76.5** | LOC: 60
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **111**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (97.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (51.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (48.4%)
- **Documentation Coverage:** 96.5116% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `atom_expr` **(Compute Cores)** (Impact: 149.3)
    * *Intent:* // Parse all atomic expressions which don't have to worry about precedence // interactions, as they ...
  * `parse_expr` **(Many-Argument Workhorses)** (Impact: 84.5)
  * `parse_with_earlier_boundary_rule` **(Compute Cores)** (Impact: 76.5)
  * `trailer_helper` **(Many-Argument Workhorses)** (Impact: 70.4)
  * `parse_range_end` **(Compute Cores)** (Impact: 55.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **99**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (95.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (52.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (32.0%)
- **Documentation Coverage:** 98.1481% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_rest_of_item` **(Many-Argument Workhorses)** (Impact: 236.8)
  * `parse_impl` **(Many-Argument Workhorses)** (Impact: 84.4)
  * `parse` **(Compute Cores)** (Impact: 75.5)
  * `parse_use_tree` **(Compute Cores)** (Impact: 74.8)
  * `parse` **(Compute Cores)** (Impact: 66.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **68**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.4%), Mutation Surface (formerly State Flux) (93.9%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (73.9%)
- **Documentation Coverage:** 93.0348% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_lit_float` **(Compute Cores)** (Impact: 44.1)
    * *Intent:* // Returns base 10 digits and suffix.
  * `parse_lit_int` **(Compute Cores)** (Impact: 41.2)
    * *Intent:* // Returns base 10 digits and suffix.
  * `parse_lit_c_str_cooked` **(Compute Cores)** (Impact: 29.3)
  * `from_str` **(Defensive Guards)** (Impact: 28.4)
  * `parse_lit_str_cooked` **(Compute Cores)** (Impact: 23.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **55**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (82.3%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (49.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (24.4%)
- **Documentation Coverage:** 76.3158% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `do_parse` **(Compute Cores)** (Impact: 51.3)
  * `parse` **(Compute Cores)** (Impact: 51.3)
  * `parse_multiple` **(Stateful Encapsulated Methods)** (Impact: 37.1)
  * `parse` **(Compute Cores)** (Impact: 35.5)
  * `parse` **(Compute Cores)** (Impact: 29.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **52**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (56.2%), Mutation Surface (formerly State Flux) (32.9%), Debt Markers (formerly Tech Debt) (24.5%)
- **Documentation Coverage:** 96.4912% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ambig_ty` **(Many-Argument Workhorses)** (Impact: 256.7)
  * `parse_bare_fn_arg` **(Compute Cores)** (Impact: 52.6)
  * `parse` **(Compute Cores)** (Impact: 30.4)
  * `parse` **(Stateful Encapsulated Methods)** (Impact: 16.3)
  * `parse_bounds` **(Many-Argument Workhorses)** (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **51**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (86.7%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (47.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.6%)
- **Documentation Coverage:** 83.7838% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_single` **(Compute Cores)** (Impact: 57.4)
    * *Intent:* /// ```compile_fail /// fn f(Some(_) | None: Option<T>) { /// let _ = |Some(_) | None: Option<T>| {}...
  * `pat_range_bound` **(Compute Cores)** (Impact: 37.0)
  * `field_pat` **(Compute Cores)** (Impact: 29.2)
  * `multi_pat_impl` **(Compute Cores)** (Impact: 23.6)
  * `pat_struct` **(Many-Argument Workhorses)** (Impact: 21.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **43**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (75.1%), Guard Balance (formerly Safety Score) (47.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (18.5%)
- **Documentation Coverage:** 82.7586% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse` **(Compute Cores)** (Impact: 49.7)
  * `parse_helper` **(Compute Cores)** (Impact: 34.4)
  * `qpath` **(Stateful Encapsulated Methods)** (Impact: 33.7)
  * `parse_mod_style` **(Compute Cores)** (Impact: 24.2)
    * *Intent:* /// struct SingleUse { /// use_token: Token![use], /// path: Path, /// } /// /// impl Parse for Sing...
  * `do_parse` **(Stateful Encapsulated Methods)** (Impact: 18.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
* *Amplified Cascading Flux:* 3 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **29**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.4%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (69.0%), Guard Balance (formerly Safety Score) (49.9%)
- **Documentation Coverage:** 49.6296% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_terminated_with` **(Generic / Templated Code)** (Impact: 15.2)
    * *Intent:* /// Parses zero or more occurrences of `T` using the given parse function, /// separated by punctuat...
  * `parse_separated_nonempty_with` **(Generic / Templated Code)** (Impact: 11.5)
    * *Intent:* /// Parses one or more occurrences of `T` using the given parse function, /// separated by punctuati...
  * `fold` **(Stateful Encapsulated Methods)** (Impact: 9.4)
  * `get_mut` **(Defensive Guards)** (Impact: 9.2)
    * *Intent:* /// Mutably borrows the element at the given index.
  * `get` **(Defensive Guards)** (Impact: 9.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Dead Code Surface (formerly Dead Code) (40.1%), Debt Markers (formerly Tech Debt) (39.0%), Complexity Load (formerly Cognitive Load) (7.3%)
- **Documentation Coverage:** 40.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `scan_right` **(Many-Argument Workhorses)** (Impact: 212.5)
  * `parenthesize` **(Compute Cores)** (Impact: 32.4)
    * *Intent:* /// Determine whether parentheses are needed around the given expression to /// head off the early t...
  * `precedence` **(Defensive Guards)** (Impact: 17.6)
    * *Intent:* /// Determines the effective precedence of a subexpression. Some expressions /// have higher or lowe...
  * `rightmost_subexpression_precedence` **(Defensive Guards)** (Impact: 13.3)
  * `leftmost_subexpression_precedence` **(Defensive Guards)** (Impact: 9.3)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **44**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Debt Markers (formerly Tech Debt) (91.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (42.4%)
- **Documentation Coverage:** 83.5616% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `punct_helper` **(Many-Argument Workhorses)** (Impact: 23.2)
  * `peek_punct` **(Defensive Guards)** (Impact: 20.0)
  * `parse` **(Defensive Guards)** (Impact: 7.8)
  * `keyword` **(Stateful Encapsulated Methods)** (Impact: 5.7)
  * `peek_keyword` **(Stateful Encapsulated Methods)** (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **45**; blast radius 6.697; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (79.0%), Connectivity (formerly Api Exposure) (51.9%), Mutation Surface (formerly State Flux) (50.8%)
- **Documentation Coverage:** 94.2857% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `do_load_file` **(Many-Argument Workhorses)** (Impact: 61.2)
  * `parse_features` **(Compute Cores)** (Impact: 18.3)
  * `introspect_type` **(Many-Argument Workhorses)** (Impact: 17.0)
  * `ast_enum_of_structs` **(Compute Cores)** (Impact: 14.5)
  * `load_token_file` **(Defensive Guards)** (Impact: 13.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **76**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (56.4%), Concurrency Surface (formerly Concurrency) (16.5%), Complexity Load (formerly Cognitive Load) (3.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `iter` **(Many-Argument Workhorses)** (Impact: 70.3)
  * `test_permutations` **(I/O & Config Routines)** (Impact: 69.5)
  * `test_fixup` **(Defensive Guards)** (Impact: 45.5)
  * `test_ambiguous_label` **(Generic / Templated Code)** (Impact: 18.4)
  * `test_extended_interpolated_path` **(I/O & Config Routines)** (Impact: 10.4)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **61**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (66.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (13.2%), Complexity Load (formerly Cognitive Load) (6.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (3.6%)
- **Documentation Coverage:** 97.2222% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `librustc_parenthesize` **(Compute Cores)** (Impact: 37.3)
  * `test_expressions` **(Many-Argument Workhorses)** (Impact: 36.0)
  * `syn_parenthesize` **(Compute Cores)** (Impact: 21.7)
  * `make_parens_invisible` **(Defensive Guards)** (Impact: 9.0)
  * `flat_map_assoc_item` **(Generic / Templated Code)** (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Cascading Flux:* 12 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **41**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (68.7%), Dead Code Surface (formerly Dead Code) (53.8%), Guard Balance (formerly Safety Score) (47.7%)
- **Documentation Coverage:** 83.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_stmt` **(Compute Cores)** (Impact: 103.8)
  * `stmt_local` **(Compute Cores)** (Impact: 33.7)
  * `stmt_expr` **(Many-Argument Workhorses)** (Impact: 31.9)
  * `parse_within` **(Compute Cores)** (Impact: 19.7)
    * *Intent:* /// Ok(MiniFunction { /// attrs: { /// let mut attrs = outer_attrs; /// attrs.extend(inner_attrs); /...
  * `to_tokens` **(Defensive Guards)** (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (72.2%), Complexity Load (formerly Cognitive Load) (37.8%), Connectivity (formerly Api Exposure) (6.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `download_and_unpack` **(Compute Cores)** (Impact: 15.4)
  * `for_each_rust_file` **(Compute Cores)** (Impact: 14.3)
  * `clone_rust` **(I/O & Config Routines)** (Impact: 14.2)
  * `base_dir_filter` **(Defensive Guards)** (Impact: 12.6)
  * `edition` **(Generic / Templated Code)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 6.697; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (48.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (24.5%), Mutation Surface (formerly State Flux) (23.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expand_impl_body` **(Many-Argument Workhorses)** (Impact: 118.1)
  * `format_field` **(Many-Argument Workhorses)** (Impact: 23.8)
  * `syntax_tree_enum` **(Generic / Templated Code)** (Impact: 11.0)
  * `fmt` **(Defensive Guards)** (Impact: 9.2)
  * `expand_impl` **(Generic / Templated Code)** (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **46**; blast radius 6.697; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Dead Code Surface (formerly Dead Code) (96.3%), Debt Markers (formerly Tech Debt) (94.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (30.4%)
- **Documentation Coverage:** 52.7273% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_meta_name_value_after_path` **(Defensive Guards)** (Impact: 16.6)
  * `parse_meta_after_path` **(Stateful Encapsulated Methods)** (Impact: 16.0)
  * `fmt` **(Compute Cores)** (Impact: 10.8)
  * `parse_inner` **(Stateful Encapsulated Methods)** (Impact: 7.2)
  * `single_parse_inner` **(Stateful Encapsulated Methods)** (Impact: 6.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **75**; blast radius 6.697; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Dead Code Surface (formerly Dead Code) (99.4%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (55.7%), Connectivity (formerly Api Exposure) (52.1%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__parse_scoped` **(Defensive Guards)** (Impact: 10.8)
  * `span_of_unexpected_ignoring_nones` **(Defensive Guards)** (Impact: 9.3)
  * `parse2` **(Defensive Guards)** (Impact: 9.3)
  * `step` **(Generic / Templated Code)** (Impact: 6.6)
    * *Intent:* /// # /// # fn remainder_after_skipping_past_next_at( /// # input: ParseStream, /// # ) -> Result<pr...
  * `parse` **(Generic / Templated Code)** (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (98.9%), Mutation Surface (formerly State Flux) (82.2%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (58.3%)
- **Documentation Coverage:** 30.9091% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `skip` **(Stateful Encapsulated Methods)** (Impact: 8.1)
    * *Intent:* /// Skip over the next token that is not a None-delimited group, without /// cloning it. Returns `No...
  * `group` **(Generic / Templated Code)** (Impact: 7.9)
    * *Intent:* /// If the cursor is pointing at a `Group` with the given delimiter, returns /// a cursor into that ...
  * `create` **(Defensive Guards)** (Impact: 7.8)
    * *Intent:* /// This create method intelligently exits non-explicitly-entered /// `None`-delimited scopes when t...
  * `lifetime` **(Compute Cores)** (Impact: 7.8)
    * *Intent:* /// If the cursor is pointing at a `Lifetime`, returns it along with a /// cursor pointing at the ne...
  * `ignore_none` **(Defensive Guards)** (Impact: 7.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **208**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Complexity Load (formerly Cognitive Load) (7.6%), Connectivity (formerly Api Exposure) (2.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `doc_comment` **(Many-Argument Workhorses)** (Impact: 22.6)
  * `eq` **(Compute Cores)** (Impact: 18.9)
  * `eq` **(Compute Cores)** (Impact: 13.5)
  * `eq` **(Compute Cores)** (Impact: 13.4)
  * `eq` **(State Mutators)** (Impact: 7.9)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 6.697; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.1%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (69.8%), Complexity Load (formerly Cognitive Load) (22.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `node` **(Many-Argument Workhorses)** (Impact: 53.1)
  * `visit` **(Many-Argument Workhorses)** (Impact: 44.0)
  * `generate` **(Annotated & Test Methods)** (Impact: 4.6)
  * `visit_attributes_mut` **(Parameter Forwarders)** (Impact: 3.7)
    * *Intent:* #features
  * `requires_full` **(State Mutators)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 13 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 6.697; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.1%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (78.0%), Complexity Load (formerly Cognitive Load) (34.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `node` **(Many-Argument Workhorses)** (Impact: 54.6)
  * `visit` **(Many-Argument Workhorses)** (Impact: 34.8)
  * `generate` **(Generic / Templated Code)** (Impact: 5.0)
  * `requires_full` **(State Mutators)** (Impact: 3.0)
  * `fold_vec` **(Generic / Templated Code)** (Impact: 2.4)
    * *Intent:* #impls
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 13 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 6.697; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.1%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (69.6%), Connectivity (formerly Api Exposure) (28.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `node` **(Many-Argument Workhorses)** (Impact: 52.7)
  * `visit` **(Many-Argument Workhorses)** (Impact: 37.0)
  * `generate` **(Annotated & Test Methods)** (Impact: 4.4)
  * `requires_full` **(State Mutators)** (Impact: 3.0)
  * `simple_visit` **(Parameter Forwarders)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 12 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **29**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (43.4%), Dead Code Surface (formerly Dead Code) (24.4%), Debt Markers (formerly Tech Debt) (22.5%)
- **Documentation Coverage:** 46.1538% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse` **(Compute Cores)** (Impact: 27.4)
  * `parse_named` **(Compute Cores)** (Impact: 24.4)
    * *Intent:* /// Parses a named (braced struct) field.
  * `parse_unnamed` **(State Mutators)** (Impact: 6.2)
    * *Intent:* /// Parses an unnamed (tuple struct) field.
  * `next` **(Defensive Guards)** (Impact: 5.1)
  * `to_tokens` **(Defensive Guards)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 6.697; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (57.6%), Mutation Surface (formerly State Flux) (54.9%), Debt Markers (formerly Tech Debt) (11.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse` **(Compute Cores)** (Impact: 32.9)
  * `data_struct` **(Stateful Encapsulated Methods)** (Impact: 27.2)
  * `to_tokens` **(Compute Cores)** (Impact: 10.7)
  * `data_enum` **(Stateful Encapsulated Methods)** (Impact: 5.0)
  * `data_union` **(Encapsulated Accessors)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
