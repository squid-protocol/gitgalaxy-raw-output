...
## 1. EXECUTIVE SUMMARY
- **Scope:** 218 analyzed artifact(s), 27085 LOC.
- **Load-bearing artifact:** `coverage-7.13.5/tests/gold/html/support/coverage_html_cb_bcae5fc4.js` -- 45 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `coverage-7.13.5/tests/test_process.py` -- pulls in 42 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `coverage-7.13.5/tests/test_data.py` at magnitude 679.28 (structural weight, not risk).
...
| Avg Path Length | 1.3195 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `4.285`
> **Composition Archetype:** `Small Flat Repo (2)` (z +4.29; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 60%, Generic / Templated Code Files 22%, Declarative / Non-Code 5%, Compute Cores Files 5%, Large Core Modules (2) 4%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 26.9 | 0.0 | 0.0 |
...
- `one_file` **(Many-Argument Workhorses)** (@ `coverage-7.13.5/lab/parser.py`) -> Impact: **51.4** | LOC: 67
  * *Intent:* """Process just one file."""
- `_make_summary` **(Many-Argument Workhorses)** (@ `coverage-7.13.5/tests/test_json.py`) -> Impact: **48.5** | LOC: 65
- `check_coverage` **(Many-Argument Workhorses)** (@ `coverage-7.13.5/tests/coveragetest.py`) -> Impact: **40.8** | LOC: 86
- `should_skip` **(Compute Cores)** (@ `coverage-7.13.5/igor.py`) -> Impact: **34.7** | LOC: 35
...
- `assert_warnings` **(Many-Argument Workhorses)** (@ `coverage-7.13.5/tests/coveragetest.py`) -> Impact: **32.2** | LOC: 62
- `test_if_else` **(Compute Cores)** (@ `coverage-7.13.5/tests/test_templite.py`) -> Impact: **31.2** | LOC: 31
- `compare` **(Many-Argument Workhorses)** (@ `coverage-7.13.5/tests/goldtest.py`) -> Impact: **30.9** | LOC: 80
- `arc_ascii_art` **(Compute Cores)** (@ `coverage-7.13.5/lab/parser.py`) -> Impact: **29.6** | LOC: 38
  * *Intent:* """Draw arcs as ascii art. Returns a dictionary mapping line numbers to ascii strings to draw for that line. """
- `wire_up_filter` **(I/O & Config Routines)** (@ `coverage-7.13.5/tests/gold/html/support/coverage_html_cb_bcae5fc4.js`) -> Impact: **29.6** | LOC: 132
  * *Intent:* // Create the events for the filter box.
- `show_code` **(Many-Argument Workhorses)** (@ `coverage-7.13.5/lab/show_pyc.py`) -> Impact: **28.8** | LOC: 55
  * *Intent:* # fmt: on
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (70.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (35.0%), Concurrency Surface (formerly Concurrency) (20.2%)
- **Documentation Coverage:** 92.0455% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `make_data_files` **(Type Conversions)** (Impact: 14.9)
    * *Intent:* """Make a number data files. `spec` is a string dictating the data for each file. Same characters in...
  * `test_update_conflicting_file_tracers` **(Compute Cores)** (Impact: 7.9)
  * `test_update_file_tracer_vs_no_file_tracer` **(Compute Cores)** (Impact: 7.8)
  * `test_thread_stress` **(Defensive Guards)** (Impact: 6.6)
  * `test_meta_data` **(Defensive Guards)** (Impact: 5.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **32**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (38.9%), Concurrency Surface (formerly Concurrency) (17.4%), Connectivity (formerly Api Exposure) (12.8%)
- **Documentation Coverage:** 83.8384% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `clean_files` **(Generic / Templated Code)** (Impact: 10.5)
    * *Intent:* """Remove names matching `pats` from `files`, a list of file names."""
  * `test_combining_bad_data` **(Many-Argument Workhorses)** (Impact: 7.0)
    * *Intent:* # If you try to combine a bad data file, then you will get a warning, # and the file will remain. se...
  * `test_moving_stuff` **(Defensive Guards)** (Impact: 5.1)
    * *Intent:* # When using absolute file names, moving the source around results in # "No source for code" errors ...
  * `test_switch_context_unstarted` **(Generic / Templated Code)** (Impact: 4.8)
    * *Intent:* # Coverage must be started to switch context msg = "Cannot switch context, coverage is not started" ...
  * `test_combine_no_usable_files` **(Defensive Guards)** (Impact: 4.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **35**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (58.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (12.3%), Complexity Load (formerly Cognitive Load) (11.3%)
- **Documentation Coverage:** 80.6061% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `assert_valid_hrefs` **(Defensive Guards)** (Impact: 11.5)
    * *Intent:* """Assert that the hrefs in htmlcov/*.html are valid. Doesn't check external links (those with a pro...
  * `test_dynamic_contexts` **(Defensive Guards)** (Impact: 9.4)
  * `test_dynamic_contexts_relative_files` **(Defensive Guards)** (Impact: 9.3)
  * `handle_starttag` **(Defensive Guards)** (Impact: 8.7)
  * `assert_htmlcov_files_exist` **(Defensive Guards)** (Impact: 8.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **31**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (80.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (22.6%)
- **Documentation Coverage:** 81.0811% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_thread_safe_save_data` **(Compute Cores)** (Impact: 18.4)
    * *Intent:* # Non-regression test for: https://github.com/coveragepy/coveragepy/issues/581 # Create some Python ...
  * `cant_trace_msg` **(Compute Cores)** (Impact: 16.7)
    * *Intent:* """What might coverage.py say about a concurrency setting and imported module?"""
  * `try_some_code` **(Many-Argument Workhorses)** (Impact: 12.1)
  * `measurable_line` **(Generic / Templated Code)** (Impact: 7.7)
    * *Intent:* """Is this a line of code coverage will measure? Not blank, not a comment, and not "else" """
  * `test_multiprocessing_simple` **(Generic / Templated Code)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **45** in-repo importer(s); blast radius 117.744; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (92.2%), Complexity Load (formerly Cognitive Load) (78.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Concurrency Surface (formerly Concurrency) (23.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `wire_up_filter` **(I/O & Config Routines)** (Impact: 29.6)
    * *Intent:* // Create the events for the filter box.
  * `sortColumn` **(Defensive Guards)** (Impact: 19.6)
  * `select_line_or_chunk` **(Defensive Guards)** (Impact: 13.2)
    * *Intent:* // Select line number lineno, or if it is in a colored chunk, select the // entire chunk
  * `getCellValue` **(Defensive Guards)** (Impact: 11.0)
    * *Intent:* // Helpers for table sorting
  * `wire_up_sorting` **(I/O & Config Routines)** (Impact: 10.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **42**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (24.1%), Connectivity (formerly Api Exposure) (12.1%), Complexity Load (formerly Cognitive Load) (11.9%)
- **Documentation Coverage:** 89.7436% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_pth_and_source_work_together` **(Many-Argument Workhorses)** (Impact: 13.5)
  * `fullname` **(Generic / Templated Code)** (Impact: 6.0)
    * *Intent:* """What is the full module name for `modname` for this test?"""
  * `assert_pydoc_ok` **(Defensive Guards)** (Impact: 4.5)
    * *Intent:* """Check that pydoc of `name` finds the docstring from `thing`."""
  * `test_aliases_used_in_messages` **(Defensive Guards)** (Impact: 3.7)
  * `test_warns_if_never_run` **(Defensive Guards)** (Impact: 3.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.8%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (68.3%)
- **Documentation Coverage:** 1.9608% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `should_skip` **(Compute Cores)** (Impact: 34.7)
    * *Intent:* """Is there a reason to skip these tests? Return empty string to run tests, or a message about why w...
  * `run_tests_with_coverage` **(Many-Argument Workhorses)** (Impact: 13.3)
    * *Intent:* """Run tests, but with coverage."""
  * `main` **(Compute Cores)** (Impact: 11.2)
    * *Intent:* """Main command-line execution for igor. Verbs are taken from the command line, and extra words take...
  * `message` **(Generic / Templated Code)** (Impact: 10.8)
    * *Intent:* """All messages come through here, keep the data we want."""
  * `do_test_with_core` **(Compute Cores)** (Impact: 9.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (38.1%), Concurrency Surface (formerly Concurrency) (28.3%), Connectivity (formerly Api Exposure) (11.8%)
- **Documentation Coverage:** 94.4444% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `mock_command_line` **(Many-Argument Workhorses)** (Impact: 13.5)
  * `command_line` **(Defensive Guards)** (Impact: 11.2)
    * *Intent:* """Stub for command_line, the arg determines what it will do."""
  * `cmd_executes` **(Many-Argument Workhorses)** (Impact: 11.1)
  * `cmd_help` **(Defensive Guards)** (Impact: 8.3)
  * `test_fail_under` **(Generic / Templated Code)** (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **38**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (14.5%), Connectivity (formerly Api Exposure) (11.7%), Complexity Load (formerly Cognitive Load) (11.3%)
- **Documentation Coverage:** 85.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `make_rigged_file` **(Generic / Templated Code)** (Impact: 5.1)
    * *Intent:* """Create a file that will have specific results. `stmts` and `miss` are ints, the number of stateme...
  * `get_summary_text` **(Generic / Templated Code)** (Impact: 4.4)
    * *Intent:* """Get text output from the SummaryReporter. The arguments are tuples: (name, value) for Coverage.se...
  * `test_dotpy_not_python_ignored` **(Generic / Templated Code)** (Impact: 4.2)
    * *Intent:* # We run a Python file, and when reporting, we can't parse it as Python, # but we've said to ignore ...
  * `test_test_data` **(Defensive Guards)** (Impact: 4.0)
    * *Intent:* # We use our own test files as test data. Check that our assumptions # about them are still valid. W...
  * `test_dotpy_not_python` **(Generic / Templated Code)** (Impact: 3.4)
...
- **Blast Radius:** changing it is visible to **40** in-repo importer(s); it depends on **25**; blast radius 34.182; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (96.3%), Guard Balance (formerly Safety Score) (89.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (22.0%)
- **Documentation Coverage:** 31.0345% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `check_coverage` **(Many-Argument Workhorses)** (Impact: 40.8)
  * `assert_warnings` **(Many-Argument Workhorses)** (Impact: 32.2)
  * `make_data_file` **(Many-Argument Workhorses)** (Impact: 16.9)
  * `run_command_status` **(Many-Argument Workhorses)** (Impact: 11.0)
    * *Intent:* """Run the command-line `cmd` in a subprocess, and print its output. Use this when you need to test ...
  * `get_report` **(Many-Argument Workhorses)** (Impact: 10.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Concurrency Surface (formerly Concurrency) (23.5%), Connectivity (formerly Api Exposure) (11.9%), Guard Balance (formerly Safety Score) (11.7%)
- **Documentation Coverage:** 84.127% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `assert_config_settings_are_correct` **(Defensive Guards)** (Impact: 9.3)
    * *Intent:* """Check that `cov` has all the settings from LOTSA_SETTINGS."""
  * `test_tweak_error_checking` **(Generic / Templated Code)** (Impact: 7.6)
    * *Intent:* # Trying to set an unknown config value raises an error. cov = coverage.Coverage() with pytest.raise...
  * `test_tweak_plugin_options` **(Defensive Guards)** (Impact: 4.9)
    * *Intent:* # Plugin options have a more flexible syntax. cov = coverage.Coverage() cov.set_option("run:plugins"...
  * `test_toml_parse_errors` **(Generic / Templated Code)** (Impact: 4.7)
    * *Intent:* # Im-parsable values raise ConfigError, with details. self.make_file(filename, bad_config) with pyte...
  * `test_core_option` **(Generic / Templated Code)** (Impact: 4.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **27**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (37.7%), Connectivity (formerly Api Exposure) (11.6%), Complexity Load (formerly Cognitive Load) (9.9%)
- **Documentation Coverage:** 83.7838% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_bad_plugin` **(Many-Argument Workhorses)** (Impact: 26.3)
  * `run_all_functions` **(Defensive Guards)** (Impact: 6.6)
    * *Intent:* """Run all functions in `suite_name` under coverage."""
  * `get_plugin_options` **(Generic / Templated Code)** (Impact: 5.5)
    * *Intent:* """Just return the options for `plugin` if this is the right module."""
  * `test_exception_if_plugins_on_pytracer` **(Defensive Guards)** (Impact: 5.1)
  * `test_plugin2_with_xml_report` **(Defensive Guards)** (Impact: 4.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (25.9%), Connectivity (formerly Api Exposure) (12.5%), Complexity Load (formerly Cognitive Load) (1.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_pathologically_long_code_object` **(Defensive Guards)** (Impact: 4.7)
    * *Intent:* # https://github.com/coveragepy/coveragepy/issues/359 # Long code objects sometimes cause problems. ...
  * `test_exception_group` **(Generic / Templated Code)** (Impact: 4.6)
    * *Intent:* # PyPy3.11 traces this incorrectly: https://github.com/pypy/pypy/issues/5354 if env.PYPY: missing = ...
  * `test_zero_coverage_while_loop` **(Generic / Templated Code)** (Impact: 1.7)
    * *Intent:* # https://github.com/coveragepy/coveragepy/issues/502 self.make_file("main.py", "print('done')") sel...
  * `test_bug_212` **(Generic / Templated Code)** (Impact: 1.7)
    * *Intent:* # "except Exception as e" is crucial here. # Bug 212 said that the "if exc" line was incorrectly mar...
  * `test_bug_324` **(Generic / Templated Code)** (Impact: 1.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (13.5%), Complexity Load (formerly Cognitive Load) (12.3%), Connectivity (formerly Api Exposure) (11.7%)
- **Documentation Coverage:** 96.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_line_endings` **(Defensive Guards)** (Impact: 4.9)
  * `test_fuzzed_double_parse` **(Generic / Templated Code)** (Impact: 4.8)
    * *Intent:* # https://bugs.chromium.org/p/oss-fuzz/issues/detail?id=50381 # The second parse used to raise `Type...
  * `test_os_error` **(Generic / Templated Code)** (Impact: 4.5)
  * `test_not_python` **(Generic / Templated Code)** (Impact: 3.7)
  * `test_multiline_exclusion_match_all` **(Generic / Templated Code)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Cascading Flux:* 14 instances
* *High Risk Execution (weighted view):* 0
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (47.7%), Connectivity (formerly Api Exposure) (11.9%), Complexity Load (formerly Cognitive Load) (8.7%)
- **Documentation Coverage:** 89.1304% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `globs_to_regex_params` **(Many-Argument Workhorses)** (Impact: 11.6)
  * `assert_mapped` **(Defensive Guards)** (Impact: 7.4)
    * *Intent:* """Assert that `inp` mapped through `aliases` produces `out`. If the aliases are not relative, then ...
  * `test_multiple_patterns` **(Generic / Templated Code)** (Impact: 6.8)
    * *Intent:* # also test the debugfn... msgs: list[str] = [] aliases = PathAliases(debugfn=msgs.append, relative=...
  * `test_dot` **(Defensive Guards)** (Impact: 5.8)
  * `test_globs_to_regex` **(Type Conversions)** (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (97.5%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (28.2%), Connectivity (formerly Api Exposure) (3.8%)
- **Documentation Coverage:** 88.2353% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_make_summary` **(Many-Argument Workhorses)** (Impact: 48.5)
  * `_make_file_result` **(Many-Argument Workhorses)** (Impact: 16.6)
  * `test_line_and_branch_coverage` **(Many-Argument Workhorses)** (Impact: 12.7)
  * `test_regions` **(Many-Argument Workhorses)** (Impact: 10.4)
  * `_make_region` **(Many-Argument Workhorses)** (Impact: 10.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **11**; blast radius 8.928; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.2%), Test Surface (formerly Verification) (80.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `one_file` **(Many-Argument Workhorses)** (Impact: 51.4)
    * *Intent:* """Process just one file."""
  * `arc_ascii_art` **(Compute Cores)** (Impact: 29.6)
    * *Intent:* """Draw arcs as ascii art. Returns a dictionary mapping line numbers to ascii strings to draw for th...
  * `main` **(Compute Cores)** (Impact: 16.9)
    * *Intent:* """A main function for trying the code from the command line."""
  * `disassemble` **(Compute Cores)** (Impact: 15.3)
    * *Intent:* """Disassemble code, for ad-hoc experimenting."""
  * `first_all_blanks` **(Type Conversions)** (Impact: 7.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (32.4%), Complexity Load (formerly Cognitive Load) (18.9%), Connectivity (formerly Api Exposure) (12.6%)
- **Documentation Coverage:** 86.3636% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_short_stack_full` **(Defensive Guards)** (Impact: 8.0)
  * `test_debug_sys_ctracer` **(Defensive Guards)** (Impact: 4.6)
  * `test_short_stack_frame_ids` **(Defensive Guards)** (Impact: 4.6)
  * `test_debug_callers` **(Defensive Guards)** (Impact: 3.6)
  * `test_short_filename` **(Defensive Guards)** (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (44.0%), Connectivity (formerly Api Exposure) (12.2%), Complexity Load (formerly Cognitive Load) (9.1%)
- **Documentation Coverage:** 88.2353% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_if_else` **(Compute Cores)** (Impact: 31.2)
  * `test_malformed_else` **(Compute Cores)** (Impact: 17.3)
  * `test_if` **(Compute Cores)** (Impact: 16.9)
  * `test_malformed_for` **(Compute Cores)** (Impact: 14.5)
  * `test_malformed_if` **(Compute Cores)** (Impact: 13.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (48.0%), Connectivity (formerly Api Exposure) (11.8%), Complexity Load (formerly Cognitive Load) (10.0%)
- **Documentation Coverage:** 76.9231% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `make_tree` **(Many-Argument Workhorses)** (Impact: 12.4)
    * *Intent:* """Make a tree of packages. Makes `width` directories, named d0 .. d{width-1}. Each directory has __...
  * `test_relative_source` **(Defensive Guards)** (Impact: 9.3)
  * `package_and_class_tags` **(Generic / Templated Code)** (Impact: 9.0)
    * *Intent:* """Run an XML report on `cov`, and get the package and class tags."""
  * `unbackslash` **(Defensive Guards)** (Impact: 8.9)
    * *Intent:* """Find strings in `v`, and replace backslashes with slashes throughout."""
  * `assert_source` **(Defensive Guards)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **27** in-repo importer(s); it depends on **21**; blast radius 34.926; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (95.9%), Connectivity (formerly Api Exposure) (74.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (23.5%)
- **Documentation Coverage:** 30.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `make_file` **(Many-Argument Workhorses)** (Impact: 27.7)
  * `re_lines` **(Generic / Templated Code)** (Impact: 10.6)
    * *Intent:* """Return a list of lines selected by `pat` in the string `text`. If `match` is false, the selection...
  * `re_lines_text` **(Generic / Templated Code)** (Impact: 10.2)
    * *Intent:* """Return the multi-line text of lines selected by `pat`."""
  * `arcz_to_arcs` **(Defensive Guards)** (Impact: 10.2)
    * *Intent:* """Convert a compact textual representation of arcs to a list of pairs. The text has space-separated...
  * `assert_coverage_warnings` **(Defensive Guards)** (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (35.5%), Complexity Load (formerly Cognitive Load) (13.3%), Connectivity (formerly Api Exposure) (11.7%)
- **Documentation Coverage:** 95.4545% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_same_python_executable` **(Stateful Encapsulated Methods)** (Impact: 9.9)
    * *Intent:* """Determine if `e1` and `e2` refer to the same Python executable. Either path could include symboli...
  * `test_assert_warnings` **(Compute Cores)** (Impact: 9.5)
  * `test_file_count` **(Compute Cores)** (Impact: 8.6)
  * `test_re_lines_inverted` **(Defensive Guards)** (Impact: 6.9)
  * `test_file_exists` **(Generic / Templated Code)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 20 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (32.7%), Connectivity (formerly Api Exposure) (11.7%), Complexity Load (formerly Cognitive Load) (1.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_attribute_annotation` **(Generic / Templated Code)** (Impact: 4.5)
  * `test_no_data_to_report_on_annotate` **(Generic / Templated Code)** (Impact: 3.1)
    * *Intent:* # Reporting with no data produces a nice message and no output # directory. with pytest.raises(NoDat...
  * `test_no_data_to_report_on_html` **(Generic / Templated Code)** (Impact: 3.1)
    * *Intent:* # Reporting with no data produces a nice message and no output # directory. with pytest.raises(NoDat...
  * `test_no_data_to_report_on_xml` **(Generic / Templated Code)** (Impact: 3.1)
    * *Intent:* # Reporting with no data produces a nice message. with pytest.raises(NoDataError, match="No data to ...
  * `test_failed_coverage` **(Generic / Templated Code)** (Impact: 3.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 3.0; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (83.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (24.8%), Connectivity (formerly Api Exposure) (12.7%)
- **Documentation Coverage:** 91.5254% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_combining_arc_contexts` **(Defensive Guards)** (Impact: 10.6)
  * `test_combining_line_contexts` **(Defensive Guards)** (Impact: 6.9)
  * `test_dynamic_alone` **(Generic / Templated Code)** (Impact: 3.9)
  * `test_static_and_dynamic` **(Generic / Templated Code)** (Impact: 3.9)
  * `get_qualname` **(Generic / Templated Code)** (Impact: 3.5)
