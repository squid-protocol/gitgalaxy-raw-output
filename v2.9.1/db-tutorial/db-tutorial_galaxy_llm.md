...
## 1. EXECUTIVE SUMMARY
- **Scope:** 314 analyzed artifact(s), 18580 LOC.
- **Load-bearing artifact:** `codes/javadb/redis/src/test/resources/redis.xml` -- 12 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `README.md` -- pulls in 73 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter07.java` at magnitude 949.42 (structural weight, not risk).
...
| Avg Path Length | 1.4476 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `2.915`
> **Composition Archetype:** `Mid Flat Project` (z +2.92; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 35%, Declarative / Non-Code 28%, Large Core Modules (3) 10%, State Mutators Files 9%, Generic / Templated Code Files 7%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 33.8 | 0.0 | 0.0 |
...
- `updateCpms` **(Many-Argument Workhorses)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter07.java`) -> Impact: **73.1** | LOC: 76
- `indexAd` **(Many-Argument Workhorses)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter07.java`) -> Impact: **51.5** | LOC: 25
- `recordClick` **(Many-Argument Workhorses)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter07.java`) -> Impact: **41.7** | LOC: 30
- `createIndex` **(Stateful Encapsulated Methods)** (@ `codes/javadb/elasticsearch/elasticsearch6/src/main/java/io/github/dunwu/javadb/elasticsearch/mapper/BaseEsMapper.java`) -> Impact: **40.2** | LOC: 21
- `testFileDistribution` **(Compute Cores)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter06.java`) -> Impact: **38.6** | LOC: 64
- `recordTargetingResult` **(Many-Argument Workhorses)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter07.java`) -> Impact: **37.1** | LOC: 27
- `testMultiRecipientMessaging` **(Compute Cores)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter06.java`) -> Impact: **36.9** | LOC: 31
- `fetchPendingMessages` **(Generic / Templated Code)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter06.java`) -> Impact: **36.1** | LOC: 63
- `cleanTimelines` **(Many-Argument Workhorses)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter08.java`) -> Impact: **33.4** | LOC: 31
- `testAddUpdateContact` **(Defensive Guards)** (@ `codes/redis/redis-in-action/src/main/java/io/github/dunwu/db/redis/Chapter06.java`) -> Impact: **29.5** | LOC: 52
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.4%), Complexity Load (formerly Cognitive Load) (86.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `updateCpms` **(Many-Argument Workhorses)** (Impact: 73.1)
  * `indexAd` **(Many-Argument Workhorses)** (Impact: 51.5)
  * `recordClick` **(Many-Argument Workhorses)** (Impact: 41.7)
  * `recordTargetingResult` **(Many-Argument Workhorses)** (Impact: 37.1)
  * `searchAndZsort` **(Many-Argument Workhorses)** (Impact: 28.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.5%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (72.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `update_cpms` **(Compute Cores)** (Impact: 29.1)
    * *Intent:* # <end id="record_click"/> # 代码清单 7-16 # <start id="update_cpms"/>
  * `search_and_zsort` **(Many-Argument Workhorses)** (Impact: 20.8)
    * *Intent:* # <end id="sorted-searches"/> # 代码清单 7-6 # <start id="zset_scored_composite"/> # 和之前一样，函数接受一个已有搜索结果的...
  * `parse` **(Type Conversions)** (Impact: 19.1)
    * *Intent:* # 这个集合将用于储存不需要的单词。 unwanted = set() # 这个列表将用于储存需要执行交集计算的单词。 all = [] # 这个集合将用于储存目前已发现的同义词。 current =...
  * `parse_and_search` **(Many-Argument Workhorses)** (Impact: 17.6)
    * *Intent:* # <end id="parse-query"/> # 代码清单 7-4 # <start id="search-query"/> # 对查询语句进行分析。 all, unwanted = parse...
  * `record_click` **(Many-Argument Workhorses)** (Impact: 17.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (71.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get_shard_results_thread` **(Many-Argument Workhorses)** (Impact: 21.2)
  * `search_and_zsort` **(Many-Argument Workhorses)** (Impact: 20.2)
  * `syndicate_status` **(Many-Argument Workhorses)** (Impact: 18.0)
    * *Intent:* # <end id="sharded-zrangebyscore"/> # 代码清单 10-15 # <start id="sharded-syndicate-posts"/>
  * `search_shards_zset` **(Many-Argument Workhorses)** (Impact: 15.6)
    * *Intent:* # <end id="zset-search-with-values"/> # 代码清单 10-9 # <start id="search-shards-zset"/> # 函数需要接受所有分片参数以...
  * `test_sharded_follow_user_and_syndicate_status` **(Compute Cores)** (Impact: 14.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.8%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (75.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `copy_logs_to_redis` **(Many-Argument Workhorses)** (Impact: 28.7)
    * *Intent:* # <end id="_1314_15044_3669"/> # 代码清单 6-30 # <start id="_1314_14473_9209"/>
  * `readblocks_gz` **(Compute Cores)** (Impact: 22.9)
    * *Intent:* # <end id="_1314_14473_9225"/> # 代码清单 6-34 # <start id="_1314_14473_9229"/>
  * `process_logs_from_redis` **(Many-Argument Workhorses)** (Impact: 19.6)
    * *Intent:* # <end id="_1314_14473_9209"/> # 代码清单 6-31 # <start id="_1314_14473_9213"/>
  * `test_file_distribution` **(Compute Cores)** (Impact: 12.6)
  * `readlines` **(Compute Cores)** (Impact: 10.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (67.3%)
- **Documentation Coverage:** 83.7209% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testFileDistribution` **(Compute Cores)** (Impact: 38.6)
  * `testMultiRecipientMessaging` **(Compute Cores)** (Impact: 36.9)
  * `fetchPendingMessages` **(Generic / Templated Code)** (Impact: 36.1)
  * `testAddUpdateContact` **(Defensive Guards)** (Impact: 29.5)
  * `processLogsFromRedis` **(Many-Argument Workhorses)** (Impact: 28.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.3%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (72.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `filter_content` **(Many-Argument Workhorses)** (Impact: 22.8)
    * *Intent:* # <end id="delete-message-streaming"/> # 代码清单 8-15 # <start id="message-subscription"/> # 使用第 5 章介绍的...
  * `refill_timeline` **(Many-Argument Workhorses)** (Impact: 17.1)
  * `process_filters` **(Compute Cores)** (Impact: 15.5)
  * `syndicate_status_list` **(Many-Argument Workhorses)** (Impact: 13.4)
    * *Intent:* # <end id="syndicate-message"/> # <start id="syndicate-message-list"/>
  * `clean_timelines` **(Many-Argument Workhorses)** (Impact: 13.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (71.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `clean_counters` **(Compute Cores)** (Impact: 17.7)
    * *Intent:* # <end id="get_counter"/> # <start id="clean_counters"/>
  * `import_ips_to_redis` **(Compute Cores)** (Impact: 16.5)
    * *Intent:* # a faster version with pipelines for actual testing
  * `import_ips_to_redis` **(Compute Cores)** (Impact: 14.8)
    * *Intent:* # <end id="_1314_14473_9188"/> # 代码清单 5-10 # <start id="_1314_14473_9191"/> # 这个函数在执行时需要给定GeoLiteCit...
  * `get_config` **(Many-Argument Workhorses)** (Impact: 12.2)
  * `update_stats` **(Many-Argument Workhorses)** (Impact: 12.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (85.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (63.2%)
- **Documentation Coverage:** 90.4762% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `logCommon` **(Many-Argument Workhorses)** (Impact: 28.5)
  * `updateStats` **(Many-Argument Workhorses)** (Impact: 22.3)
  * `testIpLookup` **(Compute Cores)** (Impact: 19.1)
  * `testCounters` **(Defensive Guards)** (Impact: 18.8)
  * `run` **(I/O & Config Routines)** (Impact: 17.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **51**; blast radius 5.869; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (90.3%), Guard Balance (formerly Safety Score) (89.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (54.0%)
- **Documentation Coverage:** 75.4098% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `newPut` **(Many-Argument Workhorses)** (Impact: 27.8)
  * `getPageData` **(Many-Argument Workhorses)** (Impact: 19.1)
  * `getScrollData` **(Stateful Encapsulated Methods)** (Impact: 17.3)
  * `toEntity` **(Stateful Encapsulated Methods)** (Impact: 17.0)
  * `getRowFromResult` **(Stateful Encapsulated Methods)** (Impact: 11.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Debt Markers (formerly Tech Debt) (77.1%), Guard Balance (formerly Safety Score) (65.4%), Complexity Load (formerly Cognitive Load) (65.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `cleanTimelines` **(Many-Argument Workhorses)** (Impact: 33.4)
  * `refillTimeline` **(Many-Argument Workhorses)** (Impact: 26.7)
  * `followUser` **(Many-Argument Workhorses)** (Impact: 25.8)
  * `testFollowUnfollowUser` **(Defensive Guards)** (Impact: 25.4)
  * `unfollowUser` **(Many-Argument Workhorses)** (Impact: 23.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 25 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.6%), Debt Markers (formerly Tech Debt) (94.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `sharded_bpop_helper` **(Many-Argument Workhorses)** (Impact: 21.3)
    * *Intent:* # 定义一个辅助函数， # 这个函数会为左端阻塞弹出操作以及右端阻塞弹出操作执行实际的弹出动作。 # 准备好流水线对象和超时信息。 pipe = conn.pipeline(False) timeou...
  * `purchase_item_with_lock` **(Many-Argument Workhorses)** (Impact: 10.4)
    * *Intent:* # <end id="autocomplete-on-prefix-lua"/> # 代码清单 11-12 # <start id="ch06-purchase-item-with-lock"/>
  * `call` **(Many-Argument Workhorses)** (Impact: 10.2)
    * *Intent:* # 在调用已载入脚本的时候， # 用户需要将 Redis 连接、脚本要处理的键以及脚本的其他参数传递给脚本。
  * `acquire_lock_with_timeout` **(Many-Argument Workhorses)** (Impact: 10.0)
    * *Intent:* # <end id="post-status-lua"/> # 代码清单 11-4 # <start id="old-lock"/>
  * `acquire_lock_with_timeout` **(Type Conversions)** (Impact: 9.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (76.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testUserLocation` **(Defensive Guards)** (Impact: 21.7)
  * `getExpected` **(Many-Argument Workhorses)** (Impact: 19.3)
  * `updateAggregates` **(Many-Argument Workhorses)** (Impact: 15.7)
  * `longZiplistPerformance` **(Many-Argument Workhorses)** (Impact: 10.6)
  * `setLocation` **(Many-Argument Workhorses)** (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **72**; blast radius 5.608; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (86.3%), Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (73.5%), Connectivity (formerly Api Exposure) (70.2%)
- **Documentation Coverage:** 87.8049% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `pojoListByIds` **(Many-Argument Workhorses)** (Impact: 24.0)
  * `save` **(Many-Argument Workhorses)** (Impact: 17.6)
    * *Intent:* // ==================================================================== // CRUD 操作 // ==============...
  * `updateById` **(Many-Argument Workhorses)** (Impact: 17.6)
  * `createIndex` **(Many-Argument Workhorses)** (Impact: 15.7)
    * *Intent:* // ==================================================================== // 索引管理操作 // ===============...
  * `pojoPageByScrollId` **(Many-Argument Workhorses)** (Impact: 14.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.9%), Debt Markers (formerly Tech Debt) (89.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `update_aggregates` **(Many-Argument Workhorses)** (Impact: 17.6)
    * *Intent:* # <end id="aggregate-population"/> # 代码清单 9-17 # <start id="code-to-location"/>
  * `test_user_location` **(Compute Cores)** (Impact: 13.9)
  * `get_code` **(Compute Cores)** (Impact: 13.4)
    * *Intent:* # <end id="location-tables"/> # 代码清单 9-14 # <start id="location-to-code"/> # 寻找国家对应的偏移量。 cindex = bi...
  * `get_expected` **(Many-Argument Workhorses)** (Impact: 11.5)
    * *Intent:* # 如果程序已经计算出或者获取到了当日的预计访客人数， # 那么直接使用已计算出的数字。 if key in EXPECTED: return EXPECTED[key] exkey = key + ...
  * `long_ziplist_performance` **(Many-Argument Workhorses)** (Impact: 10.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.5%), Mutation Surface (formerly State Flux) (97.5%), Debt Markers (formerly Tech Debt) (84.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 81.25% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testShopppingCartCookies` **(Defensive Guards)** (Impact: 21.5)
  * `testCacheRows` **(Defensive Guards)** (Impact: 20.6)
  * `testLoginCookies` **(Defensive Guards)** (Impact: 19.9)
  * `updateToken` **(Many-Argument Workhorses)** (Impact: 16.4)
    * *Intent:* /** * 代码清单 2-2、2-9 管理令牌-更新令牌 */
  * `canCache` **(Defensive Guards)** (Impact: 15.1)
    * *Intent:* /** * 代码清单 2-11 */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (97.2%), Complexity Load (formerly Cognitive Load) (25.6%), Connectivity (formerly Api Exposure) (2.8%)
- **Documentation Coverage:** 6.4516% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `saveRankWithRegions` **(Many-Argument Workhorses)** (Impact: 21.8)
    * *Intent:* /** * 更新【分区】排行榜 * * @param member 榜单成员 * @param score 榜单成员分值 */
  * `getRankElementListWithRegions` **(Many-Argument Workhorses)** (Impact: 20.2)
    * *Intent:* /** * 根据从总排名的范围获取元素列表 * * @param begin 总排名中的起始位置 * @param end 总排名中的结束位置 * @param isAsc true：从低到高 / f...
  * `getRankByMemberWithRegions` **(Type Conversions)** (Impact: 19.0)
    * *Intent:* /** * 根据 member，查询成员在排行榜中的排名，从 0 开始计数 * <p> * 如果成员不在排行榜，则统一返回 {@link #TOTAL_RANK_LENGTH} * * @param ...
  * `deleteWithAutoAdjust` **(Stateful Encapsulated Methods)** (Impact: 18.1)
    * *Intent:* /** * 先将原始排名记录从所属分区中删除，并动态调整之后的分区 */
  * `addWithAutoAdjust` **(Many-Argument Workhorses)** (Impact: 15.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (99.4%), Guard Balance (formerly Safety Score) (97.0%), Complexity Load (formerly Cognitive Load) (84.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `cache_rows` **(Compute Cores)** (Impact: 8.4)
    * *Intent:* # <end id="_1311_14471_8287"/> # 代码清单 2-8 # <start id="_1311_14471_8292"/>
  * `can_cache` **(Compute Cores)** (Impact: 7.4)
    * *Intent:* # <end id="_1311_14471_8288"/> # 代码清单 2-11 # <start id="_1311_14471_8289"/> # 尝试从页面里面取出商品ID。 item_id...
  * `add_to_cart` **(Compute Cores)** (Impact: 7.1)
    * *Intent:* # <end id="_1311_14471_8270"/> # 代码清单 2-4 # <start id="_1311_14471_8279"/>
  * `cache_request` **(Compute Cores)** (Impact: 6.9)
    * *Intent:* # <end id="_1311_14471_8271"/> # 代码清单 2-6 # <start id="_1311_14471_8291"/> # 对于不能被缓存的请求，直接调用回调函数。 if...
  * `clean_sessions` **(Compute Cores)** (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **28**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (88.5%), Guard Balance (formerly Safety Score) (80.0%), Concurrency Surface (formerly Concurrency) (22.7%), Complexity Load (formerly Cognitive Load) (15.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createIndex` **(Stateful Encapsulated Methods)** (Impact: 40.2)
  * `onResponse` **(Compute Cores)** (Impact: 10.3)
  * `asyncSaveBatch` **(Defensive Guards)** (Impact: 9.4)
  * `asyncUpdateBatchIds` **(Defensive Guards)** (Impact: 9.4)
  * `asyncDeleteBatchIds` **(Defensive Guards)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **24**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (69.2%), Complexity Load (formerly Cognitive Load) (22.9%), Connectivity (formerly Api Exposure) (8.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `pojoPageByLastId` **(Annotated & Test Methods)** (Impact: 9.9)
  * `pojoPageByLastIdInDay` **(Annotated & Test Methods)** (Impact: 9.9)
  * `pojoScroll` **(Annotated & Test Methods)** (Impact: 9.0)
  * `pojoScrollInDay` **(Annotated & Test Methods)** (Impact: 9.0)
  * `batchSave` **(Annotated & Test Methods)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 20 instances
* *Api Near Db Sink:* 8 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (95.5%), Guard Balance (formerly Safety Score) (38.0%), Connectivity (formerly Api Exposure) (10.8%), Complexity Load (formerly Cognitive Load) (10.2%)
- **Documentation Coverage:** 87.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `asyncSaveBatchInDay` **(Defensive Guards)** (Impact: 10.7)
  * `asyncUpdateBatchIdsInDay` **(Defensive Guards)** (Impact: 10.7)
  * `saveInDay` **(Defensive Guards)** (Impact: 9.4)
    * *Intent:* /** * 根据日期动态选择索引并更新 * * @param day 日期，格式为：yyyy-MM-dd * @param entity 待更新的数据 * @return / */
  * `saveBatchInDay` **(Defensive Guards)** (Impact: 9.4)
    * *Intent:* /** * 根据日期动态选择索引并批量更新 * * @param day 日期，格式为：yyyy-MM-dd * @param list 待更新的数据 * @return / */
  * `deleteByIdInDay` **(Defensive Guards)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
* *Api Near Db Sink:* 1 instances
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.4%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (77.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `process_logs` **(Many-Argument Workhorses)** (Impact: 16.6)
    * *Intent:* # 代码清单 4-2 # <start id="process-logs-progress"/> # 日志处理函数接受的其中一个参数为回调函数， # 这个回调函数接受一个Redis连接和一个日志行作为...
  * `purchase_item` **(Many-Argument Workhorses)** (Impact: 11.5)
    * *Intent:* # <end id="_1313_14472_8342"/> # 代码清单 4-6 # <start id="_1313_14472_8353"/>
  * `wait_for_sync` **(Compute Cores)** (Impact: 9.9)
    * *Intent:* # <end id="process-logs-progress"/> # 代码清单 4-3 # <start id="wait-for-sync"/>
  * `list_item` **(Many-Argument Workhorses)** (Impact: 8.2)
    * *Intent:* # 代码清单 4-5 # <start id="_1313_14472_8342"/>
  * `benchmark_update_token` **(Compute Cores)** (Impact: 5.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 2.628; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (91.3%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (65.5%), Complexity Load (formerly Cognitive Load) (37.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testPurchaseItem` **(Defensive Guards)** (Impact: 27.3)
  * `purchaseItem` **(Many-Argument Workhorses)** (Impact: 26.2)
  * `testListItem` **(Defensive Guards)** (Impact: 22.2)
  * `updateTokenPipeline` **(Compute Cores)** (Impact: 16.3)
  * `updateToken` **(Compute Cores)** (Impact: 16.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (92.4%), Complexity Load (formerly Cognitive Load) (66.0%), Connectivity (formerly Api Exposure) (7.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `clear` **(Annotated & Test Methods)** (Impact: 6.4)
  * `test04` **(Annotated & Test Methods)** (Impact: 6.3)
  * `test05` **(Annotated & Test Methods)** (Impact: 6.3)
  * `test06` **(Annotated & Test Methods)** (Impact: 6.2)
  * `test02` **(Annotated & Test Methods)** (Impact: 5.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (64.0%), Complexity Load (formerly Cognitive Load) (19.4%), Connectivity (formerly Api Exposure) (6.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `onResponse` **(Compute Cores)** (Impact: 10.3)
  * `pojoScroll` **(I/O & Config Routines)** (Impact: 9.9)
  * `pojoPageByLastId` **(Callbacks & Closures)** (Impact: 9.8)
  * `deleteIndex` **(Stateful Encapsulated Methods)** (Impact: 6.8)
  * `saveBatch` **(Encapsulated Accessors)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 2.628; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (99.7%), Guard Balance (formerly Safety Score) (96.9%), Debt Markers (formerly Tech Debt) (89.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `article_vote` **(Defensive Guards)** (Impact: 9.1)
    * *Intent:* # <end id="exercise-fix-article-vote"/> # 从技术上来将，上面的 article_vote() 函数仍然有一些问题， # 这些问题可以通过下面展示的这段代码来解...
  * `add_to_cart` **(Compute Cores)** (Impact: 7.1)
  * `article_vote` **(Many-Argument Workhorses)** (Impact: 6.9)
    * *Intent:* # <start id="exercise-fix-article-vote"/> # 在进行投票之前，先检查这篇文章是否仍然处于可投票的时间之内 cutoff = time.time() - ONE...
  * `update_token` **(Many-Argument Workhorses)** (Impact: 5.1)
