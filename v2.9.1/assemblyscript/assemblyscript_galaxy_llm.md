...
## 1. EXECUTIVE SUMMARY
- **Scope:** 710 analyzed artifact(s), 90476 LOC.
- **Load-bearing artifact:** `std/assembly/util/error.ts` -- 16 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/index-js.ts` -- pulls in 16 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/compiler.ts` at magnitude 8223.54 (structural weight, not risk).
...
| Avg Path Length | 1.6683 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `2.826`
> **Composition Archetype:** `Mid Flat Project` (z +2.83; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 47%, Declarative / Non-Code 13%, Parameter Forwarders Files 10%, Interface Declarations Files 7%, State Mutators Files 6%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 43.1 | 0.0 | 0.0 |
...
| connectivity | 5100 | 270 | 10 | `std/assembly/builtins.ts` |
...
| dead_code | 1204 | 182 | 3 | `std/assembly/index.d.ts` |
| credential | 5 | 2 | 0 | `tests/compiler/std/string-casemapping.ts` |
| threat | 473 | 35 | 0 | `src/compiler.ts` |
...
- `replaceChild` **(Many-Argument Workhorses)** (@ `src/passes/pass.ts`) -> Impact: **425.9** | LOC: 799
  * *Intent:* /** Replaces an expression within a parent expression. Returns the replaced expression on success, otherwise `0`. */
- `main` **(Many-Argument Workhorses)** (@ `cli/index.js`) -> Impact: **411.8** | LOC: 961
  * *Intent:* /** Runs the command line utility using the specified arguments array. */
- `compileBinaryExpression` **(Many-Argument Workhorses)** (@ `src/compiler.ts`) -> Impact: **351.4** | LOC: 747
- `parseClassMember` **(Many-Argument Workhorses)** (@ `src/parser.ts`) -> Impact: **296.4** | LOC: 559
- `unsafeNext` **(Many-Argument Workhorses)** (@ `src/tokenizer.ts`) -> Impact: **275.1** | LOC: 445
- `convertExpression` **(Many-Argument Workhorses)** (@ `src/compiler.ts`) -> Impact: **247.6** | LOC: 249
  * *Intent:* /** Converts an expression's result from one type to another. */
- `canOverflow` **(Many-Argument Workhorses)** (@ `src/flow.ts`) -> Impact: **225.8** | LOC: 256
...
- `compileUnaryPrefixExpression` **(Many-Argument Workhorses)** (@ `src/compiler.ts`) -> Impact: **205.8** | LOC: 355
- `resolveBinaryExpression` **(Many-Argument Workhorses)** (@ `src/resolver.ts`) -> Impact: **180.3** | LOC: 207
  * *Intent:* /** Resolves a binary expression to its static type. */
- `build` **(I/O & Config Routines)** (@ `src/bindings/js.ts`) -> Impact: **179.3** | LOC: 507
...
| `std/assembly` | 34 | 9369.31 | 19.7% | 59.7% |
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 29.8779% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `compileBinaryExpression` **(Many-Argument Workhorses)** (Impact: 351.4)
  * `convertExpression` **(Many-Argument Workhorses)** (Impact: 247.6)
    * *Intent:* /** Converts an expression's result from one type to another. */
  * `compileUnaryPrefixExpression` **(Many-Argument Workhorses)** (Impact: 205.8)
  * `makePow` **(Many-Argument Workhorses)** (Impact: 145.9)
  * `compileIdentifierExpression` **(Many-Argument Workhorses)** (Impact: 137.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (84.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 53.7989% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `builtin_assert` **(Compute Cores)** (Impact: 122.6)
    * *Intent:* // assert<T?>(isTrueish: T, message?: string) -> T{!= null}
  * `builtin_v128_shuffle` **(Compute Cores)** (Impact: 46.5)
    * *Intent:* // v128.shuffle<T!>(a: v128, b: v128, ...lanes: u8[]) -> v128
  * `ensureVisitMembersOf` **(Many-Argument Workhorses)** (Impact: 43.2)
    * *Intent:* /** Ensures that the visitor function of the specified class is compiled. */
  * `builtin_v128_load_lane` **(Compute Cores)** (Impact: 39.5)
    * *Intent:* // v128.load_lane<TFrom!>(ptr: usize, vec: v128, idx: u8, immOffset?: usize, immAlign?: usize) -> v1...
  * `builtin_v128_store_lane` **(Compute Cores)** (Impact: 39.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (69.8%)
- **Documentation Coverage:** 95.3368% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `pow` **(Many-Argument Workhorses)** (Impact: 137.3)
  * `atan2` **(Compute Cores)** (Impact: 54.2)
  * `atan2` **(Compute Cores)** (Impact: 54.1)
  * `rem` **(Compute Cores)** (Impact: 48.4)
  * `rem` **(Compute Cores)** (Impact: 48.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 1.143; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (51.7%)
- **Documentation Coverage:** 41.1456% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseClassMember` **(Many-Argument Workhorses)** (Impact: 296.4)
  * `parseExpressionStart` **(Compute Cores)** (Impact: 158.1)
    * *Intent:* // expressions
  * `parseExpression` **(Many-Argument Workhorses)** (Impact: 144.3)
  * `parseType` **(Many-Argument Workhorses)** (Impact: 133.9)
    * *Intent:* /** Parses a type. */
  * `parseTopLevelStatement` **(Many-Argument Workhorses)** (Impact: 131.9)
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **15**; blast radius 21.18; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (77.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (68.3%)
- **Documentation Coverage:** 1.9231% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `initialize` **(I/O & Config Routines)** (Impact: 137.8)
    * *Intent:* /** Initializes the program and its elements prior to compilation. */
  * `fromDecorator` **(Compute Cores)** (Impact: 107.8)
    * *Intent:* /** Returns the operator kind represented by the specified decorator and string argument. */
  * `doProcessOverride` **(Many-Argument Workhorses)** (Impact: 72.6)
    * *Intent:* /** Processes a single overridden member by this class in a base class. */
  * `writeField` **(Many-Argument Workhorses)** (Impact: 61.6)
    * *Intent:* /** Writes a field value to a buffer and returns the number of bytes written. */
  * `tryMerge` **(Many-Argument Workhorses)** (Impact: 60.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.5e-05 | `Ripple Effect (Closeness):` 0.020617
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **9**; blast radius 7.729; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.2%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (70.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (68.3%)
- **Documentation Coverage:** 3.5687% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolveBinaryExpression` **(Many-Argument Workhorses)** (Impact: 180.3)
    * *Intent:* /** Resolves a binary expression to its static type. */
  * `finishResolveClass` **(Many-Argument Workhorses)** (Impact: 145.2)
    * *Intent:* /** Finishes resolving the specified class. */
  * `resolveFunction` **(Many-Argument Workhorses)** (Impact: 131.2)
    * *Intent:* // ==================================================== Elements ===================================...
  * `lookupPropertyAccessExpression` **(Many-Argument Workhorses)** (Impact: 130.7)
    * *Intent:* /** Looks up the program element the specified property access expression refers to. */
  * `resolveNamedType` **(Many-Argument Workhorses)** (Impact: 114.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **8**; blast radius 5.188; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%), Complexity Load (formerly Cognitive Load) (87.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 68.4211% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `build` **(I/O & Config Routines)** (Impact: 179.3)
  * `makeLowerToValue` **(Many-Argument Workhorses)** (Impact: 115.5)
    * *Intent:* /** Lowers a JavaScript value to a WebAssembly value, as an expression. */
  * `makeLiftFromValue` **(Many-Argument Workhorses)** (Impact: 59.6)
    * *Intent:* /** Lifts a WebAssembly value to a JavaScript value, as an expression. */
  * `makeFunctionImport` **(Many-Argument Workhorses)** (Impact: 40.2)
  * `visitFunction` **(Many-Argument Workhorses)** (Impact: 37.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.5e-05 | `Ripple Effect (Closeness):` 0.011314
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (99.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (72.0%)
- **Documentation Coverage:** 53.1489% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `optimize` **(Many-Argument Workhorses)** (Impact: 124.9)
  * `binary` **(Compute Cores)** (Impact: 109.9)
  * `prepareType` **(Many-Argument Workhorses)** (Impact: 76.3)
    * *Intent:* /** Recursively prepares the given GC type, potentially returning a temporary type. */
  * `tryEnsureBasicType` **(Compute Cores)** (Impact: 40.5)
    * *Intent:* /** Obtains the basic type of the given type, if any. */
  * `leastUpperBound` **(Compute Cores)** (Impact: 38.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 1.143; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Connectivity (formerly Api Exposure) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (50.3%)
- **Documentation Coverage:** 99.9127% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `trace` **(Many-Argument Workhorses)** (Impact: 20.2)
  * `abort` **(Compute Cores)** (Impact: 11.5)
  * `load_lane` **(Generic / Templated Code)** (Impact: 7.4)
  * `store_lane` **(Generic / Templated Code)** (Impact: 7.4)
  * `load8_lane` **(Compute Cores)** (Impact: 7.4)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 1.143; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.4%)
- **Documentation Coverage:** 52.9473% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `unsafeNext` **(Many-Argument Workhorses)** (Impact: 275.1)
  * `tokenFromKeyword` **(Compute Cores)** (Impact: 158.4)
  * `operatorTokenToString` **(Compute Cores)** (Impact: 77.8)
  * `readEscapeSequence` **(Compute Cores)** (Impact: 43.9)
  * `tokenIsAlsoIdentifier` **(Compute Cores)** (Impact: 30.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.9%), Complexity Load (formerly Cognitive Load) (84.7%)
- **Documentation Coverage:** 43.0963% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(Many-Argument Workhorses)** (Impact: 411.8)
    * *Intent:* /** Runs the command line utility using the specified arguments array. */
  * `prepareResult` **(Many-Argument Workhorses)** (Impact: 57.1)
    * *Intent:* // Prepares the result object
  * `getFile` **(Compute Cores)** (Impact: 52.1)
    * *Intent:* // Gets the file matching the specified source path, imported at the given dependee path
  * `checkDiagnostics` **(Many-Argument Workhorses)** (Impact: 43.7)
    * *Intent:* /** Checks diagnostics emitted so far for errors. */
  * `getConfig` **(Defensive Guards)** (Impact: 26.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **7**; blast radius 1.368; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (69.2%)
- **Documentation Coverage:** 97.4359% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `visitNode` **(Type Conversions)** (Impact: 104.4)
  * `visitClassDeclaration` **(Many-Argument Workhorses)** (Impact: 43.3)
  * `visitFunctionCommon` **(Compute Cores)** (Impact: 35.0)
  * `visitInterfaceDeclaration` **(Compute Cores)** (Impact: 21.1)
  * `visitNamespaceDeclaration` **(Compute Cores)** (Impact: 17.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **2**; blast radius 2.557; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (77.5%), Complexity Load (formerly Cognitive Load) (63.0%)
- **Documentation Coverage:** 83.1776% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `replaceChild` **(Many-Argument Workhorses)** (Impact: 425.9)
    * *Intent:* /** Replaces an expression within a parent expression. Returns the replaced expression on success, o...
  * `visit` **(Compute Cores)** (Impact: 174.4)
    * *Intent:* // Delegate /** Visits any expression, delegating to the respective visitor methods. */
  * `replaceCurrent` **(Compute Cores)** (Impact: 6.3)
    * *Intent:* // Utility /** Replaces the current expression with the specified replacement. */
  * `walkFunction` **(Compute Cores)** (Impact: 3.2)
    * *Intent:* /** Walks a specific function. */
  * `walkGlobal` **(Parameter Forwarders)** (Impact: 3.1)
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **9**; blast radius 9.91; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.1%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (62.0%)
- **Documentation Coverage:** 6.4237% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `canOverflow` **(Many-Argument Workhorses)** (Impact: 225.8)
    * *Intent:* /** * Tests if an expression can possibly overflow in the context of this flow. Assumes that the * e...
  * `inheritAlternatives` **(Many-Argument Workhorses)** (Impact: 102.4)
    * *Intent:* /** Inherits two alternate branches to become this flow, i.e. then with else. */
  * `inheritNonnullIfTrue` **(Many-Argument Workhorses)** (Impact: 78.3)
    * *Intent:* /** Updates local states to reflect that this branch is only taken when `expr` is true-ish. */
  * `inheritNonnullIfFalse` **(Compute Cores)** (Impact: 65.1)
    * *Intent:* /** Updates local states to reflect that this branch is only taken when `expr` is false-ish. */
  * `mergeSideEffects` **(Compute Cores)** (Impact: 41.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (50.4%), Mutation Surface (formerly State Flux) (41.9%)
- **Documentation Coverage:** 24.6532% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fromNode` **(Compute Cores)** (Impact: 53.0)
  * `hasGenericComponent` **(Compute Cores)** (Impact: 27.0)
    * *Intent:* /** Tests if this type has a generic component matching one of the given type parameters. */
  * `lineAt` **(Compute Cores)** (Impact: 15.5)
    * *Intent:* /** Determines the line number at the specified position. Starts at `1`. */
  * `constructor` **(Many-Argument Workhorses)** (Impact: 14.6)
    * *Intent:* /** Represents an `export` statement. */
  * `compilesToConst` **(Compute Cores)** (Impact: 10.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (96.0%), Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (64.3%), Guard Balance (formerly Safety Score) (61.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `SET` **(Generic / Templated Code)** (Impact: 49.0)
  * `INCLUDES` **(Generic / Templated Code)** (Impact: 19.1)
  * `WRAP` **(Generic / Templated Code)** (Impact: 17.6)
  * `LAST_INDEX_OF` **(Generic / Templated Code)** (Impact: 14.8)
  * `INDEX_OF` **(Generic / Templated Code)** (Impact: 12.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **2**; blast radius 3.087; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (72.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `genDigits` **(Many-Argument Workhorses)** (Impact: 56.6)
  * `itoa_buffered` **(Many-Argument Workhorses)** (Impact: 36.8)
  * `prettify` **(Many-Argument Workhorses)** (Impact: 32.8)
  * `itoa64` **(Generic / Templated Code)** (Impact: 22.4)
  * `utoa64` **(Generic / Templated Code)** (Impact: 18.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.143; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (63.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `encodeUnsafe` **(Many-Argument Workhorses)** (Impact: 37.0)
  * `split` **(Generic / Templated Code)** (Impact: 30.4)
  * `replaceAll` **(Many-Argument Workhorses)** (Impact: 30.2)
  * `decodeUnsafe` **(Many-Argument Workhorses)** (Impact: 30.1)
  * `toLowerCase` **(I/O & Config Routines)** (Impact: 21.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **6**; blast radius 1.972; role: Transceiver (Middle-Tier)
...
**Top Internal Functions/Classes:**
  * `toTypeScriptType` **(Compute Cores)** (Impact: 87.6)
  * `build` **(Compute Cores)** (Impact: 21.1)
  * `visitFunction` **(Compute Cores)** (Impact: 16.2)
  * `makeRecordType` **(Compute Cores)** (Impact: 13.6)
  * `isPlainObject` **(Compute Cores)** (Impact: 12.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6e-06 | `Ripple Effect (Closeness):` 0.002805
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **1**; blast radius 7.221; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (92.2%), Guard Balance (formerly Safety Score) (78.7%), Connectivity (formerly Api Exposure) (66.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `normalizeStringPosix` **(Defensive Guards)** (Impact: 60.4)
    * *Intent:* // Resolves . and .. elements in a path with directory names
  * `relative` **(Defensive Guards)** (Impact: 58.2)
  * `basename` **(Defensive Guards)** (Impact: 50.3)
  * `parse` **(Defensive Guards)** (Impact: 47.6)
  * `extname` **(Defensive Guards)** (Impact: 26.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.3e-05 | `Ripple Effect (Closeness):` 0.014025
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 1.143; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (79.1%), Complexity Load (formerly Cognitive Load) (44.3%), Concurrency Surface (formerly Concurrency) (33.3%), Connectivity (formerly Api Exposure) (12.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `instantiate` **(Defensive Guards)** (Impact: 138.7)
  * `__lowerStaticArray` **(Many-Argument Workhorses)** (Impact: 12.9)
  * `__release` **(Defensive Guards)** (Impact: 10.3)
  * `__lowerRecord13` **(Compute Cores)** (Impact: 9.7)
  * `arrayOfStringsFunction` **(Defensive Guards)** (Impact: 9.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (78.8%), Complexity Load (formerly Cognitive Load) (44.6%), Concurrency Surface (formerly Concurrency) (15.6%), Connectivity (formerly Api Exposure) (12.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `instantiate` **(Defensive Guards)** (Impact: 138.7)
  * `__lowerStaticArray` **(Many-Argument Workhorses)** (Impact: 12.9)
...
- `util/node.js` -> **Severity: 0.007** (Bridge: 0.0001 * Flux: 100.0%)
- `src/bindings/js.ts` -> **Severity: 0.007** (Bridge: 0.0001 * Flux: 100.0%)
- `src/program.ts` -> **Severity: 0.007** (Bridge: 0.0001 * Flux: 99.9411%)
- `src/types.ts` -> **Severity: 0.007** (Bridge: 0.0001 * Flux: 90.7238%)
- `util/browser/url.js` -> **Severity: 0.004** (Bridge: 0.0 * Flux: 100.0%)
...
- `src/bindings/js.ts` -> **Severity: 1.127** (Embedded: 0.0113 * Error Risk: 99.6006%)
