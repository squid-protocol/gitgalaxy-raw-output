...
## 1. EXECUTIVE SUMMARY
- **Scope:** 171 analyzed artifact(s), 25641 LOC.
- **Load-bearing artifact:** `js/src/util/index.js` -- 31 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `scss/bootstrap.scss` -- pulls in 40 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `js/tests/unit/dropdown.spec.js` at magnitude 673.98 (structural weight, not risk).
...
| Avg Path Length | 1.7452 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Small Flat Repo (2)` (z +4.26; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 45%, Data / Markup / Trivial 19%, Callbacks & Closures Files 12%, Large Core Modules (3) 9%, Large Core Modules 5%
...
| Connectivity (formerly API Exposure) | 0.0 | 81.7 | 8.0 | 0.0 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 16.8 | 0.2 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 35.7 | 0.0 | 0.0 |
| Credential Material (formerly Hardcoded Payload Artifacts) | 0.0 | 99.9 | 0.6 | 0.0 | 0.0 |
...
- `onSlid2` **(Compute Cores)** (@ `js/tests/unit/carousel.spec.js`) -> Impact: **63.0** | LOC: 779
- `expectDone` **(I/O & Config Routines)** (@ `js/tests/unit/tab.spec.js`) -> Impact: **61.2** | LOC: 904
- `addHandler` **(Many-Argument Workhorses)** (@ `js/src/dom/event-handler.js`) -> Impact: **34.0** | LOC: 44
- `parseSelector` **(Defensive Guards)** (@ `js/src/util/index.js`) -> Impact: **31.0** | LOC: 83
  * *Intent:* /** * Properly escape IDs selectors to handle weird IDs */
- `getActiveId` **(I/O & Config Routines)** (@ `js/tests/unit/carousel.spec.js`) -> Impact: **30.8** | LOC: 416
- `off` **(Defensive Guards)** (@ `js/src/dom/event-handler.js`) -> Impact: **28.6** | LOC: 35
- `el` **(Callbacks & Closures)** (@ `js/tests/unit/util/index.spec.js`) -> Impact: **28.3** | LOC: 199
- `expectedDone` **(Callbacks & Closures)** (@ `js/tests/unit/tooltip.spec.js`) -> Impact: **26.4** | LOC: 347
- `clearMenus` **(Defensive Guards)** (@ `js/src/dropdown.js`) -> Impact: **25.9** | LOC: 37
- `restorePointerEvents` **(Callbacks & Closures)** (@ `js/tests/unit/carousel.spec.js`) -> Impact: **24.6** | LOC: 272
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (84.4%), Complexity Load (formerly Cognitive Load) (40.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test` **(Many-Argument Workhorses)** (Impact: 18.2)
  * `expectDropdownToBeOpened` **(Callbacks & Closures)** (Impact: 13.3)
  * `handleArrowUp` **(Callbacks & Closures)** (Impact: 6.3)
  * `expectDropdownToBeOpened` **(Callbacks & Closures)** (Impact: 2.7)
  * `expectDropdownToBeOpened` **(Callbacks & Closures)** (Impact: 2.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (93.7%), Complexity Load (formerly Cognitive Load) (45.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expectedDone` **(Callbacks & Closures)** (Impact: 26.4)
  * `assertDone` **(Callbacks & Closures)** (Impact: 14.3)
  * `tip` **(I/O & Config Routines)** (Impact: 13.6)
  * `firstCallback` **(Callbacks & Closures)** (Impact: 9.1)
  * `title` **(Callbacks & Closures)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (89.2%), Complexity Load (formerly Cognitive Load) (23.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `onSlid2` **(Compute Cores)** (Impact: 63.0)
  * `getActiveId` **(I/O & Config Routines)** (Impact: 30.8)
  * `restorePointerEvents` **(Callbacks & Closures)** (Impact: 24.6)
  * `doneTest` **(Callbacks & Closures)** (Impact: 2.8)
  * `onSlide2` **(Callbacks & Closures)** (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (91.5%), Complexity Load (formerly Cognitive Load) (38.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expectedDone` **(Callbacks & Closures)** (Impact: 19.1)
  * `shownCallback` **(Callbacks & Closures)** (Impact: 15.7)
  * `showListener` **(Callbacks & Closures)** (Impact: 12.3)
  * `hideCallback` **(Callbacks & Closures)** (Impact: 10.9)
  * `hideListener` **(Callbacks & Closures)** (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (85.5%), Complexity Load (formerly Cognitive Load) (20.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expectDone` **(I/O & Config Routines)** (Impact: 61.2)
  * `expectDone` **(Callbacks & Closures)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (94.5%), Complexity Load (formerly Cognitive Load) (38.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expectEnd` **(Callbacks & Closures)** (Impact: 22.6)
  * `expectEnd` **(Callbacks & Closures)** (Impact: 10.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **7**; blast radius 13.262; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (67.5%), Connectivity (formerly Api Exposure) (49.1%), Complexity Load (formerly Cognitive Load) (37.2%)
- **Documentation Coverage:** 98.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `show` **(I/O & Config Routines)** (Impact: 13.8)
  * `_setListeners` **(Defensive Guards)** (Impact: 13.1)
  * `hide` **(I/O & Config Routines)** (Impact: 9.1)
  * `_configAfterMerge` **(Defensive Guards)** (Impact: 8.1)
  * `_getConfig` **(Defensive Guards)** (Impact: 8.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (82.1%), Complexity Load (formerly Cognitive Load) (33.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expectEnd` **(Callbacks & Closures)** (Impact: 17.6)
  * `expectEnd` **(Callbacks & Closures)** (Impact: 9.1)
  * `target2Shown` **(Callbacks & Closures)** (Impact: 9.1)
  * `el` **(Callbacks & Closures)** (Impact: 7.3)
  * `handlerNestedCollapseOne` **(Callbacks & Closures)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **28** in-repo importer(s); it depends on **1**; blast radius 41.137; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (62.6%), Connectivity (formerly Api Exposure) (56.5%), Complexity Load (formerly Cognitive Load) (44.6%)
- **Documentation Coverage:** 96.7742% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `addHandler` **(Many-Argument Workhorses)** (Impact: 34.0)
  * `off` **(Defensive Guards)** (Impact: 28.6)
  * `trigger` **(Many-Argument Workhorses)** (Impact: 19.9)
  * `wrapFunction` **(Compute Cores)** (Impact: 15.8)
  * `bootstrapDelegationHandler` **(Defensive Guards)** (Impact: 13.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.0%), Guard Balance (formerly Safety Score) (86.9%), Complexity Load (formerly Cognitive Load) (23.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expectToBeCloseToArray` **(Many-Argument Workhorses)** (Impact: 18.7)
    * *Intent:* // See https://stackoverflow.com/a/45592926
  * `testElementIsActiveAfterScroll` **(Callbacks & Closures)** (Impact: 12.9)
  * `active` **(Callbacks & Closures)** (Impact: 4.3)
  * `activeId` **(Callbacks & Closures)** (Impact: 3.6)
  * `getElementScrollSpy` **(Parameter Forwarders)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (92.0%), Complexity Load (formerly Cognitive Load) (29.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `assertDone` **(Callbacks & Closures)** (Impact: 23.5)
  * `expected` **(Callbacks & Closures)** (Impact: 8.8)
  * `assertDone` **(Callbacks & Closures)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **6**; blast radius 5.218; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (89.4%), Concurrency Surface (formerly Concurrency) (66.1%), Guard Balance (formerly Safety Score) (53.8%), Connectivity (formerly Api Exposure) (43.1%)
- **Documentation Coverage:** 97.3684% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_slide` **(Many-Argument Workhorses)** (Impact: 20.9)
  * `triggerEvent` **(Compute Cores)** (Impact: 10.5)
  * `to` **(Defensive Guards)** (Impact: 9.5)
  * `jQueryInterface` **(Defensive Guards)** (Impact: 9.4)
    * *Intent:* // Static
  * `_directionToOrder` **(Defensive Guards)** (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (87.1%), Complexity Load (formerly Cognitive Load) (48.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handler` **(Callbacks & Closures)** (Impact: 11.8)
  * `handler` **(Callbacks & Closures)** (Impact: 4.1)
  * `moveMouse` **(Callbacks & Closures)** (Impact: 3.8)
  * `handler` **(Callbacks & Closures)** (Impact: 2.2)
  * `oneListener` **(State Mutators)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **6**; blast radius 5.387; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (91.2%), Guard Balance (formerly Safety Score) (42.3%), Connectivity (formerly Api Exposure) (40.6%), Complexity Load (formerly Cognitive Load) (20.0%)
- **Documentation Coverage:** 92.3077% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `clearMenus` **(Defensive Guards)** (Impact: 25.9)
  * `dataApiKeydownHandler` **(Compute Cores)** (Impact: 16.1)
  * `_getPlacement` **(I/O & Config Routines)** (Impact: 9.4)
  * `show` **(I/O & Config Routines)** (Impact: 8.7)
  * `_completeHide` **(Compute Cores)** (Impact: 8.3)
    * *Intent:* // Private
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.978; role: Isolated/Orphan
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (7.3%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (75.7%), Concurrency Surface (formerly Concurrency) (71.0%), Complexity Load (formerly Cognitive Load) (26.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `el` **(Callbacks & Closures)** (Impact: 28.3)
  * `functionBar` **(Callbacks & Closures)** (Impact: 20.6)
  * `el` **(Callbacks & Closures)** (Impact: 3.9)
  * `functionFoo` **(Parameter Forwarders)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 21 instances
...
- **Blast Radius:** changing it is visible to **31** in-repo importer(s); blast radius 105.611; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Connectivity (formerly Api Exposure) (47.6%), Guard Balance (formerly Safety Score) (37.3%), Complexity Load (formerly Cognitive Load) (29.4%)
- **Documentation Coverage:** 65.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseSelector` **(Defensive Guards)** (Impact: 31.0)
    * *Intent:* /** * Properly escape IDs selectors to handle weird IDs */
  * `getNextActiveElement` **(Many-Argument Workhorses)** (Impact: 15.4)
    * *Intent:* /** * Return the previous/next element of a list. * */
  * `isVisible` **(Defensive Guards)** (Impact: 12.7)
  * `findShadowRoot` **(Defensive Guards)** (Impact: 9.7)
  * `isDisabled` **(Defensive Guards)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 10 instances
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **4**; blast radius 5.669; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (77.7%), Connectivity (formerly Api Exposure) (36.1%), Complexity Load (formerly Cognitive Load) (31.9%)
- **Documentation Coverage:** 95.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_observerCallback` **(Compute Cores)** (Impact: 14.6)
    * *Intent:* // The logic of selection
  * `activate` **(Compute Cores)** (Impact: 14.5)
  * `jQueryInterface` **(Defensive Guards)** (Impact: 7.8)
    * *Intent:* // Static
  * `_activateParents` **(Compute Cores)** (Impact: 6.5)
  * `_configAfterMerge` **(Defensive Guards)** (Impact: 6.3)
    * *Intent:* // Private
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 18 instances
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **8**; blast radius 7.472; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Guard Balance (formerly Safety Score) (68.7%), Connectivity (formerly Api Exposure) (41.8%), Complexity Load (formerly Cognitive Load) (34.2%)
- **Documentation Coverage:** 88.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_addEventListeners` **(Defensive Guards)** (Impact: 10.9)
  * `_adjustDialog` **(I/O & Config Routines)** (Impact: 7.8)
    * *Intent:* /** * The following methods are used to handle overflowing modals */
  * `_showElement` **(Compute Cores)** (Impact: 7.4)
  * `show` **(Compute Cores)** (Impact: 6.9)
  * `_triggerBackdropTransition` **(Callbacks & Closures)** (Impact: 6.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (92.2%), Complexity Load (formerly Cognitive Load) (30.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `content` **(Callbacks & Closures)** (Impact: 2.1)
  * `title` **(Parameter Forwarders)** (Impact: 1.6)
  * `content` **(Parameter Forwarders)** (Impact: 1.6)
  * `title` **(Callbacks & Closures)** (Impact: 1.1)
  * `title` **(Interface Declarations)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (82.5%), Complexity Load (formerly Cognitive Load) (52.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `isScrollBarHidden` **(I/O & Config Routines)** (Impact: 23.4)
    * *Intent:* // iOS, Android devices and macOS browsers hide scrollbar by default and show it only while scrollin...
  * `hasMarginAttr` **(I/O & Config Routines)** (Impact: 7.8)
  * `windowCalculations` **(Callbacks & Closures)** (Impact: 1.8)
  * `parseIntDecimal` **(Parameter Forwarders)** (Impact: 1.5)
  * `getPaddingX` **(Parameter Forwarders)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 16 instances
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **4**; blast radius 5.669; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.1%), Guard Balance (formerly Safety Score) (62.6%), Connectivity (formerly Api Exposure) (36.1%), Complexity Load (formerly Cognitive Load) (22.0%)
- **Documentation Coverage:** 89.4737% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Defensive Guards)** (Impact: 11.8)
  * `show` **(I/O & Config Routines)** (Impact: 10.7)
  * `hide` **(I/O & Config Routines)** (Impact: 9.0)
  * `jQueryInterface` **(Defensive Guards)** (Impact: 8.0)
    * *Intent:* // Static
  * `_addAriaAndCollapsedClass` **(Compute Cores)** (Impact: 5.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **4**; blast radius 6.232; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (64.4%), Guard Balance (formerly Safety Score) (53.1%), Connectivity (formerly Api Exposure) (24.4%), Debt Markers (formerly Tech Debt) (15.7%)
- **Documentation Coverage:** 95.2381% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_keydown` **(Defensive Guards)** (Impact: 9.6)
  * `jQueryInterface` **(Defensive Guards)** (Impact: 7.8)
    * *Intent:* // Static
  * `show` **(I/O & Config Routines)** (Impact: 7.1)
    * *Intent:* // Public
  * `_activate` **(Defensive Guards)** (Impact: 6.4)
    * *Intent:* // Private
  * `_deactivate` **(Defensive Guards)** (Impact: 6.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.978; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (99.5%), Complexity Load (formerly Cognitive Load) (12.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.978; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (52.3%), Connectivity (formerly Api Exposure) (5.2%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
