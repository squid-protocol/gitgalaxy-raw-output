...
## 1. EXECUTIVE SUMMARY
- **Scope:** 824 analyzed artifact(s), 192857 LOC.
- **Load-bearing artifact:** `packages/excalidraw/i18n.ts` -- 106 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `packages/excalidraw/components/App.tsx` -- pulls in 80 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `.env.production` at magnitude 5000.0 (structural weight, not risk).
...
| Avg Path Length | 3.2595 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `2.513`
> **Composition Archetype:** `Hub-Coupled App` (z +2.51; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 21%, Data / Markup / Trivial 18%, Callbacks & Closures Files 17%, Large Core Modules (3) 9%, Many-Argument Workhorses Files 8%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 97.0 | 29.7 | 20.6 | 0.0 |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 24.0 | 20.1 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 9.2 | 5.5 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 58.0 | 100.0 | 100.0 |
...
| credential | 59 | 13 | 0 | `packages/excalidraw/locales/ja-JP.json` |
...
- `bindOrUnbindBindingElementEdge` **(Many-Argument Workhorses)** (@ `packages/element/src/binding.ts`) -> Impact: **511.6** | LOC: 777
- `textPropertiesUpdated` **(Many-Argument Workhorses)** (@ `packages/excalidraw/wysiwyg/textWysiwyg.tsx`) -> Impact: **290.4** | LOC: 785
- `resizeMultipleElements` **(Many-Argument Workhorses)** (@ `packages/element/src/resizeElements.ts`) -> Impact: **261.0** | LOC: 352
- `addToRoot` **(Many-Argument Workhorses)** (@ `packages/excalidraw/renderer/staticSvgScene.ts`) -> Impact: **203.9** | LOC: 579
- `updateElbowArrowPoints` **(Many-Argument Workhorses)** (@ `packages/element/src/elbowArrow.ts`) -> Impact: **202.1** | LOC: 286
  * *Intent:* /** * */
- `generateDynamicAABBs` **(Many-Argument Workhorses)** (@ `packages/element/src/elbowArrow.ts`) -> Impact: **201.3** | LOC: 186
...
- `convertToExcalidrawElements` **(Many-Argument Workhorses)** (@ `packages/element/src/transform.ts`) -> Impact: **200.4** | LOC: 301
- `repairBinding` **(Many-Argument Workhorses)** (@ `packages/excalidraw/data/restore.ts`) -> Impact: **192.0** | LOC: 214
- `handleEndpointDrag` **(Many-Argument Workhorses)** (@ `packages/element/src/elbowArrow.ts`) -> Impact: **180.9** | LOC: 202
- `createIcon` **(Many-Argument Workhorses)** (@ `packages/excalidraw/components/icons.tsx`) -> Impact: **180.6** | LOC: 2468
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 2.157; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (89.8%), Guard Balance (formerly Safety Score) (84.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `craftInvokerFunction` **(Many-Argument Workhorses)** (Impact: 68.6)
  * `__embind_register_std_string` **(Compute Cores)** (Impact: 53.5)
  * `genericPointerToWireType` **(Many-Argument Workhorses)** (Impact: 43.8)
  * `__embind_register_class` **(Many-Argument Workhorses)** (Impact: 39.4)
  * `stringToUTF8Array` **(Many-Argument Workhorses)** (Impact: 38.3)
...
- **Magnitude:** 2820.18 | **LOC:** 12839 | **CtrlFlow:** 20.4% | **Authorship Centralization:** 43.5%
- **Blast Radius:** changing it is visible to **18** in-repo importer(s); it depends on **80**; blast radius 9.37; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Guard Balance (formerly Safety Score) (61.1%), Complexity Load (formerly Cognitive Load) (47.6%)
- **Documentation Coverage:** 91.7647% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handleDelayedBindModeChange` **(Many-Argument Workhorses)** (Impact: 136.7)
  * `effector` **(I/O & Config Routines)** (Impact: 80.9)
  * `maybeDragNewGenericElement` **(Many-Argument Workhorses)** (Impact: 45.9)
  * `handleAppOnDrop` **(Defensive Guards)** (Impact: 43.0)
  * `handleIframeLikeCenterClick` **(I/O & Config Routines)** (Impact: 37.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.012085 | `Ripple Effect (Closeness):` 0.069232
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.671; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (63.6%), Guard Balance (formerly Safety Score) (16.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.2%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (5.3%)
...
- **Blast Radius:** changing it is visible to **15** in-repo importer(s); it depends on **16**; blast radius 3.083; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (92.3%), Connectivity (formerly Api Exposure) (71.7%), Guard Balance (formerly Safety Score) (61.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (59.2%)
- **Documentation Coverage:** 86.1111% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `bindOrUnbindBindingElementEdge` **(Many-Argument Workhorses)** (Impact: 511.6)
  * `updateBoundPoint` **(Many-Argument Workhorses)** (Impact: 155.0)
  * `getBindingSideMidPoint` **(Many-Argument Workhorses)** (Impact: 106.5)
  * `bindPointToSnapToElementOutline` **(Many-Argument Workhorses)** (Impact: 75.7)
  * `snapToMid` **(Many-Argument Workhorses)** (Impact: 67.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000993 | `Ripple Effect (Closeness):` 0.026918
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **11**; blast radius 2.022; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (93.3%), Guard Balance (formerly Safety Score) (59.1%), Complexity Load (formerly Cognitive Load) (28.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (28.1%)
- **Documentation Coverage:** 66.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `updateElbowArrowPoints` **(Many-Argument Workhorses)** (Impact: 202.1)
    * *Intent:* /** * */
  * `generateDynamicAABBs` **(Many-Argument Workhorses)** (Impact: 201.3)
    * *Intent:* /** * Create dynamically resizing, always touching * bounding boxes having a minimum extent represen...
  * `handleEndpointDrag` **(Many-Argument Workhorses)** (Impact: 180.9)
  * `handleSegmentMove` **(Many-Argument Workhorses)** (Impact: 162.9)
    * *Intent:* /** * */
  * `estimateSegmentCount` **(Many-Argument Workhorses)** (Impact: 131.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00044 | `Ripple Effect (Closeness):` 0.021871
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 0.671; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (27.8%), Complexity Load (formerly Cognitive Load) (3.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **15** in-repo importer(s); it depends on **15**; blast radius 3.394; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (88.1%), Connectivity (formerly Api Exposure) (63.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (62.7%), Guard Balance (formerly Safety Score) (60.3%)
- **Documentation Coverage:** 87.7551% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handlePointDragging` **(Many-Argument Workhorses)** (Impact: 144.1)
  * `normalizeSelectedPoints` **(Many-Argument Workhorses)** (Impact: 139.8)
  * `movePoints` **(Many-Argument Workhorses)** (Impact: 85.1)
  * `handlePointerMove` **(Many-Argument Workhorses)** (Impact: 81.0)
  * `handlePointerDown` **(Many-Argument Workhorses)** (Impact: 71.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000579 | `Ripple Effect (Closeness):` 0.029162
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **13**; blast radius 1.137; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.5%), Guard Balance (formerly Safety Score) (63.9%), Complexity Load (formerly Cognitive Load) (44.1%), Connectivity (formerly Api Exposure) (41.1%)
- **Documentation Coverage:** 53.0303% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `filterInvisibleChanges` **(Many-Argument Workhorses)** (Impact: 79.5)
    * *Intent:* /** * Mutates `nextAppState` be filtering out state related to deleted elements. * */
  * `mergeBoundElements` **(Defensive Guards)** (Impact: 77.4)
  * `squash` **(Defensive Guards)** (Impact: 65.8)
  * `squash` **(Defensive Guards)** (Impact: 48.8)
  * `distinctKeysIterator` **(Many-Argument Workhorses)** (Impact: 42.7)
    * *Intent:* /** * Iterator comparing values of object properties based on the passed joining strategy. * * * WAR...
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000164 | `Ripple Effect (Closeness):` 0.003645
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **12**; blast radius 1.913; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (76.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (64.6%), Guard Balance (formerly Safety Score) (56.8%)
- **Documentation Coverage:** 92.5926% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderResetAutoResizeHandle` **(Many-Argument Workhorses)** (Impact: 174.0)
  * `renderBindingHighlightForBindableElement_simple` **(Many-Argument Workhorses)** (Impact: 139.1)
  * `renderBindingHighlightForBindableElement_complex` **(Many-Argument Workhorses)** (Impact: 127.0)
  * `renderLinearPointHandles` **(Many-Argument Workhorses)** (Impact: 86.8)
  * `addSelectionForGroupId` **(Compute Cores)** (Impact: 56.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8.3e-05 | `Ripple Effect (Closeness):` 0.007442
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **16**; blast radius 0.799; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (90.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (67.3%), Connectivity (formerly Api Exposure) (38.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resizeMultipleElements` **(Many-Argument Workhorses)** (Impact: 261.0)
  * `resizeSingleElement` **(Many-Argument Workhorses)** (Impact: 115.3)
  * `getResizeAnchor` **(Many-Argument Workhorses)** (Impact: 61.5)
  * `getNextMultipleWidthAndHeightFromPointer` **(Many-Argument Workhorses)** (Impact: 54.5)
  * `transformElements` **(Many-Argument Workhorses)** (Impact: 54.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.1e-05 | `Ripple Effect (Closeness):` 0.00243
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 0.671; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (76.3%), Concurrency Surface (formerly Concurrency) (69.1%)
- **Documentation Coverage:** 76.8786% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `isShallowEqual` **(Many-Argument Workhorses)** (Impact: 46.3)
    * *Intent:* /** * Returns whether object/array is shallow equal. * Considers empty object/arrays as equal (wheth...
  * `escapeDoubleQuotes` **(Defensive Guards)** (Impact: 34.4)
    * *Intent:* /** * use when you need to render unsafe string as HTML attribute, but MAKE SURE * the attribute is ...
  * `normalizeEOL` **(Generic / Templated Code)** (Impact: 25.6)
  * `updateActiveTool` **(Defensive Guards)** (Impact: 22.5)
  * `addEventListener` **(Defensive Guards)** (Impact: 16.9)
    * *Intent:* // implem
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **5**; blast radius 1.984; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.7%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (75.7%), Connectivity (formerly Api Exposure) (58.8%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `snapResizingElements` **(Many-Argument Workhorses)** (Impact: 81.0)
  * `getGapSnaps` **(Many-Argument Workhorses)** (Impact: 70.8)
  * `getElementsCorners` **(Many-Argument Workhorses)** (Impact: 39.9)
  * `createGapSnapLines` **(Many-Argument Workhorses)** (Impact: 39.7)
  * `getPointSnaps` **(Many-Argument Workhorses)** (Impact: 34.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 0.671; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (54.2%), Complexity Load (formerly Cognitive Load) (3.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **19**; blast radius 0.911; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (54.5%), Guard Balance (formerly Safety Score) (54.2%), Connectivity (formerly Api Exposure) (51.0%), Mutation Surface (formerly State Flux) (29.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `perform` **(Many-Argument Workhorses)** (Impact: 92.4)
  * `perform` **(Many-Argument Workhorses)** (Impact: 65.2)
  * `getFontFamily` **(Many-Argument Workhorses)** (Impact: 50.5)
  * `getFormValue` **(Many-Argument Workhorses)** (Impact: 26.4)
  * `perform` **(Many-Argument Workhorses)** (Impact: 22.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000444 | `Ripple Effect (Closeness):` 0.039964
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 0.671; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (87.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (66.7%), Connectivity (formerly Api Exposure) (39.7%)
- **Documentation Coverage:** 73.0769% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getArrowheadShapes` **(Many-Argument Workhorses)** (Impact: 136.2)
  * `_generateElementShape` **(Many-Argument Workhorses)** (Impact: 84.5)
    * *Intent:* /** * Generates the roughjs shape for given element. * * Low-level. Use `ShapeCache.generateElementS...
  * `generateRoughOptions` **(Many-Argument Workhorses)** (Impact: 47.7)
  * `getElementShape` **(Many-Argument Workhorses)** (Impact: 41.4)
    * *Intent:* /** * get the pure geometric shape of an excalidraw elementw * which is then used for hit detection ...
  * `generateElbowArrowShape` **(Many-Argument Workhorses)** (Impact: 29.5)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 608.82 | **LOC:** 1129 | **CtrlFlow:** 16.6% | **Authorship Centralization:** 50.0%
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **16**; blast radius 0.78; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (95.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (70.7%), Connectivity (formerly Api Exposure) (36.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderElement` **(Many-Argument Workhorses)** (Impact: 147.7)
  * `drawElementOnCanvas` **(Many-Argument Workhorses)** (Impact: 111.7)
  * `generateElementWithCanvas` **(Many-Argument Workhorses)** (Impact: 47.6)
  * `generateElementCanvas` **(Many-Argument Workhorses)** (Impact: 47.1)
  * `getRenderOpacity` **(Many-Argument Workhorses)** (Impact: 44.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 37 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6.8e-05 | `Ripple Effect (Closeness):` 0.00243
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 0.671; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (89.7%), Connectivity (formerly Api Exposure) (72.4%), Guard Balance (formerly Safety Score) (65.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (50.5%)
- **Documentation Coverage:** 81.25% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getElementLineSegments` **(Many-Argument Workhorses)** (Impact: 45.8)
    * *Intent:* /* * for a given element, `getElementLineSegments` returns line segments * that can be used for visu...
  * `equation` **(Many-Argument Workhorses)** (Impact: 35.5)
    * *Intent:* // B(t) = p0 * (1-t)^3 + 3p1 * t * (1-t)^2 + 3p2 * t^2 * (1-t) + p3 * t^3
  * `getMinMaxXYFromCurvePathOps` **(Many-Argument Workhorses)** (Impact: 25.2)
  * `getArrowheadPoints` **(Many-Argument Workhorses)** (Impact: 21.9)
  * `getBounds` **(Many-Argument Workhorses)** (Impact: 20.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **13**; blast radius 1.412; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (96.4%), Connectivity (formerly Api Exposure) (69.9%), Guard Balance (formerly Safety Score) (68.1%), Complexity Load (formerly Cognitive Load) (25.3%)
- **Documentation Coverage:** 78.7879% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getElementsInResizingFrame` **(Many-Argument Workhorses)** (Impact: 45.0)
  * `shouldApplyFrameClip` **(Many-Argument Workhorses)** (Impact: 44.8)
  * `addElementsToFrame` **(Many-Argument Workhorses)** (Impact: 44.0)
    * *Intent:* /** * Retains (or repairs for target frame) the ordering invriant where children * elements come rig...
  * `setGroupsInFrame` **(Defensive Guards)** (Impact: 34.9)
  * `filterElementsEligibleAsFrameChildren` **(Compute Cores)** (Impact: 27.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000392 | `Ripple Effect (Closeness):` 0.012725
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **9**; blast radius 2.87; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (72.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (59.8%), Guard Balance (formerly Safety Score) (46.8%), Connectivity (formerly Api Exposure) (43.0%)
- **Documentation Coverage:** 75.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `repairBinding` **(Many-Argument Workhorses)** (Impact: 192.0)
  * `restoreElements` **(Many-Argument Workhorses)** (Impact: 101.7)
  * `restoreAppState` **(Defensive Guards)** (Impact: 58.1)
  * `restoreElement` **(Many-Argument Workhorses)** (Impact: 24.3)
  * `repairBoundElement` **(Defensive Guards)** (Impact: 21.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.063938
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **10**; blast radius 0.834; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.2%), Guard Balance (formerly Safety Score) (69.8%), Complexity Load (formerly Cognitive Load) (62.4%), Concurrency Surface (formerly Concurrency) (58.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `textPropertiesUpdated` **(Many-Argument Workhorses)** (Impact: 290.4)
  * `getLineCaretOffsetFromNativeLayout` **(Compute Cores)** (Impact: 15.2)
  * `getTransform` **(Many-Argument Workhorses)** (Impact: 14.3)
  * `textWysiwyg` **(Compute Cores)** (Impact: 7.7)
  * `getLineDirection` **(Callbacks & Closures)** (Impact: 5.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000639 | `Ripple Effect (Closeness):` 0.049799
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **34**; blast radius 0.671; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (50.5%), Connectivity (formerly Api Exposure) (42.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (40.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `CommandPaletteInner` **(Compute Cores)** (Impact: 180.3)
  * `executeCommand` **(Many-Argument Workhorses)** (Impact: 77.5)
  * `extract` **(Defensive Guards)** (Impact: 23.6)
  * `getNextCommandsByCategory` **(Defensive Guards)** (Impact: 20.7)
  * `perform` **(I/O & Config Routines)** (Impact: 14.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **13**; blast radius 1.827; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.2%), Guard Balance (formerly Safety Score) (64.1%), Complexity Load (formerly Cognitive Load) (48.9%), Connectivity (formerly Api Exposure) (29.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `convertToExcalidrawElements` **(Many-Argument Workhorses)** (Impact: 200.4)
  * `bindTextToContainer` **(Many-Argument Workhorses)** (Impact: 157.3)
  * `add` **(Callbacks & Closures)** (Impact: 4.6)
  * `getElement` **(Callbacks & Closures)** (Impact: 1.7)
  * `getElementsMap` **(Callbacks & Closures)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000278 | `Ripple Effect (Closeness):` 0.003645
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **13**; blast radius 2.004; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.2%), Guard Balance (formerly Safety Score) (63.5%), Complexity Load (formerly Cognitive Load) (58.4%)
- **Documentation Coverage:** 80.4348% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `onHashChange` **(Defensive Guards)** (Impact: 26.6)
  * `updateLibrary` **(Compute Cores)** (Impact: 25.8)
  * `getMaxWidthPerCol` **(Compute Cores)** (Impact: 15.1)
  * `createLibraryUpdate` **(Stateful Encapsulated Methods)** (Impact: 14.1)
    * *Intent:* /** * Returns { deletedItems, addedItems } maps of all added and deleted items * since last onLibrar...
  * `importLibraryFromURL` **(Defensive Guards)** (Impact: 13.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001489 | `Ripple Effect (Closeness):` 0.057668
...
- `packages/excalidraw/components/App.tsx` -> **Severity: 1.209** (Bridge: 0.0121 * Flux: 100.0%)
- `packages/element/src/Scene.ts` -> **Severity: 0.408** (Bridge: 0.0041 * Flux: 98.5907%)
- `packages/excalidraw/tests/helpers/api.ts` -> **Severity: 0.348** (Bridge: 0.0035 * Flux: 100.0%)
- `packages/excalidraw/tests/helpers/ui.ts` -> **Severity: 0.188** (Bridge: 0.0028 * Flux: 67.7012%)
- `packages/excalidraw/data/library.ts` -> **Severity: 0.148** (Bridge: 0.0015 * Flux: 99.192%)
...
- `packages/excalidraw/i18n.ts` -> **Severity: 10.712** (Embedded: 0.1367 * Error Risk: 78.3508%)
- `packages/excalidraw/errors.ts` -> **Severity: 7.111** (Embedded: 0.0847 * Error Risk: 83.9933%)
