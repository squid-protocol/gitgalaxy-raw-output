...
## 1. EXECUTIVE SUMMARY
- **Scope:** 10511 analyzed artifact(s), 726729 LOC.
- **Load-bearing artifact:** `tests/unit/test_util.ts` -- 127 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `libs/core/lib.rs` -- pulls in 169 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `tests/specs/cert/cafile_compile/RootCA.pem` at magnitude 5000.0 (structural weight, not risk).
...
| Analyzed Artifacts (Scanned) | 10511 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 1174 |
| Total LOC | 726729 |
...
| Assortativity | -0.2526 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.1% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 2.8893 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
| TYPESCRIPT | 2806 | 182579 | 26.7% |
| JSON | 2630 | 51183 | 25.0% |
| JAVASCRIPT | 1178 | 80991 | 11.2% |
| RUST | 898 | 410990 | 8.5% |
| MARKDOWN | 103 | 0 | 1.0% |
| HTML | 17 | 287 | 0.2% |
| CSS | 11 | 69 | 0.1% |
...
> **Composition Archetype:** `Flat Modular Platform` (z +6.52; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 78%, Declarative / Non-Code 5%, Large Core Modules (3) 4%, State Mutators Files 3%, Generic / Templated Code Files 2%
...
|---|---|---|
| Unclassified | 7576 | 72.1% |
| Unknown | 81 | 0.8% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 2850 | 27.1% |
| Static: Minified & Vendor Opaque Mass | 4 | 0.0% |

## 5. EXCLUDED ARTIFACTS (Unparsable or Shielded Files)
*Total Excluded Artifacts: 1174*

**Composition by Extension & Reason:**
- `.tgz`: 242x Excluded (Explicitly Denied Extension: '.tgz')
- `.ts`: 156x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 7x Excluded: Neighborhood Micro-Mass Limit Exceeded, 2x Excluded (Binary Format Detected)
...
- `no_extension`: 66x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 27x Unsupported Format (.undeterminable), 5x Excluded (Binary Format Detected)
- `.json`: 69x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 4x Excluded: Neighborhood Micro-Mass Limit Exceeded, 1x Excluded (Static Asset Blob without Intent: 1109 LOC)
...
| Concurrency Surface (formerly Concurrency Exposure) | 0.0 | 100.0 | 12.1 | 0.0 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 26.5 | 0.0 | 0.0 |
...
|---|---|---|---|---|
| memory | 19639 | 1076 | 1 | `ext/napi/js_native_api.rs` |
| cleanup | 2149 | 407 | 0 | `tests/integration/lsp_tests.rs` |
| guards | 22790 | 1396 | 2 | `cli/tsc/dts/lib.webworker.d.ts` |
| danger | 16530 | 1412 | 1 | `tests/integration/lsp_tests.rs` |
| concurrency | 24998 | 1455 | 2 | `tests/unit/serve_test.ts` |
| connectivity | 23169 | 2291 | 2 | `cli/tsc/dts/typescript.d.ts` |
| io | 6992 | 825 | 0 | `tests/unit_node/http_test.ts` |
| crypto | 37 | 16 | 0 | `ext/node/polyfills/01_require.js` |
| ipc | 1005 | 263 | 0 | `tests/unit_node/worker_threads_test.ts` |
| time | 1297 | 414 | 0 | `tests/unit/timers_test.ts` |
| serialization | 411 | 187 | 0 | `libs/npm/registry.rs` |
| regex | 291 | 113 | 0 | `ext/node/polyfills/deps/minimatch.js` |
| events | 4297 | 492 | 0 | `tests/unit_node/http_test.ts` |
| tests | 11395 | 615 | 0 | `tests/integration/lsp_tests.rs` |
| docs | 20930 | 878 | 0 | `cli/tsc/dts/lib.webworker.d.ts` |
| debt | 7557 | 2084 | 1 | `cli/tsc/dts/node/http2.d.cts` |
| mutation | 91290 | 2664 | 10 | `tests/integration/lsp_tests.rs` |
| dead_code | 11978 | 1510 | 1 | `cli/tsc/dts/lib.webworker.d.ts` |
| credential | 548 | 246 | 0 | `tools/deno.lock.json` |
| threat | 3369 | 438 | 0 | `libs/core/core.d.ts` |
| ml_ai | 744 | 141 | 0 | `libs/core/core.d.ts` |
...
2. **errors.ts** (`ext/node/polyfills/internal/errors.ts`) — 109 inbound connections
...
- `formatRaw` **(Many-Argument Workhorses)** (@ `ext/web/01_console.js`) -> Impact: **474.2** | LOC: 420
- `check_special_file` **(Many-Argument Workhorses)** (@ `runtime/permissions/lib.rs`) -> Impact: **301.0** | LOC: 787
  * *Intent:* /// Checks special file access, returning the failed permission type if /// not successful.
- `urlParse` **(Many-Argument Workhorses)** (@ `ext/node/polyfills/url.ts`) -> Impact: **264.2** | LOC: 324
- `sync_resolution_with_fs` **(Many-Argument Workhorses)** (@ `libs/npm_installer/local.rs`) -> Impact: **256.4** | LOC: 764
- `objectComparisonStart` **(Many-Argument Workhorses)** (@ `ext/node/polyfills/internal/util/comparisons.ts`) -> Impact: **255.3** | LOC: 187
- `iterateSubpatterns` **(Many-Argument Workhorses)** (@ `ext/node/polyfills/_fs/_fs_glob.ts`) -> Impact: **217.5** | LOC: 262
- `importKeyRSA` **(Many-Argument Workhorses)** (@ `ext/crypto/00_crypto.js`) -> Impact: **214.5** | LOC: 420
- `emitKeys` **(Many-Argument Workhorses)** (@ `ext/node/polyfills/internal/readline/utils.mjs`) -> Impact: **208.1** | LOC: 456
  * *Intent:* */
- `write_standalone_binary` **(Many-Argument Workhorses)** (@ `cli/standalone/binary.rs`) -> Impact: **202.1** | LOC: 521
...
- `formatter` **(Defensive Guards)** (@ `ext/web/01_console.js`) -> Impact: **197.6** | LOC: 391
...
- **Unknown Dependencies:** `19566` packages imported that bypass the Zero-Trust whitelist.
...
- `cli/lsp/config.rs` -> Churn: **61.55%** | Cog Load: 7.0205% | Debt: 69.9616%
- `ext/node/polyfills/internal/fs/handle.ts` -> Churn: **60.49%** | Cog Load: 68.0792% | Debt: 100.0%
...
- `tests/unit/test_util.ts` -> **Severity: 0.941** (Embedded: 0.0121 * Error Risk: 77.9949%)
- `ext/web/01_console.js` -> **Severity: 0.802** (Embedded: 0.0104 * Error Risk: 77.0386%)
- `ext/node/polyfills/internal/util/colors.ts` -> **Severity: 0.683** (Embedded: 0.0069 * Error Risk: 99.0075%)
- `ext/node/polyfills/internal/errors.ts` -> **Severity: 0.679** (Embedded: 0.0108 * Error Risk: 62.6572%)
- `ext/webidl/00_webidl.js` -> **Severity: 0.569** (Embedded: 0.0095 * Error Risk: 59.9908%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `ext/node/polyfills/internal/util/colors.ts` -> **Severity: 446.8** (Blast Radius: 4.468 * Doc Risk: 100.0%)
- `ext/node/polyfills/internal/errors.ts` -> **Severity: 434.439** (Blast Radius: 4.411 * Doc Risk: 98.4899%)
- `ext/webidl/00_webidl.js` -> **Severity: 401.909** (Blast Radius: 4.588 * Doc Risk: 87.6%)
- `ext/io/12_io.js` -> **Severity: 359.573** (Blast Radius: 4.15 * Doc Risk: 86.644%)
- `tests/napi/common.js` -> **Severity: 278.8** (Blast Radius: 2.788 * Doc Risk: 100.0%)
