...
## 1. EXECUTIVE SUMMARY
- **Scope:** 158 analyzed artifact(s), 25907 LOC.
- **Load-bearing artifact:** `Source/Features/Combine.swift` -- 2 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `Source/Features/MultipartFormData.swift` -- pulls in 4 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `Tests/Resources/Certificates/alamofire-org/alamofire-root-ca.cer` at magnitude 5000.0 (structural weight, not risk).
...
> **File Composition:** Data / Markup / Trivial 41%, Large Core Modules (2) 15%, Large Core Modules (3) 12%, Declarative / Non-Code 8%, Large Core Modules 6%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 45.1 | 22.2 | 0.0 |
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 6.229; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (70.4%), Complexity Load (formerly Cognitive Load) (40.5%), Concurrency Surface (formerly Concurrency) (16.3%), Dead Code Surface (formerly Dead Code) (5.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testThatDataStreamWorksCorrectlyWithMultipleSerialQueues` **(I/O & Config Routines)** (Impact: 27.9)
  * `testThatDataStreamWorksCorrectlyWithMultipleConcurrentQueues` **(I/O & Config Routines)** (Impact: 26.6)
  * `testThatDataCanBeStreamedAndDecodedAtTheSameTime` **(I/O & Config Routines)** (Impact: 24.7)
  * `testThatDataCanBeStreamedManyTimes` **(I/O & Config Routines)** (Impact: 21.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 6.229; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (26.3%), Complexity Load (formerly Cognitive Load) (24.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `retry` **(Many-Argument Workhorses)** (Impact: 19.4)
  * `executeAuthorizationHeaderTest` **(Stateful Encapsulated Methods)** (Impact: 19.1)
  * `testThatSessionCallsRequestRetrierForAllResponseSerializersThatThrowError` **(I/O & Config Routines)** (Impact: 16.7)
  * `testThatDownloadRequestWithInvalidURLStringThrowsResponseHandlerError` **(Annotated & Test Methods)** (Impact: 14.3)
  * `testThatRetriedRequestsCanBeMassCancelled` **(Annotated & Test Methods)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 6.229; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.8%), Debt Markers (formerly Tech Debt) (48.3%), Mutation Surface (formerly State Flux) (45.8%), Guard Balance (formerly Safety Score) (28.9%)
- **Documentation Coverage:** 44.3182% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 6.229; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (83.3%), Guard Balance (formerly Safety Score) (42.6%), Complexity Load (formerly Cognitive Load) (31.0%), Dead Code Surface (formerly Dead Code) (5.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testRequestResponseWithProgress` **(Annotated & Test Methods)** (Impact: 14.1)
  * `testPOSTRequestWithJSONParametersCURLDescription` **(I/O & Config Routines)** (Impact: 12.7)
  * `testThatDataRequestOnHTTPResponseCanCancel` **(Annotated & Test Methods)** (Impact: 12.6)
  * `testPOSTRequestWithBase64EncodedImages` **(Annotated & Test Methods)** (Impact: 12.4)
  * `testThatDataRequestOnHTTPResponseCanAllow` **(Annotated & Test Methods)** (Impact: 11.5)
**Contextual Mitigations & Amplifications:**
