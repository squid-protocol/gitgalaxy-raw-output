...
## 1. EXECUTIVE SUMMARY
- **Scope:** 5 analyzed artifact(s), 666 LOC.
- **Load-bearing artifact:** `tomli-2.4.1/src/tomli/_types.py` -- 2 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `tomli-2.4.1/src/tomli/_parser.py` -- pulls in 8 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `tomli-2.4.1/src/tomli/_parser.py` at magnitude 737.34 (structural weight, not risk).
...
> **File Composition:** Data / Markup / Trivial 60%, Large Core Modules (2) 20%, Type Conversions Files 20%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 91.2 | 41.5 | 37.5 | 0.0 |
...
- `parse_value` **(Many-Argument Workhorses)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **50.7** | LOC: 74
- `__init__` **(Many-Argument Workhorses)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **36.7** | LOC: 48
- `parse_inline_table` **(Many-Argument Workhorses)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **21.8** | LOC: 34
- `loads` **(Defensive Guards)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **20.8** | LOC: 69
  * *Intent:* """Parse TOML from a string."""
- `parse_basic_str` **(Defensive Guards)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **19.5** | LOC: 30
- `key_value_rule` **(Many-Argument Workhorses)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **18.7** | LOC: 32
- `parse_basic_str_escape` **(Defensive Guards)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **17.4** | LOC: 29
- `match_to_datetime` **(Type Conversions)** (@ `tomli-2.4.1/src/tomli/_re.py`) -> Impact: **17.3** | LOC: 34
...
- `is_` **(Compute Cores)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **14.8** | LOC: 16
- `parse_array` **(Many-Argument Workhorses)** (@ `tomli-2.4.1/src/tomli/_parser.py`) -> Impact: **14.6** | LOC: 24
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **8**; blast radius 206.916; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (62.5%)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **6**; blast radius 199.786; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.8%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (39.3%)
- **Documentation Coverage:** 75.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `match_to_datetime` **(Type Conversions)** (Impact: 17.3)
    * *Intent:* """Convert a `RE_DATETIME` match to `datetime.datetime` or `datetime.date`. Raises ValueError if the...
  * `match_to_number` **(Type Conversions)** (Impact: 10.6)
  * `match_to_localtime` **(Type Conversions)** (Impact: 10.1)
  * `cached_tz` **(Type Conversions)** (Impact: 6.4)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **1**; blast radius 369.604; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (75.3%), Mutation Surface (formerly State Flux) (50.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Test Surface (formerly Verification) (2.3%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 111.847; role: Pure Consumer (Orchestrator)
