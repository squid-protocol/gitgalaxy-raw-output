...
## 1. EXECUTIVE SUMMARY
- **Scope:** 39 analyzed artifact(s), 32409 LOC.
- **Load-bearing artifact:** `package/polyfills/util.js` -- 10 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `package/polyfills/stream.js` -- pulls in 7 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `package/polyfills/browserify-fs.js` at magnitude 16816.52 (structural weight, not risk).
...
| Avg Path Length | 1.7356 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.621`
> **Composition Archetype:** `Small Flat Repo (2)` (z +1.62; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules (2) 38%, Data / Markup / Trivial 23%, Large Core Modules (3) 15%, State Mutators Files 10%, Callbacks & Closures Files 8%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 86.8 | 100.0 | 100.0 |
...
- `levelFilesystem` **(Defensive Guards)** (@ `package/polyfills/browserify-fs.js`) -> Impact: **340.7** | LOC: 579
- `isPrintstr` **(Defensive Guards)** (@ `package/polyfills/crypto-browserify.js`) -> Impact: **220.5** | LOC: 648
- `parse` **(Many-Argument Workhorses)** (@ `package/polyfills/url.js`) -> Impact: **178.5** | LOC: 260
- `resolveObject` **(Compute Cores)** (@ `package/polyfills/url.js`) -> Impact: **174.6** | LOC: 268
- `levelBlobs` **(Many-Argument Workhorses)** (@ `package/polyfills/browserify-fs.js`) -> Impact: **134.1** | LOC: 361
- `makeConstructor` **(Many-Argument Workhorses)** (@ `package/polyfills/browserify-fs.js`) -> Impact: **114.4** | LOC: 208
  * *Intent:* // // 5 The Typed Array View Types //
- `decode` **(Defensive Guards)** (@ `package/polyfills/crypto-browserify.js`) -> Impact: **96.3** | LOC: 125
  * *Intent:* // // Decoding //
- `encode` **(Defensive Guards)** (@ `package/polyfills/crypto-browserify.js`) -> Impact: **82.8** | LOC: 96
- `write` **(Many-Argument Workhorses)** (@ `package/polyfills/buffer-es6.js`) -> Impact: **77.3** | LOC: 71
- `_deepEqual` **(Defensive Guards)** (@ `package/polyfills/assert.js`) -> Impact: **77.0** | LOC: 65
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 15.931; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.4%), Complexity Load (formerly Cognitive Load) (81.6%)
- **Documentation Coverage:** 91.844% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `levelFilesystem` **(Defensive Guards)** (Impact: 340.7)
  * `levelBlobs` **(Many-Argument Workhorses)** (Impact: 134.1)
  * `makeConstructor` **(Many-Argument Workhorses)** (Impact: 114.4)
    * *Intent:* // // 5 The Typed Array View Types //
  * `open` **(Defensive Guards)** (Impact: 69.9)
  * `readableAddChunk` **(Many-Argument Workhorses)** (Impact: 63.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 15.931; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (95.7%), Guard Balance (formerly Safety Score) (95.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 99.4835% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `isPrintstr` **(Defensive Guards)** (Impact: 220.5)
  * `decode` **(Defensive Guards)** (Impact: 96.3)
    * *Intent:* // // Decoding //
  * `encode` **(Defensive Guards)** (Impact: 82.8)
  * `_wnafMulAdd` **(Many-Argument Workhorses)** (Impact: 74.7)
  * `decodeGeneric` **(Defensive Guards)** (Impact: 55.5)
**Contextual Mitigations & Amplifications:**
...
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.0%), Complexity Load (formerly Cognitive Load) (88.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 94.8905% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `write` **(Many-Argument Workhorses)** (Impact: 77.3)
  * `bidirectionalIndexOf` **(Many-Argument Workhorses)** (Impact: 66.5)
    * *Intent:* // Finds either the first index of `val` in `buffer` at offset >= `byteOffset`, // OR the last index...
  * `arrayIndexOf` **(Many-Argument Workhorses)** (Impact: 61.5)
  * `copy` **(Many-Argument Workhorses)** (Impact: 60.5)
    * *Intent:* // copy(targetBuffer, targetStart=0, sourceStart=0, sourceEnd=buffer.length)
  * `write` **(Many-Argument Workhorses)** (Impact: 58.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 29.473; role: Transceiver (Middle-Tier)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **6**; blast radius 33.722; role: Transceiver (Middle-Tier)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **5**; blast radius 33.722; role: Transceiver (Middle-Tier)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 15.931; role: Pure Consumer (Orchestrator)
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **1**; blast radius 46.797; role: Pure Producer (Foundation)
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **2**; blast radius 107.196; role: Pure Producer (Foundation)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 15.931; role: Pure Consumer (Orchestrator)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 28.457; role: Pure Producer (Foundation)
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (17.8%), Complexity Load (formerly Cognitive Load) (4.8%), Test Surface (formerly Verification) (2.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 19.317; role: Pure Producer (Foundation)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 15.931; role: Isolated/Orphan
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (98.2%), Guard Balance (formerly Safety Score) (75.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 29.473; role: Pure Producer (Foundation)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 22.702; role: Pure Producer (Foundation)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **2**; blast radius 28.388; role: Transceiver (Middle-Tier)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 23.097; role: Pure Producer (Foundation)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **7**; blast radius 32.859; role: Transceiver (Middle-Tier)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 15.931; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (97.7%), Debt Markers (formerly Tech Debt) (96.7%), Mutation Surface (formerly State Flux) (92.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.0%), Debt Markers (formerly Tech Debt) (86.7%), Guard Balance (formerly Safety Score) (53.1%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **4**; blast radius 48.706; role: Transceiver (Middle-Tier)
