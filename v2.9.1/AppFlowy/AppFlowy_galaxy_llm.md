...
## 1. EXECUTIVE SUMMARY
- **Scope:** 1799 analyzed artifact(s), 102670 LOC.
- **Load-bearing artifact:** `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/theme/appflowy_theme.dart` -- 13 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `frontend/rust-lib/flowy-folder/src/manager.rs` -- pulls in 113 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `frontend/appflowy_flutter/dsa_pub.pem` at magnitude 5000.0 (structural weight, not risk).
...
| Avg Path Length | 2.7393 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `3.395`
> **Composition Archetype:** `Mid Flat Project` (z +3.40; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 62%, Generic / Templated Code Files 8%, State Mutators Files 8%, Declarative / Non-Code 6%, Large Core Modules (3) 5%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 31.5 | 0.0 | 0.0 |
...
| connectivity | 6446 | 693 | 9 | `frontend/rust-lib/flowy-ai/src/entities.rs` |
...
| dead_code | 3109 | 475 | 5 | `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/theme/data/appflowy_default/primitive.dart` |
| credential | 0 | 0 | 0 | - |
...
- `duplicate_view_with_parent_id` **(Many-Argument Workhorses)** (@ `frontend/rust-lib/flowy-folder/src/manager.rs`) -> Impact: **113.3** | LOC: 173
...
- `stream_response` **(Many-Argument Workhorses)** (@ `frontend/rust-lib/flowy-ai/src/chat.rs`) -> Impact: **82.2** | LOC: 145
- `getPositionForChild` **(Many-Argument Workhorses)** (@ `frontend/appflowy_flutter/packages/appflowy_popover/lib/src/layout.dart`) -> Impact: **79.9** | LOC: 143
- `_showOverlay` **(Many-Argument Workhorses)** (@ `frontend/appflowy_flutter/packages/flowy_infra_ui/lib/src/flowy_overlay/flowy_overlay.dart`) -> Impact: **79.9** | LOC: 72
- `init_with_callback` **(Many-Argument Workhorses)** (@ `frontend/rust-lib/flowy-user/src/user_manager/manager.rs`) -> Impact: **71.1** | LOC: 182
...
- `getPositionForChild` **(Many-Argument Workhorses)** (@ `frontend/appflowy_flutter/packages/flowy_infra_ui/lib/src/flowy_overlay/flowy_popover_layout.dart`) -> Impact: **69.5** | LOC: 108
- `getPositionForChild` **(Many-Argument Workhorses)** (@ `frontend/appflowy_flutter/packages/flowy_infra_ui/lib/src/flowy_overlay/layout.dart`) -> Impact: **69.5** | LOC: 108
- `generate_import_data` **(Many-Argument Workhorses)** (@ `frontend/rust-lib/flowy-user/src/services/data_import/appflowy_data_import.rs`) -> Impact: **64.8** | LOC: 255
...
- `getConstraintsForChild` **(Compute Cores)** (@ `frontend/appflowy_flutter/packages/flowy_infra_ui/lib/src/flowy_overlay/flowy_popover_layout.dart`) -> Impact: **63.8** | LOC: 117
- `getConstraintsForChild` **(Compute Cores)** (@ `frontend/appflowy_flutter/packages/flowy_infra_ui/lib/src/flowy_overlay/layout.dart`) -> Impact: **63.8** | LOC: 117
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **94**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (94.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (67.8%)
- **Documentation Coverage:** 91.4692% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `async_load_rows` **(Many-Argument Workhorses)** (Impact: 53.4)
  * `open_database_view` **(Many-Argument Workhorses)** (Impact: 37.1)
    * *Intent:* /// Open database view /// When opening database view, it will load database rows from remote if the...
  * `move_group_row` **(Many-Argument Workhorses)** (Impact: 29.2)
  * `delete_select_options` **(Many-Argument Workhorses)** (Impact: 24.0)
  * `init_database_row` **(Defensive Guards)** (Impact: 23.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **113**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (95.4%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (78.4%), Guard Balance (formerly Safety Score) (38.2%)
- **Documentation Coverage:** 45.0549% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `duplicate_view_with_parent_id` **(Many-Argument Workhorses)** (Impact: 113.3)
    * *Intent:* /// Duplicate the view with the given view id and parent view id. /// /// If the view id is the same...
  * `move_view` **(Many-Argument Workhorses)** (Impact: 31.6)
    * *Intent:* /// Move the view with given id from one position to another position. /// The view will be moved to...
  * `initialize_after_sign_up` **(Many-Argument Workhorses)** (Impact: 31.5)
    * *Intent:* /// Initialize the folder for the new user. /// Using the [DefaultFolderBuilder] to create the defau...
  * `publish_view` **(Many-Argument Workhorses)** (Impact: 29.8)
    * *Intent:* /// Publishes a view identified by the given `view_id`. /// /// If `publish_name` is `None`, a defau...
  * `create_view_with_params` **(Many-Argument Workhorses)** (Impact: 28.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **26**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (99.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (48.3%)
- **Documentation Coverage:** 97.2414% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `update_database_setting_handler` **(Defensive Guards)** (Impact: 42.5)
  * `rename_media_cell_file_handler` **(Many-Argument Workhorses)** (Impact: 22.9)
    * *Intent:* /// We use a custom handler to rename the media file, as the ordering /// of the files must be maint...
  * `update_field_type_option_handler` **(Stateful Encapsulated Methods)** (Impact: 13.1)
  * `update_relation_cell_handler` **(Stateful Encapsulated Methods)** (Impact: 12.6)
  * `get_primary_field_handler` **(Stateful Encapsulated Methods)** (Impact: 11.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **87**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (81.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (68.1%)
- **Documentation Coverage:** 84.6154% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `v_did_update_row` **(Many-Argument Workhorses)** (Impact: 34.4)
    * *Intent:* /// Notify the view that the row has been updated. If the view has groups, /// send the group notifi...
  * `v_did_create_row` **(Many-Argument Workhorses)** (Impact: 18.2)
  * `v_set_layout_settings` **(Many-Argument Workhorses)** (Impact: 16.5)
    * *Intent:* /// Update the layout settings and send the notification to refresh the UI
  * `v_get_layout_settings` **(Defensive Guards)** (Impact: 15.9)
    * *Intent:* /// Returns the current calendar settings
  * `v_delete_group` **(Defensive Guards)** (Impact: 15.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **65**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (78.1%), Complexity Load (formerly Cognitive Load) (60.9%)
- **Documentation Coverage:** 84.7059% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `build_collab` **(Many-Argument Workhorses)** (Impact: 40.9)
    * *Intent:* ///NOTE: this method doesn't initialize plugins, however it is passed into WorkspaceDatabase, /// th...
  * `translate_row` **(Many-Argument Workhorses)** (Impact: 30.5)
  * `summarize_row` **(Many-Argument Workhorses)** (Impact: 29.1)
  * `initialize` **(Many-Argument Workhorses)** (Impact: 20.4)
    * *Intent:* /// When initialize with new workspace, all the resources will be cleared.
  * `create_linked_view` **(Many-Argument Workhorses)** (Impact: 20.0)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 1.643; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Complexity Load (formerly Cognitive Load) (29.6%), Connectivity (formerly Api Exposure) (3.5%), Test Surface (formerly Verification) (2.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `underline` **(Defensive Guards)** (Impact: 16.7)
  * `standard` **(Defensive Guards)** (Impact: 16.6)
  * `enhanced` **(Defensive Guards)** (Impact: 16.6)
  * `prominent` **(Defensive Guards)** (Impact: 16.6)
  * `standard` **(Defensive Guards)** (Impact: 16.4)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.002995
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (98.8%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (48.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `set_cloud_config_handler` **(Many-Argument Workhorses)** (Impact: 17.9)
  * `import_appflowy_data_folder_handler` **(Generic / Templated Code)** (Impact: 11.7)
  * `get_user_workspace_handler` **(Generic / Templated Code)** (Impact: 11.0)
  * `get_user_profile_handler` **(Generic / Templated Code)** (Impact: 10.1)
  * `get_cloud_config_handler` **(Generic / Templated Code)** (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **48**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.9%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (66.2%), Complexity Load (formerly Cognitive Load) (36.3%)
- **Documentation Coverage:** 96.5517% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `open_workspace` **(Many-Argument Workhorses)** (Impact: 53.5)
  * `get_all_user_workspaces` **(Many-Argument Workhorses)** (Impact: 31.1)
  * `get_workspace_settings` **(Compute Cores)** (Impact: 26.5)
  * `upload_collab_data` **(Many-Argument Workhorses)** (Impact: 24.0)
  * `create_workspace` **(Compute Cores)** (Impact: 21.4)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **73**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (69.1%), Guard Balance (formerly Safety Score) (54.3%), Concurrency Surface (formerly Concurrency) (15.7%)
- **Documentation Coverage:** 92.8571% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `generate_import_data` **(Many-Argument Workhorses)** (Impact: 64.8)
    * *Intent:* /// This path refers to the directory where AppFlowy stores its data. The directory structure is as ...
  * `migrate_folder_views` **(Many-Argument Workhorses)** (Impact: 60.2)
  * `migrate_databases` **(Many-Argument Workhorses)** (Impact: 33.0)
  * `replace_document_ref_ids` **(Defensive Guards)** (Impact: 32.7)
  * `upload_collab_objects_data` **(Many-Argument Workhorses)** (Impact: 29.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **57**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (93.8%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (74.2%), Guard Balance (formerly Safety Score) (40.6%)
- **Documentation Coverage:** 87.8049% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `init_with_callback` **(Many-Argument Workhorses)** (Impact: 71.1)
    * *Intent:* /// Initializes the user session, including data migrations and user awareness configuration. This f...
  * `refresh_user_profile` **(Many-Argument Workhorses)** (Impact: 25.1)
  * `continue_sign_up` **(Many-Argument Workhorses)** (Impact: 22.9)
  * `sign_in` **(Many-Argument Workhorses)** (Impact: 18.4)
    * *Intent:* /// Performs a user sign-in, initializing user awareness and sending relevant notifications. /// ///...
  * `new` **(Many-Argument Workhorses)** (Impact: 17.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **77**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (99.7%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (44.2%)
- **Documentation Coverage:** 95.6522% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get_plugins` **(Many-Argument Workhorses)** (Impact: 19.2)
  * `upload_part` **(Many-Argument Workhorses)** (Impact: 9.6)
  * `create_upload` **(Many-Argument Workhorses)** (Impact: 8.6)
  * `complete_upload` **(Many-Argument Workhorses)** (Impact: 8.6)
  * `generate_search_summary` **(Generic / Templated Code)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **58**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (61.8%), Guard Balance (formerly Safety Score) (46.1%)
- **Documentation Coverage:** 95.5224% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `reload_with_workspace_id` **(Defensive Guards)** (Impact: 21.5)
  * `sync_chat_documents` **(Defensive Guards)** (Impact: 19.6)
  * `open_chat` **(Compute Cores)** (Impact: 18.4)
  * `update_rag_ids` **(Compute Cores)** (Impact: 15.6)
  * `stream_regenerate_response` **(Many-Argument Workhorses)** (Impact: 14.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **61**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (40.5%), Complexity Load (formerly Cognitive Load) (32.1%)
- **Documentation Coverage:** 95.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `start_upload` **(Many-Argument Workhorses)** (Impact: 60.4)
  * `create_upload` **(Many-Argument Workhorses)** (Impact: 37.8)
  * `complete_upload` **(Many-Argument Workhorses)** (Impact: 37.8)
  * `new` **(Defensive Guards)** (Impact: 21.2)
  * `query_file_state` **(Defensive Guards)** (Impact: 18.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (100.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (57.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_view_handler` **(Stateful Encapsulated Methods)** (Impact: 9.3)
  * `create_orphan_view_handler` **(Stateful Encapsulated Methods)** (Impact: 9.3)
  * `update_recent_views_handler` **(Stateful Encapsulated Methods)** (Impact: 7.6)
  * `set_default_publish_view_handler` **(Stateful Encapsulated Methods)** (Impact: 7.6)
  * `remove_user_from_shared_page_handler` **(Stateful Encapsulated Methods)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **69**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (97.8%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (47.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get_user_profile` **(Defensive Guards)** (Impact: 19.4)
  * `create_collab_object` **(Compute Cores)** (Impact: 11.0)
  * `user_sign_in_with_url` **(Generic / Templated Code)** (Impact: 10.0)
  * `get_user_awareness_doc_state` **(Generic / Templated Code)** (Impact: 9.8)
  * `generate_sign_in_url_with_email` **(Generic / Templated Code)** (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **41**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (73.6%), Guard Balance (formerly Safety Score) (48.8%)
- **Documentation Coverage:** 76.2712% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_document_instance` **(Many-Argument Workhorses)** (Impact: 27.1)
    * *Intent:* /// Returns Document for given object id /// If the document does not exist in local disk, try get t...
  * `collab_for_document` **(Many-Argument Workhorses)** (Impact: 18.4)
  * `create_document` **(Many-Argument Workhorses)** (Impact: 17.3)
    * *Intent:* /// Create a new document. /// /// if the document already exists, return the existing document. ///...
  * `is_doc_exist` **(Defensive Guards)** (Impact: 12.7)
  * `doc_state_from_document_data` **(Defensive Guards)** (Impact: 11.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (97.5%), Mutation Surface (formerly State Flux) (95.6%), Debt Markers (formerly Tech Debt) (78.9%)
- **Documentation Coverage:** 60.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `next_chunk` **(Compute Cores)** (Impact: 9.7)
    * *Intent:* /// Read the next chunk from the file.
  * `from_file` **(Generic / Templated Code)** (Impact: 8.0)
    * *Intent:* /// Create a `ChunkedBytes` instance from a file.
  * `set_offset` **(Compute Cores)** (Impact: 5.7)
    * *Intent:* /// Set the offset for the next chunk to be read.
  * `calculate_offsets` **(Parameter Forwarders)** (Impact: 4.1)
  * `test_chunked_bytes_large_file` **(Annotated & Test Methods)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **39**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (42.0%), Debt Markers (formerly Tech Debt) (39.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `on_launch_if_authenticated` **(Many-Argument Workhorses)** (Impact: 46.6)
  * `on_workspace_opened` **(Many-Argument Workhorses)** (Impact: 40.4)
  * `on_sign_up` **(Many-Argument Workhorses)** (Impact: 38.2)
  * `on_sign_in` **(Many-Argument Workhorses)** (Impact: 35.4)
  * `folder_init_data_source` **(Many-Argument Workhorses)** (Impact: 12.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **40**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (96.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (47.9%), Complexity Load (formerly Cognitive Load) (36.7%)
- **Documentation Coverage:** 94.1176% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `stream_response` **(Many-Argument Workhorses)** (Impact: 82.2)
  * `load_remote_chat_messages` **(Many-Argument Workhorses)** (Impact: 28.3)
  * `get_question_id_from_answer_id` **(Defensive Guards)** (Impact: 13.2)
  * `stream_chat_message` **(Many-Argument Workhorses)** (Impact: 12.8)
  * `load_prev_chat_messages` **(Many-Argument Workhorses)** (Impact: 12.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **37**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.7%), Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (73.5%), Complexity Load (formerly Cognitive Load) (69.2%)
- **Documentation Coverage:** 93.9394% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `apply_filter` **(Many-Argument Workhorses)** (Impact: 41.4)
    * *Intent:* /// Recursively applies a `Filter` to a `Row`'s cells.
  * `apply_changeset` **(Defensive Guards)** (Impact: 29.7)
  * `fill_cells` **(Defensive Guards)** (Impact: 22.8)
  * `delete_filter` **(Defensive Guards)** (Impact: 20.2)
  * `new` **(Many-Argument Workhorses)** (Impact: 18.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **45**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (80.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (72.2%)
- **Documentation Coverage:** 86.3636% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `stream` **(Many-Argument Workhorses)** (Impact: 31.8)
  * `execute` **(Many-Argument Workhorses)** (Impact: 25.2)
  * `get_documents_or_result` **(Many-Argument Workhorses)** (Impact: 24.6)
  * `get_question` **(Many-Argument Workhorses)** (Impact: 11.5)
  * `get_related_questions` **(Defensive Guards)** (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (72.2%), Concurrency Surface (formerly Concurrency) (58.1%), Guard Balance (formerly Safety Score) (58.0%)
- **Documentation Coverage:** 80.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `upsert_collabs_embeddings` **(Many-Argument Workhorses)** (Impact: 48.9)
    * *Intent:* /// Inserts or replaces all of `fragments` for the given (workspace_id, object_id), /// deleting any...
  * `select_all_embedded_documents` **(Many-Argument Workhorses)** (Impact: 34.2)
  * `select_all_embedded_content` **(Many-Argument Workhorses)** (Impact: 24.8)
  * `select_collabs_fragment_ids` **(Compute Cores)** (Impact: 19.4)
  * `process_search_results` **(Generic / Templated Code)** (Impact: 17.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 0.531; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Debt Markers (formerly Tech Debt) (99.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (54.3%)
- **Documentation Coverage:** 98.2143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `insert_option` **(Many-Argument Workhorses)** (Impact: 3.9)
  * `create_calendar` **(Many-Argument Workhorses)** (Impact: 3.5)
  * `update_group` **(Generic / Templated Code)** (Impact: 3.4)
  * `create_grid` **(Many-Argument Workhorses)** (Impact: 3.3)
    * *Intent:* /// The initial data can refer to the [FolderOperationHandler::create_view_with_view_data] method.
  * `create_board` **(Many-Argument Workhorses)** (Impact: 3.3)
**Structural Signatures (Net Mitigated Signals):**
...
- `frontend/appflowy_flutter/lib/workspace/presentation/settings/widgets/emoji_picker/src/emoji_picker.dart` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
- `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/component/popover/popover.dart` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
...
- `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/theme/appflowy_theme.dart` -> **Severity: 0.303** (Embedded: 0.0075 * Error Risk: 40.4904%)
- `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/theme/definition/text_style/text_style.dart` -> **Severity: 0.263** (Embedded: 0.0038 * Error Risk: 70.2063%)
- `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/theme/definition/color_scheme/badge_color_scheme.dart` -> **Severity: 0.24** (Embedded: 0.003 * Error Risk: 80.0%)
- `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/theme/definition/color_scheme/border_color_scheme.dart` -> **Severity: 0.24** (Embedded: 0.003 * Error Risk: 80.0%)
- `frontend/appflowy_flutter/packages/appflowy_ui/lib/src/theme/definition/color_scheme/brand_color_scheme.dart` -> **Severity: 0.24** (Embedded: 0.003 * Error Risk: 80.0%)
