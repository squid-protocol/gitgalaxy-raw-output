...
## 1. EXECUTIVE SUMMARY
- **Scope:** 33 analyzed artifact(s), 9015 LOC.
- **Load-bearing artifact:** `src/httpz.zig` -- 21 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/httpz.zig` -- pulls in 14 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/worker.zig` at magnitude 1006.44 (structural weight, not risk).
...
| Avg Path Length | 2.1955 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `0.93`
> **Composition Archetype:** `Small Flat Repo (2)` (z +0.93; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules (2) 45%, State Mutators Files 18%, Large Core Modules (3) 15%, Data / Markup / Trivial 6%, Declarative / Non-Code 3%
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 16.2 | 2.9 | 2.1 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 21.1 | 16.3 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 85.3 | 100.0 | 100.0 |
...
- `NonBlocking` **(Many-Argument Workhorses)** (@ `src/worker.zig`) -> Impact: **112.2** | LOC: 615
...
- `Server` **(Defensive Guards)** (@ `src/httpz.zig`) -> Impact: **106.8** | LOC: 411
- `Blocking` **(Defensive Guards)** (@ `src/worker.zig`) -> Impact: **79.7** | LOC: 312
...
- `Router` **(Defensive Guards)** (@ `src/router.zig`) -> Impact: **58.1** | LOC: 262
- `getRoute` **(Many-Argument Workhorses)** (@ `src/router.zig`) -> Impact: **56.7** | LOC: 60
- `addRoute` **(Many-Argument Workhorses)** (@ `src/router.zig`) -> Impact: **53.1** | LOC: 83
- `handleRequest` **(Many-Argument Workhorses)** (@ `src/worker.zig`) -> Impact: **48.8** | LOC: 81
- `Worker` **(Compute Cores)** (@ `src/thread_pool.zig`) -> Impact: **43.2** | LOC: 156
- `parse` **(Defensive Guards)** (@ `src/request.zig`) -> Impact: **43.1** | LOC: 62
  * *Intent:* // returns true if the header has been fully parsed
- `main` **(Defensive Guards)** (@ `test_runner.zig`) -> Impact: **39.9** | LOC: 118
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **8**; blast radius 80.069; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (93.6%), Connectivity (formerly Api Exposure) (65.1%), Guard Balance (formerly Safety Score) (37.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `NonBlocking` **(Many-Argument Workhorses)** (Impact: 112.2)
    * *Intent:* // This is a NonBlocking worker. We have N workers, each accepting connections // and largely workin...
  * `Blocking` **(Defensive Guards)** (Impact: 79.7)
    * *Intent:* // This is our Blocking worker. It's very different than NonBlocking and much // simpler. (WSH is ou...
  * `handleRequest` **(Many-Argument Workhorses)** (Impact: 48.8)
  * `run` **(Many-Argument Workhorses)** (Impact: 33.6)
  * `init` **(Defensive Guards)** (Impact: 23.1)
...
- **Global Archetype:** `file_cluster_12` (Drift: 0.0 IQR)
- **Magnitude:** 793.14 | **LOC:** 2432 | **CtrlFlow:** 7.5% | **Authorship Centralization:** 28.6%
- **Blast Radius:** changing it is visible to **21** in-repo importer(s); it depends on **14**; blast radius 210.653; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Connectivity (formerly Api Exposure) (83.9%), Mutation Surface (formerly State Flux) (74.6%), Complexity Load (formerly Cognitive Load) (26.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Server` **(Defensive Guards)** (Impact: 106.8)
  * `upgradeWebsocket` **(Defensive Guards)** (Impact: 31.6)
  * `init` **(Defensive Guards)** (Impact: 30.8)
  * `handleRequest` **(Many-Argument Workhorses)** (Impact: 29.4)
    * *Intent:* // This is always called from within a threadpool thread. For nonblocking, // notifyingHandler (abov...
  * `listen` **(Defensive Guards)** (Impact: 27.5)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 732.54 | **LOC:** 1854 | **CtrlFlow:** 14.3% | **Authorship Centralization:** 50.0%
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **10**; blast radius 32.477; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (52.8%), Connectivity (formerly Api Exposure) (52.8%), Complexity Load (formerly Cognitive Load) (31.4%)
- **Documentation Coverage:** 93.1034% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse` **(Defensive Guards)** (Impact: 43.1)
    * *Intent:* // returns true if the header has been fully parsed
  * `parseHeaders` **(Many-Argument Workhorses)** (Impact: 34.4)
  * `prepareForBody` **(Many-Argument Workhorses)** (Impact: 29.4)
    * *Intent:* // we've finished reading the header
  * `getContentDispotionAttributes` **(Defensive Guards)** (Impact: 29.2)
    * *Intent:* // I'm sorry
  * `parseMultiFormData` **(Defensive Guards)** (Impact: 28.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 23.613; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (94.1%), Connectivity (formerly Api Exposure) (67.2%), Complexity Load (formerly Cognitive Load) (34.0%), Dead Code Surface (formerly Dead Code) (19.9%)
- **Documentation Coverage:** 96.7742% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Router` **(Defensive Guards)** (Impact: 58.1)
  * `getRoute` **(Many-Argument Workhorses)** (Impact: 56.7)
  * `addRoute` **(Many-Argument Workhorses)** (Impact: 53.1)
  * `Group` **(Defensive Guards)** (Impact: 22.4)
  * `tryMergeConfig` **(Defensive Guards)** (Impact: 10.8)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 318.64 | **LOC:** 668 | **CtrlFlow:** 11.7% | **Authorship Centralization:** 66.7%
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 23.613; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (66.7%), Connectivity (formerly Api Exposure) (58.3%), Complexity Load (formerly Cognitive Load) (40.8%)
- **Documentation Coverage:** 97.0588% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `compareValue` **(Defensive Guards)** (Impact: 35.0)
  * `parseWithAllocator` **(Defensive Guards)** (Impact: 17.7)
  * `escapeString` **(Defensive Guards)** (Impact: 13.3)
  * `decodeChunkedEncoding` **(Defensive Guards)** (Impact: 12.1)
  * `compare` **(Defensive Guards)** (Impact: 11.5)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 20.948; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (58.1%), Guard Balance (formerly Safety Score) (52.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Worker` **(Compute Cores)** (Impact: 43.2)
  * `spawn` **(Compute Cores)** (Impact: 22.5)
  * `ThreadPool` **(Defensive Guards)** (Impact: 18.3)
  * `getNext` **(Compute Cores)** (Impact: 15.4)
  * `init` **(Defensive Guards)** (Impact: 8.6)
...
- **Global Archetype:** `file_cluster_12` (Drift: 0.0 IQR)
- **Magnitude:** 239.14 | **LOC:** 676 | **CtrlFlow:** 7.2% | **Authorship Centralization:** 100.0%
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **8**; blast radius 32.477; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.8%), Connectivity (formerly Api Exposure) (60.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (44.1%), Complexity Load (formerly Cognitive Load) (35.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `prepareHeader` **(Compute Cores)** (Impact: 27.4)
  * `serializeCookie` **(Many-Argument Workhorses)** (Impact: 27.4)
    * *Intent:* // we expect arena to be an ArenaAllocator
  * `headerOpts` **(Defensive Guards)** (Impact: 11.4)
  * `write` **(Defensive Guards)** (Impact: 8.2)
  * `writeInt` **(Compute Cores)** (Impact: 7.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **11** in-repo importer(s); it depends on **4**; blast radius 148.576; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (80.1%), Guard Balance (formerly Safety Score) (80.0%), Connectivity (formerly Api Exposure) (80.0%), Concurrency Surface (formerly Concurrency) (40.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `allocInit` **(Defensive Guards)** (Impact: 23.0)
  * `expect` **(Defensive Guards)** (Impact: 14.1)
  * `read` **(Defensive Guards)** (Impact: 13.3)
  * `stream` **(Defensive Guards)** (Impact: 6.7)
  * `write` **(Defensive Guards)** (Impact: 5.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (89.3%), Guard Balance (formerly Safety Score) (39.2%), Complexity Load (formerly Cognitive Load) (34.6%), Debt Markers (formerly Tech Debt) (12.2%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(Defensive Guards)** (Impact: 39.9)
  * `endTiming` **(Defensive Guards)** (Impact: 8.3)
  * `status` **(Generic / Templated Code)** (Impact: 6.5)
  * `display` **(Defensive Guards)** (Impact: 6.1)
  * `readEnvBool` **(Defensive Guards)** (Impact: 4.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 4 instances
* *Mitigated Memory Allocs:* 3 instances
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **4**; blast radius 26.681; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (60.4%), Connectivity (formerly Api Exposure) (45.3%)
- **Documentation Coverage:** 60.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `unescape` **(Many-Argument Workhorses)** (Impact: 30.6)
    * *Intent:* // std.Url.unescapeString has 2 problems // First, it doesn't convert '+' -> ' ' // Second, it _alwa...
  * `isValid` **(Defensive Guards)** (Impact: 11.0)
  * `parse` **(Defensive Guards)** (Impact: 3.6)
  * `asUint` **(Type Conversions)** (Impact: 3.4)
    * *Intent:* /// converts ascii to unsigned int of appropriate size
  * `decodeHex` **(Type Conversions)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Amplified Cascading Flux:* 19 instances
* *Memory Alloc (weighted view):* 2
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **2**; blast radius 31.282; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (80.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (60.5%), Debt Markers (formerly Tech Debt) (33.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `KeyValue` **(Type Conversions)** (Impact: 24.0)
  * `init` **(Type Conversions)** (Impact: 9.3)
  * `get` **(Compute Cores)** (Impact: 5.6)
  * `deinit` **(Type Conversions)** (Impact: 5.4)
  * `strHash` **(Defensive Guards)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **4**; blast radius 70.713; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (57.8%), Mutation Surface (formerly State Flux) (52.5%), Guard Balance (formerly Safety Score) (11.5%), Complexity Load (formerly Cognitive Load) (10.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `init` **(Defensive Guards)** (Impact: 11.3)
  * `allocType` **(Many-Argument Workhorses)** (Impact: 8.0)
  * `grow` **(Defensive Guards)** (Impact: 5.4)
  * `release` **(Generic / Templated Code)** (Impact: 4.1)
  * `free` **(Generic / Templated Code)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 27.409; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (85.0%), Connectivity (formerly Api Exposure) (38.6%), Guard Balance (formerly Safety Score) (31.2%), Complexity Load (formerly Cognitive Load) (18.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `execute` **(Many-Argument Workhorses)** (Impact: 31.5)
  * `parseOrigin` **(Defensive Guards)** (Impact: 8.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (36.2%), Complexity Load (formerly Cognitive Load) (17.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (3.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `formPost` **(Defensive Guards)** (Impact: 4.0)
  * `hello` **(Defensive Guards)** (Impact: 3.8)
  * `main` **(I/O & Config Routines)** (Impact: 3.1)
    * *Intent:* // This example demonstrates basic httpz usage, with focus on using the // httpz.Request and httpz.R...
  * `index` **(State Mutators)** (Impact: 2.3)
  * `writer` **(State Mutators)** (Impact: 2.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **4**; blast radius 31.282; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (74.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (37.6%), Complexity Load (formerly Cognitive Load) (4.2%), Test Surface (formerly Verification) (2.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseAddress` **(Defensive Guards)** (Impact: 4.9)
  * `workerCount` **(Defensive Guards)** (Impact: 4.5)
  * `isUnixAddress` **(Defensive Guards)** (Impact: 3.2)
  * `threadPoolCount` **(Defensive Guards)** (Impact: 3.0)
  * `localhost` **(Parameter Forwarders)** (Impact: 1.6)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Dead Code Surface (formerly Dead Code) (32.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (28.4%), Complexity Load (formerly Cognitive Load) (9.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (3.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `upload` **(Defensive Guards)** (Impact: 23.0)
  * `main` **(I/O & Config Routines)** (Impact: 2.4)
    * *Intent:* // This example demonstrates handling file uploads using multipart/form-data. // It shows how to: //...
  * `index` **(Generic / Templated Code)** (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Memory Alloc (weighted view):* 0
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **2**; blast radius 17.094; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (90.6%), Connectivity (formerly Api Exposure) (56.4%), Guard Balance (formerly Safety Score) (35.4%), Complexity Load (formerly Cognitive Load) (18.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get` **(Compute Cores)** (Impact: 5.7)
  * `addValue` **(Parameter Forwarders)** (Impact: 3.8)
  * `init` **(Defensive Guards)** (Impact: 2.1)
  * `deinit` **(State Mutators)** (Impact: 1.9)
  * `addNames` **(Defensive Guards)** (Impact: 1.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (57.6%), Complexity Load (formerly Cognitive Load) (27.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Connectivity (formerly Api Exposure) (6.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dispatch` **(Defensive Guards)** (Impact: 4.8)
  * `main` **(I/O & Config Routines)** (Impact: 2.9)
    * *Intent:* // This example shows more advanced routing example, namely route groups // and route configuration....
  * `index` **(State Mutators)** (Impact: 2.5)
  * `infoDispatch` **(Parameter Forwarders)** (Impact: 2.4)
    * *Intent:* // special dispatch set in the info route
  * `page1` **(State Mutators)** (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Amplified Cascading Flux:* 2 instances
* *Memory Alloc (weighted view):* 0
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (53.6%), Complexity Load (formerly Cognitive Load) (18.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Connectivity (formerly Api Exposure) (8.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ws` **(State Mutators)** (Impact: 4.7)
  * `index` **(State Mutators)** (Impact: 2.7)
  * `main` **(Defensive Guards)** (Impact: 2.2)
    * *Intent:* // This example show how to upgrade a request to websocket.
  * `init` **(Parameter Forwarders)** (Impact: 2.0)
    * *Intent:* // context is any abitrary data that you want, you'll pass it to upgradeWebsocket
  * `clientMessage` **(Parameter Forwarders)** (Impact: 1.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Amplified Cascading Flux:* 2 instances
* *Memory Alloc (weighted view):* 0
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **2**; blast radius 55.39; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (73.2%), Test Surface (formerly Verification) (2.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `write` **(Defensive Guards)** (Impact: 2.1)
  * `allocBufferEmpty` **(State Mutators)** (Impact: 1.6)
  * `allocBufferLarge` **(State Mutators)** (Impact: 1.6)
  * `allocUnescape` **(State Mutators)** (Impact: 1.6)
  * `timeoutRequest` **(State Mutators)** (Impact: 1.6)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (55.9%), Complexity Load (formerly Cognitive Load) (26.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Connectivity (formerly Api Exposure) (4.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dispatch` **(Many-Argument Workhorses)** (Impact: 8.0)
    * *Intent:* // In example_3, our action type was: httpz.Action(*Handler). // In this example, we've changed it t...
  * `index` **(State Mutators)** (Impact: 2.5)
  * `main` **(Defensive Guards)** (Impact: 2.4)
    * *Intent:* // This example is very similar to 03_dispatch.zig, but shows how the action // state can be a diffe...
  * `admin` **(Defensive Guards)** (Impact: 2.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (54.2%), Concurrency Surface (formerly Concurrency) (27.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Connectivity (formerly Api Exposure) (7.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `uncaughtError` **(State Mutators)** (Impact: 2.8)
    * *Intent:* // If the handler defines the special "uncaughtError" function, it'll be // called when an action re...
  * `index` **(State Mutators)** (Impact: 2.6)
  * `main` **(I/O & Config Routines)** (Impact: 2.4)
    * *Intent:* // This example demonstrates using a custom Handler. It shows how to have // global state (here we s...
  * `hits` **(Parameter Forwarders)** (Impact: 2.4)
  * `notFound` **(State Mutators)** (Impact: 2.2)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 23.613; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (43.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (17.7%), Connectivity (formerly Api Exposure) (16.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (12.7%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `build` **(I/O & Config Routines)** (Impact: 15.8)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 7.336; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (43.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Complexity Load (formerly Cognitive Load) (11.5%), Connectivity (formerly Api Exposure) (5.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `index` **(State Mutators)** (Impact: 6.5)
  * `dispatch` **(Defensive Guards)** (Impact: 2.8)
    * *Intent:* // In addition to the special "notFound" and "uncaughtError" shown in example 2 // the special "disp...
  * `main` **(Defensive Guards)** (Impact: 2.0)
...
- `src/response.zig` -> **Karl Seguin** (100.0% isolated ownership) | Magnitude: 239.14
