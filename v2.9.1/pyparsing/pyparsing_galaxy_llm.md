...
## 1. EXECUTIVE SUMMARY
- **Scope:** 197 analyzed artifact(s), 64011 LOC.
- **Load-bearing artifact:** `pyparsing-3.3.2/pyparsing/util.py` -- 8 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `pyparsing-3.3.2/examples/README.md` -- pulls in 106 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `pyparsing-3.3.2/pyparsing/core.py` at magnitude 5607.8 (structural weight, not risk).
...
| Avg Path Length | 1.1841 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `2.097`
> **Composition Archetype:** `Mid Flat Project` (z +2.10; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 29%, Declarative / Non-Code 17%, Parameter Forwarders Files 14%, Large Core Modules (3) 12%, Large Core Modules (2) 8%
...
| Connectivity (formerly API Exposure) | 0.0 | 79.1 | 20.9 | 10.8 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 36.3 | 50.0 | 50.0 |
...
| dead_code | 528 | 48 | 2 | `pyparsing-3.3.2/tests/test_unit.py` |
...
- `_to_diagram_element` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/diagram/__init__.py`) -> Impact: **224.5** | LOC: 252
- `run_tests` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/core.py`) -> Impact: **172.1** | LOC: 241
- `__init__` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/core.py`) -> Impact: **170.4** | LOC: 120
- `with_line_numbers` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/testing.py`) -> Impact: **100.9** | LOC: 121
- `Test` **(Compute Cores)** (@ `pyparsing-3.3.2/examples/booleansearchparser.py`) -> Impact: **97.7** | LOC: 115
  * *Intent:* # fmt: off exprs = { "0": "help", "1": "help or hulp", "2": "help and hulp", "3": "help hulp", "4": "help and hulp or hilp", "5": "help or hulp and hi...
- `__init__` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/core.py`) -> Impact: **94.8** | LOC: 126
- `_parseNoCache` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/core.py`) -> Impact: **90.7** | LOC: 100
  * *Intent:* # @profile
- `make_compressed_re` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/util.py`) -> Impact: **89.6** | LOC: 93
- `one_of` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/helpers.py`) -> Impact: **87.7** | LOC: 138
- `infix_notation` **(Many-Argument Workhorses)** (@ `pyparsing-3.3.2/pyparsing/helpers.py`) -> Impact: **86.2** | LOC: 203
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **29**; blast radius 17.967; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.3%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (56.2%)
- **Documentation Coverage:** 72.8216% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(Many-Argument Workhorses)** (Impact: 172.1)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 170.4)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 94.8)
  * `_parseNoCache` **(Many-Argument Workhorses)** (Impact: 90.7)
    * *Intent:* # @profile
  * `parseImpl` **(Many-Argument Workhorses)** (Impact: 73.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **35**; blast radius 4.176; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (86.1%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (27.6%), Connectivity (formerly Api Exposure) (13.0%)
- **Documentation Coverage:** 42.1909% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testQuotedStrings` **(Compute Cores)** (Impact: 60.3)
  * `testInfixNotationEvalBoolExprUsingAstClasses` **(Compute Cores)** (Impact: 46.9)
  * `testRepeater` **(Compute Cores)** (Impact: 39.1)
  * `testNumericExpressions` **(Compute Cores)** (Impact: 34.3)
    * *Intent:* # disable parse actions that do type conversion so we don't accidentally trigger # conversion except...
  * `testNestedExpressionRandom` **(Compute Cores)** (Impact: 23.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 4.176; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.5%), Complexity Load (formerly Cognitive Load) (79.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
...
- **Global Archetype:** `file_cluster_7` (Drift: 0.0 IQR)
- **Magnitude:** 1121.59 | **LOC:** 250 | **CtrlFlow:** 8.6% | **Authorship Centralization:** 0.0%
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 4.222; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (88.5%), Complexity Load (formerly Cognitive Load) (79.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (7.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 5 instances
* *Amplified Cascading Flux:* 21 instances
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **8**; blast radius 7.344; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (63.4%)
- **Documentation Coverage:** 71.1538% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `one_of` **(Many-Argument Workhorses)** (Impact: 87.7)
  * `infix_notation` **(Many-Argument Workhorses)** (Impact: 86.2)
  * `nested_expr` **(Many-Argument Workhorses)** (Impact: 64.3)
  * `indentedBlock` **(Many-Argument Workhorses)** (Impact: 41.1)
    * *Intent:* """ .. deprecated:: 3.0.0 Use the :class:`IndentedBlock` class instead. Note that `IndentedBlock` ha...
  * `counted_array` **(Many-Argument Workhorses)** (Impact: 15.5)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **4**; blast radius 4.444; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (97.3%), Complexity Load (formerly Cognitive Load) (73.8%), Connectivity (formerly Api Exposure) (62.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 49.1228% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createRooms` **(Compute Cores)** (Impact: 28.3)
    * *Intent:* """ create rooms, using multiline string showing map layout string contains symbols for the followin...
  * `_do_command` **(Stateful Encapsulated Methods)** (Impact: 16.5)
  * `_do_command` **(Stateful Encapsulated Methods)** (Impact: 12.8)
  * `_do_command` **(Stateful Encapsulated Methods)** (Impact: 12.8)
  * `_do_command` **(Stateful Encapsulated Methods)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 4.176; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (56.1%)
- **Documentation Coverage:** 55.2632% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_to_diagram_element` **(Many-Argument Workhorses)** (Impact: 224.5)
  * `to_railroad` **(Many-Argument Workhorses)** (Impact: 35.0)
  * `mark_for_extraction` **(Many-Argument Workhorses)** (Impact: 21.0)
  * `_inner` **(Many-Argument Workhorses)** (Impact: 20.8)
  * `extract_into_diagram` **(Compute Cores)** (Impact: 12.0)
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **8**; blast radius 8.883; role: Transceiver (Middle-Tier)
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **10**; blast radius 27.125; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.8%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (73.1%)
- **Documentation Coverage:** 89.8305% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `make_compressed_re` **(Many-Argument Workhorses)** (Impact: 89.6)
  * `_collapse_string_to_ranges` **(Many-Argument Workhorses)** (Impact: 21.6)
  * `replaced_by_pep8` **(Stateful Encapsulated Methods)** (Impact: 14.3)
    * *Intent:* # Unwrap staticmethod/classmethod fn = getattr(fn, "__func__", fn) # (Presence of 'self' arg in sign...
  * `_set` **(Stateful Encapsulated Methods)** (Impact: 10.7)
  * `deprecate_argument` **(Generic / Templated Code)** (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 4.176; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (78.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (32.9%), Connectivity (formerly Api Exposure) (12.7%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_one_of_asKeyword_kwarg_emits_DeprecationWarning` **(Defensive Guards)** (Impact: 5.2)
  * `test_scanString_emits_DeprecationWarning` **(Type Conversions)** (Impact: 4.2)
  * `test_enablePackrat_emits_DeprecationWarning_and_cleanup` **(Defensive Guards)** (Impact: 3.5)
    * *Intent:* # Record initial packrat state; restore it after test completes initially_enabled = getattr(ParserEl...
  * `test_runTests_emits_DeprecationWarning` **(Parameter Forwarders)** (Impact: 3.1)
  * `test_infixNotation_emits_DeprecationWarning` **(Callbacks & Closures)** (Impact: 2.6)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **6**; blast radius 4.444; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (97.2%), Complexity Load (formerly Cognitive Load) (58.1%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (32.3%)
- **Documentation Coverage:** 45.122% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_all_tests` **(I/O & Config Routines)** (Impact: 26.7)
  * `offset_weekday` **(Many-Argument Workhorses)** (Impact: 19.6)
  * `_convert_abs_day_reference_to_date` **(Stateful Encapsulated Methods)** (Impact: 17.1)
  * `make_weekday_time_references` **(I/O & Config Routines)** (Impact: 13.4)
  * `_compute_timestamp` **(Stateful Encapsulated Methods)** (Impact: 11.3)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **6**; blast radius 6.439; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (53.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `with_line_numbers` **(Many-Argument Workhorses)** (Impact: 100.9)
  * `assertRunTestResults` **(Many-Argument Workhorses)** (Impact: 59.5)
  * `restore` **(Compute Cores)** (Impact: 13.1)
    * *Intent:* # reset pyparsing global state if ( ParserElement.DEFAULT_WHITE_CHARS != self._save_context["default...
  * `assertRaisesParseException` **(Defensive Guards)** (Impact: 9.5)
  * `assertParseAndCheckList` **(Many-Argument Workhorses)** (Impact: 8.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **5**; blast radius 4.222; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (87.9%), Complexity Load (formerly Cognitive Load) (64.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (11.0%)
- **Documentation Coverage:** 36.9565% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `post_test` **(Defensive Guards)** (Impact: 27.1)
    * *Intent:* # copy exprStack to evaluate and clear before running next test parsed_stack = exprStack[:] exprStac...
  * `_expfunc` **(Stateful Encapsulated Methods)** (Impact: 25.1)
    * *Intent:* ## The '^' operator is used for exponentiation on scalars and ## as a marker for unary operations on...
  * `_mulfunc` **(Stateful Encapsulated Methods)** (Impact: 25.0)
  * `test` **(I/O & Config Routines)** (Impact: 22.9)
    * *Intent:* ##----------------------------------------------------------------------------------- """ Tests the ...
  * `parse` **(Defensive Guards)** (Impact: 22.0)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **4**; blast radius 7.682; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (85.5%), Complexity Load (formerly Cognitive Load) (52.8%), Connectivity (formerly Api Exposure) (52.2%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 37.7358% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `eval_expr` **(Defensive Guards)** (Impact: 34.1)
    * *Intent:* # ----- Expression Evaluation ----- """Evaluate an expression built by pyparsing's infix_notation an...
  * `_apply_op` **(Stateful Encapsulated Methods)** (Impact: 30.9)
    * *Intent:* # Boolean ops if op in {"&&", "||"}: lv = bool(lhs) rv = bool(rhs) return _op_map[op](lv, rv) # Rela...
  * `call_function` **(Defensive Guards)** (Impact: 15.4)
    * *Intent:* # ----- Functions API (execution helper to share with CallStmtNode) ----- """Call a user-defined fun...
  * `assign_var` **(Defensive Guards)** (Impact: 13.2)
    * *Intent:* """Assign to an existing variable; if undeclared, declare using inferred type."""
  * `declare_var` **(Many-Argument Workhorses)** (Impact: 12.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 4.176; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.8%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (44.3%), Connectivity (formerly Api Exposure) (11.3%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `gen_expr_sequence` **(Compute Cores)** (Impact: 20.8)
  * `main` **(I/O & Config Routines)** (Impact: 16.2)
    * *Intent:* # ---------- Main suite ----------
  * `gen_csv` **(Type Conversions)** (Impact: 12.8)
    * *Intent:* # ---------- Corpora generators ----------
  * `bench_matchfirst_vs_or` **(Compute Cores)** (Impact: 11.2)
  * `make_term` **(Type Conversions)** (Impact: 10.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 4.176; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (94.2%), Complexity Load (formerly Cognitive Load) (74.9%), Connectivity (formerly Api Exposure) (50.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 45.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Test` **(Compute Cores)** (Impact: 97.7)
    * *Intent:* # fmt: off exprs = { "0": "help", "1": "help or hulp", "2": "help and hulp", "3": "help hulp", "4": ...
  * `parser` **(Compute Cores)** (Impact: 19.0)
    * *Intent:* """ This function returns a parser. The grammar should be like most full text search engines (Google...
  * `evaluateWord` **(Compute Cores)** (Impact: 16.4)
  * `_split_words` **(Stateful Encapsulated Methods)** (Impact: 11.4)
  * `GetWordWildcard` **(Compute Cores)** (Impact: 6.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 35 instances
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **6**; blast radius 4.222; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (89.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (49.0%), Connectivity (formerly Api Exposure) (26.7%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `make_verilog_bnf` **(I/O & Config Routines)** (Impact: 23.0)
  * `main` **(I/O & Config Routines)** (Impact: 11.8)
  * `test` **(Defensive Guards)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **5**; blast radius 14.212; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (94.7%), Connectivity (formerly Api Exposure) (79.1%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (44.4%)
- **Documentation Coverage:** 41.2281% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `from_parsed` **(Generic / Templated Code)** (Impact: 11.2)
  * `execute` **(Type Conversions)** (Impact: 10.9)
    * *Intent:* # Repeat-Until is a do-while: execute body, then check condition; stop when condition is true while ...
  * `from_parsed` **(Type Conversions)** (Impact: 10.9)
  * `from_parsed` **(Generic / Templated Code)** (Impact: 10.8)
  * `from_parsed` **(Generic / Templated Code)** (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 4.222; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (65.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **11**; blast radius 21.925; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.6%), Complexity Load (formerly Cognitive Load) (72.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (42.3%)
- **Documentation Coverage:** 59.375% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `expand_named_state_definition` **(Many-Argument Workhorses)** (Impact: 38.8)
    * *Intent:* """ Parse action to convert statemachine with named transitions to corresponding Python classes and ...
  * `expand_state_definition` **(Stateful Encapsulated Methods)** (Impact: 15.1)
    * *Intent:* """ Parse action to convert statemachine to corresponding Python classes and methods """
  * `find_module` **(Compute Cores)** (Impact: 6.3)
  * `load_module` **(Defensive Guards)** (Impact: 6.0)
  * `checkpath_iter` **(Defensive Guards)** (Impact: 5.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 4.222; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (65.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **6**; blast radius 4.222; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (83.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (48.1%), Complexity Load (formerly Cognitive Load) (48.0%)
- **Documentation Coverage:** 20.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `binary_array_comparison_op` **(Defensive Guards)** (Impact: 36.8)
    * *Intent:* """ Parse action to handle array membership tests, such as 'in', 'not in', etc. """
  * `binary_multi_op` **(Compute Cores)** (Impact: 29.4)
    * *Intent:* """ Parse action to handle binary Boolean operators 'and' and 'or', combining 2 or more condition ex...
  * `binary_comparison_op` **(Many-Argument Workhorses)** (Impact: 16.9)
    * *Intent:* """ Parse action called for '<', '>', '<=', and '>=' comparisons. Includes logic to handle chained i...
  * `regex_comparison_op` **(Many-Argument Workhorses)** (Impact: 14.6)
...
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (10.4%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
  * `style` **(I/O & Config Routines)** (Impact: 1.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 4.176; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (91.9%), Complexity Load (formerly Cognitive Load) (62.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (3.4%)
- **Documentation Coverage:** 47.3684% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handle_meta_command` **(Many-Argument Workhorses)** (Impact: 48.5)
    * *Intent:* # normalize to lowercase, and collapse whitespace lower = " ".join(cmd.lower().split()) line = cmd.s...
  * `repl` **(Defensive Guards)** (Impact: 25.6)
  * `_load_functions_from_file` **(Many-Argument Workhorses)** (Impact: 18.6)
  * `_print_functions` **(Stateful Encapsulated Methods)** (Impact: 5.5)
  * `_build_nodes_from_stmt_seq` **(Stateful Encapsulated Methods)** (Impact: 4.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 4.176; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (74.3%)
- **Documentation Coverage:** 51.1321% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handle_repetition` **(Type Conversions)** (Impact: 12.3)
  * `handle_macro` **(Type Conversions)** (Impact: 12.3)
  * `make_generator` **(Compute Cores)** (Impact: 9.2)
  * `handle_literal` **(Compute Cores)** (Impact: 9.0)
  * `recurse_list` **(Compute Cores)** (Impact: 7.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 21 instances
