...
## 1. EXECUTIVE SUMMARY
- **Scope:** 7 analyzed artifact(s), 2035 LOC.
- **Load-bearing artifact:** `fixdeletetree.zig` -- 2 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `runtest.zig` -- pulls in 3 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `zigup.zig` at magnitude 588.86 (structural weight, not risk).
...
> **File Composition:** Large Core Modules (2) 71%, Data / Markup / Trivial 14%, Large Core Modules (3) 14%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 48.1 | 50.0 | 49.7 | 50.0 | 50.0 |
...
- `main2` **(Defensive Guards)** (@ `zigup.zig`) -> Impact: **56.9** | LOC: 177
- `copyEnvDir` **(Many-Argument Workhorses)** (@ `runtest.zig`) -> Impact: **46.0** | LOC: 71
- `main` **(Defensive Guards)** (@ `runtest.zig`) -> Impact: **41.2** | LOC: 184
- `verifyPathLink` **(Defensive Guards)** (@ `zigup.zig`) -> Impact: **35.4** | LOC: 84
...
- `cleanCompilers` **(Defensive Guards)** (@ `zigup.zig`) -> Impact: **28.3** | LOC: 47
- `byteSwapAllFields` **(Many-Argument Workhorses)** (@ `zip.zig`) -> Impact: **20.9** | LOC: 36
- `installCompiler` **(Defensive Guards)** (@ `zigup.zig`) -> Impact: **19.9** | LOC: 79
- `saveInstallDir` **(Defensive Guards)** (@ `zigup.zig`) -> Impact: **17.3** | LOC: 34
- `writeZip` **(Many-Argument Workhorses)** (@ `zip.zig`) -> Impact: **17.3** | LOC: 66
- `parse` **(Defensive Guards)** (@ `zigup.zig`) -> Impact: **17.1** | LOC: 31
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 114.942; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (48.3%), Debt Markers (formerly Tech Debt) (15.9%), Complexity Load (formerly Cognitive Load) (14.5%), Connectivity (formerly Api Exposure) (8.9%)
- **Documentation Coverage:** 48.125% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main2` **(Defensive Guards)** (Impact: 56.9)
  * `verifyPathLink` **(Defensive Guards)** (Impact: 35.4)
    * *Intent:* /// Verify that path_link will work. It verifies that `path_link` is /// in PATH and there is no zig...
  * `cleanCompilers` **(Defensive Guards)** (Impact: 28.3)
  * `installCompiler` **(Defensive Guards)** (Impact: 19.9)
  * `saveInstallDir` **(Defensive Guards)** (Impact: 17.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 114.942; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (80.5%), Guard Balance (formerly Safety Score) (60.1%), Complexity Load (formerly Cognitive Load) (22.6%), Debt Markers (formerly Tech Debt) (21.3%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `byteSwapAllFields` **(Many-Argument Workhorses)** (Impact: 20.9)
  * `writeZip` **(Many-Argument Workhorses)** (Impact: 17.3)
  * `main` **(I/O & Config Routines)** (Impact: 16.6)
  * `Zipper` **(Type Conversions)** (Impact: 8.7)
  * `isBadFilename` **(Defensive Guards)** (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 114.942; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (34.1%), Complexity Load (formerly Cognitive Load) (14.4%), Debt Markers (formerly Tech Debt) (11.4%), Dead Code Surface (formerly Dead Code) (10.4%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `copyEnvDir` **(Many-Argument Workhorses)** (Impact: 46.0)
  * `main` **(Defensive Guards)** (Impact: 41.2)
  * `isCompilerFilesEntry` **(Defensive Guards)** (Impact: 12.0)
  * `containsCompiler` **(Defensive Guards)** (Impact: 5.5)
  * `compilersArg` **(Compute Cores)** (Impact: 4.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 114.942; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Guard Balance (formerly Safety Score) (54.6%), Complexity Load (formerly Cognitive Load) (36.4%), Debt Markers (formerly Tech Debt) (29.4%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(Defensive Guards)** (Impact: 14.5)
  * `cmdlineArgs` **(Defensive Guards)** (Impact: 5.8)
  * `fatal` **(Generic / Templated Code)** (Impact: 1.9)
  * `oom` **(State Mutators)** (Impact: 1.6)
  * `usage` **(Defensive Guards)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 114.942; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (84.4%), Debt Markers (formerly Tech Debt) (61.8%), Guard Balance (formerly Safety Score) (48.6%), Complexity Load (formerly Cognitive Load) (15.7%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(Defensive Guards)** (Impact: 9.0)
  * `consoleCtrlHandler` **(Defensive Guards)** (Impact: 7.0)
  * `SetConsoleCtrlHandler` **(State Mutators)** (Impact: 1.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **2**; blast radius 310.345; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (34.1%), Debt Markers (formerly Tech Debt) (26.9%), Complexity Load (formerly Cognitive Load) (8.6%), Test Surface (formerly Verification) (2.5%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `deleteTree` **(Defensive Guards)** (Impact: 15.0)
    * *Intent:* // // TODO: we should fix std library to address these issues //
  * `deleteTreeAbsolute` **(Defensive Guards)** (Impact: 3.2)
**Structural Signatures (Net Mitigated Signals):**
