...
## 1. EXECUTIVE SUMMARY
- **Scope:** 11083 analyzed artifact(s), 1215767 LOC.
- **Load-bearing artifact:** `includes/MediaWikiServices.php` -- 481 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `languages/messages/MessagesSr_ec.php` -- pulls in 1213 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `includes/Parser/Parser.php` at magnitude 9114.26 (structural weight, not risk).
...
|---|---|---|
| Modularity | 0.4529 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 4.3714 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **File Composition:** Data / Markup / Trivial 50%, Declarative / Non-Code 10%, Large Core Modules (3) 9%, Interface Declarations Files 7%, Parameter Forwarders Files 6%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 20.5 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 3.8 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 89.5 | 3.1 | 0.0 | 0.0 |
...
|---|---|---|---|---|
| memory | 19415 | 2800 | 4 | `includes/ServiceWiring.php` |
| cleanup | 1100 | 499 | 0 | `resources/lib/ooui/oojs-ui-core.js` |
| guards | 47250 | 3824 | 11 | `resources/lib/vue-test-utils/vue-test-utils.browser.js` |
| danger | 9578 | 1728 | 2 | `includes/Language/Language.php` |
| concurrency | 6348 | 703 | 0 | `tests/phpunit/unit/includes/libs/Leximorph/Handler/GrammarTest.php` |
| connectivity | 34252 | 4943 | 8 | `includes/MainConfigSchema.php` |
| io | 2585 | 480 | 0 | `tests/phan/TaintCheckAnnotationsTest.php` |
| crypto | 0 | 0 | 0 | - |
...
| serialization | 730 | 333 | 0 | `includes/JobQueue/JobQueueRedis.php` |
| regex | 2554 | 642 | 0 | `resources/lib/vue-test-utils/vue-test-utils.browser.js` |
| events | 1827 | 265 | 0 | `resources/lib/qunitjs/qunit.js` |
| tests | 20112 | 1406 | 2 | `tests/phpunit/includes/Auth/AuthManagerTest.php` |
| docs | 47410 | 5694 | 11 | `includes/MainConfigSchema.php` |
| debt | 4941 | 1807 | 1 | `tests/phan/TaintCheckAnnotationsTest.php` |
| mutation | 153319 | 5254 | 31 | `resources/lib/vue-test-utils/vue-test-utils.browser.js` |
| dead_code | 26247 | 4054 | 7 | `includes/MediaWikiServices.php` |
| credential | 604 | 130 | 0 | `includes/Installer/i18n/ja.json` |
| threat | 3888 | 447 | 0 | `resources/lib/ooui/oojs-ui-core.js` |
...
- `sprintfDate` **(Many-Argument Workhorses)** (@ `includes/Language/Language.php`) -> Impact: **1229.0** | LOC: 1862
...
- `getPermissionStatus` **(Many-Argument Workhorses)** (@ `includes/Permissions/PermissionManager.php`) -> Impact: **953.2** | LOC: 1428
...
- `internalParse` **(Many-Argument Workhorses)** (@ `includes/Parser/Parser.php`) -> Impact: **951.4** | LOC: 1868
...
- `requireBeautifier$1` **(Many-Argument Workhorses)** (@ `resources/lib/vue-test-utils/vue-test-utils.browser.js`) -> Impact: **781.1** | LOC: 1454
- `handleExternalLinks` **(Many-Argument Workhorses)** (@ `includes/Parser/Parser.php`) -> Impact: **712.1** | LOC: 1910
...
- `handleTables` **(Many-Argument Workhorses)** (@ `includes/Parser/Parser.php`) -> Impact: **691.8** | LOC: 1872
  * *Intent:* /** * Parse the wiki syntax used to render tables. * * @param string $text * @return string */
- `handleInternalLinks2` **(Many-Argument Workhorses)** (@ `includes/Parser/Parser.php`) -> Impact: **683.6** | LOC: 1877
  * *Intent:* /** * Process [[ ]] wikilinks (RIL) * @param string &$s * @return LinkHolderArray */
- `cleanSig` **(Many-Argument Workhorses)** (@ `includes/Parser/Parser.php`) -> Impact: **647.2** | LOC: 1721
...
- `makeFormattedData` **(Many-Argument Workhorses)** (@ `includes/Media/FormatMetadata.php`) -> Impact: **558.7** | LOC: 709
...
- `requireTokenizer` **(Defensive Guards)** (@ `resources/lib/vue-test-utils/vue-test-utils.browser.js`) -> Impact: **501.4** | LOC: 1427
...
| `maintenance` | 200 | 18249.56 | 36.43% | 33.79% |
...
- **Unknown Dependencies:** `96081` packages imported that bypass the Zero-Trust whitelist.
...
- **Blast Radius:** changing it is visible to **67** in-repo importer(s); it depends on **136**; blast radius 1.515; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (81.0%), Complexity Load (formerly Cognitive Load) (54.3%)
- **Documentation Coverage:** 1.5789% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `internalParse` **(Many-Argument Workhorses)** (Impact: 951.4)
    * *Intent:* /** * Helper function for parse() that transforms wiki markup into half-parsed * HTML. Only called f...
  * `handleExternalLinks` **(Many-Argument Workhorses)** (Impact: 712.1)
    * *Intent:* /** * Replace external links (REL) * * Note: this is all very hackish and the order of execution mat...
  * `handleTables` **(Many-Argument Workhorses)** (Impact: 691.8)
    * *Intent:* /** * Parse the wiki syntax used to render tables. * * @param string $text * @return string */
  * `handleInternalLinks2` **(Many-Argument Workhorses)** (Impact: 683.6)
    * *Intent:* /** * Process [[ ]] wikilinks (RIL) * @param string &$s * @return LinkHolderArray */
  * `cleanSig` **(Many-Argument Workhorses)** (Impact: 647.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001258 | `Ripple Effect (Closeness):` 0.092907
...
- **Blast Radius:** changing it is visible to **197** in-repo importer(s); it depends on **65**; blast radius 7.665; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (66.7%), Complexity Load (formerly Cognitive Load) (49.6%)
- **Documentation Coverage:** 1.227% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `sprintfDate` **(Many-Argument Workhorses)** (Impact: 1229.0)
    * *Intent:* * $ts is UTC if $zone is not given. * * @param string $format * @param string $ts 14-character times...
  * `truncateInternal` **(Many-Argument Workhorses)** (Impact: 202.4)
    * *Intent:* * one common method, allowing users to provide the length measurement function and * function for fi...
  * `formatNumInternal` **(Many-Argument Workhorses)** (Impact: 76.8)
    * *Intent:* /** * Internal implementation function, shared between formatNum and formatNumNoSeparators. * * @par...
  * `formatTimePeriod` **(Many-Argument Workhorses)** (Impact: 47.9)
    * *Intent:* /** * Formats a time given in seconds into a string representation of that time. * * @param int|floa...
  * `translateBlockExpiry` **(Compute Cores)** (Impact: 32.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001679 | `Ripple Effect (Closeness):` 0.108797
...
- **Blast Radius:** changing it is visible to **99** in-repo importer(s); it depends on **124**; blast radius 2.606; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.3%), Debt Markers (formerly Tech Debt) (85.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (57.7%)
- **Documentation Coverage:** 1.6713% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `showPermissionStatus` **(Many-Argument Workhorses)** (Impact: 484.9)
    * *Intent:* /** * Output a standard permission error page * * @param PermissionStatus $status * @param string|nu...
  * `sendCacheControl` **(I/O & Config Routines)** (Impact: 326.1)
    * *Intent:* /** * Send cache control HTTP headers */
  * `getHeadLinksSyndicationArray` **(Stateful Encapsulated Methods)** (Impact: 85.0)
    * *Intent:* /** * Get head links relating to syndication feeds. * * @param Config $config * @return array */
  * `getJSVars` **(Compute Cores)** (Impact: 58.3)
    * *Intent:* * in other words, page-independent/site-wide variables (without state). * These would add a blocking...
  * `addParserOutputMetadata` **(Compute Cores)** (Impact: 47.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001089 | `Ripple Effect (Closeness):` 0.087479
...
- **Blast Radius:** changing it is visible to **26** in-repo importer(s); it depends on **135**; blast radius 0.676; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (76.0%), Complexity Load (formerly Cognitive Load) (44.9%)
- **Documentation Coverage:** 15.6522% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getPreviewText` **(I/O & Config Routines)** (Impact: 125.1)
    * *Intent:* /** * Get the rendered text for previewing. * @throws MWException * @return string */
  * `internalAttemptSave` **(Many-Argument Workhorses)** (Impact: 116.0)
    * *Intent:* * @param bool $markAsBot True if edit is being made under the bot right * and the bot wishes the edi...
  * `handleStatus` **(Many-Argument Workhorses)** (Impact: 85.5)
    * *Intent:* /** * Handle status, such as after attempt save * * @param EditPageStatus $status * @param array|fal...
  * `importFormDataPosted` **(Compute Cores)** (Impact: 77.9)
  * `edit` **(I/O & Config Routines)** (Impact: 49.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000786 | `Ripple Effect (Closeness):` 0.058435
...
- **Blast Radius:** changing it is visible to **30** in-repo importer(s); it depends on **33**; blast radius 0.428; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.4%), Guard Balance (formerly Safety Score) (93.0%), Mutation Surface (formerly State Flux) (85.0%), Complexity Load (formerly Cognitive Load) (44.3%)
- **Documentation Coverage:** 1.4793% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `sourceStream` **(Many-Argument Workhorses)** (Impact: 52.4)
    * *Intent:* /** @inheritDoc */
  * `assessConnectionLoss` **(Many-Argument Workhorses)** (Impact: 48.3)
    * *Intent:* * is considered acceptable for DBO_TRX logic. * * If state was lost, but that loss was discovered du...
  * `__construct` **(Defensive Guards)** (Impact: 37.0)
    * *Intent:* /** * @note exceptions for missing libraries/drivers should be thrown in initConnection() * @param a...
  * `attemptQuery` **(Many-Argument Workhorses)** (Impact: 34.5)
    * *Intent:* * - write callers * - last write time * - affected row count of the last write * - whether writes oc...
  * `handleErroredQuery` **(Many-Argument Workhorses)** (Impact: 32.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000191 | `Ripple Effect (Closeness):` 0.077734
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **60**; blast radius 0.051; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.9%), Connectivity (formerly Api Exposure) (68.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.3%)
- **Documentation Coverage:** 2.3411% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `secureAndSplit` **(Many-Argument Workhorses)** (Impact: 256.9)
    * *Intent:* * * Assumes that $text is urldecoded * and uses underscores, but not otherwise munged. This function...
  * `getLocalURL` **(Compute Cores)** (Impact: 39.2)
    * *Intent:* * $wgServer is prepended to make an absolute URL. * * @see self::getFullURL to always get an absolut...
  * `convertByteClassToUnicodeClass` **(Compute Cores)** (Impact: 38.6)
    * *Intent:* /** * Utility method for converting a character sequence from bytes to Unicode. * * Primary usecase ...
  * `makeName` **(Many-Argument Workhorses)** (Impact: 18.2)
    * *Intent:* /** * Make a prefixed DB key from a DB key and a namespace index * * @param int $ns Numerical repres...
  * `loadFromRow` **(Defensive Guards)** (Impact: 15.6)
...
- **Blast Radius:** changing it is visible to **66** in-repo importer(s); it depends on **59**; blast radius 0.661; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.4%), Debt Markers (formerly Tech Debt) (54.7%), Complexity Load (formerly Cognitive Load) (45.6%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `makeImageLink` **(Many-Argument Workhorses)** (Impact: 198.8)
    * *Intent:* * link-url URL to link to * link-title LinkTarget object to link to * link-target Value for the targ...
  * `makeThumbLink2` **(Many-Argument Workhorses)** (Impact: 195.6)
    * *Intent:* /** * @param LinkTarget $title * @param File|false $file * @param array $frameParams * @param array ...
  * `formatHiddenCategories` **(Compute Cores)** (Impact: 66.8)
    * *Intent:* /** * Returns HTML for the "hidden categories on this page" list. * * @since 1.16.3 * @param array $...
  * `makeBrokenImageLinkObj` **(Many-Argument Workhorses)** (Impact: 42.0)
    * *Intent:* /** * Make a "broken" link to an image * * @since 1.16.3 * @param LinkTarget $title * @param string ...
  * `userToolLinkArray` **(Many-Argument Workhorses)** (Impact: 39.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000233 | `Ripple Effect (Closeness):` 0.076241
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **96**; blast radius 0.051; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (63.5%), Complexity Load (formerly Cognitive Load) (47.8%)
- **Documentation Coverage:** 20.4301% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `addParserOutputInfo` **(Many-Argument Workhorses)** (Impact: 125.1)
    * *Intent:* /** * Add information from the parser output to the result string * * @param string &$out The "actua...
  * `processResults` **(Many-Argument Workhorses)** (Impact: 69.6)
    * *Intent:* /** * This processes test results and updates the known failures info for the test * * @param Parser...
  * `runParsoidTest` **(Many-Argument Workhorses)** (Impact: 60.0)
    * *Intent:* * running in 'integrated' mode, and compare the output against the * expected results. * * Prints st...
  * `wt2html` **(Many-Argument Workhorses)** (Impact: 50.8)
    * *Intent:* /** * Run wt2html on the test * * @param Parsoid $parsoid * @param PageConfig $pageConfig * @param P...
  * `updateTests` **(Many-Argument Workhorses)** (Impact: 46.8)
...
- **Blast Radius:** changing it is visible to **65** in-repo importer(s); it depends on **172**; blast radius 0.526; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.1%), Complexity Load (formerly Cognitive Load) (42.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (34.9%)
- **Documentation Coverage:** 5.2632% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `newRevisionsFromBatch` **(Many-Argument Workhorses)** (Impact: 94.5)
    * *Intent:* * always be loaded. * 'content' - whether the actual content of the slots should be * preloaded. * '...
  * `newRevisionFromRowAndSlots` **(Many-Argument Workhorses)** (Impact: 71.5)
    * *Intent:* * @param stdClass $row A database row generated from a query based on RevisionSelectQueryBuilder * @...
  * `getSlotRowsForBatch` **(Many-Argument Workhorses)** (Impact: 48.9)
    * *Intent:* * all slots are fetched * 'blobs' - whether the serialized content of each slot should be loaded. * ...
  * `newRevisionFromArchiveRowAndSlots` **(Many-Argument Workhorses)** (Impact: 47.0)
    * *Intent:* * @param stdClass $row * @param null|stdClass[]|RevisionSlots $slots * - Database rows generated fro...
  * `getRevisionIdsBetween` **(Many-Argument Workhorses)** (Impact: 41.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000395 | `Ripple Effect (Closeness):` 0.091823
...
- **Blast Radius:** changing it is visible to **21** in-repo importer(s); it depends on **53**; blast radius 0.285; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.6%), Complexity Load (formerly Cognitive Load) (44.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (43.1%)
- **Documentation Coverage:** 1.0309% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `upgradeRow` **(I/O & Config Routines)** (Impact: 325.6)
    * *Intent:* /** * Fix assorted version-related problems with the image row by reloading it from the file * @stab...
  * `recordUpload3` **(Many-Argument Workhorses)** (Impact: 153.9)
    * *Intent:* * @since 1.35 * @stable to override * @param string $oldver * @param string $comment * @param string...
  * `upload` **(Many-Argument Workhorses)** (Impact: 67.1)
    * *Intent:* * @param array|false $props File properties, if known. This can be used to * reduce the upload time ...
  * `publishTo` **(Many-Argument Workhorses)** (Impact: 33.9)
    * *Intent:* * object with the archive name in the "value" member on success. * * The archive name should be pass...
  * `getHistory` **(Many-Argument Workhorses)** (Impact: 25.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000238 | `Ripple Effect (Closeness):` 0.060023
...
- **Blast Radius:** changing it is visible to **75** in-repo importer(s); it depends on **65**; blast radius 1.721; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (42.1%), Connectivity (formerly Api Exposure) (37.3%)
- **Documentation Coverage:** 1.6529% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getPersonalToolsForMakeListItem` **(Many-Argument Workhorses)** (Impact: 79.5)
    * *Intent:* /** * Create an array of personal tools items from the data in the quicktemplate * stored by SkinTem...
  * `buildNavUrls` **(I/O & Config Routines)** (Impact: 47.8)
    * *Intent:* /** * Build array of common navigation links. * Assumes thispage property has been set before execut...
  * `makeToolbox` **(Defensive Guards)** (Impact: 30.2)
    * *Intent:* /** * Create an array of common toolbox items from the data in the quicktemplate * stored by SkinTem...
  * `createSidebarItem` **(Many-Argument Workhorses)** (Impact: 27.1)
    * *Intent:* /** * Generates an array item for the sidebar * @param string $target Target link in the form of an ...
  * `getDefaultModules` **(I/O & Config Routines)** (Impact: 26.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000873 | `Ripple Effect (Closeness):` 0.086996
...
- **Blast Radius:** changing it is visible to **19** in-repo importer(s); it depends on **58**; blast radius 0.593; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.5%), Complexity Load (formerly Cognitive Load) (45.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (41.8%)
- **Documentation Coverage:** 2.6087% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `showDiffPage` **(Many-Argument Workhorses)** (Impact: 438.1)
    * *Intent:* /** * @param bool $diffOnly */
  * `getDiffBody` **(I/O & Config Routines)** (Impact: 28.6)
    * *Intent:* /** * Get the diff table body, without header * * @return string|false */
  * `addHeader` **(Many-Argument Workhorses)** (Impact: 27.9)
    * *Intent:* /** * Add the header to a diff body * * @param string $diff Diff body * @param string $otitle Old re...
  * `loadRevisionData` **(I/O & Config Routines)** (Impact: 26.4)
    * *Intent:* /** * Load revision metadata for the specified revisions. If newid is 0, then compare * the old revi...
  * `getMultiNotice` **(I/O & Config Routines)** (Impact: 24.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000174 | `Ripple Effect (Closeness):` 0.061291
...
- **Blast Radius:** changing it is visible to **155** in-repo importer(s); it depends on **77**; blast radius 2.803; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (60.1%), Connectivity (formerly Api Exposure) (49.1%)
- **Documentation Coverage:** 5.4237% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `initFromJson` **(Defensive Guards)** (Impact: 109.8)
    * *Intent:* /** * Initialize member fields from an array returned by jsonSerialize(). * @param array $jsonData *...
  * `getLinkList` **(Many-Argument Workhorses)** (Impact: 95.9)
    * *Intent:* * the metadata. * * Each element of the returned array has a LinkTarget as the 'link' * property. Lo...
  * `collectMetadata` **(Compute Cores)** (Impact: 71.2)
    * *Intent:* /** * Adds the metadata collected in this ParserOutput to the supplied * ContentMetadataCollector. T...
  * `mergeMapStrategy` **(Stateful Encapsulated Methods)** (Impact: 35.2)
  * `mergeInternalMetaDataFrom` **(Compute Cores)** (Impact: 27.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000691 | `Ripple Effect (Closeness):` 0.087461
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **28**; blast radius 0.78; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (98.2%), Guard Balance (formerly Safety Score) (97.2%), Complexity Load (formerly Cognitive Load) (46.6%)
- **Documentation Coverage:** 2.2857% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `selectSQLText` **(Many-Argument Workhorses)** (Impact: 136.8)
    * *Intent:* /** @inheritDoc */
  * `makeList` **(Many-Argument Workhorses)** (Impact: 104.5)
    * *Intent:* /** @inheritDoc */
  * `tableNamesWithIndexClauseOrJOIN` **(Many-Argument Workhorses)** (Impact: 42.6)
    * *Intent:* /** * Get the aliased table name clause for a FROM clause * which might have a JOIN and/or USE INDEX...
  * `assertValidUpsertSetArray` **(Many-Argument Workhorses)** (Impact: 28.1)
    * *Intent:* /** * @param array $set Combined column/literal assignment map and SQL assignment list * @param stri...
  * `buildSuperlative` **(Stateful Encapsulated Methods)** (Impact: 25.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.3e-05 | `Ripple Effect (Closeness):` 0.061658
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **117**; blast radius 0.051; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (60.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (57.7%), Complexity Load (formerly Cognitive Load) (27.6%), Connectivity (formerly Api Exposure) (6.2%)
- **Documentation Coverage:** 61.3636% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testAccountCreation` **(Many-Argument Workhorses)** (Impact: 97.8)
    * *Intent:* /** * @dataProvider provideAccountCreation */
  * `testAuthentication` **(Many-Argument Workhorses)** (Impact: 96.7)
    * *Intent:* /** * @dataProvider provideAuthentication */
  * `testAccountLink` **(Many-Argument Workhorses)** (Impact: 54.8)
    * *Intent:* /** * @dataProvider provideAccountLink */
  * `initializeManager` **(Defensive Guards)** (Impact: 40.7)
    * *Intent:* /** * Initialize $this->manager * @param bool $regen Force a call to $this->initializeConfig() */
  * `testGetAuthenticationRequests` **(Many-Argument Workhorses)** (Impact: 38.5)
...
- **Blast Radius:** changing it is visible to **55** in-repo importer(s); it depends on **62**; blast radius 0.69; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (63.5%), Complexity Load (formerly Cognitive Load) (49.7%)
- **Documentation Coverage:** 10.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `autoCreateUser` **(Many-Argument Workhorses)** (Impact: 128.2)
    * *Intent:* * - the ID of a PrimaryAuthenticationProvider, * - one of the self::AUTOCREATE_SOURCE_* constants * ...
  * `continueAuthentication` **(Many-Argument Workhorses)** (Impact: 118.6)
    * *Intent:* * new AuthenticationRequests should be made (if any), then * AuthManager::continueAuthentication() s...
  * `continueAccountCreation` **(Many-Argument Workhorses)** (Impact: 115.3)
    * *Intent:* /** * Continue an account creation flow * @param AuthenticationRequest[] $reqs * @return Authenticat...
  * `getAuthenticationRequestsInternal` **(Many-Argument Workhorses)** (Impact: 47.9)
    * *Intent:* /** * Internal request lookup for self::getAuthenticationRequests * * @param string $providerAction ...
  * `getAuthenticationRequests` **(Compute Cores)** (Impact: 44.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000473 | `Ripple Effect (Closeness):` 0.089036
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **27**; blast radius 0.066; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (57.7%), Complexity Load (formerly Cognitive Load) (35.5%)
- **Documentation Coverage:** 1.7341% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `urlFunction` **(Many-Argument Workhorses)** (Impact: 60.6)
    * *Intent:* /** * @param string $func * @param string $s * @param string|null $arg * @return array|string */
  * `displaytitle` **(Many-Argument Workhorses)** (Impact: 38.4)
    * *Intent:* /** * Override the title of the page when viewed, provided we've been given a * title which will nor...
  * `revisionid` **(Compute Cores)** (Impact: 31.8)
    * *Intent:* /** * Get the id from the last revision of a specified page. * @param Parser $parser * @param string...
  * `tagObj` **(Many-Argument Workhorses)** (Impact: 31.1)
    * *Intent:* /** * Parser function to extension tag adaptor * @param Parser $parser * @param PPFrame $frame * @pa...
  * `getCachedRevisionObject` **(Many-Argument Workhorses)** (Impact: 31.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-05 | `Ripple Effect (Closeness):` 9e-05
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **63**; blast radius 0.064; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (61.7%), Complexity Load (formerly Cognitive Load) (47.6%)
- **Documentation Coverage:** 4.878% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `profilePreferences` **(Many-Argument Workhorses)** (Impact: 156.1)
    * *Intent:* /** * @todo Inject user Language instead of using context. * @param User $user * @param IContextSour...
  * `getResetKinds` **(Defensive Guards)** (Impact: 65.8)
    * *Intent:* /** * @param User $user * @param IContextSource $context * @param array|null $options * @return stri...
  * `__construct` **(Many-Argument Workhorses)** (Impact: 50.6)
    * *Intent:* * @param AuthManager $authManager * @param LinkRenderer $linkRenderer * @param NamespaceInfo $nsInfo...
  * `getOptionFromUser` **(Defensive Guards)** (Impact: 50.1)
    * *Intent:* /** * Pull option from a user account. Handles stuff like array-type preferences. * * @deprecated si...
  * `watchlistPreferences` **(Many-Argument Workhorses)** (Impact: 44.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.1e-05 | `Ripple Effect (Closeness):` 0.000376
...
- **Global Archetype:** `file_cluster_15` (Drift: 0.0 IQR)
- **Magnitude:** 1722.52 | **LOC:** 3429 | **CtrlFlow:** 16.7% | **Authorship Centralization:** 15.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **97**; blast radius 0.051; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.8%), Connectivity (formerly Api Exposure) (76.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (67.1%)
- **Documentation Coverage:** 1.8868% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `loadFromRow` **(Many-Argument Workhorses)** (Impact: 53.7)
    * *Intent:* /** * Initialize this object from a row from the user table. * * @param stdClass $row Row from the u...
  * `load` **(Compute Cores)** (Impact: 48.8)
    * *Intent:* /** * Load the user table data for this object from the source given by mFrom. * * @param int $flags...
  * `newSystemUser` **(Many-Argument Workhorses)** (Impact: 23.3)
    * *Intent:* * - validate: Type of validation to use: * - false No validation * - 'valid' Valid for batch process...
  * `setEmailWithConfirmation` **(Compute Cores)** (Impact: 20.9)
    * *Intent:* /** * Set the user's e-mail address and send a confirmation mail if needed. * * @since 1.20 * @param...
  * `clearInstanceCache` **(Compute Cores)** (Impact: 19.1)
...
* *Risk/State:* `safety_bypasses: 9`, `state_mutation: 347`, `dead_code: 4`, `planned_debt: 4`, `fragile_debt: 3`
* *Architecture:* `api: 139`, `import: 59`
...
- **Blast Radius:** changing it is visible to **98** in-repo importer(s); it depends on **120**; blast radius 2.07; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.6%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (50.5%)
- **Documentation Coverage:** 1.0929% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `doUpdateRestrictions` **(Many-Argument Workhorses)** (Impact: 124.3)
    * *Intent:* /** * Update the article's restriction field, and leave a log entry. * This works for protection bot...
  * `doUserEditContent` **(Many-Argument Workhorses)** (Impact: 34.6)
    * *Intent:* * edit-already-exists: In creation mode, but the article already exists. * * Extensions may define a...
  * `getDerivedDataUpdater` **(Many-Argument Workhorses)** (Impact: 28.9)
    * *Intent:* * * @since 1.32 * * @param UserIdentity|null $forUser The user that will be used for, or was used fo...
  * `replaceSectionAtRev` **(Many-Argument Workhorses)** (Impact: 26.4)
    * *Intent:* /** * @param string|int|null|false $sectionId Section identifier as a number or string * (e.g. 0, 1 ...
  * `updateRevisionOn` **(Many-Argument Workhorses)** (Impact: 21.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002333 | `Ripple Effect (Closeness):` 0.090092
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **33**; blast radius 0.186; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.4%), Debt Markers (formerly Tech Debt) (72.1%), Complexity Load (formerly Cognitive Load) (36.1%)
- **Documentation Coverage:** 2.9412% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getDirListPageInternal` **(Many-Argument Workhorses)** (Impact: 54.9)
    * *Intent:* /** * Do not call this function outside of SwiftFileBackendFileList * * @param string $fullCont Reso...
  * `getFileListPageInternal` **(Many-Argument Workhorses)** (Impact: 34.1)
    * *Intent:* /** * Do not call this function outside of SwiftFileBackendFileList * * @param string $fullCont Reso...
  * `requestMultiWithAuth` **(Stateful Encapsulated Methods)** (Impact: 31.4)
    * *Intent:* /** * Perform a batch of authenticated HTTP requests * * @param array $reqs An array of request data...
  * `objectListing` **(Many-Argument Workhorses)** (Impact: 31.2)
    * *Intent:* /** * Get a list of objects under a container. * Either just the names or a list of stdClass objects...
  * `__construct` **(Defensive Guards)** (Impact: 26.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3e-05 | `Ripple Effect (Closeness):` 0.000361
...
- `includes/Specials/SpecialWatchlist.php` -> Churn: **63.46%** | Cog Load: 60.4234% | Debt: 91.6162%
...
- `includes/MediaWikiServices.php` -> **Severity: 0.93** (Bridge: 0.0217 * Flux: 42.8204%)
- `includes/SpecialPage/SpecialPageFactory.php` -> **Severity: 0.729** (Bridge: 0.0077 * Flux: 94.6894%)
- `includes/Page/WikiPage.php` -> **Severity: 0.233** (Bridge: 0.0023 * Flux: 100.0%)
- `includes/Revision/RevisionRecord.php` -> **Severity: 0.17** (Bridge: 0.0017 * Flux: 99.9992%)
- `includes/Language/Language.php` -> **Severity: 0.168** (Bridge: 0.0017 * Flux: 100.0%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `includes/Language/Language.php` -> **Severity: 10.827** (Embedded: 0.1088 * Error Risk: 99.518%)
- `includes/Language/Message/Message.php` -> **Severity: 9.677** (Embedded: 0.1038 * Error Risk: 93.262%)
- `includes/Context/RequestContext.php` -> **Severity: 9.471** (Embedded: 0.0999 * Error Risk: 94.8471%)
- `includes/libs/ObjectCache/BagOStuff.php` -> **Severity: 9.224** (Embedded: 0.0974 * Error Risk: 94.7218%)
- `includes/Parser/Parser.php` -> **Severity: 9.223** (Embedded: 0.0929 * Error Risk: 99.2744%)
