...
## 1. EXECUTIVE SUMMARY
- **Scope:** 6 analyzed artifact(s), 1865 LOC.
- **Load-bearing artifact:** `include/fds.h` -- 3 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/fds_asm.c` -- pulls in 5 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/fds_cpu.c` at magnitude 1073.3 (structural weight, not risk).
...
> **File Composition:** Compute Cores Files 33%, Data / Markup / Trivial 17%, Declarative / Non-Code 17%, Large Core Modules (3) 17%, State Mutators Files 17%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 93.7 | 68.0 | 83.2 | 73.7 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 70.0 | 100.0 | 100.0 |
...
- `parse_number` **(Stateful Encapsulated Methods)** (@ `src/fds_asm.c`) -> Impact: **32.0** | LOC: 40
  * *Intent:* /* Parse number (hex or decimal) */
- `run_test_program` **(Many-Argument Workhorses)** (@ `src/main.c`) -> Impact: **29.4** | LOC: 68
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 116.959; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (97.7%), Complexity Load (formerly Cognitive Load) (94.7%), Guard Balance (formerly Safety Score) (93.7%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 116.959; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (97.7%), Guard Balance (formerly Safety Score) (89.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `assemble_line` **(Many-Argument Workhorses)** (Impact: 93.5)
    * *Intent:* /* ============================================================================ * ASSEMBLER CORE * =...
  * `parse_number` **(Stateful Encapsulated Methods)** (Impact: 32.0)
    * *Intent:* /* Parse number (hex or decimal) */
  * `fds_assemble` **(Compute Cores)** (Impact: 23.5)
    * *Intent:* /* ============================================================================ * PUBLIC API * =====...
  * `fds_asm_error_str` **(Compute Cores)** (Impact: 16.3)
  * `add_label` **(Stateful Encapsulated Methods)** (Impact: 12.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 116.959; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (97.7%), Guard Balance (formerly Safety Score) (83.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(Compute Cores)** (Impact: 68.5)
  * `run_interactive` **(Compute Cores)** (Impact: 36.8)
  * `run_test_program` **(Many-Argument Workhorses)** (Impact: 29.4)
  * `write_binary` **(Stateful Encapsulated Methods)** (Impact: 6.1)
    * *Intent:* /* Write assembled program to binary file */
  * `print_usage` **(Stateful Encapsulated Methods)** (Impact: 5.3)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **2**; blast radius 415.205; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Connectivity (formerly Api Exposure) (79.7%), Test Surface (formerly Verification) (2.3%)
...
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (100.0%), Debt Markers (formerly Tech Debt) (95.3%), Mutation Surface (formerly State Flux) (91.7%), Guard Balance (formerly Safety Score) (73.7%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 116.959; role: Isolated/Orphan
