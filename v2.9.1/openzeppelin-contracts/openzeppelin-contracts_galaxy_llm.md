...
## 1. EXECUTIVE SUMMARY
- **Scope:** 670 analyzed artifact(s), 58055 LOC.
- **Load-bearing artifact:** `contracts/utils/math/Math.sol` -- 35 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `contracts/mocks/Stateless.sol` -- pulls in 55 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `test/access/manager/AccessManager.test.js` at magnitude 1524.44 (structural weight, not risk).
...
| Avg Path Length | 2.0869 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.328`
> **Composition Archetype:** `Hub-Coupled App` (z +1.33; from the repo's file-archetype mix)
> **File Composition:** Encapsulated Accessors Files 19%, Parameter Forwarders Files 18%, Data / Markup / Trivial 14%, Interface Declarations Files 13%, Callbacks & Closures Files 9%
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 5.2 | 1.0 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 54.7 | 7.4 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 55.5 | 100.0 | 100.0 |
...
| connectivity | 2748 | 420 | 10 | `contracts/utils/Packing.sol` |
...
- `shouldBehaveLikeAccessControlDefaultAdminRules` **(Callbacks & Closures)** (@ `test/access/AccessControl.behavior.js`) -> Impact: **101.2** | LOC: 604
- `shouldBehaveLikeAccountERC7579` **(Callbacks & Closures)** (@ `test/account/extensions/AccountERC7579.behavior.js`) -> Impact: **88.3** | LOC: 692
- `shouldBehaveLikeERC721` **(Callbacks & Closures)** (@ `test/token/ERC721/ERC721.behavior.js`) -> Impact: **80.1** | LOC: 741
- `encodePayload` **(Many-Argument Workhorses)** (@ `test/crosschain/BridgeERC1155.behavior.js`) -> Impact: **67.5** | LOC: 322
  * *Intent:* // helper
- `tryTraverse` **(Many-Argument Workhorses)** (@ `contracts/utils/cryptography/TrieProof.sol`) -> Impact: **59.5** | LOC: 110
...
- `template` **(Compute Cores)** (@ `scripts/generate/templates/Checkpoints.js`) -> Impact: **54.7** | LOC: 217
- `fixture` **(I/O & Config Routines)** (@ `test/governance/extensions/GovernorTimelockAccess.test.js`) -> Impact: **52.9** | LOC: 738
- `shouldTransferTokensByUsers` **(Callbacks & Closures)** (@ `test/token/ERC721/ERC721.behavior.js`) -> Impact: **50.9** | LOC: 117
- `shouldBehaveLikeERC1155` **(Callbacks & Closures)** (@ `test/token/ERC1155/ERC1155.behavior.js`) -> Impact: **49.7** | LOC: 854
- `shouldBehaveLikeBridgeERC1155` **(Compute Cores)** (@ `test/crosschain/BridgeERC1155.behavior.js`) -> Impact: **48.6** | LOC: 322
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (50.0%), Guard Balance (formerly Safety Score) (22.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fixture` **(I/O & Config Routines)** (Impact: 9.3)
  * `self` **(Callbacks & Closures)** (Impact: 4.1)
  * `before` **(Callbacks & Closures)** (Impact: 2.5)
  * `self` **(Callbacks & Closures)** (Impact: 2.2)
  * `self` **(Callbacks & Closures)** (Impact: 2.2)
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); blast radius 2.57; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (83.5%), Complexity Load (formerly Cognitive Load) (58.1%), Connectivity (formerly Api Exposure) (33.9%)
- **Documentation Coverage:** 99.4624% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `extract_2_1` **(Encapsulated Accessors)** (Impact: 3.8)
  * `extract_4_1` **(Encapsulated Accessors)** (Impact: 3.8)
  * `extract_4_2` **(Encapsulated Accessors)** (Impact: 3.8)
  * `extract_6_1` **(Encapsulated Accessors)** (Impact: 3.8)
  * `extract_6_2` **(Encapsulated Accessors)** (Impact: 3.8)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **6**; blast radius 2.618; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (49.5%), Guard Balance (formerly Safety Score) (14.8%), Connectivity (formerly Api Exposure) (12.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeERC721` **(Callbacks & Closures)** (Impact: 80.1)
  * `shouldTransferTokensByUsers` **(Callbacks & Closures)** (Impact: 50.9)
  * `tx` **(Callbacks & Closures)** (Impact: 22.1)
  * `shouldTransferSafely` **(Callbacks & Closures)** (Impact: 16.2)
  * `shouldBehaveLikeERC721Enumerable` **(Callbacks & Closures)** (Impact: 13.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.7e-05 | `Ripple Effect (Closeness):` 0.004471
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (49.9%), Guard Balance (formerly Safety Score) (41.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fixture` **(I/O & Config Routines)** (Impact: 52.9)
  * `setAccessManagerIgnored` **(Callbacks & Closures)** (Impact: 8.1)
  * `prepareOperation` **(Parameter Forwarders)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (50.0%), Guard Balance (formerly Safety Score) (28.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fixture` **(Callbacks & Closures)** (Impact: 40.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Connectivity (formerly Api Exposure) (14.2%), Complexity Load (formerly Cognitive Load) (11.5%), Guard Balance (formerly Safety Score) (6.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testSymbolicReplace` **(Type Conversions)** (Impact: 2.4)
  * `testSymbolicReplace` **(Type Conversions)** (Impact: 2.4)
  * `testSymbolicReplace` **(Type Conversions)** (Impact: 2.4)
  * `testSymbolicReplace` **(Type Conversions)** (Impact: 2.4)
  * `testSymbolicReplace` **(Type Conversions)** (Impact: 2.4)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **4**; blast radius 2.305; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (48.8%), Guard Balance (formerly Safety Score) (15.9%), Connectivity (formerly Api Exposure) (13.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeAccessControlDefaultAdminRules` **(Callbacks & Closures)** (Impact: 101.2)
  * `shouldBehaveLikeAccessControl` **(Callbacks & Closures)** (Impact: 23.7)
  * `shouldBehaveLikeAccessControlEnumerable` **(Callbacks & Closures)** (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.2e-05 | `Ripple Effect (Closeness):` 0.004471
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (47.3%), Guard Balance (formerly Safety Score) (26.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `signature` **(Callbacks & Closures)** (Impact: 31.5)
  * `fixture` **(Callbacks & Closures)** (Impact: 26.1)
  * `shouldPropose` **(Callbacks & Closures)** (Impact: 3.9)
  * `deployToken` **(Defensive Guards)** (Impact: 3.4)
  * `tamper` **(Parameter Forwarders)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (46.3%), Guard Balance (formerly Safety Score) (12.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseShare` **(Callbacks & Closures)** (Impact: 34.5)
  * `parseToken` **(Parameter Forwarders)** (Impact: 1.5)
  * `fixture` **(Interface Declarations)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **5**; blast radius 0.946; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (44.7%), Guard Balance (formerly Safety Score) (30.0%), Connectivity (formerly Api Exposure) (11.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeERC1155` **(Callbacks & Closures)** (Impact: 49.7)
  * `batchTransferWasSuccessful` **(Callbacks & Closures)** (Impact: 4.5)
  * `transferWasSuccessful` **(Callbacks & Closures)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.00149
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **6**; blast radius 1.53; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (43.8%), Complexity Load (formerly Cognitive Load) (38.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.0%)
- **Documentation Coverage:** 61.9048% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testAsSchedulableOperation` **(Callbacks & Closures)** (Impact: 10.0)
    * *Intent:* // ============ OPERATION ============ /** */
  * `testAsDelayedOperation` **(Callbacks & Closures)** (Impact: 8.7)
    * *Intent:* /** */
  * `testAsRestrictedOperation` **(Callbacks & Closures)** (Impact: 6.0)
    * *Intent:* /** */
  * `testAsGetAccess` **(Callbacks & Closures)** (Impact: 4.9)
    * *Intent:* /** */
  * `testAsDelay` **(Callbacks & Closures)** (Impact: 2.8)
    * *Intent:* // ============ DELAY ============ /** */
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.3e-05 | `Ripple Effect (Closeness):` 0.002981
...
- **Magnitude:** 380.34 | **LOC:** 834 | **CtrlFlow:** 16.1% | **Authorship Centralization:** 100.0%
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **1**; blast radius 3.228; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (73.0%), Complexity Load (formerly Cognitive Load) (20.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (6.4%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_insert` **(Stateful Encapsulated Methods)** (Impact: 13.4)
  ...
  * `_insert` **(Stateful Encapsulated Methods)** (Impact: 13.4)
    * *Intent:* /** * @dev Pushes a (`key`, `value`) pair into an ordered list of checkpoints, either by inserting a...
  * `_upperBinaryLookup` **(Stateful Encapsulated Methods)** (Impact: 9.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5e-05 | `Ripple Effect (Closeness):` 0.027022
...
- **Blast Radius:** changing it is visible to **25** in-repo importer(s); blast radius 22.909; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Connectivity (formerly Api Exposure) (48.3%), Guard Balance (formerly Safety Score) (30.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (22.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `toInt256` **(Encapsulated Accessors)** (Impact: 3.2)
    * *Intent:* /** * @dev Converts an unsigned uint256 into a signed int256. * * Requirements: * * - input must be ...
  * `toUint248` **(Encapsulated Accessors)** (Impact: 3.1)
    * *Intent:* /** * @dev Returns the downcasted uint248 from uint256, reverting on * overflow (when the input is g...
  * `toUint240` **(Encapsulated Accessors)** (Impact: 3.1)
    * *Intent:* /** * @dev Returns the downcasted uint240 from uint256, reverting on * overflow (when the input is g...
  * `toUint232` **(Encapsulated Accessors)** (Impact: 3.1)
    * *Intent:* /** * @dev Returns the downcasted uint232 from uint256, reverting on * overflow (when the input is g...
  * `toUint224` **(Encapsulated Accessors)** (Impact: 3.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (41.1%), Guard Balance (formerly Safety Score) (36.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (5.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getAddress` **(Defensive Guards)** (Impact: 7.1)
  * `genOperation` **(Parameter Forwarders)** (Impact: 2.9)
  * `genOperationBatch` **(Parameter Forwarders)** (Impact: 2.9)
  * `fixture` **(Interface Declarations)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 15 instances
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **7**; blast radius 1.025; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (48.3%), Complexity Load (formerly Cognitive Load) (43.1%), Guard Balance (formerly Safety Score) (33.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeAccountERC7579` **(Callbacks & Closures)** (Impact: 88.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.8e-05 | `Ripple Effect (Closeness):` 0.004471
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.0%), Complexity Load (formerly Cognitive Load) (67.1%), Connectivity (formerly Api Exposure) (3.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testLookup` **(Many-Argument Workhorses)** (Impact: 13.8)
  * `testLookup` **(Many-Argument Workhorses)** (Impact: 13.8)
  * `testLookup` **(Many-Argument Workhorses)** (Impact: 13.8)
  * `testLookup` **(Many-Argument Workhorses)** (Impact: 13.8)
  * `testPush` **(Many-Argument Workhorses)** (Impact: 11.7)
    * *Intent:* // tests
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (66.9%), Complexity Load (formerly Cognitive Load) (50.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (24.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `forgeRequests` **(Callbacks & Closures)** (Impact: 35.5)
  * `estimateRequest` **(Callbacks & Closures)** (Impact: 16.6)
  * `fixture` **(Interface Declarations)** (Impact: 3.2)
  * `forgeRequest` **(Callbacks & Closures)** (Impact: 2.5)
  * `requestsValue` **(Callbacks & Closures)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (96.6%), Mutation Surface (formerly State Flux) (14.1%), Guard Balance (formerly Safety Score) (11.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testCommutative` **(Parameter Forwarders)** (Impact: 2.8)
  * `bytes` **(Parameter Forwarders)** (Impact: 1.8)
  * `uint256` **(Parameter Forwarders)** (Impact: 1.7)
  * `fixture` **(Interface Declarations)** (Impact: 1.6)
  * `splitHighLow` **(Parameter Forwarders)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 1.051; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (50.0%), Guard Balance (formerly Safety Score) (21.4%), Connectivity (formerly Api Exposure) (21.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createProxyWithImpersonatedProxyAdmin` **(Many-Argument Workhorses)** (Impact: 21.8)
  * `shouldBehaveLikeTransparentUpgradeableProxy` **(Callbacks & Closures)** (Impact: 20.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3e-06 | `Ripple Effect (Closeness):` 0.00149
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.737; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (24.9%), Mutation Surface (formerly State Flux) (23.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `shouldOnlyRevertOnErrors` **(Callbacks & Closures)** (Impact: 5.7)
  * `fixture` **(Interface Declarations)** (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 22 instances
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **6**; blast radius 1.521; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (53.8%), Connectivity (formerly Api Exposure) (25.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `shouldBehaveLikeVotes` **(Many-Argument Workhorses)** (Impact: 36.4)
  * `getWeight` **(Callbacks & Closures)** (Impact: 32.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.2e-05 | `Ripple Effect (Closeness):` 0.005961
...
- **Blast Radius:** changing it is visible to **14** in-repo importer(s); it depends on **2**; blast radius 2.516; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (70.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Defensive Guards)** (Impact: 41.9)
  * `newAccount` **(Defensive Guards)** (Impact: 19.8)
  * `packValidationData` **(Type Conversions)** (Impact: 16.7)
  * `createUserOp` **(Defensive Guards)** (Impact: 14.6)
  * `getAddress` **(Defensive Guards)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.9e-05 | `Ripple Effect (Closeness):` 0.020864
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 0.737; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (48.9%), Guard Balance (formerly Safety Score) (29.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (14.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fixture` **(Callbacks & Closures)** (Impact: 29.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **4**; blast radius 6.581; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.9%), Complexity Load (formerly Cognitive Load) (49.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.1%)
- **Documentation Coverage:** 2.381% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `escapeJSON` **(Stateful Encapsulated Methods)** (Impact: 27.7)
    * *Intent:* /** * @dev Escape special characters in JSON strings. This can be useful to prevent JSON injection i...
  * `_tryParseIntUncheckedBounds` **(Stateful Encapsulated Methods)** (Impact: 15.1)
    * *Intent:* /** * @dev Implementation of {tryParseInt-string-uint256-uint256} that does not check bounds. Caller...
  * `_tryParseChr` **(Stateful Encapsulated Methods)** (Impact: 10.7)
  * `tryParseAddress` **(Type Conversions)** (Impact: 8.9)
    * *Intent:* /** * @dev Variant of {parseAddress-string-uint256-uint256} that returns false if the parsing fails ...
  * `_tryParseHexUintUncheckedBounds` **(Stateful Encapsulated Methods)** (Impact: 7.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000508 | `Ripple Effect (Closeness):` 0.054322
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.737; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (33.6%), Guard Balance (formerly Safety Score) (8.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fixture` **(Interface Declarations)** (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
- `contracts/governance/Governor.sol` -> **Severity: 0.058** (Bridge: 0.002 * Flux: 29.6352%)
- `contracts/utils/Strings.sol` -> **Severity: 0.051** (Bridge: 0.0005 * Flux: 100.0%)
- `contracts/utils/math/Math.sol` -> **Severity: 0.042** (Bridge: 0.0004 * Flux: 100.0%)
- `contracts/utils/cryptography/SignatureChecker.sol` -> **Severity: 0.029** (Bridge: 0.0003 * Flux: 86.909%)
- `contracts/token/ERC721/ERC721.sol` -> **Severity: 0.025** (Bridge: 0.0003 * Flux: 78.1336%)
