...
## 1. EXECUTIVE SUMMARY
- **Scope:** 687 analyzed artifact(s), 78851 LOC.
- **Load-bearing artifact:** `js/index.js` -- 96 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `js/browser.js` -- pulls in 51 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `js/browser.js` at magnitude 1838.44 (structural weight, not risk).
...
|---|---|---|
| Modularity | 0.5504 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 4.5177 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `3.541`
> **Composition Archetype:** `Hub-Coupled App` (z +3.54; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 44%, Declarative / Non-Code 14%, Large Core Modules (3) 13%, Large Core Modules (2) 12%, State Mutators Files 4%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 43.0 | 0.0 | 0.0 |
...
| credential | 209 | 76 | 1 | `dev/misc/functional-url.html` |
...
- `renderJunction` **(Many-Argument Workhorses)** (@ `js/feature/spliceJunctionTrack.js`) -> Impact: **273.0** | LOC: 221
  * *Intent:* /** * */
- `draw` **(Compute Cores)** (@ `js/bam/alignmentTrack.js`) -> Impact: **176.1** | LOC: 467
- `drawSingleAlignment` **(Many-Argument Workhorses)** (@ `js/bam/alignmentTrack.js`) -> Impact: **171.2** | LOC: 303
- `call_2d` **(Many-Argument Workhorses)** (@ `js/cnvpytor/CombinedCaller.js`) -> Impact: **166.7** | LOC: 336
- `search` **(Many-Argument Workhorses)** (@ `js/genome/hgvs.js`) -> Impact: **154.2** | LOC: 244
...
- `renderAminoAcidSequence` **(Many-Argument Workhorses)** (@ `js/feature/render/renderFeature.js`) -> Impact: **127.0** | LOC: 152
- `createHGVSAnnotation` **(Many-Argument Workhorses)** (@ `js/genome/hgvs.js`) -> Impact: **117.3** | LOC: 141
...
- `decodeBed` **(Many-Argument Workhorses)** (@ `js/feature/decode/ucsc.js`) -> Impact: **107.8** | LOC: 115
...
- `readAlignments` **(Many-Argument Workhorses)** (@ `js/cram/cramReader.js`) -> Impact: **104.5** | LOC: 210
- `renderFeature` **(Many-Argument Workhorses)** (@ `js/feature/render/renderFeature.js`) -> Impact: **99.8** | LOC: 144
  * *Intent:* /** * */
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **51**; blast radius 7.818; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (96.3%), Guard Balance (formerly Safety Score) (79.0%)
- **Documentation Coverage:** 61.8497% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `loadSessionObject` **(Defensive Guards)** (Impact: 79.0)
    * *Intent:* /** * Note: public API function */
  * `createTrack` **(Compute Cores)** (Impact: 50.6)
    * *Intent:* /** * Create a Track object. */
  * `loadGenome` **(Defensive Guards)** (Impact: 38.1)
    * *Intent:* /** * Load a genome, defined by a string ID or a json-like configuration object. This includes a fas...
  * `keyUpHandler` **(Defensive Guards)** (Impact: 33.6)
    * *Intent:* /** * Handle keyup event, used for navigating feature tracks with hot keys. This will get bound to t...
  * `_validateAndWarnResources` **(Compute Cores)** (Impact: 30.5)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 7.818
  * `Choke Point (Betweenness):` 0.048607 | `Ripple Effect (Closeness):` 0.11227
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **14**; blast radius 0.932; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (89.8%), Guard Balance (formerly Safety Score) (86.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 89.6907% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Compute Cores)** (Impact: 176.1)
  * `drawSingleAlignment` **(Many-Argument Workhorses)** (Impact: 171.2)
  * `drawBlock` **(Many-Argument Workhorses)** (Impact: 79.9)
  * `contextMenuItemList` **(Defensive Guards)** (Impact: 77.1)
  * `getAlignmentColor` **(Defensive Guards)** (Impact: 56.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.932
  * `Choke Point (Betweenness):` 0.002146 | `Ripple Effect (Closeness):` 0.063112
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **11**; blast radius 1.21; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (77.8%)
- **Documentation Coverage:** 83.871% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `drawProportional` **(Defensive Guards)** (Impact: 70.4)
  * `drawNested` **(Compute Cores)** (Impact: 55.6)
  * `getWGFeatures` **(Compute Cores)** (Impact: 42.3)
    * *Intent:* /** * Called in the context of FeatureSource (i.e. this == the feature source (a TextFeatureSource) ...
  * `makeWGFeature` **(Compute Cores)** (Impact: 42.2)
  * `init` **(Defensive Guards)** (Impact: 37.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.21
  * `Choke Point (Betweenness):` 0.000721 | `Ripple Effect (Closeness):` 0.080802
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 1.112; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (72.6%)
- **Documentation Coverage:** 95.2381% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `meanShiftCaller` **(Compute Cores)** (Impact: 68.1)
  * `call_mean_shift` **(Compute Cores)** (Impact: 60.6)
  * `cnv_calling` **(Compute Cores)** (Impact: 57.3)
  * `cnvCalling` **(Compute Cores)** (Impact: 57.1)
  * `partition` **(Compute Cores)** (Impact: 55.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000669 | `Ripple Effect (Closeness):` 0.051874
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); blast radius 1.141; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.7%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (59.4%)
- **Documentation Coverage:** 29.8077% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `arcTo` **(Many-Argument Workhorses)** (Impact: 38.6)
    * *Intent:* /** * Adds the arcTo to the current path * */
  * `__applyStyleToCurrentElement` **(Defensive Guards)** (Impact: 35.7)
    * *Intent:* /** * Apples the current styles to the current SVG element. On "ctx.fill" or "ctx.stroke" */
  * `__parseFont` **(Compute Cores)** (Impact: 31.6)
    * *Intent:* /** * Parses the font string and returns svg mapping */
  * `arc` **(Many-Argument Workhorses)** (Impact: 28.7)
    * *Intent:* /** * Arc command! */
  * `drawImage` **(I/O & Config Routines)** (Impact: 26.2)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 1.141
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.081872
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **4**; blast radius 2.46; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.6%), Complexity Load (formerly Cognitive Load) (69.7%), Connectivity (formerly Api Exposure) (38.0%)
- **Documentation Coverage:** 66.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `bam_tag2cigar` **(Defensive Guards)** (Impact: 90.2)
  * `decodeBamRecords` **(Many-Argument Workhorses)** (Impact: 84.5)
    * *Intent:* /** * * */
  * `decodeSamRecords` **(Many-Argument Workhorses)** (Impact: 61.9)
  * `decodeBamTags` **(Defensive Guards)** (Impact: 51.1)
    * *Intent:* /** * Decode bam tags from the supplied UInt8Array * * A [!-~] Printable character * i [-+]?[0-9]+ S...
  * `makeBlocks` **(Many-Argument Workhorses)** (Impact: 33.7)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 2.46
  * `Choke Point (Betweenness):` 0.000973 | `Ripple Effect (Closeness):` 0.04624
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **12**; blast radius 1.298; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (99.9%), Guard Balance (formerly Safety Score) (82.9%), Complexity Load (formerly Cognitive Load) (82.8%)
- **Documentation Coverage:** 77.2727% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Defensive Guards)** (Impact: 74.7)
  * `postInit` **(Defensive Guards)** (Impact: 27.6)
  * `sortSamplesByGenotype` **(Defensive Guards)** (Impact: 24.2)
  * `getFeatures` **(Defensive Guards)** (Impact: 21.4)
  * `getColorForFeature` **(Defensive Guards)** (Impact: 21.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.298
  * `Choke Point (Betweenness):` 0.000558 | `Ripple Effect (Closeness):` 0.081039
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **7**; blast radius 1.39; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.4%), Complexity Load (formerly Cognitive Load) (96.5%)
- **Documentation Coverage:** 95.4545% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Compute Cores)** (Impact: 48.9)
  * `paintAxis` **(Many-Argument Workhorses)** (Impact: 42.3)
  * `postInit` **(I/O & Config Routines)** (Impact: 26.1)
  * `getFeatures` **(Many-Argument Workhorses)** (Impact: 24.6)
  * `recreate_tracks` **(Compute Cores)** (Impact: 24.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.39
  * `Choke Point (Betweenness):` 0.003072 | `Ripple Effect (Closeness):` 0.080567
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **9**; blast radius 3.442; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.5%), Complexity Load (formerly Cognitive Load) (85.1%)
- **Documentation Coverage:** 65.7895% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `readFeatures` **(Many-Argument Workhorses)** (Impact: 51.5)
  * `decodeWigData` **(Many-Argument Workhorses)** (Impact: 50.4)
  * `decodeZoomData` **(Many-Argument Workhorses)** (Impact: 47.0)
  * `loadHeader` **(I/O & Config Routines)** (Impact: 23.5)
    * *Intent:* /** * The BB header consists of * (1) the common header * (2) the zoom headers * (3) autosql * (4) t...
  * `search` **(Compute Cores)** (Impact: 18.6)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 3.442
  * `Choke Point (Betweenness):` 0.004115 | `Ripple Effect (Closeness):` 0.059902
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **4**; blast radius 2.46; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.5%), Complexity Load (formerly Cognitive Load) (72.0%), Connectivity (formerly Api Exposure) (49.5%)
- **Documentation Coverage:** 94.3396% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `packAlignmentRows` **(Many-Argument Workhorses)** (Impact: 58.9)
  * `incCounts` **(Compute Cores)** (Impact: 31.6)
  * `packFull` **(Many-Argument Workhorses)** (Impact: 28.5)
  * `getPosCount` **(Compute Cores)** (Impact: 27.3)
  * `getNegCount` **(Compute Cores)** (Impact: 27.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 2.46
  * `Choke Point (Betweenness):` 0.000932 | `Ripple Effect (Closeness):` 0.046279
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **4**; blast radius 1.788; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (74.6%)
- **Documentation Coverage:** 73.6842% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderAminoAcidSequence` **(Many-Argument Workhorses)** (Impact: 127.0)
  * `renderFeature` **(Many-Argument Workhorses)** (Impact: 99.8)
    * *Intent:* /** * */
  * `doPaint` **(Many-Argument Workhorses)** (Impact: 99.2)
  * `getAminoAcidLetterWithExonGap` **(Many-Argument Workhorses)** (Impact: 55.4)
  * `renderFeatureLabel` **(Many-Argument Workhorses)** (Impact: 47.9)
    * *Intent:* /** */
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.788
  * `Choke Point (Betweenness):` 0.000717 | `Ripple Effect (Closeness):` 0.06466
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **9**; blast radius 1.664; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Concurrency Surface (formerly Concurrency) (74.9%), Guard Balance (formerly Safety Score) (69.8%)
- **Documentation Coverage:** 84.6154% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `addViewportClickHandler` **(Compute Cores)** (Impact: 38.5)
  * `checkZoomIn` **(Defensive Guards)** (Impact: 21.5)
    * *Intent:* /** * Test to determine if we are zoomed in far enough to see features. Applicable to tracks with vi...
  * `zoomedOutOfWindow` **(I/O & Config Routines)** (Impact: 21.4)
  * `containsRange` **(Defensive Guards)** (Impact: 20.0)
  * `mu` **(Defensive Guards)** (Impact: 19.1)
    * *Intent:* // Mouse up
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.664
  * `Choke Point (Betweenness):` 0.001113 | `Ripple Effect (Closeness):` 0.081636
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.829; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (75.7%), Complexity Load (formerly Cognitive Load) (37.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `updateFilterCounts` **(Many-Argument Workhorses)** (Impact: 33.7)
    * *Intent:* /** Get counts for each filter, in each facet */
  * `buildFilter` **(Compute Cores)** (Impact: 28.7)
    * *Intent:* /** * Build a filter function taking an igvFeature as an argument and returning true for "pass" (sho...
  * `initFacets` **(Defensive Guards)** (Impact: 25.3)
    * *Intent:* * // For integer or float: numeric values observed for this facet * "filterNumbers": [<Number>], * *...
  * `handleBrushMove` **(Many-Argument Workhorses)** (Impact: 17.9)
    * *Intent:* /** Handle slider move event (i.e., drag or resize) */
  * `getInputStyle` **(Many-Argument Workhorses)** (Impact: 15.5)
...
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 0.829
...
- **Blast Radius:** changing it is visible to **21** in-repo importer(s); it depends on **5**; blast radius 10.442; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.1%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (73.7%)
- **Documentation Coverage:** 32.7273% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `setTrackProperties` **(Defensive Guards)** (Impact: 74.7)
    * *Intent:* /** * Set certain track properties, usually from a "track" line. Not all UCSC properties are support...
  * `extractPopupData` **(Many-Argument Workhorses)** (Impact: 42.1)
    * *Intent:* /** * Default popup text function -- just extracts string and number properties in random order. */
  * `init` **(Defensive Guards)** (Impact: 39.7)
    * *Intent:* /** * Initialize track properties from the config object. This method is typically overriden in subc...
  * `isJSONable` **(Defensive Guards)** (Impact: 29.3)
  * `createKeyValueRow` **(Compute Cores)** (Impact: 25.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 10.442
  * `Choke Point (Betweenness):` 0.00252 | `Ripple Effect (Closeness):` 0.119545
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **10**; blast radius 1.11; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.9%), Concurrency Surface (formerly Concurrency) (87.3%), Complexity Load (formerly Cognitive Load) (80.3%)
- **Documentation Coverage:** 86.9565% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Compute Cores)** (Impact: 55.1)
  * `computeRegionScores` **(Compute Cores)** (Impact: 24.6)
  * `filter` **(Defensive Guards)** (Impact: 23.9)
    * *Intent:* /** * Filter function for sample keys. Applies multiple filters in pipeline fashion. * Each filter m...
  * `getFeatures` **(Defensive Guards)** (Impact: 22.9)
  * `init` **(Compute Cores)** (Impact: 20.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.11
  * `Choke Point (Betweenness):` 0.000765 | `Ripple Effect (Closeness):` 0.08045
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **11**; blast radius 0.965; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (65.2%), Complexity Load (formerly Cognitive Load) (60.4%), Connectivity (formerly Api Exposure) (44.4%)
- **Documentation Coverage:** 80.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `updateViews` **(Defensive Guards)** (Impact: 41.5)
    * *Intent:* /** * Update viewports to reflect current genomic state, possibly loading additional data. * */
  * `addTrackDragMouseHandlers` **(Defensive Guards)** (Impact: 30.0)
  * `presentColorPicker` **(Defensive Guards)** (Impact: 26.8)
  * `setTrackHeight` **(Defensive Guards)** (Impact: 16.0)
  * `createTrackGearPopup` **(Defensive Guards)** (Impact: 13.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.965
  * `Choke Point (Betweenness):` 0.001323 | `Ripple Effect (Closeness):` 0.080684
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **11**; blast radius 1.619; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (99.6%), Guard Balance (formerly Safety Score) (86.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Compute Cores)** (Impact: 46.5)
    * *Intent:* * pixelXOffset, * pixelWidth, * pixelHeight, * pixelTop, * bpStart, * bpEnd: bpEnd, * bpPerPixel, * ...
  * `popupData` **(Defensive Guards)** (Impact: 46.5)
    * *Intent:* /** * Return "popup data" for feature @ genomic location. Data is an array of key-value pairs */
  * `getColorForFeature` **(Defensive Guards)** (Impact: 40.5)
    * *Intent:* /** * Return color for feature. */
  * `init` **(Compute Cores)** (Impact: 39.4)
  * `contextMenuItemList` **(Defensive Guards)** (Impact: 22.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.619
  * `Choke Point (Betweenness):` 0.00253 | `Ripple Effect (Closeness):` 0.082365
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **2**; blast radius 1.922; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.6%), Complexity Load (formerly Cognitive Load) (78.3%), Concurrency Surface (formerly Concurrency) (70.8%)
- **Documentation Coverage:** 44.4444% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `search` **(Many-Argument Workhorses)** (Impact: 154.2)
    * *Intent:* /** * Searches for the given HGVS notation in the provided genome. * Returns a SearchResult with the...
  * `createHGVSAnnotation` **(Many-Argument Workhorses)** (Impact: 117.3)
    * *Intent:* /** * Returns HGVS annotation for the position, for ref and alt bases. If a MANE transcript is avail...
  * `transcriptPositionToGenomicPosition` **(Defensive Guards)** (Impact: 24.8)
    * *Intent:* /** * Convert a transcript position (1-based, from transcription start) to genomic position * for no...
  * `getHGVSPosition` **(Defensive Guards)** (Impact: 23.3)
    * *Intent:* /** * Returns genomic HGVS notation: <RefSeqAccession>:g.<position> * Example: NC_000001.11:g.123456...
  * `codingToGenomePosition` **(Compute Cores)** (Impact: 15.6)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.922
  * `Choke Point (Betweenness):` 0.000281 | `Ripple Effect (Closeness):` 0.066526
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **11**; blast radius 1.798; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (94.3%), Complexity Load (formerly Cognitive Load) (92.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 91.8919% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Defensive Guards)** (Impact: 68.1)
  * `yScale` **(Defensive Guards)** (Impact: 67.5)
  * `getFeatures` **(Many-Argument Workhorses)** (Impact: 38.0)
  * `drawSVGPath` **(Many-Argument Workhorses)** (Impact: 25.7)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.798
  * `Choke Point (Betweenness):` 0.003511 | `Ripple Effect (Closeness):` 0.081516
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **13**; blast radius 1.101; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.4%), Complexity Load (formerly Cognitive Load) (81.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `setDecoder` **(Compute Cores)** (Impact: 79.8)
  * `parseHeader` **(Defensive Guards)** (Impact: 48.7)
    * *Intent:* /** * Parse metadata from the file. A variety of conventions are in use to supply metadata about fil...
  * `parseFeatures` **(Defensive Guards)** (Impact: 36.7)
  * `parseTrackLine` **(Defensive Guards)** (Impact: 34.9)
  * `constructor` **(Compute Cores)** (Impact: 11.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.101
  * `Choke Point (Betweenness):` 0.003788 | `Ripple Effect (Closeness):` 0.049025
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 1.11; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (93.5%), Complexity Load (formerly Cognitive Load) (86.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 52.1739% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderJunction` **(Many-Argument Workhorses)** (Impact: 273.0)
    * *Intent:* /** * */
  * `drawArrowhead` **(Many-Argument Workhorses)** (Impact: 14.6)
  * `popupData` **(Defensive Guards)** (Impact: 13.3)
    * *Intent:* /** * Return "popup data" for feature @ genomic location. Data is an array of key-value pairs */
  * `draw` **(Compute Cores)** (Impact: 11.9)
  * `postInit` **(Defensive Guards)** (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
* *Amplified Cascading Flux:* 49 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.080567
...
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (57.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (37.0%), Complexity Load (formerly Cognitive Load) (7.4%), Connectivity (formerly Api Exposure) (4.0%)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 1.101; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (77.7%)
- **Documentation Coverage:** 90.9091% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `AedFeature` **(Defensive Guards)** (Impact: 44.6)
    * *Intent:* /** * AED file feature. * * * Other values are parsed one by one */
  * `parseFeatures` **(Compute Cores)** (Impact: 41.3)
  * `decodeAed` **(Defensive Guards)** (Impact: 33.8)
    * *Intent:* /** * Decode the AED file format */
  * `parseHeader` **(Compute Cores)** (Impact: 25.6)
  * `parseTrackLine` **(Defensive Guards)** (Impact: 21.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.048982
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **12**; blast radius 3.522; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.4%), Complexity Load (formerly Cognitive Load) (89.8%)
- **Documentation Coverage:** 66.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_parse` **(Many-Argument Workhorses)** (Impact: 33.7)
  * `readFeatures` **(Defensive Guards)** (Impact: 23.6)
    * *Intent:* /** * Return a promise to load features for the genomic interval */
  * `loadFeaturesWithIndex` **(Many-Argument Workhorses)** (Impact: 22.1)
  * `readHeader` **(I/O & Config Routines)** (Impact: 20.5)
  * `constructor` **(Defensive Guards)** (Impact: 18.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 3.522
  * `Choke Point (Betweenness):` 0.010815 | `Ripple Effect (Closeness):` 0.058882
...
- `js/browser.js` -> **Severity: 4.861** (Bridge: 0.0486 * Flux: 99.9992%)
- `js/trackFactory.js` -> **Severity: 3.42** (Bridge: 0.0347 * Flux: 98.6166%)
- `js/index.js` -> **Severity: 1.453** (Bridge: 0.0898 * Flux: 16.1747%)
- `js/bam/bamTrack.js` -> **Severity: 1.142** (Bridge: 0.0114 * Flux: 99.9962%)
- `js/feature/featureFileReader.js` -> **Severity: 1.081** (Bridge: 0.0108 * Flux: 100.0%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `js/index.js` -> **Severity: 11.464** (Embedded: 0.1791 * Error Risk: 63.9995%)
- `js/ui/components/alertDialog.js` -> **Severity: 10.699** (Embedded: 0.1125 * Error Risk: 95.0734%)
- `js/igv-create.js` -> **Severity: 10.536** (Embedded: 0.11 * Error Risk: 95.7529%)
- `js/trackBase.js` -> **Severity: 10.531** (Embedded: 0.1195 * Error Risk: 88.0962%)
- `js/ucsc/hub/hub.js` -> **Severity: 10.443** (Embedded: 0.1134 * Error Risk: 92.072%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `js/index.js` -> **Severity: 7749.9** (Blast Radius: 77.499 * Doc Risk: 100.0%)
- `js/ui/utils/dom-utils.js` -> **Severity: 2426.954** (Blast Radius: 26.292 * Doc Risk: 92.3077%)
- `js/util/colorPalletes.js` -> **Severity: 1601.5** (Blast Radius: 16.015 * Doc Risk: 100.0%)
- `js/igv-canvas.js` -> **Severity: 1346.8** (Blast Radius: 13.468 * Doc Risk: 100.0%)
- `js/util/sequenceUtils.js` -> **Severity: 881.0** (Blast Radius: 8.81 * Doc Risk: 100.0%)
