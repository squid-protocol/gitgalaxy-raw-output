...
## 1. EXECUTIVE SUMMARY
- **Scope:** 306 analyzed artifact(s), 28358 LOC.
- **Load-bearing artifact:** `Sources/CVaporBcrypt/bcrypt.h` -- 2 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `Tests/VaporTests/ServerTests.swift` -- pulls in 16 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `Tests/VaporTests/Utilities/expired.crt` at magnitude 5000.0 (structural weight, not risk).
...
> **Architectural Drift Z-Score:** `1.227`
> **Composition Archetype:** `Mid Flat Project` (z +1.23; from the repo's file-archetype mix)
> **File Composition:** Parameter Forwarders Files 17%, Generic / Templated Code Files 16%, Large Core Modules (2) 15%, Large Core Modules (3) 14%, Interface Declarations Files 10%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 56.1 | 70.0 | 100.0 |
...
| threat | 51 | 17 | 0 | `Sources/Vapor/HTTP/Headers/HTTPHeaderCacheControl.swift` |
...
- `getData` **(Stateful Encapsulated Methods)** (@ `Sources/Vapor/URLEncodedForm/URLEncodedFormEncoder.swift`) -> Impact: **62.5** | LOC: 269
...
- `write` **(Defensive Guards)** (@ `Sources/Vapor/HTTP/Server/HTTPServerResponseEncoder.swift`) -> Impact: **49.4** | LOC: 52
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (54.9%), Guard Balance (formerly Safety Score) (42.6%), Complexity Load (formerly Cognitive Load) (36.2%)
- **Documentation Coverage:** 98.1132% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testHTTP2RequestDecompression` **(I/O & Config Routines)** (Impact: 19.9)
  * `testDeprecatedServerStartMethods` **(Annotated & Test Methods)** (Impact: 18.4)
  * `testHTTP1RequestDecompression` **(I/O & Config Routines)** (Impact: 17.6)
  * `testRequestBodyBackpressureWorks` **(I/O & Config Routines)** (Impact: 14.3)
  * `testRequestBodyStreamGetsFinalisedEvenIfClientDisappears` **(Defensive Guards)** (Impact: 13.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (53.8%), Complexity Load (formerly Cognitive Load) (33.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.9%)
...
**Top Internal Functions/Classes:**
  ...
  * `testUnknownType` **(Callbacks & Closures)** (Impact: 3.5)
  * `testImage` **(Callbacks & Closures)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (69.7%), Concurrency Surface (formerly Concurrency) (49.4%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (98.1%), Debt Markers (formerly Tech Debt) (96.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 34.1463% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getData` **(Stateful Encapsulated Methods)** (Impact: 62.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (49.0%), Guard Balance (formerly Safety Score) (37.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testEchoHandlers` **(Compute Cores)** (Impact: 22.4)
  * `testAsyncEchoHandlers` **(I/O & Config Routines)** (Impact: 10.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (47.4%), Guard Balance (formerly Safety Score) (37.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_testRequestBodyBackpressureWorksWithAsyncStreaming` **(I/O & Config Routines)** (Impact: 15.7)
  ...
  * `testStreamingRequest` **(I/O & Config Routines)** (Impact: 4.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.228; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (50.0%), Guard Balance (formerly Safety Score) (48.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `withRemoteApp` **(Defensive Guards)** (Impact: 9.5)
    * *Intent:* // MARK: - Helpers
  * `testBoilerplateClient` **(Annotated & Test Methods)** (Impact: 7.5)
  * `log` **(Many-Argument Workhorses)** (Impact: 6.2)
  * `clientConfigurationChange` **(Annotated & Test Methods)** (Impact: 3.0)
  * `clientConfigurationCantBeChangedAfterClientHasBeenUsed` **(Annotated & Test Methods)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 25 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (42.8%)
...
**Top Internal Functions/Classes:**
  ...
  * `asByteBufferBounds` **(Stateful Encapsulated Methods)** (Impact: 16.9)
  * `generateETagHash` **(Stateful Encapsulated Methods)** (Impact: 9.4)
    * *Intent:* /// Generates a fresh ETag for a file or returns its currently cached one. /// - Parameters: /// - p...
  * `read` **(Stateful Encapsulated Methods)** (Impact: 8.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (50.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.9%), Guard Balance (formerly Safety Score) (24.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `testStreamFileNull` **(Callbacks & Closures)** (Impact: 4.0)
  * `testStreamFileContentHeadersOnlyFirstByte` **(Callbacks & Closures)** (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (93.1%), Concurrency Surface (formerly Concurrency) (87.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (31.6%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (67.6%), Guard Balance (formerly Safety Score) (48.0%), Complexity Load (formerly Cognitive Load) (20.1%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (0.5%)
- **Documentation Coverage:** 85.2941% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testContentDispositionQuotedFilename` **(I/O & Config Routines)** (Impact: 15.9)
    * *Intent:* // https://github.com/vapor/vapor/issues/2439
  * `testCookie_parsing` **(Compute Cores)** (Impact: 11.8)
  * `testAcceptType` **(I/O & Config Routines)** (Impact: 10.6)
  * `testCookie_invalidCookie` **(Compute Cores)** (Impact: 9.6)
  * `testLinkHeaderSerialization` **(Compute Cores)** (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (67.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (30.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `errorCaught` **(Type Conversions)** (Impact: 11.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (48.5%), Guard Balance (formerly Safety Score) (26.3%), Connectivity (formerly Api Exposure) (17.0%), Mutation Surface (formerly State Flux) (17.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `first` **(Parameter Forwarders)** (Impact: 3.0)
  ...
  * `hash` **(Parameter Forwarders)** (Impact: 1.6)
    * *Intent:* /// See `Hashable`
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.228; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.2%), Complexity Load (formerly Cognitive Load) (87.2%), Debt Markers (formerly Tech Debt) (16.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `vapor_bcrypt_hashpass` **(Many-Argument Workhorses)** (Impact: 52.5)
    * *Intent:* /* * the core bcrypt function */
  * `decode_base64` **(Stateful Encapsulated Methods)** (Impact: 17.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.2%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (67.6%)
...
**Top Internal Functions/Classes:**
  ...
  * `init` **(Parameter Forwarders)** (Impact: 3.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.9%), Concurrency Surface (formerly Concurrency) (18.1%), Dead Code Surface (formerly Dead Code) (5.7%), Complexity Load (formerly Cognitive Load) (5.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (72.4%), Complexity Load (formerly Cognitive Load) (48.0%), Guard Balance (formerly Safety Score) (45.6%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (94.0%), Guard Balance (formerly Safety Score) (52.7%), Complexity Load (formerly Cognitive Load) (37.8%)
- **Documentation Coverage:** 66.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `init` **(Stateful Encapsulated Methods)** (Impact: 10.4)
    * *Intent:* // async flag here is just to stop the compiler from complaining about duplicates
  * `asyncShutdown` **(Defensive Guards)** (Impact: 6.5)
  ...
  * `startup` **(Defensive Guards)** (Impact: 4.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (77.6%), Complexity Load (formerly Cognitive Load) (31.9%)
- **Documentation Coverage:** 84.2105% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `nextDirective` **(Stateful Encapsulated Methods)** (Impact: 12.7)
  * `nextDirectiveValue` **(Stateful Encapsulated Methods)** (Impact: 12.5)
  * `firstUnescapedDoubleQuote` **(Stateful Encapsulated Methods)** (Impact: 10.9)
  * `firstIndex` **(Stateful Encapsulated Methods)** (Impact: 9.0)
    * *Intent:* /// Returns the first index matching any of the passed in Characters, nil if no match
  * `getSeparatorCharacters` **(Stateful Encapsulated Methods)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (42.2%), Guard Balance (formerly Safety Score) (33.4%), Complexity Load (formerly Cognitive Load) (14.8%)
- **Documentation Coverage:** 96.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testSessionAuthentication` **(I/O & Config Routines)** (Impact: 11.1)
  * `testBasicAuthenticatorWithRedirect` **(Interface Declarations)** (Impact: 8.0)
  * `authenticate` **(Generic / Templated Code)** (Impact: 7.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (94.2%), Complexity Load (formerly Cognitive Load) (35.6%), Guard Balance (formerly Safety Score) (25.9%), Concurrency Surface (formerly Concurrency) (18.4%)
- **Documentation Coverage:** 70.5882% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `decode` **(Stateful Encapsulated Methods)** (Impact: 43.7)
    * *Intent:* /// Fundamental decode: Transform any given byte sequence encoded with BaseN to an unencoded byte se...
  * `init` **(Stateful Encapsulated Methods)** (Impact: 35.5)
  * `init` **(Stateful Encapsulated Methods)** (Impact: 25.2)
  * `encode` **(Stateful Encapsulated Methods)** (Impact: 21.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (93.6%), Mutation Surface (formerly State Flux) (89.3%), Dead Code Surface (formerly Dead Code) (85.2%), Guard Balance (formerly Safety Score) (37.9%)
...
**Top Internal Functions/Classes:**
  ...
  * `load` **(Many-Argument Workhorses)** (Impact: 17.5)
    * *Intent:* /// Reads the dotenv files relevant to the environment and loads them into the process. /// /// let ...
  * `parseLineValue` **(Stateful Encapsulated Methods)** (Impact: 11.2)
  * `countDistance` **(Stateful Encapsulated Methods)** (Impact: 10.8)
  * `parseNext` **(Stateful Encapsulated Methods)** (Impact: 8.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 3.228; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (77.4%)
- **Documentation Coverage:** 80.6452% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `decode64` **(Stateful Encapsulated Methods)** (Impact: 36.9)
    * *Intent:* /// Specialization of ``decode(_:base:using:)`` for Base64 with no ignores and optional padding.
  * `encode64` **(Stateful Encapsulated Methods)** (Impact: 25.4)
