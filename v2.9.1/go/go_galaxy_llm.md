...
## 1. EXECUTIVE SUMMARY
- **Scope:** 9294 analyzed artifact(s), 1266619 LOC.
- **Load-bearing artifact:** `src/bytes/bytes.go` -- 774 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/crypto/internal/fips140test/mldsa_test.go` -- pulls in 466 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/regexp/all_test.go` at magnitude 9263.8 (structural weight, not risk).
...
|---|---|---|
| Modularity | 0.5909 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.1475 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 1.8336 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `5.561`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +5.56; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 33%, Encapsulated Accessors Files 16%, Declarative / Non-Code 9%, Large Core Modules (3) 7%, Callbacks & Closures Files 7%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 38.3 | 29.4 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 15.6 | 2.3 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 8.8 | 3.1 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 57.7 | 63.3 | 100.0 |
...
| connectivity | 79931 | 4600 | 16 | `src/syscall/types_windows.go` |
...
| dead_code | 29997 | 4998 | 8 | `src/internal/types/errors/codes.go` |
| credential | 117 | 29 | 0 | `src/crypto/x509/verify_test.go` |
| threat | 11854 | 1492 | 2 | `src/reflect/value.go` |
...
- `main` **(Many-Argument Workhorses)** (@ `src/regexp/testdata/testregex.c`) -> Impact: **753.3** | LOC: 1071
- `ssaGenValue` **(Many-Argument Workhorses)** (@ `src/cmd/compile/internal/amd64/ssa.go`) -> Impact: **579.1** | LOC: 1674
- `TestDirectoryJunction` **(Many-Argument Workhorses)** (@ `src/os/os_windows_test.go`) -> Impact: **571.7** | LOC: 2044
- `Load` **(Many-Argument Workhorses)** (@ `src/cmd/link/internal/loadelf/ldelf.go`) -> Impact: **555.0** | LOC: 600
...
- `builtin` **(Many-Argument Workhorses)** (@ `src/cmd/compile/internal/types2/builtins.go`) -> Impact: **550.1** | LOC: 962
...
- `regalloc` **(Many-Argument Workhorses)** (@ `src/cmd/compile/internal/ssa/regalloc.go`) -> Impact: **513.2** | LOC: 1242
- `ssaGenValue` **(Many-Argument Workhorses)** (@ `src/cmd/compile/internal/ppc64/ssa.go`) -> Impact: **420.9** | LOC: 1957
- `ssaGenValue` **(Many-Argument Workhorses)** (@ `src/cmd/compile/internal/arm64/ssa.go`) -> Impact: **400.6** | LOC: 1361
- `update` **(Many-Argument Workhorses)** (@ `src/cmd/compile/internal/ssa/prove.go`) -> Impact: **399.0** | LOC: 387
  * *Intent:* // update updates the set of relations between v and w in domain d // restricting it to r.
- `parseSpans` **(Stateful Encapsulated Methods)** (@ `src/go/doc/comment/parse.go`) -> Impact: **383.0** | LOC: 901
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 0.076; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (96.4%), Guard Balance (formerly Safety Score) (58.1%), Complexity Load (formerly Cognitive Load) (17.3%), Debt Markers (formerly Tech Debt) (10.5%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **36**; blast radius 0.076; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.9%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (73.7%)
- **Documentation Coverage:** 72.0833% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `exprCheckPtr` **(Many-Argument Workhorses)** (Impact: 341.2)
  * `genssa` **(Many-Argument Workhorses)** (Impact: 271.5)
    * *Intent:* // genssa appends entries to pp for each instruction in f.
  * `stmt` **(Many-Argument Workhorses)** (Impact: 252.5)
    * *Intent:* // stmt converts the statement n to SSA and adds it to s.
  * `conv` **(Many-Argument Workhorses)** (Impact: 222.4)
  * `buildssa` **(Many-Argument Workhorses)** (Impact: 164.3)
...
- `src/cmd/link/link_test.go` -> Churn: **76.9%** | Cog Load: 29.8234% | Debt: 64.4776%
- `src/cmd/compile/internal/ssagen/ssa.go` -> Churn: **73.66%** | Cog Load: 51.3922% | Debt: 11.385%
- `src/cmd/link/internal/ld/data.go` -> Churn: **72.29%** | Cog Load: 52.1458% | Debt: 11.1651%
- `src/runtime/mheap.go` -> Churn: **69.62%** | Cog Load: 40.4571% | Debt: 79.6143%
- `src/runtime/malloc.go` -> Churn: **65.9%** | Cog Load: 45.7901% | Debt: 53.8705%
...
- `src/bytes/bytes.go` -> **Severity: 0.003** (Bridge: 0.0 * Flux: 99.9997%)
- `src/cmd/compile/internal/reflectdata/reflect.go` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 99.9864%)
- `src/internal/testenv/testenv.go` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 78.7156%)
- `src/archive/tar/strconv.go` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 99.9998%)
- `src/crypto/x509/x509.go` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 99.9608%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `src/bytes/bytes.go` -> **Severity: 9.264** (Embedded: 0.1026 * Error Risk: 90.3034%)
- `src/math/bits.go` -> **Severity: 6.39** (Embedded: 0.0695 * Error Risk: 91.9405%)
- `src/archive/tar/strconv.go` -> **Severity: 5.534** (Embedded: 0.0642 * Error Risk: 86.1762%)
- `src/cmd/compile/internal/reflectdata/reflect.go` -> **Severity: 2.946** (Embedded: 0.0348 * Error Risk: 84.6567%)
- `src/os/exec.go` -> **Severity: 2.604** (Embedded: 0.033 * Error Risk: 78.89%)
