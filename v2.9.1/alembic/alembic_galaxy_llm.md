...
## 1. EXECUTIVE SUMMARY
- **Scope:** 119 analyzed artifact(s), 42393 LOC.
- **Load-bearing artifact:** `alembic-1.18.4/alembic/testing/fixtures.py` -- 28 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `alembic-1.18.4/alembic/ddl/postgresql.py` -- pulls in 29 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `alembic-1.18.4/alembic/script/revision.py` at magnitude 1354.12 (structural weight, not risk).
...
| Avg Path Length | 2.9034 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.413`
> **Composition Archetype:** `Small Flat Repo (2)` (z +1.41; from the repo's file-archetype mix)
> **File Composition:** Parameter Forwarders Files 24%, Generic / Templated Code Files 22%, Data / Markup / Trivial 18%, Large Core Modules (3) 13%, Large Core Modules 5%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 71.9 | 94.1 | 100.0 |
...
- `_compare_indexes_and_uniques` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/alembic/autogenerate/compare/constraints.py`) -> Impact: **236.4** | LOC: 389
- `alter_column` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/alembic/ddl/mysql.py`) -> Impact: **170.9** | LOC: 119
- `_generate_stub_for_meth` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/tools/write_pyi.py`) -> Impact: **129.2** | LOC: 123
- `generate_revision` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/alembic/script/base.py`) -> Impact: **123.6** | LOC: 151
- `tox_parameters` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/tools/toxnox.py`) -> Impact: **108.0** | LOC: 151
- `_compare_foreign_keys` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/alembic/autogenerate/compare/constraints.py`) -> Impact: **104.1** | LOC: 125
- `configure` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/alembic/runtime/environment.py`) -> Impact: **80.7** | LOC: 532
- `alter_column` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/alembic/ddl/mssql.py`) -> Impact: **77.2** | LOC: 102
- `_recur_param` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/tools/toxnox.py`) -> Impact: **65.7** | LOC: 74
- `init` **(Many-Argument Workhorses)** (@ `alembic-1.18.4/alembic/command.py`) -> Impact: **65.6** | LOC: 150
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **7**; blast radius 42.795; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.2%), Complexity Load (formerly Cognitive Load) (68.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 75.2688% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_parse_downgrade_target` **(Many-Argument Workhorses)** (Impact: 64.1)
  * `_parse_upgrade_target` **(Many-Argument Workhorses)** (Impact: 61.4)
  * `_collect_upgrade_revisions` **(Many-Argument Workhorses)** (Impact: 60.6)
  * `_collect_downgrade_revisions` **(Many-Argument Workhorses)** (Impact: 58.0)
  * `_walk` **(Many-Argument Workhorses)** (Impact: 52.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (73.7%), Connectivity (formerly Api Exposure) (63.2%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (18.9%)
- **Documentation Coverage:** 90.5882% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_drop_fk_with_mixed_case_name` **(Many-Argument Workhorses)** (Impact: 30.2)
    * *Intent:* """Test #1743"""
  * `test_include_object` **(Defensive Guards)** (Impact: 22.3)
  * `include_object` **(Defensive Guards)** (Impact: 20.4)
  * `test_include_name` **(Compute Cores)** (Impact: 17.5)
  * `test_compare_metadata_include_name` **(Compute Cores)** (Impact: 15.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **18**; blast radius 3.638; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (72.7%)
- **Documentation Coverage:** 98.7342% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_repr_type` **(Stateful Encapsulated Methods)** (Impact: 40.1)
  * `_alter_column` **(Many-Argument Workhorses)** (Impact: 32.5)
  * `_uq_constraint` **(Stateful Encapsulated Methods)** (Impact: 32.1)
  * `_add_table` **(Many-Argument Workhorses)** (Impact: 30.5)
  * `_render_column` **(Many-Argument Workhorses)** (Impact: 30.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000321 | `Ripple Effect (Closeness):` 0.022222
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (68.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (14.7%), Connectivity (formerly Api Exposure) (13.0%)
- **Documentation Coverage:** 83.6364% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_repr_custom_type` **(Many-Argument Workhorses)** (Impact: 19.6)
    * *Intent:* """test #1167 as well as other user defined type variations"""
  * `render` **(Defensive Guards)** (Impact: 16.8)
  * `test_render_custom` **(Compute Cores)** (Impact: 13.7)
  * `test_render_create_table_comment_op_batch` **(Compute Cores)** (Impact: 13.6)
    * *Intent:* """test #1361"""
  * `test_render_drop_table_comment_op_batch` **(Compute Cores)** (Impact: 13.5)
    * *Intent:* """test #1361"""
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (62.5%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (13.0%), Connectivity (formerly Api Exposure) (10.2%)
- **Documentation Coverage:** 96.2264% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_assert_impl` **(Many-Argument Workhorses)** (Impact: 50.0)
  * `test_change_type_int_to_boolean` **(Compute Cores)** (Impact: 11.2)
  * `test_add_col_table_has_native_boolean` **(Compute Cores)** (Impact: 10.4)
  * `_multi_fk_fixture` **(Stateful Encapsulated Methods)** (Impact: 7.9)
  * `_literal_ck_fixture` **(Stateful Encapsulated Methods)** (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **13** in-repo importer(s); it depends on **23**; blast radius 69.132; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.8%), Guard Balance (formerly Safety Score) (89.6%), Connectivity (formerly Api Exposure) (85.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 93.75% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_foreign_key` **(Many-Argument Workhorses)** (Impact: 20.2)
  * `from_constraint` **(Many-Argument Workhorses)** (Impact: 19.3)
  * `batch_create_foreign_key` **(Many-Argument Workhorses)** (Impact: 17.6)
  * `reverse` **(Compute Cores)** (Impact: 17.1)
  * `to_table` **(Compute Cores)** (Impact: 16.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.010351 | `Ripple Effect (Closeness):` 0.255183
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (70.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (16.0%), Connectivity (formerly Api Exposure) (11.3%)
- **Documentation Coverage:** 96.319% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_remove_connection_uq` **(Many-Argument Workhorses)** (Impact: 18.9)
  * `test_remove_connection_index` **(Compute Cores)** (Impact: 18.5)
  * `include_object` **(Defensive Guards)** (Impact: 15.2)
  * `test_drop_table_w_uq_constraint` **(Defensive Guards)** (Impact: 13.2)
  * `include_object` **(Defensive Guards)** (Impact: 12.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (77.9%), Guard Balance (formerly Safety Score) (57.8%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Concurrency Surface (formerly Concurrency) (21.2%)
- **Documentation Coverage:** 95.0617% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_generic_alter_column_type_and_nullability` **(Many-Argument Workhorses)** (Impact: 11.1)
  * `test_alter_column_schema_type_unnamed` **(Compute Cores)** (Impact: 10.0)
  * `test_add_column_primary_key` **(Compute Cores)** (Impact: 9.9)
  * `test_add_foreign_key_dialect_kw` **(I/O & Config Routines)** (Impact: 7.7)
  * `test_alter_column_computed_not_supported` **(Compute Cores)** (Impact: 6.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **21**; blast radius 3.067; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.7%), Complexity Load (formerly Cognitive Load) (68.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 67.1642% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `generate_revision` **(Many-Argument Workhorses)** (Impact: 123.6)
  * `_head_only` **(Many-Argument Workhorses)** (Impact: 57.5)
  * `_from_path` **(Many-Argument Workhorses)** (Impact: 38.6)
  * `_stamp_revs` **(Many-Argument Workhorses)** (Impact: 32.6)
  * `_catch_revision_errors` **(Many-Argument Workhorses)** (Impact: 28.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **17**; blast radius 2.837; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_compare_indexes_and_uniques` **(Many-Argument Workhorses)** (Impact: 236.4)
  * `_compare_foreign_keys` **(Many-Argument Workhorses)** (Impact: 104.1)
  * `_correct_for_uq_duplicates_uix` **(Many-Argument Workhorses)** (Impact: 45.4)
  * `_compare_nullable` **(Many-Argument Workhorses)** (Impact: 24.7)
  * `_make_foreign_key` **(Many-Argument Workhorses)** (Impact: 22.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (63.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (15.1%), Connectivity (formerly Api Exposure) (12.2%)
- **Documentation Coverage:** 94.5312% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_compare_default_roundtrip` **(Many-Argument Workhorses)** (Impact: 12.1)
  * `test_add_identity_to_column` **(Compute Cores)** (Impact: 10.7)
  * `test_add_column_identity` **(Compute Cores)** (Impact: 10.6)
  * `test_toggle_not_distinct` **(Many-Argument Workhorses)** (Impact: 10.1)
  * `test_add` **(Many-Argument Workhorses)** (Impact: 10.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **22**; blast radius 20.778; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (82.9%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (69.7%)
- **Documentation Coverage:** 79.1667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `configure` **(Many-Argument Workhorses)** (Impact: 29.7)
  * `run_migrations` **(Many-Argument Workhorses)** (Impact: 27.8)
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 26.5)
  * `begin_transaction` **(Many-Argument Workhorses)** (Impact: 25.7)
  * `autocommit_block` **(Stateful Encapsulated Methods)** (Impact: 20.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.007152 | `Ripple Effect (Closeness):` 0.165119
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (49.7%), Complexity Load (formerly Cognitive Load) (16.7%), Connectivity (formerly Api Exposure) (12.4%)
- **Documentation Coverage:** 96.0289% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_assert_sql` **(Stateful Encapsulated Methods)** (Impact: 21.3)
  * `test_help_text` **(Defensive Guards)** (Impact: 16.0)
  * `_eq_cmd_output` **(Stateful Encapsulated Methods)** (Impact: 11.2)
  * `test_init_file_exists_and_is_empty` **(Type Conversions)** (Impact: 9.7)
  * `test_init_custom_template_location` **(Many-Argument Workhorses)** (Impact: 9.1)
    * *Intent:* """test #1660"""
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **12**; blast radius 6.769; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.3%), Complexity Load (formerly Cognitive Load) (88.0%), Connectivity (formerly Api Exposure) (53.1%)
- **Documentation Coverage:** 91.3043% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `alter_column` **(Many-Argument Workhorses)** (Impact: 50.2)
  * `_setup_dependencies_for_add_column` **(Many-Argument Workhorses)** (Impact: 47.1)
  * `_grab_table_elements` **(Stateful Encapsulated Methods)** (Impact: 24.9)
  * `_setup_referent` **(Stateful Encapsulated Methods)** (Impact: 21.8)
  * `drop_constraint` **(Defensive Guards)** (Impact: 20.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **11** in-repo importer(s); it depends on **21**; blast radius 12.66; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.1%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (65.4%)
- **Documentation Coverage:** 91.3978% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `alter_column` **(Many-Argument Workhorses)** (Impact: 59.4)
  * `_exec` **(Many-Argument Workhorses)** (Impact: 41.4)
  * `bulk_insert` **(Many-Argument Workhorses)** (Impact: 33.2)
  * `compare_indexes` **(Many-Argument Workhorses)** (Impact: 20.1)
  * `_column_args_match` **(Stateful Encapsulated Methods)** (Impact: 19.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004834 | `Ripple Effect (Closeness):` 0.10744
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (45.9%), Connectivity (formerly Api Exposure) (13.3%), Complexity Load (formerly Cognitive Load) (5.3%)
- **Documentation Coverage:** 99.3311% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_assert_iteration` **(Many-Argument Workhorses)** (Impact: 19.8)
  * `test_all` **(Type Conversions)** (Impact: 9.5)
  * `test_no_revision_exists` **(Compute Cores)** (Impact: 6.5)
  * `test_partial_id_resolve_too_short` **(Compute Cores)** (Impact: 4.7)
  * `test_ancestor_nodes` **(I/O & Config Routines)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **29**; blast radius 3.228; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.7%), Debt Markers (formerly Tech Debt) (91.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_exclude_constraint` **(Many-Argument Workhorses)** (Impact: 33.0)
  * `_cleanup_index_expr` **(Stateful Encapsulated Methods)** (Impact: 31.6)
  * `compare_indexes` **(Many-Argument Workhorses)** (Impact: 30.9)
  * `autogen_column_reflect` **(Many-Argument Workhorses)** (Impact: 28.7)
  * `compare_server_default` **(Many-Argument Workhorses)** (Impact: 27.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000871 | `Ripple Effect (Closeness):` 0.016667
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (60.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (10.9%), Complexity Load (formerly Cognitive Load) (10.3%)
- **Documentation Coverage:** 97.7011% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_reordering_example_wo_copy` **(I/O & Config Routines)** (Impact: 11.6)
    * *Intent:* """test related to #1692 in that we identified the recipe for rewriting column ordering was using co...
  * `order_columns` **(Many-Argument Workhorses)** (Impact: 11.3)
  * `test_file_template_with_directory_path` **(Defensive Guards)** (Impact: 11.0)
  * `test_create_script_branches_old_template` **(Compute Cores)** (Impact: 8.4)
  * `_test_tz` **(Stateful Encapsulated Methods)** (Impact: 8.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (63.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (18.8%), Connectivity (formerly Api Exposure) (12.9%)
- **Documentation Coverage:** 82.0755% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_downgrade_branch_dependency` **(Defensive Guards)** (Impact: 5.6)
    * *Intent:* """c2branch depends on c1branch so taking down c1branch requires taking down both"""
  * `_assert_downgrade` **(Stateful Encapsulated Methods)** (Impact: 5.3)
  * `_assert_upgrade` **(Stateful Encapsulated Methods)** (Impact: 5.3)
  * `test_upgrade` **(I/O & Config Routines)** (Impact: 4.1)
  * `setup_class` **(Generic / Templated Code)** (Impact: 4.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **16** in-repo importer(s); it depends on **16**; blast radius 46.1; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (63.2%)
- **Documentation Coverage:** 68.4211% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_inis_from_config` **(Stateful Encapsulated Methods)** (Impact: 27.9)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 20.3)
  * `_inspect_function` **(Stateful Encapsulated Methods)** (Impact: 18.8)
  * `_generate_args` **(Many-Argument Workhorses)** (Impact: 18.4)
  * `get_prepend_sys_paths_list` **(Compute Cores)** (Impact: 14.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.01774 | `Ripple Effect (Closeness):` 0.26534
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (84.1%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (57.0%)
- **Documentation Coverage:** 78.1818% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `init` **(Many-Argument Workhorses)** (Impact: 65.6)
  * `history` **(Stateful Encapsulated Methods)** (Impact: 41.4)
  * `revision` **(Many-Argument Workhorses)** (Impact: 37.5)
  * `stamp` **(Many-Argument Workhorses)** (Impact: 27.7)
  * `current` **(Many-Argument Workhorses)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (57.8%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (20.6%), Connectivity (formerly Api Exposure) (9.0%)
- **Documentation Coverage:** 96.4286% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_setup_revision_files` **(Stateful Encapsulated Methods)** (Impact: 19.0)
  * `_assert_setup` **(Many-Argument Workhorses)** (Impact: 16.6)
  * `test_steps` **(Defensive Guards)** (Impact: 11.7)
  * `_opened_transaction_fixture` **(Many-Argument Workhorses)** (Impact: 10.9)
  * `_setup_for_fixture` **(Stateful Encapsulated Methods)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **15**; blast radius 3.804; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.1%), Debt Markers (formerly Tech Debt) (86.8%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (74.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `alter_column` **(Many-Argument Workhorses)** (Impact: 170.9)
  * `compare_server_default` **(Many-Argument Workhorses)** (Impact: 52.3)
  * `correct_for_autogen_constraints` **(Many-Argument Workhorses)** (Impact: 26.3)
  * `correct_for_autogen_foreignkeys` **(Many-Argument Workhorses)** (Impact: 21.4)
  * `_mysql_colspec` **(Stateful Encapsulated Methods)** (Impact: 19.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 2.53; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (71.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (31.3%), Connectivity (formerly Api Exposure) (11.5%)
- **Documentation Coverage:** 97.8495% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_bool` **(Many-Argument Workhorses)** (Impact: 24.3)
  * `test_truncate_slug_length_types` **(Many-Argument Workhorses)** (Impact: 24.1)
  * `test_prepend_sys_path_locations` **(Many-Argument Workhorses)** (Impact: 19.5)
  * `test_version_locations` **(Many-Argument Workhorses)** (Impact: 17.0)
  * `test_write_hooks` **(Compute Cores)** (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **28**; blast radius 5.379; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (90.9%), Guard Balance (formerly Safety Score) (90.4%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (76.6%)
- **Documentation Coverage:** 88.4615% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `register_operation` **(Many-Argument Workhorses)** (Impact: 20.9)
  * `register` **(Compute Cores)** (Impact: 15.1)
  * `create_foreign_key` **(Many-Argument Workhorses)** (Impact: 11.1)
  * `run_async` **(Many-Argument Workhorses)** (Impact: 10.9)
  * `batch_alter_table` **(Many-Argument Workhorses)** (Impact: 10.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001956 | `Ripple Effect (Closeness):` 0.025
...
- `alembic-1.18.4/alembic/autogenerate/api.py` -> **Severity: 2.488** (Bridge: 0.0249 * Flux: 99.9995%)
- `alembic-1.18.4/alembic/autogenerate/compare/schema.py` -> **Severity: 2.116** (Bridge: 0.0212 * Flux: 99.9932%)
- `alembic-1.18.4/alembic/config.py` -> **Severity: 1.774** (Bridge: 0.0177 * Flux: 99.9992%)
- `alembic-1.18.4/alembic/util/pyfiles.py` -> **Severity: 1.459** (Bridge: 0.0146 * Flux: 100.0%)
- `alembic-1.18.4/alembic/autogenerate/compare/types.py` -> **Severity: 1.398** (Bridge: 0.014 * Flux: 99.9169%)
