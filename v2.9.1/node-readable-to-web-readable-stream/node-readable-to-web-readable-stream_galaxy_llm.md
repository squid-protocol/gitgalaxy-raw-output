# ARCHITECTURAL_BRIEF: node-readable-to-web-readable-stream
...
## 1. EXECUTIVE SUMMARY
- **Scope:** 3 analyzed artifact(s), 164 LOC.
- **Load-bearing artifact:** `package/LICENSE.txt` -- 1 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `package/README.md` -- pulls in 1 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `package/README.md` at magnitude 1.78 (structural weight, not risk).
...
1. **LICENSE.txt** (`package/LICENSE.txt`) — 1 inbound connections
...
2. **LICENSE.txt** (`package/LICENSE.txt`) — 0 outbound dependencies
...
- **Magnitude:** 1.78 | **LOC:** 89 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 259.74; role: Pure Consumer (Orchestrator)
...
* *Network Topology:*
  ...
  * `Imports (Out-Degree: 1):` LICENSE.txt
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 480.52; role: Pure Producer (Foundation)
- **Top Surface Vectors:** None above 0%
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
* *Risk/State:* None
