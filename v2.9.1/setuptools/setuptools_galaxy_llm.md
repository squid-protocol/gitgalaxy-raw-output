...
## 1. EXECUTIVE SUMMARY
- **Scope:** 349 analyzed artifact(s), 47769 LOC.
- **Load-bearing artifact:** `setuptools/warnings.py` -- 54 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `setuptools/tests/test_editable_install.py` -- pulls in 48 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `setuptools/_vendor/more_itertools/more.py` at magnitude 3239.84 (structural weight, not risk).
...
| Avg Path Length | 2.8388 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.526`
> **Composition Archetype:** `Hub-Coupled App` (z +1.53; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules (2) 19%, Large Core Modules (3) 19%, Data / Markup / Trivial 18%, Generic / Templated Code Files 11%, Parameter Forwarders Files 10%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 56.6 | 66.7 | 100.0 |
...
- `__init__` **(Many-Argument Workhorses)** (@ `setuptools/_vendor/backports/tarfile/__init__.py`) -> Impact: **121.0** | LOC: 101
- `__init__` **(Many-Argument Workhorses)** (@ `setuptools/_distutils/extension.py`) -> Impact: **85.8** | LOC: 59
  * *Intent:* # When adding arguments to this constructor, be sure to update # setup_keywords in core.py.
- `_islice_helper` **(Many-Argument Workhorses)** (@ `setuptools/_vendor/more_itertools/more.py`) -> Impact: **72.8** | LOC: 106
- `copy_file` **(Many-Argument Workhorses)** (@ `setuptools/_distutils/file_util.py`) -> Impact: **70.1** | LOC: 101
- `link` **(Many-Argument Workhorses)** (@ `setuptools/_distutils/compilers/C/unix.py`) -> Impact: **69.0** | LOC: 63
- `gettarinfo` **(Many-Argument Workhorses)** (@ `setuptools/_vendor/backports/tarfile/__init__.py`) -> Impact: **65.3** | LOC: 98
...
- `_add_arguments` **(Many-Argument Workhorses)** (@ `setuptools/_vendor/autocommand/autoparse.py`) -> Impact: **61.2** | LOC: 106
...
- `parse_makefile` **(Many-Argument Workhorses)** (@ `setuptools/_distutils/sysconfig.py`) -> Impact: **60.9** | LOC: 110
...
- `byte_compile` **(Many-Argument Workhorses)** (@ `setuptools/_distutils/util.py`) -> Impact: **60.3** | LOC: 131
- `distinct_permutations` **(Many-Argument Workhorses)** (@ `setuptools/_vendor/more_itertools/more.py`) -> Impact: **57.6** | LOC: 148
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.7%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (53.1%)
- **Documentation Coverage:** 36.1702% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_islice_helper` **(Many-Argument Workhorses)** (Impact: 72.8)
  ...
  * `_get_slice` **(Stateful Encapsulated Methods)** (Impact: 32.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.3%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (58.9%)
- **Documentation Coverage:** 44.3966% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 121.0)
  * `gettarinfo` **(Many-Argument Workhorses)** (Impact: 65.3)
    * *Intent:* """Create a TarInfo object from the result of os.stat or equivalent on an existing file. The file is...
  * `open` **(Many-Argument Workhorses)** (Impact: 52.3)
    * *Intent:* #-------------------------------------------------------------------------- # Below are the classmet...
  * `_get_filtered_attrs` **(Many-Argument Workhorses)** (Impact: 51.1)
  * `_proc_pax` **(Many-Argument Workhorses)** (Impact: 50.0)
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **27**; blast radius 4.683; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.3%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (77.8%)
- **Documentation Coverage:** 64.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_parse_command_opts` **(Many-Argument Workhorses)** (Impact: 46.9)
    * *Intent:* """Parse the command-line options for a single command. 'parser' must be a FancyGetopt instance; 'ar...
  * `__init__` **(Many-Argument Workhorses)** (Impact: 37.2)
    * *Intent:* # -- Creation/initialization methods ------------------------------- # Can't Unpack a TypedDict with...
  * `_show_help` **(Many-Argument Workhorses)** (Impact: 34.3)
  * `parse_config_files` **(Many-Argument Workhorses)** (Impact: 31.0)
  * `_set_command_options` **(Defensive Guards)** (Impact: 28.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002444 | `Ripple Effect (Closeness):` 0.089212
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (45.9%)
- **Documentation Coverage:** 10.5263% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `WindowsSdkDir` **(Compute Cores)** (Impact: 23.3)
    * *Intent:* """ Microsoft Windows SDK directory. Return ------ str path """
  * `_find_dot_net_versions` **(Stateful Encapsulated Methods)** (Impact: 17.0)
    * *Intent:* """ Find Microsoft .NET Framework versions. Parameters ---------- bits: int Platform number of bits:...
  * `WindowsSDKExecutablePath` **(Compute Cores)** (Impact: 15.9)
    * *Intent:* """ Microsoft Windows SDK executable directory. Return ------ str | None path """
  * `_sdk_tools` **(Stateful Encapsulated Methods)** (Impact: 15.8)
    * *Intent:* """ Microsoft Windows SDK Tools paths generator. Return ------ generator of str paths """
  * `FxTools` **(Compute Cores)** (Impact: 15.6)
...
- **Blast Radius:** changing it is visible to **25** in-repo importer(s); it depends on **40**; blast radius 8.445; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.8%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (50.8%)
- **Documentation Coverage:** 40.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_parse_config_files` **(Many-Argument Workhorses)** (Impact: 31.6)
    * *Intent:* # FIXME: 'Distribution._parse_config_files' is too complex (14) """ Adapted from distutils.dist.Dist...
  * `_set_command_options` **(Defensive Guards)** (Impact: 28.2)
    * *Intent:* # FIXME: 'Distribution._set_command_options' is too complex (14) """ Set the options for 'command_ob...
  * `exclude_package` **(Compute Cores)** (Impact: 23.5)
    * *Intent:* """Remove packages, modules, and extensions in named package"""
  * `check_nsp` **(Many-Argument Workhorses)** (Impact: 15.3)
    * *Intent:* """Verify that namespace packages are valid"""
  * `_include_misc` **(Stateful Encapsulated Methods)** (Impact: 15.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.009096 | `Ripple Effect (Closeness):` 0.071633
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 1.196; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.2%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (51.6%)
...
**Top Internal Functions/Classes:**
  ...
  * `nth_combination` **(Many-Argument Workhorses)** (Impact: 20.0)
  ...
  * `grouper` **(Many-Argument Workhorses)** (Impact: 15.3)
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **23**; blast radius 6.98; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (56.4%)
- **Documentation Coverage:** 62.9921% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_fix_lib_args` **(Many-Argument Workhorses)** (Impact: 37.8)
  * `has_function` **(Many-Argument Workhorses)** (Impact: 36.2)
  * `_fix_compile_args` **(Stateful Encapsulated Methods)** (Impact: 33.2)
  * `gen_lib_options` **(Many-Argument Workhorses)** (Impact: 19.7)
  * `gen_preprocess_options` **(Many-Argument Workhorses)** (Impact: 19.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00292 | `Ripple Effect (Closeness):` 0.097959
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.4%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (66.6%)
...
**Top Internal Functions/Classes:**
  ...
  * `parse_inline_table` **(Many-Argument Workhorses)** (Impact: 21.8)
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **20**; blast radius 1.834; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (65.4%)
- **Documentation Coverage:** 55.5556% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `finalize_options` **(I/O & Config Routines)** (Impact: 52.2)
  * `check_extensions_list` **(Many-Argument Workhorses)** (Impact: 42.5)
    * *Intent:* """Ensure that the list of extensions (presumably provided as a command option 'extensions') is vali...
  * `swig_sources` **(Many-Argument Workhorses)** (Impact: 30.8)
    * *Intent:* """Walk the list of source files in 'sources', looking for SWIG interface (.i) files. Run SWIG on al...
  * `run` **(Compute Cores)** (Impact: 27.4)
    * *Intent:* # 'self.extensions', as supplied by setup.py, is a list of # Extension instances. See the documentat...
  * `get_libraries` **(Many-Argument Workhorses)** (Impact: 26.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000249 | `Ripple Effect (Closeness):` 0.014327
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.196; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (96.5%), Connectivity (formerly Api Exposure) (82.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (70.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `zip_broadcast` **(Generic / Templated Code)** (Impact: 3.7)
  * `zip_equal` **(Generic / Templated Code)** (Impact: 3.3)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **11**; blast radius 1.763; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (99.6%), Guard Balance (formerly Safety Score) (98.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 73.5849% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `cpython_tags` **(Many-Argument Workhorses)** (Impact: 36.2)
  * `mac_platforms` **(Many-Argument Workhorses)** (Impact: 31.4)
  * `_cpython_abis` **(Stateful Encapsulated Methods)** (Impact: 27.5)
  * `_mac_binary_formats` **(Stateful Encapsulated Methods)** (Impact: 22.3)
  * `generic_tags` **(Many-Argument Workhorses)** (Impact: 19.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000347 | `Ripple Effect (Closeness):` 0.025033
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **24**; blast radius 2.726; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.5%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (67.5%)
- **Documentation Coverage:** 53.6082% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `translate_pattern` **(Compute Cores)** (Impact: 32.3)
    * *Intent:* """ Translate a file path glob like '*.txt' in to a regular expression. This differs from fnmatch.tr...
  * `process_template_line` **(Many-Argument Workhorses)** (Impact: 17.3)
    * *Intent:* # Parse the line: split it up, make sure the right number of words # is there, and return the releva...
  * `write_or_delete_file` **(Many-Argument Workhorses)** (Impact: 15.5)
    * *Intent:* """Write `data` to `filename` or delete if empty If `data` is non-empty, this routine is the same as...
  * `_safe_path` **(Stateful Encapsulated Methods)** (Impact: 15.2)
  * `global_include` **(Compute Cores)** (Impact: 12.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002099 | `Ripple Effect (Closeness):` 0.040524
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **36**; blast radius 1.309; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (36.7%)
- **Documentation Coverage:** 82.0896% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_create_links` **(Stateful Encapsulated Methods)** (Impact: 20.7)
  * `_find_packages` **(Stateful Encapsulated Methods)** (Impact: 16.3)
  * `template_vars` **(Compute Cores)** (Impact: 12.6)
  * `_select_strategy` **(Stateful Encapsulated Methods)** (Impact: 12.4)
  * `_find_top_level_modules` **(Stateful Encapsulated Methods)** (Impact: 11.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 74 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000106 | `Ripple Effect (Closeness):` 0.002865
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **32**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (89.1%), Guard Balance (formerly Safety Score) (85.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_convert_egg_info_reqs_to_simple_reqs` **(Stateful Encapsulated Methods)** (Impact: 14.4)
    * *Intent:* """ Historically, setuptools would solicit and store 'extra' requirements, including those with envi...
  * `discover` **(Compute Cores)** (Impact: 12.9)
  * `files` **(Type Conversions)** (Impact: 11.9)
    * *Intent:* """Files in this distribution. :return: List of PackagePath for this distribution or None Result is ...
  * `make_file` **(Type Conversions)** (Impact: 10.3)
  * `quoted_marker` **(Compute Cores)** (Impact: 10.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **48**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (72.9%), Complexity Load (formerly Cognitive Load) (16.0%), Connectivity (formerly Api Exposure) (10.7%)
- **Documentation Coverage:** 70.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_editable_with_prefix` **(Many-Argument Workhorses)** (Impact: 15.9)
    * *Intent:* """ Editable install to a prefix should be discoverable. """
  * `test_case_sensitivity` **(Defensive Guards)** (Impact: 12.4)
  * `test_namespace_case_sensitivity` **(Defensive Guards)** (Impact: 10.7)
  * `test_combine_namespaces_nested` **(Type Conversions)** (Impact: 9.0)
    * *Intent:* """ Users may attempt to combine namespace packages in a nested way via ``package_dir`` as shown in ...
  * `test_distutils_leave_inplace_files` **(Defensive Guards)** (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **9**; blast radius 8.622; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (48.1%)
- **Documentation Coverage:** 53.9474% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `filter` **(Many-Argument Workhorses)** (Impact: 44.6)
  * `filter` **(Many-Argument Workhorses)** (Impact: 33.0)
  * `__and__` **(Defensive Guards)** (Impact: 18.8)
    * *Intent:* """Return a SpecifierSet which is a combination of the two sets. :param other: The other object to c...
  * `_compare_greater_than` **(Stateful Encapsulated Methods)** (Impact: 15.7)
    * *Intent:* # Convert our spec to a Version instance, since we'll want to work with # it as a version. spec = se...
  * `contains` **(Many-Argument Workhorses)** (Impact: 15.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **12**; blast radius 1.564; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (36.7%)
- **Documentation Coverage:** 67.8571% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_email` **(Many-Argument Workhorses)** (Impact: 45.7)
    * *Intent:* """Parse a distribution's metadata stored as email headers (e.g. from ``METADATA``). This function r...
  * `_write_metadata` **(Stateful Encapsulated Methods)** (Impact: 25.5)
    * *Intent:* """ Return an RFC822 message with the metadata. """
  * `_process_license_files` **(Stateful Encapsulated Methods)** (Impact: 20.3)
  * `_process_import_names` **(Stateful Encapsulated Methods)** (Impact: 18.4)
  * `from_email` **(Defensive Guards)** (Impact: 15.3)
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **9**; blast radius 6.722; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (45.8%)
- **Documentation Coverage:** 48.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_cmpkey` **(Many-Argument Workhorses)** (Impact: 53.2)
  * `__replace__` **(Compute Cores)** (Impact: 34.5)
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 32.4)
    * *Intent:* """Initialize a Version object. :param version: The string representation of a version which will be...
  * `_validate_release` **(Stateful Encapsulated Methods)** (Impact: 11.8)
  * `_validate_pre` **(Stateful Encapsulated Methods)** (Impact: 10.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **25**; blast radius 1.535; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Complexity Load (formerly Cognitive Load) (61.0%), Connectivity (formerly Api Exposure) (49.0%)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8.3e-05 | `Ripple Effect (Closeness):` 0.002865
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **23**; blast radius 2.047; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (60.5%)
- **Documentation Coverage:** 67.5676% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get_tag` **(Compute Cores)** (Impact: 29.7)
    * *Intent:* # bdist sets self.plat_name if unset, we should only use it for purepy # wheels if the user supplied...
  * `license_paths` **(Compute Cores)** (Impact: 24.8)
  * `run` **(I/O & Config Routines)** (Impact: 21.6)
  * `egg2dist` **(Many-Argument Workhorses)** (Impact: 21.1)
    * *Intent:* """Convert an .egg-info directory into a .dist-info directory"""
  * `get_abi_tag` **(Defensive Guards)** (Impact: 19.6)
    * *Intent:* """Return the ABI tag based on SOABI (if available) or emulate SOABI (PyPy2)."""
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002119 | `Ripple Effect (Closeness):` 0.040957
...
- **Blast Radius:** changing it is visible to **27** in-repo importer(s); it depends on **17**; blast radius 17.204; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (47.6%)
- **Documentation Coverage:** 42.8571% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_makefile` **(Many-Argument Workhorses)** (Impact: 60.9)
    * *Intent:* """Parse a Makefile-style file. A dictionary containing name/value pairs is returned. If an optional...
  * `get_python_lib` **(Many-Argument Workhorses)** (Impact: 40.2)
  * `customize_compiler` **(I/O & Config Routines)** (Impact: 17.0)
    * *Intent:* """Do any platform-specific customization of a CCompiler instance. Mainly needed on Unix, so we can ...
  * `get_python_inc` **(Defensive Guards)** (Impact: 13.3)
    * *Intent:* """Return the directory containing installed Python header files. If 'plat_specific' is false (the d...
  * `expand_makefile_vars` **(Compute Cores)** (Impact: 9.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.006712 | `Ripple Effect (Closeness):` 0.162968
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **19**; blast radius 2.043; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (51.2%)
- **Documentation Coverage:** 16.129% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `finalize_options` **(I/O & Config Routines)** (Impact: 50.0)
    * *Intent:* # -- Option finalizing methods ------------------------------------- # (This is rather more involved...
  * `finalize_unix` **(Compute Cores)** (Impact: 27.9)
    * *Intent:* """Finalizes options for posix platforms."""
  * `run` **(Compute Cores)** (Impact: 20.8)
    * *Intent:* # -- Command execution methods ------------------------------------- """Runs the command."""
  * `handle_extra_path` **(Compute Cores)** (Impact: 17.3)
    * *Intent:* """Set `path_file` and `extra_dirs` using `extra_path`."""
  * `dump_dirs` **(Compute Cores)** (Impact: 11.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000443 | `Ripple Effect (Closeness):` 0.009169
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **24**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (71.7%), Complexity Load (formerly Cognitive Load) (24.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (11.7%), Connectivity (formerly Api Exposure) (11.5%)
- **Documentation Coverage:** 70.2128% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_invalid_extension_depends` **(Many-Argument Workhorses)** (Impact: 12.1)
    * *Intent:* """ Due to backwards compatibility reasons, `Extension.depends` should accept invalid/weird paths, b...
  * `test_exclude_dev_only_cache_folders` **(Defensive Guards)** (Impact: 10.6)
  * `test_sdist_with_utf8_encoded_filename` **(Defensive Guards)** (Impact: 9.9)
    * *Intent:* # Test for #303. dist = Distribution(self.make_strings(SETUP_ATTRS)) dist.script_name = 'setup.py' c...
  * `make_strings` **(Defensive Guards)** (Impact: 7.2)
  * `test_sdist_with_latin1_encoded_filename` **(Defensive Guards)** (Impact: 7.1)
    * *Intent:* # Test for #303. dist = Distribution(self.make_strings(SETUP_ATTRS)) dist.script_name = 'setup.py' c...
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 1.196; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (57.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (41.6%), Complexity Load (formerly Cognitive Load) (37.0%)
- **Documentation Coverage:** 83.9506% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_apply_pyproject_equivalent_to_setupcfg` **(Defensive Guards)** (Impact: 44.4)
  * `test_missing_patterns` **(Type Conversions)** (Impact: 7.5)
  * `core_metadata` **(Type Conversions)** (Impact: 7.0)
    * *Intent:* # --- Auxiliary Functions ---
  * `_mock_expand_patterns` **(Stateful Encapsulated Methods)** (Impact: 6.4)
    * *Intent:* """ Allow comparing the given patterns for 2 dist objects. We need to strip special chars to avoid e...
  * `test_both_license_and_license_files_defined` **(Defensive Guards)** (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **8**; blast radius 5.392; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.8%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (53.1%)
- **Documentation Coverage:** 32.3529% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getopt` **(Many-Argument Workhorses)** (Impact: 37.0)
    * *Intent:* """Parse command-line options in args. Store as attributes on object. If 'args' is None or not suppl...
  * `_grok_option_table` **(Compute Cores)** (Impact: 33.6)
    * *Intent:* """Populate the various data structures that keep tabs on the option table. Called by 'getopt()' bef...
  * `generate_help` **(Many-Argument Workhorses)** (Impact: 31.6)
    * *Intent:* """Generate help text (a list of strings, one per suggested line of output) from the option table fo...
  * `wrap_text` **(Compute Cores)** (Impact: 26.8)
    * *Intent:* """wrap_text(text : string, width : int) -> [string] Split 'text' into multiple lines of no more tha...
  * `_check_alias_dict` **(Stateful Encapsulated Methods)** (Impact: 8.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 76 instances
...
- `setuptools/dist.py` -> **Severity: 0.91** (Bridge: 0.0091 * Flux: 100.0%)
- `setuptools/_distutils/util.py` -> **Severity: 0.735** (Bridge: 0.0074 * Flux: 100.0%)
- `setuptools/_distutils/sysconfig.py` -> **Severity: 0.671** (Bridge: 0.0067 * Flux: 100.0%)
- `setuptools/_distutils/core.py` -> **Severity: 0.41** (Bridge: 0.0041 * Flux: 99.9999%)
- `setuptools/_distutils/cmd.py` -> **Severity: 0.373** (Bridge: 0.0037 * Flux: 99.8105%)
