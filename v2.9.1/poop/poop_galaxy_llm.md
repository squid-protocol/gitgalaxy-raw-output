...
## 1. EXECUTIVE SUMMARY
- **Scope:** 3 analyzed artifact(s), 764 LOC.
- **Load-bearing artifact:** `src/progress.zig` -- 1 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/main.zig` -- pulls in 2 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/main.zig` at magnitude 325.52 (structural weight, not risk).
...
> **File Composition:** Large Core Modules (2) 67%, Data / Markup / Trivial 33%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 87.5 | 100.0 | 93.8 | 93.8 | 100.0 |
...
- `getStatScore95` **(Type Conversions)** (@ `src/main.zig`) -> Impact: **7.7** | LOC: 12
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 259.74; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (40.6%), Guard Balance (formerly Safety Score) (26.8%), Complexity Load (formerly Cognitive Load) (25.6%)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 480.52; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (74.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (40.6%), Mutation Surface (formerly State Flux) (28.5%), Guard Balance (formerly Safety Score) (27.0%)
...
**Top Internal Functions/Classes:**
  ...
  * `get` **(Parameter Forwarders)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
