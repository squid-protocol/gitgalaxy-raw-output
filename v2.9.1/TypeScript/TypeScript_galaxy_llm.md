...
## 1. EXECUTIVE SUMMARY
- **Scope:** 50256 analyzed artifact(s), 2272541 LOC.
- **Load-bearing artifact:** `package.json` -- 91 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/compiler/_namespaces/ts.ts` -- pulls in 75 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `tests/baselines/reference/largeControlFlowGraph.js` at magnitude 20415.06 (structural weight, not risk).
...
|---|---|---|
| Modularity | 0.8927 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.2399 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 1.0869 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `6.254`
> **Composition Archetype:** `Flat Modular Platform` (z +6.25; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 52%, Declarative / Non-Code 21%, State Mutators Files 11%, Interface Declarations Files 6%, Callbacks & Closures Files 3%
...
- `.map`: 665x Unresolved Ambiguity (No Retainable Structure), 105x Excluded (Saturation: Line 3 exceeds 500 chars), 85x Excluded (Saturation: Line 2 exceeds 500 chars)
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 31.9 | 0.0 | 0.0 |
...
| threat | 18612 | 4504 | 0 | `tests/baselines/reference/superInStaticMembers1(target=es5).js` |
...
- `getCompletionData` **(Many-Argument Workhorses)** (@ `src/services/completions.ts`) -> Impact: **1290.5** | LOC: 1135
- `createScanner` **(Many-Argument Workhorses)** (@ `src/compiler/scanner.ts`) -> Impact: **1121.7** | LOC: 1278
  * *Intent:* // Creates a scanner over a (possibly unspecified) range of a piece of text.
- `getSymbolDisplayPartsDocumentationAndSymbolKindWorker` **(Many-Argument Workhorses)** (@ `src/services/symbolDisplay.ts`) -> Impact: **919.7** | LOC: 749
- `run` **(Compute Cores)** (@ `tests/baselines/reference/enumLiteralsSubtypeReduction.js`) -> Impact: **778.3** | LOC: 1028
- `run` **(Compute Cores)** (@ `tests/baselines/reference/enumLiteralsSubtypeReduction.js`) -> Impact: **778.3** | LOC: 1028
- `formatSpanWorker` **(Many-Argument Workhorses)** (@ `src/services/formatting/formatting.ts`) -> Impact: **749.0** | LOC: 877
- `scanRegularExpressionWorker` **(Many-Argument Workhorses)** (@ `src/compiler/scanner.ts`) -> Impact: **721.9** | LOC: 997
- `createProgram` **(Many-Argument Workhorses)** (@ `src/compiler/program.ts`) -> Impact: **672.3** | LOC: 1051
- `createSyntacticTypeNodeBuilder` **(Many-Argument Workhorses)** (@ `src/compiler/expressionToTypeNode.ts`) -> Impact: **654.3** | LOC: 927
- `createResolutionCache` **(Many-Argument Workhorses)** (@ `src/compiler/resolutionCache.ts`) -> Impact: **651.0** | LOC: 1060
...
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (99.8%)
...
* *Defense:* None
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 0.019
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (96.1%), Connectivity (formerly Api Exposure) (20.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **23**; blast radius 0.119; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.0%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (73.2%), Guard Balance (formerly Safety Score) (72.6%)
- **Documentation Coverage:** 94.5977% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseJSDocCommentWorker` **(Many-Argument Workhorses)** (Impact: 544.5)
  * `isListElement` **(Compute Cores)** (Impact: 116.3)
    * *Intent:* // True if positioned at the start of a list element
  * `parseTagComments` **(Many-Argument Workhorses)** (Impact: 71.5)
  * `parseJsonText` **(Many-Argument Workhorses)** (Impact: 70.5)
  * `isStartOfType` **(Compute Cores)** (Impact: 66.2)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 0.119
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000118
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.019; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (89.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (89.5%), Mutation Surface (formerly State Flux) (81.5%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 15.3981% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolveNameHelper` **(Many-Argument Workhorses)** (Impact: 472.1)
  * `createNameResolver` **(Compute Cores)** (Impact: 349.3)
  * `isSourceElement` **(Compute Cores)** (Impact: 139.3)
  * `createEvaluator` **(Compute Cores)** (Impact: 80.1)
  * `evaluate` **(Compute Cores)** (Impact: 79.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (68.6%), Complexity Load (formerly Cognitive Load) (54.5%)
- **Documentation Coverage:** 77.4648% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getCompletionData` **(Many-Argument Workhorses)** (Impact: 1290.5)
  * `createCompletionEntry` **(Many-Argument Workhorses)** (Impact: 527.9)
  * `getCompletionEntriesFromSymbols` **(Many-Argument Workhorses)** (Impact: 362.4)
  * `isSolelyIdentifierDefinitionLocation` **(Compute Cores)** (Impact: 146.3)
    * *Intent:* /** */
  * `getCompletionEntryCodeActionsAndSourceDisplay` **(Many-Argument Workhorses)** (Impact: 133.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (53.8%), Complexity Load (formerly Cognitive Load) (8.3%), Dead Code Surface (formerly Dead Code) (6.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **75**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (85.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (74.5%)
- **Documentation Coverage:** 95.3301% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createNodeFactory` **(Many-Argument Workhorses)** (Impact: 331.0)
    * *Intent:* /** * Creates a `NodeFactory` that can be used to create and update a syntax tree. * */
  * `getTransformFlagsSubtreeExclusions` **(Compute Cores)** (Impact: 74.3)
    * *Intent:* /** * Gets the transform flags to exclude when unioning the transform flags of a subtree. */
  * `replaceModifiers` **(Compute Cores)** (Impact: 55.4)
  * `createToken` **(Compute Cores)** (Impact: 49.8)
  * `createCallBinding` **(Many-Argument Workhorses)** (Impact: 43.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.5%), Complexity Load (formerly Cognitive Load) (80.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 62.0553% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createScanner` **(Many-Argument Workhorses)** (Impact: 1121.7)
    * *Intent:* // Creates a scanner over a (possibly unspecified) range of a piece of text.
  * `scanRegularExpressionWorker` **(Many-Argument Workhorses)** (Impact: 721.9)
  * `scan` **(I/O & Config Routines)** (Impact: 207.9)
  * `iterateCommentRanges` **(Many-Argument Workhorses)** (Impact: 109.9)
    * *Intent:* * * to reduceLeft. If false, iteration stops when the callback returns a truthy value. * between tha...
  * `scanEscapeSequence` **(Compute Cores)** (Impact: 108.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (82.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (64.3%), Connectivity (formerly Api Exposure) (42.3%)
- **Documentation Coverage:** 86.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createPrinter` **(Many-Argument Workhorses)** (Impact: 623.5)
  * `pipelineEmitWithHintWorker` **(Many-Argument Workhorses)** (Impact: 469.8)
  * `emitFiles` **(Many-Argument Workhorses)** (Impact: 304.1)
    * *Intent:* // targetSourceFile is when users only want one file in entire project to be emitted. This is used i...
  * `emitNodeListItems` **(Many-Argument Workhorses)** (Impact: 145.6)
    * *Intent:* /** * Emits a list without brackets or raising events. * * NOTE: You probably don't want to call thi...
  * `emitNodeList` **(Many-Argument Workhorses)** (Impact: 81.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **71**; blast radius 0.019; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (87.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (74.1%)
- **Documentation Coverage:** 72.5888% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createProgram` **(Many-Argument Workhorses)** (Impact: 672.3)
  * `verifyCompilerOptions` **(I/O & Config Routines)** (Impact: 186.8)
  * `getJSSyntacticDiagnosticsForFile` **(Compute Cores)** (Impact: 163.9)
  * `actualResolveLibrary` **(Many-Argument Workhorses)** (Impact: 125.3)
  * `findSourceFileWorker` **(Many-Argument Workhorses)** (Impact: 119.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.019; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (56.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (47.0%), Connectivity (formerly Api Exposure) (7.7%), Complexity Load (formerly Cognitive Load) (6.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.8%), Guard Balance (formerly Safety Score) (80.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (71.9%)
- **Documentation Coverage:** 97.9003% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `verifyImportFixModuleSpecifiers` **(Many-Argument Workhorses)** (Impact: 518.7)
  * `getBaselineContentForFile` **(Many-Argument Workhorses)** (Impact: 219.5)
  * `parseFileContent` **(Many-Argument Workhorses)** (Impact: 112.5)
  * `verifyCompletionsWorker` **(Defensive Guards)** (Impact: 98.3)
  * `constructor` **(Many-Argument Workhorses)** (Impact: 84.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (77.1%), Connectivity (formerly Api Exposure) (35.3%)
- **Documentation Coverage:** 54.6269% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `forEachResolvedProjectReferenceProjectLoad` **(Many-Argument Workhorses)** (Impact: 118.6)
    * *Intent:* /** * Goes through parentConfig's project references and finds, creates or reloads project per kind ...
  * `findCreateOrReloadConfiguredProject` **(Many-Argument Workhorses)** (Impact: 111.0)
    * *Intent:* /** * Depending on kind * - Find the configuedProject and return it - if allowDeferredClosed is set ...
  * `tryFindDefaultConfiguredProjectForOpenScriptInfoOrClosedFileInfo` **(Many-Argument Workhorses)** (Impact: 106.1)
  * `onWildCardDirectoryWatcherInvoke` **(Many-Argument Workhorses)** (Impact: 72.7)
  * `applySafeListWorker` **(Many-Argument Workhorses)** (Impact: 63.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **44**; blast radius 0.019; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (96.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (64.3%), Connectivity (formerly Api Exposure) (59.8%)
- **Documentation Coverage:** 51.1278% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getLoadModuleFromTargetExportOrImport` **(Many-Argument Workhorses)** (Impact: 272.0)
    * *Intent:* /** * Gets the self-recursive function specialized to retrieving the targeted import/export element ...
  * `nodeModuleNameResolverWorker` **(Many-Argument Workhorses)** (Impact: 220.1)
  * `loadModuleFromTargetExportOrImport` **(Many-Argument Workhorses)** (Impact: 217.4)
  * `resolveTypeReferenceDirective` **(Many-Argument Workhorses)** (Impact: 187.5)
    * *Intent:* /** * This is possible in case if resolution is performed for directives specified via 'types' param...
  * `tryAddingExtensions` **(Compute Cores)** (Impact: 162.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (90.5%), Connectivity (formerly Api Exposure) (82.1%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (62.2%)
- **Documentation Coverage:** 16.3968% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getAdjustedLocation` **(Compute Cores)** (Impact: 191.8)
  * `isCompletedNode` **(Compute Cores)** (Impact: 129.7)
  * `getNodeKind` **(Compute Cores)** (Impact: 95.8)
  * `getTokenAtPositionWorker` **(Many-Argument Workhorses)** (Impact: 88.0)
    * *Intent:* /** Get the token whose text contains the position */
  * `createPackageJsonImportFilter` **(Many-Argument Workhorses)** (Impact: 77.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.019; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (56.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (47.0%), Connectivity (formerly Api Exposure) (7.6%), Complexity Load (formerly Cognitive Load) (6.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (100.0%), Mutation Surface (formerly State Flux) (99.0%), Guard Balance (formerly Safety Score) (75.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.019; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (56.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (47.0%), Connectivity (formerly Api Exposure) (7.2%), Complexity Load (formerly Cognitive Load) (6.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (95.8%), Guard Balance (formerly Safety Score) (84.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 90.6593% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `bindWorker` **(Compute Cores)** (Impact: 183.6)
  * `declareSymbol` **(Many-Argument Workhorses)** (Impact: 139.8)
    * *Intent:* /** * Declares a Symbol for the node and adds it to symbols. Reports errors for conflicting identifi...
  * `bindChildren` **(Type Conversions)** (Impact: 78.0)
  * `bindContainer` **(Many-Argument Workhorses)** (Impact: 75.5)
    * *Intent:* // All container nodes are kept on a linked list in declaration order. This list is used by // the g...
  * `getContainerFlags` **(Compute Cores)** (Impact: 65.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **8**; blast radius 0.053; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (92.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (70.6%), Connectivity (formerly Api Exposure) (67.8%)
- **Documentation Coverage:** 92.5414% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createLanguageService` **(Many-Argument Workhorses)** (Impact: 498.0)
  * `toggleMultilineComment` **(Many-Argument Workhorses)** (Impact: 77.6)
  * `visit` **(Compute Cores)** (Impact: 75.2)
  * `computeNamedDeclarations` **(I/O & Config Routines)** (Impact: 62.4)
  * `synchronizeHostDataWorker` **(I/O & Config Routines)** (Impact: 56.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 3.9e-05
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (89.6%), Concurrency Surface (formerly Concurrency) (64.1%), Complexity Load (formerly Cognitive Load) (38.3%), Connectivity (formerly Api Exposure) (11.0%)
- **Documentation Coverage:** 43.3754% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get` **(Defensive Guards)** (Impact: 55.8)
  * `generateDeclFile` **(Many-Argument Workhorses)** (Impact: 39.8)
    * *Intent:* /** Generates a .d.ts file for the given code */
  * `makeUnitsFromTest` **(Many-Argument Workhorses)** (Impact: 39.4)
  * `getTypeInfoName` **(Compute Cores)** (Impact: 39.2)
  * `makeUnitsFromTest` **(Many-Argument Workhorses)** (Impact: 39.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (82.0%), Complexity Load (formerly Cognitive Load) (25.2%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run` **(Compute Cores)** (Impact: 778.3)
  * `run` **(Compute Cores)** (Impact: 778.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (99.8%), Complexity Load (formerly Cognitive Load) (25.2%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `foo` **(I/O & Config Routines)** (Impact: 10.6)
    * *Intent:* //// [tests/cases/compiler/binaryArithmeticControlFlowGraphNotTooLarge.ts] //// //// [binaryArithmet...
  * `foo` **(I/O & Config Routines)** (Impact: 8.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.019; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Mutation Surface (formerly State Flux) (92.8%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (70.3%)
- **Documentation Coverage:** 51.7647% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseJsonConfigFileContentWorker` **(Many-Argument Workhorses)** (Impact: 168.8)
    * *Intent:* /** * Parse the contents of a config file from json or json source file (tsconfig.json). * file to. ...
  * `parseConfig` **(Many-Argument Workhorses)** (Impact: 103.5)
    * *Intent:* /** * This *just* extracts options/include/exclude/files out of a config file. * It does *not* resol...
  * `convertToJson` **(Many-Argument Workhorses)** (Impact: 91.7)
    * *Intent:* /** * Convert the json syntax tree into the json value and report errors * This returns the json val...
  * `parseOwnConfigOfJsonSourceFile` **(Many-Argument Workhorses)** (Impact: 84.7)
  * `parseOptionValue` **(Many-Argument Workhorses)** (Impact: 78.0)
**Contextual Mitigations & Amplifications:**
...
- `src/compiler/transformers/module/module.ts` -> **Severity: 0.111** (Embedded: 0.0016 * Error Risk: 67.9301%)
- `src/compiler/path.ts` -> **Severity: 0.076** (Embedded: 0.0009 * Error Risk: 82.8053%)
- `scripts/eslint/rules/utils.cjs` -> **Severity: 0.051** (Embedded: 0.0008 * Error Risk: 64.1725%)
- `src/compiler/core.ts` -> **Severity: 0.051** (Embedded: 0.0006 * Error Risk: 87.3385%)
- `src/compiler/transformers/jsx.ts` -> **Severity: 0.028** (Embedded: 0.0004 * Error Risk: 67.8346%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `src/compiler/transformers/module/module.ts` -> **Severity: 43.516** (Blast Radius: 1.378 * Doc Risk: 31.5789%)
- `src/typescript/typescript.ts` -> **Severity: 35.3** (Blast Radius: 0.353 * Doc Risk: 100.0%)
- `tests/cases/projects/MultipleLevels/A/A.ts` -> **Severity: 32.0** (Blast Radius: 0.32 * Doc Risk: 100.0%)
- `src/compiler/transformers/jsx.ts` -> **Severity: 28.654** (Blast Radius: 0.356 * Doc Risk: 80.4878%)
- `src/testRunner/unittests/tsbuildWatch/reexport.ts` -> **Severity: 20.0** (Blast Radius: 0.2 * Doc Risk: 100.0%)
