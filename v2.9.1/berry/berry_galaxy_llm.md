...
## 1. EXECUTIVE SUMMARY
- **Scope:** 1131 analyzed artifact(s), 86539 LOC.
- **Load-bearing artifact:** `packages/plugin-essentials/sources/commands/entries/clipanion.ts` -- 82 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `packages/acceptance-tests/pkg-tests-specs/sources/pnp.test.js` -- pulls in 46 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `packages/acceptance-tests/pkg-tests-specs/sources/node-modules.test.ts` at magnitude 3211.04 (structural weight, not risk).
...
| Avg Path Length | 2.6003 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `4.464`
> **Composition Archetype:** `Hub-Coupled App` (z +4.46; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 51%, Declarative / Non-Code 18%, Large Core Modules (3) 6%, Callbacks & Closures Files 5%, Large Core Modules (2) 4%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.2 | 45.9 | 60.5 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 5.0 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 7.2 | 2.3 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 6.8 | 0.2 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 2.0 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 42.8 | 0.0 | 0.0 |
...
- `makeApi` **(Many-Argument Workhorses)** (@ `packages/yarnpkg-pnp/sources/loader/makeApi.ts`) -> Impact: **403.7** | LOC: 1007
- `makePathWrapper` **(Many-Argument Workhorses)** (@ `packages/yarnpkg-core/sources/scriptUtils.ts`) -> Impact: **356.0** | LOC: 770
- `load` **(Many-Argument Workhorses)** (@ `packages/yarnpkg-core/sources/Manifest.ts`) -> Impact: **314.2** | LOC: 465
- `addPackageToTree` **(Many-Argument Workhorses)** (@ `packages/yarnpkg-nm/sources/buildNodeModulesTree.ts`) -> Impact: **187.8** | LOC: 157
- `linkEverything` **(Compute Cores)** (@ `packages/yarnpkg-core/sources/Project.ts`) -> Impact: **183.3** | LOC: 526
- `resolve` **(Defensive Guards)** (@ `packages/acceptance-tests/pkg-tests-fixtures/packages/resolve-1.9.0/lib/async.js`) -> Impact: **166.6** | LOC: 212
- `fetchPackageFromCache` **(Many-Argument Workhorses)** (@ `packages/yarnpkg-core/sources/Cache.ts`) -> Impact: **165.8** | LOC: 277
- `persistNodeModules` **(Many-Argument Workhorses)** (@ `packages/plugin-nm/sources/NodeModulesLinker.ts`) -> Impact: **164.1** | LOC: 282
- `exportTo` **(Many-Argument Workhorses)** (@ `packages/yarnpkg-core/sources/Manifest.ts`) -> Impact: **154.6** | LOC: 217
- `resolvePeerDependenciesImpl` **(Many-Argument Workhorses)** (@ `packages/yarnpkg-core/sources/Project.ts`) -> Impact: **153.0** | LOC: 331
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 0.564; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.6%), Guard Balance (formerly Safety Score) (33.6%), Complexity Load (formerly Cognitive Load) (24.3%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **31**; blast radius 0.564; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (37.1%), Complexity Load (formerly Cognitive Load) (27.9%), Mutation Surface (formerly State Flux) (11.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **38**; blast radius 4.118; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (88.8%), Guard Balance (formerly Safety Score) (77.9%)
- **Documentation Coverage:** 97.6744% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `linkEverything` **(Compute Cores)** (Impact: 183.3)
  * `resolvePeerDependenciesImpl` **(Many-Argument Workhorses)** (Impact: 153.0)
  * `applyVirtualResolutionMutations` **(Many-Argument Workhorses)** (Impact: 145.4)
    * *Intent:* /** * This function is worth some documentation. It takes a set of packages, * traverses them all, a...
  * `linkPackage` **(Many-Argument Workhorses)** (Impact: 69.1)
  * `install` **(Defensive Guards)** (Impact: 66.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000646 | `Ripple Effect (Closeness):` 0.015915
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 0.564; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (43.4%), Guard Balance (formerly Safety Score) (21.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (10.2%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **11**; blast radius 0.803; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (95.4%), Guard Balance (formerly Safety Score) (82.7%)
- **Documentation Coverage:** 94.5946% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `persistNodeModules` **(Many-Argument Workhorses)** (Impact: 164.1)
  * `cleanNewDirs` **(Many-Argument Workhorses)** (Impact: 98.5)
  * `syncNodeWithDisk` **(Many-Argument Workhorses)** (Impact: 60.7)
  * `syncPreinstallStateWithDisk` **(Many-Argument Workhorses)** (Impact: 56.1)
    * *Intent:* /** * Synchronizes previous install state with the actual directories available on disk * * */
  * `removeDir` **(Many-Argument Workhorses)** (Impact: 46.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.000884
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **6**; blast radius 3.061; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (98.1%), Complexity Load (formerly Cognitive Load) (91.3%), Guard Balance (formerly Safety Score) (70.9%)
- **Documentation Coverage:** 99.177% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Many-Argument Workhorses)** (Impact: 45.2)
  * `resolveFilename` **(Many-Argument Workhorses)** (Impact: 45.1)
  * `readdirSync` **(Defensive Guards)** (Impact: 37.8)
  * `getFileSource` **(Stateful Encapsulated Methods)** (Impact: 24.2)
  * `statImpl` **(Many-Argument Workhorses)** (Impact: 21.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.5e-05 | `Ripple Effect (Closeness):` 0.005659
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (30.3%), Mutation Surface (formerly State Flux) (17.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 1350.22 | **LOC:** 2084 | **CtrlFlow:** 27.1% | **Authorship Centralization:** 50.0%
- **Blast Radius:** changing it is visible to **15** in-repo importer(s); it depends on **32**; blast radius 5.325; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.6%), Concurrency Surface (formerly Concurrency) (98.3%), Connectivity (formerly Api Exposure) (76.4%), Guard Balance (formerly Safety Score) (73.9%)
- **Documentation Coverage:** 88.75% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `find` **(Many-Argument Workhorses)** (Impact: 120.2)
    * *Intent:* * * Note that some extra plugins will be automatically added based on the * content of the rc files ...
  * `use` **(Many-Argument Workhorses)** (Impact: 73.4)
  * `parseSingleValue` **(Many-Argument Workhorses)** (Impact: 71.7)
  * `getDefaultValue` **(Compute Cores)** (Impact: 70.5)
  * `normalizePackage` **(Compute Cores)** (Impact: 48.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000648 | `Ripple Effect (Closeness):` 0.019743
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.564; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (45.9%), Guard Balance (formerly Safety Score) (38.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.6%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.564; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (81.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_10` (Drift: 0.0 IQR)
- **Magnitude:** 1256.49 | **LOC:** 375 | **CtrlFlow:** 5.0% | **Authorship Centralization:** 0.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (75.4%), Guard Balance (formerly Safety Score) (52.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (74.6%), Guard Balance (formerly Safety Score) (31.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **8**; blast radius 2.151; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (97.8%), Guard Balance (formerly Safety Score) (97.5%), Concurrency Surface (formerly Concurrency) (84.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `load` **(Many-Argument Workhorses)** (Impact: 314.2)
  * `exportTo` **(Many-Argument Workhorses)** (Impact: 154.6)
  * `setRawField` **(Many-Argument Workhorses)** (Impact: 17.4)
  * `ensureDependencyMeta` **(Compute Cores)** (Impact: 10.7)
  * `getForScope` **(Compute Cores)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.4e-05 | `Ripple Effect (Closeness):` 0.016247
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (89.9%), Guard Balance (formerly Safety Score) (48.8%), Mutation Surface (formerly State Flux) (17.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 5 instances
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **6**; blast radius 0.59; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (92.6%), Mutation Surface (formerly State Flux) (88.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `limitOpenFiles` **(Stateful Encapsulated Methods)** (Impact: 28.5)
  * `writePromise` **(Many-Argument Workhorses)** (Impact: 25.5)
  * `writeSync` **(Many-Argument Workhorses)** (Impact: 25.5)
  * `fallback` **(Defensive Guards)** (Impact: 15.1)
  * `findMount` **(Stateful Encapsulated Methods)** (Impact: 14.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
...
- **Blast Radius:** changing it is visible to **18** in-repo importer(s); it depends on **6**; blast radius 4.678; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (95.2%), Connectivity (formerly Api Exposure) (95.1%), Complexity Load (formerly Cognitive Load) (92.3%)
- **Documentation Coverage:** 96.2264% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `copySync` **(Many-Argument Workhorses)** (Impact: 33.6)
  * `mkdirpPromise` **(Defensive Guards)** (Impact: 24.5)
  * `mkdirpSync` **(Defensive Guards)** (Impact: 24.5)
  * `removePromise` **(Defensive Guards)** (Impact: 24.4)
  * `lockPromise` **(Defensive Guards)** (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.1e-05 | `Ripple Effect (Closeness):` 0.016976
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (18.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **9**; blast radius 1.167; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.7%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (67.5%), Complexity Load (formerly Cognitive Load) (56.6%)
- **Documentation Coverage:** 55.3191% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `makeApi` **(Many-Argument Workhorses)** (Impact: 403.7)
  * `resolveToUnqualified` **(Many-Argument Workhorses)** (Impact: 146.8)
    * *Intent:* /** * Transforms a request (what's typically passed as argument to the require function) into an unq...
  * `applyNodeExtensionResolution` **(Many-Argument Workhorses)** (Impact: 35.2)
    * *Intent:* /** * Implements the node resolution for folder access and extension selection */
  * `resolveUnqualified` **(Defensive Guards)** (Impact: 28.4)
    * *Intent:* /** * Transforms an unqualified path into a qualified path by using the Node resolution algorithm (w...
  * `trace` **(Defensive Guards)** (Impact: 25.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.6e-05 | `Ripple Effect (Closeness):` 0.003158
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 0.564; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.9%), Complexity Load (formerly Cognitive Load) (91.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 93.0233% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `evaluateVariable` **(Many-Argument Workhorses)** (Impact: 95.4)
  * `interpolateArguments` **(Many-Argument Workhorses)** (Impact: 55.7)
  * `redirect` **(Many-Argument Workhorses)** (Impact: 52.6)
  * `executeCommandChainImpl` **(Many-Argument Workhorses)** (Impact: 42.5)
  * `executeCommandLine` **(Many-Argument Workhorses)** (Impact: 36.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (36.6%), Complexity Load (formerly Cognitive Load) (24.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (95.7%), Guard Balance (formerly Safety Score) (51.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (45.5%), Guard Balance (formerly Safety Score) (35.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `onentry` **(Annotated & Test Methods)** (Impact: 17.8)
  * `onentry` **(Callbacks & Closures)** (Impact: 2.7)
  * `onentry` **(Callbacks & Closures)** (Impact: 2.5)
  * `genPackList` **(Callbacks & Closures)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (47.4%), Guard Balance (formerly Safety Score) (35.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.6%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.564; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (95.9%), Guard Balance (formerly Safety Score) (58.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **21**; blast radius 0.564; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (38.6%), Complexity Load (formerly Cognitive Load) (24.5%), Mutation Surface (formerly State Flux) (12.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 10 instances
* *Amplified Cascading Flux:* 2 instances
...
- `packages/plugin-essentials/sources/commands/install.ts` -> Churn: **71.24%** | Cog Load: 94.8966% | Debt: 12.7684%
- `packages/plugin-npm/sources/npmHttpUtils.ts` -> Churn: **71.24%** | Cog Load: 88.5312% | Debt: 0.0%
- `packages/acceptance-tests/pkg-tests-specs/sources/commands/install.test.ts` -> Churn: **56.46%** | Cog Load: 100.0% | Debt: 0.0%
- `packages/plugin-git/sources/gitUtils.ts` -> Churn: **56.46%** | Cog Load: 71.9759% | Debt: 0.0%
- `packages/yarnpkg-core/sources/Configuration.ts` -> Churn: **56.46%** | Cog Load: 55.2593% | Debt: 8.8943%
...
- `packages/yarnpkg-pnp/sources/loader/makeApi.ts` -> **Clement Yan** (100.0% isolated ownership) | Magnitude: 1011.64
...
- `packages/yarnpkg-core/sources/Project.ts` -> **Severity: 0.065** (Bridge: 0.0006 * Flux: 99.9591%)
- `packages/yarnpkg-core/sources/Configuration.ts` -> **Severity: 0.064** (Bridge: 0.0006 * Flux: 98.552%)
- `packages/plugin-essentials/sources/commands/entries/clipanion.ts` -> **Severity: 0.032** (Bridge: 0.0004 * Flux: 91.6827%)
- `packages/yarnpkg-core/sources/miscUtils.ts` -> **Severity: 0.023** (Bridge: 0.0002 * Flux: 99.9294%)
...
- `packages/acceptance-tests/pkg-tests-core/sources/utils/fs.ts` -> **Severity: 3.314** (Embedded: 0.0437 * Error Risk: 75.9114%)
- `packages/yarnpkg-cli/sources/tools/getPluginConfiguration.ts` -> **Severity: 3.136** (Embedded: 0.0472 * Error Risk: 66.5013%)
- `packages/yarnpkg-cli/sources/lib.ts` -> **Severity: 3.08** (Embedded: 0.0471 * Error Risk: 65.4539%)
