...
## 1. EXECUTIVE SUMMARY
- **Scope:** 13 analyzed artifact(s), 358 LOC.
- **Load-bearing artifact:** `package/src/chartJSNodeCanvasBase.ts` -- 3 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `package/README.md` -- pulls in 9 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `package/src/chartJSNodeCanvasBase.ts` at magnitude 103.28 (structural weight, not risk).
...
> **File Composition:** Data / Markup / Trivial 54%, Callbacks & Closures Files 15%, Compute Cores Files 8%, Encapsulated Accessors Files 8%, Large Core Modules (2) 8%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 98.0 | 47.5 | 59.5 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 50.0 | 69.2 | 100.0 |
...
- `initialize` **(Defensive Guards)** (@ `package/src/chartJSNodeCanvasBase.ts`) -> Impact: **29.5** | LOC: 52
- `renderChart` **(Stateful Encapsulated Methods)** (@ `package/src/animatedChartJSNodeCanvas.ts`) -> Impact: **22.1** | LOC: 41
- `renderToStream` **(Defensive Guards)** (@ `package/src/chartJSNodeCanvas.ts`) -> Impact: **13.1** | LOC: 19
  * *Intent:* /** * Render to a stream. * */
- `constructor` **(Defensive Guards)** (@ `package/src/chartJSNodeCanvasBase.ts`) -> Impact: **12.4** | LOC: 21
  * *Intent:* /** * Create a new instance of CanvasRenderService. * */
- `onComplete` **(Compute Cores)** (@ `package/src/animatedChartJSNodeCanvas.ts`) -> Impact: **8.1** | LOC: 20
- `renderToDataURL` **(Callbacks & Closures)** (@ `package/src/chartJSNodeCanvas.ts`) -> Impact: **6.0** | LOC: 17
  * *Intent:* /** * Render to a data url. * */
- `renderToBuffer` **(Generic / Templated Code)** (@ `package/src/chartJSNodeCanvas.ts`) -> Impact: **6.0** | LOC: 17
  * *Intent:* /** * Render to a buffer. * */
- `toBuffer` **(Compute Cores)** (@ `package/src/chartJSNodeCanvasBase.ts`) -> Impact: **6.0** | LOC: 1
  * *Intent:* // https://github.com/Automattic/node-canvas#non-standard-apis
- `registerFont` **(Compute Cores)** (@ `package/src/chartJSNodeCanvasBase.ts`) -> Impact: **5.4** | LOC: 4
...
- `toBuffer` **(Compute Cores)** (@ `package/src/chartJSNodeCanvasBase.ts`) -> Impact: **5.2** | LOC: 1
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **6**; blast radius 174.763; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (86.2%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (79.2%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 69.2308% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `initialize` **(Defensive Guards)** (Impact: 29.5)
  * `constructor` **(Defensive Guards)** (Impact: 12.4)
    * *Intent:* /** * Create a new instance of CanvasRenderService. * */
  * `toBuffer` **(Compute Cores)** (Impact: 6.0)
    * *Intent:* // https://github.com/Automattic/node-canvas#non-standard-apis
  * `registerFont` **(Compute Cores)** (Impact: 5.4)
    * *Intent:* /** * Use to register the font with Canvas to use a font file that is not installed as a system font...
  * `toBuffer` **(Compute Cores)** (Impact: 5.2)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 64.727; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.0%), Complexity Load (formerly Cognitive Load) (72.8%)
- **Documentation Coverage:** 71.4286% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderChart` **(Stateful Encapsulated Methods)** (Impact: 22.1)
  * `onComplete` **(Compute Cores)** (Impact: 8.1)
  * `onProgress` **(Callbacks & Closures)** (Impact: 4.8)
  * `renderToBuffer` **(Generic / Templated Code)** (Impact: 4.5)
    * *Intent:* /** * Render to a buffer. * */
  * `renderToDataURL` **(Callbacks & Closures)** (Impact: 4.4)
    * *Intent:* /** * Render to a data url array. * */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 64.727; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (85.0%), Concurrency Surface (formerly Concurrency) (82.8%), Guard Balance (formerly Safety Score) (69.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 9.0909% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderToStream` **(Defensive Guards)** (Impact: 13.1)
    * *Intent:* /** * Render to a stream. * */
  * `renderToDataURL` **(Callbacks & Closures)** (Impact: 6.0)
    * *Intent:* /** * Render to a data url. * */
  * `renderToBuffer` **(Generic / Templated Code)** (Impact: 6.0)
    * *Intent:* /** * Render to a buffer. * */
  * `renderChart` **(Stateful Encapsulated Methods)** (Impact: 4.9)
  * `renderToDataURLSync` **(Defensive Guards)** (Impact: 4.2)
    * *Intent:* /** * Render to a data url synchronously. * */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **5**; blast radius 59.011; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (86.5%), Guard Balance (formerly Safety Score) (59.5%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Mutation Surface (formerly State Flux) (41.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main` **(I/O & Config Routines)** (Impact: 4.4)
  * `chartCallback` **(Callbacks & Closures)** (Impact: 2.7)
  * `beforeDraw` **(Callbacks & Closures)** (Impact: 1.9)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 50.437; role: Pure Consumer (Orchestrator)
...
- **Top Surface Vectors:** Dead Code Surface (formerly Dead Code) (72.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Test Surface (formerly Verification) (2.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **1**; blast radius 133.284; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (58.5%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (28.5%), Test Surface (formerly Verification) (2.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Encapsulated Accessors)** (Impact: 2.2)
  * `beforeDraw` **(Parameter Forwarders)** (Impact: 1.9)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 50.437; role: Pure Consumer (Orchestrator)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 124.71; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (62.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (20.1%), Mutation Surface (formerly State Flux) (16.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `freshRequire` **(Callbacks & Closures)** (Impact: 1.9)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 59.011; role: Pure Producer (Foundation)
- **Top Surface Vectors:** None above 0%
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* None
* *Risk/State:* None
...
- `package/src/chartJSNodeCanvasBase.ts` -> **Severity: 19.794** (Embedded: 0.25 * Error Risk: 79.1746%)
...
- `package/src/animatedChartJSNodeCanvas.ts` -> **Severity: 8.164** (Embedded: 0.0833 * Error Risk: 97.9688%)
- `package/src/chartJSNodeCanvas.ts` -> **Severity: 5.822** (Embedded: 0.0833 * Error Risk: 69.8701%)
