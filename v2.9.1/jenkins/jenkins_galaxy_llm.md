...
## 1. EXECUTIVE SUMMARY
- **Scope:** 11977 analyzed artifact(s), 249769 LOC.
- **Load-bearing artifact:** `core/src/main/java/jenkins/model/Jenkins.java` -- 511 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `core/src/main/java/jenkins/model/Jenkins.java` -- pulls in 317 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `core/src/main/java/jenkins/model/Jenkins.java` at magnitude 2597.64 (structural weight, not risk).
...
|---|---|---|
| Modularity | 0.4033 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 3.4554 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `6.704`
> **Composition Archetype:** `Flat Modular Platform` (z +6.70; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 83%, Declarative / Non-Code 3%, Encapsulated Accessors Files 3%, Large Core Modules (3) 3%, Large Core Modules (2) 2%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 29.2 | 0.0 | 0.0 |
...
| connectivity | 18305 | 1697 | 3 | `core/src/main/java/jenkins/model/Jenkins.java` |
...
| dead_code | 6172 | 1170 | 0 | `test/src/test/java/jenkins/security/stapler/DoActionFilterTest.java` |
| credential | 49 | 23 | 0 | `test/src/test/resources/plugins/warnings-update-center-malformed.json` |
...
- `__global_context__` **(Compute Cores)** (@ `core/report-l10n.rb`) -> Impact: **496.2** | LOC: 363
- `createPluginSetupWizard` **(Compute Cores)** (@ `src/main/js/pluginSetupWizardGui.js`) -> Impact: **348.1** | LOC: 1419
  * *Intent:* // Setup the dialog, exported
- `printThrowable` **(Compute Cores)** (@ `core/src/test/java/hudson/FunctionsTest.java`) -> Impact: **210.3** | LOC: 226
- `Anonymous_Block_[Truncated]` **(Compute Cores)** (@ `core/report-l10n.rb`) -> Impact: **138.7** | LOC: 134
- `serveFile` **(Many-Argument Workhorses)** (@ `core/src/main/java/hudson/model/DirectoryBrowserSupport.java`) -> Impact: **118.6** | LOC: 217
- `_main` **(Compute Cores)** (@ `cli/src/main/java/hudson/cli/CLI.java`) -> Impact: **92.5** | LOC: 209
- `doXml` **(Many-Argument Workhorses)** (@ `core/src/main/java/hudson/model/Api.java`) -> Impact: **84.8** | LOC: 108
  * *Intent:* /** * Exposes the bean as XML. */
- `buildFormTree` **(Compute Cores)** (@ `core/src/main/resources/jenkins/formelementpath/form-element-path.js`) -> Impact: **80.1** | LOC: 188
  * *Intent:* // most of this is copied from hudson-behaviour.js
- `maintain` **(I/O & Config Routines)** (@ `core/src/main/java/hudson/model/Queue.java`) -> Impact: **78.8** | LOC: 237
  * *Intent:* /** * Queue maintenance. * * <p> * Move projects between {@link #waitingList}, {@link #blockedProjects}, {@link #buildables}, and {@link #pendings} * ...
- `verifyChecksums` **(Compute Cores)** (@ `core/src/main/java/hudson/model/UpdateCenter.java`) -> Impact: **78.7** | LOC: 53
...
- **Blast Radius:** changing it is visible to **511** in-repo importer(s); it depends on **317**; blast radius 14.763; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (94.6%), Guard Balance (formerly Safety Score) (81.5%), Concurrency Surface (formerly Concurrency) (53.4%)
- **Documentation Coverage:** 46.3768% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Jenkins` **(Many-Argument Workhorses)** (Impact: 50.0)
    * *Intent:* /** * @param pluginManager * If non-null, use existing plugin manager. create a new one. */
  * `loadTasks` **(I/O & Config Routines)** (Impact: 29.6)
  * `_cleanUpAwaitDisconnects` **(Defensive Guards)** (Impact: 24.1)
  * `refreshExtensions` **(Compute Cores)** (Impact: 22.9)
    * *Intent:* /** * Refresh {@link ExtensionList}s by adding all the newly discovered extensions. * * Exposed only...
  * `getItem` **(Defensive Guards)** (Impact: 22.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004371 | `Ripple Effect (Closeness):` 0.065775
...
- **Magnitude:** 1838.36 | **LOC:** 2761 | **CtrlFlow:** 20.4% | **Authorship Centralization:** 22.2%
- **Blast Radius:** isolated in the scanned graph -- no in-repo artifact imports it and it imports none
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.0%), Complexity Load (formerly Cognitive Load) (68.9%), Concurrency Surface (formerly Concurrency) (66.8%)
- **Documentation Coverage:** 72.1429% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `buildFormTree` **(Compute Cores)** (Impact: 62.2)
    * *Intent:* // // structured form submission handling // see https://www.jenkins.io/redirect/developer/structure...
  * `registerMinMaxValidator` **(Defensive Guards)** (Impact: 50.2)
    * *Intent:* /** * Add a validator for number fields which contains 'min', 'max' attribute */
  * `encode` **(Compute Cores)** (Impact: 33.7)
    * *Intent:* */ // eslint-disable-next-line no-unused-vars
  * `onchange` **(Defensive Guards)** (Impact: 32.8)
  * `registerValidator` **(Defensive Guards)** (Impact: 32.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **101** in-repo importer(s); it depends on **118**; blast radius 2.501; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (69.5%), Mutation Surface (formerly State Flux) (67.9%), Connectivity (formerly Api Exposure) (58.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (27.0%)
- **Documentation Coverage:** 50.9804% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `installIfNecessaryFrom` **(Many-Argument Workhorses)** (Impact: 67.8)
  * `invoke` **(Many-Argument Workhorses)** (Impact: 48.5)
  * `readFromTar` **(Many-Argument Workhorses)** (Impact: 47.8)
    * *Intent:* /** * Reads from a tar stream and stores obtained files to the base dir. * Supports large files &gt;...
  * `normalize` **(Compute Cores)** (Impact: 39.6)
    * *Intent:* /** * {@link File#getParent()} etc cannot handle ".." and "." in the path component very well, * so ...
  * `validateRelativePath` **(Many-Argument Workhorses)** (Impact: 31.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000244 | `Ripple Effect (Closeness):` 0.047564
...
- **Blast Radius:** changing it is visible to **136** in-repo importer(s); it depends on **167**; blast radius 5.219; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (97.2%), Guard Balance (formerly Safety Score) (91.4%), Concurrency Surface (formerly Concurrency) (54.9%)
- **Documentation Coverage:** 39.8458% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dumpThreadInfo` **(Compute Cores)** (Impact: 46.4)
    * *Intent:* // ThreadInfo.toString() truncates the stack trace by first 8, so needed my own version
  * `getPasswordValue` **(Defensive Guards)** (Impact: 43.4)
    * *Intent:* /** * Used by {@code <f:password/>} so that we send an encrypted value to the client. */
  * `doPrintStackTrace` **(Many-Argument Workhorses)** (Impact: 41.5)
  * `getRelativeNameFrom` **(Many-Argument Workhorses)** (Impact: 38.0)
    * *Intent:* /** * Gets the relative name or display name to the given item from the specified group. * * @since ...
  * `guessIcon` **(Compute Cores)** (Impact: 18.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000829 | `Ripple Effect (Closeness):` 0.049821
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **111**; blast radius 0.062; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (96.3%), Debt Markers (formerly Tech Debt) (90.7%), Guard Balance (formerly Safety Score) (81.5%), Connectivity (formerly Api Exposure) (74.8%)
- **Documentation Coverage:** 51.5068% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `verifyChecksums` **(Compute Cores)** (Impact: 78.7)
    * *Intent:* /** * Implements the checksum verification logic with fallback to weaker algorithm for {@link Downlo...
  * `download` **(Defensive Guards)** (Impact: 40.9)
    * *Intent:* /** * Download a plugin or core upgrade in preparation for installing it * into its final location. ...
  * `doConnectionStatusImpl` **(Stateful Encapsulated Methods)** (Impact: 27.9)
  * `createUpdateCenter` **(Defensive Guards)** (Impact: 27.1)
    * *Intent:* /** * Creates an update center. * @param config Requested configuration. May be {@code null} if defa...
  * `getBadge` **(Compute Cores)** (Impact: 21.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **14** in-repo importer(s); it depends on **154**; blast radius 0.551; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Guard Balance (formerly Safety Score) (84.4%), Connectivity (formerly Api Exposure) (68.8%), Complexity Load (formerly Cognitive Load) (42.7%)
- **Documentation Coverage:** 54.8117% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `install` **(Many-Argument Workhorses)** (Impact: 50.3)
  * `doPluginsSearch` **(Many-Argument Workhorses)** (Impact: 41.8)
  * `initTasks` **(Stateful Encapsulated Methods)** (Impact: 40.8)
    * *Intent:* /** * Called immediately after the construction. * This is a separate method so that code executed f...
  * `dynamicLoad` **(Many-Argument Workhorses)** (Impact: 35.5)
    * *Intent:* /** * Try the dynamicLoad, removeExisting to attempt to dynamic load disabled plugins */
  * `addDependencies` **(Many-Argument Workhorses)** (Impact: 26.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000182 | `Ripple Effect (Closeness):` 0.042347
...
- **Global Archetype:** `file_cluster_5` (Drift: 0.0 IQR)
- **Magnitude:** 1442.96 | **LOC:** 3253 | **CtrlFlow:** 22.2% | **Authorship Centralization:** 40.0%
- **Blast Radius:** changing it is visible to **46** in-repo importer(s); it depends on **115**; blast radius 0.881; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (90.3%), Guard Balance (formerly Safety Score) (83.0%), Connectivity (formerly Api Exposure) (79.6%), Complexity Load (formerly Cognitive Load) (30.7%)
- **Documentation Coverage:** 43.2796% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `maintain` **(I/O & Config Routines)** (Impact: 78.8)
    * *Intent:* /** * Queue maintenance. * * <p> * Move projects between {@link #waitingList}, {@link #blockedProjec...
  * `scheduleInternal` **(Many-Argument Workhorses)** (Impact: 39.3)
    * *Intent:* /** * Schedules an execution of a task. * * @since 1.311 * @return * {@link hudson.model.queue.Sched...
  * `makeFlyWeightTaskBuildable` **(Stateful Encapsulated Methods)** (Impact: 20.9)
    * *Intent:* /** * This method checks if the flyweight task can be run on any of the available executors * @param...
  * `getItems` **(Compute Cores)** (Impact: 19.6)
    * *Intent:* /** * Gets the information about the queue item for the given project. * * @return empty if the proj...
  * `getItem` **(Compute Cores)** (Impact: 19.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 6 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000289 | `Ripple Effect (Closeness):` 0.043294
...
- **Blast Radius:** changing it is visible to **108** in-repo importer(s); it depends on **125**; blast radius 2.082; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.2%), Connectivity (formerly Api Exposure) (92.6%), Guard Balance (formerly Safety Score) (78.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (45.0%)
- **Documentation Coverage:** 33.1343% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `addArtifacts` **(Many-Argument Workhorses)** (Impact: 52.2)
  * `getBuildStatusSummary` **(Compute Cores)** (Impact: 29.2)
    * *Intent:* /** * Gets an object which represents the single line summary of the status of this build * (especia...
  * `execute` **(Defensive Guards)** (Impact: 21.2)
  * `waitForCheckpoint` **(Defensive Guards)** (Impact: 17.0)
    * *Intent:* /** * @see CheckPoint#block() */
  * `getLog` **(Defensive Guards)** (Impact: 14.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000364 | `Ripple Effect (Closeness):` 0.039436
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **21**; blast radius 0.115; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (76.2%), Complexity Load (formerly Cognitive Load) (69.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createPluginSetupWizard` **(Compute Cores)** (Impact: 348.1)
    * *Intent:* // Setup the dialog, exported
  * `setPanel` **(Many-Argument Workhorses)** (Impact: 48.0)
    * *Intent:* // call this to set the panel in the app, this performs some additional things & adds common transit...
  * `showInstallProgress` **(I/O & Config Routines)** (Impact: 46.8)
    * *Intent:* // Define actions
  * `updateStatus` **(I/O & Config Routines)** (Impact: 36.5)
    * *Intent:* // call to the installStatus, update progress bar & plugin details; transition on complete
  * `showInitialSetupWizard` **(I/O & Config Routines)** (Impact: 23.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 8.3e-05
...
- **Blast Radius:** changing it is visible to **74** in-repo importer(s); it depends on **98**; blast radius 1.324; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.9%), Connectivity (formerly Api Exposure) (93.0%), Guard Balance (formerly Safety Score) (91.8%), Complexity Load (formerly Cognitive Load) (29.0%)
- **Documentation Coverage:** 43.2526% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `submitImpl` **(Many-Argument Workhorses)** (Impact: 36.1)
  * `_poll` **(Many-Argument Workhorses)** (Impact: 31.4)
    * *Intent:* /** * {@link #poll(TaskListener)} method without the try/catch block that does listener notification...
  * `poll` **(Defensive Guards)** (Impact: 18.6)
    * *Intent:* /** * Checks if there's any update in SCM, and returns true if any is found. * * <p> * The implement...
  * `onLoad` **(Defensive Guards)** (Impact: 15.0)
  * `workspaceOffline` **(Stateful Encapsulated Methods)** (Impact: 13.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000391 | `Ripple Effect (Closeness):` 0.045735
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **51**; blast radius 0.258; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (86.2%), Guard Balance (formerly Safety Score) (70.8%), Concurrency Surface (formerly Concurrency) (60.9%)
- **Documentation Coverage:** 70.5584% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse` **(I/O & Config Routines)** (Impact: 20.5)
  * `get` **(Defensive Guards)** (Impact: 20.3)
    * *Intent:* /* package */ static volatile Boolean vetoersExist; /** * Gets the {@link ProcessTree} of the curren...
  * `FreeBSD` **(Compute Cores)** (Impact: 17.6)
  * `AIXProcess` **(I/O & Config Routines)** (Impact: 16.7)
  * `Darwin` **(Defensive Guards)** (Impact: 16.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.7e-05 | `Ripple Effect (Closeness):` 0.034971
...
- **Blast Radius:** changing it is visible to **61** in-repo importer(s); it depends on **114**; blast radius 0.979; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.4%), Connectivity (formerly Api Exposure) (96.9%), Guard Balance (formerly Safety Score) (89.4%), Concurrency Surface (formerly Concurrency) (49.7%)
- **Documentation Coverage:** 45.4167% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `doRssChangelog` **(Compute Cores)** (Impact: 33.1)
    * *Intent:* /** * RSS feed for changes in this project. * * @since 2.60 */
  * `onLoad` **(Defensive Guards)** (Impact: 26.2)
  * `getBuildStabilityHealthReport` **(I/O & Config Routines)** (Impact: 26.1)
  * `onLocationChanged` **(Defensive Guards)** (Impact: 15.4)
  * `getBuildHealthReports` **(I/O & Config Routines)** (Impact: 15.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00039 | `Ripple Effect (Closeness):` 0.044879
...
- **Blast Radius:** changing it is visible to **217** in-repo importer(s); it depends on **102**; blast radius 9.756; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.7%), Connectivity (formerly Api Exposure) (96.7%), Guard Balance (formerly Safety Score) (40.7%), Complexity Load (formerly Cognitive Load) (29.1%)
- **Documentation Coverage:** 17.9487% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getMethod` **(Many-Argument Workhorses)** (Impact: 38.0)
  * `escape` **(Compute Cores)** (Impact: 31.5)
    * *Intent:* /** * Escapes HTML unsafe characters like &lt;, &amp; to the respective character entities. */
  * `encode` **(Many-Argument Workhorses)** (Impact: 30.6)
  * `createDirectories` **(Defensive Guards)** (Impact: 26.5)
    * *Intent:* * * <p>If this method fails, * then it may do so after creating some, but not all, of the parent dir...
  * `getTimeSpanString` **(Compute Cores)** (Impact: 25.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000342 | `Ripple Effect (Closeness):` 0.056024
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **66**; blast radius 0.245; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.1%), Guard Balance (formerly Safety Score) (92.3%), Connectivity (formerly Api Exposure) (83.7%), Complexity Load (formerly Cognitive Load) (34.9%)
- **Documentation Coverage:** 31.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Plugin` **(Defensive Guards)** (Impact: 35.8)
  * `deploy` **(Many-Argument Workhorses)** (Impact: 33.0)
    * *Intent:* * <p> * This is mainly intended to be called from the UI. The actual installation work happens * asy...
  * `Data` **(Defensive Guards)** (Impact: 31.8)
  * `getNeededDependencies` **(I/O & Config Routines)** (Impact: 21.4)
    * *Intent:* /** * Returns a list of dependent plugins which need to be installed or upgraded for this plugin to ...
  * `isRelevant` **(Compute Cores)** (Impact: 13.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.1e-05 | `Ripple Effect (Closeness):` 0.032202
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **46**; blast radius 0.274; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.4%), Guard Balance (formerly Safety Score) (82.6%), Connectivity (formerly Api Exposure) (75.2%), Complexity Load (formerly Cognitive Load) (37.3%)
- **Documentation Coverage:** 45.1777% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fromString` **(Many-Argument Workhorses)** (Impact: 40.7)
    * *Intent:* /** * Parses a {@link RangeSet} from a string like "1-3,5,7-9" */
  * `locationChanged` **(Stateful Encapsulated Methods)** (Impact: 18.9)
  * `canDiscoverItem` **(Defensive Guards)** (Impact: 16.2)
    * *Intent:* /** * Checks if the current user can Discover the item. * If yes, it may be displayed as a text in F...
  * `removeAll` **(Compute Cores)** (Impact: 15.9)
    * *Intent:* /** * Updates this range set by removing all the values in the given range set. * * @return true if ...
  * `trim` **(I/O & Config Routines)** (Impact: 13.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5.2e-05 | `Ripple Effect (Closeness):` 0.042147
...
- **Blast Radius:** changing it is visible to **103** in-repo importer(s); it depends on **115**; blast radius 2.224; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (84.6%), Guard Balance (formerly Safety Score) (84.3%), Concurrency Surface (formerly Concurrency) (61.8%)
- **Documentation Coverage:** 27.0386% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `doRssLatest` **(Generic / Templated Code)** (Impact: 19.8)
    * *Intent:* /** * Retrieve the RSS feed for the last build for each project executed in this computer. * Only th...
  * `doConfigSubmit` **(Compute Cores)** (Impact: 15.8)
    * *Intent:* /** * Accepts the update to the node configuration. */
  * `doConfigDotXml` **(Compute Cores)** (Impact: 12.0)
    * *Intent:* /** * Accepts {@code config.xml} submission, as well as serve it. */
  * `buildEnvironment` **(Defensive Guards)** (Impact: 11.0)
    * *Intent:* /** * Creates an environment variable override to be used for launching processes on this node. * * ...
  * `relocateOldLogs` **(Defensive Guards)** (Impact: 11.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000276 | `Ripple Effect (Closeness):` 0.047145
...
- **Blast Radius:** changing it is visible to **73** in-repo importer(s); it depends on **69**; blast radius 0.911; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.4%), Guard Balance (formerly Safety Score) (89.2%), Connectivity (formerly Api Exposure) (75.2%), Concurrency Surface (formerly Concurrency) (31.9%)
- **Documentation Coverage:** 38.6207% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run` **(Defensive Guards)** (Impact: 28.1)
  * `reportError` **(Stateful Encapsulated Methods)** (Impact: 21.2)
  * `getWhyKeepLog` **(Compute Cores)** (Impact: 20.4)
    * *Intent:* // // // fingerprint related stuff // //
  * `createLauncher` **(Stateful Encapsulated Methods)** (Impact: 20.0)
    * *Intent:* /** * Creates a {@link Launcher} that this build will use. This can be overridden by derived types *...
  * `performAllBuildSteps` **(Stateful Encapsulated Methods)** (Impact: 16.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000131 | `Ripple Effect (Closeness):` 0.036697
...
- **Blast Radius:** changing it is visible to **130** in-repo importer(s); it depends on **74**; blast radius 3.669; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.7%), Guard Balance (formerly Safety Score) (90.2%), Connectivity (formerly Api Exposure) (85.8%), Complexity Load (formerly Cognitive Load) (24.7%)
- **Documentation Coverage:** 26.4151% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `newInstancesFromHeteroList` **(Many-Argument Workhorses)** (Impact: 26.2)
    * *Intent:* /** * @since 2.475 */
  * `getHelpFile` **(Defensive Guards)** (Impact: 18.6)
  * `getStaticHelpUrl` **(Generic / Templated Code)** (Impact: 17.2)
    * *Intent:* /** * @since 2.475 */
  * `doHelpImpl` **(Stateful Encapsulated Methods)** (Impact: 15.6)
  * `buildFillDependencies` **(Stateful Encapsulated Methods)** (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00015 | `Ripple Effect (Closeness):` 0.047186
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.062; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (56.8%), Complexity Load (formerly Cognitive Load) (33.6%), Mutation Surface (formerly State Flux) (31.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__global_context__` **(Compute Cores)** (Impact: 496.2)
  * `Anonymous_Block_[Truncated]` **(Compute Cores)** (Impact: 138.7)
  * `Anonymous_Block` **(I/O & Config Routines)** (Impact: 7.7)
  * `Anonymous_Block` **(I/O & Config Routines)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_5` (Drift: 0.0 IQR)
- **Magnitude:** 622.78 | **LOC:** 1562 | **CtrlFlow:** 5.7% | **Authorship Centralization:** 25.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **114**; blast radius 0.062; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (98.6%), Connectivity (formerly Api Exposure) (58.7%), Guard Balance (formerly Safety Score) (55.2%), Complexity Load (formerly Cognitive Load) (52.0%)
- **Documentation Coverage:** 92.1348% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `foldableCauseAction` **(Annotated & Test Methods)** (Impact: 16.6)
  * `pendingsConsistenceAfterErrorDuringMaintain` **(Annotated & Test Methods)** (Impact: 15.9)
  * `queueApiOutputShouldBeFilteredByUserPermission` **(I/O & Config Routines)** (Impact: 12.2)
  * `waitUntilWaitingListIsEmpty` **(Stateful Encapsulated Methods)** (Impact: 7.7)
  * `flyweightTasks` **(Annotated & Test Methods)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **87** in-repo importer(s); it depends on **41**; blast radius 1.276; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (96.3%), Guard Balance (formerly Safety Score) (64.8%), Mutation Surface (formerly State Flux) (56.8%), Debt Markers (formerly Tech Debt) (40.9%)
- **Documentation Coverage:** 50.4132% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `printCommandLine` **(Stateful Encapsulated Methods)** (Impact: 18.5)
    * *Intent:* /** * Prints out the command line to the listener so that users know what we are doing. */
  * `launch` **(Compute Cores)** (Impact: 15.4)
  * `launch` **(Defensive Guards)** (Impact: 14.3)
  * `call` **(Annotated & Test Methods)** (Impact: 12.9)
  * `maskedPrintCommandLine` **(Stateful Encapsulated Methods)** (Impact: 10.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000125 | `Ripple Effect (Closeness):` 0.045866
...
- **Blast Radius:** changing it is visible to **39** in-repo importer(s); it depends on **97**; blast radius 1.045; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (91.1%), Connectivity (formerly Api Exposure) (91.1%), Guard Balance (formerly Safety Score) (77.1%), Complexity Load (formerly Cognitive Load) (21.6%)
- **Documentation Coverage:** 31.694% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `doItemCategories` **(Many-Argument Workhorses)** (Impact: 26.9)
    * *Intent:* /** * An API REST method to get the allowed {$link TopLevelItem}s and its categories. * * @return A ...
  * `filterQueueItemTest` **(Stateful Encapsulated Methods)** (Impact: 20.6)
  * `create` **(Many-Argument Workhorses)** (Impact: 20.6)
    * *Intent:* /** * @since 2.475 */
  * `getComputers` **(Generic / Templated Code)** (Impact: 13.1)
  * `isRelevant` **(Stateful Encapsulated Methods)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000132 | `Ripple Effect (Closeness):` 0.044212
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 0.062; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (70.1%), Guard Balance (formerly Safety Score) (45.3%), Connectivity (formerly Api Exposure) (1.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dummyRangeRangeSingleShouldSuccess` **(I/O & Config Routines)** (Impact: 19.1)
  * `dummyRangeRangeSingleShouldFailIfBuildRangeContainsZeroAndNegative` **(I/O & Config Routines)** (Impact: 17.3)
  * `act` **(Stateful Encapsulated Methods)** (Impact: 14.9)
  * `dummyRangeRangeSingleShouldFailIfBuildRangeContainsANegativeNumber` **(I/O & Config Routines)** (Impact: 14.6)
  * `dummyRangeNumberSingleShouldSuccess` **(I/O & Config Routines)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **23** in-repo importer(s); it depends on **59**; blast radius 1.06; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (91.1%), Connectivity (formerly Api Exposure) (86.1%), Guard Balance (formerly Safety Score) (81.7%), Complexity Load (formerly Cognitive Load) (20.0%)
- **Documentation Coverage:** 47.8873% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolvePluginDependencies` **(I/O & Config Routines)** (Impact: 27.1)
    * *Intent:* /** * Makes sure that all the dependencies exist, and then accept optional dependencies * as real de...
  * `disable` **(Defensive Guards)** (Impact: 23.5)
    * *Intent:* /** * Disable this plugin using a strategy. * @param strategy strategy to use * @return an object re...
  * `PluginWrapper` **(Many-Argument Workhorses)** (Impact: 12.8)
    * *Intent:* /** * @param archive * A .jpi archive file jar file, or a .jpl linked plugin. * @param manifest * Th...
  * `Dependency` **(Compute Cores)** (Impact: 12.5)
  * `getVersionOf` **(Stateful Encapsulated Methods)** (Impact: 9.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.8e-05 | `Ripple Effect (Closeness):` 0.044175
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.062; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (84.0%), Guard Balance (formerly Safety Score) (46.6%), Connectivity (formerly Api Exposure) (0.4%)
- **Documentation Coverage:** 92.3077% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testValidator276` **(Compute Cores)** (Impact: 58.6)
  * `testValidator290` **(I/O & Config Routines)** (Impact: 35.9)
  * `main` **(Defensive Guards)** (Impact: 28.3)
    * *Intent:* /** * Validator for checking URL parsing * @param args - URLs to validate */
  * `testIsValid` **(Stateful Encapsulated Methods)** (Impact: 26.3)
    * *Intent:* /** * Create set of tests by taking the testUrlXXX arrays and * running through all possible permuta...
  * `testValidator353` **(Compute Cores)** (Impact: 25.6)
**Contextual Mitigations & Amplifications:**
...
- `core/src/main/java/jenkins/model/Jenkins.java` -> **Severity: 0.414** (Bridge: 0.0044 * Flux: 94.6069%)
- `core/src/main/java/hudson/Functions.java` -> **Severity: 0.081** (Bridge: 0.0008 * Flux: 97.1796%)
- `core/src/main/java/hudson/model/Job.java` -> **Severity: 0.039** (Bridge: 0.0004 * Flux: 99.4279%)
- `core/src/main/java/hudson/model/AbstractProject.java` -> **Severity: 0.038** (Bridge: 0.0004 * Flux: 97.8627%)
- `core/src/main/java/hudson/ExtensionList.java` -> **Severity: 0.037** (Bridge: 0.0004 * Flux: 99.186%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `core/src/main/java/jenkins/model/Jenkins.java` -> **Severity: 5.363** (Embedded: 0.0658 * Error Risk: 81.538%)
- `core/src/main/java/hudson/Functions.java` -> **Severity: 4.554** (Embedded: 0.0498 * Error Risk: 91.4058%)
- `core/src/main/java/hudson/model/Descriptor.java` -> **Severity: 4.255** (Embedded: 0.0472 * Error Risk: 90.1655%)
- `core/src/main/java/hudson/model/AbstractProject.java` -> **Severity: 4.2** (Embedded: 0.0457 * Error Risk: 91.8328%)
- `core/src/main/java/hudson/ExtensionList.java` -> **Severity: 4.165** (Embedded: 0.052 * Error Risk: 80.0936%)
