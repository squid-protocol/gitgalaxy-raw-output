...
## 1. EXECUTIVE SUMMARY
- **Scope:** 12756 analyzed artifact(s), 2296344 LOC.
- **Load-bearing artifact:** `pkg/analyzer_testing/lib/src/mock_packages/test_reflective_loader/test_reflective_loader.dart` -- 1528 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `pkg/analyzer/test/src/diagnostics/test_all.dart` -- pulls in 623 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `runtime/vm/object.cc` at magnitude 15284.98 (structural weight, not risk).
...
| Analyzed Artifacts (Scanned) | 12756 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 41306 |
| Total LOC | 2296344 |
...
|---|---|---|
| Modularity | 0.8023 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 4.9019 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `3.683`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +3.68; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 28%, State Mutators Files 17%, Large Core Modules (2) 16%, Large Core Modules (3) 9%, Callbacks & Closures Files 7%
...
*Total Excluded Artifacts: 41306*
...
- `.yaml`: 140x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 4x Zero-Density Threshold (LOC: 56, Signals: 0), 3x Zero-Density Threshold (LOC: 73, Signals: 0)
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 33.8 | 13.2 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 9.8 | 2.3 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 18.1 | 5.6 | 0.0 |
...
| connectivity | 113492 | 9860 | 16 | `benchmarks/SubtypeTestCache/dart/SubtypeTestCache.dart` |
...
| mutation | 345451 | 8428 | 58 | `runtime/vm/object.cc` |
| dead_code | 46528 | 11323 | 5 | `runtime/vm/object.cc` |
...
- `ConstantInstr::EmitMoveToLocation` **(Many-Argument Workhorses)** (@ `runtime/vm/compiler/backend/il_riscv.cc`) -> Impact: **726.2** | LOC: 1510
- `deserialize` **(Many-Argument Workhorses)** (@ `pkg/wasm_builder/lib/src/ir/instruction.dart`) -> Impact: **681.7** | LOC: 494
- `CallSpecializer::TryInlineRecognizedMethod` **(Many-Argument Workhorses)** (@ `runtime/vm/compiler/call_specializer.cc`) -> Impact: **612.7** | LOC: 356
- `ImageWriter::GetDataOffsetFor` **(Many-Argument Workhorses)** (@ `runtime/vm/image_snapshot.cc`) -> Impact: **587.2** | LOC: 1594
  * *Intent:* #if defined(SNAPSHOT_BACKTRACE)
- `parseMethod` **(Many-Argument Workhorses)** (@ `pkg/_fe_analyzer_shared/lib/src/parser/parser_impl.dart`) -> Impact: **546.9** | LOC: 383
- `resolveAndBuildConstructorInvocation` **(Many-Argument Workhorses)** (@ `pkg/front_end/lib/src/kernel/body_builder.dart`) -> Impact: **546.2** | LOC: 541
- `_process` **(Many-Argument Workhorses)** (@ `pkg/front_end/tool/coverage_merger.dart`) -> Impact: **501.1** | LOC: 471
- `Simulator::DecodeSIMDDataProcessing` **(Compute Cores)** (@ `runtime/vm/simulator_arm.cc`) -> Impact: **498.6** | LOC: 469
- `Interpreter::Run` **(Many-Argument Workhorses)** (@ `runtime/vm/interpreter.cc`) -> Impact: **451.1** | LOC: 1902
- `RegExpDisjunction::RationalizeConsecutiveAtoms` **(Many-Argument Workhorses)** (@ `runtime/vm/regexp/regexp-compiler-tonode.cc`) -> Impact: **450.7** | LOC: 1377
...
| `runtime/vm` | 375 | 110134.05 | 26.53% | 48.94% |
...
| `runtime/bin` | 211 | 26550.9 | 26.75% | 48.59% |
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **76**; blast radius 0.037; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.7%), Mutation Surface (formerly State Flux) (80.2%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (73.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Class::IsSubtypeOf` **(Many-Argument Workhorses)** (Impact: 110.5)
    * *Intent:* // Checks if type T0 is a subtype of type T1. // Type T0 is specified by class 'cls' parameterized w...
  * `Function::RecognizedKindForceOptimize` **(Compute Cores)** (Impact: 100.7)
  * `VerifyEntryPoint` **(Many-Argument Workhorses)** (Impact: 100.3)
    * *Intent:* #endif
  * `Class::GenerateUserVisibleName` **(Compute Cores)** (Impact: 97.2)
    * *Intent:* #endif // !defined(PRODUCT) || defined(FORCE_INCLUDE_SAMPLING_HEAP_PROFILER)
  * `AbstractType::IsSubtypeOf` **(Many-Argument Workhorses)** (Impact: 86.6)
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **27**; blast radius 0.179; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.4%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (71.2%)
- **Documentation Coverage:** 36.6224% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseMethod` **(Many-Argument Workhorses)** (Impact: 546.9)
  * `parseClassOrMixinOrExtensionOrEnumMemberImpl` **(Many-Argument Workhorses)** (Impact: 267.2)
    * *Intent:* /// constructorDeclaration | /// methodDeclaration /// ; /// /// mixinMember: /// fieldDeclaration |...
  * `_parsePrecedenceExpressionLoop` **(Many-Argument Workhorses)** (Impact: 256.2)
  * `parseFormalParameter` **(Many-Argument Workhorses)** (Impact: 235.4)
    * *Intent:* /// ; /// /// functionFormalParameter: /// metadata 'covariant'? returnType? identifier formalParame...
  * `parseTopLevelKeywordDeclaration` **(Many-Argument Workhorses)** (Impact: 224.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5.2e-05 | `Ripple Effect (Closeness):` 0.005278
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **43**; blast radius 0.106; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Mutation Surface (formerly State Flux) (99.8%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (66.8%)
- **Documentation Coverage:** 95.3608% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `InferenceVisitor` **(I/O & Config Routines)** (Impact: 277.6)
  * `_computePropertySet` **(Many-Argument Workhorses)** (Impact: 146.1)
    * *Intent:* /// /// [fileOffset] is used as the file offset for created nodes. [propertyName] /// is used for er...
  * `_inferSpreadMapEntry` **(Many-Argument Workhorses)** (Impact: 142.7)
  * `_computeIndexSet` **(Many-Argument Workhorses)** (Impact: 136.3)
    * *Intent:* /// Creates an index set operation of [writeTarget] on [receiver] using /// [index] and [value] as t...
  * `_computeIndexGet` **(Many-Argument Workhorses)** (Impact: 131.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5.3e-05 | `Ripple Effect (Closeness):` 0.00687
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **78**; blast radius 0.115; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (94.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (91.2%), Connectivity (formerly Api Exposure) (72.7%), Guard Balance (formerly Safety Score) (41.4%)
- **Documentation Coverage:** 92.3634% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolveAndBuildConstructorInvocation` **(Many-Argument Workhorses)** (Impact: 546.2)
  * `processLookupResult` **(Many-Argument Workhorses)** (Impact: 318.2)
  * `endFormalParameter` **(Many-Argument Workhorses)** (Impact: 208.4)
  * `endSwitchCase` **(Many-Argument Workhorses)** (Impact: 168.6)
  * `beginSwitchCase` **(Many-Argument Workhorses)** (Impact: 129.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4e-05 | `Ripple Effect (Closeness):` 0.00654
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **12**; blast radius 0.169; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.4%), Connectivity (formerly Api Exposure) (91.5%), Concurrency Surface (formerly Concurrency) (64.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `beginClassDeclaration` **(Many-Argument Workhorses)** (Impact: 31.2)
  * `beginNamedMixinApplication` **(Many-Argument Workhorses)** (Impact: 31.2)
  * `beginConstructor` **(Many-Argument Workhorses)** (Impact: 31.2)
  * `endFields` **(Many-Argument Workhorses)** (Impact: 29.2)
  * `beginMethod` **(Many-Argument Workhorses)** (Impact: 26.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.4e-05 | `Ripple Effect (Closeness):` 0.000701
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **32**; blast radius 0.037; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.7%), Debt Markers (formerly Tech Debt) (88.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (77.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Serializer::NewClusterForClass` **(Many-Argument Workhorses)** (Impact: 151.3)
  * `Deserializer::ReadCluster` **(Compute Cores)** (Impact: 88.5)
  * `WriteFill` **(Many-Argument Workhorses)** (Impact: 72.6)
  * `Serializer::CreateArtificialNodeIfNeeded` **(Compute Cores)** (Impact: 59.1)
    * *Intent:* #if !defined(DART_PRECOMPILED_RUNTIME)
  * `Serializer::PrepareInstructions` **(Compute Cores)** (Impact: 57.3)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **40**; blast radius 0.06; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (58.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (43.8%)
- **Documentation Coverage:** 68.1467% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ExpressionVisitor` **(Stateful Encapsulated Methods)** (Impact: 309.6)
  * `visitStaticInvocation` **(Compute Cores)** (Impact: 143.5)
  * `_emitClassSignature` **(Many-Argument Workhorses)** (Impact: 92.3)
    * *Intent:* /// Emit the signature on the class recording the runtime type information
  * `_defineClass` **(Many-Argument Workhorses)** (Impact: 87.1)
    * *Intent:* /// Emits code required to represent [c] as a series of statements in [body]. /// /// [properties] h...
  * `_emitBinaryOperator` **(Many-Argument Workhorses)** (Impact: 70.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.000318
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **37**; blast radius 0.056; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.3%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (59.9%), Concurrency Surface (formerly Concurrency) (37.9%)
- **Documentation Coverage:** 69.4444% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Compiler` **(Stateful Encapsulated Methods)** (Impact: 313.9)
  * `visitStaticInvocation` **(Compute Cores)** (Impact: 133.9)
  * `_defineClass` **(Many-Argument Workhorses)** (Impact: 114.7)
  * `_emitClassSignature` **(Many-Argument Workhorses)** (Impact: 92.2)
    * *Intent:* /// Emit the signature on the class recording the runtime type information
  * `_emitBinaryOperator` **(Many-Argument Workhorses)** (Impact: 70.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 20 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.000294
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **42**; blast radius 0.037; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.9%), Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (74.6%), Guard Balance (formerly Safety Score) (69.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `BinaryIntegerOpInstr::Canonicalize` **(Compute Cores)** (Impact: 152.5)
  * `LoadFieldInstr::Canonicalize` **(Compute Cores)** (Impact: 134.2)
  * `FfiCallInstr::EmitParamMoves` **(Many-Argument Workhorses)** (Impact: 84.3)
  * `MemoryCopyInstr::EmitUnrolledCopy` **(Many-Argument Workhorses)** (Impact: 62.4)
    * *Intent:* // EmitUnrolledCopy on ARM is different enough that it is defined separately. #if !defined(TARGET_AR...
  * `StoreFieldInstr::EmitNativeCode` **(Compute Cores)** (Impact: 58.2)
**Contextual Mitigations & Amplifications:**
...
- `pkg/analyzer/lib/src/dart/micro/resolve_file.dart` -> **Severity: 0.125** (Bridge: 0.0015 * Flux: 85.7637%)
- `pkg/analyzer/lib/src/dart/analysis/library_analyzer.dart` -> **Severity: 0.067** (Bridge: 0.0011 * Flux: 60.2316%)
- `pkg/analyzer_plugin/lib/src/utilities/change_builder/change_builder_core.dart` -> **Severity: 0.057** (Bridge: 0.0006 * Flux: 99.9419%)
- `pkg/compiler/lib/src/common/codegen.dart` -> **Severity: 0.04** (Bridge: 0.0004 * Flux: 100.0%)
- `pkg/front_end/lib/src/source/source_library_builder.dart` -> **Severity: 0.039** (Bridge: 0.0004 * Flux: 96.8162%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `pkg/_fe_analyzer_shared/lib/src/scanner/string_canonicalizer.dart` -> **Severity: 3.466** (Embedded: 0.0435 * Error Risk: 79.7211%)
- `pkg/analyzer/lib/src/dartdoc/dartdoc_directive_info.dart` -> **Severity: 3.306** (Embedded: 0.0343 * Error Risk: 96.3191%)
- `pkg/analyzer/lib/src/dart/analysis/cache.dart` -> **Severity: 3.167** (Embedded: 0.0366 * Error Risk: 86.6293%)
- `pkg/analyzer/lib/src/dart/element/type_algebra.dart` -> **Severity: 2.946** (Embedded: 0.0513 * Error Risk: 57.4285%)
- `pkg/analyzer/lib/src/ignore_comments/ignore_info.dart` -> **Severity: 2.943** (Embedded: 0.0335 * Error Risk: 87.9308%)
