...
## 1. EXECUTIVE SUMMARY
- **Scope:** 52 analyzed artifact(s), 5769 LOC.
- **Load-bearing artifact:** `charset_normalizer-3.4.7/src/charset_normalizer/constant.py` -- 8 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `charset_normalizer-3.4.7/src/charset_normalizer/cli/__main__.py` -- pulls in 12 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `charset_normalizer-3.4.7/src/charset_normalizer/api.py` at magnitude 882.12 (structural weight, not risk).
...
| Avg Path Length | 1.575 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `3.616`
> **Composition Archetype:** `Small Flat Repo (2)` (z +3.62; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 52%, Large Core Modules (2) 21%, Generic / Templated Code Files 10%, Declarative / Non-Code 6%, Large Core Modules (3) 4%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 56.9 | 65.7 | 100.0 |
...
- `from_bytes` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/api.py`) -> Impact: **650.5** | LOC: 805
- `cli_detect` **(Compute Cores)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/cli/__main__.py`) -> Impact: **86.9** | LOC: 268
  * *Intent:* """ CLI assistant using ARGV and ArgumentParser :param argv: :return: 0 if everything is fine, anything else equal trouble """
- `is_suspiciously_successive_range` **(Compute Cores)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/md.py`) -> Impact: **67.6** | LOC: 71
- `cut_sequence_chunks` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/utils.py`) -> Impact: **65.9** | LOC: 54
- `feed_info` **(Stateful Encapsulated Methods)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/md.py`) -> Impact: **57.7** | LOC: 74
  * *Intent:* """Optimized feed using pre-computed character info."""
- `characters_popularity_compare` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/cd.py`) -> Impact: **50.0** | LOC: 99
- `detect` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/legacy.py`) -> Impact: **47.1** | LOC: 62
- `update` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/md.py`) -> Impact: **38.3** | LOC: 107
  * *Intent:* """Update all properties for *character* (called once per character)."""
- `mess_ratio` **(Many-Argument Workhorses)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/md.py`) -> Impact: **38.3** | LOC: 126
- `feed_info` **(Stateful Encapsulated Methods)** (@ `charset_normalizer-3.4.7/src/charset_normalizer/md.py`) -> Impact: **36.2** | LOC: 44
  * *Intent:* """Optimized feed using pre-computed character info."""
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **9**; blast radius 53.587; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (82.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (68.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `from_bytes` **(Many-Argument Workhorses)** (Impact: 650.5)
  * `is_binary` **(Many-Argument Workhorses)** (Impact: 16.5)
  * `from_path` **(Many-Argument Workhorses)** (Impact: 4.8)
  * `from_fp` **(Many-Argument Workhorses)** (Impact: 4.7)
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **8**; blast radius 44.024; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (70.4%)
- **Documentation Coverage:** 66.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `is_suspiciously_successive_range` **(Compute Cores)** (Impact: 67.6)
  * `feed_info` **(Stateful Encapsulated Methods)** (Impact: 57.7)
    * *Intent:* """Optimized feed using pre-computed character info."""
  * `update` **(Many-Argument Workhorses)** (Impact: 38.3)
    * *Intent:* """Update all properties for *character* (called once per character)."""
  * `mess_ratio` **(Many-Argument Workhorses)** (Impact: 38.3)
  * `feed_info` **(Stateful Encapsulated Methods)** (Impact: 36.2)
    * *Intent:* """Optimized feed using pre-computed character info."""
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **10**; blast radius 43.054; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.4%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (51.8%)
- **Documentation Coverage:** 27.2727% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `characters_popularity_compare` **(Many-Argument Workhorses)** (Impact: 50.0)
  * `alpha_unicode_split` **(Compute Cores)** (Impact: 29.1)
    * *Intent:* """ Given a decoded text sequence, return a list of str. Unicode range / alphabet separation. Ex. a ...
  * `coherence_ratio` **(Many-Argument Workhorses)** (Impact: 26.4)
  * `alphabet_languages` **(Generic / Templated Code)** (Impact: 17.2)
  * `encoding_unicode_range` **(Compute Cores)** (Impact: 14.6)
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **12**; blast radius 86.655; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.8%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (77.4%), Guard Balance (formerly Safety Score) (67.3%)
- **Documentation Coverage:** 80.7018% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `cut_sequence_chunks` **(Many-Argument Workhorses)** (Impact: 65.9)
  * `_character_flags` **(Stateful Encapsulated Methods)** (Impact: 17.2)
    * *Intent:* """Compute all name-based classification flags with a single unicodedata.name() call."""
  * `any_specified_encoding` **(Compute Cores)** (Impact: 13.7)
  * `cp_similarity` **(Compute Cores)** (Impact: 9.6)
  * `iana_name` **(Compute Cores)** (Impact: 9.4)
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **8**; blast radius 40.945; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (77.9%), Connectivity (formerly Api Exposure) (69.4%)
- **Documentation Coverage:** 64.7059% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `append` **(Stateful Encapsulated Methods)** (Impact: 18.3)
    * *Intent:* """ Insert a single match. Will be inserted accordingly to preserve sort. Can be inserted as a subma...
  * `__lt__` **(Defensive Guards)** (Impact: 13.2)
    * *Intent:* """ Implemented to make sorted available upon CharsetMatches items. """
  * `language` **(Compute Cores)** (Impact: 11.2)
    * *Intent:* """ Most probable language found in decoded sequence. If none were detected or inferred, the propert...
  * `output` **(Stateful Encapsulated Methods)** (Impact: 10.1)
    * *Intent:* """ Method to get re-encoded bytes payload using given target encoding. Default to UTF-8. Any errors...
  * `__getitem__` **(Defensive Guards)** (Impact: 9.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (73.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 53.8462% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `cli_detect` **(Compute Cores)** (Impact: 86.9)
    * *Intent:* """ CLI assistant using ARGV and ArgumentParser :param argv: :return: 0 if everything is fine, anyth...
  * `__call__` **(Defensive Guards)** (Impact: 18.2)
    * *Intent:* # the special argument "-" means sys.std{in,out} if string == "-": if "r" in self._mode: return sys....
  * `query_yes_no` **(Compute Cores)** (Impact: 14.5)
    * *Intent:* """Ask a yes/no question via input() and return the answer as a bool."""
  * `__repr__` **(Generic / Templated Code)** (Impact: 7.5)
  * `__init__` **(Encapsulated Accessors)** (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 17 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 12.078; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (93.2%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (56.9%), Mutation Surface (formerly State Flux) (50.2%)
- **Documentation Coverage:** 41.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_impl` **(Compute Cores)** (Impact: 12.5)
  * `git_clone` **(Compute Cores)** (Impact: 7.7)
    * *Intent:* """We either clone the target repository or if already exist simply reset the state and pull. """
  * `downstream_niquests` **(I/O & Config Routines)** (Impact: 7.2)
  * `downstream_requests` **(I/O & Config Routines)** (Impact: 7.1)
  * `coverage` **(Compute Cores)** (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 12.078; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (39.2%), Connectivity (formerly Api Exposure) (10.8%), Complexity Load (formerly Cognitive Load) (6.5%)
- **Documentation Coverage:** 86.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_multiple_file_normalize` **(Defensive Guards)** (Impact: 7.8)
    * *Intent:* """Ensure --normalize with multiple files writes each output to the correct path and sets unicode_pa...
  * `test_multiple_file_normalize_with_alternatives` **(Defensive Guards)** (Impact: 7.1)
    * *Intent:* """Same as above but with --with-alternative, ensuring that alternative entries appended between fil...
  * `test_with_minimal_and_alt` **(Parameter Forwarders)** (Impact: 2.1)
  * `test_single_file_normalize` **(Defensive Guards)** (Impact: 2.0)
  * `test_multiple_file` **(Parameter Forwarders)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **5**; blast radius 24.397; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.5%), Complexity Load (formerly Cognitive Load) (67.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `detect` **(Many-Argument Workhorses)** (Impact: 47.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **5**; blast radius 158.36; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (50.8%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Mutation Surface (formerly State Flux) (8.7%), Complexity Load (formerly Cognitive Load) (5.0%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (11.0%), Connectivity (formerly Api Exposure) (9.6%), Complexity Load (formerly Cognitive Load) (7.1%)
- **Documentation Coverage:** 91.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_empty_but_with_bom_or_sig` **(Defensive Guards)** (Impact: 4.1)
  * `test_md_triggered_but_with_bom_or_sig` **(Defensive Guards)** (Impact: 4.1)
  * `test_mb_cutting_chk` **(Defensive Guards)** (Impact: 2.7)
    * *Intent:* # This payload should be wrongfully split and the autofix should ran automatically # on chunks extra...
  * `test_content_with_bom_or_sig` **(Defensive Guards)** (Impact: 2.1)
  * `test_utf7_sig_content_is_stripped` **(Defensive Guards)** (Impact: 2.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (67.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (28.2%)
- **Documentation Coverage:** 20.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_detect` **(Stateful Encapsulated Methods)** (Impact: 13.3)
  * `test_concurrent_detection` **(Defensive Guards)** (Impact: 9.3)
    * *Intent:* """Three files detected concurrently must each return the correct encoding and language, proving no ...
  * `test_concurrent_detection_repeated` **(Generic / Templated Code)** (Impact: 3.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 12.078; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.9%), Debt Markers (formerly Tech Debt) (62.2%), Complexity Load (formerly Cognitive Load) (52.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get_requires_for_build_wheel` **(Generic / Templated Code)** (Impact: 9.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (27.5%), Guard Balance (formerly Safety Score) (21.7%), Connectivity (formerly Api Exposure) (6.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_misleading_large_sequence` **(Defensive Guards)** (Impact: 7.7)
  * `test_large_payload_u8_sig_basic_entry` **(Defensive Guards)** (Impact: 3.8)
  * `test_large_payload_ascii_basic_entry` **(Defensive Guards)** (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 12.078; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (82.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (42.6%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 6 instances
* *State Mutation (weighted view):* 18
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (23.1%), Connectivity (formerly Api Exposure) (9.7%), Complexity Load (formerly Cognitive Load) (6.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_explain_false_handler_set_behavior` **(Defensive Guards)** (Impact: 5.7)
  * `test_explain_true_behavior` **(Defensive Guards)** (Impact: 3.8)
  * `test_set_stream_handler` **(Defensive Guards)** (Impact: 3.8)
  * `test_set_stream_handler_format` **(Defensive Guards)** (Impact: 2.2)
  * `setup_method` **(Parameter Forwarders)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (56.4%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (9.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_small_payload_confidence_altered` **(Parameter Forwarders)** (Impact: 2.2)
  * `test_detect_dict_keys` **(Parameter Forwarders)** (Impact: 2.0)
  * `test_detect_dict_value_type` **(Parameter Forwarders)** (Impact: 2.0)
  * `test_detect_dict_value` **(Parameter Forwarders)** (Impact: 1.7)
  * `test_utf8_sig_not_striped` **(Parameter Forwarders)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (58.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Complexity Load (formerly Cognitive Load) (6.4%), Connectivity (formerly Api Exposure) (4.8%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_preemptive_mark_replacement` **(Defensive Guards)** (Impact: 6.3)
    * *Intent:* """ When generating (to Unicode converted) bytes, we want to change any potential declarative charse...
  * `test_detect_most_common_body_encoding` **(Defensive Guards)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
* *State Mutation (weighted view):* 11
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (41.1%), Connectivity (formerly Api Exposure) (5.9%), Complexity Load (formerly Cognitive Load) (3.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_infer_language_from_cp` **(Defensive Guards)** (Impact: 9.2)
  * `test_target_features` **(Defensive Guards)** (Impact: 2.2)
  * `test_filter_alt_coherence_matches` **(Defensive Guards)** (Impact: 1.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 12.078; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (28.7%), Connectivity (formerly Api Exposure) (7.8%), Complexity Load (formerly Cognitive Load) (4.6%)
- **Documentation Coverage:** 25.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_issue_gh520` **(Defensive Guards)** (Impact: 1.5)
    * *Intent:* """Verify that minorities does not strip basic latin characters!"""
  * `test_issue_gh509` **(Defensive Guards)** (Impact: 1.5)
    * *Intent:* """Two common ASCII punctuations should render as-is."""
  * `test_issue_gh498` **(Defensive Guards)** (Impact: 1.5)
    * *Intent:* """This case was mistaken for utf-16-le, this should never happen again."""
  * `test_unicode_edge_case` **(Defensive Guards)** (Impact: 1.4)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 12.078; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (60.1%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (25.0%), Mutation Surface (formerly State Flux) (16.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 12.078; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (60.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (21.5%), Mutation Surface (formerly State Flux) (16.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **1**; blast radius 17.553; role: Pure Producer (Foundation)
