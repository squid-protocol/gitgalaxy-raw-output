...
## 1. EXECUTIVE SUMMARY
- **Scope:** 363 analyzed artifact(s), 73664 LOC.
- **Load-bearing artifact:** `gitgalaxy/standards/language_standards/languages/json.py` -- 53 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `gitgalaxy/galaxyscope.py` -- pulls in 57 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `gitgalaxy/core/detector.py` at magnitude 6980.3 (structural weight, not risk).
...
|---|---|---|
| Modularity | 0.7047 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.3536 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.0% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 1.5683 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
| Articulation Pts | 41 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| PYTHON | 320 | 72634 | 88.2% |
| MARKDOWN | 28 | 0 | 7.7% |
| YAML | 11 | 1008 | 3.0% |
| PLAINTEXT | 3 | 0 | 0.8% |
| SHELL | 1 | 22 | 0.3% |
...
> **File Composition:** Large Core Modules (2) 30%, Declarative / Non-Code 18%, Data / Markup / Trivial 14%, Parameter Forwarders Files 10%, Many-Argument Workhorses Files 8%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 54.8 | 56.8 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 13.4 | 0.0 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 26.5 | 13.6 | 0.0 |
...
| danger | 1849 | 245 | 12 | `gitgalaxy/galaxyscope.py` |
| concurrency | 390 | 88 | 3 | `gitgalaxy/standards/language_standards/languages/python.py` |
| connectivity | 2991 | 258 | 21 | `tests/core_engine/test_detector.py` |
| io | 706 | 221 | 5 | `tests/core_engine/test_galaxyscope.py` |
...
| regex | 2495 | 112 | 39 | `gitgalaxy/core/detector.py` |
| events | 519 | 96 | 3 | `gitgalaxy/galaxyscope.py` |
| tests | 3980 | 186 | 31 | `tests/core_engine/test_galaxyscope.py` |
...
| dead_code | 2615 | 206 | 21 | `tests/core_engine/test_detector.py` |
| credential | 169 | 57 | 2 | `gitgalaxy/standards/language_standards/languages/apex.py` |
| threat | 134 | 40 | 1 | `gitgalaxy/metrics/signal_processor.py` |
| ml_ai | 215 | 51 | 1 | `tests/extraction/languages/test_java.py` |
...
- `_slice_by_braces` **(Many-Argument Workhorses)** (@ `gitgalaxy/core/detector.py`) -> Impact: **1131.1** | LOC: 1244
- `_build_markdown` **(Many-Argument Workhorses)** (@ `gitgalaxy/recorders/llm_recorder.py`) -> Impact: **773.7** | LOC: 1029
- `inspect` **(Many-Argument Workhorses)** (@ `gitgalaxy/standards/language_lens.py`) -> Impact: **353.4** | LOC: 407
- `_calculate_block_metrics` **(Many-Argument Workhorses)** (@ `gitgalaxy/core/detector.py`) -> Impact: **314.8** | LOC: 455
- `execute_pipeline` **(Many-Argument Workhorses)** (@ `gitgalaxy/galaxyscope.py`) -> Impact: **275.4** | LOC: 588
...
- `splice` **(Many-Argument Workhorses)** (@ `gitgalaxy/core/detector.py`) -> Impact: **266.7** | LOC: 465
- `generate_report` **(Many-Argument Workhorses)** (@ `gitgalaxy/recorders/audit_recorder.py`) -> Impact: **264.4** | LOC: 480
- `calculate_risk_vector` **(Many-Argument Workhorses)** (@ `gitgalaxy/metrics/signal_processor.py`) -> Impact: **250.1** | LOC: 620
- `_slice_by_keywords` **(Many-Argument Workhorses)** (@ `gitgalaxy/core/detector.py`) -> Impact: **241.9** | LOC: 393
...
| `gitgalaxy/standards/language_standards/languages` | 59 | 1156.0 | 5.86% | 67.24% |
...
- **Unknown Dependencies:** `1950` packages imported that bypass the Zero-Trust whitelist.
...
- **Blast Radius:** changing it is visible to **25** in-repo importer(s); it depends on **12**; blast radius 16.009; role: Pure Producer (Foundation)
...
- **Documentation Coverage:** 24.026% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_slice_by_braces` **(Many-Argument Workhorses)** (Impact: 1131.1)
  * `_calculate_block_metrics` **(Many-Argument Workhorses)** (Impact: 314.8)
  * `splice` **(Many-Argument Workhorses)** (Impact: 266.7)
  * `_slice_by_keywords` **(Many-Argument Workhorses)** (Impact: 241.9)
  * `_build_brace_safe_stream` **(Many-Argument Workhorses)** (Impact: 177.8)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 16.009
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **57**; blast radius 3.167; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (68.8%)
- **Documentation Coverage:** 21.0526% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `execute_pipeline` **(Many-Argument Workhorses)** (Impact: 275.4)
    * *Intent:* """ Executes the synthesis protocol with a multi-recorder exit strategy. PIPELINE ONBOARDING (Execut...
  * `_process_file_worker` **(Many-Argument Workhorses)** (Impact: 135.5)
    * *Intent:* """Processes a single file path using the worker's cached hardware modules."""
  * `_resolve_dependency_graph` **(Compute Cores)** (Impact: 105.8)
    * *Intent:* """ Pass 1.5: Optimized relational token aggregation & Fuzzy Suffix Matching. Defused O(N^2) Bomb us...
  * `_calculate_risk_exposures` **(Many-Argument Workhorses)** (Impact: 94.2)
    * *Intent:* """ Phase 3: Universal Exposure Framework & Signal Processing. Translates raw Structural Signatures ...
  * `main` **(I/O & Config Routines)** (Impact: 90.8)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 3.167
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **11**; blast radius 2.284; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.9%), Complexity Load (formerly Cognitive Load) (83.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 45.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_build_markdown` **(Many-Argument Workhorses)** (Impact: 773.7)
  * `generate_artifacts` **(Many-Argument Workhorses)** (Impact: 37.7)
  * `_generate_sqlite_graph` **(Many-Argument Workhorses)** (Impact: 13.4)
  * `__init__` **(Generic / Templated Code)** (Impact: 5.8)
  * `_outbound` **(Encapsulated Accessors)** (Impact: 4.4)
**Contextual Mitigations & Amplifications:**
* *Sec High Risk Execution:* 1 instances
* *Amplified Race Conditions:* 2 instances
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 2.284
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **9**; blast radius 7.97; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.2%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (64.6%)
- **Documentation Coverage:** 32.7273% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `calculate_risk_vector` **(Many-Argument Workhorses)** (Impact: 250.1)
  * `summarize_galaxy_metrics` **(Many-Argument Workhorses)** (Impact: 193.3)
    * *Intent:* # ========================================================================== # GLOBAL SYNTHESIS & 2-...
  * `generate_forensic_report` **(Many-Argument Workhorses)** (Impact: 69.1)
    * *Intent:* # -------------------------------------------------------------------------- # REPORTING UTILITIES #...
  * `_calc_verification` **(Many-Argument Workhorses)** (Impact: 57.2)
  * `_calc_documentation` **(Many-Argument Workhorses)** (Impact: 30.5)
**Contextual Mitigations & Amplifications:**
* *Sec High Risk Execution:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (73.2%), Guard Balance (formerly Safety Score) (57.5%), Complexity Load (formerly Cognitive Load) (15.0%), Connectivity (formerly Api Exposure) (12.0%)
- **Documentation Coverage:** 1.8116% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `test_detector_mode_d_livecode_script_handlers` **(Defensive Guards)** (Impact: 11.0)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.54
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **9**; blast radius 3.471; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (67.8%)
- **Documentation Coverage:** 41.1765% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `inspect` **(Many-Argument Workhorses)** (Impact: 353.4)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 3.471
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **4**; blast radius 7.194; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.7%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (68.0%)
- **Documentation Coverage:** 23.75% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `_strip_segment_comments` **(Many-Argument Workhorses)** (Impact: 44.9)
    * *Intent:* """Surgically strips documentation using an ordered, additive pipeline."""
  * `_partition_embedded_languages` **(Many-Argument Workhorses)** (Impact: 43.8)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 7.194
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **6**; blast radius 3.252; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (67.6%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (80.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (47.0%), Complexity Load (formerly Cognitive Load) (22.2%), Concurrency Surface (formerly Concurrency) (20.7%)
- **Documentation Coverage:** 9.0226% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **17**; blast radius 3.201; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.8%), Complexity Load (formerly Cognitive Load) (60.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (28.3%)
- **Documentation Coverage:** 16.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `build_dependency_graph` **(Many-Argument Workhorses)** (Impact: 83.6)
  ...
  * `_fallback_build_graph` **(Many-Argument Workhorses)** (Impact: 34.5)
  ...
  * `_build_folded_resolution_map` **(Stateful Encapsulated Methods)** (Impact: 12.0)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 3.201
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **8**; blast radius 2.284; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.5%), Complexity Load (formerly Cognitive Load) (81.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 25.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `generate_report` **(Many-Argument Workhorses)** (Impact: 264.4)
  ...
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 5.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (41.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (39.7%), Complexity Load (formerly Cognitive Load) (17.5%), Connectivity (formerly Api Exposure) (11.3%)
...
**Top Internal Functions/Classes:**
  ...
  * `test_signal_processor_report_fallback` **(Defensive Guards)** (Impact: 5.4)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **4**; blast radius 2.939; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.4%)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 2.939
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **7**; blast radius 3.855; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.5%), Complexity Load (formerly Cognitive Load) (54.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (49.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `record_mission` **(Many-Argument Workhorses)** (Impact: 9.5)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 3.855
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **9**; blast radius 3.911; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (58.6%)
- **Documentation Coverage:** 12.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_resolve_dependency_graph` **(Many-Argument Workhorses)** (Impact: 50.2)
  ...
  * `_construct_feature_matrix` **(Many-Argument Workhorses)** (Impact: 31.5)
  ...
  * `get_nth_degree` **(Type Conversions)** (Impact: 10.6)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 3.911
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (30.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (28.3%), Complexity Load (formerly Cognitive Load) (13.3%), Connectivity (formerly Api Exposure) (10.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `test_network_duplicate_filename_fallback_mode` **(Defensive Guards)** (Impact: 9.4)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **8**; blast radius 2.284; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.1%), Complexity Load (formerly Cognitive Load) (37.0%), Connectivity (formerly Api Exposure) (9.7%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (47.0%), Complexity Load (formerly Cognitive Load) (13.7%), Connectivity (formerly Api Exposure) (11.1%), Guard Balance (formerly Safety Score) (10.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **7**; blast radius 2.939; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (55.7%)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 2.939
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (32.9%), Complexity Load (formerly Cognitive Load) (18.6%), Guard Balance (formerly Safety Score) (16.7%), Connectivity (formerly Api Exposure) (9.3%)
- **Documentation Coverage:** 12.9032% of unit weight undocumented
**Top Internal Functions/Classes:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (39.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (36.6%), Guard Balance (formerly Safety Score) (31.5%), Complexity Load (formerly Cognitive Load) (19.0%)
...
**Top Internal Functions/Classes:**
  ...
  * `test_livecode_dependency_capture_extracts_path` **(Defensive Guards)** (Impact: 9.9)
  ...
  * `_measure_scaling_point` **(Stateful Encapsulated Methods)** (Impact: 6.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (32.9%), Guard Balance (formerly Safety Score) (27.0%), Complexity Load (formerly Cognitive Load) (17.7%), Connectivity (formerly Api Exposure) (10.1%)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **12**; blast radius 2.502; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.2%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (55.7%)
- **Documentation Coverage:** 25.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `_audit_capped_sample` **(Stateful Encapsulated Methods)** (Impact: 17.2)
    * *Intent:* """ LEGACY mode (no cache configured): per-directory capped sampling (#254). Coverage is honestly di...
  * `_iter_candidate_files` **(Stateful Encapsulated Methods)** (Impact: 11.5)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 2.502
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **9**; blast radius 4.248; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (45.7%)
...
**Top Internal Functions/Classes:**
  ...
  * `_initialize_history_scan` **(Stateful Encapsulated Methods)** (Impact: 9.1)
    * *Intent:* """Dispatches the survey engines to establish boundaries and churn cache."""
  * `_load_ignored_revs` **(Stateful Encapsulated Methods)** (Impact: 7.9)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 4.248
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.54; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (96.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (28.3%), Guard Balance (formerly Safety Score) (19.5%), Complexity Load (formerly Cognitive Load) (19.3%)
- **Documentation Coverage:** 22.2222% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_html_star_ngif_leading_boundary_regression` **(Defensive Guards)** (Impact: 12.0)
    * *Intent:* """ Regression test for a real bug: `branch`'s `*ngIf` (Angular's structural directive) was inside t...
  * `test_html_single_quote_bug_reproduces_on_old_double_quote_only_patterns` **(Defensive Guards)** (Impact: 11.3)
    * *Intent:* # NOTE: this test was originally grouped under a shared "cross-language sweep" # section in tests/co...
  * `test_html_signature_positive_and_negative` **(Defensive Guards)** (Impact: 10.3)
  * `test_html_signature_adversarial` **(Defensive Guards)** (Impact: 10.3)
  * `test_html_signature_deep` **(Defensive Guards)** (Impact: 10.3)
**Contextual Mitigations & Amplifications:**
...
- `gitgalaxy/standards/language_standards/_shared_patterns.py` -> **Severity: 9.857** (Embedded: 0.1172 * Error Risk: 84.1131%)
...
- `tests/tools/fidelity_table.py` -> **Severity: 672.525** (Blast Radius: 8.967 * Doc Risk: 75.0%)
- `gitgalaxy/standards/config_resolver.py` -> **Severity: 597.502** (Blast Radius: 12.653 * Doc Risk: 47.2222%)
- `tests/tools/tri_comparison_reconcile.py` -> **Severity: 488.08** (Blast Radius: 6.101 * Doc Risk: 80.0%)
- `tests/tools/tree_sitter_accuracy_audit.py` -> **Severity: 387.097** (Blast Radius: 8.0 * Doc Risk: 48.3871%)
- `gitgalaxy/core/detector.py` -> **Severity: 384.632** (Blast Radius: 16.009 * Doc Risk: 24.026%)
