...
## 1. EXECUTIVE SUMMARY
- **Scope:** 6 analyzed artifact(s), 498 LOC.
- **Load-bearing artifact:** `package/package.json` -- 2 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `package/index.js` -- pulls in 9 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `package/index.js` at magnitude 273.9 (structural weight, not risk).
...
> **File Composition:** Data / Markup / Trivial 50%, Compute Cores Files 33%, Parameter Forwarders Files 17%
...
- `.map`: 1x Excluded (Saturation: Line 1 exceeds 500 chars)
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 50.0 | 21.6 | 14.7 | 50.0 |
...
- `toObject` **(I/O & Config Routines)** (@ `package/cli.js`) -> Impact: **8.1** | LOC: 49
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 129.87; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (87.6%), Complexity Load (formerly Cognitive Load) (57.7%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 129.87; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (91.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Debt Markers (formerly Tech Debt) (48.3%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run` **(Compute Cores)** (Impact: 21.0)
  * `toObject` **(I/O & Config Routines)** (Impact: 8.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 129.87; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (47.4%), Concurrency Surface (formerly Concurrency) (20.4%)
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); blast radius 350.649; role: Pure Producer (Foundation)
