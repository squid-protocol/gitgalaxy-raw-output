...
## 1. EXECUTIVE SUMMARY
- **Scope:** 1021 analyzed artifact(s), 122963 LOC.
- **Load-bearing artifact:** `framework/Yii.php` -- 244 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `tests/framework/web/UrlRuleTest.php` -- pulls in 273 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `framework/helpers/BaseHtml.php` at magnitude 1675.38 (structural weight, not risk).
...
| Avg Path Length | 1.9406 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.739`
> **Composition Archetype:** `Hub-Coupled App` (z +1.74; from the repo's file-archetype mix)
> **File Composition:** Interface Declarations Files 18%, Data / Markup / Trivial 16%, Large Core Modules (3) 15%, Parameter Forwarders Files 11%, Declarative / Non-Code 11%
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 92.5 | 12.0 | 4.1 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 56.6 | 63.4 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 4.7 | 0.8 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 9.6 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 23.8 | 0.0 | 0.0 |
...
| cleanup | 505 | 118 | 1 | `tests/data/oci.sql` |
| guards | 5715 | 620 | 15 | `tests/data/mssql.sql` |
| danger | 2389 | 369 | 5 | `tests/framework/db/mssql/QueryBuilderTest.php` |
| concurrency | 121 | 24 | 0 | `framework/db/BaseActiveRecord.php` |
| connectivity | 7726 | 835 | 21 | `tests/framework/helpers/HtmlTest.php` |
| io | 545 | 113 | 1 | `tests/js/data/yii.html` |
| crypto | 0 | 0 | 0 | - |
| ipc | 43 | 23 | 0 | `framework/i18n/Formatter.php` |
| time | 272 | 59 | 0 | `tests/framework/i18n/FormatterDateTest.php` |
...
- `email` **(Many-Argument Workhorses)** (@ `framework/assets/yii.validation.js`) -> Impact: **198.0** | LOC: 240
- `saveMessagesToDb` **(Many-Argument Workhorses)** (@ `framework/console/controllers/MessageController.php`) -> Impact: **106.0** | LOC: 141
- `parseToken` **(Many-Argument Workhorses)** (@ `framework/i18n/MessageFormatter.php`) -> Impact: **99.0** | LOC: 100
...
- `saveMessagesToPO` **(Many-Argument Workhorses)** (@ `framework/console/controllers/MessageController.php`) -> Impact: **77.7** | LOC: 84
- `createUrl` **(Many-Argument Workhorses)** (@ `framework/web/UrlRule.php`) -> Impact: **76.2** | LOC: 85
...
- `resolveCallableDependencies` **(Defensive Guards)** (@ `framework/di/Container.php`) -> Impact: **71.7** | LOC: 83
...
- `saveMessagesCategoryToPHP` **(Many-Argument Workhorses)** (@ `framework/console/controllers/MessageController.php`) -> Impact: **71.1** | LOC: 64
- `handleAction` **(Many-Argument Workhorses)** (@ `framework/assets/yii.js`) -> Impact: **69.0** | LOC: 132
  * *Intent:* * echo Html::a('submit', ['site/foobar'], [ * 'data' => [ * 'method' => 'post', * 'params' => [ * 'name1' => 'value1', * 'name2' => 'value2', * ], * ]...
- `extractMessagesFromTokens` **(Many-Argument Workhorses)** (@ `framework/console/controllers/MessageController.php`) -> Impact: **67.0** | LOC: 101
- `unlink` **(Many-Argument Workhorses)** (@ `framework/db/BaseActiveRecord.php`) -> Impact: **66.3** | LOC: 87
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.1%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (75.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderSelectOptions` **(Many-Argument Workhorses)** (Impact: 64.0)
    * *Intent:* * @param array $items the option data items. The array keys are option values, and the array values ...
  * `checkboxList` **(Many-Argument Workhorses)** (Impact: 38.5)
    * *Intent:* * corresponding to a single item in $items. The signature of this callback must be: * * ``` * functi...
  * `renderTagAttributes` **(Compute Cores)** (Impact: 36.7)
    * *Intent:* * the array will be "expanded" and a list of ARIA/data attributes will be rendered. For example, * `...
  * `radioList` **(Many-Argument Workhorses)** (Impact: 31.5)
    * *Intent:* * corresponding to a single item in $items. The signature of this callback must be: * * ``` * functi...
  * `beginForm` **(Many-Argument Workhorses)** (Impact: 30.2)
...
- **Blast Radius:** changing it is visible to **6** in-repo importer(s); it depends on **75**; blast radius 1.403; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.3%), Debt Markers (formerly Tech Debt) (53.3%), Complexity Load (formerly Cognitive Load) (29.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `formatDateTimeValue` **(Many-Argument Workhorses)** (Impact: 54.1)
    * *Intent:* /** * @param int|string|DateTime|DateTimeInterface|null $value the value to be formatted. The follow...
  * `asDecimalStringFallback` **(Many-Argument Workhorses)** (Impact: 51.9)
    * *Intent:* /** * Fallback for formatting value as a decimal number. * * Property [[decimalSeparator]] will be u...
  * `formatNumber` **(Many-Argument Workhorses)** (Impact: 50.4)
    * *Intent:* /** * Given the value in bytes formats number part of the human readable form. * * @param string|int...
  * `asShortSize` **(Many-Argument Workhorses)** (Impact: 42.2)
    * *Intent:* * This is the short form of [[asSize]]. * * If [[sizeFormatBase]] is 1024, [binary prefixes](https:/...
  * `asSize` **(Many-Argument Workhorses)** (Impact: 42.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.4e-05 | `Ripple Effect (Closeness):` 0.008284
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **18**; blast radius 1.867; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (95.5%), Guard Balance (formerly Safety Score) (91.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (69.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `unlink` **(Many-Argument Workhorses)** (Impact: 66.3)
    * *Intent:* * The model with the foreign key of the relationship will be deleted if `$delete` is `true`. * Other...
  * `link` **(Many-Argument Workhorses)** (Impact: 57.9)
    * *Intent:* * and **without** events/behaviors. * * If the relationship involves a junction table, a new row wil...
  * `unlinkAll` **(Many-Argument Workhorses)** (Impact: 47.0)
    * *Intent:* * Destroys the relationship in current model. * * The model with the foreign key of the relationship...
  * `updateInternal` **(Stateful Encapsulated Methods)** (Impact: 16.0)
    * *Intent:* /** * @see update() * @param array|null $attributes attributes to update * @return int|false the num...
  * `getRelation` **(Defensive Guards)** (Impact: 15.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000218 | `Ripple Effect (Closeness):` 0.012353
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (99.4%), Guard Balance (formerly Safety Score) (94.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (58.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseAcceptHeader` **(Compute Cores)** (Impact: 30.2)
    * *Intent:* * $accepts = $request->parseAcceptHeader($header); * print_r($accepts); * // displays: * // [ * // '...
  * `getUserIpFromIpHeaders` **(Stateful Encapsulated Methods)** (Impact: 23.4)
    * *Intent:* /** * Returns the user IP address from [[ipHeaders]]. * @return string|null user IP address, null if...
  * `loadCookies` **(I/O & Config Routines)** (Impact: 18.1)
    * *Intent:* /** * Converts `$_COOKIE` into an array of [[Cookie]]. * @return array the cookies obtained from req...
  * `getUserIpFromIpHeader` **(Stateful Encapsulated Methods)** (Impact: 17.2)
    * *Intent:* /** * Return user IP's from IP header. * * @param string $ips comma separated IP list * @return stri...
  * `getHostInfo` **(I/O & Config Routines)** (Impact: 14.2)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **21**; blast radius 0.597; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (61.0%)
- **Documentation Coverage:** 70.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `saveMessagesToDb` **(Many-Argument Workhorses)** (Impact: 106.0)
  * `saveMessagesToPO` **(Many-Argument Workhorses)** (Impact: 77.7)
  * `saveMessagesCategoryToPHP` **(Many-Argument Workhorses)** (Impact: 71.1)
  * `extractMessagesFromTokens` **(Many-Argument Workhorses)** (Impact: 67.0)
  * `initConfig` **(Defensive Guards)** (Impact: 20.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.3e-05 | `Ripple Effect (Closeness):` 0.00098
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.2%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (42.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `changeOwnership` **(Many-Argument Workhorses)** (Impact: 58.9)
    * *Intent:* * @param string|array|int|null $ownership the user and/or group ownership for the file or directory....
  * `copyDirectory` **(Defensive Guards)** (Impact: 52.8)
    * *Intent:* * - recursive: boolean, whether the files under the subdirectories should also be copied. Defaults t...
  * `normalizePath` **(Defensive Guards)** (Impact: 32.7)
    * *Intent:* * The normalization does the following work: * * - Convert all directory separators into `DIRECTORY_...
  * `matchPathname` **(Many-Argument Workhorses)** (Impact: 31.5)
    * *Intent:* /** * Compares a path part against a pattern with optional wildcards. * * Based on match_pathname() ...
  * `lastExcludeMatchingFromList` **(Stateful Encapsulated Methods)** (Impact: 27.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **36**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (61.3%), Complexity Load (formerly Cognitive Load) (16.4%), Connectivity (formerly Api Exposure) (11.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (11.6%)
- **Documentation Coverage:** 58.2888% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testJoinWithAlias` **(Compute Cores)** (Impact: 58.6)
    * *Intent:* /** * Tests the alias syntax for joinWith: 'alias' => 'relation'. * @dataProvider aliasMethodProvide...
  * `testPopulateWithoutPk` **(I/O & Config Routines)** (Impact: 13.3)
  * `testAmbiguousColumnIndexBy` **(I/O & Config Routines)** (Impact: 13.2)
    * *Intent:* /** * Ensure no ambiguous column error occurs on indexBy with JOIN. * * @see https://github.com/yiis...
  * `testJoinWith` **(I/O & Config Routines)** (Impact: 10.0)
  * `testLoadRelations` **(I/O & Config Routines)** (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 2 instances
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **18**; blast radius 0.745; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (99.7%), Guard Balance (formerly Safety Score) (97.2%), Complexity Load (formerly Cognitive Load) (47.9%)
- **Documentation Coverage:** 0.9524% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ansiToHtml` **(Many-Argument Workhorses)** (Impact: 34.4)
    * *Intent:* /** * Converts an ANSI formatted string to HTML. * * Note: xTerm 256 bit colors are currently not su...
  * `ansiColorizedSubstr` **(Many-Argument Workhorses)** (Impact: 30.3)
    * *Intent:* /** * Returns the portion with ANSI color codes of string specified by the start and length paramete...
  * `getScreenSize` **(Type Conversions)** (Impact: 28.1)
    * *Intent:* /** * Returns terminal screen size. * * Usage: * * ``` * list($width, $height) = ConsoleHelper::getS...
  * `prompt` **(Compute Cores)** (Impact: 27.8)
    * *Intent:* * Prompts the user for input and validates it. * * @param string $text prompt string * @param array ...
  * `updateProgress` **(Many-Argument Workhorses)** (Impact: 23.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 9e-06 | `Ripple Effect (Closeness):` 0.00098
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.9%), Debt Markers (formerly Tech Debt) (97.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `toArray` **(Many-Argument Workhorses)** (Impact: 38.2)
    * *Intent:* * The result of `ArrayHelper::toArray($post, $properties)` could be like the following: * * ``` * [ ...
  * `filter` **(Compute Cores)** (Impact: 37.4)
    * *Intent:* * // [ * // 'B' => ['D' => 2], * // ] * ``` * * @param array $array Source array * @param iterable $...
  * `getValue` **(Defensive Guards)** (Impact: 26.2)
    * *Intent:* * $street = \yii\helpers\ArrayHelper::getValue($users, 'address.street'); * // using an array of key...
  * `merge` **(Defensive Guards)** (Impact: 22.1)
    * *Intent:* * Merges two or more arrays into one recursively. * If each array has an element with the same strin...
  * `multisort` **(Many-Argument Workhorses)** (Impact: 21.8)
...
- **Blast Radius:** changing it is visible to **50** in-repo importer(s); it depends on **14**; blast radius 7.416; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.7%), Debt Markers (formerly Tech Debt) (90.1%), Complexity Load (formerly Cognitive Load) (34.7%)
- **Documentation Coverage:** 43.0556% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `column` **(Compute Cores)** (Impact: 23.2)
    * *Intent:* /** * Executes the query and returns the first column of the result. * @param Connection|null $db th...
  * `normalizeSelect` **(Stateful Encapsulated Methods)** (Impact: 23.0)
    * *Intent:* /** * Add more columns to the SELECT part of the query. * * Note, that if [[select]] has not been sp...
  * `cleanUpTableNames` **(Stateful Encapsulated Methods)** (Impact: 17.1)
    * *Intent:* /** * Clean up table names and aliases * Both aliases and names are enclosed into {{ and }}. * @para...
  * `getUniqueColumns` **(Stateful Encapsulated Methods)** (Impact: 16.8)
  * `queryScalar` **(Defensive Guards)** (Impact: 14.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000332 | `Ripple Effect (Closeness):` 0.052713
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **33**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (56.4%), Complexity Load (formerly Cognitive Load) (19.5%), Connectivity (formerly Api Exposure) (11.5%)
- **Documentation Coverage:** 84.8684% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testNonStrictBooleanDropDownList` **(Many-Argument Workhorses)** (Impact: 9.5)
  * `testStrictBooleanDropDownList` **(Many-Argument Workhorses)** (Impact: 9.5)
    * *Intent:* /** * @dataProvider providerForNonStrictBooleanDropDownList
  * `setActivePlaceholder` **(Stateful Encapsulated Methods)** (Impact: 6.3)
  * `testErrorSummary` **(Many-Argument Workhorses)** (Impact: 5.0)
  * `testCheckboxList` **(I/O & Config Routines)** (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **13** in-repo importer(s); it depends on **3**; blast radius 5.083; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (80.2%), Debt Markers (formerly Tech Debt) (40.4%)
- **Documentation Coverage:** 2.439% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `joinWithRelation` **(Many-Argument Workhorses)** (Impact: 41.3)
    * *Intent:* /** * Joins a parent query with a child query. * The current query object will be modified according...
  * `prepare` **(Defensive Guards)** (Impact: 31.8)
    * *Intent:* /** * {@inheritdoc} */
  * `joinWithRelations` **(Many-Argument Workhorses)** (Impact: 24.2)
    * *Intent:* /** * Modifies the current query by adding join fragments based on the given relations. * @param Act...
  * `removeDuplicatedModels` **(Defensive Guards)** (Impact: 23.5)
    * *Intent:* /** * Removes duplicated models by checking their primary key values. * This method is mainly called...
  * `joinWith` **(Many-Argument Workhorses)** (Impact: 19.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8.7e-05 | `Ripple Effect (Closeness):` 0.018007
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.961; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (58.2%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `populateRelation` **(Many-Argument Workhorses)** (Impact: 64.1)
    * *Intent:* /** * Finds the related records and populates them into the primary models. * @param string $name th...
  * `populateInverseRelation` **(Many-Argument Workhorses)** (Impact: 58.7)
    * *Intent:* /** * @param ActiveRecordInterface[]|array<array-key, array<string, mixed>> $primaryModels primary m...
  * `buildBuckets` **(Many-Argument Workhorses)** (Impact: 36.8)
    * *Intent:* /** * @param array $models * @param array $link * @param array|null $viaModels * @param self<ActiveR...
  * `filterByModels` **(Defensive Guards)** (Impact: 31.2)
    * *Intent:* /** * @param array $models */
  * `prefixKeyColumns` **(Stateful Encapsulated Methods)** (Impact: 16.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.8e-05 | `Ripple Effect (Closeness):` 0.011777
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **35**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (99.6%), Guard Balance (formerly Safety Score) (98.2%), Debt Markers (formerly Tech Debt) (83.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `bindActionParams` **(Defensive Guards)** (Impact: 48.7)
    * *Intent:* /** * Binds the parameters to the action. * This method is invoked by [[Action]] when it begins to r...
  * `runAction` **(Defensive Guards)** (Impact: 43.3)
    * *Intent:* /** * Runs an action with the specified action ID and parameters. * If the action ID is empty, the m...
  * `getActionArgsHelp` **(Defensive Guards)** (Impact: 39.6)
    * *Intent:* * - required: bool, whether this argument is required * - type: string|null, the PHP type(s) of this...
  * `getActionOptionsHelp` **(Defensive Guards)** (Impact: 22.2)
    * *Intent:* * * - type: string, the PHP type of this argument. * - default: string, the default value of this ar...
  * `parseDocCommentTags` **(Stateful Encapsulated Methods)** (Impact: 10.9)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **19**; blast radius 0.605; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (84.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `actionUp` **(Compute Cores)** (Impact: 26.5)
    * *Intent:* * Upgrades the application by applying new migrations. * * For example, * * ``` * yii migrate # appl...
  * `actionRedo` **(Compute Cores)** (Impact: 23.7)
    * *Intent:* * them again. For example, * * ``` * yii migrate/redo # redo the last applied migration * yii migrat...
  * `actionMark` **(Compute Cores)** (Impact: 23.5)
    * *Intent:* * ``` * yii migrate/mark 101129_185401 # using timestamp * yii migrate/mark m101129_185401_create_us...
  * `actionDown` **(Compute Cores)** (Impact: 22.0)
    * *Intent:* * For example, * * ``` * yii migrate/down # revert the last migration * yii migrate/down 3 # revert ...
  * `getNewMigrations` **(I/O & Config Routines)** (Impact: 21.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8e-06 | `Ripple Effect (Closeness):` 0.001307
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **21**; blast radius 0.572; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.1%), Complexity Load (formerly Cognitive Load) (37.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (32.6%)
- **Documentation Coverage:** 48.1481% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `adjustCssUrl` **(Many-Argument Workhorses)** (Impact: 51.5)
  * `loadTargets` **(Defensive Guards)** (Impact: 35.7)
    * *Intent:* /** * Creates full list of output asset bundles. * @param array $targets output asset bundles config...
  * `buildTarget` **(Stateful Encapsulated Methods)** (Impact: 25.9)
    * *Intent:* /** * Builds output asset bundle. * @param \yii\web\AssetBundle $target output asset bundle * @param...
  * `adjustDependency` **(Stateful Encapsulated Methods)** (Impact: 17.6)
    * *Intent:* /** * Adjust dependencies between asset bundles in the way source bundles begin to depend on output ...
  * `saveTargets` **(Stateful Encapsulated Methods)** (Impact: 13.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2e-06 | `Ripple Effect (Closeness):` 0.00098
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (77.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (50.2%), Complexity Load (formerly Cognitive Load) (21.5%), Connectivity (formerly Api Exposure) (10.3%)
- **Documentation Coverage:** 98.3607% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `testBindParamValue` **(I/O & Config Routines)** (Impact: 20.4)
  * `testInsertExpression` **(I/O & Config Routines)** (Impact: 13.8)
  * `upsertProvider` **(I/O & Config Routines)** (Impact: 8.5)
  * `testAddDropForeignKey` **(Many-Argument Workhorses)** (Impact: 6.5)
    * *Intent:* /** * @dataProvider addForeignKeyProvider * * @param string $name * @param string $tableName
  * `testAutoRefreshTableSchema` **(I/O & Config Routines)** (Impact: 6.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 4 instances
* *Amplified Cascading Flux:* 58 instances
* *Api Near Db Sink:* 7 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **49**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.0%), Debt Markers (formerly Tech Debt) (92.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (50.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `redirect` **(Many-Argument Workhorses)** (Impact: 29.9)
    * *Intent:* * * Any relative URL that starts with a single forward slash "/" will be converted * into an absolut...
  * `sendStreamAsFile` **(Defensive Guards)** (Impact: 21.8)
    * *Intent:* * @param string $attachmentName the file name shown to the user. * @param array $options additional ...
  * `sendContent` **(I/O & Config Routines)** (Impact: 18.8)
    * *Intent:* /** * Sends the response content to the client. */
  * `sendCookies` **(Defensive Guards)** (Impact: 18.3)
    * *Intent:* /** * Sends the cookies to the client. */
  * `prepare` **(Stateful Encapsulated Methods)** (Impact: 16.1)
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **12**; blast radius 1.483; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (85.5%), Debt Markers (formerly Tech Debt) (84.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (52.2%)
- **Documentation Coverage:** 9.2105% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `checkAccessFromCache` **(Stateful Encapsulated Methods)** (Impact: 21.5)
    * *Intent:* /** * Performs access check for the specified user based on the data loaded from cache. * This metho...
  * `checkAccessRecursive` **(Stateful Encapsulated Methods)** (Impact: 19.3)
    * *Intent:* /** * Performs access check for the specified user. * This method is internally called by [[checkAcc...
  * `loadFromCache` **(Defensive Guards)** (Impact: 13.0)
  * `checkAccess` **(Type Conversions)** (Impact: 11.0)
    * *Intent:* /** * {@inheritdoc} */
  * `getRolesByUser` **(Compute Cores)** (Impact: 10.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.1e-05 | `Ripple Effect (Closeness):` 0.006863
...
- **Blast Radius:** changing it is visible to **42** in-repo importer(s); it depends on **11**; blast radius 8.504; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.6%), Debt Markers (formerly Tech Debt) (82.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (62.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `openFromPoolSequentially` **(Defensive Guards)** (Impact: 25.7)
    * *Intent:* * connection attempts to unavailable servers saves time when the connection attempts fail due to tim...
  * `getQueryCacheInfo` **(Compute Cores)** (Impact: 20.5)
    * *Intent:* /** * Returns the current query cache information. * This method is used internally by [[Command]]. ...
  * `createPdoInstance` **(I/O & Config Routines)** (Impact: 14.6)
    * *Intent:* /** * Creates the PDO instance. * This method is called by [[open]] to establish a DB connection. * ...
  * `open` **(Defensive Guards)** (Impact: 11.2)
    * *Intent:* /** * Establishes a DB connection. * It does nothing if a DB connection has already been established...
  * `initConnection` **(Stateful Encapsulated Methods)** (Impact: 11.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000396 | `Ripple Effect (Closeness):` 0.069007
...
- **Blast Radius:** changing it is visible to **7** in-repo importer(s); it depends on **17**; blast radius 1.352; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.5%), Debt Markers (formerly Tech Debt) (67.0%), Complexity Load (formerly Cognitive Load) (42.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `resolveCallableDependencies` **(Defensive Guards)** (Impact: 71.7)
    * *Intent:* /** * Resolve dependencies for a function. * * This method can be used to implement similar function...
  * `getDependencies` **(Defensive Guards)** (Impact: 30.4)
    * *Intent:* /** * Returns the dependencies of the specified class. * * @template T of object * * @param class-st...
  * `build` **(Many-Argument Workhorses)** (Impact: 24.4)
    * *Intent:* /** * Creates an instance of the specified class. * This method will resolve dependencies of the spe...
  * `get` **(Many-Argument Workhorses)** (Impact: 24.1)
    * *Intent:* * * @param string|class-string<T>|Instance $class the class Instance, name, or an alias name (e.g. `...
  * `normalizeDefinition` **(Stateful Encapsulated Methods)** (Impact: 23.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4.8e-05 | `Ripple Effect (Closeness):` 0.006972
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.1%), Debt Markers (formerly Tech Debt) (85.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (44.1%)
- **Documentation Coverage:** 67.8571% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `loadColumnSchema` **(Stateful Encapsulated Methods)** (Impact: 32.6)
  * `loadTableConstraints` **(Many-Argument Workhorses)** (Impact: 29.3)
  * `findColumns` **(Stateful Encapsulated Methods)** (Impact: 16.1)
  * `insert` **(Defensive Guards)** (Impact: 15.3)
    * *Intent:* /** * Loads multiple types of constraints and returns the specified ones. * @param string $tableName...
  * `resolveTableNames` **(Stateful Encapsulated Methods)** (Impact: 11.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.1%), Debt Markers (formerly Tech Debt) (89.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (63.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `insert` **(Many-Argument Workhorses)** (Impact: 34.4)
    * *Intent:* /** * {@inheritdoc} * Added OUTPUT construction for getting inserted data (for SQL Server 2005 or la...
  * `upsert` **(Many-Argument Workhorses)** (Impact: 32.5)
    * *Intent:* /** * {@inheritdoc} * @see https://docs.microsoft.com/en-us/sql/t-sql/statements/merge-transact-sql ...
  * `oldBuildOrderByAndLimit` **(Stateful Encapsulated Methods)** (Impact: 19.1)
    * *Intent:* /** * Builds the ORDER BY/LIMIT/OFFSET clauses for SQL SERVER 2005 to 2008. * @param string $sql the...
  * `alterColumn` **(Many-Argument Workhorses)** (Impact: 15.9)
    * *Intent:* /** * Builds a SQL statement for changing the definition of a column. * @param string $table the tab...
  * `normalizeTableRowData` **(Stateful Encapsulated Methods)** (Impact: 14.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 0.523; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (100.0%), Guard Balance (formerly Safety Score) (87.7%), Complexity Load (formerly Cognitive Load) (44.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getRawSql` **(Defensive Guards)** (Impact: 18.6)
    * *Intent:* /** * Returns the raw SQL by inserting parameter values into the corresponding placeholders in [[sql...
  * `queryInternal` **(Stateful Encapsulated Methods)** (Impact: 18.0)
    * *Intent:* /** * Performs the actual DB query of a SQL statement. * @param string $method method of PDOStatemen...
  * `internalExecute` **(Stateful Encapsulated Methods)** (Impact: 15.4)
    * *Intent:* /** * Executes a prepared statement. * * It's a wrapper around [[\PDOStatement::execute()]] to suppo...
  * `prepare` **(Defensive Guards)** (Impact: 14.4)
    * *Intent:* /** * Prepares the SQL statement to be executed. * For complex SQL statement that is to be executed ...
  * `bindValues` **(Defensive Guards)** (Impact: 14.2)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **12**; blast radius 0.572; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.2%), Debt Markers (formerly Tech Debt) (61.9%), Complexity Load (formerly Cognitive Load) (35.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getClientOptions` **(Defensive Guards)** (Impact: 27.6)
    * *Intent:* /** * Returns the JS options for the field. * @return array the JS options. */
  * `radio` **(Defensive Guards)** (Impact: 13.4)
    * *Intent:* * When this option is specified, the radio button will be enclosed by a label tag. If you do not wan...
  * `checkbox` **(Defensive Guards)** (Impact: 13.4)
    * *Intent:* * When this option is specified, the checkbox will be enclosed by a label tag. If you do not want an...
  * `widget` **(Defensive Guards)** (Impact: 11.8)
    * *Intent:* * * ``` * $form->field($model, 'date')->widget(\yii\widgets\MaskedInput::class, [ * 'mask' => '99/99...
  * `render` **(Defensive Guards)** (Impact: 11.0)
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.572
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.00098
...
- `framework/console/Controller.php` -> Churn: **99.59%** | Cog Load: 47.8185% | Debt: 83.69%
- `framework/web/Controller.php` -> Churn: **93.75%** | Cog Load: 47.342% | Debt: 80.7708%
- `framework/base/Application.php` -> Churn: **88.61%** | Cog Load: 32.8461% | Debt: 95.1742%
- `framework/console/controllers/BaseMigrateController.php` -> Churn: **84.3%** | Cog Load: 38.1639% | Debt: 57.7924%
- `framework/console/controllers/MigrateController.php` -> Churn: **84.3%** | Cog Load: 35.8876% | Debt: 60.2685%
...
- `framework/base/Model.php` -> **Severity: 0.057** (Bridge: 0.0006 * Flux: 100.0%)
- `framework/db/Connection.php` -> **Severity: 0.04** (Bridge: 0.0004 * Flux: 100.0%)
- `framework/db/Query.php` -> **Severity: 0.033** (Bridge: 0.0003 * Flux: 100.0%)
- `framework/db/Migration.php` -> **Severity: 0.03** (Bridge: 0.0003 * Flux: 99.9369%)
- `framework/base/Component.php` -> **Severity: 0.028** (Bridge: 0.0003 * Flux: 99.9998%)
