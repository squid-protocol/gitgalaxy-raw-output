...
## 1. EXECUTIVE SUMMARY
- **Scope:** 626 analyzed artifact(s), 54328 LOC.
- **Load-bearing artifact:** `src/util/index.mjs` -- 85 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/collection/index.mjs` -- pulls in 21 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `documentation/template.html` at magnitude 2417.39 (structural weight, not risk).
...
| Avg Path Length | 2.2587 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `2.948`
> **Composition Archetype:** `Mid Flat Project` (z +2.95; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 44%, Callbacks & Closures Files 21%, Declarative / Non-Code 18%, Large Core Modules (3) 6%, Large Core Modules (2) 5%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 55.0 | 100.0 | 100.0 |
...
- `load` **(I/O & Config Routines)** (@ `src/extensions/renderer/base/load-listeners.mjs`) -> Impact: **525.7** | LOC: 1954
- `drawNode` **(Many-Argument Workhorses)** (@ `src/extensions/renderer/canvas/drawing-nodes.mjs`) -> Impact: **375.0** | LOC: 569
- `parseImpl` **(Many-Argument Workhorses)** (@ `src/style/parse.mjs`) -> Impact: **311.6** | LOC: 373
...
- `drawText` **(Many-Argument Workhorses)** (@ `src/extensions/renderer/canvas/drawing-label-text.mjs`) -> Impact: **192.4** | LOC: 223
- `getAllInBox` **(Many-Argument Workhorses)** (@ `src/extensions/renderer/base/coord-ele-math/coords.mjs`) -> Impact: **185.0** | LOC: 256
  * *Intent:* // 'Give me everything from this box'
- `render` **(Compute Cores)** (@ `src/extensions/renderer/canvas/drawing-redraw.mjs`) -> Impact: **171.7** | LOC: 352
- `makeEvent` **(Compute Cores)** (@ `src/extensions/renderer/base/load-listeners.mjs`) -> Impact: **169.9** | LOC: 429
- `findNearestElements` **(Many-Argument Workhorses)** (@ `src/extensions/renderer/base/coord-ele-math/coords.mjs`) -> Impact: **150.7** | LOC: 242
- `getElement` **(Many-Argument Workhorses)** (@ `src/extensions/renderer/canvas/ele-texture-cache.mjs`) -> Impact: **136.4** | LOC: 181
- `boundingBoxImpl` **(Many-Argument Workhorses)** (@ `src/collection/dimensions/bounds.mjs`) -> Impact: **134.8** | LOC: 306
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 1.14; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (95.4%), Guard Balance (formerly Safety Score) (70.7%), Complexity Load (formerly Cognitive Load) (29.9%), Connectivity (formerly Api Exposure) (11.9%)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 1.369; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (97.0%), Guard Balance (formerly Safety Score) (92.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `load` **(I/O & Config Routines)** (Impact: 525.7)
  * `makeEvent` **(Compute Cores)** (Impact: 169.9)
  * `mousemoveHandler` **(Compute Cores)** (Impact: 120.8)
  * `mouseupHandler` **(Compute Cores)** (Impact: 85.3)
  * `makeEvent` **(Compute Cores)** (Impact: 68.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 2.109; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.7%), Complexity Load (formerly Cognitive Load) (89.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `roundRectangleIntersectLine` **(Many-Argument Workhorses)** (Impact: 73.2)
  * `finiteLinesIntersect` **(Many-Argument Workhorses)** (Impact: 53.9)
    * *Intent:* // (x1,y1)=>(x2,y2) intersect with (x3,y3)=>(x4,y4)
  * `polygonIntersectLine` **(Many-Argument Workhorses)** (Impact: 36.1)
    * *Intent:* // math.polygonIntersectLine( x, y, basePoints, centerX, centerY, width, height, padding ) // inters...
  * `intersectLineCircle` **(Many-Argument Workhorses)** (Impact: 34.0)
    * *Intent:* // Returns intersections of increasing distance from line's start point
  * `median` **(Many-Argument Workhorses)** (Impact: 33.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 1.407; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.2%), Complexity Load (formerly Cognitive Load) (92.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `boundingBoxImpl` **(Many-Argument Workhorses)** (Impact: 134.8)
    * *Intent:* // get the bounding box of the elements (in raw model position)
  * `isDisplayed` **(Compute Cores)** (Impact: 109.3)
    * *Intent:* // must use `display` prop only, as reading `compound.width()` causes recursion // (other factors li...
  * `updateBoundsFromLabel` **(Many-Argument Workhorses)** (Impact: 68.8)
  * `updateCompoundBounds` **(Defensive Guards)** (Impact: 67.9)
  * `update` **(Defensive Guards)** (Impact: 55.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2e-06 | `Ripple Effect (Closeness):` 0.004126
  * `Imports (Out-Degree: 2):` is.mjs, math.mjs, index.mjs
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **5**; blast radius 1.256; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.2%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (78.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getKey` **(Many-Argument Workhorses)** (Impact: 106.2)
  * `findTaxiPoints` **(Many-Argument Workhorses)** (Impact: 95.3)
  * `findEdgeControlPoints` **(Compute Cores)** (Impact: 92.6)
  * `subDWH` **(Defensive Guards)** (Impact: 51.3)
    * *Intent:* // take away the effective w/h from the magnitude of the delta value
  * `tryToCorrectInvalidPoints` **(Many-Argument Workhorses)** (Impact: 34.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.4e-05 | `Ripple Effect (Closeness):` 0.002743
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 1.246; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (93.1%), Guard Balance (formerly Safety Score) (89.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `drawNode` **(Many-Argument Workhorses)** (Impact: 375.0)
  * `drawOutline` **(I/O & Config Routines)** (Impact: 68.3)
  * `drawPie` **(Many-Argument Workhorses)** (Impact: 42.1)
  * `drawStripe` **(Many-Argument Workhorses)** (Impact: 35.3)
  * `drawBorder` **(I/O & Config Routines)** (Impact: 22.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.002667
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 1.321; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (50.7%)
- **Documentation Coverage:** 40.5405% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createLayoutInfo` **(Many-Argument Workhorses)** (Impact: 49.0)
    * *Intent:* /** * used in the layout process */
  * `findClippingPoint` **(Many-Argument Workhorses)** (Impact: 42.1)
    * *Intent:* /** * the rectangular bounding box of it's source/target node */
  * `run` **(I/O & Config Routines)** (Impact: 28.0)
    * *Intent:* /** */
  * `findLCA_aux` **(Many-Argument Workhorses)** (Impact: 24.3)
    * *Intent:* /** * * * X is the number of ancestors (max: 2) found in * graphIx (and it's subgraphs), * Y is the ...
  * `nodeRepulsion` **(Many-Argument Workhorses)** (Impact: 23.8)
    * *Intent:* /** */
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8e-06 | `Ripple Effect (Closeness):` 0.00256
  * `Imports (Out-Degree: 2):` is.mjs, math.mjs, index.mjs
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **6**; blast radius 1.352; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.0%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (77.7%)
- **Documentation Coverage:** 45.4545% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_createShaderProgram` **(Compute Cores)** (Impact: 116.5)
  * `drawNode` **(Many-Argument Workhorses)** (Impact: 36.1)
    * *Intent:* /** * Draw a node using either a texture or a "simple shape". */
  * `drawTexture` **(Many-Argument Workhorses)** (Impact: 33.4)
    * *Intent:* /** * Draws a texture using the texture atlas. */
  * `drawEdgeArrow` **(Many-Argument Workhorses)** (Impact: 26.9)
    * *Intent:* /** * Only supports drawing triangles at the moment. */
  * `drawEdgeLine` **(Many-Argument Workhorses)** (Impact: 23.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.1e-05 | `Ripple Effect (Closeness):` 0.002743
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 1.309; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.6%), Complexity Load (formerly Cognitive Load) (96.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `applyParsedProperty` **(Many-Argument Workhorses)** (Impact: 125.4)
    * *Intent:* // // now, this function flattens the property, and here's how: // // for parsedProp:{ bypass: true,...
  * `updateStyleHints` **(Defensive Guards)** (Impact: 46.9)
  * `updateTransitions` **(Many-Argument Workhorses)** (Impact: 44.4)
    * *Intent:* // diffProps : { name => { prev, next } }
  * `cleanNum` **(Defensive Guards)** (Impact: 40.3)
  * `getPropertiesDiff` **(Defensive Guards)** (Impact: 36.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8e-06 | `Ripple Effect (Closeness):` 0.0032
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.246; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (95.9%), Guard Balance (formerly Safety Score) (95.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `render` **(Compute Cores)** (Impact: 171.7)
  * `createGradientStyleFor` **(Many-Argument Workhorses)** (Impact: 64.9)
  * `drawSelectionRectangle` **(Many-Argument Workhorses)** (Impact: 32.3)
  * `setContextTransform` **(Defensive Guards)** (Impact: 24.5)
  * `matchCanvasSize` **(Defensive Guards)** (Impact: 14.5)
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **21**; blast radius 2.223; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Complexity Load (formerly Cognitive Load) (94.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `restore` **(Many-Argument Workhorses)** (Impact: 103.0)
  * `remove` **(Many-Argument Workhorses)** (Impact: 56.5)
  * `Collection` **(Many-Argument Workhorses)** (Impact: 56.3)
    * *Intent:* // represents a set of nodes, edges, or both together
  * `json` **(Compute Cores)** (Impact: 45.4)
  * `toString` **(Defensive Guards)** (Impact: 32.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000571 | `Ripple Effect (Closeness):` 0.006667
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.256; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (93.1%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (79.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getAllInBox` **(Many-Argument Workhorses)** (Impact: 185.0)
    * *Intent:* // 'Give me everything from this box'
  * `findNearestElements` **(Many-Argument Workhorses)** (Impact: 150.7)
  * `checkEdge` **(Defensive Guards)** (Impact: 34.9)
  * `addEle` **(Defensive Guards)** (Impact: 27.6)
  * `checkLabel` **(Compute Cores)** (Impact: 20.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.245; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (78.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getFitViewport` **(Compute Cores)** (Impact: 44.6)
  * `getZoomedViewport` **(Compute Cores)** (Impact: 31.5)
  * `viewport` **(Compute Cores)** (Impact: 29.5)
  * `zoomRange` **(Defensive Guards)** (Impact: 23.6)
  * `panBy` **(Compute Cores)** (Impact: 23.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **5**; blast radius 1.246; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getLayers` **(Many-Argument Workhorses)** (Impact: 91.6)
  * `drawEleInLayer` **(Defensive Guards)** (Impact: 18.9)
  * `dequeue` **(Defensive Guards)** (Impact: 18.8)
  * `updateElementsInLayers` **(Compute Cores)** (Impact: 16.9)
  * `validateLayersElesOrdering` **(Defensive Guards)** (Impact: 15.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.246
  * `Choke Point (Betweenness):` 1e-05 | `Ripple Effect (Closeness):` 0.002667
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.196; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (70.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `kMeans` **(Defensive Guards)** (Impact: 25.4)
  * `kMedoids` **(Defensive Guards)** (Impact: 23.4)
  * `fuzzyCMeans` **(Compute Cores)** (Impact: 19.2)
  * `updateCentroids` **(Many-Argument Workhorses)** (Impact: 15.7)
  * `assign` **(Many-Argument Workhorses)** (Impact: 14.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 109 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6e-06 | `Ripple Effect (Closeness):` 0.004126
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **3**; blast radius 1.741; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (78.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getLabelText` **(Many-Argument Workhorses)** (Impact: 58.1)
  * `recalculateEdgeLabelProjections` **(Compute Cores)** (Impact: 56.8)
  * `calculateEndProjection` **(Compute Cores)** (Impact: 36.9)
  * `recalculateNodeLabelProjection` **(Compute Cores)** (Impact: 16.6)
  * `calculateLabelAngle` **(Defensive Guards)** (Impact: 16.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.2e-05 | `Ripple Effect (Closeness):` 0.003564
  * `Imports (Out-Degree: 2):` is.mjs, math.mjs, index.mjs
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 1.14; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (84.3%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (67.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseImpl` **(Many-Argument Workhorses)** (Impact: 311.6)
    * *Intent:* // parse a property; return null on invalid; return parsed property otherwise // fields : // - name ...
  * `parse` **(Defensive Guards)** (Impact: 28.4)
    * *Intent:* // a caching layer for property parsing
  * `parseImplWarn` **(Defensive Guards)** (Impact: 18.5)
  * `checkEnums` **(Callbacks & Closures)** (Impact: 3.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 1.14; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (90.4%), Mutation Surface (formerly State Flux) (76.3%), Debt Markers (formerly Tech Debt) (64.7%)
- **Documentation Coverage:** 25.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createDefaultTodos` **(Compute Cores)** (Impact: 4.7)
  * `checkNumberOfTodosInLocalStorage` **(Callbacks & Closures)** (Impact: 2.0)
    * *Intent:* /** */
  * `checkNumberOfCompletedTodosInLocalStorage` **(Callbacks & Closures)** (Impact: 2.0)
    * *Intent:* /** */
  * `checkTodosInLocalStorage` **(Callbacks & Closures)** (Impact: 2.0)
    * *Intent:* /** */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **5**; blast radius 1.458; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.6%), Complexity Load (formerly Cognitive Load) (91.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getElement` **(Many-Argument Workhorses)** (Impact: 136.4)
  * `invalidateElement` **(Compute Cores)** (Impact: 10.4)
  * `shouldRedraw` **(Many-Argument Workhorses)** (Impact: 9.7)
  * `dequeue` **(Compute Cores)** (Impact: 8.7)
  * `removeFromQueue` **(Defensive Guards)** (Impact: 6.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4.9e-05 | `Ripple Effect (Closeness):` 0.0036
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.246; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (95.2%), Guard Balance (formerly Safety Score) (89.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `drawText` **(Many-Argument Workhorses)** (Impact: 192.4)
  * `drawElementText` **(Many-Argument Workhorses)** (Impact: 61.4)
  * `getTextAngle` **(Defensive Guards)** (Impact: 13.0)
  * `getFontCache` **(Defensive Guards)** (Impact: 6.7)
  * `eleTextBiggerThanMin` **(Compute Cores)** (Impact: 6.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 1.14; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (73.7%), Debt Markers (formerly Tech Debt) (41.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `checkPoint` **(Many-Argument Workhorses)** (Impact: 48.5)
  * `checkPoint` **(Many-Argument Workhorses)** (Impact: 27.1)
  * `checkPoint` **(Many-Argument Workhorses)** (Impact: 23.6)
  * `generateBarrel` **(Callbacks & Closures)** (Impact: 22.4)
  * `checkPoint` **(Many-Argument Workhorses)** (Impact: 21.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.246; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (85.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (74.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `drawArrowShape` **(Many-Argument Workhorses)** (Impact: 73.0)
  * `drawEdgePath` **(Many-Argument Workhorses)** (Impact: 71.5)
  * `drawEdge` **(Many-Argument Workhorses)** (Impact: 59.7)
  * `drawArrowhead` **(Many-Argument Workhorses)** (Impact: 41.8)
  * `drawEdgeOverlayUnderlay` **(Defensive Guards)** (Impact: 11.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **2**; blast radius 2.396; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.8%), Complexity Load (formerly Cognitive Load) (83.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 93.8462% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Many-Argument Workhorses)** (Impact: 34.2)
  * `getOrCreateAtlas` **(Many-Argument Workhorses)** (Impact: 17.3)
  * `gc` **(I/O & Config Routines)** (Impact: 11.6)
  * `draw` **(Compute Cores)** (Impact: 10.9)
  * `moveToStartOfNextRow` **(I/O & Config Routines)** (Impact: 10.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.3e-05 | `Ripple Effect (Closeness):` 0.003531
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 1.196; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `affinityPropagation` **(Compute Cores)** (Impact: 58.3)
  * `getPreference` **(Defensive Guards)** (Impact: 16.4)
  * `assign` **(Many-Argument Workhorses)** (Impact: 15.7)
  * `assignClusters` **(Compute Cores)** (Impact: 13.3)
  * `setOptions` **(Defensive Guards)** (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.196
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.004126
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **2**; blast radius 1.196; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (76.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `mergeClosest` **(Many-Argument Workhorses)** (Impact: 71.1)
  * `getDist` **(Defensive Guards)** (Impact: 51.5)
  * `buildClustersFromTree` **(Many-Argument Workhorses)** (Impact: 34.2)
  * `getDist` **(Defensive Guards)** (Impact: 29.6)
  * `hierarchicalClustering` **(Defensive Guards)** (Impact: 23.7)
...
- `src/util/index.mjs` -> **Severity: 0.261** (Bridge: 0.0026 * Flux: 100.0%)
- `src/collection/index.mjs` -> **Severity: 0.057** (Bridge: 0.0006 * Flux: 100.0%)
- `src/selector/index.mjs` -> **Severity: 0.023** (Bridge: 0.0002 * Flux: 100.0%)
- `src/extensions/renderer/canvas/index.mjs` -> **Severity: 0.022** (Bridge: 0.0002 * Flux: 99.9999%)
- `src/extension.mjs` -> **Severity: 0.017** (Bridge: 0.0002 * Flux: 99.9994%)
