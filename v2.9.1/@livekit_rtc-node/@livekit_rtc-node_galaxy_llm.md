...
## 1. EXECUTIVE SUMMARY
- **Scope:** 38 analyzed artifact(s), 4497 LOC.
- **Load-bearing artifact:** `package/src/napi/native.js` -- 3 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `package/src/napi/native.cjs` -- pulls in 39 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `package/src/room.ts` at magnitude 608.34 (structural weight, not risk).
...
| Avg Path Length | 1.4286 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.256`
> **Composition Archetype:** `Small Flat Repo` (z +1.26; from the repo's file-archetype mix)
> **File Composition:** Large Core Modules (3) 26%, Large Core Modules (2) 16%, Data / Markup / Trivial 13%, Declarative / Non-Code 13%, Generic / Templated Code Files 13%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 94.2 | 46.4 | 59.8 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 59.7 | 73.0 | 100.0 |
...
- `processFfiEvent` **(Defensive Guards)** (@ `package/src/room.ts`) -> Impact: **212.9** | LOC: 298
- `streamBytes` **(Defensive Guards)** (@ `package/src/participant.ts`) -> Impact: **41.7** | LOC: 99
- `handleStreamHeader` **(Defensive Guards)** (@ `package/src/room.ts`) -> Impact: **41.6** | LOC: 70
- `constructor` **(Many-Argument Workhorses)** (@ `package/src/audio_stream.ts`) -> Impact: **38.2** | LOC: 44
- `connect` **(Many-Argument Workhorses)** (@ `package/src/room.ts`) -> Impact: **34.8** | LOC: 56
  * *Intent:* /** * Connects to a LiveKit room using the provided URL and access token. */
- `streamText` **(Defensive Guards)** (@ `package/src/participant.ts`) -> Impact: **32.6** | LOC: 87
...
- `handleRpcMethodInvocation` **(Many-Argument Workhorses)** (@ `package/src/participant.ts`) -> Impact: **31.4** | LOC: 46
- `editChatMessage` **(Many-Argument Workhorses)** (@ `package/src/participant.ts`) -> Impact: **28.9** | LOC: 42
  * *Intent:* /** */
- `getPlaneLength` **(Compute Cores)** (@ `package/src/video_frame.ts`) -> Impact: **27.3** | LOC: 26
- `getPlaneInfos` **(Many-Argument Workhorses)** (@ `package/src/video_frame.ts`) -> Impact: **25.2** | LOC: 101
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (96.3%), Guard Balance (formerly Safety Score) (91.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (61.7%)
- **Documentation Coverage:** 67.3469% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `processFfiEvent` **(Defensive Guards)** (Impact: 212.9)
  * `handleStreamHeader` **(Defensive Guards)** (Impact: 41.6)
  * `connect` **(Many-Argument Workhorses)** (Impact: 34.8)
    * *Intent:* /** * Connects to a LiveKit room using the provided URL and access token. */
  * `start` **(Defensive Guards)** (Impact: 20.1)
  * `requireParticipantByIdentity` **(Stateful Encapsulated Methods)** (Impact: 8.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.8%), Test Surface (formerly Verification) (80.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Guard Balance (formerly Safety Score) (48.2%)
- **Documentation Coverage:** 70.9091% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `streamBytes` **(Defensive Guards)** (Impact: 41.7)
  * `streamText` **(Defensive Guards)** (Impact: 32.6)
    * *Intent:* /** * Returns a `StreamWriter` instance that allows to write individual chunks of text to a stream. ...
  * `handleRpcMethodInvocation` **(Many-Argument Workhorses)** (Impact: 31.4)
  * `editChatMessage` **(Many-Argument Workhorses)** (Impact: 28.9)
    * *Intent:* /** */
  * `sendChatMessage` **(Many-Argument Workhorses)** (Impact: 23.7)
    * *Intent:* /** * Sends a chat message to participants in the room * * */
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (89.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 35.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getContribution` **(Compute Cores)** (Impact: 25.1)
  * `mixAudio` **(Stateful Encapsulated Methods)** (Impact: 21.5)
  * `mixer` **(I/O & Config Routines)** (Impact: 20.4)
  * `constructor` **(Defensive Guards)** (Impact: 14.8)
    * *Intent:* /** * Initialize the AudioMixer. * */
  * `getNextFrame` **(Stateful Encapsulated Methods)** (Impact: 6.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **39**; blast radius 92.211; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.2%), Complexity Load (formerly Cognitive Load) (74.5%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `isMusl` **(Defensive Guards)** (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (7.9%), Complexity Load (formerly Cognitive Load) (5.8%)
- **Documentation Coverage:** 77.193% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fromBinary` **(Generic / Templated Code)** (Impact: 3.6)
  * `fromJson` **(Generic / Templated Code)** (Impact: 3.6)
  * `fromJsonString` **(Generic / Templated Code)** (Impact: 3.6)
  * `fromBinary` **(Generic / Templated Code)** (Impact: 3.6)
  * `fromJson` **(Generic / Templated Code)** (Impact: 3.6)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (79.5%), Complexity Load (formerly Cognitive Load) (64.6%)
- **Documentation Coverage:** 73.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handleChunkReceived` **(Stateful Encapsulated Methods)** (Impact: 7.7)
  * `handleChunkReceived` **(Stateful Encapsulated Methods)** (Impact: 4.6)
  * `[Symbol.asyncIterator]` **(Defensive Guards)** (Impact: 4.5)
    * *Intent:* /** * Async iterator implementation to allow usage of `for await...of` syntax. * Yields structured c...
  * `constructor` **(Generic / Templated Code)** (Impact: 4.4)
    * *Intent:* /** * A TextStreamReader instance can be used as an AsyncIterator that returns the entire string * t...
  * `constructor` **(Generic / Templated Code)** (Impact: 4.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (88.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (74.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 95.2381% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Defensive Guards)** (Impact: 7.4)
  * `exportKey` **(Type Conversions)** (Impact: 6.2)
  * `ratchetKey` **(Type Conversions)** (Impact: 6.2)
  * `setSharedKey` **(State Mutators)** (Impact: 6.1)
  * `setKey` **(State Mutators)** (Impact: 6.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 8 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 22.953; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.6%), Connectivity (formerly Api Exposure) (50.4%)
- **Documentation Coverage:** 85.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `captureFrame` **(Compute Cores)** (Impact: 14.7)
  * `constructor` **(Many-Argument Workhorses)** (Impact: 5.2)
  * `clearQueue` **(I/O & Config Routines)** (Impact: 2.8)
  * `release` **(Callbacks & Closures)** (Impact: 2.5)
  * `waitForPlayout` **(Callbacks & Closures)** (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 6 instances
* *Amplified Cascading Flux:* 9 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (79.2%), Complexity Load (formerly Cognitive Load) (73.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Many-Argument Workhorses)** (Impact: 38.2)
  * `onEvent` **(Defensive Guards)** (Impact: 18.6)
  * `constructor` **(State Mutators)** (Impact: 6.3)
  * `constructor` **(State Mutators)** (Impact: 2.0)
  * `constructor` **(State Mutators)** (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 9 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (91.8%), Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (65.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 72.7273% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getPlaneLength` **(Compute Cores)** (Impact: 27.3)
  * `getPlaneInfos` **(Many-Argument Workhorses)** (Impact: 25.2)
  * `convert` **(Compute Cores)** (Impact: 13.2)
  * `protoInfo` **(I/O & Config Routines)** (Impact: 9.3)
  * `getPlane` **(Parameter Forwarders)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (7.4%), Complexity Load (formerly Cognitive Load) (4.9%)
- **Documentation Coverage:** 72.7273% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fromBinary` **(Generic / Templated Code)** (Impact: 3.6)
  * `fromJson` **(Generic / Templated Code)** (Impact: 3.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Concurrency Surface (formerly Concurrency) (92.8%), Mutation Surface (formerly State Flux) (70.4%), Connectivity (formerly Api Exposure) (69.9%)
- **Documentation Coverage:** 95.6522% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `close` **(Defensive Guards)** (Impact: 4.5)
  * `close` **(Defensive Guards)** (Impact: 4.5)
  * `createAudioTrack` **(Generic / Templated Code)** (Impact: 4.1)
  * `createVideoTrack` **(Generic / Templated Code)** (Impact: 4.1)
  * `constructor` **(State Mutators)** (Impact: 3.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 22.953; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (99.5%), Mutation Surface (formerly State Flux) (71.0%), Connectivity (formerly Api Exposure) (67.9%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 89.6552% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `setSubscribed` **(State Mutators)** (Impact: 3.3)
  * `resolveFirstSubscription` **(Interface Declarations)** (Impact: 2.3)
  * `sid` **(Defensive Guards)** (Impact: 2.1)
  * `name` **(Defensive Guards)** (Impact: 2.1)
  * `kind` **(Defensive Guards)** (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **25**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (16.3%), Test Surface (formerly Verification) (2.3%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Concurrency Surface (formerly Concurrency) (99.6%), Guard Balance (formerly Safety Score) (84.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 80.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `put` **(Compute Cores)** (Impact: 8.0)
  * `waitForItem` **(Interface Declarations)** (Impact: 3.3)
    * *Intent:* /** * Wait until an item is available or the queue is closed. * Returns immediately if items are alr...
  * `get` **(Interface Declarations)** (Impact: 2.4)
  * `close` **(Interface Declarations)** (Impact: 1.6)
  * `constructor` **(Encapsulated Accessors)** (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (63.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (39.2%), Complexity Load (formerly Cognitive Load) (4.1%)
- **Documentation Coverage:** 33.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Many-Argument Workhorses)** (Impact: 17.5)
    * *Intent:* #inputRate: number; #outputRate: number; #channels: number; #ffiHandle: FfiHandle; /** * Initializes...
  * `push` **(Compute Cores)** (Impact: 7.3)
    * *Intent:* /** * Push audio data into the resampler and retrieve any available resampled data. * * This method ...
  * `flush` **(I/O & Config Routines)** (Impact: 5.5)
    * *Intent:* /** * Flush any remaining audio data through the resampler and retrieve the resampled data. * * This...
  * `inputRate` **(Interface Declarations)** (Impact: 1.1)
  * `outputRate` **(Interface Declarations)** (Impact: 1.1)
**Structural Signatures (Net Mitigated Signals):**
...
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Debt Markers (formerly Tech Debt) (98.2%), Guard Balance (formerly Safety Score) (76.2%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 33.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `splitUtf8` **(Compute Cores)** (Impact: 13.3)
  * `bigIntToNumber` **(Generic / Templated Code)** (Impact: 5.9)
    * *Intent:* // SPDX-FileCopyrightText: 2024 LiveKit, Inc. // // SPDX-License-Identifier: Apache-2.0 /** convert ...
  * `numberToBigInt` **(Generic / Templated Code)** (Impact: 5.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Concurrency Surface (formerly Concurrency) (97.3%), Guard Balance (formerly Safety Score) (83.0%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `captureFrame` **(Many-Argument Workhorses)** (Impact: 7.0)
  * `constructor` **(Defensive Guards)** (Impact: 6.3)
  * `close` **(State Mutators)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
* *Amplified Cascading Flux:* 4 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (6.1%), Complexity Load (formerly Cognitive Load) (3.8%), Test Surface (formerly Verification) (2.5%)
- **Documentation Coverage:** 80.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fromBinary` **(Generic / Templated Code)** (Impact: 3.6)
  * `fromJson` **(Generic / Templated Code)** (Impact: 3.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.4%), Guard Balance (formerly Safety Score) (91.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Connectivity (formerly Api Exposure) (49.4%)
- **Documentation Coverage:** 36.3636% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `combineAudioFrames` **(Defensive Guards)** (Impact: 9.9)
    * *Intent:* /** * Combines one or more `rtc.AudioFrame` objects into a single `rtc.AudioFrame`. * * This functio...
  * `constructor` **(Generic / Templated Code)** (Impact: 3.1)
    * *Intent:* // note: if converting from Uint8Array to Int16Array, *do not* use buffer.slice! // it is marked uns...
  * `create` **(Parameter Forwarders)** (Impact: 2.2)
  * `fromOwnedInfo` **(Parameter Forwarders)** (Impact: 2.0)
  * `protoInfo` **(Interface Declarations)** (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.8%), Guard Balance (formerly Safety Score) (83.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Concurrency Surface (formerly Concurrency) (30.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `livekit_ffi_request` **(Compute Cores)** (Impact: 5.5)
  * `livekit_initialize` **(Callbacks & Closures)** (Impact: 5.2)
  * `dispose` **(Compute Cores)** (Impact: 4.9)
  * `livekit_copy_buffer` **(Type Conversions)** (Impact: 2.0)
  * `finalize` **(Parameter Forwarders)** (Impact: 1.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 22.953; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (86.5%), Concurrency Surface (formerly Concurrency) (65.4%), Guard Balance (formerly Safety Score) (58.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 85.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `waitFor` **(Generic / Templated Code)** (Impact: 3.4)
  * `listener` **(Callbacks & Closures)** (Impact: 3.3)
  * `instance` **(Interface Declarations)** (Impact: 2.3)
  * `copyBuffer` **(Parameter Forwarders)** (Impact: 1.9)
  * `request` **(Generic / Templated Code)** (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (82.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%), Mutation Surface (formerly State Flux) (45.6%), Connectivity (formerly Api Exposure) (23.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `onEvent` **(Compute Cores)** (Impact: 14.3)
  * `constructor` **(Generic / Templated Code)** (Impact: 3.6)
  * `start` **(Generic / Templated Code)** (Impact: 1.6)
  * `constructor` **(State Mutators)** (Impact: 1.6)
  * `cancel` **(State Mutators)** (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 4
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Concurrency Surface (formerly Concurrency) (76.9%), Guard Balance (formerly Safety Score) (73.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(Generic / Templated Code)** (Impact: 4.3)
  * `close` **(Defensive Guards)** (Impact: 2.2)
  * `write` **(Parameter Forwarders)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
* *State Mutation (weighted view):* 12
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 22.953; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (94.9%), Guard Balance (formerly Safety Score) (74.4%), Connectivity (formerly Api Exposure) (52.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (50.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `constructor` **(State Mutators)** (Impact: 4.3)
    * *Intent:* /** * Creates an error object with the given code and message, plus an optional data payload. * * If...
  * `builtIn` **(Generic / Templated Code)** (Impact: 3.6)
    * *Intent:* /** * Creates an error object from the code, with an auto-populated message. * */
  * `fromProto` **(Parameter Forwarders)** (Impact: 1.6)
  * `toProto` **(Type Conversions)** (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
