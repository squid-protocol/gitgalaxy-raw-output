...
## 1. EXECUTIVE SUMMARY
- **Scope:** 2501 analyzed artifact(s), 173011 LOC.
- **Load-bearing artifact:** `core/src/css/core.scss` -- 383 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `packages/react/src/components/proxies.ts` -- pulls in 74 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `core/src/components/range/test/custom/range.e2e.ts` at magnitude 5770.4 (structural weight, not risk).
...
|---|---|---|
| Modularity | 0.7729 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 2.42 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `7.149`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +7.15; from the repo's file-archetype mix)
> **File Composition:** Declarative / Non-Code 36%, Data / Markup / Trivial 34%, Callbacks & Closures Files 8%, State Mutators Files 4%, Large Core Modules (3) 4%
...
- `.map`: 3x Unresolved Ambiguity (No Retainable Structure), 1x Excluded (Saturation: Line 1 exceeds 500 chars)
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 22.4 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 3.1 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 2.3 | 0.0 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 12.7 | 4.6 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 85.0 | 1.1 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 33.2 | 0.0 | 0.0 |
...
| connectivity | 8253 | 1447 | 8 | `packages/react/test/base/src/theme/variables.css` |
...
| dead_code | 287 | 142 | 0 | `core/src/components/menu/menu.tsx` |
| credential | 0 | 0 | 0 | - |
...
- `handleHistoryChange` **(Many-Argument Workhorses)** (@ `packages/vue-router/src/router.ts`) -> Impact: **150.7** | LOC: 285
  * *Intent:* // TODO RouteLocationNormalized
- `iosTransitionAnimation` **(Many-Argument Workhorses)** (@ `core/src/utils/transition/ios.transition.ts`) -> Impact: **131.6** | LOC: 381
- `renderMonth` **(Many-Argument Workhorses)** (@ `core/src/components/datetime/datetime.tsx`) -> Impact: **90.5** | LOC: 216
- `setup` **(I/O & Config Routines)** (@ `packages/vue/src/components/IonRouterOutlet.ts`) -> Impact: **89.5** | LOC: 510
- `generateTime` **(Many-Argument Workhorses)** (@ `core/src/components/datetime/utils/data.ts`) -> Impact: **85.5** | LOC: 127
...
- `getClosestValidDate` **(Compute Cores)** (@ `core/src/components/datetime/utils/manipulation.ts`) -> Impact: **75.2** | LOC: 146
  * *Intent:* /** * Returns the closest date to refParts * that also meets the constraints of * the *Values params. */
- `handleHistoryChange` **(Many-Argument Workhorses)** (@ `packages/react-router/src/ReactRouter/IonRouter.tsx`) -> Impact: **75.0** | LOC: 114
- `transitionPage` **(Many-Argument Workhorses)** (@ `packages/react-router/src/ReactRouter/StackManager.tsx`) -> Impact: **67.5** | LOC: 77
- `attachView` **(Many-Argument Workhorses)** (@ `packages/angular/common/src/providers/angular-delegate.ts`) -> Impact: **66.1** | LOC: 124
- `calculateWindowAdjustment` **(Many-Argument Workhorses)** (@ `core/src/components/popover/utils.ts`) -> Impact: **58.2** | LOC: 124
  * *Intent:* /** * Adjusts popover positioning coordinates * such that popover does not appear offscreen * or overlapping safe area bounds. * */
...
- `core/src/components/item-options/item-options.tsx` -> **99.6827%** Exposure
...
- `core/src/components/menu/menu.tsx` -> **10** Orphaned Functions | **0** Duplicates
...
- `core/src/components/header/header.tsx` -> **6** Orphaned Functions | **0** Duplicates
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (44.8%), Guard Balance (formerly Safety Score) (16.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (11.7%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (26.6%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (96.8%), Complexity Load (formerly Cognitive Load) (89.1%), Guard Balance (formerly Safety Score) (47.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (72.2%), Guard Balance (formerly Safety Score) (66.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (56.0%)
- **Documentation Coverage:** 55.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `renderMonth` **(Many-Argument Workhorses)** (Impact: 90.5)
  * `processValue` **(Compute Cores)** (Impact: 35.8)
  * `destroyKeyboardMO` **(I/O & Config Routines)** (Impact: 21.1)
  * `renderDatetime` **(Stateful Encapsulated Methods)** (Impact: 20.6)
    * *Intent:* /** * Render entry point * All presentation types are rendered from here. */
  * `setActiveParts` **(Compute Cores)** (Impact: 17.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (86.4%), Guard Balance (formerly Safety Score) (22.6%), Mutation Surface (formerly State Flux) (15.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (23.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (10.9%), Connectivity (formerly Api Exposure) (8.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (60.4%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (89.4%), Guard Balance (formerly Safety Score) (25.4%), Mutation Surface (formerly State Flux) (16.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (48.6%), Complexity Load (formerly Cognitive Load) (31.8%), Mutation Surface (formerly State Flux) (12.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (80.6%), Complexity Load (formerly Cognitive Load) (40.9%)
- **Documentation Coverage:** 45.4545% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `postViewInit` **(Many-Argument Workhorses)** (Impact: 53.9)
    * *Intent:* /** * Adds and Removes the views from the navigation stack. * */
  * `prepareTI` **(Compute Cores)** (Impact: 31.2)
  * `setRouteId` **(Many-Argument Workhorses)** (Impact: 25.5)
    * *Intent:* /** * Called by the router to update the view. * * */
  * `queueTrns` **(Stateful Encapsulated Methods)** (Impact: 24.8)
    * *Intent:* /** * Adds a navigation stack change to the queue and schedules it to run. * */
  * `runTransition` **(Stateful Encapsulated Methods)** (Impact: 24.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **25**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (77.5%), Complexity Load (formerly Cognitive Load) (37.9%)
- **Documentation Coverage:** 51.7241% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dismiss` **(Many-Argument Workhorses)** (Impact: 32.3)
    * *Intent:* /** * Dismiss the modal overlay after it has been presented. * This is a no-op if the overlay has no...
  * `present` **(I/O & Config Routines)** (Impact: 20.5)
    * *Intent:* /** * Present the modal overlay after it has been created. */
  * `initParentRemovalObserver` **(I/O & Config Routines)** (Impact: 20.1)
  * `render` **(I/O & Config Routines)** (Impact: 17.1)
  * `setupChildRoutePassthrough` **(Defensive Guards)** (Impact: 16.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (76.2%), Guard Balance (formerly Safety Score) (30.9%), Mutation Surface (formerly State Flux) (17.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 4 instances
* *Amplified Cascading Flux:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (20.0%), Guard Balance (formerly Safety Score) (17.7%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `simulatePaste` **(Callbacks & Closures)** (Impact: 4.1)
    * *Intent:* /** * Simulates a paste event in an input element with the given value */
  * `verifyInputValues` **(Parameter Forwarders)** (Impact: 4.0)
    * *Intent:* /** * Helper function to verify input values in both the input * boxes and the input-otp component's...
  * `simulateAutofill` **(Type Conversions)** (Impact: 2.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (53.3%), Mutation Surface (formerly State Flux) (21.1%), Connectivity (formerly Api Exposure) (8.2%), Complexity Load (formerly Cognitive Load) (6.2%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_4` (Drift: 0.0 IQR)
- **Magnitude:** 589.8 | **LOC:** 928 | **CtrlFlow:** 21.1% | **Authorship Centralization:** 100.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.5%), Complexity Load (formerly Cognitive Load) (46.7%)
- **Documentation Coverage:** 83.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `setupiOSNativeRefresher` **(Many-Argument Workhorses)** (Impact: 48.9)
  * `setupMDNativeRefresher` **(Many-Argument Workhorses)** (Impact: 33.2)
  * `onMove` **(Compute Cores)** (Impact: 27.8)
  * `scrollListenerCallback` **(I/O & Config Routines)** (Impact: 21.6)
  * `setCss` **(Many-Argument Workhorses)** (Impact: 18.8)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `safety_bypasses: 16`, `state_mutation: 79`, `dead_code: 4`, `planned_debt: 1`, `unreferenced_by_name: 5`
* *Architecture:* `api: 1`, `concurrency: 22`, `import: 14`
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (52.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (9.4%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (82.1%), Concurrency Surface (formerly Concurrency) (37.5%), Complexity Load (formerly Cognitive Load) (26.6%)
- **Documentation Coverage:** 57.1429% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `knobStyle` **(I/O & Config Routines)** (Impact: 52.8)
  * `tickStyle` **(Compute Cores)** (Impact: 27.1)
  * `renderRangeSlider` **(I/O & Config Routines)** (Impact: 25.1)
  * `render` **(I/O & Config Routines)** (Impact: 23.1)
  * `getKnobPosition` **(Compute Cores)** (Impact: 16.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Connectivity (formerly Api Exposure) (69.9%), Guard Balance (formerly Safety Score) (66.5%), Concurrency Surface (formerly Concurrency) (50.7%)
- **Documentation Coverage:** 25.5319% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `calculateWindowAdjustment` **(Many-Argument Workhorses)** (Impact: 58.2)
    * *Intent:* /** * Adjusts popover positioning coordinates * such that popover does not appear offscreen * or ove...
  * `getPopoverPosition` **(Many-Argument Workhorses)** (Impact: 54.9)
    * *Intent:* /** * Positions a popover by taking into account * the reference point, preferred side, alignment * ...
  * `calculateArrowPosition` **(Many-Argument Workhorses)** (Impact: 35.6)
    * *Intent:* /** * Calculates where the arrow positioning * should be relative to the popover content. */
  * `callback` **(Compute Cores)** (Impact: 35.4)
  * `calculatePopoverSide` **(Many-Argument Workhorses)** (Impact: 30.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (99.0%), Guard Balance (formerly Safety Score) (35.1%), Mutation Surface (formerly State Flux) (22.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (38.6%), Guard Balance (formerly Safety Score) (23.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 7 instances
* *Amplified Cascading Flux:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (57.5%), Connectivity (formerly Api Exposure) (6.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (60.8%), Connectivity (formerly Api Exposure) (6.1%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 7`, `structural_boundaries: 64`, `args: 22`, `func_start: 14`
* *Risk/State:* None
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.205; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (87.6%), Guard Balance (formerly Safety Score) (27.3%), Mutation Surface (formerly State Flux) (18.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **39** in-repo importer(s); it depends on **4**; blast radius 3.159; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (85.9%), Complexity Load (formerly Cognitive Load) (35.0%), Connectivity (formerly Api Exposure) (28.9%)
- **Documentation Coverage:** 78.8732% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `progressEnd` **(Many-Argument Workhorses)** (Impact: 30.4)
  * `play` **(Callbacks & Closures)** (Impact: 11.6)
  * `createAnimation` **(I/O & Config Routines)** (Impact: 11.3)
  * `addElement` **(Type Conversions)** (Impact: 10.7)
  * `from` **(Type Conversions)** (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 3.159
  * `Choke Point (Betweenness):` 1.3e-05 | `Ripple Effect (Closeness):` 0.01554
...
- `core/src/components/refresher/refresher.tsx` -> **Shane** (100.0% isolated ownership) | Magnitude: 589.8
...
- `core/src/global/ionic-global.ts` -> **Severity: 0.005** (Bridge: 0.0001 * Flux: 100.0%)
- `core/src/components/modal/gestures/swipe-to-close.ts` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
- `core/src/utils/animation/animation.ts` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
- `core/src/utils/platform.ts` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
- `packages/angular/common/src/directives/navigation/router-outlet.ts` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `core/scripts/testing/scripts.js` -> **Severity: 9.658** (Embedded: 0.1369 * Error Risk: 70.5637%)
- `core/src/components/router/utils/interface.ts` -> **Severity: 3.815** (Embedded: 0.0587 * Error Risk: 64.946%)
- `core/src/global/ionic-global.ts` -> **Severity: 3.619** (Embedded: 0.0379 * Error Risk: 95.4304%)
- `core/src/utils/input-shims/hacks/common.ts` -> **Severity: 3.562** (Embedded: 0.0368 * Error Risk: 96.7705%)
- `core/src/utils/platform.ts` -> **Severity: 1.551** (Embedded: 0.0196 * Error Risk: 79.3283%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `core/src/components/router/utils/interface.ts` -> **Severity: 1921.2** (Blast Radius: 19.212 * Doc Risk: 100.0%)
- `core/src/utils/input-shims/hacks/common.ts` -> **Severity: 743.25** (Blast Radius: 8.919 * Doc Risk: 83.3333%)
- `core/src/global/ionic-global.ts` -> **Severity: 445.1** (Blast Radius: 4.451 * Doc Risk: 100.0%)
- `core/src/global/config.ts` -> **Severity: 372.7** (Blast Radius: 3.727 * Doc Risk: 100.0%)
- `core/src/utils/animation/animation.ts` -> **Severity: 249.16** (Blast Radius: 3.159 * Doc Risk: 78.8732%)
