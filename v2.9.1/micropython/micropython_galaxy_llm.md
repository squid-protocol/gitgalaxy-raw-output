...
## 1. EXECUTIVE SUMMARY
- **Scope:** 4989 analyzed artifact(s), 419648 LOC.
- **Load-bearing artifact:** `py/runtime.h` -- 476 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `ports/stm32/boards/stm32u5xx_hal_conf_base.h` -- pulls in 64 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `lib/oofatfs/ff.c` at magnitude 9338.02 (structural weight, not risk).
...
| Analyzed Artifacts (Scanned) | 4989 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 1489 |
| Total LOC | 419648 |
...
|---|---|---|
| Modularity | 0.6159 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.1177 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.5% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 2.5313 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
|---|---|---|---|
| C | 2149 | 316846 | 43.1% |
| EMBEDDED_PYTHON | 1204 | 38929 | 24.1% |
| PYTHON | 670 | 27605 | 13.4% |
| CSV | 262 | 20030 | 5.3% |
...
> **Architectural Drift Z-Score:** `6.196`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +6.20; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 30%, Declarative / Non-Code 30%, Parameter Forwarders Files 9%, Encapsulated Accessors Files 7%, Large Core Modules (3) 7%
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 14.3 | 0.0 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 37.5 | 39.4 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 11.6 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 9.3 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 9.0 | 0.1 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 36.7 | 0.0 | 0.0 |
...
|---|---|---|---|---|
| memory | 79131 | 1917 | 34 | `lib/littlefs/lfs2.c` |
| cleanup | 355 | 183 | 0 | `tools/pyboard.py` |
| guards | 30540 | 2372 | 17 | `lib/littlefs/lfs2.c` |
| danger | 6374 | 1601 | 3 | `tools/ci.sh` |
| concurrency | 1706 | 235 | 0 | `tests/extmod/asyncio_wait_for.py` |
| connectivity | 16876 | 2444 | 9 | `py/obj.h` |
| io | 2736 | 415 | 0 | `tests/ports/cc3200/pin.py` |
| crypto | 32 | 31 | 0 | `tools/mpremote/mpremote/transport.py` |
| ipc | 778 | 63 | 0 | `extmod/modlwip.c` |
| time | 144 | 49 | 0 | `tests/run-multitests.py` |
| serialization | 8 | 5 | 0 | `tools/pyboard.py` |
| regex | 201 | 63 | 0 | `ports/stm32/Makefile` |
| events | 1032 | 236 | 0 | `ports/stm32/timer.c` |
| tests | 279 | 55 | 0 | `tests/extmod_hardware/machine_i2c_target.py` |
| docs | 22988 | 603 | 1 | `lib/cmsis/inc/core_cm55.h` |
| debt | 13526 | 1880 | 8 | `tools/mpy-tool.py` |
| mutation | 82105 | 2723 | 41 | `lib/oofatfs/ff.c` |
| dead_code | 6268 | 1386 | 3 | `tools/ci.sh` |
| credential | 5 | 5 | 0 | `ports/stm32/boards/NUCLEO_WB55/rfcore_makefirmware.py` |
| threat | 3891 | 799 | 2 | `py/obj.h` |
| ml_ai | 542 | 162 | 0 | `py/emitinlinerv32.c` |
| ui | 369 | 36 | 0 | `tests/extmod/framebuf_bounds.py` |
...
- `dir_alloc` **(Many-Argument Workhorses)** (@ `lib/oofatfs/ff.c`) -> Impact: **1252.7** | LOC: 1949
  * *Intent:* #if !FF_FS_READONLY /*-----------------------------------------------------------------------*/ /* Directory handling - Reserve a block of directory e...
- `lfs2_file_opencfg_` **(Many-Argument Workhorses)** (@ `lib/littlefs/lfs2.c`) -> Impact: **852.4** | LOC: 1862
  * *Intent:* /// Top level file operations ///
- `f_mkfs` **(Many-Argument Workhorses)** (@ `lib/oofatfs/ff.c`) -> Impact: **473.4** | LOC: 454
  * *Intent:* #endif /* FF_USE_FORWARD */ #if FF_USE_MKFS && !FF_FS_READONLY /*-----------------------------------------------------------------------*/ /* Create a...
- `uart_init` **(Many-Argument Workhorses)** (@ `ports/stm32/uart.c`) -> Impact: **457.2** | LOC: 490
  * *Intent:* // assumes Init parameters have been set up correctly
- `mp_obj_str_format_helper` **(Many-Argument Workhorses)** (@ `py/objstr.c`) -> Impact: **443.0** | LOC: 446
  * *Intent:* #else // define to nothing to improve coverage #define terse_str_format_value_error() #endif
- `adc_wait_for_eoc_or_timeout` **(Stateful Encapsulated Methods)** (@ `ports/stm32/adc.c`) -> Impact: **406.0** | LOC: 845
...
- `parse_string_literal` **(Many-Argument Workhorses)** (@ `py/lexer.c`) -> Impact: **307.0** | LOC: 371
- `run_tests` **(Many-Argument Workhorses)** (@ `tests/run-tests.py`) -> Impact: **301.2** | LOC: 440
- `mp_bytecode_print_str` **(Many-Argument Workhorses)** (@ `py/showbc.c`) -> Impact: **239.9** | LOC: 388
...
|---|---|---|---|---|
| `py` | 206 | 40262.14 | 38.42% | 25.13% |
| `ports/stm32` | 161 | 28017.92 | 27.71% | 28.74% |
| `extmod` | 92 | 16135.02 | 35.5% | 11.34% |
| `tests/basics` | 570 | 9945.54 | 4.18% | 0.0% |
| `lib/oofatfs` | 4 | 9463.28 | 27.25% | 8.34% |
| `lib/littlefs` | 9 | 8242.26 | 39.48% | 17.24% |
| `ports/esp32` | 74 | 8220.8 | 24.62% | 28.2% |
| `tests/extmod` | 195 | 7498.3 | 14.55% | 0.0% |
| `ports/mimxrt` | 68 | 6436.0 | 26.22% | 38.62% |
| `lib/cmsis/inc` | 30 | 6249.09 | 7.9% | 12.76% |

## 10. TARGETED STRUCTURAL SURFACE VECTORS (formerly Risk Vectors, Top 5 by Surface)
### Highest Debt Markers (formerly Tech Debt; Fragile/Planned)
- `ports/cc3200/hal/systick.c` -> **100.0%** Exposure
- `ports/pic16bit/board.c` -> **100.0%** Exposure
...
- **Unknown Dependencies:** `10189` packages imported that bypass the Zero-Trust whitelist.
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%), Complexity Load (formerly Cognitive Load) (84.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dir_alloc` **(Many-Argument Workhorses)** (Impact: 1252.7)
    * *Intent:* #if !FF_FS_READONLY /*-----------------------------------------------------------------------*/ /* D...
  * `f_mkfs` **(Many-Argument Workhorses)** (Impact: 473.4)
    * *Intent:* #endif /* FF_USE_FORWARD */ #if FF_USE_MKFS && !FF_FS_READONLY /*-----------------------------------...
  * `create_name` **(Many-Argument Workhorses)** (Impact: 182.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.8%), Complexity Load (formerly Cognitive Load) (89.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 68.6432% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `lfs2_file_opencfg_` **(Many-Argument Workhorses)** (Impact: 852.4)
    * *Intent:* /// Top level file operations ///
  * `lfs2_dir_fetchmatch` **(Many-Argument Workhorses)** (Impact: 208.9)
    * *Intent:* #endif
  * `lfs2_dir_relocatingcommit` **(Many-Argument Workhorses)** (Impact: 132.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (84.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (67.9%)
- **Documentation Coverage:** 98.4962% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `mp_compile_to_raw_code` **(Many-Argument Workhorses)** (Impact: 135.9)
    * *Intent:* #if !MICROPY_EXPOSE_MP_COMPILE_TO_RAW_CODE
  * `compile_scope_inline_asm` **(Many-Argument Workhorses)** (Impact: 102.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 0.122; role: Pure Consumer (Orchestrator)
...
**Top Internal Functions/Classes:**
  * `mp_obj_str_format_helper` **(Many-Argument Workhorses)** (Impact: 443.0)
    * *Intent:* #else // define to nothing to improve coverage #define terse_str_format_value_error() #endif
  * `str_modulo_format` **(Many-Argument Workhorses)** (Impact: 173.4)
    * *Intent:* #if MICROPY_PY_BUILTINS_STR_OP_MODULO
  * `mp_obj_str_binary_op` **(Many-Argument Workhorses)** (Impact: 63.6)
    * *Intent:* // Note: this function is used to check if an object is a str or bytes, which // works because both ...
  * `bytes_make_new` **(Many-Argument Workhorses)** (Impact: 55.9)
...
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `mpz_as_str_inpl` **(Many-Argument Workhorses)** (Impact: 54.5)
...
- **Global Archetype:** `file_cluster_8` (Drift: 0.0 IQR)
- **Magnitude:** 1680.24 | **LOC:** 1387 | **CtrlFlow:** 40.4% | **Authorship Centralization:** 33.3%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.7%), Complexity Load (formerly Cognitive Load) (84.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `uart_init` **(Many-Argument Workhorses)** (Impact: 457.2)
    * *Intent:* // assumes Init parameters have been set up correctly
  * `uart_deinit` **(Compute Cores)** (Impact: 87.5)
  * `uart_get_source_freq` **(Compute Cores)** (Impact: 78.6)
  * `uart_irq_handler` **(Compute Cores)** (Impact: 64.0)
    * *Intent:* // This IRQ handler is set up to handle RXNE, IDLE and ORE interrupts only. // Notes: // - ORE (over...
  * `uart_exists` **(Compute Cores)** (Impact: 60.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **7**; blast radius 0.502; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (65.3%), Complexity Load (formerly Cognitive Load) (52.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `emit_native_binary_op` **(Many-Argument Workhorses)** (Impact: 146.3)
  * `emit_native_start_pass` **(Many-Argument Workhorses)** (Impact: 140.6)
  * `emit_native_store_subscr` **(Compute Cores)** (Impact: 65.9)
  * `emit_native_load_subscr` **(Compute Cores)** (Impact: 48.2)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 0.502
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.001602
...
- **Blast Radius:** changing it is visible to **36** in-repo importer(s); it depends on **12**; blast radius 4.611; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (90.7%), Guard Balance (formerly Safety Score) (52.8%), Complexity Load (formerly Cognitive Load) (3.1%), Test Surface (formerly Verification) (2.3%)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 4.611
  * `Choke Point (Betweenness):` 2.5e-05 | `Ripple Effect (Closeness):` 0.007925
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (41.2%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `USBH_AUDIO_ClassRequest` **(Compute Cores)** (Impact: 45.5)
    * *Intent:* /** * @brief USBH_AUDIO_ClassRequest * The function is responsible for handling Standard requests * ...
  * `ParseCSDescriptors` **(Many-Argument Workhorses)** (Impact: 34.6)
    * *Intent:* /** * @brief Parse AC interfaces * @param phost: Host handle * @retval USBH Status */
  * `USBH_AUDIO_CSRequest` **(Many-Argument Workhorses)** (Impact: 29.9)
    * *Intent:* /** * @brief USBH_AUDIO_CSRequest * The function is responsible for handling AC Specific requests fo...
  * `USBH_AUDIO_InterfaceInit` **(I/O & Config Routines)** (Impact: 29.6)
    * *Intent:* /** * @} */ /** @defgroup USBH_AUDIO_CORE_Private_Functions * @{ */ /** * @brief USBH_AUDIO_Interfac...
  * `USBH_AUDIO_SetFrequency` **(Many-Argument Workhorses)** (Impact: 25.2)
...
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 1373.86 | **LOC:** 1366 | **CtrlFlow:** 24.5% | **Authorship Centralization:** 47.1%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Guard Balance (formerly Safety Score) (98.1%), Complexity Load (formerly Cognitive Load) (46.5%), Concurrency Surface (formerly Concurrency) (28.2%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_tests` **(Many-Argument Workhorses)** (Impact: 301.2)
  * `run_micropython` **(Many-Argument Workhorses)** (Impact: 121.6)
  * `run_one_test` **(Compute Cores)** (Impact: 100.4)
  * `detect_test_platform` **(Many-Argument Workhorses)** (Impact: 28.3)
    * *Intent:* # Run a script to detect various bits of information about the target test instance. output = run_fe...
  * `detect_target_wiring_script` **(Compute Cores)** (Impact: 26.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (63.0%)
- **Documentation Coverage:** 93.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `lwip_socket_ioctl` **(Many-Argument Workhorses)** (Impact: 109.1)
  * `lwip_tcp_receive` **(Many-Argument Workhorses)** (Impact: 63.0)
    * *Intent:* // Helper function for recv/recvfrom to handle TCP packets
  * `lwip_getaddrinfo` **(Many-Argument Workhorses)** (Impact: 49.1)
    * *Intent:* // lwip.getaddrinfo
  * `lwip_socket_make_new` **(Many-Argument Workhorses)** (Impact: 46.8)
    * *Intent:* // FIXME: Only supports two arguments at present
  * `lwip_raw_udp_send` **(Many-Argument Workhorses)** (Impact: 37.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (35.4%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `USBH_PTP_Process` **(Compute Cores)** (Impact: 89.8)
    * *Intent:* /** * @brief USBH_PTP_Process * The function handle the BOT protocol. * @param phost: Host handle * ...
  * `PTP_GetDevicePropValue` **(Many-Argument Workhorses)** (Impact: 37.3)
    * *Intent:* /** * @brief PTP_GetDevicePropValue * Gets objectInfo and fills object_info structure. * @param phos...
  * `USBH_PTP_GetPartialObject` **(Many-Argument Workhorses)** (Impact: 21.7)
    * *Intent:* /** * @brief USBH_PTP_GetPartialObject * Gets object partially * @param phost: Host handle * @param ...
  * `USBH_PTP_GetObjectHandles` **(Many-Argument Workhorses)** (Impact: 17.4)
    * *Intent:* /** * @brief USBH_PTP_GetObjectHandles * Gets device info dataset and fills deviceinfo structure. * ...
  * `USBH_PTP_GetNumObjects` **(Many-Argument Workhorses)** (Impact: 17.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (48.2%)
- **Documentation Coverage:** 42.4242% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `adc_wait_for_eoc_or_timeout` **(Stateful Encapsulated Methods)** (Impact: 406.0)
  * `adc_config_channel` **(Many-Argument Workhorses)** (Impact: 58.0)
  * `adc_read_timed_multi` **(Many-Argument Workhorses)** (Impact: 54.6)
    * *Intent:* // read_timed_multi((adcx, adcy, ...), (bufx, bufy, ...), timer) // // Read analog values from multi...
  * `adcx_init_periph` **(Many-Argument Workhorses)** (Impact: 49.6)
  * `adc_read_timed` **(Many-Argument Workhorses)** (Impact: 49.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (90.0%), Guard Balance (formerly Safety Score) (80.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `l2cap_channel_event` **(Many-Argument Workhorses)** (Impact: 38.9)
  * `characteristic_access_cb` **(Many-Argument Workhorses)** (Impact: 33.9)
  * `mp_bluetooth_gatts_register_service` **(Many-Argument Workhorses)** (Impact: 33.5)
  * `central_gap_event_cb` **(Many-Argument Workhorses)** (Impact: 33.3)
  * `mp_bluetooth_l2cap_send` **(Many-Argument Workhorses)** (Impact: 27.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (78.8%), Debt Markers (formerly Tech Debt) (61.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `xQueueGenericSend` **(Many-Argument Workhorses)** (Impact: 92.5)
    * *Intent:* #endif /* ( ( configUSE_COUNTING_SEMAPHORES == 1 ) && ( configSUPPORT_DYNAMIC_ALLOCATION == 1 ) ) */...
  * `xQueueGenericReceive` **(Many-Argument Workhorses)** (Impact: 90.2)
    * *Intent:* /*-----------------------------------------------------------*/
  * `xQueueGenericSendFromISR` **(Many-Argument Workhorses)** (Impact: 70.1)
    * *Intent:* /*-----------------------------------------------------------*/
  * `xQueueGiveFromISR` **(Many-Argument Workhorses)** (Impact: 53.2)
    * *Intent:* /*-----------------------------------------------------------*/
  * `prvCopyDataToQueue` **(Many-Argument Workhorses)** (Impact: 35.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (73.7%), Guard Balance (formerly Safety Score) (71.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `bluetooth_ble_config` **(Many-Argument Workhorses)** (Impact: 62.2)
  * `invoke_irq_handler_run` **(Many-Argument Workhorses)** (Impact: 61.0)
    * *Intent:* #endif // !MICROPY_PY_BLUETOOTH_USE_SYNC_EVENTS // -------------------------------------------------...
  * `bluetooth_ble_invoke_irq` **(Compute Cores)** (Impact: 57.5)
  * `ringbuf_extract` **(Many-Argument Workhorses)** (Impact: 41.0)
    * *Intent:* // Helpers #if !MICROPY_PY_BLUETOOTH_USE_SYNC_EVENTS
  * `bluetooth_uuid_make_new` **(Many-Argument Workhorses)** (Impact: 36.3)
    * *Intent:* // ---------------------------------------------------------------------------- // UUID object // --...
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **17**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.1%), Complexity Load (formerly Cognitive Load) (86.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 48.2456% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `stm32_main` **(Compute Cores)** (Impact: 76.5)
  * `i2c_slave_process_rx_end` **(Compute Cores)** (Impact: 75.5)
  * `dfu_handle_tx` **(Many-Argument Workhorses)** (Impact: 38.8)
  * `pyb_usbdd_StrDescriptor` **(Many-Argument Workhorses)** (Impact: 36.0)
  * `hw_read` **(Compute Cores)** (Impact: 33.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **20**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (94.5%), Guard Balance (formerly Safety Score) (84.4%), Debt Markers (formerly Tech Debt) (83.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `mp_call_prepare_args_n_kw_var` **(Many-Argument Workhorses)** (Impact: 86.0)
  ...
  * `mp_convert_member_lookup` **(Many-Argument Workhorses)** (Impact: 45.2)
    * *Intent:* #endif // MICROPY_BUILTIN_METHOD_CHECK_SELF_ARG // Given a member that was extracted from an instanc...
  * `mp_resume` **(Many-Argument Workhorses)** (Impact: 39.8)
  * `mp_init` **(I/O & Config Routines)** (Impact: 36.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (84.3%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (44.9%)
- **Documentation Coverage:** 36.1702% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `pyb_timer_channel` **(Many-Argument Workhorses)** (Impact: 144.8)
    * *Intent:* /// /// Notes for Timer.ENC modes: /// /// - Requires 2 pins, so one or both pins will need to be co...
  * `timer_clock_enable` **(Compute Cores)** (Impact: 69.3)
  * `pyb_timer_init_helper` **(Many-Argument Workhorses)** (Impact: 67.3)
    * *Intent:* /// measures ticks of `source_freq` divided by `div` clock ticks. /// `deadtime` is only available o...
  * `timer_get_source_freq` **(Compute Cores)** (Impact: 53.4)
    * *Intent:* // Get the frequency (in Hz) of the source clock for the given timer. // On STM32F405/407/415/417 th...
  * `pyb_timer_channel_callback` **(Compute Cores)** (Impact: 38.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (97.3%), Guard Balance (formerly Safety Score) (91.5%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parse_string_literal` **(Many-Argument Workhorses)** (Impact: 307.0)
  * `mp_lexer_to_next` **(Compute Cores)** (Impact: 176.5)
  * `is_string_or_bytes` **(Stateful Encapsulated Methods)** (Impact: 29.1)
  * `next_char` **(Compute Cores)** (Impact: 27.2)
  * `skip_whitespace` **(Stateful Encapsulated Methods)** (Impact: 25.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (85.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (78.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `mp_parse` **(Many-Argument Workhorses)** (Impact: 170.1)
  * `fold_constants` **(Many-Argument Workhorses)** (Impact: 118.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.3%), Complexity Load (formerly Cognitive Load) (91.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `eth_mac_init` **(Many-Argument Workhorses)** (Impact: 185.2)
  * `ETH_IRQHandler` **(Compute Cores)** (Impact: 33.0)
  * `eth_init` **(Many-Argument Workhorses)** (Impact: 32.3)
  * `eth_tx_buf_get` **(Compute Cores)** (Impact: 26.1)
    * *Intent:* #if !USE_PBUF_REF_FOR_TX
  * `eth_netif_output` **(Many-Argument Workhorses)** (Impact: 25.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.8%), Debt Markers (formerly Tech Debt) (87.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (76.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `btstack_packet_handler_generic` **(Many-Argument Workhorses)** (Impact: 154.2)
    * *Intent:* #endif
  * `mp_bluetooth_gatts_register_service` **(Many-Argument Workhorses)** (Impact: 54.5)
  * `mp_bluetooth_gatts_notify_indicate` **(Many-Argument Workhorses)** (Impact: 35.2)
  * `mp_bluetooth_gatts_write` **(Many-Argument Workhorses)** (Impact: 33.8)
  * `btstack_packet_handler_att_server` **(Many-Argument Workhorses)** (Impact: 30.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.122; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.8%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (75.6%), Complexity Load (formerly Cognitive Load) (47.1%)
- **Documentation Coverage:** 90.6977% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `mp_obj_new_type` **(Many-Argument Workhorses)** (Impact: 71.1)
  * `mp_obj_class_lookup` **(Many-Argument Workhorses)** (Impact: 64.3)
  * `mp_obj_instance_store_attr` **(Many-Argument Workhorses)** (Impact: 56.1)
  * `type_attr` **(Many-Argument Workhorses)** (Impact: 52.5)
  * `mp_obj_instance_load_attr` **(Many-Argument Workhorses)** (Impact: 49.1)
**Contextual Mitigations & Amplifications:**
...
- `tools/ci.sh` -> Churn: **93.41%** | Cog Load: 29.9748% | Debt: 100.0%
- `ports/stm32/pyb_can.c` -> Churn: **50.82%** | Cog Load: 78.579% | Debt: 11.5494%
- `ports/stm32/fdcan.c` -> Churn: **50.23%** | Cog Load: 69.8288% | Debt: 95.2889%
...
- `py/runtime.h` -> **Severity: 0.016** (Bridge: 0.0002 * Flux: 75.0692%)
- `extmod/modre.c` -> **Severity: 0.005** (Bridge: 0.0 * Flux: 99.9917%)
- `py/dynruntime.h` -> **Severity: 0.004** (Bridge: 0.0001 * Flux: 78.3897%)
- `extmod/moddeflate.c` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 99.9996%)
- `extmod/modframebuf.c` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `py/mpprint.h` -> **Severity: 3.584** (Embedded: 0.0635 * Error Risk: 56.3934%)
- `py/runtime.h` -> **Severity: 3.459** (Embedded: 0.0852 * Error Risk: 40.6188%)
- `py/obj.h` -> **Severity: 3.44** (Embedded: 0.0867 * Error Risk: 39.6814%)
- `py/cstack.h` -> **Severity: 2.803** (Embedded: 0.0538 * Error Risk: 52.1415%)
- `py/misc.h` -> **Severity: 2.528** (Embedded: 0.0699 * Error Risk: 36.1567%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `py/obj.h` -> **Severity: 5238.7** (Blast Radius: 52.387 * Doc Risk: 100.0%)
- `py/misc.h` -> **Severity: 2733.5** (Blast Radius: 27.335 * Doc Risk: 100.0%)
- `py/runtime.h` -> **Severity: 1838.7** (Blast Radius: 18.387 * Doc Risk: 100.0%)
- `tests/internal_bench/bench.py` -> **Severity: 902.7** (Blast Radius: 9.027 * Doc Risk: 100.0%)
- `py/cstack.h` -> **Severity: 572.4** (Blast Radius: 5.724 * Doc Risk: 100.0%)
