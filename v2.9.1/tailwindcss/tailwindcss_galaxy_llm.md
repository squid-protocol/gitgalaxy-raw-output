...
## 1. EXECUTIVE SUMMARY
- **Scope:** 477 analyzed artifact(s), 116347 LOC.
- **Load-bearing artifact:** `packages/tailwindcss/src/design-system.ts` -- 53 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `integrations/upgrade/index.test.ts` -- pulls in 50 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `packages/tailwindcss/src/utilities.test.ts` at magnitude 26514.86 (structural weight, not risk).
...
| Avg Path Length | 3.031 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `3.162`
> **Composition Archetype:** `Hub-Coupled App` (z +3.16; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 28%, Declarative / Non-Code 24%, Compute Cores Files 12%, Large Core Modules (3) 9%, Large Core Modules (2) 5%
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.9 | 44.6 | 54.3 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 99.6 | 8.9 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 7.7 | 0.0 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 13.8 | 3.5 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 5.8 | 1.2 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 7.7 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 53.1 | 77.4 | 100.0 |
...
| connectivity | 1777 | 270 | 7 | `packages/tailwindcss/theme.css` |
...
| dead_code | 285 | 84 | 1 | `crates/oxide/src/extractor/mod.rs` |
...
- `createUtilities` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/utilities.ts`) -> Impact: **320.9** | LOC: 1779
- `optimizeAst` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/ast.ts`) -> Impact: **303.1** | LOC: 461
...
- `parse` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/css-parser.ts`) -> Impact: **276.0** | LOC: 531
- `buildPluginApi` **(Defensive Guards)** (@ `packages/tailwindcss/src/compat/plugin-api.ts`) -> Impact: **256.4** | LOC: 462
- `parseCss` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/index.ts`) -> Impact: **244.9** | LOC: 567
- `modernizeArbitraryValuesVariant` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/canonicalize-candidates.ts`) -> Impact: **240.3** | LOC: 372
- `test` **(Many-Argument Workhorses)** (@ `integrations/utils.ts`) -> Impact: **213.4** | LOC: 377
- `constantFoldDeclarationAst` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/constant-fold-declaration.ts`) -> Impact: **194.1** | LOC: 244
- `transform` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/ast.ts`) -> Impact: **184.8** | LOC: 208
- `suggest` **(Many-Argument Workhorses)** (@ `packages/tailwindcss/src/utilities.ts`) -> Impact: **184.3** | LOC: 499
...
| `crates/ignore/src` | 8 | 2094.18 | 7.68% | 37.62% |
...
- **Magnitude:** 26514.86 | **LOC:** 29983 | **CtrlFlow:** 0.2% | **Authorship Centralization:** 50.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (90.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (74.6%)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (45.3%), Guard Balance (formerly Safety Score) (40.7%), Complexity Load (formerly Cognitive Load) (32.5%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **15**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.5%), Guard Balance (formerly Safety Score) (73.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (53.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (89.9%), Guard Balance (formerly Safety Score) (89.8%), Complexity Load (formerly Cognitive Load) (74.9%)
- **Documentation Coverage:** 67.1233% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `createUtilities` **(Many-Argument Workhorses)** (Impact: 320.9)
  * `suggest` **(Many-Argument Workhorses)** (Impact: 184.3)
  * `createCssUtility` **(Compute Cores)** (Impact: 139.9)
  * `functionalUtility` **(Many-Argument Workhorses)** (Impact: 115.3)
  * `resolveValueFunction` **(Many-Argument Workhorses)** (Impact: 107.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (54.8%), Guard Balance (formerly Safety Score) (54.2%), Complexity Load (formerly Cognitive Load) (46.2%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **5** in-repo importer(s); it depends on **20**; blast radius 3.788; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (96.6%), Guard Balance (formerly Safety Score) (78.2%), Complexity Load (formerly Cognitive Load) (61.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `modernizeArbitraryValuesVariant` **(Many-Argument Workhorses)** (Impact: 240.3)
  * `collapseCandidates` **(Many-Argument Workhorses)** (Impact: 126.0)
  * `arbitraryUtilities` **(Many-Argument Workhorses)** (Impact: 103.2)
  * `collapseGroup` **(Compute Cores)** (Impact: 88.8)
  * `tryReplacements` **(Many-Argument Workhorses)** (Impact: 87.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (96.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (74.6%), Guard Balance (formerly Safety Score) (60.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `loadStylesheet` **(Many-Argument Workhorses)** (Impact: 12.6)
  * `loadStylesheet` **(Annotated & Test Methods)** (Impact: 6.0)
  * `loadStylesheet` **(Annotated & Test Methods)** (Impact: 5.1)
  * `module` **(Annotated & Test Methods)** (Impact: 4.2)
  * `module` **(Annotated & Test Methods)** (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (79.1%), Guard Balance (formerly Safety Score) (51.1%), Complexity Load (formerly Cognitive Load) (26.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (18.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (52.5%), Mutation Surface (formerly State Flux) (47.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (27.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `module` **(Compute Cores)** (Impact: 10.2)
  * `loadModule` **(Callbacks & Closures)** (Impact: 7.9)
  * `parseValue` **(Defensive Guards)** (Impact: 7.4)
  * `scrollbar` **(Many-Argument Workhorses)** (Impact: 7.2)
  * `module` **(I/O & Config Routines)** (Impact: 6.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **50**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (95.8%), Guard Balance (formerly Safety Score) (67.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (53.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `withBOM` **(Parameter Forwarders)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (69.4%), Complexity Load (formerly Cognitive Load) (49.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `loadModule` **(Callbacks & Closures)** (Impact: 5.5)
  * `module` **(Annotated & Test Methods)** (Impact: 4.2)
  * `loadModule` **(I/O & Config Routines)** (Impact: 3.0)
  * `loadModule` **(Callbacks & Closures)** (Impact: 2.1)
  * `loadStylesheet` **(Interface Declarations)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (40.1%), Mutation Surface (formerly State Flux) (13.6%), Connectivity (formerly Api Exposure) (12.7%), Complexity Load (formerly Cognitive Load) (6.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (89.6%), Guard Balance (formerly Safety Score) (59.6%), Complexity Load (formerly Cognitive Load) (41.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `withBOM` **(Parameter Forwarders)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 1.229; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (81.0%), Complexity Load (formerly Cognitive Load) (69.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `loadModule` **(I/O & Config Routines)** (Impact: 13.0)
  * `typography` **(Annotated & Test Methods)** (Impact: 6.8)
  * `loadModule` **(I/O & Config Routines)** (Impact: 5.2)
  * `loadModule` **(I/O & Config Routines)** (Impact: 5.2)
  * `loadModule` **(I/O & Config Routines)** (Impact: 4.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **8** in-repo importer(s); it depends on **11**; blast radius 4.888; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (95.6%), Guard Balance (formerly Safety Score) (92.5%), Connectivity (formerly Api Exposure) (75.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `optimizeAst` **(Many-Argument Workhorses)** (Impact: 303.1)
    * *Intent:* // Optimize the AST for printing where all the special nodes that require custom // handling are han...
  * `transform` **(Many-Argument Workhorses)** (Impact: 184.8)
  * `toCss` **(Many-Argument Workhorses)** (Impact: 64.0)
  * `stringify` **(Many-Argument Workhorses)** (Impact: 59.6)
  * `isVariableUsed` **(Many-Argument Workhorses)** (Impact: 21.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (45.7%), Complexity Load (formerly Cognitive Load) (35.8%), Mutation Surface (formerly State Flux) (30.4%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (77.6%), Guard Balance (formerly Safety Score) (59.3%), Complexity Load (formerly Cognitive Load) (38.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **30** in-repo importer(s); it depends on **19**; blast radius 16.495; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (91.9%), Guard Balance (formerly Safety Score) (79.6%), Connectivity (formerly Api Exposure) (62.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `buildPluginApi` **(Defensive Guards)** (Impact: 256.4)
  * `matchUtilities` **(Defensive Guards)** (Impact: 134.5)
  * `matchVariant` **(Defensive Guards)** (Impact: 111.2)
  * `compileFn` **(Defensive Guards)** (Impact: 81.9)
  * `addVariant` **(Compute Cores)** (Impact: 31.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **36** in-repo importer(s); it depends on **11**; blast radius 25.773; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.9%), Complexity Load (formerly Cognitive Load) (94.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 97.7273% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test` **(Many-Argument Workhorses)** (Impact: 213.4)
  * `spawn` **(Compute Cores)** (Impact: 49.9)
  * `exec` **(Many-Argument Workhorses)** (Impact: 36.2)
  * `parseSourceMap` **(Defensive Guards)** (Impact: 24.8)
  * `dumpFiles` **(Defensive Guards)** (Impact: 21.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **44**; blast radius 1.229; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (63.0%), Connectivity (formerly Api Exposure) (58.6%), Guard Balance (formerly Safety Score) (54.0%), Concurrency Surface (formerly Concurrency) (46.8%)
- **Documentation Coverage:** 40.9091% of unit weight undocumented
**Top Internal Functions/Classes:**
  ...
  * `run_one` **(Compute Cores)** (Impact: 34.9)
  * `visit` **(Compute Cores)** (Impact: 29.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Guard Balance (formerly Safety Score) (49.0%), Complexity Load (formerly Cognitive Load) (22.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (20.8%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.229; role: Isolated/Orphan
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.9%), Complexity Load (formerly Cognitive Load) (99.5%), Guard Balance (formerly Safety Score) (71.0%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (99.4%), Complexity Load (formerly Cognitive Load) (85.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (72.0%)
- **Documentation Coverage:** 90.625% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `tailwindcss` **(Compute Cores)** (Impact: 117.7)
  * `generate` **(Many-Argument Workhorses)** (Impact: 83.8)
    * *Intent:* // Generate the CSS for the root file. This can return false if the file is // not considered a Tail...
  * `onDependency` **(Compute Cores)** (Impact: 47.9)
  * `createRoot` **(Defensive Guards)** (Impact: 33.7)
  * `handler` **(Defensive Guards)** (Impact: 31.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 1.229; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Concurrency Surface (formerly Concurrency) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.9%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Contextual Mitigations & Amplifications:**
...
- `packages/tailwindcss/src/value-parser.ts` -> **Severity: 8.699** (Embedded: 0.0913 * Error Risk: 95.3212%)
- `packages/tailwindcss/src/design-system.ts` -> **Severity: 8.078** (Embedded: 0.126 * Error Risk: 64.1001%)
- `packages/tailwindcss/src/source-maps/line-table.ts` -> **Severity: 7.837** (Embedded: 0.0899 * Error Risk: 87.2138%)
