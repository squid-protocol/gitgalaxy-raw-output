...
## 1. EXECUTIVE SUMMARY
- **Scope:** 5290 analyzed artifact(s), 437039 LOC.
- **Load-bearing artifact:** `src/modules/cmdpal/extensionsdk/Microsoft.CommandPalette.Extensions/Microsoft.CommandPalette.Extensions.def` -- 228 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `src/modules/ZoomIt/ZoomIt/pch.h` -- pulls in 82 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp` at magnitude 10859.32 (structural weight, not risk).
...
| Analyzed Artifacts (Scanned) | 5290 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 2463 |
| Total LOC | 437039 |
...
|---|---|---|
| Modularity | 0.7441 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
| Assortativity | -0.3031 | Positive = Resilient core. Negative = Fragile single-points-of-failure. |
| Cyclic Density | 0.1% | % of files trapped in dependency loops (Static Friction). |
| Avg Path Length | 1.9232 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Composition Archetype:** `Flat Modular Platform` (z +3.74; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 22%, Declarative / Non-Code 20%, State Mutators Files 14%, Encapsulated Accessors Files 10%, Large Core Modules (2) 9%
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 28.3 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 7.6 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 6.8 | 3.7 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 8.6 | 1.4 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 5.9 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 54.9 | 96.6 | 100.0 |
...
|---|---|---|---|---|
| memory | 18947 | 1282 | 5 | `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp` |
| cleanup | 901 | 362 | 0 | `src/modules/MouseWithoutBorders/App/Class/SocketStuff.cs` |
| guards | 45184 | 3208 | 23 | `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp` |
| danger | 2433 | 894 | 1 | `src/modules/MouseUtils/MouseUtils.UITests/FindMyMouseTests.cs` |
| concurrency | 6878 | 745 | 2 | `src/modules/AdvancedPaste/UITest-AdvancedPaste/AdvancedPasteUITest.cs` |
| connectivity | 24925 | 3783 | 11 | `src/modules/MouseWithoutBorders/App/Class/NativeMethods.cs` |
| io | 2065 | 448 | 0 | `.github/skills/winmd-api-search/scripts/cache-generator/Program.cs` |
| crypto | 0 | 0 | 0 | - |
...
| debt | 1290 | 326 | 0 | `src/modules/MouseUtils/CursorWrap/CursorWrapTests/analyze_test_results.py` |
| mutation | 107385 | 3404 | 51 | `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp` |
| dead_code | 9009 | 2510 | 5 | `src/common/interop/Constants.cpp` |
| credential | 64 | 26 | 0 | `src/common/interop/shared_constants.h` |
| threat | 2118 | 541 | 1 | `src/modules/ZoomIt/ZoomIt/resource.h` |
...
- `MainWndProc` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/Zoomit.cpp`) -> Impact: **755.7** | LOC: 1340
  * *Intent:* //---------------------------------------------------------------------------- // // MainWndProc // //------------------------------------------------...
- `BuildFixedOverlayMask` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp`) -> Impact: **730.9** | LOC: 817
- `VideoRecordingSession::TrimDialogProc` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/VideoRecordingSession.cpp`) -> Impact: **523.5** | LOC: 1302
  * *Intent:* //---------------------------------------------------------------------------- // // VideoRecordingSession::TrimDialogProc // // Dialog procedure for ...
- `OptionsProc` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/Zoomit.cpp`) -> Impact: **476.8** | LOC: 860
  * *Intent:* //---------------------------------------------------------------------------- // // OptionsProc // //------------------------------------------------...
- `StitchPanoramaFrames` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp`) -> Impact: **424.1** | LOC: 962
- `LogStitchedBandDiagnostics` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp`) -> Impact: **336.0** | LOC: 304
  * *Intent:* // Detect visually dark stitched bands even when the canvas has no unwritten // gaps so we can correlate full-width artifacts with a specific frame ha...
- `LiveZoomWndProc` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/Zoomit.cpp`) -> Impact: **331.6** | LOC: 505
  * *Intent:* //---------------------------------------------------------------------------- // // LiveZoomWndProc // //--------------------------------------------...
- `ValidateShortcutBufferElement` **(Many-Argument Workhorses)** (@ `src/modules/keyboardmanager/KeyboardManagerEditorLibrary/BufferValidationHelpers.cpp`) -> Impact: **267.1** | LOC: 241
  * *Intent:* // Function to validate an element of the shortcut remap buffer when the selection has changed
- `RunPanoramaStitchSelfTest` **(I/O & Config Routines)** (@ `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp`) -> Impact: **259.4** | LOC: 1328
  * *Intent:* // next to the solution root (i.e. <repo>\Debug\). // 3. Run: ZoomIt64a.exe /panorama-selftest // Optional stress targeting: // - /panorama-selftest-s...
- `TimelineSubclassProc` **(Many-Argument Workhorses)** (@ `src/modules/ZoomIt/ZoomIt/VideoRecordingSession.cpp`) -> Impact: **253.7** | LOC: 417
...
|---|---|---|---|---|
| `src/modules/ZoomIt/ZoomIt` | 29 | 23615.96 | 25.3% | 13.3% |
...
- **Unknown Dependencies:** `20069` packages imported that bypass the Zero-Trust whitelist.

## 11. RANKED ARTIFACTS (Top 25 by Structural Magnitude)
> Ranked by Structural Magnitude: the file's structural weight and centralization within the system. Magnitude is **not** a risk score and is independent of the surface vectors in section 6. Each entry carries a **Blast Radius** line stating what a change to it would reach -- that, not the vector percentages, is the actionable part.

### `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp` (CPP | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_8` (Drift: 0.0 IQR)
- **Magnitude:** 10859.32 | **LOC:** 18014 | **CtrlFlow:** 25.1% | **Authorship Centralization:** 100.0%
...
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.3%), Complexity Load (formerly Cognitive Load) (92.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `BuildFixedOverlayMask` **(Many-Argument Workhorses)** (Impact: 730.9)
  * `StitchPanoramaFrames` **(Many-Argument Workhorses)** (Impact: 424.1)
  * `LogStitchedBandDiagnostics` **(Many-Argument Workhorses)** (Impact: 336.0)
    * *Intent:* // Detect visually dark stitched bands even when the canvas has no unwritten // gaps so we can corre...
  * `RunPanoramaStitchSelfTest` **(I/O & Config Routines)** (Impact: 259.4)
    * *Intent:* // next to the solution root (i.e. <repo>\Debug\). // 3. Run: ZoomIt64a.exe /panorama-selftest // Op...
  * `RepairOverlayDarkBands` **(Many-Argument Workhorses)** (Impact: 210.7)
    * *Intent:* // Repairs dark bands caused by overlay suppression breaking the blend chain. // After FixedOverlay ...
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 1931 instances
* *Concurrency (weighted view):* 13
* *State Mutation (weighted view):* 6538
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 3732`, `structural_boundaries: 484`, `args: 607`, `func_start: 84`, `class_start: 18`
* *Risk/State:* `safety_bypasses: 4`, `state_mutation: 2676`, `dead_code: 7`, `fragile_debt: 2`, `unreferenced_by_name: 11`
* *Architecture:* `io: 43`, `api: 1`, `concurrency: 3`, `import: 15`
* *Defense:* `safety: 8`, `immutability_locks: 1998`, `cleanup: 3`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.142
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 3):` PanoramaCapture.h, Utility.h, WindowsVersions.h, arm_neon.h, atomic, cmath, commctrl.h, emmintrin.h...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)

### `src/modules/ZoomIt/ZoomIt/Zoomit.cpp` (CPP | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `file_cluster_6` (Drift: 0.0 IQR)
- **Magnitude:** 6671.66 | **LOC:** 11756 | **CtrlFlow:** 25.3% | **Authorship Centralization:** 21.4%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (89.0%), Guard Balance (formerly Safety Score) (85.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (82.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `MainWndProc` **(Many-Argument Workhorses)** (Impact: 755.7)
    * *Intent:* //---------------------------------------------------------------------------- // // MainWndProc // ...
  * `OptionsProc` **(Many-Argument Workhorses)** (Impact: 476.8)
    * *Intent:* //---------------------------------------------------------------------------- // // OptionsProc // ...
  * `LiveZoomWndProc` **(Many-Argument Workhorses)** (Impact: 331.6)
    * *Intent:* //---------------------------------------------------------------------------- // // LiveZoomWndProc...
  * `wWinMain` **(Many-Argument Workhorses)** (Impact: 206.1)
    * *Intent:* #endif //---------------------------------------------------------------------------- // // WinMain ...
  * `AdvancedBreakProc` **(Many-Argument Workhorses)** (Impact: 179.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (76.4%), Complexity Load (formerly Cognitive Load) (66.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `VideoRecordingSession::TrimDialogProc` **(Many-Argument Workhorses)** (Impact: 523.5)
    * *Intent:* //---------------------------------------------------------------------------- // // VideoRecordingS...
  * `TimelineSubclassProc` **(Many-Argument Workhorses)** (Impact: 253.7)
  * `StartPlaybackAsync` **(Many-Argument Workhorses)** (Impact: 150.6)
  * `LoadGifFrames` **(Many-Argument Workhorses)** (Impact: 107.3)
  * `VideoRecordingSession::ShowTrimDialogInternal` **(Many-Argument Workhorses)** (Impact: 94.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **9**; blast radius 0.203; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.3%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (45.6%)
- **Documentation Coverage:** 9.7403% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_analyze_wrap_problem` **(Many-Argument Workhorses)** (Impact: 73.5)
    * *Intent:* """ Analyze why a wrap destination doesn't exist for a given edge range. This provides detailed diag...
  * `_show_segment_detail` **(Many-Argument Workhorses)** (Impact: 56.1)
    * *Intent:* """Show detailed information about a segment in a popup."""
  * `find_nearest_opposite_edge` **(Many-Argument Workhorses)** (Impact: 50.6)
  * `_update_info_panel` **(Many-Argument Workhorses)** (Impact: 50.6)
    * *Intent:* """Update the info panel with loaded data."""
  * `_draw_cursor_position` **(Many-Argument Workhorses)** (Impact: 35.6)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **21**; blast radius 0.203; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (82.8%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (48.0%)
- **Documentation Coverage:** 77.6316% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `Canvas_PaintSurface` **(Many-Argument Workhorses)** (Impact: 147.0)
  * `OnKeyDown` **(I/O & Config Routines)** (Impact: 84.7)
    * *Intent:* /// <inheritdoc/>
  * `ReadFormattedData` **(I/O & Config Routines)** (Impact: 56.9)
  * `CalculateDataColumnCharWidth` **(I/O & Config Routines)** (Impact: 55.4)
  * `DrawSelectionGeometry` **(Many-Argument Workhorses)** (Impact: 47.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (59.4%), Guard Balance (formerly Safety Score) (57.7%), Concurrency Surface (formerly Concurrency) (31.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `MainTCPRoutine` **(Many-Argument Workhorses)** (Impact: 117.2)
  * `SendClipboardData` **(Defensive Guards)** (Impact: 71.0)
  * `SocketStuff` **(Many-Argument Workhorses)** (Impact: 64.2)
  * `UpdateTcpSockets` **(Many-Argument Workhorses)** (Impact: 56.7)
  * `StartNewTcpClient` **(Compute Cores)** (Impact: 55.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 0.142; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (97.9%), Guard Balance (formerly Safety Score) (95.9%), Mutation Surface (formerly State Flux) (95.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `HelperTimer_Tick` **(Many-Argument Workhorses)** (Impact: 46.0)
  * `FrmMatrix_Load` **(Many-Argument Workhorses)** (Impact: 43.9)
  * `Form_DragOver` **(Many-Argument Workhorses)** (Impact: 27.9)
  * `ButtonOK_Click` **(Many-Argument Workhorses)** (Impact: 23.9)
  * `LoadMachines` **(I/O & Config Routines)** (Impact: 13.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **2**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (86.6%), Complexity Load (formerly Cognitive Load) (85.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `DemoTypeHookProc` **(Many-Argument Workhorses)** (Impact: 83.2)
    * *Intent:* //---------------------------------------------------------------------------- // // DemoTypeHookPro...
  * `InjectByClipboard` **(Many-Argument Workhorses)** (Impact: 41.3)
    * *Intent:* //---------------------------------------------------------------------------- // // InjectByClipboa...
  * `StartDemoType` **(Many-Argument Workhorses)** (Impact: 31.4)
    * *Intent:* //---------------------------------------------------------------------------- // // StartDemoType /...
  * `HandleControlKeyword` **(I/O & Config Routines)** (Impact: 31.2)
    * *Intent:* //---------------------------------------------------------------------------- // // HandleControlKe...
  * `GetDemoTypeFile` **(Compute Cores)** (Impact: 30.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 0.142; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (43.9%), Guard Balance (formerly Safety Score) (37.8%), Complexity Load (formerly Cognitive Load) (31.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ShouldReplacePasteAIConfiguration` **(Defensive Guards)** (Impact: 93.7)
  * `OnCustomActionsCollectionChanged` **(Stateful Encapsulated Methods)** (Impact: 24.7)
  * `AdvancedPasteViewModel` **(Defensive Guards)** (Impact: 23.0)
  * `MigrateLegacyAIEnablement` **(I/O & Config Routines)** (Impact: 21.1)
  * `OnPasteAIProvidersCollectionChanged` **(Stateful Encapsulated Methods)** (Impact: 20.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **24**; blast radius 0.155; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Guard Balance (formerly Safety Score) (81.9%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (54.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `SetBundleInstallLocationCA` **(Compute Cores)** (Impact: 29.7)
  * `TerminateProcessesCA` **(Compute Cores)** (Impact: 25.6)
  * `RemoveScheduledTasksCA` **(Compute Cores)** (Impact: 24.0)
    * *Intent:* // Removes all Scheduled Tasks in the PowerToys folder and deletes the folder afterwards. // Based o...
  * `UninstallPackageIdentityMSIXCA` **(Defensive Guards)** (Impact: 21.7)
  * `InstallPackageIdentityMSIXCA` **(Compute Cores)** (Impact: 21.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.000189
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **11**; blast radius 0.156; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.5%), Complexity Load (formerly Cognitive Load) (92.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `D2DOverlayWindow::render` **(Compute Cores)** (Impact: 122.4)
  * `D2DOverlayWindow::show` **(Many-Argument Workhorses)** (Impact: 36.2)
  * `get_window_state` **(Compute Cores)** (Impact: 30.8)
  * `render_arrow` **(Many-Argument Workhorses)** (Impact: 28.3)
  * `D2DOverlaySVG::get_thumbnail_rect_and_scale` **(Many-Argument Workhorses)** (Impact: 18.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.000284
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.2%), Complexity Load (formerly Cognitive Load) (71.1%), Debt Markers (formerly Tech Debt) (45.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `WICMetadataExtractor::ReadDouble` **(Many-Argument Workhorses)** (Impact: 94.6)
  * `ParseIso8601DateTime` **(Compute Cores)** (Impact: 80.8)
  * `ValidateAndBuildSystemTime` **(Many-Argument Workhorses)** (Impact: 49.5)
  * `WICMetadataExtractor::ExtractAllEXIFFields` **(Many-Argument Workhorses)** (Impact: 43.2)
  * `WICMetadataExtractor::ReadDateTime` **(Compute Cores)** (Impact: 36.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **21**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (66.9%), Complexity Load (formerly Cognitive Load) (42.1%)
- **Documentation Coverage:** 98.4% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `SkSend` **(Many-Argument Workhorses)** (Impact: 34.9)
    * *Intent:* #endif
  * `ExecuteAndTrace` **(Many-Argument Workhorses)** (Impact: 21.4)
  * `ShowToolTip` **(Stateful Encapsulated Methods)** (Impact: 20.9)
  * `SendOrReceiveARandomDataBlockPerInitialIV` **(Stateful Encapsulated Methods)** (Impact: 20.7)
  * `InvokeInFormThread` **(Many-Argument Workhorses)** (Impact: 16.0)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_4` (Drift: 0.0 IQR)
- **Magnitude:** 823.2 | **LOC:** 927 | **CtrlFlow:** 25.1% | **Authorship Centralization:** 66.7%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Complexity Load (formerly Cognitive Load) (91.8%), Guard Balance (formerly Safety Score) (90.7%), Debt Markers (formerly Tech Debt) (38.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `GetTransformedFileName` **(Many-Argument Workhorses)** (Impact: 201.2)
  * `GetEnumeratedFileName` **(Many-Argument Workhorses)** (Impact: 135.1)
  * `GetMetadataFileName` **(Many-Argument Workhorses)** (Impact: 37.7)
  * `GetDatedFileName` **(Many-Argument Workhorses)** (Impact: 26.1)
  * `isMetadataUsed` **(Many-Argument Workhorses)** (Impact: 21.4)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `safety_bypasses: 2`, `state_mutation: 120`, `unreferenced_by_name: 15`
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 0.142; role: Isolated/Orphan
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (100.0%), Mutation Surface (formerly State Flux) (95.0%), Guard Balance (formerly Safety Score) (88.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `HelperTimer_Tick` **(Many-Argument Workhorses)** (Impact: 113.4)
  * `WndProc` **(I/O & Config Routines)** (Impact: 46.2)
    * *Intent:* // private bool checkClipboard = false;
  * `PaintMyNameOnDesktop` **(I/O & Config Routines)** (Impact: 26.6)
  * `ShowMessageOnLogonDesktop` **(Stateful Encapsulated Methods)** (Impact: 18.3)
  * `ChangeIcon` **(I/O & Config Routines)** (Impact: 16.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (89.3%), Guard Balance (formerly Safety Score) (88.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 11.5385% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ParseRegistryFile` **(Many-Argument Workhorses)** (Impact: 159.6)
    * *Intent:* /// <summary> /// Parses the text that is passed in, which should be the same text that's in editor ...
  * `AddTextToTree` **(Many-Argument Workhorses)** (Impact: 37.8)
    * *Intent:* /// <summary> /// Helper method that creates a new TreeView node, attaches it to a parent if any, an...
  * `HandleDirtyClosing` **(Many-Argument Workhorses)** (Impact: 27.6)
    * *Intent:* /// <summary> /// Wrapper method that shows a Save/Don't Save/Cancel message box, parented by the ma...
  * `SetValueToolTip` **(Stateful Encapsulated Methods)** (Impact: 18.1)
    * *Intent:* /// <summary> /// Loads a string for a given Value's image in the grid, based off the current type a...
  * `GetFolderToolTip` **(Stateful Encapsulated Methods)** (Impact: 12.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.142; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.3%), Mutation Surface (formerly State Flux) (95.0%), Complexity Load (formerly Cognitive Load) (51.4%), Debt Markers (formerly Tech Debt) (9.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `InitializeComponent` **(I/O & Config Routines)** (Impact: 38.5)
    * *Intent:* #region Windows Form Designer generated code /// <summary> /// Required method for Designer support ...
  * `Dispose` **(Stateful Encapsulated Methods)** (Impact: 3.2)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (45.5%)
- **Documentation Coverage:** 23.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_print_layout_diagram` **(Many-Argument Workhorses)** (Impact: 63.2)
    * *Intent:* """Print a text-based diagram of the monitor layout"""
  * `_run_test_config` **(Many-Argument Workhorses)** (Impact: 33.7)
  * `_is_outer_edge` **(Stateful Encapsulated Methods)** (Impact: 17.4)
    * *Intent:* """ Determine if an edge is "outer" (can wrap) Rules: 1. If edge has an adjacent opposite edge (with...
  * `_get_test_points_on_edge` **(Stateful Encapsulated Methods)** (Impact: 17.0)
  * `_wrap_from_edge` **(Stateful Encapsulated Methods)** (Impact: 16.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (82.9%), Guard Balance (formerly Safety Score) (61.6%), Complexity Load (formerly Cognitive Load) (41.3%)
- **Documentation Coverage:** 90.566% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `TrySetSelectionAfterUpdate` **(Many-Argument Workhorses)** (Impact: 57.1)
    * *Intent:* /// <summary> /// Applies selection after an items update. Returns false if ItemView.Items /// is no...
  * `HandleGridArrowNavigation` **(Compute Cores)** (Impact: 45.8)
    * *Intent:* // Find a logical neighbor in the requested direction using containers' positions.
  * `CalculateTargetIndexPageUpDownScrollTo` **(Compute Cores)** (Impact: 34.6)
    * *Intent:* /// <summary> /// Calculates the item index to target when performing a page up or page down /// nav...
  * `DelayRenderer` **(Stateful Encapsulated Methods)** (Impact: 21.6)
  * `Items_PreviewKeyDown` **(Stateful Encapsulated Methods)** (Impact: 20.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 0.142; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (83.4%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (55.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `MoveRight` **(Many-Argument Workhorses)** (Impact: 67.6)
    * *Intent:* #endif
  * `MoveLeft` **(Many-Argument Workhorses)** (Impact: 60.8)
  * `MoveToMyNeighbourIfNeeded` **(Compute Cores)** (Impact: 60.6)
    * *Intent:* #if OLD_VERSION
  * `MoveToMyNeighbourIfNeeded` **(Many-Argument Workhorses)** (Impact: 50.0)
    * *Intent:* /* Let's say we have 3 machines A, B, and C. A is the controller machine. * (x, y) is the current Mo...
  * `MoveUp` **(Many-Argument Workhorses)** (Impact: 36.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.2%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (77.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `MonitorTopology::IsOnOuterEdge` **(Many-Argument Workhorses)** (Impact: 101.8)
  * `MonitorTopology::FindNearestOppositeEdge` **(Many-Argument Workhorses)** (Impact: 64.0)
  * `MonitorTopology::GetWrapDestination` **(Many-Argument Workhorses)** (Impact: 50.4)
  * `MonitorTopology::PrioritizeEdgeByDirection` **(Compute Cores)** (Impact: 48.4)
  * `MonitorTopology::FindOppositeOuterEdge` **(Compute Cores)** (Impact: 38.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 0.142; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (81.9%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (43.0%)
- **Documentation Coverage:** 90.1639% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `ScoreNonContiguousWithPositions` **(Many-Argument Workhorses)** (Impact: 59.3)
    * *Intent:* // ============================================================ // Non-contiguous matching (with pos...
  * `ScoreBestVariantWithPositions` **(Many-Argument Workhorses)** (Impact: 37.6)
  * `ScoreNonContiguous` **(Many-Argument Workhorses)** (Impact: 33.3)
    * *Intent:* // ============================================================ // Non-contiguous matching (score on...
  * `FoldForComparison` **(Compute Cores)** (Impact: 23.6)
    * *Intent:* /// <summary> /// Creates a folded string for fast equality comparisons: /// - ALWAYS normalizes sla...
  * `ScoreContiguous` **(Many-Argument Workhorses)** (Impact: 20.4)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **9**; blast radius 0.167; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (85.3%), Connectivity (formerly Api Exposure) (67.0%), Complexity Load (formerly Cognitive Load) (47.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `CPowerRenameManager::s_fileOpWorkerThread` **(Compute Cores)** (Impact: 38.5)
  * `CPowerRenameManager::_WndProc` **(Many-Argument Workhorses)** (Impact: 35.7)
  * `CPowerRenameManager::_PerformFileOperation` **(I/O & Config Routines)** (Impact: 13.7)
  * `CPowerRenameManager::UpdateChildrenPath` **(Compute Cores)** (Impact: 12.2)
  * `CPowerRenameManager::SetVisible` **(I/O & Config Routines)** (Impact: 12.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 0.167
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.000284
...
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (75.2%), Complexity Load (formerly Cognitive Load) (43.8%), Debt Markers (formerly Tech Debt) (43.3%)
- **Documentation Coverage:** 93.75% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `QueryResults` **(Defensive Guards)** (Impact: 41.2)
  * `SetHotkey` **(Many-Argument Workhorses)** (Impact: 21.2)
  * `OpenResultsEvent` **(Many-Argument Workhorses)** (Impact: 20.3)
  * `RegisterHotkey` **(Defensive Guards)** (Impact: 16.8)
  * `UpdateResultsListViewAfterQuery` **(Stateful Encapsulated Methods)** (Impact: 13.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 0.142; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.8%), Complexity Load (formerly Cognitive Load) (92.9%), Debt Markers (formerly Tech Debt) (83.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `GetShortcutRemapByType` **(Many-Argument Workhorses)** (Impact: 138.4)
  * `GetShortcutRemapCountByType` **(Many-Argument Workhorses)** (Impact: 67.2)
  * `AddShortcutRemap` **(Many-Argument Workhorses)** (Impact: 55.0)
  * `GetShortcutRemap` **(Many-Argument Workhorses)** (Impact: 34.4)
  * `DeleteShortcutRemap` **(Many-Argument Workhorses)** (Impact: 20.4)
...
- `src/modules/ZoomIt/ZoomIt/Zoomit.cpp` -> Churn: **82.17%** | Cog Load: 89.0414% | Debt: 8.317%
...
- `src/modules/cmdpal/Microsoft.CmdPal.UI.ViewModels/TopLevelCommandManager.cs` -> Churn: **71.65%** | Cog Load: 85.4437% | Debt: 22.4604%

### 👤 Key Person Dependencies (High Impact + Siloed Knowledge)
These are massive, load-bearing files written almost entirely by a single developer. They represent severe 'Bus Factor' risk.

- `src/modules/ZoomIt/ZoomIt/PanoramaCapture.cpp` -> **Alex Mihaiuc** (100.0% isolated ownership) | Magnitude: 10859.32
...
- `src/modules/keyboardmanager/common/Input.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 13.7083%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `src/modules/keyboardmanager/common/Input.h` -> **Severity: 1.523** (Embedded: 0.0252 * Error Risk: 60.5532%)
- `src/common/utils/json.h` -> **Severity: 1.394** (Embedded: 0.0236 * Error Risk: 58.9541%)
- `src/common/utils/winapi_error.h` -> **Severity: 1.331** (Embedded: 0.0266 * Error Risk: 50.0%)
- `src/common/utils/resources.h` -> **Severity: 0.988** (Embedded: 0.0101 * Error Risk: 97.7957%)
- `src/common/utils/process_path.h` -> **Severity: 0.838** (Embedded: 0.0114 * Error Risk: 73.7456%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `src/modules/keyboardmanager/common/Input.h` -> **Severity: 1422.6** (Blast Radius: 14.226 * Doc Risk: 100.0%)
- `src/common/logger/logger.h` -> **Severity: 939.4** (Blast Radius: 9.394 * Doc Risk: 100.0%)
- `src/common/utils/json.h` -> **Severity: 878.6** (Blast Radius: 8.786 * Doc Risk: 100.0%)
- `src/common/Telemetry/TraceBase.h` -> **Severity: 683.896** (Blast Radius: 7.845 * Doc Risk: 87.176%)
- `src/common/utils/winapi_error.h` -> **Severity: 556.0** (Blast Radius: 5.56 * Doc Risk: 100.0%)
