...
## 1. EXECUTIVE SUMMARY
- **Scope:** 8980 analyzed artifact(s), 1014107 LOC.
- **Load-bearing artifact:** `providers/common/compat/src/airflow/providers/common/compat/sdk.py` -- 1205 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `clients/python/README.md` -- pulls in 157 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `dev/breeze/src/airflow_breeze/commands/pr_commands.py` at magnitude 13439.42 (structural weight, not risk).
...
| Analyzed Artifacts (Scanned) | 8980 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 3080 |
| Total LOC | 1014107 |
| Volatility Index | 0.002 |
| % Scanned of codebase = | 74.5% |
| Dominant Lang | PYTHON |

## 3.5 MACRO-NETWORK TOPOLOGY (Resilience & Coupling)
| Metric | Value | Interpretation |
|---|---|---|
| Modularity | 0.5941 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 5.7019 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
| Articulation Pts | 549 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| PYTHON | 6731 | 874139 | 75.0% |
| TYPESCRIPT | 766 | 49998 | 8.5% |
| JSON | 382 | 40031 | 4.3% |
| PLAINTEXT | 314 | 0 | 3.5% |
| YAML | 296 | 24996 | 3.3% |
| XML | 134 | 6 | 1.5% |
...
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +4.89; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 38%, Large Core Modules (2) 16%, Generic / Templated Code Files 11%, Declarative / Non-Code 10%, Large Core Modules (3) 7%
...
|---|---|---|
| Unclassified | 8549 | 95.2% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 426 | 4.7% |
| Static: Minified & Vendor Opaque Mass | 5 | 0.1% |

## 5. EXCLUDED ARTIFACTS (Unparsable or Shielded Files)
*Total Excluded Artifacts: 3080*
...
- `.yml`: 61x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Massive Static Asset Blob: 2970 LOC)
...
- `.yaml`: 19x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Massive Static Asset Blob: 6240 LOC), 1x Excluded (Massive Static Asset Blob: 3631 LOC)
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 16.1 | 3.3 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.9 | 42.5 | 52.7 | 0.0 |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 13.6 | 5.6 | 0.0 |
| Concurrency Surface (formerly Concurrency Exposure) | 0.0 | 100.0 | 6.4 | 0.0 | 0.0 |
| Mutation Surface (formerly State Flux Exposure) | 0.0 | 100.0 | 27.9 | 0.0 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 41.4 | 14.3 | 0.0 |
...
| connectivity | 52423 | 5194 | 16 | `task-sdk/tests/task_sdk/execution_time/test_task_runner.py` |
...
| docs | 24489 | 3722 | 7 | `providers/google/src/airflow/providers/google/cloud/hooks/vertex_ai/custom_job.py` |
| debt | 7729 | 1117 | 1 | `Dockerfile` |
| mutation | 416427 | 6098 | 114 | `airflow-core/tests/unit/jobs/test_scheduler_job.py` |
| dead_code | 20940 | 2195 | 6 | `airflow-core/tests/unit/jobs/test_scheduler_job.py` |
| credential | 338 | 173 | 0 | `Dockerfile` |
| threat | 4704 | 1564 | 1 | `airflow-core/src/airflow/serialization/serialized_objects.py` |
...
- `_run_tui_triage` **(Many-Argument Workhorses)** (@ `dev/breeze/src/airflow_breeze/commands/pr_commands.py`) -> Impact: **1845.0** | LOC: 1392
- `auto_triage` **(Many-Argument Workhorses)** (@ `dev/breeze/src/airflow_breeze/commands/pr_commands.py`) -> Impact: **844.3** | LOC: 799
- `_evaluate_trigger_rule` **(Many-Argument Workhorses)** (@ `airflow-core/src/airflow/ti_deps/deps/trigger_rule_dep.py`) -> Impact: **424.3** | LOC: 571
- `__init__` **(Many-Argument Workhorses)** (@ `providers/cncf/kubernetes/src/airflow/providers/cncf/kubernetes/operators/pod.py`) -> Impact: **384.6** | LOC: 174
- `_review_ready_prs_review_mode` **(Many-Argument Workhorses)** (@ `dev/breeze/src/airflow_breeze/commands/pr_commands.py`) -> Impact: **319.7** | LOC: 414
- `_run_startup_enrichment` **(Many-Argument Workhorses)** (@ `dev/breeze/src/airflow_breeze/commands/pr_commands.py`) -> Impact: **273.0** | LOC: 183
- `upgrade` **(Many-Argument Workhorses)** (@ `dev/breeze/src/airflow_breeze/commands/ci_commands.py`) -> Impact: **266.5** | LOC: 342
- `_review_workflow_approval_prs` **(Many-Argument Workhorses)** (@ `dev/breeze/src/airflow_breeze/commands/pr_commands.py`) -> Impact: **263.4** | LOC: 488
  * *Intent:* """Present NOT_RUN PRs for workflow approval. Mutates ctx.stats."""
- `__init__` **(Many-Argument Workhorses)** (@ `task-sdk/src/airflow/sdk/bases/operator.py`) -> Impact: **244.9** | LOC: 217
- `find_installation_spec` **(Many-Argument Workhorses)** (@ `scripts/in_container/install_airflow_and_providers.py`) -> Impact: **241.5** | LOC: 247
...
| `airflow-core/src/airflow/models` | 40 | 8809.26 | 30.29% | 6.37% |
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **47**; blast radius 0.085; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (83.7%), Complexity Load (formerly Cognitive Load) (81.8%)
- **Documentation Coverage:** 50.2488% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_run_tui_triage` **(Many-Argument Workhorses)** (Impact: 1845.0)
  * `auto_triage` **(Many-Argument Workhorses)** (Impact: 844.3)
  * `_review_ready_prs_review_mode` **(Many-Argument Workhorses)** (Impact: 319.7)
  * `_run_startup_enrichment` **(Many-Argument Workhorses)** (Impact: 273.0)
  * `_review_workflow_approval_prs` **(Many-Argument Workhorses)** (Impact: 263.4)
    * *Intent:* """Present NOT_RUN PRs for workflow approval. Mutates ctx.stats."""
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6e-06 | `Ripple Effect (Closeness):` 0.000223
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **82**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (96.2%), Guard Balance (formerly Safety Score) (70.0%), Complexity Load (formerly Cognitive Load) (31.7%), Connectivity (formerly Api Exposure) (10.8%)
- **Documentation Coverage:** 54.3396% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_find_executable_task_instances_executor_with_teams` **(Many-Argument Workhorses)** (Impact: 31.1)
    * *Intent:* """ Test that tasks are correctly routed to team-specific executors when multi-team is enabled """
  * `test_handle_stuck_queued_tasks_reschedule_sensors` **(Many-Argument Workhorses)** (Impact: 29.6)
    * *Intent:* """Reschedule sensors go in and out of running repeatedly using the same try_number Make sure that t...
  * `task_maker` **(Many-Argument Workhorses)** (Impact: 25.2)
  * `test_should_mark_empty_task_as_success` **(Defensive Guards)** (Impact: 24.4)
  * `test_handle_stuck_queued_tasks_multiple_attempts` **(Many-Argument Workhorses)** (Impact: 23.9)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **67**; blast radius 0.048; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (99.1%), Guard Balance (formerly Safety Score) (91.5%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 85.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `prepare_provider_documentation` **(Many-Argument Workhorses)** (Impact: 142.8)
  * `prepare_python_client` **(Many-Argument Workhorses)** (Impact: 116.3)
  * `generate_issue_content_providers` **(Many-Argument Workhorses)** (Impact: 107.4)
  * `prepare_helm_chart_tarball` **(Many-Argument Workhorses)** (Impact: 83.8)
  * `prepare_provider_distributions` **(Many-Argument Workhorses)** (Impact: 81.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.000308
...
- **Blast Radius:** changing it is visible to **10** in-repo importer(s); it depends on **106**; blast radius 0.079; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.0%), Complexity Load (formerly Cognitive Load) (75.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (74.4%)
- **Documentation Coverage:** 71.1744% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `dag_maker` **(Compute Cores)** (Impact: 159.1)
    * *Intent:* """ Fixture to help create DAG, DagModel, and SerializedDAG automatically. You have to use the dag_m...
  * `_create_task_instance` **(Many-Argument Workhorses)** (Impact: 106.6)
  * `maker` **(Many-Argument Workhorses)** (Impact: 85.2)
  * `create_dagrun` **(Many-Argument Workhorses)** (Impact: 67.0)
  * `run_task` **(Stateful Encapsulated Methods)** (Impact: 64.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.5e-05 | `Ripple Effect (Closeness):` 0.001114
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **20**; blast radius 0.067; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (65.5%)
- **Documentation Coverage:** 9.3333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_build_detail_lines` **(Many-Argument Workhorses)** (Impact: 152.6)
    * *Intent:* """Build the full list of detail lines for a PR entry (not truncated)."""
  * `run_interactive` **(Many-Argument Workhorses)** (Impact: 148.4)
  * `_build_pr_table` **(Many-Argument Workhorses)** (Impact: 126.1)
    * *Intent:* """Build the scrollable PR list table."""
  * `_read_tui_key` **(Compute Cores)** (Impact: 62.5)
    * *Intent:* """Read a keypress or mouse event and map it to a TUIAction, MouseEvent, or raw character. Returns `...
  * `_build_header` **(Stateful Encapsulated Methods)** (Impact: 52.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3e-06 | `Ripple Effect (Closeness):` 0.000297
...
- **Blast Radius:** changing it is visible to **47** in-repo importer(s); it depends on **74**; blast radius 2.256; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (63.0%)
- **Documentation Coverage:** 60.5714% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `serialize` **(Defensive Guards)** (Impact: 101.5)
  * `populate_operator` **(Many-Argument Workhorses)** (Impact: 101.2)
  * `conversion_v1_to_v2` **(Compute Cores)** (Impact: 72.9)
  * `deserialize` **(Compute Cores)** (Impact: 72.0)
    * *Intent:* """ Deserialize an object; helper function of depth first search for deserialization. :meta private:...
  * `_is_excluded` **(Many-Argument Workhorses)** (Impact: 59.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003573 | `Ripple Effect (Closeness):` 0.131684
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **78**; blast radius 0.187; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (97.2%), Guard Balance (formerly Safety Score) (89.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 30.9211% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `process_executor_events` **(Many-Argument Workhorses)** (Impact: 154.5)
  * `_executable_task_instances_to_queued` **(Many-Argument Workhorses)** (Impact: 142.8)
    * *Intent:* """ Find TIs that are ready for execution based on conditions. Conditions include: - pool limits - D...
  * `_try_to_load_executor` **(Many-Argument Workhorses)** (Impact: 52.4)
  * `_schedule_dag_run` **(Many-Argument Workhorses)** (Impact: 52.2)
  * `_create_dag_runs` **(Many-Argument Workhorses)** (Impact: 38.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000468 | `Ripple Effect (Closeness):` 0.102019
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **56**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (81.9%), Guard Balance (formerly Safety Score) (54.9%), Concurrency Surface (formerly Concurrency) (15.2%), Complexity Load (formerly Cognitive Load) (13.9%)
- **Documentation Coverage:** 67.6887% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_xcom_pull` **(Many-Argument Workhorses)** (Impact: 63.2)
  * `test_xcom_pull_return_values` **(Many-Argument Workhorses)** (Impact: 18.0)
  * `test_task_span_is_child_of_dag_run_span` **(Defensive Guards)** (Impact: 12.5)
    * *Intent:* """Full trace hierarchy: dag_run → task_run.my_task (API server) → worker.my_task (task runner)."""
  * `test_rendered_templates_mask_secrets_with_truncation` **(Defensive Guards)** (Impact: 12.4)
    * *Intent:* """Test that secrets are masked before truncation when rendered fields exceed max_templated_field_le...
  * `test_task_runner_both_callbacks_have_timing_info` **(Defensive Guards)** (Impact: 12.0)
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **52**; blast radius 0.216; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (93.1%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (68.8%)
- **Documentation Coverage:** 26.9912% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `auth_user_ldap` **(Many-Argument Workhorses)** (Impact: 98.5)
    * *Intent:* """ Authenticate user with LDAP. NOTE: this depends on python-ldap module. :param username: the user...
  * `_sync_dag_view_permissions` **(Many-Argument Workhorses)** (Impact: 56.5)
  * `auth_user_oauth` **(Many-Argument Workhorses)** (Impact: 43.0)
    * *Intent:* """ Authenticate user with OAuth. :userinfo: dict with user information (keys are the same as User m...
  * `get_oauth_user_info` **(Many-Argument Workhorses)** (Impact: 36.0)
    * *Intent:* """ There are different OAuth APIs with different ways to retrieve user info. All providers have dif...
  * `_search_ldap` **(Many-Argument Workhorses)** (Impact: 35.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000905 | `Ripple Effect (Closeness):` 0.07172
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **51**; blast radius 0.176; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (96.3%), Concurrency Surface (formerly Concurrency) (94.1%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 59.7561% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_prepare_query_configuration` **(Many-Argument Workhorses)** (Impact: 172.5)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 65.2)
  * `split_tablename` **(Many-Argument Workhorses)** (Impact: 45.4)
  * `interval_check` **(Many-Argument Workhorses)** (Impact: 41.1)
  * `create_empty_dataset` **(Many-Argument Workhorses)** (Impact: 34.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.8e-05 | `Ripple Effect (Closeness):` 0.00317
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **37**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (73.0%), Guard Balance (formerly Safety Score) (42.6%), Complexity Load (formerly Cognitive Load) (23.0%), Concurrency Surface (formerly Concurrency) (14.0%)
- **Documentation Coverage:** 90.9091% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_should_handle_task_instance_deletion` **(Many-Argument Workhorses)** (Impact: 56.1)
  * `create_task_instances` **(Many-Argument Workhorses)** (Impact: 43.3)
  * `test_should_respond_200_with_mapped_task_at_different_try_numbers` **(Many-Argument Workhorses)** (Impact: 26.1)
  * `test_bulk_task_instances` **(Many-Argument Workhorses)** (Impact: 18.9)
  * `test_mapped_task_should_respond_200` **(Many-Argument Workhorses)** (Impact: 17.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Blast Radius:** changing it is visible to **124** in-repo importer(s); it depends on **76**; blast radius 3.57; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (89.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (86.9%), Connectivity (formerly Api Exposure) (80.7%)
- **Documentation Coverage:** 48.1752% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `register_asset_changes_in_db` **(Many-Argument Workhorses)** (Impact: 101.9)
  * `_check_and_change_state_before_execution` **(Many-Argument Workhorses)** (Impact: 92.1)
  * `clear_task_instances` **(Many-Argument Workhorses)** (Impact: 89.4)
  * `xcom_pull` **(Many-Argument Workhorses)** (Impact: 79.9)
  * `find_relevant_relatives` **(Many-Argument Workhorses)** (Impact: 32.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004042 | `Ripple Effect (Closeness):` 0.147108
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **58**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (75.2%), Guard Balance (formerly Safety Score) (51.7%), Complexity Load (formerly Cognitive Load) (29.9%), Concurrency Surface (formerly Concurrency) (12.9%)
- **Documentation Coverage:** 77.6316% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_tis_considered_for_state` **(Many-Argument Workhorses)** (Impact: 22.6)
    * *Intent:* """ We use a convenience notation to wire up test scenarios: t<num> -- teardown task t<num>_ -- tear...
  * `create_dag_run` **(Many-Argument Workhorses)** (Impact: 20.7)
  * `test_schedulable_task_exist_when_rerun_removed_upstream_mapped_task` **(Defensive Guards)** (Impact: 19.8)
  * `test_mapped_task_rerun_with_different_length_of_args` **(Many-Argument Workhorses)** (Impact: 18.5)
  * `test_mapped_expand_kwargs` **(Defensive Guards)** (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **27** in-repo importer(s); it depends on **30**; blast radius 0.236; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.4%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (57.1%)
- **Documentation Coverage:** 77.686% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 131.9)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 48.2)
  * `_build_cluster_data` **(Compute Cores)** (Impact: 48.0)
  * `execute` **(Many-Argument Workhorses)** (Impact: 39.6)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 30.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.4e-05 | `Ripple Effect (Closeness):` 0.003007
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **14**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Concurrency Surface (formerly Concurrency) (100.0%), Complexity Load (formerly Cognitive Load) (82.0%), Guard Balance (formerly Safety Score) (62.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (26.9%)
- **Documentation Coverage:** 68.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_a_get_federated_token_databricks_error` **(Many-Argument Workhorses)** (Impact: 8.7)
    * *Intent:* """Test async error handling when Databricks token exchange fails."""
  * `test_a_get_sp_token_retry_error` **(Defensive Guards)** (Impact: 8.5)
  * `test_get_federated_token_databricks_error` **(Many-Argument Workhorses)** (Impact: 7.4)
    * *Intent:* """Test error handling when Databricks token exchange fails."""
  * `test_get_federated_token` **(Defensive Guards)** (Impact: 5.4)
    * *Intent:* """Test Kubernetes OIDC token federation flow."""
  * `test_get_token_managed_identity` **(Defensive Guards)** (Impact: 5.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **24**; blast radius 3.312; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.9%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (71.8%)
- **Documentation Coverage:** 37.2414% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_replace_section_config_with_display_sources` **(Many-Argument Workhorses)** (Impact: 75.3)
  * `_write_option_header` **(Many-Argument Workhorses)** (Impact: 61.5)
  * `_write_value` **(Many-Argument Workhorses)** (Impact: 48.6)
  * `as_dict` **(Many-Argument Workhorses)** (Impact: 47.3)
  * `write` **(Many-Argument Workhorses)** (Impact: 42.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8.6e-05 | `Ripple Effect (Closeness):` 0.132521
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **34**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (80.7%), Complexity Load (formerly Cognitive Load) (57.2%), Guard Balance (formerly Safety Score) (54.2%)
- **Documentation Coverage:** 65.5172% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_get_tasks_details` **(I/O & Config Routines)** (Impact: 14.0)
  * `test_get_tasks_details_with_edge_labels` **(I/O & Config Routines)** (Impact: 10.3)
    * *Intent:* """ Comprehensive test for dag with labeled edges. See ``_build_labeled_edge_map`` docstring for cla...
  * `test_get_airflow_dag_run_facet` **(I/O & Config Routines)** (Impact: 7.0)
  * `test_get_tasks_large_dag` **(I/O & Config Routines)** (Impact: 7.0)
    * *Intent:* """Test how get_tasks behaves for a large dag with many dependent tasks."""
  * `test_dag_info_schedule_dataset_or_time_schedule` **(I/O & Config Routines)** (Impact: 6.7)
...
- **Blast Radius:** changing it is visible to **89** in-repo importer(s); it depends on **65**; blast radius 2.017; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (88.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (81.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 77.3109% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `update_state` **(Many-Argument Workhorses)** (Impact: 111.6)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 83.7)
  * `schedule_tis` **(Many-Argument Workhorses)** (Impact: 64.9)
  * `find` **(Many-Argument Workhorses)** (Impact: 52.4)
  * `_get_ready_tis` **(Many-Argument Workhorses)** (Impact: 43.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001751 | `Ripple Effect (Closeness):` 0.140505
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **85**; blast radius 0.05; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (67.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (54.9%), Guard Balance (formerly Safety Score) (44.1%), Concurrency Surface (formerly Concurrency) (32.4%)
- **Documentation Coverage:** 50.9036% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `validate_deserialized_task` **(Many-Argument Workhorses)** (Impact: 44.6)
  * `test_task_callback_boolean_optimization` **(Defensive Guards)** (Impact: 24.4)
    * *Intent:* """Test that task callbacks are optimized using has_on_*_callback boolean flags."""
  * `validate_deserialized_dag` **(Defensive Guards)** (Impact: 24.3)
    * *Intent:* """ Verify that all example DAGs work with DAG Serialization by checking fields between Serialized D...
  * `collect_dags` **(Compute Cores)** (Impact: 15.9)
    * *Intent:* """Collects DAGs to test."""
  * `test_mapped_operator_client_defaults_optimization` **(Many-Argument Workhorses)** (Impact: 15.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.000111
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **45**; blast radius 0.236; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (67.6%)
- **Documentation Coverage:** 63.75% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 384.6)
  * `cleanup` **(Many-Argument Workhorses)** (Impact: 89.4)
  * `build_pod_request_obj` **(Many-Argument Workhorses)** (Impact: 42.3)
    * *Intent:* """ Return V1Pod object based on pod template file, full pod spec, and other operator parameters. Th...
  * `invoke_defer_method` **(Many-Argument Workhorses)** (Impact: 35.2)
  * `trigger_reentry` **(Many-Argument Workhorses)** (Impact: 31.4)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 0.236
  * `Choke Point (Betweenness):` 0.000557 | `Ripple Effect (Closeness):` 0.060664
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **75**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (81.9%), Guard Balance (formerly Safety Score) (31.1%), Complexity Load (formerly Cognitive Load) (24.0%), Concurrency Surface (formerly Concurrency) (12.9%)
- **Documentation Coverage:** 78.6127% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_check_task_dependencies` **(Many-Argument Workhorses)** (Impact: 21.7)
  * `test_outlet_multiple_asset_alias` **(Defensive Guards)** (Impact: 19.6)
  * `test_clear_task_instances_recalculates_dagrun_queued_deadlines` **(Many-Argument Workhorses)** (Impact: 16.1)
    * *Intent:* """Test that clearing tasks recalculates all (and only) DAGRUN_QUEUED_AT deadlines."""
  * `test_refresh_from_db` **(Many-Argument Workhorses)** (Impact: 15.8)
  * `test_check_task_dependencies_for_mapped` **(Many-Argument Workhorses)** (Impact: 15.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **50** in-repo importer(s); it depends on **35**; blast radius 0.835; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Concurrency Surface (formerly Concurrency) (99.8%), Guard Balance (formerly Safety Score) (96.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 66.3551% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `list_keys` **(Many-Argument Workhorses)** (Impact: 52.9)
  * `is_keys_unchanged_async` **(Many-Argument Workhorses)** (Impact: 38.7)
  * `copy_object` **(Many-Argument Workhorses)** (Impact: 35.9)
  * `download_file` **(Many-Argument Workhorses)** (Impact: 35.8)
  * `get_files_async` **(Many-Argument Workhorses)** (Impact: 25.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8.3e-05 | `Ripple Effect (Closeness):` 0.009384
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **26**; blast radius 0.078; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.6%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (73.2%)
- **Documentation Coverage:** 90.1869% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_check_provider_deps_in_list` **(Many-Argument Workhorses)** (Impact: 44.9)
  * `_get_providers_test_types_to_run` **(Stateful Encapsulated Methods)** (Impact: 36.9)
  * `provider_dependency_bump` **(Defensive Guards)** (Impact: 34.9)
    * *Intent:* """Check for apache-airflow-providers dependency bumps in pyproject.toml files."""
  * `docs_list_as_string` **(Compute Cores)** (Impact: 34.4)
  * `get_job_label` **(Many-Argument Workhorses)** (Impact: 32.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7e-06 | `Ripple Effect (Closeness):` 0.000784
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **32**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Complexity Load (formerly Cognitive Load) (57.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (57.2%), Guard Balance (formerly Safety Score) (46.2%), Concurrency Surface (formerly Concurrency) (29.3%)
- **Documentation Coverage:** 83.0435% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_mark_checked_if_not_deleted` **(Many-Argument Workhorses)** (Impact: 27.8)
  * `create_context` **(Many-Argument Workhorses)** (Impact: 24.9)
  * `test_omitted_namespace_with_conn` **(Many-Argument Workhorses)** (Impact: 24.4)
  * `test_pod_with_istio_delete_after_await_container_error` **(Many-Argument Workhorses)** (Impact: 24.1)
  * `test_pod_delete_not_called_when_creation_fails` **(Many-Argument Workhorses)** (Impact: 16.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **66**; blast radius 0.045; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (75.2%), Guard Balance (formerly Safety Score) (33.6%), Complexity Load (formerly Cognitive Load) (18.5%), Concurrency Surface (formerly Concurrency) (16.7%)
- **Documentation Coverage:** 76.1905% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_bulk_write_to_db` **(Defensive Guards)** (Impact: 24.8)
  * `make_tasks` **(Compute Cores)** (Impact: 15.9)
    * *Intent:* """ Helper for building setup and teardown tasks for testing. Given an input such as 's1, w1, t1, tf...
  * `test_bulk_write_to_db_max_active_runs` **(Many-Argument Workhorses)** (Impact: 15.7)
    * *Intent:* """ Test that DagModel.next_dagrun_create_after is set to NULL when the dag cannot be created due to...
  * `test_bulk_write_to_db_assets` **(Many-Argument Workhorses)** (Impact: 14.1)
    * *Intent:* """ Ensure that assets referenced in a dag are correctly loaded into the database. """
  * `test_dag_with_multiple_deadlines` **(Many-Argument Workhorses)** (Impact: 11.8)
...
- `providers/common/compat/src/airflow/providers/common/compat/sdk.py` -> **Severity: 0.459** (Bridge: 0.0094 * Flux: 48.8053%)
- `airflow-core/src/airflow/models/taskinstance.py` -> **Severity: 0.404** (Bridge: 0.004 * Flux: 100.0%)
- `airflow-core/src/airflow/serialization/serialized_objects.py` -> **Severity: 0.357** (Bridge: 0.0036 * Flux: 100.0%)
- `airflow-core/src/airflow/models/xcom.py` -> **Severity: 0.316** (Bridge: 0.0032 * Flux: 99.9995%)
- `task-sdk/src/airflow/sdk/definitions/dag.py` -> **Severity: 0.25** (Bridge: 0.0025 * Flux: 99.9999%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `task-sdk/src/airflow/sdk/definitions/context.py` -> **Severity: 16.027** (Embedded: 0.1626 * Error Risk: 98.554%)
- `task-sdk/src/airflow/sdk/configuration.py` -> **Severity: 14.946** (Embedded: 0.1575 * Error Risk: 94.8698%)
- `task-sdk/src/airflow/sdk/bases/decorator.py` -> **Severity: 14.831** (Embedded: 0.1509 * Error Risk: 98.2899%)
- `task-sdk/src/airflow/sdk/log.py` -> **Severity: 14.737** (Embedded: 0.16 * Error Risk: 92.1195%)
- `shared/logging/src/airflow_shared/logging/structlog.py` -> **Severity: 14.613** (Embedded: 0.1507 * Error Risk: 96.9552%)
