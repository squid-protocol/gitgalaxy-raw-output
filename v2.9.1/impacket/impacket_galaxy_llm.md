...
## 1. EXECUTIVE SUMMARY
- **Scope:** 325 analyzed artifact(s), 120784 LOC.
- **Load-bearing artifact:** `impacket/dcerpc/v5/rpcrt.py` -- 74 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `ChangeLog.md` -- pulls in 66 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `impacket/smbserver.py` at magnitude 7216.08 (structural weight, not risk).
...
| Avg Path Length | 2.0808 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.221`
> **Composition Archetype:** `Hub-Coupled App` (z +1.22; from the repo's file-archetype mix)
> **File Composition:** Parameter Forwarders Files 39%, Large Core Modules (3) 29%, Large Core Modules (2) 6%, Data / Markup / Trivial 6%, Declarative / Non-Code 5%
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 7.5 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 86.9 | 100.0 | 100.0 |
...
| connectivity | 10173 | 293 | 76 | `impacket/dcerpc/v5/tsts.py` |
...
- `__decryptHash` **(Many-Argument Workhorses)** (@ `impacket/examples/secretsdump.py`) -> Impact: **215.3** | LOC: 236
- `run` **(Compute Cores)** (@ `examples/dpapi.py`) -> Impact: **196.4** | LOC: 449
- `parseRow` **(Many-Argument Workhorses)** (@ `impacket/tds.py`) -> Impact: **192.0** | LOC: 320
...
- `getKerberosTGT` **(Many-Argument Workhorses)** (@ `impacket/krb5/kerberosv5.py`) -> Impact: **176.1** | LOC: 271
...
- `kerberosLogin` **(Many-Argument Workhorses)** (@ `impacket/tds.py`) -> Impact: **168.8** | LOC: 275
- `doS4U` **(Many-Argument Workhorses)** (@ `examples/getST.py`) -> Impact: **164.8** | LOC: 415
- `dump` **(Many-Argument Workhorses)** (@ `impacket/examples/secretsdump.py`) -> Impact: **153.0** | LOC: 316
- `_parseValue` **(Many-Argument Workhorses)** (@ `impacket/tds.py`) -> Impact: **142.9** | LOC: 264
...
- `customizeTicket` **(Many-Argument Workhorses)** (@ `examples/ticketer.py`) -> Impact: **135.6** | LOC: 271
- `smbComNtCreateAndX` **(Many-Argument Workhorses)** (@ `impacket/smbserver.py`) -> Impact: **134.2** | LOC: 179
...
- **Blast Radius:** changing it is visible to **11** in-repo importer(s); it depends on **35**; blast radius 4.537; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.9%), Complexity Load (formerly Cognitive Load) (92.6%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 98.6842% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `smbComNtCreateAndX` **(Many-Argument Workhorses)** (Impact: 134.2)
    * *Intent:* # TODO: Fully implement this connData = smbServer.getConnectionData(connId) respSMBCommand = smb.SMB...
  * `findFirst2` **(Many-Argument Workhorses)** (Impact: 117.6)
    * *Intent:* # TODO: Depending on the level, this could be done much simpler # Let's choose the right encoding de...
  * `smb2Create` **(Many-Argument Workhorses)** (Impact: 112.1)
  * `processRequest` **(Many-Argument Workhorses)** (Impact: 96.1)
    * *Intent:* # print "%s" % packet['Signature'].encode('hex') # TODO: Process batched commands. isSMB2 = False SM...
  * `smbComSessionSetupAndX` **(Many-Argument Workhorses)** (Impact: 93.3)
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **43**; blast radius 5.876; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Guard Balance (formerly Safety Score) (99.4%), Complexity Load (formerly Cognitive Load) (91.4%), Connectivity (formerly Api Exposure) (32.8%)
- **Documentation Coverage:** 98.5222% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__decryptHash` **(Many-Argument Workhorses)** (Impact: 215.3)
  * `dump` **(Many-Argument Workhorses)** (Impact: 153.0)
  * `__printSecret` **(Many-Argument Workhorses)** (Impact: 93.0)
    * *Intent:* # Based on [MS-LSAD] section 3.1.1.4 # First off, let's discard NULL secrets. if len(secretItem) == ...
  * `__decryptSupplementalInfo` **(Many-Argument Workhorses)** (Impact: 89.1)
    * *Intent:* # This is based on [MS-SAMR] 2.2.10 Supplemental Credentials Structures haveInfo = False LOG.debug('...
  * `dump` **(Compute Cores)** (Impact: 82.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **20**; blast radius 6.172; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (97.8%), Complexity Load (formerly Cognitive Load) (82.2%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 92.233% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `kerberos_login` **(Stateful Encapsulated Methods)** (Impact: 61.2)
    * *Intent:* # Importing down here so pyasn1 is not required if kerberos is not used. from impacket.krb5.asn1 imp...
  * `login_extended` **(Stateful Encapsulated Methods)** (Impact: 61.0)
    * *Intent:* # login feature does not support unicode # disable it if enabled flags2 = self.__flags2 if flags2 & ...
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 55.0)
  * `list_path` **(Many-Argument Workhorses)** (Impact: 51.3)
  * `read_andx` **(Many-Argument Workhorses)** (Impact: 42.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **18**; blast radius 1.665; role: Pure Consumer (Orchestrator)
...
**Top Internal Functions/Classes:**
  * `marshalMe` **(Compute Cores)** (Impact: 85.9)
  * `createMethods` **(Many-Argument Workhorses)** (Impact: 64.0)
  * `printClass` **(Many-Argument Workhorses)** (Impact: 59.3)
  * `innerMethod` **(Many-Argument Workhorses)** (Impact: 54.4)
  * `getValue` **(Many-Argument Workhorses)** (Impact: 39.2)
...
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **29**; blast radius 1.93; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.8%), Complexity Load (formerly Cognitive Load) (89.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `kerberosLogin` **(Many-Argument Workhorses)** (Impact: 116.5)
    * *Intent:* # If TGT or TGS are specified, they are in the form of: # TGS['KDC_REP'] = the response from the ser...
  * `login` **(Many-Argument Workhorses)** (Impact: 97.5)
    * *Intent:* # If we have hashes, normalize them if lmhash != '' or nthash != '': if len(lmhash) % 2: lmhash = '0...
  * `create` **(Many-Argument Workhorses)** (Impact: 77.1)
  * `negotiateSession` **(Many-Argument Workhorses)** (Impact: 52.4)
    * *Intent:* # Let's store some data for later use self._Connection['ClientSecurityMode'] = SMB2_NEGOTIATE_SIGNIN...
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 52.0)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `file_cluster_4` (Drift: 0.0 IQR)
- **Magnitude:** 2430.58 | **LOC:** 2318 | **CtrlFlow:** 19.3% | **Authorship Centralization:** 50.0%
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **26**; blast radius 2.632; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (96.3%), Complexity Load (formerly Cognitive Load) (88.8%)
- **Documentation Coverage:** 96.1538% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `parseRow` **(Many-Argument Workhorses)** (Impact: 192.0)
    * *Intent:* # TODO: This REALLY needs to be improved. Right now we don't support correctly all the data types # ...
  * `kerberosLogin` **(Many-Argument Workhorses)** (Impact: 168.8)
  * `_parseValue` **(Many-Argument Workhorses)** (Impact: 142.9)
    * *Intent:* """ Parse the value based on the base type following MS-TDS 2.2.5.5.4. Args: baseType: The SQL Serve...
  * `login` **(Many-Argument Workhorses)** (Impact: 52.0)
  * `parseReply` **(Many-Argument Workhorses)** (Impact: 37.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **22** in-repo importer(s); it depends on **4**; blast radius 19.328; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (99.7%), Guard Balance (formerly Safety Score) (88.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_set_element` **(Stateful Encapsulated Methods)** (Impact: 23.7)
  * `__set_field_values` **(Stateful Encapsulated Methods)** (Impact: 17.2)
  * `delete_element` **(Compute Cores)** (Impact: 15.0)
  * `_find_element` **(Stateful Encapsulated Methods)** (Impact: 10.9)
  * `__get_field_position` **(Stateful Encapsulated Methods)** (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **20** in-repo importer(s); it depends on **8**; blast radius 38.267; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 95.3642% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__init__` **(Many-Argument Workhorses)** (Impact: 38.4)
  * `__init__` **(Many-Argument Workhorses)** (Impact: 30.0)
  * `fragment_by_list` **(Many-Argument Workhorses)** (Impact: 20.2)
  * `list_as_hex` **(Compute Cores)** (Impact: 18.7)
  * `get_packet` **(Compute Cores)** (Impact: 17.4)
...
- **Blast Radius:** changing it is visible to **74** in-repo importer(s); it depends on **14**; blast radius 30.922; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.0%), Complexity Load (formerly Cognitive Load) (85.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 97.931% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `bind` **(Stateful Encapsulated Methods)** (Impact: 107.5)
  * `recv` **(Compute Cores)** (Impact: 44.0)
  * `_transport_send` **(Many-Argument Workhorses)** (Impact: 37.6)
  * `send` **(Many-Argument Workhorses)** (Impact: 32.6)
  * `request` **(Many-Argument Workhorses)** (Impact: 21.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **10**; blast radius 1.606; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.8%), Connectivity (formerly Api Exposure) (81.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `hRpcWinStationGetAllProcesses` **(Compute Cores)** (Impact: 21.5)
    * *Intent:* # 3.7.4.1.22 RpcWinStationGetAllProcesses (Opnum 43) # i'm giving up constructing legitimate structu...
  * `hRpcShowMessageBox` **(Many-Argument Workhorses)** (Impact: 14.7)
    * *Intent:* # 3.3.4.1.10 RpcShowMessageBox (Opnum 9)
  * `__getitem__` **(Many-Argument Workhorses)** (Impact: 13.2)
  * `known_sid` **(Type Conversions)** (Impact: 11.4)
  * `__getitem__` **(Compute Cores)** (Impact: 8.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **53** in-repo importer(s); it depends on **8**; blast radius 40.416; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.4%), Complexity Load (formerly Cognitive Load) (77.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `pack` **(Many-Argument Workhorses)** (Impact: 42.3)
    * *Intent:* # array specifier two = fieldTypeOrClass.split('*') if len(two) == 2: answer = b'' if self.isNDR(sel...
  * `unpack` **(Many-Argument Workhorses)** (Impact: 42.3)
    * *Intent:* # array specifier two = fieldTypeOrClass.split('*') answer = [] soFarItems = 0 offset0 = offset if l...
  * `fromString` **(Many-Argument Workhorses)** (Impact: 41.5)
  * `__init__` **(Stateful Encapsulated Methods)** (Impact: 33.0)
    * *Intent:* #ret = NDR.__init__(self,None, isNDR64=isNDR64) self.topLevel = topLevel self._isNDR64 = isNDR64 sel...
  * `getData` **(Many-Argument Workhorses)** (Impact: 32.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 1.37; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (94.4%), Complexity Load (formerly Cognitive Load) (92.4%), Connectivity (formerly Api Exposure) (11.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (11.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `process` **(Compute Cores)** (Impact: 41.6)
    * *Intent:* # R, DFI, T*, TG*, TOSI, CD, SI, DLI* if self.icmp_num_responses != 2: self.add_result("R","N") retu...
  * `calc_ti` **(Compute Cores)** (Impact: 40.2)
  * `process` **(Many-Argument Workhorses)** (Impact: 38.6)
  * `process` **(Compute Cores)** (Impact: 37.5)
  * `process` **(Compute Cores)** (Impact: 30.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **10**; blast radius 1.37; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (96.5%), Complexity Load (formerly Cognitive Load) (26.8%), Connectivity (formerly Api Exposure) (11.8%), Dead Code Surface (formerly Dead Code) (4.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_SamrQueryInformationUser2_SamrSetInformationUser2` **(I/O & Config Routines)** (Impact: 9.2)
  * `test_hSamrQueryInformationUser2_hSamrSetInformationUser2` **(I/O & Config Routines)** (Impact: 9.2)
  * `test_SamrUnicodeChangePasswordUser2` **(I/O & Config Routines)** (Impact: 7.9)
  * `test_SamrAddMemberToGroup_SamrRemoveMemberFromGroup` **(Defensive Guards)** (Impact: 7.5)
  * `test_SamrQueryInformationAlias_SamrSetInformationAlias` **(I/O & Config Routines)** (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **12** in-repo importer(s); it depends on **11**; blast radius 4.247; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.7%), Complexity Load (formerly Cognitive Load) (85.4%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `connect` **(Many-Argument Workhorses)** (Impact: 46.2)
  * `RemoteCreateInstance` **(Many-Argument Workhorses)** (Impact: 40.1)
    * *Intent:* # Only supports one interface at a time self.__portmap.bind(IID_IRemoteSCMActivator) ORPCthis = ORPC...
  * `RemoteGetClassObject` **(Many-Argument Workhorses)** (Impact: 40.0)
    * *Intent:* # iid should be IID_IClassFactory self.__portmap.bind(IID_IRemoteSCMActivator) ORPCthis = ORPCTHIS()...
  * `RemoteActivation` **(Many-Argument Workhorses)** (Impact: 24.8)
...
- **Blast Radius:** changing it is visible to **11** in-repo importer(s); it depends on **14**; blast radius 4.476; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (92.2%), Complexity Load (formerly Cognitive Load) (80.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `hSamrChangePasswordUser` **(Many-Argument Workhorses)** (Impact: 22.0)
  * `hSamrUnicodeChangePasswordUser2` **(Many-Argument Workhorses)** (Impact: 13.8)
  * `hSamrCreateUser2InDomain` **(Defensive Guards)** (Impact: 13.1)
  * `hSamrSetNTInternal1` **(Many-Argument Workhorses)** (Impact: 8.0)
  * `dump` **(Compute Cores)** (Impact: 6.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **13**; blast radius 1.389; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.9%), Complexity Load (formerly Cognitive Load) (83.7%), Connectivity (formerly Api Exposure) (64.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (27.2%)
- **Documentation Coverage:** 94.4134% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `searchAttribute` **(Many-Argument Workhorses)** (Impact: 47.6)
  * `findFirst` **(Compute Cores)** (Impact: 31.1)
    * *Intent:* # Searches for a file and returns an Index Entry. None if not found def getFileName(entry): if len(e...
  * `findFirstSubNode` **(Compute Cores)** (Impact: 29.4)
  * `complete_get` **(Many-Argument Workhorses)** (Impact: 24.8)
    * *Intent:* # include means # 1 just files # 2 just directories items = [] if include == 1: mask = 0 else: mask ...
  * `getINode` **(Compute Cores)** (Impact: 22.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **15**; blast radius 2.061; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Connectivity (formerly Api Exposure) (94.8%), Guard Balance (formerly Safety Score) (89.9%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `getSSPType1` **(Many-Argument Workhorses)** (Impact: 27.6)
  * `SIGN` **(Many-Argument Workhorses)** (Impact: 18.3)
  * `hNetrLogonGetDomainInfo` **(Many-Argument Workhorses)** (Impact: 14.5)
  * `SEAL` **(Many-Argument Workhorses)** (Impact: 11.5)
  * `UNSEAL` **(Many-Argument Workhorses)** (Impact: 10.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **7**; blast radius 1.852; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (87.5%), Connectivity (formerly Api Exposure) (84.7%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__setitem__` **(Defensive Guards)** (Impact: 6.5)
  * `dump` **(Compute Cores)** (Impact: 6.3)
  * `hNetrShareEnum` **(Many-Argument Workhorses)** (Impact: 5.5)
    * *Intent:* # serverName example: "\\\\1.2.3.4\x00" if serverName[-1] != '\x00': serverName += '\x00' # final NU...
  * `__getitem__` **(Compute Cores)** (Impact: 5.4)
  * `__str__` **(Compute Cores)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **21**; blast radius 1.665; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (99.8%), Complexity Load (formerly Cognitive Load) (71.9%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (28.4%), Connectivity (formerly Api Exposure) (14.5%)
- **Documentation Coverage:** 97.0149% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__printSecret` **(Many-Argument Workhorses)** (Impact: 98.0)
    * *Intent:* # Based on [MS-LSAD] section 3.1.1.4 # First off, let's discard NULL secrets. if len(secretItem) == ...
  * `dump` **(Compute Cores)** (Impact: 49.8)
  * `__extract_local_history` **(Many-Argument Workhorses)** (Impact: 28.6)
  * `__parse_lp_data` **(Stateful Encapsulated Methods)** (Impact: 23.2)
  * `dumpCachedHashes` **(Compute Cores)** (Impact: 21.4)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **31**; blast radius 1.389; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (99.9%), Complexity Load (formerly Cognitive Load) (92.9%), Connectivity (formerly Api Exposure) (28.1%), Dead Code Surface (formerly Dead Code) (1.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `customizeTicket` **(Many-Argument Workhorses)** (Impact: 135.6)
  * `signEncryptTicket` **(Many-Argument Workhorses)** (Impact: 81.8)
  * `createBasicTicket` **(Compute Cores)** (Impact: 74.6)
  * `getKerberosS4U2SelfU2U` **(I/O & Config Routines)** (Impact: 15.1)
  * `createBasicPac` **(Compute Cores)** (Impact: 14.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **29**; blast radius 1.37; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (98.4%), Complexity Load (formerly Cognitive Load) (71.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (10.7%), Connectivity (formerly Api Exposure) (9.6%)
- **Documentation Coverage:** 89.2857% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `checkSecurityDescriptors` **(Many-Argument Workhorses)** (Impact: 107.5)
  * `run` **(Compute Cores)** (Impact: 96.4)
    * *Intent:* #self.client.search('dc=vulnerable,dc=contoso,dc=com', '(objectclass=person)') #print self.client.en...
  * `dumpADCS` **(Compute Cores)** (Impact: 48.8)
  * `addDnsRecord` **(Many-Argument Workhorses)** (Impact: 48.4)
    * *Intent:* # https://github.com/Kevin-Robertson/Powermad/blob/master/Powermad.ps1 def new_dns_namearray(data): ...
  * `delegateAttack` **(Many-Argument Workhorses)** (Impact: 44.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 175 instances
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **41**; blast radius 1.389; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (99.2%), Complexity Load (formerly Cognitive Load) (67.6%), Connectivity (formerly Api Exposure) (29.5%), Dead Code Surface (formerly Dead Code) (2.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `makeGolden` **(Many-Argument Workhorses)** (Impact: 90.6)
  * `raiseUp` **(Many-Argument Workhorses)** (Impact: 43.3)
  * `__connectDrds` **(Stateful Encapsulated Methods)** (Impact: 33.5)
  * `__decryptHash` **(Stateful Encapsulated Methods)** (Impact: 31.9)
  * `__decryptSupplementalInfo` **(Many-Argument Workhorses)** (Impact: 24.3)
...
- **Blast Radius:** changing it is visible to **4** in-repo importer(s); it depends on **22**; blast radius 3.778; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (66.7%)
- **Documentation Coverage:** 78.4314% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `login` **(Many-Argument Workhorses)** (Impact: 77.5)
    * *Intent:* """ logins into the target system :param string user: username :param string password: password for ...
  * `kerberosLogin` **(Many-Argument Workhorses)** (Impact: 69.8)
  * `_compileSimpleFilter` **(Many-Argument Workhorses)** (Impact: 60.2)
  * `search` **(Many-Argument Workhorses)** (Impact: 50.9)
    * *Intent:* # searchFilter expects a string (not bytes), otherwise it will raise an exception
  * `modify` **(Many-Argument Workhorses)** (Impact: 31.1)
...
- **Blast Radius:** changing it is visible to **15** in-repo importer(s); it depends on **3**; blast radius 10.805; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (100.0%), Connectivity (formerly Api Exposure) (100.0%), Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (99.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `decode` **(Compute Cores)** (Impact: 35.0)
  * `decode` **(Compute Cores)** (Impact: 22.8)
  * `decode` **(Compute Cores)** (Impact: 15.3)
  * `decode` **(Compute Cores)** (Impact: 15.1)
  * `decode` **(Compute Cores)** (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 1.37; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.8%), Debt Markers (formerly Tech Debt) (88.3%), Complexity Load (formerly Cognitive Load) (81.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__str__` **(Compute Cores)** (Impact: 4.6)
  * `Next` **(Type Conversions)** (Impact: 3.9)
  * `PutPublisherProperty` **(Parameter Forwarders)** (Impact: 2.4)
  * `PutSubscriberProperty` **(Parameter Forwarders)** (Impact: 2.4)
  * `Query` **(Parameter Forwarders)** (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
...
- `impacket/examples/secretsdump.py` -> Churn: **100.0%** | Cog Load: 91.4485% | Debt: 0.0%
- `impacket/tds.py` -> Churn: **96.34%** | Cog Load: 88.7585% | Debt: 9.9713%
- `examples/secretsdump.py` -> Churn: **94.03%** | Cog Load: 81.8113% | Debt: 0.0%
- `examples/ntlmrelayx.py` -> Churn: **92.28%** | Cog Load: 77.6254% | Debt: 0.0%
- `impacket/examples/ntlmrelayx/servers/smbrelayserver.py` -> Churn: **82.38%** | Cog Load: 69.3108% | Debt: 0.0%
