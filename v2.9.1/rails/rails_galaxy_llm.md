...
## 1. EXECUTIVE SUMMARY
- **Scope:** 4376 analyzed artifact(s), 398126 LOC.
- **Load-bearing artifact:** `railties/test/isolation/abstract_unit.rb` -- 99 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `activerecord/lib/active_record/base.rb` -- pulls in 58 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `actionview/test/template/date_helper_test.rb` at magnitude 2290.08 (structural weight, not risk).
...
| Avg Path Length | 3.9824 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `5.084`
> **Composition Archetype:** `Hub-Coupled Monorepo` (z +5.08; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 30%, Large Core Modules 14%, Large Core Modules (3) 13%, Interface Declarations Files 11%, Declarative / Non-Code 11%
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 31.0 | 0.7 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 98.3 | 3.1 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 58.1 | 87.5 | 100.0 |
...
| connectivity | 11556 | 1728 | 5 | `actionpack/test/dispatch/routing_test.rb` |
...
| dead_code | 20058 | 1626 | 10 | `actionview/test/template/form_helper_test.rb` |
| credential | 49 | 30 | 0 | `actionmailbox/test/controllers/ingresses/mailgun/inbound_emails_controller_test.rb` |
| threat | 1141 | 502 | 1 | `activestorage/app/assets/javascripts/activestorage.js` |
...
- `indexes` **(Many-Argument Workhorses)** (@ `activerecord/lib/active_record/connection_adapters/postgresql/schema_statements.rb`) -> Impact: **191.1** | LOC: 1248
  * *Intent:* # Returns an array of indexes for the given table.
- `merge_conditional_options` **(Many-Argument Workhorses)** (@ `activesupport/lib/active_support/callbacks.rb`) -> Impact: **179.2** | LOC: 703
- `test_remove_column_with_if_not_exists_not_set` **(Many-Argument Workhorses)** (@ `activerecord/test/cases/migration_test.rb`) -> Impact: **148.6** | LOC: 1813
- `Anonymous_Block_[Truncated]` **(Compute Cores)** (@ `Rakefile`) -> Impact: **126.8** | LOC: 216
- `rails_require_statement` **(Compute Cores)** (@ `railties/lib/rails/generators/app_base.rb`) -> Impact: **118.8** | LOC: 510
- `Anonymous_Block` **(Unclassified)** (@ `activerecord/lib/active_record/railties/databases.rake`) -> Impact: **114.3** | LOC: 448
- `uuid_column_and_sequence_sql` **(Compute Cores)** (@ `activerecord/lib/active_record/connection_adapters/postgresql/schema_statements.rb`) -> Impact: **114.0** | LOC: 865
- `store_nested_param` **(Many-Argument Workhorses)** (@ `actionpack/lib/action_dispatch/http/param_builder.rb`) -> Impact: **112.0** | LOC: 85
- `load_defaults` **(Many-Argument Workhorses)** (@ `railties/lib/rails/application/configuration.rb`) -> Impact: **100.4** | LOC: 310
...
- `indexes` **(Compute Cores)** (@ `activerecord/lib/active_record/connection_adapters/mysql/schema_statements.rb`) -> Impact: **81.0** | LOC: 319
  * *Intent:* # Returns an array of indexes for the given table.
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (94.5%), Complexity Load (formerly Cognitive Load) (40.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_select_date_with_no_end_year` **(I/O & Config Routines)** (Impact: 56.5)
  * `test_select_date_with_no_start_or_end_year` **(I/O & Config Routines)** (Impact: 56.5)
  * `test_select_date_with_no_start_year` **(I/O & Config Routines)** (Impact: 55.0)
  * `test_select_time_with_seconds` **(I/O & Config Routines)** (Impact: 6.2)
  * `assert_distance_of_time_in_words` **(I/O & Config Routines)** (Impact: 6.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 0.146; role: Pure Consumer (Orchestrator)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (21.2%), Connectivity (formerly Api Exposure) (16.0%), Complexity Load (formerly Cognitive Load) (3.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_shallow_nested_resources` **(Callbacks & Closures)** (Impact: 6.5)
  * `test_colon_containing_custom_param` **(I/O & Config Routines)** (Impact: 4.4)
  * `test_custom_resource_routes_are_scoped` **(Callbacks & Closures)** (Impact: 3.6)
  * `test_namespace_containing_numbers` **(I/O & Config Routines)** (Impact: 3.5)
  * `test_namespaced_shallow_routes_with_shallow_prefix_option` **(I/O & Config Routines)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); blast radius 0.167; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.4%), Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (73.8%)
- **Documentation Coverage:** 99.5327% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `visit_Arel_Nodes_BoundSqlLiteral` **(Compute Cores)** (Impact: 25.2)
  * `build_subselect` **(Compute Cores)** (Impact: 12.8)
    * *Intent:* # FIXME: we should probably have a 2-pass visitor for this
  * `infix_value_with_paren` **(Many-Argument Workhorses)** (Impact: 12.0)
  * `visit_Arel_Nodes_ValuesList` **(Callbacks & Closures)** (Impact: 9.6)
  * `visit_Arel_Nodes_Case` **(Compute Cores)** (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000405
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (50.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (24.0%), Complexity Load (formerly Cognitive Load) (10.5%), Connectivity (formerly Api Exposure) (3.5%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `whole_form` **(Many-Argument Workhorses)** (Impact: 7.1)
  * `test_nested_fields_for_on_a_nested_attributes_collection_association_yields_only_builder` **(I/O & Config Routines)** (Impact: 5.1)
  * `test_nested_fields_for_with_child_index_as_lambda_option_override_on_a_nested_attributes_collection_association` **(I/O & Config Routines)** (Impact: 4.8)
  * `test_label_with_symbols` **(I/O & Config Routines)** (Impact: 4.2)
  * `test_nested_fields_uses_unique_indices_for_different_collection_associations` **(I/O & Config Routines)** (Impact: 4.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **51**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (47.1%), Complexity Load (formerly Cognitive Load) (29.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (9.3%), Dead Code Surface (formerly Dead Code) (4.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_find_one_message_on_primary_key` **(I/O & Config Routines)** (Impact: 4.0)
  * `__global_context__` **(I/O & Config Routines)** (Impact: 3.7)
  * `test_build_and_create_from_association_should_respect_passed_attributes_over_default_scope` **(I/O & Config Routines)** (Impact: 2.7)
  * `test_create_with_bang_on_has_many_when_parent_is_new_raises` **(I/O & Config Routines)** (Impact: 2.5)
  * `test_create` **(Callbacks & Closures)** (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **1**; blast radius 0.154; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (59.3%)
- **Documentation Coverage:** 68.1416% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `indexes` **(Many-Argument Workhorses)** (Impact: 191.1)
    * *Intent:* # Returns an array of indexes for the given table.
  * `uuid_column_and_sequence_sql` **(Compute Cores)** (Impact: 114.0)
  * `create_database` **(Compute Cores)** (Impact: 18.8)
    * *Intent:* # Create a new PostgreSQL database. Options include <tt>:owner</tt>, <tt>:template</tt>, # <tt>:enco...
  * `drop_schema` **(Compute Cores)** (Impact: 14.0)
    * *Intent:* # Drops the schema for the given schema name.
  * `create_schema` **(Compute Cores)** (Impact: 12.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.2%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (48.5%)
- **Documentation Coverage:** 63.0769% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `preprocess_order_args` **(Compute Cores)** (Impact: 21.6)
  * `build_join_buckets` **(Callbacks & Closures)** (Impact: 20.8)
  * `build_named_bound_sql_literal` **(Compute Cores)** (Impact: 18.3)
  * `build_joins` **(Compute Cores)** (Impact: 16.6)
  * `does_not_support_reverse?` **(Compute Cores)** (Impact: 16.1)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **35**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (32.8%), Complexity Load (formerly Cognitive Load) (24.1%), Concurrency Surface (formerly Concurrency) (23.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_blank_like_arguments_to_query_methods_dont_raise_errors` **(I/O & Config Routines)** (Impact: 6.8)
  * `test_count_with_distinct` **(I/O & Config Routines)** (Impact: 4.5)
  * `test_unscope_with_arel_sql` **(I/O & Config Routines)** (Impact: 4.5)
  * `test_where_with_take_memoization` **(I/O & Config Routines)** (Impact: 3.6)
  * `test_find_by_with_take_memoization` **(I/O & Config Routines)** (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
...
- **Magnitude:** 843.96 | **LOC:** 2501 | **CtrlFlow:** 1.4% | **Authorship Centralization:** 100.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **44**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (52.3%), Complexity Load (formerly Cognitive Load) (22.7%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (5.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (5.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_autosave_has_one_association_callbacks_get_called_once` **(Annotated & Test Methods)** (Impact: 5.2)
    * *Intent:* # a bidirectional autosave is required to trigger multiple calls to # save_has_one_association asser...
  * `__global_context__` **(I/O & Config Routines)** (Impact: 4.5)
  * `test_indexed_errors_on_base_attribute_should_be_properly_translated` **(I/O & Config Routines)** (Impact: 4.4)
  * `test_indexed_errors_should_be_properly_translated` **(I/O & Config Routines)** (Impact: 4.3)
  * `test_autosave_collection_association_callbacks_get_called_once` **(Stateful Encapsulated Methods)** (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (15.6%), Complexity Load (formerly Cognitive Load) (4.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_invokes_default_orm` **(Many-Argument Workhorses)** (Impact: 38.0)
  * `test_model_with_parent_option` **(Many-Argument Workhorses)** (Impact: 37.8)
  * `test_model_with_database_option` **(Many-Argument Workhorses)** (Impact: 37.5)
  * `test_model_with_parent_and_database_option` **(Many-Argument Workhorses)** (Impact: 37.0)
  * `test_model_with_no_migration_and_database_option` **(Many-Argument Workhorses)** (Impact: 36.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.3%), Test Surface (formerly Verification) (80.0%), Dead Code Surface (formerly Dead Code) (43.5%)
- **Documentation Coverage:** 68.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `copy` **(Defensive Guards)** (Impact: 34.5)
  * `method_missing` **(Compute Cores)** (Impact: 16.8)
  * `move` **(Defensive Guards)** (Impact: 9.6)
  * `exec_migration` **(Compute Cores)** (Impact: 9.4)
  * `initialize` **(Compute Cores)** (Impact: 9.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (89.1%), Guard Balance (formerly Safety Score) (87.3%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 76.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_table` **(Many-Argument Workhorses)** (Impact: 28.3)
    * *Intent:* # supplier_id bigint # ) # # ====== Create a temporary table based on a query # # create_table(:long...
  * `add_index_options` **(Many-Argument Workhorses)** (Impact: 17.2)
  * `distinct_relation_for_primary_key` **(Compute Cores)** (Impact: 16.8)
  * `rename_table_indexes` **(Compute Cores)** (Impact: 10.4)
  * `check_constraint_options` **(Compute Cores)** (Impact: 10.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.146; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (63.6%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (20.1%), Complexity Load (formerly Cognitive Load) (15.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (3.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `form_text` **(Compute Cores)** (Impact: 6.6)
  * `whole_form` **(Many-Argument Workhorses)** (Impact: 5.6)
  * `test_nested_fields_with_existing_records_on_a_nested_attributes_collection_association_with_disabled_hidden_id` **(Callbacks & Closures)** (Impact: 5.3)
  * `test_form_with_with_nil_index_option_override` **(I/O & Config Routines)** (Impact: 4.8)
  * `url_for` **(Compute Cores)** (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **35**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (58.0%), Complexity Load (formerly Cognitive Load) (36.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (16.3%), Historical Stability (predictive layer, promotion pending #2987) (formerly Stability) (2.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `assert_equal_after_sort` **(Callbacks & Closures)** (Impact: 4.2)
    * *Intent:* # Eager includes of has many and habtm associations aren't necessarily sorted in the same way
  * `test_eager_with_has_and_belongs_to_many_and_limit` **(Callbacks & Closures)** (Impact: 3.5)
  * `test_association_loading_notification` **(Callbacks & Closures)** (Impact: 3.5)
  * `test_eager_with_default_scope_as_class_method_using_find_method` **(I/O & Config Routines)** (Impact: 3.4)
  * `test_eager_count_performed_on_a_has_many_association_with_multi_table_conditional` **(I/O & Config Routines)** (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (44.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (32.7%), Complexity Load (formerly Cognitive Load) (17.5%), Concurrency Surface (formerly Concurrency) (17.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_remove_column_with_if_not_exists_not_set` **(Many-Argument Workhorses)** (Impact: 148.6)
  * `test_migration_raises_if_timestamp_greater_than_14_digits` **(Callbacks & Closures)** (Impact: 14.0)
  * `setup` **(Callbacks & Closures)** (Impact: 6.5)
  * `test_internal_metadata_create_table_wont_be_affected_by_schema_cache` **(I/O & Config Routines)** (Impact: 5.3)
  * `test_filtering_migrations` **(I/O & Config Routines)** (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Global Archetype:** `file_cluster_15` (Drift: 0.0 IQR)
- **Magnitude:** 751.42 | **LOC:** 1347 | **CtrlFlow:** 16.2% | **Authorship Centralization:** 53.8%
- **Blast Radius:** changing it is visible to **3** in-repo importer(s); it depends on **17**; blast radius 0.234; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (78.8%), Connectivity (formerly Api Exposure) (76.9%)
- **Documentation Coverage:** 68.4211% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `log` **(Many-Argument Workhorses)** (Impact: 39.2)
  * `initialize` **(Many-Argument Workhorses)** (Impact: 38.5)
  * `with_raw_connection` **(Defensive Guards)** (Impact: 29.3)
    * *Intent:* # methods, and internal transaction-agnostic queries. # ### # # It's not the primary use case, so no...
  * `find_cmd_and_exec` **(Compute Cores)** (Impact: 13.5)
  * `case_insensitive_comparison` **(Compute Cores)** (Impact: 10.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6.1e-05 | `Ripple Effect (Closeness):` 0.008419
...
- **Global Archetype:** `file_cluster_15` (Drift: 0.0 IQR)
- **Magnitude:** 739.78 | **LOC:** 1539 | **CtrlFlow:** 23.5% | **Authorship Centralization:** 25.0%
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 0.146; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (99.5%), Guard Balance (formerly Safety Score) (92.8%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 75.4902% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `update_all` **(Defensive Guards)** (Impact: 19.1)
    * *Intent:* # # Update all customers with the given attributes # Customer.update_all wants_email: true # # # Upd...
  * `compute_cache_version` **(Compute Cores)** (Impact: 17.8)
  * `exec_main_query` **(Compute Cores)** (Impact: 17.1)
  * `initialize` **(Many-Argument Workhorses)** (Impact: 12.1)
  * `instantiate_records` **(Defensive Guards)** (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **41**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (57.1%), Complexity Load (formerly Cognitive Load) (30.4%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_belongs_to_with_primary_key_counter` **(I/O & Config Routines)** (Impact: 4.7)
  * `test_optional_relation_can_be_set_per_model` **(I/O & Config Routines)** (Impact: 4.3)
  * `test_eager_loading_with_primary_key` **(I/O & Config Routines)** (Impact: 3.3)
  * `__global_context__` **(I/O & Config Routines)** (Impact: 3.2)
  * `test_optional_relation` **(Annotated & Test Methods)** (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **34**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (42.1%), Concurrency Surface (formerly Concurrency) (35.6%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (34.9%), Complexity Load (formerly Cognitive Load) (26.1%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_column_types_typecast` **(I/O & Config Routines)** (Impact: 5.1)
  * `test_dup_with_aggregate_of_same_name_as_attribute` **(I/O & Config Routines)** (Impact: 4.2)
  * `__global_context__` **(I/O & Config Routines)** (Impact: 3.8)
  * `test_benchmark_with_log_level` **(I/O & Config Routines)** (Impact: 3.7)
  * `test_clear_cache!` **(I/O & Config Routines)** (Impact: 3.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **31**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (24.6%), Guard Balance (formerly Safety Score) (23.8%), Complexity Load (formerly Cognitive Load) (12.0%), Dead Code Surface (formerly Dead Code) (4.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `__global_context__` **(I/O & Config Routines)** (Impact: 16.0)
  * `test_bind_variables` **(I/O & Config Routines)** (Impact: 8.6)
  * `test_find_by_records` **(Compute Cores)** (Impact: 5.2)
  * `test_count_by_sql` **(I/O & Config Routines)** (Impact: 4.3)
  * `test_dynamic_finder_on_one_attribute_with_conditions_returns_same_results_after_caching` **(I/O & Config Routines)** (Impact: 4.3)
...
- **Blast Radius:** changing it is visible to **9** in-repo importer(s); it depends on **11**; blast radius 1.188; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (91.7%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (75.5%)
- **Documentation Coverage:** 85.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `merge_conditional_options` **(Many-Argument Workhorses)** (Impact: 179.2)
  * `run_callbacks` **(Many-Argument Workhorses)** (Impact: 28.4)
    * *Intent:* # if callbacks have been set but no block is given. # # run_callbacks :save do # save # end # #-- # ...
  * `initialize` **(Compute Cores)** (Impact: 14.4)
  * `reset_callbacks` **(Compute Cores)** (Impact: 11.9)
    * *Intent:* # Remove all set callbacks for the given event.
  * `duplicates?` **(Compute Cores)** (Impact: 8.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.5e-05 | `Ripple Effect (Closeness):` 0.010544
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (58.9%), Complexity Load (formerly Cognitive Load) (18.5%), Credential Material (formerly Secrets Risk) (18.0%), Connectivity (formerly Api Exposure) (13.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `test_setting_cookie_with_secure_when_always_write_cookie_is_true` **(Callbacks & Closures)** (Impact: 2.4)
  * `rails_5_2_stable_encrypted_cookie_with_authenticated_encryption_flag_on` **(I/O & Config Routines)** (Impact: 2.3)
    * *Intent:* # cookies.encrypted[:favorite] = { value: "5-2-Stable Chocolate Cookies", expires: 1000.years } cook...
  * `test_setting_cookie_with_secure` **(I/O & Config Routines)** (Impact: 2.3)
  * `test_legacy_hmac_aes_cbc_json_mode_falls_back_to_authenticated_encrypted_cookie` **(I/O & Config Routines)** (Impact: 2.3)
  * `rails_5_2_stable_encrypted_cookie_with_authenticated_encryption_flag_off` **(Interface Declarations)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **2** in-repo importer(s); it depends on **14**; blast radius 0.728; role: Transceiver (Middle-Tier)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (94.8%), Complexity Load (formerly Cognitive Load) (90.0%), Test Surface (formerly Verification) (80.0%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `recognize_path_with_request` **(Many-Argument Workhorses)** (Impact: 21.6)
  * `handle_positional_args` **(Many-Argument Workhorses)** (Impact: 18.5)
  * `call` **(Many-Argument Workhorses)** (Impact: 15.7)
  * `generate_url_helpers` **(Compute Cores)** (Impact: 15.0)
  * `url_for` **(Many-Argument Workhorses)** (Impact: 12.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000115 | `Ripple Effect (Closeness):` 0.011626
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 0.146; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (90.2%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (51.7%)
- **Documentation Coverage:** 82.3009% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `derive_fk_query_constraints` **(I/O & Config Routines)** (Impact: 27.4)
  * `initialize` **(Many-Argument Workhorses)** (Impact: 17.0)
  * `reflection_class_for` **(Stateful Encapsulated Methods)** (Impact: 10.6)
  * `association_scope_cache` **(Defensive Guards)** (Impact: 10.4)
  * `check_validity!` **(Defensive Guards)** (Impact: 9.8)
**Contextual Mitigations & Amplifications:**
...
- `activerecord/lib/active_record/connection_adapters/abstract/database_statements.rb` -> Churn: **98.29%** | Cog Load: 47.5203% | Debt: 99.9902%
- `railties/lib/rails/application/configuration.rb` -> Churn: **94.1%** | Cog Load: 84.594% | Debt: 95.1407%
- `activerecord/lib/active_record/connection_adapters/abstract/connection_pool.rb` -> Churn: **65.94%** | Cog Load: 51.5412% | Debt: 11.562%
- `activerecord/lib/active_record/connection_adapters/postgresql_adapter.rb` -> Churn: **63.82%** | Cog Load: 47.415% | Debt: 99.9958%
- `actionpack/lib/action_controller/metal/live.rb` -> Churn: **62.4%** | Cog Load: 71.7748% | Debt: 99.5648%
...
- `activerecord/test/cases/autosave_association_test.rb` -> **Ian Terrell** (100.0% isolated ownership) | Magnitude: 843.96
...
- `activesupport/lib/active_support/test_case.rb` -> **Severity: 0.036** (Bridge: 0.0005 * Flux: 71.0738%)
- `activesupport/lib/active_support/core_ext/time/calculations.rb` -> **Severity: 0.019** (Bridge: 0.0002 * Flux: 99.375%)
- `activesupport/lib/active_support/deprecation.rb` -> **Severity: 0.016** (Bridge: 0.0002 * Flux: 99.9254%)
- `activesupport/lib/active_support/duration.rb` -> **Severity: 0.014** (Bridge: 0.0001 * Flux: 99.3596%)
- `railties/lib/rails/generators.rb` -> **Severity: 0.013** (Bridge: 0.0001 * Flux: 100.0%)

### 🃏 House of Cards (Closeness * Error Risk)
These files are deeply embedded (1 or 2 hops from the entire codebase) but possess high error exposure. A runtime exception here will cascade instantly across the application.

- `activesupport/lib/active_support/core_ext/module/attribute_accessors.rb` -> **Severity: 3.165** (Embedded: 0.0332 * Error Risk: 95.2574%)
- `activesupport/lib/active_support/inflector/methods.rb` -> **Severity: 3.018** (Embedded: 0.0338 * Error Risk: 89.279%)
- `activesupport/lib/active_support/inflector/inflections.rb` -> **Severity: 2.352** (Embedded: 0.0246 * Error Risk: 95.4835%)
- `activesupport/lib/active_support/core_ext/hash/slice.rb` -> **Severity: 2.254** (Embedded: 0.027 * Error Risk: 83.5974%)
- `activesupport/lib/active_support/lazy_load_hooks.rb` -> **Severity: 2.251** (Embedded: 0.0263 * Error Risk: 85.6049%)
