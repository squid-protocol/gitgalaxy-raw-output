...
## 1. EXECUTIVE SUMMARY
- **Scope:** 119 analyzed artifact(s), 7904 LOC.
- **Load-bearing artifact:** `common/src/serial.rs` -- 7 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `uefi/src/main.rs` -- pulls in 47 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `common/src/load_kernel.rs` at magnitude 248.1 (structural weight, not risk).
...
| Avg Path Length | 1.2353 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `1.136`
> **Composition Archetype:** `Small Flat Repo` (z +1.14; from the repo's file-archetype mix)
> **File Composition:** Generic / Templated Code Files 18%, Type Conversions Files 17%, Data / Markup / Trivial 14%, State Mutators Files 12%, Large Core Modules (3) 11%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 68.5 | 67.5 | 100.0 |
...
- `set_up_mappings` **(Many-Argument Workhorses)** (@ `common/src/lib.rs`) -> Impact: **79.4** | LOC: 266
...
- `create_disk_image` **(Many-Argument Workhorses)** (@ `src/disk_image.rs`) -> Impact: **53.0** | LOC: 99
  * *Intent:* /// Creates a bootable disk image from the given bootloader executable.
- `new` **(Many-Argument Workhorses)** (@ `common/src/level_4_entries.rs`) -> Impact: **49.2** | LOC: 102
  * *Intent:* /// Initializes a new instance. /// /// Marks the statically configured virtual address ranges from the config as used.
- `handle_dynamic_segment` **(Many-Argument Workhorses)** (@ `common/src/load_kernel.rs`) -> Impact: **45.6** | LOC: 73
- `deserialize` **(Compute Cores)** (@ `api/src/config.rs`) -> Impact: **41.3** | LOC: 119
...
- `create_mbr_disk` **(Many-Argument Workhorses)** (@ `src/mbr.rs`) -> Impact: **38.0** | LOC: 90
- `get_best_mode` **(Many-Argument Workhorses)** (@ `bios/stage-2/src/vesa.rs`) -> Impact: **34.3** | LOC: 46
- `create_gpt_disk` **(Compute Cores)** (@ `src/gpt.rs`) -> Impact: **29.1** | LOC: 63
- `main_inner` **(Many-Argument Workhorses)** (@ `uefi/src/main.rs`) -> Impact: **26.3** | LOC: 111
- `create_boot_info` **(Many-Argument Workhorses)** (@ `common/src/lib.rs`) -> Impact: **24.4** | LOC: 118
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **30**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (64.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.4%), Mutation Surface (formerly State Flux) (15.8%)
- **Documentation Coverage:** 57.6923% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handle_dynamic_segment` **(Many-Argument Workhorses)** (Impact: 45.6)
  * `load_segments` **(Defensive Guards)** (Impact: 23.5)
  * `new` **(Many-Argument Workhorses)** (Impact: 21.3)
  * `remove_copied_flags` **(Compute Cores)** (Impact: 19.3)
    * *Intent:* /// Cleans up the custom flags set by [`Inner::make_mut`].
  * `handle_load_segment` **(Compute Cores)** (Impact: 16.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **35**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (73.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (48.2%), Mutation Surface (formerly State Flux) (22.1%)
- **Documentation Coverage:** 38.8889% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `set_up_mappings` **(Many-Argument Workhorses)** (Impact: 79.4)
    * *Intent:* /// Sets up mappings for a kernel stack and the framebuffer. /// /// The `kernel_bytes` slice should...
  * `create_boot_info` **(Many-Argument Workhorses)** (Impact: 24.4)
    * *Intent:* /// Allocates and initializes the boot info struct and the memory map. /// /// The boot info and mem...
  * `mapping_addr` **(Many-Argument Workhorses)** (Impact: 9.7)
  * `init_logger` **(Many-Argument Workhorses)** (Impact: 5.8)
    * *Intent:* /// Initialize a text-based logger using the given pixel-based framebuffer as output.
  * `mapping_addr_page_aligned` **(Many-Argument Workhorses)** (Impact: 5.0)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (78.7%), Guard Balance (formerly Safety Score) (69.7%), Mutation Surface (formerly State Flux) (15.6%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `classify_fat_entry` **(Many-Argument Workhorses)** (Impact: 17.1)
  * `find_file_in_root_dir` **(Many-Argument Workhorses)** (Impact: 14.3)
  * `parse` **(Type Conversions)** (Impact: 10.7)
  * `fat_entry_of_nth_cluster` **(Type Conversions)** (Impact: 10.5)
  * `fat_type` **(Compute Cores)** (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **47**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (74.6%), Mutation Surface (formerly State Flux) (50.1%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (17.9%)
- **Documentation Coverage:** 87.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `main_inner` **(Many-Argument Workhorses)** (Impact: 26.3)
  * `init_logger` **(Many-Argument Workhorses)** (Impact: 18.1)
  * `create_page_tables` **(Many-Argument Workhorses)** (Impact: 13.9)
    * *Intent:* /// Creates page table abstraction types for both the bootloader and kernel page tables.
  * `load_file_from_disk` **(Many-Argument Workhorses)** (Impact: 10.1)
  * `load_file_from_tftp_boot_server` **(Many-Argument Workhorses)** (Impact: 9.9)
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **3**; blast radius 11.002; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (71.6%), Connectivity (formerly Api Exposure) (59.1%), Guard Balance (formerly Safety Score) (35.8%)
- **Documentation Coverage:** 35.1351% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `deserialize` **(Compute Cores)** (Impact: 41.3)
    * *Intent:* /// Tries to deserialize a config byte array that was created using [`Self::serialize`]. /// /// Thi...
  * `serialize` **(Defensive Guards)** (Impact: 14.5)
    * *Intent:* /// Serializes the configuration to a byte array. /// /// This is used by the [`crate::entry_point`]...
  * `random` **(I/O & Config Routines)** (Impact: 10.7)
  * `deserialize` **(Generic / Templated Code)** (Impact: 6.3)
  * `random` **(I/O & Config Routines)** (Impact: 5.8)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (43.7%), Debt Markers (formerly Tech Debt) (33.0%), Mutation Surface (formerly State Flux) (28.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (17.9%)
- **Documentation Coverage:** 36.3636% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `construct_memory_map` **(Many-Argument Workhorses)** (Impact: 18.8)
    * *Intent:* /// Converts this type to a boot info memory map. /// /// The memory map is placed in the given `reg...
  * `split_and_add_region` **(Many-Argument Workhorses)** (Impact: 14.0)
  * `allocate_frame` **(Defensive Guards)** (Impact: 11.0)
  * `allocate_frame_from_descriptor` **(Compute Cores)** (Impact: 7.9)
  * `test_multiple_regions` **(I/O & Config Routines)** (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **4**; blast radius 21.443; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Connectivity (formerly Api Exposure) (71.4%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.4%), Concurrency Surface (formerly Concurrency) (21.4%), Dead Code Surface (formerly Dead Code) (15.9%)
- **Documentation Coverage:** 26.4706% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `as_ref` **(Defensive Guards)** (Impact: 3.1)
    * *Intent:* /// Converts from `&Optional<T>` to `Option<&T>`. /// /// For convenience, this method directly perf...
  * `as_mut` **(Defensive Guards)** (Impact: 3.1)
    * *Intent:* /// Converts from `&mut Optional<T>` to `Option<&mut T>`. /// /// For convenience, this method direc...
  * `from` **(Defensive Guards)** (Impact: 3.1)
  * `from` **(Generic / Templated Code)** (Impact: 3.1)
  * `new` **(State Mutators)** (Impact: 2.4)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **12**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (84.1%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (59.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (52.3%)
- **Documentation Coverage:** 29.1667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_uefi_tftp_folder` **(Compute Cores)** (Impact: 15.5)
  * `create_fat_filesystem_image` **(Many-Argument Workhorses)** (Impact: 11.2)
  * `create_bios_image` **(Compute Cores)** (Impact: 10.1)
  * `create_uefi_image` **(Compute Cores)** (Impact: 9.4)
  * `create_uefi_fat_partition` **(Parameter Forwarders)** (Impact: 4.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
* *State Mutation (weighted view):* 13
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (91.2%), Test Surface (formerly Verification) (80.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (67.0%), Concurrency Surface (formerly Concurrency) (62.2%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `build_bios_stage_2` **(I/O & Config Routines)** (Impact: 10.3)
  * `build_uefi_bootloader` **(Annotated & Test Methods)** (Impact: 9.9)
  * `build_bios_boot_sector` **(I/O & Config Routines)** (Impact: 9.2)
  * `build_bios_stage_3` **(I/O & Config Routines)** (Impact: 9.2)
  * `build_bios_stage_4` **(I/O & Config Routines)** (Impact: 9.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 5 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **24**; blast radius 7.721; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (36.4%), Mutation Surface (formerly State Flux) (35.5%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (35.4%)
- **Documentation Coverage:** 10.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `new` **(Many-Argument Workhorses)** (Impact: 49.2)
    * *Intent:* /// Initializes a new instance. /// /// Marks the statically configured virtual address ranges from ...
  * `get_free_entries` **(Defensive Guards)** (Impact: 10.1)
    * *Intent:* /// Returns the first index of a `num` contiguous unused level 4 entries and marks them as /// used....
  * `get_free_address` **(Many-Argument Workhorses)** (Impact: 7.2)
    * *Intent:* /// Returns a virtual address in one or more unused level 4 entries and marks them as used. /// /// ...
  * `mark_segments` **(Generic / Templated Code)** (Impact: 4.6)
    * *Intent:* /// Marks the virtual address range of all segments as used.
  * `mark_range_as_used` **(Generic / Templated Code)** (Impact: 4.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (53.5%), Debt Markers (formerly Tech Debt) (49.6%), Mutation Surface (formerly State Flux) (23.0%)
- **Documentation Coverage:** 50.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_disk_image` **(Many-Argument Workhorses)** (Impact: 53.0)
    * *Intent:* /// Creates a bootable disk image from the given bootloader executable.
  * `pad_to_nearest_block_size` **(Compute Cores)** (Impact: 8.5)
  * `from` **(State Mutators)** (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Cascading Flux:* 2 instances
* *High Risk Execution (weighted view):* 0
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **4**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (51.3%), Mutation Surface (formerly State Flux) (20.8%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (17.9%), Debt Markers (formerly Tech Debt) (17.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `get_best_mode` **(Many-Argument Workhorses)** (Impact: 34.3)
  * `get_mode` **(Type Conversions)** (Impact: 5.8)
  * `query` **(Generic / Templated Code)** (Impact: 4.2)
  * `query` **(Generic / Templated Code)** (Impact: 1.9)
  * `enable` **(Parameter Forwarders)** (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **11**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (95.7%), Guard Balance (formerly Safety Score) (71.4%), Debt Markers (formerly Tech Debt) (31.8%), Concurrency Surface (formerly Concurrency) (29.8%)
- **Documentation Coverage:** 42.8571% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `write_pixel` **(Many-Argument Workhorses)** (Impact: 9.9)
  * `write_char` **(Compute Cores)** (Impact: 7.8)
    * *Intent:* /// Writes a single char to the framebuffer. Takes care of special control characters, such as /// n...
  * `write_rendered_char` **(Compute Cores)** (Impact: 5.6)
    * *Intent:* /// Prints a rendered char into the framebuffer. /// Updates `self.x_pos`.
  * `write_str` **(Parameter Forwarders)** (Impact: 3.8)
  * `get_char_raster` **(Defensive Guards)** (Impact: 3.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **31**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (73.9%), Guard Balance (formerly Safety Score) (64.8%), Mutation Surface (formerly State Flux) (35.7%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (17.9%)
- **Documentation Coverage:** 87.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_start` **(Compute Cores)** (Impact: 20.1)
  * `init_logger` **(Many-Argument Workhorses)** (Impact: 6.7)
  * `detect_rsdp` **(Generic / Templated Code)** (Impact: 4.0)
  * `panic` **(State Mutators)** (Impact: 3.5)
  * `create_page_tables` **(Generic / Templated Code)** (Impact: 3.1)
    * *Intent:* /// Creates page table abstraction types for both the bootloader and kernel page tables.
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **1**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Mutation Surface (formerly State Flux) (97.7%), Guard Balance (formerly Safety Score) (75.9%), Complexity Load (formerly Cognitive Load) (31.0%)
- **Documentation Coverage:** 90.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `read_exact_into` **(Type Conversions)** (Impact: 9.7)
  * `read_exact_at` **(Many-Argument Workhorses)** (Impact: 5.2)
  * `seek` **(Parameter Forwarders)** (Impact: 3.9)
  * `read_exact_at` **(Parameter Forwarders)** (Impact: 2.0)
  * `read_exact_into` **(Parameter Forwarders)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
* *State Mutation (weighted view):* 16
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (100.0%), Guard Balance (formerly Safety Score) (87.1%), Debt Markers (formerly Tech Debt) (38.4%), Mutation Surface (formerly State Flux) (11.5%)
- **Documentation Coverage:** 85.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `try_load_file` **(Many-Argument Workhorses)** (Impact: 19.7)
  * `start` **(Many-Argument Workhorses)** (Impact: 17.6)
  * `split_array_ref` **(Parameter Forwarders)** (Impact: 3.2)
    * *Intent:* /// Taken from https://github.com/rust-lang/rust/blob/e100ec5bc7cd768ec17d75448b29c9ab4a39272b/libra...
  * `load_file` **(Parameter Forwarders)** (Impact: 2.9)
  * `_start` **(State Mutators)** (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 6
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (88.5%), Guard Balance (formerly Safety Score) (80.0%), Concurrency Surface (formerly Concurrency) (31.1%), Complexity Load (formerly Cognitive Load) (25.1%)
- **Documentation Coverage:** 86.6667% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `write_char` **(Compute Cores)** (Impact: 7.7)
  * `write_rendered_char` **(Compute Cores)** (Impact: 5.6)
  * `write_pixel` **(Many-Argument Workhorses)** (Impact: 5.4)
  * `write_str` **(Parameter Forwarders)** (Impact: 3.8)
  * `new` **(Generic / Templated Code)** (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
* *State Mutation (weighted view):* 13
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (84.5%), Guard Balance (formerly Safety Score) (61.4%), Debt Markers (formerly Tech Debt) (55.3%), Complexity Load (formerly Cognitive Load) (18.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_mbr_disk` **(Many-Argument Workhorses)** (Impact: 38.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
* *State Mutation (weighted view):* 9
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **8**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (93.9%), Guard Balance (formerly Safety Score) (72.3%), Debt Markers (formerly Tech Debt) (24.1%), Complexity Load (formerly Cognitive Load) (23.8%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `add_files_to_image` **(Compute Cores)** (Impact: 17.3)
  * `create_fat_filesystem` **(Many-Argument Workhorses)** (Impact: 14.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (91.2%), Concurrency Surface (formerly Concurrency) (63.7%), Connectivity (formerly Api Exposure) (8.3%), Complexity Load (formerly Cognitive Load) (5.2%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `run_qemu` **(Generic / Templated Code)** (Impact: 8.7)
  * `run_test_kernel_internal` **(Defensive Guards)** (Impact: 7.7)
  * `run_test_kernel_on_uefi_pxe` **(Annotated & Test Methods)** (Impact: 2.2)
  * `run_test_kernel_with_config_file` **(State Mutators)** (Impact: 2.0)
  * `run_test_kernel_on_uefi` **(Annotated & Test Methods)** (Impact: 2.0)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **6**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (52.0%), Mutation Surface (formerly State Flux) (45.6%), Debt Markers (formerly Tech Debt) (34.3%), Dead Code Surface (formerly Dead Code) (20.7%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `create_gpt_disk` **(Compute Cores)** (Impact: 29.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (66.3%), Connectivity (formerly Api Exposure) (15.7%), Complexity Load (formerly Cognitive Load) (5.3%), Test Surface (formerly Verification) (2.3%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `is_unknown` **(State Mutators)** (Impact: 3.1)
  * `hlt` **(State Mutators)** (Impact: 1.1)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **7**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (31.5%), Connectivity (formerly Api Exposure) (6.6%), Complexity Load (formerly Cognitive Load) (5.1%), Test Surface (formerly Verification) (2.6%)
- **Documentation Coverage:** 20.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `copy_to` **(Compute Cores)** (Impact: 15.0)
    * *Intent:* /// Copy this data source to the specified target that implements io::Write
  * `len` **(Type Conversions)** (Impact: 6.1)
    * *Intent:* /// Get the length of the inner data source
  * `fmt` **(Generic / Templated Code)** (Impact: 4.1)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **3**; blast radius 7.721; role: Isolated/Orphan
- **Top Surface Vectors:** Guard Balance (formerly Safety Score) (51.7%), Debt Markers (formerly Tech Debt) (47.5%), Concurrency Surface (formerly Concurrency) (28.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (17.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `copy_to_protected_mode` **(Parameter Forwarders)** (Impact: 3.8)
  * `new` **(Interface Declarations)** (Impact: 2.1)
  * `enter_protected_mode_and_jump_to_stage_3` **(Parameter Forwarders)** (Impact: 1.9)
  * `clear_interrupts_and_load` **(Type Conversions)** (Impact: 1.8)
  * `read_from_protected_mode` **(Annotated & Test Methods)** (Impact: 1.7)
**Structural Signatures (Net Mitigated Signals):**
...
- **Blast Radius:** changing it is visible to **1** in-repo importer(s); it depends on **5**; blast radius 11.002; role: Pure Producer (Foundation)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (44.4%), Guard Balance (formerly Safety Score) (38.2%), Connectivity (formerly Api Exposure) (17.1%), Complexity Load (formerly Cognitive Load) (7.6%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `tsc_entropy` **(I/O & Config Routines)** (Impact: 5.0)
    * *Intent:* /// Gather entropy by reading the current time with the `RDTSC` instruction if it's available. /// /...
  * `rd_rand_entropy` **(Defensive Guards)** (Impact: 4.7)
    * *Intent:* /// Gather entropy by requesting random numbers with `RDRAND` instruction if it's available. /// ///...
  * `get_random_64` **(Defensive Guards)** (Impact: 4.7)
    * *Intent:* /// Try to fetch a 64 bit random value with a retry count limit of 10. /// /// This function is a po...
  * `build_rng` **(I/O & Config Routines)** (Impact: 3.8)
    * *Intent:* /// Gather entropy from various sources to seed a RNG.
  * `pit_entropy` **(Interface Declarations)** (Impact: 2.9)
