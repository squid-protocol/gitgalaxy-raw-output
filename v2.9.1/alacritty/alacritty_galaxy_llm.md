...
## 1. EXECUTIVE SUMMARY
- **Scope:** 260 analyzed artifact(s), 26023 LOC.
- **Load-bearing artifact:** `CONTRIBUTING.md` -- 2 in-repo importer(s) depend on it. Changes here propagate furthest.
- **Top orchestrator:** `alacritty/src/event.rs` -- pulls in 99 dependencies, the widest assembly point in the scan.
- **Heaviest artifact:** `alacritty_terminal/src/term/mod.rs` at magnitude 1349.74 (structural weight, not risk).
...
| Avg Path Length | 1.0714 | Mean import hops from a file to each file it transitively depends on. Higher = Longer dependency chains. |
...
> **Architectural Drift Z-Score:** `2.968`
> **Composition Archetype:** `Mid Flat Project` (z +2.97; from the repo's file-archetype mix)
> **File Composition:** Data / Markup / Trivial 61%, Large Core Modules (3) 15%, State Mutators Files 8%, Generic / Templated Code Files 5%, Declarative / Non-Code 4%
...
| Doc Surface (formerly Documentation Exposure) _(coverage)_ | 0.0 | 100.0 | 22.2 | 0.0 | 0.0 |
...
- `_alacritty` **(Many-Argument Workhorses)** (@ `extra/completions/alacritty.bash`) -> Impact: **194.2** | LOC: 525
- `box_drawing` **(Many-Argument Workhorses)** (@ `alacritty/src/renderer/text/builtin_font.rs`) -> Impact: **121.5** | LOC: 549
- `draw` **(Many-Argument Workhorses)** (@ `alacritty/src/display/mod.rs`) -> Impact: **101.0** | LOC: 273
...
- `regex_search_internal` **(Many-Argument Workhorses)** (@ `alacritty_terminal/src/term/search.rs`) -> Impact: **97.4** | LOC: 160
  * *Intent:* /// Find the next regex match. /// /// To automatically log regex complexity errors, use [`Self::regex_search`] instead.
- `shrink_columns` **(Many-Argument Workhorses)** (@ `alacritty_terminal/src/grid/resize.rs`) -> Impact: **87.2** | LOC: 144
  * *Intent:* /// Shrink number of columns in each row, reflowing if necessary.
- `user_event` **(Many-Argument Workhorses)** (@ `alacritty/src/event.rs`) -> Impact: **81.0** | LOC: 180
- `grow_columns` **(Many-Argument Workhorses)** (@ `alacritty_terminal/src/grid/resize.rs`) -> Impact: **73.1** | LOC: 142
  * *Intent:* /// Grow number of columns in each row, reflowing if necessary.
- `visit_map` **(Many-Argument Workhorses)** (@ `alacritty/src/config/bindings.rs`) -> Impact: **69.8** | LOC: 149
- `scroll_terminal` **(Many-Argument Workhorses)** (@ `alacritty/src/input/mod.rs`) -> Impact: **68.3** | LOC: 69
- `deserialize` **(Generic / Templated Code)** (@ `alacritty/src/config/bindings.rs`) -> Impact: **67.2** | LOC: 212
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **63**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.5%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (67.9%), Debt Markers (formerly Tech Debt) (55.1%)
- **Documentation Coverage:** 60.3175% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `line_to_string` **(Many-Argument Workhorses)** (Impact: 43.3)
    * *Intent:* /// Convert a single line in the grid to a String.
  * `input` **(Many-Argument Workhorses)** (Impact: 31.6)
    * *Intent:* /// A character to be displayed.
  * `clear_screen` **(Many-Argument Workhorses)** (Impact: 19.1)
  * `put_tab` **(Compute Cores)** (Impact: 18.8)
    * *Intent:* /// Insert tab at cursor position.
  * `write_at_cursor` **(Compute Cores)** (Impact: 15.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **99**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.7%), Connectivity (formerly Api Exposure) (81.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (65.7%)
- **Documentation Coverage:** 55.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `user_event` **(Many-Argument Workhorses)** (Impact: 81.0)
  * `handle_event` **(Many-Argument Workhorses)** (Impact: 58.2)
    * *Intent:* /// Handle events from winit.
  * `paste` **(Many-Argument Workhorses)** (Impact: 24.1)
    * *Intent:* /// Paste a text into the terminal.
  * `semantic_word` **(Compute Cores)** (Impact: 21.4)
    * *Intent:* /// Get the semantic word at the specified point.
  * `scroll` **(Compute Cores)** (Impact: 19.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **73**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (60.6%), Mutation Surface (formerly State Flux) (48.2%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (29.4%)
- **Documentation Coverage:** 84.4828% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `scroll_terminal` **(Many-Argument Workhorses)** (Impact: 68.3)
  * `execute` **(Many-Argument Workhorses)** (Impact: 45.0)
  * `mouse_moved` **(Compute Cores)** (Impact: 39.6)
  * `normal_mouse_report` **(Type Conversions)** (Impact: 23.6)
  * `mouse_input` **(Many-Argument Workhorses)** (Impact: 20.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **85**; blast radius 3.703; role: Pure Consumer (Orchestrator)
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (90.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (62.6%), Debt Markers (formerly Tech Debt) (39.5%)
- **Documentation Coverage:** 60.6742% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `draw` **(Many-Argument Workhorses)** (Impact: 101.0)
    * *Intent:* /// Draw the screen. /// /// A reference to Term whose state is being drawn must be provided. /// //...
  * `update_highlighted_hints` **(Many-Argument Workhorses)** (Impact: 40.1)
    * *Intent:* /// Update the mouse/vi mode cursor hint highlighting. /// /// This will return whether the highligh...
  * `new` **(Many-Argument Workhorses)** (Impact: 38.4)
  * `handle_update` **(Many-Argument Workhorses)** (Impact: 28.2)
    * *Intent:* // XXX: this function must not call to any `OpenGL` related tasks. Renderer updates are // performed...
  * `draw_ime_preview` **(Many-Argument Workhorses)** (Impact: 25.6)
**Contextual Mitigations & Amplifications:**
...
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (98.1%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (65.9%)
- **Documentation Coverage:** 100.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `_alacritty` **(Many-Argument Workhorses)** (Impact: 194.2)
  * `Anonymous_Block` **(Compute Cores)** (Impact: 5.2)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **34**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (99.1%), Mutation Surface (formerly State Flux) (92.4%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (71.5%)
- **Documentation Coverage:** 54.4118% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `regex_search_internal` **(Many-Argument Workhorses)** (Impact: 97.4)
    * *Intent:* /// Find the next regex match. /// /// To automatically log regex complexity errors, use [`Self::reg...
  * `bracket_search` **(Compute Cores)** (Impact: 31.5)
    * *Intent:* /// Find next matching bracket.
  * `skip_fullwidth` **(Many-Argument Workhorses)** (Impact: 21.8)
    * *Intent:* /// Advance a grid iterator over fullwidth characters.
  * `inline_search_right` **(Compute Cores)** (Impact: 19.3)
    * *Intent:* /// Searching to the right, find the next character contained in `needles`.
  * `next_match_right` **(Many-Argument Workhorses)** (Impact: 16.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **31**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (43.1%), Debt Markers (formerly Tech Debt) (36.6%), Mutation Surface (formerly State Flux) (32.3%)
- **Documentation Coverage:** 90.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `visit_map` **(Many-Argument Workhorses)** (Impact: 69.8)
  * `deserialize` **(Generic / Templated Code)** (Impact: 67.2)
  * `triggers_match` **(Compute Cores)** (Impact: 18.4)
  * `deserialize` **(I/O & Config Routines)** (Impact: 18.1)
  * `deserialize` **(Generic / Templated Code)** (Impact: 9.8)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **21**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (63.2%), Debt Markers (formerly Tech Debt) (59.9%), Complexity Load (formerly Cognitive Load) (36.0%)
- **Documentation Coverage:** 62.1622% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `semantic` **(Many-Argument Workhorses)** (Impact: 34.0)
    * *Intent:* /// Move by semantically separated word, like w/b/e/ge in vi.
  * `motion` **(Many-Argument Workhorses)** (Impact: 31.7)
    * *Intent:* /// Move vi mode cursor.
  * `word` **(Many-Argument Workhorses)** (Impact: 26.5)
    * *Intent:* /// Move by whitespace separated word, like W/B/E/gE in vi.
  * `first_occupied` **(Defensive Guards)** (Impact: 24.7)
    * *Intent:* /// Find next non-empty cell to move to.
  * `is_boundary` **(Generic / Templated Code)** (Impact: 12.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **9**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (60.8%), Mutation Surface (formerly State Flux) (43.0%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (13.8%)
- **Documentation Coverage:** 26.9231% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `box_drawing` **(Many-Argument Workhorses)** (Impact: 121.5)
  * `draw_rounded_corner` **(Compute Cores)** (Impact: 36.2)
    * *Intent:* /// Draws a quarter of a circle centered in `(0., self.height - radius)` with radius /// `self.width...
  * `draw_line` **(Type Conversions)** (Impact: 35.2)
    * *Intent:* /// Xiaolin Wu's line drawing from (`from_x`, `from_y`) to (`to_x`, `to_y`).
  * `powerline_drawing` **(Type Conversions)** (Impact: 29.6)
  * `put_pixel` **(Type Conversions)** (Impact: 13.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **27**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (98.9%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (54.2%), Debt Markers (formerly Tech Debt) (44.0%)
- **Documentation Coverage:** 23.913% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `hint_post_processing` **(Compute Cores)** (Impact: 37.5)
    * *Intent:* /// Apply some hint post processing heuristics. /// /// This will check the end of the hint and make...
  * `update_matches` **(Many-Argument Workhorses)** (Impact: 21.9)
    * *Intent:* /// Update the visible hint matches and key labels.
  * `keyboard_input` **(Many-Argument Workhorses)** (Impact: 20.1)
    * *Intent:* /// Handle keyboard input during hint selection.
  * `highlighted_at` **(Many-Argument Workhorses)** (Impact: 19.6)
    * *Intent:* /// Check if there is a hint highlighted at the specified point.
  * `hyperlink_at` **(Compute Cores)** (Impact: 18.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **13**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Guard Balance (formerly Safety Score) (95.5%), Test Surface (formerly Verification) (80.0%), Complexity Load (formerly Cognitive Load) (38.3%)
- **Documentation Coverage:** 0.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `shrink_columns` **(Many-Argument Workhorses)** (Impact: 87.2)
    * *Intent:* /// Shrink number of columns in each row, reflowing if necessary.
  * `grow_columns` **(Many-Argument Workhorses)** (Impact: 73.1)
    * *Intent:* /// Grow number of columns in each row, reflowing if necessary.
  * `resize` **(Generic / Templated Code)** (Impact: 7.9)
    * *Intent:* /// Resize the grid's width and/or height.
  * `grow_lines` **(Generic / Templated Code)** (Impact: 4.8)
    * *Intent:* /// Add lines to the visible area. /// /// Alacritty keeps the cursor at the bottom of the terminal ...
  * `shrink_lines` **(Type Conversions)** (Impact: 4.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **23**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (50.8%), Mutation Surface (formerly State Flux) (29.3%), Historical Churn (predictive layer, promotion pending #2987) (formerly Churn) (18.0%)
- **Documentation Coverage:** 20.0% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `build_sequence` **(Many-Argument Workhorses)** (Impact: 43.5)
    * *Intent:* /// Build a key's keyboard escape sequence based on the given `key`, `mods`, and `mode`. /// /// The...
  * `key_input` **(Compute Cores)** (Impact: 35.3)
    * *Intent:* /// Process key input.
  * `try_build_textual` **(Many-Argument Workhorses)** (Impact: 34.4)
    * *Intent:* /// Try building sequence from the event's emitting text.
  * `process_key_bindings` **(Defensive Guards)** (Impact: 33.0)
    * *Intent:* /// Attempt to find a binding and execute its action. /// /// The provided mode, mods, and key must ...
  * `try_build_control_char_or_mod` **(Many-Argument Workhorses)** (Impact: 25.3)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **22**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (97.5%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (78.5%), Guard Balance (formerly Safety Score) (67.3%)
- **Documentation Coverage:** 52.381% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `rotate` **(Many-Argument Workhorses)** (Impact: 38.5)
  * `contains_cell` **(Many-Argument Workhorses)** (Impact: 28.3)
    * *Intent:* /// Check if the cell at a point is part of the selection.
  * `range_simple` **(Many-Argument Workhorses)** (Impact: 21.8)
  * `is_empty` **(Compute Cores)** (Impact: 21.4)
  * `range_block` **(Compute Cores)** (Impact: 17.1)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **19**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (91.6%), Test Surface (formerly Verification) (80.0%), Debt Markers (formerly Tech Debt) (73.5%), Guard Balance (formerly Safety Score) (59.2%)
- **Documentation Coverage:** 47.6923% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `scroll_up` **(Many-Argument Workhorses)** (Impact: 24.8)
    * *Intent:* /// Move lines at the bottom toward the top. /// /// This is the performance-sensitive part of scrol...
  * `scroll_down` **(Many-Argument Workhorses)** (Impact: 22.9)
  * `clear_viewport` **(Type Conversions)** (Impact: 9.7)
  * `reset_region` **(Type Conversions)** (Impact: 8.1)
    * *Intent:* /// Reset a visible region within the grid.
  * `eq` **(State Mutators)** (Impact: 7.3)
**Contextual Mitigations & Amplifications:**
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **34**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (86.6%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (61.5%), Complexity Load (formerly Cognitive Load) (14.4%)
- **Documentation Coverage:** 45.7143% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `new` **(Many-Argument Workhorses)** (Impact: 39.2)
  * `new` **(Many-Argument Workhorses)** (Impact: 27.0)
  * `compute_fg_rgb` **(Type Conversions)** (Impact: 16.2)
    * *Intent:* /// Get the RGB color from a cell's foreground color.
  * `renderable_cursor` **(Compute Cores)** (Impact: 15.8)
    * *Intent:* /// Assemble the information required to render the terminal cursor.
  * `advance` **(Defensive Guards)** (Impact: 12.7)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **32**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (57.1%), Mutation Surface (formerly State Flux) (54.8%), Concurrency Surface (formerly Concurrency) (35.8%)
- **Documentation Coverage:** 85.1852% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `spawn` **(Compute Cores)** (Impact: 56.9)
  * `pty_read` **(Many-Argument Workhorses)** (Impact: 41.5)
  * `pty_write` **(Compute Cores)** (Impact: 17.1)
  * `recv` **(Generic / Templated Code)** (Impact: 7.6)
  * `new` **(Generic / Templated Code)** (Impact: 5.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **52**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (52.5%), Debt Markers (formerly Tech Debt) (50.2%), Mutation Surface (formerly State Flux) (33.6%)
- **Documentation Coverage:** 71.6981% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `visit_map` **(Generic / Templated Code)** (Impact: 15.9)
  * `deserialize` **(Generic / Templated Code)** (Impact: 15.6)
  * `deserialize_bindings` **(Generic / Templated Code)** (Impact: 10.2)
  * `deserialize` **(Generic / Templated Code)** (Impact: 8.1)
  * `compiled` **(Compute Cores)** (Impact: 6.9)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **51**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (76.1%), Guard Balance (formerly Safety Score) (67.3%), Debt Markers (formerly Tech Debt) (37.2%)
- **Documentation Coverage:** 4.3478% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `handle_event` **(Many-Argument Workhorses)** (Impact: 33.8)
    * *Intent:* /// Process events for this terminal window.
  * `submit_display_update` **(Many-Argument Workhorses)** (Impact: 32.7)
    * *Intent:* /// Submit the pending changes to the `Display`.
  * `update_config` **(Many-Argument Workhorses)** (Impact: 21.0)
    * *Intent:* /// Update the terminal window to the latest config.
  * `initial` **(Many-Argument Workhorses)** (Impact: 18.0)
    * *Intent:* /// Create initial window context that does bootstrapping the graphics API we're going to use.
  * `additional` **(Many-Argument Workhorses)** (Impact: 15.5)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **32**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (56.9%), Debt Markers (formerly Tech Debt) (26.6%)
- **Documentation Coverage:** 37.5% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `load_imports` **(Many-Argument Workhorses)** (Impact: 19.9)
    * *Intent:* /// Load all referenced configuration files.
  * `imports` **(Many-Argument Workhorses)** (Impact: 17.8)
    * *Intent:* /// Get all import paths for a configuration.
  * `deserialize_config` **(Compute Cores)** (Impact: 16.8)
    * *Intent:* /// Deserialize a configuration file.
  * `prune_yaml_nulls` **(Defensive Guards)** (Impact: 10.0)
    * *Intent:* /// Prune the nulls from the YAML to ensure TOML compatibility.
  * `installed_config` **(Defensive Guards)** (Impact: 9.8)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **44**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Connectivity (formerly Api Exposure) (78.6%), Guard Balance (formerly Safety Score) (58.0%), Mutation Surface (formerly State Flux) (41.1%)
- **Documentation Coverage:** 61.9048% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `new` **(Many-Argument Workhorses)** (Impact: 19.1)
    * *Intent:* /// Create a new window. /// /// This creates a window and fully initializes a window.
  * `get_platform_window` **(Defensive Guards)** (Impact: 7.3)
  * `update_ime_position` **(Type Conversions)** (Impact: 7.0)
    * *Intent:* /// Adjust the IME editor position according to the new location of the cursor.
  * `get_platform_window` **(Many-Argument Workhorses)** (Impact: 5.7)
  * `set_fullscreen` **(State Mutators)** (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 7 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **5**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (100.0%), Debt Markers (formerly Tech Debt) (89.1%), Guard Balance (formerly Safety Score) (80.8%), Complexity Load (formerly Cognitive Load) (34.3%)
- **Documentation Coverage:** 39.4737% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `text` **(Many-Argument Workhorses)** (Impact: 31.3)
    * *Intent:* /// Formatted message text lines.
  * `remove_target` **(Annotated & Test Methods)** (Impact: 6.1)
  * `remove_duplicates` **(Annotated & Test Methods)** (Impact: 4.1)
  * `pop` **(Defensive Guards)** (Impact: 3.3)
    * *Intent:* /// Remove the currently visible message.
  * `truncates_long_messages` **(Annotated & Test Methods)** (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 21 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **21**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (99.8%), Test Surface (formerly Verification) (80.0%), Guard Balance (formerly Safety Score) (72.4%), Connectivity (formerly Api Exposure) (53.4%)
- **Documentation Coverage:** 91.4286% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `fill_cube` **(Compute Cores)** (Impact: 22.0)
  * `from_str` **(Type Conversions)** (Impact: 12.4)
  * `fill_gray_ramp` **(Defensive Guards)** (Impact: 8.1)
  * `deserialize` **(Generic / Templated Code)** (Impact: 7.9)
  * `fill_named` **(Many-Argument Workhorses)** (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 10 instances
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **29**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Debt Markers (formerly Tech Debt) (87.7%), Mutation Surface (formerly State Flux) (76.3%), Guard Balance (formerly Safety Score) (51.6%), Connectivity (formerly Api Exposure) (12.5%)
- **Documentation Coverage:** 45.9459% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `override_pty_config` **(Defensive Guards)** (Impact: 9.4)
    * *Intent:* /// Override the [`PtyOptions`]'s fields with the [`TerminalOptions`].
  * `from_options` **(Compute Cores)** (Impact: 6.5)
    * *Intent:* /// Parse CLI config overrides.
  * `override_config` **(Compute Cores)** (Impact: 6.1)
    * *Intent:* /// Override configuration file with options from the CLI.
  * `override_config` **(Compute Cores)** (Impact: 6.0)
    * *Intent:* /// Apply CLI config overrides, removing broken ones.
  * `override_identity_config` **(Defensive Guards)** (Impact: 5.6)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **18**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Test Surface (formerly Verification) (80.0%), Mutation Surface (formerly State Flux) (66.5%), Guard Balance (formerly Safety Score) (56.4%), Debt Markers (formerly Tech Debt) (33.1%)
- **Documentation Coverage:** 70.8333% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `update_flag` **(Many-Argument Workhorses)** (Impact: 21.8)
    * *Intent:* /// Update the lines for a specific flag.
  * `update_uniforms` **(Defensive Guards)** (Impact: 17.6)
  * `draw` **(Many-Argument Workhorses)** (Impact: 13.7)
  * `new` **(Compute Cores)** (Impact: 11.5)
  * `push_rects` **(Many-Argument Workhorses)** (Impact: 11.0)
...
- **Blast Radius:** nothing in-repo imports it (entrypoint or orphan); it depends on **16**; blast radius 3.703; role: Isolated/Orphan
- **Top Surface Vectors:** Mutation Surface (formerly State Flux) (65.3%), Connectivity (formerly Api Exposure) (58.8%), Guard Balance (formerly Safety Score) (35.2%), Dead Code Surface (formerly Dead Code) (7.6%)
- **Documentation Coverage:** 38.4615% of unit weight undocumented
**Top Internal Functions/Classes:**
  * `initialize` **(Generic / Templated Code)** (Impact: 6.7)
    * *Intent:* /// Dynamically grow the storage buffer at runtime.
  * `compute_index` **(Type Conversions)** (Impact: 6.0)
    * *Intent:* /// Compute actual index in underlying storage given the requested index.
  * `swap` **(Many-Argument Workhorses)** (Impact: 5.2)
    * *Intent:* /// Swap implementation for Row<T>. /// /// Exploits the known size of Row<T> to produce a slightly ...
  * `with_capacity` **(Generic / Templated Code)** (Impact: 4.0)
  * `shrink_lines` **(Parameter Forwarders)** (Impact: 3.9)
