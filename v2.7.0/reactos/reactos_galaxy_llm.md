...
| connectivity | 166525 | 11615 | 21 | `sdk/include/wine/winternl.h` |
...
| dead_code | 55438 | 8246 | 8 | `modules/rosapps/applications/net/tsclient/porting-tools/rdesktop-core-tester/activex.cpp` |
...
- `xmlSchemaValAtomicType` (@ `sdk/lib/3rdparty/libxml2/xmlschemastypes.c`) -> Impact: **1832.0** | LOC: 1223
...
- `EDIT_EM_ReplaceSel` (@ `dll/win32/comctl32/edit.c`) -> Impact: **1471.8** | LOC: 2344
  * *Intent:* /********************************************************************* * * EM_REPLACESEL * * FIXME: handle ES_NUMBER and ES_OEMCONVERT here * */
- `Ext2StatusToString` (@ `sdk/lib/fslib/ext2lib/Disk.c`) -> Impact: **1284.3** | LOC: 880
  * *Intent:* * PROGRAMMER: Matt Wu <mattwu@163.com> * HOMEPAGE: http://ext2.yeah.net */ /* INCLUDES **************************************************************/...
- `recv_udp` (@ `drivers/network/tcpip/lwip/src/api/api_msg.c`) -> Impact: **1254.2** | LOC: 1961
...
- `TreeListProc` (@ `base/setup/reactos/treelist.c`) -> Impact: **1249.2** | LOC: 2266
  * *Intent:* //***************************************************************************** //* //* TreeListProc //* //*******************************************...
- `Ext2NtStatusToString` (@ `drivers/filesystems/ext2/src/debug.c`) -> Impact: **1164.4** | LOC: 1538
- `main` (@ `sdk/tools/mkisofs/schilytools/mkisofs/mkisofs.c`) -> Impact: **1103.8** | LOC: 1984
- `AtapiDmaInit` (@ `drivers/storage/ide/uniata/id_dma.cpp`) -> Impact: **950.2** | LOC: 1384
- `shader_get_registers_used` (@ `dll/directx/wine/wined3d/shader.c`) -> Impact: **946.6** | LOC: 836
  * *Intent:* /* Note that this does not count the loop register as an address register. */
- `CreateProcessInternalW` (@ `dll/win32/kernel32/client/proc.c`) -> Impact: **907.0** | LOC: 1699
  * *Intent:* /* * @implemented */

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Complexity Load | Avg Debt Markers |
|---|---|---|---|---|
| `sdk/lib/3rdparty/libxml2` | 37 | 135242.56 | 50.92% | 37.62% |
| `drivers/filesystems/btrfs` | 44 | 74516.28 | 57.65% | 20.84% |
| `dll/win32/comctl32` | 43 | 72234.06 | 54.95% | 31.33% |
| `dll/directx/wine/wined3d` | 32 | 61972.52 | 58.8% | 53.19% |
| `win32ss/user/ntuser` | 95 | 56797.8 | 39.12% | 37.11% |
| `dll/win32/msvcrt` | 50 | 56202.46 | 34.58% | 59.46% |
...
### 1. `base/applications/network/netsh/netsh.c` (C) -> Cumulative Risk: **754.81**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 338.72 | **LOC:** 510 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 85.7%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9986%), Churn (94.06%)
- **Heaviest Functions:** `wmain` (Impact: 86.8), `MergeStrings` (Impact: 15.5), `PrintError` (Impact: 13.8)
...
- **Heaviest Functions:** `__init__` (Impact: 33.8), `search_tree` (Impact: 27.2), `key_objectid_str` (Impact: 21.7)
...
- **Heaviest Functions:** `wine_dbgstr_an` (Impact: 35.6), `wine_dbgstr_wn` (Impact: 35.5), `winetest_vok` (Impact: 28.4)
...
- **Heaviest Functions:** `UefiFindAcpiTable` (Impact: 23.7), `DetectAcpiBios` (Impact: 12.3), `DetectDisplayController` (Impact: 10.2)
...
- **Heaviest Functions:** `Main_DirectDraw_SetCooperativeLevel` (Impact: 54.5)
...
- **Heaviest Functions:** `KdDebuggerInitialize0` (Impact: 35.4), `KdpDbgPrint` (Impact: 8.5), `KdpPollBreakIn` (Impact: 8.2)
...
- **Heaviest Functions:** `KdDebuggerInitialize0` (Impact: 33.8), `KdpPollByte` (Impact: 9.5), `KdpReceiveByte` (Impact: 9.5)
...
- **Heaviest Functions:** `PcVideoSetMode` (Impact: 29.5), `PcVideoGetDisplayMode` (Impact: 25.4), `PcVideoSetDisplayMode` (Impact: 25.1)
...
- **Heaviest Functions:** `CDefView::OnNotify` (Impact: 193.6), `CDefView::OnChangeNotify` (Impact: 116.9), `CDefView::OnCommand` (Impact: 109.6)

### 10. `sdk/lib/dnslib/string.c` (C) -> Cumulative Risk: **695.35**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `Dns_StringCopy` (Impact: 67.0), `Dns_GetBufferLengthForStringCopy` (Impact: 42.2), `Dns_CreateStringCopy_W` (Impact: 7.4)
...
**Top Internal Functions/Classes:**
  * `TreeListProc` (Impact: 1249.2)
    * *Intent:* //***************************************************************************** //* //* TreeListProc...
  * `TreeListDraw` (Impact: 538.6)
    * *Intent:* //***************************************************************************** //* //* TreeListDraw...
  * `TreeListMouseClick` (Impact: 450.2)
    * *Intent:* //***************************************************************************** //* //* TreeListMous...
  * `TreeListKeyDown` (Impact: 304.5)
    * *Intent:* //***************************************************************************** //* //* TreeListKeyD...
  * `TreeListInsertItem` (Impact: 241.8)
    * *Intent:* //***************************************************************************** //* //* TreeListInse...
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `safety_bypasses: 61`, `state_mutation: 1960`, `dead_code: 1`, `planned_debt: 7`, `fragile_debt: 7`, `unreferenced_by_name: 45`
* *Architecture:* `api: 123`, `import: 27`
...
**Top Internal Functions/Classes:**
  * `ITypeInfo_fnInvoke` (Impact: 455.5)
  * `WMSFT_compile_typeinfo_aux` (Impact: 172.5)
  * `DispCallFunc` (Impact: 161.9)
  * `DispCallFunc` (Impact: 123.2)
  * `DispCallFunc` (Impact: 113.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `AtapiInterrupt__` (Impact: 676.9)
  * `AtapiStartIo__` (Impact: 604.2)
  * `IssueIdentify` (Impact: 441.4)
    * *Intent:* --*/
  * `AtapiSendCommand` (Impact: 417.6)
    * *Intent:* --*/
  * `IdeSendCommand` (Impact: 411.2)
...
**Top Internal Functions/Classes:**
  * `D3DXLoadMeshFromXInMemory` (Impact: 280.4)
  * `D3DXLoadSkinMeshFromXof` (Impact: 220.9)
  * `D3DXCreateTextW` (Impact: 208.9)
  * `D3DXComputeTangentFrameEx` (Impact: 204.8)
    * *Intent:* /************************************************************************* * D3DXComputeTangentFrame...
  * `d3dx9_mesh_OptimizeInplace` (Impact: 155.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Uri_GetPropertyBSTR` (Impact: 220.3)
  * `canonicalize_path_hierarchical` (Impact: 195.8)
    * *Intent:* * start with a '/' then one is appended. * Ex: file://c:/test.mp3 -> file:///c:/test.mp3 * * 5). Dot...
  * `combine_uri` (Impact: 177.2)
  * `Uri_GetPropertyLength` (Impact: 143.2)
  * `compare_uris` (Impact: 130.8)
...
**Top Internal Functions/Classes:**
  * `flush_fcb` (Impact: 348.9)
  * `drop_chunk` (Impact: 220.8)
  * `update_tree_extents` (Impact: 212.2)
  * `try_tree_amalgamate` (Impact: 151.7)
  * `rationalize_extents` (Impact: 137.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `GdipDrawImagePointsRect` (Impact: 344.5)
  * `brush_fill_pixels` (Impact: 226.6)
  * `gdip_format_string` (Impact: 176.5)
  * `SOFTWARE_GdipDrawThinPath` (Impact: 161.2)
  * `apply_image_attributes` (Impact: 152.1)
...
**Top Internal Functions/Classes:**
  * `ok_sequence_` (Impact: 126.0)
  * `MsgCheckProc` (Impact: 103.2)
    * *Intent:* /************* window procedures ********************/
  * `subtest_swp_paint_regions_` (Impact: 100.4)
    * *Intent:* #define subtest_swp_paint_regions(w,p,c) subtest_swp_paint_regions_(__LINE__,w,p,c)
  * `test_PeekMessage` (Impact: 86.1)
  * `add_message_` (Impact: 73.8)
...
**Top Internal Functions/Classes:**
  * `_debug_message` (Impact: 821.4)
    * *Intent:* #define DEBUG_MESSAGE_LEN 1024 #ifdef DEBUG_LONG_MESSAGES
  * `_free_fcb` (Impact: 643.6)
    * *Intent:* #ifdef DEBUG_FCB_REFCOUNTS
  * `mount_vol` (Impact: 268.8)
  * `drv_cleanup` (Impact: 117.2)
  * `load_chunk_root` (Impact: 104.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ClassInterpretSenseInfo` (Impact: 490.1)
    * *Intent:* --*/
  * `ClassIoComplete` (Impact: 432.1)
    * *Intent:* --*/
  * `ClassDeviceControl` (Impact: 343.6)
    * *Intent:* --*/
  * `ClassDispatchPnp` (Impact: 265.2)
    * *Intent:* --*/
  * `ClassPnpStartDevice` (Impact: 143.6)
    * *Intent:* --*/
**Contextual Mitigations & Amplifications:**
...
- `base/applications/network/netsh/netsh.c` -> Churn: **94.06%** | Cog Load: 91.6845% | Debt: 92.3274%
- `base/applications/network/netsh/context.c` -> Churn: **86.08%** | Cog Load: 62.2166% | Debt: 15.5838%
...
- `sdk/lib/3rdparty/libxml2/parser.c` -> **Timo Kreuzer** (100.0% isolated ownership) | Magnitude: 12982.92
...
- `dll/directx/ddraw/rosdraw.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 88.1296%)
- `modules/rostests/winetests/d3drm/vector.c` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
- `sdk/include/psdk/specstrings.h` -> **Severity: 0.001** (Bridge: 0.0001 * Flux: 10.7254%)
- `sdk/include/psdk/strsafe.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
- `sdk/include/wine/test.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 99.993%)
