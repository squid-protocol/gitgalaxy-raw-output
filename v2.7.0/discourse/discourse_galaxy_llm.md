...
| Analyzed Artifacts (Scanned) | 19138 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 2179 |
| Total LOC | 1410525 |
...
|---|---|---|---|
| RUBY | 9018 | 762229 | 47.1% |
| JAVASCRIPT | 5227 | 471562 | 27.3% |
| YAML | 3558 | 90533 | 18.6% |
| CSS | 640 | 65023 | 3.3% |
| HTML | 321 | 7134 | 1.7% |
| JSON | 158 | 12714 | 0.8% |
| PLAINTEXT | 75 | 6 | 0.4% |
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 13.3 | 4.6 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 28.2 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 23.1 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 4.8 | 2.3 | 2.3 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 87.2 | 4.5 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 50.7 | 91.7 | 100.0 |
...
| connectivity | 24027 | 5773 | 2 | `config/routes.rb` |
...
| docs | 8074 | 1617 | 0 | `frontend/discourse/app/lib/plugin-api.gjs` |
| debt | 9482 | 1475 | 0 | `spec/lib/pretty_text_spec.rb` |
| mutation | 216853 | 10647 | 28 | `spec/lib/search_spec.rb` |
| dead_code | 15937 | 6420 | 2 | `app/services/staff_action_logger.rb` |
| credential | 4161 | 141 | 0 | `script/import_scripts/socialcast/test/test_data.rb` |
| threat | 1174 | 559 | 0 | `frontend/discourse/tests/unit/models/post-stream-test.js` |
...
- `Anonymous_Block` (@ `spec/requests/users_controller_spec.rb`) -> Impact: **1059.7** | LOC: 8133
- `Anonymous_Block` (@ `spec/lib/guardian_spec.rb`) -> Impact: **814.5** | LOC: 3448
  * *Intent:* # frozen_string_literal: true
- `Anonymous_Block` (@ `spec/requests/topics_controller_spec.rb`) -> Impact: **761.8** | LOC: 7084
  * *Intent:* # coding: utf-8 # frozen_string_literal: true
- `Anonymous_Block` (@ `spec/requests/admin/users_controller_spec.rb`) -> Impact: **534.0** | LOC: 2931
- `Anonymous_Block` (@ `spec/lib/topics_filter_spec.rb`) -> Impact: **528.6** | LOC: 2850
  * *Intent:* # frozen_string_literal: true
- `Anonymous_Block` (@ `spec/requests/posts_controller_spec.rb`) -> Impact: **480.8** | LOC: 3612
- `Anonymous_Block` (@ `spec/requests/session_controller_spec.rb`) -> Impact: **466.9** | LOC: 3414
- `Anonymous_Block` (@ `spec/lib/guardian/post_guardian_spec.rb`) -> Impact: **361.7** | LOC: 1345
  * *Intent:* # frozen_string_literal: true
- `extract_email_address_and_name` (@ `lib/email/receiver.rb`) -> Impact: **323.1** | LOC: 918
- `Anonymous_Block` (@ `plugins/discourse-assign/plugin.rb`) -> Impact: **304.6** | LOC: 1029

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Complexity Load | Avg Debt Markers |
|---|---|---|---|---|
| `lib` | 235 | 26811.22 | 45.1% | 35.49% |
| `app/models` | 278 | 20928.72 | 27.55% | 52.43% |
| `spec/requests` | 88 | 17112.9 | 7.9% | 0.0% |
| `script/import_scripts` | 63 | 16719.08 | 62.35% | 30.04% |
| `spec/lib` | 170 | 15912.5 | 8.49% | 0.0% |
| `frontend/discourse/app/lib` | 200 | 15161.36 | 35.49% | 60.1% |
| `frontend/discourse/app/components` | 351 | 14581.48 | 20.89% | 16.64% |
...
- `plugins/chat/assets/javascripts/discourse/services/chat-api.js` -> **43** Orphaned Functions | **0** Duplicates
...
- **Heaviest Functions:** `bulkSelectItemToggle` (Impact: 15.8), `_refreshUsers` (Impact: 4.2), `columnCount` (Impact: 3.6)
...
- **Heaviest Functions:** `topicsFrom` (Impact: 20.0), `loadMore` (Impact: 11.4), `displayCategoryInList` (Impact: 9.5)
...
- **Heaviest Functions:** `actOnSelection` (Impact: 18.2), `actOnGroup` (Impact: 13.2), `reloadMembers` (Impact: 9.4)
...
- **Heaviest Functions:** `performAjax` (Impact: 61.4), `ajax` (Impact: 52.7), `error` (Impact: 48.0)

### 5. `frontend/discourse/app/controllers/topic.js` (JAVASCRIPT) -> Cumulative Risk: **768.19**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1247.64 | **LOC:** 2067 | **CtrlFlow:** 16.7% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Api Exposure (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (99.5918%)
- **Heaviest Functions:** `onMessage` (Impact: 46.0), `deletePost` (Impact: 35.6), `replyToPost` (Impact: 31.5)

### 6. `frontend/discourse/app/routes/topic/from-params.js` (JAVASCRIPT) -> Cumulative Risk: **765.44**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `setupController` (Impact: 34.0), `model` (Impact: 6.9), `afterModel` (Impact: 4.6)

### 7. `frontend/discourse/app/models/category.js` (JAVASCRIPT) -> Cumulative Risk: **764.32**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `search` (Impact: 48.7), `validCategoryParent` (Impact: 30.2), `findBySlug` (Impact: 24.5)

### 8. `frontend/discourse/app/models/post-stream.js` (JAVASCRIPT) -> Cumulative Risk: **758.31**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `triggerNewPostsInStream` (Impact: 29.0), `loadIntoIdentityMap` (Impact: 19.5), `refresh` (Impact: 14.3)

### 9. `frontend/discourse/app/models/group.js` (JAVASCRIPT) -> Cumulative Risk: **750.79**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 304.1 | **LOC:** 491 | **CtrlFlow:** 13.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9982%), State Flux (97.7438%)
- **Heaviest Functions:** `asJSON` (Impact: 15.8), `reloadMembers` (Impact: 11.8), `findRequesters` (Impact: 11.8)

### 10. `frontend/discourse/app/lib/text.js` (JAVASCRIPT) -> Cumulative Risk: **748.7**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 154.8 | **LOC:** 211 | **CtrlFlow:** 17.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9665%), Concurrency (99.2287%), Documentation (88.8889%)
- **Heaviest Functions:** `excerpt` (Impact: 22.7), `buildBBCodeAttrs` (Impact: 11.2), `emojiUnescape` (Impact: 7.3)
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` (Impact: 1059.7)
  * `create_totp` (Impact: 3.2)
  * `enabled?` (Impact: 3.1)
  * `create_and_like_post` (Impact: 2.0)
  * `revoke` (Impact: 1.9)
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` (Impact: 761.8)
    * *Intent:* # coding: utf-8 # frozen_string_literal: true
  * `fabricate_topic` (Impact: 3.7)
  * `topic_user_post_timings_count` (Impact: 3.6)
  * `invite_group` (Impact: 1.9)
  * `extract_post_stream` (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `create_records` (Impact: 161.5)
  * `pre_cook` (Impact: 63.9)
    * *Intent:* # TODO Check if this is still up-to-date # Convert YouTube URLs to lazyYT DOMs before being transfor...
  * `process_raw` (Impact: 41.3)
  * `process_post` (Impact: 24.7)
  * `process_chat_channel` (Impact: 24.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `extract_email_address_and_name` (Impact: 323.1)
  * `process_internal` (Impact: 45.5)
  * `forwarded_email_create_topic` (Impact: 44.3)
  * `create_post` (Impact: 42.3)
  * `create_group_post` (Impact: 41.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `raw_with_placeholders_interpolated` (Impact: 26.0)
  * `import_bookmarks` (Impact: 20.9)
  * `update_topic_users` (Impact: 13.5)
  * `import_post_events` (Impact: 12.7)
  * `import_user_stats` (Impact: 12.0)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `process_advanced_search!` (Impact: 255.7)
  * `posts_query` (Impact: 70.0)
  * `apply_order` (Impact: 66.1)
  * `execute` (Impact: 30.7)
    * *Intent:* # Query a term
  * `sort_by_relevance` (Impact: 30.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `create` (Impact: 39.6)
  * `password_reset_update` (Impact: 26.4)
  * `user_menu_messages` (Impact: 25.3)
  * `search_users` (Impact: 25.1)
    * *Intent:* # the search can specify the parameter term or usernames, term will perform the classic user search ...
  * `notification_level` (Impact: 20.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` (Impact: 188.0)
    * *Intent:* # frozen_string_literal: true
  * `Anonymous_Block` (Impact: 77.3)
  * `Anonymous_Block` (Impact: 58.5)
  * `Anonymous_Block` (Impact: 18.9)
...
**Top Internal Functions/Classes:**
  * `bbcode_list_to_md` (Impact: 41.9)
  * `postprocess_post_raw` (Impact: 31.8)
    * *Intent:* # [QUOTE=<username>;<post_id>] raw.gsub!(/\[quote=([^;]+);(\d+)\]/im) do old_username, post_id = $1,...
  * `setup_category_moderator_groups` (Impact: 21.9)
  * `import_users` (Impact: 21.5)
  * `import_topics` (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` (Impact: 466.9)
  * `get_sso` (Impact: 2.0)
  * `login_with_sso_and_invite` (Impact: 1.9)
  * `sso_for_ip_specs` (Impact: 1.4)
...
**Top Internal Functions/Classes:**
  * `onMessage` (Impact: 46.0)
  * `deletePost` (Impact: 35.6)
  * `replyToPost` (Impact: 31.5)
    * *Intent:* // Post related methods
  * `retryOnRateLimit` (Impact: 26.4)
  * `_openComposerForEdit` (Impact: 22.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` (Impact: 480.8)
  * `Anonymous_Block` (Impact: 6.3)
...
**Top Internal Functions/Classes:**
  * `condition` (Impact: 6.1)
  * `condition` (Impact: 5.0)
  * `condition` (Impact: 4.3)
  * `jumpEnd` (Impact: 3.3)
  * `testCase` (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `copy_model` (Impact: 93.1)
  * `copy_solved` (Impact: 92.4)
  * `process_user_action` (Impact: 20.1)
  * `fix_user_columns` (Impact: 17.6)
  * `process_raw` (Impact: 17.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `system_avatar_template` (Impact: 12.2)
  * `update_ip_address!` (Impact: 11.8)
  * `count_by_signup_date` (Impact: 10.8)
  * `user_fields` (Impact: 9.5)
  * `letter_avatar_color` (Impact: 9.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `Anonymous_Block` (Impact: 105.9)
    * *Intent:* # frozen_string_literal: true
  * `Anonymous_Block` (Impact: 58.6)
  * `Anonymous_Block` (Impact: 50.3)
  * `Anonymous_Block` (Impact: 40.1)
...
- `frontend/discourse/tests/helpers/qunit-helpers.js` -> **Severity: 0.011** (Bridge: 0.0001 * Flux: 100.0%)
- `frontend/discourse/app/components/d-modal.gjs` -> **Severity: 0.005** (Bridge: 0.0 * Flux: 100.0%)
- `frontend/discourse/float-kit/components/d-menu.gjs` -> **Severity: 0.005** (Bridge: 0.0001 * Flux: 96.273%)
- `frontend/discourse/app/components/bulk-select-topics-dropdown.gjs` -> **Severity: 0.004** (Bridge: 0.0 * Flux: 94.0317%)
- `frontend/discourse/app/components/user-menu/notifications-list.js` -> **Severity: 0.004** (Bridge: 0.0 * Flux: 100.0%)
