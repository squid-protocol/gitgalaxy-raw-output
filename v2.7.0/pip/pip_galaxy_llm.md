...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 87.3 | 23.0 | 14.3 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 3.0 | 0.5 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 5.9 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 51.5 | 58.3 | 0.0 |
...
|---|---|---|---|---|
| memory | 1751 | 279 | 7 | `tests/functional/test_install.py` |
| cleanup | 80 | 36 | 0 | `src/pip/_vendor/urllib3/response.py` |
| guards | 7305 | 424 | 30 | `src/pip/_vendor/pkg_resources/__init__.py` |
| danger | 2867 | 349 | 11 | `src/pip/_vendor/pkg_resources/__init__.py` |
| concurrency | 980 | 147 | 4 | `src/pip/_vendor/packaging/tags.py` |
| connectivity | 6569 | 510 | 24 | `src/pip/_vendor/pkg_resources/__init__.py` |
| io | 2909 | 301 | 11 | `src/pip/_vendor/distlib/util.py` |
...
| docs | 4274 | 457 | 17 | `src/pip/_vendor/pkg_resources/__init__.py` |
| debt | 564 | 181 | 2 | `src/pip/_vendor/rich/_win32_console.py` |
| mutation | 39534 | 555 | 165 | `src/pip/_vendor/distlib/util.py` |
...
- `__init__` (@ `src/pip/_vendor/rich/style.py`) -> Impact: **247.8** | LOC: 75
- `__init__` (@ `src/pip/_vendor/rich/console.py`) -> Impact: **225.7** | LOC: 133
- `traverse` (@ `src/pip/_vendor/rich/pretty.py`) -> Impact: **184.7** | LOC: 296
- `urlopen` (@ `src/pip/_vendor/urllib3/connectionpool.py`) -> Impact: **158.4** | LOC: 379
- `_traverse` (@ `src/pip/_vendor/rich/pretty.py`) -> Impact: **156.6** | LOC: 252
  * *Intent:* """Walk the object depth first."""
- `_render` (@ `src/pip/_vendor/rich/table.py`) -> Impact: **152.1** | LOC: 180
- `extract` (@ `src/pip/_vendor/rich/traceback.py`) -> Impact: **152.1** | LOC: 189
- `get_scheme` (@ `src/pip/_internal/locations/__init__.py`) -> Impact: **137.8** | LOC: 164
- `_pack` (@ `src/pip/_vendor/msgpack/fallback.py`) -> Impact: **128.6** | LOC: 123
- `export_svg` (@ `src/pip/_vendor/rich/console.py`) -> Impact: **122.8** | LOC: 250
...
- **Unknown Dependencies:** `4169` packages imported that bypass the Zero-Trust whitelist.

## 11. CUMULATIVE RISK HITLIST (Top 10 Highest Risk Files)
> Cumulative Risk is the sum of all individual risk exposures. These files represent the highest multi-dimensional technical debt and architectural fragility.

### 1. `src/pip/_vendor/requests/auth.py` (PYTHON) -> Cumulative Risk: **733.22**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 313.6 | **LOC:** 315 | **CtrlFlow:** 18.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.9792%), Safety Score (98.1146%)
- **Heaviest Functions:** `build_digest_header` (Impact: 57.5), `_basic_auth_str` (Impact: 14.2), `handle_401` (Impact: 12.2)

### 2. `src/pip/_internal/resolution/resolvelib/requirements.py` (PYTHON) -> Cumulative Risk: **716.5**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 204.06 | **LOC:** 252 | **CtrlFlow:** 14.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9989%), Tech Debt (99.8802%)
- **Heaviest Functions:** `__init__` (Impact: 14.6), `format_for_error` (Impact: 7.7), `__eq__` (Impact: 5.5)

### 3. `src/pip/_internal/resolution/resolvelib/candidates.py` (PYTHON) -> Cumulative Risk: **712.28**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 420.32 | **LOC:** 592 | **CtrlFlow:** 12.6% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9998%), Tech Debt (98.843%), Documentation (92.2222%)
- **Heaviest Functions:** `__init__` (Impact: 34.1), `_check_metadata_consistency` (Impact: 9.8), `__init__` (Impact: 9.8)

### 4. `src/pip/_internal/resolution/resolvelib/base.py` (PYTHON) -> Cumulative Risk: **685.12**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 97.9 | **LOC:** 143 | **CtrlFlow:** 12.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.5343%), Tech Debt (98.1487%), Api Exposure (94.474%)
- **Heaviest Functions:** `is_satisfied_by` (Impact: 7.3), `__and__` (Impact: 5.6), `from_ireq` (Impact: 5.3)

### 5. `src/pip/_vendor/packaging/tags.py` (PYTHON) -> Cumulative Risk: **675.86**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
### 6. `src/pip/_internal/network/session.py` (PYTHON) -> Cumulative Risk: **654.39**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 273.38 | **LOC:** 533 | **CtrlFlow:** 15.9% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9984%), Safety Score (92.7602%), Verification (80.0%)
- **Heaviest Functions:** `is_secure_origin` (Impact: 31.0), `user_agent` (Impact: 27.9), `__init__` (Impact: 16.8)

### 7. `src/pip/_vendor/distlib/compat.py` (PYTHON) -> Cumulative Risk: **651.05**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1120.98 | **LOC:** 1138 | **CtrlFlow:** 23.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (95.9021%), Api Exposure (84.0164%)
- **Heaviest Functions:** `which` (Impact: 39.0), `match_hostname` (Impact: 35.0), `detect_encoding` (Impact: 30.0)

### 8. `src/pip/_internal/locations/_distutils.py` (PYTHON) -> Cumulative Risk: **649.21**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 139.04 | **LOC:** 174 | **CtrlFlow:** 20.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.7298%)
- **Heaviest Functions:** `distutils_scheme` (Impact: 57.6), `get_bin_prefix` (Impact: 5.8), `get_scheme` (Impact: 4.3)

### 9. `src/pip/_vendor/rich/align.py` (PYTHON) -> Cumulative Risk: **648.59**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 300.46 | **LOC:** 307 | **CtrlFlow:** 19.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (95.2056%)
- **Heaviest Functions:** `__rich_console__` (Impact: 70.5), `__init__` (Impact: 25.3), `generate_segments` (Impact: 17.9)

### 10. `src/pip/_internal/utils/logging.py` (PYTHON) -> Cumulative Risk: **643.87**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 260.2 | **LOC:** 397 | **CtrlFlow:** 13.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (90.5655%), Api Exposure (84.6713%)
- **Heaviest Functions:** `setup_logging` (Impact: 31.9), `emit` (Impact: 11.7), `handleError` (Impact: 9.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5.8e-05 | `Ripple Effect (Closeness):` 0.086531
...
- **Risk Profile:** Cognitive Load (47.2895%), Tech Debt (8.6812%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 225.7)
  * `export_svg` (Impact: 122.8)
  * `print` (Impact: 83.2)
  * `render_str` (Impact: 50.4)
  * `export_html` (Impact: 50.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.057486 | `Ripple Effect (Closeness):` 0.193696
...
- **Risk Profile:** Cognitive Load (38.3178%), Tech Debt (8.8566%)
**Top Internal Functions/Classes:**
  * `wrap` (Impact: 39.1)
  * `divide` (Impact: 35.1)
    * *Intent:* """Divide text in to a number of lines at given offsets. Args: offsets (Iterable[int]): Offsets used...
  * `split` (Impact: 33.5)
  * `highlight_regex` (Impact: 33.3)
  * `with_indent_guides` (Impact: 31.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.039644 | `Ripple Effect (Closeness):` 0.213663
...
- **Risk Profile:** Cognitive Load (45.4744%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `update` (Impact: 47.3)
  * `open` (Impact: 40.3)
  * `track` (Impact: 27.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000826 | `Ripple Effect (Closeness):` 0.085465
...
- **Risk Profile:** Cognitive Load (32.2891%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `assert_installed` (Impact: 81.6)
  * `run` (Impact: 61.0)
  * `diff_states` (Impact: 26.1)
    * *Intent:* # FIXME ScriptTest does something similar, but only within a single # ProcResult; this generalizes i...
  * `pip_install_local` (Impact: 18.5)
  * `_check_stderr` (Impact: 18.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (10.3501%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_install_dry_run_nothing_installed` (Impact: 15.0)
  * `test_install_builds_wheels` (Impact: 14.1)
    * *Intent:* # We need to use a subprocess to get the right value on Windows. res = script.run( "python", "-c", (...
  * `install_find_links` (Impact: 13.5)
  * `test_pep518_uses_build_env` (Impact: 11.0)
  * `test_install_editable_with_wrong_egg_name` (Impact: 10.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (47.1484%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_render` (Impact: 152.1)
  * `_calculate_column_widths` (Impact: 65.2)
  * `_get_cells` (Impact: 52.8)
  * `__init__` (Impact: 40.2)
  * `_collapse_widths` (Impact: 37.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003324 | `Ripple Effect (Closeness):` 0.145781
...
**Top Internal Functions/Classes:**
  * `traverse` (Impact: 184.7)
  * `_traverse` (Impact: 156.6)
    * *Intent:* """Walk the object depth first."""
  * `install` (Impact: 28.0)
  * `_ipy_display_hook` (Impact: 24.4)
  * `__rich_console__` (Impact: 19.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00079 | `Ripple Effect (Closeness):` 0.141993
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.001447
...
- **Risk Profile:** Cognitive Load (41.2979%), Tech Debt (16.2299%)
**Top Internal Functions/Classes:**
  * `get_tokens_unprocessed` (Impact: 53.2)
    * *Intent:* """ Split ``text`` into (tokentype, text) pairs. If ``context`` is given, use this lexer context ins...
  * `get_tokens_unprocessed` (Impact: 45.0)
  ...
  * `_preprocess_lexer_input` (Impact: 31.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00164 | `Ripple Effect (Closeness):` 0.112177
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000329 | `Ripple Effect (Closeness):` 0.132694
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.006404 | `Ripple Effect (Closeness):` 0.131798
...
- **Risk Profile:** Cognitive Load (37.7277%), Tech Debt (20.2058%)
**Top Internal Functions/Classes:**
  * `should_bypass_proxies` (Impact: 34.1)
    * *Intent:* """ Returns whether we should bypass proxies or not. :rtype: bool """
  * `super_len` (Impact: 27.5)
  * `get_netrc_auth` (Impact: 21.2)
    * *Intent:* """Returns the Requests tuple auth for a given url from netrc."""
  * `_validate_header_part` (Impact: 20.9)
  * `guess_json_utf` (Impact: 17.1)
    * *Intent:* """ :rtype: str """
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000812 | `Ripple Effect (Closeness):` 0.075754
...
- **Risk Profile:** Cognitive Load (58.6881%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `evaluate_link` (Impact: 75.6)
    * *Intent:* """ Determine whether a link is a candidate for installation. :return: A tuple (result, detail), whe...
  * `find_requirement` (Impact: 39.0)
  * `filter_unallowed_hashes` (Impact: 29.6)
  * `find_all_candidates` (Impact: 21.9)
    * *Intent:* """Find all available InstallationCandidate for project_name This checks index_urls and find_links. ...
  * `get_applicable_candidates` (Impact: 19.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.006625 | `Ripple Effect (Closeness):` 0.091912
...
- **Risk Profile:** Cognitive Load (56.9903%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `extract` (Impact: 152.1)
  * `install` (Impact: 56.8)
  * `__init__` (Impact: 54.6)
  * `_render_stack` (Impact: 44.4)
  * `__rich_console__` (Impact: 31.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.008526 | `Ripple Effect (Closeness):` 0.171255
...
- **Risk Profile:** Cognitive Load (62.301%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `parse_value` (Impact: 50.7)
  * `__init__` (Impact: 36.7)
  * `loads` (Impact: 20.8)
    * *Intent:* """Parse TOML from a string."""
  * `parse_inline_table` (Impact: 19.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003822 | `Ripple Effect (Closeness):` 0.140994
...
- **Risk Profile:** Cognitive Load (67.4192%), Tech Debt (11.5151%)
**Top Internal Functions/Classes:**
  * `get_installation_error` (Impact: 61.9)
  * `find_candidates` (Impact: 46.2)
  * `_iter_found_candidates` (Impact: 44.7)
  * `_report_single_requirement_conflict` (Impact: 41.1)
  * `_make_requirements_from_install_req` (Impact: 34.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001541 | `Ripple Effect (Closeness):` 0.076412
...
- **Risk Profile:** Cognitive Load (56.9007%), Tech Debt (12.9005%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 65.4)
  * `check_if_exists` (Impact: 22.6)
  ...
  * `__str__` (Impact: 18.1)
  * `ensure_build_location` (Impact: 17.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Rce:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.049548 | `Ripple Effect (Closeness):` 0.130109
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.002894
...
- `src/pip/_internal/exceptions.py` -> **Severity: 10.363** (Bridge: 0.1036 * Flux: 99.9999%)
- `src/pip/_vendor/rich/console.py` -> **Severity: 5.749** (Bridge: 0.0575 * Flux: 100.0%)
- `src/pip/_internal/req/req_install.py` -> **Severity: 4.955** (Bridge: 0.0495 * Flux: 100.0%)
- `src/pip/_vendor/rich/text.py` -> **Severity: 3.964** (Bridge: 0.0396 * Flux: 100.0%)
- `src/pip/_internal/cli/base_command.py` -> **Severity: 3.854** (Bridge: 0.0386 * Flux: 99.8975%)
