...
| Avg Path Length | 3.4094 | Hops between files. Lower = Tighter coupling. |
...
- `write_device` (@ `tools/regz/src/gen.zig`) -> Impact: **518.2** | LOC: 1465
- `write_interrupt_list` (@ `tools/regz/src/gen.zig`) -> Impact: **478.3** | LOC: 1472
- `write_struct` (@ `tools/regz/src/gen.zig`) -> Impact: **462.5** | LOC: 1461
- `Tokenizer` (@ `port/raspberrypi/rp2xxx/src/hal/pio/assembler/tokenizer.zig`) -> Impact: **369.6** | LOC: 1056
  * *Intent:* // the characters we're interested in are: // ';' -> line comment // '/' -> '/' -> line comment // '/' -> '*' -> block comment // '%' -> <whitespace> ...
- `write_fields` (@ `tools/regz/src/gen.zig`) -> Impact: **228.4** | LOC: 1551
- `load_into_db` (@ `tools/regz/src/embassy.zig`) -> Impact: **194.2** | LOC: 445
- `Encoder` (@ `port/raspberrypi/rp2xxx/src/hal/pio/assembler/encoder.zig`) -> Impact: **155.2** | LOC: 540
- `PioImpl` (@ `port/raspberrypi/rp2xxx/src/hal/pio/common.zig`) -> Impact: **116.2** | LOC: 453
- `DeviceController` (@ `core/src/core/usb.zig`) -> Impact: **114.7** | LOC: 388
...
- `to_string` (@ `drivers/base/DateTime.zig`) -> Impact: **103.4** | LOC: 144
...
- **Heaviest Functions:** `spawn` (Impact: 17.1), `interrupt_handler_c` (Impact: 16.9), `put` (Impact: 12.5)
...
- **Heaviest Functions:** `Uart` (Impact: 32.0), `init` (Impact: 10.6), `log` (Impact: 9.5)
...
- **Heaviest Functions:** `init` (Impact: 31.6), `set_baudrate` (Impact: 18.1), `set_enabled` (Impact: 10.4)
...
- **Heaviest Functions:** `wait_for_reset_done` (Impact: 3.3), `reset` (Impact: 1.8), `unreset_block_wait` (Impact: 1.8)
...
- **Heaviest Functions:** `setup_transfer` (Impact: 98.2), `get_increment` (Impact: 12.0), `setup_transfer_raw` (Impact: 11.5)
...
- **Heaviest Functions:** `configure_output` (Impact: 23.4), `configure_input` (Impact: 10.9), `configure_channel` (Impact: 6.8)
...
- **Heaviest Functions:** `__assert_func` (Impact: 12.2), `set_isr` (Impact: 10.9), `limit` (Impact: 10.2)
...
- **Heaviest Functions:** `comptime_check` (Impact: 8.4), `load_and_check` (Impact: 8.3), `RX_to_buffer` (Impact: 8.1)
...
- **Heaviest Functions:** `irq_set_mask_n_enabled_internal` (Impact: 19.5), `irq_set_exclusive_handler` (Impact: 6.0), `irq_set_priority` (Impact: 4.3)
...
- **Heaviest Functions:** `sub_peripheral_enum` (Impact: 15.7), `create_peripheral_enum` (Impact: 8.6), `load_timer_interrupt` (Impact: 7.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.005352
...
**Top Internal Functions/Classes:**
  * `run` (Impact: 47.1)
  * `format` (Impact: 30.0)
  * `read_wide_reg` (Impact: 22.8)
  * `generic_sub_cp` (Impact: 22.4)
  * `format_instruction` (Impact: 15.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.003641
...
**Top Internal Functions/Classes:**
  * `spawn` (Impact: 17.1)
  * `interrupt_handler_c` (Impact: 16.9)
...
**Top Internal Functions/Classes:**
  * `__assert_func` (Impact: 12.2)
  * `set_isr` (Impact: 10.9)
  * `limit` (Impact: 10.2)
  * `task_create_common` (Impact: 9.9)
  * `queue_send` (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.002949
...
**Top Internal Functions/Classes:**
  * `check_regular_simultaneous` (Impact: 23.3)
  * `check_injected_simultaneous` (Impact: 23.3)
  * `set_regular_seq` (Impact: 17.4)
  * `set_regular_discontinuous` (Impact: 17.1)
  * `set_injected_discontinuous` (Impact: 15.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Encoder` (Impact: 155.2)
  * `encode_instruction` (Impact: 52.3)
  * `encode_instruction_body` (Impact: 38.7)
  * `calc_delay_side_set` (Impact: 27.1)
  * `evaluate_impl` (Impact: 22.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 12 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.006422
...
**Top Internal Functions/Classes:**
  * `PioImpl` (Impact: 116.2)
  * `sm_set_pin_mappings` (Impact: 34.5)
  * `sm_load_and_start_program` (Impact: 31.4)
  * `find_offset_for_program` (Impact: 14.5)
  * `add_program_at_offset_unlocked` (Impact: 8.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.001747
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003102 | `Ripple Effect (Closeness):` 0.147427
...
**Top Internal Functions/Classes:**
  * `init` (Impact: 35.2)
  * `join` (Impact: 27.9)
  * `handle_event` (Impact: 20.7)
  * `response_poll` (Impact: 17.6)
  * `log_read` (Impact: 13.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.6e-05 | `Ripple Effect (Closeness):` 0.012531
...
**Top Internal Functions/Classes:**
  * `Uart` (Impact: 76.4)
  * `I2CController` (Impact: 72.4)
  * `init` (Impact: 22.0)
  * `is_valid_pin` (Impact: 13.2)
    * *Intent:* /// Checks if a pin is valid for a given uart index and direction
  * `is_valid_pin` (Impact: 12.9)
...
**Top Internal Functions/Classes:**
  ...
  * `write_assume_capacity` (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
**Top Internal Functions/Classes:**
  * `configure_output` (Impact: 23.4)
  * `configure_input` (Impact: 10.9)
  * `configure_channel` (Impact: 6.8)
    * *Intent:* ///This function configures the output channel for PWM mode.
  * `set_channel_interrupt` (Impact: 6.4)
  * `set_channel_dma_request` (Impact: 6.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.002621
...
**Top Internal Functions/Classes:**
  * `setup_read` (Impact: 43.0)
  * `writev_operation_blocking` (Impact: 26.5)
  * `setup_write` (Impact: 23.2)
  * `write_operation_blocking` (Impact: 20.3)
  * `set_filter` (Impact: 18.4)
...
**Top Internal Functions/Classes:**
  * `c_on_recv` (Impact: 24.2)
  * `poll` (Impact: 17.9)
    * *Intent:* /// Poll underlying link layer for data packet.
  * `init` (Impact: 13.8)
  * `c_netif_linkoutput` (Impact: 13.6)
...
**Top Internal Functions/Classes:**
  * `read` (Impact: 15.3)
  * `set_direction` (Impact: 13.3)
  * `put` (Impact: 13.3)
    * *Intent:* /// Drive a single GPIO high/low
  * `set_irq_enabled` (Impact: 11.2)
...
**Top Internal Functions/Classes:**
  ...
  * `handle_event` (Impact: 26.5)
    * *Intent:* /// Handle event from chip
  * `pack_scan_params` (Impact: 19.9)
  * `parse_escan_partial` (Impact: 9.4)
...
- `core/src/core/usb.zig` -> **Severity: 0.3** (Bridge: 0.0031 * Flux: 96.6856%)
- `tools/regz/src/Database.zig` -> **Severity: 0.017** (Bridge: 0.0005 * Flux: 32.4145%)
- `tools/regz/src/gen.zig` -> **Severity: 0.005** (Bridge: 0.0002 * Flux: 21.6102%)
- `drivers/wireless/cyw43439.zig` -> **Severity: 0.003** (Bridge: 0.0001 * Flux: 25.3257%)
- `tools/uf2/src/uf2.zig` -> **Severity: 0.003** (Bridge: 0.0 * Flux: 82.1981%)
