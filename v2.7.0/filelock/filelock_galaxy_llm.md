...
| Avg Path Length | 2.0667 | Hops between files. Lower = Tighter coupling. |
...
- `acquire` (@ `filelock-3.25.2/src/filelock/_api.py`) -> Impact: **52.9** | LOC: 106
- `__call__` (@ `filelock-3.25.2/src/filelock/_api.py`) -> Impact: **36.4** | LOC: 64
- `acquire` (@ `filelock-3.25.2/src/filelock/asyncio.py`) -> Impact: **23.6** | LOC: 80
- `open_permission_then_unlink` (@ `filelock-3.25.2/tests/test_filelock.py`) -> Impact: **22.8** | LOC: 9
- `_acquire` (@ `filelock-3.25.2/src/filelock/_read_write.py`) -> Impact: **21.7** | LOC: 32
- `timeout_for_sqlite` (@ `filelock-3.25.2/src/filelock/_read_write.py`) -> Impact: **20.9** | LOC: 17
- `test_sticky_bit_fallback_handles_concurrent_unlink` (@ `filelock-3.25.2/tests/test_filelock.py`) -> Impact: **18.5** | LOC: 23
- `test_write_non_starvation` (@ `filelock-3.25.2/tests/test_read_write.py`) -> Impact: **18.3** | LOC: 54
...
- `_acquire` (@ `filelock-3.25.2/src/filelock/_unix.py`) -> Impact: **18.0** | LOC: 48
- `acquire_lock` (@ `filelock-3.25.2/tests/test_read_write.py`) -> Impact: **18.0** | LOC: 20
...
- **Heaviest Functions:** `acquire` (Impact: 23.6), `__init__` (Impact: 13.8), `__call__` (Impact: 12.4)
...
- **Heaviest Functions:** `_acquire` (Impact: 18.0), `_fallback_to_soft_lock` (Impact: 4.6), `_release` (Impact: 1.8)
...
- **Heaviest Functions:** `_acquire` (Impact: 21.7), `timeout_for_sqlite` (Impact: 20.9), `__call__` (Impact: 16.0)
...
- **Heaviest Functions:** `acquire` (Impact: 52.9), `__call__` (Impact: 36.4), `_check_give_up` (Impact: 16.8)
...
- **Heaviest Functions:** `read_lock` (Impact: 7.0), `write_lock` (Impact: 7.0), `_run` (Impact: 4.6)
...
- **Heaviest Functions:** `_acquire` (Impact: 12.5), `_is_process_alive` (Impact: 8.0), `_windows_unlink_with_retry` (Impact: 7.8)
...
- **Heaviest Functions:** `_acquire` (Impact: 9.9), `_is_reparse_point` (Impact: 6.8), `_release` (Impact: 1.8)
...
- **Heaviest Functions:** `test_double_check_wrong_mode_raises` (Impact: 15.9), `test_sequential_mode_switch` (Impact: 10.4), `test_operational_error_handling` (Impact: 8.0)
...
- **Heaviest Functions:** `test_lock_mtime_updated_on_acquire` (Impact: 3.8), `test_lifetime_singleton_mismatch` (Impact: 3.2), `test_expired_lock_race_rename_fails` (Impact: 2.2)
...
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), Stability (50.0%)
- **Heaviest Functions:** `test_non_blocking` (Impact: 17.3), `test_simple` (Impact: 8.6), `test_acquire` (Impact: 8.6)
...
**Top Internal Functions/Classes:**
  * `open_permission_then_unlink` (Impact: 22.8)
  * `test_sticky_bit_fallback_handles_concurrent_unlink` (Impact: 18.5)
  * `test_non_blocking` (Impact: 17.2)
    * *Intent:* # raises Timeout error when the lock cannot be acquired lock_path = tmp_path / "a" lock_1, lock_2 = ...
  * `test_threaded_lock_different_lock_obj` (Impact: 17.1)
  * `open_enoent_then_succeed` (Impact: 16.0)
...
**Top Internal Functions/Classes:**
  * `_clear_singleton_cache` (Impact: 3.4)
  * `test_close` (Impact: 3.2)
  * `test_upgrade_prohibited` (Impact: 3.1)
  * `test_downgrade_prohibited` (Impact: 3.1)
  * `test_close_on_unheld_lock` (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_double_check_wrong_mode_raises` (Impact: 15.9)
  * `test_sequential_mode_switch` (Impact: 10.4)
  * `test_operational_error_handling` (Impact: 8.0)
  * `test_busy_timeout_recomputed_after_journal_mode` (Impact: 6.5)
  * `test_reentrant_lock` (Impact: 5.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_non_blocking` (Impact: 17.3)
    * *Intent:* # raises Timeout error when the lock cannot be acquired lock_path = tmp_path / "a" lock_1, lock_2 = ...
  * `test_simple` (Impact: 8.6)
  * `test_acquire` (Impact: 8.6)
  * `test_attempting_to_release` (Impact: 6.6)
  * `test_wait_message_logged` (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_acquire` (Impact: 21.7)
  * `timeout_for_sqlite` (Impact: 20.9)
  * `__call__` (Impact: 16.0)
  * `release` (Impact: 13.5)
    * *Intent:* """ Release one level of the current lock. When the lock level reaches zero the underlying SQLite tr...
  * `_configure_and_begin` (Impact: 13.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 6 instances
* *Amplified Cascading Flux:* 39 instances
...
**Top Internal Functions/Classes:**
  * `acquire` (Impact: 52.9)
  * `__call__` (Impact: 36.4)
  * `_check_give_up` (Impact: 16.8)
...
**Top Internal Functions/Classes:**
  * `acquire` (Impact: 23.6)
  * `__init__` (Impact: 13.8)
  * `__call__` (Impact: 12.4)
  * `_run_internal_method` (Impact: 9.1)
  * `release` (Impact: 7.8)
    * *Intent:* """ Release the file lock. The lock is only completely released when the lock counter reaches 0. The...
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_lock_mtime_updated_on_acquire` (Impact: 3.8)
  * `test_lifetime_singleton_mismatch` (Impact: 3.2)
  * `test_expired_lock_race_rename_fails` (Impact: 2.2)
  * `test_expired_lock_becomes_acquirable` (Impact: 2.2)
  * `test_expired_lock_is_broken` (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 6 instances
* *Amplified Cascading Flux:* 3 instances
...
**Top Internal Functions/Classes:**
  * `_acquire` (Impact: 12.5)
  * `_is_process_alive` (Impact: 8.0)
  * `_windows_unlink_with_retry` (Impact: 7.8)
  * `_try_break_stale_lock` (Impact: 6.4)
  * `_release` (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_symlink_same_canonical_path` (Impact: 4.0)
  * `test_same_thread_different_instances_raises` (Impact: 3.8)
  * `test_different_threads_no_false_positive` (Impact: 2.8)
  * `test_force_release_cleans_registry` (Impact: 2.3)
  * `test_cleanup_on_release` (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
**Top Internal Functions/Classes:**
  * `test_singleton_default_vs_explicit_mode_differ` (Impact: 3.1)
  * `test_default_mode_respects_umask` (Impact: 2.5)
  * `test_default_mode_skips_fchmod` (Impact: 2.2)
  * `test_explicit_mode_calls_fchmod` (Impact: 2.2)
  * `test_explicit_mode_overrides_umask` (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_acquire` (Impact: 18.0)
  * `_fallback_to_soft_lock` (Impact: 4.6)
  * `_release` (Impact: 1.8)
  * `_acquire` (Impact: 1.5)
  * `_release` (Impact: 1.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_stale_lock_rename_race` (Impact: 2.3)
  * `test_stale_lock_broken_when_process_dead` (Impact: 2.2)
  * `test_stale_lock_not_broken_when_eperm` (Impact: 2.2)
  * `test_stale_lock_unexpected_kill_error_suppressed` (Impact: 2.2)
  * `test_stale_detection_errors_suppressed` (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `test_fallback_emits_warning` (Impact: 3.8)
  * `_close_eio` (Impact: 3.0)
  * `test_fallback_subsequent_acquire_skips_flock` (Impact: 2.3)
  * `test_fallback_release_unlinks_file` (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 1.9)
  * `lock_file` (Impact: 1.6)
    * *Intent:* """:returns: The path of the file lock."""
  * `__reduce__` (Impact: 1.5)
  * `__str__` (Impact: 1.5)
  * `__repr__` (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_timeout_pickle` (Impact: 1.4)
  * `test_timeout_str` (Impact: 1.1)
  * `test_timeout_repr` (Impact: 1.1)
  * `test_timeout_lock_file` (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `pytest_sessionfinish` (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
