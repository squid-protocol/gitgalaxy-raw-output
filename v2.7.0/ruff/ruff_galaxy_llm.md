...
| Avg Path Length | 2.0249 | Hops between files. Lower = Tighter coupling. |
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 6.3 | 4.7 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.9 | 22.7 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 36.3 | 25.5 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 17.9 | 2.4 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 10.3 | 2.5 | 0.0 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 77.4 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 45.5 | 44.4 | 0.0 |
...
| guards | 9043 | 1318 | 10 | `ruff-0.15.9/crates/ruff_python_codegen/src/generator.rs` |
| danger | 2213 | 406 | 2 | `ruff-0.15.9/crates/ty_module_resolver/src/resolve.rs` |
| concurrency | 584 | 183 | 0 | `ruff-0.15.9/crates/ruff/src/cache.rs` |
| connectivity | 10506 | 1494 | 13 | `ruff-0.15.9/crates/ruff_python_ast/src/nodes.rs` |
| io | 223 | 62 | 0 | `ruff-0.15.9/crates/ruff_db/src/system/os.rs` |
| crypto | 0 | 0 | 0 | - |
...
| debt | 846 | 211 | 1 | `ruff-0.15.9/crates/ruff_python_ast/src/nodes.rs` |
| mutation | 22554 | 1287 | 26 | `ruff-0.15.9/crates/ruff_workspace/src/options.rs` |
| dead_code | 5441 | 1166 | 5 | `ruff-0.15.9/crates/ruff_formatter/src/builders.rs` |
...
- `format_line` (@ `ruff-0.15.9/crates/ruff_annotate_snippets/src/renderer/display_list.rs`) -> Impact: **294.1** | LOC: 481
...
- `visit_expr` (@ `ruff-0.15.9/crates/ruff_linter/src/checkers/ast/mod.rs`) -> Impact: **219.1** | LOC: 607
- `fmt` (@ `ruff-0.15.9/crates/ruff_python_formatter/src/statement/suite.rs`) -> Impact: **211.5** | LOC: 350
- `visit_stmt` (@ `ruff-0.15.9/crates/ruff_linter/src/checkers/ast/mod.rs`) -> Impact: **195.9** | LOC: 697
- `check_noqa` (@ `ruff-0.15.9/crates/ruff_linter/src/checkers/noqa.rs`) -> Impact: **195.7** | LOC: 254
  * *Intent:* /// RUF100
- `definition` (@ `ruff-0.15.9/crates/ruff_linter/src/rules/flake8_annotations/rules/definition.rs`) -> Impact: **183.3** | LOC: 346
  * *Intent:* /// Generate flake8-annotation checks for a given `Definition`. /// ANN001, ANN401
- `fmt` (@ `ruff-0.15.9/crates/ruff_formatter/src/format_element/document.rs`) -> Impact: **172.4** | LOC: 400
- `path_exists_case_sensitive_fast` (@ `ruff-0.15.9/crates/ruff_db/src/system/os.rs`) -> Impact: **170.2** | LOC: 667
...
- **Unknown Dependencies:** `18168` packages imported that bypass the Zero-Trust whitelist.

## 11. CUMULATIVE RISK HITLIST (Top 10 Highest Risk Files)
> Cumulative Risk is the sum of all individual risk exposures. These files represent the highest multi-dimensional technical debt and architectural fragility.

### 1. `ruff-0.15.9/crates/ruff_linter/src/directives.rs` (RUST) -> Cumulative Risk: **641.25**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 286.08 | **LOC:** 731 | **CtrlFlow:** 8.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (98.4576%), Tech Debt (97.9651%), Verification (80.0%)
- **Heaviest Functions:** `extract_noqa_line_for` (Impact: 45.1), `extract_isort_directives` (Impact: 32.0), `from_comment` (Impact: 14.1)

### 2. `ruff-0.15.9/crates/ruff_linter/src/registry/rule_set.rs` (RUST) -> Cumulative Risk: **619.4**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 152.34 | **LOC:** 421 | **CtrlFlow:** 13.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.7473%), Tech Debt (97.8253%), Verification (80.0%)
- **Heaviest Functions:** `fmt` (Impact: 14.5), `next` (Impact: 6.6), `set` (Impact: 6.4)

### 3. `ruff-0.15.9/crates/ruff_python_parser/resources/inline/err/invalid_annotation_function.py` (PYTHON) -> Cumulative Risk: **613.28**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 36.0 | **LOC:** 21 | **CtrlFlow:** 0.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (98.7872%)
- **Heaviest Functions:** `e[T]` (Impact: 1.5), `g[T]` (Impact: 1.5), `h[T]` (Impact: 1.5)

### 4. `ruff-0.15.9/crates/ruff_linter/src/rules/pylint/rules/too_many_return_statements.rs` (RUST) -> Cumulative Risk: **601.21**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 77.16 | **LOC:** 259 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (97.0997%), Verification (80.0%), Documentation (78.5714%)
- **Heaviest Functions:** `if_` (Impact: 12.3), `too_many_return_statements` (Impact: 5.3), `nested_def_ignored` (Impact: 4.8)

### 5. `ruff-0.15.9/crates/ruff_db/src/system/path.rs` (RUST) -> Cumulative Risk: **595.47**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 215.5 | **LOC:** 932 | **CtrlFlow:** 10.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.8482%), Dead Code (82.3997%), Verification (80.0%)
- **Heaviest Functions:** `absolute` (Impact: 10.5), `absolute` (Impact: 10.3), `next` (Impact: 7.7)

### 6. `ruff-0.15.9/crates/ruff_linter/src/rules/pylint/rules/too_many_statements.rs` (RUST) -> Cumulative Risk: **593.43**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 188.22 | **LOC:** 469 | **CtrlFlow:** 13.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (98.7282%), Documentation (91.6667%), Tech Debt (91.3673%)
- **Heaviest Functions:** `num_statements` (Impact: 16.3), `many_statements` (Impact: 8.4), `if_else_if_corner` (Impact: 5.7)

### 7. `ruff-0.15.9/crates/ruff_python_literal/src/escape.rs` (RUST) -> Cumulative Risk: **588.02**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 212.56 | **LOC:** 418 | **CtrlFlow:** 14.0% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (98.7715%), Documentation (96.2963%), Verification (80.0%)
- **Heaviest Functions:** `write_char` (Impact: 15.7), `output_layout_with_checker` (Impact: 13.7), `output_layout_with_checker` (Impact: 12.4)

### 8. `ruff-0.15.9/crates/ruff_linter/src/rules/pycodestyle/rules/blank_lines.rs` (RUST) -> Cumulative Risk: **587.14**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 421.66 | **LOC:** 1123 | **CtrlFlow:** 18.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (94.9286%), Tech Debt (94.0757%), Documentation (81.8182%)
- **Heaviest Functions:** `check_line` (Impact: 164.6), `next` (Impact: 37.2), `check_lines` (Impact: 32.7)

### 9. `ruff-0.15.9/crates/ruff_python_trivia/src/comment_ranges.rs` (RUST) -> Cumulative Risk: **584.43**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 145.46 | **LOC:** 221 | **CtrlFlow:** 21.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (86.6451%), Verification (80.0%)
- **Heaviest Functions:** `block_comments` (Impact: 38.9), `has_comments` (Impact: 10.8), `intersects` (Impact: 9.3)

### 10. `ruff-0.15.9/crates/ruff_linter/src/message/grouped.rs` (RUST) -> Cumulative Risk: **581.98**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 112.84 | **LOC:** 269 | **CtrlFlow:** 10.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.0311%), Verification (80.0%)
- **Heaviest Functions:** `emit` (Impact: 18.0), `fmt` (Impact: 15.6), `fmt` (Impact: 12.7)
...
- **Risk Profile:** Cognitive Load (13.0706%), Tech Debt (9.0862%)
**Top Internal Functions/Classes:**
  * `visit_expr` (Impact: 219.1)
  * `visit_stmt` (Impact: 195.9)
  * `handle_node_store` (Impact: 45.8)
  * `add_binding` (Impact: 38.7)
    * *Intent:* /// Add a [`Binding`] to the current scope, bound to the given name.
  * `report_semantic_error` (Impact: 31.7)
...
- **Risk Profile:** Cognitive Load (4.15%), Tech Debt (99.7454%)
**Top Internal Functions/Classes:**
  * `prefix` (Impact: 20.8)
  * `prefix` (Impact: 10.8)
  * `next` (Impact: 9.3)
...
- **Risk Profile:** Cognitive Load (10.9101%), Tech Debt (8.0284%)
**Top Internal Functions/Classes:**
  * `defined_in_class` (Impact: 13.2)
  * `undefined_in_gen_exp_nested` (Impact: 9.7)
  * `f401_preview_refined_submodule_handling` (Impact: 8.2)
  * `defined_in_gen_exp` (Impact: 7.4)
  * `f401_preview_local_init_import` (Impact: 6.5)
...
- **Risk Profile:** Cognitive Load (19.288%), Tech Debt (11.4296%)
**Top Internal Functions/Classes:**
  * `format_line` (Impact: 294.1)
    * *Intent:* // Adapted from https://github.com/rust-lang/rust/blob/d371d17496f2ce3a56da76aa083f4ef157572c20/comp...
  * `format_body` (Impact: 126.6)
  * `format_annotation` (Impact: 55.6)
  * `format_header` (Impact: 35.2)
  * `format_raw_line` (Impact: 29.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Risk Profile:** Cognitive Load (6.4145%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `output_format` (Impact: 130.6)
    * *Intent:* #[test_case::test_case("concise")] #[test_case::test_case("full")] #[test_case::test_case("json")] #...
  * `create_a005_module_structure` (Impact: 16.4)
    * *Intent:* /// │ └── __init__.py /// ├── foobar /// │ ├── __init__.py /// │ ├── abc /// │ │ └── __init__.py ///...
  * `value_given_to_table_key_is_not_inline_table_1` (Impact: 12.8)
  * `exclude` (Impact: 11.4)
  * `match_before_py310` (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (21.0679%), Tech Debt (23.8662%)
**Top Internal Functions/Classes:**
  * `as_rule_table` (Impact: 116.8)
  * `warn_about_deprecated_top_level_lint_options` (Impact: 95.8)
  * `into_settings` (Impact: 76.2)
  * `from_options` (Impact: 26.1)
    * *Intent:* /// Convert the [`Options`] read from the given [`Path`] into a [`Configuration`]. /// If `None` is ...
  * `conflicting_import_settings` (Impact: 12.2)
...
- **Risk Profile:** Cognitive Load (8.0677%), Tech Debt (16.6533%)
**Top Internal Functions/Classes:**
  * `try_into_settings` (Impact: 53.8)
  * `try_into_settings` (Impact: 14.1)
  * `try_into_settings` (Impact: 9.7)
  * `deserialize` (Impact: 8.2)
  * `deserialize` (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (6.1244%), Tech Debt (11.63%)
**Top Internal Functions/Classes:**
  * `find_noqa_comments` (Impact: 54.5)
  * `extract` (Impact: 26.0)
    * *Intent:* /// Extract the [`FileNoqaDirectives`] for a given Python source file, enumerating any rules /// tha...
  * `lex_file_exemption` (Impact: 22.7)
  * `lex_code` (Impact: 18.4)
  * `build_noqa_edits_by_line` (Impact: 15.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Risk Profile:** Cognitive Load (13.7697%), Tech Debt (12.3526%)
**Top Internal Functions/Classes:**
  * `unparse_stmt` (Impact: 130.3)
  * `unparse_expr` (Impact: 110.7)
  * `unparse` (Impact: 39.1)
  * `quote` (Impact: 25.0)
  * `unparse_pattern` (Impact: 23.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (5.7007%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `any_over_stmt` (Impact: 73.4)
  * `from_expr` (Impact: 69.3)
    * *Intent:* /// Return the truthiness of an expression.
  * `any_over_expr` (Impact: 45.0)
    * *Intent:* /// Call `func` over every `Expr` in `expr`, returning `true` if any expression /// returns `true`..
  * `is_unpacking_assignment` (Impact: 25.4)
...
- **Risk Profile:** Cognitive Load (45.3273%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 34.6)
  * `write_source_order` (Impact: 30.3)
    * *Intent:* # ------------------------------------------------------------------------------ # Source order visi...
  * `write_node` (Impact: 27.9)
    * *Intent:* # ------------------------------------------------------------------------------ # Node structs
  * `fields_in_source_order` (Impact: 10.7)
  * `write_owned_enum` (Impact: 10.6)
...
- **Risk Profile:** Cognitive Load (8.6814%), Tech Debt (8.8779%)
**Top Internal Functions/Classes:**
  * `check_stmt` (Impact: 77.8)
  * `visit_pattern` (Impact: 42.3)
  * `check_annotation` (Impact: 38.2)
...
- **Risk Profile:** Cognitive Load (20.4057%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `format` (Impact: 166.7)
...
- **Risk Profile:** Cognitive Load (10.0566%), Tech Debt (11.1675%)
**Top Internal Functions/Classes:**
  * `format` (Impact: 99.2)
    * *Intent:* /// Format a set of files, and return the exit status.
  * `format_source` (Impact: 61.8)
    * *Intent:* /// Format a [`SourceKind`], returning the transformed [`SourceKind`], or `None` if the source was /...
  * `warn_incompatible_formatter_settings` (Impact: 56.6)
  * `format_path` (Impact: 48.8)
...
- **Risk Profile:** Cognitive Load (12.237%), Tech Debt (23.6584%)
**Top Internal Functions/Classes:**
  * `normalize_string` (Impact: 63.3)
  * `preferred_quote_style` (Impact: 47.2)
    * *Intent:* /// Determines the preferred quote style for `string`. /// The formatter should use the preferred qu...
  * `normalize` (Impact: 40.9)
...
- **Risk Profile:** Cognitive Load (12.8806%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `statement` (Impact: 492.6)
    * *Intent:* /// Run lint rules over a [`Stmt`] syntax node.
  * `bad_version_info_comparison` (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
- `ruff-0.15.9/crates/ruff_db/src/system/os.rs` -> **Severity: 224.166** (Blast Radius: 2.484 * Doc Risk: 90.2439%)
