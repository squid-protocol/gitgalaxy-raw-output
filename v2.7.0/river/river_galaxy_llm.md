...
- `handleRequest` (@ `river/LibinputDevice.zig`) -> Impact: **198.5** | LOC: 330
- `handleRequest` (@ `river/Window.zig`) -> Impact: **76.3** | LOC: 167
- `createObject` (@ `river/LibinputDevice.zig`) -> Impact: **63.7** | LOC: 130
- `manageStart` (@ `river/Window.zig`) -> Impact: **55.5** | LOC: 149
  * *Intent:* /// Send dirty state as part of a manage sequence.
- `commitOutputState` (@ `river/OutputManager.zig`) -> Impact: **51.2** | LOC: 147
- `manageStart` (@ `river/Seat.zig`) -> Impact: **45.5** | LOC: 147
- `Deque` (@ `common/deque.zig`) -> Impact: **45.0** | LOC: 250
  * *Intent:* /// A contiguous, growable, double-ended queue. /// /// Pushing/popping items from either end of the queue is O(1).
- `handleKey` (@ `river/KeyboardGroup.zig`) -> Impact: **44.9** | LOC: 101
- `processButton` (@ `river/Cursor.zig`) -> Impact: **36.4** | LOC: 104
- `main` (@ `river/main.zig`) -> Impact: **35.3** | LOC: 126
...
- **Heaviest Functions:** `renderAndCommit` (Impact: 27.2), `manageStart` (Impact: 22.6), `create` (Impact: 13.1)
...
- **Heaviest Functions:** `configure` (Impact: 15.2), `handleSetSizeHints` (Impact: 9.4), `handleSetOverrideRedirect` (Impact: 5.3)
...
- **Heaviest Functions:** `configure` (Impact: 24.4), `handleCommit` (Impact: 19.0), `needsConfigure` (Impact: 16.5)
...
- **Heaviest Functions:** `handleRequest` (Impact: 76.3), `manageStart` (Impact: 55.5), `manageFinish` (Impact: 24.1)
...
- **Heaviest Functions:** `manageStart` (Impact: 45.5), `handleRequest` (Impact: 23.1), `focus` (Impact: 23.1)
...
- **Heaviest Functions:** `handleKey` (Impact: 44.9), `processKey` (Impact: 15.5), `match` (Impact: 13.6)
...
- **Heaviest Functions:** `create` (Impact: 20.3), `processKey` (Impact: 9.3), `setGroup` (Impact: 8.1)
...
- **Heaviest Functions:** `match` (Impact: 27.4), `handleRequest` (Impact: 11.1), `handleDestroy` (Impact: 7.6)
...
- **Heaviest Functions:** `init` (Impact: 34.3), `handleRequest` (Impact: 34.2), `deinit` (Impact: 8.4)
...
- **Heaviest Functions:** `renderFinish` (Impact: 29.3), `manageStart` (Impact: 16.5), `manageFinish` (Impact: 11.7)
...
**Top Internal Functions/Classes:**
  * `handleRequest` (Impact: 76.3)
  * `manageStart` (Impact: 55.5)
    * *Intent:* /// Send dirty state as part of a manage sequence.
  * `manageFinish` (Impact: 24.1)
    * *Intent:* /// Applies window management state from the window manager and sends a configure /// to the window ...
  * `renderStart` (Impact: 17.7)
  * `applySurfaceClip` (Impact: 17.4)
...
**Top Internal Functions/Classes:**
  * `manageStart` (Impact: 45.5)
  * `handleRequest` (Impact: 23.1)
  * `focus` (Impact: 23.1)
  * `manageFinish` (Impact: 22.9)
  * `matchXkbBinding` (Impact: 22.7)
...
**Top Internal Functions/Classes:**
  * `processButton` (Impact: 36.4)
  * `processMotionRelative` (Impact: 16.8)
  * `handleRequestSetCursor` (Impact: 15.7)
  * `updateHovered` (Impact: 14.6)
  * `setTheme` (Impact: 13.6)
...
**Top Internal Functions/Classes:**
  * `handleRequest` (Impact: 198.5)
  * `createObject` (Impact: 63.7)
  * `send` (Impact: 5.6)
  * `handleRequestInert` (Impact: 4.3)
  * `init` (Impact: 4.1)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `renderFinish` (Impact: 29.3)
    * *Intent:* /// Finish the update sequence and drop stashed buffers. This means that /// the next frame drawn wi...
  * `manageStart` (Impact: 16.5)
  * `manageFinish` (Impact: 11.7)
  * `handleRequest` (Impact: 10.8)
  * `handleDestroy` (Impact: 9.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `renderAndCommit` (Impact: 27.2)
  * `manageStart` (Impact: 22.6)
  * `create` (Impact: 13.1)
  * `handleDestroy` (Impact: 10.6)
  * `handlePresent` (Impact: 10.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `commitOutputState` (Impact: 51.2)
  * `handleManagerApply` (Impact: 15.6)
  * `validateConfigCoordinates` (Impact: 15.6)
  * `handlePowerManagerSetMode` (Impact: 15.1)
  * `autoLayout` (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 17 instances
...
**Top Internal Functions/Classes:**
  ...
  * `needsConfigure` (Impact: 16.5)
  * `handleRequestFullscreen` (Impact: 7.8)
  * `handleAckConfigure` (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 20 instances
...
**Top Internal Functions/Classes:**
  * `handleKey` (Impact: 44.9)
  * `processKey` (Impact: 15.5)
  * `match` (Impact: 13.6)
  * `unref` (Impact: 10.5)
  * `handleBuiltinBinding` (Impact: 7.8)
...
**Top Internal Functions/Classes:**
  * `allowlist` (Impact: 16.3)
    * *Intent:* /// Returns true if the global is allowlisted for security contexts
  * `init` (Impact: 15.9)
  * `globalFilter` (Impact: 15.2)
  * `handleRequestSetCursorShape` (Impact: 13.8)
  * `start` (Impact: 7.7)
...
**Top Internal Functions/Classes:**
  * `init` (Impact: 34.3)
  * `handleRequest` (Impact: 34.2)
  * `deinit` (Impact: 8.4)
  * `handleRemove` (Impact: 6.4)
  * `createObject` (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
**Top Internal Functions/Classes:**
  * `configure` (Impact: 15.2)
    * *Intent:* /// Always returns false as we do not care about frame perfection for Xwayland windows.
  * `handleSetSizeHints` (Impact: 9.4)
  * `handleSetOverrideRedirect` (Impact: 5.3)
  * `handleSetDecorations` (Impact: 4.7)
  * `handleRequestMaximize` (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 9 instances
...
**Top Internal Functions/Classes:**
  * `main` (Impact: 35.3)
  * `logFn` (Impact: 11.9)
  * `defaultInitPath` (Impact: 9.2)
  * `grepRiverctl` (Impact: 8.0)
  * `detectClassic` (Impact: 7.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `sendState` (Impact: 23.8)
  * `createObject` (Impact: 19.2)
  * `handleRequest` (Impact: 14.9)
  * `handleRequestInert` (Impact: 4.3)
  * `init` (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `handleRequest` (Impact: 11.1)
  * `handleDestroy` (Impact: 7.6)
  * `create` (Impact: 4.4)
  * `handleRequestInert` (Impact: 4.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
**Top Internal Functions/Classes:**
  * `maybeLock` (Impact: 13.0)
  * `handleLock` (Impact: 12.6)
  * `lockSurfaceFromOutput` (Impact: 7.6)
  * `handleUnlock` (Impact: 4.0)
  * `handleDestroy` (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 7 instances
...
**Top Internal Functions/Classes:**
  * `handleRequest` (Impact: 18.1)
  * `bind` (Impact: 7.6)
  * `init` (Impact: 6.1)
  * `handleNewVirtualPointer` (Impact: 6.0)
  * `handleRequestInert` (Impact: 4.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `handleNewSurface` (Impact: 17.9)
  * `checkExclusiveFocus` (Impact: 14.3)
  * `handleRequest` (Impact: 13.7)
  * `supported` (Impact: 6.1)
  * `bind` (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
**Top Internal Functions/Classes:**
  ...
  * `handleRequest` (Impact: 8.9)
  * `create` (Impact: 6.6)
  * `clientCommit` (Impact: 6.1)
  * `handleRequestInert` (Impact: 4.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 7 instances
...
**Top Internal Functions/Classes:**
  * `focus` (Impact: 15.5)
  * `handleInputMethodCommit` (Impact: 10.0)
  * `sendInputMethodState` (Impact: 8.7)
  * `newInputMethod` (Impact: 6.3)
  * `disableTextInput` (Impact: 4.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
**Top Internal Functions/Classes:**
  * `sendConfigures` (Impact: 16.4)
  * `manageStart` (Impact: 7.0)
  * `handleRequest` (Impact: 6.8)
  * `handleRequestInert` (Impact: 4.3)
  * `arrange` (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
