...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 19.5 | 6.6 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 68.5 | 4.7 | 0.0 | 0.0 |
...
| dead_code | 2026 | 1198 | 1 | `packages/core/schematics/utils/template_ast_visitor.ts` |
...
- `getInlayHintsForTemplate` (@ `packages/language-service/src/inlay_hints.ts`) -> Impact: **829.3** | LOC: 1738
  * *Intent:* /** * Get Angular-specific inlay hints for a template. */
- `patchEventTarget` (@ `packages/zone.js/lib/common/events.ts`) -> Impact: **413.4** | LOC: 710
- `assertValueSpan` (@ `packages/compiler/test/render3/r3_ast_spans_spec.ts`) -> Impact: **403.9** | LOC: 638
- `patchEventTargetMethods` (@ `packages/zone.js/lib/common/events.ts`) -> Impact: **265.2** | LOC: 593
- `_scopeLocalKeyframeDeclarations` (@ `packages/compiler/src/shadow_css.ts`) -> Impact: **260.8** | LOC: 775
...
- `reifyCreateOperations` (@ `packages/compiler/src/template/pipeline/src/phases/reify.ts`) -> Impact: **234.1** | LOC: 525
- `initZone` (@ `packages/zone.js/lib/zone-impl.ts`) -> Impact: **222.1** | LOC: 841
- `analyze` (@ `packages/compiler-cli/src/ngtsc/annotations/component/src/handler.ts`) -> Impact: **206.7** | LOC: 565
- `extractDirectiveMetadata` (@ `packages/compiler-cli/src/ngtsc/annotations/directive/src/shared.ts`) -> Impact: **195.8** | LOC: 352
...
- `patchPromise` (@ `packages/zone.js/lib/common/promise.ts`) -> Impact: **187.1** | LOC: 631
...
- **Heaviest Functions:** `_convertPerfRecordsToEvents` (Impact: 33.1), `createEvent` (Impact: 5.5), `timeEnd` (Impact: 3.8)

### 2. `packages/compiler/src/template/pipeline/ir/src/expression.ts` (TYPESCRIPT) -> Cumulative Risk: **722.42**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `transformExpressionsInOp` (Impact: 174.0), `transformExpressionsInExpression` (Impact: 114.7), `transformExpressionsInStatement` (Impact: 25.2)

### 3. `packages/zone.js/lib/jasmine/jasmine.ts` (TYPESCRIPT) -> Cumulative Risk: **718.2**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `patchJasmine` (Impact: 97.5), `ZoneQueueRunner` (Impact: 27.2), `runInTestZone` (Impact: 19.0)
...
- **Heaviest Functions:** `_convertEvent` (Impact: 67.5), `normalizeEvent` (Impact: 24.4), `normalizeGCEvent` (Impact: 11.6)

### 5. `vscode-ng-language-service/client/src/client.ts` (TYPESCRIPT) -> Cumulative Risk: **691.59**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `constructor` (Impact: 56.4), `provideCompletionItem` (Impact: 24.0), `constructArgs` (Impact: 22.1)

### 6. `packages/common/src/pipes/async_pipe.ts` (TYPESCRIPT) -> Cumulative Risk: **687.21**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `createSubscription` (Impact: 8.0), `_updateLatestValue` (Impact: 7.3), `transform` (Impact: 6.8)

### 7. `adev/src/app/features/home/components/hydration-example/hydration-example.ts` (TYPESCRIPT) -> Cumulative Risk: **684.97**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 161.06 | **LOC:** 243 | **CtrlFlow:** 12.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `handleCardClick` (Impact: 12.7), `getCardSignal` (Impact: 10.6), `startLifecycle` (Impact: 10.1)
...
- **Heaviest Functions:** `patchWtf` (Impact: 50.0), `shallowObj` (Impact: 21.8), `onInvoke` (Impact: 15.2)

### 9. `devtools/projects/ng-devtools-backend/src/lib/hooks/profiler/native.ts` (TYPESCRIPT) -> Cumulative Risk: **676.89**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `ProfilerEvent.TemplateUpdateStart]` (Impact: 12.0), `ProfilerEvent.TemplateUpdateEnd]` (Impact: 7.7), `constructor` (Impact: 3.4)

### 10. `packages/compiler-cli/src/perform_watch.ts` (TYPESCRIPT) -> Cumulative Risk: **673.09**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `performWatchCompilation` (Impact: 51.3), `createPerformWatchHost` (Impact: 36.3), `doCompilation` (Impact: 20.6)
...
**Top Internal Functions/Classes:**
  * `verifyFormState` (Impact: 6.4)
  * `getValues` (Impact: 5.3)
  * `verifyFormState` (Impact: 5.1)
  * `onNgModelChange` (Impact: 4.8)
  * `add` (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `extractDirectiveMetadata` (Impact: 195.8)
    * *Intent:* /** * Helper function to extract metadata from a `Directive` or `Component`. `Directive`s without a ...
  * `extractHostBindings` (Impact: 117.7)
  * `parseOutputFields` (Impact: 97.2)
    * *Intent:* /** Parses the class members that are outputs. */
  * `tryParseInputFieldMapping` (Impact: 90.9)
  * `extractDecoratorQueryMetadata` (Impact: 69.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `loadComponent` (Impact: 4.5)
  * `ngAfterViewInit` (Impact: 4.0)
  * `loadComponent` (Impact: 3.5)
  * `ngAfterViewInit` (Impact: 3.4)
  * `ngAfterViewInit` (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `popStateListener` (Impact: 6.6)
  * `precommitHandler` (Impact: 4.9)
  * `setUpEntries` (Impact: 3.9)
  * `setUpEntriesWithHistory` (Impact: 3.6)
  * `precommitHandler` (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `patchEventTarget` (Impact: 413.4)
  * `patchEventTargetMethods` (Impact: 265.2)
  * `makeAddListener` (Impact: 131.6)
  * `globalCallback` (Impact: 36.2)
  * `findEventTasks` (Impact: 25.8)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1267.64 | **LOC:** 1049 | **CtrlFlow:** 1.4% | **Authorship Centralization:** 33.3%
...
**Top Internal Functions/Classes:**
  * `escapeForTemplateLiteral` (Impact: 183.8)
  * `constructor` (Impact: 24.4)
  * `constructor` (Impact: 17.6)
  * `constructor` (Impact: 14.2)
  * `constructor` (Impact: 11.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_flushAnimations` (Impact: 123.7)
  * `trigger` (Impact: 51.0)
  * `_getPreviousPlayers` (Impact: 45.7)
  * `removeNode` (Impact: 27.0)
  * `_buildAnimation` (Impact: 25.3)
**Contextual Mitigations & Amplifications:**
...
- `packages/core/src/render3/component_ref.ts` -> **Severity: 0.035** (Bridge: 0.0004 * Flux: 96.7204%)
- `packages/core/src/render3/ng_module_ref.ts` -> **Severity: 0.025** (Bridge: 0.0004 * Flux: 66.3052%)
- `packages/core/src/render3/errors_di.ts` -> **Severity: 0.023** (Bridge: 0.0002 * Flux: 99.9544%)
- `packages/core/src/hydration/utils.ts` -> **Severity: 0.013** (Bridge: 0.0001 * Flux: 98.4778%)
- `packages/core/src/di/inject_switch.ts` -> **Severity: 0.012** (Bridge: 0.0002 * Flux: 50.0%)
