...
| Avg Path Length | 2.0137 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 95.4 | 5.2 | 0.0 | 0.0 |
...
| connectivity | 6381 | 436 | 7 | `crates/cargo-util-schemas/src/manifest/mod.rs` |
...
| dead_code | 3846 | 855 | 4 | `tests/testsuite/test.rs` |
...
- `to_real_manifest` (@ `src/cargo/util/toml/mod.rs`) -> Impact: **330.2** | LOC: 619
- `new` (@ `src/cargo/ops/cargo_install.rs`) -> Impact: **209.0** | LOC: 213
  * *Intent:* // Returns pkg to install. None if pkg is already installed
- `resolve_dependency` (@ `src/cargo/ops/cargo_add/mod.rs`) -> Impact: **207.2** | LOC: 184
- `create_bcx` (@ `src/cargo/ops/cargo_compile/mod.rs`) -> Impact: **195.0** | LOC: 456
...
- `sync` (@ `src/cargo/ops/vendor.rs`) -> Impact: **179.2** | LOC: 304
- `normalize_package_toml` (@ `src/cargo/util/toml/mod.rs`) -> Impact: **174.7** | LOC: 213
- `parse` (@ `src/cargo/core/compiler/custom_build.rs`) -> Impact: **173.3** | LOC: 346
...
- `clean_specs` (@ `src/cargo/ops/cargo_clean.rs`) -> Impact: **169.0** | LOC: 258
- `from_toml` (@ `src/cargo/util/toml_mut/dependency.rs`) -> Impact: **166.7** | LOC: 159
  * *Intent:* /// Create a dependency from a TOML table entry.
- `build_work` (@ `src/cargo/core/compiler/custom_build.rs`) -> Impact: **152.1** | LOC: 409
...
|---|---|---|---|---|
| `tests/testsuite` | 151 | 34295.0 | 6.69% | 0.0% |
...
- `crates/build-rs/src/input.rs` -> **66** Orphaned Functions | **0** Duplicates
...
- **Heaviest Functions:** `_cargo` (Impact: 37.5), `_get_names_from_array` (Impact: 13.0), `_toolchains` (Impact: 7.2)
...
- **Heaviest Functions:** `activation_error` (Impact: 145.0), `describe_path` (Impact: 14.7), `alt_names` (Impact: 10.2)
...
- **Heaviest Functions:** `patch` (Impact: 66.7), `query` (Impact: 58.7), `summary_for_patch` (Impact: 40.2)
...
- **Heaviest Functions:** `list_files_walk` (Impact: 66.3), `list_files_gix` (Impact: 64.1), `_list_files` (Impact: 41.5)
...
- **Heaviest Functions:** `print` (Impact: 29.8), `run` (Impact: 15.1), `post` (Impact: 13.2)
...
- **Heaviest Functions:** `emit_pkg_lints` (Impact: 45.5), `find_path_deps` (Impact: 40.8), `report_unknown_features_error` (Impact: 39.8)
...
- **Heaviest Functions:** `emit_diag` (Impact: 23.5), `run_to_finish` (Impact: 11.0), `stderr` (Impact: 9.2)
...
- **Heaviest Functions:** `update` (Impact: 29.4), `verify` (Impact: 15.5), `query` (Impact: 12.2)
...
- **Heaviest Functions:** `write_pkg_lockfile` (Impact: 31.0), `serialize_resolve` (Impact: 30.1), `are_equal_lockfiles` (Impact: 16.9)
...
- **Heaviest Functions:** `implicit_minimum_version_req_ws` (Impact: 38.5), `implicit_minimum_version_req_pkg` (Impact: 32.6), `get_suggested_version_req` (Impact: 22.5)
...
**Top Internal Functions/Classes:**
  * `git_dependency_no_version` (Impact: 92.5)
  * `workspace_noconflict_readme` (Impact: 92.5)
  * `package_verbose` (Impact: 7.5)
  * `checksum_changed` (Impact: 7.5)
    * *Intent:* // This is a companion to `publish::checksum_changed`, but because this one // is packaging without ...
  * `long_file_names` (Impact: 7.4)
...
**Top Internal Functions/Classes:**
  * `to_real_manifest` (Impact: 330.2)
  * `normalize_package_toml` (Impact: 174.7)
  * `prepare_toml_for_publish` (Impact: 129.0)
    * *Intent:* /// Prepares the manifest for publishing. // - Path and git components of dependency specifications ...
  * `normalize_toml` (Impact: 119.5)
    * *Intent:* /// See [`Manifest::normalized_toml`] for more details
  * `dep_to_dependency` (Impact: 95.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_rename_with_link_search_path` (Impact: 31.2)
  * `code_generation` (Impact: 27.0)
  * `rerun_if_directory` (Impact: 15.5)
  * `generate_good_d_files` (Impact: 9.5)
  * `custom_build_env_vars` (Impact: 8.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `publish_git_with_version` (Impact: 89.5)
    * *Intent:* // A dependency with both `git` and `version`.
  * `git_deps` (Impact: 53.5)
  * `old_token_location` (Impact: 6.0)
  * `simple` (Impact: 5.0)
  * `duplicate_version` (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `sparse_retry_multiple` (Impact: 12.6)
  * `dl_retry_multiple` (Impact: 12.4)
  * `wrong_version_http` (Impact: 8.2)
  * `wrong_version_git` (Impact: 8.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `crate_env_vars` (Impact: 103.5)
  * `repro` (Impact: 7.2)
  * `primary_package_env_var` (Impact: 6.0)
  * `main` (Impact: 5.5)
  * `incompatible_dependencies` (Impact: 5.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `find_path_deps` (Impact: 40.8)
  * `report_unknown_features_error` (Impact: 39.8)
  * `find_members` (Impact: 39.3)
    * *Intent:* /// After the root of a workspace has been located, probes for all members /// of a workspace. /// /...
  * `emit_ws_lints` (Impact: 32.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `git_worktree_with_original_repo_renamed` (Impact: 28.2)
  * `dirty_submodule` (Impact: 10.8)
  * `failed_submodule_checkout` (Impact: 8.4)
  * `git_with_force_push` (Impact: 8.4)
  * `metadata_master_consistency` (Impact: 8.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `assert_artifact_executable_output` (Impact: 10.7)
  * `build_script_with_bin_artifacts` (Impact: 8.7)
  * `build_script_features_for_shared_dependency` (Impact: 8.5)
  * `features_are_not_unified_among_lib_and_bin_dep_of_different_target` (Impact: 7.5)
  * `cross_doctests_works_with_artifacts` (Impact: 7.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `cargo_test_doctest_xcompile_runner` (Impact: 6.1)
  * `cargo_doc_test_quiet` (Impact: 6.0)
  * `cyclical_dep_with_missing_feature` (Impact: 4.7)
  * `doctest_skip_staticlib` (Impact: 4.2)
  * `cargo_test_env` (Impact: 4.0)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `resolve_dependency` (Impact: 207.2)
  * `add` (Impact: 110.1)
    * *Intent:* /// Add dependencies to a manifest
  * `get_public_dependency` (Impact: 76.8)
...
**Top Internal Functions/Classes:**
  * `redefined_sources` (Impact: 76.5)
  * `duplicate_packages_in_cargo_lock` (Impact: 5.0)
  * `bad1` (Impact: 3.0)
  * `unsupported_float` (Impact: 2.5)
  * `unsupported_datetime` (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `patch_to_git_pull_request` (Impact: 52.5)
  * `perform_old_git_patch` (Impact: 12.8)
  * `mismatched_version_with_prerelease` (Impact: 8.2)
  * `patch_eq_conflict_panic` (Impact: 5.0)
    * *Intent:* // From https://github.com/rust-lang/cargo/issues/7463
  * `mismatched_version2` (Impact: 5.0)
...
**Top Internal Functions/Classes:**
  * `git_deterministic` (Impact: 7.0)
  * `discovery_inferred_build_rs_included` (Impact: 6.0)
  * `discovery_inferred_build_rs_excluded` (Impact: 6.0)
  * `discovery_inferred_lib_included` (Impact: 6.0)
  * `discovery_inferred_lib_excluded` (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `mergeable_info_rebuild_detection` (Impact: 9.5)
  * `mergeable_info_rebuild_with_depinfo` (Impact: 9.5)
  * `doc_fingerprint_is_versioning_consistent` (Impact: 8.5)
  * `mergeable_info_multi_targets` (Impact: 8.0)
  * `mergeable_info_additive` (Impact: 6.5)
**Contextual Mitigations & Amplifications:**
...
### `tests/testsuite/workspaces.rs` (RUST | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 772.08 | **LOC:** 2932 | **CtrlFlow:** 0.9% | **Authorship Centralization:** 40.0%
- **Risk Profile:** Cognitive Load (13.9058%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `virtual_default_member_is_not_a_member` (Impact: 5.2)
  * `ws_warn_unused` (Impact: 5.0)
  * `member_dep_missing` (Impact: 5.0)
  * `parent_manifest_error_mentions_workspace_search` (Impact: 4.8)
  * `dont_recurse_out_of_cargo_home` (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
* *State Mutation (weighted view):* 443
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 22`, `structural_boundaries: 153`, `args: 127`, `func_start: 79`
* *Risk/State:* `safety_bypasses: 8`, `high_risk_execution: 4`, `state_mutation: 441`, `fragile_debt: 2`
* *Architecture:* `io: 1`, `api: 86`, `import: 11`
* *Defense:* `doc: 9`, `test: 102`, `sync_locks: 27`
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 0.474
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 0):` Command, basic_lib_manifest, basic_manifest, cargo_test_support::
    ProjectBuilder, cargo_test_support::registry::Package, cargo_test_support::str, crate::prelude::*, git...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
**Top Internal Functions/Classes:**
  * `from_toml` (Impact: 166.7)
    * *Intent:* /// Create a dependency from a TOML table entry.
  * `update_toml` (Impact: 113.0)
...
**Top Internal Functions/Classes:**
  * `normalize_lib` (Impact: 61.8)
  * `normalize_targets_with_legacy_path` (Impact: 55.6)
  * `toml_targets_and_inferred` (Impact: 55.3)
  * `normalize_bins` (Impact: 51.9)
  * `to_targets` (Impact: 50.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
**Top Internal Functions/Classes:**
  * `check_authorized` (Impact: 54.0)
  * `build` (Impact: 47.2)
    * *Intent:* /// Initializes the registry.
  * `append_manifest` (Impact: 40.6)
  * `start` (Impact: 26.7)
  * `index` (Impact: 23.2)
