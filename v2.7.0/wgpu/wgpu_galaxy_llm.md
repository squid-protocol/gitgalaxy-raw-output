...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 26.7 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 20.1 | 2.3 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 97.2 | 14.6 | 6.7 | 0.0 |
...
| connectivity | 10117 | 712 | 30 | `wgpu-hal/src/lib.rs` |
...
| dead_code | 3412 | 554 | 9 | `naga/tests/naga/wgsl_errors.rs` |
...
- `write_expr` (@ `naga/src/back/hlsl/writer.rs`) -> Impact: **852.9** | LOC: 1137
  * *Intent:* /// Helper method to write expressions /// /// # Notes /// Doesn't add any newlines or leading/trailing spaces
- `call_builtin` (@ `naga/src/front/wgsl/lower/mod.rs`) -> Impact: **780.6** | LOC: 853
  * *Intent:* /// Generate Naga IR for a call to a WGSL builtin function.
- `put_expression` (@ `naga/src/back/msl/writer.rs`) -> Impact: **761.0** | LOC: 998
...
- `write_stmt` (@ `naga/src/back/hlsl/writer.rs`) -> Impact: **756.8** | LOC: 830
  * *Intent:* /// Helper method used to write statements /// /// # Notes /// Always adds a newline
- `write_functions` (@ `naga/src/back/msl/writer.rs`) -> Impact: **729.6** | LOC: 1217
- `put_block` (@ `naga/src/back/msl/writer.rs`) -> Impact: **658.0** | LOC: 683
- `write_stmt` (@ `naga/src/back/wgsl/writer.rs`) -> Impact: **559.1** | LOC: 503
  * *Intent:* /// Helper method used to write statements /// /// # Notes /// Always adds a newline
- `validate_expression` (@ `naga/src/valid/expression.rs`) -> Impact: **526.5** | LOC: 1049
- `next_block` (@ `naga/src/front/spv/next_block.rs`) -> Impact: **472.6** | LOC: 1132
...
- `lower_inner` (@ `naga/src/front/glsl/context.rs`) -> Impact: **436.9** | LOC: 851
...
- **Heaviest Functions:** `begin_render_pass` (Impact: 70.7), `set_render_pipeline` (Impact: 47.1), `rebind_vertex_data` (Impact: 28.9)
...
- **Heaviest Functions:** `map_async` (Impact: 23.2), `get_mapped_range` (Impact: 13.7), `unmap` (Impact: 6.0)
...
- **Heaviest Functions:** `deref` (Impact: 3.5), `deref` (Impact: 3.5), `deref_mut` (Impact: 3.5)
...
- **Heaviest Functions:** `map_texture_format` (Impact: 31.9), `map_buffer_usage_to_barrier` (Impact: 21.7), `map_texture_usage_to_barrier` (Impact: 20.1)
...
- **Heaviest Functions:** `create_pipeline_layout` (Impact: 121.0), `create_render_pipeline` (Impact: 74.7), `new` (Impact: 67.3)
...
- **Heaviest Functions:** `from_extensions_and_requested_features` (Impact: 187.4), `device_from_raw` (Impact: 187.0), `to_wgpu` (Impact: 111.6)
...
- **Heaviest Functions:** `write_initialize_function` (Impact: 247.2), `write_generate_intersection` (Impact: 96.0), `write_candidate_intersection_function` (Impact: 79.5)
...
- **Heaviest Functions:** `create_render_pipeline` (Impact: 107.4), `load_shader` (Impact: 81.8), `create_pipeline_layout` (Impact: 44.8)
...
- **Heaviest Functions:** `begin_render_pass` (Impact: 47.5), `transition_textures` (Impact: 36.1), `set_bind_group` (Impact: 28.0)
...
- **Heaviest Functions:** `create_render_pipeline` (Impact: 68.2), `compile_stage` (Impact: 57.0), `create_bind_group_layout` (Impact: 35.7)
...
**Top Internal Functions/Classes:**
  * `put_expression` (Impact: 761.0)
    * *Intent:* /// Emit code for the expression `expr_handle`. /// /// The `is_scoped` argument is true if the surr...
  * `write_functions` (Impact: 729.6)
  * `put_block` (Impact: 658.0)
  * `write_unpacking_function` (Impact: 256.5)
  * `write_wrapped_image_sample` (Impact: 161.6)
...
**Top Internal Functions/Classes:**
  * `call_builtin` (Impact: 780.6)
    * *Intent:* /// Generate Naga IR for a call to a WGSL builtin function.
  * `statement` (Impact: 194.3)
  * `texture_sample_helper` (Impact: 108.4)
  * `expression_for_reference` (Impact: 88.7)
  * `function` (Impact: 81.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `create_render_pipeline` (Impact: 324.5)
  * `create_texture_view` (Impact: 133.2)
  * `create_texture` (Impact: 121.5)
  * `configure_surface` (Impact: 95.5)
  * `create_bind_group_layout_internal` (Impact: 85.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `write_function` (Impact: 248.3)
  * `write_logical_layout` (Impact: 136.2)
  * `map_binding` (Impact: 105.7)
  * `write_global_variable` (Impact: 66.6)
  * `write_std140_compat_type_declaration` (Impact: 62.2)
...
**Top Internal Functions/Classes:**
  * `start` (Impact: 386.2)
  * `encode_render_pass` (Impact: 136.4)
  * `fill_arc_desc` (Impact: 85.3)
  ...
  * `multi_draw_indirect` (Impact: 80.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `write_stmt` (Impact: 559.1)
    * *Intent:* /// Helper method used to write statements /// /// # Notes /// Always adds a newline
  * `write_expr_plain_form` (Impact: 435.1)
    * *Intent:* /// Write the 'plain form' of `expr`. /// /// An expression's 'plain form' is the most general rendi...
  * `write_possibly_const_expression` (Impact: 87.5)
  * `write` (Impact: 75.2)
  * `write_function` (Impact: 73.8)
...
**Top Internal Functions/Classes:**
  * `parse_global_variable` (Impact: 76.8)
  * `parse` (Impact: 60.0)
  * `next_decoration` (Impact: 58.7)
  * `parse_type_image` (Impact: 56.8)
  * `parse_expr_binary_op_sign_adjusted` (Impact: 42.7)
...
**Top Internal Functions/Classes:**
  * `write_wrapped_image_query_function` (Impact: 119.7)
  * `write_wrapped_image_sample_function` (Impact: 119.3)
  * `write_wrapped_constructor_function` (Impact: 117.4)
  * `write_wrapped_math_functions` (Impact: 106.7)
  * `write_wrapped_binary_ops` (Impact: 103.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `statement` (Impact: 186.7)
  * `parse` (Impact: 77.5)
  * `function_decl` (Impact: 55.3)
  * `variable_or_value_or_func_call_or_variable_updating_statement` (Impact: 54.4)
...
**Top Internal Functions/Classes:**
  * `inject_standard_builtins` (Impact: 195.2)
...
**Top Internal Functions/Classes:**
  ...
  * `pop_error_scope` (Impact: 21.0)
  * `handle_error_or_return_handler` (Impact: 13.5)
    * *Intent:* /// Deliver the error to /// /// * the innermost error scope, if any, or /// * the uncaptured error ...
  * `handle_error_inner` (Impact: 12.9)
  * `create_mesh_pipeline` (Impact: 11.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.003413
...
**Top Internal Functions/Classes:**
  * `create_render_pipeline` (Impact: 68.2)
  * `compile_stage` (Impact: 57.0)
  * `create_bind_group_layout` (Impact: 35.7)
  * `error_if_would_oom_on_resource_allocation` (Impact: 32.5)
  * `make_render_pass` (Impact: 30.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
**Top Internal Functions/Classes:**
  * `begin_render_pass` (Impact: 29.9)
  * `create_render_pipeline` (Impact: 21.1)
  * `create_bind_group_layout` (Impact: 17.8)
  * `create_shader_module` (Impact: 16.9)
  * `create_bind_group` (Impact: 15.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `bad_for_initializer` (Impact: 189.4)
  * `too_many_unclosed_loops` (Impact: 138.7)
  * `invalid_functions` (Impact: 16.8)
  * `subgroup_capability` (Impact: 16.0)
  * `reserved_keyword` (Impact: 15.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `create_pipeline_layout` (Impact: 121.0)
  * `create_render_pipeline` (Impact: 74.7)
  * `new` (Impact: 67.3)
  * `load_shader` (Impact: 55.0)
    * *Intent:* /// When generating the vertex shader, the fragment stage must be passed if it exists! /// Otherwise...
  * `create_bind_group` (Impact: 52.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `function_call` (Impact: 176.1)
  * `constructor_single` (Impact: 95.6)
  * `process_lhs_argument` (Impact: 80.5)
    * *Intent:* /// Processes a function call argument that appears in place of an output /// parameter.
  * `matrix_one_arg` (Impact: 79.3)
  * `constructor_many` (Impact: 46.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `device_create_general_render_pipeline` (Impact: 77.2)
  * `device_create_bind_group` (Impact: 53.4)
  * `device_create_compute_pipeline` (Impact: 38.1)
  * `resolve_entry` (Impact: 33.8)
  * `device_create_pipeline_layout` (Impact: 23.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `write_initialize_function` (Impact: 247.2)
  * `write_generate_intersection` (Impact: 96.0)
  * `write_candidate_intersection_function` (Impact: 79.5)
  * `write_committed_intersection_function` (Impact: 73.9)
...
**Top Internal Functions/Classes:**
  * `lower_inner` (Impact: 436.9)
    * *Intent:* /// Internal implementation of [`lower`](Self::lower)
  * `add_function_arg` (Impact: 26.8)
    * *Intent:* /// Add function argument to current scope
  * `add_global` (Impact: 23.8)
  * `binary_implicit_conversion` (Impact: 23.4)
  * `lower_store` (Impact: 20.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `validate_block_impl` (Impact: 340.6)
  * `validate_atomic` (Impact: 102.0)
  * `validate_function` (Impact: 60.0)
  * `validate_call` (Impact: 37.3)
  * `validate_subgroup_gather` (Impact: 32.5)
**Contextual Mitigations & Amplifications:**
