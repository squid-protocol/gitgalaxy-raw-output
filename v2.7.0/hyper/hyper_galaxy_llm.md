...
| Connectivity (formerly API Exposure) | 0.0 | 60.8 | 14.3 | 6.9 | 0.0 |
...
| connectivity | 876 | 85 | 26 | `src/error.rs` |
...
| dead_code | 501 | 49 | 15 | `tests/server.rs` |
| credential | 0 | 0 | 0 | - |
...
- `encode_headers` (@ `src/proto/h1/role.rs`) -> Impact: **183.9** | LOC: 344
- `parse` (@ `src/proto/h1/role.rs`) -> Impact: **75.3** | LOC: 225
- `poll_write` (@ `src/proto/h1/dispatch.rs`) -> Impact: **72.2** | LOC: 94
- `parse` (@ `src/proto/h1/role.rs`) -> Impact: **65.6** | LOC: 169
- `main` (@ `capi/examples/upload.c`) -> Impact: **60.2** | LOC: 234
  * *Intent:* #define STR_ARG(XX) (uint8_t *)XX, strlen(XX)
- `main` (@ `capi/examples/client.c`) -> Impact: **53.4** | LOC: 202
  * *Intent:* #define STR_ARG(XX) (uint8_t *)XX, strlen(XX)
- `poll` (@ `src/proto/h2/client.rs`) -> Impact: **50.9** | LOC: 118
- `set_length` (@ `src/proto/h1/role.rs`) -> Impact: **48.2** | LOC: 132
- `poll_read` (@ `src/proto/h1/dispatch.rs`) -> Impact: **41.9** | LOC: 75
- `encode` (@ `src/proto/h1/role.rs`) -> Impact: **41.4** | LOC: 101
...
- **Heaviest Functions:** `try_send_request` (Impact: 6.4), `send_request` (Impact: 6.3), `poll_ready` (Impact: 5.5)
...
- **Heaviest Functions:** `decode` (Impact: 41.1), `read_body` (Impact: 17.4), `test_read_chunk_size` (Impact: 16.8)
...
- **Heaviest Functions:** `poll` (Impact: 33.1), `strip_connection_headers` (Impact: 19.2), `advance` (Impact: 3.9)
...
- **Heaviest Functions:** `giver_queue_throughput` (Impact: 8.2), `poll` (Impact: 6.8), `drop` (Impact: 6.5)
...
- **Heaviest Functions:** `poll` (Impact: 50.9), `poll` (Impact: 13.0), `poll_pipe` (Impact: 12.7)
...
- **Heaviest Functions:** `raw_tcp_throughput_large_payload` (Impact: 12.1), `raw_tcp_throughput_small_payload` (Impact: 7.5), `throughput_fixedsize_many_chunks` (Impact: 3.3)
...
- **Heaviest Functions:** `bench` (Impact: 38.8), `spawn_server` (Impact: 12.8), `request_chunks` (Impact: 2.3)
...
- **Heaviest Functions:** `poll_read_head` (Impact: 37.4), `poll_read_body` (Impact: 29.2), `maybe_notify` (Impact: 17.9)
...
- **Heaviest Functions:** `encode_headers` (Impact: 183.9), `parse` (Impact: 75.3), `parse` (Impact: 65.6)
...
- **Heaviest Functions:** `poll` (Impact: 18.5), `calculate` (Impact: 18.1), `record_data` (Impact: 17.2)
...
**Top Internal Functions/Classes:**
  * `encode_headers` (Impact: 183.9)
  * `parse` (Impact: 75.3)
  * `parse` (Impact: 65.6)
  * `set_length` (Impact: 48.2)
  * `encode` (Impact: 41.4)
...
**Top Internal Functions/Classes:**
  * `run_test` (Impact: 15.6)
  * `h2_connect_multiplex` (Impact: 15.4)
  * `serve` (Impact: 12.4)
  * `call` (Impact: 11.8)
...
**Top Internal Functions/Classes:**
  * `test_try_send_request` (Impact: 12.1)
  * `http2_detect_conn_eof` (Impact: 7.0)
  * `connect_method` (Impact: 6.5)
  * `upgrade` (Impact: 6.2)
  * `drain_til_eof` (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `poll_read_head` (Impact: 37.4)
  * `poll_read_body` (Impact: 29.2)
...
**Top Internal Functions/Classes:**
  * `decode` (Impact: 41.1)
  * `read_body` (Impact: 17.4)
  * `test_read_chunk_size` (Impact: 16.8)
    * *Intent:* */
...
**Top Internal Functions/Classes:**
  * `poll_write` (Impact: 72.2)
  * `poll_read` (Impact: 41.9)
  * `poll_inner` (Impact: 21.1)
  * `recv_msg` (Impact: 19.4)
  * `poll_read_head` (Impact: 16.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 4 instances
...
**Top Internal Functions/Classes:**
  * `poll_flush` (Impact: 24.3)
  * `parse` (Impact: 20.6)
  * `record` (Impact: 15.5)
  * `poll_flush_flattened` (Impact: 13.0)
...
**Top Internal Functions/Classes:**
  * `poll` (Impact: 50.9)
  * `poll` (Impact: 13.0)
  * `poll_pipe` (Impact: 12.7)
  * `handshake` (Impact: 12.6)
  * `new_builder` (Impact: 9.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 5 instances
...
**Top Internal Functions/Classes:**
  * `async_test` (Impact: 32.0)
  * `naive_proxy` (Impact: 20.4)
  * `__run_test` (Impact: 1.6)
  * `default` (Impact: 1.4)
  * `default` (Impact: 1.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 4 instances
...
**Top Internal Functions/Classes:**
  * `poll` (Impact: 18.5)
  * `calculate` (Impact: 18.1)
  * `record_data` (Impact: 17.2)
  * `maybe_ping` (Impact: 14.7)
  * `maybe_schedule` (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `encode_trailers` (Impact: 26.8)
  * `encode` (Impact: 8.6)
  * `encode_and_end` (Impact: 8.2)
  * `advance` (Impact: 4.0)
  * `chunks_vectored` (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `poll_frame` (Impact: 18.5)
  * `h2` (Impact: 7.0)
  * `size_hint` (Impact: 6.8)
  * `new_channel` (Impact: 6.4)
  * `eq` (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `find_source` (Impact: 4.8)
  * `description` (Impact: 4.8)
  * `new_h2` (Impact: 4.6)
  * `fmt` (Impact: 3.9)
  * `from` (Impact: 3.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `poll_server` (Impact: 31.6)
  * `poll2` (Impact: 28.8)
  * `new` (Impact: 17.3)
  * `poll` (Impact: 17.3)
  * `poll_ping` (Impact: 5.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
**Top Internal Functions/Classes:**
  * `handshake` (Impact: 21.8)
    * *Intent:* /// Constructs a connection with the configured options and IO. /// See [`client::conn`](crate::clie...
  * `try_send_request` (Impact: 6.4)
    * *Intent:* /// Sends a `Request` on the associated connection. /// /// Returns a future that if successful, yie...
  * `send_request` (Impact: 6.2)
...
**Top Internal Functions/Classes:**
  * `giver_queue_throughput` (Impact: 8.2)
  * `poll` (Impact: 6.8)
  * `drop` (Impact: 6.5)
  * `can_send` (Impact: 6.3)
  * `try_send` (Impact: 4.0)
...
**Top Internal Functions/Classes:**
  ...
  * `request_chunks` (Impact: 2.3)
  * `http2_parallel_x10_req_10mb` (Impact: 2.0)
  * `http2_parallel_x10_req_10kb_100_chunks_adaptive_window` (Impact: 2.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Rce:* 1 instances
* *Amplified Cascading Flux:* 1 instances
* *Sec Tainted Injection (weighted view):* 1
...
**Top Internal Functions/Classes:**
  * `http1_client` (Impact: 16.9)
  * `http2_client` (Impact: 16.9)
  * `main` (Impact: 8.3)
  * `http2_server` (Impact: 7.2)
  * `http1_server` (Impact: 6.6)
**Structural Signatures (Net Mitigated Signals):**
