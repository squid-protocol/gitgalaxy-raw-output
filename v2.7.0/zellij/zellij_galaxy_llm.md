...
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 1193 |
| Total LOC | 245335 |
...
| Avg Path Length | 1.6923 | Hops between files. Lower = Tighter coupling. |
| Articulation Pts | 7 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| RUST | 302 | 239156 | 79.9% |
| MARKDOWN | 25 | 0 | 6.6% |
| PROTO | 17 | 3646 | 4.5% |
| YAML | 12 | 969 | 3.2% |
...
|---|---|---|
| Unclassified | 353 | 93.4% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 25 | 6.6% |

## 5. EXCLUDED ARTIFACTS (Unparsable or Shielded Files)
*Total Excluded Artifacts: 1193*
...
- `.yaml`: 3x Zero-Density Threshold (LOC: 557, Signals: 0), 1x Zero-Density Threshold (LOC: 251, Signals: 0), 1x Zero-Density Threshold (LOC: 569, Signals: 0)
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 14.1 | 9.6 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 95.3 | 41.1 | 52.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 34.1 | 14.3 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 34.1 | 2.4 | 80.0 |
| Connectivity (formerly API Exposure) | 0.0 | 98.5 | 11.5 | 7.4 | 0.0 |
| Concurrency Surface (formerly Concurrency Exposure) | 0.0 | 100.0 | 8.1 | 0.0 | 0.0 |
| Mutation Surface (formerly State Flux Exposure) | 0.0 | 100.0 | 37.4 | 17.9 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 10.0 | 1.7 | 0.6 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 83.6 | 17.2 | 8.5 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 73.4 | 100.0 | 100.0 |
...
| connectivity | 7137 | 284 | 38 | `zellij-utils/assets/prost_ipc/client_server_contract.rs` |
...
| dead_code | 3784 | 233 | 23 | `zellij-server/src/tab/unit/tab_integration_tests.rs` |
...
- `guard_balance_ratio` (guards / (danger + 1)): **0.4808**
...
- `route_action` (@ `zellij-server/src/route.rs`) -> Impact: **708.4** | LOC: 1372
- `screen_thread_main` (@ `zellij-server/src/screen.rs`) -> Impact: **612.3** | LOC: 1134
  * *Intent:* // The box is here in order to make the // NewClient enum smaller
- `csi_dispatch` (@ `zellij-server/src/panes/grid.rs`) -> Impact: **427.1** | LOC: 605
- `plugin_thread_main` (@ `zellij-server/src/plugins/mod.rs`) -> Impact: **386.3** | LOC: 1014
- `try_from` (@ `zellij-utils/src/plugin_api/plugin_command.rs`) -> Impact: **384.1** | LOC: 951
- `actions_from_cli` (@ `zellij-utils/src/input/actions.rs`) -> Impact: **348.1** | LOC: 1001
- `try_from` (@ `zellij-utils/src/plugin_api/action.rs`) -> Impact: **344.6** | LOC: 952
- `update` (@ `default-plugins/fixture-plugin-for-tests/src/main.rs`) -> Impact: **310.0** | LOC: 866
- `try_from` (@ `zellij-utils/src/ipc/protobuf_conversion.rs`) -> Impact: **299.1** | LOC: 919
- `encode` (@ `zellij-utils/src/vendored/termwiz/input.rs`) -> Impact: **228.3** | LOC: 318
...
- **Heaviest Functions:** `add_keybinds` (Impact: 4.4), `sync_tab_full` (Impact: 1.9), `sync_tab_medium` (Impact: 1.9)
...
- **Heaviest Functions:** `handle_websocket` (Impact: 25.7), `test_control_message_handling` (Impact: 7.2), `test_resize_signal_sends_control_message` (Impact: 6.5)
...
- **Heaviest Functions:** `csi_dispatch` (Impact: 427.1), `osc_dispatch` (Impact: 113.2), `change_size` (Impact: 93.3)
...
- **Heaviest Functions:** `test_kicked_by_host_sends_close_code_4001` (Impact: 26.1), `test_full_session_flow` (Impact: 21.3), `test_normal_exit_sends_normal_close_code` (Impact: 18.2)
...
- **Heaviest Functions:** `update_filter` (Impact: 20.9), `render_filter_line` (Impact: 8.8), `get_matched_indices_for_visible` (Impact: 6.8)
...
- **Heaviest Functions:** `remove_covered_sixel_parts` (Impact: 97.4), `serialize_chunks` (Impact: 75.9), `remove_covered_parts` (Impact: 41.2)
...
- **Heaviest Functions:** `calculate_list_bounds` (Impact: 33.9), `render_current_path` (Impact: 24.7), `truncate_path` (Impact: 17.1)
...
- **Heaviest Functions:** `handle_ws_terminal` (Impact: 11.6), `handle_ws_control` (Impact: 11.6), `handle_session` (Impact: 6.3)
...
- **Heaviest Functions:** `next` (Impact: 15.9), `enable_mouse_support` (Impact: 9.3), `disable_mouse_support` (Impact: 9.1)
...
**Top Internal Functions/Classes:**
  * `suppress_pane_and_replace_with_pid` (Impact: 64.0)
  * `write_to_pane_id` (Impact: 60.1)
  * `new_no_preference_pane` (Impact: 52.9)
  * `new_stacked_pane` (Impact: 52.1)
  * `extract_pane` (Impact: 49.9)
...
**Top Internal Functions/Classes:**
  * `csi_dispatch` (Impact: 427.1)
  * `osc_dispatch` (Impact: 113.2)
  * `change_size` (Impact: 93.3)
  * `compute_plugin_highlight_selections` (Impact: 46.0)
    * *Intent:* /// Pre-compute plugin highlight selections across all logical line groups in /// the viewport. Hove...
  * `get_selected_text` (Impact: 42.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `screen_thread_main` (Impact: 612.3)
    * *Intent:* // The box is here in order to make the // NewClient enum smaller
  * `apply_layout` (Impact: 105.4)
  * `render_to_clients` (Impact: 68.5)
  * `reconfigure` (Impact: 61.1)
  * `focus_plugin_pane` (Impact: 60.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `to_kdl` (Impact: 178.5)
  * `try_from` (Impact: 111.5)
  * `to_kdl` (Impact: 87.1)
  * `from_kdl` (Impact: 61.1)
  * `from_string` (Impact: 56.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `mode_update_payload_is_lightweight_for_opted_in_plugins` (Impact: 21.8)
  * `create_plugin_thread_with_pty_receiver` (Impact: 16.7)
  * `create_plugin_thread` (Impact: 15.5)
  * `reconfiguration_resends_keybinds_to_opted_in_plugins` (Impact: 14.1)
  * `create_plugin_thread_with_background_jobs_receiver` (Impact: 13.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `parse_floating_pane_node_with_template` (Impact: 78.1)
  * `populate_layout_child` (Impact: 76.3)
  * `differentiate_pane_and_floating_pane_template` (Impact: 57.9)
  * `parse_pane_node_with_template` (Impact: 51.6)
  * `layout_with_one_pane` (Impact: 47.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `run_with_alias` (Impact: 12.6)
    * *Intent:* // same as the above function, but starts a plugin with a plugin alias
  * `run` (Impact: 12.4)
  * `new` (Impact: 12.3)
  * `background_plugin_receives_broadcasts_regardless_of_active_tab` (Impact: 12.3)
  * `integration_scrollback_from_pre_subscription_pty_bytes` (Impact: 10.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `mirrored_sessions` (Impact: 39.5)
  * `watcher_client_functionality` (Impact: 33.6)
  * `multiple_users_in_different_tabs` (Impact: 31.9)
  * `multiple_users_in_different_panes_and_same_tab` (Impact: 27.8)
  * `multiple_users_in_same_pane_and_tab` (Impact: 26.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `focus_follows_mouse_skips_stacked_one_liner_pane` (Impact: 10.3)
  * `render_stacks_without_pane_frames` (Impact: 9.7)
  * `start` (Impact: 8.2)
  * `non_overlapping_highlights_from_different_layers_coexist` (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `switch_session` (Impact: 23.7)
  * `get_pane_scrollback` (Impact: 23.1)
  * `try_save_layout` (Impact: 22.9)
  * `load_new_plugin` (Impact: 20.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `render` (Impact: 120.9)
  * `stacked_resize_pane_with_id` (Impact: 96.1)
  * `set_pane_frames` (Impact: 30.7)
  * `add_pane_with_stacked_resize` (Impact: 25.8)
  * `toggle_pane_fullscreen` (Impact: 23.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `route_action` (Impact: 708.4)
  * `route_thread_main` (Impact: 179.9)
  * `build_tabs_table_row` (Impact: 14.6)
  * `wait_for_action_completion` (Impact: 14.2)
  * `build_table_row` (Impact: 14.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `new` (Impact: 29.0)
  * `from_cli` (Impact: 22.6)
  * `from_config` (Impact: 22.0)
  * `extract_text_by_columns` (Impact: 15.4)
...
**Top Internal Functions/Classes:**
  * `split_space` (Impact: 84.4)
  * `position_panes_in_space` (Impact: 37.2)
  * `adjust_geoms_for_rounding_errors` (Impact: 30.2)
  * `list_available_layouts` (Impact: 27.0)
  * `truncate` (Impact: 22.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `pty_thread_main` (Impact: 186.8)
  * `spawn_terminals_for_layout` (Impact: 183.9)
  * `spawn_terminals_for_layout_override` (Impact: 131.9)
  * `apply_run_instruction` (Impact: 72.9)
  * `spawn_terminal` (Impact: 44.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  * `combine_horizontally_aligned_panes_to_stack` (Impact: 41.9)
  * `combine_vertically_aligned_panes_to_stack` (Impact: 39.8)
  * `expand_pane` (Impact: 35.9)
  * `fill_space_over_visible_stacked_pane` (Impact: 28.2)
  * `resize_panes_in_stack` (Impact: 26.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 84 instances
...
**Top Internal Functions/Classes:**
  * `remove_covered_sixel_parts` (Impact: 97.4)
  * `serialize_chunks` (Impact: 75.9)
  * `remove_covered_parts` (Impact: 41.2)
  * `serialize_chunks_with_newlines` (Impact: 37.5)
  * `serialize_with_size` (Impact: 36.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `render_unified_results` (Impact: 169.7)
  * `render_controls_line` (Impact: 54.1)
  * `render_new_session_block` (Impact: 49.7)
  * `render_new_session_folder_prompt` (Impact: 36.0)
  * `rebuild` (Impact: 34.6)
...
**Top Internal Functions/Classes:**
  ...
  * `edge_and_delta_to_strategies` (Impact: 57.8)
  * `execute_send_to_terminal` (Impact: 50.3)
  * `execute_mouse_action` (Impact: 49.9)
  * `execute_update_hover` (Impact: 38.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `toggle_focused_pane_fullscreen` (Impact: 10.2)
  * `toggle_focused_pane_fullscreen_with_stacked_resizes` (Impact: 8.2)
  * `close_pane_with_multiple_panes_to_the_left_away_from_screen_edges` (Impact: 8.0)
  * `close_pane_with_multiple_panes_to_the_right_away_from_screen_edges` (Impact: 8.0)
  * `create_new_tab_with_layout` (Impact: 7.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `load_plugin` (Impact: 56.7)
  * `apply_event_to_plugin` (Impact: 53.0)
  * `apply_cached_events_and_resizes_for_plugin` (Impact: 36.1)
  * `reconfigure` (Impact: 35.6)
  * `get_or_load_plugins` (Impact: 32.7)
...
**Top Internal Functions/Classes:**
  * `try_from` (Impact: 299.1)
  * `try_from` (Impact: 42.7)
  * `try_from` (Impact: 33.1)
  * `try_from` (Impact: 26.4)
  * `try_from` (Impact: 20.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `handle_selection` (Impact: 104.9)
  * `render` (Impact: 99.7)
  * `handle_single_screen_search_key` (Impact: 87.7)
  * `handle_attach_to_session` (Impact: 83.3)
  * `handle_new_session_key` (Impact: 29.5)
**Contextual Mitigations & Amplifications:**
