...
| Avg Path Length | 3.0355 | Hops between files. Lower = Tighter coupling. |
...
- `tcp_input` (@ `sys/netinet/tcp_input.c`) -> Impact: **464.7** | LOC: 1119
  * *Intent:* /* * TCP input routine, follows pages 65-76 of the * protocol specification dated September, 1981 very closely. */
- `tcp_output` (@ `sys/netinet/tcp_output.c`) -> Impact: **172.8** | LOC: 515
...
- `tcp_usrreq` (@ `sys/netinet/tcp_usrreq.c`) -> Impact: **155.8** | LOC: 274
...
- `ip_output` (@ `sys/netinet/ip_output.c`) -> Impact: **155.7** | LOC: 321
...
- `ip_ctloutput` (@ `sys/netinet/ip_output.c`) -> Impact: **147.0** | LOC: 147
  * *Intent:* /* * IP socket option processing. */
- `icmp_input` (@ `sys/netinet/ip_icmp.c`) -> Impact: **122.6** | LOC: 234
  * *Intent:* /* * Process a received ICMP message. */
- `ip_setmoptions` (@ `sys/netinet/ip_output.c`) -> Impact: **111.8** | LOC: 235
  * *Intent:* /* * Set the IP multicast options in response to user setsockopt(). */
- `rip_usrreq` (@ `sys/netinet/raw_ip.c`) -> Impact: **110.7** | LOC: 156
  * *Intent:* /*ARGSUSED*/
- `udp_usrreq` (@ `sys/netinet/udp_usrreq.c`) -> Impact: **92.5** | LOC: 136
  * *Intent:* /* 40 1K datagrams */ /*ARGSUSED*/
- `rip_ctloutput` (@ `sys/netinet/raw_ip.c`) -> Impact: **92.1** | LOC: 78
...
- **Heaviest Functions:** `tcp_timers` (Impact: 46.5), `tcp_slowtimo` (Impact: 13.2), `tcp_fasttimo` (Impact: 6.0)
...
- **Heaviest Functions:** `in_pcbconnect` (Impact: 61.3), `in_pcblookup` (Impact: 55.2), `in_pcbnotify` (Impact: 50.6)
...
- **Heaviest Functions:** `igmp_input` (Impact: 42.8), `igmp_fasttimo` (Impact: 8.4), `igmp_joingroup` (Impact: 6.5)
...
- **Heaviest Functions:** `udp_usrreq` (Impact: 92.5), `udp_input` (Impact: 81.0), `udp_output` (Impact: 28.9)
...
- **Heaviest Functions:** `tcp_input` (Impact: 464.7), `tcp_mss` (Impact: 53.5), `tcp_dooptions` (Impact: 51.5)
...
- **Heaviest Functions:** `ip_forward` (Impact: 67.8), `ip_dooptions` (Impact: 63.1), `ipintr` (Impact: 62.9)
...
- **Heaviest Functions:** `rip_usrreq` (Impact: 110.7), `rip_ctloutput` (Impact: 92.1), `rip_input` (Impact: 24.8)
...
- **Heaviest Functions:** `tcp_usrreq` (Impact: 155.8), `tcp_ctloutput` (Impact: 67.6), `tcp_usrclosed` (Impact: 15.5)
...
- **Heaviest Functions:** `tcp_close` (Impact: 36.0), `tcp_respond` (Impact: 24.3), `tcp_notify` (Impact: 18.7)
...
- **Heaviest Functions:** `arpresolve` (Impact: 47.3), `arp_rtrequest` (Impact: 43.2), `in_arpinput` (Impact: 37.5)
...
**Top Internal Functions/Classes:**
  ...
  * `tcp_dooptions` (Impact: 51.5)
  * `tcp_reass` (Impact: 45.1)
    * *Intent:* #ifndef TUBA_INCLUDE
  * `tcp_xmit_timer` (Impact: 13.8)
...
**Top Internal Functions/Classes:**
  * `udp_usrreq` (Impact: 92.5)
    * *Intent:* /* 40 1K datagrams */ /*ARGSUSED*/
  * `udp_input` (Impact: 81.0)
  * `udp_output` (Impact: 28.9)
  * `udp_ctlinput` (Impact: 13.0)
  * `udp_saveopt` (Impact: 5.0)
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `VOP_IOCTL` (Impact: 3.6)
  * `VOP_RENAME` (Impact: 3.6)
  * `VOP_SELECT` (Impact: 3.3)
  * `VOP_SYMLINK` (Impact: 3.3)
  * `VOP_ADVLOCK` (Impact: 3.3)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `igmp_input` (Impact: 42.8)
  * `igmp_fasttimo` (Impact: 8.4)
  * `igmp_joingroup` (Impact: 6.5)
  * `igmp_sendreport` (Impact: 5.7)
  * `igmp_leavegroup` (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
