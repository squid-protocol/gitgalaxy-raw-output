...
| Analyzed Artifacts (Scanned) | 4053 |
...
|---|---|---|
| Modularity | 0.5062 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 3.3954 | Hops between files. Lower = Tighter coupling. |
...
| PLAINTEXT | 260 | 0 | 6.4% |
| HTML | 254 | 19230 | 6.3% |
| MAKEFILE | 161 | 7047 | 4.0% |
| ASSEMBLY | 93 | 16259 | 2.3% |
| PYTHON | 65 | 24262 | 1.6% |
| XML | 46 | 0 | 1.1% |
| OBJECTIVE-C | 25 | 7873 | 0.6% |
| SHELL | 21 | 2637 | 0.5% |
| MARKDOWN | 15 | 0 | 0.4% |
| LUA | 7 | 950 | 0.2% |
| PERL | 3 | 559 | 0.1% |
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 99.6 | 21.7 | 5.5 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 39.1 | 0.0 | 0.0 |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 19.3 | 4.8 | 0.0 |
...
|---|---|---|---|---|
| memory | 331314 | 2417 | 163 | `EXTERNAL_HEADERS/AvailabilityInternal.h` |
| cleanup | 1156 | 310 | 0 | `libkern/c++/OSKext.cpp` |
| guards | 45610 | 1975 | 31 | `osfmk/arm/pmap.c` |
| danger | 11445 | 1041 | 6 | `tests/kqueue_file_tests.c` |
| concurrency | 2331 | 291 | 0 | `osfmk/tests/kernel_tests.c` |
| connectivity | 43784 | 2510 | 28 | `osfmk/arm/pmap.c` |
...
| mutation | 302973 | 2341 | 171 | `EXTERNAL_HEADERS/AvailabilityInternal.h` |
| dead_code | 13885 | 1430 | 8 | `bsd/net/if.c` |
| credential | 2 | 2 | 0 | `tests/poll_select_kevent_paired_fds.c` |
| threat | 17960 | 1290 | 10 | `bsd/vfs/vfs_syscalls.c` |
...
- `pf_test_rule` (@ `bsd/net/pf.c`) -> Impact: **1414.0** | LOC: 1191
- `m_clalloc` (@ `bsd/kern/uipc_mbuf.c`) -> Impact: **1254.7** | LOC: 2853
  * *Intent:* /* * Allocate some number of mbuf clusters and place on cluster freelist. */
- `dtrace_disx86` (@ `bsd/dev/i386/dis_tables.c`) -> Impact: **1214.1** | LOC: 2458
...
- `vm_fault_internal` (@ `osfmk/vm/vm_fault.c`) -> Impact: **1187.5** | LOC: 2121
- `vm_map_enter` (@ `osfmk/vm/vm_map.c`) -> Impact: **1129.0** | LOC: 1178
- `ip_output_list` (@ `bsd/netinet/ip_output.c`) -> Impact: **1055.0** | LOC: 1528
...
- `tcp_input` (@ `bsd/netinet/tcp_input.c`) -> Impact: **1038.5** | LOC: 2548
- `sys_fcntl_nocancel` (@ `bsd/kern/kern_descrip.c`) -> Impact: **1024.9** | LOC: 2218
...
- `dtrace_dif_subr` (@ `bsd/dev/dtrace/dtrace.c`) -> Impact: **1022.3** | LOC: 1553
...
- `ip6_output_list` (@ `bsd/netinet6/ip6_output.c`) -> Impact: **985.8** | LOC: 1415
...
| `bsd/net` | 152 | 99386.68 | 30.83% | 19.77% |
| `osfmk/kern` | 203 | 72931.91 | 30.52% | 28.8% |
...
- `iokit/conf/Makefile.arm` -> **100.0%** Exposure
...
### 1. `osfmk/arm/commpage/commpage.c` (C) -> Cumulative Risk: **737.2**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 364.12 | **LOC:** 643 | **CtrlFlow:** 16.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9996%), Concurrency (97.0656%)
- **Heaviest Functions:** `commpage_init_cpu_capabilities` (Impact: 23.4), `commpage_populate` (Impact: 17.6), `commpage_is_in_pfz64` (Impact: 9.2)

### 2. `bsd/security/audit/audit_bsd.c` (C) -> Cumulative Risk: **735.34**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 437.42 | **LOC:** 669 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9995%), Tech Debt (97.1549%)
- **Heaviest Functions:** `_audit_ppsratecheck` (Impact: 21.8), `audit_sysctl_malloc_debug` (Impact: 21.2), `_audit_malloc` (Impact: 19.8)

### 3. `osfmk/kern/kern_monotonic.c` (C) -> Cumulative Risk: **703.94**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 354.14 | **LOC:** 579 | **CtrlFlow:** 12.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9999%), Tech Debt (97.5909%), Concurrency (96.2411%)
- **Heaviest Functions:** `mt_fixed_task_counts` (Impact: 20.4), `mt_fixed_thread_counts` (Impact: 15.1), `mt_mtc_update_fixed_counts` (Impact: 11.3)

### 4. `osfmk/i386/commpage/commpage.c` (C) -> Cumulative Risk: **689.34**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 706.54 | **LOC:** 1019 | **CtrlFlow:** 11.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (97.5573%)
- **Heaviest Functions:** `commpage_init_cpu_capabilities` (Impact: 27.6), `commpage_allocate` (Impact: 25.1), `commpage_populate_one` (Impact: 18.1)

### 5. `bsd/security/audit/audit_syscalls.c` (C) -> Cumulative Risk: **682.57**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1160.84 | **LOC:** 1314 | **CtrlFlow:** 26.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%), Cognitive Load (99.615%)
- **Heaviest Functions:** `auditon` (Impact: 414.9), `audit` (Impact: 68.9), `setaudit_addr` (Impact: 27.2)

### 6. `bsd/dev/arm64/dtrace_isa.c` (C) -> Cumulative Risk: **681.26**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 550.22 | **LOC:** 706 | **CtrlFlow:** 19.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.4196%)
- **Heaviest Functions:** `dtrace_getpcstack` (Impact: 87.3), `dtrace_getufpstack` (Impact: 39.6), `dtrace_getarg` (Impact: 28.6)

### 7. `bsd/kern/kern_prot.c` (C) -> Cumulative Risk: **679.9**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1014.12 | **LOC:** 2165 | **CtrlFlow:** 18.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.9964%)
- **Heaviest Functions:** `setreuid` (Impact: 56.5), `setregid` (Impact: 49.4), `setpgid` (Impact: 43.8)

### 8. `bsd/vfs/kpi_vfs.c` (C) -> Cumulative Risk: **679.49**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 4130.38 | **LOC:** 6221 | **CtrlFlow:** 16.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%), Tech Debt (99.459%)
- **Heaviest Functions:** `vn_rename` (Impact: 205.7), `vnode_getattr` (Impact: 92.0), `vnode_setattr` (Impact: 81.4)

### 9. `bsd/kern/kern_fork.c` (C) -> Cumulative Risk: **679.42**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 803.36 | **LOC:** 1755 | **CtrlFlow:** 17.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (97.5944%)
- **Heaviest Functions:** `fork1` (Impact: 84.7), `forkproc` (Impact: 65.7), `uthread_alloc` (Impact: 46.0)

### 10. `osfmk/kdp/ml/x86_64/kdp_machdep.c` (C) -> Cumulative Risk: **679.41**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 566.64 | **LOC:** 655 | **CtrlFlow:** 13.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (98.3858%)
- **Heaviest Functions:** `machine_trace_thread64` (Impact: 61.4), `kdp_i386_trap` (Impact: 50.1), `machine_trace_thread` (Impact: 43.9)
...
- **Risk Profile:** Cognitive Load (92.2372%), Tech Debt (23.6054%)
**Top Internal Functions/Classes:**
  * `vm_map_enter` (Impact: 1129.0)
  * `vm_map_lookup_locked` (Impact: 571.4)
    * *Intent:* * If contended != NULL, *contended will be set to * true iff the thread had to spin or block to acqu...
  * `vm_map_enter_mem_object_helper` (Impact: 570.6)
  * `vm_map_remap_extract` (Impact: 437.6)
    * *Intent:* /* * Routine: vm_map_remap_extract * * Description: This routine returns a vm_entry list from a map....
  * `vm_map_enter_fourk` (Impact: 435.9)
...
### `EXTERNAL_HEADERS/AvailabilityInternal.h` (C | Tier 4 | AI Safe: 0.0%)
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 14635.52 | **LOC:** 23427 | **CtrlFlow:** 6.6% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (48.1128%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
* *Defense:* None
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 6.226
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (94.9043%), Tech Debt (12.8067%)
**Top Internal Functions/Classes:**
  * `pf_test_rule` (Impact: 1414.0)
  * `pf_test_state_icmp` (Impact: 496.9)
  * `pf_test_state_tcp` (Impact: 486.2)
  * `pf_test6` (Impact: 454.4)
  * `pf_test` (Impact: 407.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (74.4673%), Tech Debt (54.5816%)
**Top Internal Functions/Classes:**
  * `OSKext::load` (Impact: 796.9)
    * *Intent:* /********************************************************************* *****************************...
  * `OSKext::jettisonLinkeditSegment` (Impact: 468.4)
    * *Intent:* #endif /* VM_MAPPED_KEXTS */ /********************************************************************* ...
  * `OSKext::handleRequest` (Impact: 411.4)
    * *Intent:* /********************************************************************* * XXX - this function is a bi...
  * `OSKext::copyInfo` (Impact: 291.6)
    * *Intent:* /********************************************************************* * Any info that needs to do a...
  * `OSKext::mapKCFileSet` (Impact: 180.0)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 10525.2 | **LOC:** 15781 | **CtrlFlow:** 19.7% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (91.1936%), Tech Debt (8.4113%)
**Top Internal Functions/Classes:**
  * `pmap_enter_options_internal` (Impact: 392.5)
  * `pmap_page_protect_options_with_flush_range` (Impact: 272.0)
    * *Intent:* /* * Routine: pmap_page_protect_options * * Function: * Lower the permission for all mappings to a g...
  * `pmap_protect_options_internal` (Impact: 197.4)
  * `pmap_nest_internal` (Impact: 162.2)
    * *Intent:* #endif /* HAS_APPLE_PAC */ /* * kern_return_t pmap_nest(grand, subord, vstart, size) * * grand = the...
  * `arm_force_fast_fault_with_flush_range` (Impact: 145.9)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `safety_bypasses: 211`, `state_mutation: 1625`, `dead_code: 6`, `planned_debt: 7`, `fragile_debt: 6`
* *Architecture:* `api: 406`, `import: 53`
...
- **Risk Profile:** Cognitive Load (93.962%), Tech Debt (24.0515%)
**Top Internal Functions/Classes:**
  * `vm_object_iopl_request` (Impact: 557.7)
  * `upl_commit_range` (Impact: 550.7)
  * `vm_object_upl_request` (Impact: 539.2)
    * *Intent:* * the original object. If a page list structure * is not specified, this call is a no-op. * * Note: ...
  * `vm_map_create_upl` (Impact: 268.9)
  * `upl_abort_range` (Impact: 234.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (90.5865%), Tech Debt (66.5876%)
**Top Internal Functions/Classes:**
  * `IOPMrootDomain::evaluatePolicy` (Impact: 215.4)
    * *Intent:* //****************************************************************************** // evaluatePolicy /...
  * `IOPMrootDomain::evaluateSystemSleepPolicy` (Impact: 177.8)
  * `IOPMrootDomain::powerChangeDone` (Impact: 168.5)
    * *Intent:* //****************************************************************************** // powerChangeDone ...
  * `IOPMrootDomain::handleOurPowerChangeStart` (Impact: 150.7)
  * `IOPMrootDomain::setProperties` (Impact: 144.9)
...
- **Risk Profile:** Cognitive Load (94.8923%), Tech Debt (63.1534%)
**Top Internal Functions/Classes:**
  * `vnode_authattr` (Impact: 311.0)
    * *Intent:* /* * Check that the attribute information in vap can be legally written by the * context. * * Call t...
  * `vn_authorize_renamex_with_paths` (Impact: 229.7)
  * `vnode_authorize_callback_int` (Impact: 180.9)
  * `vnode_create_internal` (Impact: 167.6)
  * `vnode_authattr_new_internal` (Impact: 148.9)
...
- **Risk Profile:** Cognitive Load (94.585%), Tech Debt (20.8384%)
**Top Internal Functions/Classes:**
  * `m_clalloc` (Impact: 1254.7)
    * *Intent:* /* * Allocate some number of mbuf clusters and place on cluster freelist. */
  * `m_allocpacket_internal` (Impact: 248.8)
    * *Intent:* /* * Return list of mbuf linked by m_nextpkt. Try for numlist, and if * wantall is not set, return w...
  * `slab_free` (Impact: 130.8)
    * *Intent:* /* * Place a slab of object(s) back into a class's slab list. */
  * `m_copyback0` (Impact: 129.4)
  * `m_copym_with_hdrs` (Impact: 100.0)
...
- **Risk Profile:** Cognitive Load (73.0628%), Tech Debt (25.8113%)
**Top Internal Functions/Classes:**
  * `vm_page_find_contiguous` (Impact: 342.4)
    * *Intent:* #endif
  * `hibernate_page_list_setall` (Impact: 258.8)
    * *Intent:* /* * Bits zero in the bitmaps => page needs to be saved. All pages default to be saved, * pages know...
  * `vm_page_insert_internal` (Impact: 208.5)
  * `vm_page_free_list` (Impact: 131.9)
    * *Intent:* /* * Free a list of pages. The list can be up to several hundred pages, * as blocked up by vm_pageou...
  * `vm_page_do_delayed_work` (Impact: 101.6)
...
- **Risk Profile:** Cognitive Load (72.257%), Tech Debt (30.4095%)
**Top Internal Functions/Classes:**
  * `dlil_output` (Impact: 316.0)
    * *Intent:* * Caller should have a lock on the protocol domain if the protocol * doesn't support finer grained l...
  * `dlil_input_packet_list_common` (Impact: 181.2)
  * `ifnet_attach` (Impact: 131.6)
  * `ifnet_enqueue_ifclassq` (Impact: 108.4)
  * `dlil_rxpoll_input_thread_cont` (Impact: 102.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (97.2973%), Tech Debt (19.7161%)
**Top Internal Functions/Classes:**
  * `soreceive` (Impact: 540.4)
    * *Intent:* * sbwait:EBADF * sbwait:EINTR * sodelayed_copy:EFAULT * <pru_rcvoob>:EINVAL[TCP] * <pru_rcvoob>:EWOU...
  * `sosetoptlock` (Impact: 474.6)
...
- **Risk Profile:** Cognitive Load (77.179%), Tech Debt (12.7158%)
**Top Internal Functions/Classes:**
  * `cluster_io` (Impact: 531.9)
  * `cluster_read_copy` (Impact: 363.5)
  * `cluster_read_direct` (Impact: 295.9)
  * `cluster_write_copy` (Impact: 290.0)
  * `cluster_write_direct` (Impact: 209.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (77.7122%), Tech Debt (19.4321%)
**Top Internal Functions/Classes:**
  * `memorystatus_cmd_get_jetsam_snapshot` (Impact: 751.4)
  * `memorystatus_update` (Impact: 192.1)
    * *Intent:* * memlimit_active_is_fatal When a process is active and exceeds its memory footprint, * this describ...
  * `memorystatus_update_priority_locked` (Impact: 168.2)
    * *Intent:* * - if the 'jetsam aging policy' is NOT 'legacy': * When this flag is TRUE, it means we are going * ...
  * `memorystatus_dirty_set` (Impact: 136.8)
  * `memorystatus_control` (Impact: 130.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (91.102%), Tech Debt (81.4664%)
**Top Internal Functions/Classes:**
  * `IOService::matchInternal` (Impact: 181.4)
  * `IOService::probeCandidates` (Impact: 178.3)
    * *Intent:* /* * Alloc and probe matching classes, * called on the provider instance */
  * `IOService::updateConsoleUsers` (Impact: 103.3)
  * `IOService::copyExistingServices` (Impact: 99.0)
    * *Intent:* // internal - call with gNotificationLock
  * `IOService::matchPassive` (Impact: 80.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (93.1599%), Tech Debt (11.1931%)
**Top Internal Functions/Classes:**
  * `vm_fault_internal` (Impact: 1187.5)
  * `vm_fault_page` (Impact: 831.4)
  * `vm_fault_cs_handle_violation` (Impact: 214.8)
    * *Intent:* /* * Handles a code signing violation by either rejecting the page or forcing a disconnect. * @param...
  * `vm_fault_copy` (Impact: 175.5)
    * *Intent:* * destination map. * * Results: * Returns KERN_SUCCESS if no errors were encountered in * reading or...
  * `vm_fault_enqueue_page` (Impact: 123.0)
...
- **Risk Profile:** Cognitive Load (93.2104%), Tech Debt (67.4254%)
**Top Internal Functions/Classes:**
  * `IOGeneralMemoryDescriptor::initWithOptions` (Impact: 320.2)
    * *Intent:* /* * initWithOptions: * * IOMemoryDescriptor. The buffer is made up of several virtual address range...
  * `IOGeneralMemoryDescriptor::dmaCommandOperation` (Impact: 229.1)
  * `IOGeneralMemoryDescriptor::memoryReferenceMapNew` (Impact: 174.9)
    * *Intent:* #define LOGUNALIGN 0
  * `IOGeneralMemoryDescriptor::memoryReferenceMap` (Impact: 171.5)
  * `IOGeneralMemoryDescriptor::wireVirtual` (Impact: 143.8)
**Contextual Mitigations & Amplifications:**
...
- `osfmk/i386/cpu_data.h` -> **Severity: 0.255** (Bridge: 0.0026 * Flux: 99.8948%)
- `osfmk/kern/thread.h` -> **Severity: 0.109** (Bridge: 0.0126 * Flux: 8.6515%)
- `libkern/libkern/c++/OSLib.h` -> **Severity: 0.107** (Bridge: 0.0022 * Flux: 48.8237%)
- `osfmk/arm64/monotonic.h` -> **Severity: 0.088** (Bridge: 0.0009 * Flux: 94.0548%)
- `iokit/IOKit/IOLocks.h` -> **Severity: 0.069** (Bridge: 0.0026 * Flux: 26.4278%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `bsd/sys/types.h` -> **Severity: 963.9** (Blast Radius: 9.639 * Doc Risk: 100.0%)
- `libkern/libkern/c++/intrusive_shared_ptr.h` -> **Severity: 416.0** (Blast Radius: 4.16 * Doc Risk: 100.0%)
- `osfmk/mach/port.h` -> **Severity: 402.8** (Blast Radius: 4.028 * Doc Risk: 100.0%)
- `osfmk/mach/message.h` -> **Severity: 355.0** (Blast Radius: 3.55 * Doc Risk: 100.0%)
- `libkern/libkern/c++/bounded_ptr.h` -> **Severity: 335.1** (Blast Radius: 3.351 * Doc Risk: 100.0%)
