...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 14.7 | 10.7 | 0.0 |
...
- `process_function` (@ `curve25519-dalek-derive/src/lib.rs`) -> Impact: **48.9** | LOC: 177
- `process_mod` (@ `curve25519-dalek-derive/src/lib.rs`) -> Impact: **31.9** | LOC: 77
- `verify_batch` (@ `ed25519-dalek/src/batch.rs`) -> Impact: **25.4** | LOC: 107
...
- `mul` (@ `curve25519-dalek/src/backend/serial/scalar_mul/vartime_double_base.rs`) -> Impact: **22.5** | LOC: 50
  * *Intent:* /// Compute \\(aA + bB\\) in variable time, where \\(B\\) is the Ed25519 basepoint.
- `optional_multiscalar_mul` (@ `curve25519-dalek/src/backend/vector/scalar_mul/pippenger.rs`) -> Impact: **22.1** | LOC: 95
- `optional_multiscalar_mul` (@ `curve25519-dalek/src/backend/serial/scalar_mul/pippenger.rs`) -> Impact: **22.0** | LOC: 94
- `verify_prehashed_strict` (@ `ed25519-dalek/src/verifying.rs`) -> Impact: **19.8** | LOC: 38
...
- `optional_mixed_multiscalar_mul` (@ `curve25519-dalek/src/backend/serial/scalar_mul/precomputed_straus.rs`) -> Impact: **19.2** | LOC: 71
- `optional_mixed_multiscalar_mul` (@ `curve25519-dalek/src/backend/vector/scalar_mul/precomputed_straus.rs`) -> Impact: **19.1** | LOC: 69
...
- **Heaviest Functions:** `to_bytes` (Impact: 9.7), `reduce` (Impact: 7.0), `mul` (Impact: 6.8)
...
- **Heaviest Functions:** `pow2k` (Impact: 12.3), `mul` (Impact: 6.7), `to_bytes` (Impact: 5.6)
...
- **Heaviest Functions:** `deserialize` (Impact: 11.5), `visit_seq` (Impact: 11.1), `deserialize` (Impact: 10.0)

### 4. `ed25519-dalek/src/signing.rs` (RUST) -> Cumulative Risk: **493.52**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 176.1 | **LOC:** 978 | **CtrlFlow:** 9.5% | **Authorship Centralization:** 75.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (97.4861%), Verification (80.0%), Dead Code (66.9116%)
- **Heaviest Functions:** `deserialize` (Impact: 12.2), `visit_seq` (Impact: 11.7), `raw_sign_prehashed` (Impact: 7.6)

### 5. `curve25519-dalek/src/backend/vector/avx2/edwards.rs` (RUST) -> Cumulative Risk: **487.74**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `serial_add` (Impact: 5.0), `double` (Impact: 4.4), `mul_by_pow_2` (Impact: 3.8)
...
- **Heaviest Functions:** `process_function` (Impact: 48.9), `process_mod` (Impact: 31.9), `parse` (Impact: 9.2)
...
- **Heaviest Functions:** `mul` (Impact: 8.7), `square` (Impact: 7.1), `iterated_mul_matches_serial` (Impact: 5.2)
...
- **Heaviest Functions:** `mul_by_pow_2` (Impact: 3.8), `double` (Impact: 3.4), `from` (Impact: 3.3)
...
- **Heaviest Functions:** `map_pos_felem_to_curve_inverse` (Impact: 13.8), `elligator_inv` (Impact: 12.2), `lizard_decode` (Impact: 8.5)
...
- **Heaviest Functions:** `optional_mixed_multiscalar_mul` (Impact: 19.1), `new` (Impact: 2.0), `len` (Impact: 1.6)
...
**Top Internal Functions/Classes:**
  * `deserialize` (Impact: 11.5)
  * `visit_seq` (Impact: 11.1)
  * `deserialize` (Impact: 10.0)
  * `visit_seq` (Impact: 9.3)
  * `try_from_rng` (Impact: 9.0)
...
**Top Internal Functions/Classes:**
  ...
  * `deserialize` (Impact: 10.0)
  * `visit_seq` (Impact: 9.3)
  * `double_and_compress_batch` (Impact: 8.1)
...
**Top Internal Functions/Classes:**
  * `mul` (Impact: 8.7)
  * `square` (Impact: 7.1)
  * `iterated_mul_matches_serial` (Impact: 5.2)
  * `iterated_u32_mul_matches_serial` (Impact: 5.2)
  * `blend_lanes` (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `from_bytes_wide` (Impact: 5.7)
    * *Intent:* /// Reduce a 64 byte / 512 bit scalar mod l
  * `from_bytes` (Impact: 5.3)
    * *Intent:* /// Unpack a 32 byte / 256 bit scalar into 5 52-bit limbs.
  * `add` (Impact: 4.2)
    * *Intent:* /// Compute `a + b` (mod l)
  * `sub` (Impact: 4.2)
    * *Intent:* /// Compute `a - b` (mod l)
  * `conditional_add_l` (Impact: 4.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 10 instances
...
**Top Internal Functions/Classes:**
  * `process_function` (Impact: 48.9)
  * `process_mod` (Impact: 31.9)
  * `parse` (Impact: 9.2)
  * `process_item` (Impact: 8.7)
  * `unsafe_target_feature_specialize` (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `vartime_precomputed_helper` (Impact: 9.2)
  * `scalar_arith` (Impact: 8.5)
  * `vartime_precomputed_pure_static` (Impact: 7.0)
  * `vartime_multiscalar_mul` (Impact: 6.8)
  * `consttime_multiscalar_mul` (Impact: 6.7)
**Structural Signatures (Net Mitigated Signals):**
...
- `x25519-dalek/src/x25519.rs` -> **Tony Arcieri** (100.0% isolated ownership) | Magnitude: 81.06
- `curve25519-dalek/src/backend/serial/curve_models.rs` -> **Tony Arcieri** (100.0% isolated ownership) | Magnitude: 78.98
