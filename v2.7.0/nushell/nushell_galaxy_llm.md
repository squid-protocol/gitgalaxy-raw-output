...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 39.1 | 28.9 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 19.3 | 2.4 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 9.8 | 3.1 | 0.0 |
...
| connectivity | 7917 | 1294 | 11 | `crates/nu-protocol/src/value/mod.rs` |
...
| dead_code | 9324 | 1252 | 11 | `crates/nu-path/src/path.rs` |
| credential | 6 | 6 | 0 | `crates/nu-command/tests/commands/base/base32.rs` |
...
- `compile_expression` (@ `crates/nu-engine/src/compile/expression.rs`) -> Impact: **306.4** | LOC: 571
- `eval_instruction` (@ `crates/nu-engine/src/eval_ir.rs`) -> Impact: **301.4** | LOC: 640
  * *Intent:* /// Perform an instruction
- `series_to_values` (@ `crates/nu_plugin_polars/src/dataframe/values/nu_dataframe/conversion.rs`) -> Impact: **251.8** | LOC: 520
- `parse_signature_helper` (@ `crates/nu-parser/src/parser.rs`) -> Impact: **237.2** | LOC: 704
- `rm` (@ `crates/nu-command/src/filesystem/rm.rs`) -> Impact: **215.8** | LOC: 395
- `lex_item` (@ `crates/nu-parser/src/lex.rs`) -> Impact: **200.1** | LOC: 297
- `discover_captures_in_expr` (@ `crates/nu-parser/src/parser.rs`) -> Impact: **198.3** | LOC: 292
  * *Intent:* // Closes over captured variables
- `compile_try` (@ `crates/nu-engine/src/compile/keyword.rs`) -> Impact: **194.5** | LOC: 292
  * *Intent:* /// Compile a call to `try`, setting an error handler over the evaluated block
- `parse_internal_call` (@ `crates/nu-parser/src/parser.rs`) -> Impact: **180.5** | LOC: 377
- `run` (@ `crates/nu-command/src/filters/last.rs`) -> Impact: **174.4** | LOC: 255

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Complexity Load | Avg Debt Markers |
|---|---|---|---|---|
| `crates/nu-parser/src` | 12 | 8007.6 | 29.03% | 22.46% |
| `crates/nu-command/src/filters` | 53 | 4641.24 | 11.32% | 55.0% |
...
- **Heaviest Functions:** `visit_map` (Impact: 9.5), `from_raw_cols_vals` (Impact: 7.5), `serialize` (Impact: 7.4)
...
- **Heaviest Functions:** `remove_impl` (Impact: 26.5), `run` (Impact: 18.8), `remove` (Impact: 4.8)
...
- **Heaviest Functions:** `get_file_type` (Impact: 21.1), `run` (Impact: 8.4), `run_const` (Impact: 7.7)
...
- **Heaviest Functions:** `colorize_space_one` (Impact: 11.7), `clean_charset` (Impact: 7.6), `string_wrap` (Impact: 6.6)
...
- **Heaviest Functions:** `search_all_index` (Impact: 22.0), `run` (Impact: 13.1), `index_of_impl` (Impact: 11.2)
...
- **Heaviest Functions:** `lite_parse` (Impact: 68.8), `try_add_redirection` (Impact: 10.8), `last_non_comment_token` (Impact: 5.8)
...
- **Heaviest Functions:** `help_modules` (Impact: 58.2), `examples` (Impact: 5.2), `extra_description` (Impact: 4.5)
...
- **Heaviest Functions:** `run` (Impact: 110.9), `read` (Impact: 22.0), `next` (Impact: 8.7)
...
- **Heaviest Functions:** `recv_timeout` (Impact: 17.5), `try_recv` (Impact: 14.8), `pop_oldest` (Impact: 9.4)
...
- **Heaviest Functions:** `run` (Impact: 174.4), `signature` (Impact: 3.9), `examples` (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `parse_signature_helper` (Impact: 237.2)
  * `discover_captures_in_expr` (Impact: 198.3)
    * *Intent:* // Closes over captured variables
  * `parse_internal_call` (Impact: 180.5)
  * `parse_record` (Impact: 108.2)
  * `parse_string_interpolation` (Impact: 100.6)
...
**Top Internal Functions/Classes:**
  ...
  * `get_value_member` (Impact: 83.8)
  * `insert_data_at_cell_path` (Impact: 75.4)
  * `div` (Impact: 75.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `parse_alias` (Impact: 100.8)
  * `parse_def_inner` (Impact: 91.3)
    * *Intent:* // Returns also the parsed command name and ID
  * `parse_overlay_use` (Impact: 79.4)
  * `parse_export_in_module` (Impact: 78.4)
    * *Intent:* // This one will trigger only in a module
  * `parse_hide` (Impact: 75.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `eval_instruction` (Impact: 301.4)
    * *Intent:* /// Perform an instruction
  * `binary_op` (Impact: 107.0)
  * `gather_arguments` (Impact: 71.5)
    * *Intent:* /// Move arguments from the stack into variables for a custom command
  * `literal_value` (Impact: 40.0)
  * `eval_ir_block_impl` (Impact: 30.6)
...
**Top Internal Functions/Classes:**
  * `compile_try` (Impact: 194.5)
    * *Intent:* /// Compile a call to `try`, setting an error handler over the evaluated block
  * `compile_match` (Impact: 97.5)
    * *Intent:* /// Compile a call to `match`
  * `compile_if` (Impact: 77.8)
    * *Intent:* /// Compile a call to `if` as a branch-if
  * `compile_for` (Impact: 74.1)
    * *Intent:* /// Compile a call to `for` (via `iterate`)
  * `compile_while` (Impact: 62.0)
...
**Top Internal Functions/Classes:**
  * `edit_from_record` (Impact: 133.5)
  * `add_ide_menu` (Impact: 123.7)
    * *Intent:* // Adds an IDE menu to the line editor
  * `add_description_menu` (Impact: 40.9)
    * *Intent:* // Adds a description menu to the line editor
  * `add_parsed_keybinding` (Impact: 39.7)
  * `add_columnar_menu` (Impact: 38.3)
...
**Top Internal Functions/Classes:**
  * `ls_for_one_pattern` (Impact: 162.2)
  * `dir_entry_dict` (Impact: 158.0)
  * `run` (Impact: 45.6)
  * `get_file_type` (Impact: 39.9)
  * `dir_entry_dict_windows_fallback` (Impact: 28.9)
...
**Top Internal Functions/Classes:**
  * `render_ui` (Impact: 64.8)
  * `search_input_key_event` (Impact: 34.2)
  * `handle_general_key_events2` (Impact: 32.9)
  * `render_cmd_bar_cmd` (Impact: 30.1)
...
**Top Internal Functions/Classes:**
  * `test_int` (Impact: 13.2)
  * `multi_test_parse_int` (Impact: 13.0)
  * `parse_variable_range` (Impact: 12.9)
  * `parse_bounded_range` (Impact: 12.7)
  * `parse_cell_path_optional` (Impact: 12.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `series_to_values` (Impact: 251.8)
  * `typed_column_to_series` (Impact: 88.5)
  * `input_type_list_to_series` (Impact: 41.9)
  * `test_any_value_to_value` (Impact: 30.9)
  * `insert_value` (Impact: 24.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `prepared_statement_to_nu_list` (Impact: 25.5)
  * `split_select_expressions` (Impact: 19.2)
    * *Intent:* /// Splits a SELECT projection list on top-level commas. /// /// We only split commas that are outsi...
  * `nu_value_to_params` (Impact: 16.1)
  * `read_entire_sqlite_db` (Impact: 15.3)
  * `convert_sqlite_value_to_nu_value_with_adapter` (Impact: 14.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `lex_item` (Impact: 200.1)
  * `lex_internal` (Impact: 113.7)
  * `lex_raw_string` (Impact: 21.0)
  * `is_item_terminator` (Impact: 20.9)
    * *Intent:* // A baseline token is terminated if it's not nested inside of a paired // delimiter and the next ch...
  * `lex_n_tokens` (Impact: 6.2)
...
**Top Internal Functions/Classes:**
  * `transform_response_using_content_type` (Impact: 31.0)
  * `send_form_request` (Impact: 29.5)
  * `send_request` (Impact: 29.0)
    * *Intent:* // remove once all commands have been migrated
  * `request_add_custom_headers` (Impact: 25.6)
  * `send_json_request` (Impact: 24.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parse_validated_option` (Impact: 14.8)
...
**Top Internal Functions/Classes:**
  * `handle_row_stream` (Impact: 45.9)
  * `handle_record` (Impact: 30.4)
  * `get_cli_args` (Impact: 27.6)
  * `stream_collect_abbreviated` (Impact: 26.7)
  * `render_path_name` (Impact: 25.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `generic_copy` (Impact: 21.4)
    * *Intent:* // Copied from [`std::io::copy`]
  * `next` (Impact: 20.9)
  * `copy_with_signals` (Impact: 19.8)
  * `slice` (Impact: 19.7)
  * `next_string` (Impact: 17.6)
**Contextual Mitigations & Amplifications:**
