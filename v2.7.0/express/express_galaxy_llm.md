...
| Avg Path Length | 3.8488 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 74.3 | 4.8 | 0.0 | 0.0 |
...
- `send` (@ `lib/response.js`) -> Impact: **45.7** | LOC: 94
  * *Intent:* /** * Send a response. * * Examples: * * res.send(Buffer.from('wahoo')); * res.send({ some: 'json' }); * res.send('<p>some html</p>'); * * @param {str...
- `exports` (@ `examples/mvc/lib/boot.js`) -> Impact: **38.3** | LOC: 73
- `sendfile` (@ `lib/response.js`) -> Impact: **35.8** | LOC: 89
  * *Intent:* // pipe the send file stream
- `download` (@ `lib/response.js`) -> Impact: **33.8** | LOC: 50
...
- `use` (@ `lib/application.js`) -> Impact: **15.5** | LOC: 55
...
- **Heaviest Functions:** `send` (Impact: 45.7), `sendfile` (Impact: 35.8), `download` (Impact: 33.8)
...
- **Heaviest Functions:** `header` (Impact: 12.3), `host` (Impact: 6.7), `protocol` (Impact: 6.0)
...
- **Heaviest Functions:** `View` (Impact: 17.8), `resolve` (Impact: 9.6), `render` (Impact: 6.5)
...
- **Heaviest Functions:** `render` (Impact: 22.7), `use` (Impact: 15.5), `set` (Impact: 12.0)
...
- **Heaviest Functions:** `compileETag` (Impact: 13.9), `compileQueryParser` (Impact: 13.9), `acceptParams` (Impact: 12.9)
...
- **Heaviest Functions:** `load` (Impact: 6.5), `update` (Impact: 3.9), `view` (Impact: 2.0)
...
- **Heaviest Functions:** `exports` (Impact: 38.3)
...
- **Heaviest Functions:** `authenticate` (Impact: 10.7), `restrict` (Impact: 6.4)
...
- **Heaviest Functions:** `app` (Impact: 2.1), `createApplication` (Impact: 2.0)
...
**Top Internal Functions/Classes:**
  ...
  * `defaultConfiguration` (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `createApp` (Impact: 5.1)
  * `createManyParams` (Impact: 5.0)
  * `verify` (Impact: 4.3)
  * `verify` (Impact: 4.3)
  * `verify` (Impact: 4.2)
**Contextual Mitigations & Amplifications:**
