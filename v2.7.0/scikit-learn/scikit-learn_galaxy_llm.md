...
| Avg Path Length | 2.7908 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 13.5 | 7.4 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 3.6 | 0.6 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 8.5 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 46.1 | 43.0 | 0.0 |
...
| connectivity | 10581 | 790 | 23 | `sklearn/utils/tests/test_estimator_checks.py` |
...
| dead_code | 5458 | 457 | 14 | `sklearn/metrics/tests/test_classification.py` |
...
- `from_estimator` (@ `sklearn/inspection/_plot/partial_dependence.py`) -> Impact: **429.8** | LOC: 540
- `_lars_path_solver` (@ `sklearn/linear_model/_least_angle.py`) -> Impact: **417.1** | LOC: 503
- `check_array` (@ `sklearn/utils/validation.py`) -> Impact: **401.1** | LOC: 421
- `_logistic_regression_path` (@ `sklearn/linear_model/_logistic.py`) -> Impact: **311.7** | LOC: 460
- `_ridge_regression` (@ `sklearn/linear_model/_ridge.py`) -> Impact: **236.4** | LOC: 230
- `fit` (@ `sklearn/ensemble/_hist_gradient_boosting/gradient_boosting.py`) -> Impact: **231.8** | LOC: 563
- `fit` (@ `sklearn/linear_model/_logistic.py`) -> Impact: **231.4** | LOC: 415
...
- `partial_dependence` (@ `sklearn/inspection/_partial_dependence.py`) -> Impact: **226.0** | LOC: 409
- `plot` (@ `sklearn/inspection/_plot/partial_dependence.py`) -> Impact: **222.3** | LOC: 358
- `sparse_enet_coordinate_descent` (@ `sklearn/linear_model/_cd_fast.pyx`) -> Impact: **213.5** | LOC: 350
...
### 1. `asv_benchmarks/benchmarks/common.py` (PYTHON) -> Cumulative Risk: **732.56**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 230.38 | **LOC:** 257 | **CtrlFlow:** 13.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (99.4249%), Api Exposure (94.3995%)
- **Heaviest Functions:** `get_from_config` (Impact: 10.2), `get_estimator_path` (Impact: 7.4), `track_train_score` (Impact: 5.5)

### 2. `sklearn/conftest.py` (PYTHON) -> Cumulative Risk: **664.36**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 353.16 | **LOC:** 509 | **CtrlFlow:** 16.6% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.9479%), Safety Score (89.8605%)
- **Heaviest Functions:** `pytest_collection_modifyitems` (Impact: 58.1), `pytest_generate_tests` (Impact: 16.3), `pytest_configure` (Impact: 11.0)

### 3. `sklearn/tree/_splitter.pyx` (PYTHON) -> Cumulative Risk: **656.78**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 383.74 | **LOC:** 852 | **CtrlFlow:** 11.3% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.997%), Safety Score (87.3425%), Tech Debt (85.7868%)
- **Heaviest Functions:** `node_split_best` (Impact: 80.7), `node_split_random` (Impact: 67.8), `init` (Impact: 18.2)

### 4. `sklearn/utils/_testing.py` (PYTHON) -> Cumulative Risk: **647.17**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1042.92 | **LOC:** 1507 | **CtrlFlow:** 23.0% | **Authorship Centralization:** 25.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (92.9707%), Tech Debt (90.3026%)
- **Heaviest Functions:** `_convert_container` (Impact: 95.3), `check_docstring_parameters` (Impact: 63.2), `assert_docstring_consistency` (Impact: 54.4)

### 5. `sklearn/externals/array_api_compat/common/_aliases.py` (PYTHON) -> Cumulative Risk: **645.63**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 446.86 | **LOC:** 703 | **CtrlFlow:** 11.4% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (87.4496%)
- **Heaviest Functions:** `clip` (Impact: 37.5), `isdtype` (Impact: 33.6), `argsort` (Impact: 17.4)

### 6. `sklearn/utils/fixes.py` (PYTHON) -> Cumulative Risk: **638.9**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 457.86 | **LOC:** 544 | **CtrlFlow:** 28.9% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (96.5628%), Verification (80.0%)
- **Heaviest Functions:** `_safely_cast_index_arrays` (Impact: 52.9), `_smallest_admissible_index_dtype` (Impact: 33.4), `_sparse_min_or_max` (Impact: 24.9)

### 7. `asv_benchmarks/benchmarks/linear_model.py` (PYTHON) -> Cumulative Risk: **635.41**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 205.6 | **LOC:** 258 | **CtrlFlow:** 15.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `make_data` (Impact: 12.9), `make_estimator` (Impact: 5.9), `make_data` (Impact: 5.7)

### 8. `sklearn/utils/_metadata_requests.py` (PYTHON) -> Cumulative Risk: **632.7**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 818.0 | **LOC:** 1752 | **CtrlFlow:** 27.2% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (96.4066%), Safety Score (95.0031%)
- **Heaviest Functions:** `_route_params` (Impact: 47.9), `_get_class_level_metadata_request_values` (Impact: 27.4), `process_routing` (Impact: 27.0)

### 9. `sklearn/externals/_packaging/version.py` (PYTHON) -> Cumulative Risk: **629.78**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 392.58 | **LOC:** 536 | **CtrlFlow:** 29.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9999%), Documentation (96.1039%), Safety Score (89.4743%)
- **Heaviest Functions:** `_cmpkey` (Impact: 42.7), `__init__` (Impact: 34.3), `_parse_letter_version` (Impact: 17.3)

### 10. `build_tools/circle/build_doc.sh` (SHELL) -> Cumulative Risk: **628.92**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 172.98 | **LOC:** 284 | **CtrlFlow:** 20.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.7235%)
- **Heaviest Functions:** `get_build_type` (Impact: 20.5), `__global_context__` (Impact: 7.1), `Anonymous_Block` (Impact: 6.8)
...
- **Risk Profile:** Cognitive Load (67.1192%), Tech Debt (8.8783%)
**Top Internal Functions/Classes:**
  * `check_array_api_input` (Impact: 94.2)
  * `check_param_validation` (Impact: 64.5)
    * *Intent:* # Check that an informative error is raised when the value of a constructor # parameter does not hav...
  * `check_classifiers_train` (Impact: 60.5)
  * `check_dataframe_column_names_consistency` (Impact: 58.9)
  * `_check_transformer` (Impact: 51.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000253 | `Ripple Effect (Closeness):` 0.014482
...
- **Risk Profile:** Cognitive Load (26.6606%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_ridge_sample_weight_consistency` (Impact: 51.2)
  * `test_regularization_limits_ridge_classifier_gcv` (Impact: 33.9)
  * `test_ridge_regression_sample_weights` (Impact: 31.5)
  * `test_ridge_gcv_sample_weights` (Impact: 29.6)
  * `test_regularization_limits_ridge_gcv` (Impact: 25.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (57.7487%), Tech Debt (17.4563%)
**Top Internal Functions/Classes:**
  * `check_array` (Impact: 401.1)
  * `validate_data` (Impact: 80.2)
  * `check_scalar` (Impact: 75.1)
  * `_ensure_sparse_format` (Impact: 73.0)
  * `_assert_all_finite_element_wise` (Impact: 68.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002362 | `Ripple Effect (Closeness):` 0.309944
...
- **Risk Profile:** Cognitive Load (65.7211%), Tech Debt (14.023%)
**Top Internal Functions/Classes:**
  * `_ridge_regression` (Impact: 236.4)
  * `fit` (Impact: 117.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.003753
...
- **Risk Profile:** Cognitive Load (26.6564%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_standard_scaler_constant_features` (Impact: 26.1)
  * `test_normalize` (Impact: 17.7)
  ...
  * `test_quantile_transform_check_error` (Impact: 14.1)
  * `test_standard_scaler_dtype` (Impact: 13.5)
...
- **Risk Profile:** Cognitive Load (25.0008%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `check_min_weight_fraction_leaf` (Impact: 18.9)
    * *Intent:* """Test if leaves contain at least min_weight_fraction_leaf of the training set"""
  * `check_min_weight_fraction_leaf_with_min_samples_leaf` (Impact: 18.9)
  * `check_sparse_input` (Impact: 15.9)
  * `test_absolute_errors_precomputation_function` (Impact: 14.0)
    * *Intent:* """ Test the main bit of logic of the MAE(RegressionCriterion) class (used by DecisionTreeRegressor(...
  * `test_explicit_sparse_zeros` (Impact: 13.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000842
...
- **Risk Profile:** Cognitive Load (53.8713%), Tech Debt (8.806%)
**Top Internal Functions/Classes:**
  * `class_likelihood_ratios` (Impact: 127.3)
  * `classification_report` (Impact: 116.3)
  * `multilabel_confusion_matrix` (Impact: 85.3)
  * `confusion_matrix` (Impact: 78.4)
  * `_check_targets` (Impact: 43.3)
...
- **Risk Profile:** Cognitive Load (27.389%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_neighbors_metrics` (Impact: 49.1)
  * `test_neigh_predictions_algorithm_agnosticity` (Impact: 45.2)
  * `test_unsupervised_kneighbors` (Impact: 32.0)
  * `test_valid_brute_metric_for_auto_algorithm` (Impact: 16.8)
  * `test_radius_neighbors_regressor` (Impact: 16.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (37.5467%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `check_cv_results_array_types` (Impact: 21.1)
  * `test_grid_search_cv_results` (Impact: 18.4)
  * `compare_cv_results_multimetric_with_single` (Impact: 17.8)
    * *Intent:* """Compare multi-metric cv_results with the ensemble of multiple single metric cv_results from singl...
  * `test_search_cv_sample_weight_equivalence` (Impact: 17.3)
  * `test_grid_search_failing_classifier` (Impact: 14.9)
...
- **Risk Profile:** Cognitive Load (82.5021%), Tech Debt (9.2848%)
**Top Internal Functions/Classes:**
  * `_logistic_regression_path` (Impact: 311.7)
  * `fit` (Impact: 231.4)
  ...
  * `_log_reg_scoring_path` (Impact: 82.8)
    * *Intent:* # helper function for LogisticCV
  * `_check_solver` (Impact: 23.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (27.9222%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `check_sample_weight_invariance` (Impact: 44.3)
  * `check_array_api_metric` (Impact: 37.8)
  * `test_mixed_array_api_namespace_input_compliance` (Impact: 29.9)
  * `test_format_invariance_with_1d_vectors` (Impact: 23.2)
  * `test_array_api_classification_mixed_string_numeric_input` (Impact: 22.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (26.3909%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_logistic_regression_array_api_compliance` (Impact: 54.2)
  * `test_logistic_regression_solvers_multiclass_unpenalized` (Impact: 27.6)
  * `test_check_solver_option` (Impact: 26.2)
    * *Intent:* # TODO(1.11): remove filterwarnings with change of default scoring # TODO(1.10): remove filterwarnin...
  * `test_multinomial_cv_iris` (Impact: 25.2)
    * *Intent:* # TODO(1.12): remove deprecated use_legacy_attributes # Test that multinomial LogisticRegressionCV i...
  * `test_logistic_regression_solvers_multiclass` (Impact: 23.1)
...
- **Risk Profile:** Cognitive Load (36.4877%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `fit` (Impact: 34.3)
  * `check_cross_val_predict_multilabel` (Impact: 29.3)
    * *Intent:* """Check the output of cross_val_predict for 2D targets using Estimators which provide a predictions...
  * `test_cross_validate_failing_scorer` (Impact: 28.9)
  * `check_cross_validate_single_metric` (Impact: 25.2)
  * `check_cross_validate_multi_metric` (Impact: 20.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.002696
...
- **Risk Profile:** Cognitive Load (56.3096%), Tech Debt (10.8857%)
**Top Internal Functions/Classes:**
  * `_fit_and_score` (Impact: 207.9)
  * `learning_curve` (Impact: 99.1)
  * `cross_val_predict` (Impact: 92.2)
  * `cross_validate` (Impact: 65.3)
  * `_score` (Impact: 64.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.4e-05 | `Ripple Effect (Closeness):` 0.013368
...
- **Risk Profile:** Cognitive Load (47.0538%), Tech Debt (8.3188%)
**Top Internal Functions/Classes:**
  * `_multiplicative_update_w` (Impact: 88.4)
  * `_multiplicative_update_h` (Impact: 84.3)
  * `_fit_multiplicative_update` (Impact: 62.2)
  * `_check_w_h` (Impact: 49.5)
    * *Intent:* """Check W and H, or initialize them."""
  * `_minibatch_convergence` (Impact: 48.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 200 instances
...
- **Risk Profile:** Cognitive Load (60.818%), Tech Debt (17.6074%)
**Top Internal Functions/Classes:**
  * `sparse_enet_coordinate_descent` (Impact: 213.5)
  * `enet_coordinate_descent_multi_task` (Impact: 148.2)
  * `enet_coordinate_descent_gram` (Impact: 139.8)
  * `enet_coordinate_descent` (Impact: 131.0)
  * `R_plus_wj_Xj` (Impact: 32.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (59.1256%), Tech Debt (8.1768%)
**Top Internal Functions/Classes:**
  * `build` (Impact: 70.8)
  * `_add_split_node` (Impact: 56.4)
  * `build` (Impact: 50.7)
  * `_cost_complexity_prune` (Impact: 43.8)
  * `compute_partial_dependence` (Impact: 32.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (71.2596%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_check_array` (Impact: 35.5)
    * *Intent:* # accept_sparse == False # raise error on sparse inputs X = [[1, 2], [3, 4]] X_csr = sp.csr_array(X)...
  * `test_check_array_links_to_imputer_doc_only_for_X` (Impact: 17.5)
  * `test_check_array_min_samples_and_features_messages` (Impact: 17.1)
    * *Intent:* # empty list is considered 2D by default: msg = r"0 feature\(s\) \(shape=\(1, 0\)\) while a minimum ...
  * `test_check_array_dia_to_int32_indexed_csr_csc_coo` (Impact: 14.9)
    * *Intent:* """Check the consistency of the indices dtype with sparse matrices/arrays."""
  * `test_num_features_errors_1d_containers` (Impact: 14.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 151 instances
...
- **Risk Profile:** Cognitive Load (68.056%), Tech Debt (38.3563%)
**Top Internal Functions/Classes:**
  * `_allocate_parameter_mem` (Impact: 80.2)
  * `_fit` (Impact: 48.0)
  * `_fit` (Impact: 40.8)
  * `_fit` (Impact: 38.8)
  * `_partial_fit` (Impact: 35.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.00337
...
- **Risk Profile:** Cognitive Load (29.6271%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_pipeline_invalid_parameters` (Impact: 12.1)
    * *Intent:* # Test the various init parameters of the pipeline in fit # method pipeline = Pipeline([(1, 1)]) wit...
  * `test_metadata_routing_for_pipeline` (Impact: 10.2)
    * *Intent:* # split and partial_fit not relevant for pipelines """Test that metadata is routed correctly for pip...
  * `test_step_name_validation` (Impact: 9.8)
  * `test_feature_union_array_api_compliance` (Impact: 9.8)
    * *Intent:* """Test that FeatureUnion with Array API-compatible transformers works."""
  * `test_feature_union_metadata_routing_error` (Impact: 8.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.003009
...
- **Risk Profile:** Cognitive Load (28.6166%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_group_kfold` (Impact: 34.2)
  * `test_stratified_kfold_label_invariance` (Impact: 11.4)
    * *Intent:* # Check that stratified kfold gives the same indices regardless of labels n_samples = 100 y = np.arr...
  * `test_leave_one_p_group_out` (Impact: 10.7)
  * `test_kfold_valueerrors` (Impact: 10.6)
  * `test_array_api_train_test_split` (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
...
- `asv_benchmarks/benchmarks/datasets.py` -> **Severity: 1.02** (Bridge: 0.0102 * Flux: 99.9967%)
- `sklearn/base.py` -> **Severity: 0.835** (Bridge: 0.0083 * Flux: 100.0%)
- `sklearn/feature_extraction/text.py` -> **Severity: 0.787** (Bridge: 0.0079 * Flux: 100.0%)
- `sklearn/pipeline.py` -> **Severity: 0.547** (Bridge: 0.0055 * Flux: 100.0%)
- `sklearn/metrics/pairwise.py` -> **Severity: 0.428** (Bridge: 0.0043 * Flux: 100.0%)
