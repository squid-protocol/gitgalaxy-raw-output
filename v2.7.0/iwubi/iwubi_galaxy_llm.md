...
- `_match_hotkey` (@ `iwubi.py`) -> Impact: **12.4** | LOC: 25
...
- `__init__` (@ `iwubi.py`) -> Impact: **11.4** | LOC: 20
- `main` (@ `iwubi.py`) -> Impact: **9.7** | LOC: 33
- `_is_shift_hotkey` (@ `iwubi.py`) -> Impact: **9.4** | LOC: 15
...
- `set_lookup_table_cursor_pos_in_current_page` (@ `iwubi.py`) -> Impact: **5.9** | LOC: 15
  * *Intent:* '''Sets the cursor in the lookup table to index in the current page Returns True if successful, False if not. '''
- `update_candidates` (@ `iwubi.py`) -> Impact: **5.8** | LOC: 32
- `do_candidate_clicked` (@ `iwubi.py`) -> Impact: **4.6** | LOC: 3
- `bus_disconnected_cb` (@ `iwubi.py`) -> Impact: **3.8** | LOC: 6
- `__init__` (@ `iwubi.py`) -> Impact: **3.5** | LOC: 25
...
- **Heaviest Functions:** `_process_key_event` (Impact: 63.9), `_match_hotkey` (Impact: 12.4), `__init__` (Impact: 11.4)
...
- **Heaviest Functions:** `get_logger` (Impact: 2.5)
...
- **Heaviest Functions:** `uninstall` (Impact: 1.5), `install` (Impact: 1.4)
