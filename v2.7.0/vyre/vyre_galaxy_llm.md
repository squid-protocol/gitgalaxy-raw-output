...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 8.4 | 2.9 | 0.0 |
...
| connectivity | 23560 | 2470 | 17 | `vyre-driver-cuda/src/egraph_kernel_plan.rs` |
...
| dead_code | 16586 | 2866 | 11 | `vyre-driver-wgpu/src/backend_impl.rs` |
...
- `fill_upload_resident_many_repeated_sequence_read_ranges_borrowed_into` (@ `vyre-driver-cuda/src/backend/resident_dispatch.rs`) -> Impact: **396.9** | LOC: 475
...
- `flush_active_macro_segment_inner` (@ `vyre-libs/src/parsing/c/preprocess/gpu_pipeline/macro_expansion/flush.rs`) -> Impact: **343.0** | LOC: 574
- `emit_op` (@ `vyre-emit-ptx/src/emitter.rs`) -> Impact: **321.5** | LOC: 470
- `dispatch_resident_async_concrete_with_ptx_key` (@ `vyre-driver-cuda/src/backend/resident_dispatch.rs`) -> Impact: **313.5** | LOC: 432
- `run_semantic_requirement_checks` (@ `xtask/src/vyre_weir_release_gate.rs`) -> Impact: **300.6** | LOC: 1091
- `dispatch_borrowed_async_with_ptx_concrete` (@ `vyre-driver-cuda/src/backend/host_dispatch.rs`) -> Impact: **265.9** | LOC: 339
- `record_macro_token_provenance` (@ `vyre-libs/src/parsing/c/preprocess/gpu_pipeline/token_provenance/macro_record.rs`) -> Impact: **246.8** | LOC: 256
  * *Intent:* /// Records provenance for a segment after GPU macro materialization.
- `record_cuda_graph_borrowed` (@ `vyre-driver-cuda/src/backend/cuda_graph.rs`) -> Impact: **221.7** | LOC: 409
...
- **Heaviest Functions:** `bind_consecutive_results` (Impact: 11.1), `alloc_literal` (Impact: 8.0), `finish_with_return` (Impact: 4.9)
...
- **Heaviest Functions:** `Anonymous_Block_[Truncated]` (Impact: 24.3), `__global_context__` (Impact: 3.1), `Anonymous_Block` (Impact: 2.6)
...
- **Heaviest Functions:** `elementwise_add_f32_bytes_into` (Impact: 12.1), `matmul_f32_bytes` (Impact: 11.2), `attention_proxy_f32_bytes` (Impact: 10.9)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Stability (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `extract_block` (Impact: 14.9), `Anonymous_Block` (Impact: 11.9), `__global_context__` (Impact: 8.0)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Stability (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `Anonymous_Block` (Impact: 18.6), `__global_context__` (Impact: 7.8), `check_publish_field` (Impact: 5.8)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Stability (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `Anonymous_Block` (Impact: 8.6), `__global_context__` (Impact: 6.0), `Anonymous_Block` (Impact: 2.1)
...
- **Heaviest Functions:** `normalize_tokens` (Impact: 60.8), `has_control_flow_keyword` (Impact: 28.9), `collect_rust_files_recursive` (Impact: 28.4)
...
- **Heaviest Functions:** `Anonymous_Block` (Impact: 7.8), `__global_context__` (Impact: 7.3), `has_loud_abort` (Impact: 4.1)
...
- **Heaviest Functions:** `is_whitelisted` (Impact: 4.7), `Anonymous_Block` (Impact: 4.5), `__global_context__` (Impact: 4.0)
...
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Stability (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `push_atomic` (Impact: 23.4), `rename_expr_vars` (Impact: 22.2), `enter_expr_frame` (Impact: 13.4)
...
**Top Internal Functions/Classes:**
  ...
  * `inspect_pass_family_benchmark_manifest_semantics` (Impact: 96.3)
  * `inspect_backend_suite_semantics` (Impact: 95.2)
  * `inspect_release_workload_matrix_semantics` (Impact: 88.4)
  * `inspect_weir_matrix_semantics` (Impact: 81.8)
...
**Top Internal Functions/Classes:**
  * `run_semantic_requirement_checks` (Impact: 300.6)
  * `check_backend_suite_report` (Impact: 149.4)
  * `check_workload_matrix_artifact_coverage` (Impact: 97.7)
  * `check_parser_contract_evidence` (Impact: 70.7)
  * `check_single_benchmark_report` (Impact: 64.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `fill_upload_resident_many_repeated_sequence_read_ranges_borrowed_into` (Impact: 396.9)
  * `dispatch_resident_async_concrete_with_ptx_key` (Impact: 313.5)
  * `dispatch_resident_batch_async_concrete_with_ptx_key` (Impact: 207.4)
  * `dispatch_resident_via_borrowed_into` (Impact: 55.3)
  * `dispatch_resident_timed` (Impact: 34.0)
...
**Top Internal Functions/Classes:**
  * `run_egraph_structural_canonicalization_fixed_point_with_readback` (Impact: 101.6)
    * *Intent:* /// Iterate CUDA-resident structural canonicalization with explicit control /// over the final host ...
  * `plan_cuda_egraph_signature_buckets_from_column` (Impact: 74.9)
  * `plan_cuda_egraph_union_compaction` (Impact: 66.7)
    * *Intent:* /// Generate the concrete PTX kernel that compares packed e-graph rows inside /// one signature buck...
  * `run_egraph_canonical_rewrite_kernel_inner` (Impact: 59.2)
  * `run_egraph_structural_equivalence_kernel_inner` (Impact: 53.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `sym_calc_value` (Impact: 58.6)
  * `sym_string_valid` (Impact: 48.9)
  * `sym_set_string_value` (Impact: 40.5)
  * `sym_get_string_default` (Impact: 38.9)
    * *Intent:* /* * Find the default value associated to a symbol. * For tristate symbol handle the modules=n case ...
  * `sym_string_within_range` (Impact: 38.4)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `safety_bypasses: 2`, `state_mutation: 197`, `dead_code: 3`
...
**Top Internal Functions/Classes:**
  * `expr_join_and` (Impact: 148.4)
  * `expr_join_or` (Impact: 89.8)
    * *Intent:* /* * e1 || e2 -> ? */
  * `expr_transform` (Impact: 73.8)
    * *Intent:* * !(A<=B) -> A>B * !(A>=B) -> A<B * !(A<B) -> A>=B * !(A>B) -> A<=B * !(A || B) -> !A && !B * !(A &&...
  * `expr_print` (Impact: 73.5)
  * `expr_trans_compare` (Impact: 65.2)
...
**Top Internal Functions/Classes:**
  * `selected_conf` (Impact: 120.4)
  * `build_conf` (Impact: 83.5)
  * `conf_choice` (Impact: 71.0)
  * `do_match` (Impact: 40.3)
...
**Top Internal Functions/Classes:**
  * `inspect_backend_suite_artifact` (Impact: 105.4)
  * `inspect_optimization_benchmark_artifact` (Impact: 72.5)
  * `run` (Impact: 58.8)
  * `write_cpu_100x_proof` (Impact: 56.6)
  * `benchmark_artifact_is_reusable` (Impact: 37.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `is_release_evidence_valid` (Impact: 137.8)
  * `run_inner` (Impact: 84.0)
  * `parse_args` (Impact: 65.3)
  * `inspect_vyrecob2_sections` (Impact: 42.4)
  * `run_vyre_parser_file_range` (Impact: 38.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `conf_read_simple` (Impact: 72.5)
  * `conf_set_sym_val` (Impact: 52.3)
  * `conf_write` (Impact: 46.7)
  * `print_symbol_for_c` (Impact: 28.0)
  * `conf_touch_deps` (Impact: 27.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `merge_certificates` (Impact: 62.1)
  * `read_and_verify_shard` (Impact: 53.0)
  * `compare_backend_against_reference` (Impact: 37.8)
  * `prove` (Impact: 32.3)
  * `prepare_reference_cases` (Impact: 29.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `lower_node` (Impact: 131.1)
  * `lower_expr` (Impact: 80.6)
  * `walk` (Impact: 35.5)
  * `lower_child_node` (Impact: 29.3)
  * `collect_carrier_names` (Impact: 28.8)
...
**Top Internal Functions/Classes:**
  * `release_benchmark_csr_forward_baseline` (Impact: 34.3)
  * `callgraph_witness_digest` (Impact: 31.2)
  * `run` (Impact: 26.7)
  * `run` (Impact: 16.1)
  * `run_string_bitmap_scatter` (Impact: 15.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `main` (Impact: 155.9)
  * `conf_set_all_new_symbols` (Impact: 53.2)
  * `conf_sym` (Impact: 48.6)
  * `conf_choice` (Impact: 46.7)
  * `conf` (Impact: 31.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `set_node` (Impact: 62.9)
  * `on_treeview2_button_press_event` (Impact: 34.5)
    * *Intent:* /* User click: update choice (full) or goes down (single) */
  * `change_sym_value` (Impact: 29.3)
    * *Intent:* /* Change the value of a symbol and update the tree */
  * `main` (Impact: 26.8)
    * *Intent:* /* Main */
  * `on_treeview2_key_press_event` (Impact: 23.9)
...
**Top Internal Functions/Classes:**
  * `_menu_finalize` (Impact: 64.2)
  * `sym_check_prop` (Impact: 51.1)
  * `get_prompt_str` (Impact: 32.9)
  * `menu_dump` (Impact: 28.5)
    * *Intent:* /** * menu_dump - dump all menu entries in a tree-like format */
  * `get_symbol_str` (Impact: 22.6)
...
**Top Internal Functions/Classes:**
  * `normalize_tokens` (Impact: 60.8)
  * `has_control_flow_keyword` (Impact: 28.9)
  * `collect_rust_files_recursive` (Impact: 28.4)
  * `parse_args` (Impact: 23.7)
  * `source_shape` (Impact: 23.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `function_argument_prescan_macros` (Impact: 78.8)
  * `macro_use_statement_ranges` (Impact: 55.9)
  * `has_live_macro_for_segment_excluding` (Impact: 49.0)
  * `classified_segment` (Impact: 46.7)
  * `live_macro_defs_for_segment` (Impact: 42.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 63 instances
...
**Top Internal Functions/Classes:**
  * `resolve_fqns` (Impact: 35.5)
  * `process_exported_symbols` (Impact: 27.9)
  * `___process_structure_type` (Impact: 25.1)
  * `process_type` (Impact: 20.5)
  * `check_union_member_kabi_status` (Impact: 18.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
