...
| Avg Path Length | 4.3581 | Hops between files. Lower = Tighter coupling. |
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.8 | 24.8 | 0.0 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 51.5 | 0.6 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 41.7 | 0.0 | 0.0 |
...
| connectivity | 5342 | 1333 | 6 | `packages/microservices/external/kafka.interface.ts` |
...
- `extractor` (@ `integration/versioning/e2e/custom-versioning-fastify.spec.ts`) -> Impact: **95.9** | LOC: 956
- `applyVersionFilter` (@ `packages/platform-express/adapters/express-adapter.ts`) -> Impact: **93.3** | LOC: 146
- `middie` (@ `packages/platform-fastify/adapters/middie/fastify-middie.ts`) -> Impact: **70.6** | LOC: 164
  * *Intent:* /** * A clone of `@fastify/middie` engine https://github.com/fastify/middie * with an extra vulnerability fix. Path is now decoded before matching to ...
- `resolveConstructorParams` (@ `packages/core/injector/injector.ts`) -> Impact: **50.2** | LOC: 99
- `exchangeKeyForValue` (@ `packages/core/router/route-params-factory.ts`) -> Impact: **50.1** | LOC: 42
- `UNKNOWN_DEPENDENCIES_MESSAGE` (@ `packages/core/errors/messages.ts`) -> Impact: **48.3** | LOC: 86
- `extractor` (@ `integration/versioning/e2e/custom-versioning.spec.ts`) -> Impact: **45.1** | LOC: 705
- `lookupComponentInImports` (@ `packages/core/injector/injector.ts`) -> Impact: **42.3** | LOC: 66
- `transform` (@ `packages/common/pipes/parse-array.pipe.ts`) -> Impact: **41.9** | LOC: 76
  * *Intent:* /** * Method that accesses and performs optional transformation on argument for * in-flight requests. * * @param value currently processed route argum...
- `Holder` (@ `packages/platform-fastify/adapters/middie/fastify-middie.ts`) -> Impact: **39.5** | LOC: 82
...
### 1. `packages/platform-ws/adapters/ws-adapter.ts` (TYPESCRIPT) -> Cumulative Risk: **654.63**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 157.54 | **LOC:** 255 | **CtrlFlow:** 13.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9678%), Safety Score (91.7454%)
- **Heaviest Functions:** `create` (Impact: 30.8), `ensureHttpServerExists` (Impact: 10.4), `constructor` (Impact: 9.2)

### 2. `packages/microservices/external/kafka.interface.ts` (TYPESCRIPT) -> Cumulative Risk: **649.88**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 0.59 | **LOC:** 1326 | **CtrlFlow:** 12.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Api Exposure (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (97.8208%)
- **Heaviest Functions:** `produce` (Impact: 12.1), `fetch` (Impact: 10.7), `createTopics` (Impact: 6.0)

### 3. `packages/websockets/adapters/ws-adapter.ts` (TYPESCRIPT) -> Cumulative Risk: **641.69**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 49.56 | **LOC:** 59 | **CtrlFlow:** 14.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (94.2676%)
- **Heaviest Functions:** `constructor` (Impact: 7.4), `close` (Impact: 4.4), `create` (Impact: 3.5)

### 4. `integration/microservices/src/mqtt/mqtt.controller.ts` (TYPESCRIPT) -> Cumulative Risk: **639.23**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 148.2 | **LOC:** 211 | **CtrlFlow:** 5.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), Api Exposure (82.2225%)
- **Heaviest Functions:** `wildcardMessageHandler` (Impact: 4.5), `sharedWildcardMessageHandler` (Impact: 4.5), `concurrent` (Impact: 3.5)

### 5. `packages/microservices/client/client-redis.ts` (TYPESCRIPT) -> Cumulative Risk: **633.84**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 218.3 | **LOC:** 309 | **CtrlFlow:** 12.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.3638%)
- **Heaviest Functions:** `on` (Impact: 7.5), `registerEndListener` (Impact: 7.0), `createRetryStrategy` (Impact: 6.5)

### 6. `packages/platform-fastify/adapters/middie/fastify-middie.ts` (TYPESCRIPT) -> Cumulative Risk: **602.74**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 393.48 | **LOC:** 431 | **CtrlFlow:** 20.9% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9923%), Safety Score (96.2316%), Documentation (95.2381%)
- **Heaviest Functions:** `middie` (Impact: 70.6), `Holder` (Impact: 39.5), `done` (Impact: 38.5)

### 7. `packages/microservices/client/client-rmq.ts` (TYPESCRIPT) -> Cumulative Risk: **599.41**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 309.76 | **LOC:** 493 | **CtrlFlow:** 12.0% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9999%), State Flux (90.7962%)
- **Heaviest Functions:** `setupChannel` (Impact: 19.4), `handleMessage` (Impact: 15.4), `listener` (Impact: 11.7)

### 8. `packages/microservices/server/server-kafka.ts` (TYPESCRIPT) -> Cumulative Risk: **596.39**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 266.22 | **LOC:** 400 | **CtrlFlow:** 11.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9998%), State Flux (95.3163%)
- **Heaviest Functions:** `handleMessage` (Impact: 11.3), `constructor` (Impact: 10.1), `combineStreamsAndThrowIfRetriable` (Impact: 10.0)

### 9. `packages/microservices/client/client-mqtt.ts` (TYPESCRIPT) -> Cumulative Risk: **594.66**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 220.06 | **LOC:** 329 | **CtrlFlow:** 15.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9726%), State Flux (99.6997%)
- **Heaviest Functions:** `mergePacketOptions` (Impact: 15.4), `publishPacket` (Impact: 11.9), `dispatchEvent` (Impact: 10.8)

### 10. `packages/microservices/client/client-kafka.ts` (TYPESCRIPT) -> Cumulative Risk: **590.63**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 275.26 | **LOC:** 462 | **CtrlFlow:** 9.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (96.7038%)
- **Heaviest Functions:** `constructor` (Impact: 10.3), `connect` (Impact: 7.5), `createResponseCallback` (Impact: 6.5)
...
- **Risk Profile:** Cognitive Load (23.1922%), Tech Debt (0.0%)
...
- **Risk Profile:** Cognitive Load (35.8159%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `resolveConstructorParams` (Impact: 50.2)
  * `lookupComponentInImports` (Impact: 42.3)
  * `resolveComponentHost` (Impact: 38.3)
  * `instantiateClass` (Impact: 36.5)
  * `resolveProperties` (Impact: 33.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00011 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (23.3431%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (4.4364%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (52.7942%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `reply` (Impact: 35.0)
  * `deriveConstraint` (Impact: 27.2)
  * `injectRouteOptions` (Impact: 24.1)
  * `createMiddlewareFactory` (Impact: 21.2)
...
- **Risk Profile:** Cognitive Load (62.844%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `writeObservableToGrpc` (Impact: 27.4)
    * *Intent:* /** * Writes an observable to a GRPC call. * * This function will ensure that backpressure is manage...
  * `collectDeepServices` (Impact: 23.9)
    * *Intent:* /** * Recursively fetch all of the service methods available on loaded * protobuf descriptor object,...
  * `createService` (Impact: 15.0)
    * *Intent:* /** * Will create service mapping from gRPC generated Object to handlers * defined with @GrpcMethod ...
  * `createRequestStreamMethod` (Impact: 13.7)
  * `createServiceMethod` (Impact: 13.3)
...
- **Risk Profile:** Cognitive Load (27.5375%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 6 instances
...
- **Risk Profile:** Cognitive Load (65.2847%), Tech Debt (20.6086%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (34.2409%), Tech Debt (10.9052%)
**Top Internal Functions/Classes:**
  * `applyVersionFilter` (Impact: 93.3)
  * `reply` (Impact: 36.4)
  * `handlerForMediaTypeVersioning` (Impact: 32.3)
  * `handlerForCustomVersioning` (Impact: 28.1)
  * `handlerForHeaderVersioning` (Impact: 27.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (34.9696%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `addCustomProvider` (Impact: 19.0)
  * `createModuleReferenceType` (Impact: 16.6)
  * `addInjectable` (Impact: 15.7)
  * `addProvider` (Impact: 15.7)
  * `addCustomValue` (Impact: 15.2)
**Contextual Mitigations & Amplifications:**
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 393.48 | **LOC:** 431 | **CtrlFlow:** 20.9% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (69.7758%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `middie` (Impact: 70.6)
    * *Intent:* /** * A clone of `@fastify/middie` engine https://github.com/fastify/middie * with an extra vulnerab...
  * `Holder` (Impact: 39.5)
  * `done` (Impact: 38.5)
  * `fastifyMiddie` (Impact: 37.0)
  * `use` (Impact: 15.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (25.8877%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Risk Profile:** Cognitive Load (44.6912%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `isStatic` (Impact: 31.2)
  * `introspectDepsAttribute` (Impact: 19.0)
  * `isDependencyTreeDurable` (Impact: 12.9)
  * `getInstanceByContextId` (Impact: 11.3)
  * `isInRequestScope` (Impact: 11.0)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (17.1798%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `constructor` (Impact: 32.5)
  * `printMessages` (Impact: 26.5)
  * `printAsJson` (Impact: 20.4)
  * `getJsonLogObject` (Impact: 13.8)
  * `getInspectOptions` (Impact: 13.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (25.9843%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
- **Risk Profile:** Cognitive Load (95.1329%), Tech Debt (18.6253%)
**Top Internal Functions/Classes:**
  * `data` (Impact: 8.8)
  * `use` (Impact: 6.4)
  * `pong` (Impact: 5.8)
  * `validateExecutionCount` (Impact: 5.1)
  * `configure` (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `messageParser` (Impact: 2.7)
  * `createNestApp` (Impact: 1.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (25.4972%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `scanForModules` (Impact: 27.5)
  * `reflectKeyMetadata` (Impact: 15.2)
  * `insertProvider` (Impact: 12.8)
  * `insertInjectable` (Impact: 11.5)
  * `insertModule` (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 3 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000274 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (49.2906%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `error` (Impact: 3.5)
  * `on` (Impact: 1.8)
  * `subscribe` (Impact: 1.5)
  * `removeListener` (Impact: 1.2)
  * `off` (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 309.76 | **LOC:** 493 | **CtrlFlow:** 12.0% | **Authorship Centralization:** 66.7%
- **Risk Profile:** Cognitive Load (69.2481%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `setupChannel` (Impact: 19.4)
  * `handleMessage` (Impact: 15.4)
  * `listener` (Impact: 11.7)
  * `dispatchEvent` (Impact: 10.5)
  * `mergeHeaders` (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (66.1495%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `listen` (Impact: 23.1)
  * `errorHandler` (Impact: 17.4)
  * `formatAddress` (Impact: 13.8)
...
- `packages/core/inspector/serialized-graph.ts` -> **Severity: 0.063** (Bridge: 0.0006 * Flux: 99.1699%)
- `packages/core/inspector/graph-inspector.ts` -> **Severity: 0.043** (Bridge: 0.0006 * Flux: 76.754%)
- `packages/core/injector/container.ts` -> **Severity: 0.04** (Bridge: 0.0007 * Flux: 58.3912%)
- `packages/core/injector/module.ts` -> **Severity: 0.017** (Bridge: 0.0002 * Flux: 83.1086%)
- `packages/microservices/client/client-kafka.ts` -> **Severity: 0.013** (Bridge: 0.0001 * Flux: 96.7038%)
