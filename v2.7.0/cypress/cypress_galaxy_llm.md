...
| connectivity | 8497 | 2496 | 4 | `packages/driver/cypress/fixtures/dom.html` |
...
| dead_code | 1923 | 798 | 1 | `packages/errors/test/visualSnapshotErrors.spec.ts` |
| credential | 52 | 17 | 0 | `packages/proxy/test/unit/http/util/regex-rewriter.spec.ts` |
...
- `addAll` (@ `cli/types/cypress.d.ts`) -> Impact: **356.0** | LOC: 1785
  * *Intent:* /** * Add one or more custom commands that validate their prevSubject * @see https://on.cypress.io/api/commands#Validations */
- `startListening` (@ `packages/server/lib/socket-base.ts`) -> Impact: **265.3** | LOC: 603
- `click` (@ `packages/driver/cypress/e2e/commands/actions/click.cy.ts`) -> Impact: **220.0** | LOC: 2639
- `type` (@ `packages/driver/src/cy/commands/actions/type.ts`) -> Impact: **206.8** | LOC: 575
- `expectValidationFails` (@ `packages/server/test/unit/config_spec.js`) -> Impact: **201.4** | LOC: 804
- `action` (@ `packages/driver/src/cypress.ts`) -> Impact: **174.2** | LOC: 366
- `collectLogs` (@ `packages/driver/cypress/e2e/commands/querying/querying.cy.ts`) -> Impact: **169.2** | LOC: 1201
- `create` (@ `packages/driver/src/cy/chai/inspect.ts`) -> Impact: **159.0** | LOC: 437
  * *Intent:* // let config = require('../config')
- `res` (@ `system-tests/test/record_spec.js`) -> Impact: **158.1** | LOC: 356
- `origin` (@ `cli/types/cypress.d.ts`) -> Impact: **153.7** | LOC: 753
...
- **Heaviest Functions:** `startWebsockets` (Impact: 72.9), `endTelemetry` (Impact: 26.3), `getConfig` (Impact: 21.4)
...
- **Heaviest Functions:** `isStable` (Impact: 8.8), `create` (Impact: 5.6), `whenStable` (Impact: 3.7)
...
- **Heaviest Functions:** `makeE2ETasks` (Impact: 78.8), `__internal__beforeEach` (Impact: 39.6), `__internal_withCtx` (Impact: 16.4)
...
- **Heaviest Functions:** `launch` (Impact: 57.0), `create` (Impact: 23.1), `onReloadBrowser` (Impact: 12.7)

### 5. `system-tests/lib/system-tests.ts` (TYPESCRIPT) -> Cumulative Risk: **730.2**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `exec` (Impact: 83.2), `args` (Impact: 42.1), `exit` (Impact: 41.3)
...
- **Heaviest Functions:** `handle` (Impact: 25.5), `respond` (Impact: 10.7), `_get` (Impact: 7.9)
...
- **Heaviest Functions:** `retry` (Impact: 47.9), `ended` (Impact: 7.1), `create` (Impact: 5.3)
...
- **Heaviest Functions:** `scrollTo` (Impact: 113.1), `scrollIntoView` (Impact: 23.1), `ensureScrollability` (Impact: 12.7)
...
- **Heaviest Functions:** `finish` (Impact: 21.3), `reply` (Impact: 19.8), `continue` (Impact: 9.7)
...
- **Heaviest Functions:** `startListening` (Impact: 265.3), `backendRequest` (Impact: 52.1), `getFixture` (Impact: 31.4)
...
**Top Internal Functions/Classes:**
  * `expectExitWithErr` (Impact: 63.2)
    * *Intent:* // returns error object
  * `ensureDoesNotExist` (Impact: 3.8)
  * `mockEE` (Impact: 3.1)
  * `snapshotConsoleLogs` (Impact: 2.2)
  * `expectExitWith` (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `onRunnableRun` (Impact: 59.5)
  * `handleSuiteEnd` (Impact: 37.3)
  * `case` (Impact: 36.6)
  * `maybeHandleRetryOnFailure` (Impact: 32.8)
  * `suiteHasOnlyId` (Impact: 30.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `matches` (Impact: 150.5)
  * `write` (Impact: 86.2)
  * `onmessage` (Impact: 21.8)
  * `use` (Impact: 19.1)
  * `setup` (Impact: 7.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 34 instances
...
- `packages/driver/src/cypress/error_utils.ts` -> **Severity: 0.013** (Bridge: 0.0001 * Flux: 99.9248%)
- `packages/driver/src/cy/chai.ts` -> **Severity: 0.011** (Bridge: 0.0001 * Flux: 99.7679%)
- `packages/driver/src/cypress/cy.ts` -> **Severity: 0.01** (Bridge: 0.0001 * Flux: 99.702%)
- `packages/driver/src/cy/keyboard.ts` -> **Severity: 0.008** (Bridge: 0.0001 * Flux: 99.9711%)
- `system-tests/lib/system-tests.ts` -> **Severity: 0.006** (Bridge: 0.0001 * Flux: 99.9988%)
