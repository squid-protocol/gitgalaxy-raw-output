...
| Avg Path Length | 3.8967 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 29.4 | 26.2 | 0.0 |
...
| dead_code | 1325 | 367 | 2 | `src/font/shaper/harfbuzz.zig` |
| credential | 11 | 7 | 0 | `build.zig.zon` |
...
- `findSurface` (@ `macos/Sources/Ghostty/Ghostty.App.swift`) -> Impact: **579.1** | LOC: 1372
...
- `csiDispatch` (@ `src/terminal/stream.zig`) -> Impact: **484.2** | LOC: 1162
- `Stream` (@ `src/terminal/stream.zig`) -> Impact: **406.9** | LOC: 1321
...
- `rebuildRow` (@ `src/renderer/generic.zig`) -> Impact: **301.4** | LOC: 447
- `stbi__bmp_load` (@ `src/stb/stb_image.h`) -> Impact: **250.9** | LOC: 202
- `SplitTree` (@ `src/datastruct/split_tree.zig`) -> Impact: **235.5** | LOC: 1315
...
- `Renderer` (@ `src/renderer/generic.zig`) -> Impact: **223.0** | LOC: 1631
...
- `stbi__create_png_image_raw` (@ `src/stb/stb_image.h`) -> Impact: **217.6** | LOC: 211
  * *Intent:* // create the png data from post-deflated data
- `stbi__parse_png_file` (@ `src/stb/stb_image.h`) -> Impact: **203.1** | LOC: 182
  * *Intent:* #define STBI__PNG_TYPE(a,b,c,d) (((unsigned) (a) << 24) + ((unsigned) (b) << 16) + ((unsigned) (c) << 8) + (unsigned) (d))
- `load_jpeg_image` (@ `src/stb/stb_image.h`) -> Impact: **182.0** | LOC: 162
...
- **Heaviest Functions:** `init` (Impact: 45.6), `performKeyEquivalent` (Impact: 40.0), `keyDown` (Impact: 38.6)
...
- **Heaviest Functions:** `showUpdaterError` (Impact: 15.0), `showDownloadDidReceiveExpectedContentLength` (Impact: 6.4), `showDownloadDidReceiveData` (Impact: 6.4)
...
- **Heaviest Functions:** `animateWindowOut` (Impact: 25.4), `windowDidResignKey` (Impact: 24.2), `init` (Impact: 22.8)
...
- **Heaviest Functions:** `sendEvent` (Impact: 15.7), `setupTabBar` (Impact: 15.7), `toolbar` (Impact: 15.1)
...
- **Heaviest Functions:** `assertTitlebarAppearance` (Impact: 51.5), `testIssue8282` (Impact: 4.8), `testQuickTerminalThemeChange` (Impact: 4.8)
...
- **Heaviest Functions:** `syncAppearance` (Impact: 21.8), `awakeFromNib` (Impact: 20.7), `appendTabModifierSection` (Impact: 20.5)
...
- **Heaviest Functions:** `pushTabsToTitlebar` (Impact: 22.1), `toolbar` (Impact: 17.6), `update` (Impact: 16.9)
...
- **Heaviest Functions:** `init?` (Impact: 23.1), `exit` (Impact: 21.8), `enter` (Impact: 13.6)
...
- **Heaviest Functions:** `scrollWheel` (Impact: 18.8), `onControlInspector` (Impact: 15.1), `insertText` (Impact: 14.9)
...
- **Heaviest Functions:** `splitDidDrop` (Impact: 49.9), `clipboardConfirmationComplete` (Impact: 27.4), `ghosttyDidResizeSplit` (Impact: 27.0)
...
**Top Internal Functions/Classes:**
  * `stbi__bmp_load` (Impact: 250.9)
  * `stbi__create_png_image_raw` (Impact: 217.6)
    * *Intent:* // create the png data from post-deflated data
  * `stbi__parse_png_file` (Impact: 203.1)
    * *Intent:* #define STBI__PNG_TYPE(a,b,c,d) (((unsigned) (a) << 24) + ((unsigned) (b) << 16) + ((unsigned) (c) <...
  * `load_jpeg_image` (Impact: 182.0)
  * `stbi__psd_load` (Impact: 176.9)
...
**Top Internal Functions/Classes:**
  ...
  * `reflowRow` (Impact: 56.0)
    * *Intent:* /// Reflow the provided row in to this cursor.
  * `resizeWithoutReflowGrowCols` (Impact: 52.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000563 | `Ripple Effect (Closeness):` 0.058855
...
**Top Internal Functions/Classes:**
  ...
  * `commandFinished` (Impact: 53.8)
  * `promptTitle` (Impact: 46.4)
  * `gotoWindow` (Impact: 36.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `stbir__encode_scanline` (Impact: 121.8)
  * `stbir__decode_scanline` (Impact: 88.0)
    * *Intent:* #define STBIR__DECODE(type, colorspace) ((int)(type) * (STBIR_MAX_COLORSPACES) + (int)(colorspace))
  * `stbir__resize_allocated` (Impact: 74.1)
  * `stbir__normalize_downsample_coefficients` (Impact: 51.3)
  * `stbir__edge_wrap_slow` (Impact: 38.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `finalize` (Impact: 72.2)
    * *Intent:* /// Call this once after you are done setting configuration. This /// is idempotent but will waste m...
  * `calculate` (Impact: 41.4)
  * `parseCLI` (Impact: 37.3)
  * `parseCLI` (Impact: 31.9)
  * `loadRecursiveFiles` (Impact: 26.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004554 | `Ripple Effect (Closeness):` 0.072571
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000915 | `Ripple Effect (Closeness):` 0.055197
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.004588
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002376 | `Ripple Effect (Closeness):` 0.057767
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002849 | `Ripple Effect (Closeness):` 0.044383
...
**Top Internal Functions/Classes:**
  * `init` (Impact: 45.6)
  * `performKeyEquivalent` (Impact: 40.0)
    * *Intent:* /// Special case handling for some control keys
  * `keyDown` (Impact: 38.6)
  * `setCursorShape` (Impact: 28.1)
  * `flagsChanged` (Impact: 26.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000514 | `Ripple Effect (Closeness):` 0.058943
...
**Top Internal Functions/Classes:**
  * `splitDidDrop` (Impact: 49.9)
  * `clipboardConfirmationComplete` (Impact: 27.4)
  * `ghosttyDidResizeSplit` (Impact: 27.0)
  * `ghosttyDidFocusSplit` (Impact: 25.6)
  * `onConfirmClipboardRequest` (Impact: 25.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002409 | `Ripple Effect (Closeness):` 0.044816
...
**Top Internal Functions/Classes:**
  * `setupMenuImages` (Impact: 41.1)
    * *Intent:* /// Setup all the images for our menu items.
  * `ghosttyConfigDidChange` (Impact: 36.9)
  * `applicationShouldTerminate` (Impact: 28.3)
  * `application` (Impact: 27.9)
  * `applicationDidFinishLaunching` (Impact: 21.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `formatWithState` (Impact: 155.9)
  * `format` (Impact: 62.4)
  * `format` (Impact: 56.4)
  * `format` (Impact: 23.6)
  * `writeCodepointWithReplacement` (Impact: 16.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 30 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000366 | `Ripple Effect (Closeness):` 0.047084
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004049 | `Ripple Effect (Closeness):` 0.060729
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.051933
...
**Top Internal Functions/Classes:**
  * `parseAndPutRecurse` (Impact: 44.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.4e-05 | `Ripple Effect (Closeness):` 0.053644
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000426 | `Ripple Effect (Closeness):` 0.044816
...
**Top Internal Functions/Classes:**
  * `drawPreview` (Impact: 136.1)
  * `update` (Impact: 66.8)
  * `draw` (Impact: 58.6)
  * `updateFiltered` (Impact: 31.3)
  * `run` (Impact: 25.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.001606
...
- `src/apprt/gtk/class/surface.zig` -> **Severity: 0.185** (Bridge: 0.0028 * Flux: 64.7943%)
- `src/os/passwd.zig` -> **Severity: 0.185** (Bridge: 0.0019 * Flux: 95.2818%)
- `src/config/Config.zig` -> **Severity: 0.14** (Bridge: 0.0046 * Flux: 30.8096%)
- `src/inspector/widgets.zig` -> **Severity: 0.122** (Bridge: 0.0015 * Flux: 80.3691%)
- `src/apprt/embedded.zig` -> **Severity: 0.115** (Bridge: 0.004 * Flux: 28.3227%)
