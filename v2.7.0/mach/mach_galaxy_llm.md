...
| Avg Path Length | 2.5833 | Hops between files. Lower = Tighter coupling. |
...
- `renderOpcodes` (@ `src/sysgpu/tools/gen_spirv_spec.zig`) -> Impact: **189.2** | LOC: 430
- `genCall` (@ `src/sysgpu/shader/AstGen.zig`) -> Impact: **187.6** | LOC: 552
- `renderOperandKind` (@ `src/sysgpu/tools/gen_spirv_spec.zig`) -> Impact: **177.2** | LOC: 496
- `peek` (@ `src/sysgpu/shader/Tokenizer.zig`) -> Impact: **96.5** | LOC: 374
- `Printer` (@ `src/sysgpu/shader/print_air.zig`) -> Impact: **94.8** | LOC: 511
- `addFunction` (@ `src/sysgpu/utils.zig`) -> Impact: **89.4** | LOC: 179
- `wndProc` (@ `src/core/Windows.zig`) -> Impact: **88.8** | LOC: 256
- `execute` (@ `src/sysgpu/opengl.zig`) -> Impact: **87.3** | LOC: 332
  * *Intent:* // Internal
- `Objects` (@ `src/module.zig`) -> Impact: **84.6** | LOC: 444
- `emitBinaryAir` (@ `src/sysgpu/shader/codegen/spirv.zig`) -> Impact: **78.3** | LOC: 166
...
- **Heaviest Functions:** `assertDecl` (Impact: 6.2), `assertField` (Impact: 6.2), `TextRunInterface` (Impact: 1.9)
...
- **Heaviest Functions:** `peek` (Impact: 96.5), `init` (Impact: 4.5), `next` (Impact: 1.7)
...
- **Heaviest Functions:** `render` (Impact: 15.5), `shape` (Impact: 8.4), `initFreetype` (Impact: 3.4)
...
- **Heaviest Functions:** `registryHandleGlobal` (Impact: 57.7), `initWindow` (Impact: 32.6), `seatHandleCapabilities` (Impact: 20.3)
...
- **Heaviest Functions:** `loadVersion` (Impact: 49.0), `removeOptional` (Impact: 4.5), `load` (Impact: 1.7)
...
- **Heaviest Functions:** `addFunction` (Impact: 89.4), `findChained` (Impact: 5.7), `textureFormatType` (Impact: 5.3)
...
- **Heaviest Functions:** `refresh` (Impact: 63.4), `createAudioClient` (Impact: 62.6), `init` (Impact: 24.7)
...
- **Heaviest Functions:** `dxgiFormatForTextureView` (Impact: 15.2), `d3d12RasterizerDesc` (Impact: 13.7), `d3d12DescriptorRangeType` (Impact: 12.3)
...
- **Heaviest Functions:** `Objects` (Impact: 84.6), `Modules` (Impact: 47.0), `run` (Impact: 25.9)
...
- **Heaviest Functions:** `allocate` (Impact: 20.5), `free` (Impact: 14.8), `removeNodeFromBin` (Impact: 13.8)
...
**Top Internal Functions/Classes:**
  * `genCall` (Impact: 187.6)
  * `genFn` (Impact: 77.4)
  * `genBinary` (Impact: 73.0)
  * `genBitcast` (Impact: 69.6)
  * `genTextureSampleBuiltin` (Impact: 60.1)
...
**Top Internal Functions/Classes:**
  ...
  * `renderPassEncoderSetViewport` (Impact: 3.3)
  * `bufferMapAsync` (Impact: 3.1)
  * `commandEncoderCopyBufferToBuffer` (Impact: 3.1)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001233 | `Ripple Effect (Closeness):` 0.015267
...
**Top Internal Functions/Classes:**
  * `init` (Impact: 61.5)
  * `setBindGroup` (Impact: 44.3)
  * `init` (Impact: 25.4)
  * `init` (Impact: 24.5)
  * `buildBindings` (Impact: 17.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `loadVersion` (Impact: 49.0)
  * `removeOptional` (Impact: 4.5)
  * `load` (Impact: 1.7)
  * `getProcAddress` (Impact: 1.6)
  * `load` (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `emitBinaryAir` (Impact: 78.3)
  * `emitVarProto` (Impact: 62.5)
  * `emitFnVars` (Impact: 28.5)
  * `emitBinaryIntrinsic` (Impact: 22.6)
  * `emitIf` (Impact: 22.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `attribute` (Impact: 30.8)
  * `statement` (Impact: 29.9)
  * `switchStatement` (Impact: 27.3)
  * `typeSpecifierWithoutIdent` (Impact: 21.8)
  * `globalVar` (Impact: 20.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 45 instances
...
**Top Internal Functions/Classes:**
  * `deviceCreateShaderModule` (Impact: 26.5)
  * `instanceRequestAdapter` (Impact: 5.1)
  * `createInstance` (Impact: 4.6)
  * `adapterCreateDevice` (Impact: 3.9)
  * `commandEncoderFinish` (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `emitFn` (Impact: 46.3)
  * `gen` (Impact: 40.1)
  * `emitTextureType` (Impact: 21.7)
  * `emitCall` (Impact: 16.9)
  * `emitUnaryIntrinsic` (Impact: 15.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Mitigated Memory Allocs:* 1 instances
...
**Top Internal Functions/Classes:**
  * `emitFn` (Impact: 50.0)
  * `emitGlobalVar` (Impact: 39.6)
  * `emitStruct` (Impact: 35.4)
  * `structMemberLessThan` (Impact: 23.1)
  * `fnParamLessThan` (Impact: 23.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 7 instances
...
**Top Internal Functions/Classes:**
  * `refresh` (Impact: 63.4)
  * `createAudioClient` (Impact: 62.6)
  * `init` (Impact: 24.7)
  * `getDefaultAudioEndpoint` (Impact: 20.7)
  * `writeThread` (Impact: 18.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 10 instances
...
**Top Internal Functions/Classes:**
  * `registryHandleGlobal` (Impact: 57.7)
  * `initWindow` (Impact: 32.6)
  * `seatHandleCapabilities` (Impact: 20.3)
  * `keyboardHandleKeymap` (Impact: 17.6)
  * `keyboardHandleKey` (Impact: 14.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `VecShared` (Impact: 45.9)
  * `Vec3` (Impact: 10.4)
  * `Vec4` (Impact: 8.2)
  * `maxScalar` (Impact: 7.5)
    * *Intent:* // Returns the largest scalar of two vectors
  * `minScalar` (Impact: 7.5)
...
**Top Internal Functions/Classes:**
  * `emitFn` (Impact: 40.1)
  * `gen` (Impact: 40.0)
  * `emitGlobalVar` (Impact: 23.1)
  * `emitReturn` (Impact: 13.1)
  * `emitStruct` (Impact: 12.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 5 instances
...
**Top Internal Functions/Classes:**
  * `dxgiFormatForTextureView` (Impact: 15.2)
  * `d3d12RasterizerDesc` (Impact: 13.7)
  * `d3d12DescriptorRangeType` (Impact: 12.3)
  * `d3d12ResourceFlagsForTexture` (Impact: 11.3)
  * `d3d12SrvDimension` (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `createStream` (Impact: 41.3)
  * `refresh` (Impact: 41.2)
  * `deviceEventsLoop` (Impact: 23.0)
  * `toAlsaFormat` (Impact: 14.6)
  * `init` (Impact: 14.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `resolveConstExpr` (Impact: 55.8)
  * `findFunction` (Impact: 9.4)
  * `typeSize` (Impact: 9.3)
  * `resolveInt` (Impact: 9.3)
  * `mul` (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `convertTo` (Impact: 29.0)
  * `convertFrom` (Impact: 29.0)
  * `init` (Impact: 15.4)
  * `preferredFormat` (Impact: 10.9)
    * *Intent:* // TODO: don't call this in backends. let the user use it
  * `sampleRate` (Impact: 10.3)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `refresh` (Impact: 40.9)
  * `createRecorder` (Impact: 40.4)
  * `createPlayer` (Impact: 24.0)
  * `captureCallback` (Impact: 18.6)
  * `createStreamDesc` (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
...
- `src/sysgpu/utils.zig` -> **Severity: 0.291** (Bridge: 0.0032 * Flux: 90.0571%)
- `src/sysgpu/vulkan.zig` -> **Severity: 0.096** (Bridge: 0.0012 * Flux: 77.8064%)
