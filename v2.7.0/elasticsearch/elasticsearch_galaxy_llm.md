...
| connectivity | 291455 | 27161 | 20 | `x-pack/plugin/esql/src/test/java/org/elasticsearch/xpack/esql/optimizer/LogicalPlanOptimizerTests.java` |
...
| dead_code | 95822 | 15493 | 8 | `x-pack/plugin/esql/src/test/java/org/elasticsearch/xpack/esql/optimizer/LogicalPlanOptimizerTests.java` |
| credential | 533 | 129 | 0 | `x-pack/plugin/security/src/test/java/org/elasticsearch/xpack/security/authc/saml/SamlSpMetadataBuilderTests.java` |
...
- `getTableWithHeader` (@ `server/src/main/java/org/elasticsearch/rest/action/cat/RestIndicesAction.java`) -> Impact: **1024.2** | LOC: 374
- `getLegalCast` (@ `modules/lang-painless/src/main/java/org/elasticsearch/painless/AnalyzerCaster.java`) -> Impact: **839.4** | LOC: 377
- `testApacheLog` (@ `libs/grok/src/test/java/org/elasticsearch/grok/GrokTests.java`) -> Impact: **796.9** | LOC: 148
- `getTableWithHeader` (@ `server/src/main/java/org/elasticsearch/rest/action/cat/RestNodesAction.java`) -> Impact: **683.3** | LOC: 202
- `buildTable` (@ `server/src/main/java/org/elasticsearch/rest/action/cat/RestIndicesAction.java`) -> Impact: **663.0** | LOC: 291
  * *Intent:* // package private for testing
- `testBinaryComparisons` (@ `x-pack/plugin/esql/src/test/java/org/elasticsearch/xpack/esql/planner/QueryTranslatorTests.java`) -> Impact: **651.1** | LOC: 343
- `TestConfiguration` (@ `qa/vector/src/main/java/org/elasticsearch/test/knn/TestConfiguration.java`) -> Impact: **612.1** | LOC: 918
...
- `buildTable` (@ `server/src/main/java/org/elasticsearch/rest/action/cat/RestNodesAction.java`) -> Impact: **602.9** | LOC: 203
- `testFailureStoreAccess` (@ `x-pack/plugin/security/qa/security-trial/src/javaRestTest/java/org/elasticsearch/xpack/security/failurestore/FailureStoreSecurityRestIT.java`) -> Impact: **585.5** | LOC: 949
- `getTableWithHeader` (@ `server/src/main/java/org/elasticsearch/rest/action/cat/RestShardsAction.java`) -> Impact: **550.9** | LOC: 157
...
- **Heaviest Functions:** `printSeekState` (Impact: 66.4), `seekCeil` (Impact: 58.7), `seekExact` (Impact: 57.7)
...
- **Heaviest Functions:** `scanToTermNonLeaf` (Impact: 28.4), `scanToTermLeaf` (Impact: 22.0), `loadBlock` (Impact: 16.9)
...
- **Heaviest Functions:** `executeWithInfo` (Impact: 27.5), `Batch` (Impact: 4.3), `Batch` (Impact: 3.7)
...
- **Heaviest Functions:** `configureSslContext` (Impact: 7.4), `wait` (Impact: 6.7), `WaitForHttpResource` (Impact: 6.2)
...
- **Heaviest Functions:** `addPending` (Impact: 18.9), `execute` (Impact: 10.7), `wrapRunnable` (Impact: 9.2)
...
- **Heaviest Functions:** `cosi8_inner_bulk` (Impact: 23.9), `call_i8_bulk` (Impact: 17.6), `call_f32_bulk` (Impact: 17.2)
...
- **Heaviest Functions:** `DefaultBuildParameterExtension` (Impact: 9.5), `getBuildDate` (Impact: 3.6), `computeIfAbsent` (Impact: 3.2)
...
- **Heaviest Functions:** `getNumeric` (Impact: 104.6), `tryRead` (Impact: 48.6), `decodeLengthsBulk` (Impact: 47.2)
...
- **Heaviest Functions:** `readFields` (Impact: 102.8), `readNumericEntry` (Impact: 61.3), `getSortedNumeric` (Impact: 45.2)
...
- **Heaviest Functions:** `validateCurrentStage` (Impact: 7.9), `addFileDetails` (Impact: 6.9), `setStage` (Impact: 6.7)
...
- `x-pack/plugin/esql/src/main/java/org/elasticsearch/xpack/esql/expression/function/vector/VectorSimilarityFunction.java` -> **Severity: 4.191** (Bridge: 0.0422 * Flux: 99.2118%)
- `x-pack/plugin/esql/src/main/java/org/elasticsearch/xpack/esql/planner/EsPhysicalOperationProviders.java` -> **Severity: 3.463** (Bridge: 0.0411 * Flux: 84.2596%)
- `x-pack/plugin/esql/src/main/java/org/elasticsearch/xpack/esql/plugin/EsqlPlugin.java` -> **Severity: 3.213** (Bridge: 0.0327 * Flux: 98.3171%)
- `server/src/main/java/org/elasticsearch/index/mapper/vectors/DenseVectorFieldMapper.java` -> **Severity: 2.985** (Bridge: 0.0322 * Flux: 92.8077%)
- `x-pack/plugin/ml/src/main/java/org/elasticsearch/xpack/ml/MachineLearning.java` -> **Severity: 2.115** (Bridge: 0.022 * Flux: 96.3169%)
