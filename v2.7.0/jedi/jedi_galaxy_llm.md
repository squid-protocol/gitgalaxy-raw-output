...
| Avg Path Length | 3.1986 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 15.4 | 7.1 | 0.0 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 68.1 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 57.1 | 80.3 | 0.0 |
...
| dead_code | 703 | 126 | 5 | `test/test_api/test_interpreter.py` |
| credential | 0 | 0 | 0 | - |
...
- `_infer_comparison_part` (@ `jedi/inference/syntax_tree.py`) -> Impact: **112.3** | LOC: 91
- `process_params` (@ `jedi/inference/star_args.py`) -> Impact: **83.5** | LOC: 112
- `test_infer_and_goto` (@ `test/test_inference/test_gradual/test_stubs.py`) -> Impact: **81.9** | LOC: 47
- `extract_function` (@ `jedi/api/refactoring/extract.py`) -> Impact: **78.3** | LOC: 85
- `search_in_module` (@ `jedi/api/completion.py`) -> Impact: **71.6** | LOC: 40
- `_complete_python` (@ `jedi/api/completion.py`) -> Impact: **71.2** | LOC: 142
...
- `_check_for_exception_catch` (@ `jedi/inference/analysis.py`) -> Impact: **69.2** | LOC: 88
...
- `get_executed_param_names_and_issues` (@ `jedi/inference/param.py`) -> Impact: **67.5** | LOC: 173
...
- `tree_name_to_values` (@ `jedi/inference/syntax_tree.py`) -> Impact: **65.0** | LOC: 101
- `_try_to_load_stub` (@ `jedi/inference/gradual/typeshed.py`) -> Impact: **60.8** | LOC: 90
...
### 1. `jedi/inference/context.py` (PYTHON) -> Cumulative Risk: **795.02**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 493.86 | **LOC:** 499 | **CtrlFlow:** 19.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9989%), Api Exposure (99.6938%), Documentation (96.748%)
- **Heaviest Functions:** `py__getattribute__` (Impact: 54.0), `create_context` (Impact: 29.6), `_check_for_additional_knowledge` (Impact: 21.1)

### 2. `jedi/inference/value/function.py` (PYTHON) -> Cumulative Risk: **751.8**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 491.32 | **LOC:** 460 | **CtrlFlow:** 23.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Churn (100.0%), Documentation (97.6744%)
- **Heaviest Functions:** `get_yield_lazy_values` (Impact: 33.4), `_find_overload_functions` (Impact: 28.0), `get_return_values` (Impact: 24.5)

### 3. `jedi/inference/names.py` (PYTHON) -> Cumulative Risk: **747.7**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 686.48 | **LOC:** 674 | **CtrlFlow:** 23.1% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (97.2345%), Documentation (95.6522%)
- **Heaviest Functions:** `goto` (Impact: 45.9), `get_kind` (Impact: 18.3), `assignment_indexes` (Impact: 14.7)

### 4. `jedi/inference/helpers.py` (PYTHON) -> Cumulative Risk: **716.03**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 218.44 | **LOC:** 203 | **CtrlFlow:** 30.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (98.1723%), Documentation (85.1852%)
- **Heaviest Functions:** `infer_call_of_leaf` (Impact: 31.2), `parse_dotted_names` (Impact: 27.4), `get_names_of_node` (Impact: 7.6)

### 5. `jedi/inference/star_args.py` (PYTHON) -> Cumulative Risk: **698.01**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 283.96 | **LOC:** 218 | **CtrlFlow:** 38.1% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.4313%)
- **Heaviest Functions:** `process_params` (Impact: 83.5), `_remove_given_params` (Impact: 16.4), `_iter_nodes_for_param` (Impact: 12.9)

### 6. `jedi/inference/gradual/base.py` (PYTHON) -> Cumulative Risk: **691.3**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 310.56 | **LOC:** 435 | **CtrlFlow:** 14.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9875%), Tech Debt (99.9636%)
- **Heaviest Functions:** `infer_type_vars` (Impact: 21.0), `is_same_class` (Impact: 13.6), `define_generics` (Impact: 13.4)

### 7. `jedi/inference/syntax_tree.py` (PYTHON) -> Cumulative Risk: **685.82**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1168.28 | **LOC:** 906 | **CtrlFlow:** 38.0% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Churn (99.66%), Safety Score (98.8385%)
- **Heaviest Functions:** `_infer_comparison_part` (Impact: 112.3), `tree_name_to_values` (Impact: 65.0), `infer_atom` (Impact: 52.6)

### 8. `jedi/plugins/django.py` (PYTHON) -> Cumulative Risk: **677.39**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 287.06 | **LOC:** 297 | **CtrlFlow:** 24.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9895%), Tech Debt (83.5956%)
- **Heaviest Functions:** `wrapper` (Impact: 33.2), `tree_name_to_values` (Impact: 23.9), `_infer_field` (Impact: 19.3)

### 9. `jedi/inference/compiled/value.py` (PYTHON) -> Cumulative Risk: **669.82**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 531.32 | **LOC:** 627 | **CtrlFlow:** 17.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9993%), Documentation (97.1429%), Safety Score (85.6639%)
- **Heaviest Functions:** `_get` (Impact: 28.2), `_parse_function_doc` (Impact: 22.7), `execute_annotation` (Impact: 10.9)

### 10. `jedi/file_io.py` (PYTHON) -> Cumulative Risk: **663.86**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 74.48 | **LOC:** 84 | **CtrlFlow:** 10.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9998%), Tech Debt (99.8912%)
- **Heaviest Functions:** `walk` (Impact: 10.8), `__init__` (Impact: 2.4), `__init__` (Impact: 1.8)
...
- **Magnitude:** 1168.28 | **LOC:** 906 | **CtrlFlow:** 38.0% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (71.1655%), Tech Debt (12.9371%)
**Top Internal Functions/Classes:**
  * `_infer_comparison_part` (Impact: 112.3)
  * `tree_name_to_values` (Impact: 65.0)
  * `infer_atom` (Impact: 52.6)
    * *Intent:* """ Basically to process ``atom`` nodes. The parser sometimes doesn't generate the node (because it ...
  * `infer_node` (Impact: 52.0)
  * `_infer_node` (Impact: 48.9)
...
- **Risk Profile:** Cognitive Load (75.4759%), Tech Debt (9.1123%)
**Top Internal Functions/Classes:**
  * `search_in_module` (Impact: 71.6)
  * `_complete_python` (Impact: 71.2)
    * *Intent:* """ Analyzes the current context of a completion and decides what to return. Technically this works ...
  * `_extract_string_while_in_string` (Impact: 50.8)
  * `filter_names` (Impact: 35.4)
  * `_complete_in_string` (Impact: 31.6)
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 686.48 | **LOC:** 674 | **CtrlFlow:** 23.1% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (87.5339%), Tech Debt (9.1496%)
**Top Internal Functions/Classes:**
  * `goto` (Impact: 45.9)
  * `get_kind` (Impact: 18.3)
  * `assignment_indexes` (Impact: 14.7)
    * *Intent:* """ Returns an array of tuple(int, node) of the indexes that are used in tuple assignments. For exam...
  * `get_qualified_names` (Impact: 14.6)
  * `py__doc__` (Impact: 12.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.009372 | `Ripple Effect (Closeness):` 0.161636
...
- **Risk Profile:** Cognitive Load (74.6829%), Tech Debt (13.8482%)
**Top Internal Functions/Classes:**
  * `_iter_arguments` (Impact: 56.9)
  * `calculate_index` (Impact: 51.0)
  * `get_signature_details` (Impact: 33.8)
...
- **Risk Profile:** Cognitive Load (36.6758%), Tech Debt (14.8284%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 28.8)
  * `help` (Impact: 28.1)
...
- **Risk Profile:** Cognitive Load (42.3788%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_dict_completion` (Impact: 16.9)
  * `test__getattr__completions` (Impact: 15.4)
  * `test_dir_magic_method` (Impact: 12.7)
  * `_assert_interpreter_complete` (Impact: 10.2)
  * `test_custom__getitem__` (Impact: 9.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (58.2979%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `completion_names` (Impact: 36.4)
  ...
  * `load_module_from_path` (Impact: 20.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.005801 | `Ripple Effect (Closeness):` 0.134228
...
- **Risk Profile:** Cognitive Load (36.6339%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `collect_file_tests` (Impact: 42.5)
  * `collect_dir_tests` (Impact: 29.3)
  * `run_inference` (Impact: 25.9)
  * `run_get_references` (Impact: 23.4)
  * `definition` (Impact: 17.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Risk Profile:** Cognitive Load (69.0163%), Tech Debt (80.9593%)
**Top Internal Functions/Classes:**
  * `get_tree_entries` (Impact: 27.7)
  * `unpack_tuple_to_dict` (Impact: 25.8)
    * *Intent:* """ Unpacking tuple assignments in for statements and expr_stmts. """
  * `comprehension_from_atom` (Impact: 17.7)
  * `_nested` (Impact: 11.4)
  * `py__iter__` (Impact: 9.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.006754 | `Ripple Effect (Closeness):` 0.153594
...
- **Risk Profile:** Cognitive Load (53.7786%), Tech Debt (14.3599%)
**Top Internal Functions/Classes:**
  * `get_filters` (Impact: 26.1)
  * `_get_dataclass_transform_signatures` (Impact: 22.5)
    * *Intent:* """ Returns: A non-empty list if the class has dataclass semantics else an empty list. The dataclass...
  * `get_metaclasses` (Impact: 17.8)
  * `get_dataclass_param_names` (Impact: 13.1)
    * *Intent:* """ ``cls`` is a :class:`ClassMixin`. The type is only documented as mypy would complain that some f...
  * `is_typeddict` (Impact: 12.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002269 | `Ripple Effect (Closeness):` 0.1165
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 493.86 | **LOC:** 499 | **CtrlFlow:** 19.2% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (84.4275%), Tech Debt (99.9989%)
**Top Internal Functions/Classes:**
  * `py__getattribute__` (Impact: 54.0)
  * `create_context` (Impact: 29.6)
  * `_check_for_additional_knowledge` (Impact: 21.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002214 | `Ripple Effect (Closeness):` 0.122025
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 491.32 | **LOC:** 460 | **CtrlFlow:** 23.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (92.2813%), Tech Debt (12.4992%)
**Top Internal Functions/Classes:**
  * `get_yield_lazy_values` (Impact: 33.4)
  ...
  * `get_return_values` (Impact: 24.5)
  * `infer` (Impact: 15.8)
    * *Intent:* """ Created to be used by inheritance. """
  * `from_context` (Impact: 15.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (66.4147%), Tech Debt (13.7842%)
**Top Internal Functions/Classes:**
  * `py__getattribute__` (Impact: 25.5)
  * `get_type_hint` (Impact: 14.8)
  * `_getitem` (Impact: 11.2)
    * *Intent:* # The actual getitem call. result = NO_VALUES unused_values = set() for index_value in index_values:...
  * `iterate` (Impact: 10.3)
  * `_get_value_filters` (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004905 | `Ripple Effect (Closeness):` 0.176414
...
- **Risk Profile:** Cognitive Load (55.5204%), Tech Debt (10.248%)
**Top Internal Functions/Classes:**
  * `_find_type_from_comment_hint` (Impact: 28.0)
  * `_infer_param` (Impact: 26.6)
    * *Intent:* """ Infers the type of a function parameter, using type annotations. """
  * `infer_return_types` (Impact: 22.8)
    * *Intent:* """ Infers the type of a function's return value, according to type annotations. """
  * `infer_return_for_callable` (Impact: 12.7)
  * `infer_type_vars_for_execution` (Impact: 11.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000791 | `Ripple Effect (Closeness):` 0.136002
...
- **Risk Profile:** Cognitive Load (80.3239%), Tech Debt (13.2077%)
**Top Internal Functions/Classes:**
  * `getattr_paths` (Impact: 20.2)
  * `is_allowed_getattr` (Impact: 15.6)
    * *Intent:* # TODO this API is ugly. try: attr, is_get_descriptor = getattr_static(self._obj, name) except Attri...
  * `py__getitem__all_values` (Impact: 12.3)
  * `get_annotation_name_and_args` (Impact: 10.7)
    * *Intent:* """ Returns Tuple[Optional[str], Tuple[AccessPath, ...]] """
  * `get_api_type` (Impact: 10.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (70.1507%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `find_references` (Impact: 46.6)
  * `recurse_find_python_folders_and_files` (Impact: 20.7)
  * `_find_python_files_in_sys_path` (Impact: 18.3)
  * `search_in_file_ios` (Impact: 18.1)
  * `gitignored_paths` (Impact: 14.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (65.3103%), Tech Debt (48.3413%)
**Top Internal Functions/Classes:**
  * `_try_to_load_stub` (Impact: 60.8)
  * `wrapper` (Impact: 30.9)
  * `_load_from_typeshed` (Impact: 21.5)
  * `import_module_decorator` (Impact: 18.7)
  * `_get_typeshed_directories` (Impact: 17.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 46 instances
...
- `jedi/inference/gradual/typing.py` -> **Severity: 1.618** (Bridge: 0.0162 * Flux: 99.5717%)
- `jedi/inference/names.py` -> **Severity: 0.937** (Bridge: 0.0094 * Flux: 100.0%)
