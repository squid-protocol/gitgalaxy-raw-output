...
| Avg Path Length | 3.4346 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 23.4 | 7.0 | 0.0 |
...
| connectivity | 43181 | 2339 | 33 | `tests/admin_views/tests.py` |
...
| dead_code | 8820 | 785 | 6 | `tests/migrations/test_autodetector.py` |
| credential | 28 | 15 | 0 | `tests/utils_tests/files/strip_tags1.html` |
...
- `_alter_field` (@ `django/db/backends/base/schema.py`) -> Impact: **328.4** | LOC: 370
- `__new__` (@ `django/db/models/base.py`) -> Impact: **220.4** | LOC: 293
- `create_forward_many_to_many_manager` (@ `django/db/models/fields/related_descriptors.py`) -> Impact: **180.0** | LOC: 640
...
- `handle` (@ `django/core/management/commands/migrate.py`) -> Impact: **162.6** | LOC: 292
- `fields_for_model` (@ `django/forms/models.py`) -> Impact: **153.7** | LOC: 117
- `_save_table` (@ `django/db/models/base.py`) -> Impact: **151.0** | LOC: 136
- `get_related_selections` (@ `django/db/models/sql/compiler.py`) -> Impact: **140.6** | LOC: 232
- `templatize` (@ `django/utils/translation/template.py`) -> Impact: **136.8** | LOC: 208
...
- `reduce` (@ `django/db/migrations/operations/models.py`) -> Impact: **133.4** | LOC: 229
- `_ogrinspect` (@ `django/contrib/gis/utils/ogrinspect.py`) -> Impact: **131.7** | LOC: 140
...
### 1. `django/db/models/fields/__init__.py` (PYTHON) -> Cumulative Risk: **775.6**
...
- **Heaviest Functions:** `deconstruct` (Impact: 43.2), `_check_choices` (Impact: 36.2), `formfield` (Impact: 31.5)

### 2. `django/contrib/postgres/search.py` (PYTHON) -> Cumulative Risk: **771.27**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__init__` (Impact: 22.5), `as_sql` (Impact: 21.4), `__init__` (Impact: 19.8)

### 3. `django/db/models/lookups.py` (PYTHON) -> Cumulative Risk: **745.88**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `as_sql` (Impact: 30.9), `process_rhs` (Impact: 19.1), `get_prep_lookup` (Impact: 16.9)

### 4. `django/db/models/query.py` (PYTHON) -> Cumulative Risk: **739.91**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3113.94 | **LOC:** 3025 | **CtrlFlow:** 26.5% | **Authorship Centralization:** 30.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (95.1346%), Churn (94.84%)
- **Heaviest Functions:** `bulk_create` (Impact: 84.4), `in_bulk` (Impact: 60.8), `bulk_update` (Impact: 56.5)

### 5. `django/dispatch/dispatcher.py` (PYTHON) -> Cumulative Risk: **739.43**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `connect` (Impact: 35.8), `_live_receivers` (Impact: 33.6), `asend_robust` (Impact: 19.4)

### 6. `django/db/models/fields/json.py` (PYTHON) -> Cumulative Risk: **723.66**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 690.82 | **LOC:** 746 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 77.8%
- **Primary Risk Drivers:** Api Exposure (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `resolve_expression_parameter` (Impact: 35.5), `process_rhs` (Impact: 21.1), `process_rhs` (Impact: 19.1)

### 7. `django/db/backends/sqlite3/creation.py` (PYTHON) -> Cumulative Risk: **721.91**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `_create_test_db` (Impact: 28.2), `_clone_test_db` (Impact: 17.4), `get_test_db_clone_settings` (Impact: 9.6)

### 8. `django/contrib/contenttypes/fields.py` (PYTHON) -> Cumulative Risk: **714.99**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `create_generic_related_manager` (Impact: 59.9), `get_prefetch_querysets` (Impact: 29.1), `add` (Impact: 19.8)
...
- **Heaviest Functions:** `as_postgresql` (Impact: 12.6), `as_postgresql` (Impact: 12.3), `as_native` (Impact: 8.1)

### 10. `django/views/generic/base.py` (PYTHON) -> Cumulative Risk: **704.92**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `get_redirect_url` (Impact: 17.0), `as_view` (Impact: 14.3), `view_is_async` (Impact: 12.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000393 | `Ripple Effect (Closeness):` 0.0
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 2810.48 | **LOC:** 2567 | **CtrlFlow:** 30.7% | **Authorship Centralization:** 25.0%
- **Risk Profile:** Cognitive Load (76.3226%), Tech Debt (7.9468%)
**Top Internal Functions/Classes:**
  * `__new__` (Impact: 220.4)
  * `_save_table` (Impact: 151.0)
  * `refresh_from_db` (Impact: 71.2)
  ...
  * `_get_unique_checks` (Impact: 63.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00019 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `test_use_required_attribute_true` (Impact: 27.2)
  * `test_use_required_attribute_false` (Impact: 27.0)
  * `test_templates_with_forms` (Impact: 26.9)
  * `test_validating_multiple_fields` (Impact: 26.1)
    * *Intent:* # There are a couple of ways to do multiple-field validation. If you # want the validation message t...
  * `test_unicode_values` (Impact: 23.0)
...
**Top Internal Functions/Classes:**
  * `_alter_field` (Impact: 328.4)
  * `alter_field` (Impact: 85.4)
    * *Intent:* """ Allow a field's type, uniqueness, nullability, default, column, constraints, etc. to be modified...
  * `_constraint_names` (Impact: 75.3)
  * `_iter_column_sql` (Impact: 71.2)
...
**Top Internal Functions/Classes:**
  * `_check_relationship_model` (Impact: 104.7)
  * `_check_on_delete` (Impact: 49.4)
  * `_check_clashes` (Impact: 45.3)
    * *Intent:* """Check accessor and reverse query name clashes."""
  * `_check_unique_target` (Impact: 30.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000297 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `generic` (Impact: 40.3)
  * `_follow_redirect` (Impact: 31.1)
  * `generic` (Impact: 30.4)
  * `encode_multipart` (Impact: 29.0)
...
**Top Internal Functions/Classes:**
  * `_apply_cpython_128110_workaround` (Impact: 21.4)
    * *Intent:* """ Updates message in place to correct misparsed rfc2047 display-names in address headers caused by...
  * `assertStartsWith` (Impact: 12.8)
  * `assertEndsWith` (Impact: 12.8)
  * `test_address_header_injection` (Impact: 12.0)
  * `test_attach_file` (Impact: 11.1)
...
**Top Internal Functions/Classes:**
  * `test_M2M_long_column_name` (Impact: 20.8)
    * *Intent:* """ #13711 -- Model check for long M2M column names when database has column name length limits. """
  * `test_check_constraint_raw_sql_check` (Impact: 12.9)
  * `test_index_with_condition` (Impact: 7.2)
  * `test_index_with_include` (Impact: 7.2)
  * `test_unique_constraint_with_condition` (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1433.96 | **LOC:** 3055 | **CtrlFlow:** 3.6% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (21.9978%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_delta_update` (Impact: 12.1)
  * `test_lookups_subquery` (Impact: 10.7)
  * `test_expressions_range_lookups_join_choice` (Impact: 8.3)
  * `test_delta_add` (Impact: 7.9)
    * *Intent:* # Intentionally no assert
  * `test_resolve_output_field_dates` (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `assertFormSetError` (Impact: 36.7)
    * *Intent:* """ Similar to assertFormError() but for formsets. Use form_index=None to check the formset's non-fo...
  * `_get_template_used` (Impact: 35.2)
  * `assertRedirects` (Impact: 34.2)
  * `_assert_form_error` (Impact: 32.7)
  * `_assert_contains` (Impact: 28.3)
...
**Top Internal Functions/Classes:**
  ...
  * `optgroups` (Impact: 22.1)
    * *Intent:* """Return a list of optgroups for this widget."""
  * `create_option` (Impact: 22.1)
  * `format_value` (Impact: 22.0)
...
**Top Internal Functions/Classes:**
  * `has_perm` (Impact: 18.5)
  * `get_all_permissions` (Impact: 12.7)
  * `get_group_permissions` (Impact: 12.6)
  * `has_module_perms` (Impact: 6.1)
  * `test_backend_path_login_without_authenticate_multiple_backends` (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_using_is_honored_custom_qs` (Impact: 18.1)
  * `test_using_is_honored_m2m` (Impact: 13.8)
  * `traverse_qs` (Impact: 13.5)
    * *Intent:* """ Helper method that returns a list containing a list of the objects in the obj_iter. Then for eac...
  * `test_m2m_to_inheriting_model` (Impact: 13.5)
  * `test_overriding_prefetch` (Impact: 11.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_clearsessions_command` (Impact: 6.3)
    * *Intent:* """ Test clearsessions command for clearing expired sessions. """
  * `test_session_update_error_redirect` (Impact: 3.8)
  * `test_decode_failure_logged_to_security` (Impact: 3.5)
  * `count_sessions` (Impact: 3.4)
  * `test_empty_session_saved` (Impact: 3.4)
...
- `django/db/models/query.py` -> Churn: **94.84%** | Cog Load: 57.9699% | Debt: 25.9417%
- `django/db/models/fields/json.py` -> Churn: **84.23%** | Cog Load: 67.3985% | Debt: 0.0%
- `django/db/models/lookups.py` -> Churn: **74.72%** | Cog Load: 67.5787% | Debt: 32.274%
- `django/db/models/base.py` -> Churn: **71.72%** | Cog Load: 76.3226% | Debt: 7.9468%
...
- `django/template/backends/django.py` -> **Severity: 0.244** (Bridge: 0.0024 * Flux: 99.9999%)
- `django/utils/html.py` -> **Severity: 0.222** (Bridge: 0.0022 * Flux: 100.0%)
- `django/contrib/gis/geos/collections.py` -> **Severity: 0.201** (Bridge: 0.002 * Flux: 100.0%)
- `django/utils/version.py` -> **Severity: 0.14** (Bridge: 0.0014 * Flux: 100.0%)
- `django/middleware/locale.py` -> **Severity: 0.134** (Bridge: 0.0013 * Flux: 100.0%)
