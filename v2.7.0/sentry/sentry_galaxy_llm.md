...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 18.5 | 7.5 | 0.0 |
...
| connectivity | 84514 | 13605 | 12 | `src/sentry/testutils/cases.py` |
...
| dead_code | 31228 | 6197 | 4 | `tests/sentry/dashboards/endpoints/test_organization_dashboard_details.py` |
| credential | 551 | 180 | 0 | `fixtures/vsts.py` |
...
- `ReleaseComparisonChart` (@ `static/app/views/releases/detail/overview/releaseComparisonChart/index.tsx`) -> Impact: **400.1** | LOC: 1044
- `put` (@ `src/sentry/core/endpoints/project_details.py`) -> Impact: **357.6** | LOC: 511
...
- `update_alert_rule` (@ `src/sentry/incidents/logic.py`) -> Impact: **353.8** | LOC: 266
- `Visualize` (@ `static/app/views/dashboards/widgetBuilder/components/visualize/index.tsx`) -> Impact: **332.5** | LOC: 880
- `devserver` (@ `src/sentry/runner/commands/devserver.py`) -> Impact: **314.9** | LOC: 359
- `useWidgetBuilderState` (@ `static/app/views/dashboards/widgetBuilder/hooks/useWidgetBuilderState.tsx`) -> Impact: **310.8** | LOC: 915
- `getGroupActivityItem` (@ `static/app/views/issueDetails/streamline/sidebar/groupActivityItem.tsx`) -> Impact: **301.2** | LOC: 685
- `Flamegraph` (@ `static/app/components/profiling/flamegraph/flamegraph.tsx`) -> Impact: **277.5** | LOC: 1350
- `ContinuousFlamegraph` (@ `static/app/components/profiling/flamegraph/continuousFlamegraph.tsx`) -> Impact: **275.1** | LOC: 1361
- `IssueListOverview` (@ `static/app/views/issueList/overview.tsx`) -> Impact: **236.3** | LOC: 822
...
| `static/app/components` | 221 | 11216.53 | 10.0% | 21.04% |
| `static/app/utils` | 263 | 10524.62 | 10.15% | 33.45% |
| `src/sentry/api/serializers/models` | 57 | 8541.5 | 42.66% | 11.06% |
| `src/sentry/snuba` | 35 | 8392.02 | 32.92% | 11.07% |
...
- **Heaviest Functions:** `success` (Impact: 27.2), `poll` (Impact: 20.4), `error` (Impact: 7.8)

### 2. `src/sentry/utils/http.py` (PYTHON) -> Cumulative Risk: **794.67**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `is_valid_origin` (Impact: 44.5), `get_origins` (Impact: 13.6), `parse_uri_match` (Impact: 12.5)
...
- **Heaviest Functions:** `get_many` (Impact: 19.7), `resolve_from` (Impact: 13.5), `resolve_from` (Impact: 9.1)
...
- **Heaviest Functions:** `deserializeCanvasArg` (Impact: 39.4), `variableListFor` (Impact: 5.8)
...
- **Heaviest Functions:** `_request` (Impact: 28.4), `_process` (Impact: 23.4), `process_js` (Impact: 10.8)
...
- **Heaviest Functions:** `build_rpc_request` (Impact: 37.7), `get_entity_subscription` (Impact: 28.7), `build_query_builder` (Impact: 23.7)
...
- **Heaviest Functions:** `clean_data` (Impact: 18.9), `_get_teams` (Impact: 6.2), `_get_services` (Impact: 4.7)
...
- **Heaviest Functions:** `_handle` (Impact: 40.1), `is_valid_signature` (Impact: 12.9), `get_secret` (Impact: 6.3)
...
- **Heaviest Functions:** `update_work_item` (Impact: 35.0), `create_work_item` (Impact: 11.7), `_refresh_auth_if_expired` (Impact: 9.9)
...
- **Heaviest Functions:** `get_attrs` (Impact: 110.5), `get_attrs` (Impact: 101.8), `serialize` (Impact: 70.8)
...
**Top Internal Functions/Classes:**
  * `create_eap_uptime_result` (Impact: 106.2)
  * `store_segment` (Impact: 75.3)
  * `login_as` (Impact: 67.0)
    * *Intent:* # TODO(dcramer): ideally superuser_sso would be False by default, but that would require # a lot of ...
  * `store_indexed_span` (Impact: 64.5)
  * `create_processing_error` (Impact: 55.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.005386 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `test_functions_dataset_simple` (Impact: 25.5)
  * `run_test_in_query` (Impact: 15.6)
  * `test_profiles_dataset_simple` (Impact: 15.2)
  * `test_count_if` (Impact: 13.1)
  * `test_semver` (Impact: 12.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_full_stack_repo_only_detects_top_language` (Impact: 18.9)
    * *Intent:* """In a multi-language repo, only the top language's platform is processed. Only one suggestion is s...
  * `test_framework_sort_determines_ranking` (Impact: 11.2)
  * `test_framework_detection_gives_high_confidence` (Impact: 10.9)
  * `test_no_rule_has_match_content_without_file_source` (Impact: 8.9)
  * `test_cloudflare_pages_supersedes_workers` (Impact: 8.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `create_service_hook` (Impact: 44.3)
  * `create_preprod_artifact_size_metrics` (Impact: 40.5)
  * `create_release` (Impact: 40.0)
  * `create_project_rule` (Impact: 30.8)
  * `create_incident` (Impact: 29.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00392 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `run_query` (Impact: 124.4)
  * `resolve_snql_function` (Impact: 61.2)
  * `_get_metrics_query_from_on_demand_spec` (Impact: 57.6)
  * `default_filter_converter` (Impact: 53.9)
  * `__init__` (Impact: 48.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 8.3e-05 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `AskSeerTestComponent` (Impact: 32.1)
  * `mutationFn` (Impact: 30.3)
  * `parameterDependentValueType` (Impact: 11.5)
  * `aggregateGetFieldDefinition` (Impact: 9.5)
  * `getSuggestedFilterKey` (Impact: 4.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `default_filter_converter` (Impact: 130.0)
  * `__init__` (Impact: 86.1)
  * `resolve_function` (Impact: 67.6)
  * `resolve_boolean_conditions` (Impact: 54.4)
  * `resolve_orderby` (Impact: 54.2)
...
**Top Internal Functions/Classes:**
  * `save_error_events` (Impact: 45.0)
  * `_handle_regression` (Impact: 45.0)
  * `_get_severity_metadata_for_group` (Impact: 43.2)
  * `save_attachment` (Impact: 34.1)
  * `save` (Impact: 32.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00238 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `aliased_query_params` (Impact: 124.4)
  * `resolve_column` (Impact: 85.9)
  * `query` (Impact: 65.4)
  * `get_snuba_column_name` (Impact: 60.0)
    * *Intent:* """ Get corresponding Snuba column name from Sentry snuba map, if not found the column is assumed to...
  * `_resolve_column` (Impact: 56.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `update_alert_rule` (Impact: 353.8)
  * `create_alert_rule` (Impact: 146.7)
  * `update_alert_rule_trigger_action` (Impact: 54.6)
  * `create_alert_rule_trigger_action` (Impact: 47.0)
  * `get_column_from_aggregate` (Impact: 43.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 0.092
  * `Choke Point (Betweenness):` 0.001195 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `get_tag_value_paginator_for_projects` (Impact: 232.7)
  * `__get_tag_key_and_top_values` (Impact: 111.3)
  * `__get_tag_keys_for_projects` (Impact: 85.0)
  * `get_group_tag_keys_and_top_values` (Impact: 58.8)
  * `get_generic_groups_user_counts` (Impact: 39.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00039 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `test_snql_wip_project_threshold_config` (Impact: 23.8)
  * `test_count_miserable_function` (Impact: 13.0)
  * `test_user_misery_function` (Impact: 13.0)
  * `test_apdex_function` (Impact: 12.2)
  * `test_failure_rate` (Impact: 12.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 213 instances
* *Amplified Sql Injection:* 29 instances
...
**Top Internal Functions/Classes:**
  * `test_query_status_and_substatus_nonoverlapping` (Impact: 18.0)
  * `test_assigned_to_pagination` (Impact: 15.1)
    * *Intent:* # This seems like a random override, but this test needed a way to override # the orderby being sent...
  * `test_query_status_and_substatus_overlapping` (Impact: 14.0)
  * `test_bulk_delete_for_many_projects_without_option` (Impact: 13.2)
  * `test_semver` (Impact: 13.0)
**Contextual Mitigations & Amplifications:**
...
* *Amplified Sql Injection:* 31 instances
...
- `src/sentry/testutils/cases.py` -> **Severity: 0.539** (Bridge: 0.0054 * Flux: 99.9999%)
- `src/sentry/tasks/post_process.py` -> **Severity: 0.536** (Bridge: 0.0054 * Flux: 99.9997%)
- `src/sentry/models/project.py` -> **Severity: 0.533** (Bridge: 0.0053 * Flux: 99.953%)
- `src/sentry/eventstream/kafka/dispatch.py` -> **Severity: 0.51** (Bridge: 0.0052 * Flux: 98.136%)
- `src/sentry/testutils/factories.py` -> **Severity: 0.392** (Bridge: 0.0039 * Flux: 99.9946%)
