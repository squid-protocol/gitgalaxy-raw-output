...
| Avg Path Length | 3.262 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 25.9 | 23.9 | 0.0 |
...
| connectivity | 4617 | 480 | 15 | `src/engine/events.ts` |
...
| dead_code | 117 | 64 | 0 | `sandbox/src/game.ts` |
...
- `constructor` (@ `src/engine/engine.ts`) -> Impact: **116.7** | LOC: 270
...
- `constructor` (@ `src/engine/graphics/font.ts`) -> Impact: **86.6** | LOC: 34
- `rayCast` (@ `src/engine/collision/detection/sparse-hash-grid-collision-processor.ts`) -> Impact: **77.0** | LOC: 119
- `constructor` (@ `src/engine/actor.ts`) -> Impact: **72.5** | LOC: 120
  * *Intent:* // #endregion /** * * @param config */
- `glTypeToUniformTypeName` (@ `src/engine/graphics/context/shader.ts`) -> Impact: **71.4** | LOC: 112
  * *Intent:* /** * */
- `drawImage` (@ `src/engine/graphics/context/excalibur-graphics-context-webgl.ts`) -> Impact: **68.4** | LOC: 39
- `FindContactSeparation` (@ `src/engine/collision/colliders/collision-jump-table.ts`) -> Impact: **61.5** | LOC: 86
...
- **Heaviest Functions:** `emit` (Impact: 16.8), `off` (Impact: 10.9), `on` (Impact: 5.6)
...
- **Heaviest Functions:** `update` (Impact: 27.9), `_clonePad` (Impact: 8.0), `_isGamepadValid` (Impact: 7.9)
...
- **Heaviest Functions:** `checkAndCombine` (Impact: 45.9), `_updateColliders` (Impact: 30.6), `debug` (Impact: 30.6)
...
- **Heaviest Functions:** `arrayToImage` (Impact: 28.2), `lzwDecode` (Impact: 26.0), `cpRow` (Impact: 20.2)
...
- **Heaviest Functions:** `coordPlane` (Impact: 6.1), `onAdd` (Impact: 4.9), `deserialize` (Impact: 4.8)
...
- **Heaviest Functions:** `addChannel` (Impact: 13.3), `play` (Impact: 13.0), `toggle` (Impact: 12.5)
...
- **Heaviest Functions:** `isLoaderConstructor` (Impact: 8.6), `load` (Impact: 7.7), `constructor` (Impact: 7.3)
...
- **Heaviest Functions:** `update` (Impact: 16.4), `__runScheduledCbs` (Impact: 11.0), `constructor` (Impact: 10.4)
...
- **Heaviest Functions:** `constructor` (Impact: 27.4), `play` (Impact: 24.1), `wireEngine` (Impact: 9.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.044531 | `Ripple Effect (Closeness):` 0.116164
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.006675 | `Ripple Effect (Closeness):` 0.086209
...
**Top Internal Functions/Classes:**
  ...
  * `applyResolutionAndViewport` (Impact: 17.2)
  * `_setResolutionAndViewportByDisplayMode` (Impact: 15.4)
    * *Intent:* /** * Sets the resolution and viewport based on the selected display mode. */
  * `_computeFitAndFill` (Impact: 14.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001091 | `Ripple Effect (Closeness):` 0.073064
...
**Top Internal Functions/Classes:**
  * `arrayToImage` (Impact: 28.2)
  * `lzwDecode` (Impact: 26.0)
  * `cpRow` (Impact: 20.2)
  * `parseUnknownExt` (Impact: 14.3)
  * `parseUnknownAppExt` (Impact: 11.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 5 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.012145 | `Ripple Effect (Closeness):` 0.065641
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.059036
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000204 | `Ripple Effect (Closeness):` 0.061565
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.007547 | `Ripple Effect (Closeness):` 0.057329
...
**Top Internal Functions/Classes:**
  * `constructor` (Impact: 72.5)
    * *Intent:* // #endregion /** * * @param config */
  * `draggable` (Impact: 10.7)
  * `contains` (Impact: 9.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.020205 | `Ripple Effect (Closeness):` 0.092595
...
**Top Internal Functions/Classes:**
  ...
  * `constructor` (Impact: 16.6)
  * `isDataWithId` (Impact: 16.5)
...
**Top Internal Functions/Classes:**
  * `constructor` (Impact: 40.6)
  * `deserialize` (Impact: 32.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.008628 | `Ripple Effect (Closeness):` 0.074195
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001117 | `Ripple Effect (Closeness):` 0.083578
...
**Top Internal Functions/Classes:**
  ...
  * `_getClassHierarchyRoot` (Impact: 9.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.019739 | `Ripple Effect (Closeness):` 0.108698
...
**Top Internal Functions/Classes:**
  ...
  * `_addImageAsTexture` (Impact: 8.4)
  * `_getTextureIdForImage` (Impact: 6.0)
  * `_bindTextures` (Impact: 4.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000171 | `Ripple Effect (Closeness):` 0.053291
...
- `src/engine/engine.ts` -> **Severity: 4.453** (Bridge: 0.0445 * Flux: 99.9882%)
- `src/engine/scene.ts` -> **Severity: 2.011** (Bridge: 0.0203 * Flux: 99.2333%)
- `src/engine/actor.ts` -> **Severity: 2.006** (Bridge: 0.0202 * Flux: 99.2865%)
- `src/engine/entity-component-system/entity.ts` -> **Severity: 1.974** (Bridge: 0.0197 * Flux: 99.9826%)
- `src/engine/util/log.ts` -> **Severity: 1.603** (Bridge: 0.016 * Flux: 100.0%)
