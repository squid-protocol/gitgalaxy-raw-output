...
| Analyzed Artifacts (Scanned) | 14342 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 1593 |
| Total LOC | 5939 |
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 84.4 | 4.9 | 0.0 | 0.0 |
...
| Mutation Surface (formerly State Flux Exposure) | 0.0 | 100.0 | 5.2 | 0.0 | 0.0 |
...
- `postAboutDangerousContent` (@ `scripts/analyze-pr-build.js`) -> Impact: **77.0** | LOC: 100
...
- `postAboutFlaws` (@ `scripts/analyze-pr-build.js`) -> Impact: **51.2** | LOC: 88
...
- `checkFile` (@ `scripts/filecheck/checker.js`) -> Impact: **48.1** | LOC: 131
...
- `analyzePR` (@ `scripts/analyze-pr-build.js`) -> Impact: **41.3** | LOC: 99
...
- `checkCompression` (@ `scripts/filecheck/checker.js`) -> Impact: **35.4** | LOC: 85
...
- `coerce` (@ `scripts/analyze-pr-build.js`) -> Impact: **19.4** | LOC: 48
- `movedFiles` (@ `scripts/update-moved-file-links.js`) -> Impact: **18.2** | LOC: 22
...
| `files/sidebars` | 13 | 218.44 | 1.13% | 0.0% |
...
- **Heaviest Functions:** `checkFrontMatter` (Impact: 40.1), `getAjvValidator` (Impact: 3.1), `areAttributesInOrder` (Impact: 2.0)
...
- **Heaviest Functions:** `resolveDirectory` (Impact: 16.7), `lintFrontMatter` (Impact: 14.3)
...
- **Heaviest Functions:** `equalsIgnoreCase` (Impact: 26.0)
...
- **Heaviest Functions:** `movedFiles` (Impact: 18.2), `getImageSlug` (Impact: 7.6)
...
- **Heaviest Functions:** `postAboutDangerousContent` (Impact: 77.0), `postAboutFlaws` (Impact: 51.2), `analyzePR` (Impact: 41.3)
...
- **Heaviest Functions:** `execGit` (Impact: 15.3), `getLocations` (Impact: 9.9), `walkSync` (Impact: 6.2)
...
- **Heaviest Functions:** `getDeletedSlugs` (Impact: 17.7), `getFragmentDetails` (Impact: 12.6), `getFileAnchors` (Impact: 4.8)
...
- **Heaviest Functions:** `updateReleaseNotes` (Impact: 25.5), `main` (Impact: 16.3), `deactivatePreviousStable` (Impact: 5.7)
...
- **Heaviest Functions:** `correctContentPathFromEnv` (Impact: 9.3), `correctPathFromEnv` (Impact: 3.2), `parseEnvValue` (Impact: 1.9)
...
- **Heaviest Functions:** `checkFile` (Impact: 48.1), `checkCompression` (Impact: 35.4), `resolveDirectory` (Impact: 9.3)
...
**Top Internal Functions/Classes:**
  ...
  * `deactivatePreviousStable` (Impact: 5.7)
...
**Top Internal Functions/Classes:**
  * `movedFiles` (Impact: 18.2)
  * `getImageSlug` (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `getDeletedSlugs` (Impact: 17.7)
  * `getFragmentDetails` (Impact: 12.6)
  * `getFileAnchors` (Impact: 4.8)
  * `getFileContent` (Impact: 3.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `getAjvValidator` (Impact: 3.1)
  * `areAttributesInOrder` (Impact: 2.0)
  * `getRelativePath` (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `resolveDirectory` (Impact: 16.7)
  * `lintFrontMatter` (Impact: 14.3)
...
**Top Internal Functions/Classes:**
  ...
  * `walkSync` (Impact: 6.2)
  * `stringToFragment` (Impact: 4.6)
    * *Intent:* /* * Convert Markdown header into URL slug. */
  * `isImagePath` (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
