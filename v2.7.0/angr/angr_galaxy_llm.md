...
| Avg Path Length | 3.2844 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 17.9 | 8.3 | 0.0 |
...
| connectivity | 12195 | 1362 | 16 | `tests/analyses/decompiler/test_decompiler.py` |
...
| dead_code | 2983 | 653 | 3 | `tests/analyses/decompiler/test_decompiler.py` |
...
- `_try_resolve_targets_load` (@ `angr/analyses/cfg/indirect_jump_resolvers/jumptable.py`) -> Impact: **343.6** | LOC: 371
- `lift_vex` (@ `angr/engines/vex/lifter.py`) -> Impact: **328.2** | LOC: 213
- `__init__` (@ `angr/analyses/cfg/cfg_fast.py`) -> Impact: **320.4** | LOC: 304
- `lift_pcode` (@ `angr/engines/pcode/lifter.py`) -> Impact: **311.5** | LOC: 214
- `_generate_cfgnode` (@ `angr/analyses/cfg/cfg_fast.py`) -> Impact: **299.6** | LOC: 391
...
- `_make_switch_cases_core` (@ `angr/analyses/decompiler/structuring/phoenix.py`) -> Impact: **297.6** | LOC: 183
- `_process_vex_irsb` (@ `angr/analyses/stack_pointer_tracker.py`) -> Impact: **283.4** | LOC: 256
- `_unify_local_variables` (@ `angr/analyses/decompiler/ail_simplifier.py`) -> Impact: **264.3** | LOC: 449
  * *Intent:* # # Unifying local variables # """ Find variables that are definitely equivalent and then eliminate unnecessary copies. """
- `_determine` (@ `angr/analyses/typehoon/simple_solver.py`) -> Impact: **263.9** | LOC: 251
- `_resolve` (@ `angr/analyses/cfg/indirect_jump_resolvers/jumptable.py`) -> Impact: **251.2** | LOC: 283
...
- **Heaviest Functions:** `_standardize_value` (Impact: 151.4), `_classify` (Impact: 56.1), `setup_callsite` (Impact: 53.4)
...
- **Heaviest Functions:** `__init__` (Impact: 35.0), `remove_node` (Impact: 27.5), `remove_edge` (Impact: 23.2)
...
- **Heaviest Functions:** `__init__` (Impact: 62.8), `has_atom` (Impact: 37.0), `replace` (Impact: 35.7)
...
- **Heaviest Functions:** `new_dtv` (Impact: 42.0), `replace` (Impact: 34.5), `replace` (Impact: 34.5)
...
- **Heaviest Functions:** `replace` (Impact: 21.2), `replace` (Impact: 21.2), `replace` (Impact: 16.6)
...
- **Heaviest Functions:** `__init__` (Impact: 14.9), `__init__` (Impact: 14.9), `__init__` (Impact: 14.9)
...
- **Heaviest Functions:** `pre_test` (Impact: 50.6), `__init__` (Impact: 1.5), `num_args` (Impact: 1.5)
...
- **Heaviest Functions:** `handle_CDoWhileLoop` (Impact: 105.6), `handle_CAssignment` (Impact: 28.8), `handle_CBinaryOp` (Impact: 23.9)
...
- **Heaviest Functions:** `__init__` (Impact: 28.3), `run` (Impact: 18.1), `on_worker_exit` (Impact: 4.5)
...
- **Heaviest Functions:** `function` (Impact: 50.8), `_add_call_to` (Impact: 37.6), `_add_outside_transition_to` (Impact: 28.5)
...
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 320.4)
  * `_generate_cfgnode` (Impact: 299.6)
    * *Intent:* # # Other methods # """ Generate a CFGNode that starts at `cfg_job.addr`. Since lifting machine code...
  * `_process_block_arch_specific` (Impact: 242.8)
  * `_create_jobs` (Impact: 172.2)
  * `_remove_redundant_overlapping_blocks` (Impact: 130.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003941 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_access` (Impact: 138.3)
  * `_handle_Expr_Const` (Impact: 130.2)
  * `full_c_repr_chunks` (Impact: 122.3)
  * `_access_constant_offset` (Impact: 114.4)
  * `c_repr` (Impact: 90.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003142 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_make_switch_cases_core` (Impact: 297.6)
  * `_match_acyclic_switch_cases_address_loaded_from_memory` (Impact: 179.9)
  * `_refine_cyclic_core` (Impact: 178.5)
  * `_match_acyclic_ite` (Impact: 152.8)
    * *Intent:* """ Check if start_node is the beginning of an If-Then-Else region. Create a Condition node if it is...
  * `_find_node_going_to_dst` (Impact: 124.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002197 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_decl_to_type` (Impact: 237.4)
  * `parse_file` (Impact: 60.8)
  * `from_json` (Impact: 54.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.021196 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `test_decompiling_rep_stosq` (Impact: 36.2)
  * `_check_rep_stosq` (Impact: 34.0)
    * *Intent:* """ Example: for (v7 = 32; v7; v6 += 1) { v7 -= 1; *(v6) = 0; }
  * `test_decompiling_rust_fmt_main` (Impact: 33.7)
  * `test_decompiling_msvcrt_setsbuplow` (Impact: 31.5)
  * `test_decompiling_1after909_doit` (Impact: 29.4)
...
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 188.7)
  * `_link_variables_on_expr` (Impact: 184.4)
  * `_recover_and_link_variables` (Impact: 118.6)
  * `_fix_abnormal_switch_case_heads_case2` (Impact: 113.3)
  * `_recover_calling_conventions` (Impact: 91.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.010711 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `State::process_vex_expr` (Impact: 210.1)
  * `State::handle_write` (Impact: 188.7)
  * `State::propagate_taint_of_mem_read_instr_and_continue` (Impact: 133.1)
  * `State::process_vex_block` (Impact: 126.9)
  * `State::propagate_taint_of_one_stmt` (Impact: 84.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001374 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_graph_traversal_handler` (Impact: 212.9)
  * `__init__` (Impact: 197.7)
  * `_normalize_core` (Impact: 158.1)
  * `_executable_memory_regions` (Impact: 110.7)
    * *Intent:* """ Get all executable memory regions from the binaries :param objects: A collection of binary objec...
  * `_is_tail_call_optimization` (Impact: 107.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001006 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001589 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `assembly` (Impact: 201.2)
  * `_initialize` (Impact: 108.6)
    * *Intent:* # # Private methods # """ Initialize the binary. :return: None """
  * `new_label` (Impact: 72.4)
  * `assembly` (Impact: 63.8)
    * *Intent:* """ :return: """
  * `_initialize` (Impact: 58.7)
    * *Intent:* # # Private methods #
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.005142 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002232 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_try_resolve_targets_load` (Impact: 343.6)
  * `_resolve` (Impact: 251.2)
    * *Intent:* # # Private methods #
  * `_handle_Comparison` (Impact: 66.6)
  * `_all_qualified_load_stmts_in_slice` (Impact: 63.0)
    * *Intent:* """ Recognize all qualified load statements in a slice. A qualified load statements refers to those ...
  * `_get_secondary_jumptable_from_transformations` (Impact: 46.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002385 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.011809 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001538 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 0.333
  * `Choke Point (Betweenness):` 0.000323 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `render` (Impact: 139.4)
  * `__init__` (Impact: 80.8)
  * `_render` (Impact: 74.7)
  * `dissect_instruction_by_default` (Impact: 51.7)
...
**Top Internal Functions/Classes:**
  * `assign_unified_variable_names` (Impact: 92.7)
  * `parse_from_cmessage` (Impact: 70.3)
  * `record_variable` (Impact: 52.5)
  * `set_variable_type` (Impact: 50.3)
  * `assign_variable_names` (Impact: 50.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001491 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5.6e-05 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002209 | `Ripple Effect (Closeness):` 0.0
...
- `angr/project.py` -> **Severity: 5.036** (Bridge: 0.0504 * Flux: 100.0%)
- `angr/analyses/analysis.py` -> **Severity: 4.679** (Bridge: 0.0468 * Flux: 100.0%)
- `angr/sim_state.py` -> **Severity: 4.643** (Bridge: 0.0464 * Flux: 100.0%)
- `angr/sim_type.py` -> **Severity: 2.12** (Bridge: 0.0212 * Flux: 100.0%)
- `angr/ailment/expression.py` -> **Severity: 1.181** (Bridge: 0.0118 * Flux: 100.0%)
