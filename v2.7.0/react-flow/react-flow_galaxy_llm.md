...
| Avg Path Length | 3.2224 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 15.1 | 4.6 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 7.9 | 1.0 | 0.0 | 0.0 |
...
- `getDimensionsAfterResize` (@ `packages/system/src/xyresizer/utils.ts`) -> Impact: **254.3** | LOC: 167
...
- `getInitialStore` (@ `packages/svelte/src/lib/store/initial-store.svelte.ts`) -> Impact: **185.2** | LOC: 366
- `createStore` (@ `packages/svelte/src/lib/store/index.ts`) -> Impact: **146.7** | LOC: 389
- `update` (@ `packages/system/src/xydrag/XYDrag.ts`) -> Impact: **138.9** | LOC: 289
  * *Intent:* // public functions
- `XYResizer` (@ `packages/system/src/xyresizer/XYResizer.ts`) -> Impact: **108.6** | LOC: 249
- `update` (@ `packages/system/src/xyresizer/XYResizer.ts`) -> Impact: **107.4** | LOC: 225
- `NodeWrapper` (@ `packages/react/src/components/NodeWrapper/index.tsx`) -> Impact: **102.0** | LOC: 229
- `EdgeWrapper` (@ `packages/react/src/components/EdgeWrapper/index.tsx`) -> Impact: **101.5** | LOC: 249
- `Pane` (@ `packages/react/src/container/Pane/index.tsx`) -> Impact: **99.3** | LOC: 232
- `XYPanZoom` (@ `packages/system/src/xypanzoom/XYPanZoom.ts`) -> Impact: **79.8** | LOC: 266
...
- **Heaviest Functions:** `createPanOnScrollHandler` (Impact: 28.1), `createPanZoomEndHandler` (Impact: 19.1), `createPanZoomHandler` (Impact: 15.2)

### 2. `packages/svelte/src/lib/store/initial-store.svelte.ts` (TYPESCRIPT) -> Cumulative Risk: **688.9**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 381.6 | **LOC:** 489 | **CtrlFlow:** 32.6% | **Authorship Centralization:** 28.6%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (93.75%), Cognitive Load (87.9296%), State Flux (84.9353%)
- **Heaviest Functions:** `getInitialStore` (Impact: 185.2), `getInitialViewport` (Impact: 30.0), `filter` (Impact: 10.3)
...
- **Heaviest Functions:** `createStore` (Impact: 146.7), `updateNodeInternals` (Impact: 31.1), `moveSelectedNodes` (Impact: 24.9)
...
- **Heaviest Functions:** `updateNodeInternals` (Impact: 76.0), `handleExpandParent` (Impact: 55.8), `adoptUserNodes` (Impact: 47.9)
...
- **Heaviest Functions:** `update` (Impact: 138.9), `updateNodes` (Impact: 43.2), `startDrag` (Impact: 30.6)
...
- **Heaviest Functions:** `updateNodePositions` (Impact: 35.6), `createStore` (Impact: 21.3), `setCenter` (Impact: 15.0)
...
- **Heaviest Functions:** `applyChanges` (Impact: 33.7), `applyChange` (Impact: 26.4), `getElementsDiffChanges` (Impact: 17.2)

### 8. `packages/system/src/xyresizer/XYResizer.ts` (TYPESCRIPT) -> Cumulative Risk: **577.78**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 329.94 | **LOC:** 353 | **CtrlFlow:** 27.1% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9903%), Safety Score (82.9673%)
- **Heaviest Functions:** `XYResizer` (Impact: 108.6), `update` (Impact: 107.4), `nodeToChildExtent` (Impact: 10.7)

### 9. `packages/system/src/xyhandle/XYHandle.ts` (TYPESCRIPT) -> Cumulative Risk: **574.19**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `onPointerDown` (Impact: 68.4), `isValidHandle` (Impact: 41.6), `onPointerMove` (Impact: 22.0)
...
- **Heaviest Functions:** `getClosestHandle` (Impact: 51.6), `getHandle` (Impact: 46.1), `getHandleType` (Impact: 14.6)
...
**Top Internal Functions/Classes:**
  * `updateNodeInternals` (Impact: 76.0)
  * `handleExpandParent` (Impact: 55.8)
  * `adoptUserNodes` (Impact: 47.9)
  * `updateChildNode` (Impact: 39.4)
...
**Top Internal Functions/Classes:**
  * `createStore` (Impact: 146.7)
  * `updateNodeInternals` (Impact: 31.1)
  * `moveSelectedNodes` (Impact: 24.9)
  * `handleNodeSelection` (Impact: 20.9)
  * `setCenter` (Impact: 16.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
**Top Internal Functions/Classes:**
  * `getInitialStore` (Impact: 185.2)
  * `getInitialViewport` (Impact: 30.0)
  * `filter` (Impact: 10.3)
  * `resolveFitView` (Impact: 8.9)
  * `warnIfDeeplyReactive` (Impact: 7.4)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.7e-05 | `Ripple Effect (Closeness):` 0.003456
...
**Top Internal Functions/Classes:**
  * `parsePaddings` (Impact: 41.4)
    * *Intent:* /** * Parses the paddings to an object with top, right, bottom, left, x and y paddings * @internal *...
  * `getNodeDimensions` (Impact: 31.7)
  * `nodeToRect` (Impact: 28.4)
  * `nodeToBox` (Impact: 28.4)
  * `evaluateAbsolutePosition` (Impact: 22.9)
...
**Top Internal Functions/Classes:**
  * `XYResizer` (Impact: 108.6)
  * `update` (Impact: 107.4)
  * `nodeToChildExtent` (Impact: 10.7)
  * `getStoreItems` (Impact: 5.6)
  * `nodeToParentExtent` (Impact: 1.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `update` (Impact: 138.9)
    * *Intent:* // public functions
  * `updateNodes` (Impact: 43.2)
  * `startDrag` (Impact: 30.6)
  * `autoPan` (Impact: 11.3)
  * `destroy` (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.5e-05 | `Ripple Effect (Closeness):` 0.004608
...
**Top Internal Functions/Classes:**
  * `useSvelteFlow` (Impact: 28.0)
    * *Intent:* /** * Hook for accessing the SvelteFlow instance. * * @public * @returns A set of helper functions *...
  * `getIntersectingNodes` (Impact: 23.4)
  * `fitBounds` (Impact: 16.7)
  * `setViewport` (Impact: 14.8)
  * `getNodeRect` (Impact: 13.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `useReactFlow` (Impact: 70.9)
    * *Intent:* * setCount(reactFlow.getNodes().length); * // you need to pass it as a dependency if you are using i...
  * `getIntersectingNodes` (Impact: 25.4)
  * `getNodeRect` (Impact: 13.7)
  * `deleteElements` (Impact: 13.4)
  * `isNodeIntersecting` (Impact: 12.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.5e-05 | `Ripple Effect (Closeness):` 0.006912
...
**Top Internal Functions/Classes:**
  * `Pane` (Impact: 99.3)
  * `onPointerMove` (Impact: 33.8)
  * `onPointerUp` (Impact: 28.0)
  * `onPointerDownCapture` (Impact: 21.7)
    * *Intent:* // We are using capture here in order to prevent other pointer events // to be able to create a sele...
  * `onContextMenu` (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `createStore` (Impact: 21.3)
  * `setCenter` (Impact: 15.0)
  * `updateNodeInternals` (Impact: 13.5)
  * `unselectNodesAndEdges` (Impact: 13.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `onPointerDown` (Impact: 68.4)
  * `isValidHandle` (Impact: 41.6)
    * *Intent:* // checks if and returns connection in form of an object { source: 123, target: 312 }
  * `onPointerMove` (Impact: 22.0)
  * `onPointerUp` (Impact: 20.5)
  * `autoPan` (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  * `XYPanZoom` (Impact: 79.8)
  * `update` (Impact: 22.4)
    * *Intent:* // public functions
  * `setTransform` (Impact: 14.5)
  * `scaleTo` (Impact: 14.5)
  * `scaleBy` (Impact: 14.5)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.003072
...
**Top Internal Functions/Classes:**
  * `HandleComponent` (Impact: 75.5)
  * `connectingSelector` (Impact: 45.0)
  * `onClick` (Impact: 31.6)
  * `onPointerDown` (Impact: 9.9)
  * `onConnectEnd` (Impact: 8.7)
**Contextual Mitigations & Amplifications:**
...
- `packages/svelte/src/lib/store/initial-store.svelte.ts` -> Churn: **76.79%** | Cog Load: 87.9296% | Debt: 0.0%
...
- `packages/svelte/src/lib/container/Zoom/Zoom.svelte` -> **peterkogo** (100.0% isolated ownership) | Magnitude: 189.45
...
- `packages/svelte/src/lib/store/initial-store.svelte.ts` -> **Severity: 0.007** (Bridge: 0.0001 * Flux: 84.9353%)
- `packages/react/src/additional-components/MiniMap/MiniMap.tsx` -> **Severity: 0.003** (Bridge: 0.0 * Flux: 98.6166%)
- `packages/react/src/hooks/useOnInitHandler.ts` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 50.0%)
- `packages/system/src/utils/graph.ts` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 100.0%)
