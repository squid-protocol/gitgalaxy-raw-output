...
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 191 |
| Total LOC | 302425 |
...
| Avg Path Length | 4.2105 | Hops between files. Lower = Tighter coupling. |
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 99.9 | 29.5 | 0.0 | 0.0 |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 10.3 | 3.5 | 0.0 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 63.5 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 39.2 | 12.9 | 0.0 |
...
| docs | 18071 | 723 | 28 | `tsdb/db_test.go` |
| debt | 739 | 197 | 1 | `promql/parser/generated_parser.y` |
| mutation | 35588 | 804 | 65 | `tsdb/db_test.go` |
| dead_code | 3571 | 562 | 7 | `tsdb/head_test.go` |
| credential | 14 | 11 | 0 | `promql/engine_test.go` |
| threat | 149 | 50 | 0 | `discovery/registry.go` |
...
- `aggregation` (@ `promql/engine.go`) -> Impact: **315.2** | LOC: 364
...
- `refresh` (@ `discovery/aws/rds.go`) -> Impact: **268.2** | LOC: 528
- `eval` (@ `promql/engine.go`) -> Impact: **254.7** | LOC: 557
  * *Intent:* // eval evaluates the given expression as the given AST expression node requires.
- `loadWAL` (@ `tsdb/head_wal.go`) -> Impact: **242.2** | LOC: 434
- `analyzeCompletion` (@ `web/ui/module/codemirror-promql/src/complete/hybrid.ts`) -> Impact: **240.0** | LOC: 320
...
- `main` (@ `cmd/prometheus/main.go`) -> Impact: **228.8** | LOC: 1215
- `test` (@ `cmd/promtool/unittest.go`) -> Impact: **225.3** | LOC: 332
  * *Intent:* // test performs the unit tests.
- `Checkpoint` (@ `tsdb/wlog/checkpoint.go`) -> Impact: **215.9** | LOC: 301
...
- `aggregationK` (@ `promql/engine.go`) -> Impact: **208.9** | LOC: 219
...
- `unmarshalWithoutLabels` (@ `prompb/io/prometheus/client/decoder.go`) -> Impact: **175.1** | LOC: 281
...
| `storage/remote` | 29 | 6350.4 | 15.79% | 54.89% |
...
### 1. `scrape/manager.go` (GO) -> Cumulative Risk: **687.68**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 289.6 | **LOC:** 516 | **CtrlFlow:** 17.1% | **Authorship Centralization:** 55.6%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.8141%), Tech Debt (98.8618%), Concurrency (91.4771%)
- **Heaviest Functions:** `ApplyConfig` (Impact: 33.6), `NewManager` (Impact: 26.4), `Run` (Impact: 13.7)

### 2. `promql/parser/generated_parser.y` (YACC) -> Cumulative Risk: **674.61**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1310.9 | **LOC:** 1356 | **CtrlFlow:** 25.6% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.9804%), Safety Score (96.2589%)
- **Heaviest Functions:** `maybe_label` (Impact: 36.0), `metric_identifier` (Impact: 31.1), `binary_expr` (Impact: 26.4)

### 3. `cmd/prometheus/main.go` (GO) -> Cumulative Risk: **650.29**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1090.86 | **LOC:** 2181 | **CtrlFlow:** 16.9% | **Authorship Centralization:** 25.9%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.752%), Churn (96.15%), Verification (80.0%)
- **Heaviest Functions:** `main` (Impact: 228.8), `setFeatureListOptions` (Impact: 68.2), `reloadConfig` (Impact: 26.1)

### 4. `storage/merge.go` (GO) -> Cumulative Risk: **648.66**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 630.32 | **LOC:** 920 | **CtrlFlow:** 19.1% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9991%), Safety Score (88.9991%), Tech Debt (86.0388%)
- **Heaviest Functions:** `NewMergeQuerier` (Impact: 21.4), `NewMergeChunkQuerier` (Impact: 21.4), `Next` (Impact: 20.4)

### 5. `tsdb/head_wal.go` (GO) -> Cumulative Risk: **636.41**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1540.56 | **LOC:** 1876 | **CtrlFlow:** 23.1% | **Authorship Centralization:** 20.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9998%), Concurrency (93.3316%), Safety Score (86.0958%)
- **Heaviest Functions:** `loadWAL` (Impact: 242.2), `loadWBL` (Impact: 133.8), `processWALSamples` (Impact: 82.2)

### 6. `discovery/kubernetes/kubernetes.go` (GO) -> Cumulative Risk: **632.9**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 792.44 | **LOC:** 1003 | **CtrlFlow:** 15.4% | **Authorship Centralization:** 80.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.9992%), State Flux (99.9952%), Safety Score (83.6533%)
- **Heaviest Functions:** `Run` (Impact: 70.5), `New` (Impact: 31.6), `UnmarshalYAML` (Impact: 31.5)

### 7. `scrape/scrape_test.go` (GO) -> Cumulative Risk: **631.45**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3065.8 | **LOC:** 6959 | **CtrlFlow:** 6.1% | **Authorship Centralization:** 42.9%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.6474%), Concurrency (97.977%), Documentation (86.0544%)
- **Heaviest Functions:** `testConvertClassicHistogramsToNHCB` (Impact: 102.0), `testScrapeLoopAppend` (Impact: 34.3), `TestScrapeLoopAppend_WithStorage` (Impact: 30.6)

### 8. `notifier/sendloop.go` (GO) -> Cumulative Risk: **627.4**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 147.54 | **LOC:** 274 | **CtrlFlow:** 16.4% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.8482%), Concurrency (98.7621%)
- **Heaviest Functions:** `sendAll` (Impact: 13.4), `add` (Impact: 11.8), `loop` (Impact: 10.1)

### 9. `tsdb/chunkenc/varbit.go` (GO) -> Cumulative Risk: **623.04**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 212.92 | **LOC:** 264 | **CtrlFlow:** 33.5% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9998%), Tech Debt (88.6046%)
- **Heaviest Functions:** `readVarbitInt` (Impact: 33.0), `readVarbitUint` (Impact: 31.4), `putVarbitInt` (Impact: 20.6)

### 10. `tsdb/agent/series_test.go` (GO) -> Cumulative Risk: **620.68**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 80.12 | **LOC:** 151 | **CtrlFlow:** 6.6% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Concurrency (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.3025%)
- **Heaviest Functions:** `TestNoDeadlock` (Impact: 10.9), `labelsWithHashCollision` (Impact: 3.9), `stripeSeriesWithCollidingSeries` (Impact: 2.2)
...
- **Risk Profile:** Cognitive Load (14.201%), Tech Debt (44.1453%)
**Top Internal Functions/Classes:**
  * `testOOOInterleavedImplicitCounterResets` (Impact: 65.0)
  * `TestOOOHistogramCompactionWithCounterResets` (Impact: 64.4)
  * `testChunkQuerierOOOQuery` (Impact: 48.9)
  * `TestStaleSeriesCompaction` (Impact: 30.8)
  * `testHistogramAppendAndQueryHelper` (Impact: 28.6)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 3065.8 | **LOC:** 6959 | **CtrlFlow:** 6.1% | **Authorship Centralization:** 42.9%
- **Risk Profile:** Cognitive Load (43.6696%), Tech Debt (53.7535%)
**Top Internal Functions/Classes:**
  * `testConvertClassicHistogramsToNHCB` (Impact: 102.0)
  * `testScrapeLoopAppend` (Impact: 34.3)
  * `TestScrapeLoopAppend_WithStorage` (Impact: 30.6)
    * *Intent:* // TestScrapeLoopAppend_WithStorage tests appends and storage integration for the // large input fil...
  * `testScrapeLoopRun` (Impact: 29.7)
  * `testScrapeLoopRunCreatesStaleMarkersOnSampleLimit` (Impact: 24.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (13.755%), Tech Debt (57.9614%)
**Top Internal Functions/Classes:**
  * `BenchmarkLoadWLs` (Impact: 58.8)
  * `TestHead_HighConcurrencyReadAndWrite` (Impact: 56.6)
    * *Intent:* // TestHead_HighConcurrencyReadAndWrite generates 1000 series with a step of 15s and fills a whole b...
  * `testHistogramStaleSampleHelper` (Impact: 56.0)
  * `TestHistogramInWALAndMmapChunk` (Impact: 51.5)
  * `TestChunkSnapshot` (Impact: 41.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (13.1362%), Tech Debt (38.5784%)
**Top Internal Functions/Classes:**
  * `testOOOInterleavedImplicitCounterResetsV2` (Impact: 65.0)
  * `TestOOOHistogramCompactionWithCounterResets_AppendV2` (Impact: 64.4)
  * `testChunkQuerierOOOQueryAppendV2` (Impact: 48.9)
  * `testHistogramAppendAndQueryHelperAppendV2` (Impact: 28.8)
  * `testQuerierOOOQueryAppendV2` (Impact: 27.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (5.8488%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (35.3668%), Tech Debt (26.4172%)
**Top Internal Functions/Classes:**
  * `open` (Impact: 130.4)
...
- **Risk Profile:** Cognitive Load (13.1025%), Tech Debt (45.9547%)
**Top Internal Functions/Classes:**
  * `testHeadAppenderV2AppendStaleHistogram` (Impact: 56.0)
  * `TestHistogramInWALAndMmapChunk_AppenderV2` (Impact: 51.5)
  * `TestChunkSnapshot_AppenderV2` (Impact: 44.5)
  * `TestHeadAppenderV2_Delete_e2e` (Impact: 30.5)
  * `TestHeadAppenderV2_Append_DifferentEncodingSameSeries` (Impact: 26.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.010733
...
- **Risk Profile:** Cognitive Load (30.8569%), Tech Debt (26.3428%)
**Top Internal Functions/Classes:**
  * `runShard` (Impact: 49.6)
  * `populateV2TimeSeries` (Impact: 43.1)
  * `sendWriteRequestWithBackoff` (Impact: 35.2)
  * `NewQueueManager` (Impact: 31.9)
    * *Intent:* // NewQueueManager builds a new QueueManager and starts a new // WAL watcher with queue manager as t...
  * `sendV2SamplesWithBackoff` (Impact: 31.6)
...
- **Risk Profile:** Cognitive Load (29.8404%), Tech Debt (9.028%)
**Top Internal Functions/Classes:**
  * `Init` (Impact: 74.1)
...
- `discovery/http/http.go` -> **Severity: 0.017** (Bridge: 0.0002 * Flux: 99.6403%)
- `web/ui/mantine-ui/src/components/Accordion/Accordion.tsx` -> **Severity: 0.007** (Bridge: 0.0001 * Flux: 98.5211%)
- `discovery/targetgroup/targetgroup.go` -> **Severity: 0.006** (Bridge: 0.0001 * Flux: 99.9964%)
- `web/ui/react-app/src/pages/graph/Panel.tsx` -> **Severity: 0.006** (Bridge: 0.0001 * Flux: 100.0%)
- `tsdb/tombstones/tombstones.go` -> **Severity: 0.004** (Bridge: 0.0 * Flux: 99.3887%)
