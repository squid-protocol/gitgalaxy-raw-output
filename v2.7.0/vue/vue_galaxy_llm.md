...
| Avg Path Length | 3.479 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 20.7 | 14.9 | 0.0 |
...
| connectivity | 1393 | 297 | 7 | `packages/compiler-sfc/test/rewriteDefault.spec.ts` |
...
| dead_code | 329 | 117 | 2 | `packages/compiler-sfc/src/compileScript.ts` |
...
- `compileScript` (@ `packages/compiler-sfc/src/compileScript.ts`) -> Impact: **575.0** | LOC: 1176
...
- `createPatchFunction` (@ `src/core/vdom/patch.ts`) -> Impact: **381.5** | LOC: 842
- `parseHTML` (@ `src/compiler/parser/html-parser.ts`) -> Impact: **162.5** | LOC: 271
- `doWatch` (@ `src/v3/apiWatch.ts`) -> Impact: **144.0** | LOC: 200
- `addHandler` (@ `src/compiler/helpers.ts`) -> Impact: **115.0** | LOC: 79
...
- `enter` (@ `src/platforms/web/runtime/modules/transition.ts`) -> Impact: **100.7** | LOC: 143
...
- **Heaviest Functions:** `resolveAsyncComponent` (Impact: 68.4), `forceRender` (Impact: 41.3), `ensureCtor` (Impact: 10.7)
...
- **Heaviest Functions:** `getTransitionInfo` (Impact: 34.7), `whenTransitionEnds` (Impact: 13.4), `resolveTransition` (Impact: 12.2)
...
- **Heaviest Functions:** `compileModule` (Impact: 26.6), `createBundleRunner` (Impact: 25.7), `evaluateModule` (Impact: 23.6)
...
- **Heaviest Functions:** `getPreloadType` (Impact: 16.3), `renderPreloadLinks` (Impact: 15.5), `render` (Impact: 13.9)
...
- **Heaviest Functions:** `genElement` (Impact: 66.6), `genScopedSlots` (Impact: 42.8), `genChildren` (Impact: 40.8)
...
- **Heaviest Functions:** `addHandler` (Impact: 115.0), `addDirective` (Impact: 19.2), `addAttr` (Impact: 15.3)
...
- **Heaviest Functions:** `createWriteFunction` (Impact: 11.9), `cachedWrite` (Impact: 11.7), `noop` (Impact: 10.9)
...
- **Heaviest Functions:** `update` (Impact: 19.5), `notify` (Impact: 12.4), `depend` (Impact: 9.0)
...
- **Heaviest Functions:** `RenderRate` (Impact: 4.9), `ping` (Impact: 3.8), `rAFloop` (Impact: 1.2)
...
- **Heaviest Functions:** `nextTick` (Impact: 16.8), `flushCallbacks` (Impact: 2.4), `timerFunc` (Impact: 2.2)
...
**Top Internal Functions/Classes:**
  ...
  * `removeSpecifier` (Impact: 71.6)
  * `walkDeclaration` (Impact: 63.2)
  * `analyzeBindingsFromOptions` (Impact: 40.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001005 | `Ripple Effect (Closeness):` 0.110425
...
**Top Internal Functions/Classes:**
  * `createPatchFunction` (Impact: 381.5)
...
**Top Internal Functions/Classes:**
  * `updateChildComponent` (Impact: 71.4)
  * `mountComponent` (Impact: 42.9)
  * `lifecycleMixin` (Impact: 29.7)
  * `callHook` (Impact: 21.5)
  * `_update` (Impact: 19.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002594 | `Ripple Effect (Closeness):` 0.08295
...
**Top Internal Functions/Classes:**
  ...
  * `renderAsyncComponent` (Impact: 37.2)
  * `renderStartingTag` (Impact: 36.2)
  * `resolve` (Impact: 27.1)
...
**Top Internal Functions/Classes:**
  ...
  * `walkIdentifiers` (Impact: 61.5)
  * `enter` (Impact: 36.5)
  * `extractIdentifiers` (Impact: 24.7)
  * `walkBlockDeclarations` (Impact: 20.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1e-06 | `Ripple Effect (Closeness):` 0.024882
...
**Top Internal Functions/Classes:**
  * `parseHTML` (Impact: 162.5)
  * `parseEndTag` (Impact: 52.4)
  * `handleStartTag` (Impact: 26.5)
  * `parseStartTag` (Impact: 7.3)
  * `shouldIgnoreFirstNewline` (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001651 | `Ripple Effect (Closeness):` 0.100271
...
**Top Internal Functions/Classes:**
  * `addHandler` (Impact: 115.0)
  * `addDirective` (Impact: 19.2)
  * `addAttr` (Impact: 15.3)
  * `getBindingAttr` (Impact: 14.8)
  * `getAndRemoveAttr` (Impact: 13.0)
...
**Top Internal Functions/Classes:**
  * `initComputed` (Impact: 33.3)
  * `initData` (Impact: 24.6)
  * `initProps` (Impact: 21.8)
  * `defineComputed` (Impact: 21.4)
  * `initMethods` (Impact: 18.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000115 | `Ripple Effect (Closeness):` 0.020944
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 258.64 | **LOC:** 212 | **CtrlFlow:** 22.3% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (73.0059%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `generateRow` (Impact: 32.4)
  * `getData` (Impact: 20.3)
  * `getElapsedClassName` (Impact: 7.7)
  * `countClassName` (Impact: 7.7)
  * `updateQuery` (Impact: 6.6)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `high_risk_execution: 2`, `state_mutation: 59`, `dead_code: 1`
* *Architecture:* `api: 1`
* *Defense:* None
* *Network Topology:*
...
- `src/core/vdom/create-component.ts` -> **Severity: 0.329** (Bridge: 0.0033 * Flux: 99.7795%)
