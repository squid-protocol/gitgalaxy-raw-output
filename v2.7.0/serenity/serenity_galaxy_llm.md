...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 28.7 | 0.0 | 0.0 |
...
| connectivity | 16747 | 3759 | 3 | `Userland/Libraries/LibWeb/CSS/ComputedValues.h` |
...
| dead_code | 30574 | 3880 | 6 | `Userland/Libraries/LibShell/AST.cpp` |
...
- `Parser::basic_parse` (@ `Userland/Libraries/LibURL/Parser.cpp`) -> Impact: **909.7** | LOC: 950
  * *Intent:* // https://url.spec.whatwg.org/#concept-basic-url-parser
- `Instruction::to_byte_string_internal` (@ `Userland/Libraries/LibDisassembly/x86/Instruction.cpp`) -> Impact: **727.7** | LOC: 870
- `Instruction::parse` (@ `Userland/Libraries/LibWasm/Parser/Parser.cpp`) -> Impact: **666.4** | LOC: 629
- `BytecodeInterpreter::interpret_instruction` (@ `Userland/Libraries/LibWasm/AbstractMachine/BytecodeInterpreter.cpp`) -> Impact: **647.5** | LOC: 869
- `Parser::parse_css_value` (@ `Userland/Libraries/LibWeb/CSS/Parser/Parser.cpp`) -> Impact: **456.0** | LOC: 360
- `build_in_table` (@ `Userland/Libraries/LibDisassembly/x86/Instruction.cpp`) -> Impact: **421.7** | LOC: 179
- `Lexer::lex_impl` (@ `Userland/Libraries/LibCpp/Lexer.cpp`) -> Impact: **410.7** | LOC: 578
- `Type1FontProgram::parse_glyph` (@ `Userland/Libraries/LibPDF/Fonts/Type1FontProgram.cpp`) -> Impact: **399.9** | LOC: 600
- `Lexer::lex_impl` (@ `Userland/Libraries/LibGLSL/Lexer.cpp`) -> Impact: **398.1** | LOC: 551
- `cpu_feature_to_description` (@ `Kernel/Arch/aarch64/CPUID.cpp`) -> Impact: **397.5** | LOC: 482
  * *Intent:* // https://developer.arm.com/downloads/-/exploration-tools/feature-names-for-a-profile

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Complexity Load | Avg Debt Markers |
|---|---|---|---|---|
| `Userland/Utilities` | 224 | 24560.7 | 30.9% | 46.62% |
| `Userland/Libraries/LibGUI` | 255 | 23464.8 | 20.32% | 42.87% |
| `AK` | 230 | 23020.42 | 27.0% | 28.04% |
...
| `Userland/Libraries/LibWeb/CSS` | 150 | 11144.94 | 11.47% | 43.63% |
| `Userland/Libraries/LibC` | 130 | 11142.9 | 22.02% | 35.91% |
...
- **Heaviest Functions:** `decode_pem` (Impact: 23.5), `decode_pems` (Impact: 17.4), `encode_pem` (Impact: 17.3)
...
- **Heaviest Functions:** `Job::parse_body` (Impact: 72.3), `Job::parse_headers` (Impact: 48.9), `handle_content_encoding` (Impact: 24.2)
...
- **Heaviest Functions:** `ResolutionLevelPositionComponentLayerProgressionIterator::generator` (Impact: 16.4), `PositionComponentResolutionLevelLayerProgressionIterator::generator` (Impact: 16.4), `ComponentPositionResolutionLevelLayerProgressionIterator::generator` (Impact: 16.4)
...
- **Heaviest Functions:** `TLSv12::check_connection_state` (Impact: 34.3), `TLSv12::read_from_socket` (Impact: 15.3), `TLSv12::flush` (Impact: 15.1)
...
- **Heaviest Functions:** `Processor::cpu_detect` (Impact: 215.2), `Processor::smp_process_pending_messages` (Impact: 20.6), `Processor::cpu_setup` (Impact: 18.2)
...
- **Heaviest Functions:** `__complete_job_spec` (Impact: 9.6), `_complete_cd` (Impact: 6.4), `_complete_unalias` (Impact: 5.1)
...
- **Heaviest Functions:** `xHCIController::initialize_device` (Impact: 92.4), `xHCIController::initialize_endpoint_if_needed` (Impact: 77.5), `xHCIController::clear_port_feature` (Impact: 52.9)
...
- **Heaviest Functions:** `LutBToATagData::evaluate` (Impact: 33.6), `Lut16TagData::evaluate_from_pcs` (Impact: 26.9), `Lut8TagData::evaluate_from_pcs` (Impact: 26.8)
...
- **Heaviest Functions:** `symbol_dictionary_decoding_procedure` (Impact: 141.0), `text_region_decoding_procedure` (Impact: 109.0), `decode_text_region` (Impact: 86.8)
...
- **Heaviest Functions:** `already_available` (Impact: 14.0), `build_for` (Impact: 12.4), `__global_context__` (Impact: 7.2)
...
**Top Internal Functions/Classes:**
  * `Parser::parse_css_value` (Impact: 456.0)
  * `Parser::parse_css_value_for_properties` (Impact: 223.0)
  * `Parser::substitute_attr_function` (Impact: 173.1)
    * *Intent:* // https://drafts.csswg.org/css-values-5/#attr-substitution
  * `Parser::convert_to_font_face_rule` (Impact: 153.3)
  * `Parser::parse_position_value` (Impact: 126.9)
...
**Top Internal Functions/Classes:**
  * `jbig2_pattern_dictionary_from_json` (Impact: 186.1)
  * `TRY` (Impact: 113.1)
  * `TRY` (Impact: 109.4)
  * `jbig2_text_region_flags_from_json` (Impact: 93.7)
  * `jbig2_segment_from_json` (Impact: 91.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Parser::parse_class_expression` (Impact: 192.4)
  * `Parser::parse_secondary_expression` (Impact: 156.7)
  * `Parser::parse_binding_pattern` (Impact: 155.5)
  * `Parser::parse_export_statement` (Impact: 118.3)
  * `~ScopePusher` (Impact: 84.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `read_hf_coefficients` (Impact: 83.0)
    * *Intent:* // I.4 - Decoding of quantized HF coefficients
  * `get_properties` (Impact: 82.3)
  * `read_frame_header` (Impact: 71.5)
  * `read_modular_bitstream` (Impact: 67.0)
  * `dct_select_to_dct_size` (Impact: 45.1)
...
**Top Internal Functions/Classes:**
  * `symbol_dictionary_decoding_procedure` (Impact: 141.0)
    * *Intent:* // 6.5 Symbol Dictionary Decoding Procedure
  * `text_region_decoding_procedure` (Impact: 109.0)
    * *Intent:* // 6.4 Text Region Decoding Procedure
  * `decode_text_region` (Impact: 86.8)
  * `decode_symbol_dictionary` (Impact: 83.7)
  * `decode_data` (Impact: 76.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Painter::draw_line` (Impact: 155.6)
  * `Painter::split_text_into_directional_runs` (Impact: 98.9)
  * `do_draw_scaled_bitmap` (Impact: 72.9)
  * `draw_text_line` (Impact: 69.3)
  * `Painter::draw_triangle` (Impact: 56.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Execute::for_each_entry` (Impact: 86.8)
  * `MatchExpr::run` (Impact: 59.0)
  * `Node::complete_for_editor` (Impact: 51.6)
  * `ForLoop::run` (Impact: 49.8)
  * `IfCond::IfCond` (Impact: 35.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `HTMLParser::handle_in_body` (Impact: 362.1)
    * *Intent:* // https://html.spec.whatwg.org/multipage/parsing.html#parsing-main-inbody
  * `HTMLParser::process_using_the_rules_for` (Impact: 88.7)
  * `HTMLParser::serialize_html_fragment` (Impact: 81.0)
    * *Intent:* // https://html.spec.whatwg.org/multipage/parsing.html#html-fragment-serialisation-algorithm
  * `HTMLParser::handle_in_head` (Impact: 79.6)
  * `HTMLParser::handle_in_select` (Impact: 66.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `StyleComputer::compute_font_for_style_values` (Impact: 270.2)
  * `StyleComputer::for_each_property_expanding_shorthands` (Impact: 194.9)
  * `StyleComputer::collect_matching_rules` (Impact: 122.6)
  * `StyleComputer::make_rule_cache_for_cascade_origin` (Impact: 84.2)
  * `StyleComputer::compute_cascaded_values` (Impact: 81.7)
...
**Top Internal Functions/Classes:**
  * `GridFormattingContext::layout_absolutely_positioned_element` (Impact: 72.8)
  * `GridFormattingContext::resolve_grid_position` (Impact: 66.1)
  * `GridFormattingContext::distribute_extra_space_across_spanned_tracks_base_size` (Impact: 63.3)
  * `GridFormattingContext::resolve_track_spacing` (Impact: 61.3)
  * `GridFormattingContext::increase_sizes_to_accommodate_spanning_items_crossing_content_sized_tracks` (Impact: 59.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `put_by_property_key` (Impact: 105.3)
  * `put_by_value` (Impact: 86.1)
  * `Interpreter::run_bytecode` (Impact: 68.7)
    * *Intent:* // FIXME: GCC takes a *long* time to compile with flattening, and it will time out our CI. :| #if de...
  * `get_by_value` (Impact: 67.4)
  * `format_operand` (Impact: 60.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ECMA262Parser::parse_nonempty_class_ranges` (Impact: 136.9)
  * `PosixExtendedParser::parse_sub_expression` (Impact: 136.6)
  * `AbstractPosixParser::parse_bracket_expression` (Impact: 117.7)
    * *Intent:* // ============================= // Abstract Posix Parser // =============================
  * `ECMA262Parser::parse_character_escape` (Impact: 58.5)
  * `ECMA262Parser::read_capture_group_specifier` (Impact: 57.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Editor::handle_read_event` (Impact: 170.8)
  * `actual_rendered_string_length_step` (Impact: 112.9)
  * `Editor::refresh_display` (Impact: 67.5)
  * `merge` (Impact: 60.3)
  * `Editor::vt_dsr` (Impact: 59.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `TextEditor::paint_event` (Impact: 203.0)
  * `TextEditor::keydown_event` (Impact: 161.3)
  * `for_each_visual_line` (Impact: 150.0)
  * `TextEditor::text_position_at_content_position` (Impact: 41.8)
  * `TextEditor::mousedown_event` (Impact: 40.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Instruction::to_byte_string_internal` (Impact: 727.7)
  * `build_in_table` (Impact: 421.7)
  * `Instruction::to_byte_string` (Impact: 41.8)
  * `MemoryOrRegisterReference::sib_to_byte_string` (Impact: 36.1)
  * `MemoryOrRegisterReference::to_byte_string_a16` (Impact: 26.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `symbol_dictionary_encoding_procedure` (Impact: 167.7)
    * *Intent:* // 6.5 Symbol Dictionary Decoding Procedure, but in reverse.
  * `text_region_encoding_procedure` (Impact: 124.2)
    * *Intent:* // 6.4 Text Region Decoding Procedure, but in reverse.
  * `encode_text_region` (Impact: 90.7)
  * `generic_refinement_region_encoding_procedure` (Impact: 82.8)
    * *Intent:* // 6.3 Generic Refinement Region Decoding Procedure, but in reverse.
  * `generic_region_encoding_procedure` (Impact: 73.4)
...
**Top Internal Functions/Classes:**
  * `Shell::run_command` (Impact: 131.8)
  * `Shell::complete_via_program_itself` (Impact: 116.9)
  * `do_escape` (Impact: 74.4)
  * `Shell::complete_path` (Impact: 72.2)
  * `Shell::prompt` (Impact: 69.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Parser::residual` (Impact: 45.3)
  * `Parser::uncompressed_header` (Impact: 44.8)
    * *Intent:* /* (6.2) */
  * `Parser::find_reference_motion_vectors` (Impact: 35.7)
    * *Intent:* // 6.5.1 Find MV refs syntax // find_mv_refs( refFrame, block ) in the spec.
  * `Parser::decode_partition` (Impact: 35.5)
  * `Parser::inter_block_mode_info` (Impact: 34.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `AssignmentExpression::generate_bytecode` (Impact: 141.1)
  * `BinaryExpression::generate_bytecode` (Impact: 138.8)
  * `generate_object_binding_pattern_bytecode` (Impact: 66.8)
  * `constant_fold_binary_expression` (Impact: 58.5)
  * `TryStatement::generate_bytecode` (Impact: 57.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `cpu_feature_to_description` (Impact: 397.5)
    * *Intent:* // https://developer.arm.com/downloads/-/exploration-tools/feature-names-for-a-profile
  * `cpu_feature_to_name` (Impact: 357.9)
    * *Intent:* // https://developer.arm.com/downloads/-/exploration-tools/feature-names-for-a-profile
  * `detect_cpu_features` (Impact: 268.8)
  * `detect_physical_address_bit_width` (Impact: 12.2)
  * `build_cpu_feature_names` (Impact: 7.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `convert_to_bitmap` (Impact: 127.3)
  * `read_one_packet_header` (Impact: 107.0)
  * `decode_jpeg2000_header` (Impact: 75.2)
  * `parse_codestream_main_header` (Impact: 73.9)
  * `parse_codestream_tile_header` (Impact: 71.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Terminal::handle_key_press` (Impact: 154.8)
  * `Terminal::SGR` (Impact: 126.0)
  * `Terminal::execute_csi_sequence` (Impact: 125.5)
  * `Terminal::set_size` (Impact: 88.8)
    * *Intent:* #ifndef KERNEL
  * `Terminal::execute_escape_sequence` (Impact: 74.5)
**Contextual Mitigations & Amplifications:**
...
- `AK/IntrusiveList.h` -> **Severity: 0.052** (Bridge: 0.0005 * Flux: 99.5679%)
- `AK/Vector.h` -> **Severity: 0.019** (Bridge: 0.0002 * Flux: 85.1953%)
- `AK/Function.h` -> **Severity: 0.008** (Bridge: 0.0001 * Flux: 94.7996%)
- `AK/Queue.h` -> **Severity: 0.008** (Bridge: 0.0001 * Flux: 99.8392%)
- `Kernel/Memory/TypedMapping.h` -> **Severity: 0.008** (Bridge: 0.0002 * Flux: 48.559%)
