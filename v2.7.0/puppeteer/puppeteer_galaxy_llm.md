...
| Avg Path Length | 2.9475 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 92.4 | 14.1 | 0.0 | 0.0 |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 89.9 | 4.6 | 0.0 | 0.0 |
...
| credential | 5 | 3 | 0 | `packages/browsers/test/src/firefox/firefox-data.spec.ts` |
...
- `getEscapedText` (@ `tools/docgen/src/custom_markdown_documenter.ts`) -> Impact: **390.9** | LOC: 1538
- `getBidiKeyValue` (@ `packages/puppeteer-core/src/bidi/Input.ts`) -> Impact: **169.8** | LOC: 228
- `resolveBuildIdForBrowserTag` (@ `packages/browsers/src/browser-data/browser-data.ts`) -> Impact: **119.2** | LOC: 105
  * *Intent:* /** * @internal */
- `build` (@ `packages/browsers/src/CLI.ts`) -> Impact: **76.1** | LOC: 306
- `fill` (@ `packages/puppeteer-core/src/api/locators/locators.ts`) -> Impact: **69.7** | LOC: 153
- `constructor` (@ `packages/puppeteer-core/src/node/ScreenRecorder.ts`) -> Impact: **64.1** | LOC: 163
...
- `evaluate` (@ `packages/puppeteer-core/src/cdp/ExecutionContext.ts`) -> Impact: **63.7** | LOC: 154
- `SearchPageContent` (@ `website/src/theme/SearchPage/index.js`) -> Impact: **61.5** | LOC: 390
- `installUrl` (@ `packages/browsers/src/install.ts`) -> Impact: **52.3** | LOC: 126
- `launch` (@ `packages/puppeteer-core/src/node/BrowserLauncher.ts`) -> Impact: **49.2** | LOC: 193
...
### 1. `packages/testserver/src/index.ts` (TYPESCRIPT) -> Cumulative Risk: **743.57**
...
- **Heaviest Functions:** `serveFile` (Impact: 30.9), `onRequest` (Impact: 21.8), `constructor` (Impact: 11.6)

### 2. `packages/browsers/src/browser-data/chrome.ts` (TYPESCRIPT) -> Cumulative Risk: **657.68**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `resolveDefaultUserDataDir` (Impact: 43.1), `resolveSystemExecutablePaths` (Impact: 24.8), `relativeExecutablePath` (Impact: 18.4)
...
- **Heaviest Functions:** `executeCommand` (Impact: 16.2), `updateExecutablePath` (Impact: 11.4), `message` (Impact: 11.1)

### 4. `packages/puppeteer-core/src/bidi/Page.ts` (TYPESCRIPT) -> Cumulative Risk: **633.04**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `emulateNetworkConditions` (Impact: 43.0), `setViewport` (Impact: 37.7), `setUserAgent` (Impact: 31.7)
...
- **Heaviest Functions:** `keyDescriptionForString` (Impact: 22.4), `getButtonFromPressedButtons` (Impact: 15.8), `click` (Impact: 11.4)

### 6. `packages/puppeteer-core/src/bidi/HTTPRequest.ts` (TYPESCRIPT) -> Cumulative Risk: **621.02**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `_respond` (Impact: 23.7), `getBidiHeaders` (Impact: 12.3), `constructor` (Impact: 7.5)

### 7. `packages/puppeteer-core/src/common/util.ts` (TYPESCRIPT) -> Cumulative Risk: **612.63**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `parsePDFOptions` (Impact: 30.6), `getReadableAsTypedArray` (Impact: 17.6), `convertPrintParameterToInches` (Impact: 17.2)

### 8. `packages/puppeteer-core/src/cdp/FrameManager.ts` (TYPESCRIPT) -> Cumulative Risk: **609.19**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `onExecutionContextCreated` (Impact: 24.2), `initialize` (Impact: 21.0), `setupEventListeners` (Impact: 14.8)
...
- **Heaviest Functions:** `invokeAtMostOnceForArguments` (Impact: 13.7), `bubbleInitializer` (Impact: 13.3), `bubble` (Impact: 9.0)

### 10. `packages/puppeteer-core/src/common/CallbackRegistry.ts` (TYPESCRIPT) -> Cumulative Risk: **602.43**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `constructor` (Impact: 10.7), `_reject` (Impact: 9.2), `reject` (Impact: 6.3)
...
**Top Internal Functions/Classes:**
  * `task` (Impact: 5.5)
  * `onResponse` (Impact: 4.2)
  * `checkMetrics` (Impact: 4.0)
  * `onResponse` (Impact: 3.8)
  * `default` (Impact: 3.6)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `bindIsolatedHandle` (Impact: 19.8)
  ...
  * `clickableBox` (Impact: 15.1)
  * `isIntersectingViewport` (Impact: 15.1)
...
**Top Internal Functions/Classes:**
  * `emulateNetworkConditions` (Impact: 43.0)
  * `setViewport` (Impact: 37.7)
  * `setUserAgent` (Impact: 31.7)
    * *Intent:* /** * @internal */
  * `setCookie` (Impact: 27.4)
  * `_screenshot` (Impact: 26.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
* *Amplified Cascading Flux:* 21 instances
...
**Top Internal Functions/Classes:**
  * `screenshot` (Impact: 43.3)
  * `screencast` (Impact: 21.8)
    * *Intent:* * browser.close(); * ``` * * @param options - Configures screencast behavior. * * @experimental * * ...
  * `setDefaultScreenshotOptions` (Impact: 21.7)
    * *Intent:* /** * @internal */
  * `on` (Impact: 6.5)
...
**Top Internal Functions/Classes:**
  * `onConsoleAPI` (Impact: 17.7)
  * `setUserAgent` (Impact: 16.8)
  * `_screenshot` (Impact: 16.7)
  * `exposeFunction` (Impact: 13.9)
  * `_create` (Impact: 11.0)
**Contextual Mitigations & Amplifications:**
...
- `packages/puppeteer-core/src/cdp/Page.ts` -> Churn: **65.77%** | Cog Load: 80.7936% | Debt: 8.6961%
- `packages/puppeteer-core/Herebyfile.mjs` -> Churn: **63.02%** | Cog Load: 82.7828% | Debt: 0.0%
- `packages/puppeteer-core/src/bidi/Page.ts` -> Churn: **56.62%** | Cog Load: 81.3266% | Debt: 10.1521%
