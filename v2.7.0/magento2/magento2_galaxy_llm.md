...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 23.9 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 1.9 | 0.0 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 6.2 | 1.6 | 0.0 |
...
| connectivity | 55229 | 13788 | 5 | `app/code/Magento/Sales/Model/Order.php` |
...
| dead_code | 31090 | 10140 | 3 | `dev/tests/api-functional/testsuite/Magento/Catalog/Api/ProductRepositoryInterfaceTest.php` |
| credential | 94 | 55 | 0 | `app/code/Magento/Reports/view/adminhtml/templates/grid.phtml` |
...
- `__construct` (@ `app/code/Magento/Customer/Model/AccountManagement.php`) -> Impact: **224.8** | LOC: 89
...
- `getRateRequest` (@ `app/code/Magento/Tax/Model/Calculation.php`) -> Impact: **133.4** | LOC: 120
...
- `saveProductMediaGalleryPhase` (@ `app/code/Magento/CatalogImportExport/Model/Import/Product.php`) -> Impact: **114.1** | LOC: 132
...
- `__construct` (@ `app/code/Magento/Catalog/Model/ResourceModel/Product/Collection.php`) -> Impact: **113.2** | LOC: 73
...
- `insertOrder` (@ `app/code/Magento/Sales/Model/Order/Pdf/AbstractPdf.php`) -> Impact: **106.4** | LOC: 248
...
- `collect` (@ `app/code/Magento/SalesRule/Model/Quote/Discount.php`) -> Impact: **103.0** | LOC: 141
...
- `__construct` (@ `app/code/Magento/Sitemap/Model/Batch/Sitemap.php`) -> Impact: **93.0** | LOC: 59
...
- `scan` (@ `setup/src/Magento/Setup/Module/Di/Code/Reader/FileScanner.php`) -> Impact: **86.3** | LOC: 347
  * *Intent:* /** * @inheritDoc */
- `getTotalPrices` (@ `app/code/Magento/Bundle/Model/Product/Price.php`) -> Impact: **85.8** | LOC: 150
...
- **Heaviest Functions:** `initialize` (Impact: 11.6), `updateCounters` (Impact: 9.7), `clearPeriodicCounterUpdate` (Impact: 2.3)
...
- **Heaviest Functions:** `_create` (Impact: 13.8), `onStateChange` (Impact: 9.3), `_create` (Impact: 6.2)
...
- **Heaviest Functions:** `sendCreateLabelRequest` (Impact: 40.0), `packItems` (Impact: 31.1), `checkSizeAndGirthParameter` (Impact: 27.2)
...
- **Heaviest Functions:** `_getCallback` (Impact: 9.4), `defaultCallback` (Impact: 6.5), `getAction` (Impact: 5.5)
...
- **Heaviest Functions:** `initEnsureSubtotalSync` (Impact: 37.1), `parsePrice` (Impact: 12.4), `trySync` (Impact: 6.9)
...
- **Heaviest Functions:** `enumerate_l33t_subs` (Impact: 24.5), `dictionary_match` (Impact: 23.3), `l33t_match` (Impact: 22.8)
...
- **Heaviest Functions:** `getFilePreviewType` (Impact: 14.9), `initUploader` (Impact: 14.2), `onPreviewLoad` (Impact: 12.7)
...
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `gtag` (Impact: 1.1)
...
- **Heaviest Functions:** `calculateDynamicBundleAmount` (Impact: 38.4), `processOptions` (Impact: 23.2), `createSelectionPriceList` (Impact: 11.2)
...
- **Heaviest Functions:** `get` (Impact: 11.5), `parseData` (Impact: 11.4), `parseSelector` (Impact: 9.9)
...
**Top Internal Functions/Classes:**
  * `_formShipmentRequest` (Impact: 56.9)
  * `_formShipmentRestRequest` (Impact: 54.4)
  * `processShippingRestRateForItem` (Impact: 48.4)
  * `setRequest` (Impact: 46.0)
    * *Intent:* /** * Prepare and set request to this instance * * @param RateRequest $request * @return $this * @Su...
  * `processShippingXmlRateForItem` (Impact: 43.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000239 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.7e-05 | `Ripple Effect (Closeness):` 0.0
...
- `app/code/Magento/Catalog/Model/ResourceModel/Product/Collection.php` -> **Severity: 0.024** (Bridge: 0.0002 * Flux: 100.0%)
- `app/code/Magento/Catalog/Model/ResourceModel/Category.php` -> **Severity: 0.02** (Bridge: 0.0002 * Flux: 99.9999%)
- `app/code/Magento/CatalogGraphQl/DataProvider/Product/SearchCriteriaBuilder.php` -> **Severity: 0.019** (Bridge: 0.0002 * Flux: 99.9999%)
- `app/code/Magento/Catalog/Model/Attribute/ScopeOverriddenValue.php` -> **Severity: 0.016** (Bridge: 0.0002 * Flux: 100.0%)
- `app/code/Magento/Store/Model/Store.php` -> **Severity: 0.014** (Bridge: 0.0002 * Flux: 85.0%)
