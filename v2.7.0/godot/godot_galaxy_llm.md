...
| connectivity | 49297 | 4870 | 10 | `thirdparty/volk/volk.h` |
...
| dead_code | 49731 | 3536 | 11 | `scene/resources/visual_shader_nodes.cpp` |
...
- `internal_dfa_match` (@ `thirdparty/pcre2/src/pcre2_dfa_match.c`) -> Impact: **2669.7** | LOC: 2053
  * *Intent:* /* And now, here is the code */
- `componentName` (@ `thirdparty/vulkan/include/vulkan/vulkan_format_traits.hpp`) -> Impact: **2273.5** | LOC: 1683
  * *Intent:* // The name of the component
- `componentNumericFormat` (@ `thirdparty/vulkan/include/vulkan/vulkan_format_traits.hpp`) -> Impact: **2169.2** | LOC: 1606
  * *Intent:* // The numeric format of the component
- `componentBits` (@ `thirdparty/vulkan/include/vulkan/vulkan_format_traits.hpp`) -> Impact: **2018.9** | LOC: 1511
  * *Intent:* // The number of bits in this component, if not compressed, otherwise 0.
- `compile_branch` (@ `thirdparty/pcre2/src/pcre2_compile.c`) -> Impact: **1982.9** | LOC: 2465
  * *Intent:* */
- `match` (@ `thirdparty/pcre2/src/pcre2_match.c`) -> Impact: **1822.2** | LOC: 2367
  * *Intent:* */
- `emit_cum_binary` (@ `thirdparty/pcre2/deps/sljit/sljit_src/sljitNativeX86_common.c`) -> Impact: **1732.8** | LOC: 1776
- `TOutputTraverser::visitAggregate` (@ `thirdparty/glslang/glslang/MachineIndependent/intermOut.cpp`) -> Impact: **1724.7** | LOC: 546
- `emit_non_cum_binary` (@ `thirdparty/pcre2/deps/sljit/sljit_src/sljitNativeX86_common.c`) -> Impact: **1642.8** | LOC: 1776
- `ShaderLanguage::_parse_expression` (@ `servers/rendering/shader_language.cpp`) -> Impact: **1609.3** | LOC: 1787
...
- **Heaviest Functions:** `decompile` (Impact: 63.1), `_add_to` (Impact: 53.7), `serialize_header` (Impact: 36.9)
...
- **Heaviest Functions:** `WaylandThread::window_try_set_mode` (Impact: 437.9), `WaylandThread::_wl_registry_on_global_remove` (Impact: 200.4), `WaylandThread::_wl_registry_on_global` (Impact: 126.6)
...
- **Heaviest Functions:** `EditorInspector::update_tree` (Impact: 360.7), `EditorProperty::_notification` (Impact: 222.2), `EditorInspectorSection::_notification` (Impact: 145.3)
...
- **Heaviest Functions:** `get_argument_type` (Impact: 26.9), `construct` (Impact: 19.4), `construct` (Impact: 19.4)
...
- **Heaviest Functions:** `uset_serializedContains` (Impact: 56.5), `uset_getSerializedRange` (Impact: 35.7), `uset_getItem` (Impact: 24.0)
...
- **Heaviest Functions:** `DisplayServerWindows::WndProc` (Impact: 1163.0), `DisplayServerWindows::DisplayServerWindows` (Impact: 550.9), `DisplayServerWindows::_create_window` (Impact: 195.8)
...
- **Heaviest Functions:** `EditorNode::_menu_option_confirm` (Impact: 474.1), `EditorNode::_notification` (Impact: 130.7), `EditorNode::_edit_current` (Impact: 128.1)
...
- **Heaviest Functions:** `EditorFileSystem::_reimport_file` (Impact: 151.3), `EditorFileSystem::_process_file_system` (Impact: 112.3), `EditorFileSystem::_test_for_reimport` (Impact: 107.1)
...
- **Heaviest Functions:** `IsValidPeerHandshake` (Impact: 11.6), `AcceptClient` (Impact: 11.4), `BroadcastRequest` (Impact: 8.0)
...
- **Heaviest Functions:** `TextureStorage::_validate_texture_format` (Impact: 311.8), `TextureStorage::texture_create_from_native_handle` (Impact: 288.9), `TextureStorage::_texture_format_from_rd` (Impact: 208.5)
...
**Top Internal Functions/Classes:**
  * `basisu_lowlevel_etc1s_transcoder::transcode_slice` (Impact: 919.8)
  * `basisu_lowlevel_etc1s_transcoder::transcode_image` (Impact: 730.9)
  * `basisu_lowlevel_uastc_ldr_4x4_transcoder::transcode_image` (Impact: 334.6)
  * `basisu_lowlevel_uastc_ldr_4x4_transcoder::transcode_slice` (Impact: 305.8)
  * `unpack_uastc` (Impact: 284.5)
...
**Top Internal Functions/Classes:**
  * `CompilerGLSL::emit_instruction` (Impact: 1016.2)
  * `CompilerGLSL::emit_glsl_op` (Impact: 652.9)
  * `CompilerGLSL::access_chain_internal` (Impact: 479.6)
  * `CompilerGLSL::emit_block_chain_inner` (Impact: 360.8)
  * `CompilerGLSL::builtin_to_glsl` (Impact: 317.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ShaderLanguage::_parse_expression` (Impact: 1609.3)
  * `ShaderLanguage::_parse_shader` (Impact: 1171.4)
  * `ShaderLanguage::_validate_operator` (Impact: 969.1)
  * `ShaderLanguage::_parse_block` (Impact: 872.2)
  * `ShaderLanguage::complete` (Impact: 570.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `TGlslangToSpvTraverser::visitAggregate` (Impact: 1287.0)
  * `TGlslangToSpvTraverser::createMiscOperation` (Impact: 1107.8)
  * `TGlslangToSpvTraverser::createUnaryOperation` (Impact: 1093.1)
  * `TGlslangToSpvTraverser::createSubgroupOperation` (Impact: 664.1)
    * *Intent:* // Create subgroup invocation operations.
  * `TGlslangToSpvTraverser::TGlslangToSpvTraverser` (Impact: 510.0)
...
**Top Internal Functions/Classes:**
  * `emit_cum_binary` (Impact: 1732.8)
  * `emit_non_cum_binary` (Impact: 1642.8)
  * `emit_lea_binary` (Impact: 1598.5)
  * `emit_cmp_binary` (Impact: 1406.2)
  * `emit_test_binary` (Impact: 1390.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `compile_iterator_matchingpath` (Impact: 493.0)
  * `compile_bracket_matchingpath` (Impact: 491.1)
    * *Intent:* */
  * `detect_early_fail` (Impact: 474.4)
    * *Intent:* */
  * `copy_recurse_data` (Impact: 465.0)
  * `compile_matchingpath` (Impact: 452.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `compile_branch` (Impact: 1982.9)
    * *Intent:* */
  * `parse_regex` (Impact: 1535.2)
    * *Intent:* #else #define PARSED_LITERAL(c, p) *p++ = c; okquantifier = TRUE; #endif /* Here's the actual functi...
  * `pcre2_compile` (Impact: 571.8)
    * *Intent:* */
  * `get_branchlength` (Impact: 337.6)
    * *Intent:* */
  * `check_lookbehinds` (Impact: 196.3)
    * *Intent:* */
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `to_string` (Impact: 627.3)
  * `to_string` (Impact: 392.6)
  * `to_string` (Impact: 111.5)
  * `to_string` (Impact: 92.5)
  * `to_string` (Impact: 86.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `componentName` (Impact: 2273.5)
    * *Intent:* // The name of the component
  * `componentNumericFormat` (Impact: 2169.2)
    * *Intent:* // The numeric format of the component
  * `componentBits` (Impact: 2018.9)
    * *Intent:* // The number of bits in this component, if not compressed, otherwise 0.
  * `blockSize` (Impact: 391.2)
    * *Intent:* // The texel block size in bytes.
  * `compatibilityClass` (Impact: 391.2)
...
**Top Internal Functions/Classes:**
  * `TextServerAdvanced::_ensure_glyph` (Impact: 1018.8)
    * *Intent:* #endif /*************************************************************************/ /* Font Cache */ ...
  * `TextServerAdvanced::_shape_run` (Impact: 581.8)
  * `TextServerAdvanced::_ensure_cache_for_size` (Impact: 557.8)
  * `TextServerAdvanced::_shaped_text_overrun_trim_to_width` (Impact: 243.0)
  * `TextServerAdvanced::_is_valid_identifier` (Impact: 223.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `packCharts` (Impact: 254.9)
    * *Intent:* // Pack charts in the smallest possible rectangle.
  * `PackCharts` (Impact: 149.4)
  * `AddMesh` (Impact: 141.6)
  * `ComputeCharts` (Impact: 96.0)
  * `findChartLocation_bruteForce` (Impact: 95.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `RichTextLabel::append_text` (Impact: 756.6)
  * `RichTextLabel::_find_click_in_line` (Impact: 396.5)
  * `RichTextLabel::gui_input` (Impact: 186.9)
  * `RichTextLabel::_shape_line` (Impact: 178.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `FindMemoryPreferences` (Impact: 142.3)
    * *Intent:* // This is the main algorithm that guides the selection of a memory type best for an allocation - //...
  * `VmaAllocator_T::AllocateDedicatedMemory` (Impact: 130.3)
  * `VmaBlockMetadata_Linear::CreateAllocationRequest_LowerAddress` (Impact: 126.4)
  * `VmaBlockMetadata_TLSF::CreateAllocationRequest` (Impact: 121.8)
    * *Intent:* #endif
  * `VmaBlockVector::AllocatePage` (Impact: 120.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `DecodePixelData` (Impact: 575.5)
    * *Intent:* #endif // // ----------------------------------------------------------------- // // heuristics #def...
  * `LoadDeepEXR` (Impact: 529.1)
  * `DecodeChunk` (Impact: 261.9)
  * `ParseEXRHeader` (Impact: 252.9)
    * *Intent:* #endif
  * `EncodePixelData` (Impact: 221.9)
...
**Top Internal Functions/Classes:**
  * `Node3DEditorViewport::_sinput` (Impact: 670.2)
  * `Node3DEditorViewport::_notification` (Impact: 262.3)
  * `Node3DEditorViewport::_menu_option` (Impact: 177.6)
  * `Node3DEditorViewport::update_transform` (Impact: 153.5)
    * *Intent:* // Update the current transform operation in response to an input.
  * `Node3DEditorViewport::_transform_gizmo_select` (Impact: 125.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `TextEdit::_notification` (Impact: 719.3)
  * `TextEdit::gui_input` (Impact: 259.3)
  * `TextEdit::alt_input` (Impact: 153.1)
  * `TextEdit::menu_option` (Impact: 115.1)
  * `TextEdit::_offset_carets_after` (Impact: 74.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `DisplayServerWindows::WndProc` (Impact: 1163.0)
    * *Intent:* // The window procedure for our window class "Engine", used to handle processing of window-related s...
  * `DisplayServerWindows::DisplayServerWindows` (Impact: 550.9)
  * `DisplayServerWindows::_create_window` (Impact: 195.8)
  * `DisplayServerWindows::_get_window_style` (Impact: 140.1)
  * `DisplayServerWindows::window_set_flag` (Impact: 99.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `RenderingDevice::uniform_set_create` (Impact: 272.0)
  * `RenderingDevice::_render_pass_create` (Impact: 198.5)
    * *Intent:* /*********************/ /**** FRAMEBUFFER ****/ /*********************/
  * `RenderingDevice::texture_create` (Impact: 156.9)
    * *Intent:* /*****************/ /**** TEXTURE ****/ /*****************/
  * `RenderingDevice::initialize` (Impact: 98.8)
  * `RenderingDevice::render_pipeline_create` (Impact: 95.7)
    * *Intent:* /*******************/ /**** PIPELINES ****/ /*******************/
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 7233.94 | **LOC:** 7268 | **CtrlFlow:** 25.2% | **Authorship Centralization:** 30.0%
- **Risk Profile:** Cognitive Load (92.3319%), Tech Debt (61.9398%)
**Top Internal Functions/Classes:**
  * `GLTFDocument::_convert_animation_node_track` (Impact: 334.2)
  * `GLTFDocument::_import_animation` (Impact: 230.0)
  * `GLTFDocument::_parse_meshes` (Impact: 203.5)
  * `GLTFDocument::export_object_model_property` (Impact: 195.0)
  * `GLTFDocument::import_object_model_property` (Impact: 189.6)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `state_mutation: 1369`, `dead_code: 6`, `planned_debt: 5`, `fragile_debt: 2`, `unreferenced_by_name: 118`
...
**Top Internal Functions/Classes:**
  * `AnimationTrackEditor::_edit_menu_pressed` (Impact: 357.9)
  * `AnimationTrackEdit::_notification` (Impact: 167.5)
    * *Intent:* ////////////////////////////////////
  * `AnimationTrackEdit::gui_input` (Impact: 146.3)
  * `AnimationMultiTrackKeyEdit::_set` (Impact: 137.6)
  * `AnimationTrackKeyEdit::_set` (Impact: 119.7)
**Contextual Mitigations & Amplifications:**
...
### `scene/resources/2d/tile_set.cpp` (CPP | Tier 2 | AI Safe: 0.0%)
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 7178.74 | **LOC:** 7210 | **CtrlFlow:** 32.1% | **Authorship Centralization:** 27.3%
- **Risk Profile:** Cognitive Load (94.2801%), Tech Debt (79.5661%)
**Top Internal Functions/Classes:**
  * `TileSet::get_neighbor_cell` (Impact: 614.0)
  * `TileSet::_set` (Impact: 280.5)
    * *Intent:* #endif // DISABLE_DEPRECATED
  * `TileSet::transform_coords_layout` (Impact: 144.0)
  * `TileSet::_get_half_offset_corner_or_side_terrain_peering_bit_polygon` (Impact: 142.0)
  * `TileSet::_get` (Impact: 140.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 895 instances
* *State Mutation (weighted view):* 2864
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 1956`, `structural_boundaries: 658`, `args: 887`, `func_start: 364`, `class_start: 1`
* *Risk/State:* `state_mutation: 1074`, `planned_debt: 3`, `unreferenced_by_name: 155`
* *Architecture:* `import: 13`
* *Defense:* `doc: 10`, `immutability_locks: 275`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.031
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
  * `Imports (Out-Degree: 13):` engine.h, marshalls.h, geometry_2d.h, callable_mp.h, class_db.h, local_vector.h, rb_set.h, control.h...
  * `Imported By (In-Degree: 0):` None (Orphan / Entrypoint)
...
- `thirdparty/glslang/glslang/MachineIndependent/limits.cpp` -> **Severity: 0.045** (Bridge: 0.0004 * Flux: 99.9969%)
- `core/templates/hashfuncs.h` -> **Severity: 0.015** (Bridge: 0.0002 * Flux: 64.7948%)
- `core/object/ref_counted.h` -> **Severity: 0.012** (Bridge: 0.0001 * Flux: 82.2099%)
- `thirdparty/embree/kernels/geometry/primitive.h` -> **Severity: 0.011** (Bridge: 0.0001 * Flux: 96.0834%)
- `thirdparty/mingw-std-threads/mingw.shared_mutex.h` -> **Severity: 0.011** (Bridge: 0.0002 * Flux: 72.3038%)
