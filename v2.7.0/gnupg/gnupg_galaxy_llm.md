...
| Avg Path Length | 3.2932 | Hops between files. Lower = Tighter coupling. |
...
> **Architectural Drift Z-Score:** `2.985`
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 70.7 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 58.6 | 100.0 | 100.0 |
...
| guards | 18152 | 518 | 51 | `dirmngr/dns.c` |
| danger | 2269 | 298 | 6 | `autogen.sh` |
| concurrency | 22 | 13 | 0 | `tests/gpgscm/ffi.c` |
| connectivity | 5824 | 510 | 14 | `dirmngr/dns.c` |
| io | 994 | 204 | 1 | `autogen.sh` |
| crypto | 0 | 0 | 0 | - |
...
| dead_code | 2353 | 383 | 7 | `dirmngr/dns.c` |
...
- `main` (@ `g10/gpg.c`) -> Impact: **1025.2** | LOC: 1625
- `main` (@ `sm/gpgsm.c`) -> Impact: **820.1** | LOC: 1471
- `keyedit_menu` (@ `g10/keyedit.c`) -> Impact: **770.7** | LOC: 968
  * *Intent:* #endif /* HAVE_LIBREADLINE */
- `Eval_Cycle` (@ `tests/gpgscm/scheme.c`) -> Impact: **644.6** | LOC: 1668
  * *Intent:* /* Safe because we would have already returned if `fast' encountered a non-pair. */
- `parse_key_parameter_part` (@ `g10/keygen.c`) -> Impact: **612.9** | LOC: 411
...
- `import_one_real` (@ `g10/import.c`) -> Impact: **585.1** | LOC: 547
...
- `do_validate_chain` (@ `sm/certchain.c`) -> Impact: **537.5** | LOC: 669
- `main` (@ `tools/gpg-connect-agent.c`) -> Impact: **518.6** | LOC: 812
- `parse_key` (@ `g10/parse-packet.c`) -> Impact: **511.1** | LOC: 592
- `sign_uids` (@ `g10/keyedit.c`) -> Impact: **505.5** | LOC: 606
...
### 1. `g10/keydb.c` (C) -> Cumulative Risk: **683.85**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1586.54 | **LOC:** 2006 | **CtrlFlow:** 28.7% | **Authorship Centralization:** 83.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (95.1119%)
- **Heaviest Functions:** `keydb_add_resource` (Impact: 130.4), `internal_keydb_search` (Impact: 125.8), `maybe_create_keyring_or_box` (Impact: 90.5)

### 2. `common/tlv-parser.c` (C) -> Cumulative Risk: **683.16**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 790.04 | **LOC:** 828 | **CtrlFlow:** 27.0% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (97.2909%)
- **Heaviest Functions:** `_tlv_parser_next` (Impact: 82.8), `cram_octet_string` (Impact: 54.3), `tlv_expect_object` (Impact: 36.6)

### 3. `g10/misc.c` (C) -> Cumulative Risk: **680.42**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1707.26 | **LOC:** 1967 | **CtrlFlow:** 36.3% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9997%), Documentation (98.2456%), Tech Debt (95.6429%)
- **Heaviest Functions:** `pct_expando` (Impact: 146.1), `parse_options` (Impact: 85.6), `map_cipher_openpgp_to_gcry` (Impact: 53.0)

### 4. `sm/keydb.c` (C) -> Cumulative Risk: **679.73**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1782.8 | **LOC:** 2239 | **CtrlFlow:** 23.6% | **Authorship Centralization:** 83.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (94.3633%)
- **Heaviest Functions:** `keydb_search` (Impact: 149.9), `keydb_add_resource` (Impact: 84.9), `maybe_create_keybox` (Impact: 63.5)

### 5. `kbx/backend-kbx.c` (C) -> Cumulative Risk: **673.53**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 330.46 | **LOC:** 459 | **CtrlFlow:** 18.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (98.6851%)
- **Heaviest Functions:** `be_kbx_search` (Impact: 26.9), `be_kbx_update` (Impact: 26.2), `be_kbx_insert` (Impact: 25.9)

### 6. `common/percent.c` (C) -> Cumulative Risk: **671.21**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 359.42 | **LOC:** 323 | **CtrlFlow:** 33.0% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (98.5148%)
- **Heaviest Functions:** `percent_data_escape` (Impact: 73.0), `percent_plus_escape` (Impact: 24.5), `do_unescape` (Impact: 23.7)

### 7. `common/convert.c` (C) -> Cumulative Risk: **671.08**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 313.7 | **LOC:** 299 | **CtrlFlow:** 37.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (98.9634%)
- **Heaviest Functions:** `hex2str` (Impact: 37.8), `hexcolon2bin` (Impact: 35.6), `hex2fixedbuf` (Impact: 23.1)

### 8. `g10/call-agent.c` (C) -> Cumulative Risk: **670.2**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3709.78 | **LOC:** 3467 | **CtrlFlow:** 28.2% | **Authorship Centralization:** 80.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (97.9394%)
- **Heaviest Functions:** `learn_status_cb` (Impact: 285.5), `agent_pkdecrypt` (Impact: 116.5), `agent_get_passphrase` (Impact: 88.8)

### 9. `common/stringhelp.c` (C) -> Cumulative Risk: **669.05**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 2029.16 | **LOC:** 1889 | **CtrlFlow:** 26.0% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (99.2489%), Tech Debt (96.5122%)
- **Heaviest Functions:** `do_make_filename` (Impact: 100.2), `substitute_vars` (Impact: 53.2), `do_percent_escape` (Impact: 44.9)

### 10. `g10/export.c` (C) -> Cumulative Risk: **668.12**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3351.8 | **LOC:** 3326 | **CtrlFlow:** 27.2% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (98.7261%), Safety Score (97.2173%)
- **Heaviest Functions:** `do_export_one_keyblock` (Impact: 391.1), `transfer_format_to_openpgp` (Impact: 219.7), `do_export_stream` (Impact: 158.8)
...
- **Risk Profile:** Cognitive Load (86.9917%), Tech Debt (14.5781%)
**Top Internal Functions/Classes:**
  * `read_ef_aodf` (Impact: 450.7)
  * `do_sign` (Impact: 417.2)
  * `read_p15_info` (Impact: 233.2)
    * *Intent:* /* Get all the basic information from the pkcs#15 card, check the
  * `read_ef_pukdf` (Impact: 206.4)
  * `verify_pin` (Impact: 205.0)
...
- **Risk Profile:** Cognitive Load (82.7728%), Tech Debt (10.1966%)
**Top Internal Functions/Classes:**
  * `ecc_writekey` (Impact: 293.6)
  * `do_change_pin` (Impact: 278.1)
  * `do_decipher` (Impact: 276.7)
  * `rsa_writekey` (Impact: 246.6)
  * `do_setattr` (Impact: 201.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (81.0891%), Tech Debt (16.6445%)
**Top Internal Functions/Classes:**
  * `keyedit_menu` (Impact: 770.7)
    * *Intent:* #endif /* HAVE_LIBREADLINE */
  * `sign_uids` (Impact: 505.5)
    * *Intent:* /* * Loop over all LOCUSR and sign the uids after asking. If no user id * is marked, all user ids wi...
  * `keyedit_print_one_sig` (Impact: 220.8)
    * *Intent:* /* * Print information about a signature (rc is its status), check it * and return true if the signa...
  * `show_key_with_all_names` (Impact: 208.1)
    * *Intent:* /*
  * `show_key_with_all_names_colon` (Impact: 140.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (47.65%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `Eval_Cycle` (Impact: 644.6)
    * *Intent:* /* Safe because we would have already returned if `fast' encountered a non-pair. */
  * `atom2str` (Impact: 175.2)
    * *Intent:* /* print atoms */
  * `readstrexp` (Impact: 71.0)
  * `MacTS_main` (Impact: 61.7)
  * `mk_sharp_const` (Impact: 61.5)
**Contextual Mitigations & Amplifications:**
...
- `common/iobuf.h` -> **Severity: 0.002** (Bridge: 0.0001 * Flux: 12.58%)
