...
| Avg Path Length | 1.8522 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 41.3 | 40.0 | 0.0 |
...
- `next` (@ `src/tokenizer.zig`) -> Impact: **215.6** | LOC: 691
- `renderNode` (@ `src/render.zig`) -> Impact: **128.8** | LOC: 384
- `get` (@ `src/value.zig`) -> Impact: **71.2** | LOC: 82
  * *Intent:* /// Returns value in `container` at `index`.
- `MultiArrayList` (@ `src/multi_array_list.zig`) -> Impact: **70.0** | LOC: 410
...
- `lastToken` (@ `src/Tree.zig`) -> Impact: **62.6** | LOC: 178
- `genComparison` (@ `src/Compiler.zig`) -> Impact: **58.7** | LOC: 101
- `bogToZig` (@ `src/value.zig`) -> Impact: **58.2** | LOC: 84
  * *Intent:* /// Converts Bog value to Zig value. Returned string is invalidated /// on next garbage collection.
- `dump` (@ `src/value.zig`) -> Impact: **46.2** | LOC: 84
  * *Intent:* /// Prints string representation of value to writer
- `as` (@ `src/value.zig`) -> Impact: **42.9** | LOC: 58
  * *Intent:* /// `type_id` must be valid and cannot be .err, .range, .func or .native
- `genMatch` (@ `src/Compiler.zig`) -> Impact: **41.6** | LOC: 113
...
- **Heaviest Functions:** `next` (Impact: 215.6), `getIndent` (Impact: 32.6), `tokenizeRepl` (Impact: 12.7)
...
- **Heaviest Functions:** `get` (Impact: 31.3), `set` (Impact: 16.8), `eql` (Impact: 7.3)
...
- **Heaviest Functions:** `lastToken` (Impact: 62.6), `nodeItems` (Impact: 15.9), `firstToken` (Impact: 15.2)
...
- **Heaviest Functions:** `MultiArrayList` (Impact: 70.0), `shrinkAndFree` (Impact: 14.3), `setCapacity` (Impact: 9.4)
...
- **Heaviest Functions:** `format` (Impact: 37.8), `as` (Impact: 21.6), `get` (Impact: 21.3)
...
- **Heaviest Functions:** `bog_Vm_run` (Impact: 6.5), `bog_Vm_call` (Impact: 5.3), `bog_parse` (Impact: 4.5)
...
- **Heaviest Functions:** `get` (Impact: 16.6), `write` (Impact: 4.2), `close` (Impact: 3.7)
...
- **Heaviest Functions:** `getOrPutInternal` (Impact: 23.3), `insertAllEntriesIntoNewHeaderGeneric` (Impact: 11.8), `ensureTotalCapacity` (Impact: 11.1)
...
- **Heaviest Functions:** `dump` (Impact: 25.5), `dumpLineCol` (Impact: 5.9), `needsDebugInfo` (Impact: 5.0)
...
- **Heaviest Functions:** `import` (Impact: 24.8), `newVal` (Impact: 12.0), `push` (Impact: 11.4)
...
**Top Internal Functions/Classes:**
  * `genComparison` (Impact: 58.7)
  * `genMatch` (Impact: 41.6)
  * `genAs` (Impact: 40.2)
  * `genIf` (Impact: 37.6)
  * `genWhile` (Impact: 35.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00086 | `Ripple Effect (Closeness):` 0.36744
...
**Top Internal Functions/Classes:**
  * `next` (Impact: 215.6)
  * `getIndent` (Impact: 32.6)
  * `tokenizeRepl` (Impact: 12.7)
  * `tokenize` (Impact: 7.8)
  * `isWhiteSpace` (Impact: 6.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.009462 | `Ripple Effect (Closeness):` 0.361787
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 23.081
  * `Choke Point (Betweenness):` 0.00086 | `Ripple Effect (Closeness):` 0.361787
  * `Imports (Out-Degree: 2):` bog.zig, std
  * `Imported By (In-Degree: 2):` (Excluded from Brief to save tokens)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.084731 | `Ripple Effect (Closeness):` 0.356305
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00086 | `Ripple Effect (Closeness):` 0.356305
...
**Top Internal Functions/Classes:**
  * `lastToken` (Impact: 62.6)
  * `nodeItems` (Impact: 15.9)
  * `firstToken` (Impact: 15.2)
  * `get` (Impact: 10.3)
  * `prevToken` (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 19.727
  * `Choke Point (Betweenness):` 0.036344 | `Ripple Effect (Closeness):` 0.356305
...
**Top Internal Functions/Classes:**
  * `getOrPutInternal` (Impact: 23.3)
    * *Intent:* // // ------------------ No pub fns below this point ------------------ /// Must `ensureTotalCapacit...
  * `insertAllEntriesIntoNewHeaderGeneric` (Impact: 11.8)
  * `ensureTotalCapacity` (Impact: 11.1)
    * *Intent:* /// Increases capacity, guaranteeing that insertions up until the /// `expected_count` will not caus...
  * `getSlotByKey` (Impact: 11.1)
  * `getOrPutAssumeCapacityAdapted` (Impact: 10.5)
...
**Top Internal Functions/Classes:**
  * `markGray` (Impact: 26.3)
  * `allocExtra` (Impact: 17.6)
  * `markVal` (Impact: 16.9)
  * `dupe` (Impact: 15.3)
...
**Top Internal Functions/Classes:**
  * `run` (Impact: 16.7)
  * `debugTokens` (Impact: 14.3)
  * `fmtFile` (Impact: 12.9)
  * `main` (Impact: 9.4)
  * `debugDump` (Impact: 8.2)
    * *Intent:* ;
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `dump` (Impact: 25.5)
  * `dumpLineCol` (Impact: 5.9)
  * `needsDebugInfo` (Impact: 5.0)
  * `hasResult` (Impact: 4.8)
  * `dumpList` (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 19.727
  * `Choke Point (Betweenness):` 0.008387 | `Ripple Effect (Closeness):` 0.356305
  * `Imports (Out-Degree: 3):` bog.zig, multi_array_list.zig, std
  * `Imported By (In-Degree: 1):` (Excluded from Brief to save tokens)
...
**Top Internal Functions/Classes:**
  * `handleLine` (Impact: 23.4)
  * `readLine` (Impact: 14.3)
  * `run` (Impact: 9.4)
  * `init` (Impact: 7.3)
  * `deinit` (Impact: 2.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `bog_Vm_run` (Impact: 6.5)
  * `bog_Vm_call` (Impact: 5.3)
  * `bog_parse` (Impact: 4.5)
  * `bog_Tree_render` (Impact: 4.3)
  * `bog_Vm_init` (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Amplified Cascading Flux:* 8 instances
* *Memory Alloc (weighted view):* 0
...
**Top Internal Functions/Classes:**
  * `expectCallOutput` (Impact: 18.8)
  * `expectOutput` (Impact: 11.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `parseInternal` (Impact: 17.3)
  * `stringify` (Impact: 2.0)
  * `parse` (Impact: 1.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
**Top Internal Functions/Classes:**
  * `testTransform` (Impact: 6.1)
  * `testCanonical` (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
- `src/value.zig` -> **Severity: 7.832** (Bridge: 0.0847 * Flux: 92.4333%)
- `src/Tree.zig` -> **Severity: 3.625** (Bridge: 0.0363 * Flux: 99.7312%)
