...
| Avg Path Length | 2.1874 | Hops between files. Lower = Tighter coupling. |
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 16.4 | 0.0 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 5.1 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 17.5 | 2.3 | 0.0 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 65.1 | 100.0 | 100.0 |
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 7.4 | 0.7 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 4.0 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 44.7 | 33.3 | 0.0 |
...
- `pandocApp` (@ `wasm/index.js`) -> Impact: **369.4** | LOC: 1329
  * *Intent:* // Petite Vue app definition
- `inlineToMarkdown` (@ `src/Text/Pandoc/Writers/Markdown/Inline.hs`) -> Impact: **243.1** | LOC: 393
  * *Intent:* -- | Convert Pandoc inline element to markdown.
- `blockToMarkdown'` (@ `src/Text/Pandoc/Writers/Markdown.hs`) -> Impact: **213.9** | LOC: 398
- `blockToLaTeX` (@ `src/Text/Pandoc/Writers/LaTeX.hs`) -> Impact: **134.5** | LOC: 342
  * *Intent:* -- | Convert Pandoc block element to LaTeX.
- `inlineToLaTeX` (@ `src/Text/Pandoc/Writers/LaTeX.hs`) -> Impact: **131.6** | LOC: 341
  * *Intent:* -- | Convert inline element to LaTeX
- `inlineToHtml` (@ `src/Text/Pandoc/Writers/HTML.hs`) -> Impact: **131.2** | LOC: 269
  * *Intent:* -- | Convert Pandoc inline element to HTML.
- `blockToAsciiDoc` (@ `src/Text/Pandoc/Writers/AsciiDoc.hs`) -> Impact: **127.2** | LOC: 257
  * *Intent:* -- | Convert Pandoc block element to asciidoc.
- `obfuscateLink` (@ `src/Text/Pandoc/Writers/HTML.hs`) -> Impact: **124.3** | LOC: 249
  * *Intent:* -- | Obfuscate a "mailto:" link.
- `buildOptions` (@ `wasm/index.js`) -> Impact: **118.9** | LOC: 218
- `convertWithOpts'` (@ `src/Text/Pandoc/App.hs`) -> Impact: **99.7** | LOC: 205
...
### 1. `wasm/index.js` (JAVASCRIPT) -> Cumulative Risk: **808.86**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 2162.68 | **LOC:** 1489 | **CtrlFlow:** 27.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `pandocApp` (Impact: 369.4), `buildOptions` (Impact: 118.9), `loadExampleFromBuffer` (Impact: 69.0)

### 2. `wasm/pandoc.js` (JAVASCRIPT) -> Cumulative Risk: **687.24**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 106.34 | **LOC:** 168 | **CtrlFlow:** 10.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9998%), State Flux (99.8865%)
- **Heaviest Functions:** `convert` (Impact: 24.8), `query` (Impact: 3.8), `addFile` (Impact: 2.2)

### 3. `src/Text/Pandoc/Sources.hs` (HASKELL) -> Cumulative Risk: **497.97**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 73.78 | **LOC:** 208 | **CtrlFlow:** 8.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (98.5391%), Documentation (85.3659%), Verification (80.0%)
- **Heaviest Functions:** `ensureFinalNewlines` (Impact: 9.4), `updateSourcePos` (Impact: 8.5), `satisfy` (Impact: 4.5)

### 4. `tools/extract-changes.lua` (LUA) -> Cumulative Risk: **496.23**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 0.03 | **LOC:** 19 | **CtrlFlow:** 43.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9254%), Safety Score (82.388%)
- **Heaviest Functions:** `Pandoc` (Impact: 12.1)

### 5. `tools/update-readme.lua` (LUA) -> Cumulative Risk: **489.94**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 0.02 | **LOC:** 29 | **CtrlFlow:** 21.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9254%), Safety Score (79.7611%)
- **Heaviest Functions:** `Div` (Impact: 9.4), `__global_context__` (Impact: 1.5)

### 6. `tools/moduledeps.lua` (LUA) -> Cumulative Risk: **476.96**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 0.07 | **LOC:** 78 | **CtrlFlow:** 31.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (92.4953%)
- **Heaviest Functions:** `Anonymous_Block` (Impact: 7.7), `Anonymous_Block` (Impact: 6.5), `add_transitive_deps` (Impact: 6.3)

### 7. `src/Text/Pandoc/Writers/Muse.hs` (HASKELL) -> Cumulative Risk: **473.26**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 382.22 | **LOC:** 733 | **CtrlFlow:** 14.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.3489%), Verification (80.0%), Documentation (77.5281%)
- **Heaviest Functions:** `inlineToMuse` (Impact: 65.7), `blockToMuse` (Impact: 24.6), `simpleTable` (Impact: 19.2)

### 8. `src/Text/Pandoc/Writers/LaTeX.hs` (HASKELL) -> Cumulative Risk: **471.54**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 723.92 | **LOC:** 1512 | **CtrlFlow:** 18.9% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (86.7347%), Verification (80.0%), Safety Score (53.8648%)
- **Heaviest Functions:** `blockToLaTeX` (Impact: 134.5), `inlineToLaTeX` (Impact: 131.6), `pandocToLaTeX` (Impact: 74.8)

### 9. `src/Text/Pandoc/Chunks.hs` (HASKELL) -> Cumulative Risk: **455.35**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 211.26 | **LOC:** 445 | **CtrlFlow:** 9.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.98%), Verification (80.0%), Documentation (77.193%)
- **Heaviest Functions:** `fixInternalReferences` (Impact: 24.4), `tocEntryToLink` (Impact: 18.5), `makeChunks` (Impact: 14.6)

### 10. `src/Text/Pandoc/Readers/Docx.hs` (HASKELL) -> Cumulative Risk: **452.78**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 395.7 | **LOC:** 920 | **CtrlFlow:** 13.6% | **Authorship Centralization:** 80.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Verification (80.0%), Safety Score (51.6875%)
- **Heaviest Functions:** `bodyPartToBlocks` (Impact: 46.5), `parPartToInlines'` (Impact: 33.7), `runStyleToTransform` (Impact: 24.6)
...
- **Magnitude:** 2162.68 | **LOC:** 1489 | **CtrlFlow:** 27.3% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (99.9418%), Tech Debt (97.0476%)
**Top Internal Functions/Classes:**
  * `pandocApp` (Impact: 369.4)
    * *Intent:* // Petite Vue app definition
  * `buildOptions` (Impact: 118.9)
  * `loadExampleFromBuffer` (Impact: 69.0)
  * `addFile` (Impact: 60.5)
    * *Intent:* // Helper to add a file to the zip
  * `downloadAsExample` (Impact: 49.0)
...
- **Risk Profile:** Cognitive Load (7.4678%), Tech Debt (9.7152%)
**Top Internal Functions/Classes:**
  * `graphicToElement` (Impact: 32.3)
  * `metadataToElement` (Impact: 31.3)
  * `makePicElements` (Impact: 31.2)
  * `presentationToPresentationElement` (Impact: 23.7)
  * `getContentType` (Impact: 22.6)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (10.0906%), Tech Debt (33.9335%)
**Top Internal Functions/Classes:**
  * `inlineToHtml` (Impact: 131.2)
    * *Intent:* -- | Convert Pandoc inline element to HTML.
  * `obfuscateLink` (Impact: 124.3)
    * *Intent:* -- | Obfuscate a "mailto:" link.
  * `pandocToHtml` (Impact: 56.8)
    * *Intent:* -- result is (title, authors, date, toc, body, new variables)
  * `footnoteSection` (Impact: 38.7)
    * *Intent:* -- | Convert list of Note blocks to a footnote <div>. -- Assumes notes are sorted.
  * `blockListToNote` (Impact: 26.6)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000206 | `Ripple Effect (Closeness):` 0.0
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 723.92 | **LOC:** 1512 | **CtrlFlow:** 18.9% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (17.1224%), Tech Debt (28.0936%)
**Top Internal Functions/Classes:**
  * `blockToLaTeX` (Impact: 134.5)
    * *Intent:* -- | Convert Pandoc block element to LaTeX.
  * `inlineToLaTeX` (Impact: 131.6)
    * *Intent:* -- | Convert inline element to LaTeX
  * `pandocToLaTeX` (Impact: 74.8)
  * `sectionHeader` (Impact: 50.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00096 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (7.5352%), Tech Debt (7.84%)
**Top Internal Functions/Classes:**
  * `pandocToEPUB` (Impact: 54.0)
  * `metadataElement` (Impact: 41.5)
  * `createNavEntry` (Impact: 39.4)
  * `addMetadataFromXML` (Impact: 39.1)
  * `getEPUBMetadata` (Impact: 28.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
...
- **Risk Profile:** Cognitive Load (12.1848%), Tech Debt (11.5979%)
**Top Internal Functions/Classes:**
  * `blockToMarkdown'` (Impact: 213.9)
  * `pandocToMarkdown` (Impact: 35.7)
    * *Intent:* -- | Return markdown representation of document.
  * `definitionListItemToMarkdown` (Impact: 22.4)
    * *Intent:* -- | Convert definition list item (label, list of blocks) to markdown.
  * `blockListToMarkdown` (Impact: 18.2)
    * *Intent:* -- | Convert list of Pandoc block elements to markdown.
  * `orderedListItemToMarkdown` (Impact: 15.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001126 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (10.3691%), Tech Debt (12.8938%)
**Top Internal Functions/Classes:**
  * `blockToRST` (Impact: 77.1)
    * *Intent:* -- | Convert Pandoc block element to RST.
  * `inlineToRST` (Impact: 42.7)
    * *Intent:* -- | Convert Pandoc inline element to RST.
  * `escapeText` (Impact: 41.0)
    * *Intent:* -- | Escape special characters for RST.
  * `simpleTable` (Impact: 35.3)
  * `transformInlines` (Impact: 23.0)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (12.1251%), Tech Debt (53.6312%)
**Top Internal Functions/Classes:**
  * `blockToAsciiDoc` (Impact: 127.2)
    * *Intent:* -- | Convert Pandoc block element to asciidoc.
  * `inlineToAsciiDoc` (Impact: 72.7)
    * *Intent:* -- | Convert Pandoc inline element to asciidoc.
  * `imageArguments` (Impact: 20.8)
    * *Intent:* -- | Provides the arguments for both `image:` and `image::` -- e.g.: sunset.jpg[Sunset,300,200]
  * `escapeString` (Impact: 19.2)
    * *Intent:* -- | Escape special characters for AsciiDoc.
  * `adjustEmptyRows` (Impact: 14.7)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 533.74 | **LOC:** 1500 | **CtrlFlow:** 13.2% | **Authorship Centralization:** 66.7%
- **Risk Profile:** Cognitive Load (10.8465%), Tech Debt (13.6927%)
**Top Internal Functions/Classes:**
  * `elemToParPart'` (Impact: 39.5)
  * `elemToBodyPart` (Impact: 32.5)
  * `unwrapElement` (Impact: 20.1)
  * `elemToRunElem` (Impact: 19.9)
  * `childElemToRun` (Impact: 17.7)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (5.3641%), Tech Debt (0.0%)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (11.9518%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `blockToOpenDocument` (Impact: 45.5)
    * *Intent:* -- | Convert a Pandoc block element to OpenDocument.
  * `mkLink` (Impact: 28.8)
  * `inlineToOpenDocument` (Impact: 25.1)
    * *Intent:* -- | Convert an inline element to OpenDocument.
  * `figure` (Impact: 14.2)
  * `paraStyle` (Impact: 14.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 1.1e-05 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (8.2477%), Tech Debt (8.5256%)
**Top Internal Functions/Classes:**
  * `writeBibtexString` (Impact: 89.4)
    * *Intent:* -- | Write BibTeX or BibLaTeX given given a 'Reference'.
  * `itemToReference` (Impact: 82.0)
  * `transformKey` (Impact: 22.4)
    * *Intent:* -- transformKey source target key -- derived from Appendix C of bibtex manual
  * `getContentsFor` (Impact: 16.3)
  * `getOldDate` (Impact: 10.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001017 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (20.6984%), Tech Debt (14.6067%)
**Top Internal Functions/Classes:**
  * `inlineToMarkdown` (Impact: 243.1)
    * *Intent:* -- | Convert Pandoc inline element to markdown.
  * `escapeText` (Impact: 41.3)
    * *Intent:* -- | Escape special characters for Markdown.
  * `go` (Impact: 23.3)
  * `getReference` (Impact: 18.4)
    * *Intent:* -- | Get reference for target; if none exists, create unique one and return. -- Prefer label if poss...
  * `attrsToMarkua` (Impact: 15.7)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
- **Risk Profile:** Cognitive Load (7.4165%), Tech Debt (61.1107%)
**Top Internal Functions/Classes:**
  * `csvTableDirective` (Impact: 33.5)
  * `resolveReferences` (Impact: 21.2)
  * `unicodeTransform` (Impact: 16.5)
    * *Intent:* -- Can contain character codes as decimal numbers or -- hexadecimal numbers, prefixed by 0x, x, \x, ...
  * `listTableDirective` (Impact: 15.7)
    * *Intent:* -- TODO: :stub-columns:. -- Only the first row becomes the header even if header-rows: > 1, -- since...
  * `addNewRole` (Impact: 15.0)
...
- **Risk Profile:** Cognitive Load (8.4875%), Tech Debt (18.1538%)
**Top Internal Functions/Classes:**
  * `totoks` (Impact: 55.1)
  * `tokenize` (Impact: 48.6)
  * `doMacros'` (Impact: 21.7)
  * `rawLaTeXParser` (Impact: 18.0)
  * `handleMacros` (Impact: 15.0)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00024 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (4.4101%), Tech Debt (22.3303%)
**Top Internal Functions/Classes:**
  * `rowAndBottom` (Impact: 18.9)
  * `gridRows` (Impact: 17.6)
  * `toLegacyTable` (Impact: 15.1)
    * *Intent:* -- | Convert the relevant components of a new-style table (with block -- caption, row headers, row a...
  * `formatBorder` (Impact: 13.4)
  * `splitSentences` (Impact: 11.2)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004141 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (11.4928%), Tech Debt (8.1081%)
**Top Internal Functions/Classes:**
  * `parseBlock` (Impact: 66.7)
  * `parseMixed` (Impact: 58.8)
  * `wrapWithHeader` (Impact: 25.6)
  * `parseInline` (Impact: 23.7)
  * `isBlockElement` (Impact: 16.9)
**Structural Signatures (Net Mitigated Signals):**
...
- **Risk Profile:** Cognitive Load (11.3429%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `blockToOpenXML'` (Impact: 86.2)
  * `inlineToOpenXML'` (Impact: 67.0)
  * `writeOpenXML` (Impact: 20.2)
    * *Intent:* -- | Convert Pandoc document to rendered document contents plus two lists of -- OpenXML elements (fo...
  * `listItemToOpenXML` (Impact: 11.1)
  * `generateImgElt` (Impact: 10.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 3 instances
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 395.7 | **LOC:** 920 | **CtrlFlow:** 13.6% | **Authorship Centralization:** 80.0%
- **Risk Profile:** Cognitive Load (13.9183%), Tech Debt (23.7458%)
**Top Internal Functions/Classes:**
  * `bodyPartToBlocks` (Impact: 46.5)
  * `parPartToInlines'` (Impact: 33.7)
  * `runStyleToTransform` (Impact: 24.6)
  * `go` (Impact: 21.2)
  * `parStyleToTransform` (Impact: 14.3)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.323
  * `Choke Point (Betweenness):` 0.000807 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001152 | `Ripple Effect (Closeness):` 0.0
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 382.22 | **LOC:** 733 | **CtrlFlow:** 14.9% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (7.9994%), Tech Debt (99.3489%)
**Top Internal Functions/Classes:**
  * `inlineToMuse` (Impact: 65.7)
    * *Intent:* -- | Convert Pandoc inline element to Muse.
  * `blockToMuse` (Impact: 24.6)
    * *Intent:* -- | Convert Pandoc block element to Muse.
  * `simpleTable` (Impact: 19.2)
  * `fixOrEscape` (Impact: 16.6)
  * `fixOrEscapeStr` (Impact: 16.3)
**Structural Signatures (Net Mitigated Signals):**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 0.317
  * `Choke Point (Betweenness):` 0.000139 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (8.3925%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `blockToConTeXt` (Impact: 49.2)
    * *Intent:* -- | Convert Pandoc block element to ConTeXt.
  * `inlineToConTeXt` (Impact: 41.4)
    * *Intent:* -- | Convert inline element to ConTeXt
  * `tableCellToConTeXt` (Impact: 23.9)
  * `sectionLevelToText` (Impact: 19.2)
    * *Intent:* -- | Generate a textual representation of the section level
  * `pandocToConTeXt` (Impact: 18.6)
**Structural Signatures (Net Mitigated Signals):**
...
- `src/Text/Pandoc/Citeproc.hs` -> **Severity: 0.048** (Bridge: 0.0037 * Flux: 13.0619%)
- `src/Text/Pandoc/Writers/LaTeX.hs` -> **Severity: 0.029** (Bridge: 0.001 * Flux: 30.4711%)
- `src/Text/Pandoc/Readers/Docx.hs` -> **Severity: 0.021** (Bridge: 0.0008 * Flux: 26.6328%)
- `src/Text/Pandoc/Writers/Markdown.hs` -> **Severity: 0.012** (Bridge: 0.0011 * Flux: 10.2889%)
- `src/Text/Pandoc/Citeproc/BibTeX.hs` -> **Severity: 0.01** (Bridge: 0.001 * Flux: 9.7032%)
