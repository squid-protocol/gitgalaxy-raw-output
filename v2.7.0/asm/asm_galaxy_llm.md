...
- `_calculate_dot_product` (@ `float/dot_product.asm`) -> Impact: **7.5** | LOC: 29
  * *Intent:* ;; Prepare to calculate the dot product of the two vectors.
- `_parse_first_float_vector` (@ `float/dot_product.asm`) -> Impact: **6.6** | LOC: 35
  * *Intent:* ;; Parse the floating-point values from the input buffer.
- `_parse_second_float_vector` (@ `float/dot_product.asm`) -> Impact: **6.6** | LOC: 35
  * *Intent:* ;; Parse the floating-point values from the input buffer.
- `.loop` (@ `casm/casm3/casm.asm`) -> Impact: **6.1** | LOC: 9
- `_loop` (@ `float/dot_product.asm`) -> Impact: **5.8** | LOC: 18
  * *Intent:* ;; Calculate the the dot product in the loop.
- `_start` (@ `stack/stack.asm`) -> Impact: **5.2** | LOC: 34
  * *Intent:* ;; Entry point
- `__repeat` (@ `stack/stack.asm`) -> Impact: **4.9** | LOC: 18
- `reverseStringAndPrint` (@ `strings/reverse.asm`) -> Impact: **4.8** | LOC: 15
  * *Intent:* ;; Calculate the length of the input string and prepare to reverse it.
- `int_to_str` (@ `stack/stack.asm`) -> Impact: **4.5** | LOC: 20
  * *Intent:* ;; Convert the sum to a string and print it to the standard output.
- `reverseString` (@ `strings/reverse.asm`) -> Impact: **4.2** | LOC: 15
...
- **Heaviest Functions:** `reverseStringAndPrint` (Impact: 4.8), `reverseString` (Impact: 4.2), `printResult` (Impact: 3.4)
...
- **Heaviest Functions:** `_start` (Impact: 5.2), `__repeat` (Impact: 4.9), `int_to_str` (Impact: 4.5)
...
- **Heaviest Functions:** `_calculate_dot_product` (Impact: 7.5), `_parse_first_float_vector` (Impact: 6.6), `_parse_second_float_vector` (Impact: 6.6)
...
- **Heaviest Functions:** `.loop` (Impact: 6.1), `my_strlen` (Impact: 1.2), `.done` (Impact: 1.1)
...
- **Heaviest Functions:** `_start` (Impact: 2.9)
...
- **Heaviest Functions:** `.correctSum` (Impact: 2.6), `.compare` (Impact: 2.4), `.exit` (Impact: 1.8)
...
- **Heaviest Functions:** `_start` (Impact: 2.7)
...
- **Heaviest Functions:** `all` (Impact: 1.1), `clean` (Impact: 1.1)
...
**Top Internal Functions/Classes:**
  * `_calculate_dot_product` (Impact: 7.5)
    * *Intent:* ;; Prepare to calculate the dot product of the two vectors.
  * `_parse_first_float_vector` (Impact: 6.6)
    * *Intent:* ;; Parse the floating-point values from the input buffer.
  * `_parse_second_float_vector` (Impact: 6.6)
    * *Intent:* ;; Parse the floating-point values from the input buffer.
  * `_loop` (Impact: 5.8)
    * *Intent:* ;; Calculate the the dot product in the loop.
  * `_read_first_float_vector` (Impact: 4.1)
...
**Top Internal Functions/Classes:**
  * `_start` (Impact: 5.2)
    * *Intent:* ;; Entry point
  * `__repeat` (Impact: 4.9)
  * `int_to_str` (Impact: 4.5)
    * *Intent:* ;; Convert the sum to a string and print it to the standard output.
  * `printResult` (Impact: 3.7)
    * *Intent:* ;; Print the result to the standard output.
  * `argcError` (Impact: 2.6)
...
**Top Internal Functions/Classes:**
  * `reverseStringAndPrint` (Impact: 4.8)
    * *Intent:* ;; Calculate the length of the input string and prepare to reverse it.
  * `reverseString` (Impact: 4.2)
    * *Intent:* ;; Reverse the string and store it in the output buffer.
  * `printResult` (Impact: 3.4)
