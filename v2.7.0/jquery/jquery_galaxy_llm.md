...
| Avg Path Length | 3.5295 | Hops between files. Lower = Tighter coupling. |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 67.3 | 2.1 | 0.0 | 0.0 |
...
- `ajax` (@ `src/ajax.js`) -> Impact: **205.1** | LOC: 464
  * *Intent:* // Main method
- `trigger` (@ `src/event/trigger.js`) -> Impact: **130.2** | LOC: 144
- `CHILD` (@ `src/selector.js`) -> Impact: **125.6** | LOC: 111
- `defaultPrefilter` (@ `src/effects.js`) -> Impact: **104.3** | LOC: 167
- `find` (@ `src/selector.js`) -> Impact: **98.0** | LOC: 126
- `setMatcher` (@ `src/selector.js`) -> Impact: **94.9** | LOC: 98
- `superMatcher` (@ `src/selector.js`) -> Impact: **91.4** | LOC: 113
- `remove` (@ `src/event.js`) -> Impact: **89.3** | LOC: 72
  * *Intent:* // Detach an event or set of events from an element
- `done` (@ `src/ajax.js`) -> Impact: **80.2** | LOC: 129
  * *Intent:* // Callback for when everything is done
- `add` (@ `src/event.js`) -> Impact: **74.1** | LOC: 110
...
- **Heaviest Functions:** `ajax` (Impact: 205.1), `done` (Impact: 80.2), `ajaxConvert` (Impact: 65.1)
...
- **Heaviest Functions:** `send` (Impact: 37.3), `callback` (Impact: 14.2), `abort` (Impact: 2.2)
...
- **Heaviest Functions:** `Callbacks` (Impact: 56.3), `add` (Impact: 14.4), `fire` (Impact: 12.2)
...
- **Heaviest Functions:** `extend` (Impact: 27.6), `map` (Impact: 15.5), `each` (Impact: 13.1)
...
- **Heaviest Functions:** `find` (Impact: 62.3), `matches` (Impact: 1.9), `matchesSelector` (Impact: 1.9)
...
- **Heaviest Functions:** `run` (Impact: 11.1), `init` (Impact: 11.0), `set` (Impact: 10.6)
...
- **Heaviest Functions:** `closest` (Impact: 20.5), `fn[ name ]` (Impact: 13.4), `index` (Impact: 9.4)
...
- **Heaviest Functions:** `delay` (Impact: 7.5), `stop` (Impact: 1.1)
...
- **Heaviest Functions:** `trigger` (Impact: 130.2), `triggerHandler` (Impact: 3.8), `simulate` (Impact: 2.6)
...
- **Heaviest Functions:** `defaultPrefilter` (Impact: 104.3), `Animation` (Impact: 44.0), `stop` (Impact: 40.6)
...
**Top Internal Functions/Classes:**
  ...
  * `superMatcher` (Impact: 91.4)
  * `matcherFromGroupMatchers` (Impact: 68.4)
...
**Top Internal Functions/Classes:**
  * `beforeSend` (Impact: 17.9)
  * `beforeSend` (Impact: 12.7)
  * `request` (Impact: 9.5)
  * `beforeSend` (Impact: 9.2)
  * `success` (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 807.66 | **LOC:** 3227 | **CtrlFlow:** 6.0% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (34.6324%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `testAppendForObject` (Impact: 31.6)
  * `testAppend` (Impact: 21.1)
  * `testReplaceWith` (Impact: 6.9)
  * `htmlPrefilter` (Impact: 4.6)
  * `assertSpecialCharsSupport` (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `defaultPrefilter` (Impact: 104.3)
  * `Animation` (Impact: 44.0)
  * `stop` (Impact: 40.6)
  * `speed` (Impact: 40.1)
  * `finish` (Impact: 19.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `wrapperRemoveEvent` (Impact: 6.2)
  * `handler` (Impact: 6.0)
  * `handler` (Impact: 6.0)
  * `handlerWithData` (Impact: 6.0)
  * `addEventListener` (Impact: 5.8)
...
**Top Internal Functions/Classes:**
  * `onFulfilled` (Impact: 4.9)
  * `onFulfilled` (Impact: 4.9)
  * `onRejected` (Impact: 4.8)
  * `onRejected` (Impact: 4.8)
  * `createDeferred` (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `assertNoMatch` (Impact: 12.7)
  * `testLang` (Impact: 8.6)
  * `mixCase` (Impact: 4.7)
  * `getUniqueSortFixtures` (Impact: 4.0)
  * `get` (Impact: 3.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Race Conditions:* 3 instances
...
**Top Internal Functions/Classes:**
  ...
  * `boxModelAdjustment` (Impact: 43.2)
  * `getWidthOrHeight` (Impact: 37.6)
  * `css` (Impact: 31.0)
  * `set` (Impact: 15.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ajaxTest` (Impact: 46.2)
    * *Intent:* // Ajax testing helper
  * `loadTests` (Impact: 17.7)
  * `testIframe` (Impact: 17.1)
  * `callIfDefined` (Impact: 11.0)
  * `loadDep` (Impact: 7.2)
...
**Top Internal Functions/Classes:**
  * `testVal` (Impact: 14.0)
  * `testAddClass` (Impact: 9.2)
  * `testToggleClass` (Impact: 8.4)
  * `testFalseSetter` (Impact: 6.0)
  * `testRemoveClass` (Impact: 5.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 51 instances
...
**Top Internal Functions/Classes:**
  * `Deferred` (Impact: 60.1)
  * `then` (Impact: 52.2)
  * `resolve` (Impact: 48.7)
  * `adoptValue` (Impact: 17.1)
  * `mightThrow` (Impact: 15.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  ...
  * `html` (Impact: 18.9)
  * `remove` (Impact: 17.0)
  * `cleanData` (Impact: 14.4)
  * `cloneCopyEvent` (Impact: 11.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `fn[ method ]` (Impact: 15.4)
  * `position` (Impact: 15.2)
