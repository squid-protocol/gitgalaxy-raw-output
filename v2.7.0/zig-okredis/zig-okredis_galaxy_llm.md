...
| Avg Path Length | 3.4007 | Hops between files. Lower = Tighter coupling. |
...
- `parseImpl` (@ `src/parser/t_map.zig`) -> Impact: **186.6** | LOC: 334
- `pipelineImpl` (@ `src/client.zig`) -> Impact: **96.1** | LOC: 178
- `serializeCommand` (@ `src/serializer.zig`) -> Impact: **82.8** | LOC: 132
- `parseImpl` (@ `src/parser/t_list.zig`) -> Impact: **65.7** | LOC: 106
- `decodeMap` (@ `src/parser/t_map.zig`) -> Impact: **47.3** | LOC: 64
- `parseImpl` (@ `src/parser.zig`) -> Impact: **45.9** | LOC: 114
...
- `parseAlloc` (@ `src/parser/t_set.zig`) -> Impact: **44.1** | LOC: 77
- `freeReply` (@ `src/parser.zig`) -> Impact: **38.8** | LOC: 84
  * *Intent:* // Frees values created by `sendAlloc`. // If the top value is a pointer, it frees that too. // TODO: free stdlib types!
- `decodeArray` (@ `src/parser/t_list.zig`) -> Impact: **31.4** | LOC: 40
- `main` (@ `example.zig`) -> Impact: **22.5** | LOC: 250
...
- **Heaviest Functions:** `parseAlloc` (Impact: 44.1), `parseImpl` (Impact: 7.6), `isSupportedAlloc` (Impact: 6.2)
...
- **Heaviest Functions:** `parseImpl` (Impact: 186.6), `decodeMap` (Impact: 47.3), `isSupported` (Impact: 10.7)
...
- **Heaviest Functions:** `pipelineImpl` (Impact: 96.1), `init` (Impact: 17.3), `transactionImpl` (Impact: 7.6)
...
- **Heaviest Functions:** `parseImpl` (Impact: 65.7), `decodeArray` (Impact: 31.4), `isSupported` (Impact: 7.7)
...
- **Heaviest Functions:** `main` (Impact: 4.5), `toDigit` (Impact: 3.0), `parseFloat` (Impact: 1.8)
...
- **Heaviest Functions:** `serializeCommand` (Impact: 82.8), `serializeArgument` (Impact: 15.0)
...
- **Heaviest Functions:** `parse` (Impact: 13.7), `parseAlloc` (Impact: 10.5), `isSupportedAlloc` (Impact: 6.1)
...
- **Heaviest Functions:** `xaddStruct` (Impact: 2.3), `xreadStruct` (Impact: 2.3), `xadd` (Impact: 2.0)
...
- **Heaviest Functions:** `serialize` (Impact: 4.5), `validate` (Impact: 4.4), `init` (Impact: 2.1)
...
- **Heaviest Functions:** `destroy` (Impact: 9.3), `parseAlloc` (Impact: 2.5), `parse` (Impact: 2.1)
...
**Top Internal Functions/Classes:**
  * `parseImpl` (Impact: 186.6)
  * `decodeMap` (Impact: 47.3)
  * `isSupported` (Impact: 10.7)
    * *Intent:* // Understanding if we want to support a given type is more complex // than with other parsers as th...
  * `isSupportedAlloc` (Impact: 7.6)
  * `parseAlloc` (Impact: 2.6)
...
**Top Internal Functions/Classes:**
  * `parseImpl` (Impact: 65.7)
  * `decodeArray` (Impact: 31.4)
  * `isSupported` (Impact: 7.7)
    * *Intent:* // TODO: prevent users from unmarshaling structs out of strings
  * `isSupportedAlloc` (Impact: 4.5)
  * `parseAlloc` (Impact: 2.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parseAlloc` (Impact: 11.7)
  * `OrFullErr` (Impact: 11.3)
    * *Intent:* /// Like `OrErr`, but it uses an allocator to store the full error message.
  * `internalParse` (Impact: 7.8)
  * `parseAlloc` (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `serializeCommand` (Impact: 82.8)
  * `serializeArgument` (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 2 instances
...
**Top Internal Functions/Classes:**
  * `parseAlloc` (Impact: 44.1)
  * `parseImpl` (Impact: 7.6)
  * `isSupportedAlloc` (Impact: 6.2)
  * `isSupported` (Impact: 4.5)
    * *Intent:* // TODO: prevent users from unmarshaling structs out of strings
  * `parse` (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 3 instances
...
**Top Internal Functions/Classes:**
  * `_forStruct` (Impact: 8.0)
  * `validate` (Impact: 7.6)
    * *Intent:* /// Validates if the command is syntactically correct.
  * `serialize` (Impact: 4.4)
  * `serialize` (Impact: 4.3)
  * `validate` (Impact: 3.0)
    * *Intent:* /// Validates if the command is syntactically correct.
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `parseAlloc` (Impact: 22.1)
  * `parse` (Impact: 9.7)
  * `isSupported` (Impact: 4.5)
  * `isSupportedAlloc` (Impact: 4.5)
  * `MakeEmoji2` (Impact: 1.1)
...
**Top Internal Functions/Classes:**
  * `_forStruct` (Impact: 7.8)
  * `validate` (Impact: 7.6)
    * *Intent:* /// Validates if the command is syntactically correct.
  * `serialize` (Impact: 4.2)
  * `validate` (Impact: 3.0)
    * *Intent:* /// Validates if the command is syntactically correct.
  * `serialize` (Impact: 2.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `parse` (Impact: 13.7)
  * `parseAlloc` (Impact: 10.5)
  * `isSupportedAlloc` (Impact: 6.1)
  * `isSupported` (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `destroy` (Impact: 9.3)
  * `parseAlloc` (Impact: 2.5)
  * `parse` (Impact: 2.1)
  * `MakeComplexListWithAttributes` (Impact: 2.0)
    * *Intent:* // zig fmt: off
  * `MakeComplexList` (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `parseAlloc` (Impact: 14.1)
  * `destroy` (Impact: 2.4)
  * `parse` (Impact: 2.1)
  * `MakeSimpleString` (Impact: 1.1)
    * *Intent:* // TODO: get rid of these!!!
  * `MakeBlobString` (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `serialize` (Impact: 13.2)
  * `count` (Impact: 7.5)
  * `init` (Impact: 5.1)
  * `serialize` (Impact: 2.7)
  * `validate` (Impact: 1.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `isNumericStreamID` (Impact: 12.5)
  * `isAny` (Impact: 5.5)
  * `isValidStreamID` (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
