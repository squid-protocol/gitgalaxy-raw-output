...
| connectivity | 120302 | 16266 | 5 | `library/stdarch/crates/stdarch-gen-loongarch/lasxintrin.h` |
...
| dead_code | 95135 | 28965 | 4 | `library/stdarch/crates/core_arch/src/x86_64/avx512f.rs` |
| credential | 71 | 32 | 0 | `tests/codegen-llvm/sanitizer/cfi/emit-type-metadata-id-itanium-cxx-abi-trait-types.rs` |
...
- `banana` (@ `tests/ui/expr/if/expr-stack-overflow.rs`) -> Impact: **3730.8** | LOC: 2548
...
- `emulate_foreign_item_inner` (@ `src/tools/miri/src/shims/unix/foreign_items.rs`) -> Impact: **1230.4** | LOC: 1190
- `eval_simd_intrinsic` (@ `compiler/rustc_const_eval/src/interpret/intrinsics/simd.rs`) -> Impact: **846.6** | LOC: 792
...
- `initSearch` (@ `src/librustdoc/html/static/js/search.js`) -> Impact: **816.4** | LOC: 1967
...
- `emulate_foreign_item_inner` (@ `src/tools/miri/src/shims/windows/foreign_items.rs`) -> Impact: **804.0** | LOC: 1187
- `exec_intrinsic` (@ `src/tools/rust-analyzer/crates/hir-ty/src/mir/eval/shim.rs`) -> Impact: **657.1** | LOC: 783
- `execQuery` (@ `src/librustdoc/html/static/js/search.js`) -> Impact: **591.4** | LOC: 1387
...
- `emulate_foreign_item_inner` (@ `src/tools/miri/src/shims/foreign_items.rs`) -> Impact: **577.8** | LOC: 631
- `note_obligation_cause_code` (@ `compiler/rustc_trait_selection/src/error_reporting/traits/suggestions.rs`) -> Impact: **550.6** | LOC: 1053
- `LLVMRustOptimize` (@ `compiler/rustc_llvm/llvm-wrapper/PassWrapper.cpp`) -> Impact: **510.4** | LOC: 388
...
- **Heaviest Functions:** `update` (Impact: 39.4), `sequence_formatter` (Impact: 15.2), `MSVCEnumSummaryProvider` (Impact: 14.3)
...
- **Heaviest Functions:** `children_of_btree_map` (Impact: 23.7), `children_of_node` (Impact: 22.7), `__init__` (Impact: 11.2)
...
### 4. `src/tools/rust-analyzer/editors/code/src/run.ts` (TYPESCRIPT) -> Cumulative Risk: **662.66**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 0.27 | **LOC:** 296 | **CtrlFlow:** 23.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9988%), State Flux (99.6197%)
- **Heaviest Functions:** `selectRunnable` (Impact: 26.7), `selectRunnableAtCursor` (Impact: 25.6), `getRunnables` (Impact: 19.8)

### 5. `src/ci/run.sh` (SHELL) -> Cumulative Risk: **661.96**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 21.24 | **LOC:** 283 | **CtrlFlow:** 29.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.8552%)
- **Heaviest Functions:** `Anonymous_Block` (Impact: 24.4), `__global_context__` (Impact: 8.6), `Anonymous_Block` (Impact: 7.5)

### 6. `library/alloc/src/collections/btree/map/tests.rs` (RUST) -> Cumulative Risk: **658.04**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `test_clone_panic_leak` (Impact: 22.3), `test_basic_large` (Impact: 13.6), `test_ord_absence` (Impact: 11.3)

### 7. `compiler/rustc_errors/src/lib.rs` (RUST) -> Cumulative Risk: **655.29**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `emit_diagnostic` (Impact: 68.4), `print_error_count` (Impact: 26.7), `flush_delayed` (Impact: 16.3)

### 8. `src/tools/rust-analyzer/editors/code/src/commands.ts` (TYPESCRIPT) -> Cumulative Risk: **652.0**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
### 9. `library/std/src/io/cursor.rs` (RUST) -> Cumulative Risk: **645.31**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `slice_write_vectored` (Impact: 10.8), `reserve_and_pad` (Impact: 10.0), `vec_write_all_vectored` (Impact: 9.4)

### 10. `library/core/src/slice/iter.rs` (RUST) -> Cumulative Risk: **643.25**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `next` (Impact: 10.7), `next_back` (Impact: 10.7), `next` (Impact: 10.6)
...
**Top Internal Functions/Classes:**
  * `test_msa_binsl_b` (Impact: 3.0)
  * `test_msa_binsr_b` (Impact: 3.0)
  * `test_msa_bmnz_v` (Impact: 3.0)
  * `test_msa_bmz_v` (Impact: 3.0)
  * `test_msa_bsel_v` (Impact: 3.0)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `note_obligation_cause_code` (Impact: 550.6)
  * `note_function_argument_obligation` (Impact: 188.2)
  * `suggest_add_reference_to_arg` (Impact: 148.3)
  * `suggest_dereferences` (Impact: 140.8)
    * *Intent:* /// Provide a suggestion to dereference arguments to functions and binary operators, if that /// wou...
  * `point_at_assoc_type_restriction` (Impact: 105.3)
...
**Top Internal Functions/Classes:**
  * `suggest_traits_to_import` (Impact: 462.4)
  * `handle_unsatisfied_predicates` (Impact: 353.8)
  * `report_failed_method_call_on_numerical_infer_var` (Impact: 112.8)
  * `report_method_error` (Impact: 112.1)
  * `note_candidates_on_method_error` (Impact: 112.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `suggest_ref_or_clone` (Impact: 195.4)
  * `suggest_cloning` (Impact: 140.3)
  * `explain_iterator_advancement_in_for_loop_if_applicable` (Impact: 119.9)
    * *Intent:* /// Suggest using `while let` for call `next` on an iterator in a for loop. /// /// For example: ///...
  * `report_use_of_moved_or_uninitialized` (Impact: 119.5)
  * `suggest_slice_method_if_applicable` (Impact: 104.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 89 instances
...
**Top Internal Functions/Classes:**
  * `smart_resolve_context_dependent_help` (Impact: 251.2)
    * *Intent:* /// Provides context-dependent help for errors reported by the `smart_resolve_path_fragment` /// fun...
  * `add_missing_lifetime_specifiers_label` (Impact: 214.2)
  * `try_lookup_name_relaxed` (Impact: 144.1)
  * `suggest_assoc_type_from_bounds` (Impact: 123.8)
    * *Intent:* /// This does best-effort work to generate suggestions for associated types.
  * `make_base_error` (Impact: 115.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `smart_resolve_path_fragment` (Impact: 133.0)
  * `with_generic_param_rib` (Impact: 94.1)
  * `resolve_anonymous_lifetime` (Impact: 91.5)
  * `resolve_elided_lifetimes_in_path` (Impact: 91.2)
  * `resolve_qpath` (Impact: 52.6)
...
**Top Internal Functions/Classes:**
  * `__lasx_xvmadd_b` (Impact: 2.2)
  * `__lasx_xvmadd_h` (Impact: 2.2)
  * `__lasx_xvmadd_w` (Impact: 2.2)
  * `__lasx_xvmadd_d` (Impact: 2.2)
  * `__lasx_xvmsub_b` (Impact: 2.2)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `test_mm512_mask_i64gather_epi32` (Impact: 2.8)
  * `test_mm512_i32scatter_pd` (Impact: 2.8)
  * `test_mm512_mask_i32scatter_pd` (Impact: 2.8)
  * `test_mm512_i64scatter_pd` (Impact: 2.8)
  * `test_mm512_mask_i64scatter_pd` (Impact: 2.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `suggest_deref_or_ref` (Impact: 220.5)
    * *Intent:* /// /// ```compile_fail,E0308 /// fn some_fn(s: &str) {} /// /// let x = "hey!".to_owned(); /// some...
  * `suggest_cast` (Impact: 200.7)
  * `suggest_deref_ref_or_into` (Impact: 152.3)
  * `suggest_compatible_variants` (Impact: 102.9)
    * *Intent:* /// If the expected type is an enum (Issue #55250) with any variants whose /// sole field is of the ...
  * `suggest_missing_break_or_return_expr` (Impact: 94.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
**Top Internal Functions/Classes:**
  * `pretty_print_type` (Impact: 327.6)
  * `pretty_print_opaque_impl_type` (Impact: 195.0)
  * `pretty_print_const_valtree` (Impact: 115.0)
  * `pretty_print_const_scalar_int` (Impact: 88.7)
  * `try_print_visible_def_path_recur` (Impact: 59.0)
...
**Top Internal Functions/Classes:**
  * `__lsx_vmadd_b` (Impact: 2.2)
  * `__lsx_vmadd_h` (Impact: 2.2)
  * `__lsx_vmadd_w` (Impact: 2.2)
  * `__lsx_vmadd_d` (Impact: 2.2)
  * `__lsx_vmsub_b` (Impact: 2.2)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `report_path_resolution_error` (Impact: 212.4)
  * `show_candidates` (Impact: 198.6)
    * *Intent:* /// When an entity with a given name is not available in scope, we search for /// entities with that...
  * `lookup_import_candidates_from_module` (Impact: 160.1)
  * `into_struct_error` (Impact: 121.3)
  * `report_privacy_error` (Impact: 95.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1991.74 | **LOC:** 7617 | **CtrlFlow:** 1.7% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (4.2605%), Tech Debt (10.3812%)
**Top Internal Functions/Classes:**
  * `vec_msum_u128` (Impact: 5.0)
    * *Intent:* /// Vector Multiply Sum Logical
  * `genmasks` (Impact: 4.7)
  * `validate_block_boundary` (Impact: 4.7)
  * `genmask` (Impact: 3.9)
  * `validate_compare_range_imm` (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 26 instances
* *State Mutation (weighted view):* 80
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 104`, `structural_boundaries: 797`, `args: 717`, `func_start: 717`, `class_start: 125`
* *Risk/State:* `high_risk_execution: 2`, `state_mutation: 28`, `dead_code: 14`, `fragile_debt: 6`, `duplicate_logic: 2`
* *Architecture:* `api: 397`, `import: 13`
...
- `library/stdarch/crates/core_arch/src/s390x/vector.rs` -> **Ralf Jung** (100.0% isolated ownership) | Magnitude: 1991.74
