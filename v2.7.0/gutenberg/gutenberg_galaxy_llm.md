...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 18.3 | 4.9 | 0.0 |
...
| connectivity | 16819 | 5676 | 3 | `packages/theme/src/prebuilt/css/design-tokens.css` |
...
| dead_code | 2512 | 793 | 0 | `packages/react-native-bridge/android/react-native-bridge/src/main/java/org/wordpress/mobile/WPAndroidGlue/WPAndroidGlueCode.java` |
| credential | 56 | 25 | 0 | `packages/dataviews/src/dataviews-picker/stories/fixtures.tsx` |
| threat | 1158 | 394 | 0 | `test/integration/fixtures/documents/ms-word-in.html` |
...
- `inflate` (@ `packages/global-styles-ui/src/font-library/lib/inflate.js`) -> Impact: **483.5** | LOC: 1426
- `Image` (@ `packages/block-library/src/image/image.js`) -> Impact: **351.4** | LOC: 1032
- `gutenberg_get_layout_style` (@ `lib/block-supports/layout.php`) -> Impact: **284.0** | LOC: 363
...
- `save` (@ `packages/block-editor/src/store/test/selectors.js`) -> Impact: **283.6** | LOC: 3551
- `Navigation` (@ `packages/block-library/src/navigation/edit/index.js`) -> Impact: **227.5** | LOC: 929
- `FontLibraryProvider` (@ `packages/global-styles-ui/src/font-library/context.tsx`) -> Impact: **220.0** | LOC: 525
- `gutenberg_render_layout_support_flag` (@ `lib/block-supports/layout.php`) -> Impact: **218.9** | LOC: 429
...
- `CoverEdit` (@ `packages/block-library/src/cover/edit/index.js`) -> Impact: **204.8** | LOC: 645
- `TypographyPanel` (@ `packages/block-editor/src/components/global-styles/typography-panel.js`) -> Impact: **202.3** | LOC: 539
- `GalleryEdit` (@ `packages/block-library/src/gallery/edit.js`) -> Impact: **197.9** | LOC: 875
...
- **Heaviest Functions:** `onPageStarted` (Impact: 10.4), `onProgressChanged` (Impact: 7.6), `onCreate` (Impact: 7.5)
...
- **Heaviest Functions:** `constructor` (Impact: 21.9), `createSubTable` (Impact: 20.6), `constructor` (Impact: 18.2)
...
- **Heaviest Functions:** `getBlockAtPosition` (Impact: 30.6), `swipeToolbarToElement` (Impact: 17.5), `findBlockButton` (Impact: 14.5)
...
- **Heaviest Functions:** `gutenbergDidRequestMedia` (Impact: 42.2), `gutenbergDidRequestMediaUploadActionDialog` (Impact: 18.6), `gutenbergDidEmitLog` (Impact: 12.8)
...
- **Heaviest Functions:** `startObservingGutenberg` (Impact: 7.0), `getHTMLPostContent` (Impact: 3.8), `getBlockEditorStore` (Impact: 3.6)
...
- **Heaviest Functions:** `requestMediaPickFrom` (Impact: 19.3), `postRequest` (Impact: 14.0), `requestImageFullscreenPreview` (Impact: 13.3)

### 7. `packages/react-native-bridge/android/react-native-bridge/src/main/java/org/wordpress/mobile/WPAndroidGlue/WPAndroidGlueCode.java` (JAVA) -> Cumulative Risk: **670.5**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `getPackages` (Impact: 62.5), `requestMediaPickFromMediaLibrary` (Impact: 29.2), `attachToContainer` (Impact: 18.6)

### 8. `bin/generate-php-sync-issue.mjs` (JAVASCRIPT) -> Cumulative Risk: **665.05**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `main` (Impact: 25.6), `processCommits` (Impact: 21.3), `reduceNesting` (Impact: 14.4)

### 9. `packages/react-native-aztec/android/src/main/java/org/wordpress/mobile/ReactNativeAztec/ReactAztecManager.java` (JAVA) -> Cumulative Risk: **662.15**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `setTextAlign` (Impact: 33.1), `receiveCommand` (Impact: 21.6), `getHeadingScale` (Impact: 20.7)

### 10. `packages/react-native-aztec/android/src/main/kotlin/org/wordpress/mobile/ReactNativeAztec/EnterPressedWatcher.kt` (KOTLIN) -> Cumulative Risk: **661.19**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 93.78 | **LOC:** 101 | **CtrlFlow:** 20.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9987%), Tech Debt (98.4412%)
- **Heaviest Functions:** `shouldDeleteEnter` (Impact: 21.3), `onTextChanged` (Impact: 19.5), `beforeTextChanged` (Impact: 16.5)
...
**Top Internal Functions/Classes:**
  * `inflate` (Impact: 483.5)
  * `inflate_table` (Impact: 159.9)
  * `inflate_fast` (Impact: 110.9)
    * *Intent:* */
  * `push` (Impact: 64.8)
...
**Top Internal Functions/Classes:**
  ...
  * `buildWoffLazyLookups` (Impact: 17.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `BrotliDecompress` (Impact: 102.3)
  * `ReadHuffmanCode` (Impact: 70.3)
  * `CopyUncompressedBlockToOutput` (Impact: 44.7)
  * `ReadHuffmanCodeLengths` (Impact: 37.9)
  * `BrotliBuildHuffmanTable` (Impact: 36.3)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (100.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `writingFlowUtils` (Impact: 59.0)
  * `addDemoContent` (Impact: 3.0)
  * `getActiveBlockName` (Impact: 2.4)
  * `constructor` (Impact: 1.7)
  * `getHeight` (Impact: 1.2)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1201.78 | **LOC:** 1259 | **CtrlFlow:** 28.3% | **Authorship Centralization:** 83.3%
- **Risk Profile:** Cognitive Load (66.0539%), Tech Debt (8.4329%)
**Top Internal Functions/Classes:**
  * `gutenberg_get_layout_style` (Impact: 284.0)
    * *Intent:* /** * Generates the CSS corresponding to the provided layout. * * @param string $selector CSS select...
  * `gutenberg_render_layout_support_flag` (Impact: 218.9)
    * *Intent:* /** * Renders the layout config to the block wrapper. * * @param string $block_content Rendered bloc...
  * `gutenberg_restore_group_inner_container` (Impact: 25.3)
    * *Intent:* /** * For themes without theme.json file, make sure * to restore the inner div for the group block *...
  * `gutenberg_restore_image_outer_container` (Impact: 19.8)
    * *Intent:* /** * For themes without theme.json file, make sure * to restore the outer div for the aligned image...
  * `gutenberg_get_block_style_variation_name_from_registered_style` (Impact: 14.9)
...
* *Risk/State:* `safety_bypasses: 1`, `state_mutation: 224`, `planned_debt: 1`
* *Architecture:* `api: 1`
...
* *Risk/State:* `safety_bypasses: 1`, `state_mutation: 260`, `planned_debt: 3`, `fragile_debt: 1`
* *Architecture:* `api: 3`
...
**Top Internal Functions/Classes:**
  * `getPackages` (Impact: 62.5)
  * `requestMediaPickFromMediaLibrary` (Impact: 29.2)
  * `attachToContainer` (Impact: 18.6)
  * `getOtherMediaPickerOptions` (Impact: 18.3)
  * `editorDidEmitLog` (Impact: 18.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (100.0%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `LinkUtils` (Impact: 75.7)
  * `toggleFixedToolbar` (Impact: 3.4)
  * `createLink` (Impact: 2.5)
  * `constructor` (Impact: 1.7)
  * `getLinkPopover` (Impact: 1.2)
...
- `lib/block-supports/layout.php` -> **tellthemachines** (83.3% isolated ownership) | Magnitude: 1201.78
