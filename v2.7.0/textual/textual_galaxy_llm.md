...
| Avg Path Length | 2.6899 | Hops between files. Lower = Tighter coupling. |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 10.6 | 0.9 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 3.3 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 59.7 | 66.7 | 100.0 |
...
- `render_line` (@ `src/textual/_styles_cache.py`) -> Impact: **270.8** | LOC: 252
- `arrange` (@ `src/textual/layouts/grid.py`) -> Impact: **153.6** | LOC: 328
- `css_lines` (@ `src/textual/css/styles.py`) -> Impact: **148.2** | LOC: 249
- `_render_line` (@ `src/textual/widgets/_text_area.py`) -> Impact: **134.4** | LOC: 229
...
- `_get_box_model` (@ `src/textual/widget.py`) -> Impact: **114.7** | LOC: 144
- `scroll_to_region` (@ `src/textual/widget.py`) -> Impact: **108.5** | LOC: 108
- `parse` (@ `src/textual/_xterm_parser.py`) -> Impact: **96.2** | LOC: 191
- `resolve` (@ `src/textual/_resolve.py`) -> Impact: **95.0** | LOC: 101
- `resolve_box_models` (@ `src/textual/_resolve.py`) -> Impact: **91.4** | LOC: 130
- `_animate` (@ `src/textual/_animator.py`) -> Impact: **88.6** | LOC: 114
...
### 1. `src/textual/app.py` (PYTHON) -> Cumulative Risk: **727.98**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3202.0 | **LOC:** 4986 | **CtrlFlow:** 18.6% | **Authorship Centralization:** 96.4%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.9996%), State Flux (99.9963%), Churn (89.53%)
- **Heaviest Functions:** `__init__` (Impact: 82.0), `on_event` (Impact: 54.7), `_process_messages` (Impact: 52.6)

### 2. `src/textual/widget.py` (PYTHON) -> Cumulative Risk: **713.85**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3419.6 | **LOC:** 4955 | **CtrlFlow:** 20.2% | **Authorship Centralization:** 97.6%
- **Primary Risk Drivers:** Spec Match (100.0%), Churn (100.0%), State Flux (99.9995%), Safety Score (86.2965%)
- **Heaviest Functions:** `_get_box_model` (Impact: 114.7), `scroll_to_region` (Impact: 108.5), `_scroll_to` (Impact: 74.5)

### 3. `src/textual/screen.py` (PYTHON) -> Cumulative Risk: **682.82**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1618.38 | **LOC:** 2234 | **CtrlFlow:** 24.7% | **Authorship Centralization:** 97.1%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Churn (94.53%), Safety Score (91.3944%)
- **Heaviest Functions:** `_forward_event` (Impact: 73.6), `_refresh_layout` (Impact: 46.0), `_move_focus` (Impact: 39.4)

### 4. `src/textual/_sleep.py` (PYTHON) -> Cumulative Risk: **661.88**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 87.72 | **LOC:** 61 | **CtrlFlow:** 8.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `run` (Impact: 4.7), `check_sleeps` (Impact: 4.0), `profile_sleep` (Impact: 3.2)

### 5. `src/textual/_doc.py` (PYTHON) -> Cumulative Risk: **657.8**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 193.44 | **LOC:** 192 | **CtrlFlow:** 12.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Safety Score (92.4326%)
- **Heaviest Functions:** `take_svg_screenshot` (Impact: 45.8), `format_svg` (Impact: 13.4), `auto_pilot` (Impact: 8.0)

### 6. `src/textual/demo/game.py` (PYTHON) -> Cumulative Risk: **649.69**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 349.52 | **LOC:** 590 | **CtrlFlow:** 12.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (95.6506%), Concurrency (95.5353%)
- **Heaviest Functions:** `action_move` (Impact: 11.3), `__init__` (Impact: 8.0), `on_tile_clicked` (Impact: 7.3)

### 7. `src/textual/widgets/_list_view.py` (PYTHON) -> Cumulative Risk: **643.93**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 295.8 | **LOC:** 397 | **CtrlFlow:** 27.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Concurrency (99.9023%), Safety Score (90.864%)
- **Heaviest Functions:** `remove_items` (Impact: 20.8), `watch_index` (Impact: 19.4), `pop` (Impact: 19.0)

### 8. `src/textual/scrollbar.py` (PYTHON) -> Cumulative Risk: **643.1**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 385.92 | **LOC:** 412 | **CtrlFlow:** 18.6% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (95.8393%), Verification (80.0%)
- **Heaviest Functions:** `render_bar` (Impact: 67.9), `__rich_console__` (Impact: 23.4), `_on_mouse_move` (Impact: 9.7)

### 9. `src/textual/dom.py` (PYTHON) -> Cumulative Risk: **637.24**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1221.96 | **LOC:** 1917 | **CtrlFlow:** 20.0% | **Authorship Centralization:** 92.9%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (90.8392%), Api Exposure (81.9875%)
- **Heaviest Functions:** `query_one` (Impact: 45.6), `query_exactly_one` (Impact: 39.3), `query_ancestor` (Impact: 21.9)

### 10. `src/textual/events.py` (PYTHON) -> Cumulative Risk: **636.94**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 333.16 | **LOC:** 990 | **CtrlFlow:** 5.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.394%), Tech Debt (96.0531%), Api Exposure (89.5961%)
- **Heaviest Functions:** `__init__` (Impact: 24.4), `__init__` (Impact: 10.4), `__init__` (Impact: 8.1)
...
- **Magnitude:** 3419.6 | **LOC:** 4955 | **CtrlFlow:** 20.2% | **Authorship Centralization:** 97.6%
- **Risk Profile:** Cognitive Load (48.3907%), Tech Debt (9.7396%)
**Top Internal Functions/Classes:**
  * `_get_box_model` (Impact: 114.7)
  * `scroll_to_region` (Impact: 108.5)
  * `_scroll_to` (Impact: 74.5)
  * `mount` (Impact: 55.6)
  * `scroll_end` (Impact: 44.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.026412 | `Ripple Effect (Closeness):` 0.371913
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.078527 | `Ripple Effect (Closeness):` 0.500353
...
- **Risk Profile:** Cognitive Load (15.6579%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_click_selection_disabled_when_allow_select_is_false` (Impact: 12.7)
  * `test_datatable_auto_height_future_updates` (Impact: 7.7)
    * *Intent:* """https://github.com/Textualize/textual/issues/4928 meant that when height=None, in add_row, future...
  * `test_add_separator` (Impact: 7.4)
    * *Intent:* """Regression test for https://github.com/Textualize/textual/issues/5431 You should see a button on ...
  * `compose` (Impact: 7.4)
  * `test_panel_border_title_colors` (Impact: 6.5)
...
- **Risk Profile:** Cognitive Load (37.4849%), Tech Debt (84.246%)
**Top Internal Functions/Classes:**
  * `_render_cell` (Impact: 53.1)
  * `_get_styles_to_render_cell` (Impact: 52.6)
  * `_render_line_in_row` (Impact: 52.3)
  * `_update_dimensions` (Impact: 38.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000187 | `Ripple Effect (Closeness):` 0.004438
...
- **Risk Profile:** Cognitive Load (39.3715%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_sort_by_function_sum` (Impact: 6.8)
    * *Intent:* """Test sorting a `DataTable` using a custom sort function."""
  * `test_datatable_message_emission` (Impact: 6.2)
  * `test_move_cursor_respects_animate_parameter` (Impact: 3.8)
    * *Intent:* """Regression test for https://github.com/Textualize/textual/issues/3840 Make sure that the call to ...
  * `record_data_table_event` (Impact: 3.7)
  * `on_mount` (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001786 | `Ripple Effect (Closeness):` 0.012681
...
- **Risk Profile:** Cognitive Load (39.409%), Tech Debt (8.8721%)
**Top Internal Functions/Classes:**
  * `_wrap_and_format` (Impact: 63.5)
  * `render` (Impact: 44.7)
  * `split` (Impact: 38.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.005526 | `Ripple Effect (Closeness):` 0.348706
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1618.38 | **LOC:** 2234 | **CtrlFlow:** 24.7% | **Authorship Centralization:** 97.1%
- **Risk Profile:** Cognitive Load (43.2244%), Tech Debt (8.1283%)
**Top Internal Functions/Classes:**
  * `_forward_event` (Impact: 73.6)
  * `_refresh_layout` (Impact: 46.0)
    * *Intent:* """Refresh the layout (can change size and positions of widgets)."""
  * `_move_focus` (Impact: 39.4)
  * `_check_auto_scroll` (Impact: 32.1)
  * `_handle_mouse_move` (Impact: 28.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.012765 | `Ripple Effect (Closeness):` 0.350125
...
- **Risk Profile:** Cognitive Load (63.3482%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `process_hatch` (Impact: 39.7)
  * `process_transition` (Impact: 31.1)
  * `process_color` (Impact: 28.1)
    * *Intent:* """Processes a simple color declaration."""
  * `_parse_border` (Impact: 21.9)
  * `process_keyline` (Impact: 19.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003068 | `Ripple Effect (Closeness):` 0.206381
...
- **Risk Profile:** Cognitive Load (49.9921%), Tech Debt (17.4455%)
**Top Internal Functions/Classes:**
  * `_parse_markdown` (Impact: 58.1)
    * *Intent:* """Create a stream of MarkdownBlock widgets from markdown. Args: tokens: List of tokens. Yields: Wid...
  * `_token_to_content` (Impact: 31.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00442 | `Ripple Effect (Closeness):` 0.008322
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1221.96 | **LOC:** 1917 | **CtrlFlow:** 20.0% | **Authorship Centralization:** 92.9%
- **Risk Profile:** Cognitive Load (48.6265%), Tech Debt (9.9146%)
**Top Internal Functions/Classes:**
  * `query_one` (Impact: 45.6)
  * `query_exactly_one` (Impact: 39.3)
  * `query_ancestor` (Impact: 21.9)
  * `data_bind` (Impact: 20.4)
  * `rich_style` (Impact: 18.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.01621 | `Ripple Effect (Closeness):` 0.35041
...
- **Risk Profile:** Cognitive Load (48.1471%), Tech Debt (43.85%)
**Top Internal Functions/Classes:**
  * `_render_line` (Impact: 79.8)
  * `add` (Impact: 51.9)
  * `add_json` (Impact: 20.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001782 | `Ripple Effect (Closeness):` 0.189896
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 897.08 | **LOC:** 769 | **CtrlFlow:** 2.2% | **Authorship Centralization:** 75.0%
- **Risk Profile:** Cognitive Load (39.9818%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_compose_order` (Impact: 7.1)
  * `test_sort_children_no_key` (Impact: 5.8)
    * *Intent:* """Test sorting with no key."""
  * `test_sort_children` (Impact: 5.7)
    * *Intent:* """Test the sort_children method."""
  * `test_of_type` (Impact: 4.2)
  * `compose` (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (27.7365%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_public_and_private_validate_order` (Impact: 4.0)
    * *Intent:* """The private validate should be called first."""
  * `validate_value` (Impact: 3.7)
  * `_validate_value` (Impact: 3.7)
  * `test_watch_compute` (Impact: 3.5)
    * *Intent:* """Check that watching a computed attribute works."""
  * `test_reactive_inheritance` (Impact: 3.1)
...
- **Risk Profile:** Cognitive Load (43.7391%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_tabbed_content_switch_via_ui` (Impact: 3.9)
    * *Intent:* """Check tab navigation via the user interface."""
  * `test_tabbed_content_add_before_pane` (Impact: 3.2)
  * `test_tabbed_content_add_after_pane` (Impact: 3.2)
  * `test_tabbed_content_switch_via_code` (Impact: 3.0)
    * *Intent:* """Check tab navigation via code."""
  * `test_tabbed_content_add_before_id` (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001242 | `Ripple Effect (Closeness):` 0.175634
...
- **Risk Profile:** Cognitive Load (40.8714%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `align` (Impact: 66.9)
  * `crop` (Impact: 36.9)
  ...
  * `adjust_cell_length` (Impact: 20.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000743 | `Ripple Effect (Closeness):` 0.271485
...
- **Risk Profile:** Cognitive Load (45.9316%), Tech Debt (82.5757%)
**Top Internal Functions/Classes:**
  * `__set__` (Impact: 32.9)
  * `__set__` (Impact: 30.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002569 | `Ripple Effect (Closeness):` 0.208987
...
- **Risk Profile:** Cognitive Load (41.2843%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `css_lines` (Impact: 148.2)
  * `__textual_animation__` (Impact: 25.5)
  * `_get_border_css_lines` (Impact: 22.6)
  * `refresh` (Impact: 18.0)
  * `extract_rules` (Impact: 15.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.008872 | `Ripple Effect (Closeness):` 0.286297
...
- **Risk Profile:** Cognitive Load (49.9876%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_get_dispatch_methods` (Impact: 46.9)
  * `__new__` (Impact: 28.8)
  * `_on_message` (Impact: 18.8)
    * *Intent:* """Called to process a message. Args: message: A Message object. """
  * `_process_messages_loop` (Impact: 18.6)
    * *Intent:* """Process messages until the queue is closed."""
  * `_close_messages` (Impact: 16.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004645 | `Ripple Effect (Closeness):` 0.278694
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004647 | `Ripple Effect (Closeness):` 0.251493
...
**Top Internal Functions/Classes:**
  * `test_cut` (Impact: 2.8)
  * `test_tab_with_spaces_goes_to_tab_stop` (Impact: 2.6)
  * `test_deletion_with_non_empty_selection` (Impact: 2.6)
    * *Intent:* """When there's a selection, pressing backspace or delete should delete everything that is selected ...
  * `test_delete_word_left` (Impact: 2.5)
  * `test_delete_word_left_with_tabs` (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (46.2631%), Tech Debt (29.938%)
**Top Internal Functions/Classes:**
  * `_search_for` (Impact: 28.5)
  * `_gather_commands` (Impact: 26.7)
    * *Intent:* """Gather up all of the commands that match the search value. Args: search_value: The value to searc...
  * `_select_or_command` (Impact: 15.5)
  * `_provider_classes` (Impact: 14.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.007801 | `Ripple Effect (Closeness):` 0.310464
...
- **Risk Profile:** Cognitive Load (39.1268%), Tech Debt (24.5297%)
**Top Internal Functions/Classes:**
  * `intersection` (Impact: 30.6)
    * *Intent:* """Get the overlapping portion of the two regions. Args: region: A region that overlaps this region....
  * `constrain` (Impact: 28.3)
  * `get_scroll_to_visible` (Impact: 20.5)
  * `selection_bounds` (Impact: 19.6)
...
- **Risk Profile:** Cognitive Load (35.6055%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_screens` (Impact: 5.5)
  * `test_worker_cancellation` (Impact: 3.9)
    * *Intent:* """Regression test for https://github.com/Textualize/textual/issues/4884 The MRE below was pushing a...
  * `test_auto_focus_on_screen_if_app_auto_focus_is_none` (Impact: 3.1)
    * *Intent:* """Setting app.AUTO_FOCUS = `None` means it is not taken into consideration."""
  * `test_auto_focus_on_screen_if_app_auto_focus_is_disabled` (Impact: 3.1)
    * *Intent:* """Setting app.AUTO_FOCUS = `None` means it is not taken into consideration."""
  * `test_mouse_move_event_bubbles_to_screen_from_widget` (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
...
- `src/textual/app.py` -> **Severity: 7.852** (Bridge: 0.0785 * Flux: 99.9963%)
- `src/textual/widget.py` -> **Severity: 2.641** (Bridge: 0.0264 * Flux: 99.9995%)
- `src/textual/dom.py` -> **Severity: 1.621** (Bridge: 0.0162 * Flux: 100.0%)
- `src/textual/screen.py` -> **Severity: 1.276** (Bridge: 0.0128 * Flux: 100.0%)
- `src/textual/demo/widgets.py` -> **Severity: 1.017** (Bridge: 0.0102 * Flux: 99.9875%)
