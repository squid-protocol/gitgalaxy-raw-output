...
- `testValidate` (@ `Tests/VaporTests/ValidationTests.swift`) -> Impact: **133.5** | LOC: 910
- `routes` (@ `Sources/Development/routes.swift`) -> Impact: **86.1** | LOC: 307
- `init` (@ `Sources/Vapor/Utilities/URI.swift`) -> Impact: **67.9** | LOC: 56
...
- `getData` (@ `Sources/Vapor/URLEncodedForm/URLEncodedFormEncoder.swift`) -> Impact: **62.5** | LOC: 269
- `download` (@ `Sources/Vapor/HTTP/EndpointCache.swift`) -> Impact: **55.2** | LOC: 105
- `channelRead` (@ `Sources/Vapor/HTTP/Server/HTTPServerRequestDecoder.swift`) -> Impact: **53.3** | LOC: 96
- `vapor_bcrypt_hashpass` (@ `Sources/CVaporBcrypt/bcrypt.c`) -> Impact: **52.5** | LOC: 110
  * *Intent:* /* * the core bcrypt function */
- `write` (@ `Sources/Vapor/HTTP/Server/HTTPServerResponseEncoder.swift`) -> Impact: **49.4** | LOC: 52
...
- `report` (@ `Sources/Vapor/Logging/Logger+Report.swift`) -> Impact: **46.0** | LOC: 38
  * *Intent:* /// Reports an `Error` to this `Logger`. /// /// - parameters: /// - error: `Error` to log.
- `init` (@ `Sources/Vapor/HTTP/Server/HTTPServer.swift`) -> Impact: **45.9** | LOC: 38
...
- **Heaviest Functions:** `encode` (Impact: 9.1), `encode` (Impact: 5.1), `encode` (Impact: 2.2)
...
- **Heaviest Functions:** `performTest` (Impact: 16.0), `performTest` (Impact: 14.9), `performTest` (Impact: 7.0)
...
- **Heaviest Functions:** `readSession` (Impact: 5.5), `updateSession` (Impact: 4.4), `createSession` (Impact: 4.0)
...
- **Heaviest Functions:** `write0` (Impact: 21.7), `consume` (Impact: 16.7), `write` (Impact: 7.4)
...
- **Heaviest Functions:** `getData` (Impact: 62.5), `encode` (Impact: 35.9), `encodeDate` (Impact: 16.8)
...
- **Heaviest Functions:** `configure` (Impact: 6.4), `set` (Impact: 3.5), `get` (Impact: 2.9)
...
- **Heaviest Functions:** `losslessDecode` (Impact: 4.5), `decode` (Impact: 4.5), `decode` (Impact: 4.3)
...
- **Heaviest Functions:** `subscript` (Impact: 3.0), `init` (Impact: 1.6), `init` (Impact: 1.6)
...
- **Heaviest Functions:** `collect` (Impact: 10.5), `drain` (Impact: 7.8), `init` (Impact: 1.6)
...
- **Heaviest Functions:** `decode` (Impact: 23.5), `encode` (Impact: 7.7), `encode` (Impact: 2.2)
...
**Top Internal Functions/Classes:**
  * `testHTTP2RequestDecompression` (Impact: 19.9)
  * `testDeprecatedServerStartMethods` (Impact: 18.4)
  * `testHTTP1RequestDecompression` (Impact: 17.6)
  * `testRequestBodyBackpressureWorks` (Impact: 14.3)
  * `testRequestBodyStreamGetsFinalisedEvenIfClientDisappears` (Impact: 13.7)
...
**Top Internal Functions/Classes:**
  * `assertResponseCompression` (Impact: 12.8)
  * `assertCompressed` (Impact: 7.0)
  * `assertUncompressed` (Impact: 7.0)
  * `testUnknownType` (Impact: 3.5)
  * `testImage` (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `testEchoHandlers` (Impact: 22.4)
  * `testAsyncEchoHandlers` (Impact: 10.1)
  * `testStreamingOffEventLoop` (Impact: 8.1)
  * `testEOFFraming` (Impact: 7.3)
  * `testCorrectResponseOrderOverVaporTCP` (Impact: 7.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_testRequestBodyBackpressureWorksWithAsyncStreaming` (Impact: 15.7)
    * *Intent:* // TODO: Re-enable once it reliably works and doesn't cause issues with trying to shut the applicati...
  * `testStreamingRequestBodyCleansUp` (Impact: 8.7)
  * `testStreamingRequest` (Impact: 4.8)
  * `testRequestIdForwarding` (Impact: 4.8)
  * `testLargeBodyCollectionDoesntCrash` (Impact: 4.7)
...
**Top Internal Functions/Classes:**
  * `withRemoteApp` (Impact: 9.5)
    * *Intent:* // MARK: - Helpers
  * `testBoilerplateClient` (Impact: 7.5)
  * `log` (Impact: 6.2)
  * `clientConfigurationChange` (Impact: 3.0)
  * `clientConfigurationCantBeChangedAfterClientHasBeenUsed` (Impact: 3.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 25 instances
...
**Top Internal Functions/Classes:**
  * `testStreamFileContentHeaderTail` (Impact: 10.3)
  * `testStreamFileContentHeaderStart` (Impact: 10.3)
  * `testStreamFileContentHeadersWithin` (Impact: 10.1)
  * `testStreamFileNull` (Impact: 4.0)
  * `testStreamFileContentHeadersOnlyFirstByte` (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `testContentDispositionQuotedFilename` (Impact: 15.9)
    * *Intent:* // https://github.com/vapor/vapor/issues/2439
  * `testCookie_parsing` (Impact: 11.8)
  * `testAcceptType` (Impact: 10.6)
  * `testCookie_invalidCookie` (Impact: 9.6)
  * `testLinkHeaderSerialization` (Impact: 9.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `vapor_bcrypt_hashpass` (Impact: 52.5)
    * *Intent:* /* * the core bcrypt function */
  * `decode_base64` (Impact: 17.9)
    * *Intent:* #define CHAR64(c) ( (c) > 127 ? 255 : index_64[(c)]) /* * read buflen (after decoding) bytes of data...
  * `vapor_encode_base64` (Impact: 9.6)
...
**Top Internal Functions/Classes:**
  * `testValidate` (Impact: 133.5)
  * `validations` (Impact: 29.2)
  * `init` (Impact: 6.1)
  * `init` (Impact: 1.9)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 7 instances
...
**Top Internal Functions/Classes:**
  ...
  * `asyncShutdown` (Impact: 6.5)
  * `shutdown` (Impact: 5.4)
  * `asyncBoot` (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  * `nextDirective` (Impact: 12.7)
  * `nextDirectiveValue` (Impact: 12.5)
  * `firstUnescapedDoubleQuote` (Impact: 10.9)
  * `firstIndex` (Impact: 9.0)
    * *Intent:* /// Returns the first index matching any of the passed in Characters, nil if no match
  * `getSeparatorCharacters` (Impact: 7.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `testSessionAuthentication` (Impact: 11.1)
  * `testBasicAuthenticatorWithRedirect` (Impact: 8.0)
  * `authenticate` (Impact: 7.6)
  * `authenticate` (Impact: 7.3)
  * `authenticate` (Impact: 7.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
* *Amplified Cascading Flux:* 1 instances
...
**Top Internal Functions/Classes:**
  ...
  * `load` (Impact: 17.5)
    * *Intent:* /// Reads the dotenv files relevant to the environment and loads them into the process. /// /// let ...
  * `parseLineValue` (Impact: 11.2)
  * `countDistance` (Impact: 10.8)
  * `parseNext` (Impact: 8.1)
**Contextual Mitigations & Amplifications:**
