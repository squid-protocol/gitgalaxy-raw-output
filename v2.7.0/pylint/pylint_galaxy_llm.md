...
| Avg Path Length | 3.5252 | Hops between files. Lower = Tighter coupling. |
...
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 75.3 | 1.4 | 0.0 | 0.0 |
...
- `_is_variable_violation` (@ `pylint/checkers/variables.py`) -> Impact: **172.7** | LOC: 154
- `_check_consumer` (@ `pylint/checkers/variables.py`) -> Impact: **142.7** | LOC: 209
  * *Intent:* # pylint: disable = too-many-return-statements, too-many-branches
- `_check_is_unused` (@ `pylint/checkers/variables.py`) -> Impact: **118.1** | LOC: 99
  * *Intent:* # pylint: disable = too-many-branches
- `visit_call` (@ `pylint/checkers/typecheck.py`) -> Impact: **116.6** | LOC: 219
...
- `_get_out_of_order_string` (@ `pylint/checkers/imports.py`) -> Impact: **115.8** | LOC: 125
- `_emit_no_member` (@ `pylint/checkers/typecheck.py`) -> Impact: **100.4** | LOC: 103
- `visit_assignname` (@ `pylint/checkers/base/name_checker/checker.py`) -> Impact: **95.9** | LOC: 151
- `_make_linter_options` (@ `pylint/lint/base_options.py`) -> Impact: **94.0** | LOC: 380
  * *Intent:* """Return the options used in a PyLinter class."""
- `_loopvar_name` (@ `pylint/checkers/variables.py`) -> Impact: **85.3** | LOC: 147
...
- `visit_binop` (@ `pylint/checkers/strings.py`) -> Impact: **83.3** | LOC: 142
...
- **Heaviest Functions:** `visit_with` (Impact: 12.6), `register` (Impact: 1.5)

### 2. `pylint/checkers/async_checker.py` (PYTHON) -> Cumulative Risk: **660.64**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `visit_asyncwith` (Impact: 29.6), `visit_asyncfunctiondef` (Impact: 9.0), `open` (Impact: 1.6)

### 3. `pylint/checkers/stdlib.py` (PYTHON) -> Cumulative Risk: **643.79**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 600.34 | **LOC:** 1005 | **CtrlFlow:** 16.9% | **Authorship Centralization:** 28.6%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9245%), Documentation (89.4737%), Concurrency (80.5547%)
- **Heaviest Functions:** `_check_open_call` (Impact: 57.4), `_check_env_function` (Impact: 29.9), `visit_call` (Impact: 29.3)

### 4. `pylint/extensions/code_style.py` (PYTHON) -> Cumulative Risk: **624.96**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 239.98 | **LOC:** 360 | **CtrlFlow:** 32.3% | **Authorship Centralization:** 25.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9691%), Documentation (90.0%), Verification (80.0%)
- **Heaviest Functions:** `_check_dict_consider_namedtuple_dataclass` (Impact: 54.0), `visit_call` (Impact: 32.9), `_check_consider_using_assignment_expr` (Impact: 20.2)

### 5. `pylint/checkers/typecheck.py` (PYTHON) -> Cumulative Risk: **622.13**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1724.94 | **LOC:** 2356 | **CtrlFlow:** 29.0% | **Authorship Centralization:** 77.8%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9963%), Documentation (83.6207%), Verification (80.0%)
- **Heaviest Functions:** `visit_call` (Impact: 116.6), `_emit_no_member` (Impact: 100.4), `visit_attribute` (Impact: 60.3)

### 6. `pylint/lint/run.py` (PYTHON) -> Cumulative Risk: **621.08**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__init__` (Impact: 73.1), `_query_cpu_cgroupsv1` (Impact: 7.2), `_cpu_count` (Impact: 6.2)

### 7. `pylint/checkers/variables.py` (PYTHON) -> Cumulative Risk: **604.54**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `_is_variable_violation` (Impact: 172.7), `_check_consumer` (Impact: 142.7), `_check_is_unused` (Impact: 118.1)

### 8. `pylint/checkers/utils.py` (PYTHON) -> Cumulative Risk: **602.77**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1740.56 | **LOC:** 2352 | **CtrlFlow:** 32.4% | **Authorship Centralization:** 41.2%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.993%), Api Exposure (88.7388%), Verification (80.0%)
- **Heaviest Functions:** `defnode_in_scope` (Impact: 54.3), `safe_infer` (Impact: 41.2), `parse_format_string` (Impact: 32.0)

### 9. `pylint/checkers/base/name_checker/checker.py` (PYTHON) -> Cumulative Risk: **600.45**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `visit_assignname` (Impact: 95.9), `_check_typevar` (Impact: 66.5), `_check_name` (Impact: 41.9)

### 10. `pylint/checkers/refactoring/refactoring_checker.py` (PYTHON) -> Cumulative Risk: **597.0**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1877.28 | **LOC:** 2455 | **CtrlFlow:** 34.0% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9961%), Safety Score (82.9051%), Verification (80.0%)
- **Heaviest Functions:** `_check_unnecessary_comprehension` (Impact: 67.9), `_check_unnecessary_dict_index_lookup` (Impact: 67.8), `_check_unnecessary_list_index_lookup` (Impact: 62.2)
...
**Top Internal Functions/Classes:**
  * `_is_variable_violation` (Impact: 172.7)
  * `_check_consumer` (Impact: 142.7)
    * *Intent:* # pylint: disable = too-many-return-statements, too-many-branches
  * `_check_is_unused` (Impact: 118.1)
    * *Intent:* # pylint: disable = too-many-branches
  * `_loopvar_name` (Impact: 85.3)
    * *Intent:* # pylint: disable-next=too-many-branches,too-many-statements # filter variables according to node's ...
  * `_check_imports` (Impact: 66.8)
    * *Intent:* # pylint: disable = too-many-branches
...
**Top Internal Functions/Classes:**
  * `_check_unnecessary_comprehension` (Impact: 67.9)
  * `_check_unnecessary_dict_index_lookup` (Impact: 67.8)
  * `_check_unnecessary_list_index_lookup` (Impact: 62.2)
  * `_check_simplifiable_if` (Impact: 41.7)
    * *Intent:* """Check if the given if node can be simplified. The if statement can be reduced to a boolean expres...
  * `_check_chained_comparison` (Impact: 40.9)
...
**Top Internal Functions/Classes:**
  * `defnode_in_scope` (Impact: 54.3)
    * *Intent:* # pylint: disable = too-many-branches
  * `safe_infer` (Impact: 41.2)
  * `parse_format_string` (Impact: 32.0)
  * `is_defined_before` (Impact: 29.7)
    * *Intent:* """Check if the given variable node is defined before. Verify that the variable node is defined by a...
  * `unimplemented_abstract_methods` (Impact: 25.0)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000168 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_get_out_of_order_string` (Impact: 115.8)
  * `_get_first_import` (Impact: 74.1)
  * `_add_imported_module` (Impact: 23.9)
    * *Intent:* """Notify an imported module, used to analyze dependencies."""
  * `_check_reimport` (Impact: 23.8)
...
**Top Internal Functions/Classes:**
  * `visit_binop` (Impact: 83.3)
  * `_check_new_format` (Impact: 66.2)
    * *Intent:* """Check the new string formatting."""
  * `_check_new_format_specifiers` (Impact: 54.2)
    * *Intent:* # pylint: disable = too-many-statements
  * `process_non_raw_string_token` (Impact: 34.9)
  * `check_for_consistent_string_delimiters` (Impact: 28.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_add_one_message` (Impact: 74.1)
  * `_discover_files` (Impact: 24.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000481 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_runtest` (Impact: 15.7)
  * `test_fail_under` (Impact: 10.6)
  * `test_fail_on` (Impact: 8.0)
  * `test_help_msg` (Impact: 7.6)
  * `test_modify_sys_path` (Impact: 5.5)
...
**Top Internal Functions/Classes:**
  ...
  * `_check_reversed` (Impact: 37.5)
    * *Intent:* """Check that the argument to `reversed` is a sequence."""
  * `_check_using_constant_test` (Impact: 33.9)
...
**Top Internal Functions/Classes:**
  * `visit_assignname` (Impact: 95.9)
  * `_check_typevar` (Impact: 66.5)
    * *Intent:* """Check for TypeVar lint violations."""
  * `_check_name` (Impact: 41.9)
  * `_determine_function_name_type` (Impact: 24.3)
  * `_is_multi_naming_match` (Impact: 18.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_check_open_call` (Impact: 57.4)
  * `_check_env_function` (Impact: 29.9)
  * `visit_call` (Impact: 29.3)
    * *Intent:* """Visit a Call node."""
  * `_check_invalid_envvar_value` (Impact: 27.7)
  * `_check_lru_cache_decorators` (Impact: 22.8)
...
**Top Internal Functions/Classes:**
  * `stripped_lines` (Impact: 70.7)
  * `filter_noncode_lines` (Impact: 21.2)
  * `append_stream` (Impact: 15.0)
  * `_get_similarity_report` (Impact: 15.0)
  * `_find_common` (Impact: 13.7)
...
**Top Internal Functions/Classes:**
  * `project_from_files` (Impact: 31.1)
  * `handle` (Impact: 22.7)
  * `handle` (Impact: 22.7)
  * `visit_classdef` (Impact: 18.5)
    * *Intent:* """Visit an nodes.Class node and optionally assign a unique id."""
  * `visit_assignname` (Impact: 13.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5.7e-05 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_check_redefinition` (Impact: 49.5)
  * `visit_functiondef` (Impact: 29.4)
  * `_check_in_loop` (Impact: 23.2)
  * `_check_name_used_prior_global` (Impact: 18.6)
  * `_check_inferred_class_is_abstract` (Impact: 17.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_error_message_17` (Impact: 27.8)
  * `test_multi_break_condition_4` (Impact: 12.2)
  * `test_multi_break_condition_3` (Impact: 10.6)
  * `test_multi_break_condition_1` (Impact: 9.2)
  * `test_error_message_16` (Impact: 7.6)
...
**Top Internal Functions/Classes:**
  * `_check_catching_non_exception` (Impact: 47.7)
  * `visit_try` (Impact: 36.7)
    * *Intent:* """Check for empty except."""
  * `_check_try_except_raise` (Impact: 34.0)
...
**Top Internal Functions/Classes:**
  * `check_arguments_in_docstring` (Impact: 51.0)
  * `visit_raise` (Impact: 42.7)
  * `visit_return` (Impact: 25.6)
  * `visit_yield` (Impact: 22.4)
  * `check_functiondef_returns` (Impact: 16.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `get_methods` (Impact: 10.9)
    * *Intent:* """Return visible methods."""
  * `get_module` (Impact: 10.8)
...
**Top Internal Functions/Classes:**
  * `_check_spelling` (Impact: 55.4)
    * *Intent:* # pylint: disable = too-many-statements
  * `process_tokens` (Impact: 14.8)
  * `open` (Impact: 13.5)
  * `_get_enchant_dict_help` (Impact: 12.8)
  * `next` (Impact: 12.4)
**Contextual Mitigations & Amplifications:**
...
- `pylint/checkers/variables.py` -> Churn: **75.32%** | Cog Load: 62.0653% | Debt: 37.2071%
- `pylint/checkers/base/name_checker/checker.py` -> Churn: **64.7%** | Cog Load: 64.6035% | Debt: 8.7006%
- `pylint/testutils/_primer/primer_compare_command.py` -> Churn: **57.52%** | Cog Load: 60.2112% | Debt: 0.0%
- `pylint/checkers/imports.py` -> Churn: **56.63%** | Cog Load: 55.4937% | Debt: 8.2642%
- `pylint/extensions/code_style.py` -> Churn: **52.92%** | Cog Load: 51.7003% | Debt: 65.9227%
...
- `pylint/lint/pylinter.py` -> **Severity: 0.048** (Bridge: 0.0005 * Flux: 99.9968%)
- `pylint/typing.py` -> **Severity: 0.042** (Bridge: 0.0007 * Flux: 59.6451%)
- `pylint/lint/run.py` -> **Severity: 0.039** (Bridge: 0.0004 * Flux: 100.0%)
- `pylint/message/message.py` -> **Severity: 0.029** (Bridge: 0.0003 * Flux: 100.0%)
- `pylint/checkers/utils.py` -> **Severity: 0.017** (Bridge: 0.0002 * Flux: 99.993%)
