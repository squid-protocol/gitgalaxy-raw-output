...
| Avg Path Length | 2.6779 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 92.7 | 20.0 | 7.9 | 0.0 |
...
- `format_meter` (@ `tqdm-4.67.3/tqdm/std.py`) -> Impact: **289.9** | LOC: 197
- `__init__` (@ `tqdm-4.67.3/tqdm/std.py`) -> Impact: **228.3** | LOC: 150
- `main` (@ `tqdm-4.67.3/tqdm/cli.py`) -> Impact: **82.9** | LOC: 169
  * *Intent:* """ Parameters (internal use only) --------- fp : file-like object for tqdm argv : list (default: sys.argv[1:]) """
- `simple_progress` (@ `tqdm-4.67.3/tests/tests_perf.py`) -> Impact: **56.8** | LOC: 56
- `display` (@ `tqdm-4.67.3/tqdm/notebook.py`) -> Impact: **44.9** | LOC: 52
...
- `pandas` (@ `tqdm-4.67.3/tqdm/std.py`) -> Impact: **42.0** | LOC: 182
...
- `posix_pipe` (@ `tqdm-4.67.3/tqdm/cli.py`) -> Impact: **39.8** | LOC: 55
- `pos_line_diff` (@ `tqdm-4.67.3/tests/tests_tqdm.py`) -> Impact: **39.3** | LOC: 26
  * *Intent:* """ Return differences between two bar output lists. To be used with `RE_pos` """
- `inner` (@ `tqdm-4.67.3/tqdm/std.py`) -> Impact: **34.8** | LOC: 70
...
- `update` (@ `tqdm-4.67.3/tqdm/std.py`) -> Impact: **34.5** | LOC: 66
...
- **Heaviest Functions:** `main` (Impact: 82.9), `posix_pipe` (Impact: 39.8), `cast` (Impact: 29.5)
...
- **Heaviest Functions:** `display` (Impact: 29.0), `__init__` (Impact: 21.0), `close` (Impact: 8.0)
...
- **Heaviest Functions:** `__init__` (Impact: 11.8), `gather` (Impact: 11.1), `as_completed` (Impact: 8.5)
...
- **Heaviest Functions:** `format_meter` (Impact: 289.9), `__init__` (Impact: 228.3), `pandas` (Impact: 42.0)
...
- **Heaviest Functions:** `display` (Impact: 44.9), `__init__` (Impact: 32.2), `status_printer` (Impact: 20.0)
...
- **Heaviest Functions:** `on_epoch_begin` (Impact: 23.5), `__init__` (Impact: 18.7), `bar2callback` (Impact: 8.6)
...
- **Heaviest Functions:** `submit` (Impact: 12.0), `__init__` (Impact: 1.6)
...
- **Heaviest Functions:** `run` (Impact: 13.3), `get_instances` (Impact: 4.5), `exit` (Impact: 3.1)
...
- **Heaviest Functions:** `envwrap` (Impact: 31.3), `wrap` (Impact: 13.9), `disp_trim` (Impact: 12.8)
...
- **Heaviest Functions:** `render` (Impact: 11.2), `__init__` (Impact: 10.0), `render` (Impact: 9.5)
...
**Top Internal Functions/Classes:**
  * `format_meter` (Impact: 289.9)
  * `__init__` (Impact: 228.3)
  * `pandas` (Impact: 42.0)
...
**Top Internal Functions/Classes:**
  * `pos_line_diff` (Impact: 39.3)
    * *Intent:* """ Return differences between two bar output lists. To be used with `RE_pos` """
  * `test_max_interval` (Impact: 19.4)
    * *Intent:* """Test maxinterval"""
  * `test_smoothing` (Impact: 15.7)
    * *Intent:* """Test exponential weighted average smoothing"""
  * `test_position` (Impact: 15.7)
    * *Intent:* """Test positioned progress bars"""
  * `squash_ctrlchars` (Impact: 14.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.011194 | `Ripple Effect (Closeness):` 0.205882
...
**Top Internal Functions/Classes:**
  * `simple_progress` (Impact: 56.8)
  * `update_and_print` (Impact: 19.9)
  * `retry_on_except` (Impact: 9.6)
    * *Intent:* """decroator for retrying `n` times before raising Exceptions"""
  * `test_inner` (Impact: 9.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_screen_shape_windows` (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_as_completed` (Impact: 9.4)
    * *Intent:* """Test asyncio as_completed"""
  * `test_generators` (Impact: 8.0)
    * *Intent:* """Test asyncio generators"""
  * `count` (Impact: 7.3)
  * `test_coroutines` (Impact: 4.5)
...
**Top Internal Functions/Classes:**
  ...
  * `as_completed` (Impact: 8.5)
    * *Intent:* """ Wrapper for `asyncio.as_completed`. """
  * `__anext__` (Impact: 4.9)
  * `send` (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000878 | `Ripple Effect (Closeness):` 0.161765
...
**Top Internal Functions/Classes:**
  * `test_main` (Impact: 11.6)
    * *Intent:* """Test misc CLI options"""
  * `test_exceptions` (Impact: 11.4)
    * *Intent:* """Test CLI Exceptions"""
  * `test_main_log` (Impact: 7.9)
    * *Intent:* """Test CLI --log"""
  * `norm` (Impact: 4.4)
    * *Intent:* """Normalise line endings."""
  * `test_comppath` (Impact: 3.5)
...
**Top Internal Functions/Classes:**
  * `write` (Impact: 2.1)
  * `write` (Impact: 2.1)
  * `test_should_not_swallow_certain_exceptions` (Impact: 2.1)
  * `test_should_format_message` (Impact: 2.0)
  * `test_should_call_handle_error_if_exception_was_thrown` (Impact: 1.9)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `submodules` (Impact: 2.2)
  * `Dockerfile` (Impact: 2.1)
  * `tqdm/tqdm.1` (Impact: 1.4)
  * `coverclean` (Impact: 1.4)
  * `clean` (Impact: 1.4)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  ...
  * `test_enumerate` (Impact: 2.1)
    * *Intent:* """Test contrib.tenumerate"""
  * `incr` (Impact: 1.6)
    * *Intent:* """Dummy function"""
  * `test_enumerate_numpy` (Impact: 1.3)
...
- `tqdm-4.67.3/tqdm/contrib/concurrent.py` -> **Severity: 0.176** (Bridge: 0.0018 * Flux: 100.0%)
