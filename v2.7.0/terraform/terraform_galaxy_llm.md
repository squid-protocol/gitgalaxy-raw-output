...
| Avg Path Length | 2.8173 | Hops between files. Lower = Tighter coupling. |
...
| credential | 26 | 9 | 0 | `internal/lang/funcs/crypto_test.go` |
| threat | 467 | 144 | 0 | `internal/copy/copy_value.go` |
...
- `TestContextPlan_actions` (@ `internal/terraform/context_plan_actions_test.go`) -> Impact: **259.7** | LOC: 1800
- `EnsureProviderVersions` (@ `internal/providercache/installer.go`) -> Impact: **235.2** | LOC: 544
...
- `plan` (@ `internal/terraform/node_resource_abstract_instance.go`) -> Impact: **217.2** | LOC: 572
- `opApply` (@ `internal/backend/local/backend_apply.go`) -> Impact: **209.8** | LOC: 440
- `validateProviderConfigs` (@ `internal/configs/provider_validation.go`) -> Impact: **205.5** | LOC: 510
...
- `TestContextApply_actions` (@ `internal/terraform/context_apply_action_test.go`) -> Impact: **186.3** | LOC: 1803
- `apply` (@ `internal/terraform/node_resource_abstract_instance.go`) -> Impact: **175.2** | LOC: 383
...
- `backendFromConfig` (@ `internal/command/meta_backend.go`) -> Impact: **174.7** | LOC: 440
...
- `getType` (@ `internal/stacks/stackconfig/typeexpr/typeexpr.go`) -> Impact: **170.9** | LOC: 333
- `opApply` (@ `internal/backend/remote/backend_apply.go`) -> Impact: **165.9** | LOC: 276
...
### 1. `internal/command/views/hook_json.go` (GO) -> Cumulative Risk: **729.57**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 196.34 | **LOC:** 297 | **CtrlFlow:** 11.9% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `PostApply` (Impact: 12.5), `PostEphemeralOp` (Impact: 11.3), `PreApply` (Impact: 8.4)

### 2. `internal/command/test.go` (GO) -> Cumulative Risk: **728.26**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `setupTestExecution` (Impact: 52.9), `Run` (Impact: 31.9), `Help` (Impact: 8.0)

### 3. `internal/command/views/hook_ui.go` (GO) -> Cumulative Risk: **719.63**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 462.7 | **LOC:** 646 | **CtrlFlow:** 17.1% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Concurrency (99.3996%), Tech Debt (98.6236%)
- **Heaviest Functions:** `PreApply` (Impact: 40.3), `PostApply` (Impact: 31.7), `stillRunning` (Impact: 23.7)

### 4. `internal/plugin6/grpc_provider.go` (GO) -> Cumulative Risk: **687.94**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1498.78 | **LOC:** 2090 | **CtrlFlow:** 17.1% | **Authorship Centralization:** 20.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (98.75%), Safety Score (92.1213%)
- **Heaviest Functions:** `ListResource` (Impact: 46.2), `PlanResourceChange` (Impact: 37.2), `ReadStateBytes` (Impact: 34.5)

### 5. `internal/backend/local/backend_plan.go` (GO) -> Cumulative Risk: **672.55**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `opPlan` (Impact: 68.9), `maybeWriteGeneratedConfig` (Impact: 18.0)

### 6. `internal/plugin/grpc_provider.go` (GO) -> Cumulative Risk: **672.08**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1227.36 | **LOC:** 1677 | **CtrlFlow:** 17.5% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (98.6842%), Safety Score (94.116%)
- **Heaviest Functions:** `ListResource` (Impact: 46.2), `PlanResourceChange` (Impact: 37.2), `ApplyResourceChange` (Impact: 33.4)

### 7. `internal/command/meta.go` (GO) -> Cumulative Risk: **667.14**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `RunOperation` (Impact: 24.9), `showDiagnostics` (Impact: 21.0), `MaybeGetSchemas` (Impact: 17.5)

### 8. `internal/backend/local/backend.go` (GO) -> Cumulative Risk: **666.07**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `StatePaths` (Impact: 17.3), `opWait` (Impact: 16.6), `Operation` (Impact: 15.0)

### 9. `internal/command/apply.go` (GO) -> Cumulative Risk: **658.62**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `Run` (Impact: 30.2), `OperationRequest` (Impact: 24.1), `helpApply` (Impact: 18.6)

### 10. `internal/command/ui_input.go` (GO) -> Cumulative Risk: **657.28**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `Input` (Impact: 51.6), `init` (Impact: 2.5)
...
**Top Internal Functions/Classes:**
  * `TestContext2Apply_scaleInCBD` (Impact: 63.8)
  * `TestContext2Apply_stop` (Impact: 40.0)
  * `TestContext2Apply_multiDepose_createBeforeDestroy` (Impact: 37.2)
  * `TestContext2Apply_taintedDestroyFailure` (Impact: 29.3)
  * `TestContext2Apply_ProviderMeta_refreshdata_set` (Impact: 28.7)
...
**Top Internal Functions/Classes:**
  * `TestContext2Plan_countIncreaseFromOneCorrupted` (Impact: 28.4)
    * *Intent:* // https://github.com/PeoplePerHour/terraform/pull/11 // // This tests a case where both a "resource...
  * `TestContext2Plan_scaleInForEach` (Impact: 28.1)
    * *Intent:* // for_each can reference a resource with 0 instances
  * `TestContext2Plan_countDecreaseToOne` (Impact: 27.5)
  * `TestContext2Plan_createBeforeDestroy_depends_datasource` (Impact: 26.3)
  * `TestContext2Plan_countIncreaseWithSplatReference` (Impact: 26.2)
...
**Top Internal Functions/Classes:**
  * `TestContext2Plan_dataSourcePreconditionPostcondition` (Impact: 73.3)
  * `TestContext2Plan_resourcePreconditionPostcondition` (Impact: 67.8)
  * `TestContext2Plan_dataResourceChecksManagedResourceChange` (Impact: 48.2)
  * `TestContext2Plan_refreshOnlyMode_orphan` (Impact: 45.2)
  * `TestContext2Plan_refreshOnlyMode_deposed` (Impact: 43.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `processIgnoreChangesIndividual` (Impact: 71.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `TestContext2Validate_providerContributedFunctions` (Impact: 66.0)
  * `TestContext2Validate_deprecatedAttr` (Impact: 54.5)
  * `TestContext2Validate_queryList` (Impact: 37.2)
  * `TestContext2Validate_action` (Impact: 29.9)
    * *Intent:* // Action Validation is largely exercised in context_plan_actions_test.go
  * `TestContext2Validate_varSensitive` (Impact: 15.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `TestTest_Runs` (Impact: 69.3)
  * `TestTest_ParallelTeardown` (Impact: 57.8)
  * `TestTest_Parallel_Divided_Order` (Impact: 39.2)
  * `TestTest_SkipCleanup_FileLevelFlag` (Impact: 27.5)
  * `TestTest_UseOfBackends_whenStateArtifactsAreMade` (Impact: 27.2)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1686.64 | **LOC:** 2603 | **CtrlFlow:** 12.9% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (54.264%), Tech Debt (90.7718%)
**Top Internal Functions/Classes:**
  * `ReadWithOptions` (Impact: 49.3)
  * `List` (Impact: 46.3)
  * `Create` (Impact: 36.1)
  * `Logs` (Impact: 31.6)
  * `Create` (Impact: 26.7)
...
**Top Internal Functions/Classes:**
  * `TestContext2Apply_resourceConditionApplyTimeFail` (Impact: 45.9)
  * `TestContext2Apply_deprecatedOutputsAndVariables` (Impact: 32.8)
  * `TestContext2Apply_resourcePostcondition` (Impact: 32.3)
  * `TestContext2Apply_outputValuePrecondition` (Impact: 28.2)
  * `TestApply_updateDependencies` (Impact: 22.6)
...
**Top Internal Functions/Classes:**
  * `TestInit_stateStore_configChanges` (Impact: 78.2)
    * *Intent:* // Testing init's behaviors with `state_store` when run in a working directory where the configurati...
  * `TestInit_stateStore_newWorkingDir` (Impact: 65.4)
    * *Intent:* // Testing init's behaviors with `state_store` when run in an empty working directory
  * `TestInit_backendCloudInvalidOptions` (Impact: 48.4)
  * `TestInit_backend_to_stateStore_multipleWorkspaces` (Impact: 45.0)
  * `TestInit_backend_to_stateStore_singleWorkspace` (Impact: 35.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ListResource` (Impact: 46.2)
  * `PlanResourceChange` (Impact: 37.2)
  * `ReadStateBytes` (Impact: 34.5)
  * `ApplyResourceChange` (Impact: 33.4)
  * `GetProviderSchema` (Impact: 30.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `internalValidate` (Impact: 136.2)
  * `diffList` (Impact: 82.0)
  * `diffSet` (Impact: 79.8)
  * `Diff` (Impact: 78.2)
...
**Top Internal Functions/Classes:**
  * `readTfplan` (Impact: 79.1)
    * *Intent:* // --------------------------------------------------------------------------- // This file deals wi...
  * `writeTfplan` (Impact: 71.5)
    * *Intent:* // writeTfplan serializes the given plan into the protobuf-based format used // for the "tfplan" por...
  * `resourceChangeFromTfplan` (Impact: 60.4)
  * `changeFromTfplan` (Impact: 55.6)
  * `CheckResultsFromPlanProto` (Impact: 47.5)
...
- `internal/deprecation/deprecation.go` -> Churn: **83.21%** | Cog Load: 8.5616% | Debt: 92.7444%
- `internal/command/meta_backend_errors.go` -> Churn: **62.4%** | Cog Load: 5.5715% | Debt: 51.8374%
- `internal/terraform/node_resource_abstract_instance.go` -> Churn: **60.99%** | Cog Load: 33.1288% | Debt: 92.1848%
- `internal/terraform/evaluate.go` -> Churn: **60.62%** | Cog Load: 34.1946% | Debt: 82.4591%
- `internal/terraform/eval_for_each.go` -> Churn: **57.68%** | Cog Load: 23.3866% | Debt: 60.6751%
...
- `internal/stacks/stackruntime/apply_test.go` -> **Liam Cervante** (100.0% isolated ownership) | Magnitude: 884.46
...
- `internal/terraform/context.go` -> **Severity: 0.021** (Bridge: 0.0002 * Flux: 84.804%)
- `internal/command/cliconfig/credentials.go` -> **Severity: 0.003** (Bridge: 0.0 * Flux: 99.3044%)
- `internal/backend/remote-state/oci/auth.go` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 99.9572%)
