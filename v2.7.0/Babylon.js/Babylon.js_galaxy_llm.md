...
| Avg Path Length | 3.6149 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 32.9 | 33.7 | 0.0 |
...
| credential | 27 | 11 | 0 | `packages/tools/playground/public/index.js` |
...
- `dartProgram` (@ `packages/tools/babylonServer/public/gltf_validator.js`) -> Impact: **999.2** | LOC: 1844
- `_mergeCoroutine` (@ `packages/dev/core/src/Meshes/mesh.vertexData.ts`) -> Impact: **440.4** | LOC: 436
  * *Intent:* /** * @internal */
- `GetModuleDeclaration` (@ `packages/dev/buildTools/src/generateDeclaration.ts`) -> Impact: **438.4** | LOC: 685
- `attachControl` (@ `packages/dev/core/src/Inputs/scene.inputManager.ts`) -> Impact: **434.3** | LOC: 546
...
- `GetBlockFromString` (@ `packages/tools/nodeEditor/src/blockTools.ts`) -> Impact: **410.4** | LOC: 609
- `CreateDecal` (@ `packages/dev/core/src/Meshes/Builders/decalBuilder.ts`) -> Impact: **406.2** | LOC: 484
...
- `ShowPickerDialogAsync` (@ `packages/dev/gui/src/2D/controls/colorpicker.ts`) -> Impact: **353.5** | LOC: 1042
...
- `valueEquals` (@ `packages/tools/viewer-configurator/src/components/configurator/configurator.tsx`) -> Impact: **334.9** | LOC: 1156
- `_createTextureBase` (@ `packages/dev/core/src/Engines/abstractEngine.ts`) -> Impact: **304.7** | LOC: 239
- `isReadyForSubMesh` (@ `packages/dev/core/src/Materials/standardMaterial.ts`) -> Impact: **299.6** | LOC: 593
...
- **Heaviest Functions:** `enableLightGizmo` (Impact: 19.1), `enableCameraGizmo` (Impact: 19.1), `prepareGLTFPlugin` (Impact: 14.5)
...
- **Heaviest Functions:** `moveToFrame` (Impact: 15.5), `play` (Impact: 14.1), `getKeyAtAnyFrameIndex` (Impact: 12.3)
...
- **Heaviest Functions:** `LoadAssetContainer` (Impact: 263.9), `loadSubMaterial` (Impact: 167.7), `load` (Impact: 138.1)
...
- **Heaviest Functions:** `_exportBuffers` (Impact: 76.1), `_exportMeshAsync` (Impact: 73.5), `_exportIndices` (Impact: 66.4)
...
- **Heaviest Functions:** `applyConfig` (Impact: 48.7), `workerFunction` (Impact: 23.9), `onmessage` (Impact: 20.9)
...
- **Heaviest Functions:** `constructor` (Impact: 123.8), `onGetNodeFromBlock` (Impact: 17.7), `getPortColor` (Impact: 16.3)
...
- **Heaviest Functions:** `constructor` (Impact: 18.1), `_gatherImports` (Impact: 5.6), `bind` (Impact: 1.9)
...
- **Heaviest Functions:** `_recreatePostProcesses` (Impact: 11.0), `setTextureDimensions` (Impact: 8.7), `readMinMax` (Impact: 8.4)
...
- **Heaviest Functions:** `_LoadScriptWeb` (Impact: 43.7), `EncodeScreenshotCanvasData` (Impact: 29.1), `SyncAsyncForLoop` (Impact: 25.5)
...
- **Heaviest Functions:** `ShowInspector` (Impact: 55.4), `factory` (Impact: 13.2), `disposeAsync` (Impact: 8.6)
...
**Top Internal Functions/Classes:**
  ...
  * `_updateInstancedBuffers` (Impact: 148.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004057 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000868 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  ...
  * `ProjectOnTriangleToRef` (Impact: 41.4)
  ...
  * `PerspectiveFovLHToRef` (Impact: 36.3)
...
**Top Internal Functions/Classes:**
  ...
  * `_renderWithFrameGraph` (Impact: 82.3)
  * `freezeActiveMeshes` (Impact: 78.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.030248 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_startRenderTargetRenderPass` (Impact: 212.0)
    * *Intent:* //------------------------------------------------------------------------------ // Render Pass //--...
  * `_draw` (Impact: 117.0)
  * `createEffect` (Impact: 110.0)
    * *Intent:* * Create a new effect (used to store vertex/fragment shaders) * @param baseName defines the base nam...
  * `_createInternalTexture` (Impact: 93.7)
    * *Intent:* /** * Creates an internal texture without binding it to a framebuffer * @internal * @param size defi...
  * `createTexture` (Impact: 85.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004694 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_mergeCoroutine` (Impact: 440.4)
    * *Intent:* /** * @internal */
  * `ComputeNormals` (Impact: 119.6)
    * *Intent:* * @param normals an array of vertex normals, [...., x, y, z, ......] * @param options an object used...
  * `_applyToCoroutine` (Impact: 86.2)
    * *Intent:* /** * @internal */
  * `_ComputeSides` (Impact: 51.6)
    * *Intent:* /** * @internal */
  * `_ExtractFrom` (Impact: 50.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000388 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_createTextureBase` (Impact: 304.7)
  * `constructor` (Impact: 74.2)
    * *Intent:* /** * Creates a new engine * @param antialias defines whether anti-aliasing should be enabled. If un...
  * `createTexture` (Impact: 48.9)
    * *Intent:* * @param scene needed for loading to the correct scene * @param samplingMode mode with should be use...
  * `createEffect` (Impact: 38.8)
    * *Intent:* * Create a new effect (used to store vertex/fragment shaders) * @param baseName defines the base nam...
  * `onload` (Impact: 34.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.008467 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_processObservables` (Impact: 62.8)
    * *Intent:* /** * @internal */
  * `_computeAlignment` (Impact: 42.1)
    * *Intent:* /** * @internal */
  * `_onPointerUp` (Impact: 38.4)
    * *Intent:* /** * @internal */
  * `_parseFromContent` (Impact: 36.5)
    * *Intent:* /** * @internal */
  * `_onPointerPick` (Impact: 27.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.003282 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  ...
  * `_getData` (Impact: 59.4)
    * *Intent:* /** @internal */
  * `getVertexData` (Impact: 42.8)
  * `getClosestFacetAtLocalCoordinates` (Impact: 37.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004156 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  ...
  * `_prepareEffect` (Impact: 125.3)
  * `getAnimatables` (Impact: 47.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000787 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_ValueNameToEnum` (Impact: 136.2)
  * `_GetSplat` (Impact: 104.8)
  * `_updateData` (Impact: 85.8)
  * `_postToWorker` (Impact: 50.6)
    * *Intent:* /** @internal */
  * `_GetCompressedChunks` (Impact: 49.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 6 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000992 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `LoadAssetContainer` (Impact: 263.9)
  * `loadSubMaterial` (Impact: 167.7)
    * *Intent:* // Loads a submaterial of a multimaterial
  * `load` (Impact: 138.1)
  * `importMesh` (Impact: 112.8)
  * `targetLookup` (Impact: 100.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 12 instances
...
**Top Internal Functions/Classes:**
  ...
  * `_insertNewParticle` (Impact: 41.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001081 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `_prepareDefines` (Impact: 168.8)
  * `_prepareEffect` (Impact: 131.8)
  * `isReadyForSubMesh` (Impact: 120.0)
    * *Intent:* /** * Specifies that the submesh is ready to be used. * @param mesh - BJS mesh. * @param subMesh - A...
  * `bindForSubMesh` (Impact: 109.9)
    * *Intent:* /** * Binds the submesh data. * @param world - The world matrix. * @param mesh - The BJS mesh. * @pa...
  * `constructor` (Impact: 61.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000907 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000616 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001219 | `Ripple Effect (Closeness):` 0.0
...
- `packages/dev/core/src/scene.ts` -> **Severity: 3.025** (Bridge: 0.0302 * Flux: 99.9966%)
- `packages/dev/core/src/Engines/abstractEngine.ts` -> **Severity: 0.846** (Bridge: 0.0085 * Flux: 99.965%)
- `packages/dev/core/src/Materials/Node/nodeMaterial.ts` -> **Severity: 0.635** (Bridge: 0.0064 * Flux: 99.9869%)
- `packages/dev/core/src/Audio/sound.ts` -> **Severity: 0.541** (Bridge: 0.0054 * Flux: 100.0%)
- `packages/dev/core/src/Engines/webgpuEngine.ts` -> **Severity: 0.469** (Bridge: 0.0047 * Flux: 99.9993%)
