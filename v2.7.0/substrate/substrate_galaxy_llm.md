...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 21.7 | 7.6 | 0.0 |
...
| dead_code | 7138 | 1145 | 9 | `frame/staking/src/tests.rs` |
| credential | 16 | 5 | 0 | `client/cli/src/params/network_params.rs` |
...
- `execute_and_import_block` (@ `client/service/src/client/client.rs`) -> Impact: **251.9** | LOC: 248
- `on_connection_handler_event` (@ `client/network/src/protocol/notifications/behaviour.rs`) -> Impact: **171.7** | LOC: 528
- `try_commit_operation` (@ `client/db/src/lib.rs`) -> Impact: **134.0** | LOC: 325
- `on_swarm_event` (@ `client/network/src/protocol/notifications/behaviour.rs`) -> Impact: **123.6** | LOC: 429
- `handle_swarm_event` (@ `client/network/src/service.rs`) -> Impact: **116.6** | LOC: 426
  * *Intent:* /// Process the next event coming from `Swarm`.
- `parse` (@ `frame/support/procedural/src/storage_alias.rs`) -> Impact: **116.2** | LOC: 90
- `run` (@ `utils/frame/benchmarking-cli/src/pallet/command.rs`) -> Impact: **111.6** | LOC: 362
  * *Intent:* /// Runs the command and benchmarks the chain.
- `on_block_data` (@ `client/network/sync/src/lib.rs`) -> Impact: **109.0** | LOC: 257
- `reduce_all` (@ `primitives/npos-elections/src/reduce.rs`) -> Impact: **107.2** | LOC: 333
...
- `try_from` (@ `frame/support/procedural/src/pallet/parse/call.rs`) -> Impact: **102.3** | LOC: 185
...
- **Heaviest Functions:** `run` (Impact: 7.9), `path` (Impact: 6.8), `setup` (Impact: 3.7)
...
- **Heaviest Functions:** `poll_next` (Impact: 20.7), `upgrade_outbound` (Impact: 17.6), `upgrade_inbound` (Impact: 15.7)
...
- **Heaviest Functions:** `poll` (Impact: 55.4), `prop` (Impact: 29.8), `forwarding_to_different_size_and_topic_channels` (Impact: 26.4)
...
- **Heaviest Functions:** `wait_n_finalized_blocks` (Impact: 9.6), `block_hash` (Impact: 7.8), `extract_info_from_output` (Impact: 7.1)
...
- **Heaviest Functions:** `import_justified` (Impact: 3.0), `import_justified` (Impact: 3.0), `import_as_best` (Impact: 2.6)
...
- **Heaviest Functions:** `open` (Impact: 27.1), `commit` (Impact: 23.2), `handle_err` (Impact: 3.2)
...
- **Heaviest Functions:** `poll_ready` (Impact: 42.1), `try_send_connection_messages` (Impact: 8.7), `start_send` (Impact: 6.4)
...
- **Heaviest Functions:** `run` (Impact: 22.6)
...
- **Heaviest Functions:** `add` (Impact: 24.1), `remove` (Impact: 24.1), `from_vote` (Impact: 9.1)
...
- **Heaviest Functions:** `finality_target` (Impact: 34.5), `best_header` (Impact: 4.6), `best_hash` (Impact: 3.3)
...
**Top Internal Functions/Classes:**
  * `try_commit_operation` (Impact: 134.0)
  * `revert` (Impact: 63.0)
  * `prune_blocks` (Impact: 33.8)
  * `set_head_with_transaction` (Impact: 32.3)
    * *Intent:* /// Handle setting head within a transaction. `route_to` should be the last /// block that existed i...
  * `append_justification` (Impact: 28.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `on_connection_handler_event` (Impact: 171.7)
  * `on_swarm_event` (Impact: 123.6)
  * `poll` (Impact: 39.0)
  * `disconnect_peer_inner` (Impact: 31.6)
    * *Intent:* /// Inner implementation of `disconnect_peer`.
  * `peerset_report_connect` (Impact: 26.8)
...
**Top Internal Functions/Classes:**
  * `execute_and_import_block` (Impact: 251.9)
  * `storage_collection` (Impact: 83.8)
  * `apply_finality_with_block_hash` (Impact: 52.3)
...
**Top Internal Functions/Classes:**
  * `reshuffle` (Impact: 31.9)
  * `handle_catch_up_request` (Impact: 29.0)
  * `message_allowed` (Impact: 25.0)
  * `update_peer_state` (Impact: 20.2)
    * *Intent:* // returns a reference to the new view, if the peer is known.
  * `validate_catch_up_message` (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `finalize_with_descendent_if` (Impact: 62.4)
...
**Top Internal Functions/Classes:**
  * `check_continue_operation` (Impact: 29.9)
  * `get_storage_multi_query_iter` (Impact: 26.9)
  * `get_storage_hash` (Impact: 23.9)
  * `get_storage_value` (Impact: 23.9)
  * `call_runtime` (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `call` (Impact: 14.1)
  * `lazy_removal_partial_remove_works` (Impact: 12.6)
  * `lazy_removal_does_not_use_all_weight` (Impact: 10.0)
  * `add_remove_delegate_dependency_works` (Impact: 9.7)
  * `native_dependency_deposit_works` (Impact: 9.7)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `test_payout_stakers` (Impact: 15.7)
  * `chill_other_works` (Impact: 9.5)
  * `proportional_ledger_slash_works` (Impact: 9.5)
  * `payout_stakers_handles_weight_refund` (Impact: 9.3)
  * `nominating_and_rewards_should_work` (Impact: 9.0)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 4 instances
...
**Top Internal Functions/Classes:**
  * `sync_cycle_from_offline_to_syncing_to_offline` (Impact: 13.8)
  * `block_announce_data_is_propagated` (Impact: 13.2)
    * *Intent:* /// Ensures that when we receive a block announcement with some data attached, that we propagate ///...
  * `syncs_state` (Impact: 11.7)
  * `can_sync_small_non_best_forks` (Impact: 9.2)
  * `can_sync_explicit_forks` (Impact: 7.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `read` (Impact: 18.6)
  * `post_dispatch` (Impact: 10.6)
  * `try_from_sub_account` (Impact: 7.8)
  * `try_into_sub_account` (Impact: 5.8)
...
**Top Internal Functions/Classes:**
  * `dispatch_benchmark` (Impact: 11.7)
  * `create_transaction` (Impact: 8.5)
  * `track_for` (Impact: 6.2)
  * `is_superset` (Impact: 5.6)
  * `on_unbalanceds` (Impact: 4.8)
**Structural Signatures (Net Mitigated Signals):**
