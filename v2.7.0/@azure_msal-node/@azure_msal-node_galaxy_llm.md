...
- `sendRequest` (@ `package/src/network/HttpClient.ts`) -> Impact: **35.5** | LOC: 84
...
- `acquireTokenWithManagedIdentity` (@ `package/src/client/ManagedIdentitySources/BaseManagedIdentitySource.ts`) -> Impact: **32.8** | LOC: 120
...
- `getServerTokenResponseAsync` (@ `package/src/client/ManagedIdentitySources/AzureArc.ts`) -> Impact: **30.4** | LOC: 117
...
- `constructor` (@ `package/src/client/ConfidentialClientApplication.ts`) -> Impact: **28.5** | LOC: 61
...
- `acquireTokenInteractive` (@ `package/src/client/PublicClientApplication.ts`) -> Impact: **28.0** | LOC: 108
  * *Intent:* /** * Acquires a token interactively via the browser by requesting an authorization code then exchanging it for a token. */
- `constructor` (@ `package/src/config/ManagedIdentityId.ts`) -> Impact: **26.0** | LOC: 40
- `listenForAuthCode` (@ `package/src/network/LoopbackClient.ts`) -> Impact: **25.1** | LOC: 52
...
- `tryCreate` (@ `package/src/client/ManagedIdentitySources/AzureArc.ts`) -> Impact: **24.8** | LOC: 72
...
- **Heaviest Functions:** `pauseForRetry` (Impact: 20.8), `isNewRequest` (Impact: 1.6), `MIN_EXPONENTIAL_BACKOFF_MS` (Impact: 1.1)
...
- **Heaviest Functions:** `constructor` (Impact: 6.7), `computeUri` (Impact: 2.9), `computeParametersBodyString` (Impact: 2.6)
...
- **Heaviest Functions:** `afterCacheAccess` (Impact: 7.0), `constructor` (Impact: 1.9), `beforeCacheAccess` (Impact: 1.8)
...
- **Heaviest Functions:** `mergeUpdates` (Impact: 19.0), `removeAccount` (Impact: 13.2), `mergeRemovals` (Impact: 12.5)
...
- **Heaviest Functions:** `sendRequest` (Impact: 35.5), `sendGetRequestAsync` (Impact: 6.3), `getFetchHeaders` (Impact: 6.3)
...
- **Heaviest Functions:** `urlToHttpOptions` (Impact: 12.5), `getNetworkResponse` (Impact: 2.5)
...
- **Heaviest Functions:** `constructor` (Impact: 28.5), `acquireTokenByClientCredential` (Impact: 18.3), `acquireTokenOnBehalfOf` (Impact: 6.1)
...
- **Heaviest Functions:** `constructor` (Impact: 26.0), `id` (Impact: 1.6), `idType` (Impact: 1.6)
...
- **Heaviest Functions:** `acquireTokenWithManagedIdentity` (Impact: 32.8), `getManagedIdentityUserAssignedIdQueryParameterKey` (Impact: 23.9), `getServerTokenResponse` (Impact: 16.5)
...
**Top Internal Functions/Classes:**
  * `cacheToInMemoryCache` (Impact: 19.9)
...
**Top Internal Functions/Classes:**
  * `getCachedAuthenticationResult` (Impact: 55.3)
    * *Intent:* /** * looks up cache if the tokens are cached already */
  * `createTokenRequestBody` (Impact: 16.8)
...
**Top Internal Functions/Classes:**
  ...
  * `acquireTokenWithDeviceCode` (Impact: 20.2)
...
**Top Internal Functions/Classes:**
  * `getServerTokenResponseAsync` (Impact: 30.4)
  ...
  * `getEnvironmentVariables` (Impact: 4.8)
...
**Top Internal Functions/Classes:**
  * `buildManagedIdentityConfiguration` (Impact: 14.3)
  * `buildAppConfiguration` (Impact: 12.9)
    * *Intent:* /** * Sets the default options when not explicitly configured from app developer * * @param auth - A...
  * `loggerCallback` (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `deserializeAllCache` (Impact: 9.4)
  ...
  * `deserializeIdTokens` (Impact: 3.9)
...
**Top Internal Functions/Classes:**
  ...
  * `getEnvironmentVariables` (Impact: 1.4)
...
**Top Internal Functions/Classes:**
  ...
  * `getEnvironmentVariables` (Impact: 1.9)
