...
| Avg Path Length | 3.1833 | Hops between files. Lower = Tighter coupling. |
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 35.8 | 10.6 | 0.0 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 85.3 | 100.0 | 100.0 |
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 6.2 | 0.8 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 100.0 | 5.7 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 78.6 | 100.0 | 100.0 |
...
| connectivity | 26306 | 1169 | 45 | `mypy/typeshed/stdlib/tkinter/__init__.pyi` |
...
| dead_code | 3261 | 604 | 5 | `mypy/test/testtypes.py` |
...
- `vgetargskeywords` (@ `mypyc/lib-rt/getargs.c`) -> Impact: **289.7** | LOC: 343
  * *Intent:* #define IS_END_OF_FORMAT(c) (c == '\0' || c == ';' || c == ':')
- `vgetargskeywordsfast_impl` (@ `mypyc/lib-rt/getargsfast.c`) -> Impact: **273.1** | LOC: 276
- `emit_cast` (@ `mypyc/codegen/emit.py`) -> Impact: **269.9** | LOC: 224
- `format_type_inner` (@ `mypy/messages.py`) -> Impact: **262.9** | LOC: 231
- `is_overlapping_types` (@ `mypy/meet.py`) -> Impact: **231.3** | LOC: 314
- `check_assignment` (@ `mypy/checker.py`) -> Impact: **215.9** | LOC: 202
- `process_options` (@ `mypy/main.py`) -> Impact: **203.5** | LOC: 213
- `define_options` (@ `mypy/main.py`) -> Impact: **201.4** | LOC: 893
- `emit_unbox` (@ `mypyc/codegen/emit.py`) -> Impact: **199.3** | LOC: 191
- `analyze_class_attribute_access` (@ `mypy/checkmember.py`) -> Impact: **198.0** | LOC: 203
...
| `mypyc/lib-rt` | 37 | 7937.48 | 32.97% | 38.24% |
...
### 1. `mypy/typeshed/stdlib/contextlib.pyi` (PYTHON) -> Cumulative Risk: **787.71**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 169.12 | **LOC:** 226 | **CtrlFlow:** 4.5% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (98.8087%)
- **Heaviest Functions:** `__exit__` (Impact: 2.4), `__aexit__` (Impact: 2.4), `__exit__` (Impact: 2.4)

### 2. `mypy/nodes.py` (PYTHON) -> Cumulative Risk: **778.74**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3197.56 | **LOC:** 5368 | **CtrlFlow:** 10.6% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Api Exposure (100.0%), Spec Match (100.0%), State Flux (99.9997%), Documentation (93.1338%)
- **Heaviest Functions:** `check_arg_kinds` (Impact: 47.8), `deserialize` (Impact: 36.1), `serialize` (Impact: 30.2)

### 3. `mypy/typeshed/stdlib/enum.pyi` (PYTHON) -> Cumulative Risk: **777.13**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 290.82 | **LOC:** 374 | **CtrlFlow:** 12.0% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.9163%)
- **Heaviest Functions:** `__call__` (Impact: 3.5), `__new__` (Impact: 3.3), `__call__` (Impact: 3.3)

### 4. `mypyc/ir/rtypes.py` (PYTHON) -> Cumulative Risk: **756.78**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 937.18 | **LOC:** 1445 | **CtrlFlow:** 16.4% | **Authorship Centralization:** 80.0%
- **Primary Risk Drivers:** Api Exposure (100.0%), Spec Match (100.0%), State Flux (99.9985%), Tech Debt (93.691%)
- **Heaviest Functions:** `__init__` (Impact: 36.9), `deserialize_type` (Impact: 16.9), `compute_rtype_alignment` (Impact: 16.6)

### 5. `mypyc/irbuild/for_helpers.py` (PYTHON) -> Cumulative Risk: **755.38**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1112.52 | **LOC:** 1354 | **CtrlFlow:** 17.7% | **Authorship Centralization:** 60.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (96.5173%), Safety Score (93.3125%)
- **Heaviest Functions:** `make_for_loop_generator` (Impact: 142.8), `get_expr_length` (Impact: 58.6), `sequence_from_generator_preallocate_helper` (Impact: 40.0)

### 6. `mypyc/analysis/dataflow.py` (PYTHON) -> Cumulative Risk: **752.03**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 560.46 | **LOC:** 646 | **CtrlFlow:** 14.5% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9998%), Tech Debt (99.9472%), Documentation (96.25%)
- **Heaviest Functions:** `run_analysis` (Impact: 90.9), `get_cfg` (Impact: 22.9), `cleanup_cfg` (Impact: 12.6)

### 7. `mypyc/ir/ops.py` (PYTHON) -> Cumulative Risk: **751.52**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1227.84 | **LOC:** 2107 | **CtrlFlow:** 5.5% | **Authorship Centralization:** 53.8%
- **Primary Risk Drivers:** Tech Debt (100.0%), Api Exposure (100.0%), Spec Match (100.0%), State Flux (99.9841%)
- **Heaviest Functions:** `__init__` (Impact: 14.9), `__init__` (Impact: 12.6), `__init__` (Impact: 11.7)

### 8. `mypy/typeshed/stdlib/asyncio/locks.pyi` (PYTHON) -> Cumulative Risk: **733.94**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 126.54 | **LOC:** 105 | **CtrlFlow:** 14.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (99.9554%)
- **Heaviest Functions:** `__aexit__` (Impact: 2.4), `__init__` (Impact: 2.0), `__init__` (Impact: 2.0)

### 9. `mypy/typeshed/stdlib/asyncio/windows_events.pyi` (PYTHON) -> Cumulative Risk: **732.02**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 107.82 | **LOC:** 122 | **CtrlFlow:** 9.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Tech Debt (100.0%), Spec Match (100.0%), Documentation (100.0%), State Flux (98.9861%)
- **Heaviest Functions:** `sendto` (Impact: 2.6), `sendfile` (Impact: 2.5), `recvfrom` (Impact: 2.4)

### 10. `mypy/typeshed/stdlib/random.pyi` (PYTHON) -> Cumulative Risk: **726.14**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 147.82 | **LOC:** 138 | **CtrlFlow:** 9.9% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9995%), Safety Score (96.508%)
- **Heaviest Functions:** `choices` (Impact: 2.8), `sample` (Impact: 2.4), `randrange` (Impact: 2.3)
...
- **Risk Profile:** Cognitive Load (61.0449%), Tech Debt (14.5132%)
**Top Internal Functions/Classes:**
  * `check_assignment` (Impact: 215.9)
  * `check_func_def` (Impact: 185.6)
  * `check_override` (Impact: 176.3)
  * `narrow_type_by_identity_equality` (Impact: 169.2)
  * `check_method_override_for_base_with_name` (Impact: 147.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000585 | `Ripple Effect (Closeness):` 0.018169
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001311 | `Ripple Effect (Closeness):` 0.025403
...
- **Risk Profile:** Cognitive Load (57.6852%), Tech Debt (19.1556%)
**Top Internal Functions/Classes:**
  * `check_callable_call` (Impact: 194.4)
  * `visit_call_expr_inner` (Impact: 129.5)
  * `dangerous_comparison` (Impact: 125.3)
  * `accept` (Impact: 114.2)
    * *Intent:* # # Helpers #
  * `check_argument_types` (Impact: 101.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000118 | `Ripple Effect (Closeness):` 0.015625
...
- **Risk Profile:** Cognitive Load (53.8268%), Tech Debt (16.0444%)
**Top Internal Functions/Classes:**
  * `new_state` (Impact: 131.3)
  * `validate_meta` (Impact: 121.7)
  * `find_cache_meta` (Impact: 111.4)
  * `load_graph` (Impact: 92.9)
  * `process_stale_scc` (Impact: 89.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.004206 | `Ripple Effect (Closeness):` 0.021344
...
- **Risk Profile:** Cognitive Load (75.135%), Tech Debt (77.4803%)
**Top Internal Functions/Classes:**
  * `copy_modified` (Impact: 195.1)
  * `slice` (Impact: 88.5)
  * `visit_callable_type` (Impact: 76.7)
  * `copy_modified` (Impact: 40.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002876 | `Ripple Effect (Closeness):` 0.107527
...
- **Risk Profile:** Cognitive Load (78.3868%), Tech Debt (8.793%)
**Top Internal Functions/Classes:**
  * `format_type_inner` (Impact: 262.9)
  * `incompatible_argument` (Impact: 182.0)
  * `report_protocol_problems` (Impact: 173.1)
  * `has_no_attr` (Impact: 147.9)
    * *Intent:* # # Specific operations # # The following operations are for generating specific error messages. The...
  * `pretty_callable` (Impact: 110.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001823 | `Ripple Effect (Closeness):` 0.066035
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 3197.56 | **LOC:** 5368 | **CtrlFlow:** 10.6% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (66.168%), Tech Debt (83.7371%)
**Top Internal Functions/Classes:**
  * `check_arg_kinds` (Impact: 47.8)
  * `deserialize` (Impact: 36.1)
  * `serialize` (Impact: 30.2)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001629 | `Ripple Effect (Closeness):` 0.110664
...
- **Risk Profile:** Cognitive Load (60.6636%), Tech Debt (12.3951%)
**Top Internal Functions/Classes:**
  * `_construct_varargs` (Impact: 161.1)
    * *Intent:* # Calls
  * `binary_op` (Impact: 138.1)
    * *Intent:* # Other primitive operations """Perform a binary operation. Generate specialized operations based on...
  * `coerce` (Impact: 132.3)
  * `native_args_to_positional` (Impact: 86.7)
  * `gen_method_call` (Impact: 66.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002511 | `Ripple Effect (Closeness):` 0.021956
...
- **Risk Profile:** Cognitive Load (83.6727%), Tech Debt (36.2603%)
**Top Internal Functions/Classes:**
  * `instantiate_type_alias` (Impact: 166.9)
  * `visit_unbound_type_nonoptional` (Impact: 159.2)
  * `try_analyze_special_unbound_type` (Impact: 142.8)
    * *Intent:* """Bind special type that is recognized through magic name such as 'typing.Any'. Return the bound ty...
  * `analyze_unbound_type_without_type_info` (Impact: 75.4)
  * `set_any_tvars` (Impact: 66.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5e-06 | `Ripple Effect (Closeness):` 0.021384
...
- **Risk Profile:** Cognitive Load (64.6158%), Tech Debt (8.6846%)
**Top Internal Functions/Classes:**
  * `read_expression` (Impact: 164.4)
  * `read_statement` (Impact: 116.5)
  * `fix_function_overloads` (Impact: 87.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.3e-05 | `Ripple Effect (Closeness):` 0.01495
...
- **Risk Profile:** Cognitive Load (7.7412%), Tech Debt (98.4965%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 10.3)
  * `configure` (Impact: 10.0)
  * `__init__` (Impact: 9.9)
  * `__init__` (Impact: 9.8)
  * `configure` (Impact: 9.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (83.5368%), Tech Debt (7.9958%)
**Top Internal Functions/Classes:**
  * `do_func_def` (Impact: 114.0)
  * `fix_function_overloads` (Impact: 87.0)
  * `translate_stmt_list` (Impact: 64.9)
  * `parse` (Impact: 43.5)
  * `_check_ifstmt_for_overloads` (Impact: 32.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00019 | `Ripple Effect (Closeness):` 0.020861
...
- **Risk Profile:** Cognitive Load (75.1096%), Tech Debt (15.1346%)
**Top Internal Functions/Classes:**
  * `are_parameters_compatible` (Impact: 170.9)
  * `visit_instance` (Impact: 124.4)
  * `is_protocol_implementation` (Impact: 103.9)
  * `find_node_type` (Impact: 71.6)
  * `find_member` (Impact: 69.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002045 | `Ripple Effect (Closeness):` 0.066099
...
- **Risk Profile:** Cognitive Load (76.4875%), Tech Debt (99.6733%)
**Top Internal Functions/Classes:**
  * `_verify_signature` (Impact: 140.0)
  * `get_mypy_type_of_runtime_value` (Impact: 84.5)
  * `verify_mypyfile` (Impact: 79.5)
  * `verify_typeinfo` (Impact: 72.3)
  * `test_stubs` (Impact: 65.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000693
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 2151.74 | **LOC:** 2227 | **CtrlFlow:** 3.4% | **Authorship Centralization:** 55.6%
- **Risk Profile:** Cognitive Load (24.27%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `__new__` (Impact: 4.0)
  * `__new__` (Impact: 3.8)
  * `__new__` (Impact: 3.8)
...
- **Risk Profile:** Cognitive Load (76.52%), Tech Debt (20.8129%)
**Top Internal Functions/Classes:**
  * `visit_instance` (Impact: 155.7)
    * *Intent:* # Non-leaf types
  * `infer_constraints_for_callable` (Impact: 122.1)
  ...
  * `_infer_constraints` (Impact: 72.4)
  * `build_constraints_for_simple_unpack` (Impact: 54.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000122 | `Ripple Effect (Closeness):` 0.052242
...
- **Risk Profile:** Cognitive Load (69.1714%), Tech Debt (10.4356%)
**Top Internal Functions/Classes:**
  * `analyze_class_attribute_access` (Impact: 198.0)
  * `analyze_var` (Impact: 134.4)
  * `analyze_member_var_access` (Impact: 93.8)
  * `analyze_instance_member_access` (Impact: 72.8)
    * *Intent:* # The several functions that follow implement analyze_member_access for various # types and aren't d...
  * `check_self_arg` (Impact: 59.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000423 | `Ripple Effect (Closeness):` 0.052082
...
- **Risk Profile:** Cognitive Load (65.5345%), Tech Debt (14.749%)
**Top Internal Functions/Classes:**
  * `emit_cast` (Impact: 269.9)
  * `emit_unbox` (Impact: 199.3)
  * `emit_box` (Impact: 72.4)
  * `emit_dec_ref` (Impact: 48.4)
    * *Intent:* # Otherwise assume it's an unboxed, pointerless value and do nothing.
  * `tuple_undefined_check_cond` (Impact: 33.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000317 | `Ripple Effect (Closeness):` 0.005099
...
- **Risk Profile:** Cognitive Load (22.5508%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 3.8)
  * `__replace__` (Impact: 3.8)
  * `__init__` (Impact: 3.8)
  * `__replace__` (Impact: 3.8)
  * `__init__` (Impact: 3.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.2e-05 | `Ripple Effect (Closeness):` 0.040079
...
- **Risk Profile:** Cognitive Load (63.0975%), Tech Debt (8.7864%)
**Top Internal Functions/Classes:**
  * `get_assignment_target` (Impact: 59.7)
  * `assign` (Impact: 46.9)
  * `call_refexpr_with_args` (Impact: 31.2)
  * `read` (Impact: 26.2)
  * `get_dict_base_type_from_type` (Impact: 22.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001426 | `Ripple Effect (Closeness):` 0.012316
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.00431
...
- **Risk Profile:** Cognitive Load (40.5184%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `PyConfig_Get` (Impact: 53.5)
...
- **Risk Profile:** Cognitive Load (44.7724%), Tech Debt (14.6655%)
**Top Internal Functions/Classes:**
  * `report` (Impact: 83.0)
  * `add_error_info` (Impact: 55.5)
  * `report_internal_error` (Impact: 52.1)
  * `format_messages_default` (Impact: 50.9)
  * `render_messages` (Impact: 47.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002321 | `Ripple Effect (Closeness):` 0.069887
...
- **Risk Profile:** Cognitive Load (67.9157%), Tech Debt (13.3266%)
**Top Internal Functions/Classes:**
  * `try_specialize_in_expr` (Impact: 60.2)
  * `translate_super_method_call` (Impact: 59.5)
  * `transform_name_expr` (Impact: 56.5)
    * *Intent:* # Name and attribute references
  * `translate_method_call` (Impact: 44.9)
    * *Intent:* """Generate IR for an arbitrary call of form e.m(...). This can also deal with calls to module-level...
  * `transform_basic_comparison` (Impact: 44.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.004885
...
- `mypy/util.py` -> **Severity: 0.651** (Bridge: 0.0065 * Flux: 100.0%)
- `mypy/dmypy/client.py` -> **Severity: 0.46** (Bridge: 0.0046 * Flux: 99.9965%)
- `mypy/build.py` -> **Severity: 0.421** (Bridge: 0.0042 * Flux: 100.0%)
- `mypy/dmypy_server.py` -> **Severity: 0.361** (Bridge: 0.0036 * Flux: 100.0%)
- `mypy/api.py` -> **Severity: 0.307** (Bridge: 0.0031 * Flux: 99.6316%)
