...
| Analyzed Artifacts (Scanned) | 81607 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 11404 |
| Total LOC | 24774860 |
...
*Total Excluded Artifacts: 11404*
...
- `.yaml`: 39x Zero-Density Threshold (LOC: 51, Signals: 0), 35x Zero-Density Threshold (LOC: 52, Signals: 0), 32x Zero-Density Threshold (LOC: 56, Signals: 0)
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 10.6 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 23.1 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 14.3 | 0.7 | 0.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 51.5 | 74.3 | 100.0 |
...
| connectivity | 518899 | 51285 | 15 | `drivers/gpu/drm/amd/include/vega10_enum.h` |
...
| mutation | 5648755 | 56696 | 182 | `drivers/gpu/drm/amd/include/navi10_enum.h` |
| dead_code | 118224 | 22457 | 4 | `drivers/net/ethernet/sfc/mcdi_pcol.h` |
| credential | 134 | 38 | 0 | `Documentation/RCU/Design/Data-Structures/BigTreePreemptRCUBHdyntickCB.svg` |
...
- `guard_balance_ratio` (guards / (danger + 1)): **1.9615**
...
- `ctx_statement_block` (@ `scripts/checkpatch.pl`) -> Impact: **8495.0** | LOC: 2091
- `CalculatePrefetchSchedule` (@ `drivers/gpu/drm/amd/display/dc/dml/dcn314/display_mode_vba_314.c`) -> Impact: **2554.9** | LOC: 1335
- `CalculatePrefetchSchedule` (@ `drivers/gpu/drm/amd/display/dc/dml/dcn31/display_mode_vba_31.c`) -> Impact: **2554.8** | LOC: 1332
- `wm5110_readable_register` (@ `drivers/mfd/wm5110-tables.c`) -> Impact: **2026.4** | LOC: 1142
- `wm5102_readable_register` (@ `drivers/mfd/wm5102-tables.c`) -> Impact: **1479.2** | LOC: 832
- `wm5100_readable_register` (@ `sound/soc/codecs/wm5100-tables.c`) -> Impact: **1372.0** | LOC: 767
- `cs47l24_readable_register` (@ `drivers/mfd/cs47l24-tables.c`) -> Impact: **1240.6** | LOC: 701
- `bpf_jit_build_body` (@ `arch/powerpc/net/bpf_jit_comp64.c`) -> Impact: **1211.3** | LOC: 1033
  * *Intent:* /* Assemble the body code between the prologue & epilogue */
- `wm8998_readable_register` (@ `drivers/mfd/wm8998-tables.c`) -> Impact: **1208.5** | LOC: 683
- `bpf_jit_build_body` (@ `arch/powerpc/net/bpf_jit_comp32.c`) -> Impact: **1200.6** | LOC: 1102
  * *Intent:* /* Assemble the body code between the prologue & epilogue */
...
| `fs/btrfs` | 128 | 108598.82 | 38.59% | 17.69% |
...
- **Heaviest Functions:** `__arch_prepare_bpf_trampoline` (Impact: 187.8), `bpf_arch_text_poke` (Impact: 84.5), `bpf_int_jit_compile` (Impact: 43.7)
...
- **Heaviest Functions:** `tnum_sbin` (Impact: 14.9), `tnum_mul` (Impact: 11.6), `tnum_step` (Impact: 10.7)
...
- **Heaviest Functions:** `r600_parse_extended_power_table` (Impact: 100.3), `r600_dpm_print_class_info` (Impact: 48.1), `r600_get_pcie_gen_support` (Impact: 27.9)
...
- **Heaviest Functions:** `amdgpu_amdkfd_get_dmabuf_info` (Impact: 40.7), `amdgpu_amdkfd_alloc_kernel_mem` (Impact: 34.7), `amdgpu_amdkfd_get_pcie_bandwidth_mbytes` (Impact: 32.2)
...
- **Heaviest Functions:** `mqd_symmetrically_map_cu_mask` (Impact: 47.1), `kfd_check_hiq_mqd_doorbell_id` (Impact: 14.8), `kfd_free_mqd_cp` (Impact: 6.5)

### 6. `arch/parisc/kernel/module.c` (C) -> Cumulative Risk: **689.91**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 771.1 | **LOC:** 962 | **CtrlFlow:** 15.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (98.8312%)
- **Heaviest Functions:** `apply_relocate_add` (Impact: 67.0), `apply_relocate_add` (Impact: 53.7), `module_finalize` (Impact: 41.8)

### 7. `drivers/gpu/drm/nouveau/nvif/outp.c` (C) -> Cumulative Risk: **689.86**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `nvif_outp_ctor` (Impact: 34.4), `nvif_outp_detect` (Impact: 11.1), `nvif_outp_edid_get` (Impact: 8.3)

### 8. `drivers/gpu/drm/bridge/analogix/analogix_dp_reg.c` (C) -> Cumulative Risk: **689.39**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `analogix_dp_set_analog_power_down` (Impact: 73.2), `analogix_dp_transfer` (Impact: 55.1), `analogix_dp_send_psr_spd` (Impact: 19.6)

### 9. `arch/um/os-Linux/helper.c` (C) -> Cumulative Risk: **688.43**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `run_helper` (Impact: 25.7), `run_helper_thread` (Impact: 22.0), `os_run_helper_thread` (Impact: 15.6)

### 10. `drivers/gpu/drm/nouveau/nvif/vmm.c` (C) -> Cumulative Risk: **688.38**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `nvif_vmm_ctor` (Impact: 44.4), `nvif_vmm_get` (Impact: 21.3), `nvif_vmm_map` (Impact: 15.5)
...
**Top Internal Functions/Classes:**
  * `nl80211_send_wiphy` (Impact: 646.9)
  * `nl80211_send_station` (Impact: 248.9)
  * `nl80211_parse_sched_scan` (Impact: 184.7)
  * `nl80211_set_wiphy` (Impact: 165.2)
  * `nl80211_msg_put_channel` (Impact: 163.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `bnxt_rx_pkt` (Impact: 145.7)
    * *Intent:* /* returns the following: * 1 - 1 packet successfully received * 0 - successful TPA_START, packet no...
  * `bnxt_start_xmit` (Impact: 115.6)
  * `bnxt_async_event_process` (Impact: 101.3)
    * *Intent:* #define BNXT_PHC_BITS 48
  * `bnxt_init_one` (Impact: 100.4)
  * `bnxt_tpa_end` (Impact: 76.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `tg3_get_invariants` (Impact: 425.2)
  * `tg3_reset_hw` (Impact: 400.7)
    * *Intent:* /* tp->lock is held. */
  * `tg3_setup_copper_phy` (Impact: 190.5)
  * `tg3_init_one` (Impact: 172.0)
  * `__tg3_start_xmit` (Impact: 122.1)
...
**Top Internal Functions/Classes:**
  * `CalculateOutputLink` (Impact: 464.1)
  * `CalculateDCCConfiguration` (Impact: 428.6)
  * `dml_core_mode_support` (Impact: 331.7)
  * `CalculateBytePerPixelAndBlockSizes` (Impact: 292.8)
  * `CalculatePrefetchSchedule` (Impact: 282.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `SiS_GetLCDResInfo` (Impact: 485.9)
  * `SiS_SetGroup1_LVDS` (Impact: 452.6)
    * *Intent:* /* Setup panel link * This is used for LVDS, LCDA and Chrontel TV output * 300/LVDS+TV, 300/301B-DH,...
  * `SiS_SetGroup2` (Impact: 452.5)
  * `SiS_GetCRT2Data301` (Impact: 329.1)
  * `SiS_SetGroup1` (Impact: 275.8)
    * *Intent:* /* Set Part 1 */
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ctx_statement_block` (Impact: 8495.0)
  * `process` (Impact: 910.3)
  * `annotate_values` (Impact: 200.5)
  * `build_types` (Impact: 87.4)
  * `ctx_block_get` (Impact: 71.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ath12k_mac_bss_info_changed` (Impact: 243.4)
  * `ath12k_mac_op_sta_state` (Impact: 117.6)
  * `ath12k_mac_bitrate_mask_get_single_nss` (Impact: 109.2)
  * `ath12k_peer_assoc_h_phymode` (Impact: 107.9)
  * `ath12k_peer_assoc_h_he` (Impact: 103.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_chk_btc_report` (Impact: 526.4)
  * `rtw89_btc_set_policy_v1` (Impact: 226.8)
  * `rtw89_btc_set_policy` (Impact: 147.1)
    * *Intent:* #define BTC_B1_MAX 250 /* unit ms */
  * `_chk_btc_err` (Impact: 146.4)
    * *Intent:* #define BTC_RPT_HDR_SIZE 3 #define BTC_CHK_WLSLOT_DRIFT_MAX 15 #define BTC_CHK_BTSLOT_DRIFT_MAX 15 #...
  * `rtw89_btc_fw_rpt_ver` (Impact: 142.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `CalculatePrefetchSchedule` (Impact: 2554.9)
  * `dml314_ModeSupportAndSystemConfigurationFull` (Impact: 568.2)
  * `CalculateDCCConfiguration` (Impact: 545.4)
  * `CalculateStutterEfficiency` (Impact: 381.4)
  * `CalculateSwathAndDETConfiguration` (Impact: 308.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `CalculatePrefetchSchedule` (Impact: 2554.8)
  * `dml31_ModeSupportAndSystemConfigurationFull` (Impact: 583.2)
  * `CalculateDCCConfiguration` (Impact: 545.4)
  * `CalculateStutterEfficiency` (Impact: 381.4)
  * `CalculateSwathAndDETConfiguration` (Impact: 325.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `kvm_set_msr_common` (Impact: 272.8)
  * `kvm_arch_vcpu_ioctl` (Impact: 243.9)
  * `kvm_get_msr_common` (Impact: 217.7)
  * `kvm_vm_ioctl_check_extension` (Impact: 209.8)
  * `kvm_arch_vm_ioctl` (Impact: 188.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `bnx2x_get_hwinfo` (Impact: 103.1)
  * `bnx2x_check_blocks_with_parity1` (Impact: 100.2)
  * `bnx2x_init_hw_common` (Impact: 92.7)
    * *Intent:* /** * bnx2x_init_hw_common - initialize the HW at the COMMON phase. * * @bp: driver handle */
  * `bnx2x_init_hw_port` (Impact: 84.9)
  * `bnx2x_panic_dump` (Impact: 83.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ath12k_wmi_op_rx` (Impact: 99.9)
  * `ath12k_pull_reg_chan_list_ext_update_ev` (Impact: 93.3)
  * `ath12k_wmi_svc_rdy_ext_parse` (Impact: 67.6)
  * `ath12k_wmi_svc_rdy_ext2_parse` (Impact: 62.9)
  * `ath12k_wmi_copy_peer_flags` (Impact: 58.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `hclge_init_ae_dev` (Impact: 62.5)
  * `hclge_get_mac_vlan_cmd_status` (Impact: 58.8)
  * `hclge_reset_ae_dev` (Impact: 39.6)
  * `hclge_fd_check_spec` (Impact: 39.2)
  * `hclge_set_vlan_filter` (Impact: 38.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ath11k_mac_op_bss_info_changed` (Impact: 245.7)
  * `ath11k_mac_op_set_key` (Impact: 135.9)
  * `ath11k_mac_op_sta_state` (Impact: 122.0)
  * `ath11k_peer_assoc_h_he` (Impact: 103.9)
  * `ath11k_peer_assoc_h_phymode` (Impact: 86.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `mii_init_common` (Impact: 80.2)
  * `niu_add_ethtool_tcam_entry` (Impact: 75.4)
  * `walk_phys` (Impact: 70.5)
  * `link_status_mii` (Impact: 69.7)
  * `niu_determine_phy_disposition` (Impact: 54.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `ieee80211_assoc_config_link` (Impact: 310.4)
  * `ieee80211_determine_chan_mode` (Impact: 159.8)
  * `ieee80211_mgd_assoc` (Impact: 154.6)
  * `ieee80211_determine_ap_chan` (Impact: 154.0)
  * `ieee80211_rx_mgmt_beacon` (Impact: 148.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `CalculateDCCConfiguration` (Impact: 542.5)
  * `dml30_ModeSupportAndSystemConfigurationFull` (Impact: 533.5)
  * `CalculateSwathAndDETConfiguration` (Impact: 309.3)
...
**Top Internal Functions/Classes:**
  * `dml32_CalculatePrefetchSchedule` (Impact: 924.8)
  * `dml32_CalculateWatermarksMALLUseAndDRAMSpeedChangeSupport` (Impact: 490.2)
  * `dml32_CalculateDCCConfiguration` (Impact: 417.2)
  * `dml32_CalculateVMAndRowBytes` (Impact: 376.5)
...
- `include/linux/memcontrol.h` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 58.8448%)
- `include/linux/rcuwait.h` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 96.4916%)
- `include/linux/skbuff.h` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 90.8678%)
