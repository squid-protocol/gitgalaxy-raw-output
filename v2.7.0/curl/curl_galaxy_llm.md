...
| Avg Path Length | 2.9775 | Hops between files. Lower = Tighter coupling. |
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 99.6 | 10.2 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 14.1 | 2.3 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 15.2 | 2.5 | 0.0 |
...
| connectivity | 5429 | 649 | 14 | `configure.ac` |
...
| dead_code | 2418 | 660 | 6 | `lib/cfilters.c` |
...
- `APPEND_imap` (@ `tests/ftpserver.pl`) -> Impact: **2135.3** | LOC: 1672
- `singletest_check` (@ `tests/runtests.pl`) -> Impact: **605.4** | LOC: 679
  * *Intent:* ####################################################################### # Verify test succeeded
- `ossl_connect_step2` (@ `lib/vtls/openssl.c`) -> Impact: **548.0** | LOC: 1400
  * *Intent:* #endif
- `single` (@ `scripts/managen`) -> Impact: **507.8** | LOC: 406
- `opt_string` (@ `src/tool_getparam.c`) -> Impact: **418.8** | LOC: 536
  * *Intent:* /* opt_string handles string options */
- `startservers` (@ `tests/servers.pm`) -> Impact: **403.5** | LOC: 660
  * *Intent:* ####################################################################### # startservers() starts all the named servers # # Returns: string with error r...
- `parsefmt` (@ `lib/mprintf.c`) -> Impact: **383.8** | LOC: 426
...
- `setopt_cptr` (@ `lib/setopt.c`) -> Impact: **376.8** | LOC: 695
  * *Intent:* #endif
- `opt_bool` (@ `src/tool_getparam.c`) -> Impact: **325.2** | LOC: 385
  * *Intent:* /* opt_bool is the function that handles boolean options */
- `single` (@ `scripts/cd2nroff`) -> Impact: **319.9** | LOC: 374

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Complexity Load | Avg Debt Markers |
|---|---|---|---|---|
| `lib` | 264 | 84083.3 | 33.91% | 17.55% |
| `src` | 91 | 18650.46 | 30.98% | 16.78% |
| `tests` | 53 | 18466.94 | 32.45% | 0.0% |
| `lib/vtls` | 29 | 17537.08 | 35.07% | 10.41% |
| `tests/libtest` | 262 | 16547.38 | 18.97% | 0.0% |
| `tests/server` | 14 | 9518.36 | 28.22% | 0.0% |
...
- **Heaviest Functions:** `curlx_pnow` (Impact: 30.1), `curlx_gmtime` (Impact: 11.5), `curlx_ptimediff_ms` (Impact: 7.4)
...
- **Heaviest Functions:** `sendrecv_dl` (Impact: 68.2), `Curl_pretransfer` (Impact: 60.6), `xfer_recv_resp` (Impact: 31.3)
...
- **Heaviest Functions:** `Curl_extract_certinfo` (Impact: 105.3), `do_pubkey` (Impact: 95.9), `utf8asn1str` (Impact: 59.7)
...
- **Heaviest Functions:** `Curl_conn_connect` (Impact: 54.3), `Curl_conn_shutdown` (Impact: 30.7), `Curl_conn_send` (Impact: 25.4)
...
- **Heaviest Functions:** `Curl_llist_insert_next` (Impact: 22.1), `Curl_node_take_elem` (Impact: 16.3), `Curl_node_uremove` (Impact: 7.7)

### 6. `lib/ratelimit.c` (C) -> Cumulative Risk: **700.5**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `rlimit_tune_steps` (Impact: 27.9), `rlimit_update` (Impact: 21.2), `Curl_rlimit_drain` (Impact: 17.1)

### 7. `lib/curlx/strparse.c` (C) -> Cumulative Risk: **699.78**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `str_num_base` (Impact: 37.5), `curlx_str_quotedword` (Impact: 25.4), `curlx_str_until` (Impact: 21.2)

### 8. `src/tool_operhlp.c` (C) -> Cumulative Risk: **695.93**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `add_file_name_to_url` (Impact: 50.6), `get_url_file_name` (Impact: 35.2), `urlerr_cvt` (Impact: 11.9)

### 9. `lib/multi.c` (C) -> Cumulative Risk: **685.54**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `multi_runsingle` (Impact: 174.1), `curl_multi_setopt` (Impact: 94.2), `multi_winsock_select` (Impact: 89.3)

### 10. `lib/http.c` (C) -> Cumulative Risk: **679.69**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 4752.48 | **LOC:** 5020 | **CtrlFlow:** 32.1% | **Authorship Centralization:** 47.3%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (98.3146%), Safety Score (93.9472%)
- **Heaviest Functions:** `http_on_response` (Impact: 231.4), `http_add_hd` (Impact: 150.3), `Curl_http_follow` (Impact: 150.2)
...
**Top Internal Functions/Classes:**
  * `ossl_connect_step2` (Impact: 548.0)
    * *Intent:* #endif
  * `Curl_ossl_ctx_init` (Impact: 197.3)
  * `client_cert` (Impact: 179.4)
  * `ossl_verifyhost` (Impact: 117.1)
    * *Intent:* */
  * `ossl_init_ech` (Impact: 93.9)
...
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 4752.48 | **LOC:** 5020 | **CtrlFlow:** 32.1% | **Authorship Centralization:** 47.3%
- **Risk Profile:** Cognitive Load (81.8902%), Tech Debt (8.9113%)
**Top Internal Functions/Classes:**
  * `http_on_response` (Impact: 231.4)
  * `http_add_hd` (Impact: 150.3)
  * `Curl_http_follow` (Impact: 150.2)
  * `http_rw_hd` (Impact: 119.6)
  * `Curl_add_custom_headers` (Impact: 113.4)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `safety_bypasses: 1`, `state_mutation: 612`, `dead_code: 14`, `fragile_debt: 4`
...
**Top Internal Functions/Classes:**
  * `APPEND_imap` (Impact: 2135.3)
  * `customize` (Impact: 91.7)
    * *Intent:* #********************************************************************** # customize configures test ...
  * `eXsysread` (Impact: 86.7)
    * *Intent:* #********************************************************************** # eXsysread is a wrapper aro...
  * `PASV_ftp` (Impact: 70.9)
  * `PORT_ftp` (Impact: 57.0)
...
**Top Internal Functions/Classes:**
  * `ftp_pp_statemachine` (Impact: 175.3)
  * `ftp_done` (Impact: 169.9)
    * *Intent:* /*********************************************************************** * * ftp_done() * * The DONE...
  * `ftp_state_pasv_resp` (Impact: 95.5)
  * `ftp_state_quote` (Impact: 89.1)
  * `ftp_port_parse_string` (Impact: 88.5)
...
**Top Internal Functions/Classes:**
  * `multi_runsingle` (Impact: 174.1)
  * `curl_multi_setopt` (Impact: 94.2)
    * *Intent:* #undef curl_multi_setopt
  * `multi_winsock_select` (Impact: 89.3)
  * `multi_wait` (Impact: 77.8)
    * *Intent:* #endif /* !USE_WINSOCK */ #define NUM_POLLS_ON_STACK 10
  * `Curl_multi_pollset` (Impact: 73.3)
...
**Top Internal Functions/Classes:**
  * `startservers` (Impact: 403.5)
    * *Intent:* ####################################################################### # startservers() starts all ...
  * `verifyhttp` (Impact: 115.1)
    * *Intent:* ####################################################################### # Verify that the server tha...
  * `verifyhttptls` (Impact: 89.5)
    * *Intent:* ####################################################################### # Verify that the non-stunne...
  * `verifyrtsp` (Impact: 84.3)
    * *Intent:* ####################################################################### # Verify that the server tha...
  * `runsshserver` (Impact: 75.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2.9e-05 | `Ripple Effect (Closeness):` 0.002828
...
**Top Internal Functions/Classes:**
  * `ssh_statemachine` (Impact: 310.8)
    * *Intent:* /* * ssh_statemachine() runs the SSH state machine as far as it can without * blocking and without r...
  * `sftp_upload_init` (Impact: 87.3)
  * `sftp_quote` (Impact: 84.0)
  * `sftp_quote_stat` (Impact: 68.1)
  * `ssh_knownhost` (Impact: 63.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `setopt_cptr` (Impact: 376.8)
    * *Intent:* #endif
  * `setopt_long_bool` (Impact: 184.3)
    * *Intent:* #endif
  * `setopt_long_net` (Impact: 120.4)
  * `setopt_func` (Impact: 95.4)
  * `setopt_long_proto` (Impact: 95.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `singletest_check` (Impact: 605.4)
    * *Intent:* ####################################################################### # Verify test succeeded
  * `checksystemfeatures` (Impact: 130.2)
    * *Intent:* ####################################################################### # Check & display informatio...
  * `displaylogs` (Impact: 82.5)
  * `singletest_shouldrun` (Impact: 69.0)
...
**Top Internal Functions/Classes:**
  * `http_connect` (Impact: 303.6)
    * *Intent:* * This function needs to connect to the server, and then pass data between * the client and the serv...
  * `test_sws` (Impact: 237.3)
  * `sws_ProcessRequest` (Impact: 202.9)
  * `sws_send_doc` (Impact: 128.2)
    * *Intent:* /* returns -1 on failure */
  * `sws_get_request` (Impact: 76.1)
...
**Top Internal Functions/Classes:**
  * `test_unsigned_int_formatting` (Impact: 12.0)
  * `test_signed_int_formatting` (Impact: 12.0)
  * `test_unsigned_long_formatting` (Impact: 12.0)
  * `test_signed_long_formatting` (Impact: 12.0)
  * `test_weird_arguments` (Impact: 12.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `on_header` (Impact: 113.7)
    * *Intent:* /* frame->hd.type is either NGHTTP2_HEADERS or NGHTTP2_PUSH_PROMISE */
  * `h2_submit` (Impact: 85.8)
  * `on_stream_frame` (Impact: 69.7)
  * `cf_h2_send` (Impact: 57.4)
  * `h2_progress_ingress` (Impact: 51.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `retrycheck` (Impact: 187.7)
  * `create_single` (Impact: 170.1)
    * *Intent:* /* create a transfer */
  * `operate` (Impact: 88.4)
  * `setup_outfile` (Impact: 77.7)
  * `post_per_transfer` (Impact: 59.7)
...
**Top Internal Functions/Classes:**
  * `h3_stream_open` (Impact: 70.3)
  * `cf_ngtcp2_query` (Impact: 67.6)
  * `cf_ngtcp2_connect` (Impact: 63.2)
  * `cf_ngtcp2_send` (Impact: 57.6)
  * `cf_progress_egress` (Impact: 53.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `Curl_mime_prepare_headers` (Impact: 154.0)
  * `cr_mime_read` (Impact: 75.5)
    * *Intent:* /* Real client reader to installed client callbacks. */
  * `readback_part` (Impact: 68.9)
    * *Intent:* /* Readback a mime part. */
  * `read_part_content` (Impact: 61.3)
    * *Intent:* /* Read a non-encoded part content. */
  * `encoder_qp_read` (Impact: 60.9)
...
**Top Internal Functions/Classes:**
  * `curl_url_set` (Impact: 166.2)
  * `urlget_url` (Impact: 141.8)
  * `curl_url_get` (Impact: 85.2)
  * `parse_file` (Impact: 74.3)
  * `dedotdotify` (Impact: 61.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `schannel_recv` (Impact: 153.6)
  * `get_client_cert` (Impact: 115.0)
  * `schannel_connect_step2` (Impact: 103.4)
  * `schannel_recv_renegotiate` (Impact: 99.1)
    * *Intent:* first to complete the renegotiation. */
  * `schannel_shutdown` (Impact: 69.6)
...
**Top Internal Functions/Classes:**
  * `Curl_gtls_verifyserver` (Impact: 217.0)
    * *Intent:* #endif /* USE_APPLE_SECTRUST */
  * `gtls_client_init` (Impact: 110.2)
  * `Curl_gtls_ctx_init` (Impact: 97.3)
    * *Intent:* #endif
  * `gtls_populate_creds` (Impact: 67.7)
  * `gtls_shutdown` (Impact: 50.6)
...
**Top Internal Functions/Classes:**
  * `single` (Impact: 507.8)
  * `render` (Impact: 302.3)
  * `getshortlong` (Impact: 55.0)
  * `printdesc` (Impact: 36.7)
  * `listglobals` (Impact: 36.3)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 2 instances
...
**Top Internal Functions/Classes:**
  * `imap_endofresp` (Impact: 120.3)
    * *Intent:* /*********************************************************************** * * imap_endofresp() * * Ch...
  * `imap_state_capability_resp` (Impact: 77.5)
    * *Intent:* /* For CAPABILITY responses */
  * `imap_parse_url_path` (Impact: 73.4)
    * *Intent:* /*********************************************************************** * * imap_parse_url_path() *...
  * `imap_perform` (Impact: 73.2)
    * *Intent:* /*********************************************************************** * * imap_perform() * * This...
  * `imap_state_listsearch_resp` (Impact: 68.9)
...
**Top Internal Functions/Classes:**
  * `bindlocal` (Impact: 142.0)
    * *Intent:* #ifndef CURL_DISABLE_BINDLOCAL
  * `cf_socket_open` (Impact: 60.7)
  * `cf_socket_send` (Impact: 54.4)
    * *Intent:* #endif /* USE_WINSOCK */
  * `cf_socket_query` (Impact: 52.1)
  * `tcpkeepalive` (Impact: 47.6)
...
**Top Internal Functions/Classes:**
  ...
  * `check_response` (Impact: 63.8)
  * `__init__` (Impact: 56.2)
  * `_run` (Impact: 54.1)
  * `_parse_headerfile` (Impact: 38.8)
**Contextual Mitigations & Amplifications:**
...
- `lib/http.c` -> Churn: **92.04%** | Cog Load: 81.8902% | Debt: 8.9113%
...
- `lib/doh.h` -> **Severity: 0.004** (Bridge: 0.0004 * Flux: 9.9229%)
