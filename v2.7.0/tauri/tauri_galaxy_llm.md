...
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 383 |
| Total LOC | 85588 |
...
| Avg Path Length | 3.9389 | Hops between files. Lower = Tighter coupling. |
...
- `.md`: 23x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Lexical Monotony: High structural repetition detected in 2402 LOC), 1x Excluded (Lexical Monotony: High structural repetition detected in 4031 LOC)
...
- `.rs`: 4x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 1x Excluded (Saturation: Line 7 exceeds 500 chars), 1x Statistical Anomaly (Z-Score: -4.68 < -4.55)
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 13.8 | 5.6 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 7.7 | 1.1 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 88.1 | 7.4 | 0.0 | 0.0 |
...
|---|---|---|---|---|
| memory | 4132 | 290 | 13 | `crates/tauri-runtime-wry/src/lib.rs` |
| cleanup | 39 | 22 | 0 | `crates/tauri-runtime-wry/src/lib.rs` |
| guards | 2519 | 310 | 12 | `crates/tauri-runtime-wry/src/lib.rs` |
| danger | 2005 | 235 | 8 | `crates/tauri-runtime-wry/src/lib.rs` |
| concurrency | 1815 | 131 | 6 | `packages/api/src/window.ts` |
| connectivity | 4768 | 374 | 15 | `crates/tauri-utils/src/config.rs` |
...
| events | 545 | 138 | 2 | `crates/tauri-runtime-wry/src/lib.rs` |
| tests | 817 | 80 | 2 | `crates/tauri-utils/src/config.rs` |
| docs | 12469 | 245 | 24 | `crates/tauri-utils/src/config.rs` |
| debt | 576 | 117 | 1 | `crates/tauri-bundler/src/bundle/macos/dmg/bundle_dmg` |
| mutation | 11682 | 395 | 47 | `crates/tauri-runtime-wry/src/lib.rs` |
| dead_code | 1650 | 255 | 4 | `crates/tauri/src/webview/webview_window.rs` |
| credential | 8 | 6 | 0 | `crates/tauri-bundler/src/bundle/windows/msi/mod.rs` |
...
- `build_nsis_app_installer` (@ `crates/tauri-bundler/src/bundle/windows/nsis/mod.rs`) -> Impact: **249.1** | LOC: 509
- `create_webview` (@ `crates/tauri-runtime-wry/src/lib.rs`) -> Impact: **231.3** | LOC: 666
- `build_wix_app_installer` (@ `crates/tauri-bundler/src/bundle/windows/msi/mod.rs`) -> Impact: **209.4** | LOC: 468
...
- `handle_user_message` (@ `crates/tauri-runtime-wry/src/lib.rs`) -> Impact: **167.6** | LOC: 831
- `resolve_access_message` (@ `crates/tauri/src/ipc/authority.rs`) -> Impact: **137.4** | LOC: 209
- `tauri_config_to_bundle_settings` (@ `crates/tauri-cli/src/interface/rust.rs`) -> Impact: **121.5** | LOC: 337
- `get_size` (@ `crates/tauri-bundler/src/bundle/macos/dmg/bundle_dmg`) -> Impact: **118.5** | LOC: 559
- `context_codegen` (@ `crates/tauri-codegen/src/context.rs`) -> Impact: **111.3** | LOC: 360
  * *Intent:* /// Build a `tauri::Context` for including in application code.
- `run_build` (@ `crates/tauri-cli/src/mobile/ios/build.rs`) -> Impact: **108.3** | LOC: 187
- `migrate_imports` (@ `crates/tauri-cli/src/migrate/migrations/v1/frontend.rs`) -> Impact: **104.8** | LOC: 218

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Complexity Load | Avg Debt Markers |
|---|---|---|---|---|
| `crates/tauri-runtime-wry/src` | 4 | 2173.98 | 10.4% | 22.22% |
| `crates/tauri-utils/src` | 12 | 1845.3 | 6.76% | 42.59% |
| `crates/tauri/src` | 9 | 1806.72 | 14.57% | 45.88% |
| `packages/api/src` | 14 | 1667.31 | 28.11% | 9.67% |
| `crates/tauri/src/webview` | 3 | 1541.32 | 12.95% | 65.17% |
| `crates/tauri-cli/src` | 12 | 1491.56 | 15.73% | 7.96% |
| `crates/tauri/src/menu` | 8 | 1362.96 | 5.97% | 48.28% |
| `crates/tauri-cli/src/helpers` | 17 | 1325.6 | 14.39% | 14.68% |
| `crates/tauri/src/ipc` | 7 | 1192.32 | 8.69% | 43.75% |
...
- `crates/tauri/src/webview/webview_window.rs` -> **161** Orphaned Functions | **0** Duplicates
- `crates/tauri/src/test/mock_runtime.rs` -> **113** Orphaned Functions | **23** Duplicates
- `crates/tauri-runtime/src/lib.rs` -> **95** Orphaned Functions | **27** Duplicates
- `crates/tauri/src/window/mod.rs` -> **104** Orphaned Functions | **0** Duplicates
- `crates/tauri/src/webview/mod.rs` -> **50** Orphaned Functions | **0** Duplicates
...
### 1. `crates/tauri/mobile/android/src/main/java/app/tauri/plugin/PluginManager.kt` (KOTLIN) -> Cumulative Risk: **776.47**
...
- **Heaviest Functions:** `dispatchPluginMessage` (Impact: 13.3), `onActivityCreate` (Impact: 8.6), `runCommand` (Impact: 7.6)

### 2. `crates/tauri-bundler/src/bundle/linux/appimage/linuxdeploy-plugin-gtk.sh` (SHELL) -> Cumulative Risk: **669.87**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__global_context__` (Impact: 13.9), `get_pkgconf_variable` (Impact: 10.8), `Anonymous_Block` (Impact: 9.8)

### 3. `crates/tauri/src/webview/webview_window.rs` (RUST) -> Cumulative Risk: **648.63**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 830.14 | **LOC:** 2661 | **CtrlFlow:** 2.0% | **Authorship Centralization:** 37.5%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.9983%), Concurrency (95.0875%), Verification (80.0%)
- **Heaviest Functions:** `window_features` (Impact: 7.6), `on_download` (Impact: 6.3), `transient_for` (Impact: 4.0)
...
- **Heaviest Functions:** `get_size` (Impact: 118.5), `find_mount_dir` (Impact: 68.3), `usage_[Truncated]` (Impact: 12.9)
...
- **Heaviest Functions:** `onmessage` (Impact: 3.2), `reload_upon_connect` (Impact: 1.4), `onopen` (Impact: 1.4)

### 6. `crates/tauri-cli/src/dev/builtin_dev_server.rs` (RUST) -> Cumulative Risk: **608.73**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `start` (Impact: 22.6), `watch` (Impact: 11.5), `handler` (Impact: 10.2)

### 7. `crates/tauri-cli/src/helpers/pbxproj.rs` (RUST) -> Cumulative Risk: **598.87**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `parse` (Impact: 60.9), `set_build_settings` (Impact: 17.4), `serialize` (Impact: 6.6)

### 8. `packages/api/src/dpi.ts` (TYPESCRIPT) -> Cumulative Risk: **598.61**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `constructor` (Impact: 8.0), `constructor` (Impact: 8.0), `constructor` (Impact: 8.0)

### 9. `crates/tauri-runtime/src/webview.rs` (RUST) -> Cumulative Risk: **587.75**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `from` (Impact: 16.9), `new` (Impact: 6.4), `new` (Impact: 2.5)

### 10. `packages/api/src/core.ts` (TYPESCRIPT) -> Cumulative Risk: **587.61**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `constructor` (Impact: 16.4), `invoke` (Impact: 4.3), `transformCallback` (Impact: 3.8)
...
- **Magnitude:** 1952.08 | **LOC:** 5354 | **CtrlFlow:** 7.0% | **Authorship Centralization:** 28.6%
- **Risk Profile:** Cognitive Load (20.7404%), Tech Debt (34.964%)
**Top Internal Functions/Classes:**
  * `create_webview` (Impact: 231.3)
  * `handle_user_message` (Impact: 167.6)
  * `create_window` (Impact: 93.1)
  * `handle_event_loop` (Impact: 84.0)
  * `with_config` (Impact: 30.8)
...
* *Structure:* `branch: 330`, `structural_boundaries: 682`, `args: 370`, `func_start: 264`, `class_start: 46`
* *Risk/State:* `safety_bypasses: 109`, `high_risk_execution: 7`, `state_mutation: 208`, `dead_code: 2`, `planned_debt: 1`, `duplicate_logic: 9`, `unreferenced_by_name: 36`
* *Architecture:* `api: 98`, `concurrency: 43`, `import: 49`
* *Defense:* `safety: 116`, `doc: 34`, `sync_locks: 74`, `immutability_locks: 1`, `cleanup: 5`
* *Network Topology:*
...
**Top Internal Functions/Classes:**
  * `tauri_config_to_bundle_settings` (Impact: 121.5)
  * `run_dev_watcher` (Impact: 60.1)
  * `new` (Impact: 48.2)
  * `get_bundle_settings` (Impact: 43.2)
  * `get_binaries` (Impact: 36.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 3 instances
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 830.14 | **LOC:** 2661 | **CtrlFlow:** 2.0% | **Authorship Centralization:** 37.5%
- **Risk Profile:** Cognitive Load (17.422%), Tech Debt (99.9983%)
**Top Internal Functions/Classes:**
...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 24`, `structural_boundaries: 149`, `args: 215`, `func_start: 209`, `class_start: 2`
* *Risk/State:* `safety_bypasses: 3`, `state_mutation: 96`, `dead_code: 91`, `unreferenced_by_name: 161`
* *Architecture:* `api: 197`, `concurrency: 22`, `import: 11`
* *Defense:* `safety: 2`, `doc: 1238`, `sync_locks: 1`, `cleanup: 1`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.225
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 808.56 | **LOC:** 2606 | **CtrlFlow:** 10.3% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (19.8655%), Tech Debt (63.2555%)
**Top Internal Functions/Classes:**
  * `build` (Impact: 58.7)
    * *Intent:* /// Builds the application.
  * `on_event_loop_event` (Impact: 29.8)
  * `set_menu` (Impact: 15.6)
    * *Intent:* /// Sets the app-wide menu and returns the previous one. /// /// If a window was not created with an...
  * `register_core_plugins` (Impact: 15.0)
  * `remove_data_store` (Impact: 13.1)
...
* *Risk/State:* `safety_bypasses: 33`, `high_risk_execution: 12`, `state_mutation: 29`, `dead_code: 56`, `planned_debt: 2`, `duplicate_logic: 12`
* *Architecture:* `io: 1`, `api: 117`, `concurrency: 61`, `import: 18`
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 637.46 | **LOC:** 2358 | **CtrlFlow:** 7.0% | **Authorship Centralization:** 42.9%
- **Risk Profile:** Cognitive Load (8.7801%), Tech Debt (76.9906%)
**Top Internal Functions/Classes:**
  * `on_message` (Impact: 48.5)
    * *Intent:* /// Handles this window receiving an [`InvokeRequest`].
  * `resolve_command_scope` (Impact: 17.9)
    * *Intent:* /// use tauri::Manager; /// /// #[derive(Debug, serde::Deserialize)] /// struct ScopeType { /// some...
  * `is_local_url` (Impact: 14.2)
  * `create_window` (Impact: 8.6)
...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 143`, `structural_boundaries: 307`, `args: 168`, `func_start: 117`, `class_start: 9`
* *Risk/State:* `safety_bypasses: 46`, `high_risk_execution: 1`, `state_mutation: 53`, `dead_code: 13`, `planned_debt: 1`, `unreferenced_by_name: 50`
* *Architecture:* `api: 141`, `concurrency: 38`, `import: 51`
* *Defense:* `safety: 18`, `doc: 537`, `test: 4`, `sync_locks: 14`, `immutability_locks: 2`, `cleanup: 1`
* *Network Topology:*
  * `Ecosystem Role:` Pure Consumer (Orchestrator) | `Dependency Blast Radius (PageRank):` 1.225
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `process_bundle` (Impact: 65.2)
  * `migrate_config` (Impact: 37.8)
  * `allowlist_to_permissions` (Impact: 37.8)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 548.08 | **LOC:** 1400 | **CtrlFlow:** 3.2% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (6.5172%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `run` (Impact: 25.9)
  * `create_window` (Impact: 8.2)
...
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 36`, `structural_boundaries: 97`, `args: 210`, `func_start: 205`, `class_start: 10`
* *Risk/State:* `safety_bypasses: 8`, `high_risk_execution: 19`, `state_mutation: 28`, `planned_debt: 4`, `duplicate_logic: 23`, `unreferenced_by_name: 113`
* *Architecture:* `api: 9`, `concurrency: 18`, `import: 6`
* *Defense:* `safety: 8`, `doc: 4`, `sync_locks: 16`
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 1.225
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  * `build_nsis_app_installer` (Impact: 249.1)
...
- **Risk Profile:** Cognitive Load (15.0354%), Tech Debt (8.4648%)
**Top Internal Functions/Classes:**
  * `new` (Impact: 37.1)
  * `insert` (Impact: 21.0)
  * `popup` (Impact: 20.0)
  * `create_item` (Impact: 19.8)
  * `create_item` (Impact: 19.4)
**Contextual Mitigations & Amplifications:**
...
* *Risk/State:* `safety_bypasses: 6`, `state_mutation: 24`, `unreferenced_by_name: 1`
...
**Top Internal Functions/Classes:**
  * `get_size` (Impact: 118.5)
  * `find_mount_dir` (Impact: 68.3)
    * *Intent:* --format)
  * `usage_[Truncated]` (Impact: 12.9)
  * `hdiutil_detach_retry` (Impact: 8.0)
...
**Top Internal Functions/Classes:**
  * `resolve_access_message` (Impact: 137.4)
  * `has_permissions_allowing_command` (Impact: 23.2)
  * `resolve_access` (Impact: 18.8)
    * *Intent:* /// Checks if the given IPC execution is allowed and returns the [`ResolvedCommand`] if it is.
  * `get_command_scope_typed` (Impact: 18.0)
  * `add_capability_inner` (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Cascading Flux:* 20 instances
* *High Risk Execution (weighted view):* 0
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 410.76 | **LOC:** 768 | **CtrlFlow:** 18.6% | **Authorship Centralization:** 62.5%
- **Risk Profile:** Cognitive Load (29.9629%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `create_info_plist` (Impact: 72.6)
    * *Intent:* // Creates the Info.plist file.
  * `bundle_project` (Impact: 47.4)
    * *Intent:* /// Bundles the project. /// Returns a vector of PathBuf that shows where the .app was created.
  * `copy_frameworks_to_bundle` (Impact: 42.8)
    * *Intent:* // Copies the macOS application bundle frameworks to the .app
  * `add_nested_code_sign_path` (Impact: 36.1)
  * `copy_custom_files_to_bundle` (Impact: 25.6)
...
* *Risk/State:* `safety_bypasses: 21`, `high_risk_execution: 1`, `state_mutation: 50`
...
**Top Internal Functions/Classes:**
  * `get_asset` (Impact: 30.1)
    * *Intent:* // TODO: Change to return `crate::Result` here in v3
  * `emit_to` (Impact: 18.2)
  * `emit_filter` (Impact: 15.1)
  * `replace_csp_nonce` (Impact: 13.6)
  * `emit` (Impact: 13.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 21 instances
...
**Top Internal Functions/Classes:**
  * `synchronize_project_config` (Impact: 81.8)
  * `get_config` (Impact: 81.5)
  * `simulator_prompt` (Impact: 28.9)
  * `connected_device_prompt` (Impact: 23.1)
  * `inject_resources` (Impact: 13.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 10 instances
...
**Top Internal Functions/Classes:**
  * `prepare_webview` (Impact: 89.4)
  * `prepare_pending_webview` (Impact: 87.3)
  * `on_webview_event` (Impact: 19.3)
  * `initialization_script` (Impact: 17.5)
  * `attach_webview` (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Cascading Flux:* 14 instances
* *High Risk Execution (weighted view):* 1
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 334.74 | **LOC:** 1004 | **CtrlFlow:** 0.4% | **Authorship Centralization:** 25.0%
- **Risk Profile:** Cognitive Load (5.253%), Tech Debt (100.0%)
**Top Internal Functions/Classes:**
  * `find_class` (Impact: 2.6)
    * *Intent:* /// Finds an Android class in the project scope.
  * `remove_data_store` (Impact: 2.4)
  * `new_any_thread` (Impact: 2.4)
    * *Intent:* /// Creates a new webview runtime on any thread.
  * `create_window` (Impact: 2.2)
    * *Intent:* /// Create a new window.
  * `create_webview` (Impact: 2.2)
    * *Intent:* /// Create a new webview.
**Structural Signatures (Net Mitigated Signals):**
* *Structure:* `branch: 2`, `structural_boundaries: 59`, `args: 151`, `func_start: 151`, `class_start: 17`
* *Risk/State:* `state_mutation: 6`, `planned_debt: 1`, `duplicate_logic: 27`, `unreferenced_by_name: 95`
* *Architecture:* `api: 33`, `concurrency: 28`, `import: 13`
* *Defense:* `doc: 356`
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 1.225
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
...
- `crates/tauri/src/webview/webview_window.rs` -> Churn: **64.6%** | Cog Load: 17.422% | Debt: 99.9983%
- `crates/tauri/src/webview/mod.rs` -> Churn: **61.14%** | Cog Load: 8.7801% | Debt: 76.9906%
...
- `crates/tauri/src/plugin.rs` -> **Tony** (100.0% isolated ownership) | Magnitude: 276.68
...
- `crates/tauri/mobile/android/src/main/java/app/tauri/plugin/PluginHandle.kt` -> **Severity: 0.009** (Bridge: 0.0001 * Flux: 99.7916%)
- `crates/tauri/mobile/android/src/main/java/app/tauri/plugin/Plugin.kt` -> **Severity: 0.008** (Bridge: 0.0001 * Flux: 99.2493%)
- `crates/tauri/mobile/android/src/main/java/app/tauri/plugin/Invoke.kt` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 93.9509%)
