...
| Avg Path Length | 2.8166 | Hops between files. Lower = Tighter coupling. |
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 29.3 | 11.6 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 21.2 | 2.4 | 80.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 19.5 | 6.8 | 0.0 |
...
| connectivity | 19437 | 1407 | 25 | `winch/codegen/src/visitor.rs` |
...
| dead_code | 10302 | 1274 | 12 | `pulley/src/interp.rs` |
| credential | 1 | 1 | 0 | `ci/build-release-artifacts.sh` |
| threat | 906 | 350 | 1 | `crates/c-api/include/wasmtime/component/types/val.h` |
...
- `step` (@ `cranelift/interpreter/src/step.rs`) -> Impact: **818.8** | LOC: 1204
...
- `parse_inst_operands` (@ `cranelift/reader/src/parser.rs`) -> Impact: **365.5** | LOC: 422
  * *Intent:* // Parse the operands following the instruction opcode. // This depends on the format of the opcode.
- `try_emit_compressed` (@ `cranelift/codegen/src/isa/riscv64/inst/emit.rs`) -> Impact: **344.3** | LOC: 566
  * *Intent:* /// Tries to emit an instruction as compressed, if we can't return false.
- `translate_operator` (@ `crates/cranelift/src/translate/code_translator.rs`) -> Impact: **333.2** | LOC: 1325
  * *Intent:* /// Translates wasm operators into Cranelift IR instructions.
- `emit` (@ `cranelift/codegen/src/isa/aarch64/inst/emit.rs`) -> Impact: **287.3** | LOC: 1274
- `emit_with_alloc_consumer` (@ `cranelift/codegen/src/isa/s390x/inst/emit.rs`) -> Impact: **254.9** | LOC: 1163
- `generate` (@ `crates/fuzzing/src/generators/component_async.rs`) -> Impact: **253.8** | LOC: 694
- `emit` (@ `cranelift/codegen/src/isa/x64/inst/emit.rs`) -> Impact: **248.7** | LOC: 1575
...
- `emit_block_contents` (@ `cranelift/isle/isle/src/codegen.rs`) -> Impact: **236.9** | LOC: 240
- `translate_payload` (@ `crates/environ/src/component/translate.rs`) -> Impact: **220.8** | LOC: 697

## 9. DIRECTORY GROUPS (Top 10 Heaviest Modules)
| Folder Path | Files | Total Impact | Avg Complexity Load | Avg Debt Markers |
|---|---|---|---|---|
| `tests/all` | 56 | 8684.58 | 7.09% | 0.0% |
...
- **Heaviest Functions:** `run` (Impact: 40.2), `handle_event` (Impact: 28.2), `update_on_stop` (Impact: 6.6)
...
- **Heaviest Functions:** `make` (Impact: 16.4), `get_memory` (Impact: 14.1), `get_global` (Impact: 14.1)
...
- **Heaviest Functions:** `poll_read` (Impact: 18.5), `poll_write` (Impact: 14.3), `write` (Impact: 11.7)
...
- **Heaviest Functions:** `type_info_func` (Impact: 26.5), `type_id_info` (Impact: 14.6), `analyze` (Impact: 13.7)
...
- **Heaviest Functions:** `fmt` (Impact: 25.8), `fmt` (Impact: 11.1), `error_ext_chain_mut` (Impact: 8.2)
...
- **Heaviest Functions:** `from` (Impact: 11.0), `from` (Impact: 11.0), `from` (Impact: 7.9)
...
- **Heaviest Functions:** `read_directory` (Impact: 29.1), `link_at` (Impact: 16.7), `set_times_at` (Impact: 16.6)
...
- **Heaviest Functions:** `blocking_write` (Impact: 21.8), `blocking_write_and_flush` (Impact: 12.0), `read` (Impact: 10.2)
...
- **Heaviest Functions:** `emit` (Impact: 66.0), `_define_func` (Impact: 27.6), `finish_block` (Impact: 7.6)

### 10. `crates/wasmtime/src/runtime/trampoline/memory.rs` (RUST) -> Cumulative Risk: **653.49**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 125.36 | **LOC:** 277 | **CtrlFlow:** 5.3% | **Authorship Centralization:** 80.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Api Exposure (97.0135%), Documentation (95.9184%), Safety Score (89.597%)
- **Heaviest Functions:** `create_memory` (Impact: 15.6), `allocate_memory` (Impact: 7.9), `validate_module` (Impact: 6.4)
...
**Top Internal Functions/Classes:**
  * `generate_guest_import_closure` (Impact: 113.4)
  * `generate_trait` (Impact: 97.4)
  * `export` (Impact: 75.0)
  * `define_rust_guest_export` (Impact: 65.0)
  * `world_add_to_linker` (Impact: 64.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `visit_br_table` (Impact: 41.2)
  * `visit_br_if` (Impact: 36.1)
  * `visit_call_indirect` (Impact: 17.6)
  * `visit_table_set` (Impact: 17.3)
  * `visit_memory_copy` (Impact: 17.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_s390x_binemit` (Impact: 2.5)
  * `simm20_zero` (Impact: 1.2)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `check_indirect_call_type_signature` (Impact: 36.7)
  * `make_table` (Impact: 21.7)
  * `translate_global_get` (Impact: 21.1)
  * `translate_global_set` (Impact: 18.1)
  * `translate_memory_size` (Impact: 18.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 4 instances
...
**Top Internal Functions/Classes:**
  * `v128_shift` (Impact: 46.9)
  * `wasm_load` (Impact: 41.1)
  * `splat` (Impact: 39.5)
  * `atomic_rmw` (Impact: 37.6)
  * `popcnt` (Impact: 27.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `make` (Impact: 16.4)
  * `get_memory` (Impact: 14.1)
  * `get_global` (Impact: 14.1)
  * `get_table` (Impact: 14.1)
  * `get_func` (Impact: 14.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `add_rule_constraints` (Impact: 121.5)
  * `solve_constraints` (Impact: 113.8)
    * *Intent:* // // t4 = bv // t1 = bv // t7 = bv // // would result in: // bv16 -> t2, t6, t1 // bv8 -> t3, t5, t...
  * `add_annotation_constraints` (Impact: 69.6)
  * `type_annotations_using_rule` (Impact: 65.9)
  * `create_parse_tree_pattern` (Impact: 45.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `poll_oneoff` (Impact: 111.1)
  * `fd_pread` (Impact: 49.9)
  * `fd_read` (Impact: 45.9)
  * `sock_recv` (Impact: 45.0)
  * `path_open` (Impact: 43.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `preview2_stdin` (Impact: 27.1)
  * `cli_serve_sleep` (Impact: 24.9)
  * `cli_serve_hello_world` (Impact: 24.7)
  * `spawn` (Impact: 21.5)
  * `config_cli_flag` (Impact: 14.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
**Top Internal Functions/Classes:**
  * `lower_string` (Impact: 45.6)
  * `lift_try_map` (Impact: 31.0)
  * `lower_map_iter` (Impact: 23.9)
  * `prepare_call` (Impact: 23.6)
    * *Intent:* /// Calls `concurrent::prepare_call` with monomorphized functions for /// lowering the parameters an...
  * `typecheck_variant` (Impact: 20.1)
...
**Top Internal Functions/Classes:**
  * `test_magic_generators_give_correct_numbers` (Impact: 77.2)
  * `test_magic_generators_dont_panic` (Impact: 37.4)
  * `test_magic_s32_inner` (Impact: 28.3)
    * *Intent:* // Compute the magic numbers for `d` and then use them to compute and // check `n / d` for around 10...
  * `test_magic_s64_inner` (Impact: 28.3)
    * *Intent:* // Compute the magic numbers for `d` and then use them to compute and // check `n / d` for around 10...
  * `magic_u32` (Impact: 20.6)
...
**Top Internal Functions/Classes:**
  * `func_constructors` (Impact: 84.9)
  * `wrap_multiple_results` (Impact: 77.5)
  * `calls_with_funcref_and_externref` (Impact: 41.3)
  * `get_from_wrapper` (Impact: 37.8)
  * `typed_v128_imports` (Impact: 28.0)
**Contextual Mitigations & Amplifications:**
...
- `crates/c-api/include/wasmtime/func.hh` -> **Severity: 0.003** (Bridge: 0.0001 * Flux: 53.6726%)
- `crates/c-api/include/wasmtime/component/val.hh` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 100.0%)
- `crates/c-api/include/wasmtime/config.hh` -> **Severity: 0.002** (Bridge: 0.0001 * Flux: 16.5448%)
- `crates/c-api/include/wasmtime/extern.hh` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 100.0%)
- `crates/c-api/include/wasmtime/types/func.hh` -> **Severity: 0.002** (Bridge: 0.0 * Flux: 81.3204%)
...
- `cranelift/codegen/src/isa/riscv64/inst/vector.rs` -> **Severity: 173.711** (Blast Radius: 2.009 * Doc Risk: 86.4662%)
