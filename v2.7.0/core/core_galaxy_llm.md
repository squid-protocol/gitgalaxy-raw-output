...
| Analyzed Artifacts (Scanned) | 22673 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 2145 |
| Total LOC | 2855865 |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 21.7 | 7.3 | 0.0 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 63.6 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 31.8 | 11.1 | 0.0 |
...
| danger | 50454 | 8263 | 6 | `homeassistant/helpers/config_validation.py` |
| concurrency | 210063 | 12909 | 22 | `tests/test_config_entries.py` |
| connectivity | 106531 | 14392 | 11 | `tests/test_config_entries.py` |
| io | 6044 | 2254 | 0 | `tests/test_config.py` |
...
| debt | 4179 | 774 | 0 | `tests/test_config_entries.py` |
| mutation | 749012 | 14878 | 81 | `tests/test_config_entries.py` |
| dead_code | 22101 | 4219 | 2 | `tests/test_core.py` |
| credential | 400 | 155 | 0 | `tests/components/freedompro/const.py` |
| threat | 11257 | 3508 | 1 | `homeassistant/components/sonarr/helpers.py` |
| ml_ai | 930 | 320 | 0 | `tests/components/melcloud/test_config_flow.py` |
...
- `_async_update_device` (@ `homeassistant/helpers/device_registry.py`) -> Impact: **349.0** | LOC: 303
- `async_match_targets` (@ `homeassistant/helpers/intent.py`) -> Impact: **251.0** | LOC: 267
- `async_get_broker_settings` (@ `homeassistant/components/mqtt/config_flow.py`) -> Impact: **196.0** | LOC: 322
- `async_get_or_create` (@ `homeassistant/helpers/device_registry.py`) -> Impact: **193.8** | LOC: 177
- `_async_update_entity` (@ `homeassistant/helpers/entity_registry.py`) -> Impact: **192.4** | LOC: 173
- `compile_statistics` (@ `homeassistant/components/sensor/recorder.py`) -> Impact: **166.6** | LOC: 294
- `put` (@ `homeassistant/components/emulated_hue/hue_api.py`) -> Impact: **157.4** | LOC: 286
- `get_accessory` (@ `homeassistant/components/homekit/accessories.py`) -> Impact: **155.8** | LOC: 177
- `async_get_or_create` (@ `homeassistant/helpers/entity_registry.py`) -> Impact: **155.4** | LOC: 200
- `_create_or_update_task` (@ `homeassistant/components/habitica/services.py`) -> Impact: **145.6** | LOC: 282
...
### 1. `homeassistant/helpers/trigger.py` (PYTHON) -> Cumulative Risk: **770.72**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1195.76 | **LOC:** 1615 | **CtrlFlow:** 13.5% | **Authorship Centralization:** 66.7%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Concurrency (99.9966%), Safety Score (94.2561%)
- **Heaviest Functions:** `async_initialize_triggers` (Impact: 55.1), `_register_trigger_platform` (Impact: 30.7), `async_get_all_descriptions` (Impact: 25.1)

### 2. `homeassistant/components/open_router/config_flow.py` (PYTHON) -> Cumulative Risk: **761.69**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 202.98 | **LOC:** 303 | **CtrlFlow:** 7.4% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Tech Debt (97.9214%)
- **Heaviest Functions:** `async_step_init` (Impact: 23.4), `async_step_init` (Impact: 12.7), `async_step_user` (Impact: 6.7)

### 3. `homeassistant/components/frontend/storage.py` (PYTHON) -> Cumulative Risk: **758.1**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 267.1 | **LOC:** 322 | **CtrlFlow:** 4.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Safety Score (96.5643%)
- **Heaviest Functions:** `websocket_subscribe_user_data` (Impact: 7.7), `websocket_get_user_data` (Impact: 7.3), `async_set_item` (Impact: 6.4)

### 4. `homeassistant/components/openai_conversation/config_flow.py` (PYTHON) -> Cumulative Risk: **751.75**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 593.74 | **LOC:** 818 | **CtrlFlow:** 11.1% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Safety Score (92.5857%)
- **Heaviest Functions:** `async_step_model` (Impact: 61.3), `async_step_init` (Impact: 35.3), `async_step_init` (Impact: 13.5)

### 5. `homeassistant/components/shelly/repairs.py` (PYTHON) -> Cumulative Risk: **728.62**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 327.66 | **LOC:** 404 | **CtrlFlow:** 11.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Safety Score (85.8149%)
- **Heaviest Functions:** `async_create_fix_flow` (Impact: 21.7), `async_manage_ble_scanner_firmware_unsupported_issue` (Impact: 12.2), `async_manage_open_wifi_ap_issue` (Impact: 12.2)

### 6. `homeassistant/components/wilight/switch.py` (PYTHON) -> Cumulative Risk: **724.11**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 262.74 | **LOC:** 307 | **CtrlFlow:** 11.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.5358%)
- **Heaviest Functions:** `extra_state_attributes` (Impact: 15.7), `async_setup_entry` (Impact: 10.9), `async_set_trigger` (Impact: 10.5)

### 7. `homeassistant/helpers/schema_config_entry_flow.py` (PYTHON) -> Cumulative Risk: **723.47**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 393.06 | **LOC:** 561 | **CtrlFlow:** 15.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Safety Score (98.0375%)
- **Heaviest Functions:** `_async_form_step` (Impact: 34.1), `_show_next_step` (Impact: 27.4), `_update_and_remove_omitted_optional_keys` (Impact: 25.9)

### 8. `homeassistant/helpers/condition.py` (PYTHON) -> Cumulative Risk: **721.77**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1548.78 | **LOC:** 1814 | **CtrlFlow:** 16.5% | **Authorship Centralization:** 63.6%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Concurrency (99.9665%), Safety Score (92.2753%)
- **Heaviest Functions:** `async_numeric_state` (Impact: 79.7), `time` (Impact: 67.5), `state` (Impact: 51.2)

### 9. `homeassistant/components/alarm_control_panel/device_action.py` (PYTHON) -> Cumulative Risk: **718.65**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 141.62 | **LOC:** 140 | **CtrlFlow:** 19.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `async_call_action_from_config` (Impact: 19.2), `async_get_actions` (Impact: 15.6), `async_get_action_capabilities` (Impact: 14.7)

### 10. `homeassistant/components/homekit_controller/switch.py` (PYTHON) -> Cumulative Risk: **717.94**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 152.08 | **LOC:** 261 | **CtrlFlow:** 3.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.7891%)
- **Heaviest Functions:** `async_setup_entry` (Impact: 7.8), `extra_state_attributes` (Impact: 6.5), `async_add_characteristic` (Impact: 3.4)
...
- **Risk Profile:** Cognitive Load (24.8925%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_update_entry_and_reload` (Impact: 20.1)
  * `test_unique_id_collision_issues` (Impact: 16.4)
  * `test_saving_and_loading` (Impact: 14.2)
  * `test_as_dict` (Impact: 12.1)
    * *Intent:* """Test ConfigEntry.as_dict."""
  * `test_create_entry_next_flow` (Impact: 12.0)
...
- **Risk Profile:** Cognitive Load (43.0528%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_add_node` (Impact: 35.3)
  * `test_replace_failed_node` (Impact: 30.2)
  * `test_update_log_config` (Impact: 19.0)
  * `test_node_metadata` (Impact: 17.4)
  * `test_restore_nvm` (Impact: 14.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (78.6973%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_repeat_nested` (Impact: 42.6)
  * `test_if` (Impact: 39.1)
  * `test_max_exceeded` (Impact: 32.2)
  * `test_wait_variables_out` (Impact: 29.3)
    * *Intent:* """Test the wait output variable."""
  * `test_repeat_conditional` (Impact: 27.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (43.5153%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_compile_statistics_hourly_daily_monthly_summary` (Impact: 94.7)
  * `test_compile_hourly_statistics_changing_device_class_1` (Impact: 29.6)
  * `verify_stats` (Impact: 29.1)
  * `test_list_statistic_ids` (Impact: 27.1)
  * `test_compile_hourly_sum_statistics_amount` (Impact: 25.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (42.1844%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_subscribe_unsubscribe_logbook_stream_included_entities` (Impact: 22.4)
  * `test_subscribe_unsubscribe_logbook_stream` (Impact: 19.3)
  * `test_subscribe_entities_some_have_uom_multiple` (Impact: 17.2)
  * `test_subscribe_all_entities_are_continuous_with_device` (Impact: 16.0)
  * `test_logbook_stream_excluded_entities_inherits_filters_from_recorder` (Impact: 13.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (39.9371%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_reconfigure_migrate_with_addon` (Impact: 14.3)
  * `test_addon_rf_region_migrate_network` (Impact: 10.5)
  * `test_usb_discovery_migration` (Impact: 9.8)
  * `test_usb_discovery_migration_restore_driver_ready_timeout` (Impact: 9.4)
  * `test_reconfigure_migrate_restore_driver_ready_timeout` (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (44.891%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_statistic_during_period` (Impact: 113.1)
  * `test_statistic_during_period_hole` (Impact: 36.9)
  * `test_statistic_during_period_partial_overlap` (Impact: 27.6)
  * `test_list_statistic_ids` (Impact: 25.7)
  * `test_statistic_during_period_circular_mean` (Impact: 24.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (36.4897%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_user_flow_with_ble_devices` (Impact: 13.6)
  * `test_bluetooth_provisioning_clears_match_history` (Impact: 12.4)
  * `test_zeroconf` (Impact: 11.2)
  * `test_zeroconf_sleeping_device` (Impact: 9.9)
  * `test_bluetooth_provision_timeout_ble_fallback_succeeds` (Impact: 8.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (78.7618%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_track_template_result_super_template_2` (Impact: 29.4)
  * `test_track_template_result_super_template_2_initially_false` (Impact: 29.4)
  * `test_track_template_result_super_template_initially_false` (Impact: 24.7)
  * `test_track_template_result_super_template` (Impact: 24.5)
    * *Intent:* """Test tracking template with super template listening to same entity."""
  * `test_track_template_result_none` (Impact: 19.3)
...
- **Risk Profile:** Cognitive Load (38.0103%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_thermostat` (Impact: 16.3)
    * *Intent:* """Test thermostat discovery."""
  * `test_valve_position` (Impact: 13.2)
  * `assert_percentage_changes` (Impact: 13.0)
  * `test_water_heater` (Impact: 12.9)
    * *Intent:* """Test water_heater discovery."""
  * `get_capability` (Impact: 12.4)
...
- **Risk Profile:** Cognitive Load (35.9267%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_assert_extract_from_target_command_result` (Impact: 25.1)
  * `test_get_triggers_conditions_for_target` (Impact: 23.5)
  * `test_get_services_for_target` (Impact: 21.8)
  * `_apply_entities_changes` (Impact: 16.6)
    * *Intent:* """Apply a diff set to a dict. Port of the client side merging """
  * `test_render_template_error_in_template_code` (Impact: 15.0)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (46.3196%), Tech Debt (8.2222%)
**Top Internal Functions/Classes:**
  * `__async_setup_with_context` (Impact: 81.0)
  * `async_finish_flow` (Impact: 64.2)
  * `_async_update_entry` (Impact: 62.2)
  * `__init__` (Impact: 50.1)
  * `async_init` (Impact: 49.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  * `Ecosystem Role:` Pure Producer (Foundation) | `Dependency Blast Radius (PageRank):` 16.302
...
- **Risk Profile:** Cognitive Load (43.6388%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_upload_progress_debounced` (Impact: 24.4)
  * `test_initiate_backup` (Impact: 24.2)
  * `test_initiate_backup_per_agent_encryption` (Impact: 19.1)
  * `test_initiate_backup_with_agent_error` (Impact: 16.8)
  * `test_upload_progress_event` (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 350 instances
...
- **Risk Profile:** Cognitive Load (38.7903%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_device_capture_override` (Impact: 19.0)
  * `test_device_capture` (Impact: 17.4)
  * `test_wake_word_cooldown_different_entities` (Impact: 14.3)
  * `test_intent_progress_event` (Impact: 12.8)
  * `test_wake_word_cooldown_same_id` (Impact: 11.3)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (24.9988%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_async_get_hass_can_be_called` (Impact: 11.6)
    * *Intent:* """Test calling async_get_hass via different paths. The test asserts async_get_hass can be called fr...
  * `test_stage_shutdown_with_exit_code` (Impact: 10.4)
    * *Intent:* """Simulate a shutdown, test calling stuff with exit code checks."""
  * `test_async_all` (Impact: 6.9)
    * *Intent:* """Test async_all."""
  * `test_chained_logging_hits_log_timeout` (Impact: 6.5)
  * `test_chained_logging_misses_log_timeout` (Impact: 6.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (36.6609%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_assert_entry` (Impact: 20.8)
  * `test_logbook_entity_context_parent_id` (Impact: 12.5)
  * `test_logbook_entity_context_id` (Impact: 11.0)
  * `test_get_events_with_device_ids` (Impact: 10.9)
  * `test_logbook_select_entities_context_id` (Impact: 10.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (50.8562%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_get_matching_entries_ws` (Impact: 20.7)
  * `test_get_progress_subscribe_in_progress` (Impact: 16.3)
  * `test_get_progress_subscribe` (Impact: 16.0)
  * `test_get_progress_subscribe_in_progress_bad_flow` (Impact: 14.3)
  * `test_subscribe_entries_ws_filtered` (Impact: 13.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (46.3784%), Tech Debt (47.9028%)
**Top Internal Functions/Classes:**
  * `execute` (Impact: 62.7)
...
* *Amplified Sql Injection:* 24 instances
...
- **Risk Profile:** Cognitive Load (34.6648%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_plug_in_new_radio_retry` (Impact: 12.7)
  * `consume_progress_flow` (Impact: 10.2)
  * `test_options_flow_defaults` (Impact: 9.5)
  * `test_plug_in_old_radio_retry` (Impact: 9.4)
    * *Intent:* """Test plug_in_old_radio step when reset fails due to unplugged adapter."""
  * `test_plug_in_old_radio_config_entry_removed` (Impact: 8.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (69.2111%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_async_get_all_descriptions` (Impact: 21.2)
  * `test_numeric_state_raises` (Impact: 17.1)
    * *Intent:* """Test that numeric_state raises ConditionError on errors."""
  * `assert_element` (Impact: 12.9)
    * *Intent:* """Assert a trace element is as expected. Note: Unused variable 'path' is passed to get helpful erro...
  * `test_subscribe_conditions_experimental_conditions` (Impact: 11.0)
  * `test_subscribe_conditions` (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (34.6539%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_satellite_area_context` (Impact: 47.0)
  * `test_fuzzy_matching` (Impact: 15.8)
  * `test_intent_tool_call_in_chat_log` (Impact: 13.7)
    * *Intent:* """Test that intent tool calls are stored in the chat log."""
  * `test_handle_failed_intents` (Impact: 12.7)
  * `test_handle_intents_with_response_errors` (Impact: 11.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (41.6145%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_reader_writer_create_per_agent_encryption` (Impact: 27.7)
  * `test_agent_upload` (Impact: 24.1)
  * `test_reader_writer_create_report_progress` (Impact: 19.2)
  * `test_reader_writer_restore_report_progress` (Impact: 15.0)
  * `mock_backup_agent` (Impact: 14.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (49.5894%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `statistic_during_period` (Impact: 105.5)
  * `_statistics_during_period_with_session` (Impact: 81.6)
  * `_reduce_statistics` (Impact: 72.2)
  * `_sorted_statistics_to_dict` (Impact: 70.2)
  * `_get_max_mean_min_statistic_in_sub_period` (Impact: 67.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (52.4965%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `test_async_parallel_updates_with_one` (Impact: 15.0)
    * *Intent:* """Test parallel updates with 1 (sequential)."""
  * `test_extending_entity_description` (Impact: 14.2)
    * *Intent:* """Test extending entity descriptions."""
  * `test_async_parallel_updates_with_two` (Impact: 11.8)
    * *Intent:* """Test parallel updates with 2 (parallel)."""
  * `test_cached_entity_property_class_attribute` (Impact: 10.4)
    * *Intent:* """Test entity properties on class level work in derived classes."""
  * `test_entity_name_translation_placeholder_errors` (Impact: 8.8)
**Contextual Mitigations & Amplifications:**
...
- `homeassistant/core.py` -> **Severity: 0.072** (Bridge: 0.0007 * Flux: 99.9945%)
- `homeassistant/config_entries.py` -> **Severity: 0.03** (Bridge: 0.0003 * Flux: 99.9906%)
- `homeassistant/core_config.py` -> **Severity: 0.026** (Bridge: 0.0003 * Flux: 99.9935%)
- `homeassistant/helpers/entity.py` -> **Severity: 0.017** (Bridge: 0.0002 * Flux: 100.0%)
- `homeassistant/helpers/entity_platform.py` -> **Severity: 0.011** (Bridge: 0.0001 * Flux: 100.0%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `homeassistant/core.py` -> **Severity: 5702.322** (Blast Radius: 151.693 * Doc Risk: 37.5912%)
- `homeassistant/util/logging.py` -> **Severity: 4666.659** (Blast Radius: 87.843 * Doc Risk: 53.125%)
- `homeassistant/helpers/translation.py` -> **Severity: 1283.158** (Blast Radius: 19.504 * Doc Risk: 65.7895%)
- `homeassistant/util/async_.py` -> **Severity: 1105.294** (Blast Radius: 23.334 * Doc Risk: 47.3684%)
- `homeassistant/util/json.py` -> **Severity: 890.828** (Blast Radius: 20.786 * Doc Risk: 42.8571%)
