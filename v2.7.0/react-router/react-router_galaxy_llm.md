...
| Avg Path Length | 7.8689 | Hops between files. Lower = Tighter coupling. |
...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 16.4 | 5.6 | 0.0 |
...
| connectivity | 4463 | 635 | 6 | `integration/single-fetch-test.ts` |
...
| dead_code | 243 | 119 | 1 | `packages/react-router-dev/vite/route-chunks.ts` |
| credential | 50 | 6 | 0 | `tutorials/address-book/app/data.ts` |
| threat | 84 | 37 | 0 | `packages/react-router/lib/components.tsx` |
...
- `createRouter` (@ `packages/react-router/lib/router/router.ts`) -> Impact: **583.0** | LOC: 2100
  * *Intent:* //#endregion //////////////////////////////////////////////////////////////////////////////// //#region createRouter /////////////////////////////////...
- `getMatchesToLoad` (@ `packages/react-router/lib/router/router.ts`) -> Impact: **277.7** | LOC: 300
- `createStaticHandler` (@ `packages/react-router/lib/router/router.ts`) -> Impact: **267.2** | LOC: 944
- `reactRouterRSCVitePlugin` (@ `packages/react-router-dev/vite/rsc/plugin.ts`) -> Impact: **232.9** | LOC: 938
- `handleLoaders` (@ `packages/react-router/lib/router/router.ts`) -> Impact: **192.8** | LOC: 265
  * *Intent:* // Call all applicable loaders for the given matches, handling redirects, // errors, etc.
- `createClientRoutes` (@ `packages/react-router/lib/dom/ssr/routes.tsx`) -> Impact: **173.5** | LOC: 350
- `resolveConfig` (@ `packages/react-router-dev/config/config.ts`) -> Impact: **145.5** | LOC: 308
- `useRoutesImpl` (@ `packages/react-router/lib/hooks.tsx`) -> Impact: **145.5** | LOC: 1235
  * *Intent:* // Internal implementation with accept optional param for RouterProvider usage
- `handleFetcherAction` (@ `packages/react-router/lib/router/router.ts`) -> Impact: **135.3** | LOC: 318
  * *Intent:* // Call the action for the matched fetcher.submit(), and then handle redirects, // errors, and revalidation
- `setup` (@ `packages/react-router/__tests__/router/utils/data-router-setup.ts`) -> Impact: **134.2** | LOC: 535
...
- **Heaviest Functions:** `processServerAction` (Impact: 80.2), `replace` (Impact: 79.8), `generateMiddlewareResponse` (Impact: 34.9)
...
- **Heaviest Functions:** `setup` (Impact: 134.2), `getFetcherHelpers` (Impact: 42.0), `fetch` (Impact: 30.7)
...
- **Heaviest Functions:** `createRouter` (Impact: 583.0), `getMatchesToLoad` (Impact: 277.7), `createStaticHandler` (Impact: 267.2)
...
- **Heaviest Functions:** `generateEntry` (Impact: 63.0), `routes` (Impact: 16.7), `typegen` (Impact: 15.7)
...
- **Heaviest Functions:** `reactRouterRSCVitePlugin` (Impact: 232.9), `postProcess` (Impact: 117.8), `config` (Impact: 93.9)
...
- **Heaviest Functions:** `fetchAndDecodeViaTurboStream` (Impact: 49.2), `singleFetchUrl` (Impact: 28.7), `unwrapSingleFetchResult` (Impact: 24.3)
...
- **Heaviest Functions:** `EXPRESS_SERVER` (Impact: 35.8), `reactRouterConfig` (Impact: 25.0), `basic` (Impact: 23.6)
...
- **Heaviest Functions:** `fetchAndApplyManifestPatches` (Impact: 69.5), `useFogOFWarDiscovery` (Impact: 33.7), `getPatchRoutesOnNavigationFunction` (Impact: 15.9)
...
- **Heaviest Functions:** `resolveConfig` (Impact: 145.5), `isValidPrerenderPathsConfig` (Impact: 112.6), `createConfigLoader` (Impact: 58.8)
...
- **Heaviest Functions:** `getEnvironmentOptionsResolvers` (Impact: 109.3), `handlePrerender` (Impact: 66.7), `handler` (Impact: 53.5)
...
**Top Internal Functions/Classes:**
  * `createRouter` (Impact: 583.0)
    * *Intent:* //#endregion //////////////////////////////////////////////////////////////////////////////// //#reg...
  * `getMatchesToLoad` (Impact: 277.7)
  * `createStaticHandler` (Impact: 267.2)
  * `handleLoaders` (Impact: 192.8)
    * *Intent:* // Call all applicable loaders for the given matches, handling redirects, // errors, etc.
  * `handleFetcherAction` (Impact: 135.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001452 | `Ripple Effect (Closeness):` 0.050501
...
**Top Internal Functions/Classes:**
  * `getEnvironmentOptionsResolvers` (Impact: 109.3)
  * `handlePrerender` (Impact: 66.7)
  * `handler` (Impact: 53.5)
    * *Intent:* // After the SSR build is finished, we inspect the Vite manifest for // the SSR build and move serve...
  * `postProcess` (Impact: 50.4)
  * `resolveId` (Impact: 47.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.000914
...
**Top Internal Functions/Classes:**
  * `respondWithJson` (Impact: 6.4)
  * `generateMiddlewareResponse` (Impact: 4.0)
  * `generateMiddlewareResponse` (Impact: 4.0)
  * `pushOrderContext` (Impact: 3.8)
  * `generateMiddlewareResponse` (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `createFixture` (Impact: 20.6)
  * `listAllFiles` (Impact: 6.8)
  * `recurse` (Impact: 6.3)
  * `captureRequests` (Impact: 4.9)
  * `clearRequests` (Impact: 3.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `testDomRouter` (Impact: 117.9)
  * `assertLocation` (Impact: 12.7)
    * *Intent:* // Utility to assert location info based on the type of router
  * `setupTest` (Impact: 9.7)
  * `Component` (Impact: 7.0)
  * `ErrorBoundary` (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 40 instances
...
**Top Internal Functions/Classes:**
  * `loader` (Impact: 8.0)
  * `queryRoute` (Impact: 5.7)
  * `action` (Impact: 4.9)
  * `setupFlexRouteTest` (Impact: 4.9)
  * `loader` (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `patchRoutesOnNavigation` (Impact: 7.6)
  * `patchRoutesOnNavigation` (Impact: 6.9)
  * `patchRoutesOnNavigation` (Impact: 6.9)
  * `patchRoutesOnNavigation` (Impact: 6.9)
  * `patchRoutesOnNavigation` (Impact: 6.9)
**Contextual Mitigations & Amplifications:**
...
- `packages/react-router/lib/server-runtime/server.ts` -> **Severity: 0.152** (Bridge: 0.0015 * Flux: 97.9202%)
- `packages/react-router/lib/router/router.ts` -> **Severity: 0.142** (Bridge: 0.0015 * Flux: 97.751%)
- `packages/react-router/lib/router/instrumentation.ts` -> **Severity: 0.14** (Bridge: 0.0015 * Flux: 93.0148%)
- `packages/react-router/lib/dom/dom.ts` -> **Severity: 0.023** (Bridge: 0.0002 * Flux: 99.8466%)
- `packages/react-router/lib/dom/ssr/routes.tsx` -> **Severity: 0.023** (Bridge: 0.0004 * Flux: 52.0678%)
