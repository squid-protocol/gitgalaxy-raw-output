...
| connectivity | 11438 | 1814 | 12 | `MediaBrowser.Controller/Entities/BaseItem.cs` |
...
| credential | 32 | 8 | 0 | `Emby.Naming/Common/NamingOptions.cs` |
...
- `ApplyTranscodingConditions` (@ `MediaBrowser.Model/Dlna/StreamBuilder.cs`) -> Impact: **532.2** | LOC: 503
- `TranslateQuery` (@ `Jellyfin.Server.Implementations/Item/BaseItemRepository.cs`) -> Impact: **425.3** | LOC: 946
- `GetItems` (@ `Jellyfin.Api/Controllers/ItemsController.cs`) -> Impact: **408.9** | LOC: 343
...
- `FetchDataFromXmlNode` (@ `MediaBrowser.XbmcMetadata/Parsers/BaseNfoParser.cs`) -> Impact: **272.5** | LOC: 393
  * *Intent:* /// <summary> /// Fetches metadata from an XML node. /// </summary> /// <param name="reader">The <see cref="XmlReader"/>.</param> /// <param name="ite...
- `Filter` (@ `MediaBrowser.Controller/Entities/UserViewBuilder.cs`) -> Impact: **263.7** | LOC: 473
...
- `GetPostedPlaybackInfo` (@ `Jellyfin.Api/Controllers/MediaInfoController.cs`) -> Impact: **254.1** | LOC: 134
...
- `BuildStreamVideoItem` (@ `MediaBrowser.Model/Dlna/StreamBuilder.cs`) -> Impact: **244.6** | LOC: 211
- `GetMediaStream` (@ `MediaBrowser.MediaEncoding/Probing/ProbeResultNormalizer.cs`) -> Impact: **200.7** | LOC: 347
...
### 1. `src/Jellyfin.LiveTv/LiveTvManager.cs` (CSHARP) -> Cumulative Risk: **697.17**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 929.0 | **LOC:** 1293 | **CtrlFlow:** 12.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9987%), Concurrency (99.9866%), Documentation (91.4286%)
- **Heaviest Functions:** `GetEmbyRecordingsAsync` (Impact: 39.3), `GetTimers` (Impact: 26.6), `AddRecordingInfo` (Impact: 23.3)

### 2. `src/Jellyfin.LiveTv/LiveTvMediaSourceProvider.cs` (CSHARP) -> Cumulative Risk: **683.57**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 258.74 | **LOC:** 333 | **CtrlFlow:** 13.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9992%), Concurrency (99.9986%), Cognitive Load (89.8907%)
- **Heaviest Functions:** `Normalize` (Impact: 45.8), `GetMediaSourcesInternal` (Impact: 23.1), `GetChannelStream` (Impact: 11.4)

### 3. `src/Jellyfin.LiveTv/DefaultLiveTvService.cs` (CSHARP) -> Cumulative Risk: **676.74**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 719.98 | **LOC:** 1000 | **CtrlFlow:** 13.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9975%), Concurrency (99.9219%), Documentation (96.6102%)
- **Heaviest Functions:** `UpdateTimersForSeriesTimer` (Impact: 38.6), `CopyProgramInfoToTimerInfo` (Impact: 23.4), `CreateTimer` (Impact: 21.4)

### 4. `MediaBrowser.Controller/MediaEncoding/JobLogger.cs` (CSHARP) -> Cumulative Risk: **674.81**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 119.0 | **LOC:** 164 | **CtrlFlow:** 20.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9599%), Concurrency (99.7527%)
- **Heaviest Functions:** `ParseLogLine` (Impact: 44.8), `StartStreamingLog` (Impact: 15.9), `JobLogger` (Impact: 1.6)

### 5. `MediaBrowser.Controller/Entities/TV/Series.cs` (CSHARP) -> Cumulative Risk: **670.84**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 310.72 | **LOC:** 529 | **CtrlFlow:** 11.7% | **Authorship Centralization:** 75.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9686%), Concurrency (99.4524%), Verification (80.0%)
- **Heaviest Functions:** `RefreshAllMetadata` (Impact: 25.2), `GetSeasonEpisodes` (Impact: 17.9), `FilterEpisodesBySeason` (Impact: 13.3)

### 6. `MediaBrowser.Providers/Plugins/Omdb/OmdbItemProvider.cs` (CSHARP) -> Cumulative Risk: **645.54**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 204.34 | **LOC:** 313 | **CtrlFlow:** 10.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9079%), State Flux (99.5361%)
- **Heaviest Functions:** `GetSearchResultsInternal` (Impact: 46.5), `ResultToMetadataResult` (Impact: 9.6), `GetResult` (Impact: 6.5)

### 7. `MediaBrowser.Providers/Plugins/Omdb/OmdbEpisodeProvider.cs` (CSHARP) -> Cumulative Risk: **641.08**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 42.32 | **LOC:** 80 | **CtrlFlow:** 4.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.8878%), State Flux (92.0451%)
- **Heaviest Functions:** `GetMetadata` (Impact: 8.5), `OmdbEpisodeProvider` (Impact: 2.7), `GetSearchResults` (Impact: 1.9)

### 8. `src/Jellyfin.LiveTv/TunerHosts/M3uParser.cs` (CSHARP) -> Cumulative Risk: **636.27**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 244.44 | **LOC:** 326 | **CtrlFlow:** 17.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%), Safety Score (90.6732%)
- **Heaviest Functions:** `GetChannelNumber` (Impact: 40.3), `GetChannelInfo` (Impact: 20.6), `GetChannelsAsync` (Impact: 19.8)

### 9. `Jellyfin.Server.Implementations/Activity/ActivityManager.cs` (CSHARP) -> Cumulative Risk: **629.06**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 138.12 | **LOC:** 214 | **CtrlFlow:** 14.6% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Concurrency (99.9818%), State Flux (99.9668%), Verification (80.0%)
- **Heaviest Functions:** `GetPagedResultAsync` (Impact: 29.6), `ApplyOrdering` (Impact: 13.6), `MapOrderBy` (Impact: 3.5)

### 10. `MediaBrowser.Providers/Plugins/AudioDb/AudioDbAlbumProvider.cs` (CSHARP) -> Cumulative Risk: **624.98**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 196.14 | **LOC:** 304 | **CtrlFlow:** 11.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.7254%), Concurrency (99.2911%), Verification (80.0%)
...
- **Risk Profile:** Cognitive Load (31.4072%), Tech Debt (12.0947%)
**Top Internal Functions/Classes:**
  * `GetVideoQualityParam` (Impact: 154.2)
    * *Intent:* /// <summary> /// Gets the video bitrate to specify on the command line. /// </summary> /// <param n...
  * `GetIntelQsvDx11VidFiltersPrefered` (Impact: 151.1)
  * `GetIntelQsvVaapiVidFiltersPrefered` (Impact: 142.9)
  * `GetRkmppVidFiltersPrefered` (Impact: 124.2)
  * `GetIntelVaapiFullVidFiltersPrefered` (Impact: 123.7)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 2168.5 | **LOC:** 2737 | **CtrlFlow:** 19.1% | **Authorship Centralization:** 41.0%
- **Risk Profile:** Cognitive Load (60.5564%), Tech Debt (38.0995%)
**Top Internal Functions/Classes:**
  * `TranslateQuery` (Impact: 425.3)
  * `Map` (Impact: 112.5)
  ...
  * `ApplyOrder` (Impact: 39.2)
  * `UpdateOrInsertItems` (Impact: 38.8)
...
- **Risk Profile:** Cognitive Load (45.4837%), Tech Debt (10.7383%)
**Top Internal Functions/Classes:**
  * `ApplyTranscodingConditions` (Impact: 532.2)
  * `BuildStreamVideoItem` (Impact: 244.6)
  * `BuildVideoItem` (Impact: 110.0)
  * `GetSubtitleProfile` (Impact: 71.8)
    * *Intent:* /// <summary> /// Normalizes input container. /// </summary> /// <param name="mediaSource">The <see ...
  * `GetOptimalAudioStream` (Impact: 69.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (38.5435%), Tech Debt (20.073%)
**Top Internal Functions/Classes:**
  * `GetSessions` (Impact: 54.0)
    * *Intent:* /// <inheritdoc/>
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1349.4 | **LOC:** 2689 | **CtrlFlow:** 14.0% | **Authorship Centralization:** 42.9%
- **Risk Profile:** Cognitive Load (24.6003%), Tech Debt (75.7351%)
**Top Internal Functions/Classes:**
  * `GetVersionInfo` (Impact: 40.6)
  * `GetMediaSourceName` (Impact: 30.3)
  * `RefreshMetadataForOwnedItem` (Impact: 25.5)
  * `AddImages` (Impact: 18.2)
    * *Intent:* /// <summary> /// Adds the images, updating metadata if they already are part of this item. /// </su...
  * `IsParentalAllowed` (Impact: 18.1)
...
- **Risk Profile:** Cognitive Load (69.7087%), Tech Debt (12.9627%)
**Top Internal Functions/Classes:**
  ...
  * `GetBaseItemDtoInternal` (Impact: 47.3)
  * `AttachPeople` (Impact: 41.1)
    * *Intent:* /// <summary> /// Attaches People DTO's to a DTOBaseItem. /// </summary> /// <param name="dto">The d...
  * `AttachUserSpecificInfo` (Impact: 34.0)
...
- **Risk Profile:** Cognitive Load (35.2797%), Tech Debt (9.0476%)
**Top Internal Functions/Classes:**
  * `GetMediaStream` (Impact: 200.7)
    * *Intent:* /// <summary> /// Converts ffprobe stream info to our MediaStream class. /// </summary> /// <param n...
  * `GetMediaInfo` (Impact: 123.8)
    * *Intent:* /// <summary> /// Transforms a FFprobe response into its <see cref="MediaInfo"/> equivalent. /// </s...
  * `SetAudioInfoFromTags` (Impact: 55.2)
  * `FetchWtvInfo` (Impact: 38.4)
  * `ReadFromDictNode` (Impact: 34.3)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1234.16 | **LOC:** 1483 | **CtrlFlow:** 16.9% | **Authorship Centralization:** 60.0%
- **Risk Profile:** Cognitive Load (69.6751%), Tech Debt (10.9772%)
**Top Internal Functions/Classes:**
  * `GetItem` (Impact: 129.5)
  * `Perform` (Impact: 72.8)
    * *Intent:* /// <inheritdoc/>
  * `GetMediaStream` (Impact: 62.9)
    * *Intent:* /// <summary> /// Gets the media stream. /// </summary> /// <param name="reader">The reader.</param>...
  * `ItemImageInfoFromValueString` (Impact: 26.4)
...
- **Risk Profile:** Cognitive Load (58.395%), Tech Debt (12.3114%)
**Top Internal Functions/Classes:**
  * `MergeBaseItemData` (Impact: 143.9)
  * `RefreshWithProviders` (Impact: 84.0)
  * `RefreshMetadata` (Impact: 68.3)
  * `ExecuteRemoteProviders` (Impact: 23.5)
  * `GetProviders` (Impact: 23.0)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 985.3 | **LOC:** 1882 | **CtrlFlow:** 17.1% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (19.7917%), Tech Debt (15.4932%)
**Top Internal Functions/Classes:**
  * `ValidateChildrenInternal2` (Impact: 100.5)
  * `AllowBoxSetCollapsing` (Impact: 58.3)
  * `RequiresPostFiltering` (Impact: 32.0)
  * `AddChildrenFromCollection` (Impact: 24.5)
  * `GetLinkedChildren` (Impact: 24.3)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 929.0 | **LOC:** 1293 | **CtrlFlow:** 12.7% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (75.732%), Tech Debt (44.8605%)
**Top Internal Functions/Classes:**
  * `GetEmbyRecordingsAsync` (Impact: 39.3)
  * `GetTimers` (Impact: 26.6)
  * `AddRecordingInfo` (Impact: 23.3)
  * `AddChannelInfo` (Impact: 20.9)
...
- **Risk Profile:** Cognitive Load (48.3056%), Tech Debt (37.6682%)
**Top Internal Functions/Classes:**
  * `GetChannelItemEntityAsync` (Impact: 118.6)
  * `GetChannelItems` (Impact: 33.4)
  * `GetChannelsInternalAsync` (Impact: 26.6)
    * *Intent:* /// <inheritdoc />
  * `GetChannelItemsInternal` (Impact: 23.6)
    * *Intent:* /// <inheritdoc />
  * `GetChannelDataCachePath` (Impact: 19.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 10 instances
...
- **Risk Profile:** Cognitive Load (32.5641%), Tech Debt (41.1895%)
**Top Internal Functions/Classes:**
  * `ExtractImageInternal` (Impact: 87.5)
  * `ExtractVideoImagesOnIntervalAccelerated` (Impact: 75.7)
    * *Intent:* /// <inheritdoc />
  * `ExtractVideoImagesOnIntervalInternal` (Impact: 56.0)
  * `GetMediaInfoInternal` (Impact: 34.5)
    * *Intent:* /// <summary> /// Gets the media info internal. /// </summary> /// <returns>Task{MediaInfoResult}.</...
  * `SetFFmpegPath` (Impact: 24.3)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 719.98 | **LOC:** 1000 | **CtrlFlow:** 13.8% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (81.4172%), Tech Debt (34.8296%)
**Top Internal Functions/Classes:**
  * `UpdateTimersForSeriesTimer` (Impact: 38.6)
  * `CopyProgramInfoToTimerInfo` (Impact: 23.4)
  * `CreateTimer` (Impact: 21.4)
  * `OnTimerManagerTimerFired` (Impact: 20.2)
  * `CreateTimer` (Impact: 18.6)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 9 instances
* *Amplified Cascading Flux:* 59 instances
...
- **Risk Profile:** Cognitive Load (46.1578%), Tech Debt (57.6872%)
**Top Internal Functions/Classes:**
  * `AuthenticateUser` (Impact: 77.2)
    * *Intent:* /// <inheritdoc/>
  * `GetUserDto` (Impact: 23.2)
    * *Intent:* /// <inheritdoc/>
  * `UpdatePolicyAsync` (Impact: 12.5)
    * *Intent:* /// <inheritdoc/>
  * `RenameUser` (Impact: 10.6)
    * *Intent:* /// <inheritdoc/>
  * `GetAuthenticationProviders` (Impact: 9.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (21.3435%), Tech Debt (8.9596%)
**Top Internal Functions/Classes:**
  ...
  * `FetchFromTagsNode` (Impact: 19.1)
  * `FetchFromTaglinesNode` (Impact: 18.9)
...
- **Risk Profile:** Cognitive Load (19.6226%), Tech Debt (67.8044%)
**Top Internal Functions/Classes:**
  * `IsSaverEnabledForItem` (Impact: 32.2)
    * *Intent:* /// <summary> /// Determines whether [is saver enabled for item] [the specified saver]. /// </summar...
  * `GetRemoteSearchResults` (Impact: 28.4)
  * `SaveImage` (Impact: 23.0)
    * *Intent:* /// <inheritdoc/>
  * `CanRefreshMetadata` (Impact: 21.1)
  * `SaveMetadataAsync` (Impact: 16.8)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 577.5 | **LOC:** 1028 | **CtrlFlow:** 17.2% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (41.2339%), Tech Debt (9.7622%)
**Top Internal Functions/Classes:**
  * `Filter` (Impact: 263.7)
  * `GetUserItems` (Impact: 53.2)
  * `SortAndPage` (Impact: 17.1)
  * `GetMediaFolders` (Impact: 11.4)
  * `GetTvView` (Impact: 7.7)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (22.0077%), Tech Debt (10.8789%)
**Top Internal Functions/Classes:**
  * `GetBindAddress` (Impact: 31.9)
    * *Intent:* /// <inheritdoc/>
  * `InitializeOverrides` (Impact: 29.7)
  ...
  * `MatchesPublishedServerUrl` (Impact: 25.1)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 571.0 | **LOC:** 1054 | **CtrlFlow:** 12.4% | **Authorship Centralization:** 55.6%
- **Risk Profile:** Cognitive Load (27.9427%), Tech Debt (9.6762%)
**Top Internal Functions/Classes:**
  * `ConvertTextSubtitleToSrtInternal` (Impact: 37.1)
  ...
  * `ExtractTextSubtitleInternal` (Impact: 32.7)
  * `ExtractSubtitlesForFile` (Impact: 32.3)
...
- `Jellyfin.Server/Migrations/Routines/MigrateLibraryDb.cs` -> Churn: **73.55%** | Cog Load: 69.6751% | Debt: 10.9772%
- `MediaBrowser.Controller/Entities/BaseItem.cs` -> Churn: **73.46%** | Cog Load: 24.6003% | Debt: 75.7351%
