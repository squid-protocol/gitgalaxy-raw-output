...
| Avg Path Length | 6.0906 | Hops between files. Lower = Tighter coupling. |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 7.5 | 0.8 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 71.6 | 2.3 | 0.0 | 0.0 |
...
- `nullishTypeTest` (@ `packages/eslint-plugin/tests/rules/prefer-nullish-coalescing.test.ts`) -> Impact: **2152.8** | LOC: 6315
- `unnecessaryConditionTest` (@ `packages/eslint-plugin/tests/rules/no-unnecessary-condition.test.ts`) -> Impact: **1174.5** | LOC: 3813
- `convertNode` (@ `packages/typescript-estree/src/convert.ts`) -> Impact: **464.3** | LOC: 1457
...
- `isValidEscape` (@ `packages/typescript-estree/src/convert.ts`) -> Impact: **418.9** | LOC: 1590
- `create` (@ `packages/eslint-plugin/src/rules/no-unused-vars.ts`) -> Impact: **364.4** | LOC: 1052
- `checkSyntaxError` (@ `packages/typescript-estree/src/check-syntax-errors.ts`) -> Impact: **358.9** | LOC: 618
- `create` (@ `packages/eslint-plugin/src/rules/no-unnecessary-condition.ts`) -> Impact: **258.4** | LOC: 699
- `create` (@ `packages/eslint-plugin/src/rules/no-misused-promises.ts`) -> Impact: **258.2** | LOC: 523
- `create` (@ `packages/eslint-plugin/src/rules/consistent-type-imports.ts`) -> Impact: **250.6** | LOC: 855
- `create` (@ `packages/eslint-plugin/src/rules/strict-boolean-expressions.ts`) -> Impact: **246.2** | LOC: 872
...
### 1. `packages/website/src/components/editor/createProvideTwoslashInlay.ts` (TYPESCRIPT) -> Cumulative Risk: **660.7**
...
- **Heaviest Functions:** `createTwoslashInlayProvider` (Impact: 16.6), `resolveInlayHint` (Impact: 8.9), `provideInlayHints` (Impact: 6.4)
...
- **Heaviest Functions:** `normalizeArray` (Impact: 16.8), `relative` (Impact: 16.0), `resolve` (Impact: 14.2)

### 3. `packages/website-eslint/src/index.js` (JAVASCRIPT) -> Cumulative Risk: **578.98**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `createLinter` (Impact: 1.1)

### 4. `packages/website/src/components/editor/loadSandbox.ts` (TYPESCRIPT) -> Cumulative Risk: **568.61**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `loadSandbox` (Impact: 5.1), `onload` (Impact: 4.2), `sandboxSingleton` (Impact: 3.1)

### 5. `packages/eslint-plugin/src/rules/naming-convention.ts` (TYPESCRIPT) -> Cumulative Risk: **568.12**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `create` (Impact: 136.3), `isExported` (Impact: 25.6), `getMemberModifiers` (Impact: 21.6)
...
- **Heaviest Functions:** `maybeInvalidateProgram` (Impact: 36.5), `getWatchProgramsForProjects` (Impact: 31.1), `createWatchProgram` (Impact: 17.3)

### 7. `packages/website/src/hooks/useDebouncedToggle.ts` (TYPESCRIPT) -> Cumulative Risk: **565.1**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `useDebouncedToggle` (Impact: 4.7)

### 8. `packages/integration-tests/tools/integration-test-base.ts` (TYPESCRIPT) -> Cumulative Risk: **556.59**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `eslintIntegrationTest` (Impact: 15.3), `typescriptIntegrationTest` (Impact: 10.7), `integrationTest` (Impact: 2.9)

### 9. `packages/scope-manager/src/scope/ScopeBase.ts` (TYPESCRIPT) -> Cumulative Risk: **552.74**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `isStrictScope` (Impact: 61.8), `defineVariable` (Impact: 16.1), `constructor` (Impact: 11.4)

### 10. `packages/eslint-plugin/src/rules/prefer-optional-chain-utils/gatherLogicalOperands.ts` (TYPESCRIPT) -> Cumulative Risk: **550.7**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `gatherLogicalOperands` (Impact: 162.6), `isValidFalseBooleanCheckType` (Impact: 38.8), `getComparisonValueType` (Impact: 12.5)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 7071.87 | **LOC:** 1445 | **CtrlFlow:** 5.0% | **Authorship Centralization:** 66.7%
...
- `packages/website-eslint/src/index.js` -> **Severity: 0.031** (Bridge: 0.0003 * Flux: 100.0%)
- `packages/typescript-estree/src/ast-converter.ts` -> **Severity: 0.017** (Bridge: 0.0002 * Flux: 91.6827%)
- `packages/rule-tester/src/RuleTester.ts` -> **Severity: 0.009** (Bridge: 0.0001 * Flux: 64.9481%)
- `packages/website/src/components/Playground.tsx` -> **Severity: 0.005** (Bridge: 0.0003 * Flux: 17.1974%)
