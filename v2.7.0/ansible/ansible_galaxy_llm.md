...
| Analyzed Artifacts (Scanned) | 4688 |
| Excluded Artifacts (Unparsable data, binaries, unsupported formats) | 1134 |
| Total LOC | 230064 |
...
|---|---|---|
| Modularity | 0.5565 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 3.2015 | Hops between files. Lower = Tighter coupling. |
| Articulation Pts | 120 | Number of single files that, if removed, shatter the network. |

## 4. COMPOSITION
| Lang | Files | LOC | Share |
|---|---|---|---|
| YAML | 2069 | 53727 | 44.1% |
| PYTHON | 1766 | 147365 | 37.7% |
| SHELL | 252 | 4726 | 5.4% |
| POWERSHELL | 192 | 15138 | 4.1% |
| PLAINTEXT | 181 | 12 | 3.9% |
| JSON | 148 | 3200 | 3.2% |
| MARKDOWN | 35 | 0 | 0.7% |
| CSHARP | 17 | 5772 | 0.4% |
| M4 | 17 | 40 | 0.4% |
...
|---|---|---|
| Unclassified | 4470 | 95.3% |
| Unknown | 13 | 0.3% |

### Inert Structural Mass (Static Categories)
| Category | Count | Repo % |
|---|---|---|
| Static: Literature & Documentation | 204 | 4.4% |
| Static: Minified & Vendor Opaque Mass | 1 | 0.0% |

## 5. EXCLUDED ARTIFACTS (Unparsable or Shielded Files)
*Total Excluded Artifacts: 1134*

**Composition by Extension & Reason:**
- `no_extension`: 541x Unsupported Format (.undeterminable), 32x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 3x Unresolved Ambiguity (No Retainable Structure)
- `.j2`: 130x Excluded (Unsupported Extension: '.j2')
- `.yml`: 14x Excluded (System Exclusion, Hidden Directory, or Dynamic Ignored Dir), 5x Zero-Density Threshold (LOC: 54, Signals: 0), 5x Zero-Density Threshold (LOC: 52, Signals: 0)
...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 27.5 | 0.0 | 0.0 |
...
| Mutation Surface (formerly State Flux Exposure) | 0.0 | 100.0 | 12.4 | 0.0 | 0.0 |
| Dead Code Surface (formerly Commented Logic Exposure) | 0.0 | 89.2 | 0.6 | 0.0 | 0.0 |
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 34.0 | 0.0 | 0.0 |
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 5.0 | 0.2 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 68.3 | 2.3 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 24.7 | 0.0 | 0.0 |
...
| danger | 7311 | 995 | 3 | `test/integration/targets/module_utils_Ansible.Basic/library/ansible_basic_tests.ps1` |
| concurrency | 621 | 209 | 0 | `test/integration/targets/ansible-galaxy/runme.sh` |
| connectivity | 11441 | 1511 | 5 | `test/support/windows-integration/plugins/module_utils/Ansible.Service.cs` |
...
| docs | 7957 | 1470 | 4 | `test/lib/ansible_test/_internal/host_profiles.py` |
| debt | 1941 | 630 | 1 | `test/integration/targets/ansible-vault/runme.sh` |
| mutation | 83822 | 2000 | 36 | `lib/ansible/modules/user.py` |
| dead_code | 3476 | 856 | 1 | `test/units/parsing/vault/test_vault.py` |
| credential | 129 | 26 | 0 | `test/integration/targets/ansible-vault/password-script.py` |
...
- `_validate_argument_spec` (@ `test/lib/ansible_test/_util/controller/sanity/validate-modules/validate_modules/main.py`) -> Impact: **440.3** | LOC: 604
- `run_command` (@ `lib/ansible/module_utils/basic.py`) -> Impact: **339.0** | LOC: 273
- `Add-CSharpType` (@ `lib/ansible/module_utils/powershell/Ansible.ModuleUtils.AddType.psm1`) -> Impact: **277.1** | LOC: 450
...
- `present` (@ `lib/ansible/modules/lineinfile.py`) -> Impact: **273.8** | LOC: 211
- `get_virtual_facts` (@ `lib/ansible/module_utils/facts/virtual/linux.py`) -> Impact: **215.1** | LOC: 371
  * *Intent:* # For more information, check: http://people.redhat.com/~rjones/virt-what/
- `get_config_value_and_origin` (@ `lib/ansible/config/manager.py`) -> Impact: **211.8** | LOC: 155
  * *Intent:* """ Given a config key figure out the actual value and report on the origin of the settings """
- `_process_pending_results` (@ `lib/ansible/plugins/strategy/__init__.py`) -> Impact: **204.5** | LOC: 244
...
- `install` (@ `lib/ansible/modules/apt.py`) -> Impact: **200.1** | LOC: 126
- `get_vars` (@ `lib/ansible/vars/manager.py`) -> Impact: **194.5** | LOC: 286
- `_ensure_impl` (@ `lib/ansible/module_utils/_embed/dnf.py`) -> Impact: **193.5** | LOC: 270
...
| `lib/ansible/playbook` | 20 | 4395.46 | 49.31% | 14.93% |
...
- `test/support/windows-integration/plugins/modules/win_certificate_store.ps1` -> **98.8911%** Exposure
...
### 1. `lib/ansible/module_utils/facts/system/service_mgr.py` (PYTHON) -> Cumulative Risk: **763.18**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 181.56 | **LOC:** 155 | **CtrlFlow:** 45.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Cognitive Load (99.7252%)
- **Heaviest Functions:** `collect` (Impact: 72.5), `is_systemd_managed` (Impact: 6.2), `is_systemd_managed_offline` (Impact: 6.1)

### 2. `lib/ansible/executor/task_queue_manager.py` (PYTHON) -> Cumulative Risk: **739.74**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 408.54 | **LOC:** 519 | **CtrlFlow:** 21.7% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Concurrency (97.4628%), Safety Score (94.4909%)
- **Heaviest Functions:** `load_callbacks` (Impact: 32.4), `send_callback` (Impact: 31.4), `__init__` (Impact: 25.1)

### 3. `lib/ansible/module_utils/facts/system/distribution.py` (PYTHON) -> Cumulative Risk: **715.53**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1162.76 | **LOC:** 743 | **CtrlFlow:** 29.3% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.9527%)
- **Heaviest Functions:** `parse_distribution_file_Debian` (Impact: 95.0), `parse_distribution_file_SUSE` (Impact: 71.9), `parse_distribution_file_Amazon` (Impact: 28.1)

### 4. `lib/ansible/utils/collection_loader/_collection_finder.py` (PYTHON) -> Cumulative Risk: **707.57**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1272.86 | **LOC:** 1275 | **CtrlFlow:** 29.7% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (99.2196%), Tech Debt (97.8476%)
- **Heaviest Functions:** `__init__` (Impact: 37.1), `_get_loader` (Impact: 30.0), `_iter_modules_impl` (Impact: 22.0)

### 5. `lib/ansible/errors/__init__.py` (PYTHON) -> Cumulative Risk: **697.53**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 278.62 | **LOC:** 483 | **CtrlFlow:** 12.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Api Exposure (100.0%), Spec Match (100.0%), State Flux (99.9999%), Tech Debt (99.1913%)
- **Heaviest Functions:** `__init__` (Impact: 30.5), `__init__` (Impact: 28.1), `from_value` (Impact: 7.2)

### 6. `lib/ansible/module_utils/urls.py` (PYTHON) -> Cumulative Risk: **695.28**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1315.38 | **LOC:** 1409 | **CtrlFlow:** 20.7% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Safety Score (98.8115%), Verification (80.0%)
- **Heaviest Functions:** `open` (Impact: 106.1), `fetch_url` (Impact: 62.7), `redirect_request` (Impact: 62.4)

### 7. `lib/ansible/galaxy/dependency_resolution/providers.py` (PYTHON) -> Cumulative Risk: **689.89**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 262.7 | **LOC:** 453 | **CtrlFlow:** 28.7% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9999%), Tech Debt (99.6862%)
- **Heaviest Functions:** `_find_matches` (Impact: 90.3), `find_matches` (Impact: 19.1), `get_dependencies` (Impact: 12.2)

### 8. `lib/ansible/module_utils/facts/network/sunos.py` (PYTHON) -> Cumulative Risk: **688.94**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 115.4 | **LOC:** 116 | **CtrlFlow:** 32.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.3653%)
- **Heaviest Functions:** `get_interfaces_info` (Impact: 35.3), `parse_interface_line` (Impact: 11.9), `parse_ether_line` (Impact: 4.8)

### 9. `lib/ansible/_internal/_wrapt.py` (PYTHON) -> Cumulative Risk: **687.85**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 530.2 | **LOC:** 857 | **CtrlFlow:** 13.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.5971%), Documentation (99.2032%), Cognitive Load (93.8131%)
- **Heaviest Functions:** `__init__` (Impact: 34.4), `__call__` (Impact: 26.0), `__get__` (Impact: 17.1)

### 10. `lib/ansible/module_utils/compat/version.py` (PYTHON) -> Cumulative Risk: **682.09**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 191.12 | **LOC:** 343 | **CtrlFlow:** 38.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.4824%)
- **Heaviest Functions:** `_cmp` (Impact: 32.9), `parse` (Impact: 18.2), `_cmp` (Impact: 11.0)
...
- **Risk Profile:** Cognitive Load (72.6676%), Tech Debt (15.3456%)
**Top Internal Functions/Classes:**
  * `modify_user_usermod` (Impact: 107.2)
  * `create_user_useradd` (Impact: 80.9)
  * `modify_user` (Impact: 76.3)
  * `modify_user_usermod` (Impact: 65.0)
  * `modify_user` (Impact: 61.4)
...
- **Risk Profile:** Cognitive Load (93.0006%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_validate_argument_spec` (Impact: 440.3)
  * `_validate_docs` (Impact: 97.8)
  * `_validate_required_if` (Impact: 76.5)
  * `validate` (Impact: 63.9)
  * `_ensure_imports_below_docs` (Impact: 49.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
- **Risk Profile:** Cognitive Load (75.6407%), Tech Debt (9.4015%)
**Top Internal Functions/Classes:**
  * `run_command` (Impact: 339.0)
  * `atomic_move` (Impact: 81.8)
    * *Intent:* """atomically move src to dest, copying attributes from dest, returns true on success it uses os.ren...
  * `set_mode_if_different` (Impact: 62.4)
  * `_get_octal_mode_from_symbolic_perms` (Impact: 62.1)
  * `__init__` (Impact: 46.6)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002879 | `Ripple Effect (Closeness):` 0.0
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000572 | `Ripple Effect (Closeness):` 0.0
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1859.84 | **LOC:** 1890 | **CtrlFlow:** 28.1% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (64.9921%), Tech Debt (8.3737%)
**Top Internal Functions/Classes:**
  * `add_install_options` (Impact: 87.8)
  * `_parse_requirements_file` (Impact: 66.4)
    * *Intent:* """ Parses an Ansible requirement.yml file and returns all the roles and/or collections defined in i...
  * `execute_init` (Impact: 66.3)
    * *Intent:* """ Creates the skeleton framework of a role or collection that complies with the Galaxy metadata fo...
  * `_execute_install_role` (Impact: 64.7)
  * `execute_install` (Impact: 54.6)
...
- **Risk Profile:** Cognitive Load (63.8939%), Tech Debt (27.7002%)
**Top Internal Functions/Classes:**
  * `install_collections` (Impact: 145.5)
    * *Intent:* % (api.name, api.api_server, import_uri))
  * `_resolve_depenency_map` (Impact: 110.8)
  * `verify_local_collection` (Impact: 80.7)
    * *Intent:* # type: (Candidate, t.Optional[Candidate], ConcreteArtifactsManager) -> CollectionVerifyResult """Ve...
  * `verify_collections` (Impact: 67.7)
  * `verify_file_signatures` (Impact: 58.0)
...
- **Risk Profile:** Cognitive Load (73.5914%), Tech Debt (29.5345%)
**Top Internal Functions/Classes:**
  * `service_enable` (Impact: 123.8)
  * `execute_command` (Impact: 48.4)
    * *Intent:* # =========================================== # Generic methods that should be used on all platforms...
  * `get_service_status` (Impact: 42.6)
  * `service_control` (Impact: 40.1)
    * *Intent:* # Decide what command to run svc_cmd = '' arguments = self.arguments if self.svc_cmd: if not self.sv...
  * `get_service_tools` (Impact: 36.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Rce:* 4 instances
...
- **Risk Profile:** Cognitive Load (59.4204%), Tech Debt (30.691%)
**Top Internal Functions/Classes:**
  * `_find_module_utils` (Impact: 144.6)
  * `recursive_finder` (Impact: 98.4)
  * `modify_module` (Impact: 60.3)
  * `visit_ImportFrom` (Impact: 43.6)
    * *Intent:* """ Handle from ansible.module_utils.MODLIB import [.MODLIBn] [as asname] Also has to handle relativ...
  * `_get_shebang` (Impact: 43.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
* *Amplified Rce:* 1 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000589 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (68.1642%), Tech Debt (9.2754%)
**Top Internal Functions/Classes:**
  * `install` (Impact: 200.1)
  * `upgrade` (Impact: 137.6)
  * `install_deb` (Impact: 102.1)
  * `main` (Impact: 81.8)
  * `remove` (Impact: 63.0)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (58.2399%), Tech Debt (15.8021%)
**Top Internal Functions/Classes:**
  * `clone` (Impact: 114.8)
  * `main` (Impact: 78.3)
    * *Intent:* # ===========================================
  ...
  * `switch_version` (Impact: 47.0)
  * `get_remote_head` (Impact: 44.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (53.888%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `command_sanity` (Impact: 117.7)
    * *Intent:* """Run sanity tests."""
  * `__init__` (Impact: 100.8)
  * `test_script` (Impact: 55.9)
    * *Intent:* """Run the sanity test and return the result."""
  * `filter_targets` (Impact: 49.9)
    * *Intent:* """Return the given list of test targets, filtered to include only those relevant for the test."""
  * `create_sanity_virtualenv` (Impact: 35.2)
**Contextual Mitigations & Amplifications:**
...
- `lib/ansible/plugins/action/__init__.py` -> Churn: **68.3%** | Cog Load: 61.2559% | Debt: 23.7598%
- `lib/ansible/modules/user.py` -> Churn: **63.42%** | Cog Load: 72.6676% | Debt: 15.3456%
- `lib/ansible/executor/module_common.py` -> Churn: **58.85%** | Cog Load: 59.4204% | Debt: 30.691%
- `lib/ansible/cli/galaxy.py` -> Churn: **52.86%** | Cog Load: 64.9921% | Debt: 8.3737%
- `lib/ansible/modules/apt.py` -> Churn: **52.86%** | Cog Load: 68.1642% | Debt: 9.2754%
...
- `lib/ansible/module_utils/basic.py` -> **Severity: 0.288** (Bridge: 0.0029 * Flux: 100.0%)
- `lib/ansible/galaxy/dependency_resolution/dataclasses.py` -> **Severity: 0.254** (Bridge: 0.0025 * Flux: 99.9997%)
- `lib/ansible/utils/display.py` -> **Severity: 0.183** (Bridge: 0.0018 * Flux: 100.0%)
- `lib/ansible/playbook/task.py` -> **Severity: 0.132** (Bridge: 0.0013 * Flux: 100.0%)
