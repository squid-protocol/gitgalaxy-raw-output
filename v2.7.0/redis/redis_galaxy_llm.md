...
|---|---|---|
| Modularity | 0.6969 | High = Clean micro-boundaries. Low = Spaghetti coupling. |
...
| Avg Path Length | 3.9189 | Hops between files. Lower = Tighter coupling. |
...
- `.tcl`: 4x Unsupported Format (.undeterminable), 1x Excluded (Machine-Generated Source Code Signature: 161 LOC), 1x Excluded (Machine-Generated Source Code Signature: 3920 LOC)
...
|---|---|---|---|---|---|
| Complexity Load (formerly Cognitive Load Exposure) | 0.0 | 100.0 | 20.6 | 6.5 | 0.0 |
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 41.8 | 47.9 | 0.0 |
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 7.5 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 11.2 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 12.1 | 0.7 | 0.0 |
| Concurrency Surface (formerly Concurrency Exposure) | 0.0 | 100.0 | 6.3 | 0.0 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 65.4 | 2.7 | 0.0 | 0.0 |
...
| cleanup | 1162 | 228 | 1 | `tests/unit/type/list.tcl` |
| guards | 23550 | 877 | 37 | `deps/jemalloc/src/ctl.c` |
| danger | 4671 | 502 | 7 | `src/redis-cli.c` |
| concurrency | 1296 | 185 | 1 | `tests/unit/type/stream-cgroups.tcl` |
| connectivity | 13672 | 821 | 19 | `src/server.h` |
| io | 1796 | 205 | 1 | `deps/lua/doc/contents.html` |
| crypto | 0 | 0 | 0 | - |
...
| tests | 833 | 115 | 0 | `tests/unit/functions.tcl` |
| docs | 822 | 111 | 0 | `deps/xxhash/xxhash.h` |
| debt | 2775 | 312 | 3 | `src/redis-cli.c` |
| mutation | 77675 | 942 | 110 | `src/redis-cli.c` |
| dead_code | 2919 | 517 | 3 | `src/module.c` |
| credential | 5 | 4 | 0 | `deps/lua/doc/contents.html` |
| threat | 2895 | 332 | 3 | `deps/jemalloc/src/ctl.c` |
| ml_ai | 1406 | 207 | 1 | `tests/unit/type/hash-field-expire.tcl` |
| ui | 124 | 12 | 0 | `deps/lua/doc/lua.html` |

**Relations (repo medians):**
- `guard_balance_ratio` (guards / (danger + 1)): **0.4669**
...
- `rdbLoadObject` (@ `src/rdb.c`) -> Impact: **795.6** | LOC: 1313
...
- `parseOptions` (@ `src/redis-cli.c`) -> Impact: **583.8** | LOC: 418
  * *Intent:* /*------------------------------------------------------------------------------ * User interface *---------------------------------------------------...
- `parser_tokenize` (@ `deps/jemalloc/test/unit/stats_print.c`) -> Impact: **562.4** | LOC: 557
- `debugCommand` (@ `src/debug.c`) -> Impact: **492.6** | LOC: 772
  * *Intent:* #endif
- `quicklistTest` (@ `src/quicklist.c`) -> Impact: **420.0** | LOC: 1240
  * *Intent:* /* main test, but callable from other files */
- `malloc_conf_init_helper` (@ `deps/jemalloc/src/jemalloc.c`) -> Impact: **373.8** | LOC: 666
- `clientCommand` (@ `src/networking.c`) -> Impact: **343.4** | LOC: 560
- `genRedisInfoString` (@ `src/server.c`) -> Impact: **324.9** | LOC: 739
...
- `crashlog_from_file` (@ `tests/support/util.tcl`) -> Impact: **306.7** | LOC: 1241
...
- `rdbLoadRioWithLoadingCtx` (@ `src/rdb.c`) -> Impact: **301.6** | LOC: 397
...
|---|---|---|---|---|
| `src` | 201 | 143265.84 | 47.83% | 26.78% |
| `deps/jemalloc/src` | 65 | 19370.7 | 36.35% | 54.91% |
| `deps/lua/src` | 64 | 14045.76 | 50.93% | 5.34% |
| `src/commands` | 424 | 11856.52 | 0.0% | 0.0% |
| `tests/unit` | 47 | 10424.48 | 28.18% | 0.0% |
| `deps/jemalloc/test/unit` | 134 | 10394.4 | 14.4% | 0.0% |
| `tests/modules` | 46 | 8503.66 | 18.72% | 0.0% |
| `deps/hiredis` | 35 | 8154.76 | 31.02% | 15.16% |
...
### 1. `src/module.c` (C) -> Cumulative Risk: **762.37**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 9521.32 | **LOC:** 15586 | **CtrlFlow:** 24.8% | **Authorship Centralization:** 29.6%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Tech Debt (99.5432%), Documentation (98.4496%)
- **Heaviest Functions:** `RM_Call` (Impact: 208.4), `moduleCreateArgvFromUserFormat` (Impact: 134.2), `RM_SetCommandInfo` (Impact: 72.7)

### 2. `src/db.c` (C) -> Cumulative Risk: **751.19**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 3182.86 | **LOC:** 3915 | **CtrlFlow:** 29.9% | **Authorship Centralization:** 19.4%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (99.6241%), Safety Score (95.2608%)
- **Heaviest Functions:** `scanGenericCommand` (Impact: 185.1), `getKeysUsingKeySpecs` (Impact: 103.7), `dbSetValue` (Impact: 100.2)

### 3. `src/t_string.c` (C) -> Cumulative Risk: **733.99**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1342.08 | **LOC:** 1241 | **CtrlFlow:** 43.0% | **Authorship Centralization:** 30.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Cognitive Load (97.6856%)
- **Heaviest Functions:** `parseExtendedStringArgumentsOrReply` (Impact: 281.5), `setGenericCommand` (Impact: 184.4), `lcsCommand` (Impact: 105.4)

### 4. `src/bio.c` (C) -> Cumulative Risk: **733.85**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 255.2 | **LOC:** 446 | **CtrlFlow:** 18.6% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9891%), Concurrency (97.4043%)
- **Heaviest Functions:** `bioProcessBackgroundJobs` (Impact: 48.1), `bioCreateCompRq` (Impact: 14.5), `bioInit` (Impact: 12.7)

### 5. `src/server.c` (C) -> Cumulative Risk: **698.93**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 6083.78 | **LOC:** 8171 | **CtrlFlow:** 25.7% | **Authorship Centralization:** 21.1%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (95.6665%)
- **Heaviest Functions:** `genRedisInfoString` (Impact: 324.9), `processCommand` (Impact: 212.8), `main` (Impact: 187.1)

### 6. `src/object.c` (C) -> Cumulative Risk: **698.25**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1414.62 | **LOC:** 1822 | **CtrlFlow:** 28.4% | **Authorship Centralization:** 27.3%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9996%), Safety Score (89.3471%)
- **Heaviest Functions:** `memoryCommand` (Impact: 58.6), `getMemoryDoctorReport` (Impact: 42.5), `objectCommand` (Impact: 33.7)

### 7. `src/cluster.c` (C) -> Cumulative Risk: **692.7**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1966.68 | **LOC:** 2348 | **CtrlFlow:** 33.4% | **Authorship Centralization:** 21.1%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Cognitive Load (95.4721%)
- **Heaviest Functions:** `getNodeByQuery` (Impact: 268.8), `migrateCommand` (Impact: 123.0), `clusterCommand` (Impact: 86.7)

### 8. `src/adlist.c` (C) -> Cumulative Risk: **688.58**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 419.04 | **LOC:** 396 | **CtrlFlow:** 19.8% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.685%)
- **Heaviest Functions:** `listInsertNode` (Impact: 19.3), `listDup` (Impact: 13.1), `listIndex` (Impact: 12.8)

### 9. `src/t_stream.c` (C) -> Cumulative Risk: **687.0**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 4651.9 | **LOC:** 5934 | **CtrlFlow:** 28.7% | **Authorship Centralization:** 29.2%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (94.9949%)
- **Heaviest Functions:** `streamReplyWithRange` (Impact: 214.9), `streamParseAddOrTrimArgsOrReply` (Impact: 171.4), `xreadCommand` (Impact: 158.4)

### 10. `src/blocked.c` (C) -> Cumulative Risk: **682.85**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 503.82 | **LOC:** 796 | **CtrlFlow:** 29.0% | **Authorship Centralization:** 40.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (92.7639%)
- **Heaviest Functions:** `unblockClient` (Impact: 36.2), `blockForKeys` (Impact: 29.1), `signalKeyAsReadyLogic` (Impact: 24.8)
...
- **Magnitude:** 12117.2 | **LOC:** 11143 | **CtrlFlow:** 30.2% | **Authorship Centralization:** 50.0%
- **Risk Profile:** Cognitive Load (81.2847%), Tech Debt (7.9945%)
**Top Internal Functions/Classes:**
  * `parseOptions` (Impact: 583.8)
    * *Intent:* /*------------------------------------------------------------------------------ * User interface *-...
  * `cliSendCommand` (Impact: 238.2)
  * `clusterManagerCommandCreate` (Impact: 149.8)
    * *Intent:* /* Cluster Manager Commands */
  * `clusterManagerFixOpenSlot` (Impact: 141.0)
    * *Intent:* /* Slot 'slot' was found to be in importing or migrating state in one or * more nodes. This function...
  * `clusterManagerNodeLoadInfo` (Impact: 130.6)
...
* *Defense:* None
* *Network Topology:*
  * `Ecosystem Role:` Isolated/Orphan | `Dependency Blast Radius (PageRank):` 0.353
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (94.7439%), Tech Debt (34.3992%)
**Top Internal Functions/Classes:**
  * `clusterCommandSpecial` (Impact: 242.6)
  * `clusterProcessPacket` (Impact: 231.2)
    * *Intent:* /* When this function is called, there is a packet to process starting * at link->rcvbuf. Releasing ...
  * `clusterLoadConfig` (Impact: 162.2)
    * *Intent:* /* ----------------------------------------------------------------------------- * Initialization * ...
  * `clusterUpdateSlotsConfigWith` (Impact: 80.3)
    * *Intent:* /* This function is called when we receive a master configuration via a * PING, PONG or UPDATE packe...
  * `clusterGenNodeDescription` (Impact: 72.8)
...
- **Risk Profile:** Cognitive Load (95.4116%), Tech Debt (12.0952%)
**Top Internal Functions/Classes:**
  * `sentinelCommand` (Impact: 200.0)
  * `sentinelHandleConfiguration` (Impact: 195.4)
  * `sentinelRefreshInstanceInfo` (Impact: 158.3)
    * *Intent:* /* Process the INFO output from masters. */
  * `sentinelSetDebugConfigParameters` (Impact: 107.6)
  * `sentinelSetCommand` (Impact: 104.8)
...
- **Risk Profile:** Cognitive Load (84.0079%), Tech Debt (27.5545%)
**Top Internal Functions/Classes:**
  * `malloc_conf_init_helper` (Impact: 373.8)
  * `malloc_conf_next` (Impact: 190.0)
  * `batch_alloc` (Impact: 79.8)
  * `imalloc_body` (Impact: 78.8)
  * `isfree` (Impact: 52.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (54.129%), Tech Debt (26.4504%)
**Top Internal Functions/Classes:**
  * `loadServerConfigFromString` (Impact: 90.8)
  * `configSetCommand` (Impact: 56.6)
    * *Intent:* /*----------------------------------------------------------------------------- * CONFIG SET impleme...
  * `rewriteConfigReadOldFile` (Impact: 47.7)
    * *Intent:* /* Read the old file, split it into lines to populate a newly created * config rewrite state, and re...
  * `numericParseString` (Impact: 46.9)
  * `setNumericType` (Impact: 45.3)
...
- **Risk Profile:** Cognitive Load (82.3649%), Tech Debt (41.7384%)
**Top Internal Functions/Classes:**
  * `clusterSyncSlotsCommand` (Impact: 171.5)
  * `slotSnapshotSaveRio` (Impact: 67.2)
    * *Intent:* /* Save the slot ranges snapshot to the file. It generates the DUMP encoded * representation of each...
  * `asmSyncWithSource` (Impact: 49.0)
  * `asmTaskStateToString` (Impact: 43.0)
  * `asmReplicaHandleMasterTask` (Impact: 40.2)
...
- **Risk Profile:** Cognitive Load (62.9023%), Tech Debt (42.0086%)
**Top Internal Functions/Classes:**
  * `ctl_lookup` (Impact: 52.1)
  * `ctl_arena_stats_sdmerge` (Impact: 50.1)
  * `arena_i_dss_ctl` (Impact: 39.8)
  * `experimental_arenas_i_pactivep_ctl` (Impact: 35.7)
  * `arena_i_decay_ms_ctl_impl` (Impact: 35.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (97.1116%), Tech Debt (9.1932%)
**Top Internal Functions/Classes:**
  * `parseOptions` (Impact: 257.4)
    * *Intent:* /* Returns number of consumed options. */
  * `main` (Impact: 139.6)
  * `createClient` (Impact: 103.3)
    * *Intent:* * Also an initial SELECT command is prepended in order to make sure the right * database is selected...
  * `readHandler` (Impact: 81.6)
  * `fetchClusterConfiguration` (Impact: 58.2)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.086
  * `Choke Point (Betweenness):` 6.7e-05 | `Ripple Effect (Closeness):` 0.0
...
- `deps/jemalloc/include/jemalloc/internal/arena_inlines_b.h` -> **Severity: 0.06** (Bridge: 0.0006 * Flux: 92.3436%)
- `deps/jemalloc/include/jemalloc/internal/jemalloc_internal_inlines_c.h` -> **Severity: 0.042** (Bridge: 0.0004 * Flux: 99.3998%)
- `deps/jemalloc/include/jemalloc/internal/mutex.h` -> **Severity: 0.037** (Bridge: 0.0004 * Flux: 99.3568%)
- `deps/jemalloc/include/jemalloc/internal/tcache_inlines.h` -> **Severity: 0.034** (Bridge: 0.0003 * Flux: 98.9261%)
- `deps/jemalloc/include/jemalloc/internal/jemalloc_internal_inlines_a.h` -> **Severity: 0.028** (Bridge: 0.0003 * Flux: 94.3608%)

### 🙈 Opaque Critical Nodes (Dependency Blast Radius * Doc Risk)
These are 'Core Architecture Nodes' that the entire ecosystem relies upon, but they lack human intent, documentation, or ownership metadata. Modifying them is flying blind.

- `src/redismodule.h` -> **Severity: 1288.0** (Blast Radius: 12.88 * Doc Risk: 100.0%)
- `deps/jemalloc/test/include/test/math.h` -> **Severity: 1173.2** (Blast Radius: 11.732 * Doc Risk: 100.0%)
- `deps/jemalloc/include/jemalloc/internal/util.h` -> **Severity: 1036.7** (Blast Radius: 10.367 * Doc Risk: 100.0%)
- `src/server.h` -> **Severity: 1027.5** (Blast Radius: 10.275 * Doc Risk: 100.0%)
- `deps/jemalloc/include/jemalloc/internal/malloc_io.h` -> **Severity: 959.8** (Blast Radius: 9.598 * Doc Risk: 100.0%)
