...
| Avg Path Length | 2.4897 | Hops between files. Lower = Tighter coupling. |
...
- `register` (@ `src/flask/sansio/blueprints.py`) -> Impact: **59.2** | LOC: 105
...
- `make_response` (@ `src/flask/app.py`) -> Impact: **52.1** | LOC: 141
...
- `url_for` (@ `src/flask/app.py`) -> Impact: **51.3** | LOC: 121
- `run` (@ `src/flask/app.py`) -> Impact: **45.8** | LOC: 122
- `__init__` (@ `src/flask/testing.py`) -> Impact: **40.9** | LOC: 38
- `add_url_rule` (@ `src/flask/sansio/app.py`) -> Impact: **39.9** | LOC: 57
- `explain_template_loading_attempts` (@ `src/flask/debughelpers.py`) -> Impact: **38.8** | LOC: 56
- `routes_command` (@ `src/flask/cli.py`) -> Impact: **38.7** | LOC: 47
  * *Intent:* """Show all registered routes with endpoints and methods."""
- `find_app_by_string` (@ `src/flask/cli.py`) -> Impact: **28.1** | LOC: 78
...
- `open` (@ `src/flask/testing.py`) -> Impact: **24.2** | LOC: 44
...
- **Heaviest Functions:** `register` (Impact: 15.0), `_untag_scan` (Impact: 9.2), `untag` (Impact: 5.7)
...
- **Heaviest Functions:** `make_response` (Impact: 52.1), `url_for` (Impact: 51.3), `run` (Impact: 45.8)
...
- **Heaviest Functions:** `add_url_rule` (Impact: 39.9), `_find_error_handler` (Impact: 17.2), `__init__` (Impact: 16.9)
...
- **Heaviest Functions:** `pop` (Impact: 15.1), `push` (Impact: 7.1), `pop` (Impact: 6.7)
...
- **Heaviest Functions:** `save_session` (Impact: 13.6), `should_set_cookie` (Impact: 6.8), `open_session` (Impact: 6.7)
...
- **Heaviest Functions:** `_get_source_explained` (Impact: 9.2), `list_templates` (Impact: 7.7), `_iter_loaders` (Impact: 7.4)
...
- **Heaviest Functions:** `explain_template_loading_attempts` (Impact: 38.8), `_dump_loader_info` (Impact: 12.1), `__init__` (Impact: 11.5)
...
- **Heaviest Functions:** `register` (Impact: 59.2), `_merge_blueprint_funcs` (Impact: 21.6), `add_url_rule` (Impact: 17.3)
...
- **Heaviest Functions:** `routes_command` (Impact: 38.7), `find_app_by_string` (Impact: 28.1), `load_dotenv` (Impact: 24.1)
...
- **Heaviest Functions:** `get_namespace` (Impact: 15.5), `from_prefixed_env` (Impact: 15.0), `from_file` (Impact: 14.6)
...
**Top Internal Functions/Classes:**
  * `test_server_name_matching` (Impact: 17.4)
  * `test_subdomain_matching_other_name` (Impact: 15.2)
  * `test_trap_bad_request_key_error` (Impact: 14.7)
  * `test_session_expiration` (Impact: 10.2)
  * `test_error_handler_after_processor_error` (Impact: 8.3)
...
**Top Internal Functions/Classes:**
  ...
  * `handle_user_exception` (Impact: 15.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_context_processing` (Impact: 7.1)
  * `test_nested_callback_order` (Impact: 5.6)
  * `test_templates_and_static` (Impact: 5.1)
  * `test_nested_blueprint` (Impact: 4.1)
  * `test_template_global` (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `add_url_rule` (Impact: 17.3)
  * `add_url_rule` (Impact: 16.2)
  * `__init__` (Impact: 15.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `add_url_rule` (Impact: 39.9)
  * `_find_error_handler` (Impact: 17.2)
  * `__init__` (Impact: 16.9)
  * `handle_url_build_error` (Impact: 12.8)
  * `trap_http_exception` (Impact: 12.1)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.010142 | `Ripple Effect (Closeness):` 0.091275
...
**Top Internal Functions/Classes:**
  ...
  * `test_template_loader_debugging` (Impact: 3.4)
  * `test_template_filter` (Impact: 3.0)
  * `test_template_test` (Impact: 3.0)
  * `test_templates_auto_reload` (Impact: 2.9)
...
**Top Internal Functions/Classes:**
  * `get_root_path` (Impact: 21.1)
    * *Intent:* """Find the root path of a package, or the path that contains a module. If it cannot be found, retur...
  * `get_flashed_messages` (Impact: 14.1)
  * `stream_with_context` (Impact: 8.5)
  * `send_file` (Impact: 8.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002421 | `Ripple Effect (Closeness):` 0.130201
...
**Top Internal Functions/Classes:**
  ...
  * `open` (Impact: 24.2)
  * `session_transaction` (Impact: 10.4)
  * `invoke` (Impact: 7.9)
  * `_copy_environ` (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_session_transaction_needs_cookies` (Impact: 3.1)
  * `test_test_client_context_binding` (Impact: 3.1)
  * `test_redirect_session` (Impact: 2.8)
  * `test_client_pop_all_preserved` (Impact: 2.8)
  * `test_environ_defaults` (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `get_namespace` (Impact: 15.5)
  * `from_prefixed_env` (Impact: 15.0)
  * `from_file` (Impact: 14.6)
  * `from_envvar` (Impact: 11.2)
    * *Intent:* """Loads a configuration from an environment variable pointing to a configuration file. This is basi...
  * `from_mapping` (Impact: 8.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_url_with_method` (Impact: 5.0)
  * `test_static_file` (Impact: 4.0)
    * *Intent:* # Default max_age is None. # Test with static file handler. with app.send_static_file("index.html") ...
  * `get` (Impact: 3.7)
  * `test_streaming_with_context_and_custom_close` (Impact: 3.5)
  * `test_async_view` (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001963 | `Ripple Effect (Closeness):` 0.121283
...
**Top Internal Functions/Classes:**
  ...
  * `test_json_key_sorting` (Impact: 6.9)
...
**Top Internal Functions/Classes:**
  ...
  * `test_view_provide_automatic_options_attr_enable` (Impact: 2.6)
  * `test_multiple_inheritance` (Impact: 2.6)
  * `test_remove_method_from_parent` (Impact: 2.6)
  * `test_view_inheritance` (Impact: 2.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `explain_template_loading_attempts` (Impact: 38.8)
  * `_dump_loader_info` (Impact: 12.1)
  * `__init__` (Impact: 11.5)
  * `__init__` (Impact: 8.8)
  * `attach_enctype_error_multidict` (Impact: 4.0)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.008794 | `Ripple Effect (Closeness):` 0.105401
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002787 | `Ripple Effect (Closeness):` 0.126481
...
**Top Internal Functions/Classes:**
  * `test_session_dynamic_cookie_name` (Impact: 5.5)
    * *Intent:* # This session interface will use a cookie with a different name if the # requested url ends with th...
  * `get_cookie_name` (Impact: 5.4)
  * `test_proper_test_request_context` (Impact: 3.5)
  * `test_greenlet_context_copying` (Impact: 3.4)
  * `test_greenlet_context_copying_api` (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
**Top Internal Functions/Classes:**
  * `test_error_handler_no_match` (Impact: 5.9)
  * `report_error` (Impact: 3.8)
  * `test_default_error_handler` (Impact: 3.6)
  * `test_error_handler_subclass` (Impact: 3.3)
  * `handle_500` (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 1 instances
...
**Top Internal Functions/Classes:**
  * `test_robust_teardown` (Impact: 11.2)
  * `test_app_tearing_down_with_unhandled_exception` (Impact: 4.5)
  * `test_context_refcounts` (Impact: 2.9)
  * `test_app_tearing_down_with_handled_exception_by_app_handler` (Impact: 2.8)
  * `test_app_ctx_globals_methods` (Impact: 2.8)
...
- `src/flask/wrappers.py` -> **Severity: 1.058** (Bridge: 0.0108 * Flux: 98.4042%)
- `src/flask/sansio/app.py` -> **Severity: 1.014** (Bridge: 0.0101 * Flux: 99.9966%)
- `src/flask/debughelpers.py` -> **Severity: 0.879** (Bridge: 0.0088 * Flux: 100.0%)
- `src/flask/testing.py` -> **Severity: 0.574** (Bridge: 0.0057 * Flux: 100.0%)
- `src/flask/globals.py` -> **Severity: 0.503** (Bridge: 0.0101 * Flux: 50.0%)
