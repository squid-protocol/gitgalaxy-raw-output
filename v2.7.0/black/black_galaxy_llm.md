...
| Avg Path Length | 2.5029 | Hops between files. Lower = Tighter coupling. |
...
- `main` (@ `src/black/__init__.py`) -> Impact: **311.8** | LOC: 223
- `whitespace` (@ `src/black/nodes.py`) -> Impact: **240.4** | LOC: 249
...
- `normalize_invisible_parens` (@ `src/black/linegen.py`) -> Impact: **136.8** | LOC: 143
- `get_features_used` (@ `src/black/__init__.py`) -> Impact: **127.9** | LOC: 168
- `_maybe_empty_lines_for_class_or_def` (@ `src/black/lines.py`) -> Impact: **95.6** | LOC: 79
- `_maybe_empty_lines` (@ `src/black/lines.py`) -> Impact: **91.2** | LOC: 92
- `maybe_make_parens_invisible_in_atom` (@ `src/black/linegen.py`) -> Impact: **90.2** | LOC: 111
- `get_sources` (@ `src/black/__init__.py`) -> Impact: **84.4** | LOC: 96
- `is_split_before_delimiter` (@ `src/black/brackets.py`) -> Impact: **84.4** | LOC: 94
...
- `_generate_ignored_nodes_from_fmt_skip` (@ `src/black/comments.py`) -> Impact: **82.3** | LOC: 126
...
- **Heaviest Functions:** `__init__` (Impact: 37.5), `format_code` (Impact: 13.3)
...
- **Heaviest Functions:** `schedule_formatting` (Impact: 79.0), `reformat_many` (Impact: 28.5), `shutdown` (Impact: 8.0)
...
- **Heaviest Functions:** `parse_python_variant_header` (Impact: 27.0), `handle` (Impact: 15.0), `parse_mode` (Impact: 12.3)
...
- **Heaviest Functions:** `make_label` (Impact: 36.5), `make_dfa` (Impact: 25.9), `calcfirst` (Impact: 20.6)
...
- **Heaviest Functions:** `tokenize` (Impact: 50.7), `transform_whitespace` (Impact: 15.8), `emit_stashed_lazy` (Impact: 6.4)
...
- **Heaviest Functions:** `main` (Impact: 311.8), `get_features_used` (Impact: 127.9), `get_sources` (Impact: 84.4)
...
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `cors` (Impact: 17.7), `impl` (Impact: 15.1)
...
- **Heaviest Functions:** `normalize_invisible_parens` (Impact: 136.8), `maybe_make_parens_invisible_in_atom` (Impact: 90.2), `transform_line` (Impact: 66.4)
...
- **Heaviest Functions:** `parse_tokens` (Impact: 37.5), `_partially_consume_prefix` (Impact: 21.4), `load_grammar` (Impact: 20.7)
...
- **Heaviest Functions:** `_maybe_empty_lines_for_class_or_def` (Impact: 95.6), `_maybe_empty_lines` (Impact: 91.2), `can_omit_invisible_parens` (Impact: 74.2)
...
**Top Internal Functions/Classes:**
  * `do_transform` (Impact: 81.5)
  * `_merge_one_string_group` (Impact: 56.8)
  * `do_match` (Impact: 55.5)
  * `do_transform` (Impact: 54.1)
  * `_get_max_string_length` (Impact: 49.4)
...
**Top Internal Functions/Classes:**
  * `normalize_invisible_parens` (Impact: 136.8)
  * `maybe_make_parens_invisible_in_atom` (Impact: 90.2)
  * `transform_line` (Impact: 66.4)
  * `_first_right_hand_split` (Impact: 58.7)
  * `_maybe_split_omitting_optional_parens` (Impact: 57.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `assert_collected_sources` (Impact: 37.6)
  * `test_cache_key` (Impact: 17.0)
    * *Intent:* # Test that all members of the mode enum affect the cache key. for field in fields(Mode): values: li...
  * `test_tab_comment_indentation` (Impact: 10.4)
  * `test_cache_file_length` (Impact: 9.6)
  * `tracefunc` (Impact: 9.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `main` (Impact: 311.8)
  * `get_features_used` (Impact: 127.9)
  * `get_sources` (Impact: 84.4)
  * `reformat_one` (Impact: 65.2)
    * *Intent:* # diff-shades depends on being to monkeypatch this function to operate. I know it's # not ideal, but...
  * `read_pyproject_toml` (Impact: 35.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_maybe_empty_lines_for_class_or_def` (Impact: 95.6)
  * `_maybe_empty_lines` (Impact: 91.2)
  * `can_omit_invisible_parens` (Impact: 74.2)
  * `is_line_short_enough` (Impact: 66.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 3.3e-05 | `Ripple Effect (Closeness):` 0.009726
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000853 | `Ripple Effect (Closeness):` 0.03286
...
**Top Internal Functions/Classes:**
  * `_generate_ignored_nodes_from_fmt_skip` (Impact: 82.3)
  * `_handle_regular_fmt_block` (Impact: 76.3)
  * `_handle_comment_only_fmt_block` (Impact: 37.9)
  * `generate_ignored_nodes` (Impact: 36.4)
  * `list_comments` (Impact: 32.2)
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 3.369
  * `Choke Point (Betweenness):` 4e-06 | `Ripple Effect (Closeness):` 0.009726
...
**Top Internal Functions/Classes:**
  * `test_blackd_format_code_limits_executor_queue` (Impact: 6.9)
  * `test_preserves_line_endings` (Impact: 3.2)
  * `test_blackd_main` (Impact: 3.1)
  * `test_normalizes_line_endings` (Impact: 3.1)
  * `test_blackd_python_variant` (Impact: 2.9)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000303 | `Ripple Effect (Closeness):` 0.042215
...
**Top Internal Functions/Classes:**
  * `_convert_unchanged_line_by_line` (Impact: 37.2)
    * *Intent:* """Converts unchanged to STANDALONE_COMMENT line by line."""
  * `adjusted_lines` (Impact: 21.9)
  * `_get_line_range` (Impact: 18.2)
    * *Intent:* """Returns the line range of this node or list of nodes."""
  * `_calculate_lines_mappings` (Impact: 16.0)
  * `_convert_nodes_to_standalone_comment` (Impact: 15.3)
...
**Top Internal Functions/Classes:**
  * `gen_python_files` (Impact: 67.4)
  * `find_project_root` (Impact: 30.2)
  * `strip_specifier_set` (Impact: 12.5)
    * *Intent:* """Strip minor versions for some specifiers in the specifier set. For background on version specifie...
  * `path_is_excluded` (Impact: 12.4)
  * `_path_is_ignored` (Impact: 8.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `parse_tokens` (Impact: 37.5)
    * *Intent:* """Parse a series of tokens and return the syntax tree."""
  * `_partially_consume_prefix` (Impact: 21.4)
  * `load_grammar` (Impact: 20.7)
  * `load_packaged_grammar` (Impact: 9.1)
  * `eat` (Impact: 7.4)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 1 instances
...
**Top Internal Functions/Classes:**
  * `is_split_before_delimiter` (Impact: 84.4)
...
**Top Internal Functions/Classes:**
  * `schedule_formatting` (Impact: 79.0)
  * `reformat_many` (Impact: 28.5)
    * *Intent:* # diff-shades depends on being to monkeypatch this function to operate. I know it's # not ideal, but...
  * `shutdown` (Impact: 8.0)
    * *Intent:* """Cancel all pending tasks on `loop`, wait for them, and close the loop."""
  * `maybe_use_uvloop` (Impact: 3.9)
...
**Top Internal Functions/Classes:**
  * `parse_python_variant_header` (Impact: 27.0)
  * `handle` (Impact: 15.0)
  * `parse_mode` (Impact: 12.3)
  * `format_code` (Impact: 7.0)
  * `main` (Impact: 5.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `assert_format` (Impact: 40.1)
  * `_assert_format_equal` (Impact: 17.3)
  * `_assert_format_inner` (Impact: 17.0)
  * `read_data_from_file` (Impact: 12.7)
  * `get_flags_parser` (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 27 instances
...
**Top Internal Functions/Classes:**
  * `http_get` (Impact: 17.1)
  * `comment_body` (Impact: 17.1)
  * `set_output` (Impact: 11.2)
  * `config` (Impact: 7.1)
  * `comment_details` (Impact: 5.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Rce:* 1 instances
...
**Top Internal Functions/Classes:**
  * `test_unable_to_replace_magics` (Impact: 3.2)
  * `test_ipynb_and_pyi_flags` (Impact: 2.8)
  * `test_cache_isnt_written_if_no_jupyter_deps_single` (Impact: 2.7)
  * `test_cache_isnt_written_if_no_jupyter_deps_dir` (Impact: 2.7)
  * `test_cell_magic_with_custom_python_magic` (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_stringify_ast` (Impact: 43.4)
  * `lib2to3_parse` (Impact: 16.3)
  * `get_grammars` (Impact: 11.3)
  * `parse_ast` (Impact: 8.1)
    * *Intent:* # TODO: support Python 4+ ;) versions = [(3, minor) for minor in range(3, sys.version_info[1] + 1)] ...
  * `_unwrap_tuples` (Impact: 6.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `tokenize` (Impact: 50.7)
  * `transform_whitespace` (Impact: 15.8)
  * `emit_stashed_lazy` (Impact: 6.4)
  * `printtoken` (Impact: 2.7)
**Contextual Mitigations & Amplifications:**
...
- `src/black/nodes.py` -> **Severity: 0.085** (Bridge: 0.0009 * Flux: 99.9539%)
