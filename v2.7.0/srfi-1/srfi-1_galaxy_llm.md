...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 55.4 | 27.7 | 27.7 | 55.4 |
...
- `unfold-right` (@ `srfi-1-reference.scm`) -> Impact: **204.7** | LOC: 496
  * *Intent:* ;;; fold/unfold ;;;;;;;;;;;;;;;
- `really-append-map` (@ `srfi-1-reference.scm`) -> Impact: **137.0** | LOC: 389
- `make-list` (@ `srfi-1-reference.scm`) -> Impact: **83.2** | LOC: 303
  * *Intent:* ;;;; Recursively copy every cons. ;(define (tree-copy x) ; (let recur ((x x)) ; (if (not (pair? x)) x ; (cons (recur (car x)) (recur (cdr x)))))) ;;; ...
- `iota` (@ `srfi-1-reference.scm`) -> Impact: **61.4** | LOC: 228
  * *Intent:* ;;; IOTA count [start step] (start start+step ... start+(count-1)*step)
- `append` (@ `srfi-1-reference.scm`) -> Impact: **30.1** | LOC: 117
  * *Intent:* ;;; append! append-reverse append-reverse! concatenate concatenate! ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
- `reverse` (@ `srfi-1-reference.scm`) -> Impact: **28.4** | LOC: 88
  * *Intent:* ;;; Reverse ;;;;;;;;;;; ;R4RS, so not defined here. ;(define (reverse lis) (fold cons '() lis)) ;(define (reverse! lis) ; (pair-fold (lambda (pair tai...
- `drop-while` (@ `srfi-1-reference.scm`) -> Impact: **24.4** | LOC: 37
- `filter-map` (@ `srfi-1-reference.scm`) -> Impact: **22.0** | LOC: 38
  * *Intent:* ;;; Map F across L, and save up all the non-false results.
- `lset-difference` (@ `srfi-1-reference.scm`) -> Impact: **21.5** | LOC: 73
- `any` (@ `srfi-1-reference.scm`) -> Impact: **18.8** | LOC: 19
...
### 1. `srfi-1-reference.scm` (SCHEME) -> Cumulative Risk: **468.15**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 964.4 | **LOC:** 1597 | **CtrlFlow:** 23.8% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Dead Code (91.7598%), Tech Debt (86.7969%), Verification (80.0%)
- **Heaviest Functions:** `unfold-right` (Impact: 204.7), `really-append-map` (Impact: 137.0), `make-list` (Impact: 83.2)
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (9.8233%)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 964.4 | **LOC:** 1597 | **CtrlFlow:** 23.8% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (7.7303%), Tech Debt (86.7969%)
**Top Internal Functions/Classes:**
  * `unfold-right` (Impact: 204.7)
    * *Intent:* ;;; fold/unfold ;;;;;;;;;;;;;;;
  * `really-append-map` (Impact: 137.0)
  * `make-list` (Impact: 83.2)
