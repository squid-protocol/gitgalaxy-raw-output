...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 33.2 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 19.3 | 2.3 | 2.3 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 10.3 | 0.8 | 0.0 |
...
| connectivity | 47796 | 8260 | 6 | `src/system/libroot/stubbed/libroot_stubs.c` |
...
| dead_code | 81778 | 7842 | 14 | `src/system/libroot/stubbed/libroot_stubs.c` |
...
- `_IO_vfscanf_internal` (@ `src/system/libroot/posix/glibc/stdio-common/vfscanf.c`) -> Impact: **1884.3** | LOC: 2714
  * *Intent:* Return the number of assignments made, or -1 for an input error. */ #ifdef COMPILE_WSCANF
- `borrow_from_hole` (@ `src/add-ons/kernel/file_systems/ntfs/libntfs/attrib.c`) -> Impact: **1599.2** | LOC: 2881
...
- `extract_or_test_entrylist` (@ `src/bin/unzip/extract.c`) -> Impact: **1266.0** | LOC: 1448
  * *Intent:* } /* end function store_info() */ /******************************************/ /* Function extract_or_test_entrylist() */ /***************************...
- `ServerApp::_DispatchMessage` (@ `src/servers/app/ServerApp.cpp`) -> Impact: **929.4** | LOC: 2479
  * *Intent:* */
- `rge_attach` (@ `src/add-ons/kernel/drivers/network/ether/rtl8125/dev/pci/if_rge.c`) -> Impact: **788.0** | LOC: 2079
  * *Intent:* #endif #ifdef __FreeBSD_version
- `vfprintf` (@ `src/system/libroot/posix/glibc/stdio-common/vfprintf.c`) -> Impact: **780.4** | LOC: 1847
  * *Intent:* #endif /* The function itself. */
- `PicturePlayer::_Play` (@ `src/kits/interface/PicturePlayer.cpp`) -> Impact: **752.9** | LOC: 792
- `ns_sprintrrf` (@ `src/system/libnetwork/netresolv/nameser/ns_print.c`) -> Impact: **741.6** | LOC: 987
...
- `OutlineView::Draw` (@ `src/kits/interface/ColumnListView.cpp`) -> Impact: **737.0** | LOC: 1956
- `em_if_attach_pre` (@ `src/add-ons/kernel/drivers/network/ether/ipro1000/dev/e1000/if_em.c`) -> Impact: **723.0** | LOC: 2326
...
- **Heaviest Functions:** `BlockAllocator::AllocateBlocks` (Impact: 173.6), `BlockAllocator::_Initialize` (Impact: 38.7), `BlockAllocator::Allocate` (Impact: 37.1)
...
- **Heaviest Functions:** `Inode::Create` (Impact: 257.0), `Inode::_AddBlockRun` (Impact: 170.3), `Inode::WriteAttribute` (Impact: 101.6)
...
- **Heaviest Functions:** `SET_DISPLAY_MODE` (Impact: 54.5), `MOVE_DISPLAY` (Impact: 38.5), `SET_DPMS_MODE` (Impact: 24.0)
...
- **Heaviest Functions:** `nvme_pci_ctrlr_probe` (Impact: 39.8), `nvme_ctrlr_open` (Impact: 11.2), `nvme_ctrlr_data` (Impact: 9.4)
...
- **Heaviest Functions:** `TouchpadMovement::EventToMovement` (Impact: 60.5), `TouchpadMovement::_EdgeMotion` (Impact: 40.8), `TouchpadMovement::_CheckScrollingToMovement` (Impact: 39.5)
...
- **Heaviest Functions:** `BPoseView::MessageReceived` (Impact: 352.5), `BPoseView::HandleDropCommon` (Impact: 290.4), `BPoseView::MoveSelectionInto` (Impact: 182.3)
...
- **Heaviest Functions:** `g100_g400max_maventv_vid_pll_find` (Impact: 109.3), `maventv_init` (Impact: 43.1), `gxx0_maventv_PAL_init` (Impact: 2.9)
...
- **Heaviest Functions:** `_DecodeTime` (Impact: 11.8), `GetCreationTime` (Impact: 5.6), `Dump` (Impact: 4.5)
...
**Top Internal Functions/Classes:**
  * `iwx_rx_pkt` (Impact: 244.2)
  * `iwx_attach` (Impact: 221.8)
    * *Intent:* #ifdef __FreeBSD_version
  * `iwx_read_firmware` (Impact: 170.4)
    * *Intent:* #define IWX_FW_ADDR_CACHE_CONTROL 0xC0000000
  * `iwx_rx_reorder` (Impact: 139.4)
    * *Intent:* /* * Handle re-ordering of frames which were de-aggregated in hardware. * Returns 1 if the MPDU was ...
  * `iwx_rx_frame` (Impact: 119.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `x86emuOp_opcF7_word_RM` (Impact: 172.5)
    * *Intent:* ****************************************************************************/
  * `x86emuOp_opcFF_word_RM` (Impact: 127.4)
    * *Intent:* ****************************************************************************/
  * `x86emuOp_opcF6_byte_RM` (Impact: 73.4)
    * *Intent:* ****************************************************************************/
  * `x86emuOp_opc81_word_RM_IMM` (Impact: 54.0)
    * *Intent:* ****************************************************************************/
  * `x86emuOp_opc83_word_RM_IMM` (Impact: 53.5)
    * *Intent:* ****************************************************************************/
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `BPoseView::MessageReceived` (Impact: 352.5)
  * `BPoseView::HandleDropCommon` (Impact: 290.4)
  * `BPoseView::MoveSelectionInto` (Impact: 182.3)
  * `BPoseView::KeyDown` (Impact: 167.6)
  * `BPoseView::CreatePoses` (Impact: 108.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `common_fcntl` (Impact: 170.5)
  * `fs_mount` (Impact: 170.1)
    * *Intent:* // #pragma mark - General File System functions
  * `vnode_path_to_vnode` (Impact: 146.6)
    * *Intent:* */
  * `common_file_io_vec_pages` (Impact: 114.1)
    * *Intent:* */
  * `fs_unmount` (Impact: 110.0)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 7 instances
...
**Top Internal Functions/Classes:**
  * `iflib_device_register` (Impact: 108.4)
  * `iflib_if_ioctl` (Impact: 103.2)
  * `iflib_parse_header` (Impact: 71.0)
  * `iflib_encap` (Impact: 67.9)
  * `iflib_fl_refill` (Impact: 65.2)
...
**Top Internal Functions/Classes:**
  ...
  * `AddElement__Q28BPrivate13KMessageFieldPCvl` (Impact: 1.1)
  * `AddElements__Q28BPrivate13KMessageFieldPCvll` (Impact: 1.1)
  * `AddField__Q28BPrivate8KMessagePCcUllPQ28BPrivate13KMessageField` (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
* *Memory Alloc (weighted view):* 5
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `run_read_eeprom` (Impact: 106.4)
  * `run_init_locked` (Impact: 90.4)
  * `run_select_chan_group` (Impact: 85.1)
  * `run_rx_frame` (Impact: 85.0)
  * `run_rt3593_set_chan` (Impact: 84.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `vm_create_anonymous_area` (Impact: 379.4)
    * *Intent:* */
  * `vm_soft_fault` (Impact: 173.8)
    * *Intent:* */
  * `_vm_map_file` (Impact: 136.6)
    * *Intent:* */
  * `cut_area` (Impact: 132.8)
    * *Intent:* */
  * `map_backing_store` (Impact: 132.0)
    * *Intent:* */
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `bge_attach` (Impact: 269.8)
  * `bge_blockinit` (Impact: 211.3)
  * `bge_ioctl` (Impact: 105.9)
  * `bge_reset` (Impact: 91.8)
  * `bge_encap` (Impact: 82.0)
...
**Top Internal Functions/Classes:**
  * `ABSApp::ProcessScriptingMessage` (Impact: 366.8)
    * *Intent:* with an "error" number and an "message" string with a description. */
  * `ABSApp::LoadSaveDatabase` (Impact: 139.6)
    * *Intent:* */
  * `ABSApp::RecursivelyTokenizeMailComponent` (Impact: 121.6)
    * *Intent:* their contents, up to the maximum depth specified. */
  * `CommanderLooper::ProcessArgs` (Impact: 99.3)
    * *Intent:* (so we can print the result). */
  * `ABSApp::AddPositionIOToDatabase` (Impact: 98.0)
    * *Intent:* user. */
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `em_if_attach_pre` (Impact: 723.0)
    * *Intent:* /********************************************************************* * Device initialization routi...
  * `em_reset` (Impact: 136.1)
    * *Intent:* /********************************************************************* * * Initialize the hardware t...
  * `em_newitr` (Impact: 123.8)
    * *Intent:* /********************************************************************* * * Helper to calculate next ...
  * `em_initialize_receive_unit` (Impact: 99.1)
    * *Intent:* /********************************************************************* * * Enable receive unit. * **...
  * `em_get_wakeup` (Impact: 75.1)
...
**Top Internal Functions/Classes:**
  * `BTextView::_HandleArrowKey` (Impact: 164.5)
    * *Intent:* //! Handles when an arrow key is pressed.
  * `BTextView::_HandlePageKey` (Impact: 124.7)
    * *Intent:* //! Handles when the Page Up, Page Down, Home, or End key is pressed.
  * `BTextView::MessageReceived` (Impact: 118.3)
  * `BTextView::_DrawLine` (Impact: 105.2)
  * `BTextView::_FindLineBreak` (Impact: 81.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `dataiterator_step` (Impact: 174.1)
  * `repodata_serialize_key` (Impact: 121.2)
    * *Intent:* /*********************************/ /* internalalize some key into incore/vincore data */
  * `repodata_search` (Impact: 98.3)
    * *Intent:* /* search a specific repodata */
  * `repodata_set_deltalocation` (Impact: 81.8)
    * *Intent:* /* XXX: medianr is currently not stored */
  * `repodata_internalize` (Impact: 71.2)
**Contextual Mitigations & Amplifications:**
* *Mitigated Memory Allocs:* 1 instances
...
- `headers/os/support/Archivable.h` -> **Severity: 0.002** (Bridge: 0.0001 * Flux: 34.1441%)
- `headers/os/support/Autolock.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 91.6827%)
- `headers/private/kernel/util/AVLTreeBase.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 27.3334%)
- `headers/private/libroot/errno_private.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 91.6827%)
- `headers/private/shared/cpu_type.h` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
