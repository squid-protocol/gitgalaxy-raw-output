...
| Avg Path Length | 2.9846 | Hops between files. Lower = Tighter coupling. |
...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 28.5 | 0.0 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 7.8 | 0.0 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 6.6 | 3.5 | 3.5 |
...
| Spec Alignment (formerly Specification Exposure) | 0.0 | 100.0 | 50.6 | 100.0 | 100.0 |
...
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 43.6 | 0.0 | 0.0 |
...
| connectivity | 5140 | 900 | 13 | `pkg/config/user_config.go` |
...
| dead_code | 2012 | 446 | 6 | `pkg/gui/gui.go` |
| credential | 0 | 0 | 0 | - |
| threat | 30 | 8 | 0 | `pkg/jsonschema/generate.go` |
...
- `displayCommit` (@ `pkg/gui/presentation/commits.go`) -> Impact: **125.3** | LOC: 115
- `getBranchDisplayStrings` (@ `pkg/gui/presentation/branches.go`) -> Impact: **113.1** | LOC: 140
  * *Intent:* // getBranchDisplayStrings returns the display string of branch
- `GetCommitListDisplayStrings` (@ `pkg/gui/presentation/commits.go`) -> Impact: **100.1** | LOC: 171
- `NewCmdTask` (@ `pkg/tasks/tasks.go`) -> Impact: **95.6** | LOC: 213
- `getPreset` (@ `pkg/config/editor_presets.go`) -> Impact: **78.3** | LOC: 126
  * *Intent:* // IF YOU ADD A PRESET TO THIS FUNCTION YOU MUST UPDATE THE `Supported presets` SECTION OF docs/Config.md
- `renderPipeSet` (@ `pkg/gui/presentation/graph/graph.go`) -> Impact: **69.2** | LOC: 103
- `layout` (@ `pkg/gui/layout.go`) -> Impact: **68.9** | LOC: 191
  * *Intent:* // layout is called for every screen re-render e.g. when the screen is resized
- `renderLayout` (@ `pkg/gui/controllers/helpers/window_arrangement_helper_test.go`) -> Impact: **68.2** | LOC: 147
- `getNextPipes` (@ `pkg/gui/presentation/graph/graph.go`) -> Impact: **66.2** | LOC: 165
...
### 1. `pkg/gui/gui.go` (GO) -> Cumulative Risk: **701.57**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 535.04 | **LOC:** 1194 | **CtrlFlow:** 11.8% | **Authorship Centralization:** 70.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (98.9661%), Tech Debt (98.3539%), Documentation (97.7528%)
- **Heaviest Functions:** `onNewRepo` (Impact: 40.3), `Run` (Impact: 21.7), `initialContext` (Impact: 18.4)

### 2. `pkg/snake/snake.go` (GO) -> Cumulative Risk: **693.83**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 133.66 | **LOC:** 210 | **CtrlFlow:** 20.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.987%), Concurrency (89.6771%)
- **Heaviest Functions:** `tick` (Impact: 20.1), `newDirection` (Impact: 16.1), `gameLoop` (Impact: 8.2)

### 3. `pkg/gui/background.go` (GO) -> Cumulative Risk: **681.54**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 96.92 | **LOC:** 172 | **CtrlFlow:** 14.5% | **Authorship Centralization:** 83.3%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Concurrency (99.9567%), State Flux (99.946%)
- **Heaviest Functions:** `goEvery` (Impact: 15.7), `startBackgroundRoutines` (Impact: 12.4), `startBackgroundFetch` (Impact: 4.5)

### 4. `pkg/gui/presentation/graph/cell.go` (GO) -> Cumulative Risk: **668.34**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 293.7 | **LOC:** 184 | **CtrlFlow:** 61.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9997%), Cognitive Load (98.3428%)
- **Heaviest Functions:** `getBoxDrawingChars` (Impact: 180.7), `render` (Impact: 14.4), `cachedSprint` (Impact: 11.5)

### 5. `pkg/gui/context/base_context.go` (GO) -> Cumulative Risk: **655.48**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 146.16 | **LOC:** 248 | **CtrlFlow:** 7.1% | **Authorship Centralization:** 50.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.9992%), Safety Score (87.8857%)
- **Heaviest Functions:** `AddOnDoubleClickFn` (Impact: 4.6), `AddOnClickFn` (Impact: 4.6), `AddOnClickFocusedMainViewFn` (Impact: 4.6)

### 6. `pkg/commands/git_commands/commit_loader.go` (GO) -> Cumulative Risk: **646.85**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 361.6 | **LOC:** 609 | **CtrlFlow:** 19.3% | **Authorship Centralization:** 33.3%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9134%), Tech Debt (94.5687%), Documentation (88.8889%)
- **Heaviest Functions:** `getConflictedCommitImpl` (Impact: 43.0), `getHydratedTodoCommits` (Impact: 24.9), `GetCommits` (Impact: 22.4)

### 7. `pkg/gui/controllers/helpers/fixup_helper.go` (GO) -> Cumulative Risk: **639.45**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 236.2 | **LOC:** 413 | **CtrlFlow:** 23.7% | **Authorship Centralization:** 75.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Concurrency (85.1953%), Verification (80.0%)
- **Heaviest Functions:** `blameAddedLines` (Impact: 40.4), `parseDiff` (Impact: 27.9), `HandleFindBaseCommitForFixupPress` (Impact: 20.2)

### 8. `pkg/gui/controllers/helpers/inline_status_helper.go` (GO) -> Cumulative Risk: **633.37**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 87.5 | **LOC:** 157 | **CtrlFlow:** 10.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (99.1956%)
- **Heaviest Functions:** `start` (Impact: 9.9), `WithInlineStatus` (Impact: 9.8), `stop` (Impact: 7.3)

### 9. `pkg/gui/patch_exploring/state.go` (GO) -> Cumulative Risk: **630.62**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 333.8 | **LOC:** 439 | **CtrlFlow:** 20.0% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (98.9496%), Safety Score (89.0402%), Documentation (87.8378%)
- **Heaviest Functions:** `NewState` (Impact: 52.9), `SelectPreviousHunk` (Impact: 8.9), `selectionRangeForCurrentBlockOfChanges` (Impact: 8.2)

### 10. `pkg/gui/context/menu_context.go` (GO) -> Cumulative Risk: **627.32**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 135.1 | **LOC:** 272 | **CtrlFlow:** 11.9% | **Authorship Centralization:** 100.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Tech Debt (99.8651%), Documentation (87.5%), State Flux (84.8229%)
- **Heaviest Functions:** `GetDisplayStrings` (Impact: 17.2), `OnMenuPress` (Impact: 11.1), `GetNonModelItems` (Impact: 9.0)
...
- **Risk Profile:** Cognitive Load (5.9037%), Tech Debt (31.3158%)
...
- **Risk Profile:** Cognitive Load (8.5136%), Tech Debt (27.5204%)
**Top Internal Functions/Classes:**
  * `GetDefaultConfig` (Impact: 6.0)
  * `GetDescription` (Impact: 2.4)
  * `JSONSchemaExtend` (Impact: 1.8)
  * `RefreshIntervalDuration` (Impact: 1.1)
  * `FetchIntervalDuration` (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 535.04 | **LOC:** 1194 | **CtrlFlow:** 11.8% | **Authorship Centralization:** 70.0%
- **Risk Profile:** Cognitive Load (40.7817%), Tech Debt (98.3539%)
**Top Internal Functions/Classes:**
  * `onNewRepo` (Impact: 40.3)
  * `Run` (Impact: 21.7)
    * *Intent:* // Run: setup the gui with keybindings and start the mainloop
  * `initialContext` (Impact: 18.4)
  * `checkForChangedConfigsThatDontAutoReload` (Impact: 12.9)
  * `RunAndHandleError` (Impact: 12.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000192 | `Ripple Effect (Closeness):` 0.003659
...
- **Risk Profile:** Cognitive Load (10.2366%), Tech Debt (12.3844%)
**Top Internal Functions/Classes:**
  * `pressWithLock` (Impact: 31.4)
  * `GetOnRenderToMain` (Impact: 25.9)
  * `toggleStagedAllWithLock` (Impact: 16.8)
  * `remove` (Impact: 15.6)
  * `openCopyMenu` (Impact: 15.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (47.6205%), Tech Debt (25.9247%)
**Top Internal Functions/Classes:**
  * `displayCommit` (Impact: 125.3)
  * `GetCommitListDisplayStrings` (Impact: 100.1)
  * `getHashColor` (Impact: 41.0)
  * `getBisectStatus` (Impact: 28.3)
  * `getBisectStatusText` (Impact: 16.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (8.4229%), Tech Debt (62.1118%)
**Top Internal Functions/Classes:**
  * `revert` (Impact: 18.6)
  * `midRebaseMoveCommandEnabled` (Impact: 17.4)
    * *Intent:* // Ensures that if we are mid-rebase, we're only selecting commits that can be moved
  * `canDropCommits` (Impact: 17.4)
  * `drop` (Impact: 16.8)
  * `moveFixupCommitToOwnerStackedBranch` (Impact: 13.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (14.7303%), Tech Debt (12.7104%)
**Top Internal Functions/Classes:**
  * `loadUserConfig` (Impact: 35.2)
  * `computeMigratedConfig` (Impact: 33.8)
    * *Intent:* // A pure function helper for testing purposes
  * `changeCustomCommandStreamAndOutputToOutputEnum` (Impact: 33.6)
  * `NewAppConfig` (Impact: 22.5)
    * *Intent:* // NewAppConfig makes a new app config
  * `migrateUserConfig` (Impact: 15.8)
...
- **Risk Profile:** Cognitive Load (49.9962%), Tech Debt (40.9292%)
**Top Internal Functions/Classes:**
  * `NewCmdTask` (Impact: 95.6)
  * `NewTask` (Impact: 13.5)
  * `ReadToEnd` (Impact: 6.1)
  * `Close` (Impact: 6.0)
    * *Intent:* // Close closes the task manager, killing whatever task may currently be running
  * `NewViewBufferManager` (Impact: 3.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 7.1e-05 | `Ripple Effect (Closeness):` 0.006652
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 361.6 | **LOC:** 609 | **CtrlFlow:** 19.3% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (27.868%), Tech Debt (94.5687%)
**Top Internal Functions/Classes:**
  * `getConflictedCommitImpl` (Impact: 43.0)
  * `getHydratedTodoCommits` (Impact: 24.9)
  * `GetCommits` (Impact: 22.4)
    * *Intent:* // GetCommits obtains the commits of the current branch
  * `getRebasingCommits` (Impact: 21.3)
    * *Intent:* // getRebasingCommits obtains the commits that we're in the process of rebasing // git-rebase-todo e...
  * `extractCommitFromLine` (Impact: 19.1)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 333.8 | **LOC:** 439 | **CtrlFlow:** 20.0% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (51.9957%), Tech Debt (98.9496%)
**Top Internal Functions/Classes:**
  * `NewState` (Impact: 52.9)
  * `SelectPreviousHunk` (Impact: 8.9)
  * `selectionRangeForCurrentBlockOfChanges` (Impact: 8.2)
  * `SelectedViewRange` (Impact: 7.8)
  * `CycleSelection` (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (61.3523%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `getBranchDisplayStrings` (Impact: 113.1)
    * *Intent:* // getBranchDisplayStrings returns the display string of branch
  * `BranchStatus` (Impact: 38.3)
  * `divergenceStr` (Impact: 14.4)
  * `prColor` (Impact: 10.6)
  * `match` (Impact: 9.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 37 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.003613
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 309.1 | **LOC:** 610 | **CtrlFlow:** 18.5% | **Authorship Centralization:** 66.7%
- **Risk Profile:** Cognitive Load (14.1317%), Tech Debt (95.4759%)
**Top Internal Functions/Classes:**
  * `DiscardAllDirChanges` (Impact: 28.5)
  * `WorktreeFileDiffCmdObj` (Impact: 24.2)
    * *Intent:* // WorktreeFileDiffCmdObj returns a command object for diffing a file or directory // in the working...
  * `DiscardAllFileChanges` (Impact: 24.1)
    * *Intent:* // DiscardAllFileChanges directly
  * `removeEmptyDirs` (Impact: 20.7)
    * *Intent:* // Removes empty directories left behind after deleting files, but only for directories that // are ...
  * `ShowFileDiffCmdObj` (Impact: 16.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (59.9945%), Tech Debt (13.4012%)
**Top Internal Functions/Classes:**
  * `getFileLine` (Impact: 59.0)
  * `getCommitFileLine` (Impact: 40.0)
  * `renderAux` (Impact: 19.2)
  * `fileNameAtDepth` (Impact: 17.3)
  * `getColorForChangeStatus` (Impact: 12.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (11.3769%), Tech Debt (15.8351%)
**Top Internal Functions/Classes:**
  * `viewUpstreamOptions` (Impact: 25.5)
  * `delete` (Impact: 16.0)
  * `createPullRequestMenu` (Impact: 13.5)
  * `withPrFgColor` (Impact: 12.8)
  * `withPrBgColor` (Impact: 12.8)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (49.8978%), Tech Debt (46.8617%)
**Top Internal Functions/Classes:**
  * `getPreset` (Impact: 78.3)
    * *Intent:* // IF YOU ADD A PRESET TO THIS FUNCTION YOU MUST UPDATE THE `Supported presets` SECTION OF docs/Conf...
  * `GetEditTemplate` (Impact: 4.5)
  * `GetEditAtLineTemplate` (Impact: 4.4)
  * `GetEditAtLineAndWaitTemplate` (Impact: 4.4)
  * `GetOpenDirInEditorTemplate` (Impact: 4.4)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 293.7 | **LOC:** 184 | **CtrlFlow:** 61.9% | **Authorship Centralization:** 0.0%
- **Risk Profile:** Cognitive Load (98.3428%), Tech Debt (94.3884%)
**Top Internal Functions/Classes:**
...
- **Risk Profile:** Cognitive Load (27.9099%), Tech Debt (81.9208%)
**Top Internal Functions/Classes:**
  * `assertLines` (Impact: 27.5)
  * `NavigateToLine` (Impact: 18.9)
    * *Intent:* // this will look for a list item in the current panel and if it finds it, it will // enter the keyp...
  * `ContainsLines` (Impact: 13.6)
...
- **Risk Profile:** Cognitive Load (55.7264%), Tech Debt (10.9284%)
**Top Internal Functions/Classes:**
  * `renderPipeSet` (Impact: 69.2)
  * `getNextPipes` (Impact: 66.2)
  * `RenderAux` (Impact: 11.8)
  * `equalHashes` (Impact: 5.6)
  * `RenderCommitGraph` (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (20.268%), Tech Debt (10.1759%)
**Top Internal Functions/Classes:**
  * `infoSectionChildren` (Impact: 32.0)
  * `getMidSectionWeights` (Impact: 18.4)
  * `sidePanelChildren` (Impact: 18.0)
  * `splitMainPanelSideBySide` (Impact: 12.2)
  * `GetWindowDimensions` (Impact: 10.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (9.5422%), Tech Debt (99.972%)
**Top Internal Functions/Classes:**
  * `DiscardOldFileChanges` (Impact: 23.9)
    * *Intent:* // DiscardOldFileChanges discards changes to a file from an old commit
  * `GenericAmend` (Impact: 16.9)
  * `GenericMergeOrRebaseAction` (Impact: 13.2)
    * *Intent:* // GenericMerge takes a commandType of "merge" or "rebase" and a command of "abort", "skip" or "cont...
  * `BeginInteractiveRebaseForCommitRange` (Impact: 10.4)
  * `RewordCommit` (Impact: 9.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (8.085%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `CopyFile` (Impact: 12.0)
  * `CreateRepoHistory` (Impact: 7.1)
    * *Intent:* // This creates a repo history of commits // It uses a branching strategy where each feature branch ...
  * `CreateFile` (Impact: 6.0)
  * `RunShellCommand` (Impact: 5.2)
  * `CreateNCommitsStartingAt` (Impact: 4.0)
**Contextual Mitigations & Amplifications:**
* *Mitigated Danger:* 2 instances
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.001016
...
- **Risk Profile:** Cognitive Load (36.5001%), Tech Debt (10.3542%)
**Top Internal Functions/Classes:**
  * `runAndStreamAux` (Impact: 26.1)
  * `RunAndProcessLines` (Impact: 19.9)
  * `processOutput` (Impact: 18.1)
  * `getCheckForCredentialRequestFunc` (Impact: 13.2)
    * *Intent:* // having a function that returns a function because we need to maintain some state inbetween calls ...
  * `getCredentialPromptFn` (Impact: 7.6)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (21.0772%), Tech Debt (10.7801%)
**Top Internal Functions/Classes:**
  * `renderLayout` (Impact: 68.2)
  * `TestGetWindowDimensions` (Impact: 12.1)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 236.2 | **LOC:** 413 | **CtrlFlow:** 23.7% | **Authorship Centralization:** 75.0%
- **Risk Profile:** Cognitive Load (33.8154%), Tech Debt (24.9422%)
**Top Internal Functions/Classes:**
  * `blameAddedLines` (Impact: 40.4)
  * `parseDiff` (Impact: 27.9)
    * *Intent:* // Parse the diff output into hunks, and return two lists of hunks: the first // are ones that conta...
  * `HandleFindBaseCommitForFixupPress` (Impact: 20.2)
  * `IsFixupCommit` (Impact: 9.6)
    * *Intent:* // Check whether the given subject line is the subject of a fixup commit, and // returns (trimmedSub...
  * `removeFixupCommits` (Impact: 9.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- `pkg/commands/git_commands/working_tree.go` -> Churn: **87.46%** | Cog Load: 14.1317% | Debt: 95.4759%
...
- `pkg/commands/oscommands/os.go` -> **Severity: 0.044** (Bridge: 0.0005 * Flux: 87.0519%)
- `pkg/utils/utils.go` -> **Severity: 0.031** (Bridge: 0.0009 * Flux: 36.187%)
- `pkg/theme/gocui.go` -> **Severity: 0.026** (Bridge: 0.0005 * Flux: 50.0%)
- `pkg/gui/gui.go` -> **Severity: 0.019** (Bridge: 0.0002 * Flux: 98.9661%)
- `pkg/theme/style.go` -> **Severity: 0.016** (Bridge: 0.0002 * Flux: 100.0%)
