...
| Avg Path Length | 2.7786 | Hops between files. Lower = Tighter coupling. |
...
| credential | 2 | 2 | 0 | `suite/auto-sync/src/autosync/MCUpdater.py` |
...
- `WASM_getInstruction` (@ `arch/WASM/WASMDisassembler.c`) -> Impact: **611.9** | LOC: 416
- `translateImmediate` (@ `arch/X86/X86Disassembler.c`) -> Impact: **552.2** | LOC: 669
...
- `DecodeVLDInstruction` (@ `arch/ARM/ARMDisassembler.c`) -> Impact: **491.2** | LOC: 299
- `DecodeVSTInstruction` (@ `arch/ARM/ARMDisassembler.c`) -> Impact: **474.7** | LOC: 281
- `_cs_disasm` (@ `bindings/ocaml/ocaml.c`) -> Impact: **428.8** | LOC: 1697
- `printInst` (@ `arch/AArch64/AArch64InstPrinter.c`) -> Impact: **403.3** | LOC: 508
- `add_cs_detail_general` (@ `arch/ARM/ARMMapping.c`) -> Impact: **398.8** | LOC: 736
...
- `DecodeSignedLdStInstruction` (@ `arch/AArch64/AArch64Disassembler.c`) -> Impact: **352.0** | LOC: 198
- `DecodeCopMemInstruction` (@ `arch/ARM/ARMDisassembler.c`) -> Impact: **328.7** | LOC: 179
- `DecodeAddrMode3Instruction` (@ `arch/ARM/ARMDisassembler.c`) -> Impact: **305.0** | LOC: 197
...
- **Heaviest Functions:** `RISCV_add_cs_detail_0` (Impact: 58.5), `RISCV_add_ret_group` (Impact: 24.2), `RISCV_add_privileged_group` (Impact: 12.6)
...
- **Heaviest Functions:** `get_ea_mode_op` (Impact: 163.8), `d68020_cpgen` (Impact: 106.3), `get_with_index_address_mode` (Impact: 91.5)
...
- **Heaviest Functions:** `getOptInfo` (Impact: 33.8), `CsInsn` (Impact: 23.8), `Capstone` (Impact: 7.9)
...
- **Heaviest Functions:** `Xtensa_add_cs_detail_0` (Impact: 106.5), `Xtensa_reg_access` (Impact: 39.5), `Xtensa_L32R_Value` (Impact: 9.3)
...
- **Heaviest Functions:** `printInst` (Impact: 403.3), `getNextVectorRegister` (Impact: 159.9), `printVectorList` (Impact: 93.9)
...
- **Heaviest Functions:** `X86_ATT_printInst` (Impact: 173.9), `printOperand` (Impact: 128.0), `printopaquemem` (Impact: 64.8)
...
- **Heaviest Functions:** `replace_negative` (Impact: 27.4), `replace_hex` (Impact: 25.1), `trim_str` (Impact: 9.5)
...
- **Heaviest Functions:** `ARM_getFeatureBits` (Impact: 48.9), `isVPTOpcode` (Impact: 33.3), `isValidCoprocessorNumber` (Impact: 11.4)
...
- **Heaviest Functions:** `printImm` (Impact: 124.2), `printOperand` (Impact: 95.5), `printopaquemem` (Impact: 85.8)
...
- **Heaviest Functions:** `compare` (Impact: 28.3), `__init__` (Impact: 21.2), `setup` (Impact: 17.2)
...
**Top Internal Functions/Classes:**
  * `DecodeVLDInstruction` (Impact: 491.2)
  * `DecodeVSTInstruction` (Impact: 474.7)
  * `DecodeCopMemInstruction` (Impact: 328.7)
  * `DecodeAddrMode3Instruction` (Impact: 305.0)
  * `DecodeMSRMask` (Impact: 139.5)
...
**Top Internal Functions/Classes:**
  * `fill_alu_mods` (Impact: 179.8)
  * `getInstruction` (Impact: 161.8)
  * `fill_alu_insn_name` (Impact: 131.2)
  * `decode_alu` (Impact: 124.3)
  * `fill_copr_mods` (Impact: 122.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `get_ea_mode_op` (Impact: 163.8)
    * *Intent:* /* Make string of effective address mode */
  * `d68020_cpgen` (Impact: 106.3)
  * `get_with_index_address_mode` (Impact: 91.5)
  * `update_am_reg_list` (Impact: 35.8)
  * `d68010_movec` (Impact: 35.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `cs_op_index` (Impact: 219.7)
  * `cs_op_count` (Impact: 143.8)
  * `cs_disasm` (Impact: 104.3)
    * *Intent:* #endif // dynamically allocate memory to contain disasm insn // NOTE: caller must free() the allocat...
  * `cs_option` (Impact: 67.3)
  * `skipdata_size` (Impact: 42.0)
...
**Top Internal Functions/Classes:**
  * `decode_dsp_3op` (Impact: 150.6)
  * `sh_disassemble` (Impact: 75.7)
  * `decode_long` (Impact: 56.2)
...
**Top Internal Functions/Classes:**
  * `is_sufficient_code_size` (Impact: 137.8)
  * `indexed09_hdlr` (Impact: 80.2)
    * *Intent:* // M6809/M6309 indexed mode handler
  * `M680X_getInstruction` (Impact: 79.5)
  * `indexed12_hdlr` (Impact: 48.5)
    * *Intent:* // CPU12 indexed mode handler
  * `get_indexed09_post_byte_size` (Impact: 45.8)
...
**Top Internal Functions/Classes:**
  * `getInstruction` (Impact: 157.3)
  * `Mips_getFeatureBits` (Impact: 93.7)
    * *Intent:* #define GET_SUBTARGETINFO_ENUM #include "MipsGenSubtargetInfo.inc" #define GET_INSTRINFO_ENUM #inclu...
  * `DecodeMemMMImm4` (Impact: 60.9)
  * `DecodeINSVE_DF` (Impact: 31.3)
  * `DecodeMSA128Mem` (Impact: 26.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `printInst` (Impact: 403.3)
  * `getNextVectorRegister` (Impact: 159.9)
  * `printVectorList` (Impact: 93.9)
  * `printSysAlias` (Impact: 93.7)
  * `printMemExtendImpl` (Impact: 27.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_cs_disasm` (Impact: 428.8)
  * `ocaml_cs_disasm` (Impact: 117.9)
  * `ocaml_open` (Impact: 85.3)
  * `ocaml_option` (Impact: 19.7)
  * `list_count` (Impact: 5.7)
...
**Top Internal Functions/Classes:**
  * `DecodeSignedLdStInstruction` (Impact: 352.0)
  * `DecodePairLdStInstruction` (Impact: 190.1)
  * `DecodeExclusiveLdStInstruction` (Impact: 111.5)
  * `DecodeThreeAddrSRegInstruction` (Impact: 70.2)
  * `DecodeUnsignedLdStInstruction` (Impact: 63.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `DecodeBOInstruction` (Impact: 88.0)
  * `DecodeRRInstruction` (Impact: 52.8)
  * `DecodeSROInstruction` (Impact: 49.4)
  * `DecodeBOLInstruction` (Impact: 45.5)
  * `DecodeSRRInstruction` (Impact: 40.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `diff_nodes` (Impact: 92.6)
  * `all_choices_saved` (Impact: 31.1)
  * `determine_patch_coordinates` (Impact: 30.9)
  * `get_user_choice` (Impact: 29.7)
  * `patch_files` (Impact: 29.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `X86_ATT_printInst` (Impact: 173.9)
  * `printOperand` (Impact: 128.0)
  * `printopaquemem` (Impact: 64.8)
  * `printMemReference` (Impact: 55.9)
  * `_printOperand` (Impact: 38.2)
...
**Top Internal Functions/Classes:**
  * `printImm` (Impact: 124.2)
  * `printOperand` (Impact: 95.5)
  * `printopaquemem` (Impact: 85.8)
  * `printMemReference` (Impact: 54.1)
  * `printanymem` (Impact: 31.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `CheckRegister` (Impact: 144.4)
    * *Intent:* // Verify SR and UR
  * `getInstruction` (Impact: 40.7)
  * `decodeBranchOperand` (Impact: 25.8)
  * `readInstruction24` (Impact: 12.9)
    * *Intent:* /// Read three bytes from the ArrayRef and return 24 bit data
  * `DecodeSRRegisterClass` (Impact: 12.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `printInst` (Impact: 200.3)
    * *Intent:* #define PRINT_ALIAS_INSTR #include "ARMGenAsmWriter.inc"
  * `printMSRMaskOperand` (Impact: 58.6)
  * `printRegImmShift` (Impact: 23.1)
    * *Intent:* /// Prints the shift value with an immediate value.
  * `printModImmOperand` (Impact: 18.3)
  * `printOperandAddr` (Impact: 14.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 53.6)
  * `init_tests` (Impact: 41.0)
  * `extract_llvm_mc_cmds` (Impact: 37.5)
  * `write_to_build_dir` (Impact: 34.9)
  * `parse_llvm_mc_line` (Impact: 18.5)
**Contextual Mitigations & Amplifications:**
* *Amplified Rce:* 1 instances
...
- `include/capstone/capstone.h` -> **Severity: 0.022** (Bridge: 0.0005 * Flux: 43.3485%)
- `suite/auto-sync/src/autosync/cpptranslator/patches/Helper.py` -> **Severity: 0.001** (Bridge: 0.0 * Flux: 100.0%)
