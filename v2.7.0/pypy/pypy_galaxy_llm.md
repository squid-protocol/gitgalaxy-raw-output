...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 23.8 | 10.5 | 0.0 |
...
| connectivity | 95629 | 4160 | 51 | `rpython/annotator/test/test_annrpython.py` |
...
| dead_code | 29130 | 2884 | 15 | `rpython/jit/metainterp/optimizeopt/test/test_optimizeopt.py` |
| credential | 22 | 19 | 0 | `extra_tests/test_string.py` |
| threat | 9247 | 1510 | 5 | `lib-python/2.7/test/test_descr.py` |
...
- `main` (@ `lib-python/2.7/test/regrtest.py`) -> Impact: **1147.3** | LOC: 746
- `convertsimple` (@ `pypy/module/cpyext/src/getargs.c`) -> Impact: **681.2** | LOC: 783
  * *Intent:* */
- `__Py_dg_dtoa` (@ `rpython/translator/c/src/dtoa.c`) -> Impact: **504.0** | LOC: 609
  * *Intent:* call to __Py_dg_freedtoa. */
- `__Py_dg_strtod` (@ `rpython/translator/c/src/dtoa.c`) -> Impact: **361.5** | LOC: 753
- `make_formatting_class` (@ `pypy/objspace/std/newformat.py`) -> Impact: **360.9** | LOC: 770
- `dispatch_bytecode` (@ `pypy/interpreter/pyopcode.py`) -> Impact: **328.3** | LOC: 306
- `make_timsort_class` (@ `rpython/rlib/listsort.py`) -> Impact: **300.7** | LOC: 625
- `llexternal` (@ `rpython/rtyper/lltypesystem/rffi.py`) -> Impact: **277.1** | LOC: 275
- `__init__` (@ `pypy/module/micronumpy/nditer.py`) -> Impact: **272.0** | LOC: 175
- `_parse` (@ `lib-python/2.7/sre_parse.py`) -> Impact: **267.5** | LOC: 310
...
- **Heaviest Functions:** `find_next_token` (Impact: 19.0), `make_token` (Impact: 10.2), `__init__` (Impact: 9.1)
...
- **Heaviest Functions:** `STORE_ATTR_slowpath` (Impact: 80.0), `_make_storage_mixin_size_n` (Impact: 51.5), `LOAD_ATTR_slowpath` (Impact: 29.3)
...
- **Heaviest Functions:** `make_wrapper_second_level` (Impact: 142.2), `wrapper_second_level` (Impact: 85.9), `generic_cpy_call` (Impact: 45.0)
...
- **Heaviest Functions:** `OP_GC_GCFLAG_EXTRA` (Impact: 19.4), `compilation_info` (Impact: 13.5), `OP_GC_RELOAD_POSSIBLY_MOVED` (Impact: 10.3)
...
- **Heaviest Functions:** `record_coerce` (Impact: 37.6), `make_integer_min_dtype` (Impact: 33.2), `_coerce` (Impact: 26.7)
...
- **Heaviest Functions:** `do` (Impact: 13.6), `kill_range` (Impact: 12.8), `do` (Impact: 10.6)
...
- **Heaviest Functions:** `emit_residual_call` (Impact: 14.7), `write_revdb_def_file` (Impact: 12.8), `prepare_function` (Impact: 12.4)
...
- **Heaviest Functions:** `parse_plain` (Impact: 49.5), `guess_type` (Impact: 18.2), `dot2plain_graphviz` (Impact: 11.1)
...
- **Heaviest Functions:** `__init__` (Impact: 7.1), `get_fp_offset` (Impact: 2.0), `__init__` (Impact: 1.8)
...
- **Heaviest Functions:** `s_isinstance` (Impact: 25.8), `getanyitem` (Impact: 23.1), `transform_varargs` (Impact: 16.6)
...
**Top Internal Functions/Classes:**
  * `test_cmp_fastpaths` (Impact: 32.7)
  * `test_nested_retrace` (Impact: 27.3)
  * `test_inputarg_reset_bug` (Impact: 26.2)
    * *Intent:* ## j = 0 ## while j < 100: ## j += 1 ## c = 0 ## j = 0 ## while j < 2: ## j += 1 ## if c == 0:...
  * `test_retrace_ending_up_retracing_another_loop` (Impact: 22.8)
  * `make_int` (Impact: 19.2)
...
**Top Internal Functions/Classes:**
  ...
  * `__init__` (Impact: 78.1)
  * `quantize` (Impact: 69.6)
...
**Top Internal Functions/Classes:**
  * `f` (Impact: 30.6)
  * `f` (Impact: 30.6)
  * `test_nonneg_cleverness` (Impact: 15.0)
  * `test_general_nonneg_cleverness` (Impact: 15.0)
  * `f` (Impact: 14.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_if_` (Impact: 47.3)
  * `_enclosed` (Impact: 41.6)
  * `_repetition` (Impact: 41.0)
  * `_productionargs` (Impact: 36.1)
  * `_arguments` (Impact: 34.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_call_release_gil_variable_function_and_arguments` (Impact: 61.3)
  * `nan_and_infinity` (Impact: 39.0)
  * `test_nan_and_infinity` (Impact: 38.6)
  * `test_floats_and_guards` (Impact: 37.6)
  * `test_jump` (Impact: 37.1)
...
**Top Internal Functions/Classes:**
  * `pow` (Impact: 80.8)
  * `_format_recursive` (Impact: 65.4)
  * `_int_bitwise` (Impact: 62.1)
    * *Intent:* """ Bitwise and/or/xor operations """
  * `_bitwise` (Impact: 62.0)
    * *Intent:* """ Bitwise and/or/xor operations """
  * `tobytes` (Impact: 61.7)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `parsedebug` (Impact: 162.3)
    * *Intent:* # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! # parsedebug(). # # ...
  * `parseopt` (Impact: 139.1)
    * *Intent:* #--! parsedebug-end # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ...
  * `parseopt_notrack` (Impact: 127.3)
    * *Intent:* #--! parseopt-end # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! # ...
  * `lr_parse_table` (Impact: 101.2)
...
**Top Internal Functions/Classes:**
  * `__Py_dg_dtoa` (Impact: 504.0)
    * *Intent:* call to __Py_dg_freedtoa. */
  * `__Py_dg_strtod` (Impact: 361.5)
  * `bigcomp` (Impact: 59.5)
...
**Top Internal Functions/Classes:**
  * `_test_bitfield_details` (Impact: 33.6)
  * `_test_wchar_variant` (Impact: 30.7)
  * `test_buffer` (Impact: 23.4)
  * `find_and_load_library` (Impact: 18.1)
  * `check` (Impact: 15.2)
    * *Intent:* #
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_fromstring` (Impact: 20.8)
  * `test_subarrays` (Impact: 17.9)
  * `test_data` (Impact: 15.0)
  * `test_fromstring_types` (Impact: 13.5)
  * `test_memoryview` (Impact: 12.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_prefix_preservation` (Impact: 19.6)
  * `assert_runs_after` (Impact: 12.8)
  * `test_call` (Impact: 10.5)
  * `test` (Impact: 9.2)
  * `test_import_from` (Impact: 8.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `_options` (Impact: 27.3)
    * *Intent:* """Internal function."""
  * `_grid_configure` (Impact: 20.6)
    * *Intent:* """Internal function."""
  * `_stringify` (Impact: 19.7)
    * *Intent:* """Internal function."""
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000228 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  ...
  * `_undogoto` (Impact: 39.6)
    * *Intent:* """Reverse a _goto. Used for undo() """
  * `_drawturtle` (Impact: 32.3)
...
**Top Internal Functions/Classes:**
  * `f` (Impact: 45.8)
  * `test_call_star_and_keywords_starargs` (Impact: 41.1)
  * `f` (Impact: 23.8)
  * `test_call_star_and_keywords` (Impact: 22.9)
  * `test_call_keywords` (Impact: 19.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `setslice` (Impact: 48.3)
  * `descr_sort` (Impact: 42.7)
    * *Intent:* """ L.sort(cmp=None, key=None, reverse=False) -- stable sort *IN PLACE*; cmp(x, y) -> -1, 0, 1"""
  * `find_or_count` (Impact: 35.5)
  * `_safe_find_or_count` (Impact: 32.9)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000988 | `Ripple Effect (Closeness):` 0.0
...
**Top Internal Functions/Classes:**
  ...
  * `assemble_loop` (Impact: 40.8)
  * `_build_malloc_slowpath` (Impact: 37.7)
    * *Intent:* """ While arriving on slowpath, we have a gcpattern on stack 0. The arguments are passed in ecx and ...
  * `load_from_mem` (Impact: 35.5)
    * *Intent:* # ----------
  * `generate_body` (Impact: 33.8)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 311 instances
...
**Top Internal Functions/Classes:**
  * `str_decode_unicode_escape` (Impact: 109.0)
  * `str_decode_utf_7` (Impact: 96.9)
  * `str_decode_utf_16_helper` (Impact: 95.8)
  * `make_unicode_escape_function` (Impact: 85.9)
  * `str_decode_utf_32_helper` (Impact: 85.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 121.1)
  * `gettarinfo` (Impact: 67.5)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0005 | `Ripple Effect (Closeness):` 0.0
...
- `rpython/translator/c/src/signals.c` -> Churn: **60.86%** | Cog Load: 70.4947% | Debt: 11.5151%
...
- `pypy/interpreter/baseobjspace.py` -> **Severity: 0.913** (Bridge: 0.0091 * Flux: 99.9998%)
- `rpython/annotator/annrpython.py` -> **Severity: 0.793** (Bridge: 0.0079 * Flux: 100.0%)
- `rpython/translator/translator.py` -> **Severity: 0.605** (Bridge: 0.0061 * Flux: 100.0%)
- `pypy/interpreter/module.py` -> **Severity: 0.574** (Bridge: 0.0057 * Flux: 100.0%)
- `rpython/rtyper/extfuncregistry.py` -> **Severity: 0.57** (Bridge: 0.0057 * Flux: 99.8341%)
