...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 2.2 | 0.2 | 0.0 | 0.0 |
...
| threat | 1087 | 298 | 4 | `src/twisted/names/test/test_dns.py` |
...
- `__init__` (@ `src/twisted/internet/_sslverify.py`) -> Impact: **188.1** | LOC: 310
- `_tokenize` (@ `src/twisted/persisted/_tokenize.py`) -> Impact: **119.3** | LOC: 203
- `writexml` (@ `src/twisted/web/microdom.py`) -> Impact: **92.4** | LOC: 168
- `jelly` (@ `src/twisted/spread/jelly.py`) -> Impact: **79.1** | LOC: 127
- `rebuild` (@ `src/twisted/python/rebuild.py`) -> Impact: **70.5** | LOC: 129
  * *Intent:* """ Reload a module and do as much as possible to replace its references. """
- `_flattenElement` (@ `src/twisted/web/_flatten.py`) -> Impact: **68.0** | LOC: 143
- `__init__` (@ `src/twisted/internet/process.py`) -> Impact: **65.6** | LOC: 119
- `_discoveredAuthority` (@ `src/twisted/names/root.py`) -> Impact: **64.6** | LOC: 117
...
- `addCookie` (@ `src/twisted/web/http.py`) -> Impact: **64.6** | LOC: 115
- `objgrep` (@ `src/twisted/python/reflect.py`) -> Impact: **62.9** | LOC: 117
...
### 1. `src/twisted/internet/asyncioreactor.py` (PYTHON) -> Cumulative Risk: **684.26**
...
- **Heaviest Functions:** `removeReader` (Impact: 11.5), `removeWriter` (Impact: 11.5), `_readOrWrite` (Impact: 10.7)
...
- **Heaviest Functions:** `irc_LIST` (Impact: 24.6), `irc_PART` (Impact: 13.9), `irc_NICK` (Impact: 13.7)

### 3. `src/twisted/mail/_except.py` (PYTHON) -> Cumulative Risk: **673.37**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `__init__` (Impact: 10.1), `__bytes__` (Impact: 6.1), `__init__` (Impact: 4.6)

### 4. `src/twisted/trial/_dist/workerreporter.py` (PYTHON) -> Cumulative Risk: **673.21**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `addError` (Impact: 6.2), `addFailure` (Impact: 6.0), `_call` (Impact: 6.0)
...
- **Heaviest Functions:** `dataReceived` (Impact: 3.7), `resumeProducing` (Impact: 3.1), `run` (Impact: 2.5)
...
- **Heaviest Functions:** `render` (Impact: 33.5), `render` (Impact: 23.5), `sizeHint` (Impact: 19.8)
...
- **Heaviest Functions:** `selectGraphicRendition` (Impact: 22.5), `_checkExpected` (Impact: 16.4), `toVT102` (Impact: 13.8)
...
- **Heaviest Functions:** `buildProtocol` (Impact: 11.0), `unregisterProtocol` (Impact: 11.0), `setTimeout` (Impact: 9.9)
...
- **Heaviest Functions:** `_joinleave` (Impact: 14.3), `getOutgoingInterface` (Impact: 6.2), `setOutgoingInterface` (Impact: 6.2)
...
- **Heaviest Functions:** `init` (Impact: 12.6), `synchronize` (Impact: 6.3), `_synchPre` (Impact: 4.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5e-05 | `Ripple Effect (Closeness):` 0.00112
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000264 | `Ripple Effect (Closeness):` 0.00112
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.034164 | `Ripple Effect (Closeness):` 0.228748
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1853.18 | **LOC:** 4920 | **CtrlFlow:** 1.5% | **Authorship Centralization:** 100.0%
- **Risk Profile:** Cognitive Load (13.2662%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `createServerEndpoint` (Impact: 13.7)
  * `deterministicResolvingReactor` (Impact: 10.9)
  ...
  * `resolveHostName` (Impact: 8.1)
  * `connectionTest` (Impact: 7.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.00224
...
**Top Internal Functions/Classes:**
  ...
  * `sendmail` (Impact: 33.8)
  * `dataLineReceived` (Impact: 28.0)
  * `authenticate` (Impact: 24.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 6e-06 | `Ripple Effect (Closeness):` 0.004666
...
**Top Internal Functions/Classes:**
  * `_compactRepr` (Impact: 29.6)
  * `__init__` (Impact: 28.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.00179 | `Ripple Effect (Closeness):` 0.213782
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1561.68 | **LOC:** 3491 | **CtrlFlow:** 1.8% | **Authorship Centralization:** 75.0%
- **Risk Profile:** Cognitive Load (19.7693%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `_testRedirectDefault` (Impact: 45.4)
  * `integrationTest` (Impact: 16.4)
  * `test_connectHTTPSCustomConnectionCreator` (Impact: 6.3)
    * *Intent:* """ If a custom L{WebClientConnectionCreator}-like object is passed to L{Agent.__init__} it will be ...
  * `test_cacheRemovesOldest` (Impact: 6.3)
    * *Intent:* """ Verify that when the cache is full, and a new entry is added, the oldest entry is removed. """
  * `_sensitiveHeadersTest` (Impact: 6.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `getListeningPort` (Impact: 13.7)
    * *Intent:* """ Get a TCP port from a reactor, wrapping an already-initialized file descriptor. """
  * `assertPeerClosedOnEMFILE` (Impact: 12.0)
  * `runAbortTest` (Impact: 10.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000189 | `Ripple Effect (Closeness):` 0.003359
...
**Top Internal Functions/Classes:**
  * `testQueue` (Impact: 11.7)
  * `_count` (Impact: 10.1)
  * `test_boundedStackDepth` (Impact: 8.6)
    * *Intent:* """ The depth of the call stack does not grow as more L{Deferred} instances are chained together. ""...
  * `chainDeferreds` (Impact: 8.1)
  * `testSemaphore` (Impact: 6.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.0 | `Ripple Effect (Closeness):` 0.007167
...
**Top Internal Functions/Classes:**
  * `serviceIdentitySetup` (Impact: 48.1)
  * `loopback` (Impact: 12.0)
  * `serverCertificate` (Impact: 8.7)
  * `tearDown` (Impact: 7.7)
  * `testInspectDistinguishedName` (Impact: 6.8)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000196 | `Ripple Effect (Closeness):` 0.004666
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001065 | `Ripple Effect (Closeness):` 0.00112
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000947 | `Ripple Effect (Closeness):` 0.084273
...
- `src/twisted/web/iweb.py` -> Churn: **58.05%** | Cog Load: 24.1283% | Debt: 99.5269%

### 👤 Key Person Dependencies (High Impact + Siloed Knowledge)
These are massive, load-bearing files written almost entirely by a single developer. They represent severe 'Bus Factor' risk.

- `src/twisted/mail/test/test_imap.py` -> **Glyph** (100.0% isolated ownership) | Magnitude: 3685.88
- `src/twisted/words/protocols/irc.py` -> **Glyph** (100.0% isolated ownership) | Magnitude: 2522.24
- `src/twisted/protocols/ftp.py` -> **Glyph** (100.0% isolated ownership) | Magnitude: 2103.18
- `src/twisted/internet/test/test_endpoints.py` -> **Glyph** (100.0% isolated ownership) | Magnitude: 1853.18
...
- `src/twisted/python/compat.py` -> **Severity: 3.567** (Bridge: 0.0357 * Flux: 99.9993%)
- `src/twisted/web/http.py` -> **Severity: 3.416** (Bridge: 0.0342 * Flux: 100.0%)
- `src/twisted/internet/defer.py` -> **Severity: 2.695** (Bridge: 0.0269 * Flux: 99.9997%)
- `src/twisted/internet/task.py` -> **Severity: 1.8** (Bridge: 0.018 * Flux: 100.0%)
- `src/twisted/scripts/trial.py` -> **Severity: 1.731** (Bridge: 0.0173 * Flux: 100.0%)
