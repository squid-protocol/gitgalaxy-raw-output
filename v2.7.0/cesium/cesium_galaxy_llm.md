...
| Guard Balance (formerly Error & Exception Exposure) | 0.0 | 100.0 | 36.6 | 30.8 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 3.9 | 0.2 | 0.0 | 0.0 |
| Historical Churn (predictive layer, promotion pending #2987) (formerly Volatility Exposure) | 0.0 | 99.7 | 2.5 | 0.0 | 0.0 |
| Doc Surface (formerly Documentation Exposure) | 0.0 | 100.0 | 29.9 | 0.0 | 0.0 |
...
| connectivity | 9956 | 2175 | 6 | `ThirdParty/codemirror-5.52.0/doc/manual.html` |
...
| dead_code | 2113 | 556 | 0 | `ThirdParty/dojo-release-1.10.4/dijit/_editor/RichText.js` |
| credential | 31 | 24 | 0 | `packages/sandcastle/gallery/panorama/main.js` |
| threat | 5429 | 893 | 3 | `packages/engine/Source/ThirdParty/google-earth-dbroot-parser.js` |
...
- `dispatchKey` (@ `ThirdParty/codemirror-5.52.0/lib/codemirror.js`) -> Impact: **1116.0** | LOC: 1300
- `tokenBase` (@ `ThirdParty/codemirror-5.52.0/mode/stylus/stylus.js`) -> Impact: **565.8** | LOC: 682
  * *Intent:* /** * Tokenizers */
- `cesiumGoogleEarthDbRootParser` (@ `packages/engine/Source/ThirdParty/google-earth-dbroot-parser.js`) -> Impact: **556.3** | LOC: 915
- `Vim` (@ `ThirdParty/codemirror-5.52.0/keymap/vim.js`) -> Impact: **469.1** | LOC: 1541
- `verify` (@ `packages/engine/Source/ThirdParty/google-earth-dbroot-parser.js`) -> Impact: **334.3** | LOC: 350
- `tokenPerl` (@ `ThirdParty/codemirror-5.52.0/mode/perl/perl.js`) -> Impact: **324.7** | LOC: 327
- `tokenBase` (@ `ThirdParty/codemirror-5.52.0/mode/ruby/ruby.js`) -> Impact: **307.4** | LOC: 259
- `expression` (@ `ThirdParty/codemirror-5.52.0/mode/soy/soy.js`) -> Impact: **304.3** | LOC: 371
- `toObject` (@ `packages/engine/Source/ThirdParty/google-earth-dbroot-parser.js`) -> Impact: **271.7** | LOC: 238
- `addDrawCommandsForTile` (@ `packages/engine/Source/Scene/GlobeSurfaceTileProvider.js`) -> Impact: **251.2** | LOC: 783
...
### 1. `ThirdParty/codemirror-5.52.0/src/util/misc.js` (JAVASCRIPT) -> Cumulative Risk: **777.3**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 254.2 | **LOC:** 169 | **CtrlFlow:** 39.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Api Exposure (100.0%), State Flux (100.0%), Spec Match (100.0%)
- **Heaviest Functions:** `countColumn` (Impact: 20.3), `findFirst` (Impact: 18.6), `findColumn` (Impact: 12.7)

### 2. `ThirdParty/dojo-release-1.10.4/dojo/store/JsonRest.js` (JAVASCRIPT) -> Cumulative Risk: **763.59**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 167.02 | **LOC:** 226 | **CtrlFlow:** 40.5% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Cognitive Load (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (98.6304%)
- **Heaviest Functions:** `query` (Impact: 60.0), `put` (Impact: 13.4), `_getTarget` (Impact: 6.4)

### 3. `ThirdParty/codemirror-5.52.0/src/display/scroll_events.js` (JAVASCRIPT) -> Cumulative Risk: **760.86**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 151.88 | **LOC:** 116 | **CtrlFlow:** 64.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.8692%)
- **Heaviest Functions:** `onScrollWheel` (Impact: 62.6), `wheelEventDelta` (Impact: 13.1), `wheelEventPixels` (Impact: 1.7)

### 4. `ThirdParty/dojo-release-1.10.4/dojo/_base/configRhino.js` (JAVASCRIPT) -> Cumulative Risk: **757.73**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 107.46 | **LOC:** 122 | **CtrlFlow:** 13.3% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Tech Debt (99.9914%), State Flux (99.988%)
- **Heaviest Functions:** `rhinoDojoConfig` (Impact: 34.0), `injectUrl` (Impact: 5.8), `log` (Impact: 5.5)

### 5. `ThirdParty/codemirror-5.52.0/src/input/ContentEditableInput.js` (JAVASCRIPT) -> Cumulative Risk: **748.36**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 671.24 | **LOC:** 528 | **CtrlFlow:** 39.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), Safety Score (99.1777%), Cognitive Load (98.4199%)
- **Heaviest Functions:** `locateNodeInLineView` (Impact: 62.7), `domTextBetween` (Impact: 51.6), `pollContent` (Impact: 39.5)

### 6. `ThirdParty/dojo-release-1.10.4/dojo/_base/Deferred.js` (JAVASCRIPT) -> Cumulative Risk: **747.86**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 210.42 | **LOC:** 384 | **CtrlFlow:** 21.9% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Cognitive Load (98.4586%)
- **Heaviest Functions:** `Deferred` (Impact: 63.5), `notify` (Impact: 21.9), `then` (Impact: 12.2)

### 7. `ThirdParty/codemirror-5.52.0/addon/edit/matchbrackets.js` (JAVASCRIPT) -> Cumulative Risk: **739.34**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 201.86 | **LOC:** 151 | **CtrlFlow:** 64.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Safety Score (99.133%)
- **Heaviest Functions:** `scanForBracket` (Impact: 60.1), `findMatchingBracket` (Impact: 37.1), `matchBrackets` (Impact: 31.4)

### 8. `ThirdParty/codemirror-5.52.0/src/display/focus.js` (JAVASCRIPT) -> Cumulative Risk: **737.73**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 62.58 | **LOC:** 48 | **CtrlFlow:** 28.2% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Spec Match (100.0%), Documentation (100.0%), State Flux (99.9994%), Concurrency (99.9993%)
- **Heaviest Functions:** `onFocus` (Impact: 13.1), `onBlur` (Impact: 7.5), `delayBlurEvent` (Impact: 3.2)

### 9. `ThirdParty/dojo-release-1.10.4/dojo/request/util.js` (JAVASCRIPT) -> Cumulative Risk: **734.82**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 247.3 | **LOC:** 159 | **CtrlFlow:** 33.1% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** Concurrency (100.0%), State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%)
- **Heaviest Functions:** `parseArgs` (Impact: 31.6), `deferred` (Impact: 26.8), `deepCopy` (Impact: 14.6)

### 10. `ThirdParty/codemirror-5.52.0/addon/hint/show-hint.js` (JAVASCRIPT) -> Cumulative Risk: **730.21**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 579.16 | **LOC:** 467 | **CtrlFlow:** 37.4% | **Authorship Centralization:** 0.0%
- **Primary Risk Drivers:** State Flux (100.0%), Spec Match (100.0%), Documentation (100.0%), Concurrency (99.684%)
- **Heaviest Functions:** `Widget` (Impact: 70.2), `buildKeyMap` (Impact: 24.6), `resolveAutoHints` (Impact: 20.3)
...
- **Risk Profile:** Cognitive Load (98.5175%), Tech Debt (10.1889%)
**Top Internal Functions/Classes:**
  * `dispatchKey` (Impact: 1116.0)
...
- **Risk Profile:** Cognitive Load (93.2339%), Tech Debt (31.3573%)
**Top Internal Functions/Classes:**
  * `cesiumGoogleEarthDbRootParser` (Impact: 556.3)
  * `verify` (Impact: 334.3)
  * `toObject` (Impact: 271.7)
  * `fromObject` (Impact: 215.2)
  * `decode` (Impact: 169.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (36.5513%), Tech Debt (9.5568%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (84.3254%), Tech Debt (9.8962%)
**Top Internal Functions/Classes:**
  * `Vim` (Impact: 469.1)
  * `sort` (Impact: 120.5)
  * `evalInput` (Impact: 117.0)
  * `paste` (Impact: 101.5)
  * `findKey` (Impact: 96.5)
...
- **Risk Profile:** Cognitive Load (82.1354%), Tech Debt (0.0%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (68.7754%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `processNetworkLink` (Impact: 106.5)
  * `processDescription` (Impact: 104.7)
  * `processScreenOverlay` (Impact: 101.4)
  * `applyStyle` (Impact: 93.4)
  * `getNetworkLinkUpdateCallback` (Impact: 76.2)
**Contextual Mitigations & Amplifications:**
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 2766.82 | **LOC:** 5293 | **CtrlFlow:** 18.4% | **Authorship Centralization:** 36.4%
- **Risk Profile:** Cognitive Load (46.5504%), Tech Debt (10.4124%)
**Top Internal Functions/Classes:**
  * `executeCommands` (Impact: 112.9)
    * *Intent:* /** * Execute the draw commands for all the render passes. * * @param {Scene} scene * @param {PassSt...
  * `updateAndClearFramebuffers` (Impact: 95.8)
  * `executeCommand` (Impact: 71.1)
    * *Intent:* /** * Execute a single draw command, or one of its derived commands if appropriate for the current r...
  * `Scene` (Impact: 57.0)
    * *Intent:* * @see CesiumWidget * @see {@link http://www.khronos.org/registry/webgl/specs/latest/#5.2|WebGLConte...
  * `updateEnvironment` (Impact: 44.5)
    * *Intent:* /** * @private */
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (37.182%), Tech Debt (8.3391%)
**Top Internal Functions/Classes:**
  * `unwrapInterval` (Impact: 143.6)
  * `processProperty` (Impact: 136.1)
  * `processPositionProperty` (Impact: 133.7)
  * `processMaterialProperty` (Impact: 116.1)
  * `getPropertyType` (Impact: 96.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 2 instances
...
- **Risk Profile:** Cognitive Load (0.0%), Tech Debt (8.6292%)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (60.9037%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `computeTriangleAttributes` (Impact: 89.8)
  * `splitLongitudePolyline` (Impact: 53.2)
  * `compressVertices` (Impact: 49.9)
    * *Intent:* /** * Compresses and packs geometry normal attribute values to save memory. * * @param {Geometry} ge...
  * `splitTriangle` (Impact: 49.1)
  * `splitLongitudeTriangles` (Impact: 43.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (78.1311%), Tech Debt (75.4354%)
**Top Internal Functions/Classes:**
  * `escapeXml` (Impact: 149.7)
  * `_adaptIEFormatAreaAndExec` (Impact: 112.3)
  * `_queryCommandAvailable` (Impact: 89.8)
  * `open` (Impact: 66.4)
  * `onKeyDown` (Impact: 45.1)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (48.6475%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `updateMembers` (Impact: 60.3)
  * `rectangleCameraPosition3D` (Impact: 41.0)
  * `setView` (Impact: 39.4)
    * *Intent:* * // 4. View rectangle with a top-down view * viewer.camera.setView({ * destination : Cesium.Rectang...
  * `_updateCameraChanged` (Impact: 33.0)
  * `flyTo` (Impact: 32.2)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 2039.92 | **LOC:** 3182 | **CtrlFlow:** 16.0% | **Authorship Centralization:** 38.9%
- **Risk Profile:** Cognitive Load (59.6787%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `loadPrimitive` (Impact: 73.0)
    * *Intent:* /** * Load resources associated with a mesh primitive for a glTF node * @param {GltfLoader} loader *...
  * `loadMaterial` (Impact: 42.0)
    * *Intent:* /** * Load textures and parse factors and flags for a glTF material * * @param {GltfLoader} loader *...
  * `loadVertexAttribute` (Impact: 41.2)
  * `loadAttribute` (Impact: 39.2)
  * `loadIndices` (Impact: 38.4)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000115 | `Ripple Effect (Closeness):` 0.0
...
- **Risk Profile:** Cognitive Load (92.3323%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `getShaderExpression` (Impact: 241.1)
  * `setEvaluateFunction` (Impact: 136.6)
  * `parseCall` (Impact: 109.0)
  * `getEvaluateTernaryComponentwise` (Impact: 54.0)
  * `createRuntimeAst` (Impact: 51.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (79.7429%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `addDrawCommandsForTile` (Impact: 251.2)
  * `updateTileBoundingRegion` (Impact: 69.6)
  * `computeTileVisibility` (Impact: 67.3)
    * *Intent:* /** * Determines the visibility of a given tile. The tile may be fully visible, partially visible, o...
  * `canRenderWithoutLosingDetail` (Impact: 45.6)
    * *Intent:* /** * Determines if the given not-fully-loaded tile can be rendered without losing detail that * was...
  * `_onLayerAdded` (Impact: 36.0)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (84.7857%), Tech Debt (11.2905%)
**Top Internal Functions/Classes:**
  * `getModuleInfo_` (Impact: 95.6)
  * `def` (Impact: 78.0)
  * `injectModule` (Impact: 62.7)
  * `config` (Impact: 60.9)
  * `getModule` (Impact: 38.2)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (51.2225%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `handleZoom` (Impact: 185.3)
  * `pan3D` (Impact: 73.1)
  * `look3D` (Impact: 68.8)
  * `rotateCVOnTerrain` (Impact: 58.8)
  * `spin3D` (Impact: 55.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (40.4254%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `findTileByUri` (Impact: 7.6)
  * `testColorBlendMode` (Impact: 5.9)
  * `numberOfChildrenWithoutContent` (Impact: 4.8)
  * `checkPointAndFeatureCounts` (Impact: 4.1)
  * `testBackFaceCulling` (Impact: 2.9)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (63.8956%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `update` (Impact: 78.2)
    * *Intent:* /** * Called when {@link Viewer} or {@link CesiumWidget} render the scene to * get the draw commands...
  * `updateAndQueueCommands` (Impact: 57.4)
  * `loadAsynchronous` (Impact: 51.3)
  * `Primitive` (Impact: 51.1)
    * *Intent:* * id : 'ellipsoid', * attributes : { * color : Cesium.ColorGeometryInstanceAttribute.fromColor(Cesiu...
  * `modifyForEncodedNormals` (Impact: 47.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Race Conditions:* 5 instances
...
- **Risk Profile:** Cognitive Load (52.1462%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `Model` (Impact: 130.7)
    * *Intent:* * @privateParam {boolean} [options.showCreditsOnScreen=false] Whether to display the credits of this...
  * `update` (Impact: 39.8)
    * *Intent:* /** * Called when {@link Viewer} or {@link CesiumWidget} render the scene to * get the draw commands...
  * `fromGltfAsync` (Impact: 28.2)
    * *Intent:* * } * }); * viewer.scene.primitives.add(model); * model.readyEvent.addEventListener(() => { * model....
  * `updateClamping` (Impact: 21.5)
  * `destroy` (Impact: 19.6)
...
- **Risk Profile:** Cognitive Load (46.708%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `Matrix4` (Impact: 137.8)
    * *Intent:* * @see Matrix4.fromScale * @see Matrix4.fromUniformScale * @see Matrix4.fromRotation * @see Matrix4....
  * `equalsEpsilon` (Impact: 43.2)
    * *Intent:* * * // b = [10.0, 14.0, 18.0, 22.0] * // [11.0, 15.0, 19.0, 23.0] * // [12.0, 16.0, 20.0, 24.0] * //...
  * `computeViewportTransformation` (Impact: 38.2)
    * *Intent:* * @param {number} [nearDepthRange=0.0] The near plane distance in window coordinates. * @param {numb...
  * `equals` (Impact: 34.4)
    * *Intent:* * * // b = [10.0, 14.0, 18.0, 22.0] * // [11.0, 15.0, 19.0, 23.0] * // [12.0, 16.0, 20.0, 24.0] * //...
  * `equalsArray` (Impact: 33.0)
    * *Intent:* /** * @private */
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (56.8214%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `propagateEdge` (Impact: 128.6)
  * `visitRenderedTiles` (Impact: 116.2)
  * `addEdgeMesh` (Impact: 94.4)
  * `getCornerFromEdge` (Impact: 87.8)
  * `createFillMesh` (Impact: 72.4)
**Contextual Mitigations & Amplifications:**
...
- **Risk Profile:** Cognitive Load (87.5688%), Tech Debt (9.5866%)
**Top Internal Functions/Classes:**
  * `markChanges` (Impact: 53.0)
  * `MergeView` (Impact: 37.6)
    * *Intent:* // Merge view, containing 0, 1, or 2 diff views.
  * `syncScroll` (Impact: 36.9)
  * `mergeAlignable` (Impact: 35.2)
...
- **Global Archetype:** `Unclassified` (Drift: 0.0 IQR)
- **Local Micro-Species:** `Unclassified` (Drift: 0.0 IQR)
- **Magnitude:** 1415.7 | **LOC:** 5109 | **CtrlFlow:** 2.4% | **Authorship Centralization:** 33.3%
- **Risk Profile:** Cognitive Load (79.2545%), Tech Debt (0.0%)
**Top Internal Functions/Classes:**
  * `gltfCallback` (Impact: 137.2)
  * `verifyRender` (Impact: 29.6)
  * `verifyDebugWireframe` (Impact: 26.4)
  * `setFeaturesWithOpacity` (Impact: 6.9)
  * `zoomTo` (Impact: 5.6)
**Contextual Mitigations & Amplifications:**
...
- `packages/engine/Source/Scene/Model/EdgeVisibilityPipelineStage.js` -> Churn: **87.7%** | Cog Load: 67.1154% | Debt: 0.0%
- `packages/engine/Source/Scene/CubeMapPanorama.js` -> Churn: **84.31%** | Cog Load: 52.0792% | Debt: 0.0%
- `packages/engine/Source/Scene/GltfLoader.js` -> Churn: **81.54%** | Cog Load: 59.6787% | Debt: 0.0%
...
- `packages/engine/Source/DataSources/ModelVisualizer.js` -> **Severity: 0.046** (Bridge: 0.0005 * Flux: 99.9961%)
- `packages/engine/Source/Scene/Model/Model.js` -> **Severity: 0.044** (Bridge: 0.0004 * Flux: 100.0%)
- `packages/engine/Source/Scene/Model/ModelSceneGraph.js` -> **Severity: 0.016** (Bridge: 0.0002 * Flux: 99.9965%)
- `packages/engine/Source/DataSources/GeometryVisualizer.js` -> **Severity: 0.013** (Bridge: 0.0001 * Flux: 100.0%)
- `packages/engine/Source/Scene/GltfLoader.js` -> **Severity: 0.011** (Bridge: 0.0001 * Flux: 99.9995%)
