...
| Debt Markers (formerly Tech Debt Exposure) | 0.0 | 100.0 | 36.3 | 15.8 | 0.0 |
| Test Surface (formerly Testing Exposure) | 0.0 | 80.0 | 7.0 | 2.3 | 0.0 |
| Connectivity (formerly API Exposure) | 0.0 | 94.4 | 8.3 | 0.0 | 0.0 |
...
| connectivity | 2014 | 306 | 8 | `mockwebserver/src/main/kotlin/mockwebserver3/MockResponse.kt` |
...
| dead_code | 3503 | 369 | 12 | `okhttp/src/jvmTest/kotlin/okhttp3/URLConnectionTest.kt` |
...
- `constructor` (@ `okhttp-testing-support/src/main/kotlin/okhttp3/testing/PlatformRule.kt`) -> Impact: **102.4** | LOC: 455
- `writeCanonicalized` (@ `okhttp/src/commonJvmAndroid/kotlin/okhttp3/internal/url/-Url.kt`) -> Impact: **101.2** | LOC: 63
- `constructor` (@ `okhttp-logging-interceptor/src/main/kotlin/okhttp3/logging/HttpLoggingInterceptor.kt`) -> Impact: **97.0** | LOC: 327
- `constructor` (@ `okhttp/src/commonJvmAndroid/kotlin/okhttp3/internal/http2/Hpack.kt`) -> Impact: **84.5** | LOC: 289
- `constructor` (@ `okhttp/src/commonJvmAndroid/kotlin/okhttp3/internal/http2/Hpack.kt`) -> Impact: **77.7** | LOC: 233
- `intercept` (@ `okhttp-logging-interceptor/src/main/kotlin/okhttp3/logging/HttpLoggingInterceptor.kt`) -> Impact: **73.4** | LOC: 167
- `parse` (@ `okhttp/src/commonJvmAndroid/kotlin/okhttp3/HttpUrl.kt`) -> Impact: **62.3** | LOC: 172
- `parse` (@ `okhttp/src/commonJvmAndroid/kotlin/okhttp3/Cookie.kt`) -> Impact: **57.0** | LOC: 141
- `messageDone` (@ `okhttp/src/commonJvmAndroid/kotlin/okhttp3/internal/connection/RealCall.kt`) -> Impact: **55.0** | LOC: 42
...
- `main` (@ `samples/slack/src/main/java/okhttp3/slack/SlackClient.java`) -> Impact: **53.6** | LOC: 25
...
- **Heaviest Functions:** `commonParse` (Impact: 31.1), `commonToString` (Impact: 17.2), `indexOfElement` (Impact: 5.7)
...
- **Heaviest Functions:** `fetch` (Impact: 20.4), `main` (Impact: 5.3), `drainQueue` (Impact: 4.9)
...
- **Heaviest Functions:** `plan` (Impact: 7.0), `connectTcp` (Impact: 5.7), `connectTlsEtc` (Impact: 4.5)
...
- **Heaviest Functions:** `dispatch` (Impact: 10.2), `newAuthorizeUrl` (Impact: 4.5), `start` (Impact: 2.4)
...
- **Heaviest Functions:** `main` (Impact: 53.6), `requestOauthSession` (Impact: 7.6), `awaitAccessToken` (Impact: 3.1)
...
- **Heaviest Functions:** `headersCheckValue` (Impact: 9.3), `commonValues` (Impact: 6.2), `commonHeadersGet` (Impact: 5.7)
...
- **Heaviest Functions:** `publish` (Impact: 12.1), `connectionAcquired` (Impact: 8.2), `launchWireShark` (Impact: 7.8)
...
- **Heaviest Functions:** `onClosing` (Impact: 4.2), `onOpen` (Impact: 3.6), `onMessage` (Impact: 3.6)
...
- **Heaviest Functions:** `__global_context__` (Impact: 17.1), `set_java_home` (Impact: 12.8), `Anonymous_Block` (Impact: 11.2)
...
- **Heaviest Functions:** `run` (Impact: 4.5), `connectFailed` (Impact: 2.6), `connectEnd` (Impact: 2.4)
...
**Top Internal Functions/Classes:**
  * `doUpload` (Impact: 21.3)
  * `testRedirect` (Impact: 17.6)
  * `writeTo` (Impact: 10.6)
  * `intercept` (Impact: 9.1)
  * `authCallsForHeader` (Impact: 8.8)
...
**Top Internal Functions/Classes:**
  * `requestBody` (Impact: 9.1)
  * `serverHalfClosingBeforeResponse` (Impact: 5.5)
  * `configureClientAndServerProxies` (Impact: 5.4)
    * *Intent:* /** Use a proxy to fake IPv6 connectivity, even if localhost doesn't have IPv6. */
  * `noRecoveryFromTlsHandshakeFailureWhenTlsFallbackIsDisabled` (Impact: 5.0)
  * `canRetryNormalRequestBody` (Impact: 4.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000197 | `Ripple Effect (Closeness):` 0.114698
...
**Top Internal Functions/Classes:**
  * `readRequest` (Impact: 39.5)
    * *Intent:* /** @param exchangeIndex the index of this request on this connection.*/
  * `readRequest` (Impact: 32.3)
  * `handle` (Impact: 27.1)
  * `writeResponse` (Impact: 26.9)
  * `processOneRequest` (Impact: 22.8)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.006274 | `Ripple Effect (Closeness):` 0.079341
...
**Top Internal Functions/Classes:**
  * `eventListener` (Impact: 6.2)
  * `withWriteTimeout` (Impact: 3.5)
  * `call` (Impact: 2.9)
  * `webSocketListener` (Impact: 2.9)
  * `connectFailed` (Impact: 2.8)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `assertCached` (Impact: 13.2)
  * `testRequestMethod` (Impact: 12.7)
  * `requestBodyOrNull` (Impact: 7.5)
  * `temporaryRedirectCachedWithCachingHeader` (Impact: 3.8)
  * `assertClientSuppliedCondition` (Impact: 3.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `completeEdit` (Impact: 38.1)
  * `readJournalLine` (Impact: 23.6)
  * `edit` (Impact: 21.9)
    * *Intent:* /** Returns an editor for the entry named [key], or null if another edit is in progress. */
  * `removeEntry` (Impact: 12.1)
  * `readJournal` (Impact: 11.1)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 5.7e-05 | `Ripple Effect (Closeness):` 0.067457
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.017779 | `Ripple Effect (Closeness):` 0.178684
...
**Top Internal Functions/Classes:**
  * `constructor` (Impact: 84.5)
  * `constructor` (Impact: 77.7)
  * `writeHeaders` (Impact: 29.4)
    * *Intent:* /** * This does not use "never indexed" semantics for sensitive headers. * * http://tools.ietf.org/h...
  * `insertIntoDynamicTable` (Impact: 12.3)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000192 | `Ripple Effect (Closeness):` 0.050271
...
**Top Internal Functions/Classes:**
  * `createJournalWithHeader` (Impact: 6.5)
  * `fun` (Impact: 6.2)
  * `trimToSizeWithActiveEdit` (Impact: 6.0)
...
**Top Internal Functions/Classes:**
  * `assertBytesReadWritten` (Impact: 24.6)
  * `assertSuccessfulEventOrder` (Impact: 8.1)
  * `aggregateEventListenerIsComplete` (Impact: 6.5)
    * *Intent:* /** Reflectively call every event function to confirm it is correctly forwarded. */
  * `requestBodyMultipleFailuresReportedOnlyOnce` (Impact: 5.8)
  * `timeToFirstByte` (Impact: 5.8)
...
**Top Internal Functions/Classes:**
  * `responseHeadersAfterGoaway` (Impact: 6.8)
  * `waitForConnectionShutdown` (Impact: 6.3)
  * `countFrames` (Impact: 5.8)
  * `concurrentHttp2ConnectionsDeduplicated` (Impact: 5.8)
    * *Intent:* /** * We don't know if the connection will support HTTP/2 until after we've connected. When multiple...
  * `firstFrame` (Impact: 5.7)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 2e-05 | `Ripple Effect (Closeness):` 0.081391
...
* *Network Topology:*
  * `Ecosystem Role:` Transceiver (Middle-Tier) | `Dependency Blast Radius (PageRank):` 1.514
  * `Choke Point (Betweenness):` 0.000115 | `Ripple Effect (Closeness):` 0.050271
...
**Top Internal Functions/Classes:**
  * `constructor` (Impact: 102.4)
  * `init` (Impact: 25.6)
  * `getPlatformSystemProperty` (Impact: 8.8)
  * `setupPlatform` (Impact: 5.8)
  * `interceptTestMethod` (Impact: 5.0)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.001416 | `Ripple Effect (Closeness):` 0.08321
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.016137 | `Ripple Effect (Closeness):` 0.138345
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000375 | `Ripple Effect (Closeness):` 0.07578
...
**Top Internal Functions/Classes:**
  * `writeOneFrame` (Impact: 19.3)
    * *Intent:* /** * Attempts to remove a single frame from a queue and send it. This prefers to write urgent pongs...
  * `failWebSocket` (Impact: 11.8)
  * `checkUpgradeSuccess` (Impact: 10.1)
  * `close` (Impact: 9.2)
  * `finishReader` (Impact: 8.7)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.005309 | `Ripple Effect (Closeness):` 0.135968
...
**Top Internal Functions/Classes:**
  * `assertInvalid` (Impact: 7.8)
  * `usernameCharacters` (Impact: 3.3)
  * `passwordCharacters` (Impact: 3.3)
  * `pathCharacters` (Impact: 2.9)
  * `hostnameInPunycodeNfcAndNfd` (Impact: 2.6)
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.002573 | `Ripple Effect (Closeness):` 0.082639
...
**Top Internal Functions/Classes:**
  * `constructor` (Impact: 97.0)
  * `intercept` (Impact: 73.4)
  * `redactUrl` (Impact: 9.3)
  * `logHeader` (Impact: 5.5)
  * `bodyHasUnknownEncoding` (Impact: 4.5)
**Contextual Mitigations & Amplifications:**
...
* *Network Topology:*
  ...
  * `Choke Point (Betweenness):` 0.000476 | `Ripple Effect (Closeness):` 0.010566
...
**Top Internal Functions/Classes:**
  * `readSettings` (Impact: 32.2)
  * `nextFrame` (Impact: 16.5)
  * `readHeaders` (Impact: 12.3)
  * `readData` (Impact: 12.3)
  * `readGoAway` (Impact: 12.3)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 12 instances
...
**Top Internal Functions/Classes:**
  * `readSendsWindowUpdate` (Impact: 6.5)
  * `readSendsWindowUpdateHttp2` (Impact: 6.4)
  * `sendGoAway` (Impact: 3.5)
  * `blockedStreamDoesntStarveNewStream` (Impact: 3.5)
  * `discardedDataFramesAreCounted` (Impact: 3.3)
...
- `okhttp/src/commonJvmAndroid/kotlin/okhttp3/OkHttpClient.kt` -> **Severity: 1.778** (Bridge: 0.0178 * Flux: 99.986%)
- `okhttp/src/commonJvmAndroid/kotlin/okhttp3/internal/connection/RealCall.kt` -> **Severity: 1.609** (Bridge: 0.0161 * Flux: 99.6816%)
- `mockwebserver/src/main/kotlin/mockwebserver3/MockWebServer.kt` -> **Severity: 0.625** (Bridge: 0.0063 * Flux: 99.6863%)
- `okhttp/src/commonJvmAndroid/kotlin/okhttp3/internal/ws/RealWebSocket.kt` -> **Severity: 0.53** (Bridge: 0.0053 * Flux: 99.8856%)
- `okhttp/src/commonJvmAndroid/kotlin/okhttp3/Response.kt` -> **Severity: 0.371** (Bridge: 0.0037 * Flux: 99.1589%)
