...
| Connectivity (formerly API Exposure) | 0.0 | 100.0 | 11.6 | 0.0 | 0.0 |
...
| Historical Stability (predictive layer, promotion pending #2987) (formerly Instability Exposure) | 0.0 | 10.9 | 1.5 | 0.0 | 0.0 |
...
| connectivity | 63174 | 3404 | 7 | `src/deps/boringssl.translated.zig` |
...
| dead_code | 12784 | 2180 | 2 | `src/bun.js/bindings/ncrypto.cpp` |
| credential | 2879 | 58 | 0 | `packages/bun-usockets/src/crypto/root_certs.h` |
| threat | 11487 | 1482 | 1 | `src/bun.js/bindings/node/http/llhttp/llhttp.c` |
...
- `expectBundled` (@ `test/bundler/expectBundled.ts`) -> Impact: **1189.6** | LOC: 1387
- `llhttp__internal__run` (@ `src/bun.js/bindings/node/http/llhttp/llhttp.c`) -> Impact: **786.1** | LOC: 1443
- `parse` (@ `src/cli/Arguments.zig`) -> Impact: **749.5** | LOC: 1230
- `printAs` (@ `src/bun.js/ConsoleObject.zig`) -> Impact: **710.1** | LOC: 1078
- `parse` (@ `src/css/properties/properties_generated.zig`) -> Impact: **665.5** | LOC: 1070
  * *Intent:* /// Parses a CSS property by name.
- `NewPrinter` (@ `src/js_printer.zig`) -> Impact: **660.5** | LOC: 1410
- `call_uv_func` (@ `test/napi/uv-stub-stuff/plugin.c`) -> Impact: **639.6** | LOC: 2468
...
- `setsockopt_6_or_4` (@ `packages/bun-usockets/src/bsd.c`) -> Impact: **639.1** | LOC: 1221
- `parse` (@ `src/install/npm.zig`) -> Impact: **633.9** | LOC: 915
...
- `migrateYarnLockfile` (@ `src/install/yarn.zig`) -> Impact: **631.1** | LOC: 1140
...
- **Heaviest Functions:** `doSendFileFD` (Impact: 90.8), `constructor` (Impact: 83.5), `validateSettings` (Impact: 74.9)
...
- **Heaviest Functions:** `readStreamIntoSink` (Impact: 63.4), `readableStreamFromAsyncIterator` (Impact: 59.8), `assignStreamIntoResumableSink` (Impact: 56.0)
...
- **Heaviest Functions:** `overridableRequire` (Impact: 60.3), `loadEsmIntoCjs` (Impact: 57.3), `requireResolve` (Impact: 24.6)
...
- **Heaviest Functions:** `goto` (Impact: 47.9), `readCssMetadataFallback` (Impact: 32.2), `readChunk` (Impact: 26.8)
...
- **Heaviest Functions:** `SerializedScriptValue::create` (Impact: 379.1), `dumpIfTerminal` (Impact: 178.4), `SerializedScriptValue::deserialize` (Impact: 158.8)

### 6. `src/js/internal/sql/mysql.ts` (TYPESCRIPT) -> Cumulative Risk: **753.86**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
- **Magnitude:** 1035.14 | **LOC:** 1167 | **CtrlFlow:** 26.7% | **Authorship Centralization:** 60.0%
- **Primary Risk Drivers:** Spec Match (100.0%), State Flux (99.9999%), Documentation (97.1429%), Concurrency (96.9366%)
- **Heaviest Functions:** `normalizeQuery` (Impact: 139.6), `connect` (Impact: 60.8), `detectCommand` (Impact: 44.6)

### 7. `src/js/node/fs.promises.ts` (TYPESCRIPT) -> Cumulative Risk: **752.27**
- **Archetype:** `Unclassified` (Distance: N/A IQR)
...
- **Heaviest Functions:** `writeFileAsyncIterator` (Impact: 96.2), `watch` (Impact: 36.5), `read` (Impact: 35.6)
...
- **Heaviest Functions:** `from` (Impact: 89.3), `close` (Impact: 9.2), `nextSyncWithSyncValues` (Impact: 8.5)
...
- **Heaviest Functions:** `[kRealListen]` (Impact: 163.0), `listen` (Impact: 98.0), `connect` (Impact: 91.6)
...
- **Heaviest Functions:** `parseHTTPHeader` (Impact: 99.6), `parseXSSProtectionHeader` (Impact: 44.6), `extractCharsetFromMediaType` (Impact: 36.2)
...
**Top Internal Functions/Classes:**
  * `llhttp__internal__run` (Impact: 786.1)
  * `llparse__match_sequence_to_lower` (Impact: 18.7)
  * `llhttp__internal__c_mul_add_content_length` (Impact: 14.7)
  * `llhttp__internal__c_mul_add_content_length_1` (Impact: 14.7)
  * `llhttp__internal__c_mul_add_status_code` (Impact: 14.7)
**Contextual Mitigations & Amplifications:**
...
- `src/bun.js/bindings/bindings.cpp` -> Churn: **96.97%** | Cog Load: 40.5566% | Debt: 99.9996%
- `src/bun.js/bindings/ZigGlobalObject.cpp` -> Churn: **92.91%** | Cog Load: 16.6182% | Debt: 98.8678%
...
- `src/install/PackageManager/CommandLineArguments.zig` -> **Severity: 0.024** (Bridge: 0.0002 * Flux: 99.7277%)
- `src/cli/package_manager_command.zig` -> **Severity: 0.014** (Bridge: 0.0002 * Flux: 63.7789%)
- `src/bundler/bundle_v2.zig` -> **Severity: 0.013** (Bridge: 0.0001 * Flux: 99.3384%)
- `src/cli.zig` -> **Severity: 0.013** (Bridge: 0.0002 * Flux: 71.3397%)
- `src/cli/build_command.zig` -> **Severity: 0.013** (Bridge: 0.0001 * Flux: 99.9933%)
