...
- `__init__` (@ `msal-1.35.1/msal/application.py`) -> Impact: **124.3** | LOC: 422
- `_acquire_token_silent_from_cache_and_possibly_refresh_it` (@ `msal-1.35.1/msal/application.py`) -> Impact: **111.7** | LOC: 112
- `_acquire_token_interactive_via_broker` (@ `msal-1.35.1/msal/application.py`) -> Impact: **98.2** | LOC: 89
- `acquire_token_interactive` (@ `msal-1.35.1/msal/application.py`) -> Impact: **83.0** | LOC: 189
- `_build_client` (@ `msal-1.35.1/msal/application.py`) -> Impact: **73.2** | LOC: 122
- `_signin_interactively` (@ `msal-1.35.1/msal/broker.py`) -> Impact: **61.6** | LOC: 54
- `_get_auth_response` (@ `msal-1.35.1/msal/oauth2cli/authcode.py`) -> Impact: **56.4** | LOC: 67
- `__init__` (@ `msal-1.35.1/msal/oauth2cli/oauth2.py`) -> Impact: **52.5** | LOC: 112
- `search` (@ `msal-1.35.1/msal/token_cache.py`) -> Impact: **48.8** | LOC: 46
...
- `_initialize_entra_authority` (@ `msal-1.35.1/msal/authority.py`) -> Impact: **47.0** | LOC: 46
...
- **Heaviest Functions:** `search` (Impact: 48.8), `__init__` (Impact: 29.6), `__add` (Impact: 25.5)
...
- **Heaviest Functions:** `_get_auth_response` (Impact: 56.4), `get_auth_response` (Impact: 19.4), `_browse` (Impact: 19.3)
...
- **Heaviest Functions:** `__init__` (Impact: 124.3), `_acquire_token_silent_from_cache_and_possibly_refresh_it` (Impact: 111.7), `_acquire_token_interactive_via_broker` (Impact: 98.2)
...
- **Heaviest Functions:** `__init__` (Impact: 25.8), `parse` (Impact: 10.8), `__init__` (Impact: 10.4)
...
- **Heaviest Functions:** `create_normal_assertion` (Impact: 32.8), `__init__` (Impact: 12.0), `__call__` (Impact: 4.7)
...
- **Heaviest Functions:** `__init__` (Impact: 52.5), `_obtain_token` (Impact: 45.8), `obtain_token_by_auth_code_flow` (Impact: 35.9)
...
- **Heaviest Functions:** `_initialize_entra_authority` (Impact: 47.0), `__init__` (Impact: 39.9), `has_valid_issuer` (Impact: 28.2)
...
- **Heaviest Functions:** `_signin_interactively` (Impact: 61.6), `_convert_result` (Impact: 29.6), `_acquire_token_silently` (Impact: 22.6)
...
- **Heaviest Functions:** `acquire_token_for_client` (Impact: 41.1), `_obtain_token` (Impact: 29.5), `__init__` (Impact: 25.3)
...
- **Heaviest Functions:** `__init__` (Impact: 16.5), `__call__` (Impact: 13.8), `wrapper` (Impact: 13.5)
...
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 124.3)
  * `_acquire_token_silent_from_cache_and_possibly_refresh_it` (Impact: 111.7)
  * `_acquire_token_interactive_via_broker` (Impact: 98.2)
  * `acquire_token_interactive` (Impact: 83.0)
  * `_build_client` (Impact: 73.2)
...
**Top Internal Functions/Classes:**
  * `_test_acquire_token_obo` (Impact: 35.3)
  * `assertCacheWorksForUser` (Impact: 23.4)
  * `_test_username_password` (Impact: 23.0)
  * `_get_hint` (Impact: 22.9)
...
**Top Internal Functions/Classes:**
  * `test_maintaining_offline_state_and_sending_them` (Impact: 15.6)
  * `test_acquire_token_for_client` (Impact: 7.1)
  * `mock_post` (Impact: 7.0)
  * `mock_post` (Impact: 7.0)
  * `mock_post` (Impact: 7.0)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `__init__` (Impact: 52.5)
  * `_obtain_token` (Impact: 45.8)
  * `obtain_token_by_auth_code_flow` (Impact: 35.9)
  * `_obtain_token` (Impact: 25.6)
  * `obtain_token_by_refresh_token` (Impact: 24.8)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `acquire_token_for_client` (Impact: 41.1)
  * `_obtain_token` (Impact: 29.5)
  * `__init__` (Impact: 25.3)
  * `_obtain_token_on_arc` (Impact: 22.5)
    * *Intent:* # https://learn.microsoft.com/en-us/azure/azure-arc/servers/managed-identity-authentication logger.d...
  * `_obtain_token_on_service_fabric` (Impact: 22.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 32 instances
...
**Top Internal Functions/Classes:**
  * `test_wellknown_host_and_tenant` (Impact: 5.2)
    * *Intent:* # This test makes real HTTP calls to authority endpoints. # It is intentionally network-based to val...
  * `test_new_sovereign_hosts_should_build_authority_endpoints` (Impact: 5.1)
  * `test_turning_off_instance_discovery_should_work_for_all_kinds_of_clouds` (Impact: 5.0)
  * `test_known_authority_should_use_same_host_and_skip_instance_discovery` (Impact: 4.8)
  * `test_memorize` (Impact: 3.8)
...
**Top Internal Functions/Classes:**
  * `_get_auth_response` (Impact: 56.4)
  * `get_auth_response` (Impact: 19.4)
  ...
  * `_process_auth_response` (Impact: 18.2)
...
**Top Internal Functions/Classes:**
  * `_initialize_entra_authority` (Impact: 47.0)
  * `__init__` (Impact: 39.9)
  * `has_valid_issuer` (Impact: 28.2)
    * *Intent:* """ Returns True if the issuer from OIDC discovery is valid for this authority. An issuer is valid i...
  * `canonicalize` (Impact: 20.9)
...
**Top Internal Functions/Classes:**
  * `_signin_interactively` (Impact: 61.6)
  * `_convert_result` (Impact: 29.6)
  * `_acquire_token_silently` (Impact: 22.6)
  * `_signin_silently` (Impact: 22.4)
...
**Top Internal Functions/Classes:**
  * `_test_happy_path` (Impact: 15.5)
  * `test_happy_path` (Impact: 4.4)
  * `test_arc_error_should_be_normalized` (Impact: 4.2)
  * `_test_happy_path` (Impact: 2.5)
  * `_build_app` (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_build_dummy_response` (Impact: 4.6)
  * `post` (Impact: 2.7)
  * `get` (Impact: 2.5)
  * `_test_RetryAfter_N_seconds_should_keep_entry_for_N_seconds` (Impact: 2.5)
  * `test_one_invalid_grant_should_block_a_similar_request` (Impact: 2.5)
**Structural Signatures (Net Mitigated Signals):**
...
**Top Internal Functions/Classes:**
  * `load_conf` (Impact: 9.7)
    * *Intent:* """ Example of a configuration file: { "Note": "the OpenID Discovery will be updated by following op...
  * `setUpClass` (Impact: 7.3)
  * `test_device_flow` (Impact: 5.1)
  * `test_auth_code` (Impact: 2.4)
  * `test_auth_code_flow_error_response` (Impact: 2.1)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `assertFoundAccessToken` (Impact: 12.7)
  * `build_response` (Impact: 11.5)
  * `build_id_token` (Impact: 8.4)
    * *Intent:* # NOTE: These helpers were once implemented as static methods in TokenCacheTestCase. # That would ca...
  * `tearDown` (Impact: 4.7)
  * `testAddByAad` (Impact: 3.9)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `create_normal_assertion` (Impact: 32.8)
  * `__init__` (Impact: 12.0)
  * `__call__` (Impact: 4.7)
  * `create_normal_assertion` (Impact: 3.7)
  * `create_regenerative_assertion` (Impact: 3.2)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `_obtain_token` (Impact: 28.7)
  * `_scope_to_resource` (Impact: 12.1)
  * `_is_running_in_cloud_shell` (Impact: 1.1)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 13 instances
...
**Top Internal Functions/Classes:**
  * `_verify_assertion_params` (Impact: 14.5)
  * `_setup_mocks` (Impact: 7.1)
    * *Intent:* -----END CERTIFICATE-----""" """Helper to setup Authority mock"""
  * `test_pem_with_certificate_only_uses_sha256` (Impact: 3.7)
  * `test_pem_with_adfs_uses_sha1` (Impact: 3.6)
  * `test_pem_with_both_uses_manual_thumbprint_as_sha1` (Impact: 3.3)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `get_wstrust_username_password_endpoint` (Impact: 7.5)
    * *Intent:* """Returns {"address": "https://...", "action": "the soapAction value"}"""
  * `_get_policy_ids` (Impact: 6.3)
  * `_get_bindings` (Impact: 6.3)
  * `send_request` (Impact: 2.5)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  ...
  * `update_telemetry` (Impact: 7.3)
  * `generate_headers` (Impact: 6.6)
  * `_record_failure` (Impact: 3.9)
  * `hit_an_access_token` (Impact: 1.6)
**Contextual Mitigations & Amplifications:**
...
**Top Internal Functions/Classes:**
  * `test_initiate_auth_code_flow_with_non_form_post_response_mode_should_warn` (Impact: 5.2)
  ...
  * `test_http_post_should_work_with_obtain_token_by_browser` (Impact: 2.5)
  * `__init__` (Impact: 2.4)
**Contextual Mitigations & Amplifications:**
* *Amplified Cascading Flux:* 7 instances
...
**Top Internal Functions/Classes:**
  * `foo` (Impact: 2.5)
  * `test_old_item_can_be_updated_with_new_expiry_time` (Impact: 2.0)
  * `test_setitem` (Impact: 1.8)
  * `test_set` (Impact: 1.8)
  * `test_get_should_not_purge_and_should_return_only_when_the_item_is_still_valid` (Impact: 1.8)
**Structural Signatures (Net Mitigated Signals):**
...
- `msal-1.35.1/msal/application.py` -> **Severity: 3.317** (Bridge: 0.0332 * Flux: 99.9999%)
- `msal-1.35.1/msal/token_cache.py` -> **Severity: 1.145** (Bridge: 0.0115 * Flux: 99.9999%)
